// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x22), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  or2    g013(.a(new_n57_), .b(x33), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n54_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n53_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n53_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n54_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n60_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  nor2   g025(.a(new_n50_), .b(new_n61_), .O(new_n70_));
  nand2  g026(.a(new_n65_), .b(new_n63_), .O(new_n71_));
  nand3  g027(.a(new_n56_), .b(x06), .c(x05), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n68_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n48_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  nor2   g035(.a(x20), .b(x15), .O(new_n80_));
  nor2   g036(.a(x21), .b(x16), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g038(.a(x23), .b(x18), .O(new_n83_));
  nor2   g039(.a(x24), .b(x19), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  inv1   g043(.a(x17), .O(new_n88_));
  inv1   g044(.a(x22), .O(new_n89_));
  nand3  g045(.a(new_n45_), .b(new_n89_), .c(new_n88_), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n87_), .c(new_n55_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n79_), .c(new_n78_), .d(x25), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n61_), .O(z3));
  inv1   g049(.a(new_n55_), .O(new_n94_));
  nand3  g050(.a(new_n45_), .b(x28), .c(new_n79_), .O(new_n95_));
  oai21  g051(.a(x28), .b(new_n79_), .c(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n89_), .b(new_n88_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n85_), .c(new_n82_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(new_n99_));
  nand4  g055(.a(new_n86_), .b(new_n94_), .c(x28), .d(new_n79_), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n89_), .c(new_n99_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n78_), .c(x25), .d(x01), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n48_), .O(z4));
  inv1   g059(.a(x28), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n79_), .c(x29), .O(new_n105_));
  inv1   g061(.a(x29), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x28), .c(x27), .O(new_n107_));
  oai21  g063(.a(x30), .b(new_n88_), .c(new_n89_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n86_), .O(new_n109_));
  aoi22  g065(.a(new_n109_), .b(new_n90_), .c(new_n107_), .d(new_n105_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n94_), .c(new_n78_), .d(x25), .O(new_n111_));
  nor2   g067(.a(new_n111_), .b(new_n61_), .O(z5));
  nor2   g068(.a(new_n104_), .b(new_n79_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x29), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x30), .c(x22), .O(new_n115_));
  nor2   g071(.a(x30), .b(new_n106_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  nor2   g075(.a(new_n79_), .b(x22), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n116_), .c(x28), .d(new_n88_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n94_), .c(new_n78_), .d(x25), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n61_), .O(z6));
  nand4  g080(.a(new_n94_), .b(new_n78_), .c(x25), .d(x01), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n48_), .O(new_n126_));
  nand3  g082(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x31), .O(new_n128_));
  inv1   g084(.a(new_n83_), .O(new_n129_));
  inv1   g085(.a(new_n84_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n82_), .O(new_n131_));
  inv1   g087(.a(x31), .O(new_n132_));
  nand4  g088(.a(new_n113_), .b(new_n132_), .c(x30), .d(x29), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n131_), .c(new_n128_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x22), .O(new_n135_));
  oai21  g091(.a(new_n131_), .b(new_n88_), .c(new_n132_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n45_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(new_n126_), .O(z7));
  nand2  g094(.a(new_n104_), .b(new_n79_), .O(new_n139_));
  inv1   g095(.a(x18), .O(new_n140_));
  nand3  g096(.a(x19), .b(new_n140_), .c(x13), .O(new_n141_));
  inv1   g097(.a(x19), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x18), .c(x14), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n141_), .c(new_n88_), .O(new_n144_));
  nand4  g100(.a(x19), .b(x18), .c(new_n88_), .d(x12), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  inv1   g103(.a(x16), .O(new_n148_));
  nand3  g104(.a(x19), .b(x18), .c(x17), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n148_), .c(x11), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x15), .O(new_n153_));
  inv1   g109(.a(x15), .O(new_n154_));
  nand4  g110(.a(new_n150_), .b(x16), .c(new_n154_), .d(x10), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n49_), .c(x31), .d(new_n45_), .O(new_n157_));
  nand2  g113(.a(x32), .b(x30), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n139_), .c(x29), .O(new_n160_));
  nand2  g116(.a(new_n139_), .b(x29), .O(new_n161_));
  nand4  g117(.a(new_n156_), .b(new_n49_), .c(x31), .d(x30), .O(new_n162_));
  nand2  g118(.a(x32), .b(new_n45_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g121(.a(new_n142_), .b(x14), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(x18), .c(x17), .d(x16), .O(new_n167_));
  nor2   g123(.a(new_n167_), .b(new_n154_), .O(new_n168_));
  nand2  g124(.a(x16), .b(x15), .O(new_n169_));
  nand2  g125(.a(x18), .b(x17), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n169_), .c(new_n142_), .O(new_n171_));
  nand2  g127(.a(x16), .b(x10), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n170_), .c(new_n154_), .O(new_n173_));
  inv1   g129(.a(x11), .O(new_n174_));
  oai21  g130(.a(new_n170_), .b(new_n174_), .c(new_n148_), .O(new_n175_));
  aoi21  g131(.a(x18), .b(x12), .c(x17), .O(new_n176_));
  nor2   g132(.a(x18), .b(x13), .O(new_n177_));
  nor3   g133(.a(new_n177_), .b(new_n176_), .c(new_n132_), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(new_n175_), .c(new_n173_), .d(new_n171_), .O(new_n179_));
  oai21  g135(.a(new_n179_), .b(new_n168_), .c(x32), .O(new_n180_));
  nand3  g136(.a(new_n180_), .b(new_n165_), .c(new_n160_), .O(new_n181_));
  nand3  g137(.a(new_n181_), .b(new_n78_), .c(x00), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n48_), .O(z8));
  nor2   g139(.a(new_n177_), .b(new_n176_), .O(new_n184_));
  nand4  g140(.a(new_n184_), .b(new_n175_), .c(new_n173_), .d(new_n171_), .O(new_n185_));
  oai21  g141(.a(new_n185_), .b(new_n168_), .c(new_n48_), .O(new_n186_));
  nand2  g142(.a(new_n161_), .b(new_n132_), .O(new_n187_));
  nor2   g143(.a(new_n132_), .b(new_n106_), .O(new_n188_));
  aoi21  g144(.a(new_n188_), .b(new_n139_), .c(new_n45_), .O(new_n189_));
  aoi22  g145(.a(new_n189_), .b(x22), .c(new_n187_), .d(new_n45_), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n186_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(x33), .O(new_n192_));
  nand4  g148(.a(new_n139_), .b(x31), .c(x30), .d(x29), .O(new_n193_));
  oai22  g149(.a(new_n193_), .b(new_n89_), .c(new_n187_), .d(x30), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n156_), .O(new_n195_));
  oai21  g151(.a(new_n195_), .b(x33), .c(new_n192_), .O(new_n196_));
  nand3  g152(.a(new_n196_), .b(new_n78_), .c(x00), .O(new_n197_));
  inv1   g153(.a(new_n197_), .O(z9));
endmodule


