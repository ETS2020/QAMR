// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  nand2  g000(.a(x30), .b(x23), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x32), .O(z0));
  nor2   g002(.a(x32), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nor2   g004(.a(x33), .b(new_n48_), .O(new_n49_));
  oai21  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(x32), .b(new_n48_), .O(new_n52_));
  nor2   g008(.a(x33), .b(x03), .O(new_n53_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  xnor2a g011(.a(x07), .b(x04), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  nor2   g013(.a(new_n48_), .b(new_n51_), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  nand2  g015(.a(new_n48_), .b(new_n51_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n48_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(new_n51_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n51_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x05), .c(new_n48_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n63_), .c(x01), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n61_), .c(new_n56_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  xnor2a g024(.a(x03), .b(x02), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n57_), .O(new_n70_));
  nand2  g026(.a(new_n64_), .b(new_n62_), .O(new_n71_));
  nand4  g027(.a(x06), .b(x05), .c(new_n48_), .d(new_n51_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n68_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n67_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n45_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  inv1   g035(.a(x18), .O(new_n80_));
  inv1   g036(.a(x23), .O(new_n81_));
  inv1   g037(.a(x15), .O(new_n82_));
  inv1   g038(.a(x20), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  aoi21  g040(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(new_n90_));
  aoi22  g046(.a(new_n90_), .b(new_n45_), .c(new_n81_), .d(new_n80_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n79_), .c(new_n78_), .d(x25), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n57_), .O(z3));
  inv1   g050(.a(x28), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(x27), .O(new_n96_));
  nor2   g052(.a(x28), .b(new_n79_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n96_), .c(new_n80_), .O(new_n98_));
  nand3  g054(.a(new_n90_), .b(x28), .c(new_n79_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n81_), .O(new_n101_));
  inv1   g057(.a(x30), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(x28), .c(new_n79_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n97_), .c(new_n90_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n101_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n59_), .c(new_n78_), .d(x25), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n57_), .c(new_n45_), .O(z4));
  nor2   g064(.a(new_n95_), .b(new_n79_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x29), .O(new_n111_));
  inv1   g067(.a(x29), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x28), .c(x27), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n111_), .c(new_n91_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n59_), .c(new_n78_), .d(x25), .O(new_n115_));
  nor2   g071(.a(new_n115_), .b(new_n57_), .O(z5));
  nand2  g072(.a(new_n90_), .b(x18), .O(new_n117_));
  nand3  g073(.a(x29), .b(new_n81_), .c(new_n80_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n109_), .O(new_n119_));
  aoi21  g075(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(new_n120_));
  nor3   g076(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(x29), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n119_), .c(x30), .O(new_n123_));
  oai21  g079(.a(new_n102_), .b(x23), .c(new_n90_), .O(new_n124_));
  nand3  g080(.a(new_n102_), .b(new_n81_), .c(new_n80_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(x29), .c(x28), .d(x27), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n123_), .c(new_n58_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n78_), .c(x25), .d(x01), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n45_), .O(z6));
  nand3  g086(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(new_n131_));
  nor2   g087(.a(new_n131_), .b(new_n80_), .O(new_n132_));
  nand4  g088(.a(new_n59_), .b(new_n78_), .c(x25), .d(x01), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n132_), .c(new_n45_), .O(new_n134_));
  nand3  g090(.a(x29), .b(x28), .c(x27), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x31), .O(new_n136_));
  inv1   g092(.a(x31), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x30), .c(x29), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n110_), .c(new_n136_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n81_), .O(new_n140_));
  oai21  g096(.a(new_n131_), .b(new_n81_), .c(new_n137_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n102_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n140_), .c(new_n134_), .O(z7));
  nand2  g099(.a(new_n95_), .b(new_n79_), .O(new_n144_));
  inv1   g100(.a(x32), .O(new_n145_));
  inv1   g101(.a(x17), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n80_), .c(x13), .O(new_n147_));
  inv1   g103(.a(x19), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n146_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n146_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  inv1   g109(.a(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  nand4  g115(.a(new_n156_), .b(x16), .c(new_n82_), .d(x10), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n145_), .c(x31), .d(new_n102_), .O(new_n162_));
  nand2  g118(.a(x32), .b(x30), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n144_), .c(x29), .O(new_n165_));
  nand2  g121(.a(new_n144_), .b(x29), .O(new_n166_));
  nand4  g122(.a(new_n161_), .b(new_n145_), .c(x31), .d(x30), .O(new_n167_));
  nand2  g123(.a(x32), .b(new_n102_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nand2  g126(.a(new_n148_), .b(x14), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(x18), .c(x17), .d(x16), .O(new_n172_));
  nor2   g128(.a(new_n172_), .b(new_n82_), .O(new_n173_));
  nand2  g129(.a(x16), .b(x15), .O(new_n174_));
  nand2  g130(.a(x18), .b(x17), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n174_), .c(new_n148_), .O(new_n176_));
  nand2  g132(.a(x16), .b(x10), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n175_), .c(new_n82_), .O(new_n178_));
  inv1   g134(.a(x11), .O(new_n179_));
  oai21  g135(.a(new_n175_), .b(new_n179_), .c(new_n154_), .O(new_n180_));
  aoi21  g136(.a(x18), .b(x12), .c(x17), .O(new_n181_));
  nor2   g137(.a(x18), .b(x13), .O(new_n182_));
  nor3   g138(.a(new_n182_), .b(new_n181_), .c(new_n137_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n180_), .c(new_n178_), .d(new_n176_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n173_), .c(x32), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n170_), .c(new_n165_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n78_), .c(x00), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n45_), .O(z8));
  nor2   g144(.a(new_n182_), .b(new_n181_), .O(new_n189_));
  nand4  g145(.a(new_n189_), .b(new_n180_), .c(new_n178_), .d(new_n176_), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n173_), .c(new_n45_), .O(new_n191_));
  nand2  g147(.a(new_n166_), .b(new_n137_), .O(new_n192_));
  nor2   g148(.a(new_n137_), .b(new_n112_), .O(new_n193_));
  aoi21  g149(.a(new_n193_), .b(new_n144_), .c(new_n102_), .O(new_n194_));
  aoi22  g150(.a(new_n194_), .b(new_n81_), .c(new_n192_), .d(new_n102_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(new_n191_), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(x33), .O(new_n197_));
  nand4  g153(.a(new_n144_), .b(x31), .c(x30), .d(x29), .O(new_n198_));
  oai22  g154(.a(new_n198_), .b(x23), .c(new_n192_), .d(x30), .O(new_n199_));
  nand2  g155(.a(new_n199_), .b(new_n161_), .O(new_n200_));
  oai21  g156(.a(new_n200_), .b(x33), .c(new_n197_), .O(new_n201_));
  nand3  g157(.a(new_n201_), .b(new_n78_), .c(x00), .O(new_n202_));
  inv1   g158(.a(new_n202_), .O(z9));
endmodule


