// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x19), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n49_), .b(new_n52_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n52_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n52_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n49_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n58_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xnor2a g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n59_), .O(new_n71_));
  nand2  g027(.a(new_n65_), .b(new_n63_), .O(new_n72_));
  nand4  g028(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n46_), .c(new_n57_), .d(x08), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(z2));
  inv1   g034(.a(x27), .O(new_n79_));
  inv1   g035(.a(new_n60_), .O(new_n80_));
  oai22  g036(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n81_));
  inv1   g037(.a(x17), .O(new_n82_));
  inv1   g038(.a(x22), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g040(.a(x18), .O(new_n85_));
  inv1   g041(.a(x23), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n84_), .c(x24), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n81_), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n80_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n79_), .c(new_n45_), .d(x25), .O(new_n91_));
  nor3   g047(.a(new_n91_), .b(x19), .c(new_n59_), .O(z3));
  xor2a  g048(.a(x28), .b(x27), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n90_), .c(new_n45_), .d(x25), .O(new_n94_));
  nor3   g050(.a(new_n94_), .b(x19), .c(new_n59_), .O(z4));
  inv1   g051(.a(x19), .O(new_n96_));
  inv1   g052(.a(new_n81_), .O(new_n97_));
  nand4  g053(.a(new_n87_), .b(new_n84_), .c(new_n97_), .d(x24), .O(new_n98_));
  nand2  g054(.a(x28), .b(x27), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x29), .O(new_n100_));
  inv1   g056(.a(x29), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(x28), .c(x27), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n98_), .c(new_n60_), .d(new_n45_), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(x25), .c(new_n96_), .d(x01), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z5));
  nand2  g063(.a(new_n99_), .b(x30), .O(new_n108_));
  inv1   g064(.a(x30), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x28), .c(x27), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x29), .O(new_n112_));
  nand2  g068(.a(x30), .b(new_n101_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n98_), .c(new_n60_), .d(new_n45_), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(x25), .c(new_n96_), .d(x01), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(z6));
  nand2  g074(.a(x30), .b(x29), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n99_), .c(x31), .O(new_n120_));
  nand4  g076(.a(new_n87_), .b(new_n84_), .c(new_n97_), .d(x24), .O(new_n121_));
  nand3  g077(.a(new_n60_), .b(new_n96_), .c(x01), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  inv1   g079(.a(x25), .O(new_n124_));
  inv1   g080(.a(x31), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x30), .c(x29), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n99_), .c(new_n45_), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n123_), .c(new_n121_), .d(new_n120_), .O(z7));
  inv1   g085(.a(x28), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n79_), .O(new_n131_));
  inv1   g087(.a(x32), .O(new_n132_));
  nand4  g088(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n133_));
  nand4  g089(.a(new_n132_), .b(x31), .c(new_n109_), .d(x18), .O(new_n134_));
  oai22  g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n109_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n131_), .c(x29), .O(new_n136_));
  nand2  g092(.a(new_n131_), .b(x29), .O(new_n137_));
  nand4  g093(.a(new_n132_), .b(x31), .c(x30), .d(x18), .O(new_n138_));
  oai22  g094(.a(new_n138_), .b(new_n133_), .c(new_n132_), .d(x30), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  nand3  g096(.a(x16), .b(x15), .c(x14), .O(new_n141_));
  nand3  g097(.a(x31), .b(x18), .c(x17), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x32), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n140_), .c(new_n136_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n45_), .c(new_n96_), .d(x00), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(z8));
  inv1   g102(.a(x33), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(x31), .c(x30), .d(x18), .O(new_n148_));
  oai22  g104(.a(new_n148_), .b(new_n133_), .c(new_n147_), .d(x30), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n131_), .c(x29), .O(new_n150_));
  nand4  g106(.a(new_n147_), .b(new_n125_), .c(new_n109_), .d(x18), .O(new_n151_));
  oai22  g107(.a(new_n151_), .b(new_n133_), .c(new_n147_), .d(new_n109_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n137_), .O(new_n153_));
  xnor2a g109(.a(x31), .b(x30), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x18), .c(x17), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n141_), .c(x33), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n153_), .c(new_n150_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x00), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n96_), .c(x26), .O(z9));
endmodule


