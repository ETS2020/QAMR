// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x17), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n48_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  nor3   g044(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(new_n89_));
  aoi21  g045(.a(new_n89_), .b(new_n85_), .c(new_n61_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n60_), .c(new_n48_), .O(z3));
  xor2a  g048(.a(x28), .b(x27), .O(new_n93_));
  inv1   g049(.a(new_n87_), .O(new_n94_));
  inv1   g050(.a(new_n88_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(new_n96_));
  aoi21  g052(.a(new_n96_), .b(new_n48_), .c(new_n86_), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n61_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n93_), .c(new_n81_), .d(x25), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n60_), .O(z4));
  nand2  g056(.a(x28), .b(x27), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(x29), .b(new_n102_), .O(new_n103_));
  aoi22  g059(.a(new_n103_), .b(x27), .c(new_n101_), .d(x29), .O(new_n104_));
  nor2   g060(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n60_), .O(z5));
  inv1   g063(.a(x29), .O(new_n108_));
  nand3  g064(.a(new_n101_), .b(x30), .c(new_n45_), .O(new_n109_));
  nand2  g065(.a(x30), .b(new_n45_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x28), .c(x27), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  nor2   g068(.a(new_n46_), .b(x29), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n112_), .c(new_n96_), .O(new_n114_));
  inv1   g070(.a(new_n101_), .O(new_n115_));
  nand3  g071(.a(new_n46_), .b(x28), .c(x27), .O(new_n116_));
  oai21  g072(.a(new_n115_), .b(new_n46_), .c(new_n116_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x29), .c(new_n45_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nor2   g075(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(x22), .c(new_n114_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n60_), .c(new_n48_), .O(z6));
  inv1   g079(.a(x31), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x29), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n101_), .c(new_n45_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x30), .O(new_n127_));
  nand2  g083(.a(x27), .b(new_n45_), .O(new_n128_));
  nand3  g084(.a(x30), .b(x29), .c(x28), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n128_), .c(x31), .O(new_n130_));
  inv1   g086(.a(new_n86_), .O(new_n131_));
  nand4  g087(.a(new_n95_), .b(new_n94_), .c(new_n131_), .d(new_n85_), .O(new_n132_));
  nand4  g088(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n132_), .c(new_n130_), .d(new_n127_), .O(z7));
  nor2   g091(.a(x28), .b(x27), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  inv1   g093(.a(x32), .O(new_n138_));
  inv1   g094(.a(x15), .O(new_n139_));
  inv1   g095(.a(x18), .O(new_n140_));
  nand3  g096(.a(x19), .b(new_n140_), .c(x13), .O(new_n141_));
  inv1   g097(.a(x19), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x18), .c(x14), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n141_), .c(new_n45_), .O(new_n144_));
  nand2  g100(.a(new_n45_), .b(x12), .O(new_n145_));
  nand3  g101(.a(new_n46_), .b(x19), .c(x18), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n144_), .c(x16), .O(new_n148_));
  inv1   g104(.a(x16), .O(new_n149_));
  nand3  g105(.a(x19), .b(x18), .c(x17), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n149_), .c(x11), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n148_), .c(new_n139_), .O(new_n153_));
  inv1   g109(.a(x10), .O(new_n154_));
  nor4   g110(.a(new_n150_), .b(new_n149_), .c(x15), .d(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(new_n138_), .O(new_n156_));
  nand2  g112(.a(x32), .b(x30), .O(new_n157_));
  oai21  g113(.a(new_n156_), .b(new_n124_), .c(new_n157_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n137_), .c(x29), .O(new_n159_));
  nor2   g115(.a(new_n136_), .b(new_n108_), .O(new_n160_));
  nand2  g116(.a(new_n110_), .b(x32), .O(new_n161_));
  nor2   g117(.a(new_n140_), .b(new_n149_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(x15), .c(x12), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nor2   g120(.a(x32), .b(new_n124_), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(new_n164_), .c(x30), .d(x19), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n161_), .c(new_n160_), .O(new_n167_));
  aoi21  g123(.a(new_n142_), .b(x14), .c(new_n140_), .O(new_n168_));
  nand4  g124(.a(new_n168_), .b(x17), .c(x16), .d(x15), .O(new_n169_));
  nor2   g125(.a(new_n149_), .b(new_n139_), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand2  g127(.a(x18), .b(x17), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n171_), .c(new_n142_), .O(new_n173_));
  nand2  g129(.a(x16), .b(x10), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n172_), .c(new_n139_), .O(new_n175_));
  inv1   g131(.a(x11), .O(new_n176_));
  oai21  g132(.a(new_n172_), .b(new_n176_), .c(new_n149_), .O(new_n177_));
  nand2  g133(.a(x18), .b(x12), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n45_), .O(new_n179_));
  nor2   g135(.a(x18), .b(x13), .O(new_n180_));
  nor2   g136(.a(new_n180_), .b(new_n124_), .O(new_n181_));
  nand3  g137(.a(new_n181_), .b(new_n179_), .c(new_n177_), .O(new_n182_));
  inv1   g138(.a(new_n182_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n175_), .c(new_n173_), .d(new_n169_), .O(new_n184_));
  aoi21  g140(.a(new_n184_), .b(x32), .c(new_n167_), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n159_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n81_), .c(x00), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n48_), .O(z8));
  inv1   g144(.a(x00), .O(new_n189_));
  inv1   g145(.a(x33), .O(new_n190_));
  nand4  g146(.a(new_n190_), .b(new_n124_), .c(new_n46_), .d(x19), .O(new_n191_));
  oai22  g147(.a(new_n191_), .b(new_n163_), .c(new_n190_), .d(new_n46_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n45_), .O(new_n193_));
  aoi21  g149(.a(new_n143_), .b(new_n141_), .c(new_n149_), .O(new_n194_));
  nor2   g150(.a(new_n142_), .b(new_n140_), .O(new_n195_));
  nand3  g151(.a(new_n195_), .b(new_n149_), .c(x11), .O(new_n196_));
  inv1   g152(.a(new_n196_), .O(new_n197_));
  oai21  g153(.a(new_n197_), .b(new_n194_), .c(x15), .O(new_n198_));
  nand4  g154(.a(new_n195_), .b(x16), .c(new_n139_), .d(x10), .O(new_n199_));
  aoi21  g155(.a(new_n199_), .b(new_n198_), .c(x33), .O(new_n200_));
  nand4  g156(.a(new_n200_), .b(new_n124_), .c(new_n46_), .d(x17), .O(new_n201_));
  aoi21  g157(.a(new_n201_), .b(new_n193_), .c(new_n160_), .O(new_n202_));
  nand4  g158(.a(new_n45_), .b(x16), .c(x15), .d(x12), .O(new_n203_));
  nand4  g159(.a(new_n195_), .b(new_n190_), .c(x31), .d(x30), .O(new_n204_));
  oai22  g160(.a(new_n204_), .b(new_n203_), .c(new_n190_), .d(x30), .O(new_n205_));
  nand3  g161(.a(new_n205_), .b(new_n137_), .c(x29), .O(new_n206_));
  inv1   g162(.a(new_n162_), .O(new_n207_));
  oai21  g163(.a(new_n207_), .b(new_n139_), .c(new_n142_), .O(new_n208_));
  oai21  g164(.a(new_n207_), .b(new_n154_), .c(new_n139_), .O(new_n209_));
  aoi21  g165(.a(x18), .b(x11), .c(x16), .O(new_n210_));
  nor3   g166(.a(new_n210_), .b(new_n180_), .c(x31), .O(new_n211_));
  nand4  g167(.a(new_n211_), .b(new_n209_), .c(new_n208_), .d(new_n169_), .O(new_n212_));
  aoi21  g168(.a(new_n124_), .b(x30), .c(new_n142_), .O(new_n213_));
  nand4  g169(.a(new_n213_), .b(new_n170_), .c(x18), .d(x12), .O(new_n214_));
  aoi22  g170(.a(new_n214_), .b(new_n45_), .c(new_n212_), .d(new_n46_), .O(new_n215_));
  oai21  g171(.a(new_n215_), .b(new_n190_), .c(new_n206_), .O(new_n216_));
  oai21  g172(.a(new_n216_), .b(new_n202_), .c(new_n81_), .O(new_n217_));
  nor2   g173(.a(new_n217_), .b(new_n189_), .O(z9));
endmodule


