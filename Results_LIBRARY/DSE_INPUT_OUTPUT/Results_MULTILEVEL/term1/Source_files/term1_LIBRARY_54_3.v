// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x30), .O(new_n45_));
  nand2  g001(.a(x32), .b(new_n45_), .O(z0));
  inv1   g002(.a(x33), .O(new_n47_));
  nand3  g003(.a(new_n47_), .b(new_n45_), .c(x03), .O(new_n48_));
  oai21  g004(.a(x32), .b(x03), .c(new_n48_), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  inv1   g007(.a(x03), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n52_), .O(new_n53_));
  nor3   g009(.a(x33), .b(x30), .c(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  inv1   g011(.a(x32), .O(new_n56_));
  nand2  g012(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n55_), .c(new_n50_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n52_), .b(new_n51_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n52_), .b(new_n51_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n52_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n51_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n51_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n52_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n52_), .d(new_n51_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(x32), .b(x30), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  oai22  g039(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n84_));
  inv1   g040(.a(x17), .O(new_n85_));
  inv1   g041(.a(x22), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x18), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  inv1   g047(.a(x24), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n90_), .c(new_n87_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n61_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n83_), .c(new_n82_), .d(x25), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n60_), .c(new_n80_), .O(z3));
  xor2a  g054(.a(x28), .b(x27), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n96_), .c(new_n80_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n82_), .c(x25), .d(x01), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(z4));
  inv1   g059(.a(new_n95_), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  inv1   g061(.a(x28), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n83_), .O(new_n107_));
  nand3  g063(.a(new_n105_), .b(x28), .c(x27), .O(new_n108_));
  oai21  g064(.a(new_n107_), .b(new_n105_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n104_), .c(new_n62_), .d(new_n82_), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x25), .c(x01), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n80_), .O(z5));
  inv1   g069(.a(x25), .O(new_n114_));
  nand3  g070(.a(x29), .b(x28), .c(x27), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n56_), .c(x30), .O(new_n116_));
  nand3  g072(.a(new_n107_), .b(new_n45_), .c(x29), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n104_), .c(new_n62_), .d(new_n82_), .O(new_n119_));
  nor3   g075(.a(new_n119_), .b(new_n114_), .c(new_n60_), .O(z6));
  inv1   g076(.a(x31), .O(new_n121_));
  nand3  g077(.a(new_n107_), .b(new_n121_), .c(x29), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n56_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x30), .O(new_n124_));
  inv1   g080(.a(new_n107_), .O(new_n125_));
  nand3  g081(.a(new_n56_), .b(x30), .c(x29), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n125_), .c(x31), .O(new_n127_));
  inv1   g083(.a(new_n84_), .O(new_n128_));
  nand4  g084(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n128_), .O(new_n129_));
  nand4  g085(.a(new_n62_), .b(new_n82_), .c(x25), .d(x01), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n129_), .c(new_n127_), .d(new_n124_), .O(z7));
  nand2  g088(.a(new_n106_), .b(new_n83_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n45_), .c(x29), .O(new_n134_));
  nand2  g090(.a(new_n133_), .b(x29), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x30), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand3  g093(.a(x19), .b(new_n88_), .c(x13), .O(new_n138_));
  nand3  g094(.a(new_n91_), .b(x18), .c(x14), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n138_), .c(new_n85_), .O(new_n140_));
  nand4  g096(.a(x19), .b(x18), .c(new_n85_), .d(x12), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  inv1   g099(.a(x16), .O(new_n144_));
  nand3  g100(.a(x19), .b(x18), .c(x17), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n144_), .c(x11), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x15), .O(new_n149_));
  inv1   g105(.a(x15), .O(new_n150_));
  nand4  g106(.a(new_n146_), .b(x16), .c(new_n150_), .d(x10), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n137_), .c(new_n56_), .d(x31), .O(new_n153_));
  aoi21  g109(.a(new_n91_), .b(x14), .c(new_n88_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x17), .c(x16), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n150_), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand2  g113(.a(x16), .b(x15), .O(new_n158_));
  nand2  g114(.a(x18), .b(x17), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n158_), .c(new_n91_), .O(new_n160_));
  nand2  g116(.a(x16), .b(x10), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n159_), .c(new_n150_), .O(new_n162_));
  inv1   g118(.a(new_n159_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(x11), .c(x16), .O(new_n164_));
  aoi21  g120(.a(x18), .b(x12), .c(x17), .O(new_n165_));
  inv1   g121(.a(x13), .O(new_n166_));
  nand2  g122(.a(new_n88_), .b(new_n166_), .O(new_n167_));
  nor2   g123(.a(new_n121_), .b(new_n105_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n167_), .c(new_n133_), .O(new_n169_));
  nor3   g125(.a(new_n169_), .b(new_n165_), .c(new_n164_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(new_n162_), .c(new_n160_), .d(new_n157_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(x32), .c(new_n45_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n153_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n82_), .c(x00), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(z8));
  inv1   g131(.a(new_n164_), .O(new_n176_));
  aoi21  g132(.a(new_n88_), .b(new_n166_), .c(new_n165_), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(new_n176_), .c(new_n162_), .d(new_n160_), .O(new_n178_));
  oai21  g134(.a(new_n178_), .b(new_n156_), .c(new_n80_), .O(new_n179_));
  nand2  g135(.a(new_n135_), .b(new_n121_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n45_), .O(new_n181_));
  nand2  g137(.a(new_n168_), .b(new_n133_), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n56_), .c(x30), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n181_), .c(new_n179_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(x33), .O(new_n185_));
  nand4  g141(.a(new_n133_), .b(new_n56_), .c(x31), .d(x30), .O(new_n186_));
  oai22  g142(.a(new_n186_), .b(new_n105_), .c(new_n180_), .d(x30), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n152_), .c(new_n47_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n185_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n82_), .c(x00), .O(new_n190_));
  inv1   g146(.a(new_n190_), .O(z9));
endmodule


