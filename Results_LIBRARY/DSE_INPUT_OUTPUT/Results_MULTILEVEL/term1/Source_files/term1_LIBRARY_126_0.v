// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_;
  nor2   g000(.a(x30), .b(x22), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n47_), .c(new_n48_), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  inv1   g008(.a(x03), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g010(.a(x03), .b(x02), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  or2    g012(.a(new_n56_), .b(x33), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n53_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n52_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n52_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n53_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n60_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  nor2   g025(.a(new_n49_), .b(new_n61_), .O(new_n70_));
  nand2  g026(.a(new_n65_), .b(new_n63_), .O(new_n71_));
  nand3  g027(.a(new_n55_), .b(x06), .c(x05), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n68_), .O(new_n75_));
  nand4  g031(.a(new_n75_), .b(new_n47_), .c(new_n59_), .d(x08), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  oai22  g035(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n80_));
  nor2   g036(.a(x23), .b(x18), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nor2   g038(.a(x24), .b(x19), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n80_), .c(new_n47_), .O(new_n86_));
  inv1   g042(.a(x17), .O(new_n87_));
  inv1   g043(.a(x22), .O(new_n88_));
  nand3  g044(.a(x30), .b(new_n88_), .c(new_n87_), .O(new_n89_));
  aoi21  g045(.a(new_n89_), .b(new_n86_), .c(new_n54_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n79_), .c(new_n78_), .d(x25), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n61_), .O(z3));
  xor2a  g048(.a(x28), .b(x27), .O(new_n93_));
  inv1   g049(.a(new_n80_), .O(new_n94_));
  aoi21  g050(.a(new_n88_), .b(new_n87_), .c(new_n85_), .O(new_n95_));
  aoi21  g051(.a(new_n95_), .b(new_n94_), .c(new_n54_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n93_), .c(new_n78_), .d(x25), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n61_), .c(new_n47_), .O(z4));
  inv1   g054(.a(new_n54_), .O(new_n99_));
  inv1   g055(.a(x28), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n79_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n47_), .c(x29), .O(new_n103_));
  inv1   g059(.a(x29), .O(new_n104_));
  nand3  g060(.a(new_n87_), .b(x16), .c(x15), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x30), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n104_), .c(x28), .d(x27), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n103_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n85_), .c(new_n99_), .O(new_n110_));
  nand3  g066(.a(new_n104_), .b(x28), .c(x27), .O(new_n111_));
  oai21  g067(.a(new_n101_), .b(new_n104_), .c(new_n111_), .O(new_n112_));
  oai21  g068(.a(new_n94_), .b(new_n45_), .c(new_n89_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n112_), .c(new_n99_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n78_), .c(x25), .d(x01), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(z5));
  nand2  g073(.a(new_n95_), .b(new_n94_), .O(new_n118_));
  nand2  g074(.a(new_n101_), .b(x29), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(x30), .O(new_n120_));
  nor2   g076(.a(new_n85_), .b(new_n80_), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(x30), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(x29), .c(x28), .d(x27), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n88_), .c(new_n120_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n99_), .c(new_n78_), .d(x25), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n61_), .O(z6));
  inv1   g082(.a(x30), .O(new_n127_));
  oai21  g083(.a(x31), .b(new_n88_), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n99_), .b(new_n78_), .c(x25), .d(x01), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g086(.a(x29), .b(x28), .c(x27), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x31), .O(new_n132_));
  nand3  g088(.a(new_n84_), .b(new_n82_), .c(new_n94_), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x17), .O(new_n135_));
  inv1   g091(.a(x31), .O(new_n136_));
  nand3  g092(.a(new_n101_), .b(new_n136_), .c(x29), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(new_n132_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x30), .O(new_n139_));
  nor2   g095(.a(new_n136_), .b(x30), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n134_), .c(x22), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n139_), .c(new_n130_), .O(z7));
  inv1   g098(.a(x15), .O(new_n143_));
  inv1   g099(.a(x19), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x14), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(x18), .c(x17), .d(x16), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  nand2  g103(.a(x16), .b(x15), .O(new_n148_));
  nand2  g104(.a(x18), .b(x17), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n148_), .c(new_n144_), .O(new_n150_));
  nand2  g106(.a(x16), .b(x10), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n149_), .c(new_n143_), .O(new_n152_));
  inv1   g108(.a(x11), .O(new_n153_));
  inv1   g109(.a(x16), .O(new_n154_));
  oai21  g110(.a(new_n149_), .b(new_n153_), .c(new_n154_), .O(new_n155_));
  aoi21  g111(.a(x18), .b(x12), .c(x17), .O(new_n156_));
  nor2   g112(.a(x18), .b(x13), .O(new_n157_));
  nor3   g113(.a(new_n157_), .b(new_n156_), .c(new_n136_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n155_), .c(new_n152_), .d(new_n150_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n147_), .c(new_n47_), .O(new_n160_));
  nand2  g116(.a(new_n100_), .b(new_n79_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x29), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n127_), .c(x22), .O(new_n163_));
  nand3  g119(.a(new_n161_), .b(x30), .c(x29), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n163_), .c(new_n160_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(x32), .O(new_n166_));
  nand4  g122(.a(new_n161_), .b(new_n127_), .c(x29), .d(x22), .O(new_n167_));
  nand2  g123(.a(new_n162_), .b(x30), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  inv1   g125(.a(x18), .O(new_n170_));
  nand3  g126(.a(x19), .b(new_n170_), .c(x13), .O(new_n171_));
  nand3  g127(.a(new_n144_), .b(x18), .c(x14), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n171_), .c(new_n87_), .O(new_n173_));
  nand4  g129(.a(x19), .b(x18), .c(new_n87_), .d(x12), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n173_), .c(x16), .O(new_n176_));
  nand3  g132(.a(x19), .b(x18), .c(x17), .O(new_n177_));
  inv1   g133(.a(new_n177_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n154_), .c(x11), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n176_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(x15), .O(new_n181_));
  nand4  g137(.a(new_n178_), .b(x16), .c(new_n143_), .d(x10), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n169_), .c(new_n48_), .d(x31), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n166_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n78_), .c(x00), .O(new_n186_));
  inv1   g142(.a(new_n186_), .O(z8));
  nor2   g143(.a(new_n157_), .b(new_n156_), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(new_n155_), .c(new_n152_), .d(new_n150_), .O(new_n189_));
  oai21  g145(.a(new_n189_), .b(new_n147_), .c(new_n47_), .O(new_n190_));
  aoi21  g146(.a(new_n162_), .b(new_n136_), .c(x30), .O(new_n191_));
  nor2   g147(.a(new_n136_), .b(new_n104_), .O(new_n192_));
  aoi21  g148(.a(new_n192_), .b(new_n161_), .c(new_n127_), .O(new_n193_));
  aoi21  g149(.a(new_n191_), .b(x22), .c(new_n193_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n190_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x33), .O(new_n196_));
  aoi21  g152(.a(new_n179_), .b(new_n176_), .c(new_n143_), .O(new_n197_));
  inv1   g153(.a(new_n182_), .O(new_n198_));
  nand4  g154(.a(new_n162_), .b(new_n136_), .c(new_n127_), .d(x22), .O(new_n199_));
  nand4  g155(.a(new_n161_), .b(x31), .c(x30), .d(x29), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(new_n199_), .O(new_n201_));
  oai21  g157(.a(new_n198_), .b(new_n197_), .c(new_n201_), .O(new_n202_));
  oai21  g158(.a(new_n202_), .b(x33), .c(new_n196_), .O(new_n203_));
  nand3  g159(.a(new_n203_), .b(new_n78_), .c(x00), .O(new_n204_));
  inv1   g160(.a(new_n204_), .O(z9));
endmodule


