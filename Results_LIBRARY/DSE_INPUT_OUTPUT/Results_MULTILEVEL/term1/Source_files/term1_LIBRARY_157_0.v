// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_;
  nor2   g000(.a(x30), .b(x02), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g010(.a(x33), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  nand3  g012(.a(new_n56_), .b(new_n54_), .c(x30), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n51_), .O(z1));
  inv1   g015(.a(x08), .O(new_n60_));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  nand2  g017(.a(x03), .b(x02), .O(new_n62_));
  inv1   g018(.a(x30), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x01), .O(new_n67_));
  inv1   g023(.a(x01), .O(new_n68_));
  nand2  g024(.a(x05), .b(new_n49_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(x06), .c(new_n52_), .O(new_n70_));
  inv1   g026(.a(x06), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(x05), .c(new_n49_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n70_), .c(new_n63_), .O(new_n73_));
  inv1   g029(.a(new_n69_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x02), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n68_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  xnor2a g034(.a(x07), .b(x04), .O(new_n79_));
  nand2  g035(.a(new_n49_), .b(x02), .O(new_n80_));
  nand3  g036(.a(x30), .b(x03), .c(new_n52_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x01), .O(new_n83_));
  aoi21  g039(.a(x30), .b(new_n71_), .c(x02), .O(new_n84_));
  nor2   g040(.a(x03), .b(x02), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(x30), .c(x06), .d(x05), .O(new_n86_));
  oai21  g042(.a(new_n84_), .b(new_n74_), .c(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n68_), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n83_), .c(new_n79_), .O(new_n89_));
  aoi21  g045(.a(new_n78_), .b(new_n61_), .c(new_n89_), .O(new_n90_));
  nor3   g046(.a(new_n90_), .b(x09), .c(new_n60_), .O(z2));
  inv1   g047(.a(x25), .O(new_n92_));
  inv1   g048(.a(x26), .O(new_n93_));
  inv1   g049(.a(x27), .O(new_n94_));
  nand2  g050(.a(x30), .b(new_n52_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n80_), .O(new_n96_));
  nor2   g052(.a(x20), .b(x15), .O(new_n97_));
  nor2   g053(.a(x21), .b(x16), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  inv1   g055(.a(x17), .O(new_n100_));
  inv1   g056(.a(x22), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  inv1   g058(.a(x18), .O(new_n103_));
  inv1   g059(.a(x23), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nor2   g061(.a(x24), .b(x19), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n105_), .c(new_n102_), .d(new_n99_), .O(new_n108_));
  and2   g064(.a(new_n108_), .b(new_n96_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n94_), .c(new_n93_), .O(new_n110_));
  nor3   g066(.a(new_n110_), .b(new_n92_), .c(new_n68_), .O(z3));
  xor2a  g067(.a(x28), .b(x27), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n109_), .c(new_n93_), .O(new_n113_));
  nor3   g069(.a(new_n113_), .b(new_n92_), .c(new_n68_), .O(z4));
  inv1   g070(.a(x29), .O(new_n115_));
  inv1   g071(.a(x28), .O(new_n116_));
  nor2   g072(.a(new_n116_), .b(new_n94_), .O(new_n117_));
  nand3  g073(.a(new_n115_), .b(x28), .c(x27), .O(new_n118_));
  oai21  g074(.a(new_n117_), .b(new_n115_), .c(new_n118_), .O(new_n119_));
  oai21  g075(.a(new_n45_), .b(x03), .c(new_n95_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n119_), .c(new_n108_), .d(new_n93_), .O(new_n121_));
  nor3   g077(.a(new_n121_), .b(new_n92_), .c(new_n68_), .O(z5));
  inv1   g078(.a(new_n117_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n62_), .c(x30), .O(new_n124_));
  nand4  g080(.a(new_n63_), .b(x28), .c(x27), .d(new_n49_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x29), .O(new_n127_));
  oai21  g083(.a(new_n64_), .b(new_n52_), .c(new_n115_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n108_), .c(new_n93_), .d(x25), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n68_), .c(new_n46_), .O(z6));
  oai21  g087(.a(x31), .b(new_n52_), .c(new_n63_), .O(new_n132_));
  nand3  g088(.a(new_n105_), .b(new_n102_), .c(new_n99_), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n106_), .O(new_n134_));
  nand3  g090(.a(new_n93_), .b(x25), .c(x01), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n134_), .c(new_n132_), .O(new_n136_));
  nand2  g092(.a(x31), .b(new_n63_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n49_), .c(new_n52_), .O(new_n138_));
  nand3  g094(.a(x29), .b(x28), .c(x27), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x31), .O(new_n140_));
  inv1   g096(.a(x31), .O(new_n141_));
  nand3  g097(.a(new_n117_), .b(new_n141_), .c(x29), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(x30), .c(new_n138_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n136_), .O(z7));
  nand2  g101(.a(new_n116_), .b(new_n94_), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n103_), .c(x13), .O(new_n147_));
  inv1   g103(.a(x19), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n100_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n100_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  inv1   g109(.a(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  inv1   g115(.a(x15), .O(new_n160_));
  nand4  g116(.a(new_n156_), .b(x16), .c(new_n160_), .d(x10), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n159_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n53_), .c(x31), .d(new_n63_), .O(new_n163_));
  nand2  g119(.a(x32), .b(x30), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n146_), .c(x29), .O(new_n166_));
  nand2  g122(.a(new_n146_), .b(x29), .O(new_n167_));
  nand4  g123(.a(new_n162_), .b(new_n53_), .c(x31), .d(x30), .O(new_n168_));
  nand2  g124(.a(x32), .b(new_n63_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n167_), .O(new_n171_));
  nand2  g127(.a(new_n148_), .b(x14), .O(new_n172_));
  nand4  g128(.a(new_n172_), .b(x18), .c(x17), .d(x16), .O(new_n173_));
  nor2   g129(.a(new_n173_), .b(new_n160_), .O(new_n174_));
  nand2  g130(.a(x16), .b(x15), .O(new_n175_));
  nand2  g131(.a(x18), .b(x17), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n175_), .c(new_n148_), .O(new_n177_));
  nand2  g133(.a(x16), .b(x10), .O(new_n178_));
  oai21  g134(.a(new_n178_), .b(new_n176_), .c(new_n160_), .O(new_n179_));
  inv1   g135(.a(x11), .O(new_n180_));
  oai21  g136(.a(new_n176_), .b(new_n180_), .c(new_n154_), .O(new_n181_));
  aoi21  g137(.a(x18), .b(x12), .c(x17), .O(new_n182_));
  nor2   g138(.a(x18), .b(x13), .O(new_n183_));
  nor3   g139(.a(new_n183_), .b(new_n182_), .c(new_n141_), .O(new_n184_));
  nand4  g140(.a(new_n184_), .b(new_n181_), .c(new_n179_), .d(new_n177_), .O(new_n185_));
  oai21  g141(.a(new_n185_), .b(new_n174_), .c(x32), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n171_), .c(new_n166_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n93_), .c(x00), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n46_), .O(z8));
  nand4  g145(.a(new_n162_), .b(new_n55_), .c(x31), .d(x30), .O(new_n190_));
  nand2  g146(.a(x33), .b(new_n63_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n146_), .c(x29), .O(new_n193_));
  nand4  g149(.a(new_n162_), .b(new_n55_), .c(new_n141_), .d(new_n63_), .O(new_n194_));
  nand2  g150(.a(x33), .b(x30), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(new_n194_), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n167_), .O(new_n197_));
  inv1   g153(.a(new_n182_), .O(new_n198_));
  inv1   g154(.a(new_n183_), .O(new_n199_));
  xnor2a g155(.a(x31), .b(x30), .O(new_n200_));
  nand3  g156(.a(new_n200_), .b(new_n199_), .c(new_n198_), .O(new_n201_));
  inv1   g157(.a(new_n201_), .O(new_n202_));
  nand4  g158(.a(new_n202_), .b(new_n181_), .c(new_n179_), .d(new_n177_), .O(new_n203_));
  oai21  g159(.a(new_n203_), .b(new_n174_), .c(x33), .O(new_n204_));
  nand3  g160(.a(new_n204_), .b(new_n197_), .c(new_n193_), .O(new_n205_));
  nand3  g161(.a(new_n205_), .b(new_n93_), .c(x00), .O(new_n206_));
  nand2  g162(.a(new_n206_), .b(new_n46_), .O(z9));
endmodule


