// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x30), .O(new_n45_));
  aoi21  g001(.a(new_n45_), .b(x01), .c(x32), .O(z0));
  nand2  g002(.a(new_n45_), .b(x01), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n47_), .c(new_n48_), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  inv1   g008(.a(x03), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g010(.a(x03), .b(x02), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  or2    g012(.a(new_n56_), .b(x33), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n51_), .c(new_n47_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n56_), .b(new_n60_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n53_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(new_n52_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n52_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x05), .c(new_n53_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n63_), .c(x01), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n61_), .c(new_n59_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  nor2   g024(.a(new_n49_), .b(new_n60_), .O(new_n69_));
  nand2  g025(.a(new_n64_), .b(new_n62_), .O(new_n70_));
  nand3  g026(.a(new_n55_), .b(x06), .c(x05), .O(new_n71_));
  aoi21  g027(.a(new_n71_), .b(new_n70_), .c(x01), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n67_), .c(x09), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x08), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n47_), .O(z2));
  inv1   g032(.a(x26), .O(new_n77_));
  inv1   g033(.a(x27), .O(new_n78_));
  inv1   g034(.a(new_n54_), .O(new_n79_));
  oai22  g035(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  inv1   g037(.a(x17), .O(new_n82_));
  inv1   g038(.a(x22), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g040(.a(x18), .O(new_n85_));
  inv1   g041(.a(x23), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x19), .O(new_n88_));
  inv1   g044(.a(x24), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n87_), .c(new_n84_), .d(new_n81_), .O(new_n91_));
  and2   g047(.a(new_n91_), .b(new_n79_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(x30), .c(new_n78_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n77_), .c(x25), .d(x01), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z3));
  nand3  g052(.a(x30), .b(x28), .c(new_n78_), .O(new_n97_));
  oai21  g053(.a(x28), .b(new_n78_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n92_), .c(new_n77_), .d(x25), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(x30), .c(new_n60_), .O(z4));
  inv1   g056(.a(x28), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n78_), .c(x29), .O(new_n102_));
  inv1   g058(.a(x29), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x28), .c(x27), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n91_), .c(new_n79_), .d(x30), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n77_), .c(x25), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(x30), .c(new_n60_), .O(z5));
  nand3  g065(.a(x29), .b(x28), .c(x27), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n91_), .c(new_n79_), .d(x30), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n77_), .c(x25), .d(x01), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(z6));
  nand2  g070(.a(new_n110_), .b(x31), .O(new_n115_));
  nand4  g071(.a(new_n90_), .b(new_n87_), .c(new_n84_), .d(new_n81_), .O(new_n116_));
  nand3  g072(.a(new_n79_), .b(x25), .c(x01), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(x29), .c(x28), .d(x27), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x30), .c(new_n77_), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n116_), .c(new_n115_), .O(z7));
  inv1   g078(.a(x15), .O(new_n123_));
  nand2  g079(.a(new_n88_), .b(x14), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(x18), .c(x17), .d(x16), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g082(.a(x16), .b(x15), .O(new_n127_));
  nand2  g083(.a(x18), .b(x17), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n127_), .c(new_n88_), .O(new_n129_));
  nand2  g085(.a(x16), .b(x10), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n128_), .c(new_n123_), .O(new_n131_));
  inv1   g087(.a(x11), .O(new_n132_));
  inv1   g088(.a(x16), .O(new_n133_));
  oai21  g089(.a(new_n128_), .b(new_n132_), .c(new_n133_), .O(new_n134_));
  aoi21  g090(.a(x18), .b(x12), .c(x17), .O(new_n135_));
  nor2   g091(.a(x18), .b(x13), .O(new_n136_));
  nor3   g092(.a(new_n136_), .b(new_n135_), .c(new_n118_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n134_), .c(new_n131_), .d(new_n129_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n126_), .c(new_n47_), .O(new_n139_));
  nand2  g095(.a(new_n101_), .b(new_n78_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x29), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n45_), .c(new_n60_), .O(new_n142_));
  nand3  g098(.a(new_n140_), .b(x30), .c(x29), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n142_), .c(new_n139_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x32), .O(new_n145_));
  nand4  g101(.a(new_n140_), .b(new_n45_), .c(x29), .d(new_n60_), .O(new_n146_));
  nand2  g102(.a(new_n141_), .b(x30), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand3  g104(.a(x19), .b(new_n85_), .c(x13), .O(new_n149_));
  nand3  g105(.a(new_n88_), .b(x18), .c(x14), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(new_n82_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n82_), .d(x12), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n133_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  nand4  g115(.a(new_n156_), .b(x16), .c(new_n123_), .d(x10), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n148_), .c(new_n48_), .d(x31), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n145_), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n77_), .c(x00), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(z8));
  nor2   g121(.a(new_n136_), .b(new_n135_), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(new_n134_), .c(new_n131_), .d(new_n129_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n126_), .c(new_n47_), .O(new_n168_));
  aoi21  g124(.a(new_n141_), .b(new_n118_), .c(x30), .O(new_n169_));
  nor2   g125(.a(new_n118_), .b(new_n103_), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n140_), .c(new_n45_), .O(new_n171_));
  aoi21  g127(.a(new_n169_), .b(new_n60_), .c(new_n171_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x33), .O(new_n174_));
  aoi21  g130(.a(new_n157_), .b(new_n154_), .c(new_n123_), .O(new_n175_));
  inv1   g131(.a(new_n160_), .O(new_n176_));
  nand4  g132(.a(new_n141_), .b(new_n118_), .c(new_n45_), .d(new_n60_), .O(new_n177_));
  nand4  g133(.a(new_n140_), .b(x31), .c(x30), .d(x29), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  oai21  g135(.a(new_n176_), .b(new_n175_), .c(new_n179_), .O(new_n180_));
  oai21  g136(.a(new_n180_), .b(x33), .c(new_n174_), .O(new_n181_));
  nand3  g137(.a(new_n181_), .b(new_n77_), .c(x00), .O(new_n182_));
  inv1   g138(.a(new_n182_), .O(z9));
endmodule


