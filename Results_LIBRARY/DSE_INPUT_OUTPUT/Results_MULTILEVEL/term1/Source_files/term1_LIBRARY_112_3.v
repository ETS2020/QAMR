// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  inv1   g000(.a(x03), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(new_n46_), .c(x03), .O(new_n50_));
  oai21  g006(.a(x32), .b(x03), .c(new_n50_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n46_), .c(x03), .O(new_n55_));
  oai21  g011(.a(x33), .b(x03), .c(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n52_), .O(z1));
  inv1   g014(.a(new_n47_), .O(new_n59_));
  inv1   g015(.a(x09), .O(new_n60_));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  nor2   g017(.a(new_n45_), .b(new_n53_), .O(new_n62_));
  nor2   g018(.a(x03), .b(x02), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n62_), .c(x01), .O(new_n64_));
  inv1   g020(.a(x01), .O(new_n65_));
  nand2  g021(.a(x03), .b(new_n65_), .O(new_n66_));
  oai21  g022(.a(x05), .b(x03), .c(new_n66_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x06), .c(new_n53_), .O(new_n68_));
  nand2  g024(.a(x06), .b(new_n53_), .O(new_n69_));
  nand4  g025(.a(new_n69_), .b(x05), .c(new_n45_), .d(new_n65_), .O(new_n70_));
  nand3  g026(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n61_), .O(new_n72_));
  xnor2a g028(.a(x07), .b(x04), .O(new_n73_));
  xor2a  g029(.a(x03), .b(x02), .O(new_n74_));
  nand2  g030(.a(new_n67_), .b(x02), .O(new_n75_));
  aoi21  g031(.a(x05), .b(new_n45_), .c(x06), .O(new_n76_));
  nand3  g032(.a(new_n63_), .b(x06), .c(x05), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n76_), .c(new_n65_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  aoi21  g036(.a(new_n74_), .b(x01), .c(new_n80_), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n73_), .c(new_n72_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n60_), .c(x08), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n59_), .O(z2));
  inv1   g040(.a(x25), .O(new_n85_));
  inv1   g041(.a(x27), .O(new_n86_));
  oai22  g042(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n87_));
  nor2   g043(.a(x22), .b(x17), .O(new_n88_));
  nor2   g044(.a(x23), .b(x18), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  nor4   g046(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n87_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n62_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n86_), .c(new_n46_), .O(new_n93_));
  nor3   g049(.a(new_n93_), .b(new_n85_), .c(new_n65_), .O(z3));
  xor2a  g050(.a(x28), .b(x27), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n92_), .c(new_n46_), .d(x25), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n65_), .c(new_n59_), .O(z4));
  inv1   g053(.a(new_n62_), .O(new_n98_));
  inv1   g054(.a(new_n91_), .O(new_n99_));
  inv1   g055(.a(x29), .O(new_n100_));
  inv1   g056(.a(x28), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n86_), .O(new_n102_));
  nand3  g058(.a(new_n100_), .b(x28), .c(x27), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n99_), .c(new_n98_), .d(new_n46_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x25), .c(x01), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n59_), .O(z5));
  inv1   g064(.a(x30), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x28), .c(x27), .O(new_n110_));
  oai21  g066(.a(new_n102_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nor2   g067(.a(new_n109_), .b(x29), .O(new_n112_));
  aoi21  g068(.a(new_n111_), .b(x29), .c(new_n112_), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n91_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n98_), .c(new_n46_), .d(x25), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n65_), .c(new_n59_), .O(z6));
  nand3  g072(.a(new_n102_), .b(x30), .c(x29), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x31), .O(new_n118_));
  inv1   g074(.a(new_n89_), .O(new_n119_));
  inv1   g075(.a(new_n90_), .O(new_n120_));
  nor2   g076(.a(new_n88_), .b(new_n87_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nor2   g078(.a(new_n62_), .b(new_n65_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x30), .c(x29), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n102_), .c(new_n85_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n123_), .c(new_n122_), .d(new_n118_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n46_), .O(new_n129_));
  oai21  g085(.a(new_n46_), .b(x03), .c(new_n129_), .O(z7));
  nand2  g086(.a(new_n101_), .b(new_n86_), .O(new_n131_));
  inv1   g087(.a(x17), .O(new_n132_));
  inv1   g088(.a(x18), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n133_), .c(x13), .O(new_n134_));
  inv1   g090(.a(x19), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x18), .c(x14), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n134_), .c(new_n132_), .O(new_n137_));
  nand4  g093(.a(x19), .b(x18), .c(new_n132_), .d(x12), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  inv1   g096(.a(x16), .O(new_n141_));
  nand3  g097(.a(x19), .b(x18), .c(x17), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n141_), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x15), .O(new_n146_));
  inv1   g102(.a(x15), .O(new_n147_));
  nand4  g103(.a(new_n143_), .b(x16), .c(new_n147_), .d(x10), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n54_), .c(x31), .d(new_n109_), .O(new_n150_));
  nand2  g106(.a(x32), .b(x30), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n131_), .c(x29), .O(new_n153_));
  nand2  g109(.a(new_n131_), .b(x29), .O(new_n154_));
  nand4  g110(.a(new_n149_), .b(new_n54_), .c(x31), .d(x30), .O(new_n155_));
  nand2  g111(.a(x32), .b(new_n109_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g114(.a(new_n135_), .b(x14), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(x18), .c(x17), .d(x16), .O(new_n160_));
  nor2   g116(.a(new_n160_), .b(new_n147_), .O(new_n161_));
  nand2  g117(.a(x16), .b(x15), .O(new_n162_));
  nand2  g118(.a(x18), .b(x17), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n162_), .c(new_n135_), .O(new_n164_));
  nand2  g120(.a(x16), .b(x10), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n163_), .c(new_n147_), .O(new_n166_));
  inv1   g122(.a(x11), .O(new_n167_));
  oai21  g123(.a(new_n163_), .b(new_n167_), .c(new_n141_), .O(new_n168_));
  aoi21  g124(.a(x18), .b(x12), .c(x17), .O(new_n169_));
  nor2   g125(.a(x18), .b(x13), .O(new_n170_));
  nor3   g126(.a(new_n170_), .b(new_n169_), .c(new_n124_), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(new_n168_), .c(new_n166_), .d(new_n164_), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n161_), .c(x32), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n158_), .c(new_n153_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n46_), .c(x00), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n59_), .O(z8));
  nand4  g132(.a(new_n149_), .b(new_n49_), .c(x31), .d(x30), .O(new_n177_));
  nand2  g133(.a(x33), .b(new_n109_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nand3  g135(.a(new_n179_), .b(new_n131_), .c(x29), .O(new_n180_));
  nand4  g136(.a(new_n149_), .b(new_n49_), .c(new_n124_), .d(new_n109_), .O(new_n181_));
  nand2  g137(.a(x33), .b(x30), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n154_), .O(new_n184_));
  inv1   g140(.a(new_n169_), .O(new_n185_));
  inv1   g141(.a(new_n170_), .O(new_n186_));
  xnor2a g142(.a(x31), .b(x30), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n186_), .c(new_n185_), .O(new_n188_));
  inv1   g144(.a(new_n188_), .O(new_n189_));
  nand4  g145(.a(new_n189_), .b(new_n168_), .c(new_n166_), .d(new_n164_), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n161_), .c(x33), .O(new_n191_));
  nand3  g147(.a(new_n191_), .b(new_n184_), .c(new_n180_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n46_), .c(x00), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(new_n59_), .O(z9));
endmodule


