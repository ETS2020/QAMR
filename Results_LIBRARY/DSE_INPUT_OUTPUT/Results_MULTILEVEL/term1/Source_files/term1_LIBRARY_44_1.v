// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x11), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x01), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x08), .O(new_n57_));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  nor2   g014(.a(new_n49_), .b(new_n52_), .O(new_n59_));
  nor2   g015(.a(x03), .b(x02), .O(new_n60_));
  oai21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n52_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n52_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n49_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n45_), .c(new_n62_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n61_), .c(new_n58_), .O(new_n69_));
  xnor2a g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(x01), .O(new_n72_));
  nand2  g028(.a(new_n65_), .b(new_n63_), .O(new_n73_));
  nand3  g029(.a(new_n60_), .b(x06), .c(x05), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n45_), .c(new_n62_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n72_), .c(new_n70_), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n69_), .O(new_n78_));
  nor3   g034(.a(new_n78_), .b(x09), .c(new_n57_), .O(z2));
  inv1   g035(.a(new_n46_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  nor2   g039(.a(x22), .b(x17), .O(new_n84_));
  nor2   g040(.a(x23), .b(x18), .O(new_n85_));
  nor2   g041(.a(x24), .b(x19), .O(new_n86_));
  nor4   g042(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n59_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n62_), .c(new_n80_), .O(z3));
  inv1   g046(.a(x25), .O(new_n91_));
  xor2a  g047(.a(x28), .b(x27), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n88_), .c(new_n81_), .O(new_n93_));
  nor3   g049(.a(new_n93_), .b(new_n91_), .c(new_n62_), .O(z4));
  inv1   g050(.a(new_n59_), .O(new_n95_));
  inv1   g051(.a(new_n87_), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  inv1   g053(.a(x28), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n82_), .O(new_n99_));
  nand3  g055(.a(new_n97_), .b(x28), .c(x27), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n96_), .c(new_n95_), .d(new_n81_), .O(new_n102_));
  nor3   g058(.a(new_n102_), .b(new_n91_), .c(new_n62_), .O(z5));
  inv1   g059(.a(x30), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x28), .c(x27), .O(new_n105_));
  oai21  g061(.a(new_n99_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nor2   g062(.a(new_n104_), .b(x29), .O(new_n107_));
  aoi21  g063(.a(new_n106_), .b(x29), .c(new_n107_), .O(new_n108_));
  nor2   g064(.a(new_n108_), .b(new_n87_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n95_), .c(new_n81_), .d(x25), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n62_), .c(new_n80_), .O(z6));
  nand3  g067(.a(new_n99_), .b(x30), .c(x29), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x31), .O(new_n113_));
  inv1   g069(.a(new_n85_), .O(new_n114_));
  inv1   g070(.a(new_n86_), .O(new_n115_));
  nor2   g071(.a(new_n84_), .b(new_n83_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  nor2   g073(.a(new_n59_), .b(new_n91_), .O(new_n118_));
  inv1   g074(.a(x31), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x30), .c(x29), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n99_), .c(x26), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n118_), .c(new_n117_), .d(new_n113_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x01), .O(new_n124_));
  oai21  g080(.a(x11), .b(x01), .c(new_n124_), .O(z7));
  inv1   g081(.a(x00), .O(new_n126_));
  nand2  g082(.a(new_n98_), .b(new_n82_), .O(new_n127_));
  inv1   g083(.a(x16), .O(new_n128_));
  inv1   g084(.a(x32), .O(new_n129_));
  inv1   g085(.a(x17), .O(new_n130_));
  inv1   g086(.a(x18), .O(new_n131_));
  nand3  g087(.a(x19), .b(new_n131_), .c(x13), .O(new_n132_));
  inv1   g088(.a(x19), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x18), .c(x14), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n132_), .c(new_n130_), .O(new_n135_));
  nand4  g091(.a(x19), .b(x18), .c(new_n130_), .d(x12), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n135_), .c(x15), .O(new_n138_));
  inv1   g094(.a(x15), .O(new_n139_));
  nor2   g095(.a(new_n133_), .b(new_n131_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(x17), .c(new_n139_), .d(x10), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n129_), .c(x31), .d(new_n104_), .O(new_n143_));
  oai22  g099(.a(new_n143_), .b(new_n128_), .c(new_n129_), .d(new_n104_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n127_), .c(x29), .O(new_n145_));
  nand2  g101(.a(new_n127_), .b(x29), .O(new_n146_));
  nand4  g102(.a(new_n142_), .b(new_n129_), .c(x31), .d(x30), .O(new_n147_));
  nand2  g103(.a(x32), .b(new_n104_), .O(new_n148_));
  oai21  g104(.a(new_n147_), .b(new_n128_), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(x18), .b(x17), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n139_), .c(new_n133_), .O(new_n151_));
  inv1   g107(.a(x10), .O(new_n152_));
  oai21  g108(.a(new_n150_), .b(new_n152_), .c(new_n139_), .O(new_n153_));
  nand2  g109(.a(x18), .b(x12), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n130_), .O(new_n155_));
  inv1   g111(.a(x13), .O(new_n156_));
  aoi21  g112(.a(new_n131_), .b(new_n156_), .c(new_n119_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n155_), .c(new_n153_), .d(new_n151_), .O(new_n158_));
  and2   g114(.a(new_n158_), .b(x32), .O(new_n159_));
  aoi21  g115(.a(new_n149_), .b(new_n146_), .c(new_n159_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n145_), .c(new_n46_), .O(new_n161_));
  nand2  g117(.a(new_n133_), .b(x14), .O(new_n162_));
  oai21  g118(.a(new_n128_), .b(new_n62_), .c(x11), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n162_), .c(x18), .d(x17), .O(new_n164_));
  nand3  g120(.a(new_n140_), .b(x17), .c(x15), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(x01), .c(new_n45_), .O(new_n166_));
  oai22  g122(.a(new_n166_), .b(x16), .c(new_n164_), .d(new_n139_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(x32), .O(new_n168_));
  nand3  g124(.a(new_n127_), .b(new_n104_), .c(x29), .O(new_n169_));
  nand2  g125(.a(new_n146_), .b(x30), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n169_), .c(x32), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(x31), .c(x19), .d(x18), .O(new_n172_));
  nor2   g128(.a(new_n172_), .b(new_n130_), .O(new_n173_));
  nand4  g129(.a(new_n173_), .b(new_n128_), .c(x15), .d(x11), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n62_), .c(new_n168_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n161_), .c(new_n81_), .O(new_n176_));
  nor2   g132(.a(new_n176_), .b(new_n126_), .O(z8));
  aoi21  g133(.a(new_n141_), .b(new_n138_), .c(x33), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(x31), .c(x30), .d(x16), .O(new_n179_));
  nand2  g135(.a(x33), .b(new_n104_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nand3  g137(.a(new_n181_), .b(new_n127_), .c(x29), .O(new_n182_));
  nand4  g138(.a(new_n178_), .b(new_n119_), .c(new_n104_), .d(x16), .O(new_n183_));
  nand2  g139(.a(x33), .b(x30), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n183_), .O(new_n185_));
  inv1   g141(.a(x33), .O(new_n186_));
  nand2  g142(.a(new_n131_), .b(new_n156_), .O(new_n187_));
  xnor2a g143(.a(x31), .b(x30), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(new_n187_), .c(new_n155_), .d(new_n153_), .O(new_n189_));
  inv1   g145(.a(new_n189_), .O(new_n190_));
  aoi21  g146(.a(new_n190_), .b(new_n151_), .c(new_n186_), .O(new_n191_));
  aoi21  g147(.a(new_n185_), .b(new_n146_), .c(new_n191_), .O(new_n192_));
  aoi21  g148(.a(new_n192_), .b(new_n182_), .c(new_n46_), .O(new_n193_));
  nand2  g149(.a(new_n167_), .b(x33), .O(new_n194_));
  nand4  g150(.a(new_n127_), .b(x31), .c(x30), .d(x29), .O(new_n195_));
  nand3  g151(.a(new_n146_), .b(new_n119_), .c(new_n104_), .O(new_n196_));
  aoi21  g152(.a(new_n196_), .b(new_n195_), .c(x33), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(x19), .c(x18), .d(x17), .O(new_n198_));
  nor2   g154(.a(new_n198_), .b(x16), .O(new_n199_));
  nand4  g155(.a(new_n199_), .b(x15), .c(x11), .d(x01), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(new_n194_), .O(new_n201_));
  oai21  g157(.a(new_n201_), .b(new_n193_), .c(new_n81_), .O(new_n202_));
  nor2   g158(.a(new_n202_), .b(new_n126_), .O(z9));
endmodule


