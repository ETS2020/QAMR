// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x06), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x08), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(x06), .O(new_n60_));
  inv1   g016(.a(x05), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(x03), .c(new_n52_), .O(new_n62_));
  nor2   g018(.a(new_n61_), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x02), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  nand3  g021(.a(new_n63_), .b(new_n45_), .c(new_n60_), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n65_), .c(new_n59_), .O(new_n68_));
  inv1   g024(.a(new_n46_), .O(new_n69_));
  nand2  g025(.a(x03), .b(x02), .O(new_n70_));
  nand2  g026(.a(new_n49_), .b(new_n52_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(new_n69_), .c(x01), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  xnor2a g030(.a(x07), .b(x04), .O(new_n75_));
  nand2  g031(.a(new_n45_), .b(new_n60_), .O(new_n76_));
  nand2  g032(.a(x06), .b(x02), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n76_), .c(new_n63_), .O(new_n78_));
  nand4  g034(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n78_), .c(new_n59_), .O(new_n81_));
  xor2a  g037(.a(x03), .b(x02), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n69_), .c(x01), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n81_), .c(new_n75_), .O(new_n84_));
  aoi21  g040(.a(new_n74_), .b(new_n58_), .c(new_n84_), .O(new_n85_));
  nor3   g041(.a(new_n85_), .b(x09), .c(new_n57_), .O(z2));
  inv1   g042(.a(x26), .O(new_n87_));
  inv1   g043(.a(x27), .O(new_n88_));
  nor2   g044(.a(x20), .b(x15), .O(new_n89_));
  nor2   g045(.a(x21), .b(x16), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g047(.a(x17), .O(new_n92_));
  inv1   g048(.a(x22), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  inv1   g050(.a(x18), .O(new_n95_));
  inv1   g051(.a(x23), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g053(.a(x19), .O(new_n98_));
  inv1   g054(.a(x24), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n97_), .c(new_n94_), .d(new_n91_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n70_), .c(new_n69_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n88_), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n87_), .c(x25), .d(x01), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z3));
  xor2a  g063(.a(x28), .b(x27), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n103_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n87_), .c(x25), .d(x01), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z4));
  inv1   g068(.a(x29), .O(new_n113_));
  inv1   g069(.a(x28), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n88_), .O(new_n115_));
  nand3  g071(.a(new_n113_), .b(x28), .c(x27), .O(new_n116_));
  oai21  g072(.a(new_n115_), .b(new_n113_), .c(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n101_), .c(new_n70_), .d(new_n87_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x25), .c(x01), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n69_), .O(z5));
  inv1   g077(.a(new_n101_), .O(new_n122_));
  nand3  g078(.a(new_n45_), .b(x28), .c(x27), .O(new_n123_));
  oai21  g079(.a(new_n115_), .b(new_n45_), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x29), .O(new_n125_));
  nand2  g081(.a(x30), .b(new_n113_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n125_), .c(new_n122_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n70_), .c(new_n87_), .d(x25), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n59_), .c(new_n69_), .O(z6));
  oai21  g085(.a(x31), .b(x30), .c(new_n60_), .O(new_n130_));
  nand3  g086(.a(new_n115_), .b(x30), .c(x29), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x31), .O(new_n132_));
  nand4  g088(.a(new_n100_), .b(new_n97_), .c(new_n94_), .d(new_n91_), .O(new_n133_));
  aoi21  g089(.a(x03), .b(x02), .c(new_n59_), .O(new_n134_));
  inv1   g090(.a(x31), .O(new_n135_));
  nand4  g091(.a(new_n115_), .b(new_n135_), .c(x30), .d(x29), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n134_), .c(new_n87_), .d(x25), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n133_), .c(new_n132_), .d(new_n130_), .O(z7));
  inv1   g095(.a(x15), .O(new_n140_));
  nand2  g096(.a(new_n98_), .b(x14), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(x18), .c(x17), .d(x16), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g099(.a(x16), .b(x15), .O(new_n144_));
  nand2  g100(.a(x18), .b(x17), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(new_n98_), .O(new_n146_));
  nand2  g102(.a(x16), .b(x10), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n145_), .c(new_n140_), .O(new_n148_));
  inv1   g104(.a(x11), .O(new_n149_));
  inv1   g105(.a(x16), .O(new_n150_));
  oai21  g106(.a(new_n145_), .b(new_n149_), .c(new_n150_), .O(new_n151_));
  aoi21  g107(.a(x18), .b(x12), .c(x17), .O(new_n152_));
  nor2   g108(.a(x18), .b(x13), .O(new_n153_));
  nor3   g109(.a(new_n153_), .b(new_n152_), .c(new_n135_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n151_), .c(new_n148_), .d(new_n146_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n143_), .c(new_n69_), .O(new_n156_));
  nand2  g112(.a(new_n114_), .b(new_n88_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x30), .O(new_n158_));
  nor2   g114(.a(new_n158_), .b(new_n113_), .O(new_n159_));
  nand2  g115(.a(new_n157_), .b(x29), .O(new_n160_));
  aoi22  g116(.a(new_n160_), .b(new_n45_), .c(new_n159_), .d(x06), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n156_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x32), .O(new_n163_));
  inv1   g119(.a(x32), .O(new_n164_));
  nand3  g120(.a(new_n160_), .b(x30), .c(x06), .O(new_n165_));
  nand3  g121(.a(new_n157_), .b(new_n45_), .c(x29), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand3  g123(.a(x19), .b(new_n95_), .c(x13), .O(new_n168_));
  nand3  g124(.a(new_n98_), .b(x18), .c(x14), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n168_), .c(new_n92_), .O(new_n170_));
  nand4  g126(.a(x19), .b(x18), .c(new_n92_), .d(x12), .O(new_n171_));
  inv1   g127(.a(new_n171_), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n170_), .c(x16), .O(new_n173_));
  nand3  g129(.a(x19), .b(x18), .c(x17), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(new_n175_));
  nand3  g131(.a(new_n175_), .b(new_n150_), .c(x11), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n173_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(x15), .O(new_n178_));
  nand4  g134(.a(new_n175_), .b(x16), .c(new_n140_), .d(x10), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n178_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n167_), .c(new_n164_), .d(x31), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n163_), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n87_), .c(x00), .O(new_n183_));
  inv1   g139(.a(new_n183_), .O(z8));
  nor2   g140(.a(new_n153_), .b(new_n152_), .O(new_n185_));
  nand4  g141(.a(new_n185_), .b(new_n151_), .c(new_n148_), .d(new_n146_), .O(new_n186_));
  oai21  g142(.a(new_n186_), .b(new_n143_), .c(new_n69_), .O(new_n187_));
  nand2  g143(.a(new_n160_), .b(new_n135_), .O(new_n188_));
  nor2   g144(.a(new_n135_), .b(new_n113_), .O(new_n189_));
  aoi21  g145(.a(new_n189_), .b(new_n157_), .c(new_n45_), .O(new_n190_));
  aoi22  g146(.a(new_n190_), .b(x06), .c(new_n188_), .d(new_n45_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n187_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(x33), .O(new_n193_));
  inv1   g149(.a(x33), .O(new_n194_));
  nand4  g150(.a(new_n157_), .b(x31), .c(x30), .d(x29), .O(new_n195_));
  oai22  g151(.a(new_n195_), .b(new_n60_), .c(new_n188_), .d(x30), .O(new_n196_));
  nand3  g152(.a(new_n196_), .b(new_n180_), .c(new_n194_), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(new_n193_), .O(new_n198_));
  nand3  g154(.a(new_n198_), .b(new_n87_), .c(x00), .O(new_n199_));
  inv1   g155(.a(new_n199_), .O(z9));
endmodule


