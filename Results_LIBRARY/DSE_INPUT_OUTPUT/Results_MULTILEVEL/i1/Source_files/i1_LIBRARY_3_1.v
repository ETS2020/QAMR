// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n80_, new_n81_, new_n83_,
    new_n86_, new_n88_, new_n89_, new_n91_, new_n93_, new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x07), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x07), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  inv1   g08(.a(x03), .O(new_n50_));
  inv1   g09(.a(x05), .O(new_n51_));
  inv1   g10(.a(x06), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(x19), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x04), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n43_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  nand2  g17(.a(x19), .b(new_n42_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g19(.a(x19), .O(new_n61_));
  nor3   g20(.a(x03), .b(x02), .c(x01), .O(new_n62_));
  nor3   g21(.a(x06), .b(x05), .c(x04), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n62_), .c(x20), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(x07), .c(x00), .O(new_n65_));
  nor2   g24(.a(x05), .b(x04), .O(new_n66_));
  nand4  g25(.a(new_n66_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(new_n68_));
  nor2   g27(.a(x07), .b(x06), .O(new_n69_));
  nor2   g28(.a(x20), .b(x09), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(x08), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n65_), .c(new_n61_), .O(z02));
  inv1   g31(.a(x21), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n43_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  inv1   g34(.a(x08), .O(new_n76_));
  nand4  g35(.a(new_n69_), .b(new_n43_), .c(x19), .d(new_n76_), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n67_), .c(new_n44_), .d(new_n75_), .O(z05));
  nand2  g37(.a(new_n45_), .b(new_n61_), .O(z06));
  inv1   g38(.a(x18), .O(new_n80_));
  inv1   g39(.a(x24), .O(new_n81_));
  nor3   g40(.a(new_n44_), .b(new_n81_), .c(new_n80_), .O(z07));
  inv1   g41(.a(x11), .O(new_n83_));
  nand2  g42(.a(new_n45_), .b(new_n83_), .O(z08));
  oai21  g43(.a(new_n81_), .b(new_n83_), .c(new_n45_), .O(z09));
  nand3  g44(.a(new_n81_), .b(x22), .c(x14), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n45_), .O(z10));
  nor2   g46(.a(new_n44_), .b(x24), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(x22), .c(x17), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z11));
  nand3  g49(.a(new_n81_), .b(x23), .c(x14), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n45_), .O(z12));
  nand3  g51(.a(new_n88_), .b(x23), .c(x17), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z13));
  and2   g53(.a(new_n88_), .b(x16), .O(z14));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  nor3   g55(.a(new_n44_), .b(x13), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(z15));
  buf    g57(.a(x20), .O(z03));
endmodule


