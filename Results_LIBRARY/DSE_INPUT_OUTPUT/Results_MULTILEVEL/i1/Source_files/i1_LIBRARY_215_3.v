// Benchmark "FAU" written by ABC on Thu Aug 13 20:31:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x19), .O(new_n42_));
  inv1   g01(.a(x07), .O(new_n43_));
  inv1   g02(.a(x09), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x04), .O(new_n46_));
  inv1   g05(.a(x05), .O(new_n47_));
  nor3   g06(.a(x02), .b(x01), .c(x00), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x06), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n44_), .c(x08), .d(new_n43_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n42_), .O(z02));
  inv1   g11(.a(x20), .O(new_n53_));
  nand2  g12(.a(x19), .b(x00), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n53_), .O(z03));
  or2    g14(.a(z03), .b(x21), .O(z04));
  nor2   g15(.a(x02), .b(x01), .O(new_n57_));
  nand3  g16(.a(new_n57_), .b(new_n46_), .c(new_n45_), .O(new_n58_));
  inv1   g17(.a(x08), .O(new_n59_));
  nor2   g18(.a(x06), .b(x05), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(x19), .c(new_n59_), .d(new_n43_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(x10), .O(new_n63_));
  nand2  g22(.a(new_n42_), .b(x10), .O(new_n64_));
  oai21  g23(.a(new_n63_), .b(x00), .c(new_n64_), .O(z05));
  nand3  g24(.a(new_n54_), .b(x24), .c(x18), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z07));
  and2   g26(.a(new_n54_), .b(x11), .O(z08));
  nand3  g27(.a(new_n54_), .b(x24), .c(x11), .O(new_n69_));
  inv1   g28(.a(new_n69_), .O(z09));
  inv1   g29(.a(x14), .O(new_n71_));
  inv1   g30(.a(x24), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x22), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n71_), .c(new_n54_), .O(z10));
  inv1   g33(.a(x17), .O(new_n75_));
  oai21  g34(.a(new_n73_), .b(new_n75_), .c(new_n54_), .O(z11));
  nand4  g35(.a(new_n54_), .b(new_n72_), .c(x23), .d(x14), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  nand3  g37(.a(new_n72_), .b(x23), .c(x17), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n54_), .O(z13));
  nand3  g39(.a(new_n54_), .b(new_n72_), .c(x16), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z14));
  inv1   g41(.a(x12), .O(new_n83_));
  inv1   g42(.a(x13), .O(new_n84_));
  nor2   g43(.a(x15), .b(x14), .O(new_n85_));
  nand4  g44(.a(new_n85_), .b(new_n54_), .c(new_n84_), .d(new_n83_), .O(z15));
  buf    g45(.a(x00), .O(z00));
  buf    g46(.a(x19), .O(z01));
  buf    g47(.a(x19), .O(z06));
endmodule


