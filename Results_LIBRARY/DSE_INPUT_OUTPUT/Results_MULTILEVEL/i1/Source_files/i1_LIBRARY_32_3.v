// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n84_, new_n86_, new_n88_,
    new_n90_, new_n92_, new_n93_;
  nand2  g00(.a(x19), .b(x09), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x09), .O(new_n44_));
  nor2   g03(.a(x02), .b(x01), .O(new_n45_));
  nor2   g04(.a(x04), .b(x03), .O(new_n46_));
  nor2   g05(.a(x06), .b(x05), .O(new_n47_));
  nor2   g06(.a(x08), .b(x07), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(new_n44_), .c(x00), .O(new_n50_));
  and2   g09(.a(new_n50_), .b(x19), .O(z01));
  inv1   g10(.a(x01), .O(new_n52_));
  nor2   g11(.a(x03), .b(x02), .O(new_n53_));
  nor2   g12(.a(x05), .b(x04), .O(new_n54_));
  nor2   g13(.a(x07), .b(x06), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g16(.a(x08), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x07), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g20(.a(new_n61_), .b(x19), .c(new_n44_), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nand2  g23(.a(new_n42_), .b(new_n64_), .O(z03));
  or2    g24(.a(z03), .b(x21), .O(z04));
  nand2  g25(.a(new_n46_), .b(new_n45_), .O(new_n67_));
  inv1   g26(.a(x07), .O(new_n68_));
  nand4  g27(.a(new_n47_), .b(x19), .c(new_n58_), .d(new_n68_), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(x10), .c(new_n44_), .O(new_n71_));
  inv1   g30(.a(x19), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x10), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n71_), .O(z05));
  nand2  g33(.a(x24), .b(x18), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n42_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  inv1   g36(.a(new_n42_), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n77_), .O(z08));
  inv1   g38(.a(x24), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n77_), .c(new_n42_), .O(z09));
  nand3  g40(.a(new_n80_), .b(x22), .c(x14), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n42_), .O(z10));
  nand4  g42(.a(new_n42_), .b(new_n80_), .c(x22), .d(x17), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z11));
  nand4  g44(.a(new_n42_), .b(new_n80_), .c(x23), .d(x14), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z12));
  nand3  g46(.a(new_n80_), .b(x23), .c(x17), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n42_), .O(z13));
  inv1   g48(.a(x16), .O(new_n90_));
  oai21  g49(.a(x24), .b(new_n90_), .c(new_n42_), .O(z14));
  nor2   g50(.a(x13), .b(x12), .O(new_n92_));
  nor2   g51(.a(x15), .b(x14), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n78_), .O(z15));
  buf    g53(.a(x19), .O(z06));
endmodule


