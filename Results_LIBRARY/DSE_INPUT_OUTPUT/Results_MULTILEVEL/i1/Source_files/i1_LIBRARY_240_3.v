// Benchmark "FAU" written by ABC on Thu Aug 13 20:31:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n95_, new_n97_, new_n98_;
  nand2  g00(.a(x20), .b(x15), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x01), .O(new_n44_));
  inv1   g03(.a(x02), .O(new_n45_));
  inv1   g04(.a(x03), .O(new_n46_));
  inv1   g05(.a(x05), .O(new_n47_));
  inv1   g06(.a(x06), .O(new_n48_));
  inv1   g07(.a(x07), .O(new_n49_));
  xnor2a g08(.a(x09), .b(x08), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x04), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x00), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x19), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n42_), .O(z01));
  inv1   g15(.a(x19), .O(new_n57_));
  inv1   g16(.a(x04), .O(new_n58_));
  nor3   g17(.a(x03), .b(x02), .c(x01), .O(new_n59_));
  nor2   g18(.a(x07), .b(x06), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n59_), .c(new_n47_), .d(new_n58_), .O(new_n61_));
  nand4  g20(.a(new_n58_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(new_n63_));
  inv1   g22(.a(x08), .O(new_n64_));
  nand2  g23(.a(new_n48_), .b(new_n47_), .O(new_n65_));
  nor4   g24(.a(new_n65_), .b(x09), .c(new_n64_), .d(x07), .O(new_n66_));
  aoi22  g25(.a(new_n66_), .b(new_n63_), .c(new_n61_), .d(x00), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n57_), .c(new_n42_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(x15), .O(z03));
  nand2  g29(.a(x21), .b(new_n69_), .O(new_n71_));
  oai21  g30(.a(new_n69_), .b(x15), .c(new_n71_), .O(z04));
  inv1   g31(.a(new_n42_), .O(new_n73_));
  nor4   g32(.a(new_n65_), .b(new_n57_), .c(x08), .d(x07), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n63_), .c(x10), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n73_), .O(z05));
  nor2   g35(.a(new_n73_), .b(new_n57_), .O(z06));
  nand2  g36(.a(x24), .b(x18), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n42_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n42_), .b(new_n80_), .O(z08));
  nand3  g40(.a(new_n42_), .b(x24), .c(x11), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z09));
  inv1   g42(.a(x24), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x22), .c(x14), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n42_), .O(z10));
  nand4  g45(.a(new_n42_), .b(new_n84_), .c(x22), .d(x17), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z11));
  nand3  g47(.a(new_n42_), .b(new_n84_), .c(x23), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x14), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z12));
  nand2  g51(.a(new_n90_), .b(x17), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z13));
  inv1   g53(.a(x16), .O(new_n95_));
  oai21  g54(.a(x24), .b(new_n95_), .c(new_n42_), .O(z14));
  nor3   g55(.a(x14), .b(x13), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n69_), .b(x15), .O(new_n98_));
  oai21  g57(.a(new_n97_), .b(x15), .c(new_n98_), .O(z15));
endmodule


