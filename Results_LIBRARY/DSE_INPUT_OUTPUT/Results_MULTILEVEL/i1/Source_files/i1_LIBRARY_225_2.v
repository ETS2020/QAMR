// Benchmark "FAU" written by ABC on Thu Aug 13 20:31:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n84_,
    new_n86_, new_n88_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_;
  and2   g00(.a(x19), .b(x00), .O(z00));
  inv1   g01(.a(x01), .O(new_n43_));
  inv1   g02(.a(x02), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x05), .O(new_n46_));
  inv1   g05(.a(x06), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x04), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x19), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x00), .O(new_n54_));
  inv1   g13(.a(x00), .O(new_n55_));
  nand2  g14(.a(x19), .b(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n54_), .O(z01));
  nor2   g16(.a(x02), .b(x01), .O(new_n58_));
  nor2   g17(.a(x04), .b(x03), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g19(.a(x06), .b(x05), .O(new_n61_));
  nand3  g20(.a(new_n61_), .b(x19), .c(new_n48_), .O(new_n62_));
  oai21  g21(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  nor3   g22(.a(x05), .b(x04), .c(x03), .O(new_n64_));
  inv1   g23(.a(x09), .O(new_n65_));
  nand3  g24(.a(x19), .b(new_n65_), .c(x08), .O(new_n66_));
  nor3   g25(.a(new_n66_), .b(x07), .c(x06), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(new_n64_), .c(new_n58_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n63_), .O(z02));
  inv1   g28(.a(x20), .O(new_n70_));
  nor2   g29(.a(x19), .b(new_n55_), .O(new_n71_));
  nor2   g30(.a(new_n71_), .b(new_n70_), .O(z03));
  nor2   g31(.a(x21), .b(x20), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n71_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  inv1   g34(.a(x08), .O(new_n76_));
  nand4  g35(.a(new_n61_), .b(x19), .c(new_n76_), .d(new_n48_), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n60_), .c(new_n71_), .d(new_n75_), .O(z05));
  inv1   g37(.a(new_n71_), .O(new_n79_));
  nand2  g38(.a(x24), .b(x18), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(z07));
  inv1   g40(.a(x11), .O(new_n82_));
  nor2   g41(.a(new_n71_), .b(new_n82_), .O(z08));
  inv1   g42(.a(x24), .O(new_n84_));
  nor3   g43(.a(new_n71_), .b(new_n84_), .c(new_n82_), .O(z09));
  nand3  g44(.a(new_n84_), .b(x22), .c(x14), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n79_), .O(z10));
  nand4  g46(.a(new_n79_), .b(new_n84_), .c(x22), .d(x17), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z11));
  nand4  g48(.a(new_n79_), .b(new_n84_), .c(x23), .d(x14), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z12));
  nand3  g50(.a(new_n84_), .b(x23), .c(x17), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n79_), .O(z13));
  nand2  g52(.a(new_n84_), .b(x16), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n79_), .O(z14));
  nor2   g54(.a(x13), .b(x12), .O(new_n96_));
  nor2   g55(.a(x15), .b(x14), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n71_), .O(z15));
  buf    g57(.a(x19), .O(z06));
endmodule


