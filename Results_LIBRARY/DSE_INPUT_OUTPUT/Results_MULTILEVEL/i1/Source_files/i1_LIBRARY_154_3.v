// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n71_, new_n73_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n83_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n92_;
  nand2  g00(.a(x19), .b(x09), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x19), .O(new_n44_));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x04), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  inv1   g09(.a(x05), .O(new_n51_));
  inv1   g10(.a(x06), .O(new_n52_));
  inv1   g11(.a(x07), .O(new_n53_));
  inv1   g12(.a(x08), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(new_n50_), .c(x09), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x00), .c(new_n44_), .O(z01));
  nor2   g17(.a(x03), .b(x02), .O(new_n59_));
  nor2   g18(.a(x05), .b(x04), .O(new_n60_));
  nor2   g19(.a(x07), .b(x06), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n45_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x00), .O(new_n63_));
  nand4  g22(.a(x08), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n50_), .c(x09), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n63_), .c(new_n44_), .O(z02));
  inv1   g26(.a(x20), .O(new_n68_));
  nand2  g27(.a(new_n42_), .b(new_n68_), .O(z03));
  or2    g28(.a(z03), .b(x21), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  oai21  g30(.a(new_n57_), .b(new_n44_), .c(new_n71_), .O(z05));
  nand3  g31(.a(new_n42_), .b(x24), .c(x18), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z07));
  and2   g33(.a(new_n42_), .b(x11), .O(z08));
  nand3  g34(.a(new_n42_), .b(x24), .c(x11), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z09));
  inv1   g36(.a(x24), .O(new_n78_));
  nand4  g37(.a(new_n42_), .b(new_n78_), .c(x22), .d(x14), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z10));
  nand3  g39(.a(new_n78_), .b(x22), .c(x17), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n42_), .O(z11));
  inv1   g41(.a(x14), .O(new_n83_));
  nand2  g42(.a(new_n78_), .b(x23), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n83_), .c(new_n42_), .O(z12));
  inv1   g44(.a(x17), .O(new_n86_));
  oai21  g45(.a(new_n84_), .b(new_n86_), .c(new_n42_), .O(z13));
  nand3  g46(.a(new_n42_), .b(new_n78_), .c(x16), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  inv1   g48(.a(x12), .O(new_n90_));
  inv1   g49(.a(x13), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  nand4  g51(.a(new_n92_), .b(new_n42_), .c(new_n91_), .d(new_n90_), .O(z15));
  buf    g52(.a(x19), .O(z06));
endmodule


