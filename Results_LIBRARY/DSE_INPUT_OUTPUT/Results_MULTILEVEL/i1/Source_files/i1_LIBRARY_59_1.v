// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n68_, new_n69_, new_n71_, new_n74_, new_n75_, new_n77_,
    new_n80_, new_n81_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_;
  inv1   g00(.a(x08), .O(new_n42_));
  nand2  g01(.a(x19), .b(new_n42_), .O(new_n43_));
  and2   g02(.a(new_n43_), .b(x00), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  nor2   g04(.a(x02), .b(x01), .O(new_n46_));
  nor2   g05(.a(x04), .b(x03), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  inv1   g08(.a(x07), .O(new_n50_));
  nor2   g09(.a(x06), .b(x05), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n49_), .c(new_n42_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n45_), .O(z01));
  inv1   g14(.a(x01), .O(new_n56_));
  nor2   g15(.a(x03), .b(x02), .O(new_n57_));
  nor2   g16(.a(x05), .b(x04), .O(new_n58_));
  nor2   g17(.a(x07), .b(x06), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x00), .O(new_n61_));
  nor2   g20(.a(x09), .b(x07), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n51_), .c(new_n47_), .d(new_n46_), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand3  g23(.a(new_n64_), .b(x19), .c(x08), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z02));
  and2   g25(.a(new_n43_), .b(x20), .O(z03));
  inv1   g26(.a(new_n43_), .O(new_n68_));
  nor2   g27(.a(x21), .b(x20), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n68_), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  nand2  g30(.a(new_n43_), .b(new_n71_), .O(z05));
  nor2   g31(.a(new_n45_), .b(new_n42_), .O(z06));
  inv1   g32(.a(x18), .O(new_n74_));
  nand2  g33(.a(new_n43_), .b(x24), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n74_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nand2  g36(.a(new_n43_), .b(new_n77_), .O(z08));
  nor2   g37(.a(new_n75_), .b(new_n77_), .O(z09));
  inv1   g38(.a(x24), .O(new_n80_));
  nand4  g39(.a(new_n43_), .b(new_n80_), .c(x22), .d(x14), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z10));
  nand3  g41(.a(new_n80_), .b(x22), .c(x17), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n43_), .O(z11));
  inv1   g43(.a(x23), .O(new_n85_));
  nand2  g44(.a(new_n43_), .b(new_n80_), .O(new_n86_));
  nor2   g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x14), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z12));
  nand2  g48(.a(new_n87_), .b(x17), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z13));
  inv1   g50(.a(x16), .O(new_n92_));
  nor2   g51(.a(new_n86_), .b(new_n92_), .O(z14));
  nor2   g52(.a(x13), .b(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(x14), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n68_), .O(z15));
endmodule


