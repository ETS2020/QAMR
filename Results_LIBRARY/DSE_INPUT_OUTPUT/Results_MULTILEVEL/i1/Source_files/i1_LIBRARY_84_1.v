// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n59_, new_n60_, new_n62_, new_n65_,
    new_n66_, new_n68_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x03), .O(new_n43_));
  nand2  g02(.a(x19), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand3  g04(.a(x19), .b(x03), .c(new_n42_), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(z01));
  nand3  g06(.a(x19), .b(x03), .c(x00), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z02));
  inv1   g08(.a(x20), .O(new_n50_));
  inv1   g09(.a(x19), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x03), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n50_), .O(z03));
  nor2   g12(.a(x21), .b(x20), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(new_n52_), .O(z04));
  inv1   g14(.a(x10), .O(new_n56_));
  nor2   g15(.a(new_n52_), .b(new_n56_), .O(z05));
  nor2   g16(.a(new_n51_), .b(new_n43_), .O(z06));
  inv1   g17(.a(x18), .O(new_n59_));
  oai21  g18(.a(new_n51_), .b(x03), .c(x24), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n59_), .O(z07));
  inv1   g20(.a(x11), .O(new_n62_));
  nor2   g21(.a(new_n52_), .b(new_n62_), .O(z08));
  nor2   g22(.a(new_n60_), .b(new_n62_), .O(z09));
  inv1   g23(.a(x24), .O(new_n65_));
  nand4  g24(.a(new_n44_), .b(new_n65_), .c(x22), .d(x14), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z10));
  nand4  g26(.a(new_n44_), .b(new_n65_), .c(x22), .d(x17), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z11));
  nand4  g28(.a(new_n44_), .b(new_n65_), .c(x23), .d(x14), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z12));
  nand3  g30(.a(new_n65_), .b(x23), .c(x17), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n44_), .O(z13));
  nand2  g32(.a(new_n65_), .b(x16), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n44_), .O(z14));
  inv1   g34(.a(x12), .O(new_n76_));
  inv1   g35(.a(x13), .O(new_n77_));
  nor2   g36(.a(x15), .b(x14), .O(new_n78_));
  nand4  g37(.a(new_n78_), .b(new_n44_), .c(new_n77_), .d(new_n76_), .O(z15));
endmodule


