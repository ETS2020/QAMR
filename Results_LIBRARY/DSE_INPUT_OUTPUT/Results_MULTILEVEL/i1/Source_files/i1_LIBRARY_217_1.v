// Benchmark "FAU" written by ABC on Thu Aug 13 20:31:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n77_, new_n78_, new_n80_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x09), .O(new_n43_));
  nand2  g02(.a(x19), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(x01), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x04), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  inv1   g11(.a(x05), .O(new_n53_));
  inv1   g12(.a(x06), .O(new_n54_));
  inv1   g13(.a(x07), .O(new_n55_));
  nand4  g14(.a(x08), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n52_), .c(new_n43_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x00), .c(new_n46_), .O(z01));
  nor3   g18(.a(x03), .b(x02), .c(x01), .O(new_n60_));
  nor2   g19(.a(x07), .b(x06), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n60_), .c(new_n53_), .d(new_n50_), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(x19), .c(x09), .d(x00), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z03));
  or2    g25(.a(z03), .b(x21), .O(z04));
  inv1   g26(.a(x10), .O(new_n68_));
  inv1   g27(.a(x08), .O(new_n69_));
  nor2   g28(.a(x06), .b(x05), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(x19), .c(new_n69_), .d(new_n55_), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n51_), .c(new_n68_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g32(.a(new_n46_), .b(x10), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(z05));
  nor2   g34(.a(new_n46_), .b(new_n43_), .O(z06));
  inv1   g35(.a(x18), .O(new_n77_));
  nand2  g36(.a(new_n44_), .b(x24), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n77_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n44_), .b(new_n80_), .O(z08));
  nor2   g40(.a(new_n78_), .b(new_n80_), .O(z09));
  inv1   g41(.a(x14), .O(new_n83_));
  inv1   g42(.a(x24), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x22), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n83_), .c(new_n44_), .O(z10));
  inv1   g45(.a(x17), .O(new_n87_));
  oai21  g46(.a(new_n85_), .b(new_n87_), .c(new_n44_), .O(z11));
  nand4  g47(.a(new_n44_), .b(new_n84_), .c(x23), .d(x14), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z12));
  nand3  g49(.a(new_n84_), .b(x23), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n44_), .O(z13));
  inv1   g51(.a(x16), .O(new_n93_));
  nand2  g52(.a(new_n44_), .b(new_n84_), .O(new_n94_));
  nor2   g53(.a(new_n94_), .b(new_n93_), .O(z14));
  inv1   g54(.a(x12), .O(new_n96_));
  inv1   g55(.a(x13), .O(new_n97_));
  nor2   g56(.a(x15), .b(x14), .O(new_n98_));
  nand4  g57(.a(new_n98_), .b(new_n44_), .c(new_n97_), .d(new_n96_), .O(z15));
endmodule


