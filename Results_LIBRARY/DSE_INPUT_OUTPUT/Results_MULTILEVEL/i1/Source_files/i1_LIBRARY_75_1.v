// Benchmark "FAU" written by ABC on Thu Aug 13 20:30:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_, new_n95_,
    new_n97_, new_n99_, new_n101_, new_n102_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x14), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x06), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x06), .O(new_n46_));
  inv1   g05(.a(x01), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x05), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(x19), .c(new_n51_), .d(new_n50_), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x04), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n43_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  nand2  g16(.a(x19), .b(new_n42_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n57_), .O(z01));
  nor2   g18(.a(x02), .b(x01), .O(new_n60_));
  nor2   g19(.a(x04), .b(x03), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g21(.a(x19), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(x09), .O(new_n64_));
  nand4  g23(.a(new_n64_), .b(x08), .c(new_n51_), .d(new_n50_), .O(new_n65_));
  oai21  g24(.a(new_n65_), .b(new_n62_), .c(new_n43_), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n46_), .O(new_n67_));
  nor2   g26(.a(x03), .b(x02), .O(new_n68_));
  nor2   g27(.a(x05), .b(x04), .O(new_n69_));
  nor2   g28(.a(x07), .b(x06), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n47_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(x19), .c(x00), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n67_), .O(z02));
  inv1   g32(.a(x20), .O(new_n74_));
  inv1   g33(.a(new_n44_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(z03));
  nor2   g35(.a(x21), .b(x20), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n44_), .O(z04));
  inv1   g37(.a(x10), .O(new_n79_));
  nand3  g38(.a(new_n69_), .b(new_n60_), .c(new_n49_), .O(new_n80_));
  inv1   g39(.a(x08), .O(new_n81_));
  nand4  g40(.a(new_n70_), .b(x19), .c(new_n43_), .d(new_n81_), .O(new_n82_));
  oai22  g41(.a(new_n82_), .b(new_n80_), .c(new_n44_), .d(new_n79_), .O(z05));
  nand2  g42(.a(new_n75_), .b(new_n63_), .O(z06));
  nand2  g43(.a(x24), .b(x18), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n75_), .O(z07));
  inv1   g45(.a(x11), .O(new_n87_));
  nand2  g46(.a(new_n75_), .b(new_n87_), .O(z08));
  inv1   g47(.a(x24), .O(new_n89_));
  nor3   g48(.a(new_n44_), .b(new_n89_), .c(new_n87_), .O(z09));
  nand3  g49(.a(x22), .b(x14), .c(x06), .O(new_n91_));
  nor2   g50(.a(new_n91_), .b(x24), .O(z10));
  nand3  g51(.a(new_n89_), .b(x22), .c(x17), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n75_), .O(z11));
  nand2  g53(.a(new_n89_), .b(x23), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(x06), .c(new_n43_), .O(z12));
  nand4  g55(.a(new_n75_), .b(new_n89_), .c(x23), .d(x17), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z13));
  inv1   g57(.a(x16), .O(new_n99_));
  nor3   g58(.a(new_n44_), .b(x24), .c(new_n99_), .O(z14));
  nor2   g59(.a(x15), .b(x14), .O(new_n101_));
  nor2   g60(.a(x13), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(z15));
endmodule


