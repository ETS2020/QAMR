// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x20), .b(x18), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nor3   g07(.a(new_n44_), .b(new_n48_), .c(x14), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n47_), .O(z01));
  inv1   g10(.a(x14), .O(new_n52_));
  nand3  g11(.a(new_n48_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n45_), .c(x12), .d(new_n42_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand3  g15(.a(new_n49_), .b(x12), .c(x10), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n47_), .O(z03));
  inv1   g17(.a(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z05));
  nor2   g20(.a(new_n44_), .b(new_n48_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  nand3  g27(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n45_), .b(new_n52_), .O(z11));
  nand2  g29(.a(new_n53_), .b(x13), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(x08), .c(x00), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(new_n45_), .c(x12), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z12));
  inv1   g36(.a(x01), .O(new_n78_));
  nand2  g37(.a(x10), .b(x08), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n78_), .c(x14), .O(new_n80_));
  nand3  g39(.a(x15), .b(x10), .c(x08), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n45_), .c(x12), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z13));
  nand2  g43(.a(new_n79_), .b(x15), .O(new_n85_));
  nand3  g44(.a(new_n73_), .b(x08), .c(x02), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n45_), .c(x12), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  nand3  g48(.a(new_n53_), .b(x16), .c(new_n52_), .O(new_n90_));
  nand3  g49(.a(new_n73_), .b(x08), .c(x03), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n45_), .c(x12), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z15));
  inv1   g53(.a(x12), .O(new_n95_));
  and2   g54(.a(x08), .b(x04), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n73_), .c(new_n53_), .d(x17), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n95_), .c(new_n45_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  aoi21  g58(.a(new_n53_), .b(z04), .c(new_n99_), .O(new_n100_));
  and2   g59(.a(new_n53_), .b(x18), .O(new_n101_));
  oai21  g60(.a(new_n101_), .b(new_n100_), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n45_), .O(z17));
  and2   g62(.a(x08), .b(x06), .O(new_n104_));
  aoi22  g63(.a(new_n104_), .b(new_n73_), .c(new_n53_), .d(x19), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n95_), .c(new_n45_), .O(z18));
  nand2  g65(.a(x18), .b(new_n48_), .O(new_n107_));
  oai21  g66(.a(new_n107_), .b(new_n79_), .c(new_n43_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x07), .O(new_n109_));
  nand2  g68(.a(new_n53_), .b(x20), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n95_), .O(z19));
endmodule


