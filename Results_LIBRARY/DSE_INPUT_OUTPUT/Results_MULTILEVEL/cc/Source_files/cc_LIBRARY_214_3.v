// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n66_, new_n68_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_;
  nand2  g00(.a(x12), .b(x00), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  inv1   g06(.a(new_n42_), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n47_), .O(z06));
  nand3  g08(.a(z06), .b(new_n46_), .c(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n45_), .O(z01));
  inv1   g10(.a(x00), .O(new_n52_));
  inv1   g11(.a(x11), .O(new_n53_));
  nand3  g12(.a(new_n47_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(x12), .c(new_n53_), .d(new_n52_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nand2  g17(.a(x10), .b(x08), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(x15), .c(new_n46_), .O(new_n61_));
  aoi21  g20(.a(new_n61_), .b(new_n52_), .c(new_n58_), .O(z03));
  nor2   g21(.a(new_n48_), .b(x18), .O(z04));
  and2   g22(.a(new_n42_), .b(x19), .O(z05));
  and2   g23(.a(new_n42_), .b(x17), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n42_), .b(new_n66_), .O(z08));
  nand3  g26(.a(new_n42_), .b(x09), .c(x08), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n48_), .O(z10));
  nand2  g29(.a(new_n42_), .b(new_n46_), .O(z11));
  nand2  g30(.a(new_n54_), .b(x13), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n52_), .c(new_n58_), .O(z12));
  inv1   g32(.a(x01), .O(new_n74_));
  oai21  g33(.a(new_n59_), .b(new_n74_), .c(x14), .O(new_n75_));
  nand3  g34(.a(x15), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x12), .c(new_n52_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  nand4  g38(.a(new_n47_), .b(x10), .c(x08), .d(x02), .O(new_n80_));
  oai21  g39(.a(new_n60_), .b(new_n47_), .c(new_n80_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x12), .c(new_n52_), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z14));
  nand3  g42(.a(new_n54_), .b(x16), .c(new_n46_), .O(new_n84_));
  inv1   g43(.a(x10), .O(new_n85_));
  nor2   g44(.a(x15), .b(new_n85_), .O(new_n86_));
  and2   g45(.a(x08), .b(x03), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(x00), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n84_), .c(new_n58_), .O(z15));
  nand2  g48(.a(new_n54_), .b(x17), .O(new_n90_));
  nand4  g49(.a(new_n47_), .b(x10), .c(x08), .d(x04), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x12), .c(new_n52_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  nand2  g53(.a(new_n54_), .b(x18), .O(new_n95_));
  nand4  g54(.a(new_n47_), .b(x10), .c(x08), .d(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(x12), .c(new_n52_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z17));
  nand2  g58(.a(new_n54_), .b(x19), .O(new_n100_));
  and2   g59(.a(x08), .b(x06), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n86_), .c(x00), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n100_), .c(new_n58_), .O(z18));
  nand2  g62(.a(new_n54_), .b(x20), .O(new_n104_));
  and2   g63(.a(x08), .b(x07), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n86_), .c(x00), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n104_), .c(new_n58_), .O(z19));
endmodule


