// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n56_, new_n57_, new_n60_, new_n62_, new_n64_,
    new_n65_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n82_, new_n85_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x10), .O(new_n43_));
  oai21  g02(.a(x15), .b(new_n43_), .c(x20), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(x08), .O(new_n47_));
  aoi21  g06(.a(new_n47_), .b(x15), .c(new_n43_), .O(z01));
  inv1   g07(.a(x15), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(x10), .O(new_n50_));
  nand2  g09(.a(x14), .b(x12), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x11), .c(new_n50_), .O(z02));
  nand3  g11(.a(new_n46_), .b(x12), .c(x08), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(x15), .c(new_n43_), .O(z03));
  nand2  g13(.a(new_n50_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nor2   g15(.a(x15), .b(new_n43_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n56_), .O(z05));
  nand2  g17(.a(new_n49_), .b(new_n43_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  nor2   g19(.a(new_n57_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n57_), .b(new_n62_), .O(z08));
  inv1   g22(.a(x08), .O(new_n64_));
  oai21  g23(.a(x15), .b(new_n43_), .c(x09), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n64_), .O(z09));
  nand3  g25(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nor2   g26(.a(new_n57_), .b(new_n46_), .O(z11));
  nand2  g27(.a(x13), .b(x12), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n50_), .O(z12));
  nand2  g29(.a(x12), .b(x08), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x10), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n51_), .O(z13));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x15), .c(x12), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z14));
  nand3  g36(.a(x16), .b(new_n46_), .c(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n50_), .O(z15));
  inv1   g38(.a(x12), .O(new_n80_));
  oai21  g39(.a(new_n60_), .b(new_n80_), .c(new_n50_), .O(z16));
  nand2  g40(.a(x18), .b(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n50_), .O(z17));
  oai21  g42(.a(new_n56_), .b(new_n80_), .c(new_n50_), .O(z18));
  nand2  g43(.a(x20), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n50_), .O(z19));
endmodule


