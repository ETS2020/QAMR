// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n56_, new_n58_, new_n61_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x12), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  and2   g09(.a(x10), .b(x08), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x14), .c(new_n50_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(new_n46_), .c(new_n45_), .O(z02));
  inv1   g12(.a(new_n43_), .O(z03));
  nand2  g13(.a(new_n43_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nand2  g15(.a(new_n43_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z07));
  and2   g18(.a(new_n43_), .b(x16), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n43_), .O(z09));
  nand3  g21(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nand2  g22(.a(new_n43_), .b(new_n47_), .O(z11));
  nand2  g23(.a(x10), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(x13), .O(new_n66_));
  aoi21  g25(.a(new_n51_), .b(x00), .c(x15), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n66_), .c(new_n45_), .O(z12));
  nand3  g27(.a(x10), .b(x08), .c(x01), .O(new_n69_));
  nand4  g28(.a(new_n69_), .b(new_n46_), .c(x14), .d(x12), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z13));
  nand2  g30(.a(new_n51_), .b(x02), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n46_), .c(new_n45_), .O(z14));
  nand3  g32(.a(new_n65_), .b(x16), .c(new_n47_), .O(new_n74_));
  nand3  g33(.a(x10), .b(x08), .c(x03), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n46_), .c(x12), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z15));
  nand2  g37(.a(new_n65_), .b(x17), .O(new_n79_));
  aoi21  g38(.a(new_n51_), .b(x04), .c(x15), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n45_), .O(z16));
  nand2  g40(.a(new_n65_), .b(x18), .O(new_n82_));
  aoi21  g41(.a(new_n51_), .b(x05), .c(x15), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n45_), .O(z17));
  nand2  g43(.a(new_n65_), .b(x19), .O(new_n85_));
  nand3  g44(.a(x10), .b(x08), .c(x06), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n46_), .c(x12), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z18));
  nand2  g48(.a(new_n65_), .b(x20), .O(new_n90_));
  aoi21  g49(.a(new_n51_), .b(x07), .c(x15), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n45_), .O(z19));
  buf    g51(.a(x15), .O(z06));
endmodule


