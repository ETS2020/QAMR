// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x16), .O(new_n43_));
  nor2   g02(.a(x18), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  nor2   g08(.a(new_n44_), .b(new_n49_), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n48_), .c(x10), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n47_), .O(z01));
  nand3  g11(.a(new_n49_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x12), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(x11), .c(new_n45_), .O(z02));
  nand4  g15(.a(new_n50_), .b(new_n48_), .c(x12), .d(x10), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n47_), .O(z03));
  inv1   g17(.a(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z05));
  aoi21  g20(.a(z04), .b(x16), .c(x15), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z07));
  nor2   g24(.a(z04), .b(new_n43_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  inv1   g27(.a(z09), .O(z10));
  nand2  g28(.a(new_n45_), .b(new_n48_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  and2   g32(.a(x08), .b(x00), .O(new_n74_));
  aoi22  g33(.a(new_n74_), .b(new_n73_), .c(new_n53_), .d(x13), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n71_), .c(new_n45_), .O(z12));
  nand3  g35(.a(x10), .b(x08), .c(x01), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x14), .O(new_n78_));
  nand3  g37(.a(x15), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n45_), .c(x12), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z13));
  oai21  g41(.a(new_n72_), .b(new_n47_), .c(x15), .O(new_n83_));
  nand3  g42(.a(new_n73_), .b(x08), .c(x02), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n45_), .c(x12), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z14));
  nand4  g46(.a(new_n62_), .b(x10), .c(x08), .d(x03), .O(new_n88_));
  nand4  g47(.a(new_n53_), .b(x18), .c(x16), .d(new_n48_), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n71_), .O(z15));
  and2   g49(.a(x08), .b(x04), .O(new_n91_));
  aoi22  g50(.a(new_n91_), .b(new_n73_), .c(new_n53_), .d(x17), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n71_), .c(new_n45_), .O(z16));
  and2   g52(.a(new_n53_), .b(x18), .O(new_n94_));
  inv1   g53(.a(x05), .O(new_n95_));
  aoi21  g54(.a(new_n53_), .b(z04), .c(new_n95_), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n94_), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n45_), .O(z17));
  nand2  g57(.a(new_n53_), .b(x19), .O(new_n99_));
  nand3  g58(.a(new_n73_), .b(x08), .c(x06), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n45_), .c(x12), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z18));
  and2   g62(.a(x08), .b(x07), .O(new_n104_));
  aoi22  g63(.a(new_n104_), .b(new_n73_), .c(new_n53_), .d(x20), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n71_), .c(new_n45_), .O(z19));
endmodule


