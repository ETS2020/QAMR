// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x14), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x13), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(new_n45_), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x15), .c(new_n44_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n47_), .O(z01));
  inv1   g10(.a(x13), .O(new_n52_));
  inv1   g11(.a(x15), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n44_), .c(x10), .d(x08), .O(new_n54_));
  oai21  g13(.a(new_n44_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nor4   g17(.a(new_n48_), .b(new_n53_), .c(x14), .d(new_n58_), .O(z03));
  nor2   g18(.a(new_n45_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n47_), .b(new_n61_), .O(z05));
  nor2   g21(.a(new_n45_), .b(new_n53_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n47_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x08), .O(new_n68_));
  inv1   g27(.a(x09), .O(new_n69_));
  nor3   g28(.a(new_n45_), .b(new_n69_), .c(new_n68_), .O(z09));
  aoi22  g29(.a(x14), .b(new_n52_), .c(x09), .d(x08), .O(z10));
  nor2   g30(.a(new_n44_), .b(new_n52_), .O(z11));
  nand2  g31(.a(new_n54_), .b(new_n52_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x00), .O(new_n74_));
  nand3  g33(.a(new_n53_), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x13), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n74_), .c(new_n58_), .O(z12));
  nand3  g36(.a(new_n44_), .b(x10), .c(x08), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(z11), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n49_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x14), .c(x13), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n58_), .O(z13));
  nand4  g42(.a(new_n53_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  oai21  g43(.a(new_n49_), .b(new_n53_), .c(new_n84_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n47_), .c(x12), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z14));
  nand3  g46(.a(new_n75_), .b(x16), .c(new_n44_), .O(new_n88_));
  nand4  g47(.a(new_n53_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n47_), .O(z15));
  inv1   g51(.a(x10), .O(new_n93_));
  nor2   g52(.a(x15), .b(new_n93_), .O(new_n94_));
  and2   g53(.a(x08), .b(x04), .O(new_n95_));
  aoi22  g54(.a(new_n95_), .b(new_n94_), .c(new_n75_), .d(x17), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n58_), .c(new_n47_), .O(z16));
  and2   g56(.a(x08), .b(x05), .O(new_n98_));
  aoi22  g57(.a(new_n98_), .b(new_n94_), .c(new_n75_), .d(x18), .O(new_n99_));
  oai21  g58(.a(new_n99_), .b(new_n58_), .c(new_n47_), .O(z17));
  nand2  g59(.a(new_n75_), .b(x19), .O(new_n101_));
  nand4  g60(.a(new_n53_), .b(x10), .c(x08), .d(x06), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n47_), .c(x12), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z18));
  nand2  g64(.a(new_n75_), .b(x20), .O(new_n106_));
  nand4  g65(.a(new_n53_), .b(x10), .c(x08), .d(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n47_), .c(x12), .O(new_n109_));
  inv1   g68(.a(new_n109_), .O(z19));
endmodule


