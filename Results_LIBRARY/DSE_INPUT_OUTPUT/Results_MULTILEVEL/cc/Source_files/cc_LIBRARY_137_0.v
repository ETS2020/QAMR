// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  inv1   g01(.a(x14), .O(new_n43_));
  nand4  g02(.a(x15), .b(new_n43_), .c(x10), .d(x08), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(z01));
  nand2  g04(.a(x14), .b(x12), .O(new_n46_));
  aoi21  g05(.a(new_n46_), .b(x15), .c(x11), .O(z02));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(x15), .b(x11), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nand3  g09(.a(x15), .b(new_n43_), .c(x12), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n48_), .c(new_n50_), .O(z03));
  nor2   g11(.a(new_n49_), .b(x18), .O(z04));
  inv1   g12(.a(x19), .O(new_n54_));
  nor2   g13(.a(new_n49_), .b(new_n54_), .O(z05));
  inv1   g14(.a(x17), .O(new_n56_));
  nand2  g15(.a(new_n50_), .b(new_n56_), .O(z07));
  inv1   g16(.a(x16), .O(new_n58_));
  nand2  g17(.a(new_n50_), .b(new_n58_), .O(z08));
  nand2  g18(.a(x09), .b(x08), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n50_), .O(z09));
  nand3  g20(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nand2  g21(.a(new_n50_), .b(new_n43_), .O(z11));
  inv1   g22(.a(x12), .O(new_n64_));
  nand2  g23(.a(new_n48_), .b(x13), .O(new_n65_));
  inv1   g24(.a(x15), .O(new_n66_));
  nand4  g25(.a(new_n66_), .b(x10), .c(x08), .d(x00), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(x11), .O(new_n69_));
  nand2  g28(.a(x15), .b(x13), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z12));
  nand2  g30(.a(new_n48_), .b(new_n43_), .O(new_n72_));
  nand3  g31(.a(x10), .b(x08), .c(x01), .O(new_n73_));
  aoi22  g32(.a(new_n73_), .b(x14), .c(new_n72_), .d(x15), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n64_), .c(new_n50_), .O(z13));
  nand2  g34(.a(new_n48_), .b(x15), .O(new_n76_));
  and2   g35(.a(x08), .b(x02), .O(new_n77_));
  nand4  g36(.a(new_n77_), .b(new_n66_), .c(x11), .d(x10), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(z14));
  nand4  g38(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x11), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n66_), .O(new_n82_));
  nand3  g41(.a(new_n66_), .b(x10), .c(x08), .O(new_n83_));
  nand4  g42(.a(new_n83_), .b(x16), .c(new_n43_), .d(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n82_), .O(z15));
  nand4  g44(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x11), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  nand3  g47(.a(new_n83_), .b(x17), .c(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(z16));
  nand4  g49(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x11), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n66_), .O(new_n93_));
  nand3  g52(.a(new_n83_), .b(x18), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(z17));
  nand4  g54(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x11), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n66_), .O(new_n98_));
  nand3  g57(.a(new_n83_), .b(x19), .c(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n98_), .O(z18));
  nand4  g59(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x11), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n66_), .O(new_n103_));
  nand3  g62(.a(new_n83_), .b(x20), .c(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(z19));
  buf    g64(.a(x15), .O(z06));
endmodule


