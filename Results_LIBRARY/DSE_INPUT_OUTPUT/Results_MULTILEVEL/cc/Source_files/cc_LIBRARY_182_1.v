// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x01), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(new_n45_), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x08), .O(new_n49_));
  inv1   g08(.a(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x15), .c(new_n48_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(z01));
  inv1   g12(.a(x15), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x10), .c(x08), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x01), .c(new_n44_), .O(z02));
  aoi21  g17(.a(new_n52_), .b(x01), .c(new_n44_), .O(z03));
  nor2   g18(.a(new_n45_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n45_), .b(new_n61_), .O(z05));
  nor2   g21(.a(new_n45_), .b(new_n54_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n47_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n47_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n45_), .b(new_n68_), .c(new_n49_), .O(z09));
  nand3  g28(.a(new_n47_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n45_), .b(new_n48_), .O(z11));
  nand2  g30(.a(new_n55_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n54_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x12), .c(x01), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(x14), .O(new_n77_));
  inv1   g36(.a(x01), .O(new_n78_));
  nor2   g37(.a(new_n54_), .b(new_n50_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(x08), .c(new_n78_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n77_), .c(new_n44_), .O(z13));
  nand4  g40(.a(new_n54_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  oai21  g41(.a(new_n51_), .b(new_n54_), .c(new_n82_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(x12), .c(x01), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z14));
  nand3  g44(.a(new_n55_), .b(x16), .c(new_n48_), .O(new_n86_));
  nor2   g45(.a(x15), .b(new_n50_), .O(new_n87_));
  and2   g46(.a(x08), .b(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n78_), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n86_), .c(new_n44_), .O(z15));
  nand2  g49(.a(new_n55_), .b(x17), .O(new_n91_));
  and2   g50(.a(x08), .b(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n87_), .c(new_n78_), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n91_), .c(new_n44_), .O(z16));
  nand2  g53(.a(new_n55_), .b(x18), .O(new_n95_));
  nand4  g54(.a(new_n54_), .b(x10), .c(x08), .d(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(x12), .c(x01), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z17));
  nand2  g58(.a(new_n55_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n54_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(x12), .c(x01), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z18));
  nand2  g63(.a(new_n55_), .b(x20), .O(new_n105_));
  nand4  g64(.a(new_n54_), .b(x10), .c(x08), .d(x07), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(x12), .c(x01), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z19));
endmodule


