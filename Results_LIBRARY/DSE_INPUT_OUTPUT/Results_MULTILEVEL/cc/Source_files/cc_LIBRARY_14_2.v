// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  inv1   g06(.a(x16), .O(new_n48_));
  nor2   g07(.a(x20), .b(new_n48_), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n47_), .O(z06));
  nand3  g09(.a(z06), .b(new_n46_), .c(x10), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n45_), .O(z01));
  inv1   g11(.a(new_n49_), .O(new_n53_));
  nand3  g12(.a(new_n47_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n53_), .O(z02));
  nand2  g16(.a(x10), .b(x08), .O(new_n58_));
  nand3  g17(.a(x15), .b(new_n46_), .c(x12), .O(new_n59_));
  oai21  g18(.a(new_n59_), .b(new_n58_), .c(new_n53_), .O(z03));
  nand2  g19(.a(new_n53_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n53_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n53_), .b(new_n64_), .O(z07));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n53_), .O(z09));
  nand3  g26(.a(new_n53_), .b(x09), .c(x08), .O(z10));
  nor2   g27(.a(new_n49_), .b(new_n46_), .O(z11));
  nand2  g28(.a(new_n54_), .b(x13), .O(new_n70_));
  inv1   g29(.a(x10), .O(new_n71_));
  nor2   g30(.a(x15), .b(new_n71_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x08), .c(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n53_), .c(x12), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  inv1   g35(.a(new_n58_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(x01), .c(new_n46_), .O(new_n78_));
  nand3  g37(.a(x15), .b(x10), .c(x08), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n53_), .O(z13));
  nand2  g41(.a(new_n58_), .b(x15), .O(new_n83_));
  nand3  g42(.a(new_n72_), .b(x08), .c(x02), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n53_), .O(z14));
  inv1   g46(.a(x12), .O(new_n88_));
  aoi21  g47(.a(new_n43_), .b(x16), .c(x15), .O(new_n89_));
  nand4  g48(.a(new_n89_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  nand4  g49(.a(new_n54_), .b(x20), .c(x16), .d(new_n46_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(z15));
  and2   g51(.a(x08), .b(x04), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n72_), .c(new_n54_), .d(x17), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n88_), .c(new_n53_), .O(z16));
  and2   g54(.a(x08), .b(x05), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n72_), .c(new_n54_), .d(x18), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n88_), .c(new_n53_), .O(z17));
  and2   g57(.a(x08), .b(x06), .O(new_n99_));
  aoi22  g58(.a(new_n99_), .b(new_n72_), .c(new_n54_), .d(x19), .O(new_n100_));
  oai21  g59(.a(new_n100_), .b(new_n88_), .c(new_n53_), .O(z18));
  and2   g60(.a(new_n54_), .b(x20), .O(new_n102_));
  inv1   g61(.a(x07), .O(new_n103_));
  aoi21  g62(.a(new_n54_), .b(new_n43_), .c(new_n103_), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n102_), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n53_), .O(z19));
  buf    g65(.a(x16), .O(z08));
endmodule


