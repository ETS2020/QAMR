// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n65_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(x19), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(x15), .b(new_n47_), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  nand2  g09(.a(new_n44_), .b(x14), .O(new_n51_));
  inv1   g10(.a(new_n46_), .O(new_n52_));
  nor2   g11(.a(x19), .b(x15), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n50_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nor4   g17(.a(new_n46_), .b(new_n43_), .c(x14), .d(new_n58_), .O(z03));
  nand2  g18(.a(new_n44_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n43_), .O(z05));
  and2   g21(.a(new_n44_), .b(x17), .O(z07));
  and2   g22(.a(new_n44_), .b(x16), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n44_), .O(z09));
  nand3  g25(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g26(.a(new_n44_), .b(new_n47_), .O(z11));
  aoi21  g27(.a(x10), .b(x08), .c(x19), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(x15), .c(x13), .O(new_n70_));
  nand4  g29(.a(new_n53_), .b(x10), .c(x08), .d(x00), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n70_), .c(new_n58_), .O(z12));
  oai21  g31(.a(new_n52_), .b(x14), .c(x15), .O(new_n73_));
  nand2  g32(.a(new_n52_), .b(x01), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n61_), .c(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n58_), .O(z13));
  nand4  g35(.a(new_n43_), .b(x10), .c(x08), .d(x02), .O(new_n77_));
  oai21  g36(.a(new_n52_), .b(new_n43_), .c(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n44_), .O(z14));
  nand4  g39(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n43_), .O(new_n83_));
  nand3  g42(.a(new_n43_), .b(x10), .c(x08), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(x16), .c(new_n47_), .d(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n83_), .O(z15));
  oai21  g45(.a(new_n69_), .b(x15), .c(x17), .O(new_n87_));
  nand4  g46(.a(new_n53_), .b(x10), .c(x08), .d(x04), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n58_), .O(z16));
  oai21  g48(.a(new_n69_), .b(x15), .c(x18), .O(new_n90_));
  nand4  g49(.a(new_n53_), .b(x10), .c(x08), .d(x05), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n58_), .O(z17));
  nand4  g51(.a(new_n43_), .b(x10), .c(x08), .d(x06), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n44_), .O(z18));
  nand2  g55(.a(new_n46_), .b(x20), .O(new_n97_));
  nand4  g56(.a(new_n43_), .b(x10), .c(x08), .d(x07), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  nand2  g59(.a(x20), .b(x15), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n58_), .O(z19));
  buf    g61(.a(x15), .O(z06));
endmodule


