// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x08), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand4  g06(.a(x15), .b(new_n47_), .c(x10), .d(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  aoi21  g09(.a(new_n44_), .b(x10), .c(x14), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x14), .O(new_n52_));
  oai21  g11(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  nand3  g14(.a(new_n47_), .b(x12), .c(x10), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(x08), .c(new_n44_), .O(z03));
  inv1   g16(.a(new_n45_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n58_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n58_), .O(z09));
  nand2  g26(.a(new_n44_), .b(new_n50_), .O(new_n68_));
  oai21  g27(.a(x09), .b(new_n50_), .c(new_n68_), .O(z10));
  nand2  g28(.a(new_n58_), .b(new_n47_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  inv1   g30(.a(x13), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(new_n44_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n50_), .O(new_n74_));
  inv1   g33(.a(x10), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n75_), .O(new_n76_));
  nand4  g35(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n77_));
  oai21  g36(.a(new_n76_), .b(new_n72_), .c(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n74_), .O(z12));
  aoi21  g39(.a(x10), .b(x01), .c(new_n47_), .O(new_n81_));
  nor2   g40(.a(new_n44_), .b(new_n75_), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n81_), .c(x08), .O(new_n83_));
  nand3  g42(.a(new_n44_), .b(x14), .c(new_n50_), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n71_), .O(z13));
  inv1   g44(.a(x02), .O(new_n86_));
  nand2  g45(.a(new_n44_), .b(x10), .O(new_n87_));
  nand2  g46(.a(x15), .b(new_n75_), .O(new_n88_));
  oai21  g47(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(x12), .c(x08), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z14));
  nand3  g50(.a(x16), .b(new_n47_), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n44_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n50_), .O(new_n94_));
  nand3  g53(.a(new_n87_), .b(x16), .c(new_n47_), .O(new_n95_));
  nand4  g54(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n94_), .O(z15));
  inv1   g58(.a(x04), .O(new_n100_));
  nand2  g59(.a(x10), .b(x08), .O(new_n101_));
  oai22  g60(.a(new_n101_), .b(new_n100_), .c(new_n62_), .d(x08), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n44_), .O(new_n103_));
  nand3  g62(.a(new_n87_), .b(x17), .c(x08), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n71_), .O(z16));
  inv1   g64(.a(x18), .O(new_n106_));
  oai21  g65(.a(new_n106_), .b(new_n71_), .c(new_n44_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n50_), .O(new_n108_));
  nand4  g67(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n109_));
  oai21  g68(.a(new_n76_), .b(new_n106_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n108_), .O(z17));
  oai21  g71(.a(new_n60_), .b(new_n71_), .c(new_n44_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n50_), .O(new_n114_));
  nand4  g73(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n115_));
  oai21  g74(.a(new_n76_), .b(new_n60_), .c(new_n115_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(x12), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n114_), .O(z18));
  inv1   g77(.a(x07), .O(new_n119_));
  oai22  g78(.a(new_n101_), .b(new_n119_), .c(new_n43_), .d(x08), .O(new_n120_));
  nand2  g79(.a(new_n120_), .b(new_n44_), .O(new_n121_));
  nand3  g80(.a(new_n87_), .b(x20), .c(x08), .O(new_n122_));
  aoi21  g81(.a(new_n122_), .b(new_n121_), .c(new_n71_), .O(z19));
  buf    g82(.a(x15), .O(z06));
endmodule


