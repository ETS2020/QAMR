// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n61_, new_n63_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x12), .O(new_n43_));
  nand2  g02(.a(x15), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x20), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x15), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n47_), .c(x14), .d(new_n43_), .O(z01));
  inv1   g08(.a(x14), .O(new_n50_));
  nand3  g09(.a(new_n47_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n44_), .O(z02));
  inv1   g13(.a(new_n44_), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  aoi21  g16(.a(x15), .b(new_n43_), .c(new_n57_), .O(z05));
  nor2   g17(.a(new_n47_), .b(new_n43_), .O(z06));
  and2   g18(.a(new_n44_), .b(x17), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nand2  g20(.a(new_n44_), .b(new_n61_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n44_), .O(z09));
  nand3  g23(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nor2   g24(.a(new_n55_), .b(new_n50_), .O(z11));
  nand2  g25(.a(new_n51_), .b(x13), .O(new_n67_));
  inv1   g26(.a(x10), .O(new_n68_));
  nor2   g27(.a(x15), .b(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(x08), .c(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n67_), .c(new_n43_), .O(z12));
  nand2  g30(.a(new_n48_), .b(x12), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x15), .O(new_n73_));
  nand3  g32(.a(x10), .b(x08), .c(x01), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x14), .c(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n73_), .O(z13));
  nand3  g35(.a(x12), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x15), .O(new_n78_));
  nand2  g37(.a(x08), .b(x02), .O(new_n79_));
  nand3  g38(.a(new_n47_), .b(x12), .c(x10), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(z14));
  nand3  g40(.a(new_n51_), .b(x16), .c(new_n50_), .O(new_n82_));
  nand3  g41(.a(new_n69_), .b(x08), .c(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n43_), .O(z15));
  oai21  g43(.a(x17), .b(new_n43_), .c(x15), .O(new_n85_));
  nand2  g44(.a(new_n48_), .b(x17), .O(new_n86_));
  nand4  g45(.a(new_n47_), .b(x10), .c(x08), .d(x04), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n85_), .O(z16));
  nand2  g49(.a(new_n51_), .b(x18), .O(new_n91_));
  nand3  g50(.a(new_n69_), .b(x08), .c(x05), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n43_), .O(z17));
  oai21  g52(.a(x19), .b(new_n43_), .c(x15), .O(new_n94_));
  nand2  g53(.a(new_n48_), .b(x19), .O(new_n95_));
  nand4  g54(.a(new_n47_), .b(x10), .c(x08), .d(x06), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n94_), .O(z18));
  nand2  g58(.a(new_n51_), .b(x20), .O(new_n100_));
  nand3  g59(.a(new_n69_), .b(x08), .c(x07), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n43_), .O(z19));
  nor4   g61(.a(new_n48_), .b(new_n47_), .c(x14), .d(new_n43_), .O(z03));
endmodule


