// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n56_, new_n60_, new_n62_, new_n64_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x12), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  and2   g09(.a(x10), .b(x08), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x14), .c(new_n50_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(new_n46_), .c(new_n45_), .O(z02));
  nand2  g12(.a(new_n43_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n56_));
  aoi21  g14(.a(x15), .b(x12), .c(new_n56_), .O(z05));
  nor2   g15(.a(new_n46_), .b(x12), .O(z06));
  and2   g16(.a(new_n43_), .b(x17), .O(z07));
  inv1   g17(.a(x16), .O(new_n60_));
  nand2  g18(.a(new_n43_), .b(new_n60_), .O(z08));
  nand3  g19(.a(new_n43_), .b(x09), .c(x08), .O(new_n62_));
  inv1   g20(.a(new_n62_), .O(z09));
  inv1   g21(.a(new_n43_), .O(new_n64_));
  aoi21  g22(.a(x09), .b(x08), .c(new_n64_), .O(z10));
  nor2   g23(.a(new_n64_), .b(new_n47_), .O(z11));
  nand2  g24(.a(x10), .b(x08), .O(new_n67_));
  nand2  g25(.a(new_n67_), .b(x13), .O(new_n68_));
  aoi21  g26(.a(new_n51_), .b(x00), .c(x15), .O(new_n69_));
  aoi21  g27(.a(new_n69_), .b(new_n68_), .c(new_n45_), .O(z12));
  nand3  g28(.a(x10), .b(x08), .c(x01), .O(new_n71_));
  nand4  g29(.a(new_n71_), .b(new_n46_), .c(x14), .d(x12), .O(new_n72_));
  inv1   g30(.a(new_n72_), .O(z13));
  nand2  g31(.a(new_n51_), .b(x02), .O(new_n74_));
  aoi21  g32(.a(new_n74_), .b(new_n46_), .c(new_n45_), .O(z14));
  nand3  g33(.a(new_n67_), .b(x16), .c(new_n47_), .O(new_n76_));
  nand3  g34(.a(x10), .b(x08), .c(x03), .O(new_n77_));
  nand2  g35(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g36(.a(new_n78_), .b(new_n46_), .c(x12), .O(new_n79_));
  inv1   g37(.a(new_n79_), .O(z15));
  nand2  g38(.a(new_n67_), .b(x17), .O(new_n81_));
  nand3  g39(.a(x10), .b(x08), .c(x04), .O(new_n82_));
  nand2  g40(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g41(.a(new_n83_), .b(new_n46_), .c(x12), .O(new_n84_));
  inv1   g42(.a(new_n84_), .O(z16));
  nand2  g43(.a(new_n67_), .b(x18), .O(new_n86_));
  aoi21  g44(.a(new_n51_), .b(x05), .c(x15), .O(new_n87_));
  aoi21  g45(.a(new_n87_), .b(new_n86_), .c(new_n45_), .O(z17));
  nand2  g46(.a(new_n67_), .b(x19), .O(new_n89_));
  nand3  g47(.a(x10), .b(x08), .c(x06), .O(new_n90_));
  nand2  g48(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g49(.a(new_n91_), .b(new_n46_), .c(x12), .O(new_n92_));
  inv1   g50(.a(new_n92_), .O(z18));
  nand2  g51(.a(new_n67_), .b(x20), .O(new_n94_));
  aoi21  g52(.a(new_n51_), .b(x07), .c(x15), .O(new_n95_));
  aoi21  g53(.a(new_n95_), .b(new_n94_), .c(new_n45_), .O(z19));
  zero   g54(.O(z03));
endmodule


