// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x19), .b(x09), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nor2   g07(.a(new_n43_), .b(new_n48_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(new_n47_), .c(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n46_), .O(z01));
  nand3  g10(.a(new_n48_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x12), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x11), .c(new_n44_), .O(z02));
  nand4  g14(.a(new_n49_), .b(new_n47_), .c(x12), .d(x10), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(new_n46_), .O(z03));
  nor2   g16(.a(new_n43_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x09), .O(z05));
  nand2  g19(.a(new_n44_), .b(new_n48_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n44_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n44_), .O(z09));
  nand2  g26(.a(x09), .b(new_n46_), .O(new_n68_));
  oai21  g27(.a(new_n59_), .b(x09), .c(new_n68_), .O(z10));
  nand2  g28(.a(new_n44_), .b(new_n47_), .O(z11));
  nand2  g29(.a(new_n52_), .b(x13), .O(new_n71_));
  nand4  g30(.a(new_n48_), .b(x10), .c(x08), .d(x00), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n44_), .c(x12), .O(new_n74_));
  inv1   g33(.a(new_n74_), .O(z12));
  nand3  g34(.a(x10), .b(x08), .c(x01), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x14), .O(new_n77_));
  nand3  g36(.a(x15), .b(x10), .c(x08), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n44_), .c(x12), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z13));
  nand2  g40(.a(new_n48_), .b(x10), .O(new_n82_));
  inv1   g41(.a(x10), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n46_), .c(x15), .O(new_n84_));
  nand2  g43(.a(x08), .b(x02), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n82_), .c(new_n84_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n44_), .O(z14));
  nand3  g47(.a(new_n52_), .b(x16), .c(new_n47_), .O(new_n89_));
  nand4  g48(.a(new_n48_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n44_), .O(z15));
  inv1   g52(.a(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(new_n83_), .O(new_n95_));
  and2   g54(.a(x08), .b(x04), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n95_), .c(new_n52_), .d(x17), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n94_), .c(new_n44_), .O(z16));
  nand2  g57(.a(new_n52_), .b(x18), .O(new_n99_));
  nand4  g58(.a(new_n48_), .b(x10), .c(x08), .d(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n44_), .c(x12), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z17));
  oai21  g62(.a(new_n66_), .b(new_n82_), .c(new_n59_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x06), .O(new_n105_));
  nand2  g64(.a(new_n52_), .b(x19), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n105_), .c(new_n94_), .O(z18));
  nand2  g66(.a(new_n52_), .b(x20), .O(new_n108_));
  nand4  g67(.a(new_n48_), .b(x10), .c(x08), .d(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(new_n44_), .c(x12), .O(new_n111_));
  inv1   g70(.a(new_n111_), .O(z19));
endmodule


