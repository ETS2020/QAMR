// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  inv1   g01(.a(x15), .O(new_n43_));
  inv1   g02(.a(x14), .O(new_n44_));
  nand3  g03(.a(new_n44_), .b(x10), .c(x08), .O(new_n45_));
  aoi21  g04(.a(new_n45_), .b(x11), .c(new_n43_), .O(z01));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(x14), .c(x12), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(new_n43_), .c(x11), .O(z02));
  nand3  g09(.a(new_n48_), .b(x12), .c(x11), .O(new_n51_));
  nor3   g10(.a(new_n51_), .b(new_n43_), .c(x14), .O(z03));
  nor2   g11(.a(new_n43_), .b(x11), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  oai21  g14(.a(new_n43_), .b(x11), .c(new_n55_), .O(z05));
  nand2  g15(.a(x15), .b(x11), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z06));
  inv1   g17(.a(x17), .O(new_n59_));
  nor2   g18(.a(new_n53_), .b(new_n59_), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nor2   g20(.a(new_n53_), .b(new_n61_), .O(z08));
  inv1   g21(.a(x08), .O(new_n63_));
  inv1   g22(.a(x09), .O(new_n64_));
  nor3   g23(.a(new_n53_), .b(new_n64_), .c(new_n63_), .O(z09));
  inv1   g24(.a(new_n53_), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(x09), .c(x08), .O(z10));
  nor2   g26(.a(new_n53_), .b(new_n44_), .O(z11));
  nand2  g27(.a(x13), .b(x12), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(x11), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x15), .O(new_n71_));
  nand2  g30(.a(new_n47_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n43_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(z12));
  nand3  g35(.a(x12), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x11), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x15), .O(new_n79_));
  nand2  g38(.a(new_n48_), .b(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x14), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n79_), .O(z13));
  inv1   g41(.a(x12), .O(new_n83_));
  nand3  g42(.a(new_n47_), .b(x15), .c(x11), .O(new_n84_));
  inv1   g43(.a(x10), .O(new_n85_));
  nor2   g44(.a(x15), .b(new_n85_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x08), .c(x02), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n84_), .c(new_n83_), .O(z14));
  nand2  g47(.a(new_n47_), .b(new_n43_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n57_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(x16), .c(new_n44_), .O(new_n91_));
  nand3  g50(.a(new_n86_), .b(x08), .c(x03), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n83_), .O(z15));
  oai21  g52(.a(new_n59_), .b(new_n83_), .c(x11), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n47_), .b(x17), .O(new_n96_));
  nand4  g55(.a(new_n43_), .b(x10), .c(x08), .d(x04), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n95_), .O(z16));
  nand2  g59(.a(new_n47_), .b(x18), .O(new_n101_));
  nand3  g60(.a(x10), .b(x08), .c(x05), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n43_), .O(new_n104_));
  nand3  g63(.a(x18), .b(x15), .c(x11), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n83_), .O(z17));
  nand2  g65(.a(new_n47_), .b(x19), .O(new_n107_));
  nand3  g66(.a(x10), .b(x08), .c(x06), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n43_), .O(new_n110_));
  nand3  g69(.a(x19), .b(x15), .c(x11), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n83_), .O(z18));
  nand2  g71(.a(new_n47_), .b(x20), .O(new_n113_));
  nand3  g72(.a(x10), .b(x08), .c(x07), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n43_), .O(new_n116_));
  nand3  g75(.a(x20), .b(x15), .c(x11), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n116_), .c(new_n83_), .O(z19));
endmodule


