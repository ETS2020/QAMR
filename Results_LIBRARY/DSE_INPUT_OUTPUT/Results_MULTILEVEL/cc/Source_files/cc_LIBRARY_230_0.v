// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n55_, new_n57_, new_n59_, new_n61_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  nor2   g01(.a(x15), .b(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x10), .b(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x15), .b(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z01));
  nand2  g07(.a(x14), .b(x12), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x15), .c(x11), .O(z02));
  inv1   g09(.a(x12), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nor4   g11(.a(new_n45_), .b(new_n52_), .c(x14), .d(new_n51_), .O(z03));
  nor2   g12(.a(new_n43_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nand2  g14(.a(new_n44_), .b(new_n55_), .O(z05));
  inv1   g15(.a(x17), .O(new_n57_));
  nor2   g16(.a(new_n43_), .b(new_n57_), .O(z07));
  inv1   g17(.a(x16), .O(new_n59_));
  nor2   g18(.a(new_n43_), .b(new_n59_), .O(z08));
  inv1   g19(.a(x08), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nor3   g21(.a(new_n43_), .b(new_n62_), .c(new_n61_), .O(z09));
  nand3  g22(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nor2   g23(.a(new_n43_), .b(new_n46_), .O(z11));
  nand4  g24(.a(x12), .b(x10), .c(x08), .d(x00), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(x11), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand3  g27(.a(new_n52_), .b(x10), .c(x08), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(x13), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n68_), .O(z12));
  nand2  g30(.a(new_n45_), .b(new_n46_), .O(new_n72_));
  nand3  g31(.a(x10), .b(x08), .c(x01), .O(new_n73_));
  aoi22  g32(.a(new_n73_), .b(x14), .c(new_n72_), .d(x15), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n51_), .c(new_n44_), .O(z13));
  nand2  g34(.a(new_n45_), .b(x15), .O(new_n76_));
  and2   g35(.a(x08), .b(x02), .O(new_n77_));
  nand4  g36(.a(new_n77_), .b(new_n52_), .c(x11), .d(x10), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n76_), .c(new_n51_), .O(z14));
  nand3  g38(.a(new_n45_), .b(x16), .c(new_n46_), .O(new_n80_));
  nand4  g39(.a(new_n52_), .b(x10), .c(x08), .d(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x11), .O(new_n83_));
  nand3  g42(.a(x16), .b(x15), .c(new_n46_), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n51_), .O(z15));
  nand4  g44(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x11), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n52_), .O(new_n88_));
  nand3  g47(.a(new_n69_), .b(x17), .c(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(z16));
  nand2  g49(.a(new_n45_), .b(x18), .O(new_n91_));
  nand4  g50(.a(new_n52_), .b(x10), .c(x08), .d(x05), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x11), .O(new_n94_));
  nand2  g53(.a(x18), .b(x15), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n51_), .O(z17));
  nand2  g55(.a(new_n45_), .b(x19), .O(new_n97_));
  nand4  g56(.a(new_n52_), .b(x10), .c(x08), .d(x06), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x11), .O(new_n100_));
  nand2  g59(.a(x19), .b(x15), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n51_), .O(z18));
  nand2  g61(.a(new_n45_), .b(x20), .O(new_n103_));
  nand4  g62(.a(new_n52_), .b(x10), .c(x08), .d(x07), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x11), .O(new_n106_));
  nand2  g65(.a(x20), .b(x15), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n51_), .O(z19));
  buf    g67(.a(x15), .O(z06));
endmodule


