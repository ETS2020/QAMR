// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n59_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x19), .b(x09), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x10), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n43_), .b(x15), .c(new_n47_), .O(new_n48_));
  nor3   g07(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n43_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor4   g14(.a(new_n48_), .b(new_n55_), .c(new_n46_), .d(new_n45_), .O(z03));
  aoi21  g15(.a(x19), .b(x09), .c(x18), .O(z04));
  nand2  g16(.a(new_n43_), .b(new_n50_), .O(z06));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n43_), .b(new_n59_), .O(z07));
  and2   g19(.a(new_n43_), .b(x16), .O(z08));
  oai21  g20(.a(x19), .b(x08), .c(x09), .O(z10));
  inv1   g21(.a(z10), .O(z09));
  nand2  g22(.a(new_n43_), .b(new_n47_), .O(z11));
  nand2  g23(.a(new_n51_), .b(x13), .O(new_n65_));
  nand4  g24(.a(new_n50_), .b(x10), .c(x08), .d(x00), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(new_n43_), .c(x12), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z12));
  inv1   g28(.a(x01), .O(new_n70_));
  nand2  g29(.a(x10), .b(x08), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n70_), .c(x14), .O(new_n72_));
  nand3  g31(.a(x15), .b(x10), .c(x08), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n43_), .c(x12), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z13));
  nand2  g35(.a(new_n71_), .b(x15), .O(new_n77_));
  nand4  g36(.a(new_n50_), .b(x10), .c(x08), .d(x02), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n43_), .O(z14));
  nand3  g40(.a(new_n51_), .b(x16), .c(new_n47_), .O(new_n82_));
  nand4  g41(.a(new_n50_), .b(x10), .c(x08), .d(x03), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n43_), .O(z15));
  nor2   g45(.a(x15), .b(new_n46_), .O(new_n87_));
  and2   g46(.a(x08), .b(x04), .O(new_n88_));
  aoi22  g47(.a(new_n88_), .b(new_n87_), .c(new_n51_), .d(x17), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n55_), .c(new_n43_), .O(z16));
  nand2  g49(.a(new_n51_), .b(x18), .O(new_n91_));
  nand4  g50(.a(new_n50_), .b(x10), .c(x08), .d(x05), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n43_), .c(x12), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z17));
  inv1   g54(.a(x09), .O(new_n96_));
  nand2  g55(.a(x19), .b(new_n96_), .O(new_n97_));
  inv1   g56(.a(x19), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n50_), .O(new_n99_));
  oai21  g58(.a(new_n99_), .b(new_n71_), .c(new_n97_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x06), .O(new_n101_));
  nand3  g60(.a(new_n51_), .b(x19), .c(new_n96_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n55_), .O(z18));
  nand2  g62(.a(new_n51_), .b(x20), .O(new_n104_));
  nand4  g63(.a(new_n50_), .b(x10), .c(x08), .d(x07), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n43_), .c(x12), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z19));
  buf    g67(.a(x19), .O(z05));
endmodule


