// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n64_, new_n67_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x12), .b(x00), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand4  g05(.a(new_n43_), .b(x15), .c(new_n46_), .d(x10), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(new_n45_), .O(z01));
  inv1   g07(.a(x00), .O(new_n49_));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(x12), .c(new_n50_), .d(new_n49_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nand4  g15(.a(x15), .b(new_n46_), .c(x10), .d(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n49_), .c(new_n56_), .O(z03));
  inv1   g17(.a(new_n43_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n43_), .b(new_n51_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z07));
  and2   g24(.a(new_n43_), .b(x16), .O(z08));
  nand3  g25(.a(new_n43_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  nand3  g27(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nor2   g28(.a(new_n59_), .b(new_n46_), .O(z11));
  nand4  g29(.a(new_n52_), .b(x13), .c(x12), .d(new_n49_), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z12));
  inv1   g31(.a(x01), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(x14), .O(new_n75_));
  nand3  g34(.a(x15), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x12), .c(new_n49_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  nand2  g38(.a(new_n74_), .b(x15), .O(new_n80_));
  nand4  g39(.a(new_n51_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n80_), .c(new_n49_), .O(new_n82_));
  and2   g41(.a(new_n82_), .b(x12), .O(z14));
  nand3  g42(.a(new_n52_), .b(x16), .c(new_n46_), .O(new_n84_));
  nand4  g43(.a(new_n51_), .b(x10), .c(x08), .d(x03), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x12), .c(new_n49_), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z15));
  nand2  g47(.a(new_n52_), .b(x17), .O(new_n89_));
  inv1   g48(.a(x10), .O(new_n90_));
  nor2   g49(.a(x15), .b(new_n90_), .O(new_n91_));
  and2   g50(.a(x08), .b(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(x00), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n89_), .c(new_n56_), .O(z16));
  nand2  g53(.a(new_n52_), .b(x18), .O(new_n95_));
  and2   g54(.a(x08), .b(x05), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n91_), .c(x00), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n95_), .c(new_n56_), .O(z17));
  nand2  g57(.a(new_n52_), .b(x19), .O(new_n99_));
  and2   g58(.a(x08), .b(x06), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n91_), .c(x00), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n99_), .c(new_n56_), .O(z18));
  nand2  g61(.a(new_n52_), .b(x20), .O(new_n103_));
  and2   g62(.a(x08), .b(x07), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n91_), .c(x00), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n103_), .c(new_n56_), .O(z19));
endmodule


