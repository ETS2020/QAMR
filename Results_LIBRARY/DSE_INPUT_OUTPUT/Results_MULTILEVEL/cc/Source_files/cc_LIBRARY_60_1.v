// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand3  g05(.a(x10), .b(x08), .c(x06), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n43_), .c(x14), .O(z01));
  inv1   g07(.a(x11), .O(new_n49_));
  inv1   g08(.a(x14), .O(new_n50_));
  nand3  g09(.a(new_n43_), .b(x10), .c(x08), .O(new_n51_));
  oai21  g10(.a(new_n44_), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  nand2  g13(.a(x10), .b(x08), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(new_n50_), .c(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x06), .c(new_n43_), .O(z03));
  nor2   g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x06), .O(new_n60_));
  inv1   g19(.a(x19), .O(new_n61_));
  aoi21  g20(.a(x15), .b(new_n60_), .c(new_n61_), .O(z05));
  nor2   g21(.a(new_n43_), .b(new_n60_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n44_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n45_), .O(z09));
  inv1   g28(.a(z09), .O(z10));
  nand2  g29(.a(new_n45_), .b(new_n50_), .O(z11));
  inv1   g30(.a(x12), .O(new_n72_));
  nor2   g31(.a(new_n56_), .b(x15), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(z06), .c(x13), .O(new_n74_));
  inv1   g33(.a(x10), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(z12));
  nand3  g37(.a(x12), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x15), .O(new_n81_));
  nand2  g40(.a(new_n56_), .b(x01), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x14), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n81_), .O(z13));
  and2   g43(.a(x08), .b(x02), .O(new_n85_));
  aoi22  g44(.a(new_n85_), .b(new_n76_), .c(new_n55_), .d(x15), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n72_), .c(new_n45_), .O(z14));
  nand3  g46(.a(x16), .b(new_n50_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x06), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand3  g49(.a(new_n55_), .b(x16), .c(new_n50_), .O(new_n91_));
  nand3  g50(.a(new_n76_), .b(x08), .c(x03), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  aoi21  g54(.a(x17), .b(x12), .c(new_n60_), .O(new_n96_));
  and2   g55(.a(x08), .b(x04), .O(new_n97_));
  aoi22  g56(.a(new_n97_), .b(new_n76_), .c(new_n55_), .d(x17), .O(new_n98_));
  oai22  g57(.a(new_n98_), .b(new_n72_), .c(new_n96_), .d(new_n43_), .O(z16));
  inv1   g58(.a(x05), .O(new_n100_));
  nand2  g59(.a(new_n55_), .b(x18), .O(new_n101_));
  oai21  g60(.a(new_n55_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n43_), .O(new_n103_));
  nand3  g62(.a(x18), .b(x15), .c(x06), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n72_), .O(z17));
  aoi21  g64(.a(new_n51_), .b(new_n61_), .c(new_n60_), .O(new_n106_));
  and2   g65(.a(new_n51_), .b(x19), .O(new_n107_));
  oai21  g66(.a(new_n107_), .b(new_n106_), .c(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n45_), .O(z18));
  inv1   g68(.a(x07), .O(new_n110_));
  nand2  g69(.a(new_n55_), .b(x20), .O(new_n111_));
  oai21  g70(.a(new_n55_), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n43_), .O(new_n113_));
  nand3  g72(.a(x20), .b(x15), .c(x06), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n72_), .O(z19));
endmodule


