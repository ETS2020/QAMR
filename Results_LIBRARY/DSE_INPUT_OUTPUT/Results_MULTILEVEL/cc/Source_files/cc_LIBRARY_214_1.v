// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n66_, new_n68_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x00), .O(new_n43_));
  nand2  g02(.a(x12), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x20), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand4  g07(.a(new_n44_), .b(x15), .c(new_n48_), .d(x10), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n47_), .O(z01));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(x12), .c(new_n42_), .d(x00), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nand2  g15(.a(x10), .b(x08), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand3  g17(.a(new_n58_), .b(x15), .c(new_n48_), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(x00), .c(new_n56_), .O(z03));
  inv1   g19(.a(new_n44_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(x18), .O(z04));
  and2   g21(.a(new_n44_), .b(x19), .O(z05));
  nor2   g22(.a(new_n61_), .b(new_n51_), .O(z06));
  and2   g23(.a(new_n44_), .b(x17), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n44_), .b(new_n66_), .O(z08));
  nand2  g26(.a(new_n44_), .b(x09), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n47_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n61_), .O(z10));
  nand2  g29(.a(new_n44_), .b(new_n48_), .O(z11));
  inv1   g30(.a(x13), .O(new_n72_));
  nand3  g31(.a(new_n52_), .b(new_n72_), .c(x00), .O(new_n73_));
  and2   g32(.a(new_n73_), .b(x12), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n57_), .b(new_n75_), .c(x14), .O(new_n76_));
  nand3  g35(.a(x15), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x12), .c(x00), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z13));
  nand4  g39(.a(new_n51_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  oai21  g40(.a(new_n58_), .b(new_n51_), .c(new_n81_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x12), .c(x00), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z14));
  nand3  g43(.a(new_n52_), .b(x16), .c(new_n48_), .O(new_n85_));
  inv1   g44(.a(x10), .O(new_n86_));
  nor2   g45(.a(x15), .b(new_n86_), .O(new_n87_));
  and2   g46(.a(x08), .b(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n43_), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n85_), .c(new_n56_), .O(z15));
  nand2  g49(.a(new_n52_), .b(x17), .O(new_n91_));
  nand4  g50(.a(new_n51_), .b(x10), .c(x08), .d(x04), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(x12), .c(x00), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z16));
  nand2  g54(.a(new_n52_), .b(x18), .O(new_n96_));
  nand4  g55(.a(new_n51_), .b(x10), .c(x08), .d(x05), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(x12), .c(x00), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z17));
  nand2  g59(.a(new_n52_), .b(x19), .O(new_n101_));
  and2   g60(.a(x08), .b(x06), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n87_), .c(new_n43_), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n101_), .c(new_n56_), .O(z18));
  nand2  g63(.a(new_n52_), .b(x20), .O(new_n105_));
  and2   g64(.a(x08), .b(x07), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n87_), .c(new_n43_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n105_), .c(new_n56_), .O(z19));
endmodule


