// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n62_, new_n64_, new_n66_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x04), .O(new_n43_));
  inv1   g02(.a(x09), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  aoi21  g08(.a(new_n44_), .b(new_n43_), .c(new_n49_), .O(z06));
  nand3  g09(.a(z06), .b(new_n48_), .c(x10), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g11(.a(x11), .O(new_n53_));
  nand3  g12(.a(new_n49_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n45_), .c(x12), .d(new_n53_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  nand2  g16(.a(x10), .b(x08), .O(new_n58_));
  nand3  g17(.a(x15), .b(new_n48_), .c(x12), .O(new_n59_));
  oai21  g18(.a(new_n59_), .b(new_n58_), .c(new_n45_), .O(z03));
  nand2  g19(.a(new_n45_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n45_), .b(new_n66_), .O(z08));
  oai21  g26(.a(new_n44_), .b(new_n47_), .c(new_n45_), .O(z09));
  nand2  g27(.a(new_n44_), .b(x04), .O(new_n69_));
  oai21  g28(.a(new_n44_), .b(x08), .c(new_n69_), .O(z10));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(new_n48_), .O(z11));
  nand2  g30(.a(new_n54_), .b(x13), .O(new_n72_));
  inv1   g31(.a(x10), .O(new_n73_));
  nor2   g32(.a(x15), .b(new_n73_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x08), .c(x00), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n45_), .c(x12), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  inv1   g37(.a(new_n58_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(x01), .c(new_n48_), .O(new_n80_));
  nand3  g39(.a(x15), .b(x10), .c(x08), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n80_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n45_), .O(z13));
  nand2  g43(.a(new_n58_), .b(x15), .O(new_n85_));
  nand3  g44(.a(new_n74_), .b(x08), .c(x02), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n45_), .c(x12), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  nand3  g48(.a(new_n54_), .b(x16), .c(new_n48_), .O(new_n90_));
  nand3  g49(.a(new_n74_), .b(x08), .c(x03), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n45_), .c(x12), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z15));
  aoi21  g53(.a(new_n54_), .b(new_n64_), .c(new_n43_), .O(new_n95_));
  and2   g54(.a(new_n54_), .b(x17), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n95_), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n45_), .O(z16));
  nand2  g57(.a(new_n54_), .b(x18), .O(new_n99_));
  nand3  g58(.a(new_n74_), .b(x08), .c(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n45_), .c(x12), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z17));
  nand2  g62(.a(new_n54_), .b(x19), .O(new_n104_));
  nand3  g63(.a(new_n74_), .b(x08), .c(x06), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n45_), .c(x12), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z18));
  nand2  g67(.a(new_n54_), .b(x20), .O(new_n109_));
  nand3  g68(.a(new_n74_), .b(x08), .c(x07), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(new_n45_), .c(x12), .O(new_n112_));
  inv1   g71(.a(new_n112_), .O(z19));
endmodule


