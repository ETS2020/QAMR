// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n62_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x17), .b(x15), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor4   g06(.a(new_n47_), .b(x17), .c(new_n46_), .d(x14), .O(z01));
  inv1   g07(.a(x08), .O(new_n49_));
  inv1   g08(.a(x14), .O(new_n50_));
  nand2  g09(.a(new_n46_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n49_), .c(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n44_), .O(z02));
  inv1   g13(.a(x17), .O(new_n55_));
  inv1   g14(.a(new_n47_), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(new_n50_), .c(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n55_), .c(new_n46_), .O(z03));
  nand2  g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  oai21  g19(.a(new_n55_), .b(new_n46_), .c(new_n60_), .O(z05));
  nand2  g20(.a(new_n55_), .b(x15), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z06));
  nor2   g22(.a(new_n55_), .b(x15), .O(z07));
  and2   g23(.a(new_n44_), .b(x16), .O(z08));
  nand3  g24(.a(new_n44_), .b(x09), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z09));
  aoi22  g26(.a(x17), .b(x15), .c(x09), .d(x08), .O(z10));
  nand2  g27(.a(new_n44_), .b(new_n50_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  nand2  g29(.a(new_n47_), .b(new_n46_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n62_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x13), .O(new_n73_));
  inv1   g32(.a(new_n51_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x08), .c(x00), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n70_), .O(z12));
  nand3  g35(.a(x12), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x15), .O(new_n79_));
  nand2  g38(.a(new_n56_), .b(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x14), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n79_), .O(z13));
  nand2  g41(.a(new_n47_), .b(x15), .O(new_n83_));
  nand2  g42(.a(x08), .b(x02), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n51_), .c(new_n83_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n44_), .O(z14));
  nand3  g46(.a(new_n72_), .b(x16), .c(new_n50_), .O(new_n88_));
  nand3  g47(.a(new_n74_), .b(x08), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n70_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  aoi21  g50(.a(new_n47_), .b(new_n55_), .c(new_n91_), .O(new_n92_));
  aoi21  g51(.a(x10), .b(x08), .c(new_n55_), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n92_), .c(new_n46_), .O(new_n94_));
  nor2   g53(.a(new_n94_), .b(new_n70_), .O(z16));
  nand2  g54(.a(new_n47_), .b(x18), .O(new_n96_));
  nand3  g55(.a(x10), .b(x08), .c(x05), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand3  g58(.a(x18), .b(new_n55_), .c(x15), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n70_), .O(z17));
  oai21  g60(.a(new_n60_), .b(new_n70_), .c(new_n55_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x15), .O(new_n103_));
  nand2  g62(.a(new_n47_), .b(x19), .O(new_n104_));
  nand2  g63(.a(x08), .b(x06), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n51_), .c(new_n104_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n103_), .O(z18));
  oai21  g67(.a(new_n43_), .b(new_n70_), .c(new_n55_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x15), .O(new_n110_));
  nand2  g69(.a(new_n47_), .b(x20), .O(new_n111_));
  nand2  g70(.a(x08), .b(x07), .O(new_n112_));
  oai21  g71(.a(new_n112_), .b(new_n51_), .c(new_n111_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(x12), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n110_), .O(z19));
endmodule


