// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x15), .b(x14), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n45_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  aoi21  g09(.a(x10), .b(x08), .c(x14), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n50_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  inv1   g14(.a(x15), .O(new_n56_));
  nor4   g15(.a(new_n46_), .b(new_n56_), .c(x14), .d(new_n55_), .O(z03));
  inv1   g16(.a(new_n42_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n42_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n58_), .b(new_n62_), .O(z07));
  and2   g22(.a(new_n42_), .b(x16), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n42_), .O(z09));
  inv1   g25(.a(z09), .O(z10));
  inv1   g26(.a(x13), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n55_), .c(new_n45_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(x15), .O(new_n70_));
  nand4  g29(.a(new_n56_), .b(x10), .c(x08), .d(x00), .O(new_n71_));
  oai21  g30(.a(new_n47_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x12), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n70_), .O(z12));
  nand3  g33(.a(x12), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n45_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x15), .O(new_n77_));
  nand2  g36(.a(new_n47_), .b(x01), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x14), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n77_), .O(z13));
  nand3  g39(.a(new_n46_), .b(x15), .c(new_n45_), .O(new_n81_));
  nand4  g40(.a(new_n56_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n55_), .O(z14));
  nand2  g42(.a(x15), .b(new_n45_), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(x10), .c(x08), .d(x03), .O(new_n85_));
  oai21  g44(.a(new_n51_), .b(x15), .c(x16), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n42_), .O(z15));
  oai21  g48(.a(new_n62_), .b(new_n55_), .c(new_n45_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x15), .O(new_n91_));
  nand4  g50(.a(new_n56_), .b(x10), .c(x08), .d(x04), .O(new_n92_));
  oai21  g51(.a(new_n47_), .b(new_n62_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n91_), .O(z16));
  inv1   g54(.a(x05), .O(new_n96_));
  nand2  g55(.a(new_n46_), .b(x18), .O(new_n97_));
  oai21  g56(.a(new_n46_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n56_), .O(new_n99_));
  nand3  g58(.a(x18), .b(x15), .c(new_n45_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n55_), .O(z17));
  oai21  g60(.a(new_n60_), .b(new_n55_), .c(new_n45_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x15), .O(new_n103_));
  nand4  g62(.a(new_n56_), .b(x10), .c(x08), .d(x06), .O(new_n104_));
  oai21  g63(.a(new_n47_), .b(new_n60_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n103_), .O(z18));
  inv1   g66(.a(x07), .O(new_n108_));
  nand2  g67(.a(new_n46_), .b(x20), .O(new_n109_));
  oai21  g68(.a(new_n46_), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n56_), .O(new_n111_));
  nand3  g70(.a(x20), .b(x15), .c(new_n45_), .O(new_n112_));
  aoi21  g71(.a(new_n112_), .b(new_n111_), .c(new_n55_), .O(z19));
  buf    g72(.a(x15), .O(z06));
  buf    g73(.a(x14), .O(z11));
endmodule


