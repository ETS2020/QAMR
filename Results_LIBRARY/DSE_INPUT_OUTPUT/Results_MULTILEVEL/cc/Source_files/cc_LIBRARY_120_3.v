// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n59_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_;
  inv1   g00(.a(x15), .O(new_n42_));
  nand3  g01(.a(x20), .b(new_n42_), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x11), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand3  g05(.a(new_n46_), .b(x10), .c(x08), .O(new_n47_));
  aoi21  g06(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z01));
  inv1   g07(.a(x08), .O(new_n49_));
  nand2  g08(.a(new_n42_), .b(x10), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(new_n49_), .c(new_n46_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x12), .c(new_n45_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(z02));
  nand4  g12(.a(new_n46_), .b(x12), .c(x10), .d(x08), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n45_), .c(new_n42_), .O(z03));
  nand2  g14(.a(x15), .b(x11), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x18), .O(z04));
  and2   g16(.a(new_n56_), .b(x19), .O(z05));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n56_), .b(new_n59_), .O(z07));
  and2   g19(.a(new_n56_), .b(x16), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n56_), .O(z09));
  nand3  g22(.a(new_n56_), .b(x09), .c(x08), .O(z10));
  nand2  g23(.a(new_n56_), .b(new_n46_), .O(z11));
  inv1   g24(.a(x12), .O(new_n66_));
  nand2  g25(.a(x10), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  nand2  g27(.a(x15), .b(new_n45_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x13), .O(new_n71_));
  inv1   g30(.a(new_n50_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x08), .c(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(z12));
  nand2  g33(.a(new_n67_), .b(new_n46_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x15), .c(new_n45_), .O(new_n76_));
  nand3  g35(.a(x10), .b(x08), .c(x01), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n42_), .c(x14), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n76_), .c(new_n66_), .O(z13));
  nand3  g38(.a(new_n67_), .b(x15), .c(new_n45_), .O(new_n80_));
  nand3  g39(.a(new_n72_), .b(x08), .c(x02), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n66_), .O(z14));
  nand3  g41(.a(new_n70_), .b(x16), .c(new_n46_), .O(new_n83_));
  nand3  g42(.a(new_n72_), .b(x08), .c(x03), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n66_), .O(z15));
  oai21  g44(.a(new_n59_), .b(new_n66_), .c(new_n45_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x15), .O(new_n87_));
  nand2  g46(.a(new_n67_), .b(x17), .O(new_n88_));
  nand2  g47(.a(x08), .b(x04), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n50_), .c(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n87_), .O(z16));
  nand2  g51(.a(x18), .b(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n67_), .b(x18), .O(new_n96_));
  nand2  g55(.a(x08), .b(x05), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n50_), .c(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n95_), .O(z17));
  nand2  g59(.a(new_n67_), .b(x19), .O(new_n101_));
  nand3  g60(.a(x10), .b(x08), .c(x06), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n42_), .O(new_n104_));
  nand3  g63(.a(x19), .b(x15), .c(new_n45_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n66_), .O(z18));
  nand2  g65(.a(x20), .b(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n45_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x15), .O(new_n109_));
  nand2  g68(.a(new_n67_), .b(x20), .O(new_n110_));
  nand2  g69(.a(x08), .b(x07), .O(new_n111_));
  oai21  g70(.a(new_n111_), .b(new_n50_), .c(new_n110_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x12), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n109_), .O(z19));
  buf    g73(.a(x15), .O(z06));
endmodule


