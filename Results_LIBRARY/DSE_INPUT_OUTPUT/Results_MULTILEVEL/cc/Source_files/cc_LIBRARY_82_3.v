// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n58_, new_n61_, new_n63_,
    new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_;
  inv1   g00(.a(x12), .O(new_n42_));
  inv1   g01(.a(x18), .O(z04));
  nor2   g02(.a(z04), .b(new_n42_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand3  g04(.a(new_n45_), .b(x20), .c(x11), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand4  g07(.a(x15), .b(new_n48_), .c(x10), .d(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n45_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(z04), .c(x12), .d(new_n51_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  aoi21  g15(.a(new_n49_), .b(z04), .c(new_n42_), .O(z03));
  inv1   g16(.a(x19), .O(new_n58_));
  nor2   g17(.a(new_n44_), .b(new_n58_), .O(z05));
  nor2   g18(.a(new_n44_), .b(new_n52_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z08));
  nand3  g23(.a(new_n45_), .b(x09), .c(x08), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z09));
  aoi21  g25(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nor2   g26(.a(new_n44_), .b(new_n48_), .O(z11));
  nand2  g27(.a(new_n53_), .b(x13), .O(new_n69_));
  nand4  g28(.a(new_n52_), .b(x10), .c(x08), .d(x00), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(z04), .c(x12), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  inv1   g32(.a(x01), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n74_), .c(x14), .O(new_n76_));
  inv1   g35(.a(x10), .O(new_n77_));
  nor2   g36(.a(new_n52_), .b(new_n77_), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(x08), .c(x18), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n42_), .O(z13));
  nand2  g39(.a(new_n75_), .b(x15), .O(new_n81_));
  nand4  g40(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n81_), .c(z04), .O(new_n83_));
  and2   g42(.a(new_n83_), .b(x12), .O(z14));
  nand3  g43(.a(new_n53_), .b(x16), .c(new_n48_), .O(new_n85_));
  nor2   g44(.a(x15), .b(new_n77_), .O(new_n86_));
  and2   g45(.a(x08), .b(x03), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(x18), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n85_), .c(new_n42_), .O(z15));
  nand2  g48(.a(new_n53_), .b(x17), .O(new_n90_));
  and2   g49(.a(x08), .b(x04), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n86_), .c(x18), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n90_), .c(new_n42_), .O(z16));
  nand4  g52(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n94_));
  nor3   g53(.a(new_n94_), .b(x18), .c(x15), .O(z17));
  nand2  g54(.a(new_n53_), .b(x19), .O(new_n96_));
  nand4  g55(.a(new_n52_), .b(x10), .c(x08), .d(x06), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(z04), .c(x12), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z18));
  nand2  g59(.a(new_n53_), .b(x20), .O(new_n101_));
  nand4  g60(.a(new_n52_), .b(x10), .c(x08), .d(x07), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(z04), .c(x12), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z19));
endmodule


