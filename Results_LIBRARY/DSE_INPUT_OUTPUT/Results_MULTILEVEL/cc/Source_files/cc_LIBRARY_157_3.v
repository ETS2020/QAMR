// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x12), .O(new_n42_));
  nand3  g01(.a(x20), .b(new_n42_), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  nand2  g06(.a(x12), .b(x11), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n47_), .O(z06));
  nand3  g09(.a(z06), .b(new_n46_), .c(x10), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n45_), .O(z01));
  inv1   g11(.a(x11), .O(new_n53_));
  nand3  g12(.a(new_n47_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n53_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  nand2  g16(.a(x10), .b(x08), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x11), .O(new_n59_));
  nand3  g18(.a(new_n59_), .b(new_n46_), .c(x12), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n47_), .O(z03));
  nor2   g20(.a(new_n49_), .b(x18), .O(z04));
  and2   g21(.a(new_n48_), .b(x19), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n48_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n48_), .b(new_n66_), .O(z08));
  nand3  g26(.a(new_n48_), .b(x09), .c(x08), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n49_), .O(z10));
  nor2   g29(.a(new_n49_), .b(new_n46_), .O(z11));
  nand2  g30(.a(new_n54_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n47_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x12), .c(new_n53_), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  oai21  g36(.a(new_n58_), .b(new_n77_), .c(x14), .O(new_n78_));
  nand3  g37(.a(x15), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x12), .c(new_n53_), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z13));
  nand2  g41(.a(new_n58_), .b(x15), .O(new_n83_));
  nand4  g42(.a(new_n47_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(new_n83_), .c(new_n53_), .O(new_n85_));
  and2   g44(.a(new_n85_), .b(x12), .O(z14));
  nand3  g45(.a(new_n54_), .b(x16), .c(new_n46_), .O(new_n87_));
  inv1   g46(.a(x10), .O(new_n88_));
  nor2   g47(.a(x15), .b(new_n88_), .O(new_n89_));
  and2   g48(.a(x08), .b(x03), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(x11), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n87_), .c(new_n42_), .O(z15));
  nand2  g51(.a(new_n54_), .b(x17), .O(new_n93_));
  nand4  g52(.a(new_n47_), .b(x10), .c(x08), .d(x04), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(x12), .c(new_n53_), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z16));
  nand2  g56(.a(new_n54_), .b(x18), .O(new_n98_));
  nand4  g57(.a(new_n47_), .b(x10), .c(x08), .d(x05), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(x12), .c(new_n53_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z17));
  nand2  g61(.a(new_n54_), .b(x19), .O(new_n103_));
  nand4  g62(.a(new_n47_), .b(x10), .c(x08), .d(x06), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(x12), .c(new_n53_), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z18));
  nand2  g66(.a(new_n54_), .b(x20), .O(new_n108_));
  nand4  g67(.a(new_n47_), .b(x10), .c(x08), .d(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(x12), .c(new_n53_), .O(new_n111_));
  inv1   g70(.a(new_n111_), .O(z19));
endmodule


