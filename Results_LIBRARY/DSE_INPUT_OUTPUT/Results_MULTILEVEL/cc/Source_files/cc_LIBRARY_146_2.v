// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x16), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x16), .c(new_n44_), .O(z01));
  nand3  g08(.a(new_n44_), .b(x10), .c(x08), .O(new_n50_));
  oai21  g09(.a(new_n45_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x12), .c(new_n42_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(z02));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(x16), .c(x15), .d(new_n47_), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(z03));
  inv1   g18(.a(new_n45_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(x16), .b(x15), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(x15), .c(new_n62_), .O(z05));
  inv1   g23(.a(new_n63_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x08), .O(new_n68_));
  inv1   g27(.a(x09), .O(new_n69_));
  nor3   g28(.a(new_n45_), .b(new_n69_), .c(new_n68_), .O(z09));
  nand3  g29(.a(new_n60_), .b(x09), .c(x08), .O(z10));
  nand2  g30(.a(new_n60_), .b(new_n47_), .O(z11));
  inv1   g31(.a(x12), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x15), .O(new_n76_));
  nand4  g35(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n77_));
  oai21  g36(.a(new_n55_), .b(new_n74_), .c(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n76_), .O(z12));
  nand3  g39(.a(x12), .b(x10), .c(x08), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x15), .O(new_n83_));
  nand2  g42(.a(new_n55_), .b(x01), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x14), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n83_), .O(z13));
  nand3  g45(.a(new_n54_), .b(x16), .c(x15), .O(new_n87_));
  nand4  g46(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n73_), .O(z14));
  nand4  g48(.a(new_n63_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n54_), .b(x16), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n44_), .c(x14), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n91_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n60_), .O(z15));
  oai21  g54(.a(new_n66_), .b(new_n73_), .c(x16), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x15), .O(new_n97_));
  nand4  g56(.a(new_n44_), .b(x10), .c(x08), .d(x04), .O(new_n98_));
  oai21  g57(.a(new_n55_), .b(new_n66_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n97_), .O(z16));
  inv1   g60(.a(x18), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n73_), .c(x16), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x15), .O(new_n104_));
  nand4  g63(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n105_));
  oai21  g64(.a(new_n55_), .b(new_n102_), .c(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n104_), .O(z17));
  oai21  g67(.a(new_n62_), .b(new_n73_), .c(x16), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x15), .O(new_n110_));
  nand4  g69(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n111_));
  oai21  g70(.a(new_n55_), .b(new_n62_), .c(new_n111_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x12), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n110_), .O(z18));
  inv1   g73(.a(x07), .O(new_n115_));
  nand2  g74(.a(new_n54_), .b(x20), .O(new_n116_));
  oai21  g75(.a(new_n54_), .b(new_n115_), .c(new_n116_), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n44_), .O(new_n118_));
  nand3  g77(.a(x20), .b(x16), .c(x15), .O(new_n119_));
  aoi21  g78(.a(new_n119_), .b(new_n118_), .c(new_n73_), .O(z19));
  buf    g79(.a(x16), .O(z08));
endmodule


