// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_;
  nor2   g00(.a(x20), .b(x15), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n46_), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  oai21  g10(.a(x20), .b(x15), .c(x14), .O(new_n52_));
  inv1   g11(.a(x15), .O(new_n53_));
  nand2  g12(.a(x20), .b(new_n53_), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(new_n47_), .c(new_n52_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n51_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  nand3  g16(.a(x15), .b(new_n46_), .c(x12), .O(new_n58_));
  oai21  g17(.a(new_n58_), .b(new_n47_), .c(new_n43_), .O(z03));
  nand2  g18(.a(new_n43_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z05));
  nand2  g21(.a(x20), .b(new_n53_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n42_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n42_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x08), .O(new_n68_));
  inv1   g27(.a(x09), .O(new_n69_));
  nor3   g28(.a(new_n42_), .b(new_n69_), .c(new_n68_), .O(z09));
  aoi21  g29(.a(x09), .b(x08), .c(new_n42_), .O(z10));
  nand2  g30(.a(new_n43_), .b(new_n46_), .O(z11));
  nand4  g31(.a(x12), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x20), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  nand3  g34(.a(new_n53_), .b(x10), .c(x08), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x13), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(z12));
  inv1   g37(.a(x12), .O(new_n79_));
  oai21  g38(.a(new_n48_), .b(x14), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n48_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x20), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(z13));
  nand2  g42(.a(new_n47_), .b(x15), .O(new_n84_));
  nand3  g43(.a(x20), .b(new_n53_), .c(x10), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x08), .c(x02), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n84_), .c(new_n79_), .O(z14));
  nand2  g47(.a(new_n47_), .b(x20), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n53_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(x16), .c(new_n46_), .O(new_n91_));
  nand3  g50(.a(new_n86_), .b(x08), .c(x03), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n79_), .O(z15));
  nand2  g52(.a(new_n90_), .b(x17), .O(new_n94_));
  nand3  g53(.a(new_n86_), .b(x08), .c(x04), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n79_), .O(z16));
  nand2  g55(.a(new_n90_), .b(x18), .O(new_n97_));
  nand3  g56(.a(new_n86_), .b(x08), .c(x05), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n79_), .O(z17));
  nand2  g58(.a(new_n90_), .b(x19), .O(new_n100_));
  nand3  g59(.a(new_n86_), .b(x08), .c(x06), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n79_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand4  g62(.a(new_n53_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(x20), .c(x12), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z19));
endmodule


