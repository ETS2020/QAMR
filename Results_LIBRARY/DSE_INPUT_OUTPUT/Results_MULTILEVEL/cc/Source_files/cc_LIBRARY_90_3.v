// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n61_, new_n64_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x15), .O(new_n42_));
  nand3  g01(.a(x20), .b(new_n42_), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  nand2  g03(.a(x10), .b(x08), .O(new_n45_));
  nor4   g04(.a(new_n45_), .b(x20), .c(new_n42_), .d(x14), .O(z01));
  inv1   g05(.a(x11), .O(new_n47_));
  inv1   g06(.a(x08), .O(new_n48_));
  nand2  g07(.a(x20), .b(x15), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(x14), .O(new_n50_));
  nand2  g09(.a(new_n42_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n48_), .c(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n47_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(x20), .O(new_n55_));
  inv1   g14(.a(x14), .O(new_n56_));
  inv1   g15(.a(new_n45_), .O(new_n57_));
  nand3  g16(.a(new_n57_), .b(new_n56_), .c(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n55_), .c(new_n42_), .O(z03));
  aoi21  g18(.a(x20), .b(x15), .c(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  aoi21  g20(.a(x20), .b(x15), .c(new_n61_), .O(z05));
  nor2   g21(.a(x20), .b(new_n42_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n49_), .b(new_n64_), .O(z07));
  and2   g24(.a(new_n49_), .b(x16), .O(z08));
  nand3  g25(.a(new_n49_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  nand3  g27(.a(new_n49_), .b(x09), .c(x08), .O(z10));
  inv1   g28(.a(new_n50_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  aoi21  g30(.a(x10), .b(x08), .c(x15), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(z06), .c(x13), .O(new_n73_));
  inv1   g32(.a(new_n51_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x08), .c(x00), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(z12));
  nand3  g35(.a(x12), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x15), .O(new_n79_));
  nand2  g38(.a(new_n57_), .b(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x14), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n79_), .O(z13));
  nand3  g41(.a(new_n45_), .b(new_n55_), .c(x15), .O(new_n83_));
  nand3  g42(.a(new_n74_), .b(x08), .c(x02), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n71_), .O(z14));
  nand3  g44(.a(x16), .b(new_n56_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n55_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand3  g47(.a(new_n45_), .b(x16), .c(new_n56_), .O(new_n89_));
  nand4  g48(.a(new_n42_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n88_), .O(z15));
  nand2  g52(.a(new_n45_), .b(x17), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(x04), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n42_), .O(new_n97_));
  nand3  g56(.a(new_n55_), .b(x17), .c(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n71_), .O(z16));
  nand2  g58(.a(x18), .b(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n55_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x15), .O(new_n102_));
  nand2  g61(.a(new_n45_), .b(x18), .O(new_n103_));
  nand2  g62(.a(x08), .b(x05), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n51_), .c(new_n103_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n102_), .O(z17));
  oai21  g66(.a(new_n61_), .b(new_n71_), .c(new_n55_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x15), .O(new_n109_));
  nand2  g68(.a(new_n45_), .b(x19), .O(new_n110_));
  nand2  g69(.a(x08), .b(x06), .O(new_n111_));
  oai21  g70(.a(new_n111_), .b(new_n51_), .c(new_n110_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x12), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n109_), .O(z18));
  inv1   g73(.a(x07), .O(new_n115_));
  aoi21  g74(.a(new_n45_), .b(new_n55_), .c(new_n115_), .O(new_n116_));
  aoi21  g75(.a(x10), .b(x08), .c(new_n55_), .O(new_n117_));
  oai21  g76(.a(new_n117_), .b(new_n116_), .c(new_n42_), .O(new_n118_));
  nor2   g77(.a(new_n118_), .b(new_n71_), .O(z19));
endmodule


