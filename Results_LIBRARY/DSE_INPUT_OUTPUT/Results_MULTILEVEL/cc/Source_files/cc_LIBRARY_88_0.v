// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x10), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand4  g05(.a(x15), .b(new_n46_), .c(x10), .d(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(z01));
  inv1   g07(.a(x10), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(x08), .c(x14), .O(new_n51_));
  nand2  g10(.a(x15), .b(x14), .O(new_n52_));
  oai21  g11(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(new_n44_), .O(new_n56_));
  nand2  g15(.a(x10), .b(x08), .O(new_n57_));
  nand3  g16(.a(x15), .b(new_n46_), .c(x12), .O(new_n58_));
  oai21  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(z03));
  nand2  g18(.a(new_n56_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n56_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n56_), .b(new_n65_), .O(z08));
  aoi21  g25(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  nor2   g27(.a(new_n44_), .b(new_n46_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  inv1   g29(.a(x00), .O(new_n71_));
  nand2  g30(.a(new_n50_), .b(x08), .O(new_n72_));
  inv1   g31(.a(x08), .O(new_n73_));
  nand2  g32(.a(x13), .b(new_n73_), .O(new_n74_));
  oai21  g33(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x10), .O(new_n76_));
  nand2  g35(.a(x15), .b(x13), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n70_), .O(z12));
  aoi21  g37(.a(x14), .b(x12), .c(new_n50_), .O(new_n79_));
  aoi21  g38(.a(new_n57_), .b(new_n46_), .c(new_n50_), .O(new_n80_));
  aoi21  g39(.a(x08), .b(x01), .c(new_n46_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n80_), .c(x12), .O(new_n82_));
  oai21  g41(.a(new_n79_), .b(x10), .c(new_n82_), .O(z13));
  nand2  g42(.a(new_n57_), .b(x15), .O(new_n84_));
  nand4  g43(.a(new_n50_), .b(x10), .c(x08), .d(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n70_), .O(z14));
  nand3  g45(.a(new_n50_), .b(x08), .c(x03), .O(new_n87_));
  nand3  g46(.a(x16), .b(new_n46_), .c(new_n73_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x10), .O(new_n90_));
  nand3  g49(.a(x16), .b(x15), .c(new_n46_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n70_), .O(z15));
  inv1   g51(.a(x04), .O(new_n93_));
  nand2  g52(.a(x17), .b(new_n73_), .O(new_n94_));
  oai21  g53(.a(new_n72_), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x10), .O(new_n96_));
  nand2  g55(.a(x17), .b(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n70_), .O(z16));
  nand3  g57(.a(x12), .b(x08), .c(x05), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x10), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n50_), .O(new_n101_));
  nand3  g60(.a(new_n72_), .b(x18), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(z17));
  inv1   g62(.a(x06), .O(new_n104_));
  nand2  g63(.a(x19), .b(new_n73_), .O(new_n105_));
  oai21  g64(.a(new_n72_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x10), .O(new_n107_));
  nand2  g66(.a(x19), .b(x15), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n70_), .O(z18));
  inv1   g68(.a(x07), .O(new_n110_));
  nand2  g69(.a(x20), .b(new_n73_), .O(new_n111_));
  oai21  g70(.a(new_n72_), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x10), .O(new_n113_));
  nand2  g72(.a(x20), .b(x15), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n70_), .O(z19));
  buf    g74(.a(x15), .O(z06));
endmodule


