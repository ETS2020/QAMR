// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n62_, new_n64_, new_n66_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x09), .b(x04), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  inv1   g06(.a(new_n43_), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n47_), .O(z06));
  nand3  g08(.a(z06), .b(new_n46_), .c(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n45_), .O(z01));
  inv1   g10(.a(x11), .O(new_n52_));
  nand3  g11(.a(new_n47_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n43_), .c(x12), .d(new_n52_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  and2   g15(.a(x10), .b(x08), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand3  g17(.a(x15), .b(new_n46_), .c(x12), .O(new_n59_));
  oai21  g18(.a(new_n59_), .b(new_n58_), .c(new_n43_), .O(z03));
  nand2  g19(.a(new_n43_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n43_), .b(new_n66_), .O(z08));
  oai21  g26(.a(x08), .b(x04), .c(x09), .O(z10));
  inv1   g27(.a(z10), .O(z09));
  nor2   g28(.a(new_n48_), .b(new_n46_), .O(z11));
  nand2  g29(.a(new_n53_), .b(x13), .O(new_n71_));
  nand4  g30(.a(new_n47_), .b(x10), .c(x08), .d(x00), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n43_), .c(x12), .O(new_n74_));
  inv1   g33(.a(new_n74_), .O(z12));
  aoi21  g34(.a(new_n57_), .b(x01), .c(new_n46_), .O(new_n76_));
  nand3  g35(.a(x15), .b(x10), .c(x08), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n76_), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n43_), .O(z13));
  nand4  g39(.a(new_n47_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  oai21  g40(.a(new_n57_), .b(new_n47_), .c(new_n81_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n43_), .c(x12), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z14));
  nand3  g43(.a(new_n53_), .b(x16), .c(new_n46_), .O(new_n85_));
  nand4  g44(.a(new_n47_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n43_), .c(x12), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z15));
  nand4  g48(.a(new_n47_), .b(x12), .c(x10), .d(x08), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(x09), .c(x04), .O(new_n92_));
  nand3  g51(.a(new_n53_), .b(x17), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z16));
  nand2  g53(.a(new_n53_), .b(x18), .O(new_n95_));
  nand4  g54(.a(new_n47_), .b(x10), .c(x08), .d(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n43_), .c(x12), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z17));
  nand2  g58(.a(new_n53_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n47_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(new_n43_), .c(x12), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z18));
  nand2  g63(.a(new_n53_), .b(x20), .O(new_n105_));
  nand4  g64(.a(new_n47_), .b(x10), .c(x08), .d(x07), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(new_n43_), .c(x12), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z19));
endmodule


