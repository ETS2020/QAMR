// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n61_, new_n63_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x16), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x12), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand4  g05(.a(x15), .b(new_n46_), .c(x10), .d(x08), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n44_), .O(z01));
  inv1   g07(.a(x11), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(new_n46_), .c(new_n43_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  aoi21  g14(.a(new_n47_), .b(x16), .c(new_n55_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z05));
  nand2  g18(.a(new_n44_), .b(new_n50_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nand2  g20(.a(new_n44_), .b(new_n61_), .O(z07));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n44_), .O(z09));
  inv1   g23(.a(z09), .O(z10));
  nand2  g24(.a(new_n44_), .b(new_n46_), .O(z11));
  nand2  g25(.a(new_n51_), .b(x13), .O(new_n67_));
  inv1   g26(.a(x10), .O(new_n68_));
  nor2   g27(.a(x15), .b(new_n68_), .O(new_n69_));
  and2   g28(.a(x08), .b(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n43_), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n67_), .c(new_n55_), .O(z12));
  inv1   g31(.a(x01), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(x14), .O(new_n75_));
  nand3  g34(.a(x15), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x16), .c(x12), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  nand2  g38(.a(new_n74_), .b(x15), .O(new_n80_));
  nand4  g39(.a(new_n50_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  and2   g41(.a(new_n82_), .b(x12), .O(z14));
  nand2  g42(.a(new_n51_), .b(x14), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n52_), .c(new_n55_), .O(z15));
  nand2  g45(.a(new_n51_), .b(x17), .O(new_n87_));
  nand4  g46(.a(new_n50_), .b(x10), .c(x08), .d(x04), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(x16), .c(x12), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z16));
  nand2  g50(.a(new_n51_), .b(x18), .O(new_n92_));
  nand4  g51(.a(new_n50_), .b(x10), .c(x08), .d(x05), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(x16), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z17));
  nand2  g55(.a(new_n51_), .b(x19), .O(new_n97_));
  and2   g56(.a(x08), .b(x06), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n69_), .c(new_n43_), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n97_), .c(new_n55_), .O(z18));
  nand2  g59(.a(new_n51_), .b(x20), .O(new_n101_));
  and2   g60(.a(x08), .b(x07), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n69_), .c(new_n43_), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n101_), .c(new_n55_), .O(z19));
  buf    g63(.a(x16), .O(z08));
endmodule


