// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x08), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x10), .c(new_n44_), .O(z01));
  inv1   g09(.a(x10), .O(new_n51_));
  aoi21  g10(.a(new_n44_), .b(x08), .c(x14), .O(new_n52_));
  nand2  g11(.a(new_n44_), .b(x14), .O(new_n53_));
  oai21  g12(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nand2  g16(.a(x10), .b(x08), .O(new_n58_));
  nor4   g17(.a(new_n58_), .b(new_n44_), .c(x14), .d(new_n57_), .O(z03));
  nand2  g18(.a(new_n46_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n46_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n46_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n45_), .b(new_n68_), .c(new_n67_), .O(z09));
  nand3  g28(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n46_), .b(new_n48_), .O(z11));
  inv1   g30(.a(x13), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n57_), .c(new_n44_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n51_), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n67_), .O(new_n75_));
  nand4  g34(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n76_));
  oai21  g35(.a(new_n75_), .b(new_n72_), .c(new_n76_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n74_), .O(z12));
  aoi21  g38(.a(x08), .b(x01), .c(new_n48_), .O(new_n80_));
  nor2   g39(.a(new_n44_), .b(new_n67_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n80_), .c(x10), .O(new_n82_));
  nand3  g41(.a(new_n44_), .b(x14), .c(new_n51_), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n57_), .O(z13));
  nand4  g43(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n85_));
  oai21  g44(.a(new_n44_), .b(x08), .c(new_n85_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n46_), .O(z14));
  nand3  g47(.a(x10), .b(x08), .c(x03), .O(new_n89_));
  nand3  g48(.a(x16), .b(new_n48_), .c(new_n51_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n44_), .O(new_n92_));
  nand2  g51(.a(new_n44_), .b(x08), .O(new_n93_));
  nand4  g52(.a(new_n93_), .b(x16), .c(new_n48_), .d(x10), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n92_), .c(new_n57_), .O(z15));
  oai21  g54(.a(new_n63_), .b(new_n57_), .c(new_n44_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n51_), .O(new_n97_));
  nand4  g56(.a(new_n44_), .b(x10), .c(x08), .d(x04), .O(new_n98_));
  oai21  g57(.a(new_n75_), .b(new_n63_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n97_), .O(z16));
  inv1   g60(.a(x05), .O(new_n102_));
  inv1   g61(.a(x18), .O(new_n103_));
  oai22  g62(.a(new_n58_), .b(new_n102_), .c(new_n103_), .d(x10), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n44_), .O(new_n105_));
  nand3  g64(.a(new_n93_), .b(x18), .c(x10), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n105_), .c(new_n57_), .O(z17));
  oai21  g66(.a(new_n61_), .b(new_n57_), .c(new_n44_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n51_), .O(new_n109_));
  nand4  g68(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n110_));
  oai21  g69(.a(new_n75_), .b(new_n61_), .c(new_n110_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n109_), .O(z18));
  inv1   g72(.a(x07), .O(new_n114_));
  oai22  g73(.a(new_n58_), .b(new_n114_), .c(new_n43_), .d(x10), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n44_), .O(new_n116_));
  nand3  g75(.a(new_n93_), .b(x20), .c(x10), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n116_), .c(new_n57_), .O(z19));
  buf    g77(.a(x15), .O(z06));
endmodule


