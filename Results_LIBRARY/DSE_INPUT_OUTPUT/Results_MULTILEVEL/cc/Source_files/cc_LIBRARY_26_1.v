// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand3  g05(.a(x10), .b(x08), .c(x06), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n43_), .c(x14), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(new_n43_), .b(x10), .c(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x12), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x11), .c(new_n45_), .O(z02));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n49_), .c(x12), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(x06), .c(new_n43_), .O(z03));
  nor2   g16(.a(new_n44_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  oai21  g18(.a(new_n43_), .b(x06), .c(new_n59_), .O(z05));
  inv1   g19(.a(x06), .O(new_n61_));
  nor2   g20(.a(new_n43_), .b(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n44_), .b(new_n68_), .c(new_n67_), .O(z09));
  nand3  g28(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n44_), .b(new_n49_), .O(z11));
  inv1   g30(.a(x12), .O(new_n72_));
  nor2   g31(.a(new_n55_), .b(x15), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(z06), .c(x13), .O(new_n74_));
  inv1   g33(.a(x10), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(z12));
  nand3  g37(.a(x12), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x15), .O(new_n81_));
  nand2  g40(.a(new_n55_), .b(x01), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x14), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n81_), .O(z13));
  nand3  g43(.a(new_n54_), .b(x15), .c(x06), .O(new_n85_));
  nand3  g44(.a(new_n76_), .b(x08), .c(x02), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n72_), .O(z14));
  nand3  g46(.a(x16), .b(new_n49_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x06), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand3  g49(.a(new_n54_), .b(x16), .c(new_n49_), .O(new_n91_));
  nand3  g50(.a(new_n76_), .b(x08), .c(x03), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  aoi21  g54(.a(x17), .b(x12), .c(new_n61_), .O(new_n96_));
  and2   g55(.a(x08), .b(x04), .O(new_n97_));
  aoi22  g56(.a(new_n97_), .b(new_n76_), .c(new_n54_), .d(x17), .O(new_n98_));
  oai22  g57(.a(new_n98_), .b(new_n72_), .c(new_n96_), .d(new_n43_), .O(z16));
  aoi21  g58(.a(x18), .b(x12), .c(new_n61_), .O(new_n100_));
  and2   g59(.a(x08), .b(x05), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n76_), .c(new_n54_), .d(x18), .O(new_n102_));
  oai22  g61(.a(new_n102_), .b(new_n72_), .c(new_n100_), .d(new_n43_), .O(z17));
  aoi21  g62(.a(new_n50_), .b(new_n59_), .c(new_n61_), .O(new_n104_));
  and2   g63(.a(new_n50_), .b(x19), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n104_), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n45_), .O(z18));
  inv1   g66(.a(x07), .O(new_n108_));
  nand2  g67(.a(new_n54_), .b(x20), .O(new_n109_));
  oai21  g68(.a(new_n54_), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n43_), .O(new_n111_));
  nand3  g70(.a(x20), .b(x15), .c(x06), .O(new_n112_));
  aoi21  g71(.a(new_n112_), .b(new_n111_), .c(new_n72_), .O(z19));
endmodule


