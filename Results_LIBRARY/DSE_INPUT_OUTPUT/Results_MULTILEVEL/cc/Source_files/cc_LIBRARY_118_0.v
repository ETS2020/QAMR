// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x14), .b(x13), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  aoi21  g05(.a(new_n46_), .b(x13), .c(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x13), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(new_n49_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x13), .c(x12), .O(new_n56_));
  nor3   g15(.a(new_n56_), .b(new_n50_), .c(x14), .O(z03));
  nor2   g16(.a(new_n44_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  inv1   g18(.a(new_n44_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n59_), .O(z05));
  nor2   g20(.a(new_n44_), .b(new_n50_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n60_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n44_), .b(new_n68_), .c(new_n67_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g29(.a(x12), .O(new_n71_));
  inv1   g30(.a(x13), .O(new_n72_));
  nand2  g31(.a(new_n50_), .b(x14), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n49_), .c(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x00), .O(new_n75_));
  nand3  g34(.a(new_n50_), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x13), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n71_), .O(z12));
  nand3  g37(.a(x13), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n48_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x15), .O(new_n81_));
  inv1   g40(.a(x01), .O(new_n82_));
  oai21  g41(.a(new_n49_), .b(new_n82_), .c(x14), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n81_), .c(new_n71_), .O(z13));
  nand4  g43(.a(new_n50_), .b(x10), .c(x08), .d(x02), .O(new_n85_));
  oai21  g44(.a(new_n55_), .b(new_n50_), .c(new_n85_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n60_), .c(x12), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z14));
  nand3  g47(.a(new_n76_), .b(x16), .c(new_n48_), .O(new_n89_));
  nand4  g48(.a(new_n50_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n60_), .O(z15));
  inv1   g52(.a(x10), .O(new_n94_));
  nor2   g53(.a(x15), .b(new_n94_), .O(new_n95_));
  and2   g54(.a(x08), .b(x04), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n95_), .c(new_n76_), .d(x17), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n71_), .c(new_n60_), .O(z16));
  and2   g57(.a(x08), .b(x05), .O(new_n99_));
  aoi22  g58(.a(new_n99_), .b(new_n95_), .c(new_n76_), .d(x18), .O(new_n100_));
  oai21  g59(.a(new_n100_), .b(new_n71_), .c(new_n60_), .O(z17));
  nand2  g60(.a(new_n76_), .b(x19), .O(new_n102_));
  nand4  g61(.a(new_n50_), .b(x10), .c(x08), .d(x06), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(new_n60_), .c(x12), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z18));
  nand2  g65(.a(new_n76_), .b(x20), .O(new_n107_));
  nand4  g66(.a(new_n50_), .b(x10), .c(x08), .d(x07), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n60_), .c(x12), .O(new_n110_));
  inv1   g69(.a(new_n110_), .O(z19));
  buf    g70(.a(x14), .O(z11));
endmodule


