// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  aoi21  g02(.a(x18), .b(new_n42_), .c(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  nor2   g06(.a(new_n43_), .b(x18), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n47_), .O(z06));
  nand3  g08(.a(z06), .b(new_n46_), .c(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n45_), .O(z01));
  inv1   g10(.a(new_n48_), .O(new_n52_));
  nand3  g11(.a(new_n47_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n52_), .c(x12), .d(new_n42_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand4  g15(.a(z06), .b(new_n46_), .c(x12), .d(x10), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n45_), .O(z03));
  inv1   g17(.a(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n52_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n48_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n52_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n52_), .O(z09));
  nand3  g26(.a(new_n52_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n52_), .b(new_n46_), .O(z11));
  nand2  g28(.a(new_n53_), .b(x13), .O(new_n70_));
  inv1   g29(.a(x10), .O(new_n71_));
  nor2   g30(.a(x15), .b(new_n71_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x08), .c(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n52_), .c(x12), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  nand2  g36(.a(x10), .b(x08), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n77_), .c(x14), .O(new_n79_));
  nand3  g38(.a(x15), .b(x10), .c(x08), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n52_), .c(x12), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z13));
  nand2  g42(.a(new_n78_), .b(x15), .O(new_n84_));
  nand3  g43(.a(new_n72_), .b(x08), .c(x02), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n52_), .c(x12), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z14));
  nand3  g47(.a(new_n53_), .b(x16), .c(new_n46_), .O(new_n89_));
  nand3  g48(.a(new_n72_), .b(x08), .c(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n52_), .c(x12), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z15));
  inv1   g52(.a(x12), .O(new_n94_));
  and2   g53(.a(x08), .b(x04), .O(new_n95_));
  aoi22  g54(.a(new_n95_), .b(new_n72_), .c(new_n53_), .d(x17), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n94_), .c(new_n52_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  aoi21  g57(.a(new_n53_), .b(z04), .c(new_n98_), .O(new_n99_));
  and2   g58(.a(new_n53_), .b(x18), .O(new_n100_));
  oai21  g59(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n52_), .O(z17));
  and2   g61(.a(x08), .b(x06), .O(new_n103_));
  aoi22  g62(.a(new_n103_), .b(new_n72_), .c(new_n53_), .d(x19), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n94_), .c(new_n52_), .O(z18));
  nand2  g64(.a(new_n43_), .b(new_n47_), .O(new_n106_));
  oai22  g65(.a(new_n106_), .b(new_n78_), .c(new_n43_), .d(z04), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x07), .O(new_n108_));
  nand3  g67(.a(new_n53_), .b(x20), .c(x18), .O(new_n109_));
  aoi21  g68(.a(new_n109_), .b(new_n108_), .c(new_n94_), .O(z19));
endmodule


