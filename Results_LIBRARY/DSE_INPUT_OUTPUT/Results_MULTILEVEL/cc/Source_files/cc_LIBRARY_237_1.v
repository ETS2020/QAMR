// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x05), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g05(.a(x10), .b(x08), .c(x05), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n44_), .c(x14), .O(z01));
  inv1   g07(.a(new_n45_), .O(new_n49_));
  inv1   g08(.a(x14), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n49_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor4   g14(.a(new_n47_), .b(new_n44_), .c(x14), .d(new_n55_), .O(z03));
  nor2   g15(.a(new_n45_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  oai21  g17(.a(new_n44_), .b(x05), .c(new_n58_), .O(z05));
  inv1   g18(.a(x05), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n49_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g24(.a(x08), .O(new_n66_));
  inv1   g25(.a(x09), .O(new_n67_));
  nor3   g26(.a(new_n45_), .b(new_n67_), .c(new_n66_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n45_), .O(z10));
  nand2  g28(.a(new_n49_), .b(new_n50_), .O(z11));
  aoi21  g29(.a(x13), .b(x12), .c(new_n60_), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  and2   g33(.a(x08), .b(x00), .O(new_n75_));
  aoi22  g34(.a(new_n75_), .b(new_n73_), .c(new_n74_), .d(x13), .O(new_n76_));
  oai22  g35(.a(new_n76_), .b(new_n55_), .c(new_n71_), .d(new_n44_), .O(z12));
  nand2  g36(.a(new_n74_), .b(new_n50_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x15), .c(x05), .O(new_n79_));
  nand3  g38(.a(x10), .b(x08), .c(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n44_), .c(x14), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n79_), .c(new_n55_), .O(z13));
  and2   g41(.a(x08), .b(x02), .O(new_n83_));
  aoi22  g42(.a(new_n83_), .b(new_n73_), .c(new_n74_), .d(x15), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n55_), .c(new_n49_), .O(z14));
  nand3  g44(.a(x16), .b(new_n50_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x05), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand3  g47(.a(new_n74_), .b(x16), .c(new_n50_), .O(new_n89_));
  nand3  g48(.a(new_n73_), .b(x08), .c(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n88_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand2  g53(.a(new_n74_), .b(x17), .O(new_n95_));
  oai21  g54(.a(new_n74_), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n44_), .O(new_n97_));
  nand3  g56(.a(x17), .b(x15), .c(x05), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n55_), .O(z16));
  inv1   g58(.a(x18), .O(new_n100_));
  aoi21  g59(.a(new_n51_), .b(new_n100_), .c(new_n60_), .O(new_n101_));
  and2   g60(.a(new_n51_), .b(x18), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n101_), .c(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n49_), .O(z17));
  aoi21  g63(.a(x19), .b(x12), .c(new_n60_), .O(new_n105_));
  and2   g64(.a(x08), .b(x06), .O(new_n106_));
  aoi22  g65(.a(new_n106_), .b(new_n73_), .c(new_n74_), .d(x19), .O(new_n107_));
  oai22  g66(.a(new_n107_), .b(new_n55_), .c(new_n105_), .d(new_n44_), .O(z18));
  aoi21  g67(.a(x20), .b(x12), .c(new_n60_), .O(new_n109_));
  and2   g68(.a(x08), .b(x07), .O(new_n110_));
  aoi22  g69(.a(new_n110_), .b(new_n73_), .c(new_n74_), .d(x20), .O(new_n111_));
  oai22  g70(.a(new_n111_), .b(new_n55_), .c(new_n109_), .d(new_n44_), .O(z19));
endmodule


