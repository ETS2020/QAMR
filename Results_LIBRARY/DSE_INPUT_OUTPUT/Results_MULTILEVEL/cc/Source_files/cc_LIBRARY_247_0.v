// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x08), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x15), .b(new_n48_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  aoi21  g10(.a(new_n44_), .b(x10), .c(x14), .O(new_n52_));
  nand2  g11(.a(x15), .b(x14), .O(new_n53_));
  oai21  g12(.a(new_n52_), .b(new_n43_), .c(new_n53_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n51_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nor4   g16(.a(new_n47_), .b(new_n44_), .c(x14), .d(new_n57_), .O(z03));
  aoi21  g17(.a(new_n44_), .b(new_n43_), .c(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z05));
  nand2  g20(.a(new_n44_), .b(x08), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  nand2  g27(.a(x15), .b(new_n43_), .O(new_n69_));
  oai21  g28(.a(x09), .b(new_n43_), .c(new_n69_), .O(z10));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(new_n48_), .O(z11));
  inv1   g30(.a(x00), .O(new_n72_));
  nand2  g31(.a(new_n44_), .b(x10), .O(new_n73_));
  inv1   g32(.a(x10), .O(new_n74_));
  nand2  g33(.a(x13), .b(new_n74_), .O(new_n75_));
  oai21  g34(.a(new_n73_), .b(new_n72_), .c(new_n75_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g36(.a(x15), .b(x13), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n77_), .c(new_n57_), .O(z12));
  aoi21  g38(.a(x14), .b(x12), .c(new_n44_), .O(new_n80_));
  aoi21  g39(.a(new_n47_), .b(new_n48_), .c(new_n44_), .O(new_n81_));
  aoi21  g40(.a(x10), .b(x01), .c(new_n48_), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n81_), .c(x12), .O(new_n83_));
  oai21  g42(.a(new_n80_), .b(x08), .c(new_n83_), .O(z13));
  oai21  g43(.a(new_n44_), .b(x12), .c(new_n43_), .O(new_n85_));
  inv1   g44(.a(x02), .O(new_n86_));
  nand2  g45(.a(x15), .b(new_n74_), .O(new_n87_));
  oai21  g46(.a(new_n73_), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n85_), .O(z14));
  nand3  g49(.a(new_n44_), .b(x10), .c(x03), .O(new_n91_));
  nand3  g50(.a(x16), .b(new_n48_), .c(new_n74_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x08), .O(new_n94_));
  nand3  g53(.a(x16), .b(x15), .c(new_n48_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n57_), .O(z15));
  inv1   g55(.a(x04), .O(new_n97_));
  nand2  g56(.a(x17), .b(new_n74_), .O(new_n98_));
  oai21  g57(.a(new_n73_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x08), .O(new_n100_));
  nand2  g59(.a(x17), .b(x15), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n57_), .O(z16));
  inv1   g61(.a(x05), .O(new_n103_));
  nand2  g62(.a(x18), .b(new_n74_), .O(new_n104_));
  oai21  g63(.a(new_n73_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x08), .O(new_n106_));
  nand2  g65(.a(x18), .b(x15), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n57_), .O(z17));
  nand3  g67(.a(x12), .b(x10), .c(x06), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x08), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n44_), .O(new_n111_));
  nand3  g70(.a(new_n73_), .b(x19), .c(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n111_), .O(z18));
  nand3  g72(.a(x12), .b(x10), .c(x07), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x08), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n44_), .O(new_n116_));
  nand3  g75(.a(new_n73_), .b(x20), .c(x12), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n116_), .O(z19));
endmodule


