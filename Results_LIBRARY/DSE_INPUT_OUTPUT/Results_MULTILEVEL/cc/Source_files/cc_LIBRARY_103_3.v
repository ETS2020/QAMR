// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n57_, new_n59_, new_n62_, new_n64_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x15), .b(x14), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  aoi21  g04(.a(x10), .b(x08), .c(x14), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(new_n45_), .O(z01));
  inv1   g06(.a(x08), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(new_n45_), .b(x10), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(new_n48_), .c(new_n49_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x12), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x11), .c(new_n42_), .O(z02));
  nand3  g12(.a(x12), .b(x10), .c(x08), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z03));
  aoi21  g14(.a(x15), .b(x14), .c(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  aoi21  g16(.a(x15), .b(x14), .c(new_n57_), .O(z05));
  nand2  g17(.a(x15), .b(new_n49_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z06));
  and2   g19(.a(new_n42_), .b(x17), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n42_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n42_), .O(z09));
  nand3  g24(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  inv1   g25(.a(x12), .O(new_n67_));
  nand2  g26(.a(x10), .b(x08), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(x15), .c(new_n59_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x13), .O(new_n71_));
  nand4  g30(.a(new_n45_), .b(x10), .c(x08), .d(x00), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z12));
  inv1   g32(.a(z03), .O(new_n74_));
  nand2  g33(.a(new_n69_), .b(x01), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x14), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(z13));
  nand2  g36(.a(x08), .b(x02), .O(new_n78_));
  oai22  g37(.a(new_n78_), .b(new_n50_), .c(new_n69_), .d(new_n45_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n42_), .O(z14));
  nand4  g40(.a(new_n59_), .b(x10), .c(x08), .d(x03), .O(new_n82_));
  oai21  g41(.a(new_n46_), .b(x15), .c(x16), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n42_), .O(z15));
  inv1   g45(.a(x04), .O(new_n87_));
  nand2  g46(.a(new_n68_), .b(x17), .O(new_n88_));
  oai21  g47(.a(new_n68_), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n45_), .O(new_n90_));
  nand3  g49(.a(x17), .b(x15), .c(new_n49_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n67_), .O(z16));
  inv1   g51(.a(x18), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n67_), .c(new_n49_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand2  g54(.a(x08), .b(x05), .O(new_n96_));
  oai22  g55(.a(new_n96_), .b(new_n50_), .c(new_n69_), .d(new_n93_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n95_), .O(z17));
  inv1   g58(.a(x06), .O(new_n100_));
  nand2  g59(.a(new_n68_), .b(x19), .O(new_n101_));
  oai21  g60(.a(new_n68_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n45_), .O(new_n103_));
  nand3  g62(.a(x19), .b(x15), .c(new_n49_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n67_), .O(z18));
  inv1   g64(.a(x20), .O(new_n106_));
  oai21  g65(.a(new_n106_), .b(new_n67_), .c(new_n49_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x15), .O(new_n108_));
  nand2  g67(.a(x08), .b(x07), .O(new_n109_));
  oai22  g68(.a(new_n109_), .b(new_n50_), .c(new_n69_), .d(new_n106_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n108_), .O(z19));
  buf    g71(.a(x14), .O(z11));
endmodule


