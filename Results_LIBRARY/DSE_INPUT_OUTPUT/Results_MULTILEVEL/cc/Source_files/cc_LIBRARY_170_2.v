// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n61_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x20), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x15), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n43_), .c(new_n47_), .d(x14), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x14), .O(new_n51_));
  nand2  g10(.a(new_n47_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n44_), .O(z02));
  nand4  g14(.a(new_n51_), .b(x12), .c(x10), .d(x08), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(x19), .c(new_n47_), .O(z03));
  nand2  g16(.a(new_n44_), .b(x18), .O(z04));
  and2   g17(.a(new_n44_), .b(x17), .O(z07));
  and2   g18(.a(new_n44_), .b(x16), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n44_), .O(z09));
  inv1   g21(.a(z09), .O(z10));
  nand2  g22(.a(new_n44_), .b(new_n51_), .O(z11));
  inv1   g23(.a(x12), .O(new_n65_));
  aoi21  g24(.a(x10), .b(x08), .c(x15), .O(new_n66_));
  nor2   g25(.a(new_n43_), .b(new_n47_), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n66_), .c(x13), .O(new_n68_));
  inv1   g27(.a(new_n52_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(x08), .c(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n68_), .c(new_n65_), .O(z12));
  nand2  g30(.a(new_n48_), .b(new_n51_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x19), .c(x15), .O(new_n73_));
  nand3  g32(.a(x10), .b(x08), .c(x01), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n47_), .c(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n65_), .O(z13));
  nand3  g35(.a(new_n48_), .b(x19), .c(x15), .O(new_n77_));
  nand3  g36(.a(new_n69_), .b(x08), .c(x02), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n77_), .c(new_n65_), .O(z14));
  nand3  g38(.a(x16), .b(new_n51_), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x19), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x15), .O(new_n82_));
  nand3  g41(.a(new_n48_), .b(x16), .c(new_n51_), .O(new_n83_));
  nand4  g42(.a(new_n47_), .b(x10), .c(x08), .d(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n82_), .O(z15));
  nand2  g46(.a(new_n48_), .b(x17), .O(new_n88_));
  nand3  g47(.a(x10), .b(x08), .c(x04), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  nand3  g50(.a(x19), .b(x17), .c(x15), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n65_), .O(z16));
  nand2  g52(.a(new_n48_), .b(x18), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(x05), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n47_), .O(new_n97_));
  nand3  g56(.a(x19), .b(x18), .c(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n65_), .O(z17));
  inv1   g58(.a(x06), .O(new_n100_));
  aoi21  g59(.a(new_n48_), .b(new_n43_), .c(new_n100_), .O(new_n101_));
  nand2  g60(.a(new_n48_), .b(x19), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  oai21  g62(.a(new_n103_), .b(new_n101_), .c(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n44_), .O(z18));
  nand2  g64(.a(new_n48_), .b(x20), .O(new_n106_));
  nand3  g65(.a(x10), .b(x08), .c(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n47_), .O(new_n109_));
  nand3  g68(.a(x20), .b(x19), .c(x15), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n65_), .O(z19));
  buf    g70(.a(x19), .O(z05));
  buf    g71(.a(x15), .O(z06));
endmodule


