// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x11), .b(x03), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x08), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  nor3   g06(.a(new_n43_), .b(new_n47_), .c(x14), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x10), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n46_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand3  g11(.a(new_n47_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(x12), .c(new_n51_), .d(x03), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand3  g15(.a(new_n48_), .b(x12), .c(x10), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n46_), .O(z03));
  nand2  g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n44_), .b(new_n60_), .O(z05));
  nor2   g20(.a(new_n43_), .b(new_n47_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n43_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x09), .O(new_n67_));
  nor3   g26(.a(new_n43_), .b(new_n67_), .c(new_n46_), .O(z09));
  nand3  g27(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n44_), .b(new_n52_), .O(z11));
  nand2  g29(.a(new_n53_), .b(x13), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(x08), .c(x00), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(new_n44_), .c(x12), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z12));
  nand3  g36(.a(x10), .b(x08), .c(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x14), .O(new_n79_));
  nand3  g38(.a(x15), .b(x10), .c(x08), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n44_), .O(z13));
  oai21  g42(.a(new_n72_), .b(new_n46_), .c(x15), .O(new_n84_));
  nand3  g43(.a(new_n73_), .b(x08), .c(x02), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n44_), .c(x12), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z14));
  inv1   g47(.a(x03), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n52_), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n53_), .c(new_n89_), .O(new_n91_));
  nand3  g50(.a(new_n53_), .b(x16), .c(new_n52_), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n91_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n44_), .O(z15));
  inv1   g54(.a(x12), .O(new_n96_));
  and2   g55(.a(x08), .b(x04), .O(new_n97_));
  aoi22  g56(.a(new_n97_), .b(new_n73_), .c(new_n53_), .d(x17), .O(new_n98_));
  oai21  g57(.a(new_n98_), .b(new_n96_), .c(new_n44_), .O(z16));
  and2   g58(.a(x08), .b(x05), .O(new_n100_));
  aoi22  g59(.a(new_n100_), .b(new_n73_), .c(new_n53_), .d(x18), .O(new_n101_));
  oai21  g60(.a(new_n101_), .b(new_n96_), .c(new_n44_), .O(z17));
  nand2  g61(.a(new_n53_), .b(x19), .O(new_n103_));
  nand3  g62(.a(new_n73_), .b(x08), .c(x06), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n44_), .c(x12), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z18));
  nand2  g66(.a(new_n53_), .b(x20), .O(new_n108_));
  nand3  g67(.a(new_n73_), .b(x08), .c(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(new_n44_), .c(x12), .O(new_n111_));
  inv1   g70(.a(new_n111_), .O(z19));
endmodule


