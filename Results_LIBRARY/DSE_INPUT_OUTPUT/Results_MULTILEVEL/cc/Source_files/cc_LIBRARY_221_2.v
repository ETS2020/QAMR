// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n55_, new_n58_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n80_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x08), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x10), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x15), .c(new_n46_), .O(z01));
  nand2  g08(.a(x14), .b(x12), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x11), .c(new_n44_), .O(z02));
  nand3  g10(.a(x12), .b(x10), .c(x08), .O(new_n52_));
  nor3   g11(.a(new_n52_), .b(new_n43_), .c(x14), .O(z03));
  nand2  g12(.a(new_n44_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nand2  g14(.a(new_n44_), .b(new_n55_), .O(z05));
  nand2  g15(.a(new_n43_), .b(new_n46_), .O(z06));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z07));
  and2   g18(.a(new_n44_), .b(x16), .O(z08));
  nand3  g19(.a(x15), .b(x09), .c(x08), .O(z10));
  inv1   g20(.a(z10), .O(z09));
  nand2  g21(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g22(.a(x12), .O(new_n64_));
  oai21  g23(.a(x15), .b(new_n46_), .c(x13), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n64_), .O(z12));
  oai21  g25(.a(x15), .b(new_n46_), .c(x14), .O(new_n67_));
  nand3  g26(.a(x15), .b(x10), .c(x08), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(z13));
  nand2  g28(.a(x10), .b(x08), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(x15), .c(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n44_), .O(z14));
  nand3  g31(.a(x16), .b(new_n47_), .c(x12), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n44_), .O(z15));
  oai21  g33(.a(x15), .b(new_n46_), .c(x17), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n64_), .O(z16));
  nand2  g35(.a(x18), .b(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n44_), .O(z17));
  oai21  g37(.a(new_n55_), .b(new_n64_), .c(new_n44_), .O(z18));
  oai21  g38(.a(x15), .b(new_n46_), .c(x20), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n64_), .O(z19));
endmodule


