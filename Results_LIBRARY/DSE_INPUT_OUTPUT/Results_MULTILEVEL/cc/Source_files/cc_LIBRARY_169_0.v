// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x18), .b(x16), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x08), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  nor3   g06(.a(new_n43_), .b(new_n47_), .c(x14), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x10), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n46_), .O(z01));
  inv1   g09(.a(x14), .O(new_n51_));
  nand3  g10(.a(new_n47_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x12), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x11), .c(new_n44_), .O(z02));
  nand3  g14(.a(new_n48_), .b(x12), .c(x10), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(new_n46_), .O(z03));
  inv1   g16(.a(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n44_), .b(new_n59_), .O(z05));
  nor2   g19(.a(new_n43_), .b(x15), .O(new_n61_));
  inv1   g20(.a(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n43_), .b(new_n63_), .O(z07));
  aoi21  g23(.a(x09), .b(x08), .c(new_n43_), .O(z10));
  inv1   g24(.a(z10), .O(z09));
  nand2  g25(.a(new_n44_), .b(new_n51_), .O(z11));
  inv1   g26(.a(x12), .O(new_n68_));
  inv1   g27(.a(x10), .O(new_n69_));
  nor2   g28(.a(x15), .b(new_n69_), .O(new_n70_));
  and2   g29(.a(x08), .b(x00), .O(new_n71_));
  aoi22  g30(.a(new_n71_), .b(new_n70_), .c(new_n52_), .d(x13), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n68_), .c(new_n44_), .O(z12));
  nand3  g32(.a(x10), .b(x08), .c(x01), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x14), .O(new_n75_));
  nand3  g34(.a(x15), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n44_), .c(x12), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  oai21  g38(.a(new_n69_), .b(new_n46_), .c(x15), .O(new_n80_));
  nand3  g39(.a(new_n70_), .b(x08), .c(x02), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n44_), .c(x12), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z14));
  nand4  g43(.a(new_n61_), .b(x10), .c(x08), .d(x03), .O(new_n85_));
  nand3  g44(.a(new_n52_), .b(x16), .c(new_n51_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n68_), .O(z15));
  and2   g46(.a(x08), .b(x04), .O(new_n88_));
  aoi22  g47(.a(new_n88_), .b(new_n70_), .c(new_n52_), .d(x17), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n68_), .c(new_n44_), .O(z16));
  inv1   g49(.a(x05), .O(new_n91_));
  aoi21  g50(.a(new_n52_), .b(z04), .c(new_n91_), .O(new_n92_));
  and2   g51(.a(new_n52_), .b(x18), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n44_), .O(z17));
  nand2  g54(.a(new_n52_), .b(x19), .O(new_n96_));
  nand3  g55(.a(new_n70_), .b(x08), .c(x06), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(new_n44_), .c(x12), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z18));
  and2   g59(.a(x08), .b(x07), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n70_), .c(new_n52_), .d(x20), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n68_), .c(new_n44_), .O(z19));
  buf    g62(.a(x16), .O(z08));
endmodule


