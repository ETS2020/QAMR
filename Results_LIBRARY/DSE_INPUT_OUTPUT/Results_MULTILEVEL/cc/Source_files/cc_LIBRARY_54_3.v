// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n63_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x12), .b(x07), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand2  g03(.a(x10), .b(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x15), .b(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(new_n45_), .c(new_n43_), .O(z01));
  inv1   g07(.a(x07), .O(new_n49_));
  inv1   g08(.a(x12), .O(new_n50_));
  inv1   g09(.a(x11), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n49_), .c(new_n50_), .O(z02));
  nand4  g15(.a(x12), .b(x10), .c(x08), .d(new_n49_), .O(new_n57_));
  nor3   g16(.a(new_n57_), .b(new_n52_), .c(x14), .O(z03));
  nand2  g17(.a(new_n43_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n43_), .b(new_n60_), .O(z05));
  nand2  g20(.a(new_n43_), .b(new_n52_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n43_), .b(new_n63_), .O(z07));
  and2   g23(.a(new_n43_), .b(x16), .O(z08));
  nand3  g24(.a(new_n43_), .b(x09), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z09));
  aoi22  g26(.a(x12), .b(x07), .c(x09), .d(x08), .O(z10));
  nand2  g27(.a(new_n43_), .b(new_n46_), .O(z11));
  nand2  g28(.a(new_n53_), .b(x13), .O(new_n70_));
  nand4  g29(.a(new_n52_), .b(x10), .c(x08), .d(x00), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x12), .c(new_n49_), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n45_), .b(new_n75_), .c(x14), .O(new_n76_));
  inv1   g35(.a(x10), .O(new_n77_));
  nor2   g36(.a(new_n52_), .b(new_n77_), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(x08), .c(x07), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n50_), .O(z13));
  nand2  g39(.a(new_n45_), .b(x15), .O(new_n81_));
  nand4  g40(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n81_), .c(new_n49_), .O(new_n83_));
  and2   g42(.a(new_n83_), .b(x12), .O(z14));
  nand3  g43(.a(new_n53_), .b(x16), .c(new_n46_), .O(new_n85_));
  nand4  g44(.a(new_n52_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x12), .c(new_n49_), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z15));
  nand2  g48(.a(new_n53_), .b(x17), .O(new_n90_));
  nand4  g49(.a(new_n52_), .b(x10), .c(x08), .d(x04), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x12), .c(new_n49_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  nand2  g53(.a(new_n53_), .b(x18), .O(new_n95_));
  nor2   g54(.a(x15), .b(new_n77_), .O(new_n96_));
  and2   g55(.a(x08), .b(x05), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(x07), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n95_), .c(new_n50_), .O(z17));
  nand2  g58(.a(new_n53_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n52_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(x12), .c(new_n49_), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z18));
  nand2  g63(.a(new_n53_), .b(x20), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n49_), .c(new_n50_), .O(z19));
endmodule


