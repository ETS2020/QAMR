// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n52_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  oai21  g008(.a(x18), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n52_), .c(new_n63_), .O(new_n65_));
  oai21  g014(.a(new_n54_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n59_), .b(new_n68_), .c(x18), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x13), .O(new_n71_));
  oai21  g020(.a(x18), .b(new_n71_), .c(new_n59_), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  inv1   g022(.a(x19), .O(new_n74_));
  nand3  g023(.a(new_n62_), .b(new_n74_), .c(new_n63_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  nand4  g025(.a(new_n73_), .b(new_n62_), .c(new_n74_), .d(new_n63_), .O(new_n77_));
  oai21  g026(.a(new_n76_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n52_), .c(x16), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n72_), .O(z02));
  nor3   g029(.a(x19), .b(x18), .c(x17), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n62_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n81_), .c(new_n77_), .d(x22), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n59_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n59_), .c(new_n87_), .O(z03));
  inv1   g037(.a(x11), .O(new_n89_));
  oai21  g038(.a(x18), .b(new_n89_), .c(new_n59_), .O(new_n90_));
  nand3  g039(.a(new_n82_), .b(new_n54_), .c(new_n62_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x23), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n64_), .c(new_n73_), .d(new_n63_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n52_), .c(x16), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n90_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  inv1   g047(.a(x22), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n99_), .c(new_n73_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n75_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n98_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n65_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n59_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(z05));
  nand4  g059(.a(new_n104_), .b(new_n82_), .c(new_n64_), .d(new_n63_), .O(new_n111_));
  inv1   g060(.a(x25), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n98_), .c(new_n100_), .d(new_n99_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n77_), .O(new_n114_));
  aoi21  g063(.a(new_n111_), .b(x25), .c(new_n114_), .O(new_n115_));
  inv1   g064(.a(x09), .O(new_n116_));
  aoi21  g065(.a(new_n59_), .b(new_n116_), .c(x18), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n59_), .c(new_n117_), .O(z06));
  inv1   g067(.a(x08), .O(new_n119_));
  oai21  g068(.a(x18), .b(new_n119_), .c(new_n59_), .O(new_n120_));
  inv1   g069(.a(x26), .O(new_n121_));
  nor2   g070(.a(x26), .b(x25), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n93_), .c(new_n98_), .O(new_n123_));
  oai22  g072(.a(new_n123_), .b(new_n77_), .c(new_n114_), .d(new_n121_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n52_), .c(x16), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n120_), .O(z07));
  inv1   g075(.a(x07), .O(new_n127_));
  oai21  g076(.a(x18), .b(new_n127_), .c(new_n59_), .O(new_n128_));
  oai21  g077(.a(new_n123_), .b(new_n77_), .c(x27), .O(new_n129_));
  nor2   g078(.a(x27), .b(x26), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n104_), .c(new_n112_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n91_), .c(new_n129_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n52_), .c(x16), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n128_), .O(z08));
  inv1   g083(.a(x06), .O(new_n135_));
  oai21  g084(.a(x18), .b(new_n135_), .c(new_n59_), .O(new_n136_));
  oai21  g085(.a(new_n131_), .b(new_n91_), .c(x28), .O(new_n137_));
  nor2   g086(.a(x25), .b(x24), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n138_), .c(new_n121_), .d(new_n100_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n91_), .c(new_n137_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n52_), .c(x16), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n136_), .O(z09));
  inv1   g092(.a(x05), .O(new_n144_));
  oai21  g093(.a(x18), .b(new_n144_), .c(new_n59_), .O(new_n145_));
  oai21  g094(.a(new_n140_), .b(new_n91_), .c(x29), .O(new_n146_));
  nor3   g095(.a(x26), .b(x25), .c(x24), .O(new_n147_));
  nor3   g096(.a(x29), .b(x28), .c(x27), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n147_), .c(new_n102_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n52_), .c(x16), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n145_), .O(z10));
  inv1   g101(.a(x04), .O(new_n153_));
  oai21  g102(.a(x18), .b(new_n153_), .c(new_n59_), .O(new_n154_));
  inv1   g103(.a(x27), .O(new_n155_));
  nor2   g104(.a(x29), .b(x28), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n122_), .c(new_n155_), .d(new_n98_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n94_), .c(x30), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n139_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n147_), .c(new_n102_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n158_), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n52_), .c(x16), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n154_), .O(z11));
  inv1   g114(.a(x03), .O(new_n166_));
  oai21  g115(.a(x18), .b(new_n166_), .c(new_n59_), .O(new_n167_));
  nand4  g116(.a(new_n159_), .b(new_n139_), .c(new_n122_), .d(new_n98_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n94_), .c(x31), .O(new_n169_));
  inv1   g118(.a(new_n105_), .O(new_n170_));
  nor3   g119(.a(x27), .b(x26), .c(x25), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n156_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n171_), .c(new_n170_), .d(new_n76_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n169_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n52_), .c(x16), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n167_), .O(z12));
  inv1   g127(.a(x02), .O(new_n179_));
  oai21  g128(.a(x18), .b(new_n179_), .c(new_n59_), .O(new_n180_));
  nand4  g129(.a(new_n172_), .b(new_n156_), .c(new_n130_), .d(new_n112_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n111_), .c(x32), .O(new_n182_));
  nand2  g131(.a(new_n139_), .b(new_n122_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n159_), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n184_), .c(new_n170_), .d(new_n76_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n182_), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n52_), .c(x16), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n180_), .O(z13));
  nand4  g140(.a(new_n185_), .b(new_n159_), .c(new_n139_), .d(new_n122_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n111_), .c(x33), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n172_), .c(new_n156_), .d(new_n130_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n114_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n193_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  inv1   g148(.a(x01), .O(new_n200_));
  aoi21  g149(.a(new_n59_), .b(new_n200_), .c(x18), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n199_), .O(z14));
  inv1   g151(.a(x00), .O(new_n203_));
  oai21  g152(.a(x18), .b(new_n203_), .c(new_n59_), .O(new_n204_));
  nor2   g153(.a(x21), .b(x20), .O(new_n205_));
  nand4  g154(.a(new_n138_), .b(new_n93_), .c(new_n205_), .d(new_n54_), .O(new_n206_));
  oai21  g155(.a(new_n195_), .b(new_n206_), .c(x34), .O(new_n207_));
  nand2  g156(.a(new_n156_), .b(new_n130_), .O(new_n208_));
  inv1   g157(.a(new_n208_), .O(new_n209_));
  inv1   g158(.a(x30), .O(new_n210_));
  inv1   g159(.a(x31), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n210_), .O(new_n212_));
  inv1   g161(.a(x32), .O(new_n213_));
  inv1   g162(.a(x33), .O(new_n214_));
  inv1   g163(.a(x34), .O(new_n215_));
  nand3  g164(.a(new_n215_), .b(new_n214_), .c(new_n213_), .O(new_n216_));
  nor2   g165(.a(new_n216_), .b(new_n212_), .O(new_n217_));
  nand3  g166(.a(new_n217_), .b(new_n209_), .c(new_n114_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n207_), .O(new_n219_));
  nand3  g168(.a(new_n219_), .b(new_n52_), .c(x16), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n204_), .O(z15));
endmodule


