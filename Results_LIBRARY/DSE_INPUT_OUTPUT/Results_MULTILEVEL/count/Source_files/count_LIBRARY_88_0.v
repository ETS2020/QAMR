// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nand3  g008(.a(new_n59_), .b(new_n54_), .c(new_n53_), .O(new_n60_));
  oai21  g009(.a(new_n56_), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x16), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n52_), .O(z01));
  nand2  g012(.a(new_n60_), .b(x21), .O(new_n64_));
  inv1   g013(.a(x21), .O(new_n65_));
  nand4  g014(.a(new_n65_), .b(new_n59_), .c(new_n54_), .d(new_n53_), .O(new_n66_));
  nand4  g015(.a(new_n66_), .b(new_n64_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g016(.a(new_n66_), .b(x22), .O(new_n68_));
  nor2   g017(.a(x22), .b(x21), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n56_), .c(new_n59_), .O(new_n70_));
  nand4  g019(.a(new_n70_), .b(new_n68_), .c(new_n52_), .d(x16), .O(z03));
  nand2  g020(.a(new_n70_), .b(x23), .O(new_n72_));
  nor2   g021(.a(x20), .b(x19), .O(new_n73_));
  nor2   g022(.a(x23), .b(x22), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n73_), .c(new_n65_), .d(new_n53_), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n72_), .c(new_n52_), .d(x16), .O(z04));
  nand2  g025(.a(new_n75_), .b(x24), .O(new_n77_));
  nor2   g026(.a(x24), .b(x23), .O(new_n78_));
  nand4  g027(.a(new_n78_), .b(new_n69_), .c(new_n73_), .d(new_n53_), .O(new_n79_));
  nand4  g028(.a(new_n79_), .b(new_n77_), .c(new_n52_), .d(x16), .O(z05));
  nand2  g029(.a(new_n79_), .b(x25), .O(new_n81_));
  nor2   g030(.a(x21), .b(x20), .O(new_n82_));
  nor2   g031(.a(x25), .b(x24), .O(new_n83_));
  nand4  g032(.a(new_n83_), .b(new_n74_), .c(new_n82_), .d(new_n56_), .O(new_n84_));
  nand4  g033(.a(new_n84_), .b(new_n81_), .c(new_n52_), .d(x16), .O(z06));
  nand2  g034(.a(new_n84_), .b(x26), .O(new_n86_));
  inv1   g035(.a(x24), .O(new_n87_));
  inv1   g036(.a(new_n66_), .O(new_n88_));
  nor2   g037(.a(x26), .b(x25), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n74_), .c(new_n88_), .d(new_n87_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x16), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(z07));
  inv1   g042(.a(x16), .O(new_n94_));
  inv1   g043(.a(x25), .O(new_n95_));
  nor2   g044(.a(x27), .b(x26), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n78_), .c(new_n95_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n70_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x27), .c(new_n98_), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(new_n94_), .c(new_n52_), .O(z08));
  oai21  g049(.a(new_n97_), .b(new_n70_), .c(x28), .O(new_n101_));
  nor3   g050(.a(x22), .b(x21), .c(x20), .O(new_n102_));
  nor3   g051(.a(x25), .b(x24), .c(x23), .O(new_n103_));
  nor3   g052(.a(x28), .b(x27), .c(x26), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n56_), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n101_), .c(new_n52_), .d(x16), .O(z09));
  inv1   g055(.a(x27), .O(new_n107_));
  nor2   g056(.a(x29), .b(x28), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n89_), .c(new_n107_), .d(new_n87_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n75_), .O(new_n110_));
  aoi21  g059(.a(new_n105_), .b(x29), .c(new_n110_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n94_), .c(new_n52_), .O(z10));
  oai21  g061(.a(new_n109_), .b(new_n75_), .c(x30), .O(new_n113_));
  nor2   g062(.a(x28), .b(x27), .O(new_n114_));
  nor2   g063(.a(x30), .b(x29), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n114_), .c(new_n89_), .d(new_n87_), .O(new_n116_));
  or2    g065(.a(new_n116_), .b(new_n75_), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n113_), .c(new_n52_), .d(x16), .O(z11));
  oai21  g067(.a(new_n116_), .b(new_n75_), .c(x31), .O(new_n119_));
  inv1   g068(.a(x22), .O(new_n120_));
  inv1   g069(.a(x23), .O(new_n121_));
  nand4  g070(.a(new_n87_), .b(new_n121_), .c(new_n120_), .d(new_n65_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n60_), .O(new_n123_));
  nor3   g072(.a(x27), .b(x26), .c(x25), .O(new_n124_));
  nor2   g073(.a(x31), .b(x30), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n108_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n124_), .c(new_n123_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n119_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n52_), .O(z12));
  nand4  g080(.a(new_n125_), .b(new_n108_), .c(new_n96_), .d(new_n95_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n79_), .c(x32), .O(new_n133_));
  nand2  g082(.a(new_n114_), .b(new_n89_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  inv1   g084(.a(x31), .O(new_n136_));
  inv1   g085(.a(x32), .O(new_n137_));
  nand3  g086(.a(new_n115_), .b(new_n137_), .c(new_n136_), .O(new_n138_));
  inv1   g087(.a(new_n138_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n135_), .c(new_n123_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n133_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n52_), .O(z13));
  nand2  g092(.a(new_n140_), .b(x33), .O(new_n144_));
  nor2   g093(.a(x33), .b(x32), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n125_), .c(new_n108_), .d(new_n96_), .O(new_n146_));
  or2    g095(.a(new_n146_), .b(new_n84_), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n144_), .c(new_n52_), .d(x16), .O(z14));
  oai21  g097(.a(new_n146_), .b(new_n84_), .c(x34), .O(new_n149_));
  nand4  g098(.a(new_n95_), .b(new_n87_), .c(new_n121_), .d(new_n120_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n66_), .O(new_n151_));
  nand2  g100(.a(new_n108_), .b(new_n96_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  inv1   g102(.a(x30), .O(new_n154_));
  nand2  g103(.a(new_n136_), .b(new_n154_), .O(new_n155_));
  inv1   g104(.a(x33), .O(new_n156_));
  inv1   g105(.a(x34), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n156_), .c(new_n137_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n153_), .c(new_n151_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n149_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n52_), .O(z15));
endmodule


