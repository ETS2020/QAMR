// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand4  g004(.a(new_n55_), .b(new_n54_), .c(new_n52_), .d(x16), .O(z00));
  inv1   g005(.a(x16), .O(new_n57_));
  nor3   g006(.a(x20), .b(x19), .c(x17), .O(new_n58_));
  aoi21  g007(.a(new_n54_), .b(x20), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(z01));
  inv1   g009(.a(x17), .O(new_n61_));
  nor2   g010(.a(x20), .b(x19), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x21), .O(new_n64_));
  inv1   g013(.a(x19), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n61_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n64_), .c(new_n52_), .d(x16), .O(z02));
  nor3   g018(.a(x22), .b(x21), .c(x20), .O(new_n70_));
  aoi22  g019(.a(new_n70_), .b(new_n53_), .c(new_n68_), .d(x22), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n57_), .c(new_n52_), .O(z03));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n53_), .c(new_n66_), .O(new_n74_));
  nor3   g023(.a(x23), .b(x22), .c(x21), .O(new_n75_));
  aoi22  g024(.a(new_n75_), .b(new_n58_), .c(new_n74_), .d(x23), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n57_), .c(new_n52_), .O(z04));
  inv1   g026(.a(x24), .O(new_n78_));
  aoi21  g027(.a(new_n75_), .b(new_n58_), .c(new_n78_), .O(new_n79_));
  nor2   g028(.a(x24), .b(x23), .O(new_n80_));
  nand4  g029(.a(new_n80_), .b(new_n73_), .c(new_n62_), .d(new_n61_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n79_), .c(x16), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n52_), .O(z05));
  nand2  g033(.a(new_n81_), .b(x25), .O(new_n85_));
  inv1   g034(.a(x22), .O(new_n86_));
  inv1   g035(.a(x23), .O(new_n87_));
  inv1   g036(.a(x25), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n78_), .c(new_n87_), .d(new_n86_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n85_), .c(new_n52_), .d(x16), .O(z06));
  nand2  g041(.a(new_n91_), .b(x26), .O(new_n93_));
  inv1   g042(.a(new_n68_), .O(new_n94_));
  nor2   g043(.a(x23), .b(x22), .O(new_n95_));
  nor3   g044(.a(x26), .b(x25), .c(x24), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n93_), .c(new_n52_), .d(x16), .O(z07));
  nor2   g047(.a(x27), .b(x26), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n80_), .c(new_n88_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n74_), .O(new_n101_));
  aoi21  g050(.a(new_n97_), .b(x27), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n57_), .c(new_n52_), .O(z08));
  inv1   g052(.a(x28), .O(new_n104_));
  nor3   g053(.a(x25), .b(x24), .c(x23), .O(new_n105_));
  nor3   g054(.a(x28), .b(x27), .c(x26), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n105_), .c(new_n70_), .d(new_n53_), .O(new_n107_));
  oai21  g056(.a(new_n101_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n52_), .O(z09));
  nand2  g059(.a(new_n107_), .b(x29), .O(new_n111_));
  nor3   g060(.a(x29), .b(x28), .c(x27), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n96_), .c(new_n75_), .d(new_n58_), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n111_), .c(new_n52_), .d(x16), .O(z10));
  nand4  g063(.a(new_n95_), .b(new_n62_), .c(new_n67_), .d(new_n61_), .O(new_n115_));
  nor2   g064(.a(x26), .b(x25), .O(new_n116_));
  nor2   g065(.a(x28), .b(x27), .O(new_n117_));
  nor2   g066(.a(x30), .b(x29), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n78_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  aoi21  g069(.a(new_n113_), .b(x30), .c(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n57_), .c(new_n52_), .O(z11));
  oai21  g071(.a(new_n119_), .b(new_n115_), .c(x31), .O(new_n123_));
  nor3   g072(.a(x27), .b(x26), .c(x25), .O(new_n124_));
  nor2   g073(.a(x29), .b(x28), .O(new_n125_));
  nor2   g074(.a(x31), .b(x30), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n82_), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n123_), .c(new_n52_), .d(x16), .O(z12));
  nand2  g077(.a(new_n127_), .b(x32), .O(new_n129_));
  nor2   g078(.a(x32), .b(x31), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n118_), .c(new_n117_), .d(new_n116_), .O(new_n131_));
  or2    g080(.a(new_n131_), .b(new_n81_), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n129_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g082(.a(new_n131_), .b(new_n81_), .c(x33), .O(new_n134_));
  inv1   g083(.a(new_n89_), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  inv1   g085(.a(x27), .O(new_n137_));
  inv1   g086(.a(x29), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n104_), .c(new_n137_), .d(new_n136_), .O(new_n139_));
  inv1   g088(.a(new_n139_), .O(new_n140_));
  inv1   g089(.a(x30), .O(new_n141_));
  inv1   g090(.a(x31), .O(new_n142_));
  inv1   g091(.a(x32), .O(new_n143_));
  inv1   g092(.a(x33), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n142_), .d(new_n141_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n140_), .c(new_n135_), .d(new_n94_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n134_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n52_), .O(z14));
  nand2  g099(.a(new_n147_), .b(x34), .O(new_n151_));
  nor3   g100(.a(x34), .b(x33), .c(x32), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n140_), .c(new_n126_), .d(new_n90_), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n151_), .c(new_n52_), .d(x16), .O(z15));
endmodule


