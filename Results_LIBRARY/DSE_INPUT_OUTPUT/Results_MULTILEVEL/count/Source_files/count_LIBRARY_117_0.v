// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_;
  inv1   g000(.a(x13), .O(new_n52_));
  inv1   g001(.a(x18), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x16), .b(x15), .O(new_n55_));
  aoi21  g004(.a(new_n54_), .b(x16), .c(new_n55_), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n52_), .c(new_n53_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nor2   g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  inv1   g012(.a(x16), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n63_), .c(x18), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n62_), .c(x13), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  inv1   g016(.a(x17), .O(new_n68_));
  inv1   g017(.a(x19), .O(new_n69_));
  nand4  g018(.a(new_n67_), .b(new_n58_), .c(new_n69_), .d(new_n68_), .O(new_n70_));
  oai21  g019(.a(new_n61_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(x16), .c(x13), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n53_), .O(z02));
  nand2  g022(.a(new_n70_), .b(x22), .O(new_n74_));
  nor3   g023(.a(x22), .b(x21), .c(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n59_), .O(new_n76_));
  aoi21  g025(.a(new_n76_), .b(new_n74_), .c(new_n64_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n77_), .c(x13), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n53_), .O(z03));
  nand2  g029(.a(new_n76_), .b(x23), .O(new_n81_));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n61_), .c(new_n67_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(new_n64_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x11), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n84_), .c(x13), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n53_), .O(z04));
  nand2  g036(.a(new_n83_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x22), .b(x21), .O(new_n89_));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n89_), .c(new_n61_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n88_), .c(new_n64_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n92_), .c(x13), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n53_), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  inv1   g045(.a(new_n91_), .O(new_n97_));
  nor2   g046(.a(x21), .b(x20), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n82_), .c(new_n98_), .d(new_n59_), .O(new_n100_));
  oai21  g049(.a(new_n97_), .b(new_n96_), .c(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g051(.a(x09), .O(new_n103_));
  aoi21  g052(.a(new_n64_), .b(new_n103_), .c(x18), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n102_), .c(x13), .O(z06));
  nand2  g054(.a(new_n100_), .b(x26), .O(new_n106_));
  inv1   g055(.a(x24), .O(new_n107_));
  nor2   g056(.a(x26), .b(x25), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n82_), .c(new_n107_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n70_), .c(new_n106_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g060(.a(x08), .O(new_n112_));
  aoi21  g061(.a(new_n64_), .b(new_n112_), .c(x18), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n111_), .c(x13), .O(z07));
  oai21  g063(.a(new_n109_), .b(new_n70_), .c(x27), .O(new_n115_));
  nor3   g064(.a(x27), .b(x26), .c(x25), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n90_), .c(new_n75_), .d(new_n59_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n115_), .c(new_n64_), .O(new_n118_));
  nor2   g067(.a(x16), .b(x07), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n118_), .c(x13), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n53_), .O(z08));
  nand2  g070(.a(new_n117_), .b(x28), .O(new_n122_));
  nor3   g071(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nor3   g072(.a(x28), .b(x27), .c(x26), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n123_), .c(new_n75_), .d(new_n59_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g076(.a(x06), .O(new_n128_));
  aoi21  g077(.a(new_n64_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n127_), .c(x13), .O(z09));
  nand2  g079(.a(new_n125_), .b(x29), .O(new_n131_));
  nor3   g080(.a(x23), .b(x22), .c(x21), .O(new_n132_));
  nor3   g081(.a(x26), .b(x25), .c(x24), .O(new_n133_));
  nor3   g082(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n61_), .O(new_n135_));
  and2   g084(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x05), .O(new_n137_));
  nor3   g086(.a(new_n137_), .b(x18), .c(new_n52_), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n64_), .c(new_n138_), .O(z10));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nor2   g089(.a(x30), .b(x29), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n140_), .c(new_n133_), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n83_), .O(new_n143_));
  aoi21  g092(.a(new_n135_), .b(x30), .c(new_n143_), .O(new_n144_));
  nor2   g093(.a(x16), .b(x04), .O(new_n145_));
  nor3   g094(.a(new_n145_), .b(x18), .c(new_n52_), .O(new_n146_));
  oai21  g095(.a(new_n144_), .b(new_n64_), .c(new_n146_), .O(z11));
  inv1   g096(.a(x31), .O(new_n148_));
  nor2   g097(.a(x29), .b(x28), .O(new_n149_));
  nor2   g098(.a(x31), .b(x30), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n149_), .c(new_n116_), .O(new_n151_));
  oai22  g100(.a(new_n151_), .b(new_n91_), .c(new_n143_), .d(new_n148_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nor2   g102(.a(x16), .b(x03), .O(new_n154_));
  nor3   g103(.a(new_n154_), .b(x18), .c(new_n52_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z12));
  oai21  g105(.a(new_n151_), .b(new_n91_), .c(x32), .O(new_n157_));
  nor2   g106(.a(x32), .b(x31), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n141_), .c(new_n140_), .d(new_n108_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n91_), .c(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x16), .O(new_n161_));
  inv1   g110(.a(x02), .O(new_n162_));
  aoi21  g111(.a(new_n64_), .b(new_n162_), .c(x18), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n161_), .c(x13), .O(z13));
  oai21  g113(.a(new_n159_), .b(new_n91_), .c(x33), .O(new_n165_));
  nor2   g114(.a(x27), .b(x26), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n150_), .c(new_n149_), .d(new_n166_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n100_), .c(new_n165_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  aoi21  g120(.a(new_n64_), .b(new_n171_), .c(x18), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n170_), .c(x13), .O(z14));
  oai21  g122(.a(new_n168_), .b(new_n100_), .c(x34), .O(new_n174_));
  inv1   g123(.a(x22), .O(new_n175_));
  inv1   g124(.a(x23), .O(new_n176_));
  nand4  g125(.a(new_n96_), .b(new_n107_), .c(new_n176_), .d(new_n175_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n70_), .O(new_n178_));
  nand2  g127(.a(new_n149_), .b(new_n166_), .O(new_n179_));
  inv1   g128(.a(new_n179_), .O(new_n180_));
  inv1   g129(.a(x30), .O(new_n181_));
  nand2  g130(.a(new_n148_), .b(new_n181_), .O(new_n182_));
  inv1   g131(.a(x32), .O(new_n183_));
  inv1   g132(.a(x33), .O(new_n184_));
  inv1   g133(.a(x34), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n184_), .c(new_n183_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n182_), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n180_), .c(new_n178_), .O(new_n188_));
  aoi21  g137(.a(new_n188_), .b(new_n174_), .c(new_n64_), .O(new_n189_));
  nor2   g138(.a(x16), .b(x00), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n189_), .c(x13), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n53_), .O(z15));
endmodule


