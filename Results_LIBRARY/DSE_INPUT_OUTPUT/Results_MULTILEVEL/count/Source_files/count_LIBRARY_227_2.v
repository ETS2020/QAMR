// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x29), .b(x18), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n60_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n58_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  inv1   g019(.a(new_n65_), .O(new_n71_));
  inv1   g020(.a(x19), .O(new_n72_));
  inv1   g021(.a(x20), .O(new_n73_));
  nand4  g022(.a(new_n70_), .b(new_n73_), .c(new_n72_), .d(new_n63_), .O(new_n74_));
  oai21  g023(.a(new_n71_), .b(new_n70_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x18), .c(new_n58_), .O(z02));
  nor3   g027(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  nor3   g028(.a(x22), .b(x21), .c(x20), .O(new_n80_));
  aoi22  g029(.a(new_n80_), .b(new_n79_), .c(new_n74_), .d(x22), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n61_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n61_), .c(new_n83_), .O(z03));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n53_), .c(new_n73_), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n64_), .c(new_n70_), .d(new_n63_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n86_), .b(x23), .c(new_n89_), .O(new_n90_));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n61_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n61_), .c(new_n92_), .O(z04));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n85_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi22  g045(.a(new_n96_), .b(new_n71_), .c(new_n88_), .d(x24), .O(new_n97_));
  inv1   g046(.a(x10), .O(new_n98_));
  aoi21  g047(.a(new_n61_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n61_), .c(new_n99_), .O(z05));
  nand4  g049(.a(new_n94_), .b(new_n85_), .c(new_n64_), .d(new_n63_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x25), .O(new_n102_));
  nor2   g051(.a(x21), .b(x20), .O(new_n103_));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n87_), .c(new_n103_), .d(new_n53_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n102_), .c(new_n61_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n106_), .c(new_n60_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n58_), .O(z06));
  nand2  g058(.a(new_n105_), .b(x26), .O(new_n110_));
  nor3   g059(.a(x26), .b(x25), .c(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n87_), .c(new_n103_), .d(new_n53_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n110_), .c(new_n61_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x08), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n113_), .c(new_n60_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n58_), .O(z07));
  inv1   g065(.a(x25), .O(new_n117_));
  nor2   g066(.a(x27), .b(x26), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n94_), .c(new_n117_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n86_), .O(new_n120_));
  aoi21  g069(.a(new_n112_), .b(x27), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x07), .O(new_n122_));
  aoi21  g071(.a(new_n61_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n61_), .c(new_n123_), .O(z08));
  oai21  g073(.a(new_n119_), .b(new_n86_), .c(x28), .O(new_n125_));
  nor3   g074(.a(x25), .b(x24), .c(x23), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x26), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n126_), .c(new_n80_), .d(new_n53_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n125_), .c(new_n61_), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n129_), .c(new_n60_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n58_), .O(z09));
  inv1   g081(.a(x29), .O(new_n133_));
  nand3  g082(.a(new_n80_), .b(new_n53_), .c(x16), .O(new_n134_));
  nor2   g083(.a(x29), .b(x28), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n118_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n126_), .O(new_n138_));
  oai22  g087(.a(new_n138_), .b(new_n134_), .c(x16), .d(x05), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n60_), .O(new_n140_));
  aoi21  g089(.a(new_n128_), .b(x16), .c(x18), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n133_), .c(new_n140_), .O(z10));
  inv1   g091(.a(x27), .O(new_n143_));
  nand4  g092(.a(new_n135_), .b(new_n111_), .c(new_n89_), .d(new_n143_), .O(new_n144_));
  inv1   g093(.a(x30), .O(new_n145_));
  nor2   g094(.a(x28), .b(x27), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n111_), .c(new_n145_), .d(new_n133_), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n88_), .O(new_n148_));
  aoi21  g097(.a(new_n144_), .b(x30), .c(new_n148_), .O(new_n149_));
  inv1   g098(.a(x04), .O(new_n150_));
  aoi21  g099(.a(new_n61_), .b(new_n150_), .c(x18), .O(new_n151_));
  oai21  g100(.a(new_n149_), .b(new_n61_), .c(new_n151_), .O(z11));
  inv1   g101(.a(x31), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n61_), .c(new_n60_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x29), .O(new_n155_));
  inv1   g104(.a(x24), .O(new_n156_));
  nor2   g105(.a(x26), .b(x25), .O(new_n157_));
  nor2   g106(.a(x30), .b(x28), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n157_), .c(new_n143_), .d(new_n156_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n88_), .c(x31), .O(new_n160_));
  nor3   g109(.a(x27), .b(x26), .c(x25), .O(new_n161_));
  nor2   g110(.a(x31), .b(x30), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n135_), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n161_), .c(new_n96_), .d(new_n71_), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n160_), .c(new_n61_), .O(new_n166_));
  nor2   g115(.a(x16), .b(x03), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n166_), .c(new_n60_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n155_), .O(z12));
  nand2  g118(.a(new_n165_), .b(x32), .O(new_n170_));
  nand2  g119(.a(new_n146_), .b(new_n157_), .O(new_n171_));
  inv1   g120(.a(x32), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n153_), .c(new_n145_), .d(new_n133_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n96_), .c(new_n71_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n170_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  inv1   g126(.a(x02), .O(new_n178_));
  aoi21  g127(.a(new_n61_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z13));
  nand2  g129(.a(new_n175_), .b(x33), .O(new_n181_));
  inv1   g130(.a(x22), .O(new_n182_));
  inv1   g131(.a(x23), .O(new_n183_));
  nand4  g132(.a(new_n117_), .b(new_n156_), .c(new_n183_), .d(new_n182_), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n74_), .O(new_n185_));
  nor2   g134(.a(x33), .b(x32), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n162_), .c(new_n137_), .d(new_n185_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n181_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  inv1   g138(.a(x01), .O(new_n190_));
  aoi21  g139(.a(new_n61_), .b(new_n190_), .c(x18), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n189_), .O(z14));
  inv1   g141(.a(x34), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n61_), .c(new_n60_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x29), .O(new_n195_));
  nand4  g144(.a(new_n186_), .b(new_n162_), .c(new_n146_), .d(new_n157_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n101_), .c(x34), .O(new_n197_));
  nand2  g146(.a(new_n153_), .b(new_n145_), .O(new_n198_));
  inv1   g147(.a(x33), .O(new_n199_));
  nand3  g148(.a(new_n193_), .b(new_n199_), .c(new_n172_), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n198_), .O(new_n201_));
  nand3  g150(.a(new_n201_), .b(new_n137_), .c(new_n185_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n197_), .c(new_n61_), .O(new_n203_));
  nor2   g152(.a(x16), .b(x00), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n203_), .c(new_n60_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n195_), .O(z15));
endmodule


