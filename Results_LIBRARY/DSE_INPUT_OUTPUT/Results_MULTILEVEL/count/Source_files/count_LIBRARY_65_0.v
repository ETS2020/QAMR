// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x18), .O(new_n52_));
  inv1   g01(.a(x26), .O(new_n53_));
  xnor2a g02(.a(x19), .b(x17), .O(new_n54_));
  nor2   g03(.a(x16), .b(x15), .O(new_n55_));
  aoi21  g04(.a(new_n54_), .b(x16), .c(new_n55_), .O(new_n56_));
  oai21  g05(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(z00));
  inv1   g06(.a(x20), .O(new_n58_));
  nor2   g07(.a(x19), .b(x17), .O(new_n59_));
  nor2   g08(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor3   g09(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g10(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nor2   g11(.a(x16), .b(x14), .O(new_n63_));
  nand2  g12(.a(x26), .b(new_n52_), .O(new_n64_));
  nor2   g13(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g14(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g15(.a(x21), .O(new_n67_));
  nor2   g16(.a(x21), .b(x20), .O(new_n68_));
  nand2  g17(.a(new_n68_), .b(new_n59_), .O(new_n69_));
  oai21  g18(.a(new_n61_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nor2   g19(.a(x16), .b(x13), .O(new_n71_));
  aoi21  g20(.a(new_n70_), .b(x16), .c(new_n71_), .O(new_n72_));
  oai21  g21(.a(new_n72_), .b(new_n53_), .c(new_n52_), .O(z02));
  inv1   g22(.a(x16), .O(new_n74_));
  nor3   g23(.a(x22), .b(x21), .c(x20), .O(new_n75_));
  aoi22  g24(.a(new_n75_), .b(new_n59_), .c(new_n69_), .d(x22), .O(new_n76_));
  nor2   g25(.a(x16), .b(x12), .O(new_n77_));
  nor2   g26(.a(new_n77_), .b(new_n64_), .O(new_n78_));
  oai21  g27(.a(new_n76_), .b(new_n74_), .c(new_n78_), .O(z03));
  nand2  g28(.a(new_n75_), .b(new_n59_), .O(new_n80_));
  nor3   g29(.a(x23), .b(x22), .c(x21), .O(new_n81_));
  aoi22  g30(.a(new_n81_), .b(new_n61_), .c(new_n80_), .d(x23), .O(new_n82_));
  nor2   g31(.a(x16), .b(x11), .O(new_n83_));
  nor2   g32(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  oai21  g33(.a(new_n82_), .b(new_n74_), .c(new_n84_), .O(z04));
  inv1   g34(.a(x24), .O(new_n86_));
  aoi21  g35(.a(new_n81_), .b(new_n61_), .c(new_n86_), .O(new_n87_));
  inv1   g36(.a(x17), .O(new_n88_));
  nor2   g37(.a(x20), .b(x19), .O(new_n89_));
  nor2   g38(.a(x22), .b(x21), .O(new_n90_));
  nor2   g39(.a(x24), .b(x23), .O(new_n91_));
  nand4  g40(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n92_));
  inv1   g41(.a(new_n92_), .O(new_n93_));
  oai21  g42(.a(new_n93_), .b(new_n87_), .c(x16), .O(new_n94_));
  nor2   g43(.a(x16), .b(x10), .O(new_n95_));
  nor2   g44(.a(new_n95_), .b(new_n64_), .O(new_n96_));
  nand2  g45(.a(new_n96_), .b(new_n94_), .O(z05));
  nand2  g46(.a(new_n92_), .b(x25), .O(new_n98_));
  nor2   g47(.a(x23), .b(x22), .O(new_n99_));
  nor2   g48(.a(x25), .b(x24), .O(new_n100_));
  nand4  g49(.a(new_n100_), .b(new_n99_), .c(new_n68_), .d(new_n59_), .O(new_n101_));
  aoi21  g50(.a(new_n101_), .b(new_n98_), .c(new_n74_), .O(new_n102_));
  nor2   g51(.a(x16), .b(x09), .O(new_n103_));
  oai21  g52(.a(new_n103_), .b(new_n102_), .c(x26), .O(new_n104_));
  nand2  g53(.a(new_n104_), .b(new_n52_), .O(z06));
  nor2   g54(.a(x16), .b(x08), .O(new_n106_));
  aoi21  g55(.a(new_n101_), .b(x16), .c(new_n106_), .O(new_n107_));
  oai21  g56(.a(new_n107_), .b(new_n53_), .c(new_n52_), .O(z07));
  nor2   g57(.a(x16), .b(x07), .O(new_n109_));
  and2   g58(.a(x27), .b(x16), .O(new_n110_));
  oai21  g59(.a(new_n110_), .b(new_n109_), .c(x26), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(new_n52_), .O(z08));
  nor2   g61(.a(x16), .b(x06), .O(new_n113_));
  and2   g62(.a(x28), .b(x16), .O(new_n114_));
  oai21  g63(.a(new_n114_), .b(new_n113_), .c(x26), .O(new_n115_));
  nand2  g64(.a(new_n115_), .b(new_n52_), .O(z09));
  nor2   g65(.a(x16), .b(x05), .O(new_n117_));
  and2   g66(.a(x29), .b(x16), .O(new_n118_));
  oai21  g67(.a(new_n118_), .b(new_n117_), .c(x26), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n52_), .O(z10));
  inv1   g69(.a(new_n64_), .O(new_n121_));
  nand2  g70(.a(x30), .b(x16), .O(new_n122_));
  inv1   g71(.a(x04), .O(new_n123_));
  nand2  g72(.a(new_n74_), .b(new_n123_), .O(new_n124_));
  nand3  g73(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(z11));
  nand2  g74(.a(x31), .b(x16), .O(new_n126_));
  inv1   g75(.a(x03), .O(new_n127_));
  nand2  g76(.a(new_n74_), .b(new_n127_), .O(new_n128_));
  nand3  g77(.a(new_n128_), .b(new_n126_), .c(new_n121_), .O(z12));
  nand2  g78(.a(x32), .b(x16), .O(new_n130_));
  inv1   g79(.a(x02), .O(new_n131_));
  nand2  g80(.a(new_n74_), .b(new_n131_), .O(new_n132_));
  nand3  g81(.a(new_n132_), .b(new_n130_), .c(new_n121_), .O(z13));
  nand2  g82(.a(x33), .b(x16), .O(new_n134_));
  inv1   g83(.a(x01), .O(new_n135_));
  nand2  g84(.a(new_n74_), .b(new_n135_), .O(new_n136_));
  nand3  g85(.a(new_n136_), .b(new_n134_), .c(new_n121_), .O(z14));
  nor2   g86(.a(x16), .b(x00), .O(new_n138_));
  and2   g87(.a(x34), .b(x16), .O(new_n139_));
  oai21  g88(.a(new_n139_), .b(new_n138_), .c(x26), .O(new_n140_));
  nand2  g89(.a(new_n140_), .b(new_n52_), .O(z15));
endmodule


