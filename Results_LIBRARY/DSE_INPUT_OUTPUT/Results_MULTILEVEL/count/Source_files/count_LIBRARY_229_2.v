// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x28), .b(x18), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  nand2  g019(.a(new_n66_), .b(x21), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x18), .c(new_n58_), .O(z02));
  nand2  g026(.a(new_n73_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n53_), .c(new_n65_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n61_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n60_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n58_), .O(z03));
  nand2  g033(.a(new_n80_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x20), .b(x19), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n72_), .d(new_n63_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n85_), .c(new_n61_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n89_), .c(new_n60_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n58_), .O(z04));
  nand2  g041(.a(new_n88_), .b(x24), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n79_), .c(new_n86_), .d(new_n63_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n61_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n60_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n58_), .O(z05));
  inv1   g048(.a(x22), .O(new_n100_));
  inv1   g049(.a(x23), .O(new_n101_));
  inv1   g050(.a(x24), .O(new_n102_));
  inv1   g051(.a(x25), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n73_), .O(new_n105_));
  aoi21  g054(.a(new_n95_), .b(x25), .c(new_n105_), .O(new_n106_));
  inv1   g055(.a(x09), .O(new_n107_));
  aoi21  g056(.a(new_n61_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n61_), .c(new_n108_), .O(z06));
  inv1   g058(.a(x26), .O(new_n110_));
  inv1   g059(.a(new_n73_), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n87_), .c(new_n111_), .O(new_n113_));
  oai21  g062(.a(new_n105_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x08), .O(new_n116_));
  aoi21  g065(.a(new_n61_), .b(new_n116_), .c(x18), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(z07));
  nand2  g067(.a(new_n113_), .b(x27), .O(new_n119_));
  nor3   g068(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nor3   g069(.a(x27), .b(x26), .c(x25), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n94_), .c(new_n120_), .d(new_n53_), .O(new_n122_));
  and2   g071(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  inv1   g072(.a(x07), .O(new_n124_));
  aoi21  g073(.a(new_n61_), .b(new_n124_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n61_), .c(new_n125_), .O(z08));
  inv1   g075(.a(x28), .O(new_n127_));
  nand3  g076(.a(new_n120_), .b(new_n53_), .c(x16), .O(new_n128_));
  nor2   g077(.a(x25), .b(x24), .O(new_n129_));
  nor2   g078(.a(x28), .b(x27), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n129_), .c(new_n110_), .d(new_n101_), .O(new_n131_));
  oai22  g080(.a(new_n131_), .b(new_n128_), .c(x16), .d(x06), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n60_), .O(new_n133_));
  aoi21  g082(.a(new_n122_), .b(x16), .c(x18), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n127_), .c(new_n133_), .O(z09));
  oai21  g084(.a(new_n131_), .b(new_n80_), .c(x29), .O(new_n136_));
  inv1   g085(.a(x27), .O(new_n137_));
  nand3  g086(.a(new_n101_), .b(new_n100_), .c(new_n72_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n66_), .O(new_n139_));
  nor2   g088(.a(x29), .b(x28), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n112_), .c(new_n139_), .d(new_n137_), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n136_), .c(new_n61_), .O(new_n142_));
  oai21  g091(.a(x16), .b(x05), .c(new_n60_), .O(new_n143_));
  or2    g092(.a(new_n143_), .b(new_n142_), .O(z10));
  inv1   g093(.a(x30), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n61_), .c(new_n60_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x28), .O(new_n147_));
  nor2   g096(.a(x29), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n129_), .c(new_n110_), .d(new_n101_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n80_), .c(x30), .O(new_n150_));
  nor2   g099(.a(x30), .b(x29), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n130_), .c(new_n112_), .d(new_n139_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n150_), .c(new_n61_), .O(new_n153_));
  nor2   g102(.a(x16), .b(x04), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n153_), .c(new_n60_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n147_), .O(z11));
  nand2  g105(.a(new_n152_), .b(x31), .O(new_n157_));
  inv1   g106(.a(new_n95_), .O(new_n158_));
  nor2   g107(.a(x31), .b(x30), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n140_), .c(new_n121_), .d(new_n158_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  inv1   g111(.a(x03), .O(new_n163_));
  aoi21  g112(.a(new_n61_), .b(new_n163_), .c(x18), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z12));
  inv1   g114(.a(x32), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n61_), .c(new_n60_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x28), .O(new_n168_));
  nor2   g117(.a(x26), .b(x25), .O(new_n169_));
  nand4  g118(.a(new_n159_), .b(new_n148_), .c(new_n169_), .d(new_n102_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n88_), .c(x32), .O(new_n171_));
  nand2  g120(.a(new_n130_), .b(new_n169_), .O(new_n172_));
  nor2   g121(.a(x32), .b(x31), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n151_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n158_), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n171_), .c(new_n61_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x02), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n177_), .c(new_n60_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n168_), .O(z13));
  inv1   g129(.a(x33), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n61_), .c(new_n60_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x28), .O(new_n183_));
  nor2   g132(.a(x27), .b(x26), .O(new_n184_));
  nand4  g133(.a(new_n173_), .b(new_n151_), .c(new_n184_), .d(new_n103_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n95_), .c(x33), .O(new_n186_));
  nand2  g135(.a(new_n140_), .b(new_n184_), .O(new_n187_));
  nor2   g136(.a(x33), .b(x32), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n159_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n187_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n105_), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n186_), .c(new_n61_), .O(new_n192_));
  nor2   g141(.a(x16), .b(x01), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n192_), .c(new_n60_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n183_), .O(z14));
  inv1   g144(.a(x34), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n61_), .c(new_n60_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(x28), .O(new_n198_));
  nand4  g147(.a(new_n188_), .b(new_n159_), .c(new_n148_), .d(new_n169_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n95_), .c(x34), .O(new_n200_));
  inv1   g149(.a(new_n187_), .O(new_n201_));
  inv1   g150(.a(x31), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n145_), .O(new_n203_));
  nand3  g152(.a(new_n196_), .b(new_n181_), .c(new_n166_), .O(new_n204_));
  nor2   g153(.a(new_n204_), .b(new_n203_), .O(new_n205_));
  nand3  g154(.a(new_n205_), .b(new_n201_), .c(new_n105_), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n200_), .c(new_n61_), .O(new_n207_));
  nor2   g156(.a(x16), .b(x00), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n207_), .c(new_n60_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n198_), .O(z15));
endmodule


