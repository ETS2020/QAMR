// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_;
  inv1   g00(.a(x18), .O(new_n52_));
  inv1   g01(.a(x26), .O(new_n53_));
  nand2  g02(.a(x19), .b(x17), .O(new_n54_));
  nor2   g03(.a(x19), .b(x17), .O(new_n55_));
  inv1   g04(.a(new_n55_), .O(new_n56_));
  nand2  g05(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nor2   g06(.a(x16), .b(x15), .O(new_n58_));
  aoi21  g07(.a(new_n57_), .b(x16), .c(new_n58_), .O(new_n59_));
  oai21  g08(.a(new_n59_), .b(new_n53_), .c(new_n52_), .O(z00));
  inv1   g09(.a(x16), .O(new_n61_));
  nand2  g10(.a(new_n56_), .b(x20), .O(new_n62_));
  inv1   g11(.a(x17), .O(new_n63_));
  nor2   g12(.a(x20), .b(x19), .O(new_n64_));
  nand2  g13(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g14(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(new_n66_));
  nor2   g15(.a(x16), .b(x14), .O(new_n67_));
  oai21  g16(.a(new_n67_), .b(new_n66_), .c(x26), .O(new_n68_));
  nand2  g17(.a(new_n68_), .b(new_n52_), .O(z01));
  nor2   g18(.a(x21), .b(x20), .O(new_n70_));
  aoi22  g19(.a(new_n70_), .b(new_n55_), .c(new_n65_), .d(x21), .O(new_n71_));
  nor2   g20(.a(x16), .b(x13), .O(new_n72_));
  nand2  g21(.a(x26), .b(new_n52_), .O(new_n73_));
  nor2   g22(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g23(.a(new_n71_), .b(new_n61_), .c(new_n74_), .O(z02));
  nand2  g24(.a(new_n70_), .b(new_n55_), .O(new_n76_));
  nand2  g25(.a(new_n76_), .b(x22), .O(new_n77_));
  inv1   g26(.a(x20), .O(new_n78_));
  nor2   g27(.a(x22), .b(x21), .O(new_n79_));
  nand3  g28(.a(new_n79_), .b(new_n55_), .c(new_n78_), .O(new_n80_));
  and2   g29(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nor2   g30(.a(x16), .b(x12), .O(new_n82_));
  nor2   g31(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  oai21  g32(.a(new_n81_), .b(new_n61_), .c(new_n83_), .O(z03));
  nand2  g33(.a(new_n80_), .b(x23), .O(new_n85_));
  inv1   g34(.a(x21), .O(new_n86_));
  nor2   g35(.a(x23), .b(x22), .O(new_n87_));
  nand4  g36(.a(new_n87_), .b(new_n64_), .c(new_n86_), .d(new_n63_), .O(new_n88_));
  aoi21  g37(.a(new_n88_), .b(new_n85_), .c(new_n61_), .O(new_n89_));
  nor2   g38(.a(x16), .b(x11), .O(new_n90_));
  oai21  g39(.a(new_n90_), .b(new_n89_), .c(x26), .O(new_n91_));
  nand2  g40(.a(new_n91_), .b(new_n52_), .O(z04));
  nand2  g41(.a(new_n88_), .b(x24), .O(new_n93_));
  nor2   g42(.a(x24), .b(x23), .O(new_n94_));
  nand4  g43(.a(new_n94_), .b(new_n79_), .c(new_n64_), .d(new_n63_), .O(new_n95_));
  aoi21  g44(.a(new_n95_), .b(new_n93_), .c(new_n61_), .O(new_n96_));
  nor2   g45(.a(x16), .b(x10), .O(new_n97_));
  oai21  g46(.a(new_n97_), .b(new_n96_), .c(x26), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n52_), .O(z05));
  nand2  g48(.a(new_n95_), .b(x25), .O(new_n100_));
  nor2   g49(.a(x25), .b(x24), .O(new_n101_));
  nand4  g50(.a(new_n101_), .b(new_n87_), .c(new_n70_), .d(new_n55_), .O(new_n102_));
  aoi21  g51(.a(new_n102_), .b(new_n100_), .c(new_n61_), .O(new_n103_));
  nor2   g52(.a(x16), .b(x09), .O(new_n104_));
  oai21  g53(.a(new_n104_), .b(new_n103_), .c(x26), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(new_n52_), .O(z06));
  nand2  g55(.a(new_n102_), .b(x16), .O(new_n107_));
  nor2   g56(.a(x16), .b(x08), .O(new_n108_));
  nor2   g57(.a(new_n108_), .b(new_n73_), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n107_), .O(z07));
  nor2   g59(.a(x16), .b(x07), .O(new_n111_));
  and2   g60(.a(x27), .b(x16), .O(new_n112_));
  oai21  g61(.a(new_n112_), .b(new_n111_), .c(x26), .O(new_n113_));
  nand2  g62(.a(new_n113_), .b(new_n52_), .O(z08));
  inv1   g63(.a(new_n73_), .O(new_n115_));
  nand2  g64(.a(x28), .b(x16), .O(new_n116_));
  inv1   g65(.a(x06), .O(new_n117_));
  nand2  g66(.a(new_n61_), .b(new_n117_), .O(new_n118_));
  nand3  g67(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(z09));
  nor2   g68(.a(x16), .b(x05), .O(new_n120_));
  and2   g69(.a(x29), .b(x16), .O(new_n121_));
  oai21  g70(.a(new_n121_), .b(new_n120_), .c(x26), .O(new_n122_));
  nand2  g71(.a(new_n122_), .b(new_n52_), .O(z10));
  nand2  g72(.a(x30), .b(x16), .O(new_n124_));
  inv1   g73(.a(x04), .O(new_n125_));
  nand2  g74(.a(new_n61_), .b(new_n125_), .O(new_n126_));
  nand3  g75(.a(new_n126_), .b(new_n124_), .c(new_n115_), .O(z11));
  nand2  g76(.a(x31), .b(x16), .O(new_n128_));
  inv1   g77(.a(x03), .O(new_n129_));
  nand2  g78(.a(new_n61_), .b(new_n129_), .O(new_n130_));
  nand3  g79(.a(new_n130_), .b(new_n128_), .c(new_n115_), .O(z12));
  nand2  g80(.a(x32), .b(x16), .O(new_n132_));
  inv1   g81(.a(x02), .O(new_n133_));
  nand2  g82(.a(new_n61_), .b(new_n133_), .O(new_n134_));
  nand3  g83(.a(new_n134_), .b(new_n132_), .c(new_n115_), .O(z13));
  nand2  g84(.a(x33), .b(x16), .O(new_n136_));
  inv1   g85(.a(x01), .O(new_n137_));
  nand2  g86(.a(new_n61_), .b(new_n137_), .O(new_n138_));
  nand3  g87(.a(new_n138_), .b(new_n136_), .c(new_n115_), .O(z14));
  nand2  g88(.a(x34), .b(x16), .O(new_n140_));
  inv1   g89(.a(x00), .O(new_n141_));
  nand2  g90(.a(new_n61_), .b(new_n141_), .O(new_n142_));
  nand3  g91(.a(new_n142_), .b(new_n140_), .c(new_n115_), .O(z15));
endmodule


