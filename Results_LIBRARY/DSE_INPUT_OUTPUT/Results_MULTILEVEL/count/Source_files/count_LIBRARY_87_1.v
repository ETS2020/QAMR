// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x30), .b(x18), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g012(.a(new_n58_), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n63_), .O(z01));
  inv1   g016(.a(x18), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  oai21  g020(.a(new_n62_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  aoi21  g022(.a(new_n72_), .b(x16), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(x30), .c(new_n68_), .O(z02));
  inv1   g024(.a(x30), .O(new_n76_));
  nand2  g025(.a(new_n71_), .b(x22), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n53_), .c(new_n60_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n56_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(new_n76_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n68_), .O(z03));
  nand2  g032(.a(new_n79_), .b(x23), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n62_), .O(new_n86_));
  aoi21  g035(.a(new_n86_), .b(new_n84_), .c(new_n56_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n87_), .c(new_n76_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n68_), .O(z04));
  inv1   g039(.a(x17), .O(new_n91_));
  nor2   g040(.a(x20), .b(x19), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n69_), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n93_), .c(x24), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n78_), .c(new_n92_), .d(new_n91_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n56_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n99_), .c(new_n76_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n68_), .O(z05));
  nand2  g051(.a(new_n98_), .b(x25), .O(new_n103_));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n94_), .c(new_n70_), .d(new_n53_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n56_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n106_), .c(new_n76_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n68_), .O(z06));
  nand2  g058(.a(new_n105_), .b(x26), .O(new_n110_));
  nor3   g059(.a(x26), .b(x25), .c(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n94_), .c(new_n70_), .d(new_n53_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n110_), .c(new_n56_), .O(new_n113_));
  oai21  g062(.a(x16), .b(x08), .c(new_n58_), .O(new_n114_));
  or2    g063(.a(new_n114_), .b(new_n113_), .O(z07));
  inv1   g064(.a(x25), .O(new_n116_));
  nor2   g065(.a(x27), .b(x26), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n97_), .c(new_n116_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n79_), .O(new_n119_));
  aoi21  g068(.a(new_n112_), .b(x27), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n64_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n56_), .c(new_n122_), .O(z08));
  oai21  g072(.a(new_n118_), .b(new_n79_), .c(x28), .O(new_n124_));
  nor3   g073(.a(x22), .b(x21), .c(x20), .O(new_n125_));
  nor3   g074(.a(x25), .b(x24), .c(x23), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x26), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n126_), .c(new_n125_), .d(new_n53_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n124_), .c(new_n56_), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n129_), .c(new_n76_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n68_), .O(z09));
  nand2  g081(.a(new_n128_), .b(x29), .O(new_n133_));
  nor3   g082(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n111_), .c(new_n85_), .d(new_n62_), .O(new_n135_));
  and2   g084(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x05), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n64_), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n56_), .c(new_n138_), .O(z10));
  inv1   g088(.a(x04), .O(new_n140_));
  nand2  g089(.a(new_n56_), .b(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n53_), .b(x16), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  nor2   g092(.a(x29), .b(x28), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n117_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n143_), .c(new_n126_), .d(new_n125_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n141_), .c(new_n58_), .O(z11));
  nor2   g097(.a(x28), .b(x27), .O(new_n149_));
  nor2   g098(.a(x31), .b(x29), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n149_), .c(new_n111_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n86_), .O(new_n152_));
  aoi21  g101(.a(new_n135_), .b(x31), .c(new_n152_), .O(new_n153_));
  nor2   g102(.a(x16), .b(x03), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n64_), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n56_), .c(new_n155_), .O(z12));
  oai21  g105(.a(new_n151_), .b(new_n86_), .c(x32), .O(new_n157_));
  inv1   g106(.a(new_n98_), .O(new_n158_));
  nor3   g107(.a(x27), .b(x26), .c(x25), .O(new_n159_));
  nor2   g108(.a(x32), .b(x31), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n144_), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  nor2   g112(.a(x16), .b(x02), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n64_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z13));
  nand3  g115(.a(new_n160_), .b(new_n144_), .c(new_n159_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n98_), .c(x33), .O(new_n168_));
  nor2   g117(.a(x26), .b(x25), .O(new_n169_));
  nor2   g118(.a(x33), .b(x32), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n150_), .c(new_n149_), .d(new_n169_), .O(new_n171_));
  or2    g120(.a(new_n171_), .b(new_n98_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nor2   g123(.a(x16), .b(x01), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n64_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n174_), .O(z14));
  oai21  g126(.a(new_n171_), .b(new_n98_), .c(x34), .O(new_n178_));
  nor2   g127(.a(x34), .b(x33), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n160_), .c(new_n146_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n105_), .c(new_n178_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x16), .O(new_n182_));
  nor2   g131(.a(x16), .b(x00), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n64_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n182_), .O(z15));
endmodule


