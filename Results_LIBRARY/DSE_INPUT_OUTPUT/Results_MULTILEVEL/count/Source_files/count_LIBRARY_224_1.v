// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x29), .O(new_n58_));
  nand2  g007(.a(x31), .b(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  aoi21  g009(.a(new_n56_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n54_), .O(z00));
  inv1   g011(.a(new_n59_), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n53_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n56_), .b(new_n68_), .c(x18), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z01));
  inv1   g019(.a(new_n66_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n71_), .b(x21), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(x21), .O(new_n80_));
  nand4  g029(.a(new_n53_), .b(new_n79_), .c(new_n80_), .d(new_n64_), .O(new_n81_));
  oai21  g030(.a(new_n74_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n56_), .b(new_n84_), .c(x18), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(new_n83_), .c(new_n63_), .O(z03));
  nand2  g035(.a(new_n81_), .b(x23), .O(new_n87_));
  inv1   g036(.a(x17), .O(new_n88_));
  nor2   g037(.a(x20), .b(x19), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n89_), .c(new_n80_), .d(new_n88_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n56_), .b(new_n94_), .c(x18), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n63_), .O(z04));
  and2   g045(.a(new_n91_), .b(x24), .O(new_n97_));
  inv1   g046(.a(x23), .O(new_n98_));
  inv1   g047(.a(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n98_), .c(new_n79_), .d(new_n80_), .O(new_n100_));
  inv1   g049(.a(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n66_), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n97_), .c(x16), .O(new_n104_));
  inv1   g053(.a(x10), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(new_n105_), .c(x18), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n104_), .c(new_n63_), .O(z05));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n90_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n73_), .O(new_n110_));
  aoi21  g059(.a(new_n102_), .b(x25), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x09), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n60_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n56_), .c(new_n113_), .O(z06));
  inv1   g063(.a(x26), .O(new_n115_));
  nor3   g064(.a(x26), .b(x25), .c(x24), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n90_), .O(new_n117_));
  oai22  g066(.a(new_n117_), .b(new_n73_), .c(new_n110_), .d(new_n115_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  inv1   g068(.a(x08), .O(new_n120_));
  aoi21  g069(.a(new_n56_), .b(new_n120_), .c(x18), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n119_), .c(new_n63_), .O(z07));
  oai21  g071(.a(new_n117_), .b(new_n73_), .c(x27), .O(new_n123_));
  nor3   g072(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  nor2   g073(.a(x24), .b(x23), .O(new_n125_));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n53_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n56_), .b(new_n130_), .c(x18), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n129_), .c(new_n63_), .O(z08));
  nand2  g081(.a(new_n127_), .b(x28), .O(new_n133_));
  nor2   g082(.a(x28), .b(x27), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n108_), .c(new_n115_), .d(new_n98_), .O(new_n135_));
  or2    g084(.a(new_n135_), .b(new_n81_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nor2   g087(.a(x16), .b(x06), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n60_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z09));
  oai21  g090(.a(x16), .b(x05), .c(new_n57_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n59_), .O(new_n143_));
  inv1   g092(.a(x31), .O(new_n144_));
  nand4  g093(.a(new_n134_), .b(new_n116_), .c(new_n144_), .d(new_n58_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n91_), .O(new_n146_));
  aoi21  g095(.a(new_n136_), .b(x29), .c(new_n146_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n56_), .c(new_n143_), .O(z10));
  oai21  g097(.a(x16), .b(x04), .c(new_n57_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n59_), .O(new_n150_));
  inv1   g099(.a(x30), .O(new_n151_));
  oai21  g100(.a(new_n135_), .b(new_n81_), .c(new_n144_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n58_), .c(new_n151_), .O(new_n153_));
  nor2   g102(.a(x29), .b(x28), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n126_), .c(new_n144_), .d(new_n151_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n102_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n153_), .c(x16), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n150_), .O(z11));
  inv1   g107(.a(x03), .O(new_n159_));
  nor2   g108(.a(new_n144_), .b(new_n58_), .O(new_n160_));
  nor2   g109(.a(x31), .b(x16), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n160_), .c(new_n159_), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n156_), .c(x16), .O(new_n163_));
  nand2  g112(.a(new_n59_), .b(x18), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n163_), .c(new_n162_), .O(z12));
  oai21  g114(.a(x16), .b(x02), .c(new_n57_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n59_), .O(new_n167_));
  inv1   g116(.a(x27), .O(new_n168_));
  nor2   g117(.a(x26), .b(x25), .O(new_n169_));
  nor2   g118(.a(x30), .b(x28), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n169_), .c(new_n168_), .d(new_n99_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n91_), .c(x32), .O(new_n172_));
  inv1   g121(.a(x28), .O(new_n173_));
  inv1   g122(.a(x32), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n151_), .c(new_n58_), .d(new_n173_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n126_), .c(new_n101_), .d(new_n66_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n172_), .c(x31), .O(new_n178_));
  nor2   g127(.a(new_n174_), .b(new_n58_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n178_), .c(x16), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n167_), .O(z13));
  nand4  g130(.a(new_n90_), .b(new_n72_), .c(new_n53_), .d(x16), .O(new_n182_));
  nor2   g131(.a(x27), .b(x26), .O(new_n183_));
  nor2   g132(.a(x33), .b(x32), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n170_), .c(new_n183_), .d(new_n108_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n182_), .c(new_n144_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n58_), .O(new_n187_));
  nand2  g136(.a(new_n134_), .b(new_n169_), .O(new_n188_));
  nand4  g137(.a(new_n174_), .b(new_n144_), .c(new_n151_), .d(new_n58_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n188_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n103_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(x33), .c(x16), .O(new_n192_));
  inv1   g141(.a(x01), .O(new_n193_));
  aoi21  g142(.a(new_n56_), .b(new_n193_), .c(x18), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n192_), .c(new_n187_), .O(z14));
  nand4  g144(.a(new_n101_), .b(new_n89_), .c(new_n88_), .d(x16), .O(new_n196_));
  inv1   g145(.a(new_n188_), .O(new_n197_));
  nor2   g146(.a(x34), .b(x33), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n197_), .c(new_n174_), .d(new_n151_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n196_), .c(new_n144_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n58_), .O(new_n201_));
  nand3  g150(.a(new_n184_), .b(new_n144_), .c(new_n151_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n154_), .c(new_n183_), .d(new_n110_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(x34), .c(x16), .O(new_n205_));
  inv1   g154(.a(x00), .O(new_n206_));
  aoi21  g155(.a(new_n56_), .b(new_n206_), .c(x18), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n205_), .c(new_n201_), .O(z15));
endmodule


