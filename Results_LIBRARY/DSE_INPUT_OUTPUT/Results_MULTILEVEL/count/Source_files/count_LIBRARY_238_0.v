// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x30), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x16), .b(x15), .O(new_n55_));
  aoi21  g004(.a(new_n54_), .b(x16), .c(new_n55_), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nor2   g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nand2  g012(.a(x30), .b(new_n52_), .O(new_n64_));
  nor2   g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g015(.a(x16), .O(new_n67_));
  inv1   g016(.a(new_n61_), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nand3  g018(.a(new_n59_), .b(new_n69_), .c(new_n58_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n68_), .b(x21), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  nor2   g022(.a(new_n73_), .b(new_n64_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n67_), .c(new_n74_), .O(z02));
  nor3   g024(.a(x22), .b(x21), .c(x20), .O(new_n76_));
  aoi22  g025(.a(new_n76_), .b(new_n59_), .c(new_n70_), .d(x22), .O(new_n77_));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n64_), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n67_), .c(new_n79_), .O(z03));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n59_), .c(new_n58_), .O(new_n82_));
  nor3   g031(.a(x23), .b(x22), .c(x21), .O(new_n83_));
  and2   g032(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  aoi21  g033(.a(new_n82_), .b(x23), .c(new_n84_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x11), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n67_), .c(new_n87_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n81_), .c(new_n61_), .O(new_n91_));
  oai21  g040(.a(new_n84_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nor2   g042(.a(x16), .b(x10), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n64_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n93_), .O(z05));
  nor2   g045(.a(x23), .b(x22), .O(new_n97_));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  aoi22  g049(.a(new_n100_), .b(new_n71_), .c(new_n91_), .d(x25), .O(new_n101_));
  nor2   g050(.a(x16), .b(x09), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n64_), .O(new_n103_));
  oai21  g052(.a(new_n101_), .b(new_n67_), .c(new_n103_), .O(z06));
  inv1   g053(.a(x26), .O(new_n105_));
  aoi21  g054(.a(new_n100_), .b(new_n71_), .c(new_n105_), .O(new_n106_));
  inv1   g055(.a(x25), .O(new_n107_));
  nand4  g056(.a(new_n97_), .b(new_n105_), .c(new_n107_), .d(new_n89_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n70_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nor2   g059(.a(x16), .b(x08), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n64_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(z07));
  oai21  g062(.a(new_n108_), .b(new_n70_), .c(x27), .O(new_n114_));
  nor3   g063(.a(x27), .b(x26), .c(x25), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n90_), .c(new_n76_), .d(new_n59_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n114_), .c(new_n67_), .O(new_n117_));
  nor2   g066(.a(x16), .b(x07), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n117_), .c(x30), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n52_), .O(z08));
  nor2   g069(.a(x27), .b(x26), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n90_), .c(new_n107_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n82_), .c(x28), .O(new_n123_));
  nor3   g072(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  nor3   g073(.a(x28), .b(x27), .c(x26), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n124_), .c(new_n76_), .d(new_n59_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n123_), .c(new_n67_), .O(new_n127_));
  nor2   g076(.a(x16), .b(x06), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n127_), .c(x30), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n52_), .O(z09));
  inv1   g079(.a(x23), .O(new_n131_));
  nor2   g080(.a(x28), .b(x27), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n98_), .c(new_n105_), .d(new_n131_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n82_), .c(x29), .O(new_n134_));
  nor3   g083(.a(x26), .b(x25), .c(x24), .O(new_n135_));
  nor3   g084(.a(x29), .b(x28), .c(x27), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n135_), .c(new_n83_), .d(new_n61_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n134_), .c(new_n67_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x05), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n138_), .c(x30), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n52_), .O(z10));
  nor2   g090(.a(x16), .b(x04), .O(new_n142_));
  aoi21  g091(.a(new_n137_), .b(x16), .c(new_n142_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n53_), .c(new_n52_), .O(z11));
  nor2   g093(.a(x16), .b(x03), .O(new_n145_));
  and2   g094(.a(x31), .b(x16), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(x30), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n52_), .O(z12));
  nor2   g097(.a(x16), .b(x02), .O(new_n149_));
  and2   g098(.a(x32), .b(x16), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n149_), .c(x30), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z13));
  inv1   g101(.a(new_n64_), .O(new_n153_));
  nand2  g102(.a(x33), .b(x16), .O(new_n154_));
  inv1   g103(.a(x01), .O(new_n155_));
  nand2  g104(.a(new_n67_), .b(new_n155_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n154_), .c(new_n153_), .O(z14));
  nand2  g106(.a(x34), .b(x16), .O(new_n158_));
  inv1   g107(.a(x00), .O(new_n159_));
  nand2  g108(.a(new_n67_), .b(new_n159_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n158_), .c(new_n153_), .O(z15));
endmodule


