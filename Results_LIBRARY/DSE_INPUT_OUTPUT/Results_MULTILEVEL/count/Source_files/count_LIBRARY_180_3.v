// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_;
  nand2  g000(.a(x12), .b(x07), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  inv1   g003(.a(x16), .O(new_n55_));
  oai21  g004(.a(x18), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  inv1   g005(.a(x17), .O(new_n57_));
  inv1   g006(.a(x18), .O(new_n58_));
  nand2  g007(.a(x19), .b(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  aoi21  g009(.a(x19), .b(new_n58_), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(x16), .c(new_n60_), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n56_), .c(new_n53_), .O(z00));
  inv1   g012(.a(x14), .O(new_n64_));
  oai21  g013(.a(x18), .b(new_n64_), .c(new_n55_), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n55_), .c(new_n58_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(x17), .O(new_n68_));
  inv1   g017(.a(x19), .O(new_n69_));
  nand3  g018(.a(new_n66_), .b(new_n57_), .c(x16), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g021(.a(new_n52_), .b(x18), .c(new_n57_), .O(new_n73_));
  aoi21  g022(.a(new_n73_), .b(new_n66_), .c(new_n69_), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(x16), .c(new_n53_), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n72_), .c(new_n68_), .d(new_n65_), .O(z01));
  inv1   g025(.a(x21), .O(new_n77_));
  nor3   g026(.a(x20), .b(x19), .c(x17), .O(new_n78_));
  nand4  g027(.a(new_n77_), .b(new_n66_), .c(new_n69_), .d(new_n57_), .O(new_n79_));
  oai21  g028(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  inv1   g030(.a(x13), .O(new_n82_));
  aoi21  g031(.a(new_n55_), .b(new_n82_), .c(x18), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(new_n53_), .O(z02));
  oai21  g033(.a(new_n55_), .b(x12), .c(x07), .O(new_n85_));
  nor2   g034(.a(x19), .b(x17), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  nor2   g036(.a(x22), .b(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n66_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n87_), .c(new_n58_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  inv1   g040(.a(x12), .O(new_n92_));
  oai21  g041(.a(x18), .b(new_n92_), .c(new_n55_), .O(new_n93_));
  nor2   g042(.a(new_n88_), .b(new_n58_), .O(new_n94_));
  inv1   g043(.a(x22), .O(new_n95_));
  inv1   g044(.a(new_n79_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(x16), .c(new_n94_), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n93_), .c(new_n91_), .d(new_n85_), .O(z03));
  nand3  g048(.a(new_n88_), .b(new_n86_), .c(new_n66_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x23), .O(new_n101_));
  nor3   g050(.a(x23), .b(x22), .c(x21), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n78_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x16), .O(new_n105_));
  inv1   g054(.a(x11), .O(new_n106_));
  aoi21  g055(.a(new_n55_), .b(new_n106_), .c(x18), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n105_), .c(new_n53_), .O(z04));
  inv1   g057(.a(x24), .O(new_n109_));
  aoi21  g058(.a(new_n102_), .b(new_n78_), .c(new_n109_), .O(new_n110_));
  nor2   g059(.a(x20), .b(x19), .O(new_n111_));
  nor2   g060(.a(x24), .b(x23), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n88_), .c(new_n111_), .d(new_n57_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n110_), .c(x16), .O(new_n115_));
  inv1   g064(.a(x10), .O(new_n116_));
  aoi21  g065(.a(new_n55_), .b(new_n116_), .c(x18), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n115_), .c(new_n53_), .O(z05));
  inv1   g067(.a(x25), .O(new_n119_));
  nor2   g068(.a(x21), .b(x20), .O(new_n120_));
  nor2   g069(.a(x23), .b(x22), .O(new_n121_));
  nor2   g070(.a(x25), .b(x24), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n86_), .O(new_n123_));
  oai21  g072(.a(new_n114_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  inv1   g074(.a(x09), .O(new_n126_));
  aoi21  g075(.a(new_n55_), .b(new_n126_), .c(x18), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n125_), .c(new_n52_), .O(z06));
  inv1   g077(.a(x26), .O(new_n129_));
  inv1   g078(.a(new_n123_), .O(new_n130_));
  nor3   g079(.a(x26), .b(x25), .c(x24), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n121_), .O(new_n132_));
  oai22  g081(.a(new_n132_), .b(new_n79_), .c(new_n130_), .d(new_n129_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  oai21  g083(.a(x16), .b(x08), .c(new_n58_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n53_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z07));
  oai21  g086(.a(new_n55_), .b(x07), .c(x12), .O(new_n138_));
  oai21  g087(.a(new_n132_), .b(new_n79_), .c(x27), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n112_), .c(new_n119_), .O(new_n141_));
  or2    g090(.a(new_n141_), .b(new_n100_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x07), .O(new_n145_));
  aoi21  g094(.a(new_n55_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(new_n138_), .O(z08));
  oai21  g096(.a(new_n141_), .b(new_n100_), .c(x28), .O(new_n148_));
  inv1   g097(.a(new_n100_), .O(new_n149_));
  nor3   g098(.a(x25), .b(x24), .c(x23), .O(new_n150_));
  nor3   g099(.a(x28), .b(x27), .c(x26), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(new_n149_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n148_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g103(.a(x06), .O(new_n155_));
  aoi21  g104(.a(new_n55_), .b(new_n155_), .c(x18), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n154_), .c(new_n53_), .O(z09));
  nor3   g106(.a(x29), .b(x28), .c(x27), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n131_), .c(new_n102_), .d(new_n78_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  aoi21  g109(.a(new_n152_), .b(x29), .c(new_n160_), .O(new_n161_));
  oai21  g110(.a(x16), .b(x05), .c(new_n58_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n53_), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n55_), .c(new_n163_), .O(z10));
  nor2   g113(.a(x28), .b(x27), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n165_), .c(new_n131_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n103_), .O(new_n168_));
  aoi21  g117(.a(new_n159_), .b(x30), .c(new_n168_), .O(new_n169_));
  oai21  g118(.a(x16), .b(x04), .c(new_n58_), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n53_), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n55_), .c(new_n171_), .O(z11));
  oai21  g121(.a(new_n167_), .b(new_n103_), .c(x31), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n140_), .d(new_n119_), .O(new_n176_));
  or2    g125(.a(new_n176_), .b(new_n113_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n173_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  oai21  g128(.a(x16), .b(x03), .c(new_n58_), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n53_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(z12));
  oai21  g131(.a(new_n176_), .b(new_n113_), .c(x32), .O(new_n183_));
  inv1   g132(.a(x23), .O(new_n184_));
  nand4  g133(.a(new_n109_), .b(new_n184_), .c(new_n95_), .d(new_n77_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  inv1   g135(.a(x27), .O(new_n187_));
  inv1   g136(.a(x28), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n187_), .c(new_n129_), .d(new_n119_), .O(new_n189_));
  inv1   g138(.a(new_n189_), .O(new_n190_));
  inv1   g139(.a(x29), .O(new_n191_));
  inv1   g140(.a(x30), .O(new_n192_));
  inv1   g141(.a(x31), .O(new_n193_));
  inv1   g142(.a(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n193_), .c(new_n192_), .d(new_n191_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n190_), .c(new_n186_), .d(new_n78_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n183_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  inv1   g148(.a(x02), .O(new_n200_));
  aoi21  g149(.a(new_n55_), .b(new_n200_), .c(x18), .O(new_n201_));
  aoi21  g150(.a(new_n201_), .b(new_n199_), .c(new_n53_), .O(z13));
  nor2   g151(.a(x26), .b(x25), .O(new_n203_));
  nor2   g152(.a(x32), .b(x31), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n166_), .c(new_n165_), .d(new_n203_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n113_), .c(x33), .O(new_n206_));
  nand4  g155(.a(new_n119_), .b(new_n109_), .c(new_n184_), .d(new_n95_), .O(new_n207_));
  inv1   g156(.a(new_n207_), .O(new_n208_));
  nand4  g157(.a(new_n191_), .b(new_n188_), .c(new_n187_), .d(new_n129_), .O(new_n209_));
  inv1   g158(.a(new_n209_), .O(new_n210_));
  inv1   g159(.a(x33), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n194_), .c(new_n193_), .d(new_n192_), .O(new_n212_));
  inv1   g161(.a(new_n212_), .O(new_n213_));
  nand4  g162(.a(new_n213_), .b(new_n210_), .c(new_n208_), .d(new_n96_), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n206_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x16), .O(new_n216_));
  inv1   g165(.a(x01), .O(new_n217_));
  aoi21  g166(.a(new_n55_), .b(new_n217_), .c(x18), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n216_), .c(new_n53_), .O(z14));
  nor2   g168(.a(x33), .b(x32), .O(new_n220_));
  nand4  g169(.a(new_n220_), .b(new_n175_), .c(new_n174_), .d(new_n140_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n123_), .c(x34), .O(new_n222_));
  nor3   g171(.a(x34), .b(x33), .c(x32), .O(new_n223_));
  nand4  g172(.a(new_n223_), .b(new_n210_), .c(new_n175_), .d(new_n130_), .O(new_n224_));
  nand2  g173(.a(new_n224_), .b(new_n222_), .O(new_n225_));
  nand2  g174(.a(new_n225_), .b(x16), .O(new_n226_));
  inv1   g175(.a(x00), .O(new_n227_));
  aoi21  g176(.a(new_n55_), .b(new_n227_), .c(x18), .O(new_n228_));
  nand3  g177(.a(new_n228_), .b(new_n226_), .c(new_n52_), .O(z15));
endmodule


