// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x18), .O(new_n52_));
  xnor2a g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  aoi21  g003(.a(new_n53_), .b(x16), .c(new_n54_), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(x27), .c(new_n52_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x17), .O(new_n60_));
  inv1   g009(.a(x19), .O(new_n61_));
  nand3  g010(.a(new_n57_), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n59_), .c(x16), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  inv1   g014(.a(x27), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n64_), .O(z01));
  inv1   g018(.a(x16), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n58_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n62_), .b(x21), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n67_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n70_), .c(new_n76_), .O(z02));
  nand2  g026(.a(new_n72_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n58_), .c(new_n57_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n70_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n66_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n52_), .O(z03));
  inv1   g033(.a(x21), .O(new_n85_));
  inv1   g034(.a(x22), .O(new_n86_));
  inv1   g035(.a(x23), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  aoi21  g038(.a(new_n80_), .b(x23), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n67_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n70_), .c(new_n92_), .O(z04));
  nor2   g042(.a(x20), .b(x19), .O(new_n94_));
  nor2   g043(.a(x23), .b(x22), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n94_), .c(new_n85_), .d(new_n60_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x24), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n79_), .c(new_n94_), .d(new_n60_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n70_), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n100_), .c(new_n66_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n52_), .O(z05));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n95_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  aoi22  g055(.a(new_n106_), .b(new_n73_), .c(new_n99_), .d(x25), .O(new_n107_));
  nor2   g056(.a(x16), .b(x09), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n67_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n70_), .c(new_n109_), .O(z06));
  oai21  g059(.a(new_n105_), .b(new_n72_), .c(x26), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n95_), .c(new_n73_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n111_), .c(new_n70_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x08), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n114_), .c(new_n66_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n52_), .O(z07));
  nand2  g066(.a(new_n112_), .b(new_n95_), .O(new_n118_));
  nand4  g067(.a(new_n71_), .b(new_n61_), .c(new_n60_), .d(x16), .O(new_n119_));
  oai22  g068(.a(new_n119_), .b(new_n118_), .c(x16), .d(x07), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n66_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n52_), .O(z08));
  nand2  g071(.a(new_n113_), .b(x28), .O(new_n123_));
  nor3   g072(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  nor3   g073(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  nor3   g074(.a(x28), .b(x27), .c(x26), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n58_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n67_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z09));
  inv1   g081(.a(x25), .O(new_n133_));
  nor2   g082(.a(x28), .b(x26), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n98_), .c(new_n133_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n80_), .c(x29), .O(new_n136_));
  nor3   g085(.a(x29), .b(x28), .c(x26), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n125_), .c(new_n124_), .d(new_n58_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n136_), .c(new_n70_), .O(new_n139_));
  nor2   g088(.a(x16), .b(x05), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n139_), .c(new_n66_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n52_), .O(z10));
  inv1   g091(.a(x26), .O(new_n143_));
  nor2   g092(.a(x29), .b(x28), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n104_), .c(new_n143_), .d(new_n87_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n80_), .c(x30), .O(new_n146_));
  nor3   g095(.a(x30), .b(x29), .c(x28), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n112_), .c(new_n89_), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n146_), .c(new_n70_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x04), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n149_), .c(new_n66_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z11));
  inv1   g101(.a(x24), .O(new_n153_));
  inv1   g102(.a(x28), .O(new_n154_));
  nor2   g103(.a(x26), .b(x25), .O(new_n155_));
  nor2   g104(.a(x30), .b(x29), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n154_), .d(new_n153_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n96_), .c(x31), .O(new_n158_));
  inv1   g107(.a(x30), .O(new_n159_));
  inv1   g108(.a(x31), .O(new_n160_));
  nand3  g109(.a(new_n144_), .b(new_n160_), .c(new_n159_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n112_), .c(new_n89_), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n158_), .c(new_n70_), .O(new_n164_));
  nor2   g113(.a(x16), .b(x03), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n164_), .c(new_n66_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n52_), .O(z12));
  nand2  g116(.a(new_n163_), .b(x32), .O(new_n168_));
  nor2   g117(.a(x32), .b(x31), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n156_), .c(new_n134_), .d(new_n133_), .O(new_n170_));
  or2    g119(.a(new_n170_), .b(new_n99_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x02), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n67_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n173_), .O(z13));
  oai21  g125(.a(new_n170_), .b(new_n99_), .c(x33), .O(new_n177_));
  nand4  g126(.a(new_n153_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  inv1   g128(.a(x29), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n154_), .c(new_n143_), .d(new_n133_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  inv1   g131(.a(x32), .O(new_n183_));
  inv1   g132(.a(x33), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n183_), .c(new_n160_), .d(new_n159_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n182_), .c(new_n179_), .d(new_n63_), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n177_), .c(new_n70_), .O(new_n188_));
  nor2   g137(.a(x16), .b(x01), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n188_), .c(new_n66_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n52_), .O(z14));
  nand2  g140(.a(new_n187_), .b(x34), .O(new_n192_));
  nand2  g141(.a(new_n156_), .b(new_n134_), .O(new_n193_));
  nor2   g142(.a(x34), .b(x33), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n169_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n193_), .O(new_n196_));
  nand3  g145(.a(new_n196_), .b(new_n106_), .c(new_n73_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n192_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  nor2   g148(.a(x16), .b(x00), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n67_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n199_), .O(z15));
endmodule


