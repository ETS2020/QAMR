// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n55_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n61_), .b(x20), .c(new_n62_), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n58_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n58_), .c(new_n65_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  inv1   g016(.a(x20), .O(new_n68_));
  nand4  g017(.a(new_n67_), .b(new_n68_), .c(new_n53_), .d(new_n52_), .O(new_n69_));
  oai21  g018(.a(new_n62_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  aoi21  g020(.a(new_n70_), .b(x16), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(x23), .b(x18), .O(new_n73_));
  oai21  g022(.a(new_n72_), .b(x18), .c(new_n73_), .O(z02));
  nor3   g023(.a(x19), .b(x18), .c(x17), .O(new_n75_));
  nor3   g024(.a(x22), .b(x21), .c(x20), .O(new_n76_));
  aoi22  g025(.a(new_n76_), .b(new_n75_), .c(new_n69_), .d(x22), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n58_), .c(new_n79_), .O(z03));
  inv1   g029(.a(x23), .O(new_n81_));
  and2   g030(.a(new_n76_), .b(new_n55_), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g032(.a(x20), .b(x19), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n67_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n83_), .c(x16), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n58_), .b(new_n90_), .c(x18), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n89_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  nor2   g042(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nor2   g043(.a(x22), .b(x21), .O(new_n95_));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n85_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n94_), .c(x16), .O(new_n99_));
  inv1   g048(.a(x10), .O(new_n100_));
  aoi21  g049(.a(new_n58_), .b(new_n100_), .c(x18), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n99_), .O(z05));
  aoi21  g051(.a(x25), .b(x16), .c(x18), .O(new_n103_));
  inv1   g052(.a(x18), .O(new_n104_));
  nor2   g053(.a(x24), .b(x22), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n84_), .c(new_n67_), .d(new_n52_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x25), .O(new_n107_));
  inv1   g056(.a(new_n69_), .O(new_n108_));
  inv1   g057(.a(x22), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n93_), .c(new_n81_), .d(new_n109_), .O(new_n111_));
  inv1   g060(.a(new_n111_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n107_), .c(new_n58_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n114_), .c(new_n104_), .O(new_n116_));
  oai21  g065(.a(new_n103_), .b(new_n81_), .c(new_n116_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n58_), .c(new_n104_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x23), .O(new_n120_));
  nor2   g069(.a(x25), .b(x24), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n95_), .c(new_n84_), .d(new_n52_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x26), .O(new_n123_));
  nor3   g072(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n86_), .c(new_n108_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n123_), .c(new_n58_), .O(new_n126_));
  nor2   g075(.a(x16), .b(x08), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n126_), .c(new_n104_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n120_), .O(z07));
  aoi21  g078(.a(x27), .b(x16), .c(x18), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n105_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n69_), .c(x27), .O(new_n133_));
  nor3   g082(.a(x27), .b(x26), .c(x25), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n96_), .c(new_n76_), .d(new_n55_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n133_), .c(new_n58_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x07), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n136_), .c(new_n104_), .O(new_n138_));
  oai21  g087(.a(new_n130_), .b(new_n81_), .c(new_n138_), .O(z08));
  nand2  g088(.a(new_n135_), .b(x28), .O(new_n140_));
  nor3   g089(.a(x28), .b(x27), .c(x26), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n121_), .c(new_n82_), .d(new_n81_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  aoi21  g094(.a(new_n58_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z09));
  nand3  g096(.a(new_n86_), .b(new_n62_), .c(new_n67_), .O(new_n148_));
  inv1   g097(.a(x27), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n124_), .c(new_n149_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  aoi21  g101(.a(new_n142_), .b(x29), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  aoi21  g103(.a(new_n58_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n58_), .c(new_n155_), .O(z10));
  nand4  g105(.a(new_n150_), .b(new_n124_), .c(new_n88_), .d(new_n149_), .O(new_n157_));
  nor2   g106(.a(x28), .b(x27), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n158_), .c(new_n124_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n148_), .O(new_n161_));
  aoi21  g110(.a(new_n157_), .b(x30), .c(new_n161_), .O(new_n162_));
  inv1   g111(.a(x04), .O(new_n163_));
  aoi21  g112(.a(new_n58_), .b(new_n163_), .c(x18), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n58_), .c(new_n164_), .O(z11));
  inv1   g114(.a(x31), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n58_), .c(new_n104_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x23), .O(new_n168_));
  inv1   g117(.a(x28), .O(new_n169_));
  nor2   g118(.a(x27), .b(x26), .O(new_n170_));
  nand4  g119(.a(new_n159_), .b(new_n170_), .c(new_n169_), .d(new_n110_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n106_), .c(x31), .O(new_n172_));
  inv1   g121(.a(new_n97_), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n150_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n134_), .c(new_n173_), .d(new_n62_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n172_), .c(new_n58_), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n178_), .c(new_n104_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n168_), .O(z12));
  nand2  g130(.a(new_n177_), .b(x32), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n159_), .O(new_n184_));
  inv1   g133(.a(new_n184_), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n158_), .c(new_n131_), .d(new_n98_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n182_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x16), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n58_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z13));
  inv1   g140(.a(x33), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n58_), .c(new_n104_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(x23), .O(new_n194_));
  nand4  g143(.a(new_n183_), .b(new_n159_), .c(new_n158_), .d(new_n118_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n122_), .c(x33), .O(new_n196_));
  nor2   g145(.a(new_n111_), .b(new_n69_), .O(new_n197_));
  nor2   g146(.a(x33), .b(x32), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n174_), .c(new_n150_), .d(new_n170_), .O(new_n199_));
  inv1   g148(.a(new_n199_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n197_), .O(new_n201_));
  aoi21  g150(.a(new_n201_), .b(new_n196_), .c(new_n58_), .O(new_n202_));
  nor2   g151(.a(x16), .b(x01), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n202_), .c(new_n104_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n194_), .O(z14));
  inv1   g154(.a(x34), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n58_), .c(new_n104_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(x23), .O(new_n208_));
  oai21  g157(.a(new_n199_), .b(new_n122_), .c(x34), .O(new_n209_));
  nand2  g158(.a(new_n150_), .b(new_n170_), .O(new_n210_));
  inv1   g159(.a(new_n210_), .O(new_n211_));
  inv1   g160(.a(x30), .O(new_n212_));
  nand2  g161(.a(new_n166_), .b(new_n212_), .O(new_n213_));
  inv1   g162(.a(x32), .O(new_n214_));
  nand3  g163(.a(new_n206_), .b(new_n192_), .c(new_n214_), .O(new_n215_));
  nor2   g164(.a(new_n215_), .b(new_n213_), .O(new_n216_));
  nand3  g165(.a(new_n216_), .b(new_n211_), .c(new_n197_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n209_), .c(new_n58_), .O(new_n218_));
  nor2   g167(.a(x16), .b(x00), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n218_), .c(new_n104_), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n208_), .O(z15));
endmodule


