// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x28), .O(new_n60_));
  nand2  g009(.a(x34), .b(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  aoi21  g011(.a(new_n58_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n56_), .O(z00));
  inv1   g013(.a(new_n61_), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  nor2   g015(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  nor3   g016(.a(x20), .b(x19), .c(x17), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  inv1   g018(.a(x14), .O(new_n70_));
  aoi21  g019(.a(new_n58_), .b(new_n70_), .c(x18), .O(new_n71_));
  aoi21  g020(.a(new_n71_), .b(new_n69_), .c(new_n65_), .O(z01));
  inv1   g021(.a(x21), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n55_), .O(new_n75_));
  oai21  g024(.a(new_n68_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n65_), .O(z02));
  inv1   g029(.a(x22), .O(new_n81_));
  inv1   g030(.a(new_n75_), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n55_), .c(new_n66_), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n81_), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n58_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n65_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  inv1   g039(.a(new_n84_), .O(new_n91_));
  nor2   g040(.a(x20), .b(x19), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n73_), .d(new_n52_), .O(new_n94_));
  oai21  g043(.a(new_n91_), .b(new_n90_), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  aoi21  g046(.a(new_n58_), .b(new_n97_), .c(x18), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n65_), .O(z04));
  inv1   g048(.a(x10), .O(new_n100_));
  oai21  g049(.a(x18), .b(new_n100_), .c(new_n58_), .O(new_n101_));
  nor2   g050(.a(x17), .b(new_n58_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n83_), .d(new_n92_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n59_), .O(new_n105_));
  oai21  g054(.a(x24), .b(x23), .c(x18), .O(new_n106_));
  nand3  g055(.a(new_n94_), .b(x24), .c(x16), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n106_), .c(new_n61_), .O(new_n108_));
  aoi21  g057(.a(new_n105_), .b(new_n61_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n101_), .O(z05));
  nand3  g059(.a(new_n103_), .b(new_n83_), .c(new_n68_), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n93_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n75_), .O(new_n114_));
  aoi21  g063(.a(new_n111_), .b(x25), .c(new_n114_), .O(new_n115_));
  nor2   g064(.a(x16), .b(x09), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n62_), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n58_), .c(new_n117_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n93_), .O(new_n121_));
  oai22  g070(.a(new_n121_), .b(new_n75_), .c(new_n114_), .d(new_n119_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x08), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n62_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z07));
  oai21  g075(.a(new_n121_), .b(new_n75_), .c(x27), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n103_), .c(new_n91_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x07), .O(new_n132_));
  aoi21  g081(.a(new_n58_), .b(new_n132_), .c(x18), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n131_), .c(new_n65_), .O(z08));
  inv1   g083(.a(x34), .O(new_n135_));
  nand3  g084(.a(new_n102_), .b(new_n74_), .c(new_n53_), .O(new_n136_));
  nand3  g085(.a(new_n128_), .b(new_n103_), .c(new_n81_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n60_), .O(new_n139_));
  nand3  g088(.a(new_n129_), .b(x28), .c(x16), .O(new_n140_));
  inv1   g089(.a(x06), .O(new_n141_));
  aoi21  g090(.a(new_n58_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n140_), .c(new_n139_), .O(z09));
  nand4  g092(.a(new_n83_), .b(new_n55_), .c(new_n66_), .d(x16), .O(new_n144_));
  nor2   g093(.a(x29), .b(x27), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n112_), .c(new_n119_), .d(new_n90_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n144_), .c(new_n135_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n60_), .O(new_n148_));
  inv1   g097(.a(x27), .O(new_n149_));
  nand4  g098(.a(new_n120_), .b(new_n135_), .c(new_n60_), .d(new_n149_), .O(new_n150_));
  or2    g099(.a(new_n150_), .b(new_n94_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(x29), .c(x16), .O(new_n152_));
  inv1   g101(.a(x05), .O(new_n153_));
  aoi21  g102(.a(new_n58_), .b(new_n153_), .c(x18), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n148_), .O(z10));
  oai21  g104(.a(x16), .b(x04), .c(new_n59_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n61_), .O(new_n157_));
  inv1   g106(.a(x30), .O(new_n158_));
  oai21  g107(.a(new_n146_), .b(new_n84_), .c(new_n135_), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n60_), .c(new_n158_), .O(new_n160_));
  nor2   g109(.a(x29), .b(x28), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n128_), .c(new_n135_), .d(new_n158_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n111_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n160_), .c(x16), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n157_), .O(z11));
  oai21  g114(.a(x16), .b(x03), .c(new_n59_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n61_), .O(new_n167_));
  inv1   g116(.a(x31), .O(new_n168_));
  inv1   g117(.a(x24), .O(new_n169_));
  nor2   g118(.a(x26), .b(x25), .O(new_n170_));
  nor2   g119(.a(x30), .b(x29), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n149_), .d(new_n169_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n94_), .c(new_n135_), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n60_), .c(new_n168_), .O(new_n174_));
  nor2   g123(.a(x28), .b(x27), .O(new_n175_));
  nor2   g124(.a(x34), .b(x31), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n171_), .d(new_n170_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n111_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n174_), .c(x16), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n167_), .O(z12));
  oai21  g129(.a(x16), .b(x02), .c(new_n59_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n61_), .O(new_n182_));
  inv1   g131(.a(x32), .O(new_n183_));
  nor2   g132(.a(x31), .b(x30), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n145_), .c(new_n170_), .d(new_n169_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n94_), .c(new_n135_), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n60_), .c(new_n183_), .O(new_n187_));
  nor2   g136(.a(x27), .b(x26), .O(new_n188_));
  nor2   g137(.a(x34), .b(x32), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n184_), .c(new_n161_), .d(new_n188_), .O(new_n190_));
  nor3   g139(.a(new_n190_), .b(new_n113_), .c(new_n75_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n187_), .c(x16), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n182_), .O(z13));
  inv1   g142(.a(x01), .O(new_n194_));
  oai21  g143(.a(x18), .b(new_n194_), .c(new_n58_), .O(new_n195_));
  nand2  g144(.a(new_n61_), .b(x18), .O(new_n196_));
  nor3   g145(.a(x34), .b(x33), .c(x32), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n184_), .c(new_n145_), .d(new_n170_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n104_), .c(new_n135_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n60_), .O(new_n200_));
  nor2   g149(.a(x32), .b(x31), .O(new_n201_));
  nand3  g150(.a(new_n201_), .b(new_n171_), .c(new_n128_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n111_), .c(new_n135_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n60_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(x33), .c(x16), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n200_), .c(new_n196_), .d(new_n195_), .O(z14));
  oai21  g155(.a(x16), .b(x00), .c(new_n59_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n61_), .O(new_n208_));
  nand2  g157(.a(new_n161_), .b(new_n188_), .O(new_n209_));
  nand2  g158(.a(new_n197_), .b(new_n184_), .O(new_n210_));
  nor2   g159(.a(new_n210_), .b(new_n209_), .O(new_n211_));
  aoi22  g160(.a(new_n211_), .b(new_n114_), .c(x34), .d(x28), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n58_), .c(new_n208_), .O(z15));
endmodule


