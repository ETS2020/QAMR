// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x20), .c(new_n52_), .O(z00));
  nor2   g008(.a(x16), .b(x14), .O(new_n60_));
  aoi21  g009(.a(new_n54_), .b(x16), .c(new_n60_), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(x20), .c(new_n52_), .O(z01));
  inv1   g011(.a(x20), .O(new_n63_));
  inv1   g012(.a(x16), .O(new_n64_));
  nand2  g013(.a(new_n55_), .b(x21), .O(new_n65_));
  inv1   g014(.a(x17), .O(new_n66_));
  inv1   g015(.a(x19), .O(new_n67_));
  inv1   g016(.a(x21), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n65_), .c(new_n64_), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n70_), .c(new_n63_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n54_), .b(new_n74_), .c(new_n68_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n69_), .b(x22), .c(new_n76_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  nand2  g027(.a(new_n63_), .b(new_n52_), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  oai21  g029(.a(new_n77_), .b(new_n64_), .c(new_n80_), .O(z03));
  nor3   g030(.a(x23), .b(x22), .c(x21), .O(new_n82_));
  aoi22  g031(.a(new_n82_), .b(new_n54_), .c(new_n75_), .d(x23), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n64_), .c(new_n85_), .O(z04));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n54_), .c(new_n68_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x21), .b(x19), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n74_), .d(new_n66_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n89_), .c(new_n64_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x10), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(new_n63_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n52_), .O(z05));
  inv1   g045(.a(new_n69_), .O(new_n97_));
  inv1   g046(.a(x23), .O(new_n98_));
  inv1   g047(.a(x24), .O(new_n99_));
  inv1   g048(.a(x25), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(new_n74_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  aoi22  g051(.a(new_n102_), .b(new_n97_), .c(new_n92_), .d(x25), .O(new_n103_));
  nor2   g052(.a(x16), .b(x09), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n79_), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n64_), .c(new_n105_), .O(z06));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n87_), .c(new_n90_), .d(new_n66_), .O(new_n108_));
  nor2   g057(.a(x26), .b(x25), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n91_), .O(new_n110_));
  inv1   g059(.a(new_n110_), .O(new_n111_));
  aoi22  g060(.a(new_n111_), .b(new_n76_), .c(new_n108_), .d(x26), .O(new_n112_));
  nor2   g061(.a(x16), .b(x08), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n79_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n64_), .c(new_n114_), .O(z07));
  oai21  g064(.a(new_n110_), .b(new_n75_), .c(x27), .O(new_n116_));
  nor3   g065(.a(x27), .b(x26), .c(x25), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n91_), .c(new_n76_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n79_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(z08));
  inv1   g072(.a(x26), .O(new_n124_));
  nor2   g073(.a(x28), .b(x27), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n107_), .c(new_n124_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n88_), .O(new_n127_));
  aoi21  g076(.a(new_n118_), .b(x28), .c(new_n127_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x06), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n79_), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n64_), .c(new_n130_), .O(z09));
  oai21  g080(.a(new_n126_), .b(new_n88_), .c(x29), .O(new_n132_));
  nor3   g081(.a(x26), .b(x25), .c(x24), .O(new_n133_));
  nor3   g082(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n133_), .c(new_n82_), .d(new_n54_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n132_), .c(new_n64_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x05), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n136_), .c(new_n63_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n52_), .O(z10));
  inv1   g088(.a(x28), .O(new_n140_));
  nor2   g089(.a(x27), .b(x26), .O(new_n141_));
  nor2   g090(.a(x30), .b(x29), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n100_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n92_), .O(new_n144_));
  aoi21  g093(.a(new_n135_), .b(x30), .c(new_n144_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x04), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n79_), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n64_), .c(new_n147_), .O(z11));
  oai21  g097(.a(new_n143_), .b(new_n92_), .c(x31), .O(new_n149_));
  nand3  g098(.a(new_n99_), .b(new_n98_), .c(new_n74_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n69_), .O(new_n151_));
  inv1   g100(.a(x29), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x31), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n140_), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n117_), .c(new_n151_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n149_), .c(new_n64_), .O(new_n158_));
  nor2   g107(.a(x16), .b(x03), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n158_), .c(new_n63_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n52_), .O(z12));
  nand2  g110(.a(new_n157_), .b(x32), .O(new_n162_));
  nor2   g111(.a(x32), .b(x31), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n142_), .c(new_n125_), .d(new_n124_), .O(new_n164_));
  or2    g113(.a(new_n164_), .b(new_n108_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x16), .O(new_n167_));
  nor2   g116(.a(x16), .b(x02), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n79_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(z13));
  oai21  g119(.a(new_n164_), .b(new_n108_), .c(x33), .O(new_n171_));
  inv1   g120(.a(x27), .O(new_n172_));
  nand4  g121(.a(new_n152_), .b(new_n140_), .c(new_n172_), .d(new_n124_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  inv1   g123(.a(x32), .O(new_n175_));
  inv1   g124(.a(x33), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n154_), .d(new_n153_), .O(new_n177_));
  inv1   g126(.a(new_n177_), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n174_), .c(new_n102_), .d(new_n97_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n171_), .c(new_n64_), .O(new_n180_));
  nor2   g129(.a(x16), .b(x01), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n180_), .c(new_n63_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n52_), .O(z14));
  nand2  g132(.a(new_n179_), .b(x34), .O(new_n184_));
  nand2  g133(.a(new_n142_), .b(new_n125_), .O(new_n185_));
  nor2   g134(.a(x34), .b(x33), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n163_), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(new_n185_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n111_), .c(new_n76_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n184_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(x16), .O(new_n191_));
  nor2   g140(.a(x16), .b(x00), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n79_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n191_), .O(z15));
endmodule


