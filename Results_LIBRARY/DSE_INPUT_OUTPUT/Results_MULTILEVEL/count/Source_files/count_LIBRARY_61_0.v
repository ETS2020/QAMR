// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x28), .O(new_n60_));
  inv1   g009(.a(x34), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n56_), .O(z00));
  inv1   g014(.a(new_n62_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  nor2   g016(.a(new_n55_), .b(new_n67_), .O(new_n68_));
  nor3   g017(.a(x20), .b(x19), .c(x17), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n68_), .c(x16), .O(new_n70_));
  inv1   g019(.a(x14), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(new_n71_), .c(x18), .O(new_n72_));
  aoi21  g021(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nor2   g023(.a(x21), .b(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n55_), .O(new_n76_));
  oai21  g025(.a(new_n69_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x16), .O(new_n78_));
  inv1   g027(.a(x13), .O(new_n79_));
  aoi21  g028(.a(new_n58_), .b(new_n79_), .c(x18), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(z02));
  inv1   g030(.a(x22), .O(new_n82_));
  inv1   g031(.a(new_n76_), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n55_), .c(new_n67_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  inv1   g036(.a(x12), .O(new_n88_));
  aoi21  g037(.a(new_n58_), .b(new_n88_), .c(x18), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n66_), .O(z03));
  inv1   g039(.a(x23), .O(new_n91_));
  inv1   g040(.a(new_n85_), .O(new_n92_));
  nor2   g041(.a(x20), .b(x19), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n93_), .c(new_n74_), .d(new_n52_), .O(new_n95_));
  oai21  g044(.a(new_n92_), .b(new_n91_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  inv1   g046(.a(x11), .O(new_n98_));
  aoi21  g047(.a(new_n58_), .b(new_n98_), .c(x18), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n66_), .O(z04));
  inv1   g049(.a(x10), .O(new_n101_));
  oai21  g050(.a(x18), .b(new_n101_), .c(new_n58_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n84_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n93_), .c(new_n52_), .d(x16), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n59_), .O(new_n107_));
  oai21  g056(.a(x24), .b(x23), .c(x18), .O(new_n108_));
  nand3  g057(.a(new_n95_), .b(x24), .c(x16), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n62_), .O(new_n110_));
  aoi21  g059(.a(new_n107_), .b(new_n62_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n102_), .O(z05));
  nand2  g061(.a(new_n105_), .b(new_n69_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x25), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n94_), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n83_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x09), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(z06));
  nor3   g072(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n94_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n76_), .O(new_n126_));
  aoi21  g075(.a(new_n118_), .b(x26), .c(new_n126_), .O(new_n127_));
  nor2   g076(.a(x16), .b(x08), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n63_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n58_), .c(new_n129_), .O(z07));
  oai21  g079(.a(new_n125_), .b(new_n76_), .c(x27), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n103_), .c(new_n92_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g084(.a(x07), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n135_), .c(new_n66_), .O(z08));
  nand4  g087(.a(new_n75_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n139_));
  nand3  g088(.a(new_n132_), .b(new_n103_), .c(new_n82_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n139_), .c(x34), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n60_), .O(new_n142_));
  nand3  g091(.a(new_n133_), .b(x28), .c(x16), .O(new_n143_));
  inv1   g092(.a(x06), .O(new_n144_));
  aoi21  g093(.a(new_n58_), .b(new_n144_), .c(x18), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n142_), .O(z09));
  nand4  g095(.a(new_n84_), .b(new_n55_), .c(new_n67_), .d(x16), .O(new_n147_));
  inv1   g096(.a(x26), .O(new_n148_));
  nor2   g097(.a(x29), .b(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n115_), .c(new_n148_), .d(new_n91_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n147_), .c(x34), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n60_), .O(new_n152_));
  inv1   g101(.a(x27), .O(new_n153_));
  nand4  g102(.a(new_n124_), .b(x34), .c(new_n60_), .d(new_n153_), .O(new_n154_));
  or2    g103(.a(new_n154_), .b(new_n95_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(x29), .c(x16), .O(new_n156_));
  inv1   g105(.a(x05), .O(new_n157_));
  aoi21  g106(.a(new_n58_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n156_), .c(new_n152_), .O(z10));
  oai21  g108(.a(x16), .b(x04), .c(new_n59_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n62_), .O(new_n161_));
  inv1   g110(.a(x30), .O(new_n162_));
  oai21  g111(.a(new_n150_), .b(new_n85_), .c(x34), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n60_), .c(new_n162_), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n132_), .c(x34), .d(new_n162_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n113_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(x16), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n161_), .O(z11));
  oai21  g118(.a(x16), .b(x03), .c(new_n59_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n62_), .O(new_n171_));
  inv1   g120(.a(x31), .O(new_n172_));
  inv1   g121(.a(x24), .O(new_n173_));
  nor2   g122(.a(x26), .b(x25), .O(new_n174_));
  nor2   g123(.a(x30), .b(x29), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n153_), .d(new_n173_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n95_), .c(x34), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n60_), .c(new_n172_), .O(new_n178_));
  nor2   g127(.a(x28), .b(x27), .O(new_n179_));
  nor2   g128(.a(new_n61_), .b(x31), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n179_), .c(new_n175_), .d(new_n174_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n113_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n178_), .c(x16), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n171_), .O(z12));
  oai21  g133(.a(x16), .b(x02), .c(new_n59_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n62_), .O(new_n186_));
  inv1   g135(.a(x32), .O(new_n187_));
  nor2   g136(.a(x31), .b(x30), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n149_), .c(new_n174_), .d(new_n173_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n95_), .c(x34), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n60_), .c(new_n187_), .O(new_n191_));
  nand3  g140(.a(new_n165_), .b(new_n153_), .c(new_n148_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand3  g142(.a(new_n188_), .b(x34), .c(new_n187_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n193_), .c(new_n117_), .d(new_n83_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n191_), .c(x16), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n186_), .O(z13));
  nor2   g148(.a(x33), .b(x32), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n188_), .O(new_n201_));
  inv1   g150(.a(new_n201_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n149_), .c(new_n174_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n106_), .c(x34), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n60_), .O(new_n205_));
  nand3  g154(.a(new_n196_), .b(x33), .c(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n58_), .b(new_n207_), .c(x18), .O(new_n208_));
  nand3  g157(.a(new_n208_), .b(new_n206_), .c(new_n205_), .O(z14));
  inv1   g158(.a(x00), .O(new_n210_));
  aoi21  g159(.a(new_n58_), .b(new_n210_), .c(x18), .O(new_n211_));
  nand2  g160(.a(new_n202_), .b(new_n193_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n118_), .c(x34), .O(new_n213_));
  oai22  g162(.a(new_n213_), .b(new_n58_), .c(new_n211_), .d(new_n66_), .O(z15));
endmodule


