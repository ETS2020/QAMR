// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n53_), .c(new_n52_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(new_n67_));
  nand2  g016(.a(x26), .b(x18), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n67_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand4  g019(.a(new_n70_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  aoi21  g021(.a(new_n64_), .b(x21), .c(new_n72_), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n58_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n58_), .c(new_n75_), .O(z02));
  nand2  g025(.a(new_n71_), .b(x22), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n55_), .c(new_n63_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n58_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(new_n61_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n68_), .O(z03));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(x23), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n84_), .c(new_n70_), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  aoi21  g036(.a(new_n79_), .b(x23), .c(new_n87_), .O(new_n88_));
  inv1   g037(.a(x11), .O(new_n89_));
  aoi21  g038(.a(new_n58_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(z04));
  nor2   g040(.a(x20), .b(x19), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n70_), .d(new_n52_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x24), .O(new_n95_));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n78_), .c(new_n92_), .d(new_n52_), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n58_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x10), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(new_n98_), .c(new_n61_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n68_), .O(z05));
  nand2  g050(.a(new_n97_), .b(x25), .O(new_n102_));
  inv1   g051(.a(x24), .O(new_n103_));
  inv1   g052(.a(x25), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n85_), .d(new_n84_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n72_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n102_), .c(new_n58_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n108_), .c(new_n61_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n68_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nor2   g061(.a(x21), .b(x20), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n114_));
  nor3   g063(.a(x26), .b(x25), .c(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n93_), .O(new_n116_));
  oai22  g065(.a(new_n116_), .b(new_n114_), .c(x16), .d(x08), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n61_), .O(new_n118_));
  aoi21  g067(.a(new_n107_), .b(x16), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n112_), .c(new_n118_), .O(z07));
  nand3  g069(.a(new_n115_), .b(new_n93_), .c(new_n72_), .O(new_n121_));
  nor3   g070(.a(x27), .b(x26), .c(x25), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n96_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n79_), .O(new_n124_));
  aoi21  g073(.a(new_n121_), .b(x27), .c(new_n124_), .O(new_n125_));
  inv1   g074(.a(x07), .O(new_n126_));
  aoi21  g075(.a(new_n58_), .b(new_n126_), .c(x18), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n58_), .c(new_n127_), .O(z08));
  inv1   g077(.a(x28), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n58_), .c(new_n61_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x26), .O(new_n131_));
  nor2   g080(.a(x27), .b(x25), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n93_), .c(new_n103_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n71_), .c(x28), .O(new_n134_));
  nor3   g083(.a(x22), .b(x21), .c(x20), .O(new_n135_));
  nor3   g084(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nor3   g085(.a(x28), .b(x27), .c(x26), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n136_), .c(new_n135_), .d(new_n55_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n134_), .c(new_n58_), .O(new_n139_));
  nor2   g088(.a(x16), .b(x06), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n139_), .c(new_n61_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n131_), .O(z09));
  inv1   g091(.a(x27), .O(new_n143_));
  nor2   g092(.a(x29), .b(x28), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n115_), .c(new_n143_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n94_), .O(new_n146_));
  aoi21  g095(.a(new_n138_), .b(x29), .c(new_n146_), .O(new_n147_));
  inv1   g096(.a(x05), .O(new_n148_));
  aoi21  g097(.a(new_n58_), .b(new_n148_), .c(x18), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(new_n58_), .c(new_n149_), .O(z10));
  inv1   g099(.a(x30), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n58_), .c(new_n61_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x26), .O(new_n153_));
  nor2   g102(.a(x25), .b(x24), .O(new_n154_));
  nand4  g103(.a(new_n144_), .b(new_n154_), .c(new_n143_), .d(new_n85_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n79_), .c(x30), .O(new_n156_));
  nor2   g105(.a(x28), .b(x27), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n115_), .c(new_n87_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n156_), .c(new_n58_), .O(new_n162_));
  nor2   g111(.a(x16), .b(x04), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n162_), .c(new_n61_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n153_), .O(z11));
  nand2  g114(.a(new_n161_), .b(x31), .O(new_n166_));
  inv1   g115(.a(new_n97_), .O(new_n167_));
  nor2   g116(.a(x31), .b(x30), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n144_), .c(new_n122_), .d(new_n167_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x03), .O(new_n172_));
  aoi21  g121(.a(new_n58_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z12));
  inv1   g123(.a(x32), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n58_), .c(new_n61_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x26), .O(new_n177_));
  nand4  g126(.a(new_n168_), .b(new_n144_), .c(new_n132_), .d(new_n103_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n94_), .c(x32), .O(new_n179_));
  inv1   g128(.a(new_n64_), .O(new_n180_));
  nand4  g129(.a(new_n103_), .b(new_n85_), .c(new_n84_), .d(new_n70_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  nand4  g131(.a(new_n129_), .b(new_n143_), .c(new_n112_), .d(new_n104_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  inv1   g133(.a(x29), .O(new_n185_));
  inv1   g134(.a(x31), .O(new_n186_));
  nand4  g135(.a(new_n175_), .b(new_n186_), .c(new_n151_), .d(new_n185_), .O(new_n187_));
  inv1   g136(.a(new_n187_), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n184_), .c(new_n182_), .d(new_n180_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n179_), .c(new_n58_), .O(new_n190_));
  nor2   g139(.a(x16), .b(x02), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n190_), .c(new_n61_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n177_), .O(z13));
  inv1   g142(.a(x33), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n58_), .c(new_n61_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(x26), .O(new_n196_));
  nor2   g145(.a(x32), .b(x31), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n158_), .c(new_n157_), .d(new_n104_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n97_), .c(x33), .O(new_n199_));
  nand4  g148(.a(new_n185_), .b(new_n129_), .c(new_n143_), .d(new_n112_), .O(new_n200_));
  inv1   g149(.a(new_n200_), .O(new_n201_));
  nand4  g150(.a(new_n194_), .b(new_n175_), .c(new_n186_), .d(new_n151_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n201_), .c(new_n106_), .d(new_n72_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n199_), .c(new_n58_), .O(new_n205_));
  nor2   g154(.a(x16), .b(x01), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n205_), .c(new_n61_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n196_), .O(z14));
  nand2  g157(.a(new_n204_), .b(x34), .O(new_n209_));
  nor2   g158(.a(new_n105_), .b(new_n71_), .O(new_n210_));
  nor3   g159(.a(x34), .b(x33), .c(x32), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n201_), .c(new_n168_), .d(new_n210_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n209_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(x16), .O(new_n214_));
  inv1   g163(.a(x00), .O(new_n215_));
  aoi21  g164(.a(new_n58_), .b(new_n215_), .c(x18), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n214_), .O(z15));
endmodule


