// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n217_, new_n218_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n236_, new_n237_, new_n238_, new_n239_, new_n240_,
    new_n241_, new_n242_, new_n243_, new_n244_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x34), .b(x22), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  nand3  g019(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n71_), .b(x21), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n61_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n55_), .c(new_n73_), .d(x22), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n58_), .c(new_n82_), .O(z03));
  inv1   g032(.a(new_n60_), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(x11), .O(new_n85_));
  nor2   g034(.a(x34), .b(new_n59_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(new_n58_), .O(new_n87_));
  inv1   g036(.a(x22), .O(new_n88_));
  nor2   g037(.a(x34), .b(x23), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n88_), .c(x18), .O(new_n90_));
  nand2  g039(.a(new_n73_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x21), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n55_), .c(new_n64_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(x22), .O(new_n94_));
  inv1   g043(.a(x34), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(x23), .c(x22), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n90_), .c(new_n87_), .O(z04));
  inv1   g048(.a(x10), .O(new_n100_));
  oai21  g049(.a(x18), .b(new_n100_), .c(new_n58_), .O(new_n101_));
  and2   g050(.a(x24), .b(x16), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(x34), .c(x22), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n72_), .c(new_n55_), .d(x16), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n59_), .O(new_n106_));
  oai21  g055(.a(new_n102_), .b(x18), .c(x23), .O(new_n107_));
  oai21  g056(.a(new_n89_), .b(x24), .c(x18), .O(new_n108_));
  nand3  g057(.a(new_n73_), .b(x24), .c(x16), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  aoi21  g059(.a(new_n106_), .b(new_n88_), .c(new_n110_), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n103_), .c(new_n101_), .O(z05));
  nor2   g061(.a(x16), .b(x09), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(x18), .c(new_n60_), .O(new_n114_));
  nor3   g063(.a(x24), .b(x23), .c(x21), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n66_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x25), .O(new_n117_));
  nor2   g066(.a(x25), .b(x24), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n92_), .c(new_n66_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(x22), .O(new_n120_));
  nand3  g069(.a(new_n95_), .b(x25), .c(x22), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n114_), .O(z06));
  nor2   g073(.a(new_n84_), .b(x08), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n86_), .c(new_n58_), .O(new_n126_));
  inv1   g075(.a(x26), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(x22), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n86_), .c(x25), .O(new_n129_));
  aoi21  g078(.a(new_n115_), .b(new_n66_), .c(new_n127_), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n104_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n73_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n130_), .c(new_n88_), .O(new_n134_));
  nand3  g083(.a(new_n95_), .b(x26), .c(x22), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(new_n129_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  oai21  g086(.a(x34), .b(x26), .c(x22), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(x18), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n137_), .c(new_n126_), .O(z07));
  inv1   g089(.a(x27), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n58_), .c(new_n95_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x22), .O(new_n143_));
  nand2  g092(.a(new_n79_), .b(new_n55_), .O(new_n144_));
  nor3   g093(.a(x27), .b(x26), .c(x25), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n104_), .O(new_n146_));
  oai22  g095(.a(new_n146_), .b(new_n144_), .c(new_n133_), .d(new_n141_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g097(.a(x07), .O(new_n149_));
  aoi21  g098(.a(new_n58_), .b(new_n149_), .c(x18), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n148_), .c(new_n143_), .O(z08));
  nand2  g100(.a(x28), .b(x16), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n95_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x22), .O(new_n154_));
  oai21  g103(.a(new_n146_), .b(new_n73_), .c(x28), .O(new_n155_));
  inv1   g104(.a(x23), .O(new_n156_));
  nor2   g105(.a(x28), .b(x27), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n118_), .c(new_n127_), .d(new_n156_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n144_), .c(new_n155_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  inv1   g109(.a(x06), .O(new_n161_));
  aoi21  g110(.a(new_n58_), .b(new_n161_), .c(x18), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n154_), .O(z09));
  nor2   g112(.a(new_n84_), .b(x05), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n86_), .c(new_n58_), .O(new_n165_));
  oai21  g114(.a(x34), .b(x29), .c(x22), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x18), .O(new_n167_));
  nand3  g116(.a(new_n157_), .b(new_n118_), .c(new_n127_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n93_), .c(x29), .O(new_n169_));
  nor3   g118(.a(x23), .b(x21), .c(x20), .O(new_n170_));
  nor3   g119(.a(x26), .b(x25), .c(x24), .O(new_n171_));
  nor3   g120(.a(x29), .b(x28), .c(x27), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n171_), .c(new_n170_), .d(new_n55_), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n169_), .c(x22), .O(new_n174_));
  nand3  g123(.a(new_n95_), .b(x29), .c(x22), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n174_), .c(x16), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n167_), .c(new_n165_), .O(z10));
  nand2  g127(.a(x30), .b(x16), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n95_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(x22), .O(new_n181_));
  nand2  g130(.a(new_n173_), .b(x30), .O(new_n182_));
  inv1   g131(.a(x21), .O(new_n183_));
  nand3  g132(.a(new_n156_), .b(new_n88_), .c(new_n183_), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n71_), .O(new_n185_));
  nor2   g134(.a(x30), .b(x29), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n185_), .c(new_n171_), .d(new_n157_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n182_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  inv1   g138(.a(x04), .O(new_n190_));
  aoi21  g139(.a(new_n58_), .b(new_n190_), .c(x18), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n189_), .c(new_n181_), .O(z11));
  inv1   g141(.a(x31), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n58_), .c(new_n95_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x22), .O(new_n195_));
  inv1   g144(.a(new_n116_), .O(new_n196_));
  nor3   g145(.a(x30), .b(x29), .c(x28), .O(new_n197_));
  and2   g146(.a(new_n197_), .b(new_n145_), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n196_), .c(new_n193_), .O(new_n199_));
  nand4  g148(.a(new_n104_), .b(new_n66_), .c(new_n88_), .d(new_n183_), .O(new_n200_));
  nor2   g149(.a(x29), .b(x28), .O(new_n201_));
  nor2   g150(.a(x31), .b(x30), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n201_), .c(new_n145_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n200_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n199_), .c(x16), .O(new_n205_));
  inv1   g154(.a(x03), .O(new_n206_));
  aoi21  g155(.a(new_n58_), .b(new_n206_), .c(x18), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n205_), .c(new_n195_), .O(z12));
  nand2  g157(.a(x32), .b(x16), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n95_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(x22), .O(new_n211_));
  oai21  g160(.a(new_n203_), .b(new_n116_), .c(x32), .O(new_n212_));
  nor2   g161(.a(x32), .b(x31), .O(new_n213_));
  nand4  g162(.a(new_n213_), .b(new_n186_), .c(new_n157_), .d(new_n131_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n200_), .c(new_n212_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x16), .O(new_n216_));
  inv1   g165(.a(x02), .O(new_n217_));
  aoi21  g166(.a(new_n58_), .b(new_n217_), .c(x18), .O(new_n218_));
  nand3  g167(.a(new_n218_), .b(new_n216_), .c(new_n211_), .O(z13));
  nand2  g168(.a(x33), .b(x16), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n95_), .O(new_n221_));
  nand2  g170(.a(new_n221_), .b(x22), .O(new_n222_));
  nand4  g171(.a(new_n213_), .b(new_n186_), .c(new_n157_), .d(new_n127_), .O(new_n223_));
  oai21  g172(.a(new_n223_), .b(new_n119_), .c(x33), .O(new_n224_));
  nand3  g173(.a(new_n118_), .b(new_n156_), .c(new_n88_), .O(new_n225_));
  nor2   g174(.a(new_n225_), .b(new_n73_), .O(new_n226_));
  inv1   g175(.a(new_n226_), .O(new_n227_));
  nor2   g176(.a(x27), .b(x26), .O(new_n228_));
  nor2   g177(.a(x33), .b(x32), .O(new_n229_));
  nand4  g178(.a(new_n229_), .b(new_n202_), .c(new_n201_), .d(new_n228_), .O(new_n230_));
  oai21  g179(.a(new_n230_), .b(new_n227_), .c(new_n224_), .O(new_n231_));
  nand2  g180(.a(new_n231_), .b(x16), .O(new_n232_));
  inv1   g181(.a(x01), .O(new_n233_));
  aoi21  g182(.a(new_n58_), .b(new_n233_), .c(x18), .O(new_n234_));
  nand3  g183(.a(new_n234_), .b(new_n232_), .c(new_n222_), .O(z14));
  oai21  g184(.a(new_n230_), .b(new_n119_), .c(x34), .O(new_n236_));
  nand2  g185(.a(new_n201_), .b(new_n228_), .O(new_n237_));
  inv1   g186(.a(new_n237_), .O(new_n238_));
  nor3   g187(.a(x34), .b(x33), .c(x32), .O(new_n239_));
  nand4  g188(.a(new_n239_), .b(new_n238_), .c(new_n226_), .d(new_n202_), .O(new_n240_));
  nand2  g189(.a(new_n240_), .b(new_n236_), .O(new_n241_));
  nand2  g190(.a(new_n241_), .b(x16), .O(new_n242_));
  nor2   g191(.a(x16), .b(x00), .O(new_n243_));
  nor2   g192(.a(new_n243_), .b(new_n61_), .O(new_n244_));
  nand2  g193(.a(new_n244_), .b(new_n242_), .O(z15));
endmodule


