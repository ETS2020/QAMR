// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_;
  inv1   g00(.a(x17), .O(new_n52_));
  inv1   g01(.a(x19), .O(new_n53_));
  nor2   g02(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g03(.a(x19), .b(x17), .O(new_n55_));
  oai21  g04(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g05(.a(x15), .O(new_n57_));
  inv1   g06(.a(x16), .O(new_n58_));
  inv1   g07(.a(x18), .O(new_n59_));
  nand2  g08(.a(x26), .b(new_n59_), .O(new_n60_));
  aoi21  g09(.a(new_n58_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand2  g10(.a(new_n61_), .b(new_n56_), .O(z00));
  oai21  g11(.a(x19), .b(x17), .c(x20), .O(new_n63_));
  inv1   g12(.a(x20), .O(new_n64_));
  nand3  g13(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n65_));
  aoi21  g14(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g15(.a(x16), .b(x14), .O(new_n67_));
  oai21  g16(.a(new_n67_), .b(new_n66_), .c(x26), .O(new_n68_));
  nand2  g17(.a(new_n68_), .b(new_n59_), .O(z01));
  nor2   g18(.a(x21), .b(x20), .O(new_n70_));
  aoi22  g19(.a(new_n70_), .b(new_n55_), .c(new_n65_), .d(x21), .O(new_n71_));
  nor2   g20(.a(x16), .b(x13), .O(new_n72_));
  nor2   g21(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  oai21  g22(.a(new_n71_), .b(new_n58_), .c(new_n73_), .O(z02));
  nand2  g23(.a(new_n70_), .b(new_n55_), .O(new_n75_));
  nand2  g24(.a(new_n75_), .b(x22), .O(new_n76_));
  nor2   g25(.a(x22), .b(x21), .O(new_n77_));
  nand3  g26(.a(new_n77_), .b(new_n55_), .c(new_n64_), .O(new_n78_));
  aoi21  g27(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  nor2   g28(.a(x16), .b(x12), .O(new_n80_));
  oai21  g29(.a(new_n80_), .b(new_n79_), .c(x26), .O(new_n81_));
  nand2  g30(.a(new_n81_), .b(new_n59_), .O(z03));
  inv1   g31(.a(x21), .O(new_n83_));
  inv1   g32(.a(x22), .O(new_n84_));
  inv1   g33(.a(x23), .O(new_n85_));
  nand3  g34(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  nor2   g35(.a(new_n86_), .b(new_n65_), .O(new_n87_));
  aoi21  g36(.a(new_n78_), .b(x23), .c(new_n87_), .O(new_n88_));
  nor2   g37(.a(x16), .b(x11), .O(new_n89_));
  nor2   g38(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  oai21  g39(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(z04));
  oai21  g40(.a(new_n86_), .b(new_n65_), .c(x24), .O(new_n92_));
  nor2   g41(.a(x20), .b(x19), .O(new_n93_));
  nor2   g42(.a(x24), .b(x23), .O(new_n94_));
  nand4  g43(.a(new_n94_), .b(new_n77_), .c(new_n93_), .d(new_n52_), .O(new_n95_));
  aoi21  g44(.a(new_n95_), .b(new_n92_), .c(new_n58_), .O(new_n96_));
  nor2   g45(.a(x16), .b(x10), .O(new_n97_));
  oai21  g46(.a(new_n97_), .b(new_n96_), .c(x26), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n59_), .O(z05));
  nand2  g48(.a(new_n95_), .b(x25), .O(new_n100_));
  nor2   g49(.a(x23), .b(x22), .O(new_n101_));
  nor2   g50(.a(x25), .b(x24), .O(new_n102_));
  nand4  g51(.a(new_n102_), .b(new_n101_), .c(new_n70_), .d(new_n55_), .O(new_n103_));
  aoi21  g52(.a(new_n103_), .b(new_n100_), .c(new_n58_), .O(new_n104_));
  nor2   g53(.a(x16), .b(x09), .O(new_n105_));
  oai21  g54(.a(new_n105_), .b(new_n104_), .c(x26), .O(new_n106_));
  nand2  g55(.a(new_n106_), .b(new_n59_), .O(z06));
  inv1   g56(.a(x26), .O(new_n108_));
  nor2   g57(.a(x16), .b(x08), .O(new_n109_));
  aoi21  g58(.a(new_n103_), .b(x16), .c(new_n109_), .O(new_n110_));
  oai21  g59(.a(new_n110_), .b(new_n108_), .c(new_n59_), .O(z07));
  inv1   g60(.a(new_n60_), .O(new_n112_));
  nand2  g61(.a(x27), .b(x16), .O(new_n113_));
  inv1   g62(.a(x07), .O(new_n114_));
  nand2  g63(.a(new_n58_), .b(new_n114_), .O(new_n115_));
  nand3  g64(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(z08));
  nor2   g65(.a(x16), .b(x06), .O(new_n117_));
  and2   g66(.a(x28), .b(x16), .O(new_n118_));
  oai21  g67(.a(new_n118_), .b(new_n117_), .c(x26), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n59_), .O(z09));
  nand2  g69(.a(x29), .b(x16), .O(new_n121_));
  inv1   g70(.a(x05), .O(new_n122_));
  nand2  g71(.a(new_n58_), .b(new_n122_), .O(new_n123_));
  nand3  g72(.a(new_n123_), .b(new_n121_), .c(new_n112_), .O(z10));
  nor2   g73(.a(x16), .b(x04), .O(new_n125_));
  and2   g74(.a(x30), .b(x16), .O(new_n126_));
  oai21  g75(.a(new_n126_), .b(new_n125_), .c(x26), .O(new_n127_));
  nand2  g76(.a(new_n127_), .b(new_n59_), .O(z11));
  nand2  g77(.a(x31), .b(x16), .O(new_n129_));
  inv1   g78(.a(x03), .O(new_n130_));
  nand2  g79(.a(new_n58_), .b(new_n130_), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n129_), .c(new_n112_), .O(z12));
  nor2   g81(.a(x16), .b(x02), .O(new_n133_));
  and2   g82(.a(x32), .b(x16), .O(new_n134_));
  oai21  g83(.a(new_n134_), .b(new_n133_), .c(x26), .O(new_n135_));
  nand2  g84(.a(new_n135_), .b(new_n59_), .O(z13));
  nor2   g85(.a(x16), .b(x01), .O(new_n137_));
  and2   g86(.a(x33), .b(x16), .O(new_n138_));
  oai21  g87(.a(new_n138_), .b(new_n137_), .c(x26), .O(new_n139_));
  nand2  g88(.a(new_n139_), .b(new_n59_), .O(z14));
  nand2  g89(.a(x34), .b(x16), .O(new_n141_));
  inv1   g90(.a(x00), .O(new_n142_));
  nand2  g91(.a(new_n58_), .b(new_n142_), .O(new_n143_));
  nand3  g92(.a(new_n143_), .b(new_n141_), .c(new_n112_), .O(z15));
endmodule


