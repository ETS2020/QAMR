// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nand2  g002(.a(x20), .b(x18), .O(new_n54_));
  nand3  g003(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  inv1   g004(.a(x18), .O(new_n56_));
  nand3  g005(.a(x19), .b(new_n56_), .c(x17), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(x18), .b(x16), .O(new_n62_));
  aoi22  g011(.a(new_n62_), .b(new_n60_), .c(new_n61_), .d(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n59_), .O(z00));
  nor2   g013(.a(x19), .b(x17), .O(new_n65_));
  nor3   g014(.a(new_n65_), .b(new_n61_), .c(x18), .O(new_n66_));
  nor2   g015(.a(x20), .b(x19), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n66_), .c(x16), .O(new_n70_));
  inv1   g019(.a(x14), .O(new_n71_));
  aoi22  g020(.a(new_n62_), .b(new_n71_), .c(new_n61_), .d(x18), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n70_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x18), .c(new_n61_), .O(new_n77_));
  nand3  g026(.a(new_n68_), .b(x21), .c(x16), .O(new_n78_));
  oai21  g027(.a(x16), .b(x13), .c(new_n78_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n56_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n77_), .O(z02));
  inv1   g030(.a(x16), .O(new_n82_));
  nor2   g031(.a(x21), .b(x20), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n65_), .O(new_n84_));
  nor3   g033(.a(x22), .b(x21), .c(x20), .O(new_n85_));
  aoi22  g034(.a(new_n85_), .b(new_n65_), .c(new_n84_), .d(x22), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n82_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n82_), .c(new_n88_), .O(z03));
  nor2   g038(.a(x22), .b(x21), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n65_), .c(new_n61_), .O(new_n91_));
  nor3   g040(.a(x23), .b(x22), .c(x21), .O(new_n92_));
  aoi22  g041(.a(new_n92_), .b(new_n69_), .c(new_n91_), .d(x23), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n82_), .b(new_n94_), .c(x18), .O(new_n95_));
  oai21  g044(.a(new_n93_), .b(new_n82_), .c(new_n95_), .O(z04));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n90_), .c(new_n65_), .d(x16), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n56_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  nor2   g049(.a(x23), .b(x22), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n67_), .c(new_n74_), .d(new_n52_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(x24), .c(x16), .O(new_n103_));
  oai21  g052(.a(x16), .b(x10), .c(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n56_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n100_), .O(z05));
  nand4  g055(.a(new_n97_), .b(new_n90_), .c(new_n67_), .d(new_n52_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n101_), .c(new_n83_), .d(new_n65_), .O(new_n109_));
  inv1   g058(.a(new_n109_), .O(new_n110_));
  aoi21  g059(.a(new_n107_), .b(x25), .c(new_n110_), .O(new_n111_));
  inv1   g060(.a(x09), .O(new_n112_));
  aoi21  g061(.a(new_n82_), .b(new_n112_), .c(x18), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n82_), .c(new_n113_), .O(z06));
  nand2  g063(.a(new_n109_), .b(x26), .O(new_n115_));
  nor3   g064(.a(x26), .b(x25), .c(x24), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n101_), .c(new_n83_), .d(new_n65_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n115_), .c(new_n82_), .O(new_n118_));
  oai21  g067(.a(x16), .b(x08), .c(new_n56_), .O(new_n119_));
  or2    g068(.a(new_n119_), .b(new_n118_), .O(z07));
  inv1   g069(.a(x25), .O(new_n121_));
  nor2   g070(.a(x27), .b(x26), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n97_), .c(new_n121_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n91_), .O(new_n124_));
  aoi21  g073(.a(new_n117_), .b(x27), .c(new_n124_), .O(new_n125_));
  inv1   g074(.a(x07), .O(new_n126_));
  aoi21  g075(.a(new_n82_), .b(new_n126_), .c(x18), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n82_), .c(new_n127_), .O(z08));
  inv1   g077(.a(x28), .O(new_n129_));
  nor3   g078(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nor3   g079(.a(x28), .b(x27), .c(x26), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n130_), .c(new_n85_), .d(new_n65_), .O(new_n132_));
  oai21  g081(.a(new_n124_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x06), .O(new_n135_));
  aoi21  g084(.a(new_n82_), .b(new_n135_), .c(x18), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z09));
  nand3  g086(.a(new_n92_), .b(new_n65_), .c(x16), .O(new_n138_));
  nor3   g087(.a(x29), .b(x28), .c(x27), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n116_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(new_n56_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n61_), .O(new_n142_));
  nand3  g091(.a(new_n132_), .b(x29), .c(x16), .O(new_n143_));
  inv1   g092(.a(x05), .O(new_n144_));
  nand2  g093(.a(new_n82_), .b(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n56_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n142_), .O(z10));
  nand4  g097(.a(new_n139_), .b(new_n116_), .c(new_n92_), .d(new_n69_), .O(new_n149_));
  inv1   g098(.a(x24), .O(new_n150_));
  nor2   g099(.a(x26), .b(x25), .O(new_n151_));
  nor2   g100(.a(x28), .b(x27), .O(new_n152_));
  nor2   g101(.a(x30), .b(x29), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n150_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n102_), .O(new_n155_));
  aoi21  g104(.a(new_n149_), .b(x30), .c(new_n155_), .O(new_n156_));
  inv1   g105(.a(x04), .O(new_n157_));
  aoi21  g106(.a(new_n82_), .b(new_n157_), .c(x18), .O(new_n158_));
  oai21  g107(.a(new_n156_), .b(new_n82_), .c(new_n158_), .O(z11));
  nor2   g108(.a(x29), .b(x28), .O(new_n160_));
  nor2   g109(.a(x31), .b(x30), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n160_), .c(new_n122_), .d(new_n121_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n98_), .c(new_n56_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n61_), .O(new_n164_));
  oai21  g113(.a(new_n154_), .b(new_n102_), .c(x31), .O(new_n165_));
  inv1   g114(.a(x03), .O(new_n166_));
  nand2  g115(.a(new_n82_), .b(new_n166_), .O(new_n167_));
  oai21  g116(.a(new_n165_), .b(new_n82_), .c(new_n167_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n56_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n164_), .O(z12));
  oai21  g119(.a(new_n162_), .b(new_n107_), .c(x32), .O(new_n171_));
  nor2   g120(.a(x32), .b(x31), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n153_), .c(new_n152_), .d(new_n151_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n107_), .c(new_n171_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  inv1   g124(.a(x02), .O(new_n176_));
  aoi21  g125(.a(new_n82_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z13));
  nand3  g127(.a(new_n108_), .b(new_n101_), .c(new_n76_), .O(new_n179_));
  nor2   g128(.a(x33), .b(x32), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n161_), .c(new_n160_), .d(new_n122_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n179_), .c(new_n56_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n61_), .O(new_n183_));
  oai21  g132(.a(new_n173_), .b(new_n107_), .c(x33), .O(new_n184_));
  inv1   g133(.a(x01), .O(new_n185_));
  nand2  g134(.a(new_n82_), .b(new_n185_), .O(new_n186_));
  oai21  g135(.a(new_n184_), .b(new_n82_), .c(new_n186_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n56_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n183_), .O(z14));
  oai21  g138(.a(new_n181_), .b(new_n109_), .c(x34), .O(new_n190_));
  nand2  g139(.a(new_n160_), .b(new_n122_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  inv1   g141(.a(new_n161_), .O(new_n193_));
  inv1   g142(.a(x32), .O(new_n194_));
  nor2   g143(.a(x34), .b(x33), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n194_), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n193_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n192_), .c(new_n110_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n190_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x16), .O(new_n200_));
  inv1   g149(.a(x00), .O(new_n201_));
  aoi21  g150(.a(new_n82_), .b(new_n201_), .c(x18), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n200_), .O(z15));
endmodule


