// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  inv1   g000(.a(x30), .O(new_n52_));
  inv1   g001(.a(x34), .O(new_n53_));
  nand2  g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  inv1   g004(.a(x15), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  oai21  g006(.a(x18), .b(new_n56_), .c(new_n57_), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x18), .O(new_n60_));
  nand2  g009(.a(x19), .b(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  aoi21  g011(.a(x19), .b(new_n60_), .c(x17), .O(new_n63_));
  aoi21  g012(.a(new_n63_), .b(x16), .c(new_n62_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n58_), .c(new_n55_), .O(z00));
  inv1   g014(.a(x14), .O(new_n66_));
  oai21  g015(.a(x18), .b(new_n66_), .c(new_n57_), .O(new_n67_));
  inv1   g016(.a(x20), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n57_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x17), .O(new_n70_));
  inv1   g019(.a(x19), .O(new_n71_));
  nand3  g020(.a(new_n68_), .b(new_n59_), .c(x16), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand3  g023(.a(new_n54_), .b(x18), .c(new_n59_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(x19), .c(x16), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n74_), .c(new_n54_), .O(new_n78_));
  inv1   g027(.a(new_n78_), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n70_), .c(new_n67_), .O(z01));
  nor2   g029(.a(x20), .b(x19), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n59_), .O(new_n82_));
  nor2   g031(.a(x19), .b(x17), .O(new_n83_));
  nor2   g032(.a(x21), .b(x20), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  aoi21  g035(.a(new_n82_), .b(x21), .c(new_n86_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x13), .O(new_n88_));
  nand2  g037(.a(new_n54_), .b(new_n60_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g039(.a(new_n87_), .b(new_n57_), .c(new_n90_), .O(z02));
  inv1   g040(.a(new_n83_), .O(new_n92_));
  nor2   g041(.a(x22), .b(x21), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n68_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g044(.a(new_n85_), .b(x22), .c(new_n95_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x12), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n89_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n57_), .c(new_n98_), .O(z03));
  nor3   g048(.a(x22), .b(x21), .c(x20), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  inv1   g050(.a(x21), .O(new_n102_));
  nor2   g051(.a(x23), .b(x22), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n81_), .c(new_n102_), .d(new_n59_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi21  g054(.a(new_n101_), .b(x23), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x11), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n89_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n57_), .c(new_n108_), .O(z04));
  inv1   g058(.a(x24), .O(new_n110_));
  nor2   g059(.a(x24), .b(x23), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n93_), .c(new_n81_), .d(new_n59_), .O(new_n112_));
  oai21  g061(.a(new_n105_), .b(new_n110_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g063(.a(x10), .O(new_n115_));
  aoi21  g064(.a(new_n57_), .b(new_n115_), .c(x18), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n114_), .c(new_n55_), .O(z05));
  inv1   g066(.a(x25), .O(new_n118_));
  inv1   g067(.a(new_n112_), .O(new_n119_));
  nor2   g068(.a(x25), .b(x24), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n103_), .c(new_n84_), .d(new_n83_), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x09), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n89_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z06));
  nand2  g075(.a(new_n121_), .b(x26), .O(new_n127_));
  nor3   g076(.a(x26), .b(x25), .c(x24), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n103_), .c(new_n86_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x08), .O(new_n132_));
  aoi21  g081(.a(new_n57_), .b(new_n132_), .c(x18), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n131_), .c(new_n55_), .O(z07));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n111_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n101_), .O(new_n137_));
  aoi21  g086(.a(new_n129_), .b(x27), .c(new_n137_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n89_), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n57_), .c(new_n140_), .O(z08));
  oai21  g090(.a(new_n136_), .b(new_n101_), .c(x28), .O(new_n142_));
  inv1   g091(.a(x26), .O(new_n143_));
  nor3   g092(.a(x25), .b(x24), .c(x23), .O(new_n144_));
  nor2   g093(.a(x28), .b(x27), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n144_), .c(new_n95_), .d(new_n143_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  nor2   g097(.a(x16), .b(x06), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n89_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n148_), .O(z09));
  inv1   g100(.a(x27), .O(new_n152_));
  nor2   g101(.a(x29), .b(x28), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n128_), .c(new_n152_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n104_), .O(new_n155_));
  aoi21  g104(.a(new_n146_), .b(x29), .c(new_n155_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n89_), .O(new_n158_));
  oai21  g107(.a(new_n156_), .b(new_n57_), .c(new_n158_), .O(z10));
  nand3  g108(.a(new_n100_), .b(new_n83_), .c(x16), .O(new_n160_));
  nor2   g109(.a(x27), .b(x26), .O(new_n161_));
  nand2  g110(.a(new_n153_), .b(new_n161_), .O(new_n162_));
  inv1   g111(.a(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n144_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n160_), .c(x34), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n52_), .O(new_n166_));
  nor2   g115(.a(new_n155_), .b(new_n52_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  inv1   g117(.a(x04), .O(new_n169_));
  aoi21  g118(.a(new_n57_), .b(new_n169_), .c(x18), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(new_n166_), .O(z11));
  nand4  g120(.a(new_n103_), .b(new_n84_), .c(new_n83_), .d(x16), .O(new_n172_));
  nor2   g121(.a(x26), .b(x25), .O(new_n173_));
  nor2   g122(.a(x31), .b(x29), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n145_), .c(new_n173_), .d(new_n110_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n172_), .c(x34), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n52_), .O(new_n177_));
  nor2   g126(.a(new_n53_), .b(x30), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n153_), .c(new_n135_), .d(new_n119_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(x31), .c(x16), .O(new_n180_));
  inv1   g129(.a(x03), .O(new_n181_));
  aoi21  g130(.a(new_n57_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n180_), .c(new_n177_), .O(z12));
  oai21  g132(.a(x16), .b(x02), .c(new_n60_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n54_), .O(new_n185_));
  inv1   g134(.a(x32), .O(new_n186_));
  oai21  g135(.a(new_n175_), .b(new_n104_), .c(x34), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n52_), .c(new_n186_), .O(new_n188_));
  nor2   g137(.a(x31), .b(x30), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(x34), .c(new_n186_), .O(new_n190_));
  nor3   g139(.a(new_n190_), .b(new_n162_), .c(new_n121_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n188_), .c(x16), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n185_), .O(z13));
  oai21  g142(.a(x16), .b(x01), .c(new_n60_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n54_), .O(new_n195_));
  inv1   g144(.a(x33), .O(new_n196_));
  nor2   g145(.a(x32), .b(x31), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n153_), .c(new_n161_), .d(new_n118_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n112_), .c(x34), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n52_), .c(new_n196_), .O(new_n200_));
  nand4  g149(.a(new_n189_), .b(x34), .c(new_n196_), .d(new_n186_), .O(new_n201_));
  nor3   g150(.a(new_n201_), .b(new_n162_), .c(new_n121_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n200_), .c(x16), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n195_), .O(z14));
  nand4  g153(.a(new_n189_), .b(new_n163_), .c(new_n196_), .d(new_n186_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n121_), .c(x34), .O(new_n206_));
  nor2   g155(.a(x16), .b(x00), .O(new_n207_));
  nor2   g156(.a(new_n207_), .b(new_n89_), .O(new_n208_));
  oai21  g157(.a(new_n206_), .b(new_n57_), .c(new_n208_), .O(z15));
endmodule


