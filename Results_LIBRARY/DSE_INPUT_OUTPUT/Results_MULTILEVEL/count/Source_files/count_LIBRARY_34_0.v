// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x25), .O(new_n60_));
  inv1   g009(.a(x31), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n56_), .O(z00));
  inv1   g014(.a(new_n62_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  nor2   g016(.a(new_n55_), .b(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n67_), .b(new_n53_), .c(new_n52_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  inv1   g020(.a(x14), .O(new_n72_));
  aoi21  g021(.a(new_n58_), .b(new_n72_), .c(x18), .O(new_n73_));
  aoi21  g022(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(z01));
  nor2   g023(.a(x21), .b(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n55_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  aoi21  g026(.a(new_n69_), .b(x21), .c(new_n77_), .O(new_n78_));
  nor2   g027(.a(x16), .b(x13), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n58_), .c(new_n80_), .O(z02));
  inv1   g030(.a(x22), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n55_), .c(new_n67_), .O(new_n84_));
  oai21  g033(.a(new_n77_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n58_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n66_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n82_), .O(new_n91_));
  nor3   g040(.a(new_n91_), .b(new_n69_), .c(x21), .O(new_n92_));
  aoi21  g041(.a(new_n84_), .b(x23), .c(new_n92_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n63_), .O(new_n95_));
  oai21  g044(.a(new_n93_), .b(new_n58_), .c(new_n95_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  nor2   g046(.a(new_n91_), .b(x21), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n70_), .c(new_n97_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n69_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n99_), .c(x16), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n58_), .b(new_n104_), .c(x18), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n66_), .O(z05));
  nand4  g055(.a(new_n67_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n101_), .c(x31), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n60_), .O(new_n109_));
  nor2   g058(.a(new_n102_), .b(new_n60_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g060(.a(x09), .O(new_n112_));
  aoi21  g061(.a(new_n58_), .b(new_n112_), .c(x18), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(z06));
  nor2   g063(.a(x26), .b(x24), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(new_n83_), .c(new_n90_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n107_), .c(x31), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n60_), .O(new_n118_));
  nor2   g067(.a(x23), .b(x22), .O(new_n119_));
  nor2   g068(.a(x25), .b(x24), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n119_), .c(new_n77_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(x26), .c(x16), .O(new_n122_));
  inv1   g071(.a(x08), .O(new_n123_));
  aoi21  g072(.a(new_n58_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n118_), .O(z07));
  oai21  g074(.a(x16), .b(x07), .c(new_n59_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n62_), .O(new_n127_));
  inv1   g076(.a(x27), .O(new_n128_));
  nand2  g077(.a(new_n115_), .b(new_n119_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n76_), .c(x31), .O(new_n130_));
  aoi21  g079(.a(new_n130_), .b(new_n60_), .c(new_n128_), .O(new_n131_));
  nand2  g080(.a(new_n120_), .b(new_n90_), .O(new_n132_));
  inv1   g081(.a(x26), .O(new_n133_));
  nand3  g082(.a(x31), .b(new_n128_), .c(new_n133_), .O(new_n134_));
  nor3   g083(.a(new_n134_), .b(new_n132_), .c(new_n84_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n127_), .O(z08));
  nand4  g086(.a(new_n75_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n100_), .c(new_n133_), .d(new_n82_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(x31), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n60_), .O(new_n142_));
  inv1   g091(.a(new_n135_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(x28), .c(x16), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  aoi21  g094(.a(new_n58_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(new_n142_), .O(z09));
  oai21  g096(.a(x16), .b(x05), .c(new_n59_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n62_), .O(new_n149_));
  inv1   g098(.a(x29), .O(new_n150_));
  nand3  g099(.a(new_n139_), .b(new_n100_), .c(new_n133_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n84_), .c(x31), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n60_), .c(new_n150_), .O(new_n153_));
  nand2  g102(.a(new_n98_), .b(new_n70_), .O(new_n154_));
  nor3   g103(.a(x26), .b(x25), .c(x24), .O(new_n155_));
  nor2   g104(.a(new_n61_), .b(x29), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n155_), .c(new_n139_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n153_), .c(x16), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n149_), .O(z10));
  nand4  g109(.a(new_n83_), .b(new_n55_), .c(new_n67_), .d(x16), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n139_), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n115_), .c(new_n90_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n161_), .c(x31), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n60_), .O(new_n167_));
  nand4  g116(.a(new_n156_), .b(new_n155_), .c(new_n139_), .d(new_n92_), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(x30), .c(x16), .O(new_n169_));
  inv1   g118(.a(x04), .O(new_n170_));
  aoi21  g119(.a(new_n58_), .b(new_n170_), .c(x18), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(new_n167_), .O(z11));
  inv1   g121(.a(x03), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n164_), .b(new_n155_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n154_), .c(x31), .O(new_n176_));
  oai22  g125(.a(new_n176_), .b(new_n58_), .c(new_n174_), .d(new_n66_), .O(z12));
  inv1   g126(.a(x02), .O(new_n178_));
  nand2  g127(.a(new_n58_), .b(new_n178_), .O(new_n179_));
  aoi21  g128(.a(x32), .b(x16), .c(x18), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n179_), .c(new_n66_), .O(z13));
  inv1   g130(.a(x01), .O(new_n182_));
  nand2  g131(.a(new_n58_), .b(new_n182_), .O(new_n183_));
  aoi21  g132(.a(x33), .b(x16), .c(x18), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n183_), .c(new_n66_), .O(z14));
  aoi21  g134(.a(x34), .b(x16), .c(x18), .O(new_n186_));
  inv1   g135(.a(x00), .O(new_n187_));
  nand2  g136(.a(new_n58_), .b(new_n187_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n186_), .c(new_n62_), .O(z15));
endmodule


