// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_;
  nor2   g000(.a(x16), .b(x15), .O(new_n52_));
  oai21  g001(.a(new_n52_), .b(x18), .c(x07), .O(new_n53_));
  inv1   g002(.a(x18), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  inv1   g005(.a(new_n56_), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x16), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  nor2   g009(.a(x16), .b(x14), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(x18), .c(x07), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n56_), .b(new_n63_), .O(new_n64_));
  inv1   g013(.a(x17), .O(new_n65_));
  inv1   g014(.a(x19), .O(new_n66_));
  nand3  g015(.a(new_n63_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n54_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n64_), .c(x16), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n62_), .O(z01));
  inv1   g019(.a(x16), .O(new_n71_));
  nand2  g020(.a(x13), .b(x07), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nor2   g023(.a(x20), .b(x19), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n65_), .c(new_n74_), .O(new_n76_));
  nand4  g025(.a(new_n74_), .b(new_n63_), .c(new_n66_), .d(new_n65_), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n73_), .c(new_n54_), .O(z02));
  nand2  g029(.a(x12), .b(x07), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n56_), .c(new_n63_), .O(new_n85_));
  oai21  g034(.a(new_n78_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n82_), .c(new_n54_), .O(z03));
  nand2  g037(.a(x11), .b(x07), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n71_), .O(new_n90_));
  nand2  g039(.a(new_n85_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n75_), .c(new_n74_), .d(new_n65_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n90_), .c(new_n54_), .O(z04));
  nand2  g045(.a(x10), .b(x07), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n71_), .O(new_n98_));
  inv1   g047(.a(x24), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n83_), .c(new_n74_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n67_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n84_), .c(new_n75_), .d(new_n65_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n98_), .c(new_n54_), .O(z05));
  nor2   g056(.a(x16), .b(x09), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(x18), .c(x07), .O(new_n109_));
  nor2   g058(.a(x21), .b(x20), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n92_), .c(new_n110_), .d(new_n56_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n54_), .O(new_n113_));
  aoi21  g062(.a(new_n104_), .b(x25), .c(new_n113_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n71_), .c(new_n109_), .O(z06));
  nand2  g064(.a(x08), .b(x07), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n71_), .O(new_n117_));
  nand2  g066(.a(new_n112_), .b(x26), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n92_), .c(new_n78_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n117_), .c(new_n54_), .O(z07));
  inv1   g072(.a(x25), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n103_), .c(new_n124_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n85_), .O(new_n127_));
  aoi21  g076(.a(new_n120_), .b(x27), .c(new_n127_), .O(new_n128_));
  inv1   g077(.a(x07), .O(new_n129_));
  aoi21  g078(.a(new_n71_), .b(new_n129_), .c(x18), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n71_), .c(new_n130_), .O(z08));
  nor2   g080(.a(x16), .b(x06), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(x18), .c(x07), .O(new_n133_));
  oai21  g082(.a(new_n126_), .b(new_n85_), .c(x28), .O(new_n134_));
  nor3   g083(.a(x22), .b(x21), .c(x20), .O(new_n135_));
  nor3   g084(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nor3   g085(.a(x28), .b(x27), .c(x26), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n136_), .c(new_n135_), .d(new_n56_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n134_), .c(new_n54_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n133_), .O(z09));
  nand2  g090(.a(x07), .b(x05), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n71_), .O(new_n143_));
  nand2  g092(.a(new_n138_), .b(x29), .O(new_n144_));
  inv1   g093(.a(x27), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n119_), .c(new_n102_), .d(new_n145_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n143_), .c(new_n54_), .O(z10));
  nor2   g099(.a(x16), .b(x04), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(x18), .c(x07), .O(new_n152_));
  nor2   g101(.a(x26), .b(x25), .O(new_n153_));
  nand4  g102(.a(new_n146_), .b(new_n153_), .c(new_n145_), .d(new_n99_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n93_), .c(x30), .O(new_n155_));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nor2   g105(.a(x30), .b(x29), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n156_), .c(new_n119_), .d(new_n102_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n155_), .c(new_n54_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n152_), .O(z11));
  nand2  g110(.a(x07), .b(x03), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n71_), .O(new_n163_));
  nand4  g112(.a(new_n157_), .b(new_n156_), .c(new_n153_), .d(new_n99_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n93_), .c(x31), .O(new_n165_));
  nand4  g114(.a(new_n99_), .b(new_n100_), .c(new_n83_), .d(new_n74_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n67_), .O(new_n167_));
  nor3   g116(.a(x27), .b(x26), .c(x25), .O(new_n168_));
  nor2   g117(.a(x31), .b(x30), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n146_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n168_), .c(new_n167_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n165_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n163_), .c(new_n54_), .O(z12));
  nand2  g124(.a(x07), .b(x02), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n71_), .O(new_n177_));
  nand4  g126(.a(new_n169_), .b(new_n146_), .c(new_n125_), .d(new_n124_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n104_), .c(x32), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n157_), .c(new_n156_), .d(new_n153_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n167_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n179_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(x16), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n177_), .c(new_n54_), .O(z13));
  nand2  g135(.a(x07), .b(x01), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n71_), .O(new_n188_));
  oai21  g137(.a(new_n181_), .b(new_n104_), .c(x33), .O(new_n189_));
  nand4  g138(.a(new_n124_), .b(new_n99_), .c(new_n100_), .d(new_n83_), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n77_), .O(new_n191_));
  nor2   g140(.a(x33), .b(x32), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n169_), .c(new_n146_), .d(new_n125_), .O(new_n193_));
  inv1   g142(.a(new_n193_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n191_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n189_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x16), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n188_), .c(new_n54_), .O(z14));
  nand2  g147(.a(x07), .b(x00), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n71_), .O(new_n200_));
  oai21  g149(.a(new_n193_), .b(new_n112_), .c(x34), .O(new_n201_));
  nand2  g150(.a(new_n146_), .b(new_n125_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  inv1   g152(.a(x30), .O(new_n204_));
  inv1   g153(.a(x31), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  inv1   g155(.a(x32), .O(new_n207_));
  inv1   g156(.a(x33), .O(new_n208_));
  inv1   g157(.a(x34), .O(new_n209_));
  nand3  g158(.a(new_n209_), .b(new_n208_), .c(new_n207_), .O(new_n210_));
  nor2   g159(.a(new_n210_), .b(new_n206_), .O(new_n211_));
  nand3  g160(.a(new_n211_), .b(new_n203_), .c(new_n191_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n201_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(x16), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n200_), .c(new_n54_), .O(z15));
endmodule


