// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n52_), .O(z01));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(x21), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n59_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g018(.a(new_n68_), .b(x22), .O(new_n70_));
  nor2   g019(.a(x22), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n56_), .c(new_n59_), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n52_), .d(x16), .O(z03));
  inv1   g022(.a(x16), .O(new_n74_));
  nor2   g023(.a(x23), .b(x22), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n64_), .c(new_n67_), .d(new_n53_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  aoi21  g026(.a(new_n72_), .b(x23), .c(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n74_), .c(new_n52_), .O(z04));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(x23), .O(new_n81_));
  inv1   g030(.a(x24), .O(new_n82_));
  nand4  g031(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n67_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n61_), .c(new_n76_), .d(x24), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n74_), .c(new_n52_), .O(z05));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n71_), .c(new_n64_), .d(new_n53_), .O(new_n88_));
  inv1   g037(.a(x25), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n82_), .c(new_n81_), .d(new_n80_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n68_), .O(new_n91_));
  aoi21  g040(.a(new_n88_), .b(x25), .c(new_n91_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n74_), .c(new_n52_), .O(z06));
  oai21  g042(.a(new_n90_), .b(new_n68_), .c(x26), .O(new_n94_));
  inv1   g043(.a(new_n68_), .O(new_n95_));
  nor3   g044(.a(x26), .b(x25), .c(x24), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n75_), .c(new_n95_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x16), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n52_), .O(z07));
  nor2   g049(.a(x27), .b(x26), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n87_), .c(new_n89_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n72_), .O(new_n103_));
  aoi21  g052(.a(new_n97_), .b(x27), .c(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n74_), .c(new_n52_), .O(z08));
  inv1   g054(.a(x28), .O(new_n106_));
  nor3   g055(.a(x22), .b(x21), .c(x20), .O(new_n107_));
  nor3   g056(.a(x25), .b(x24), .c(x23), .O(new_n108_));
  nor3   g057(.a(x28), .b(x27), .c(x26), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n108_), .c(new_n107_), .d(new_n56_), .O(new_n110_));
  oai21  g059(.a(new_n103_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x16), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(z09));
  nand2  g062(.a(new_n110_), .b(x29), .O(new_n114_));
  inv1   g063(.a(x27), .O(new_n115_));
  inv1   g064(.a(x29), .O(new_n116_));
  nand4  g065(.a(new_n96_), .b(new_n116_), .c(new_n106_), .d(new_n115_), .O(new_n117_));
  or2    g066(.a(new_n117_), .b(new_n76_), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n114_), .c(new_n52_), .d(x16), .O(z10));
  oai21  g068(.a(new_n117_), .b(new_n76_), .c(x30), .O(new_n120_));
  nor2   g069(.a(x26), .b(x25), .O(new_n121_));
  nor2   g070(.a(x28), .b(x27), .O(new_n122_));
  nor2   g071(.a(x30), .b(x29), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n82_), .O(new_n124_));
  or2    g073(.a(new_n124_), .b(new_n76_), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n120_), .c(new_n52_), .d(x16), .O(z11));
  oai21  g075(.a(new_n124_), .b(new_n76_), .c(x31), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  inv1   g077(.a(x30), .O(new_n129_));
  inv1   g078(.a(x31), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n129_), .c(new_n116_), .d(new_n106_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n128_), .c(new_n84_), .d(new_n61_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n127_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n52_), .O(z12));
  nand2  g085(.a(new_n133_), .b(x32), .O(new_n137_));
  nor2   g086(.a(x32), .b(x31), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n123_), .c(new_n122_), .d(new_n121_), .O(new_n139_));
  or2    g088(.a(new_n139_), .b(new_n88_), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n137_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g090(.a(new_n139_), .b(new_n88_), .c(x33), .O(new_n142_));
  inv1   g091(.a(new_n90_), .O(new_n143_));
  inv1   g092(.a(x26), .O(new_n144_));
  nand4  g093(.a(new_n116_), .b(new_n106_), .c(new_n115_), .d(new_n144_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  inv1   g095(.a(x32), .O(new_n147_));
  inv1   g096(.a(x33), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n130_), .d(new_n129_), .O(new_n149_));
  inv1   g098(.a(new_n149_), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n146_), .c(new_n143_), .d(new_n95_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n142_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n52_), .O(z14));
  nand2  g103(.a(new_n151_), .b(x34), .O(new_n155_));
  nor2   g104(.a(x31), .b(x30), .O(new_n156_));
  nor3   g105(.a(x34), .b(x33), .c(x32), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n146_), .c(new_n156_), .d(new_n91_), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n155_), .c(new_n52_), .d(x16), .O(z15));
endmodule


