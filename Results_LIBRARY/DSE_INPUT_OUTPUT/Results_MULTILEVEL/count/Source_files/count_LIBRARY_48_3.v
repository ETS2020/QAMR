// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n55_), .O(new_n70_));
  oai21  g019(.a(new_n63_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  aoi21  g021(.a(new_n71_), .b(x16), .c(new_n72_), .O(new_n73_));
  inv1   g022(.a(x23), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(x18), .c(new_n75_), .O(z02));
  nor3   g025(.a(x19), .b(x18), .c(x17), .O(new_n77_));
  nor3   g026(.a(x22), .b(x21), .c(x20), .O(new_n78_));
  aoi22  g027(.a(new_n78_), .b(new_n77_), .c(new_n70_), .d(x22), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n58_), .b(new_n80_), .c(x18), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n58_), .c(new_n81_), .O(z03));
  nand2  g031(.a(new_n78_), .b(new_n55_), .O(new_n83_));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n63_), .c(new_n83_), .d(x23), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n58_), .c(new_n87_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  aoi21  g038(.a(new_n84_), .b(new_n63_), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x20), .b(x19), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nor2   g041(.a(x24), .b(x23), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n52_), .O(new_n94_));
  inv1   g043(.a(new_n94_), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n90_), .c(x16), .O(new_n96_));
  inv1   g045(.a(x10), .O(new_n97_));
  aoi21  g046(.a(new_n58_), .b(new_n97_), .c(x18), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n96_), .O(z05));
  inv1   g048(.a(x18), .O(new_n100_));
  nor2   g049(.a(x17), .b(new_n58_), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n101_), .c(new_n92_), .d(new_n91_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n74_), .O(new_n105_));
  nand3  g054(.a(new_n94_), .b(x25), .c(x16), .O(new_n106_));
  oai21  g055(.a(x16), .b(x09), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n100_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n105_), .O(z06));
  nand2  g058(.a(new_n101_), .b(new_n91_), .O(new_n110_));
  nor3   g059(.a(x26), .b(x25), .c(x24), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n92_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n110_), .c(new_n100_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n74_), .O(new_n114_));
  nor2   g063(.a(x23), .b(x22), .O(new_n115_));
  nand4  g064(.a(new_n102_), .b(new_n115_), .c(new_n69_), .d(new_n55_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(x26), .c(x16), .O(new_n117_));
  oai21  g066(.a(x16), .b(x08), .c(new_n117_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n100_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n114_), .O(z07));
  nand3  g069(.a(new_n101_), .b(new_n69_), .c(new_n53_), .O(new_n121_));
  nor2   g070(.a(x24), .b(x22), .O(new_n122_));
  nor3   g071(.a(x27), .b(x26), .c(x25), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(new_n100_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n74_), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n115_), .c(new_n89_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n70_), .c(x27), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  nand2  g079(.a(new_n58_), .b(new_n130_), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n58_), .c(new_n131_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n100_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n126_), .O(z08));
  nand2  g083(.a(new_n123_), .b(new_n93_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n83_), .c(x28), .O(new_n136_));
  nor3   g085(.a(x25), .b(x24), .c(x23), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n137_), .c(new_n78_), .d(new_n55_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n58_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z09));
  nor3   g093(.a(x29), .b(x28), .c(x27), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n111_), .c(new_n84_), .d(new_n63_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  aoi21  g096(.a(new_n139_), .b(x29), .c(new_n147_), .O(new_n148_));
  inv1   g097(.a(x05), .O(new_n149_));
  aoi21  g098(.a(new_n58_), .b(new_n149_), .c(x18), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n58_), .c(new_n150_), .O(z10));
  nand4  g100(.a(new_n115_), .b(new_n91_), .c(new_n68_), .d(new_n52_), .O(new_n152_));
  nor2   g101(.a(x28), .b(x27), .O(new_n153_));
  nor2   g102(.a(x30), .b(x29), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n127_), .d(new_n89_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  aoi21  g105(.a(new_n146_), .b(x30), .c(new_n156_), .O(new_n157_));
  inv1   g106(.a(x04), .O(new_n158_));
  aoi21  g107(.a(new_n58_), .b(new_n158_), .c(x18), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n58_), .c(new_n159_), .O(z11));
  nand4  g109(.a(new_n122_), .b(new_n69_), .c(new_n55_), .d(x16), .O(new_n161_));
  inv1   g110(.a(x25), .O(new_n162_));
  nor2   g111(.a(x27), .b(x26), .O(new_n163_));
  nor2   g112(.a(x29), .b(x28), .O(new_n164_));
  nor2   g113(.a(x31), .b(x30), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n163_), .d(new_n162_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n161_), .c(new_n100_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n74_), .O(new_n168_));
  oai21  g117(.a(new_n155_), .b(new_n152_), .c(x31), .O(new_n169_));
  inv1   g118(.a(x03), .O(new_n170_));
  nand2  g119(.a(new_n58_), .b(new_n170_), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n58_), .c(new_n171_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n100_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n168_), .O(z12));
  oai21  g123(.a(new_n166_), .b(new_n94_), .c(x32), .O(new_n175_));
  nor2   g124(.a(x32), .b(x31), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n154_), .c(new_n153_), .d(new_n127_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n94_), .c(new_n175_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  inv1   g128(.a(x02), .O(new_n180_));
  aoi21  g129(.a(new_n58_), .b(new_n180_), .c(x18), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(z13));
  nor2   g131(.a(x33), .b(x32), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n165_), .c(new_n164_), .d(new_n163_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n103_), .c(new_n100_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n74_), .O(new_n186_));
  oai21  g135(.a(new_n177_), .b(new_n94_), .c(x33), .O(new_n187_));
  inv1   g136(.a(x01), .O(new_n188_));
  nand2  g137(.a(new_n58_), .b(new_n188_), .O(new_n189_));
  oai21  g138(.a(new_n187_), .b(new_n58_), .c(new_n189_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n100_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n186_), .O(z14));
  nor3   g141(.a(x34), .b(x33), .c(x32), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n165_), .c(new_n164_), .d(new_n163_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n103_), .c(new_n100_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n74_), .O(new_n196_));
  oai21  g145(.a(new_n184_), .b(new_n116_), .c(x34), .O(new_n197_));
  inv1   g146(.a(x00), .O(new_n198_));
  nand2  g147(.a(new_n58_), .b(new_n198_), .O(new_n199_));
  oai21  g148(.a(new_n197_), .b(new_n58_), .c(new_n199_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n100_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n196_), .O(z15));
endmodule


