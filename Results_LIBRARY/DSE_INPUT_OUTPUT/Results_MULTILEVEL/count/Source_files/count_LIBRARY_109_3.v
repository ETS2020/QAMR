// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nor2   g017(.a(x26), .b(new_n61_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n65_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n58_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z02));
  nand2  g027(.a(new_n73_), .b(x22), .O(new_n79_));
  inv1   g028(.a(x20), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n55_), .c(new_n80_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n79_), .c(new_n58_), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n83_), .c(new_n61_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n70_), .O(z03));
  nor3   g035(.a(x23), .b(x22), .c(x21), .O(new_n87_));
  aoi22  g036(.a(new_n87_), .b(new_n64_), .c(new_n82_), .d(x23), .O(new_n88_));
  inv1   g037(.a(x11), .O(new_n89_));
  aoi21  g038(.a(new_n58_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(z04));
  nand2  g040(.a(new_n87_), .b(new_n64_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x24), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n81_), .c(new_n64_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n58_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n61_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n70_), .O(z05));
  inv1   g048(.a(x22), .O(new_n100_));
  inv1   g049(.a(x23), .O(new_n101_));
  inv1   g050(.a(x24), .O(new_n102_));
  aoi21  g051(.a(x26), .b(x18), .c(x25), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(x21), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n80_), .c(new_n53_), .d(new_n52_), .O(new_n106_));
  nand3  g055(.a(new_n95_), .b(x25), .c(new_n61_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  nor3   g058(.a(x18), .b(x16), .c(x09), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n69_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nor2   g062(.a(x20), .b(x19), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n52_), .c(x16), .O(new_n115_));
  nor3   g064(.a(x25), .b(x24), .c(x23), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n81_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n115_), .c(new_n61_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n113_), .O(new_n119_));
  nor2   g068(.a(x23), .b(x22), .O(new_n120_));
  nor2   g069(.a(x25), .b(x24), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n120_), .c(new_n72_), .d(new_n55_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(x26), .c(x16), .O(new_n123_));
  oai21  g072(.a(x16), .b(x08), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n61_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n119_), .O(z07));
  nor3   g075(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n120_), .c(new_n74_), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n94_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n82_), .O(new_n131_));
  aoi21  g080(.a(new_n128_), .b(x27), .c(new_n131_), .O(new_n132_));
  inv1   g081(.a(x07), .O(new_n133_));
  aoi21  g082(.a(new_n58_), .b(new_n133_), .c(x18), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n58_), .c(new_n134_), .O(z08));
  nand4  g084(.a(new_n72_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n136_));
  inv1   g085(.a(x25), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n94_), .c(new_n137_), .d(new_n100_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n136_), .c(new_n61_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n113_), .O(new_n141_));
  oai21  g090(.a(new_n130_), .b(new_n82_), .c(x28), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  nand2  g092(.a(new_n58_), .b(new_n143_), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n58_), .c(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n61_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n141_), .O(z09));
  inv1   g096(.a(new_n82_), .O(new_n148_));
  nand4  g097(.a(new_n138_), .b(new_n116_), .c(new_n148_), .d(new_n113_), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n127_), .c(new_n87_), .d(new_n64_), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  aoi21  g101(.a(new_n149_), .b(x29), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  aoi21  g103(.a(new_n58_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n58_), .c(new_n155_), .O(z10));
  nand4  g105(.a(new_n81_), .b(new_n55_), .c(new_n80_), .d(x16), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n138_), .c(new_n116_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n157_), .c(new_n61_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n113_), .O(new_n161_));
  nand3  g110(.a(new_n151_), .b(x30), .c(x16), .O(new_n162_));
  oai21  g111(.a(x16), .b(x04), .c(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n61_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n161_), .O(z11));
  nand3  g114(.a(new_n158_), .b(new_n138_), .c(new_n127_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n92_), .c(x31), .O(new_n167_));
  nor2   g116(.a(x29), .b(x28), .O(new_n168_));
  nor2   g117(.a(x31), .b(x30), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n168_), .c(new_n129_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n95_), .c(new_n167_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  inv1   g121(.a(x03), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z12));
  nand4  g124(.a(new_n120_), .b(new_n72_), .c(new_n55_), .d(x16), .O(new_n176_));
  nor2   g125(.a(x32), .b(x31), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n158_), .c(new_n138_), .d(new_n121_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n176_), .c(new_n61_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n113_), .O(new_n180_));
  oai21  g129(.a(new_n170_), .b(new_n95_), .c(x32), .O(new_n181_));
  inv1   g130(.a(x02), .O(new_n182_));
  nand2  g131(.a(new_n58_), .b(new_n182_), .O(new_n183_));
  oai21  g132(.a(new_n181_), .b(new_n58_), .c(new_n183_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n61_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n180_), .O(z13));
  inv1   g135(.a(new_n115_), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n94_), .c(new_n81_), .O(new_n188_));
  nor2   g137(.a(x27), .b(x25), .O(new_n189_));
  nor2   g138(.a(x33), .b(x32), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n189_), .c(new_n169_), .d(new_n168_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n188_), .c(new_n61_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n113_), .O(new_n193_));
  nor2   g142(.a(x26), .b(x25), .O(new_n194_));
  nand4  g143(.a(new_n177_), .b(new_n158_), .c(new_n138_), .d(new_n194_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n95_), .c(x33), .O(new_n196_));
  inv1   g145(.a(x01), .O(new_n197_));
  nand2  g146(.a(new_n58_), .b(new_n197_), .O(new_n198_));
  oai21  g147(.a(new_n196_), .b(new_n58_), .c(new_n198_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n61_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n193_), .O(z14));
  nor2   g150(.a(x27), .b(x26), .O(new_n202_));
  nand4  g151(.a(new_n190_), .b(new_n169_), .c(new_n168_), .d(new_n202_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n122_), .c(x34), .O(new_n204_));
  nand2  g153(.a(new_n168_), .b(new_n202_), .O(new_n205_));
  inv1   g154(.a(new_n205_), .O(new_n206_));
  nor3   g155(.a(x34), .b(x33), .c(x32), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n206_), .c(new_n169_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n122_), .c(new_n204_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x16), .O(new_n210_));
  inv1   g159(.a(x00), .O(new_n211_));
  aoi21  g160(.a(new_n58_), .b(new_n211_), .c(x18), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n210_), .O(z15));
endmodule


