// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x26), .c(new_n52_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  inv1   g009(.a(x17), .O(new_n61_));
  inv1   g010(.a(x19), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n55_), .b(x20), .c(new_n65_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  inv1   g016(.a(x26), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  oai21  g019(.a(new_n66_), .b(new_n60_), .c(new_n70_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  oai21  g023(.a(new_n65_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x26), .c(new_n52_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n54_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n74_), .b(x22), .c(new_n81_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x12), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n60_), .c(new_n84_), .O(z03));
  nand2  g034(.a(new_n80_), .b(x23), .O(new_n86_));
  nor2   g035(.a(x20), .b(x19), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n87_), .c(new_n72_), .d(new_n61_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n86_), .c(new_n60_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n68_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(z04));
  nor2   g042(.a(x22), .b(x21), .O(new_n94_));
  nor2   g043(.a(x24), .b(x23), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n94_), .c(new_n87_), .d(new_n61_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi21  g046(.a(new_n89_), .b(x24), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x10), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n69_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n60_), .c(new_n100_), .O(z05));
  inv1   g050(.a(x24), .O(new_n102_));
  inv1   g051(.a(x25), .O(new_n103_));
  nand4  g052(.a(new_n88_), .b(new_n68_), .c(new_n103_), .d(new_n102_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n74_), .O(new_n105_));
  aoi21  g054(.a(new_n96_), .b(x25), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n69_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n60_), .c(new_n108_), .O(z06));
  nand3  g058(.a(new_n87_), .b(new_n61_), .c(x16), .O(new_n110_));
  nor3   g059(.a(x25), .b(x24), .c(x23), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n94_), .O(new_n112_));
  oai22  g061(.a(new_n112_), .b(new_n110_), .c(x16), .d(x08), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n68_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n52_), .O(z07));
  nand3  g064(.a(new_n88_), .b(new_n103_), .c(new_n102_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n74_), .c(x27), .O(new_n117_));
  nor3   g066(.a(x27), .b(x25), .c(x24), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n88_), .c(new_n73_), .d(new_n54_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n60_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n120_), .c(new_n68_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n52_), .O(z08));
  nor2   g072(.a(x27), .b(x25), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n88_), .c(new_n102_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n74_), .c(x28), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x25), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n95_), .c(new_n79_), .d(new_n54_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n126_), .c(new_n60_), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n129_), .c(new_n68_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z09));
  nand2  g081(.a(new_n128_), .b(x29), .O(new_n133_));
  nor3   g082(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n111_), .c(new_n81_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x05), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n69_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n137_), .O(z10));
  inv1   g089(.a(x28), .O(new_n141_));
  nor2   g090(.a(x30), .b(x29), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n124_), .c(new_n141_), .d(new_n102_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n89_), .O(new_n144_));
  aoi21  g093(.a(new_n135_), .b(x30), .c(new_n144_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x04), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n69_), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n60_), .c(new_n147_), .O(z11));
  oai21  g097(.a(new_n143_), .b(new_n89_), .c(x31), .O(new_n149_));
  inv1   g098(.a(x22), .O(new_n150_));
  inv1   g099(.a(x23), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(new_n72_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n64_), .O(new_n153_));
  inv1   g102(.a(x29), .O(new_n154_));
  inv1   g103(.a(x30), .O(new_n155_));
  inv1   g104(.a(x31), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n154_), .d(new_n141_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n118_), .c(new_n153_), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n149_), .c(new_n60_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x03), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n160_), .c(new_n68_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n52_), .O(z12));
  nand2  g112(.a(new_n159_), .b(x32), .O(new_n164_));
  nor2   g113(.a(x28), .b(x27), .O(new_n165_));
  nor2   g114(.a(x32), .b(x31), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n142_), .c(new_n165_), .d(new_n103_), .O(new_n167_));
  or2    g116(.a(new_n167_), .b(new_n96_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n164_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nor2   g119(.a(x16), .b(x02), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n69_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z13));
  oai21  g122(.a(new_n167_), .b(new_n96_), .c(x33), .O(new_n174_));
  nand4  g123(.a(new_n102_), .b(new_n151_), .c(new_n150_), .d(new_n72_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  inv1   g125(.a(x27), .O(new_n177_));
  nand4  g126(.a(new_n154_), .b(new_n141_), .c(new_n177_), .d(new_n103_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  inv1   g128(.a(x32), .O(new_n180_));
  inv1   g129(.a(x33), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n180_), .c(new_n156_), .d(new_n155_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n179_), .c(new_n176_), .d(new_n65_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n174_), .c(new_n60_), .O(new_n185_));
  nor2   g134(.a(x16), .b(x01), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n185_), .c(new_n68_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n52_), .O(z14));
  nand2  g137(.a(new_n184_), .b(x34), .O(new_n189_));
  nor2   g138(.a(new_n116_), .b(new_n74_), .O(new_n190_));
  nor2   g139(.a(x34), .b(x33), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n166_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n142_), .c(new_n165_), .d(new_n190_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n189_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(x16), .O(new_n196_));
  nor2   g145(.a(x16), .b(x00), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n69_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n196_), .O(z15));
endmodule


