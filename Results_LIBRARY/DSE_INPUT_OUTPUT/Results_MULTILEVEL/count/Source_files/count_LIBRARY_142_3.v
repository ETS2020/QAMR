// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n52_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  oai21  g008(.a(x18), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  nor2   g010(.a(x18), .b(x17), .O(new_n62_));
  nor2   g011(.a(x20), .b(x19), .O(new_n63_));
  aoi22  g012(.a(new_n63_), .b(new_n62_), .c(new_n55_), .d(x20), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n59_), .b(new_n65_), .c(x18), .O(new_n66_));
  oai21  g015(.a(new_n64_), .b(new_n59_), .c(new_n66_), .O(z01));
  inv1   g016(.a(x17), .O(new_n68_));
  inv1   g017(.a(x19), .O(new_n69_));
  inv1   g018(.a(x20), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n71_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n59_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n59_), .c(new_n77_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n54_), .c(new_n73_), .d(x22), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  aoi21  g030(.a(new_n59_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n59_), .c(new_n82_), .O(z03));
  inv1   g032(.a(x11), .O(new_n84_));
  oai21  g033(.a(x18), .b(new_n84_), .c(new_n59_), .O(new_n85_));
  nand2  g034(.a(new_n79_), .b(new_n54_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x23), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n63_), .c(new_n72_), .d(new_n68_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n52_), .c(x16), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n85_), .O(z04));
  inv1   g041(.a(x10), .O(new_n93_));
  oai21  g042(.a(x18), .b(new_n93_), .c(new_n59_), .O(new_n94_));
  inv1   g043(.a(x24), .O(new_n95_));
  inv1   g044(.a(x22), .O(new_n96_));
  inv1   g045(.a(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n96_), .c(new_n72_), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n71_), .O(new_n99_));
  nor2   g048(.a(x22), .b(x21), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n100_), .c(new_n63_), .d(new_n68_), .O(new_n102_));
  oai21  g051(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n52_), .c(x16), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n94_), .O(z05));
  inv1   g054(.a(x09), .O(new_n106_));
  oai21  g055(.a(x18), .b(new_n106_), .c(new_n59_), .O(new_n107_));
  nand2  g056(.a(new_n102_), .b(x25), .O(new_n108_));
  nor2   g057(.a(x21), .b(x20), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n88_), .c(new_n109_), .d(new_n54_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n52_), .c(x16), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n107_), .O(z06));
  nand3  g063(.a(new_n109_), .b(new_n62_), .c(new_n69_), .O(new_n115_));
  nor2   g064(.a(x26), .b(x25), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n88_), .c(new_n95_), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  aoi21  g067(.a(new_n111_), .b(x26), .c(new_n118_), .O(new_n119_));
  inv1   g068(.a(x08), .O(new_n120_));
  aoi21  g069(.a(new_n59_), .b(new_n120_), .c(x18), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n59_), .c(new_n121_), .O(z07));
  inv1   g071(.a(x07), .O(new_n123_));
  oai21  g072(.a(x18), .b(new_n123_), .c(new_n59_), .O(new_n124_));
  oai21  g073(.a(new_n117_), .b(new_n73_), .c(x27), .O(new_n125_));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n101_), .c(new_n79_), .d(new_n54_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n52_), .c(x16), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n124_), .O(z08));
  inv1   g079(.a(x06), .O(new_n131_));
  oai21  g080(.a(x18), .b(new_n131_), .c(new_n59_), .O(new_n132_));
  nand2  g081(.a(new_n127_), .b(x28), .O(new_n133_));
  nor3   g082(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nor3   g083(.a(x28), .b(x27), .c(x26), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n134_), .c(new_n79_), .d(new_n54_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n52_), .c(x16), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n132_), .O(z09));
  inv1   g088(.a(x05), .O(new_n140_));
  oai21  g089(.a(x18), .b(new_n140_), .c(new_n59_), .O(new_n141_));
  nand2  g090(.a(new_n136_), .b(x29), .O(new_n142_));
  inv1   g091(.a(x27), .O(new_n143_));
  nor3   g092(.a(x26), .b(x25), .c(x24), .O(new_n144_));
  nor2   g093(.a(x29), .b(x28), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n144_), .c(new_n99_), .d(new_n143_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n52_), .c(x16), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n141_), .O(z10));
  inv1   g098(.a(x04), .O(new_n150_));
  oai21  g099(.a(x18), .b(new_n150_), .c(new_n59_), .O(new_n151_));
  nand4  g100(.a(new_n145_), .b(new_n116_), .c(new_n143_), .d(new_n95_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n89_), .c(x30), .O(new_n153_));
  nor2   g102(.a(x28), .b(x27), .O(new_n154_));
  nor2   g103(.a(x30), .b(x29), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n144_), .d(new_n99_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n52_), .c(x16), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n151_), .O(z11));
  inv1   g108(.a(x03), .O(new_n160_));
  oai21  g109(.a(x18), .b(new_n160_), .c(new_n59_), .O(new_n161_));
  nand4  g110(.a(new_n155_), .b(new_n154_), .c(new_n116_), .d(new_n95_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n89_), .c(x31), .O(new_n163_));
  inv1   g112(.a(new_n71_), .O(new_n164_));
  nand2  g113(.a(new_n101_), .b(new_n100_), .O(new_n165_));
  inv1   g114(.a(new_n165_), .O(new_n166_));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n145_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n126_), .c(new_n166_), .d(new_n164_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n163_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n52_), .c(x16), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n161_), .O(z12));
  inv1   g122(.a(x02), .O(new_n174_));
  oai21  g123(.a(x18), .b(new_n174_), .c(new_n59_), .O(new_n175_));
  inv1   g124(.a(x25), .O(new_n176_));
  nor2   g125(.a(x27), .b(x26), .O(new_n177_));
  nand4  g126(.a(new_n167_), .b(new_n145_), .c(new_n177_), .d(new_n176_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n102_), .c(x32), .O(new_n179_));
  nand2  g128(.a(new_n154_), .b(new_n116_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n155_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n181_), .c(new_n166_), .d(new_n164_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n179_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n52_), .c(x16), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n175_), .O(z13));
  nand4  g137(.a(new_n182_), .b(new_n155_), .c(new_n154_), .d(new_n116_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n102_), .c(x33), .O(new_n190_));
  nand4  g139(.a(new_n176_), .b(new_n95_), .c(new_n97_), .d(new_n96_), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n73_), .O(new_n192_));
  nor2   g141(.a(x33), .b(x32), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n167_), .c(new_n145_), .d(new_n177_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n192_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n190_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(x16), .O(new_n198_));
  inv1   g147(.a(x01), .O(new_n199_));
  aoi21  g148(.a(new_n59_), .b(new_n199_), .c(x18), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n198_), .O(z14));
  inv1   g150(.a(x00), .O(new_n202_));
  oai21  g151(.a(x18), .b(new_n202_), .c(new_n59_), .O(new_n203_));
  oai21  g152(.a(new_n194_), .b(new_n111_), .c(x34), .O(new_n204_));
  nand2  g153(.a(new_n145_), .b(new_n177_), .O(new_n205_));
  inv1   g154(.a(new_n205_), .O(new_n206_));
  inv1   g155(.a(x30), .O(new_n207_));
  inv1   g156(.a(x31), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  inv1   g158(.a(x32), .O(new_n210_));
  inv1   g159(.a(x33), .O(new_n211_));
  inv1   g160(.a(x34), .O(new_n212_));
  nand3  g161(.a(new_n212_), .b(new_n211_), .c(new_n210_), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n209_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n206_), .c(new_n192_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n204_), .O(new_n216_));
  nand3  g165(.a(new_n216_), .b(new_n52_), .c(x16), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n203_), .O(z15));
endmodule


