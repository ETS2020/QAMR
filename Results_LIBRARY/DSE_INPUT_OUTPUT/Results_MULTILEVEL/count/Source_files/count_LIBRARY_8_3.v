// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n52_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  oai21  g009(.a(x18), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n58_), .O(z00));
  inv1   g011(.a(x14), .O(new_n63_));
  oai21  g012(.a(x18), .b(new_n63_), .c(new_n60_), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(x19), .b(x17), .O(new_n66_));
  nand3  g015(.a(new_n65_), .b(new_n55_), .c(new_n54_), .O(new_n67_));
  oai21  g016(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n52_), .c(x16), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n64_), .O(z01));
  inv1   g019(.a(x13), .O(new_n71_));
  oai21  g020(.a(x18), .b(new_n71_), .c(new_n60_), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  inv1   g022(.a(new_n67_), .O(new_n74_));
  nand4  g023(.a(new_n73_), .b(new_n65_), .c(new_n55_), .d(new_n54_), .O(new_n75_));
  oai21  g024(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n52_), .c(x16), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n72_), .O(z02));
  nor3   g027(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n73_), .c(new_n65_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  aoi22  g031(.a(new_n82_), .b(new_n79_), .c(new_n75_), .d(x22), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n60_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n60_), .c(new_n85_), .O(z03));
  nor2   g035(.a(x22), .b(x21), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n66_), .c(new_n65_), .O(new_n88_));
  inv1   g037(.a(x23), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n80_), .c(new_n73_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n67_), .O(new_n91_));
  aoi21  g040(.a(new_n88_), .b(x23), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x11), .O(new_n93_));
  aoi21  g042(.a(new_n60_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n60_), .c(new_n94_), .O(z04));
  inv1   g044(.a(x10), .O(new_n96_));
  oai21  g045(.a(x18), .b(new_n96_), .c(new_n60_), .O(new_n97_));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(x20), .b(x19), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n87_), .c(new_n99_), .d(new_n54_), .O(new_n101_));
  oai21  g050(.a(new_n91_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n52_), .c(x16), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n97_), .O(z05));
  inv1   g053(.a(x09), .O(new_n105_));
  oai21  g054(.a(x18), .b(new_n105_), .c(new_n60_), .O(new_n106_));
  nand2  g055(.a(new_n101_), .b(x25), .O(new_n107_));
  nor2   g056(.a(x21), .b(x20), .O(new_n108_));
  nor2   g057(.a(x23), .b(x22), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n109_), .c(new_n108_), .d(new_n66_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n52_), .c(x16), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n106_), .O(z06));
  inv1   g063(.a(x08), .O(new_n115_));
  oai21  g064(.a(x18), .b(new_n115_), .c(new_n60_), .O(new_n116_));
  inv1   g065(.a(x26), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n98_), .c(new_n89_), .d(new_n80_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n75_), .O(new_n120_));
  nor2   g069(.a(x26), .b(x25), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n109_), .c(new_n98_), .O(new_n122_));
  oai22  g071(.a(new_n122_), .b(new_n75_), .c(new_n120_), .d(new_n117_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n52_), .c(x16), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n116_), .O(z07));
  inv1   g074(.a(x07), .O(new_n126_));
  oai21  g075(.a(x18), .b(new_n126_), .c(new_n60_), .O(new_n127_));
  oai21  g076(.a(new_n122_), .b(new_n75_), .c(x27), .O(new_n128_));
  inv1   g077(.a(new_n88_), .O(new_n129_));
  nor3   g078(.a(x27), .b(x26), .c(x25), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n100_), .c(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n52_), .c(x16), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n127_), .O(z08));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n110_), .c(new_n117_), .d(new_n89_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n88_), .O(new_n137_));
  aoi21  g086(.a(new_n131_), .b(x28), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x06), .O(new_n139_));
  aoi21  g088(.a(new_n60_), .b(new_n139_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n60_), .c(new_n140_), .O(z09));
  nor3   g090(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n142_), .c(new_n129_), .O(new_n144_));
  nand4  g093(.a(new_n109_), .b(new_n99_), .c(new_n73_), .d(new_n54_), .O(new_n145_));
  inv1   g094(.a(x27), .O(new_n146_));
  nor2   g095(.a(x29), .b(x28), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n121_), .c(new_n146_), .d(new_n98_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  aoi21  g098(.a(new_n144_), .b(x29), .c(new_n149_), .O(new_n150_));
  inv1   g099(.a(x05), .O(new_n151_));
  aoi21  g100(.a(new_n60_), .b(new_n151_), .c(x18), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n60_), .c(new_n152_), .O(z10));
  inv1   g102(.a(x04), .O(new_n154_));
  oai21  g103(.a(x18), .b(new_n154_), .c(new_n60_), .O(new_n155_));
  oai21  g104(.a(new_n148_), .b(new_n145_), .c(x30), .O(new_n156_));
  nor2   g105(.a(x30), .b(x29), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n135_), .O(new_n158_));
  inv1   g107(.a(new_n158_), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n121_), .c(new_n91_), .d(new_n98_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n52_), .c(x16), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n155_), .O(z11));
  inv1   g112(.a(x03), .O(new_n164_));
  oai21  g113(.a(x18), .b(new_n164_), .c(new_n60_), .O(new_n165_));
  nand4  g114(.a(new_n157_), .b(new_n135_), .c(new_n121_), .d(new_n98_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n145_), .c(x31), .O(new_n167_));
  nand2  g116(.a(new_n100_), .b(new_n87_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n147_), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n130_), .c(new_n169_), .d(new_n74_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n167_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n52_), .c(x16), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n165_), .O(z12));
  nor2   g125(.a(x27), .b(x26), .O(new_n177_));
  nand4  g126(.a(new_n170_), .b(new_n147_), .c(new_n177_), .d(new_n118_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n101_), .c(x32), .O(new_n179_));
  nand2  g128(.a(new_n135_), .b(new_n121_), .O(new_n180_));
  nor2   g129(.a(x32), .b(x31), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n157_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n180_), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n169_), .c(new_n74_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n179_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(x16), .O(new_n186_));
  inv1   g135(.a(x02), .O(new_n187_));
  aoi21  g136(.a(new_n60_), .b(new_n187_), .c(x18), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n186_), .O(z13));
  inv1   g138(.a(x01), .O(new_n190_));
  oai21  g139(.a(x18), .b(new_n190_), .c(new_n60_), .O(new_n191_));
  nand4  g140(.a(new_n181_), .b(new_n157_), .c(new_n135_), .d(new_n121_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n101_), .c(x33), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n170_), .c(new_n147_), .d(new_n177_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n120_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n193_), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n52_), .c(x16), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n191_), .O(z14));
  inv1   g149(.a(x00), .O(new_n201_));
  oai21  g150(.a(x18), .b(new_n201_), .c(new_n60_), .O(new_n202_));
  oai21  g151(.a(new_n195_), .b(new_n111_), .c(x34), .O(new_n203_));
  nand2  g152(.a(new_n147_), .b(new_n177_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  inv1   g154(.a(x30), .O(new_n206_));
  inv1   g155(.a(x31), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n206_), .O(new_n208_));
  inv1   g157(.a(x32), .O(new_n209_));
  inv1   g158(.a(x33), .O(new_n210_));
  inv1   g159(.a(x34), .O(new_n211_));
  nand3  g160(.a(new_n211_), .b(new_n210_), .c(new_n209_), .O(new_n212_));
  nor2   g161(.a(new_n212_), .b(new_n208_), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n205_), .c(new_n120_), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n203_), .O(new_n215_));
  nand3  g164(.a(new_n215_), .b(new_n52_), .c(x16), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n202_), .O(z15));
endmodule


