// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n52_), .O(z01));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(x21), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n59_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g018(.a(new_n68_), .b(x22), .O(new_n70_));
  nor2   g019(.a(x22), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n56_), .c(new_n59_), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n52_), .d(x16), .O(z03));
  nand2  g022(.a(new_n72_), .b(x23), .O(new_n74_));
  nor2   g023(.a(x23), .b(x22), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n64_), .c(new_n67_), .d(new_n53_), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n74_), .c(new_n52_), .d(x16), .O(z04));
  inv1   g026(.a(x24), .O(new_n78_));
  nor3   g027(.a(x23), .b(x22), .c(x21), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n61_), .c(new_n78_), .O(new_n80_));
  nor2   g029(.a(x24), .b(x23), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n65_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n52_), .O(z05));
  oai21  g034(.a(new_n82_), .b(new_n65_), .c(x25), .O(new_n86_));
  nor2   g035(.a(x21), .b(x20), .O(new_n87_));
  nor2   g036(.a(x25), .b(x24), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n75_), .c(new_n87_), .d(new_n56_), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n86_), .c(new_n52_), .d(x16), .O(z06));
  inv1   g039(.a(x26), .O(new_n91_));
  inv1   g040(.a(x22), .O(new_n92_));
  inv1   g041(.a(x23), .O(new_n93_));
  inv1   g042(.a(x25), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n78_), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n68_), .O(new_n96_));
  nor3   g045(.a(x26), .b(x25), .c(x24), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n75_), .c(new_n87_), .d(new_n56_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n91_), .c(new_n98_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x16), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n52_), .O(z07));
  nand2  g050(.a(new_n98_), .b(x27), .O(new_n102_));
  nor3   g051(.a(x22), .b(x21), .c(x20), .O(new_n103_));
  nor3   g052(.a(x27), .b(x26), .c(x25), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n81_), .c(new_n103_), .d(new_n56_), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n102_), .c(new_n52_), .d(x16), .O(z08));
  inv1   g055(.a(x16), .O(new_n107_));
  nor2   g056(.a(x28), .b(x27), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n88_), .c(new_n91_), .d(new_n93_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n72_), .O(new_n110_));
  aoi21  g059(.a(new_n105_), .b(x28), .c(new_n110_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n107_), .c(new_n52_), .O(z09));
  oai21  g061(.a(new_n109_), .b(new_n72_), .c(x29), .O(new_n113_));
  nor3   g062(.a(x29), .b(x28), .c(x27), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n97_), .c(new_n79_), .d(new_n61_), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n113_), .c(new_n52_), .d(x16), .O(z10));
  nor2   g065(.a(x26), .b(x25), .O(new_n117_));
  nor2   g066(.a(x30), .b(x29), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n108_), .c(new_n117_), .d(new_n78_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n76_), .O(new_n120_));
  aoi21  g069(.a(new_n115_), .b(x30), .c(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n107_), .c(new_n52_), .O(z11));
  oai21  g071(.a(new_n119_), .b(new_n76_), .c(x31), .O(new_n123_));
  nor2   g072(.a(x29), .b(x28), .O(new_n124_));
  nor2   g073(.a(x31), .b(x30), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n124_), .c(new_n104_), .d(new_n83_), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n123_), .c(new_n52_), .d(x16), .O(z12));
  nand2  g076(.a(new_n126_), .b(x32), .O(new_n128_));
  inv1   g077(.a(x31), .O(new_n129_));
  inv1   g078(.a(x32), .O(new_n130_));
  nand3  g079(.a(new_n118_), .b(new_n130_), .c(new_n129_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n108_), .c(new_n117_), .d(new_n83_), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n128_), .c(new_n52_), .d(x16), .O(z13));
  nand2  g083(.a(new_n133_), .b(x33), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nor2   g085(.a(x33), .b(x32), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n125_), .c(new_n124_), .d(new_n136_), .O(new_n138_));
  or2    g087(.a(new_n138_), .b(new_n89_), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n135_), .c(new_n52_), .d(x16), .O(z14));
  oai21  g089(.a(new_n138_), .b(new_n89_), .c(x34), .O(new_n141_));
  nand2  g090(.a(new_n124_), .b(new_n136_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  inv1   g092(.a(x30), .O(new_n144_));
  nand2  g093(.a(new_n129_), .b(new_n144_), .O(new_n145_));
  inv1   g094(.a(x33), .O(new_n146_));
  inv1   g095(.a(x34), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n146_), .c(new_n130_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n143_), .c(new_n96_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n141_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n52_), .O(z15));
endmodule


