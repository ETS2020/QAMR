// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x26), .c(new_n52_), .O(z00));
  inv1   g008(.a(x26), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n55_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n52_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  inv1   g020(.a(new_n66_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x26), .c(new_n52_), .O(z02));
  nand2  g027(.a(new_n74_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n54_), .c(new_n65_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(new_n61_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x12), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n82_), .c(new_n60_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n52_), .O(z03));
  nand2  g034(.a(new_n81_), .b(x23), .O(new_n86_));
  nor2   g035(.a(x20), .b(x19), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n87_), .c(new_n71_), .d(new_n63_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n86_), .c(new_n61_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n60_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(z04));
  inv1   g042(.a(x22), .O(new_n94_));
  inv1   g043(.a(x23), .O(new_n95_));
  inv1   g044(.a(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n71_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi22  g047(.a(new_n98_), .b(new_n72_), .c(new_n89_), .d(x24), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  nor2   g049(.a(x26), .b(x18), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  oai21  g052(.a(new_n99_), .b(new_n61_), .c(new_n103_), .O(z05));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n80_), .c(new_n87_), .d(new_n63_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x25), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n88_), .c(new_n73_), .d(new_n54_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n107_), .c(new_n61_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n110_), .c(new_n60_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(z06));
  inv1   g062(.a(x08), .O(new_n114_));
  nand2  g063(.a(new_n61_), .b(new_n114_), .O(new_n115_));
  nor2   g064(.a(x17), .b(new_n61_), .O(new_n116_));
  nor3   g065(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n88_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n115_), .c(new_n101_), .O(z07));
  nand2  g069(.a(new_n109_), .b(x27), .O(new_n121_));
  inv1   g070(.a(new_n74_), .O(new_n122_));
  nor3   g071(.a(x27), .b(x25), .c(x24), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n88_), .c(new_n122_), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n121_), .c(new_n61_), .O(new_n125_));
  nor2   g074(.a(x16), .b(x07), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n125_), .c(new_n60_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n52_), .O(z08));
  nor2   g077(.a(x27), .b(x25), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n88_), .c(new_n96_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n74_), .c(x28), .O(new_n131_));
  nor3   g080(.a(x22), .b(x21), .c(x20), .O(new_n132_));
  nor3   g081(.a(x28), .b(x27), .c(x25), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n105_), .c(new_n132_), .d(new_n54_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n131_), .c(new_n61_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x06), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n135_), .c(new_n60_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n52_), .O(z09));
  inv1   g087(.a(x25), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n105_), .c(new_n139_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n81_), .c(x29), .O(new_n142_));
  nor3   g091(.a(x25), .b(x24), .c(x23), .O(new_n143_));
  nor3   g092(.a(x29), .b(x28), .c(x27), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n132_), .d(new_n54_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n142_), .c(new_n61_), .O(new_n146_));
  nor2   g095(.a(x16), .b(x05), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n146_), .c(new_n60_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n52_), .O(z10));
  inv1   g098(.a(x27), .O(new_n150_));
  nor2   g099(.a(x29), .b(x28), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n108_), .c(new_n150_), .d(new_n95_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n81_), .c(x30), .O(new_n153_));
  nand3  g102(.a(new_n95_), .b(new_n94_), .c(new_n71_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n66_), .O(new_n155_));
  nor3   g104(.a(x30), .b(x29), .c(x28), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n123_), .c(new_n155_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n153_), .c(new_n61_), .O(new_n158_));
  nor2   g107(.a(x16), .b(x04), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n158_), .c(new_n60_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n52_), .O(z11));
  inv1   g110(.a(x28), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n129_), .c(new_n162_), .d(new_n96_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n89_), .c(x31), .O(new_n165_));
  nor2   g114(.a(x31), .b(x30), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n151_), .O(new_n167_));
  inv1   g116(.a(new_n167_), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n123_), .c(new_n155_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n165_), .c(new_n61_), .O(new_n170_));
  nor2   g119(.a(x16), .b(x03), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n170_), .c(new_n60_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n52_), .O(z12));
  nand4  g122(.a(new_n166_), .b(new_n151_), .c(new_n129_), .d(new_n96_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n89_), .c(x32), .O(new_n175_));
  inv1   g124(.a(x29), .O(new_n176_));
  inv1   g125(.a(x30), .O(new_n177_));
  inv1   g126(.a(x31), .O(new_n178_));
  inv1   g127(.a(x32), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n178_), .c(new_n177_), .d(new_n176_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n133_), .c(new_n98_), .d(new_n72_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n175_), .c(new_n61_), .O(new_n183_));
  nor2   g132(.a(x16), .b(x02), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n183_), .c(new_n60_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n52_), .O(z13));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n163_), .c(new_n140_), .d(new_n139_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n106_), .c(x33), .O(new_n189_));
  nand4  g138(.a(new_n176_), .b(new_n162_), .c(new_n150_), .d(new_n139_), .O(new_n190_));
  inv1   g139(.a(new_n190_), .O(new_n191_));
  inv1   g140(.a(x33), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n179_), .c(new_n178_), .d(new_n177_), .O(new_n193_));
  inv1   g142(.a(new_n193_), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n191_), .c(new_n98_), .d(new_n72_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n189_), .c(new_n61_), .O(new_n196_));
  nor2   g145(.a(x16), .b(x01), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n196_), .c(new_n60_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n52_), .O(z14));
  nand2  g148(.a(new_n195_), .b(x34), .O(new_n200_));
  nand2  g149(.a(new_n163_), .b(new_n140_), .O(new_n201_));
  nor2   g150(.a(x34), .b(x33), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n187_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n201_), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n108_), .c(new_n88_), .d(new_n122_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n200_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(x16), .O(new_n207_));
  nor2   g156(.a(x16), .b(x00), .O(new_n208_));
  nor2   g157(.a(new_n208_), .b(new_n102_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n207_), .O(z15));
endmodule


