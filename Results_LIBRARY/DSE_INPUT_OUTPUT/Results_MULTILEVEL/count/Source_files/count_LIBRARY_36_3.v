// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x18), .O(new_n59_));
  oai21  g008(.a(x19), .b(x17), .c(x20), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  nor2   g012(.a(x16), .b(x14), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(new_n59_), .O(new_n65_));
  inv1   g014(.a(x30), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(x18), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n62_), .b(x21), .c(new_n71_), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n56_), .c(new_n74_), .O(z02));
  nand2  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  inv1   g025(.a(x20), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n53_), .c(new_n77_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n76_), .c(new_n56_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(new_n59_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n67_), .O(z03));
  nand2  g032(.a(new_n79_), .b(x23), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n61_), .O(new_n86_));
  aoi21  g035(.a(new_n86_), .b(new_n84_), .c(new_n56_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n87_), .c(new_n59_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n67_), .O(z04));
  nand2  g039(.a(new_n86_), .b(x24), .O(new_n91_));
  inv1   g040(.a(x17), .O(new_n92_));
  nor2   g041(.a(x20), .b(x19), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n78_), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n91_), .c(new_n56_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n59_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n67_), .O(z05));
  nand2  g048(.a(new_n95_), .b(x25), .O(new_n100_));
  nor2   g049(.a(x23), .b(x22), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n101_), .c(new_n69_), .d(new_n53_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n100_), .c(new_n56_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x09), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n104_), .c(new_n59_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n67_), .O(z06));
  nand2  g056(.a(new_n103_), .b(x26), .O(new_n108_));
  nor3   g057(.a(x26), .b(x25), .c(x24), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n101_), .c(new_n71_), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n108_), .c(new_n56_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x08), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n111_), .c(new_n59_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n67_), .O(z07));
  inv1   g063(.a(x24), .O(new_n115_));
  nor2   g064(.a(x26), .b(x25), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n101_), .c(new_n115_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n70_), .c(x27), .O(new_n118_));
  nor3   g067(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nor3   g068(.a(x27), .b(x26), .c(x25), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n94_), .c(new_n119_), .d(new_n53_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n118_), .c(new_n56_), .O(new_n122_));
  nor2   g071(.a(x16), .b(x07), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n122_), .c(new_n59_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n67_), .O(z08));
  inv1   g074(.a(x23), .O(new_n126_));
  inv1   g075(.a(x26), .O(new_n127_));
  nor2   g076(.a(x28), .b(x27), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n102_), .c(new_n127_), .d(new_n126_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n79_), .O(new_n130_));
  aoi21  g079(.a(new_n121_), .b(x28), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(x06), .O(new_n132_));
  aoi21  g081(.a(new_n56_), .b(new_n132_), .c(x18), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n56_), .c(new_n133_), .O(z09));
  oai21  g083(.a(new_n129_), .b(new_n79_), .c(x29), .O(new_n135_));
  nor3   g084(.a(x29), .b(x28), .c(x27), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n109_), .c(new_n85_), .d(new_n61_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n135_), .c(new_n56_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x05), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n138_), .c(new_n59_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n67_), .O(z10));
  nor2   g090(.a(x30), .b(x29), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n128_), .c(new_n109_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n86_), .O(new_n144_));
  aoi21  g093(.a(new_n137_), .b(x30), .c(new_n144_), .O(new_n145_));
  inv1   g094(.a(x04), .O(new_n146_));
  aoi21  g095(.a(new_n56_), .b(new_n146_), .c(x18), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n56_), .c(new_n147_), .O(z11));
  oai21  g097(.a(new_n143_), .b(new_n86_), .c(x31), .O(new_n149_));
  inv1   g098(.a(x25), .O(new_n150_));
  nor2   g099(.a(x27), .b(x26), .O(new_n151_));
  nor2   g100(.a(x29), .b(x28), .O(new_n152_));
  nor2   g101(.a(x31), .b(x30), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n150_), .O(new_n154_));
  or2    g103(.a(new_n154_), .b(new_n95_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n149_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  inv1   g106(.a(x03), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(new_n158_), .c(x18), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(z12));
  nand4  g109(.a(new_n101_), .b(new_n69_), .c(new_n53_), .d(x16), .O(new_n161_));
  nor2   g110(.a(x32), .b(x31), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n152_), .c(new_n151_), .d(new_n102_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n161_), .c(new_n59_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n66_), .O(new_n165_));
  oai21  g114(.a(new_n154_), .b(new_n95_), .c(x32), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n56_), .O(new_n167_));
  nor2   g116(.a(x16), .b(x02), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n167_), .c(new_n59_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n165_), .O(z13));
  nor2   g119(.a(x17), .b(new_n56_), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n94_), .c(new_n78_), .d(new_n93_), .O(new_n172_));
  nand2  g121(.a(new_n128_), .b(new_n116_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nor2   g123(.a(x31), .b(x29), .O(new_n175_));
  nor2   g124(.a(x33), .b(x32), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n175_), .c(new_n174_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n172_), .c(new_n59_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n66_), .O(new_n179_));
  nand4  g128(.a(new_n162_), .b(new_n142_), .c(new_n128_), .d(new_n116_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n95_), .c(x33), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n56_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x01), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n182_), .c(new_n59_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n179_), .O(z14));
  nor3   g134(.a(x34), .b(x33), .c(x32), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n175_), .c(new_n174_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n172_), .c(new_n59_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n66_), .O(new_n189_));
  nand4  g138(.a(new_n176_), .b(new_n153_), .c(new_n152_), .d(new_n151_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n103_), .c(x34), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n56_), .O(new_n192_));
  nor2   g141(.a(x16), .b(x00), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n192_), .c(new_n59_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n189_), .O(z15));
endmodule


