// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nand3  g009(.a(new_n59_), .b(new_n54_), .c(new_n53_), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n60_), .c(x16), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n52_), .O(z01));
  nand2  g013(.a(new_n61_), .b(x21), .O(new_n65_));
  inv1   g014(.a(x21), .O(new_n66_));
  nand4  g015(.a(new_n66_), .b(new_n59_), .c(new_n54_), .d(new_n53_), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n65_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g017(.a(new_n67_), .b(x22), .O(new_n69_));
  nor2   g018(.a(x22), .b(x21), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n56_), .c(new_n59_), .O(new_n71_));
  nand4  g020(.a(new_n71_), .b(new_n69_), .c(new_n52_), .d(x16), .O(z03));
  inv1   g021(.a(x16), .O(new_n73_));
  nor3   g022(.a(x23), .b(x22), .c(x21), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n62_), .c(new_n71_), .d(x23), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(new_n52_), .O(z04));
  inv1   g025(.a(x24), .O(new_n77_));
  aoi21  g026(.a(new_n74_), .b(new_n62_), .c(new_n77_), .O(new_n78_));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(x23), .O(new_n80_));
  nand4  g029(.a(new_n77_), .b(new_n80_), .c(new_n79_), .d(new_n66_), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n78_), .c(x16), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n52_), .O(z05));
  nor2   g033(.a(x20), .b(x19), .O(new_n85_));
  nor2   g034(.a(x24), .b(x23), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n70_), .c(new_n85_), .d(new_n53_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x25), .O(new_n88_));
  nor2   g037(.a(x21), .b(x20), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nor2   g039(.a(x25), .b(x24), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n56_), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n88_), .c(new_n52_), .d(x16), .O(z06));
  nand2  g042(.a(new_n92_), .b(x26), .O(new_n94_));
  nor3   g043(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n90_), .c(new_n89_), .d(new_n56_), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n94_), .c(new_n52_), .d(x16), .O(z07));
  inv1   g046(.a(x25), .O(new_n98_));
  nor2   g047(.a(x27), .b(x26), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n86_), .c(new_n98_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n71_), .O(new_n101_));
  aoi21  g050(.a(new_n96_), .b(x27), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n73_), .c(new_n52_), .O(z08));
  inv1   g052(.a(x28), .O(new_n104_));
  nor3   g053(.a(x22), .b(x21), .c(x20), .O(new_n105_));
  nor3   g054(.a(x25), .b(x24), .c(x23), .O(new_n106_));
  nor3   g055(.a(x28), .b(x27), .c(x26), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n56_), .O(new_n108_));
  oai21  g057(.a(new_n101_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n52_), .O(z09));
  nand4  g060(.a(new_n90_), .b(new_n85_), .c(new_n66_), .d(new_n53_), .O(new_n112_));
  inv1   g061(.a(x27), .O(new_n113_));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  nor2   g063(.a(x29), .b(x28), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n114_), .c(new_n113_), .d(new_n77_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  aoi21  g066(.a(new_n108_), .b(x29), .c(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n73_), .c(new_n52_), .O(z10));
  nor3   g068(.a(x29), .b(x28), .c(x27), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n95_), .c(new_n74_), .d(new_n62_), .O(new_n121_));
  nor2   g070(.a(x28), .b(x27), .O(new_n122_));
  nor2   g071(.a(x30), .b(x29), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n114_), .d(new_n77_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n112_), .O(new_n125_));
  aoi21  g074(.a(new_n121_), .b(x30), .c(new_n125_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n73_), .c(new_n52_), .O(z11));
  oai21  g076(.a(new_n124_), .b(new_n112_), .c(x31), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nor2   g078(.a(x31), .b(x30), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n115_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n129_), .c(new_n82_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n128_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n52_), .O(z12));
  nand2  g085(.a(new_n133_), .b(x32), .O(new_n137_));
  nor2   g086(.a(x32), .b(x31), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n123_), .c(new_n122_), .d(new_n114_), .O(new_n139_));
  or2    g088(.a(new_n139_), .b(new_n87_), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n137_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g090(.a(new_n139_), .b(new_n87_), .c(x33), .O(new_n142_));
  nand4  g091(.a(new_n98_), .b(new_n77_), .c(new_n80_), .d(new_n79_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n67_), .O(new_n144_));
  nor2   g093(.a(x33), .b(x32), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n130_), .c(new_n115_), .d(new_n99_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n142_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n52_), .O(z14));
  oai21  g100(.a(new_n146_), .b(new_n92_), .c(x34), .O(new_n152_));
  nand2  g101(.a(new_n115_), .b(new_n99_), .O(new_n153_));
  inv1   g102(.a(new_n153_), .O(new_n154_));
  inv1   g103(.a(x30), .O(new_n155_));
  inv1   g104(.a(x31), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  inv1   g106(.a(x32), .O(new_n158_));
  inv1   g107(.a(x33), .O(new_n159_));
  inv1   g108(.a(x34), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n159_), .c(new_n158_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n154_), .c(new_n144_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n152_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x16), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n52_), .O(z15));
endmodule


