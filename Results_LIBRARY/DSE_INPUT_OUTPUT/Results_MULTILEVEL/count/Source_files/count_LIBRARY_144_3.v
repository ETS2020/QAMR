// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n52_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  oai21  g009(.a(x18), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n58_), .O(z00));
  inv1   g011(.a(x14), .O(new_n63_));
  oai21  g012(.a(x18), .b(new_n63_), .c(new_n60_), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(x19), .b(x17), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n52_), .c(x16), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n64_), .O(z01));
  inv1   g020(.a(x13), .O(new_n72_));
  oai21  g021(.a(x18), .b(new_n72_), .c(new_n60_), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n65_), .c(new_n55_), .d(new_n54_), .O(new_n75_));
  oai21  g024(.a(new_n67_), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n52_), .c(x16), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n73_), .O(z02));
  nor3   g027(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n74_), .c(new_n65_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  aoi22  g031(.a(new_n82_), .b(new_n79_), .c(new_n75_), .d(x22), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n60_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n60_), .c(new_n85_), .O(z03));
  inv1   g035(.a(x11), .O(new_n87_));
  oai21  g036(.a(x18), .b(new_n87_), .c(new_n60_), .O(new_n88_));
  inv1   g037(.a(x23), .O(new_n89_));
  nor2   g038(.a(x22), .b(x21), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n66_), .c(new_n65_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  nor2   g041(.a(x20), .b(x19), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n93_), .c(new_n74_), .d(new_n54_), .O(new_n95_));
  oai21  g044(.a(new_n92_), .b(new_n89_), .c(new_n95_), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n52_), .c(x16), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n88_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  nor3   g048(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n67_), .c(new_n99_), .O(new_n101_));
  nand3  g050(.a(new_n93_), .b(new_n52_), .c(new_n54_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n90_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n101_), .c(x16), .O(new_n106_));
  inv1   g055(.a(x10), .O(new_n107_));
  aoi21  g056(.a(new_n60_), .b(new_n107_), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z05));
  inv1   g058(.a(x09), .O(new_n110_));
  oai21  g059(.a(x18), .b(new_n110_), .c(new_n60_), .O(new_n111_));
  nand4  g060(.a(new_n103_), .b(new_n90_), .c(new_n93_), .d(new_n54_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x25), .O(new_n113_));
  nor2   g062(.a(x21), .b(x20), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n94_), .c(new_n114_), .d(new_n66_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n52_), .c(x16), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n111_), .O(z06));
  inv1   g068(.a(x08), .O(new_n120_));
  oai21  g069(.a(x18), .b(new_n120_), .c(new_n60_), .O(new_n121_));
  inv1   g070(.a(x26), .O(new_n122_));
  inv1   g071(.a(x25), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n99_), .c(new_n89_), .d(new_n80_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n75_), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n94_), .c(new_n99_), .O(new_n127_));
  oai22  g076(.a(new_n127_), .b(new_n75_), .c(new_n125_), .d(new_n122_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n52_), .c(x16), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n121_), .O(z07));
  inv1   g079(.a(x07), .O(new_n131_));
  oai21  g080(.a(x18), .b(new_n131_), .c(new_n60_), .O(new_n132_));
  oai21  g081(.a(new_n127_), .b(new_n75_), .c(x27), .O(new_n133_));
  nor3   g082(.a(x27), .b(x26), .c(x25), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n103_), .c(new_n92_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n52_), .c(x16), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n132_), .O(z08));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n115_), .c(new_n122_), .d(new_n89_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n91_), .O(new_n141_));
  aoi21  g090(.a(new_n135_), .b(x28), .c(new_n141_), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  aoi21  g092(.a(new_n60_), .b(new_n143_), .c(x18), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n60_), .c(new_n144_), .O(z09));
  nor3   g094(.a(x25), .b(x24), .c(x23), .O(new_n146_));
  nor3   g095(.a(x28), .b(x27), .c(x26), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n146_), .c(new_n92_), .O(new_n148_));
  inv1   g097(.a(x27), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n126_), .c(new_n149_), .d(new_n99_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n95_), .O(new_n152_));
  aoi21  g101(.a(new_n148_), .b(x29), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  aoi21  g103(.a(new_n60_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n60_), .c(new_n155_), .O(z10));
  nor3   g105(.a(x26), .b(x25), .c(x24), .O(new_n157_));
  nor3   g106(.a(x29), .b(x28), .c(x27), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n157_), .c(new_n100_), .d(new_n67_), .O(new_n159_));
  nor2   g108(.a(x30), .b(x29), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n139_), .c(new_n126_), .d(new_n99_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n95_), .O(new_n162_));
  aoi21  g111(.a(new_n159_), .b(x30), .c(new_n162_), .O(new_n163_));
  inv1   g112(.a(x04), .O(new_n164_));
  aoi21  g113(.a(new_n60_), .b(new_n164_), .c(x18), .O(new_n165_));
  oai21  g114(.a(new_n163_), .b(new_n60_), .c(new_n165_), .O(z11));
  oai21  g115(.a(new_n161_), .b(new_n95_), .c(x31), .O(new_n167_));
  inv1   g116(.a(new_n104_), .O(new_n168_));
  nor2   g117(.a(x31), .b(x30), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n150_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n134_), .c(new_n168_), .d(new_n67_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n167_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  inv1   g123(.a(x03), .O(new_n175_));
  aoi21  g124(.a(new_n60_), .b(new_n175_), .c(x18), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n174_), .O(z12));
  nor2   g126(.a(x27), .b(x26), .O(new_n178_));
  nand4  g127(.a(new_n169_), .b(new_n150_), .c(new_n178_), .d(new_n123_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n112_), .c(x32), .O(new_n180_));
  nand2  g129(.a(new_n139_), .b(new_n126_), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n160_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n181_), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n168_), .c(new_n67_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n180_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x16), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  aoi21  g137(.a(new_n60_), .b(new_n188_), .c(x18), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(z13));
  inv1   g139(.a(x01), .O(new_n191_));
  oai21  g140(.a(x18), .b(new_n191_), .c(new_n60_), .O(new_n192_));
  nand4  g141(.a(new_n182_), .b(new_n160_), .c(new_n139_), .d(new_n126_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n112_), .c(x33), .O(new_n194_));
  nor2   g143(.a(x33), .b(x32), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n169_), .c(new_n150_), .d(new_n178_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n125_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n194_), .O(new_n199_));
  nand3  g148(.a(new_n199_), .b(new_n52_), .c(x16), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n192_), .O(z14));
  inv1   g150(.a(x00), .O(new_n202_));
  oai21  g151(.a(x18), .b(new_n202_), .c(new_n60_), .O(new_n203_));
  oai21  g152(.a(new_n196_), .b(new_n116_), .c(x34), .O(new_n204_));
  nand2  g153(.a(new_n150_), .b(new_n178_), .O(new_n205_));
  inv1   g154(.a(new_n205_), .O(new_n206_));
  inv1   g155(.a(x30), .O(new_n207_));
  inv1   g156(.a(x31), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  inv1   g158(.a(x32), .O(new_n210_));
  inv1   g159(.a(x33), .O(new_n211_));
  inv1   g160(.a(x34), .O(new_n212_));
  nand3  g161(.a(new_n212_), .b(new_n211_), .c(new_n210_), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n209_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n206_), .c(new_n125_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n204_), .O(new_n216_));
  nand3  g165(.a(new_n216_), .b(new_n52_), .c(x16), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n203_), .O(z15));
endmodule


