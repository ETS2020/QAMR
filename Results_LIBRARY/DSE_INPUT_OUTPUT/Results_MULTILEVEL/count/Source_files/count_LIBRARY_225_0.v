// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x22), .O(new_n60_));
  inv1   g009(.a(x29), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n56_), .O(z00));
  inv1   g014(.a(x20), .O(new_n66_));
  nor2   g015(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  nor3   g016(.a(x20), .b(x19), .c(x17), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  nor2   g018(.a(x16), .b(x14), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(z01));
  inv1   g021(.a(new_n62_), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n66_), .c(new_n53_), .d(new_n52_), .O(new_n75_));
  oai21  g024(.a(new_n68_), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n73_), .O(z02));
  oai21  g029(.a(new_n61_), .b(x22), .c(x21), .O(new_n81_));
  nand3  g030(.a(new_n66_), .b(new_n53_), .c(new_n52_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x22), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n55_), .c(new_n66_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(new_n81_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  nor2   g036(.a(x16), .b(x12), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n63_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n87_), .O(z03));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n58_), .b(new_n91_), .c(x18), .O(new_n92_));
  inv1   g041(.a(x23), .O(new_n93_));
  aoi21  g042(.a(new_n75_), .b(x29), .c(x22), .O(new_n94_));
  nand4  g043(.a(new_n84_), .b(new_n68_), .c(x29), .d(new_n93_), .O(new_n95_));
  oai21  g044(.a(new_n94_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  oai21  g046(.a(new_n92_), .b(new_n73_), .c(new_n97_), .O(z04));
  oai21  g047(.a(x16), .b(x10), .c(new_n59_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n62_), .O(new_n100_));
  inv1   g049(.a(x24), .O(new_n101_));
  nor2   g050(.a(x23), .b(x21), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n55_), .c(new_n66_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x29), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n60_), .c(new_n101_), .O(new_n105_));
  nor2   g054(.a(x23), .b(x22), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(x29), .c(new_n101_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n75_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n105_), .c(x16), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n100_), .O(z05));
  oai21  g059(.a(x16), .b(x09), .c(new_n59_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n62_), .O(new_n112_));
  inv1   g061(.a(x25), .O(new_n113_));
  nor2   g062(.a(x24), .b(x23), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n74_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n82_), .c(x29), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n60_), .c(new_n113_), .O(new_n117_));
  nand4  g066(.a(new_n106_), .b(x29), .c(new_n113_), .d(new_n101_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n75_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n117_), .c(x16), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n112_), .O(z06));
  oai21  g070(.a(x16), .b(x08), .c(new_n59_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n62_), .O(new_n123_));
  inv1   g072(.a(x26), .O(new_n124_));
  nor2   g073(.a(x25), .b(x24), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n102_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n82_), .c(x29), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n60_), .c(new_n124_), .O(new_n128_));
  nand4  g077(.a(new_n114_), .b(x29), .c(new_n124_), .d(new_n113_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n85_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n123_), .O(z07));
  oai21  g081(.a(x16), .b(x07), .c(new_n59_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n62_), .O(new_n134_));
  inv1   g083(.a(x27), .O(new_n135_));
  nor2   g084(.a(x26), .b(x25), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n114_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n75_), .c(x29), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n60_), .c(new_n135_), .O(new_n139_));
  nand2  g088(.a(new_n125_), .b(new_n93_), .O(new_n140_));
  nand3  g089(.a(x29), .b(new_n135_), .c(new_n124_), .O(new_n141_));
  nor3   g090(.a(new_n141_), .b(new_n140_), .c(new_n85_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n139_), .c(x16), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n134_), .O(z08));
  oai21  g093(.a(x16), .b(x06), .c(new_n59_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n62_), .O(new_n146_));
  inv1   g095(.a(x28), .O(new_n147_));
  nor2   g096(.a(x27), .b(x26), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n114_), .c(new_n113_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n75_), .c(x29), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n60_), .c(new_n147_), .O(new_n151_));
  nand3  g100(.a(new_n106_), .b(new_n68_), .c(new_n74_), .O(new_n152_));
  nor2   g101(.a(new_n61_), .b(x28), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n136_), .c(new_n135_), .d(new_n101_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n151_), .c(x16), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n146_), .O(z09));
  nor3   g106(.a(x23), .b(x21), .c(x20), .O(new_n158_));
  nor3   g107(.a(x28), .b(x27), .c(x26), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n125_), .c(new_n158_), .d(new_n55_), .O(new_n160_));
  nor2   g109(.a(new_n61_), .b(new_n60_), .O(new_n161_));
  aoi21  g110(.a(new_n160_), .b(new_n60_), .c(new_n161_), .O(new_n162_));
  nor2   g111(.a(x16), .b(x05), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n63_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n58_), .c(new_n164_), .O(z10));
  inv1   g114(.a(x04), .O(new_n166_));
  nand2  g115(.a(new_n58_), .b(new_n166_), .O(new_n167_));
  aoi21  g116(.a(x30), .b(x16), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n167_), .c(new_n73_), .O(z11));
  inv1   g118(.a(x03), .O(new_n170_));
  nand2  g119(.a(new_n58_), .b(new_n170_), .O(new_n171_));
  aoi21  g120(.a(x31), .b(x16), .c(x18), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n171_), .c(new_n73_), .O(z12));
  aoi21  g122(.a(x32), .b(x16), .c(x18), .O(new_n174_));
  inv1   g123(.a(x02), .O(new_n175_));
  nand2  g124(.a(new_n58_), .b(new_n175_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(new_n62_), .O(z13));
  aoi21  g126(.a(x33), .b(x16), .c(x18), .O(new_n178_));
  inv1   g127(.a(x01), .O(new_n179_));
  nand2  g128(.a(new_n58_), .b(new_n179_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n178_), .c(new_n62_), .O(z14));
  inv1   g130(.a(x00), .O(new_n182_));
  nand2  g131(.a(new_n58_), .b(new_n182_), .O(new_n183_));
  aoi21  g132(.a(x34), .b(x16), .c(x18), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n183_), .c(new_n73_), .O(z15));
endmodule


