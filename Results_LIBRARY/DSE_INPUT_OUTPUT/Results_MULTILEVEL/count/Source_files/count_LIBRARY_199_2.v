// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(x29), .b(x18), .O(new_n59_));
  oai21  g008(.a(new_n58_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(new_n55_), .b(x20), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n54_), .c(new_n53_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  inv1   g019(.a(new_n65_), .O(new_n71_));
  nand4  g020(.a(new_n70_), .b(new_n64_), .c(new_n54_), .d(new_n53_), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(x18), .c(new_n59_), .O(z02));
  nor3   g025(.a(x19), .b(x18), .c(x17), .O(new_n77_));
  nor3   g026(.a(x22), .b(x21), .c(x20), .O(new_n78_));
  aoi22  g027(.a(new_n78_), .b(new_n77_), .c(new_n72_), .d(x22), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n62_), .b(new_n80_), .c(x18), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n62_), .c(new_n81_), .O(z03));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n55_), .c(x23), .O(new_n85_));
  nor2   g034(.a(x20), .b(x19), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n70_), .d(new_n53_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n85_), .c(new_n62_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n89_), .c(new_n61_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n59_), .O(z04));
  nand2  g041(.a(new_n88_), .b(x24), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n83_), .c(new_n86_), .d(new_n53_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n62_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n61_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n59_), .O(z05));
  nand2  g048(.a(new_n95_), .b(x25), .O(new_n100_));
  inv1   g049(.a(x22), .O(new_n101_));
  inv1   g050(.a(x23), .O(new_n102_));
  inv1   g051(.a(x24), .O(new_n103_));
  inv1   g052(.a(x25), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n72_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n100_), .c(new_n62_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n108_), .c(new_n61_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n59_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nor2   g061(.a(x18), .b(x17), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n70_), .c(new_n64_), .d(new_n54_), .O(new_n114_));
  nor3   g063(.a(x26), .b(x25), .c(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n87_), .O(new_n116_));
  oai22  g065(.a(new_n116_), .b(new_n114_), .c(new_n106_), .d(new_n112_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g067(.a(x08), .O(new_n119_));
  aoi21  g068(.a(new_n62_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z07));
  oai21  g070(.a(new_n116_), .b(new_n72_), .c(x27), .O(new_n122_));
  nor2   g071(.a(new_n84_), .b(new_n55_), .O(new_n123_));
  nor3   g072(.a(x27), .b(x26), .c(x25), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n94_), .c(new_n123_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n122_), .c(new_n62_), .O(new_n126_));
  oai21  g075(.a(x16), .b(x07), .c(new_n61_), .O(new_n127_));
  or2    g076(.a(new_n127_), .b(new_n126_), .O(z08));
  nand2  g077(.a(new_n125_), .b(x28), .O(new_n129_));
  nor3   g078(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nor2   g079(.a(x28), .b(x27), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n130_), .c(new_n123_), .d(new_n112_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x06), .O(new_n135_));
  aoi21  g084(.a(new_n62_), .b(new_n135_), .c(x18), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z09));
  inv1   g086(.a(x29), .O(new_n138_));
  nand4  g087(.a(new_n78_), .b(new_n54_), .c(new_n53_), .d(x16), .O(new_n139_));
  inv1   g088(.a(x27), .O(new_n140_));
  nor2   g089(.a(x29), .b(x28), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n140_), .c(new_n112_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n130_), .O(new_n144_));
  oai22  g093(.a(new_n144_), .b(new_n139_), .c(x16), .d(x05), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n61_), .O(new_n146_));
  aoi21  g095(.a(new_n132_), .b(x16), .c(x18), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n138_), .c(new_n146_), .O(z10));
  inv1   g097(.a(new_n88_), .O(new_n149_));
  nand4  g098(.a(new_n141_), .b(new_n115_), .c(new_n149_), .d(new_n140_), .O(new_n150_));
  inv1   g099(.a(x30), .O(new_n151_));
  nand4  g100(.a(new_n131_), .b(new_n115_), .c(new_n151_), .d(new_n138_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n88_), .O(new_n153_));
  aoi21  g102(.a(new_n150_), .b(x30), .c(new_n153_), .O(new_n154_));
  inv1   g103(.a(x04), .O(new_n155_));
  aoi21  g104(.a(new_n62_), .b(new_n155_), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n62_), .c(new_n156_), .O(z11));
  inv1   g106(.a(x31), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n62_), .c(new_n61_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x29), .O(new_n160_));
  nor2   g109(.a(x26), .b(x25), .O(new_n161_));
  nor2   g110(.a(x30), .b(x28), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n161_), .c(new_n140_), .d(new_n103_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n88_), .c(x31), .O(new_n164_));
  nand2  g113(.a(new_n94_), .b(new_n83_), .O(new_n165_));
  inv1   g114(.a(new_n165_), .O(new_n166_));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n141_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n124_), .c(new_n166_), .d(new_n71_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n164_), .c(new_n62_), .O(new_n171_));
  nor2   g120(.a(x16), .b(x03), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n171_), .c(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n160_), .O(z12));
  inv1   g123(.a(x32), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n62_), .c(new_n61_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x29), .O(new_n177_));
  nand4  g126(.a(new_n167_), .b(new_n131_), .c(new_n161_), .d(new_n103_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n88_), .c(x32), .O(new_n179_));
  nand2  g128(.a(new_n131_), .b(new_n161_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nand4  g130(.a(new_n175_), .b(new_n158_), .c(new_n151_), .d(new_n138_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n181_), .c(new_n166_), .d(new_n71_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n179_), .c(new_n62_), .O(new_n185_));
  nor2   g134(.a(x16), .b(x02), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n185_), .c(new_n61_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n177_), .O(z13));
  nand2  g137(.a(new_n184_), .b(x33), .O(new_n189_));
  nor2   g138(.a(x33), .b(x32), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n167_), .c(new_n143_), .d(new_n106_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n189_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x16), .O(new_n193_));
  inv1   g142(.a(x01), .O(new_n194_));
  aoi21  g143(.a(new_n62_), .b(new_n194_), .c(x18), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n193_), .O(z14));
  inv1   g145(.a(x34), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n62_), .c(new_n61_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x29), .O(new_n199_));
  nand4  g148(.a(new_n190_), .b(new_n167_), .c(new_n131_), .d(new_n161_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n95_), .c(x34), .O(new_n201_));
  nand2  g150(.a(new_n158_), .b(new_n151_), .O(new_n202_));
  inv1   g151(.a(x33), .O(new_n203_));
  nand3  g152(.a(new_n197_), .b(new_n203_), .c(new_n175_), .O(new_n204_));
  nor2   g153(.a(new_n204_), .b(new_n202_), .O(new_n205_));
  nand3  g154(.a(new_n205_), .b(new_n143_), .c(new_n106_), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n201_), .c(new_n62_), .O(new_n207_));
  nor2   g156(.a(x16), .b(x00), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n207_), .c(new_n61_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n199_), .O(z15));
endmodule


