// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_;
  inv1   g000(.a(x07), .O(new_n52_));
  nand2  g001(.a(x12), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  oai21  g005(.a(x18), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x17), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x19), .b(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  aoi21  g010(.a(x19), .b(new_n59_), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(x16), .c(new_n61_), .O(new_n63_));
  aoi21  g012(.a(new_n63_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g013(.a(x14), .O(new_n65_));
  oai21  g014(.a(x18), .b(new_n65_), .c(new_n56_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n56_), .c(new_n59_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x17), .O(new_n69_));
  inv1   g018(.a(x19), .O(new_n70_));
  nand3  g019(.a(new_n67_), .b(new_n58_), .c(x16), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g022(.a(new_n53_), .b(x18), .c(new_n58_), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(new_n67_), .c(new_n70_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(x16), .c(new_n54_), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n73_), .c(new_n69_), .d(new_n66_), .O(z01));
  inv1   g026(.a(x21), .O(new_n78_));
  nor3   g027(.a(x20), .b(x19), .c(x17), .O(new_n79_));
  nand4  g028(.a(new_n78_), .b(new_n67_), .c(new_n70_), .d(new_n58_), .O(new_n80_));
  oai21  g029(.a(new_n79_), .b(new_n78_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x16), .O(new_n82_));
  inv1   g031(.a(x13), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(new_n83_), .c(x18), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n54_), .O(z02));
  nor2   g034(.a(x19), .b(x17), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  nor2   g036(.a(x22), .b(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n67_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n87_), .c(new_n59_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n53_), .O(new_n91_));
  oai21  g040(.a(new_n56_), .b(x12), .c(new_n52_), .O(new_n92_));
  inv1   g041(.a(x12), .O(new_n93_));
  oai21  g042(.a(x18), .b(new_n93_), .c(new_n56_), .O(new_n94_));
  nor2   g043(.a(new_n88_), .b(new_n59_), .O(new_n95_));
  inv1   g044(.a(x22), .O(new_n96_));
  inv1   g045(.a(new_n80_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(x16), .c(new_n95_), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n94_), .c(new_n92_), .d(new_n91_), .O(z03));
  nand3  g049(.a(new_n88_), .b(new_n86_), .c(new_n67_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x23), .O(new_n102_));
  nor3   g051(.a(x23), .b(x22), .c(x21), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n79_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g055(.a(x11), .O(new_n107_));
  aoi21  g056(.a(new_n56_), .b(new_n107_), .c(x18), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n54_), .O(z04));
  inv1   g058(.a(x24), .O(new_n110_));
  aoi21  g059(.a(new_n103_), .b(new_n79_), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x20), .b(x19), .O(new_n112_));
  nor2   g061(.a(x24), .b(x23), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n88_), .c(new_n112_), .d(new_n58_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n111_), .c(x16), .O(new_n116_));
  inv1   g065(.a(x10), .O(new_n117_));
  aoi21  g066(.a(new_n56_), .b(new_n117_), .c(x18), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n116_), .c(new_n54_), .O(z05));
  inv1   g068(.a(x25), .O(new_n120_));
  nor2   g069(.a(x21), .b(x20), .O(new_n121_));
  nor2   g070(.a(x23), .b(x22), .O(new_n122_));
  nor2   g071(.a(x25), .b(x24), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n86_), .O(new_n124_));
  oai21  g073(.a(new_n115_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x09), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(new_n53_), .O(z06));
  inv1   g078(.a(x26), .O(new_n130_));
  inv1   g079(.a(new_n124_), .O(new_n131_));
  nor3   g080(.a(x26), .b(x25), .c(x24), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n122_), .O(new_n133_));
  oai22  g082(.a(new_n133_), .b(new_n80_), .c(new_n131_), .d(new_n130_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  oai21  g084(.a(x16), .b(x08), .c(new_n59_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n54_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z07));
  oai21  g087(.a(new_n133_), .b(new_n80_), .c(x27), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n113_), .c(new_n120_), .O(new_n141_));
  or2    g090(.a(new_n141_), .b(new_n101_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n92_), .c(new_n59_), .O(z08));
  oai21  g094(.a(new_n141_), .b(new_n101_), .c(x28), .O(new_n146_));
  inv1   g095(.a(new_n101_), .O(new_n147_));
  nor3   g096(.a(x25), .b(x24), .c(x23), .O(new_n148_));
  nor3   g097(.a(x28), .b(x27), .c(x26), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n148_), .c(new_n147_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  inv1   g101(.a(x06), .O(new_n153_));
  aoi21  g102(.a(new_n56_), .b(new_n153_), .c(x18), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n152_), .c(new_n54_), .O(z09));
  nor3   g104(.a(x29), .b(x28), .c(x27), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n132_), .c(new_n103_), .d(new_n79_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  aoi21  g107(.a(new_n150_), .b(x29), .c(new_n158_), .O(new_n159_));
  oai21  g108(.a(x16), .b(x05), .c(new_n59_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n54_), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n56_), .c(new_n161_), .O(z10));
  nor2   g111(.a(x28), .b(x27), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n163_), .c(new_n132_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n104_), .O(new_n166_));
  aoi21  g115(.a(new_n157_), .b(x30), .c(new_n166_), .O(new_n167_));
  oai21  g116(.a(x16), .b(x04), .c(new_n59_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n54_), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n56_), .c(new_n169_), .O(z11));
  oai21  g119(.a(new_n165_), .b(new_n104_), .c(x31), .O(new_n171_));
  nor2   g120(.a(x29), .b(x28), .O(new_n172_));
  nor2   g121(.a(x31), .b(x30), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n172_), .c(new_n140_), .d(new_n120_), .O(new_n174_));
  or2    g123(.a(new_n174_), .b(new_n114_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n171_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  oai21  g126(.a(x16), .b(x03), .c(new_n59_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n54_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z12));
  oai21  g129(.a(new_n174_), .b(new_n114_), .c(x32), .O(new_n181_));
  inv1   g130(.a(x23), .O(new_n182_));
  nand4  g131(.a(new_n110_), .b(new_n182_), .c(new_n96_), .d(new_n78_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  inv1   g133(.a(x27), .O(new_n185_));
  inv1   g134(.a(x28), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n185_), .c(new_n130_), .d(new_n120_), .O(new_n187_));
  inv1   g136(.a(new_n187_), .O(new_n188_));
  inv1   g137(.a(x29), .O(new_n189_));
  inv1   g138(.a(x30), .O(new_n190_));
  inv1   g139(.a(x31), .O(new_n191_));
  inv1   g140(.a(x32), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n191_), .c(new_n190_), .d(new_n189_), .O(new_n193_));
  inv1   g142(.a(new_n193_), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n188_), .c(new_n184_), .d(new_n79_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n181_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x16), .O(new_n197_));
  inv1   g146(.a(x02), .O(new_n198_));
  aoi21  g147(.a(new_n56_), .b(new_n198_), .c(x18), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n197_), .c(new_n54_), .O(z13));
  nor2   g149(.a(x26), .b(x25), .O(new_n201_));
  nor2   g150(.a(x32), .b(x31), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n164_), .c(new_n163_), .d(new_n201_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n114_), .c(x33), .O(new_n204_));
  nand4  g153(.a(new_n120_), .b(new_n110_), .c(new_n182_), .d(new_n96_), .O(new_n205_));
  inv1   g154(.a(new_n205_), .O(new_n206_));
  nand4  g155(.a(new_n189_), .b(new_n186_), .c(new_n185_), .d(new_n130_), .O(new_n207_));
  inv1   g156(.a(new_n207_), .O(new_n208_));
  inv1   g157(.a(x33), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n192_), .c(new_n191_), .d(new_n190_), .O(new_n210_));
  inv1   g159(.a(new_n210_), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n208_), .c(new_n206_), .d(new_n97_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n204_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(x16), .O(new_n214_));
  inv1   g163(.a(x01), .O(new_n215_));
  aoi21  g164(.a(new_n56_), .b(new_n215_), .c(x18), .O(new_n216_));
  aoi21  g165(.a(new_n216_), .b(new_n214_), .c(new_n54_), .O(z14));
  nor2   g166(.a(x33), .b(x32), .O(new_n218_));
  nand4  g167(.a(new_n218_), .b(new_n173_), .c(new_n172_), .d(new_n140_), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n124_), .c(x34), .O(new_n220_));
  nor3   g169(.a(x34), .b(x33), .c(x32), .O(new_n221_));
  nand4  g170(.a(new_n221_), .b(new_n208_), .c(new_n173_), .d(new_n131_), .O(new_n222_));
  nand2  g171(.a(new_n222_), .b(new_n220_), .O(new_n223_));
  nand2  g172(.a(new_n223_), .b(x16), .O(new_n224_));
  inv1   g173(.a(x00), .O(new_n225_));
  aoi21  g174(.a(new_n56_), .b(new_n225_), .c(x18), .O(new_n226_));
  nand3  g175(.a(new_n226_), .b(new_n224_), .c(new_n53_), .O(z15));
endmodule


