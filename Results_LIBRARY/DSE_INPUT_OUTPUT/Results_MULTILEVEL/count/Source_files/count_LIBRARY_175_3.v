// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand3  g001(.a(x19), .b(new_n52_), .c(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(x16), .O(new_n57_));
  inv1   g006(.a(x19), .O(new_n58_));
  nor3   g007(.a(x18), .b(x16), .c(x15), .O(new_n59_));
  aoi21  g008(.a(new_n58_), .b(x18), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  inv1   g010(.a(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(x18), .c(new_n58_), .O(new_n66_));
  inv1   g015(.a(x16), .O(new_n67_));
  nor3   g016(.a(new_n54_), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n68_), .c(new_n52_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n66_), .O(z01));
  nor2   g020(.a(x20), .b(x19), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n62_), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n54_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n73_), .b(x21), .c(new_n76_), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n67_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n67_), .c(new_n79_), .O(z02));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n63_), .c(new_n62_), .d(x16), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n52_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n58_), .O(new_n84_));
  and2   g033(.a(new_n75_), .b(x22), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  aoi21  g035(.a(new_n85_), .b(x16), .c(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(x18), .c(new_n84_), .O(z03));
  nand3  g037(.a(new_n81_), .b(new_n54_), .c(new_n63_), .O(new_n89_));
  nor3   g038(.a(x20), .b(x19), .c(x17), .O(new_n90_));
  nor3   g039(.a(x23), .b(x22), .c(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  aoi21  g042(.a(new_n89_), .b(x23), .c(new_n93_), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n67_), .b(new_n95_), .c(x18), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n67_), .c(new_n96_), .O(z04));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n81_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  aoi22  g049(.a(new_n100_), .b(new_n90_), .c(new_n92_), .d(x24), .O(new_n101_));
  inv1   g050(.a(x10), .O(new_n102_));
  aoi21  g051(.a(new_n67_), .b(new_n102_), .c(x18), .O(new_n103_));
  oai21  g052(.a(new_n101_), .b(new_n67_), .c(new_n103_), .O(z05));
  nand4  g053(.a(new_n98_), .b(new_n81_), .c(new_n72_), .d(new_n62_), .O(new_n105_));
  nor2   g054(.a(x23), .b(x22), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n106_), .c(new_n74_), .d(new_n54_), .O(new_n108_));
  inv1   g057(.a(new_n108_), .O(new_n109_));
  aoi21  g058(.a(new_n105_), .b(x25), .c(new_n109_), .O(new_n110_));
  inv1   g059(.a(x09), .O(new_n111_));
  aoi21  g060(.a(new_n67_), .b(new_n111_), .c(x18), .O(new_n112_));
  oai21  g061(.a(new_n110_), .b(new_n67_), .c(new_n112_), .O(z06));
  nand3  g062(.a(new_n74_), .b(new_n62_), .c(x16), .O(new_n114_));
  inv1   g063(.a(x24), .O(new_n115_));
  nor2   g064(.a(x26), .b(x25), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n106_), .c(new_n115_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n114_), .c(new_n52_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n58_), .O(new_n119_));
  nand3  g068(.a(new_n108_), .b(x26), .c(x16), .O(new_n120_));
  oai21  g069(.a(x16), .b(x08), .c(new_n120_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n119_), .O(z07));
  inv1   g072(.a(x25), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n98_), .c(new_n124_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n82_), .c(new_n52_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n58_), .O(new_n128_));
  oai21  g077(.a(new_n117_), .b(new_n75_), .c(x27), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  nand2  g079(.a(new_n67_), .b(new_n130_), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n67_), .c(new_n131_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n52_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n128_), .O(z08));
  inv1   g083(.a(x23), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  nor2   g085(.a(x28), .b(x27), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n107_), .c(new_n136_), .d(new_n135_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n82_), .c(new_n52_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n58_), .O(new_n140_));
  oai21  g089(.a(new_n126_), .b(new_n89_), .c(x28), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  nand2  g091(.a(new_n67_), .b(new_n142_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n67_), .c(new_n143_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n52_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n140_), .O(z09));
  nand2  g095(.a(new_n91_), .b(new_n65_), .O(new_n147_));
  nor3   g096(.a(x26), .b(x25), .c(x24), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n147_), .c(new_n52_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n58_), .O(new_n152_));
  oai21  g101(.a(new_n138_), .b(new_n89_), .c(x29), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  nand2  g103(.a(new_n67_), .b(new_n154_), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n67_), .c(new_n155_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n52_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n152_), .O(z10));
  nand4  g107(.a(new_n149_), .b(new_n148_), .c(new_n91_), .d(new_n90_), .O(new_n159_));
  nor2   g108(.a(x30), .b(x29), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n137_), .c(new_n116_), .d(new_n115_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n92_), .O(new_n162_));
  aoi21  g111(.a(new_n159_), .b(x30), .c(new_n162_), .O(new_n163_));
  inv1   g112(.a(x04), .O(new_n164_));
  aoi21  g113(.a(new_n67_), .b(new_n164_), .c(x18), .O(new_n165_));
  oai21  g114(.a(new_n163_), .b(new_n67_), .c(new_n165_), .O(z11));
  oai21  g115(.a(new_n161_), .b(new_n92_), .c(x31), .O(new_n167_));
  nor3   g116(.a(x27), .b(x26), .c(x25), .O(new_n168_));
  nor2   g117(.a(x29), .b(x28), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n168_), .c(new_n100_), .d(new_n90_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n167_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  inv1   g124(.a(x03), .O(new_n176_));
  aoi21  g125(.a(new_n67_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z12));
  nand2  g127(.a(new_n100_), .b(new_n65_), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n160_), .c(new_n137_), .d(new_n116_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n179_), .c(new_n52_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n58_), .O(new_n183_));
  nand4  g132(.a(new_n170_), .b(new_n169_), .c(new_n125_), .d(new_n124_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n105_), .c(x32), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  nand2  g135(.a(new_n67_), .b(new_n186_), .O(new_n187_));
  oai21  g136(.a(new_n185_), .b(new_n67_), .c(new_n187_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n52_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n183_), .O(z13));
  nor2   g139(.a(x17), .b(new_n67_), .O(new_n191_));
  nand4  g140(.a(new_n107_), .b(new_n106_), .c(new_n191_), .d(new_n74_), .O(new_n192_));
  nor2   g141(.a(x33), .b(x32), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n170_), .c(new_n169_), .d(new_n125_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n192_), .c(new_n52_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n58_), .O(new_n196_));
  oai21  g145(.a(new_n181_), .b(new_n105_), .c(x33), .O(new_n197_));
  inv1   g146(.a(x01), .O(new_n198_));
  nand2  g147(.a(new_n67_), .b(new_n198_), .O(new_n199_));
  oai21  g148(.a(new_n197_), .b(new_n67_), .c(new_n199_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n52_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n196_), .O(z14));
  oai21  g151(.a(new_n194_), .b(new_n108_), .c(x34), .O(new_n203_));
  nand2  g152(.a(new_n169_), .b(new_n125_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  inv1   g154(.a(new_n170_), .O(new_n206_));
  inv1   g155(.a(x32), .O(new_n207_));
  nor2   g156(.a(x34), .b(x33), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n206_), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n205_), .c(new_n109_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n203_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(x16), .O(new_n213_));
  inv1   g162(.a(x00), .O(new_n214_));
  aoi21  g163(.a(new_n67_), .b(new_n214_), .c(x18), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n213_), .O(z15));
endmodule


