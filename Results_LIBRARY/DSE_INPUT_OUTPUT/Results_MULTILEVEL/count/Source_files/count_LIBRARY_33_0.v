// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  oai21  g007(.a(x19), .b(x17), .c(x20), .O(new_n59_));
  nor2   g008(.a(x20), .b(x19), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n53_), .c(x16), .O(new_n61_));
  nand4  g010(.a(new_n61_), .b(new_n59_), .c(new_n52_), .d(x16), .O(z01));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x21), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n66_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n65_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g018(.a(new_n68_), .b(x22), .O(new_n70_));
  nor2   g019(.a(x22), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n56_), .c(new_n66_), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n52_), .d(x16), .O(z03));
  nand2  g022(.a(new_n72_), .b(x23), .O(new_n74_));
  nor2   g023(.a(x23), .b(x22), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n60_), .c(new_n67_), .d(new_n53_), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n74_), .c(new_n52_), .d(x16), .O(z04));
  nand2  g026(.a(new_n76_), .b(x24), .O(new_n78_));
  nor2   g027(.a(x24), .b(x23), .O(new_n79_));
  nand4  g028(.a(new_n79_), .b(new_n71_), .c(new_n60_), .d(new_n53_), .O(new_n80_));
  nand4  g029(.a(new_n80_), .b(new_n78_), .c(new_n52_), .d(x16), .O(z05));
  nand2  g030(.a(new_n80_), .b(x25), .O(new_n82_));
  nor2   g031(.a(x21), .b(x20), .O(new_n83_));
  nor2   g032(.a(x25), .b(x24), .O(new_n84_));
  nand4  g033(.a(new_n84_), .b(new_n75_), .c(new_n83_), .d(new_n56_), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n82_), .c(new_n52_), .d(x16), .O(z06));
  nand2  g035(.a(new_n85_), .b(x26), .O(new_n87_));
  inv1   g036(.a(new_n68_), .O(new_n88_));
  nor3   g037(.a(x26), .b(x25), .c(x24), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n75_), .c(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x16), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(z07));
  nand2  g042(.a(new_n90_), .b(x27), .O(new_n94_));
  nor3   g043(.a(x22), .b(x21), .c(x20), .O(new_n95_));
  nor3   g044(.a(x27), .b(x26), .c(x25), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n79_), .c(new_n95_), .d(new_n56_), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n94_), .c(new_n52_), .d(x16), .O(z08));
  inv1   g047(.a(x16), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  inv1   g049(.a(x26), .O(new_n101_));
  nor2   g050(.a(x28), .b(x27), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n84_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n72_), .O(new_n104_));
  aoi21  g053(.a(new_n97_), .b(x28), .c(new_n104_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n99_), .c(new_n52_), .O(z09));
  oai21  g055(.a(new_n103_), .b(new_n72_), .c(x29), .O(new_n107_));
  nor3   g056(.a(x23), .b(x22), .c(x21), .O(new_n108_));
  nor3   g057(.a(x29), .b(x28), .c(x27), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n89_), .c(new_n108_), .d(new_n63_), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n107_), .c(new_n52_), .d(x16), .O(z10));
  inv1   g060(.a(x24), .O(new_n112_));
  nor2   g061(.a(x26), .b(x25), .O(new_n113_));
  nor2   g062(.a(x30), .b(x29), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n102_), .c(new_n113_), .d(new_n112_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n76_), .O(new_n116_));
  aoi21  g065(.a(new_n110_), .b(x30), .c(new_n116_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n99_), .c(new_n52_), .O(z11));
  oai21  g067(.a(new_n115_), .b(new_n76_), .c(x31), .O(new_n119_));
  inv1   g068(.a(x22), .O(new_n120_));
  nand4  g069(.a(new_n112_), .b(new_n100_), .c(new_n120_), .d(new_n67_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  inv1   g072(.a(x29), .O(new_n124_));
  inv1   g073(.a(x30), .O(new_n125_));
  inv1   g074(.a(x31), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  inv1   g076(.a(new_n127_), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n96_), .c(new_n122_), .d(new_n63_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n119_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z12));
  nand2  g081(.a(new_n129_), .b(x32), .O(new_n133_));
  nor2   g082(.a(x32), .b(x31), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n114_), .c(new_n102_), .d(new_n113_), .O(new_n135_));
  or2    g084(.a(new_n135_), .b(new_n80_), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n133_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g086(.a(new_n135_), .b(new_n80_), .c(x33), .O(new_n138_));
  inv1   g087(.a(x25), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n112_), .c(new_n100_), .d(new_n120_), .O(new_n140_));
  inv1   g089(.a(new_n140_), .O(new_n141_));
  inv1   g090(.a(x27), .O(new_n142_));
  nand4  g091(.a(new_n124_), .b(new_n123_), .c(new_n142_), .d(new_n101_), .O(new_n143_));
  inv1   g092(.a(new_n143_), .O(new_n144_));
  inv1   g093(.a(x32), .O(new_n145_));
  inv1   g094(.a(x33), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n126_), .d(new_n125_), .O(new_n147_));
  inv1   g096(.a(new_n147_), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n144_), .c(new_n141_), .d(new_n88_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n138_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z14));
  nand2  g101(.a(new_n149_), .b(x34), .O(new_n153_));
  inv1   g102(.a(new_n85_), .O(new_n154_));
  nor2   g103(.a(x31), .b(x30), .O(new_n155_));
  nor3   g104(.a(x34), .b(x33), .c(x32), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n144_), .c(new_n155_), .d(new_n154_), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n153_), .c(new_n52_), .d(x16), .O(z15));
endmodule


