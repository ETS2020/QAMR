// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x22), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(new_n54_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  inv1   g018(.a(x17), .O(new_n70_));
  inv1   g019(.a(x19), .O(new_n71_));
  inv1   g020(.a(x20), .O(new_n72_));
  aoi21  g021(.a(x22), .b(x18), .c(x21), .O(new_n73_));
  nand4  g022(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g023(.a(new_n65_), .b(x21), .c(new_n61_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  nand3  g027(.a(new_n61_), .b(new_n62_), .c(new_n78_), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n77_), .c(new_n59_), .O(z02));
  nor2   g029(.a(x17), .b(new_n62_), .O(new_n81_));
  nor2   g030(.a(x21), .b(x20), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(new_n71_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  nand2  g034(.a(new_n82_), .b(new_n53_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n58_), .O(new_n88_));
  nor2   g037(.a(x16), .b(x12), .O(new_n89_));
  aoi21  g038(.a(new_n88_), .b(x16), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(x18), .c(new_n85_), .O(z03));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n53_), .c(new_n72_), .O(new_n93_));
  nor3   g042(.a(x23), .b(x22), .c(x21), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n64_), .c(new_n93_), .d(x23), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n62_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n62_), .c(new_n97_), .O(z04));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n82_), .c(new_n53_), .d(x16), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n61_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n58_), .O(new_n102_));
  nand2  g051(.a(new_n94_), .b(new_n64_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(x24), .c(x16), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x10), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n105_), .c(new_n61_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n102_), .O(z05));
  nor2   g057(.a(x20), .b(x19), .O(new_n109_));
  nor2   g058(.a(x23), .b(x21), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n110_), .c(new_n81_), .d(new_n109_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n61_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n58_), .O(new_n114_));
  nand4  g063(.a(new_n99_), .b(new_n92_), .c(new_n109_), .d(new_n70_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(x25), .c(x16), .O(new_n116_));
  oai21  g065(.a(x16), .b(x09), .c(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n61_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n114_), .O(z06));
  nor2   g068(.a(x23), .b(x22), .O(new_n120_));
  nand4  g069(.a(new_n111_), .b(new_n120_), .c(new_n82_), .d(new_n53_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x26), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n120_), .c(new_n87_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n62_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z07));
  inv1   g078(.a(x25), .O(new_n130_));
  nor2   g079(.a(x27), .b(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n99_), .c(new_n130_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n93_), .O(new_n133_));
  aoi21  g082(.a(new_n124_), .b(x27), .c(new_n133_), .O(new_n134_));
  inv1   g083(.a(x07), .O(new_n135_));
  aoi21  g084(.a(new_n62_), .b(new_n135_), .c(x18), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n62_), .c(new_n136_), .O(z08));
  inv1   g086(.a(x23), .O(new_n138_));
  inv1   g087(.a(x26), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n111_), .c(new_n139_), .d(new_n138_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n83_), .c(new_n61_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n58_), .O(new_n143_));
  oai21  g092(.a(new_n132_), .b(new_n93_), .c(x28), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  nand2  g094(.a(new_n62_), .b(new_n145_), .O(new_n146_));
  oai21  g095(.a(new_n144_), .b(new_n62_), .c(new_n146_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n61_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n143_), .O(z09));
  oai21  g098(.a(new_n141_), .b(new_n93_), .c(x29), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n123_), .c(new_n94_), .d(new_n64_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g103(.a(x05), .O(new_n155_));
  aoi21  g104(.a(new_n62_), .b(new_n155_), .c(x18), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(z10));
  nand4  g106(.a(new_n110_), .b(new_n53_), .c(new_n72_), .d(x16), .O(new_n158_));
  inv1   g107(.a(x24), .O(new_n159_));
  nor2   g108(.a(x26), .b(x25), .O(new_n160_));
  nor2   g109(.a(x30), .b(x29), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n140_), .c(new_n160_), .d(new_n159_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n158_), .c(new_n61_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n58_), .O(new_n164_));
  nand3  g113(.a(new_n152_), .b(x30), .c(x16), .O(new_n165_));
  inv1   g114(.a(x04), .O(new_n166_));
  nand2  g115(.a(new_n62_), .b(new_n166_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n165_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n61_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n164_), .O(z11));
  oai21  g119(.a(new_n162_), .b(new_n103_), .c(x31), .O(new_n171_));
  nand2  g120(.a(new_n99_), .b(new_n92_), .O(new_n172_));
  inv1   g121(.a(new_n172_), .O(new_n173_));
  nor3   g122(.a(x27), .b(x26), .c(x25), .O(new_n174_));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nor2   g124(.a(x31), .b(x30), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  inv1   g126(.a(new_n177_), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n174_), .c(new_n173_), .d(new_n64_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n171_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(x16), .O(new_n181_));
  inv1   g130(.a(x03), .O(new_n182_));
  aoi21  g131(.a(new_n62_), .b(new_n182_), .c(x18), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n181_), .O(z12));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n161_), .c(new_n140_), .d(new_n160_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n115_), .O(new_n187_));
  aoi21  g136(.a(new_n179_), .b(x32), .c(new_n187_), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n62_), .b(new_n189_), .c(x18), .O(new_n190_));
  oai21  g139(.a(new_n188_), .b(new_n62_), .c(new_n190_), .O(z13));
  nor2   g140(.a(x33), .b(x32), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n176_), .c(new_n175_), .d(new_n131_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n112_), .c(new_n61_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n58_), .O(new_n195_));
  oai21  g144(.a(new_n186_), .b(new_n115_), .c(x33), .O(new_n196_));
  inv1   g145(.a(x01), .O(new_n197_));
  nand2  g146(.a(new_n62_), .b(new_n197_), .O(new_n198_));
  oai21  g147(.a(new_n196_), .b(new_n62_), .c(new_n198_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n61_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n195_), .O(z14));
  nor3   g150(.a(x34), .b(x33), .c(x32), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n176_), .c(new_n175_), .d(new_n131_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n112_), .c(new_n61_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n58_), .O(new_n205_));
  oai21  g154(.a(new_n193_), .b(new_n121_), .c(x34), .O(new_n206_));
  inv1   g155(.a(x00), .O(new_n207_));
  nand2  g156(.a(new_n62_), .b(new_n207_), .O(new_n208_));
  oai21  g157(.a(new_n206_), .b(new_n62_), .c(new_n208_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n61_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n205_), .O(z15));
endmodule


