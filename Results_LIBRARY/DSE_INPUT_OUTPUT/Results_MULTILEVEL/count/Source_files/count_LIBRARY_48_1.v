// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x23), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n53_), .c(new_n52_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n62_), .b(x20), .c(new_n65_), .O(new_n66_));
  inv1   g015(.a(new_n60_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g018(.a(new_n66_), .b(new_n58_), .c(new_n69_), .O(z01));
  inv1   g019(.a(x18), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n73_));
  oai21  g022(.a(new_n65_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x23), .c(new_n71_), .O(z02));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n62_), .O(new_n80_));
  aoi21  g029(.a(new_n73_), .b(x22), .c(new_n80_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n58_), .c(new_n83_), .O(z03));
  inv1   g033(.a(x11), .O(new_n85_));
  nand2  g034(.a(new_n58_), .b(new_n85_), .O(new_n86_));
  nor3   g035(.a(x22), .b(x21), .c(x20), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n55_), .c(x16), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n86_), .c(new_n60_), .O(z04));
  nand2  g038(.a(new_n87_), .b(new_n55_), .O(new_n90_));
  inv1   g039(.a(x22), .O(new_n91_));
  inv1   g040(.a(x24), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n91_), .c(new_n72_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  aoi21  g043(.a(new_n90_), .b(x24), .c(new_n94_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n58_), .c(new_n97_), .O(z05));
  inv1   g047(.a(x23), .O(new_n99_));
  nor2   g048(.a(x20), .b(x19), .O(new_n100_));
  nor2   g049(.a(x24), .b(x22), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n100_), .c(new_n72_), .d(new_n52_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x25), .O(new_n103_));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n78_), .c(new_n100_), .d(new_n52_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n58_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n106_), .c(new_n99_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n71_), .O(z06));
  nand2  g058(.a(new_n105_), .b(x26), .O(new_n110_));
  nor2   g059(.a(x21), .b(x20), .O(new_n111_));
  nor2   g060(.a(x26), .b(x25), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n101_), .c(new_n111_), .d(new_n55_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n110_), .c(new_n58_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x08), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n114_), .c(new_n99_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n71_), .O(z07));
  nand2  g066(.a(new_n113_), .b(x27), .O(new_n118_));
  inv1   g067(.a(new_n73_), .O(new_n119_));
  nor3   g068(.a(x27), .b(x26), .c(x25), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n101_), .c(new_n119_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n118_), .c(new_n58_), .O(new_n122_));
  nor2   g071(.a(x16), .b(x07), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n122_), .c(new_n99_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n71_), .O(z08));
  inv1   g074(.a(x26), .O(new_n126_));
  nor2   g075(.a(x28), .b(x27), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n104_), .c(new_n126_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n90_), .O(new_n129_));
  aoi21  g078(.a(new_n121_), .b(x28), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x06), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n67_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n58_), .c(new_n132_), .O(z09));
  oai21  g082(.a(new_n128_), .b(new_n90_), .c(x29), .O(new_n134_));
  inv1   g083(.a(x27), .O(new_n135_));
  nor3   g084(.a(x26), .b(x25), .c(x24), .O(new_n136_));
  nor2   g085(.a(x29), .b(x28), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n136_), .c(new_n80_), .d(new_n135_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  nor2   g089(.a(x16), .b(x05), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n67_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z10));
  inv1   g092(.a(x25), .O(new_n144_));
  inv1   g093(.a(x28), .O(new_n145_));
  nor2   g094(.a(x27), .b(x26), .O(new_n146_));
  nor2   g095(.a(x30), .b(x29), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n144_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n102_), .O(new_n149_));
  aoi21  g098(.a(new_n138_), .b(x30), .c(new_n149_), .O(new_n150_));
  nor2   g099(.a(x16), .b(x04), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n67_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n58_), .c(new_n152_), .O(z11));
  oai21  g102(.a(new_n148_), .b(new_n102_), .c(x31), .O(new_n154_));
  nor2   g103(.a(x31), .b(x30), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n137_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n120_), .c(new_n94_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n154_), .c(new_n58_), .O(new_n159_));
  nor2   g108(.a(x16), .b(x03), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n159_), .c(new_n99_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n71_), .O(z12));
  nand2  g111(.a(new_n158_), .b(x32), .O(new_n163_));
  nor2   g112(.a(x32), .b(x31), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n147_), .c(new_n127_), .d(new_n126_), .O(new_n165_));
  or2    g114(.a(new_n165_), .b(new_n105_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  nor2   g117(.a(x16), .b(x02), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n67_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n168_), .O(z13));
  oai21  g120(.a(new_n165_), .b(new_n105_), .c(x33), .O(new_n172_));
  nand4  g121(.a(new_n144_), .b(new_n92_), .c(new_n91_), .d(new_n72_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  inv1   g123(.a(x29), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n145_), .c(new_n135_), .d(new_n126_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  inv1   g126(.a(x30), .O(new_n178_));
  inv1   g127(.a(x31), .O(new_n179_));
  inv1   g128(.a(x32), .O(new_n180_));
  inv1   g129(.a(x33), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n180_), .c(new_n179_), .d(new_n178_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n177_), .c(new_n174_), .d(new_n65_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n172_), .c(new_n58_), .O(new_n185_));
  nor2   g134(.a(x16), .b(x01), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n185_), .c(new_n99_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n71_), .O(z14));
  nor2   g137(.a(x33), .b(x32), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n155_), .c(new_n137_), .d(new_n146_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n105_), .c(x34), .O(new_n191_));
  nand4  g140(.a(new_n126_), .b(new_n144_), .c(new_n92_), .d(new_n91_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand4  g142(.a(new_n178_), .b(new_n175_), .c(new_n145_), .d(new_n135_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  inv1   g144(.a(x34), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n181_), .c(new_n180_), .d(new_n179_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n195_), .c(new_n193_), .d(new_n119_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n191_), .c(new_n58_), .O(new_n200_));
  nor2   g149(.a(x16), .b(x00), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n200_), .c(new_n99_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n71_), .O(z15));
endmodule


