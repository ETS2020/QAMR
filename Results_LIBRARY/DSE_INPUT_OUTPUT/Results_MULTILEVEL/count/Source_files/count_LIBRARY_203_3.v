// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_;
  nand2  g000(.a(x33), .b(x29), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nand3  g013(.a(new_n63_), .b(new_n55_), .c(new_n54_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n64_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n60_), .b(new_n68_), .c(x18), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n67_), .c(new_n53_), .O(z01));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n57_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n65_), .b(x21), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  inv1   g024(.a(x18), .O(new_n76_));
  nand2  g025(.a(new_n52_), .b(new_n76_), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g027(.a(new_n74_), .b(new_n60_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(x21), .O(new_n81_));
  nand4  g030(.a(new_n57_), .b(new_n80_), .c(new_n81_), .d(new_n63_), .O(new_n82_));
  oai21  g031(.a(new_n73_), .b(new_n80_), .c(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n84_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n80_), .c(new_n81_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n65_), .O(new_n90_));
  aoi21  g039(.a(new_n82_), .b(x23), .c(new_n90_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x11), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n60_), .c(new_n93_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  nor2   g044(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand4  g045(.a(new_n95_), .b(new_n88_), .c(new_n80_), .d(new_n81_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n65_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n96_), .c(x16), .O(new_n99_));
  inv1   g048(.a(x10), .O(new_n100_));
  aoi21  g049(.a(new_n60_), .b(new_n100_), .c(x18), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n99_), .c(new_n53_), .O(z05));
  inv1   g051(.a(x25), .O(new_n103_));
  nor2   g052(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nor2   g053(.a(x23), .b(x22), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n72_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n104_), .c(x16), .O(new_n109_));
  inv1   g058(.a(x09), .O(new_n110_));
  aoi21  g059(.a(new_n60_), .b(new_n110_), .c(x18), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n109_), .c(new_n53_), .O(z06));
  inv1   g061(.a(x08), .O(new_n113_));
  oai21  g062(.a(x18), .b(new_n113_), .c(new_n60_), .O(new_n114_));
  nand4  g063(.a(new_n71_), .b(new_n55_), .c(new_n54_), .d(x16), .O(new_n115_));
  nor3   g064(.a(x26), .b(x25), .c(x24), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n105_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n115_), .c(new_n76_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n52_), .O(new_n119_));
  oai21  g068(.a(x26), .b(x25), .c(x18), .O(new_n120_));
  inv1   g069(.a(new_n107_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n73_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(x26), .c(x16), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n120_), .c(new_n52_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n119_), .c(new_n114_), .O(z07));
  nand3  g075(.a(new_n116_), .b(new_n105_), .c(new_n73_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n95_), .c(new_n88_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n82_), .O(new_n130_));
  aoi21  g079(.a(new_n127_), .b(x27), .c(new_n130_), .O(new_n131_));
  nor2   g080(.a(x16), .b(x07), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n77_), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n60_), .c(new_n133_), .O(z08));
  inv1   g083(.a(x28), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  nor2   g085(.a(x28), .b(x27), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n106_), .c(new_n136_), .d(new_n88_), .O(new_n138_));
  or2    g087(.a(new_n138_), .b(new_n82_), .O(new_n139_));
  oai21  g088(.a(new_n130_), .b(new_n135_), .c(new_n139_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  nor2   g090(.a(x16), .b(x06), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n77_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z09));
  nor2   g093(.a(x20), .b(x19), .O(new_n145_));
  nand4  g094(.a(new_n105_), .b(new_n145_), .c(new_n81_), .d(new_n54_), .O(new_n146_));
  inv1   g095(.a(x27), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n116_), .c(new_n147_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  aoi21  g099(.a(new_n139_), .b(x29), .c(new_n150_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n77_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n60_), .c(new_n153_), .O(z10));
  nor2   g103(.a(new_n53_), .b(x04), .O(new_n155_));
  nor2   g104(.a(x33), .b(new_n76_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n155_), .c(new_n60_), .O(new_n157_));
  oai21  g106(.a(x33), .b(x30), .c(x29), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x18), .O(new_n159_));
  oai21  g108(.a(new_n138_), .b(new_n82_), .c(x30), .O(new_n160_));
  nor3   g109(.a(x30), .b(x28), .c(x27), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n116_), .c(new_n90_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n160_), .c(x29), .O(new_n163_));
  inv1   g112(.a(x33), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(x30), .c(x29), .O(new_n165_));
  inv1   g114(.a(new_n165_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n159_), .c(new_n157_), .O(z11));
  nor2   g117(.a(new_n53_), .b(x03), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n156_), .c(new_n60_), .O(new_n170_));
  oai21  g119(.a(x33), .b(x31), .c(x29), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x18), .O(new_n172_));
  nor2   g121(.a(x26), .b(x25), .O(new_n173_));
  nor2   g122(.a(x30), .b(x28), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n173_), .c(new_n147_), .d(new_n95_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n146_), .c(x31), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n137_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n116_), .c(new_n90_), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n176_), .c(x29), .O(new_n181_));
  nand3  g130(.a(new_n164_), .b(x31), .c(x29), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n181_), .c(x16), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n172_), .c(new_n170_), .O(z12));
  nor2   g134(.a(new_n53_), .b(x02), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n156_), .c(new_n60_), .O(new_n187_));
  inv1   g136(.a(x29), .O(new_n188_));
  nor2   g137(.a(x33), .b(x32), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n188_), .c(x18), .O(new_n190_));
  nand4  g139(.a(new_n177_), .b(new_n137_), .c(new_n173_), .d(new_n95_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n146_), .c(x32), .O(new_n192_));
  inv1   g141(.a(new_n97_), .O(new_n193_));
  inv1   g142(.a(x30), .O(new_n194_));
  inv1   g143(.a(x31), .O(new_n195_));
  inv1   g144(.a(x32), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n195_), .c(new_n194_), .d(new_n135_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n128_), .c(new_n193_), .d(new_n66_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n192_), .c(x29), .O(new_n200_));
  nand3  g149(.a(new_n164_), .b(x32), .c(x29), .O(new_n201_));
  inv1   g150(.a(new_n201_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n200_), .c(x16), .O(new_n203_));
  nand3  g152(.a(new_n203_), .b(new_n190_), .c(new_n187_), .O(z13));
  nor2   g153(.a(x16), .b(x01), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(x18), .c(new_n52_), .O(new_n206_));
  nand2  g155(.a(new_n199_), .b(x33), .O(new_n207_));
  nand2  g156(.a(new_n189_), .b(new_n177_), .O(new_n208_));
  inv1   g157(.a(new_n208_), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n137_), .c(new_n173_), .d(new_n98_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n207_), .O(new_n211_));
  nand3  g160(.a(new_n211_), .b(new_n188_), .c(x16), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n206_), .O(z14));
  nand2  g162(.a(x34), .b(x16), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n164_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x29), .O(new_n216_));
  nand2  g165(.a(new_n193_), .b(new_n66_), .O(new_n217_));
  nand4  g166(.a(new_n189_), .b(new_n177_), .c(new_n137_), .d(new_n173_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n217_), .c(x34), .O(new_n219_));
  nand3  g168(.a(new_n148_), .b(new_n147_), .c(new_n136_), .O(new_n220_));
  inv1   g169(.a(new_n220_), .O(new_n221_));
  nor3   g170(.a(x34), .b(x33), .c(x32), .O(new_n222_));
  nand4  g171(.a(new_n222_), .b(new_n221_), .c(new_n177_), .d(new_n108_), .O(new_n223_));
  nand2  g172(.a(new_n223_), .b(new_n219_), .O(new_n224_));
  nand2  g173(.a(new_n224_), .b(x16), .O(new_n225_));
  inv1   g174(.a(x00), .O(new_n226_));
  aoi21  g175(.a(new_n60_), .b(new_n226_), .c(x18), .O(new_n227_));
  nand3  g176(.a(new_n227_), .b(new_n225_), .c(new_n216_), .O(z15));
endmodule


