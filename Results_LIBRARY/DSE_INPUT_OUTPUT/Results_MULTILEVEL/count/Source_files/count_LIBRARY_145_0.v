// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x16), .O(new_n59_));
  inv1   g008(.a(new_n56_), .O(new_n60_));
  nor2   g009(.a(x20), .b(x19), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  aoi21  g012(.a(new_n60_), .b(x20), .c(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n59_), .c(new_n52_), .O(z01));
  inv1   g014(.a(x20), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n66_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  aoi21  g018(.a(new_n62_), .b(x21), .c(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n59_), .c(new_n52_), .O(z02));
  nand2  g020(.a(new_n68_), .b(x22), .O(new_n72_));
  nor3   g021(.a(x22), .b(x21), .c(x20), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n56_), .c(x16), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n72_), .c(new_n52_), .d(x16), .O(z03));
  inv1   g024(.a(new_n73_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n60_), .c(x23), .O(new_n77_));
  nor2   g026(.a(x23), .b(x22), .O(new_n78_));
  nand4  g027(.a(new_n78_), .b(new_n61_), .c(new_n67_), .d(new_n53_), .O(new_n79_));
  nand4  g028(.a(new_n79_), .b(new_n77_), .c(new_n52_), .d(x16), .O(z04));
  nand2  g029(.a(new_n79_), .b(x24), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nor2   g031(.a(x24), .b(x23), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n62_), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(x18), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n81_), .c(x16), .O(z05));
  inv1   g036(.a(x25), .O(new_n88_));
  nor2   g037(.a(x21), .b(x20), .O(new_n89_));
  nor2   g038(.a(x25), .b(x24), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n78_), .c(new_n89_), .d(new_n56_), .O(new_n91_));
  oai21  g040(.a(new_n85_), .b(new_n88_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n52_), .O(z06));
  nand2  g043(.a(new_n91_), .b(x26), .O(new_n95_));
  inv1   g044(.a(x24), .O(new_n96_));
  nor2   g045(.a(x26), .b(x25), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n78_), .c(new_n69_), .d(new_n96_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x16), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n52_), .O(z07));
  nand2  g050(.a(new_n98_), .b(x27), .O(new_n102_));
  nor3   g051(.a(x27), .b(x26), .c(x25), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n83_), .c(new_n73_), .d(new_n56_), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n102_), .c(new_n52_), .d(x16), .O(z08));
  nand2  g054(.a(new_n104_), .b(x28), .O(new_n106_));
  nor3   g055(.a(x25), .b(x24), .c(x23), .O(new_n107_));
  nor3   g056(.a(x28), .b(x27), .c(x26), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n107_), .c(new_n73_), .d(new_n56_), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n106_), .c(new_n52_), .d(x16), .O(z09));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(x29), .b(x28), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n97_), .c(new_n111_), .d(new_n96_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n79_), .O(new_n114_));
  aoi21  g063(.a(new_n109_), .b(x29), .c(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n59_), .c(new_n52_), .O(z10));
  oai21  g065(.a(new_n113_), .b(new_n79_), .c(x30), .O(new_n117_));
  nor2   g066(.a(x28), .b(x27), .O(new_n118_));
  nor2   g067(.a(x30), .b(x29), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n118_), .c(new_n97_), .d(new_n96_), .O(new_n120_));
  or2    g069(.a(new_n120_), .b(new_n79_), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n117_), .c(new_n52_), .d(x16), .O(z11));
  oai21  g071(.a(new_n120_), .b(new_n79_), .c(x31), .O(new_n123_));
  inv1   g072(.a(new_n84_), .O(new_n124_));
  nor2   g073(.a(x31), .b(x30), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n112_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n103_), .c(new_n124_), .d(new_n63_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n123_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n52_), .O(z12));
  nand2  g080(.a(new_n128_), .b(x32), .O(new_n132_));
  inv1   g081(.a(x31), .O(new_n133_));
  inv1   g082(.a(x32), .O(new_n134_));
  nand3  g083(.a(new_n119_), .b(new_n134_), .c(new_n133_), .O(new_n135_));
  inv1   g084(.a(new_n135_), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n118_), .c(new_n97_), .d(new_n85_), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n132_), .c(new_n52_), .d(x16), .O(z13));
  nand2  g087(.a(new_n137_), .b(x33), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nor2   g089(.a(x33), .b(x32), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n125_), .c(new_n112_), .d(new_n140_), .O(new_n142_));
  or2    g091(.a(new_n142_), .b(new_n91_), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n139_), .c(new_n52_), .d(x16), .O(z14));
  oai21  g093(.a(new_n142_), .b(new_n91_), .c(x34), .O(new_n145_));
  inv1   g094(.a(x22), .O(new_n146_));
  inv1   g095(.a(x23), .O(new_n147_));
  nand4  g096(.a(new_n88_), .b(new_n96_), .c(new_n147_), .d(new_n146_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n68_), .O(new_n149_));
  nand2  g098(.a(new_n112_), .b(new_n140_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  inv1   g100(.a(x30), .O(new_n152_));
  nand2  g101(.a(new_n133_), .b(new_n152_), .O(new_n153_));
  inv1   g102(.a(x33), .O(new_n154_));
  inv1   g103(.a(x34), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n154_), .c(new_n134_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n151_), .c(new_n149_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n145_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n52_), .O(z15));
endmodule


