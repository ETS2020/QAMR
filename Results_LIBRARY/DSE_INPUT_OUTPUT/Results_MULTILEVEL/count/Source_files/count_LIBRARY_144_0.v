// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x16), .O(new_n59_));
  inv1   g008(.a(new_n56_), .O(new_n60_));
  nor2   g009(.a(x20), .b(x19), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  aoi21  g012(.a(new_n60_), .b(x20), .c(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n59_), .c(new_n52_), .O(z01));
  inv1   g014(.a(x20), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n66_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  aoi21  g018(.a(new_n62_), .b(x21), .c(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n59_), .c(new_n52_), .O(z02));
  nand2  g020(.a(new_n68_), .b(x22), .O(new_n72_));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  nand4  g022(.a(new_n73_), .b(new_n56_), .c(new_n66_), .d(x16), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n72_), .c(new_n52_), .d(x16), .O(z03));
  nand3  g024(.a(new_n73_), .b(new_n56_), .c(new_n66_), .O(new_n76_));
  nor2   g025(.a(x23), .b(x22), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(x21), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n63_), .c(new_n76_), .d(x23), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n59_), .c(new_n52_), .O(z04));
  nand2  g030(.a(new_n79_), .b(new_n63_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x24), .O(new_n83_));
  nor2   g032(.a(x24), .b(x23), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n73_), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n61_), .c(new_n53_), .d(x16), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n83_), .c(new_n52_), .d(x16), .O(z05));
  nand4  g037(.a(new_n84_), .b(new_n73_), .c(new_n61_), .d(new_n53_), .O(new_n89_));
  inv1   g038(.a(x22), .O(new_n90_));
  inv1   g039(.a(x23), .O(new_n91_));
  inv1   g040(.a(x24), .O(new_n92_));
  inv1   g041(.a(x25), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n68_), .O(new_n95_));
  aoi21  g044(.a(new_n89_), .b(x25), .c(new_n95_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n59_), .c(new_n52_), .O(z06));
  nor2   g046(.a(x21), .b(x20), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n77_), .c(new_n98_), .d(new_n56_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x26), .O(new_n101_));
  nor3   g050(.a(x26), .b(x25), .c(x24), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n77_), .c(new_n69_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x16), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n52_), .O(z07));
  nor2   g055(.a(x27), .b(x26), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n84_), .c(new_n93_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n76_), .O(new_n109_));
  aoi21  g058(.a(new_n103_), .b(x27), .c(new_n109_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n59_), .c(new_n52_), .O(z08));
  oai21  g060(.a(new_n108_), .b(new_n76_), .c(x28), .O(new_n112_));
  inv1   g061(.a(new_n76_), .O(new_n113_));
  nor3   g062(.a(x28), .b(x27), .c(x26), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n99_), .c(new_n113_), .d(new_n91_), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n112_), .c(new_n52_), .d(x16), .O(z09));
  nand2  g065(.a(new_n115_), .b(x29), .O(new_n117_));
  inv1   g066(.a(x27), .O(new_n118_));
  nor3   g067(.a(new_n78_), .b(new_n62_), .c(x21), .O(new_n119_));
  nor2   g068(.a(x29), .b(x28), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n102_), .c(new_n119_), .d(new_n118_), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n117_), .c(new_n52_), .d(x16), .O(z10));
  nand3  g071(.a(new_n120_), .b(new_n102_), .c(new_n118_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n82_), .c(x30), .O(new_n124_));
  nor2   g073(.a(x28), .b(x27), .O(new_n125_));
  nor2   g074(.a(x30), .b(x29), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n102_), .d(new_n119_), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n124_), .c(new_n52_), .d(x16), .O(z11));
  nand3  g077(.a(new_n126_), .b(new_n125_), .c(new_n102_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n82_), .c(x31), .O(new_n130_));
  inv1   g079(.a(new_n89_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nor2   g081(.a(x31), .b(x30), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n120_), .c(new_n132_), .d(new_n131_), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n130_), .c(new_n52_), .d(x16), .O(z12));
  nand2  g084(.a(new_n134_), .b(x32), .O(new_n136_));
  nor2   g085(.a(x26), .b(x25), .O(new_n137_));
  nor2   g086(.a(x32), .b(x31), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n126_), .c(new_n125_), .d(new_n137_), .O(new_n139_));
  or2    g088(.a(new_n139_), .b(new_n89_), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n136_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g090(.a(new_n139_), .b(new_n89_), .c(x33), .O(new_n142_));
  nor2   g091(.a(x33), .b(x32), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n133_), .c(new_n120_), .d(new_n107_), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n95_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n52_), .O(z14));
  oai21  g098(.a(new_n144_), .b(new_n100_), .c(x34), .O(new_n150_));
  nand2  g099(.a(new_n120_), .b(new_n107_), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x31), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  inv1   g104(.a(x32), .O(new_n156_));
  inv1   g105(.a(x33), .O(new_n157_));
  inv1   g106(.a(x34), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n157_), .c(new_n156_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n152_), .c(new_n95_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n150_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n52_), .O(z15));
endmodule


