// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x31), .b(x18), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand4  g020(.a(new_n71_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n66_), .b(x21), .c(new_n73_), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n61_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n61_), .c(new_n76_), .O(z02));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n53_), .c(new_n65_), .O(new_n79_));
  inv1   g028(.a(new_n79_), .O(new_n80_));
  aoi21  g029(.a(new_n72_), .b(x22), .c(new_n80_), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n61_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n61_), .c(new_n83_), .O(z03));
  nor2   g033(.a(x20), .b(x19), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n85_), .c(new_n71_), .d(new_n63_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n79_), .b(x23), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n61_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n61_), .c(new_n91_), .O(z04));
  nand2  g041(.a(new_n87_), .b(x24), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n78_), .c(new_n85_), .d(new_n63_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n61_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n60_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n58_), .O(z05));
  nand2  g048(.a(new_n95_), .b(x25), .O(new_n100_));
  inv1   g049(.a(x22), .O(new_n101_));
  inv1   g050(.a(x23), .O(new_n102_));
  inv1   g051(.a(x24), .O(new_n103_));
  inv1   g052(.a(x25), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n72_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n100_), .c(new_n61_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n108_), .c(new_n60_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n58_), .O(z06));
  nor2   g060(.a(x21), .b(x20), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n64_), .c(new_n60_), .d(new_n63_), .O(new_n113_));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n86_), .c(new_n103_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  aoi21  g065(.a(new_n107_), .b(x26), .c(new_n116_), .O(new_n117_));
  inv1   g066(.a(x08), .O(new_n118_));
  aoi21  g067(.a(new_n61_), .b(new_n118_), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n61_), .c(new_n119_), .O(z07));
  oai21  g069(.a(new_n115_), .b(new_n72_), .c(x27), .O(new_n121_));
  nor2   g070(.a(x27), .b(x26), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n94_), .c(new_n80_), .d(new_n104_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n121_), .c(new_n61_), .O(new_n124_));
  nor2   g073(.a(x16), .b(x07), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n124_), .c(new_n60_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n58_), .O(z08));
  nand3  g076(.a(new_n122_), .b(new_n94_), .c(new_n104_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n79_), .c(x28), .O(new_n129_));
  nor2   g078(.a(x25), .b(x24), .O(new_n130_));
  nor3   g079(.a(x28), .b(x27), .c(x26), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n130_), .c(new_n80_), .d(new_n102_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n129_), .c(new_n61_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x06), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n133_), .c(new_n60_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n58_), .O(z09));
  inv1   g085(.a(x27), .O(new_n137_));
  nor3   g086(.a(x26), .b(x25), .c(x24), .O(new_n138_));
  nor2   g087(.a(x29), .b(x28), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n138_), .c(new_n137_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n87_), .O(new_n141_));
  aoi21  g090(.a(new_n132_), .b(x29), .c(new_n141_), .O(new_n142_));
  inv1   g091(.a(x05), .O(new_n143_));
  aoi21  g092(.a(new_n61_), .b(new_n143_), .c(x18), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n61_), .c(new_n144_), .O(z10));
  nand4  g094(.a(new_n139_), .b(new_n138_), .c(new_n88_), .d(new_n137_), .O(new_n146_));
  nor2   g095(.a(x28), .b(x27), .O(new_n147_));
  nor2   g096(.a(x30), .b(x29), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n114_), .d(new_n103_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n87_), .O(new_n150_));
  aoi21  g099(.a(new_n146_), .b(x30), .c(new_n150_), .O(new_n151_));
  inv1   g100(.a(x04), .O(new_n152_));
  aoi21  g101(.a(new_n61_), .b(new_n152_), .c(x18), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n61_), .c(new_n153_), .O(z11));
  nand4  g103(.a(new_n86_), .b(new_n112_), .c(new_n53_), .d(x16), .O(new_n155_));
  nor2   g104(.a(x31), .b(x30), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n139_), .c(new_n122_), .d(new_n130_), .O(new_n157_));
  oai22  g106(.a(new_n157_), .b(new_n155_), .c(x16), .d(x03), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n60_), .O(new_n159_));
  oai21  g108(.a(new_n150_), .b(new_n61_), .c(new_n60_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x31), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z12));
  inv1   g111(.a(x32), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n61_), .c(new_n60_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x31), .O(new_n165_));
  oai21  g114(.a(new_n149_), .b(new_n87_), .c(x32), .O(new_n166_));
  inv1   g115(.a(new_n66_), .O(new_n167_));
  nand2  g116(.a(new_n94_), .b(new_n78_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand2  g118(.a(new_n147_), .b(new_n114_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  inv1   g120(.a(x31), .O(new_n172_));
  nand3  g121(.a(new_n148_), .b(new_n163_), .c(new_n172_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n171_), .c(new_n169_), .d(new_n167_), .O(new_n175_));
  aoi21  g124(.a(new_n175_), .b(new_n166_), .c(new_n61_), .O(new_n176_));
  nor2   g125(.a(x16), .b(x02), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n176_), .c(new_n60_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n165_), .O(z13));
  inv1   g128(.a(x33), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n61_), .c(new_n60_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x31), .O(new_n182_));
  nor2   g131(.a(x32), .b(x30), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n139_), .c(new_n122_), .d(new_n104_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n95_), .c(x33), .O(new_n185_));
  nand2  g134(.a(new_n139_), .b(new_n122_), .O(new_n186_));
  nor2   g135(.a(x33), .b(x32), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n156_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n186_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n106_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n185_), .c(new_n61_), .O(new_n191_));
  nor2   g140(.a(x16), .b(x01), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n191_), .c(new_n60_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n182_), .O(z14));
  inv1   g143(.a(x34), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n61_), .c(new_n60_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x31), .O(new_n197_));
  nand4  g146(.a(new_n187_), .b(new_n148_), .c(new_n147_), .d(new_n114_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n95_), .c(x34), .O(new_n199_));
  inv1   g148(.a(new_n186_), .O(new_n200_));
  inv1   g149(.a(x30), .O(new_n201_));
  nand2  g150(.a(new_n172_), .b(new_n201_), .O(new_n202_));
  nand3  g151(.a(new_n195_), .b(new_n180_), .c(new_n163_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(new_n200_), .c(new_n106_), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(new_n199_), .c(new_n61_), .O(new_n206_));
  nor2   g155(.a(x16), .b(x00), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(new_n206_), .c(new_n60_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n197_), .O(z15));
endmodule


