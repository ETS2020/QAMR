// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand3  g001(.a(x19), .b(new_n52_), .c(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(x16), .O(new_n57_));
  inv1   g006(.a(x17), .O(new_n58_));
  nor3   g007(.a(x18), .b(x16), .c(x15), .O(new_n59_));
  aoi21  g008(.a(x18), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  nor2   g010(.a(x20), .b(x19), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x16), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(x18), .c(new_n58_), .O(new_n65_));
  nand3  g014(.a(new_n55_), .b(x20), .c(x16), .O(new_n66_));
  oai21  g015(.a(x16), .b(x14), .c(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n65_), .O(z01));
  inv1   g018(.a(x16), .O(new_n70_));
  nor3   g019(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n70_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n70_), .c(new_n78_), .O(z02));
  nor3   g028(.a(x22), .b(x21), .c(x20), .O(new_n80_));
  aoi22  g029(.a(new_n80_), .b(new_n54_), .c(new_n74_), .d(x22), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n70_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n70_), .c(new_n83_), .O(z03));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n64_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n58_), .O(new_n88_));
  nand2  g037(.a(new_n80_), .b(new_n54_), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(x23), .c(x16), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x11), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n91_), .c(new_n52_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n88_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  aoi21  g044(.a(new_n85_), .b(new_n71_), .c(new_n95_), .O(new_n96_));
  nor2   g045(.a(x22), .b(x21), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n97_), .c(new_n62_), .d(new_n58_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n96_), .c(x16), .O(new_n101_));
  inv1   g050(.a(x10), .O(new_n102_));
  aoi21  g051(.a(new_n70_), .b(new_n102_), .c(x18), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n101_), .O(z05));
  nor2   g053(.a(x23), .b(x22), .O(new_n105_));
  nor2   g054(.a(x19), .b(new_n70_), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n73_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n52_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n58_), .O(new_n110_));
  nand3  g059(.a(new_n99_), .b(x25), .c(x16), .O(new_n111_));
  oai21  g060(.a(x16), .b(x09), .c(new_n111_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n110_), .O(z06));
  nand4  g063(.a(new_n107_), .b(new_n105_), .c(new_n73_), .d(new_n54_), .O(new_n115_));
  nor2   g064(.a(x26), .b(x25), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n105_), .c(new_n95_), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n74_), .O(new_n118_));
  aoi21  g067(.a(new_n115_), .b(x26), .c(new_n118_), .O(new_n119_));
  inv1   g068(.a(x08), .O(new_n120_));
  aoi21  g069(.a(new_n70_), .b(new_n120_), .c(x18), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n70_), .c(new_n121_), .O(z07));
  nand2  g071(.a(new_n106_), .b(new_n80_), .O(new_n123_));
  inv1   g072(.a(x25), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n98_), .c(new_n124_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n123_), .c(new_n52_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n58_), .O(new_n128_));
  oai21  g077(.a(new_n117_), .b(new_n74_), .c(x27), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  nand2  g079(.a(new_n70_), .b(new_n130_), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n70_), .c(new_n131_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n52_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n128_), .O(z08));
  oai21  g083(.a(new_n126_), .b(new_n89_), .c(x28), .O(new_n135_));
  nor3   g084(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nor3   g085(.a(x28), .b(x27), .c(x26), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n136_), .c(new_n80_), .d(new_n54_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  inv1   g089(.a(x06), .O(new_n141_));
  aoi21  g090(.a(new_n70_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z09));
  nor3   g092(.a(x26), .b(x25), .c(x24), .O(new_n144_));
  nor3   g093(.a(x29), .b(x28), .c(x27), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n144_), .c(new_n85_), .d(new_n71_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  aoi21  g096(.a(new_n138_), .b(x29), .c(new_n147_), .O(new_n148_));
  inv1   g097(.a(x05), .O(new_n149_));
  aoi21  g098(.a(new_n70_), .b(new_n149_), .c(x18), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n70_), .c(new_n150_), .O(z10));
  nor2   g100(.a(x28), .b(x27), .O(new_n152_));
  nor2   g101(.a(x30), .b(x29), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n152_), .c(new_n116_), .d(new_n95_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n86_), .c(new_n52_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n58_), .O(new_n156_));
  nand3  g105(.a(new_n146_), .b(x30), .c(x16), .O(new_n157_));
  inv1   g106(.a(x04), .O(new_n158_));
  nand2  g107(.a(new_n70_), .b(new_n158_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n52_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n156_), .O(z11));
  nand3  g111(.a(new_n98_), .b(new_n97_), .c(new_n64_), .O(new_n163_));
  nor2   g112(.a(x29), .b(x28), .O(new_n164_));
  nor2   g113(.a(x31), .b(x30), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n125_), .d(new_n124_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(new_n52_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n58_), .O(new_n168_));
  inv1   g117(.a(x21), .O(new_n169_));
  nand4  g118(.a(new_n105_), .b(new_n62_), .c(new_n169_), .d(new_n58_), .O(new_n170_));
  oai21  g119(.a(new_n154_), .b(new_n170_), .c(x31), .O(new_n171_));
  inv1   g120(.a(x03), .O(new_n172_));
  nand2  g121(.a(new_n70_), .b(new_n172_), .O(new_n173_));
  oai21  g122(.a(new_n171_), .b(new_n70_), .c(new_n173_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n52_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n168_), .O(z12));
  nor2   g125(.a(x32), .b(x31), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n153_), .c(new_n152_), .d(new_n116_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n163_), .c(new_n52_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n58_), .O(new_n180_));
  oai21  g129(.a(new_n166_), .b(new_n99_), .c(x32), .O(new_n181_));
  inv1   g130(.a(x02), .O(new_n182_));
  nand2  g131(.a(new_n70_), .b(new_n182_), .O(new_n183_));
  oai21  g132(.a(new_n181_), .b(new_n70_), .c(new_n183_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n180_), .O(z13));
  oai21  g135(.a(new_n178_), .b(new_n99_), .c(x33), .O(new_n187_));
  nor2   g136(.a(x33), .b(x32), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n165_), .c(new_n164_), .d(new_n125_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n115_), .c(new_n187_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(x16), .O(new_n191_));
  inv1   g140(.a(x01), .O(new_n192_));
  aoi21  g141(.a(new_n70_), .b(new_n192_), .c(x18), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n191_), .O(z14));
  nor3   g143(.a(x34), .b(x33), .c(x32), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n165_), .c(new_n164_), .d(new_n125_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n108_), .c(new_n52_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n58_), .O(new_n198_));
  oai21  g147(.a(new_n189_), .b(new_n115_), .c(x34), .O(new_n199_));
  inv1   g148(.a(x00), .O(new_n200_));
  nand2  g149(.a(new_n70_), .b(new_n200_), .O(new_n201_));
  oai21  g150(.a(new_n199_), .b(new_n70_), .c(new_n201_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n52_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n198_), .O(z15));
endmodule


