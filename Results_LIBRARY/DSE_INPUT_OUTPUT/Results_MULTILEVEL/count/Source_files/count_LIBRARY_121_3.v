// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  xnor2a g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x16), .b(x15), .O(new_n53_));
  aoi21  g002(.a(new_n52_), .b(x16), .c(new_n53_), .O(new_n54_));
  inv1   g003(.a(x26), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x18), .O(new_n56_));
  oai21  g005(.a(new_n54_), .b(x18), .c(new_n56_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nor2   g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  inv1   g009(.a(x17), .O(new_n61_));
  inv1   g010(.a(x18), .O(new_n62_));
  nor2   g011(.a(x20), .b(x19), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n60_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  inv1   g016(.a(x16), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n67_), .c(x18), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n66_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor3   g020(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n59_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x18), .c(new_n56_), .O(z02));
  nor3   g027(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  nor3   g028(.a(x22), .b(x21), .c(x20), .O(new_n80_));
  aoi22  g029(.a(new_n80_), .b(new_n79_), .c(new_n74_), .d(x22), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n68_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n68_), .c(new_n83_), .O(z03));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n59_), .c(new_n58_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x23), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n63_), .c(new_n71_), .d(new_n61_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n68_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n62_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n56_), .O(z04));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n85_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi22  g045(.a(new_n96_), .b(new_n65_), .c(new_n89_), .d(x24), .O(new_n97_));
  inv1   g046(.a(x10), .O(new_n98_));
  aoi21  g047(.a(new_n68_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n68_), .c(new_n99_), .O(z05));
  nand4  g049(.a(new_n94_), .b(new_n85_), .c(new_n63_), .d(new_n61_), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n88_), .c(new_n73_), .d(new_n59_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  aoi21  g053(.a(new_n101_), .b(x25), .c(new_n104_), .O(new_n105_));
  inv1   g054(.a(x09), .O(new_n106_));
  aoi21  g055(.a(new_n68_), .b(new_n106_), .c(x18), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n68_), .c(new_n107_), .O(z06));
  nand3  g057(.a(new_n63_), .b(new_n61_), .c(x16), .O(new_n109_));
  nor3   g058(.a(x25), .b(x24), .c(x23), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n85_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n109_), .c(new_n62_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n55_), .O(new_n113_));
  nand3  g062(.a(new_n103_), .b(x26), .c(x16), .O(new_n114_));
  oai21  g063(.a(x16), .b(x08), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n62_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n113_), .O(z07));
  inv1   g066(.a(x19), .O(new_n118_));
  nand4  g067(.a(new_n73_), .b(new_n118_), .c(new_n61_), .d(x16), .O(new_n119_));
  inv1   g068(.a(x24), .O(new_n120_));
  nor2   g069(.a(x27), .b(x25), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n88_), .c(new_n120_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n119_), .c(new_n62_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n55_), .O(new_n124_));
  nor2   g073(.a(x26), .b(x25), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n88_), .c(new_n120_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n74_), .c(x27), .O(new_n127_));
  inv1   g076(.a(x07), .O(new_n128_));
  nand2  g077(.a(new_n68_), .b(new_n128_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n68_), .c(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n62_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n124_), .O(z08));
  inv1   g081(.a(x22), .O(new_n133_));
  inv1   g082(.a(x25), .O(new_n134_));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n94_), .c(new_n134_), .d(new_n133_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n119_), .c(new_n62_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n55_), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n94_), .c(new_n134_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n86_), .c(x28), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  nand2  g091(.a(new_n68_), .b(new_n142_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n68_), .c(new_n143_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n62_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n138_), .O(z09));
  nor3   g095(.a(x28), .b(x27), .c(x26), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n110_), .c(new_n80_), .d(new_n59_), .O(new_n148_));
  nor3   g097(.a(x23), .b(x22), .c(x21), .O(new_n149_));
  nor3   g098(.a(x26), .b(x25), .c(x24), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n150_), .c(new_n149_), .d(new_n72_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  aoi21  g102(.a(new_n148_), .b(x29), .c(new_n153_), .O(new_n154_));
  inv1   g103(.a(x05), .O(new_n155_));
  aoi21  g104(.a(new_n68_), .b(new_n155_), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n68_), .c(new_n156_), .O(z10));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n135_), .c(new_n125_), .d(new_n120_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n89_), .O(new_n160_));
  aoi21  g109(.a(new_n152_), .b(x30), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  aoi21  g111(.a(new_n68_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n68_), .c(new_n163_), .O(z11));
  nand4  g113(.a(new_n88_), .b(new_n73_), .c(new_n59_), .d(x16), .O(new_n165_));
  nor2   g114(.a(x29), .b(x28), .O(new_n166_));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n166_), .c(new_n121_), .d(new_n120_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n165_), .c(new_n62_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n55_), .O(new_n170_));
  oai21  g119(.a(new_n159_), .b(new_n89_), .c(x31), .O(new_n171_));
  inv1   g120(.a(x03), .O(new_n172_));
  nand2  g121(.a(new_n68_), .b(new_n172_), .O(new_n173_));
  oai21  g122(.a(new_n171_), .b(new_n68_), .c(new_n173_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n62_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n170_), .O(z12));
  nand4  g125(.a(new_n167_), .b(new_n166_), .c(new_n139_), .d(new_n134_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n101_), .c(x32), .O(new_n178_));
  nor2   g127(.a(x32), .b(x31), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n158_), .c(new_n135_), .d(new_n125_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n101_), .c(new_n178_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x16), .O(new_n182_));
  inv1   g131(.a(x02), .O(new_n183_));
  aoi21  g132(.a(new_n68_), .b(new_n183_), .c(x18), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n182_), .O(z13));
  inv1   g134(.a(new_n109_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n96_), .O(new_n187_));
  nor2   g136(.a(x33), .b(x32), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n167_), .c(new_n166_), .d(new_n121_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n187_), .c(new_n62_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n55_), .O(new_n191_));
  oai21  g140(.a(new_n180_), .b(new_n101_), .c(x33), .O(new_n192_));
  inv1   g141(.a(x01), .O(new_n193_));
  nand2  g142(.a(new_n68_), .b(new_n193_), .O(new_n194_));
  oai21  g143(.a(new_n192_), .b(new_n68_), .c(new_n194_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n62_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n191_), .O(z14));
  nand4  g146(.a(new_n188_), .b(new_n167_), .c(new_n166_), .d(new_n139_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n103_), .c(x34), .O(new_n199_));
  nand2  g148(.a(new_n166_), .b(new_n139_), .O(new_n200_));
  inv1   g149(.a(new_n200_), .O(new_n201_));
  inv1   g150(.a(new_n167_), .O(new_n202_));
  inv1   g151(.a(x32), .O(new_n203_));
  nor2   g152(.a(x34), .b(x33), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n203_), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n202_), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n201_), .c(new_n104_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n199_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(x16), .O(new_n209_));
  inv1   g158(.a(x00), .O(new_n210_));
  aoi21  g159(.a(new_n68_), .b(new_n210_), .c(x18), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n209_), .O(z15));
endmodule


