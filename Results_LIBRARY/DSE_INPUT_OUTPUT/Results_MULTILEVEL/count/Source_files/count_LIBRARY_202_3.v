// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  xnor2a g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x16), .b(x15), .O(new_n53_));
  aoi21  g002(.a(new_n52_), .b(x16), .c(new_n53_), .O(new_n54_));
  inv1   g003(.a(x31), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x18), .O(new_n56_));
  oai21  g005(.a(new_n54_), .b(x18), .c(new_n56_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  inv1   g008(.a(x17), .O(new_n60_));
  inv1   g009(.a(x18), .O(new_n61_));
  nor2   g010(.a(x20), .b(x19), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  oai21  g012(.a(new_n59_), .b(new_n58_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  inv1   g015(.a(x16), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n66_), .c(x18), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n65_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor3   g019(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n59_), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n70_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x18), .c(new_n56_), .O(z02));
  nand2  g026(.a(new_n73_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n59_), .c(new_n58_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n67_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n61_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n56_), .O(z03));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  aoi22  g034(.a(new_n85_), .b(new_n71_), .c(new_n80_), .d(x23), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n67_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n67_), .c(new_n88_), .O(z04));
  nand2  g038(.a(new_n85_), .b(new_n71_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x24), .O(new_n91_));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n79_), .c(new_n62_), .d(new_n60_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n67_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n94_), .c(new_n61_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n56_), .O(z05));
  nand2  g046(.a(new_n93_), .b(x25), .O(new_n98_));
  nor2   g047(.a(x23), .b(x22), .O(new_n99_));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n72_), .d(new_n59_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n98_), .c(new_n67_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n102_), .c(new_n61_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n56_), .O(z06));
  nand2  g054(.a(new_n101_), .b(x26), .O(new_n106_));
  nor3   g055(.a(x26), .b(x25), .c(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n99_), .c(new_n72_), .d(new_n59_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n67_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n109_), .c(new_n61_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n56_), .O(z07));
  inv1   g061(.a(x25), .O(new_n113_));
  nor2   g062(.a(x27), .b(x26), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n92_), .c(new_n113_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n80_), .O(new_n116_));
  aoi21  g065(.a(new_n108_), .b(x27), .c(new_n116_), .O(new_n117_));
  inv1   g066(.a(x07), .O(new_n118_));
  aoi21  g067(.a(new_n67_), .b(new_n118_), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n67_), .c(new_n119_), .O(z08));
  inv1   g069(.a(x28), .O(new_n121_));
  inv1   g070(.a(x23), .O(new_n122_));
  inv1   g071(.a(x26), .O(new_n123_));
  nor2   g072(.a(x28), .b(x27), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n100_), .c(new_n123_), .d(new_n122_), .O(new_n125_));
  oai22  g074(.a(new_n125_), .b(new_n80_), .c(new_n116_), .d(new_n121_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g076(.a(x06), .O(new_n128_));
  aoi21  g077(.a(new_n67_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z09));
  oai21  g079(.a(new_n125_), .b(new_n80_), .c(x29), .O(new_n131_));
  nor3   g080(.a(x29), .b(x28), .c(x27), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n107_), .c(new_n85_), .d(new_n71_), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n131_), .c(new_n67_), .O(new_n134_));
  nor2   g083(.a(x16), .b(x05), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n134_), .c(new_n61_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n56_), .O(z10));
  inv1   g086(.a(x24), .O(new_n138_));
  nor2   g087(.a(x26), .b(x25), .O(new_n139_));
  nor2   g088(.a(x30), .b(x29), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n124_), .c(new_n139_), .d(new_n138_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n90_), .O(new_n142_));
  aoi21  g091(.a(new_n133_), .b(x30), .c(new_n142_), .O(new_n143_));
  inv1   g092(.a(x04), .O(new_n144_));
  aoi21  g093(.a(new_n67_), .b(new_n144_), .c(x18), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n67_), .c(new_n145_), .O(z11));
  nand4  g095(.a(new_n99_), .b(new_n72_), .c(new_n59_), .d(x16), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n141_), .c(new_n61_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n55_), .O(new_n149_));
  oai21  g098(.a(new_n141_), .b(new_n90_), .c(x31), .O(new_n150_));
  inv1   g099(.a(x03), .O(new_n151_));
  nand2  g100(.a(new_n67_), .b(new_n151_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n67_), .c(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n61_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n149_), .O(z12));
  nor2   g104(.a(x29), .b(x28), .O(new_n156_));
  nor2   g105(.a(x31), .b(x30), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n156_), .c(new_n114_), .d(new_n113_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n93_), .c(x32), .O(new_n159_));
  nor2   g108(.a(x32), .b(x31), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n140_), .c(new_n124_), .d(new_n139_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n93_), .c(new_n159_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  inv1   g112(.a(x02), .O(new_n164_));
  aoi21  g113(.a(new_n67_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z13));
  nor2   g115(.a(x17), .b(new_n67_), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n92_), .c(new_n79_), .d(new_n62_), .O(new_n168_));
  nand2  g117(.a(new_n124_), .b(new_n139_), .O(new_n169_));
  inv1   g118(.a(new_n169_), .O(new_n170_));
  nor2   g119(.a(x33), .b(x32), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n170_), .c(new_n140_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n168_), .c(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n55_), .O(new_n174_));
  oai21  g123(.a(new_n161_), .b(new_n93_), .c(x33), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n67_), .O(new_n176_));
  nor2   g125(.a(x16), .b(x01), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n176_), .c(new_n61_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n174_), .O(z14));
  nor3   g128(.a(x34), .b(x33), .c(x32), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n170_), .c(new_n140_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n168_), .c(new_n61_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n55_), .O(new_n183_));
  nand4  g132(.a(new_n171_), .b(new_n157_), .c(new_n156_), .d(new_n114_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n101_), .c(x34), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n67_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x00), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n186_), .c(new_n61_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n183_), .O(z15));
endmodule


