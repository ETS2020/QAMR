// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x28), .O(new_n60_));
  inv1   g009(.a(x31), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n56_), .O(z00));
  inv1   g014(.a(new_n62_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  nor2   g016(.a(new_n55_), .b(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n67_), .b(new_n53_), .c(new_n52_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  inv1   g020(.a(x14), .O(new_n72_));
  aoi21  g021(.a(new_n58_), .b(new_n72_), .c(x18), .O(new_n73_));
  aoi21  g022(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(z01));
  inv1   g023(.a(x21), .O(new_n75_));
  nor2   g024(.a(x21), .b(x20), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n55_), .O(new_n77_));
  oai21  g026(.a(new_n70_), .b(new_n75_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x16), .O(new_n79_));
  inv1   g028(.a(x13), .O(new_n80_));
  aoi21  g029(.a(new_n58_), .b(new_n80_), .c(x18), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(new_n66_), .O(z02));
  inv1   g031(.a(x12), .O(new_n83_));
  oai21  g032(.a(x18), .b(new_n83_), .c(new_n58_), .O(new_n84_));
  nand2  g033(.a(new_n55_), .b(x16), .O(new_n85_));
  nor2   g034(.a(x22), .b(x21), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n67_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n85_), .c(new_n59_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  inv1   g038(.a(x22), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x18), .O(new_n92_));
  inv1   g041(.a(new_n77_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  aoi21  g043(.a(new_n94_), .b(x16), .c(new_n66_), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n92_), .c(new_n89_), .d(new_n84_), .O(z03));
  nand3  g045(.a(new_n86_), .b(new_n55_), .c(new_n67_), .O(new_n97_));
  nor3   g046(.a(x23), .b(x22), .c(x21), .O(new_n98_));
  aoi22  g047(.a(new_n98_), .b(new_n70_), .c(new_n97_), .d(x23), .O(new_n99_));
  nor2   g048(.a(x16), .b(x11), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n63_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n58_), .c(new_n101_), .O(z04));
  inv1   g051(.a(x24), .O(new_n103_));
  aoi21  g052(.a(new_n98_), .b(new_n70_), .c(new_n103_), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n69_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n104_), .c(x16), .O(new_n108_));
  inv1   g057(.a(x10), .O(new_n109_));
  aoi21  g058(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n108_), .c(new_n66_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  nor2   g061(.a(x23), .b(x22), .O(new_n113_));
  nor2   g062(.a(x25), .b(x24), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n113_), .c(new_n76_), .d(new_n55_), .O(new_n115_));
  oai21  g064(.a(new_n107_), .b(new_n112_), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x16), .O(new_n117_));
  inv1   g066(.a(x09), .O(new_n118_));
  aoi21  g067(.a(new_n58_), .b(new_n118_), .c(x18), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n66_), .O(z06));
  nand2  g069(.a(new_n115_), .b(x26), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n113_), .c(new_n93_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  inv1   g074(.a(x08), .O(new_n126_));
  aoi21  g075(.a(new_n58_), .b(new_n126_), .c(x18), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n66_), .O(z07));
  inv1   g077(.a(x26), .O(new_n129_));
  inv1   g078(.a(x27), .O(new_n130_));
  nand4  g079(.a(new_n105_), .b(new_n130_), .c(new_n129_), .d(new_n112_), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n97_), .O(new_n132_));
  aoi21  g081(.a(new_n123_), .b(x27), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n63_), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n58_), .c(new_n135_), .O(z08));
  nand4  g085(.a(new_n76_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n137_));
  nor3   g086(.a(x27), .b(x26), .c(x25), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n105_), .c(new_n90_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n137_), .c(x31), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n60_), .O(new_n141_));
  nor2   g090(.a(new_n132_), .b(new_n60_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  inv1   g092(.a(x06), .O(new_n144_));
  aoi21  g093(.a(new_n58_), .b(new_n144_), .c(x18), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n141_), .O(z09));
  oai21  g095(.a(x16), .b(x05), .c(new_n59_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n62_), .O(new_n148_));
  inv1   g097(.a(x29), .O(new_n149_));
  oai21  g098(.a(new_n131_), .b(new_n97_), .c(x31), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n60_), .c(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n98_), .b(new_n70_), .O(new_n152_));
  nor2   g101(.a(new_n61_), .b(x29), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n122_), .c(new_n60_), .d(new_n130_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n151_), .c(x16), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n148_), .O(z10));
  oai21  g106(.a(x16), .b(x04), .c(new_n59_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n62_), .O(new_n159_));
  inv1   g108(.a(x30), .O(new_n160_));
  inv1   g109(.a(x23), .O(new_n161_));
  nor2   g110(.a(x29), .b(x27), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n114_), .c(new_n129_), .d(new_n161_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n97_), .c(x31), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n60_), .c(new_n160_), .O(new_n165_));
  nand3  g114(.a(new_n105_), .b(new_n86_), .c(new_n70_), .O(new_n166_));
  nor2   g115(.a(new_n61_), .b(x30), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n138_), .c(new_n149_), .d(new_n60_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n165_), .c(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n159_), .O(z11));
  nor3   g120(.a(x30), .b(x29), .c(x27), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n122_), .c(new_n98_), .d(new_n70_), .O(new_n173_));
  nor2   g122(.a(new_n61_), .b(new_n60_), .O(new_n174_));
  aoi21  g123(.a(new_n173_), .b(new_n60_), .c(new_n174_), .O(new_n175_));
  nor2   g124(.a(x16), .b(x03), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n63_), .O(new_n177_));
  oai21  g126(.a(new_n175_), .b(new_n58_), .c(new_n177_), .O(z12));
  inv1   g127(.a(x02), .O(new_n179_));
  nand2  g128(.a(new_n58_), .b(new_n179_), .O(new_n180_));
  aoi21  g129(.a(x32), .b(x16), .c(x18), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n180_), .c(new_n66_), .O(z13));
  inv1   g131(.a(x01), .O(new_n183_));
  nand2  g132(.a(new_n58_), .b(new_n183_), .O(new_n184_));
  aoi21  g133(.a(x33), .b(x16), .c(x18), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n184_), .c(new_n66_), .O(z14));
  aoi21  g135(.a(x34), .b(x16), .c(x18), .O(new_n187_));
  inv1   g136(.a(x00), .O(new_n188_));
  nand2  g137(.a(new_n58_), .b(new_n188_), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n187_), .c(new_n62_), .O(z15));
endmodule


