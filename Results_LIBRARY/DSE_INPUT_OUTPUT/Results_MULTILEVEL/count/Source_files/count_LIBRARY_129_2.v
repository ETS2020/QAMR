// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x26), .O(new_n60_));
  nor2   g009(.a(x34), .b(new_n60_), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n56_), .O(z00));
  inv1   g014(.a(x20), .O(new_n66_));
  nor2   g015(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  nor3   g016(.a(x20), .b(x19), .c(x17), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  nor2   g018(.a(x16), .b(x14), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(z01));
  nand3  g021(.a(new_n66_), .b(new_n53_), .c(new_n52_), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n55_), .c(new_n73_), .d(x21), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n63_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z02));
  nand2  g027(.a(new_n74_), .b(new_n55_), .O(new_n79_));
  nor3   g028(.a(x22), .b(x21), .c(x20), .O(new_n80_));
  aoi22  g029(.a(new_n80_), .b(new_n55_), .c(new_n79_), .d(x22), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n58_), .c(new_n83_), .O(z03));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n55_), .c(new_n66_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x23), .O(new_n87_));
  inv1   g036(.a(x21), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n68_), .c(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x16), .O(new_n92_));
  inv1   g041(.a(x11), .O(new_n93_));
  aoi21  g042(.a(new_n58_), .b(new_n93_), .c(x18), .O(new_n94_));
  aoi21  g043(.a(new_n94_), .b(new_n92_), .c(new_n61_), .O(z04));
  inv1   g044(.a(x10), .O(new_n96_));
  oai21  g045(.a(x18), .b(new_n96_), .c(new_n58_), .O(new_n97_));
  nand4  g046(.a(new_n66_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n85_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n98_), .c(new_n59_), .O(new_n101_));
  oai21  g050(.a(x24), .b(x23), .c(x18), .O(new_n102_));
  nand3  g051(.a(new_n90_), .b(x24), .c(x16), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n102_), .c(new_n62_), .O(new_n104_));
  aoi21  g053(.a(new_n101_), .b(new_n62_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n97_), .O(z05));
  nand3  g055(.a(new_n99_), .b(new_n85_), .c(new_n68_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n89_), .c(new_n74_), .d(new_n55_), .O(new_n109_));
  inv1   g058(.a(new_n109_), .O(new_n110_));
  aoi21  g059(.a(new_n107_), .b(x25), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x09), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n63_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n58_), .c(new_n113_), .O(z06));
  nor3   g063(.a(x26), .b(x25), .c(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n89_), .O(new_n116_));
  oai22  g065(.a(new_n116_), .b(new_n79_), .c(new_n110_), .d(new_n60_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  nor2   g067(.a(x16), .b(x08), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n63_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z07));
  inv1   g070(.a(x27), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n58_), .c(x34), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(x26), .O(new_n124_));
  nor3   g073(.a(x27), .b(x26), .c(x25), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n99_), .O(new_n126_));
  oai22  g075(.a(new_n126_), .b(new_n86_), .c(new_n110_), .d(new_n122_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  inv1   g077(.a(x07), .O(new_n129_));
  aoi21  g078(.a(new_n58_), .b(new_n129_), .c(x18), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n128_), .c(new_n124_), .O(z08));
  inv1   g080(.a(x28), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n58_), .c(x34), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x26), .O(new_n134_));
  nor3   g083(.a(x27), .b(x25), .c(x24), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n79_), .c(x28), .O(new_n137_));
  nor3   g086(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nor3   g087(.a(x28), .b(x27), .c(x26), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n138_), .c(new_n80_), .d(new_n55_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  aoi21  g092(.a(new_n58_), .b(new_n143_), .c(x18), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n134_), .O(z09));
  nor2   g094(.a(new_n61_), .b(x05), .O(new_n146_));
  inv1   g095(.a(x34), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n59_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(new_n58_), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(x29), .c(x26), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x18), .O(new_n151_));
  inv1   g100(.a(x25), .O(new_n152_));
  nor2   g101(.a(x28), .b(x27), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n99_), .c(new_n152_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n86_), .c(x29), .O(new_n155_));
  nor3   g104(.a(x29), .b(x28), .c(x27), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n138_), .c(new_n80_), .d(new_n55_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n155_), .c(x26), .O(new_n158_));
  nand3  g107(.a(x34), .b(x29), .c(x26), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n158_), .c(x16), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n151_), .c(new_n149_), .O(z10));
  nand2  g111(.a(x30), .b(x16), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x34), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x26), .O(new_n165_));
  nand2  g114(.a(new_n157_), .b(x30), .O(new_n166_));
  nand2  g115(.a(new_n89_), .b(new_n88_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n73_), .O(new_n168_));
  nor2   g117(.a(x30), .b(x29), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n153_), .c(new_n115_), .d(new_n168_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n166_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  inv1   g121(.a(x04), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n172_), .c(new_n165_), .O(z11));
  inv1   g124(.a(x31), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n58_), .c(x34), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(x26), .O(new_n178_));
  inv1   g127(.a(x24), .O(new_n179_));
  nand3  g128(.a(new_n122_), .b(new_n152_), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n169_), .b(new_n132_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n168_), .c(new_n176_), .O(new_n183_));
  nor2   g132(.a(x29), .b(x28), .O(new_n184_));
  nor2   g133(.a(x31), .b(x30), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n184_), .c(new_n125_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n107_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n183_), .c(x16), .O(new_n188_));
  inv1   g137(.a(x03), .O(new_n189_));
  aoi21  g138(.a(new_n58_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand3  g139(.a(new_n190_), .b(new_n188_), .c(new_n178_), .O(z12));
  nand2  g140(.a(x32), .b(x16), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x34), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(x26), .O(new_n194_));
  nand3  g143(.a(new_n185_), .b(new_n184_), .c(new_n135_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n90_), .c(x32), .O(new_n196_));
  nor2   g145(.a(new_n100_), .b(new_n73_), .O(new_n197_));
  nor2   g146(.a(x26), .b(x25), .O(new_n198_));
  nor2   g147(.a(x32), .b(x31), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n169_), .O(new_n200_));
  inv1   g149(.a(new_n200_), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n153_), .c(new_n198_), .d(new_n197_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n196_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(x16), .O(new_n204_));
  inv1   g153(.a(x02), .O(new_n205_));
  aoi21  g154(.a(new_n58_), .b(new_n205_), .c(x18), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n204_), .c(new_n194_), .O(z13));
  nand2  g156(.a(x33), .b(x16), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(x34), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x26), .O(new_n210_));
  nand4  g159(.a(new_n199_), .b(new_n169_), .c(new_n153_), .d(new_n152_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n107_), .c(x33), .O(new_n212_));
  nor2   g161(.a(x27), .b(x26), .O(new_n213_));
  nor2   g162(.a(x33), .b(x32), .O(new_n214_));
  nand4  g163(.a(new_n214_), .b(new_n185_), .c(new_n184_), .d(new_n213_), .O(new_n215_));
  or2    g164(.a(new_n215_), .b(new_n109_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n212_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(x16), .O(new_n218_));
  inv1   g167(.a(x01), .O(new_n219_));
  aoi21  g168(.a(new_n58_), .b(new_n219_), .c(x18), .O(new_n220_));
  nand3  g169(.a(new_n220_), .b(new_n218_), .c(new_n210_), .O(z14));
  oai21  g170(.a(x16), .b(x00), .c(new_n59_), .O(new_n222_));
  nand2  g171(.a(new_n222_), .b(new_n62_), .O(new_n223_));
  oai21  g172(.a(new_n215_), .b(new_n109_), .c(x34), .O(new_n224_));
  nand2  g173(.a(new_n184_), .b(new_n213_), .O(new_n225_));
  inv1   g174(.a(new_n225_), .O(new_n226_));
  nor3   g175(.a(x34), .b(x33), .c(x32), .O(new_n227_));
  nand3  g176(.a(new_n227_), .b(new_n226_), .c(new_n185_), .O(new_n228_));
  oai21  g177(.a(new_n228_), .b(new_n109_), .c(new_n224_), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(x16), .O(new_n230_));
  nand2  g179(.a(new_n230_), .b(new_n223_), .O(z15));
endmodule


