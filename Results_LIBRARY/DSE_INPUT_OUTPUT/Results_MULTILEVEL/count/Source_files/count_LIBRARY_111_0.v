// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand4  g004(.a(new_n55_), .b(new_n54_), .c(new_n52_), .d(x16), .O(z00));
  nand2  g005(.a(new_n54_), .b(x20), .O(new_n57_));
  inv1   g006(.a(x17), .O(new_n58_));
  nor2   g007(.a(x20), .b(x19), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand4  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .d(x16), .O(z01));
  inv1   g010(.a(x16), .O(new_n62_));
  inv1   g011(.a(x19), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  inv1   g013(.a(x21), .O(new_n65_));
  nand4  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n58_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  aoi21  g016(.a(new_n60_), .b(x21), .c(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n62_), .c(new_n52_), .O(z02));
  nand2  g018(.a(new_n66_), .b(x22), .O(new_n70_));
  nor3   g019(.a(x22), .b(x21), .c(x20), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n53_), .c(x16), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n52_), .d(x16), .O(z03));
  inv1   g022(.a(new_n71_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n54_), .c(x23), .O(new_n75_));
  nor2   g024(.a(x23), .b(x22), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n59_), .c(new_n65_), .d(new_n58_), .O(new_n77_));
  nand4  g026(.a(new_n77_), .b(new_n75_), .c(new_n52_), .d(x16), .O(z04));
  nand2  g027(.a(new_n77_), .b(x24), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nor2   g029(.a(x24), .b(x23), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n80_), .c(new_n59_), .d(new_n58_), .O(new_n82_));
  nand4  g031(.a(new_n82_), .b(new_n79_), .c(new_n52_), .d(x16), .O(z05));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(x23), .O(new_n85_));
  inv1   g034(.a(x24), .O(new_n86_));
  inv1   g035(.a(x25), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n66_), .O(new_n89_));
  aoi21  g038(.a(new_n82_), .b(x25), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n62_), .c(new_n52_), .O(z06));
  nor2   g040(.a(x21), .b(x20), .O(new_n92_));
  nor2   g041(.a(x25), .b(x24), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n76_), .c(new_n92_), .d(new_n53_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x26), .O(new_n95_));
  nor3   g044(.a(x26), .b(x25), .c(x24), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n76_), .c(new_n67_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x16), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n52_), .O(z07));
  nand2  g049(.a(new_n97_), .b(x27), .O(new_n101_));
  nor3   g050(.a(x27), .b(x26), .c(x25), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n81_), .c(new_n71_), .d(new_n53_), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n101_), .c(new_n52_), .d(x16), .O(z08));
  nand2  g053(.a(new_n103_), .b(x28), .O(new_n105_));
  nor3   g054(.a(x25), .b(x24), .c(x23), .O(new_n106_));
  nor3   g055(.a(x28), .b(x27), .c(x26), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n106_), .c(new_n71_), .d(new_n53_), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n105_), .c(new_n52_), .d(x16), .O(z09));
  inv1   g058(.a(x27), .O(new_n110_));
  nor2   g059(.a(x26), .b(x25), .O(new_n111_));
  nor2   g060(.a(x29), .b(x28), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(new_n86_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n77_), .O(new_n114_));
  aoi21  g063(.a(new_n108_), .b(x29), .c(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n62_), .c(new_n52_), .O(z10));
  nor3   g065(.a(x20), .b(x19), .c(x17), .O(new_n117_));
  nor3   g066(.a(x23), .b(x22), .c(x21), .O(new_n118_));
  nor3   g067(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n96_), .c(new_n118_), .d(new_n117_), .O(new_n120_));
  nor2   g069(.a(x28), .b(x27), .O(new_n121_));
  nor2   g070(.a(x30), .b(x29), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n121_), .c(new_n111_), .d(new_n86_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n77_), .O(new_n124_));
  aoi21  g073(.a(new_n120_), .b(x30), .c(new_n124_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n62_), .c(new_n52_), .O(z11));
  oai21  g075(.a(new_n123_), .b(new_n77_), .c(x31), .O(new_n127_));
  inv1   g076(.a(new_n82_), .O(new_n128_));
  nor2   g077(.a(x31), .b(x30), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n112_), .c(new_n102_), .d(new_n128_), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n127_), .c(new_n52_), .d(x16), .O(z12));
  nand2  g080(.a(new_n130_), .b(x32), .O(new_n132_));
  nor2   g081(.a(x32), .b(x31), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n122_), .c(new_n121_), .d(new_n111_), .O(new_n134_));
  or2    g083(.a(new_n134_), .b(new_n82_), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n132_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g085(.a(new_n134_), .b(new_n82_), .c(x33), .O(new_n137_));
  nor2   g086(.a(x27), .b(x26), .O(new_n138_));
  nor2   g087(.a(x33), .b(x32), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n129_), .c(new_n112_), .d(new_n138_), .O(new_n140_));
  inv1   g089(.a(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n89_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n52_), .O(z14));
  oai21  g094(.a(new_n140_), .b(new_n94_), .c(x34), .O(new_n146_));
  nand2  g095(.a(new_n112_), .b(new_n138_), .O(new_n147_));
  inv1   g096(.a(new_n147_), .O(new_n148_));
  inv1   g097(.a(x30), .O(new_n149_));
  inv1   g098(.a(x31), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  inv1   g100(.a(x32), .O(new_n152_));
  inv1   g101(.a(x33), .O(new_n153_));
  inv1   g102(.a(x34), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n153_), .c(new_n152_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n148_), .c(new_n89_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n146_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n52_), .O(z15));
endmodule


