// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x26), .b(x18), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  inv1   g020(.a(new_n66_), .O(new_n72_));
  nand4  g021(.a(new_n71_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n73_));
  oai21  g022(.a(new_n72_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x18), .c(new_n58_), .O(z02));
  nand2  g026(.a(new_n73_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n53_), .c(new_n65_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n61_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n60_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n58_), .O(z03));
  nand2  g033(.a(new_n80_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x20), .b(x19), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n71_), .d(new_n63_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n85_), .c(new_n61_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n89_), .c(new_n60_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n58_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  inv1   g042(.a(x22), .O(new_n94_));
  inv1   g043(.a(x23), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n94_), .c(new_n71_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n93_), .O(new_n98_));
  nor2   g047(.a(x18), .b(x17), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n86_), .O(new_n100_));
  nand4  g049(.a(new_n93_), .b(new_n95_), .c(new_n94_), .d(new_n71_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n98_), .c(x16), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n61_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z05));
  nor2   g055(.a(x24), .b(x23), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n79_), .c(new_n86_), .d(new_n63_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x25), .O(new_n109_));
  nor2   g058(.a(x21), .b(x20), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n87_), .c(new_n110_), .d(new_n53_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n109_), .c(new_n61_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x09), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n113_), .c(new_n60_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n58_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  inv1   g066(.a(new_n112_), .O(new_n118_));
  nor3   g067(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n119_), .c(new_n99_), .d(new_n87_), .O(new_n121_));
  oai21  g070(.a(new_n118_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g072(.a(x08), .O(new_n124_));
  aoi21  g073(.a(new_n61_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z07));
  aoi21  g075(.a(x27), .b(x16), .c(x18), .O(new_n127_));
  nand2  g076(.a(new_n112_), .b(x27), .O(new_n128_));
  nor3   g077(.a(x22), .b(x21), .c(x20), .O(new_n129_));
  nor3   g078(.a(x27), .b(x26), .c(x25), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n107_), .c(new_n129_), .d(new_n53_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n128_), .c(new_n61_), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n132_), .c(new_n60_), .O(new_n134_));
  oai21  g083(.a(new_n127_), .b(new_n117_), .c(new_n134_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n61_), .c(new_n60_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x26), .O(new_n138_));
  nor2   g087(.a(x27), .b(x25), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n87_), .c(new_n93_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n73_), .c(x28), .O(new_n141_));
  nor3   g090(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n129_), .d(new_n53_), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n141_), .c(new_n61_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x06), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(new_n60_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n138_), .O(z09));
  inv1   g097(.a(x29), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n61_), .c(new_n60_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x26), .O(new_n151_));
  inv1   g100(.a(x25), .O(new_n152_));
  nor2   g101(.a(x28), .b(x27), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n107_), .c(new_n152_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n80_), .c(x29), .O(new_n155_));
  nor3   g104(.a(x29), .b(x28), .c(x27), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n120_), .c(new_n97_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n155_), .c(new_n61_), .O(new_n158_));
  nor2   g107(.a(x16), .b(x05), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n158_), .c(new_n60_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n151_), .O(z10));
  inv1   g110(.a(x30), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n61_), .c(new_n60_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x26), .O(new_n164_));
  inv1   g113(.a(x27), .O(new_n165_));
  nor2   g114(.a(x29), .b(x28), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n111_), .c(new_n165_), .d(new_n95_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n80_), .c(x30), .O(new_n168_));
  nor2   g117(.a(x30), .b(x29), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n153_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n120_), .c(new_n97_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n168_), .c(new_n61_), .O(new_n173_));
  nor2   g122(.a(x16), .b(x04), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n173_), .c(new_n60_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n164_), .O(z11));
  inv1   g125(.a(x31), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n61_), .c(new_n60_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x26), .O(new_n179_));
  nand4  g128(.a(new_n169_), .b(new_n139_), .c(new_n136_), .d(new_n93_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n88_), .c(x31), .O(new_n181_));
  inv1   g130(.a(new_n101_), .O(new_n182_));
  nand4  g131(.a(new_n177_), .b(new_n162_), .c(new_n149_), .d(new_n136_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n130_), .c(new_n182_), .d(new_n72_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n181_), .c(new_n61_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x03), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n186_), .c(new_n60_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n179_), .O(z12));
  inv1   g138(.a(x32), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n61_), .c(new_n60_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x26), .O(new_n192_));
  nor2   g141(.a(x31), .b(x30), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n166_), .c(new_n139_), .d(new_n93_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n88_), .c(x32), .O(new_n195_));
  nand4  g144(.a(new_n136_), .b(new_n165_), .c(new_n117_), .d(new_n152_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand4  g146(.a(new_n190_), .b(new_n177_), .c(new_n162_), .d(new_n149_), .O(new_n198_));
  inv1   g147(.a(new_n198_), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n197_), .c(new_n182_), .d(new_n72_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n195_), .c(new_n61_), .O(new_n201_));
  nor2   g150(.a(x16), .b(x02), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(new_n60_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n192_), .O(z13));
  inv1   g153(.a(x33), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n61_), .c(new_n60_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(x26), .O(new_n207_));
  nor2   g156(.a(x32), .b(x31), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n169_), .c(new_n153_), .d(new_n152_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n108_), .c(x33), .O(new_n210_));
  inv1   g159(.a(new_n73_), .O(new_n211_));
  nand4  g160(.a(new_n152_), .b(new_n93_), .c(new_n95_), .d(new_n94_), .O(new_n212_));
  inv1   g161(.a(new_n212_), .O(new_n213_));
  nand4  g162(.a(new_n149_), .b(new_n136_), .c(new_n165_), .d(new_n117_), .O(new_n214_));
  inv1   g163(.a(new_n214_), .O(new_n215_));
  nand4  g164(.a(new_n205_), .b(new_n190_), .c(new_n177_), .d(new_n162_), .O(new_n216_));
  inv1   g165(.a(new_n216_), .O(new_n217_));
  nand4  g166(.a(new_n217_), .b(new_n215_), .c(new_n213_), .d(new_n211_), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n210_), .c(new_n61_), .O(new_n219_));
  nor2   g168(.a(x16), .b(x01), .O(new_n220_));
  oai21  g169(.a(new_n220_), .b(new_n219_), .c(new_n60_), .O(new_n221_));
  nand2  g170(.a(new_n221_), .b(new_n207_), .O(z14));
  nand2  g171(.a(new_n218_), .b(x34), .O(new_n223_));
  nor3   g172(.a(x34), .b(x33), .c(x32), .O(new_n224_));
  nand4  g173(.a(new_n224_), .b(new_n215_), .c(new_n193_), .d(new_n118_), .O(new_n225_));
  nand2  g174(.a(new_n225_), .b(new_n223_), .O(new_n226_));
  nand2  g175(.a(new_n226_), .b(x16), .O(new_n227_));
  inv1   g176(.a(x00), .O(new_n228_));
  aoi21  g177(.a(new_n61_), .b(new_n228_), .c(x18), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(new_n227_), .O(z15));
endmodule


