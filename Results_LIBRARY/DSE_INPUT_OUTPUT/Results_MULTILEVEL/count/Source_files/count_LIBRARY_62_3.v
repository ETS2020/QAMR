// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x31), .b(x28), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(new_n60_), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n55_), .b(new_n65_), .O(new_n66_));
  nand3  g015(.a(new_n65_), .b(new_n53_), .c(new_n52_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n66_), .c(x16), .O(new_n69_));
  inv1   g018(.a(x14), .O(new_n70_));
  aoi21  g019(.a(new_n58_), .b(new_n70_), .c(x18), .O(new_n71_));
  aoi21  g020(.a(new_n71_), .b(new_n69_), .c(new_n64_), .O(z01));
  inv1   g021(.a(x21), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n55_), .O(new_n75_));
  oai21  g024(.a(new_n68_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n64_), .O(z02));
  inv1   g029(.a(x12), .O(new_n81_));
  oai21  g030(.a(x18), .b(new_n81_), .c(new_n58_), .O(new_n82_));
  nor3   g031(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n55_), .c(x16), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n59_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n60_), .O(new_n86_));
  oai21  g035(.a(x22), .b(x21), .c(x18), .O(new_n87_));
  inv1   g036(.a(x22), .O(new_n88_));
  inv1   g037(.a(new_n75_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(x16), .c(new_n64_), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n87_), .c(new_n86_), .d(new_n82_), .O(z03));
  nor2   g041(.a(x22), .b(x21), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n55_), .c(new_n65_), .O(new_n94_));
  inv1   g043(.a(x23), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n88_), .c(new_n73_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  aoi21  g046(.a(new_n94_), .b(x23), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x11), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n58_), .c(new_n100_), .O(z04));
  inv1   g050(.a(x24), .O(new_n102_));
  nor2   g051(.a(x20), .b(x19), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n93_), .c(new_n103_), .d(new_n52_), .O(new_n105_));
  oai21  g054(.a(new_n97_), .b(new_n102_), .c(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n58_), .b(new_n108_), .c(x18), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n107_), .c(new_n64_), .O(z05));
  and2   g059(.a(new_n105_), .b(x25), .O(new_n111_));
  nor2   g060(.a(x23), .b(x22), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n75_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n111_), .c(x16), .O(new_n116_));
  inv1   g065(.a(x09), .O(new_n117_));
  aoi21  g066(.a(new_n58_), .b(new_n117_), .c(x18), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n116_), .c(new_n64_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n112_), .c(new_n89_), .O(new_n122_));
  oai21  g071(.a(new_n115_), .b(new_n120_), .c(new_n122_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(x16), .O(new_n124_));
  inv1   g073(.a(x08), .O(new_n125_));
  aoi21  g074(.a(new_n58_), .b(new_n125_), .c(x18), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n124_), .c(new_n64_), .O(z07));
  inv1   g076(.a(x25), .O(new_n128_));
  nor2   g077(.a(x27), .b(x26), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n104_), .c(new_n128_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n94_), .O(new_n131_));
  aoi21  g080(.a(new_n122_), .b(x27), .c(new_n131_), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n61_), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n58_), .c(new_n134_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  nor3   g085(.a(x25), .b(x24), .c(x23), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n137_), .c(new_n120_), .O(new_n139_));
  oai22  g088(.a(new_n139_), .b(new_n94_), .c(new_n131_), .d(new_n136_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  nor2   g090(.a(x16), .b(x06), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n61_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z09));
  nor2   g093(.a(new_n64_), .b(x05), .O(new_n145_));
  nor2   g094(.a(x31), .b(new_n59_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(new_n58_), .O(new_n147_));
  oai21  g096(.a(x31), .b(x29), .c(x28), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x18), .O(new_n149_));
  oai21  g098(.a(new_n130_), .b(new_n94_), .c(x29), .O(new_n150_));
  nor3   g099(.a(x29), .b(x27), .c(x26), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n137_), .c(new_n83_), .d(new_n55_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n150_), .c(x28), .O(new_n153_));
  inv1   g102(.a(x31), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(x29), .c(x28), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n153_), .c(x16), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n149_), .c(new_n147_), .O(z10));
  nor2   g107(.a(new_n64_), .b(x04), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n146_), .c(new_n58_), .O(new_n160_));
  nor2   g109(.a(x31), .b(x30), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n136_), .c(x18), .O(new_n162_));
  nor2   g111(.a(x29), .b(x27), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n113_), .c(new_n120_), .d(new_n95_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n94_), .c(x30), .O(new_n165_));
  nor3   g114(.a(x30), .b(x29), .c(x27), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n121_), .c(new_n97_), .O(new_n167_));
  aoi21  g116(.a(new_n167_), .b(new_n165_), .c(x28), .O(new_n168_));
  nand3  g117(.a(new_n154_), .b(x30), .c(x28), .O(new_n169_));
  inv1   g118(.a(new_n169_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n162_), .c(new_n160_), .O(z11));
  nor3   g121(.a(x27), .b(x26), .c(x25), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n161_), .c(new_n173_), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n105_), .O(new_n176_));
  aoi21  g125(.a(new_n167_), .b(x31), .c(new_n176_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x03), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n61_), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n58_), .c(new_n179_), .O(z12));
  nor2   g129(.a(new_n64_), .b(x02), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n146_), .c(new_n58_), .O(new_n182_));
  oai21  g131(.a(x32), .b(x31), .c(x28), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x18), .O(new_n184_));
  nand2  g133(.a(x32), .b(x28), .O(new_n185_));
  nor2   g134(.a(x32), .b(x30), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n174_), .c(new_n129_), .d(new_n128_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n105_), .c(new_n185_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n154_), .O(new_n189_));
  nand2  g138(.a(new_n161_), .b(new_n163_), .O(new_n190_));
  inv1   g139(.a(new_n190_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n121_), .c(new_n97_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(x32), .c(new_n136_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n189_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x16), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n184_), .c(new_n182_), .O(z13));
  nor2   g145(.a(new_n64_), .b(x01), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n146_), .c(new_n58_), .O(new_n198_));
  oai21  g147(.a(x33), .b(x31), .c(x28), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x18), .O(new_n200_));
  nand2  g149(.a(x33), .b(x28), .O(new_n201_));
  nor2   g150(.a(x26), .b(x25), .O(new_n202_));
  nor2   g151(.a(x30), .b(x29), .O(new_n203_));
  nor2   g152(.a(x33), .b(x32), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n203_), .c(new_n138_), .d(new_n202_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n105_), .c(new_n201_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n154_), .O(new_n207_));
  nand4  g156(.a(new_n102_), .b(new_n95_), .c(new_n88_), .d(new_n73_), .O(new_n208_));
  inv1   g157(.a(new_n208_), .O(new_n209_));
  inv1   g158(.a(x29), .O(new_n210_));
  inv1   g159(.a(x30), .O(new_n211_));
  inv1   g160(.a(x32), .O(new_n212_));
  nand4  g161(.a(new_n212_), .b(new_n154_), .c(new_n211_), .d(new_n210_), .O(new_n213_));
  inv1   g162(.a(new_n213_), .O(new_n214_));
  nand4  g163(.a(new_n214_), .b(new_n173_), .c(new_n209_), .d(new_n68_), .O(new_n215_));
  nand3  g164(.a(new_n215_), .b(x33), .c(new_n136_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n207_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(x16), .O(new_n218_));
  nand3  g167(.a(new_n218_), .b(new_n200_), .c(new_n198_), .O(z14));
  nand2  g168(.a(x34), .b(x16), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n154_), .O(new_n221_));
  nand2  g170(.a(new_n221_), .b(x28), .O(new_n222_));
  nand4  g171(.a(new_n204_), .b(new_n161_), .c(new_n163_), .d(new_n202_), .O(new_n223_));
  oai21  g172(.a(new_n223_), .b(new_n105_), .c(x34), .O(new_n224_));
  nand2  g173(.a(new_n174_), .b(new_n129_), .O(new_n225_));
  inv1   g174(.a(new_n225_), .O(new_n226_));
  nor3   g175(.a(x34), .b(x33), .c(x32), .O(new_n227_));
  nand4  g176(.a(new_n227_), .b(new_n226_), .c(new_n161_), .d(new_n115_), .O(new_n228_));
  nand2  g177(.a(new_n228_), .b(new_n224_), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(x16), .O(new_n230_));
  inv1   g179(.a(x00), .O(new_n231_));
  aoi21  g180(.a(new_n58_), .b(new_n231_), .c(x18), .O(new_n232_));
  nand3  g181(.a(new_n232_), .b(new_n230_), .c(new_n222_), .O(z15));
endmodule


