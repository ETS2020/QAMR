// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x23), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(new_n54_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  aoi21  g021(.a(new_n65_), .b(x21), .c(new_n72_), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n62_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n62_), .c(new_n75_), .O(z02));
  nand2  g025(.a(new_n71_), .b(x22), .O(new_n77_));
  inv1   g026(.a(x20), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n53_), .c(new_n78_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n77_), .c(new_n62_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n61_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n59_), .O(z03));
  nor3   g033(.a(x22), .b(x21), .c(x20), .O(new_n85_));
  nand2  g034(.a(new_n53_), .b(x16), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n61_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n58_), .O(new_n90_));
  nand3  g039(.a(new_n80_), .b(x23), .c(x16), .O(new_n91_));
  oai21  g040(.a(x16), .b(x11), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n90_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  nor3   g044(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n64_), .c(new_n95_), .O(new_n97_));
  inv1   g046(.a(x17), .O(new_n98_));
  nor2   g047(.a(x20), .b(x19), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n79_), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n97_), .c(x16), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n62_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z05));
  nor2   g055(.a(x17), .b(new_n62_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n107_), .c(new_n79_), .d(new_n99_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n61_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n58_), .O(new_n111_));
  nand3  g060(.a(new_n101_), .b(x25), .c(x16), .O(new_n112_));
  oai21  g061(.a(x16), .b(x09), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n61_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n111_), .O(z06));
  nor2   g064(.a(x23), .b(x22), .O(new_n116_));
  nand4  g065(.a(new_n108_), .b(new_n116_), .c(new_n70_), .d(new_n53_), .O(new_n117_));
  nor2   g066(.a(x26), .b(x25), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n116_), .c(new_n95_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n71_), .O(new_n120_));
  aoi21  g069(.a(new_n117_), .b(x26), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x08), .O(new_n122_));
  aoi21  g071(.a(new_n62_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n62_), .c(new_n123_), .O(z07));
  inv1   g073(.a(x19), .O(new_n125_));
  nand3  g074(.a(new_n107_), .b(new_n70_), .c(new_n125_), .O(new_n126_));
  nor2   g075(.a(x24), .b(x22), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n126_), .c(new_n61_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n58_), .O(new_n131_));
  oai21  g080(.a(new_n119_), .b(new_n71_), .c(x27), .O(new_n132_));
  inv1   g081(.a(x07), .O(new_n133_));
  nand2  g082(.a(new_n62_), .b(new_n133_), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n62_), .c(new_n134_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n61_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n131_), .O(z08));
  nand2  g086(.a(new_n128_), .b(new_n100_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n80_), .c(x28), .O(new_n139_));
  nor3   g088(.a(x25), .b(x24), .c(x23), .O(new_n140_));
  nor3   g089(.a(x28), .b(x27), .c(x26), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n140_), .c(new_n85_), .d(new_n53_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  aoi21  g094(.a(new_n62_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z09));
  nor3   g096(.a(x26), .b(x25), .c(x24), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n148_), .c(new_n96_), .d(new_n64_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  aoi21  g100(.a(new_n142_), .b(x29), .c(new_n151_), .O(new_n152_));
  inv1   g101(.a(x05), .O(new_n153_));
  aoi21  g102(.a(new_n62_), .b(new_n153_), .c(x18), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n62_), .c(new_n154_), .O(z10));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nor2   g105(.a(x30), .b(x29), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n156_), .c(new_n118_), .d(new_n95_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n88_), .c(new_n61_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n58_), .O(new_n160_));
  nand3  g109(.a(new_n150_), .b(x30), .c(x16), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  nand2  g111(.a(new_n62_), .b(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n61_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n160_), .O(z11));
  nand3  g115(.a(new_n127_), .b(new_n87_), .c(new_n70_), .O(new_n167_));
  inv1   g116(.a(x25), .O(new_n168_));
  nor2   g117(.a(x27), .b(x26), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n169_), .d(new_n168_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n167_), .c(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n58_), .O(new_n174_));
  inv1   g123(.a(x21), .O(new_n175_));
  nand4  g124(.a(new_n116_), .b(new_n99_), .c(new_n175_), .d(new_n98_), .O(new_n176_));
  oai21  g125(.a(new_n158_), .b(new_n176_), .c(x31), .O(new_n177_));
  inv1   g126(.a(x03), .O(new_n178_));
  nand2  g127(.a(new_n62_), .b(new_n178_), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n62_), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n61_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n174_), .O(z12));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n157_), .c(new_n156_), .d(new_n118_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n167_), .c(new_n61_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n58_), .O(new_n186_));
  oai21  g135(.a(new_n172_), .b(new_n101_), .c(x32), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  nand2  g137(.a(new_n62_), .b(new_n188_), .O(new_n189_));
  oai21  g138(.a(new_n187_), .b(new_n62_), .c(new_n189_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n61_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n186_), .O(z13));
  oai21  g141(.a(new_n184_), .b(new_n101_), .c(x33), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n171_), .c(new_n170_), .d(new_n169_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n117_), .c(new_n193_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x16), .O(new_n197_));
  inv1   g146(.a(x01), .O(new_n198_));
  aoi21  g147(.a(new_n62_), .b(new_n198_), .c(x18), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n197_), .O(z14));
  nor3   g149(.a(x34), .b(x33), .c(x32), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n171_), .c(new_n170_), .d(new_n169_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n109_), .c(new_n61_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n58_), .O(new_n204_));
  oai21  g153(.a(new_n195_), .b(new_n117_), .c(x34), .O(new_n205_));
  inv1   g154(.a(x00), .O(new_n206_));
  nand2  g155(.a(new_n62_), .b(new_n206_), .O(new_n207_));
  oai21  g156(.a(new_n205_), .b(new_n62_), .c(new_n207_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n61_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n204_), .O(z15));
endmodule


