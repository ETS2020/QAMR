// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_;
  nand2  g000(.a(x31), .b(x29), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  inv1   g003(.a(x16), .O(new_n55_));
  oai21  g004(.a(x18), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  inv1   g005(.a(x17), .O(new_n57_));
  inv1   g006(.a(x18), .O(new_n58_));
  nand2  g007(.a(x19), .b(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  aoi21  g009(.a(x19), .b(new_n58_), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(x16), .c(new_n60_), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n56_), .c(new_n53_), .O(z00));
  inv1   g012(.a(x14), .O(new_n64_));
  oai21  g013(.a(x18), .b(new_n64_), .c(new_n55_), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n55_), .c(new_n58_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(x17), .O(new_n68_));
  inv1   g017(.a(x19), .O(new_n69_));
  nand3  g018(.a(new_n66_), .b(new_n57_), .c(x16), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g021(.a(new_n52_), .b(x18), .c(new_n57_), .O(new_n73_));
  aoi21  g022(.a(new_n73_), .b(new_n66_), .c(new_n69_), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(x16), .c(new_n53_), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n72_), .c(new_n68_), .d(new_n65_), .O(z01));
  nand3  g025(.a(new_n66_), .b(new_n69_), .c(new_n57_), .O(new_n77_));
  nor2   g026(.a(x19), .b(x17), .O(new_n78_));
  nor2   g027(.a(x21), .b(x20), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n77_), .b(x21), .c(new_n81_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x13), .O(new_n83_));
  nand2  g032(.a(new_n52_), .b(new_n58_), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g034(.a(new_n82_), .b(new_n55_), .c(new_n85_), .O(z02));
  nand2  g035(.a(new_n80_), .b(x22), .O(new_n87_));
  nor2   g036(.a(x22), .b(x21), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n78_), .c(new_n66_), .O(new_n89_));
  and2   g038(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x12), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n55_), .c(new_n92_), .O(z03));
  nor3   g042(.a(x20), .b(x19), .c(x17), .O(new_n94_));
  nor3   g043(.a(x23), .b(x22), .c(x21), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi21  g046(.a(new_n89_), .b(x23), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x11), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n84_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n55_), .c(new_n100_), .O(z04));
  inv1   g050(.a(x21), .O(new_n102_));
  inv1   g051(.a(x22), .O(new_n103_));
  inv1   g052(.a(x23), .O(new_n104_));
  inv1   g053(.a(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n104_), .c(new_n103_), .d(new_n102_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n77_), .O(new_n107_));
  aoi21  g056(.a(new_n96_), .b(x24), .c(new_n107_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x10), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n84_), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n55_), .c(new_n110_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  nor2   g061(.a(x23), .b(x22), .O(new_n113_));
  nor2   g062(.a(x25), .b(x24), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n113_), .c(new_n79_), .d(new_n78_), .O(new_n115_));
  oai21  g064(.a(new_n107_), .b(new_n112_), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x16), .O(new_n117_));
  nor2   g066(.a(x16), .b(x09), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n84_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n117_), .O(z06));
  nand2  g069(.a(new_n115_), .b(x26), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n113_), .c(new_n81_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  nor2   g074(.a(x16), .b(x08), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n84_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(z07));
  nor2   g077(.a(x24), .b(x23), .O(new_n129_));
  nor2   g078(.a(x27), .b(x26), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n129_), .c(new_n112_), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n89_), .O(new_n132_));
  aoi21  g081(.a(new_n123_), .b(x27), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n84_), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n55_), .c(new_n135_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  inv1   g086(.a(x26), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n114_), .c(new_n138_), .d(new_n104_), .O(new_n140_));
  or2    g089(.a(new_n140_), .b(new_n89_), .O(new_n141_));
  oai21  g090(.a(new_n132_), .b(new_n137_), .c(new_n141_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n84_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n143_), .O(z09));
  inv1   g095(.a(x27), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n122_), .c(new_n147_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n96_), .O(new_n150_));
  aoi21  g099(.a(new_n141_), .b(x29), .c(new_n150_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n84_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n55_), .c(new_n153_), .O(z10));
  nor2   g103(.a(new_n53_), .b(x04), .O(new_n155_));
  nor2   g104(.a(x31), .b(new_n58_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n155_), .c(new_n55_), .O(new_n157_));
  inv1   g106(.a(x29), .O(new_n158_));
  nor2   g107(.a(x31), .b(x30), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n158_), .c(x18), .O(new_n160_));
  oai21  g109(.a(new_n140_), .b(new_n89_), .c(x30), .O(new_n161_));
  nor3   g110(.a(x30), .b(x28), .c(x27), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n122_), .c(new_n95_), .d(new_n94_), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n161_), .c(x29), .O(new_n164_));
  inv1   g113(.a(x31), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(x30), .c(x29), .O(new_n166_));
  inv1   g115(.a(new_n166_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(x16), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n160_), .c(new_n157_), .O(z11));
  nor2   g118(.a(x16), .b(x03), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(x18), .c(new_n52_), .O(new_n171_));
  nand2  g120(.a(new_n163_), .b(x31), .O(new_n172_));
  nand3  g121(.a(new_n159_), .b(new_n139_), .c(new_n122_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n96_), .c(new_n172_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n158_), .c(x16), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n171_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n55_), .c(new_n165_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x29), .O(new_n179_));
  oai21  g128(.a(new_n173_), .b(new_n96_), .c(x32), .O(new_n180_));
  nand4  g129(.a(new_n137_), .b(new_n147_), .c(new_n138_), .d(new_n112_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  nor2   g131(.a(x30), .b(x29), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n183_), .c(new_n182_), .d(new_n107_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n180_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x16), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  aoi21  g137(.a(new_n55_), .b(new_n188_), .c(x18), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n187_), .c(new_n179_), .O(z13));
  inv1   g139(.a(x33), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n55_), .c(new_n165_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x29), .O(new_n193_));
  nand3  g142(.a(new_n129_), .b(new_n88_), .c(new_n94_), .O(new_n194_));
  nor2   g143(.a(x30), .b(x28), .O(new_n195_));
  nand4  g144(.a(new_n184_), .b(new_n195_), .c(new_n130_), .d(new_n112_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n194_), .c(x33), .O(new_n197_));
  inv1   g146(.a(x30), .O(new_n198_));
  nand4  g147(.a(new_n191_), .b(new_n177_), .c(new_n165_), .d(new_n198_), .O(new_n199_));
  inv1   g148(.a(new_n199_), .O(new_n200_));
  nand3  g149(.a(new_n200_), .b(new_n148_), .c(new_n130_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n115_), .c(new_n197_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(x16), .O(new_n203_));
  inv1   g152(.a(x01), .O(new_n204_));
  aoi21  g153(.a(new_n55_), .b(new_n204_), .c(x18), .O(new_n205_));
  nand3  g154(.a(new_n205_), .b(new_n203_), .c(new_n193_), .O(z14));
  nor2   g155(.a(new_n53_), .b(x00), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(new_n156_), .c(new_n55_), .O(new_n208_));
  oai21  g157(.a(x34), .b(x31), .c(x29), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x18), .O(new_n210_));
  nand2  g159(.a(x34), .b(x29), .O(new_n211_));
  nor2   g160(.a(x32), .b(x30), .O(new_n212_));
  nor2   g161(.a(x34), .b(x33), .O(new_n213_));
  nand4  g162(.a(new_n213_), .b(new_n212_), .c(new_n148_), .d(new_n130_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n115_), .c(new_n211_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n165_), .O(new_n216_));
  inv1   g165(.a(new_n106_), .O(new_n217_));
  nand4  g166(.a(new_n200_), .b(new_n182_), .c(new_n217_), .d(new_n94_), .O(new_n218_));
  nand3  g167(.a(new_n218_), .b(x34), .c(new_n158_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n216_), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(x16), .O(new_n221_));
  nand3  g170(.a(new_n221_), .b(new_n210_), .c(new_n208_), .O(z15));
endmodule


