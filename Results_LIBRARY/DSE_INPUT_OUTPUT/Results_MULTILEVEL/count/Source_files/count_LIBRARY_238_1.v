// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x30), .c(new_n52_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  inv1   g009(.a(x17), .O(new_n61_));
  inv1   g010(.a(x19), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n55_), .b(x20), .c(new_n65_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  inv1   g016(.a(x30), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  oai21  g019(.a(new_n66_), .b(new_n60_), .c(new_n70_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n54_), .b(new_n72_), .c(new_n63_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n64_), .b(x21), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n60_), .c(new_n77_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n54_), .c(new_n73_), .d(x22), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n69_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n60_), .c(new_n82_), .O(z03));
  inv1   g032(.a(x22), .O(new_n84_));
  nand4  g033(.a(new_n54_), .b(new_n84_), .c(new_n72_), .d(new_n63_), .O(new_n85_));
  inv1   g034(.a(x23), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n72_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  aoi21  g037(.a(new_n85_), .b(x23), .c(new_n88_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n69_), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n60_), .c(new_n91_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  nor2   g042(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g043(.a(new_n93_), .b(new_n86_), .c(new_n84_), .d(new_n72_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n64_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  nor2   g046(.a(x16), .b(x10), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n69_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n97_), .O(z05));
  inv1   g049(.a(new_n95_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n65_), .O(new_n102_));
  nor2   g051(.a(x23), .b(x22), .O(new_n103_));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  aoi22  g055(.a(new_n106_), .b(new_n74_), .c(new_n102_), .d(x25), .O(new_n107_));
  nor2   g056(.a(x16), .b(x09), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n69_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n60_), .c(new_n109_), .O(z06));
  inv1   g059(.a(x26), .O(new_n111_));
  aoi21  g060(.a(new_n106_), .b(new_n74_), .c(new_n111_), .O(new_n112_));
  nor2   g061(.a(x26), .b(x25), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n103_), .c(new_n93_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n73_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n112_), .c(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x08), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n69_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z07));
  oai21  g068(.a(new_n114_), .b(new_n73_), .c(x27), .O(new_n120_));
  nor2   g069(.a(x24), .b(x23), .O(new_n121_));
  nor3   g070(.a(x27), .b(x26), .c(x25), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n121_), .c(new_n79_), .d(new_n54_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n120_), .c(new_n60_), .O(new_n124_));
  nor2   g073(.a(x16), .b(x07), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n124_), .c(new_n68_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n52_), .O(z08));
  inv1   g076(.a(x25), .O(new_n128_));
  nor2   g077(.a(x27), .b(x26), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n121_), .c(new_n128_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n85_), .c(x28), .O(new_n131_));
  nor3   g080(.a(x25), .b(x24), .c(x23), .O(new_n132_));
  nor3   g081(.a(x28), .b(x27), .c(x26), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n132_), .c(new_n79_), .d(new_n54_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n131_), .c(new_n60_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x06), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n135_), .c(new_n68_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n52_), .O(z09));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n104_), .c(new_n111_), .d(new_n86_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n85_), .c(x29), .O(new_n141_));
  nor3   g090(.a(x26), .b(x25), .c(x24), .O(new_n142_));
  nor3   g091(.a(x29), .b(x28), .c(x27), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n142_), .c(new_n88_), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n141_), .c(new_n60_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x05), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(new_n68_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n52_), .O(z10));
  nand3  g097(.a(new_n79_), .b(new_n54_), .c(x16), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n129_), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n132_), .O(new_n153_));
  oai22  g102(.a(new_n153_), .b(new_n149_), .c(x16), .d(x04), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n68_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n52_), .O(z11));
  nor2   g105(.a(x20), .b(x19), .O(new_n157_));
  nand4  g106(.a(new_n103_), .b(new_n157_), .c(new_n72_), .d(new_n61_), .O(new_n158_));
  inv1   g107(.a(x27), .O(new_n159_));
  nand4  g108(.a(new_n150_), .b(new_n113_), .c(new_n159_), .d(new_n93_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n158_), .c(x31), .O(new_n161_));
  nor2   g110(.a(x31), .b(x29), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n139_), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n142_), .c(new_n88_), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n161_), .c(new_n60_), .O(new_n166_));
  nor2   g115(.a(x16), .b(x03), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n166_), .c(new_n68_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n52_), .O(z12));
  nand4  g118(.a(new_n162_), .b(new_n139_), .c(new_n113_), .d(new_n93_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n158_), .c(x32), .O(new_n171_));
  inv1   g120(.a(x28), .O(new_n172_));
  inv1   g121(.a(x29), .O(new_n173_));
  inv1   g122(.a(x31), .O(new_n174_));
  inv1   g123(.a(x32), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n173_), .d(new_n172_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n122_), .c(new_n101_), .d(new_n65_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n171_), .c(new_n60_), .O(new_n179_));
  nor2   g128(.a(x16), .b(x02), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n179_), .c(new_n68_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n52_), .O(z13));
  nand2  g131(.a(new_n178_), .b(x33), .O(new_n183_));
  nor2   g132(.a(x33), .b(x32), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n162_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n139_), .c(new_n113_), .d(new_n96_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n183_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x01), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n69_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n189_), .O(z14));
  nand4  g141(.a(new_n184_), .b(new_n162_), .c(new_n139_), .d(new_n113_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n102_), .c(x34), .O(new_n194_));
  nor2   g143(.a(x34), .b(x33), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n175_), .c(new_n174_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n152_), .c(new_n106_), .d(new_n74_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n194_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x00), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n69_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n200_), .O(z15));
endmodule


