// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x31), .c(new_n52_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n55_), .b(x20), .c(new_n61_), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nor2   g012(.a(x31), .b(x18), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai21  g015(.a(new_n62_), .b(new_n60_), .c(new_n66_), .O(z01));
  inv1   g016(.a(x17), .O(new_n68_));
  inv1   g017(.a(x19), .O(new_n69_));
  inv1   g018(.a(x20), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n54_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n71_), .b(x21), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n60_), .c(new_n77_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n54_), .c(new_n73_), .d(x22), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n65_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n60_), .c(new_n82_), .O(z03));
  inv1   g032(.a(x31), .O(new_n84_));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n54_), .c(new_n70_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x23), .O(new_n87_));
  inv1   g036(.a(x21), .O(new_n88_));
  nor2   g037(.a(x20), .b(x19), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n68_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n87_), .c(new_n60_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x11), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n92_), .c(new_n84_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n52_), .O(z04));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n85_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n71_), .O(new_n98_));
  aoi21  g047(.a(new_n91_), .b(x24), .c(new_n98_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n65_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n60_), .c(new_n101_), .O(z05));
  nand3  g051(.a(new_n96_), .b(new_n85_), .c(new_n61_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x25), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n90_), .c(new_n72_), .d(new_n54_), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n104_), .c(new_n60_), .O(new_n107_));
  nor2   g056(.a(x16), .b(x09), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n107_), .c(new_n84_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n52_), .O(z06));
  nand2  g059(.a(new_n106_), .b(x26), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n90_), .c(new_n74_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n111_), .c(new_n60_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x08), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n114_), .c(new_n84_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n52_), .O(z07));
  inv1   g066(.a(x24), .O(new_n118_));
  nor2   g067(.a(x26), .b(x25), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n90_), .c(new_n118_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n73_), .c(x27), .O(new_n121_));
  nor3   g070(.a(x27), .b(x26), .c(x25), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n96_), .c(new_n79_), .d(new_n54_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n121_), .c(new_n60_), .O(new_n124_));
  nor2   g073(.a(x16), .b(x07), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n124_), .c(new_n84_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n52_), .O(z08));
  inv1   g076(.a(x25), .O(new_n128_));
  nor2   g077(.a(x27), .b(x26), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n96_), .c(new_n128_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n86_), .c(x28), .O(new_n131_));
  nor3   g080(.a(x25), .b(x24), .c(x23), .O(new_n132_));
  nor3   g081(.a(x28), .b(x27), .c(x26), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n132_), .c(new_n79_), .d(new_n54_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n131_), .c(new_n60_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x06), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n135_), .c(new_n84_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n52_), .O(z09));
  inv1   g087(.a(x23), .O(new_n139_));
  inv1   g088(.a(x26), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n105_), .c(new_n140_), .d(new_n139_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n86_), .c(x29), .O(new_n143_));
  inv1   g092(.a(x22), .O(new_n144_));
  nand3  g093(.a(new_n139_), .b(new_n144_), .c(new_n88_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n71_), .O(new_n146_));
  nor3   g095(.a(x29), .b(x28), .c(x27), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n112_), .c(new_n146_), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n143_), .c(new_n60_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x05), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n149_), .c(new_n84_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z10));
  inv1   g101(.a(x27), .O(new_n153_));
  nor2   g102(.a(x29), .b(x28), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n119_), .c(new_n153_), .d(new_n118_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n91_), .c(x30), .O(new_n156_));
  nor2   g105(.a(x30), .b(x29), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n141_), .O(new_n158_));
  inv1   g107(.a(new_n158_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n112_), .c(new_n146_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n156_), .c(new_n60_), .O(new_n161_));
  nor2   g110(.a(x16), .b(x04), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n161_), .c(new_n84_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n52_), .O(z11));
  inv1   g113(.a(x03), .O(new_n165_));
  nand2  g114(.a(new_n60_), .b(new_n165_), .O(new_n166_));
  nand2  g115(.a(new_n90_), .b(new_n72_), .O(new_n167_));
  nor3   g116(.a(new_n167_), .b(new_n55_), .c(new_n60_), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n159_), .c(new_n112_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n166_), .c(new_n64_), .O(z12));
  nand2  g119(.a(new_n160_), .b(x32), .O(new_n171_));
  nor2   g120(.a(x32), .b(x30), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n154_), .c(new_n122_), .d(new_n98_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  nor2   g124(.a(x16), .b(x02), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n65_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z13));
  nor2   g127(.a(x33), .b(x32), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n157_), .c(new_n141_), .d(new_n119_), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n103_), .O(new_n181_));
  aoi21  g130(.a(new_n173_), .b(x33), .c(new_n181_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x01), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n65_), .O(new_n184_));
  oai21  g133(.a(new_n182_), .b(new_n60_), .c(new_n184_), .O(z14));
  oai21  g134(.a(new_n180_), .b(new_n103_), .c(x34), .O(new_n186_));
  nor2   g135(.a(x34), .b(x33), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n172_), .c(new_n154_), .d(new_n129_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n106_), .c(new_n186_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(x16), .O(new_n190_));
  nor2   g139(.a(x16), .b(x00), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n65_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n190_), .O(z15));
endmodule


