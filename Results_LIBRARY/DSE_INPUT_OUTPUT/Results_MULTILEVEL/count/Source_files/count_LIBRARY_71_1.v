// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x31), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x18), .O(new_n62_));
  inv1   g011(.a(x31), .O(new_n63_));
  inv1   g012(.a(new_n55_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x20), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n58_), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n62_), .O(z01));
  inv1   g020(.a(x20), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nand4  g022(.a(new_n73_), .b(new_n72_), .c(new_n53_), .d(new_n52_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n67_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(new_n60_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x13), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g028(.a(new_n76_), .b(new_n58_), .c(new_n79_), .O(z02));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n55_), .c(new_n72_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n74_), .b(x22), .c(new_n83_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n58_), .c(new_n86_), .O(z03));
  nand2  g036(.a(new_n82_), .b(x23), .O(new_n88_));
  nor2   g037(.a(x20), .b(x19), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n89_), .c(new_n73_), .d(new_n52_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n88_), .c(new_n58_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x11), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n92_), .c(new_n63_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n62_), .O(z04));
  nand2  g044(.a(new_n91_), .b(x24), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n81_), .c(new_n89_), .d(new_n52_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n58_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n99_), .c(new_n63_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n62_), .O(z05));
  nand2  g051(.a(new_n98_), .b(x25), .O(new_n103_));
  nor2   g052(.a(x21), .b(x20), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n90_), .c(new_n104_), .d(new_n55_), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n103_), .c(new_n58_), .O(new_n107_));
  oai21  g056(.a(x16), .b(x09), .c(new_n60_), .O(new_n108_));
  or2    g057(.a(new_n108_), .b(new_n107_), .O(z06));
  nand2  g058(.a(new_n106_), .b(x26), .O(new_n110_));
  nor3   g059(.a(x26), .b(x25), .c(x24), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n90_), .c(new_n75_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n110_), .c(new_n58_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x08), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n113_), .c(new_n63_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n62_), .O(z07));
  inv1   g065(.a(x25), .O(new_n117_));
  nor2   g066(.a(x27), .b(x26), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n97_), .c(new_n117_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n82_), .O(new_n120_));
  aoi21  g069(.a(new_n112_), .b(x27), .c(new_n120_), .O(new_n121_));
  nor2   g070(.a(x16), .b(x07), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n77_), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n58_), .c(new_n123_), .O(z08));
  oai21  g073(.a(new_n119_), .b(new_n82_), .c(x28), .O(new_n125_));
  nor3   g074(.a(x25), .b(x24), .c(x23), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x26), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n126_), .c(new_n83_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n125_), .c(new_n58_), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n129_), .c(new_n63_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n62_), .O(z09));
  inv1   g081(.a(x23), .O(new_n133_));
  inv1   g082(.a(x26), .O(new_n134_));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n105_), .c(new_n134_), .d(new_n133_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n82_), .c(x29), .O(new_n137_));
  nor3   g086(.a(x23), .b(x22), .c(x21), .O(new_n138_));
  nor3   g087(.a(x29), .b(x28), .c(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n111_), .c(new_n138_), .d(new_n66_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n137_), .c(new_n58_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x05), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n141_), .c(new_n63_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n62_), .O(z10));
  inv1   g093(.a(x24), .O(new_n145_));
  nor2   g094(.a(x26), .b(x25), .O(new_n146_));
  nor2   g095(.a(x30), .b(x29), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n135_), .c(new_n146_), .d(new_n145_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n91_), .O(new_n149_));
  aoi21  g098(.a(new_n140_), .b(x30), .c(new_n149_), .O(new_n150_));
  nor2   g099(.a(x16), .b(x04), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n77_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n58_), .c(new_n152_), .O(z11));
  inv1   g102(.a(x03), .O(new_n154_));
  nand2  g103(.a(new_n58_), .b(new_n154_), .O(new_n155_));
  nand4  g104(.a(new_n90_), .b(new_n104_), .c(new_n55_), .d(x16), .O(new_n156_));
  or2    g105(.a(new_n156_), .b(new_n148_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(new_n60_), .O(z12));
  oai21  g107(.a(new_n148_), .b(new_n91_), .c(x32), .O(new_n159_));
  inv1   g108(.a(x22), .O(new_n160_));
  nand4  g109(.a(new_n145_), .b(new_n133_), .c(new_n160_), .d(new_n73_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  nor3   g111(.a(x27), .b(x26), .c(x25), .O(new_n163_));
  inv1   g112(.a(x28), .O(new_n164_));
  inv1   g113(.a(x29), .O(new_n165_));
  inv1   g114(.a(x30), .O(new_n166_));
  inv1   g115(.a(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n166_), .c(new_n165_), .d(new_n164_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n163_), .c(new_n162_), .d(new_n66_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n159_), .c(new_n58_), .O(new_n171_));
  nor2   g120(.a(x16), .b(x02), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n171_), .c(new_n63_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n62_), .O(z13));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nor2   g124(.a(x32), .b(x30), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n118_), .d(new_n117_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n98_), .c(x33), .O(new_n178_));
  inv1   g127(.a(x27), .O(new_n179_));
  nand4  g128(.a(new_n164_), .b(new_n179_), .c(new_n134_), .d(new_n117_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  inv1   g130(.a(x33), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n167_), .c(new_n166_), .d(new_n165_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n181_), .c(new_n162_), .d(new_n66_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n178_), .c(new_n58_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x01), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n186_), .c(new_n63_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n62_), .O(z14));
  nor2   g138(.a(x33), .b(x32), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n147_), .c(new_n135_), .d(new_n146_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n98_), .c(x34), .O(new_n192_));
  nand4  g141(.a(new_n117_), .b(new_n145_), .c(new_n133_), .d(new_n160_), .O(new_n193_));
  inv1   g142(.a(new_n193_), .O(new_n194_));
  nand4  g143(.a(new_n165_), .b(new_n164_), .c(new_n179_), .d(new_n134_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  inv1   g145(.a(x34), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n182_), .c(new_n167_), .d(new_n166_), .O(new_n198_));
  inv1   g147(.a(new_n198_), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n196_), .c(new_n194_), .d(new_n75_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n192_), .c(new_n58_), .O(new_n201_));
  nor2   g150(.a(x16), .b(x00), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(new_n63_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n62_), .O(z15));
endmodule


