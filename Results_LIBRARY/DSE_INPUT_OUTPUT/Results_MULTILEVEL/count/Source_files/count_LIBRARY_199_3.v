// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x29), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  aoi21  g012(.a(new_n54_), .b(x20), .c(new_n63_), .O(new_n64_));
  nor2   g013(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n59_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  oai21  g020(.a(new_n63_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  aoi21  g022(.a(new_n72_), .b(x16), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(x18), .c(new_n59_), .O(z02));
  nor3   g024(.a(x19), .b(x18), .c(x17), .O(new_n76_));
  nor3   g025(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  aoi22  g026(.a(new_n77_), .b(new_n76_), .c(new_n71_), .d(x22), .O(new_n78_));
  inv1   g027(.a(x12), .O(new_n79_));
  aoi21  g028(.a(new_n62_), .b(new_n79_), .c(x18), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n62_), .c(new_n80_), .O(z03));
  inv1   g030(.a(x20), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n53_), .c(new_n82_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x23), .O(new_n85_));
  inv1   g034(.a(x17), .O(new_n86_));
  nor2   g035(.a(x20), .b(x19), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n87_), .c(new_n69_), .d(new_n86_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n85_), .c(new_n62_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n61_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n59_), .O(z04));
  nand2  g042(.a(new_n89_), .b(x24), .O(new_n94_));
  nor2   g043(.a(x24), .b(x23), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n83_), .c(new_n87_), .d(new_n86_), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n94_), .c(new_n62_), .O(new_n97_));
  nor2   g046(.a(x16), .b(x10), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n97_), .c(new_n61_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n59_), .O(z05));
  nand2  g049(.a(new_n96_), .b(x25), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n88_), .c(new_n70_), .d(new_n53_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n101_), .c(new_n62_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x09), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n104_), .c(new_n61_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n59_), .O(z06));
  inv1   g056(.a(x19), .O(new_n108_));
  nand4  g057(.a(new_n70_), .b(new_n108_), .c(new_n61_), .d(new_n86_), .O(new_n109_));
  nor3   g058(.a(x26), .b(x25), .c(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  aoi21  g061(.a(new_n103_), .b(x26), .c(new_n112_), .O(new_n113_));
  inv1   g062(.a(x08), .O(new_n114_));
  aoi21  g063(.a(new_n62_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n62_), .c(new_n115_), .O(z07));
  oai21  g065(.a(new_n111_), .b(new_n71_), .c(x27), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nor2   g067(.a(x27), .b(x26), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n95_), .c(new_n118_), .O(new_n120_));
  or2    g069(.a(new_n120_), .b(new_n84_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g072(.a(x07), .O(new_n124_));
  aoi21  g073(.a(new_n62_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z08));
  oai21  g075(.a(new_n120_), .b(new_n84_), .c(x28), .O(new_n127_));
  nor3   g076(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nor3   g077(.a(x28), .b(x27), .c(x26), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n128_), .c(new_n77_), .d(new_n53_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g081(.a(x06), .O(new_n133_));
  aoi21  g082(.a(new_n62_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z09));
  nand2  g084(.a(new_n129_), .b(new_n128_), .O(new_n136_));
  nand3  g085(.a(new_n77_), .b(new_n53_), .c(x16), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n136_), .c(new_n61_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n58_), .O(new_n139_));
  nand3  g088(.a(new_n130_), .b(x29), .c(x16), .O(new_n140_));
  inv1   g089(.a(x05), .O(new_n141_));
  nand2  g090(.a(new_n62_), .b(new_n141_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n61_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n139_), .O(z10));
  nor3   g094(.a(x23), .b(x22), .c(x21), .O(new_n146_));
  nor3   g095(.a(x29), .b(x28), .c(x27), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n110_), .c(new_n146_), .d(new_n63_), .O(new_n148_));
  inv1   g097(.a(x24), .O(new_n149_));
  nor2   g098(.a(x26), .b(x25), .O(new_n150_));
  nor2   g099(.a(x28), .b(x27), .O(new_n151_));
  nor2   g100(.a(x30), .b(x29), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n151_), .c(new_n150_), .d(new_n149_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n89_), .O(new_n154_));
  aoi21  g103(.a(new_n148_), .b(x30), .c(new_n154_), .O(new_n155_));
  inv1   g104(.a(x04), .O(new_n156_));
  aoi21  g105(.a(new_n62_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n62_), .c(new_n157_), .O(z11));
  nand4  g107(.a(new_n88_), .b(new_n70_), .c(new_n53_), .d(x16), .O(new_n159_));
  nor2   g108(.a(x31), .b(x30), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n151_), .c(new_n110_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n159_), .c(new_n61_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n58_), .O(new_n163_));
  oai21  g112(.a(new_n153_), .b(new_n89_), .c(x31), .O(new_n164_));
  inv1   g113(.a(x03), .O(new_n165_));
  nand2  g114(.a(new_n62_), .b(new_n165_), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n62_), .c(new_n166_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n61_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n163_), .O(z12));
  nor2   g118(.a(x30), .b(x28), .O(new_n170_));
  nor2   g119(.a(x32), .b(x31), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n119_), .d(new_n102_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n159_), .c(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n58_), .O(new_n174_));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nand4  g124(.a(new_n160_), .b(new_n175_), .c(new_n119_), .d(new_n118_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n96_), .c(x32), .O(new_n177_));
  inv1   g126(.a(x02), .O(new_n178_));
  nand2  g127(.a(new_n62_), .b(new_n178_), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n62_), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n61_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n174_), .O(z13));
  nand4  g131(.a(new_n171_), .b(new_n152_), .c(new_n151_), .d(new_n150_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n96_), .c(x33), .O(new_n184_));
  nor2   g133(.a(x33), .b(x32), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n160_), .c(new_n175_), .d(new_n119_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n103_), .c(new_n184_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x16), .O(new_n188_));
  inv1   g137(.a(x01), .O(new_n189_));
  aoi21  g138(.a(new_n62_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z14));
  nor2   g140(.a(x17), .b(new_n62_), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n95_), .c(new_n83_), .d(new_n87_), .O(new_n193_));
  nor3   g142(.a(x34), .b(x33), .c(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n160_), .c(new_n151_), .d(new_n150_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n193_), .c(new_n61_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n58_), .O(new_n197_));
  oai21  g146(.a(new_n186_), .b(new_n103_), .c(x34), .O(new_n198_));
  inv1   g147(.a(x00), .O(new_n199_));
  nand2  g148(.a(new_n62_), .b(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n198_), .b(new_n62_), .c(new_n200_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n61_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n197_), .O(z15));
endmodule


