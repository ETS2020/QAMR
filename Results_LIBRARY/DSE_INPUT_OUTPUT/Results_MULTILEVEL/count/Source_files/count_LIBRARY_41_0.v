// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n52_), .O(z00));
  inv1   g007(.a(x16), .O(new_n59_));
  inv1   g008(.a(new_n56_), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n54_), .c(new_n53_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  aoi21  g012(.a(new_n60_), .b(x20), .c(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n59_), .c(new_n52_), .O(z01));
  nand2  g014(.a(new_n62_), .b(x21), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n61_), .c(new_n54_), .d(new_n53_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .d(x16), .O(z02));
  inv1   g018(.a(x22), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  aoi21  g020(.a(new_n71_), .b(new_n56_), .c(new_n70_), .O(new_n72_));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n56_), .c(new_n61_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n72_), .c(x16), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n52_), .O(z03));
  nor2   g026(.a(x20), .b(x19), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand4  g028(.a(new_n79_), .b(new_n78_), .c(new_n67_), .d(new_n53_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n74_), .b(x23), .c(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n59_), .c(new_n52_), .O(z04));
  inv1   g032(.a(x24), .O(new_n84_));
  nor2   g033(.a(x24), .b(x23), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n73_), .c(new_n78_), .d(new_n53_), .O(new_n86_));
  oai21  g035(.a(new_n81_), .b(new_n84_), .c(new_n86_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x16), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n52_), .O(z05));
  inv1   g038(.a(x23), .O(new_n90_));
  inv1   g039(.a(x25), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n84_), .c(new_n90_), .d(new_n70_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n68_), .O(new_n93_));
  aoi21  g042(.a(new_n86_), .b(x25), .c(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n59_), .c(new_n52_), .O(z06));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n79_), .c(new_n71_), .d(new_n56_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x26), .O(new_n98_));
  nand4  g047(.a(new_n71_), .b(new_n54_), .c(new_n53_), .d(x16), .O(new_n99_));
  nor3   g048(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n79_), .O(new_n101_));
  or2    g050(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n98_), .c(new_n52_), .d(x16), .O(z07));
  oai21  g052(.a(new_n101_), .b(new_n68_), .c(x27), .O(new_n104_));
  nor3   g053(.a(x27), .b(x26), .c(x25), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n85_), .c(new_n75_), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n104_), .c(new_n52_), .d(x16), .O(z08));
  nor2   g056(.a(x27), .b(x26), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n85_), .c(new_n91_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n74_), .c(x28), .O(new_n110_));
  nor3   g059(.a(x28), .b(x27), .c(x26), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n96_), .c(new_n75_), .d(new_n90_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n52_), .O(z09));
  nand2  g064(.a(new_n112_), .b(x29), .O(new_n116_));
  inv1   g065(.a(x27), .O(new_n117_));
  nor2   g066(.a(x29), .b(x28), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n100_), .c(new_n117_), .O(new_n119_));
  or2    g068(.a(new_n119_), .b(new_n80_), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n116_), .c(new_n52_), .d(x16), .O(z10));
  oai21  g070(.a(new_n119_), .b(new_n80_), .c(x30), .O(new_n122_));
  nor2   g071(.a(x26), .b(x25), .O(new_n123_));
  nor2   g072(.a(x28), .b(x27), .O(new_n124_));
  nor2   g073(.a(x30), .b(x29), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n84_), .O(new_n126_));
  or2    g075(.a(new_n126_), .b(new_n80_), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n122_), .c(new_n52_), .d(x16), .O(z11));
  oai21  g077(.a(new_n126_), .b(new_n80_), .c(x31), .O(new_n129_));
  inv1   g078(.a(new_n86_), .O(new_n130_));
  nor2   g079(.a(x31), .b(x30), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n118_), .O(new_n132_));
  inv1   g081(.a(new_n132_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n105_), .c(new_n130_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n129_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(x16), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n52_), .O(z12));
  nand2  g086(.a(new_n134_), .b(x32), .O(new_n138_));
  nor2   g087(.a(x32), .b(x31), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n140_));
  or2    g089(.a(new_n140_), .b(new_n86_), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n138_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g091(.a(new_n140_), .b(new_n86_), .c(x33), .O(new_n143_));
  nor2   g092(.a(x33), .b(x32), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n131_), .c(new_n118_), .d(new_n108_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n93_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n52_), .O(z14));
  oai21  g099(.a(new_n145_), .b(new_n97_), .c(x34), .O(new_n151_));
  nand2  g100(.a(new_n118_), .b(new_n108_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  inv1   g102(.a(x30), .O(new_n154_));
  inv1   g103(.a(x31), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  inv1   g105(.a(x32), .O(new_n157_));
  inv1   g106(.a(x33), .O(new_n158_));
  inv1   g107(.a(x34), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n158_), .c(new_n157_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n153_), .c(new_n93_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n151_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x16), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n52_), .O(z15));
endmodule


