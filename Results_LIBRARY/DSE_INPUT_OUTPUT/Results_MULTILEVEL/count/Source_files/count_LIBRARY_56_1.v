// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  aoi21  g003(.a(new_n53_), .b(x16), .c(new_n54_), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(x19), .c(new_n52_), .O(z00));
  xnor2a g005(.a(x20), .b(x17), .O(new_n57_));
  nor2   g006(.a(x16), .b(x14), .O(new_n58_));
  aoi21  g007(.a(new_n57_), .b(x16), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(x19), .c(new_n52_), .O(z01));
  inv1   g009(.a(x21), .O(new_n61_));
  nor2   g010(.a(x20), .b(x17), .O(new_n62_));
  nor2   g011(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor3   g012(.a(x21), .b(x20), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g014(.a(x16), .b(x13), .O(new_n66_));
  inv1   g015(.a(x19), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n65_), .O(z02));
  inv1   g019(.a(x16), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  inv1   g022(.a(x20), .O(new_n74_));
  inv1   g023(.a(x22), .O(new_n75_));
  nand4  g024(.a(new_n75_), .b(new_n61_), .c(new_n74_), .d(new_n53_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  aoi21  g026(.a(new_n73_), .b(x22), .c(new_n77_), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n68_), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n71_), .c(new_n80_), .O(z03));
  nand2  g030(.a(new_n76_), .b(x23), .O(new_n82_));
  nor3   g031(.a(x23), .b(x22), .c(x21), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n62_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n71_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x11), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(new_n67_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n52_), .O(z04));
  nand2  g037(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x22), .b(x21), .O(new_n90_));
  nor2   g039(.a(x20), .b(x19), .O(new_n91_));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n53_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n68_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z05));
  inv1   g047(.a(x25), .O(new_n99_));
  nand2  g048(.a(new_n92_), .b(new_n75_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  nor2   g050(.a(x23), .b(x22), .O(new_n102_));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n72_), .d(new_n53_), .O(new_n104_));
  oai21  g053(.a(new_n101_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n68_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z06));
  nand2  g058(.a(new_n104_), .b(x26), .O(new_n110_));
  nor2   g059(.a(x26), .b(x25), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n92_), .c(new_n90_), .d(new_n62_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n110_), .c(new_n71_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x08), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n113_), .c(new_n67_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n52_), .O(z07));
  nand2  g065(.a(new_n112_), .b(x27), .O(new_n117_));
  nor3   g066(.a(x27), .b(x26), .c(x25), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n92_), .c(new_n77_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n71_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n120_), .c(new_n67_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n52_), .O(z08));
  nor2   g072(.a(x27), .b(x26), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n92_), .c(new_n99_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n76_), .c(x28), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x26), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n103_), .c(new_n83_), .d(new_n62_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n126_), .c(new_n71_), .O(new_n129_));
  nor2   g078(.a(x16), .b(x06), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n129_), .c(new_n67_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z09));
  nand2  g081(.a(new_n128_), .b(x29), .O(new_n133_));
  nor3   g082(.a(x26), .b(x25), .c(x24), .O(new_n134_));
  nor3   g083(.a(x29), .b(x28), .c(x27), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n134_), .c(new_n83_), .d(new_n62_), .O(new_n136_));
  and2   g085(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x05), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n68_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n71_), .c(new_n139_), .O(z10));
  nand2  g089(.a(new_n136_), .b(x30), .O(new_n141_));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(x30), .b(x29), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n118_), .c(new_n101_), .d(new_n142_), .O(new_n144_));
  and2   g093(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x04), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n68_), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n71_), .c(new_n147_), .O(z11));
  nand3  g097(.a(new_n92_), .b(new_n64_), .c(new_n75_), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nor2   g099(.a(x31), .b(x30), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(new_n118_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  aoi21  g102(.a(new_n144_), .b(x31), .c(new_n153_), .O(new_n154_));
  nor2   g103(.a(x16), .b(x03), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n68_), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n71_), .c(new_n156_), .O(z12));
  oai21  g106(.a(new_n152_), .b(new_n149_), .c(x32), .O(new_n158_));
  nor2   g107(.a(x32), .b(x31), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n143_), .c(new_n127_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n104_), .c(new_n158_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  nor2   g111(.a(x16), .b(x02), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n68_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z13));
  oai21  g114(.a(new_n160_), .b(new_n104_), .c(x33), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n151_), .c(new_n150_), .d(new_n124_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n104_), .c(new_n166_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nor2   g119(.a(x16), .b(x01), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n68_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z14));
  oai21  g122(.a(new_n168_), .b(new_n104_), .c(x34), .O(new_n174_));
  inv1   g123(.a(x23), .O(new_n175_));
  inv1   g124(.a(x24), .O(new_n176_));
  inv1   g125(.a(x26), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n99_), .c(new_n176_), .d(new_n175_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  inv1   g128(.a(x27), .O(new_n180_));
  inv1   g129(.a(x29), .O(new_n181_));
  inv1   g130(.a(x30), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n181_), .c(new_n142_), .d(new_n180_), .O(new_n183_));
  inv1   g132(.a(new_n183_), .O(new_n184_));
  inv1   g133(.a(x31), .O(new_n185_));
  inv1   g134(.a(x32), .O(new_n186_));
  inv1   g135(.a(x33), .O(new_n187_));
  inv1   g136(.a(x34), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n187_), .c(new_n186_), .d(new_n185_), .O(new_n189_));
  inv1   g138(.a(new_n189_), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n184_), .c(new_n179_), .d(new_n77_), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n174_), .c(new_n71_), .O(new_n192_));
  nor2   g141(.a(x16), .b(x00), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n192_), .c(new_n67_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n52_), .O(z15));
endmodule


