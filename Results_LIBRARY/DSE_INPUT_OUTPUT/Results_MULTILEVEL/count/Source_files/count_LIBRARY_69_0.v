// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand4  g004(.a(new_n55_), .b(new_n54_), .c(new_n52_), .d(x16), .O(z00));
  inv1   g005(.a(x16), .O(new_n57_));
  nor3   g006(.a(x20), .b(x19), .c(x17), .O(new_n58_));
  aoi21  g007(.a(new_n54_), .b(x20), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(z01));
  inv1   g009(.a(x17), .O(new_n61_));
  nor2   g010(.a(x20), .b(x19), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x21), .O(new_n64_));
  inv1   g013(.a(x19), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand4  g016(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n61_), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n64_), .c(new_n52_), .d(x16), .O(z02));
  nand2  g018(.a(new_n68_), .b(x22), .O(new_n70_));
  nor2   g019(.a(x22), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n53_), .c(new_n66_), .O(new_n72_));
  nand4  g021(.a(new_n72_), .b(new_n70_), .c(new_n52_), .d(x16), .O(z03));
  nor2   g022(.a(x23), .b(x22), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n58_), .c(new_n67_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n72_), .b(x23), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n57_), .c(new_n52_), .O(z04));
  nor2   g027(.a(x24), .b(x23), .O(new_n79_));
  nand4  g028(.a(new_n79_), .b(new_n71_), .c(new_n62_), .d(new_n61_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n75_), .b(x24), .c(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n57_), .c(new_n52_), .O(z05));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(x23), .O(new_n85_));
  inv1   g034(.a(x24), .O(new_n86_));
  inv1   g035(.a(x25), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n68_), .O(new_n89_));
  aoi21  g038(.a(new_n80_), .b(x25), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n57_), .c(new_n52_), .O(z06));
  inv1   g040(.a(x26), .O(new_n92_));
  nor2   g041(.a(x21), .b(x20), .O(new_n93_));
  nor3   g042(.a(x26), .b(x25), .c(x24), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n74_), .c(new_n93_), .d(new_n53_), .O(new_n95_));
  oai21  g044(.a(new_n89_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n52_), .O(z07));
  nor2   g047(.a(x27), .b(x26), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n79_), .c(new_n87_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n72_), .O(new_n101_));
  aoi21  g050(.a(new_n95_), .b(x27), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n57_), .c(new_n52_), .O(z08));
  oai21  g052(.a(new_n100_), .b(new_n72_), .c(x28), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nor2   g054(.a(x28), .b(x27), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n105_), .c(new_n92_), .d(new_n85_), .O(new_n107_));
  or2    g056(.a(new_n107_), .b(new_n72_), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n104_), .c(new_n52_), .d(x16), .O(z09));
  nand2  g058(.a(new_n108_), .b(x29), .O(new_n110_));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(x29), .b(x28), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n94_), .c(new_n111_), .O(new_n113_));
  or2    g062(.a(new_n113_), .b(new_n75_), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n110_), .c(new_n52_), .d(x16), .O(z10));
  oai21  g064(.a(new_n113_), .b(new_n75_), .c(x30), .O(new_n116_));
  nor2   g065(.a(x30), .b(x29), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n106_), .c(new_n94_), .O(new_n118_));
  or2    g067(.a(new_n118_), .b(new_n75_), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n116_), .c(new_n52_), .d(x16), .O(z11));
  oai21  g069(.a(new_n118_), .b(new_n75_), .c(x31), .O(new_n121_));
  nor3   g070(.a(x27), .b(x26), .c(x25), .O(new_n122_));
  nor2   g071(.a(x31), .b(x30), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n112_), .c(new_n122_), .d(new_n81_), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n121_), .c(new_n52_), .d(x16), .O(z12));
  nand2  g074(.a(new_n124_), .b(x32), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nor2   g076(.a(x32), .b(x31), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n117_), .c(new_n106_), .d(new_n127_), .O(new_n129_));
  or2    g078(.a(new_n129_), .b(new_n80_), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n126_), .c(new_n52_), .d(x16), .O(z13));
  oai21  g080(.a(new_n129_), .b(new_n80_), .c(x33), .O(new_n132_));
  nor2   g081(.a(x33), .b(x32), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n123_), .c(new_n112_), .d(new_n99_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n52_), .O(z14));
  nand4  g088(.a(new_n105_), .b(new_n74_), .c(new_n93_), .d(new_n53_), .O(new_n140_));
  oai21  g089(.a(new_n134_), .b(new_n140_), .c(x34), .O(new_n141_));
  nand2  g090(.a(new_n112_), .b(new_n99_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  inv1   g092(.a(x30), .O(new_n144_));
  inv1   g093(.a(x31), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g095(.a(x32), .O(new_n147_));
  inv1   g096(.a(x33), .O(new_n148_));
  inv1   g097(.a(x34), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n148_), .c(new_n147_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n143_), .c(new_n89_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n141_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n52_), .O(z15));
endmodule


