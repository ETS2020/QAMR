// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x31), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x29), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  oai21  g005(.a(x18), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x17), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x19), .b(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  aoi21  g010(.a(x19), .b(new_n59_), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(x16), .c(new_n61_), .O(new_n63_));
  aoi21  g012(.a(new_n63_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g013(.a(x14), .O(new_n65_));
  oai21  g014(.a(x18), .b(new_n65_), .c(new_n56_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n56_), .c(new_n59_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x17), .O(new_n69_));
  inv1   g018(.a(x19), .O(new_n70_));
  nand3  g019(.a(new_n67_), .b(new_n58_), .c(x16), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g022(.a(new_n53_), .b(x18), .c(new_n58_), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(new_n67_), .c(new_n70_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(x16), .c(new_n54_), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n73_), .c(new_n69_), .d(new_n66_), .O(z01));
  nor2   g026(.a(x20), .b(x19), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n58_), .O(new_n79_));
  inv1   g028(.a(x21), .O(new_n80_));
  nand4  g029(.a(new_n80_), .b(new_n67_), .c(new_n70_), .d(new_n58_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  aoi21  g031(.a(new_n79_), .b(x21), .c(new_n82_), .O(new_n83_));
  nor2   g032(.a(x16), .b(x13), .O(new_n84_));
  nand2  g033(.a(new_n53_), .b(new_n59_), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  oai21  g035(.a(new_n83_), .b(new_n56_), .c(new_n86_), .O(z02));
  nand2  g036(.a(new_n81_), .b(x22), .O(new_n88_));
  nor2   g037(.a(x22), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n67_), .c(new_n70_), .d(new_n58_), .O(new_n90_));
  and2   g039(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x12), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n56_), .c(new_n93_), .O(z03));
  nor3   g043(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  nor3   g044(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x23), .c(new_n98_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x11), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n85_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n56_), .c(new_n101_), .O(z04));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n89_), .c(new_n78_), .d(new_n58_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi21  g054(.a(new_n97_), .b(x24), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x10), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n85_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z05));
  inv1   g058(.a(x22), .O(new_n110_));
  inv1   g059(.a(x23), .O(new_n111_));
  inv1   g060(.a(x24), .O(new_n112_));
  inv1   g061(.a(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n112_), .c(new_n111_), .d(new_n110_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  aoi22  g064(.a(new_n115_), .b(new_n82_), .c(new_n104_), .d(x25), .O(new_n116_));
  nor2   g065(.a(x16), .b(x09), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n85_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n56_), .c(new_n118_), .O(z06));
  oai21  g068(.a(new_n114_), .b(new_n81_), .c(x26), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n82_), .c(new_n111_), .d(new_n110_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(x16), .O(new_n124_));
  nor2   g073(.a(x16), .b(x08), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n85_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n124_), .O(z07));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n103_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n90_), .O(new_n130_));
  aoi21  g079(.a(new_n122_), .b(x27), .c(new_n130_), .O(new_n131_));
  nor2   g080(.a(x16), .b(x07), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n85_), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n56_), .c(new_n133_), .O(z08));
  oai21  g083(.a(new_n129_), .b(new_n90_), .c(x28), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  nor2   g085(.a(x25), .b(x24), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n111_), .O(new_n139_));
  or2    g088(.a(new_n139_), .b(new_n90_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n135_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  nor2   g091(.a(x16), .b(x06), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n85_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z09));
  inv1   g094(.a(x27), .O(new_n146_));
  nor2   g095(.a(x29), .b(x28), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n121_), .c(new_n146_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n97_), .O(new_n149_));
  aoi21  g098(.a(new_n140_), .b(x29), .c(new_n149_), .O(new_n150_));
  nor2   g099(.a(x16), .b(x05), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n85_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n56_), .c(new_n152_), .O(z10));
  nor2   g102(.a(new_n54_), .b(x04), .O(new_n154_));
  nor2   g103(.a(new_n52_), .b(new_n59_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n154_), .c(new_n56_), .O(new_n156_));
  oai21  g105(.a(new_n52_), .b(x30), .c(x29), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(x18), .O(new_n158_));
  oai21  g107(.a(new_n139_), .b(new_n90_), .c(x30), .O(new_n159_));
  nor3   g108(.a(x30), .b(x28), .c(x27), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n121_), .c(new_n96_), .d(new_n95_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(x29), .O(new_n162_));
  nand3  g111(.a(x31), .b(x30), .c(x29), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n162_), .c(x16), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n158_), .c(new_n156_), .O(z11));
  oai21  g115(.a(x16), .b(x03), .c(new_n59_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n53_), .O(new_n168_));
  nor2   g117(.a(x30), .b(x29), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n138_), .c(new_n121_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n97_), .c(x31), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n147_), .c(new_n128_), .d(new_n105_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n168_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n56_), .c(x31), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x29), .O(new_n179_));
  nand3  g128(.a(new_n172_), .b(new_n138_), .c(new_n121_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n97_), .c(x32), .O(new_n181_));
  nor2   g130(.a(x30), .b(x28), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n182_), .c(new_n128_), .d(new_n105_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n181_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(x16), .O(new_n186_));
  inv1   g135(.a(x02), .O(new_n187_));
  aoi21  g136(.a(new_n56_), .b(new_n187_), .c(x18), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n186_), .c(new_n179_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n56_), .c(x31), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x29), .O(new_n192_));
  nand3  g141(.a(new_n183_), .b(new_n182_), .c(new_n128_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n104_), .c(x33), .O(new_n194_));
  nor2   g143(.a(x26), .b(x25), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n172_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n138_), .c(new_n195_), .d(new_n105_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n194_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(x16), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  aoi21  g151(.a(new_n56_), .b(new_n202_), .c(x18), .O(new_n203_));
  nand3  g152(.a(new_n203_), .b(new_n201_), .c(new_n192_), .O(z14));
  oai21  g153(.a(x16), .b(x00), .c(new_n59_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n53_), .O(new_n206_));
  nor2   g155(.a(x27), .b(x26), .O(new_n207_));
  nand4  g156(.a(new_n196_), .b(new_n182_), .c(new_n207_), .d(new_n113_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n104_), .c(x34), .O(new_n209_));
  inv1   g158(.a(x28), .O(new_n210_));
  inv1   g159(.a(x30), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n210_), .c(new_n146_), .d(new_n136_), .O(new_n212_));
  inv1   g161(.a(new_n212_), .O(new_n213_));
  inv1   g162(.a(x34), .O(new_n214_));
  nand4  g163(.a(new_n214_), .b(new_n190_), .c(new_n177_), .d(new_n52_), .O(new_n215_));
  inv1   g164(.a(new_n215_), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n213_), .c(new_n115_), .d(new_n82_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n209_), .c(x29), .O(new_n218_));
  nor2   g167(.a(new_n214_), .b(new_n52_), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n218_), .c(x16), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n206_), .O(z15));
endmodule


