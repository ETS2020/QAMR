// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x23), .b(x18), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand4  g020(.a(new_n71_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n66_), .b(x21), .c(new_n73_), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n61_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n61_), .c(new_n76_), .O(z02));
  nand2  g026(.a(new_n72_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n53_), .c(new_n65_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n61_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(new_n60_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n58_), .O(z03));
  inv1   g033(.a(x23), .O(new_n85_));
  nor2   g034(.a(x21), .b(x20), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n86_), .c(new_n53_), .d(x16), .O(new_n88_));
  oai21  g037(.a(x16), .b(x11), .c(new_n88_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  aoi21  g039(.a(new_n80_), .b(x16), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n85_), .c(new_n90_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  inv1   g042(.a(x22), .O(new_n94_));
  nand3  g043(.a(new_n85_), .b(new_n94_), .c(new_n71_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g046(.a(x20), .b(x19), .O(new_n98_));
  nor2   g047(.a(x18), .b(x17), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n79_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n97_), .c(x16), .O(new_n104_));
  inv1   g053(.a(x10), .O(new_n105_));
  aoi21  g054(.a(new_n61_), .b(new_n105_), .c(x18), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n104_), .O(z05));
  aoi21  g056(.a(x25), .b(x16), .c(x18), .O(new_n108_));
  nor2   g057(.a(x24), .b(x22), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n98_), .c(new_n71_), .d(new_n63_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x25), .O(new_n111_));
  inv1   g060(.a(x25), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n93_), .c(new_n85_), .d(new_n94_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n73_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n111_), .c(new_n61_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x09), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n116_), .c(new_n60_), .O(new_n118_));
  oai21  g067(.a(new_n108_), .b(new_n85_), .c(new_n118_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nor2   g069(.a(new_n113_), .b(new_n72_), .O(new_n121_));
  nor3   g070(.a(x21), .b(x20), .c(x19), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n99_), .d(new_n87_), .O(new_n124_));
  oai21  g073(.a(new_n121_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n61_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z07));
  aoi21  g078(.a(x27), .b(x16), .c(x18), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n109_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n72_), .c(x27), .O(new_n133_));
  nor3   g082(.a(x22), .b(x21), .c(x20), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n101_), .c(new_n134_), .d(new_n53_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n133_), .c(new_n61_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x07), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n137_), .c(new_n60_), .O(new_n139_));
  oai21  g088(.a(new_n130_), .b(new_n85_), .c(new_n139_), .O(z08));
  nand2  g089(.a(new_n136_), .b(x28), .O(new_n141_));
  nor2   g090(.a(x25), .b(x24), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n85_), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n120_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n134_), .c(new_n53_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n141_), .c(new_n61_), .O(new_n148_));
  oai21  g097(.a(x16), .b(x06), .c(new_n60_), .O(new_n149_));
  or2    g098(.a(new_n149_), .b(new_n148_), .O(z09));
  inv1   g099(.a(x27), .O(new_n151_));
  nor2   g100(.a(x29), .b(x28), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n131_), .c(new_n151_), .d(new_n93_), .O(new_n153_));
  nor3   g102(.a(new_n153_), .b(new_n95_), .c(new_n66_), .O(new_n154_));
  aoi21  g103(.a(new_n147_), .b(x29), .c(new_n154_), .O(new_n155_));
  inv1   g104(.a(x05), .O(new_n156_));
  aoi21  g105(.a(new_n61_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n61_), .c(new_n157_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n61_), .c(new_n60_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g110(.a(new_n153_), .b(new_n80_), .c(x30), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n144_), .c(new_n123_), .d(new_n96_), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n162_), .c(new_n61_), .O(new_n165_));
  nor2   g114(.a(x16), .b(x04), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n165_), .c(new_n60_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n161_), .O(z11));
  inv1   g117(.a(x31), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n61_), .c(new_n60_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x23), .O(new_n171_));
  inv1   g120(.a(x28), .O(new_n172_));
  nor2   g121(.a(x27), .b(x26), .O(new_n173_));
  nand4  g122(.a(new_n163_), .b(new_n173_), .c(new_n172_), .d(new_n112_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n110_), .c(x31), .O(new_n175_));
  inv1   g124(.a(new_n66_), .O(new_n176_));
  inv1   g125(.a(new_n102_), .O(new_n177_));
  nor2   g126(.a(x31), .b(x30), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n152_), .O(new_n179_));
  inv1   g128(.a(new_n179_), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n135_), .c(new_n177_), .d(new_n176_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n175_), .c(new_n61_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x03), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n182_), .c(new_n60_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n171_), .O(z12));
  inv1   g134(.a(x32), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n61_), .c(new_n60_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x23), .O(new_n188_));
  nand4  g137(.a(new_n178_), .b(new_n152_), .c(new_n173_), .d(new_n112_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n110_), .c(x32), .O(new_n190_));
  nand2  g139(.a(new_n144_), .b(new_n131_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  nand3  g141(.a(new_n163_), .b(new_n186_), .c(new_n169_), .O(new_n193_));
  inv1   g142(.a(new_n193_), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n192_), .c(new_n177_), .d(new_n176_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n190_), .c(new_n61_), .O(new_n196_));
  nor2   g145(.a(x16), .b(x02), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n196_), .c(new_n60_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n188_), .O(z13));
  nand2  g148(.a(new_n195_), .b(x33), .O(new_n200_));
  nand2  g149(.a(new_n152_), .b(new_n173_), .O(new_n201_));
  inv1   g150(.a(new_n201_), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n202_), .c(new_n178_), .d(new_n121_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n200_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n61_), .b(new_n207_), .c(x18), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(z14));
  inv1   g158(.a(x34), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n61_), .c(new_n60_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(x23), .O(new_n212_));
  nand4  g161(.a(new_n203_), .b(new_n178_), .c(new_n152_), .d(new_n173_), .O(new_n213_));
  nand4  g162(.a(new_n142_), .b(new_n79_), .c(new_n98_), .d(new_n63_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n213_), .c(x34), .O(new_n215_));
  nand2  g164(.a(new_n169_), .b(new_n159_), .O(new_n216_));
  inv1   g165(.a(x33), .O(new_n217_));
  nand3  g166(.a(new_n210_), .b(new_n217_), .c(new_n186_), .O(new_n218_));
  nor2   g167(.a(new_n218_), .b(new_n216_), .O(new_n219_));
  nand3  g168(.a(new_n219_), .b(new_n202_), .c(new_n121_), .O(new_n220_));
  aoi21  g169(.a(new_n220_), .b(new_n215_), .c(new_n61_), .O(new_n221_));
  nor2   g170(.a(x16), .b(x00), .O(new_n222_));
  oai21  g171(.a(new_n222_), .b(new_n221_), .c(new_n60_), .O(new_n223_));
  nand2  g172(.a(new_n223_), .b(new_n212_), .O(z15));
endmodule


