// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  inv1   g000(.a(x11), .O(new_n52_));
  inv1   g001(.a(x18), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x16), .b(x15), .O(new_n55_));
  aoi21  g004(.a(new_n54_), .b(x16), .c(new_n55_), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n52_), .c(new_n53_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  inv1   g008(.a(x17), .O(new_n60_));
  inv1   g009(.a(x19), .O(new_n61_));
  nand3  g010(.a(new_n58_), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  oai21  g011(.a(new_n59_), .b(new_n58_), .c(new_n62_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  inv1   g014(.a(x16), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n65_), .c(x18), .O(new_n67_));
  nand3  g016(.a(new_n67_), .b(new_n64_), .c(x11), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nand4  g018(.a(new_n69_), .b(new_n58_), .c(new_n61_), .d(new_n60_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n62_), .b(x21), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  nor3   g022(.a(new_n73_), .b(x18), .c(new_n52_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n66_), .c(new_n74_), .O(z02));
  nand2  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  nor3   g025(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n66_), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n79_), .c(x11), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n53_), .O(z03));
  inv1   g031(.a(x22), .O(new_n83_));
  inv1   g032(.a(x23), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n83_), .c(new_n69_), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n62_), .O(new_n86_));
  aoi21  g035(.a(new_n78_), .b(x23), .c(new_n86_), .O(new_n87_));
  nor2   g036(.a(x18), .b(new_n52_), .O(new_n88_));
  oai21  g037(.a(new_n87_), .b(new_n66_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x24), .O(new_n90_));
  nor2   g039(.a(x20), .b(x19), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nor2   g041(.a(x24), .b(x23), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n60_), .O(new_n94_));
  oai21  g043(.a(new_n86_), .b(new_n90_), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  inv1   g045(.a(x10), .O(new_n97_));
  aoi21  g046(.a(new_n66_), .b(new_n97_), .c(x18), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(x11), .O(z05));
  nand2  g048(.a(new_n94_), .b(x25), .O(new_n100_));
  inv1   g049(.a(x25), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n90_), .c(new_n84_), .d(new_n83_), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n71_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n100_), .c(new_n66_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x09), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n105_), .c(x11), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n53_), .O(z06));
  inv1   g057(.a(x26), .O(new_n109_));
  nor2   g058(.a(new_n102_), .b(new_n70_), .O(new_n110_));
  nor2   g059(.a(x23), .b(x22), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n111_), .c(new_n71_), .O(new_n113_));
  oai21  g062(.a(new_n110_), .b(new_n109_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x08), .O(new_n116_));
  aoi21  g065(.a(new_n66_), .b(new_n116_), .c(x18), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n115_), .c(x11), .O(z07));
  nor3   g067(.a(x27), .b(x26), .c(x25), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n93_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n78_), .O(new_n121_));
  aoi21  g070(.a(new_n113_), .b(x27), .c(new_n121_), .O(new_n122_));
  nor2   g071(.a(x16), .b(x07), .O(new_n123_));
  nor3   g072(.a(new_n123_), .b(x18), .c(new_n52_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n66_), .c(new_n124_), .O(z08));
  inv1   g074(.a(x28), .O(new_n126_));
  nor3   g075(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nor3   g076(.a(x28), .b(x27), .c(x26), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n127_), .c(new_n77_), .d(new_n59_), .O(new_n129_));
  oai21  g078(.a(new_n121_), .b(new_n126_), .c(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  nor2   g080(.a(x16), .b(x06), .O(new_n132_));
  nor3   g081(.a(new_n132_), .b(x18), .c(new_n52_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z09));
  nand4  g083(.a(new_n111_), .b(new_n91_), .c(new_n69_), .d(new_n60_), .O(new_n135_));
  inv1   g084(.a(x27), .O(new_n136_));
  nor2   g085(.a(x26), .b(x25), .O(new_n137_));
  nor2   g086(.a(x29), .b(x28), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n90_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  aoi21  g089(.a(new_n129_), .b(x29), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x05), .O(new_n142_));
  nor3   g091(.a(new_n142_), .b(x18), .c(new_n52_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n66_), .c(new_n143_), .O(z10));
  oai21  g093(.a(new_n139_), .b(new_n135_), .c(x30), .O(new_n145_));
  nor2   g094(.a(x28), .b(x27), .O(new_n146_));
  nor2   g095(.a(x30), .b(x29), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n112_), .c(new_n86_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n145_), .c(new_n66_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x04), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n151_), .c(x11), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n53_), .O(z11));
  nand2  g103(.a(new_n150_), .b(x31), .O(new_n155_));
  inv1   g104(.a(new_n94_), .O(new_n156_));
  nor2   g105(.a(x31), .b(x30), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n138_), .c(new_n119_), .d(new_n156_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nor2   g109(.a(x16), .b(x03), .O(new_n161_));
  nor3   g110(.a(new_n161_), .b(x18), .c(new_n52_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n160_), .O(z12));
  nor2   g112(.a(x32), .b(x31), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n147_), .c(new_n146_), .d(new_n137_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n94_), .O(new_n166_));
  aoi21  g115(.a(new_n158_), .b(x32), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x02), .O(new_n168_));
  aoi21  g117(.a(new_n66_), .b(new_n168_), .c(x18), .O(new_n169_));
  and2   g118(.a(new_n169_), .b(x11), .O(new_n170_));
  oai21  g119(.a(new_n167_), .b(new_n66_), .c(new_n170_), .O(z13));
  oai21  g120(.a(new_n165_), .b(new_n94_), .c(x33), .O(new_n172_));
  inv1   g121(.a(x29), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n126_), .c(new_n136_), .d(new_n109_), .O(new_n174_));
  inv1   g123(.a(new_n174_), .O(new_n175_));
  inv1   g124(.a(x30), .O(new_n176_));
  inv1   g125(.a(x31), .O(new_n177_));
  inv1   g126(.a(x32), .O(new_n178_));
  inv1   g127(.a(x33), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n178_), .c(new_n177_), .d(new_n176_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n175_), .c(new_n103_), .d(new_n71_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n172_), .c(new_n66_), .O(new_n183_));
  nor2   g132(.a(x16), .b(x01), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n183_), .c(x11), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n53_), .O(z14));
  nand2  g135(.a(new_n182_), .b(x34), .O(new_n187_));
  nor3   g136(.a(x34), .b(x33), .c(x32), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n175_), .c(new_n157_), .d(new_n110_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(x16), .O(new_n191_));
  nor2   g140(.a(x16), .b(x00), .O(new_n192_));
  nor3   g141(.a(new_n192_), .b(x18), .c(new_n52_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n191_), .O(z15));
endmodule


