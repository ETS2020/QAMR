// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  xnor2a g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x16), .b(x15), .O(new_n53_));
  aoi21  g002(.a(new_n52_), .b(x16), .c(new_n53_), .O(new_n54_));
  nand2  g003(.a(x31), .b(x18), .O(new_n55_));
  oai21  g004(.a(new_n54_), .b(x18), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x18), .O(new_n60_));
  nor2   g009(.a(x20), .b(x19), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  oai21  g011(.a(new_n58_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  inv1   g014(.a(x16), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n65_), .c(x18), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n64_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor3   g018(.a(x20), .b(x19), .c(x17), .O(new_n70_));
  inv1   g019(.a(x19), .O(new_n71_));
  nand4  g020(.a(new_n69_), .b(new_n57_), .c(new_n71_), .d(new_n59_), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n69_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(x18), .c(new_n55_), .O(z02));
  nand2  g025(.a(new_n72_), .b(x22), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n58_), .c(new_n57_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(new_n60_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n55_), .O(z03));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n70_), .c(new_n79_), .d(x23), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n66_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n66_), .c(new_n87_), .O(z04));
  nand2  g037(.a(new_n84_), .b(new_n70_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n78_), .c(new_n61_), .d(new_n59_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n90_), .c(new_n66_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x10), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(new_n60_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n55_), .O(z05));
  nand2  g045(.a(new_n92_), .b(x25), .O(new_n97_));
  nor2   g046(.a(x21), .b(x20), .O(new_n98_));
  nor2   g047(.a(x23), .b(x22), .O(new_n99_));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(new_n58_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n97_), .c(new_n66_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n102_), .c(new_n60_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n55_), .O(z06));
  nand2  g054(.a(new_n101_), .b(x26), .O(new_n106_));
  nor3   g055(.a(x26), .b(x25), .c(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n99_), .c(new_n98_), .d(new_n58_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n66_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n109_), .c(new_n60_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n55_), .O(z07));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n91_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n79_), .O(new_n115_));
  aoi21  g064(.a(new_n108_), .b(x27), .c(new_n115_), .O(new_n116_));
  inv1   g065(.a(x07), .O(new_n117_));
  aoi21  g066(.a(new_n66_), .b(new_n117_), .c(x18), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n66_), .c(new_n118_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  inv1   g069(.a(x23), .O(new_n121_));
  inv1   g070(.a(x26), .O(new_n122_));
  nor2   g071(.a(x28), .b(x27), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n100_), .c(new_n122_), .d(new_n121_), .O(new_n124_));
  oai22  g073(.a(new_n124_), .b(new_n79_), .c(new_n115_), .d(new_n120_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x06), .O(new_n127_));
  aoi21  g076(.a(new_n66_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z09));
  oai21  g078(.a(new_n124_), .b(new_n79_), .c(x29), .O(new_n130_));
  nor3   g079(.a(x29), .b(x28), .c(x27), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n107_), .c(new_n84_), .d(new_n70_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n130_), .c(new_n66_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x05), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n133_), .c(new_n60_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n55_), .O(z10));
  nor2   g085(.a(x30), .b(x29), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n123_), .c(new_n107_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n89_), .O(new_n139_));
  aoi21  g088(.a(new_n132_), .b(x30), .c(new_n139_), .O(new_n140_));
  inv1   g089(.a(x04), .O(new_n141_));
  aoi21  g090(.a(new_n66_), .b(new_n141_), .c(x18), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n66_), .c(new_n142_), .O(z11));
  nand4  g092(.a(new_n99_), .b(new_n98_), .c(new_n58_), .d(x16), .O(new_n144_));
  nor2   g093(.a(x27), .b(x26), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n145_), .c(new_n100_), .O(new_n150_));
  oai22  g099(.a(new_n150_), .b(new_n144_), .c(x16), .d(x03), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n60_), .O(new_n152_));
  oai21  g101(.a(new_n139_), .b(new_n66_), .c(new_n60_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x31), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n152_), .O(z12));
  nand2  g104(.a(new_n149_), .b(new_n113_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n92_), .c(x32), .O(new_n157_));
  nor2   g106(.a(x26), .b(x25), .O(new_n158_));
  nand2  g107(.a(new_n123_), .b(new_n158_), .O(new_n159_));
  inv1   g108(.a(x31), .O(new_n160_));
  inv1   g109(.a(x32), .O(new_n161_));
  nand3  g110(.a(new_n137_), .b(new_n161_), .c(new_n160_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n91_), .c(new_n78_), .d(new_n70_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n157_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g115(.a(x02), .O(new_n167_));
  aoi21  g116(.a(new_n66_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z13));
  inv1   g118(.a(x33), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n66_), .c(new_n60_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x31), .O(new_n172_));
  inv1   g121(.a(x25), .O(new_n173_));
  nor2   g122(.a(x32), .b(x30), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n146_), .c(new_n145_), .d(new_n173_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n92_), .c(x33), .O(new_n176_));
  inv1   g125(.a(x22), .O(new_n177_));
  inv1   g126(.a(x24), .O(new_n178_));
  nand4  g127(.a(new_n173_), .b(new_n178_), .c(new_n121_), .d(new_n177_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n72_), .O(new_n180_));
  nand2  g129(.a(new_n146_), .b(new_n145_), .O(new_n181_));
  nor2   g130(.a(x33), .b(x32), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n147_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n181_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n180_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n176_), .c(new_n66_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x01), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n186_), .c(new_n60_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n172_), .O(z14));
  inv1   g138(.a(x34), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n66_), .c(new_n60_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x31), .O(new_n192_));
  nand4  g141(.a(new_n182_), .b(new_n137_), .c(new_n123_), .d(new_n158_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n92_), .c(x34), .O(new_n194_));
  inv1   g143(.a(new_n181_), .O(new_n195_));
  inv1   g144(.a(x30), .O(new_n196_));
  nand2  g145(.a(new_n160_), .b(new_n196_), .O(new_n197_));
  nand3  g146(.a(new_n190_), .b(new_n170_), .c(new_n161_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  nand3  g148(.a(new_n199_), .b(new_n195_), .c(new_n180_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n194_), .c(new_n66_), .O(new_n201_));
  nor2   g150(.a(x16), .b(x00), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(new_n60_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n192_), .O(z15));
endmodule


