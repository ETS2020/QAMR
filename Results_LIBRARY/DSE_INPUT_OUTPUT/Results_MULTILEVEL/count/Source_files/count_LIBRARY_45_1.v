// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x23), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x18), .O(new_n62_));
  inv1   g011(.a(x23), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  nor2   g013(.a(x20), .b(x19), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n63_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n62_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor3   g020(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x23), .c(new_n62_), .O(z02));
  nor3   g027(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n55_), .c(new_n74_), .d(x22), .O(new_n80_));
  inv1   g029(.a(new_n60_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g032(.a(new_n80_), .b(new_n58_), .c(new_n83_), .O(z03));
  inv1   g033(.a(x11), .O(new_n85_));
  nand2  g034(.a(new_n58_), .b(new_n85_), .O(new_n86_));
  nand3  g035(.a(new_n79_), .b(new_n55_), .c(x16), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n86_), .c(new_n60_), .O(z04));
  inv1   g037(.a(x20), .O(new_n89_));
  nor2   g038(.a(x22), .b(x21), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n55_), .c(new_n89_), .O(new_n91_));
  nor2   g040(.a(x24), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n71_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n72_), .c(new_n91_), .d(x24), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n81_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n58_), .c(new_n97_), .O(z05));
  oai21  g047(.a(new_n93_), .b(new_n66_), .c(x25), .O(new_n99_));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n90_), .c(new_n65_), .d(new_n52_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n99_), .c(new_n58_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n102_), .c(new_n63_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n62_), .O(z06));
  nand2  g054(.a(new_n101_), .b(x26), .O(new_n106_));
  nor2   g055(.a(x26), .b(x25), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n92_), .c(new_n73_), .d(new_n55_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n58_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n109_), .c(new_n63_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n62_), .O(z07));
  inv1   g061(.a(x25), .O(new_n113_));
  nor2   g062(.a(x27), .b(x26), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n92_), .c(new_n113_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n74_), .O(new_n116_));
  aoi21  g065(.a(new_n108_), .b(x27), .c(new_n116_), .O(new_n117_));
  nor2   g066(.a(x16), .b(x07), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n81_), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n58_), .c(new_n119_), .O(z08));
  oai21  g069(.a(new_n115_), .b(new_n74_), .c(x28), .O(new_n121_));
  nor3   g070(.a(x28), .b(x27), .c(x26), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n100_), .c(new_n79_), .d(new_n55_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n121_), .c(new_n58_), .O(new_n124_));
  nor2   g073(.a(x16), .b(x06), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n124_), .c(new_n63_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n62_), .O(z09));
  inv1   g076(.a(x26), .O(new_n128_));
  nor2   g077(.a(x28), .b(x27), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n100_), .c(new_n128_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n91_), .c(x29), .O(new_n131_));
  nor3   g080(.a(x26), .b(x25), .c(x24), .O(new_n132_));
  nor3   g081(.a(x29), .b(x28), .c(x27), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n132_), .c(new_n79_), .d(new_n55_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n131_), .c(new_n58_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x05), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n135_), .c(new_n63_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n62_), .O(z10));
  nand2  g087(.a(new_n134_), .b(x30), .O(new_n139_));
  nor3   g088(.a(x27), .b(x26), .c(x25), .O(new_n140_));
  nor3   g089(.a(x30), .b(x29), .c(x28), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n140_), .c(new_n94_), .d(new_n72_), .O(new_n142_));
  and2   g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x04), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n81_), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n58_), .c(new_n145_), .O(z11));
  nand3  g095(.a(new_n92_), .b(new_n72_), .c(new_n71_), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  nor2   g097(.a(x31), .b(x30), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n148_), .c(new_n140_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  aoi21  g100(.a(new_n142_), .b(x31), .c(new_n151_), .O(new_n152_));
  nor2   g101(.a(x16), .b(x03), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n81_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n58_), .c(new_n154_), .O(z12));
  oai21  g104(.a(new_n150_), .b(new_n147_), .c(x32), .O(new_n156_));
  inv1   g105(.a(new_n101_), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nor2   g107(.a(x32), .b(x31), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n158_), .c(new_n122_), .d(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  nor2   g111(.a(x16), .b(x02), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n81_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z13));
  nand3  g114(.a(new_n159_), .b(new_n158_), .c(new_n122_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n101_), .c(x33), .O(new_n167_));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n149_), .c(new_n148_), .d(new_n114_), .O(new_n169_));
  or2    g118(.a(new_n169_), .b(new_n101_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n167_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  nor2   g121(.a(x16), .b(x01), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n81_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z14));
  oai21  g124(.a(new_n169_), .b(new_n101_), .c(x34), .O(new_n176_));
  nor2   g125(.a(x34), .b(x33), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n159_), .c(new_n158_), .d(new_n129_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n108_), .c(new_n176_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(x16), .O(new_n180_));
  nor2   g129(.a(x16), .b(x00), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n81_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z15));
endmodule


