// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nand2  g002(.a(x20), .b(x18), .O(new_n54_));
  nand3  g003(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  inv1   g004(.a(x18), .O(new_n56_));
  nand3  g005(.a(x19), .b(new_n56_), .c(x17), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x16), .O(new_n59_));
  inv1   g008(.a(x20), .O(new_n60_));
  nor3   g009(.a(x18), .b(x16), .c(x15), .O(new_n61_));
  aoi21  g010(.a(new_n60_), .b(x18), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n59_), .O(z00));
  inv1   g012(.a(x16), .O(new_n64_));
  nor2   g013(.a(x19), .b(x17), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  aoi21  g016(.a(new_n66_), .b(x20), .c(new_n67_), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  aoi21  g018(.a(new_n64_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n64_), .c(new_n70_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(new_n67_), .b(new_n72_), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n65_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n73_), .c(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n64_), .b(new_n78_), .c(x18), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n77_), .O(z02));
  nor3   g029(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  aoi22  g030(.a(new_n81_), .b(new_n65_), .c(new_n75_), .d(x22), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n64_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n64_), .c(new_n84_), .O(z03));
  nand2  g034(.a(new_n65_), .b(x16), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  nor3   g036(.a(x23), .b(x22), .c(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n60_), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n65_), .c(new_n60_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(x23), .c(x16), .O(new_n94_));
  oai21  g043(.a(x16), .b(x11), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n56_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n91_), .O(z04));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n87_), .c(new_n92_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n56_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n60_), .O(new_n101_));
  nor2   g050(.a(x20), .b(x19), .O(new_n102_));
  nor2   g051(.a(x23), .b(x22), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n72_), .d(new_n52_), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(x24), .c(x16), .O(new_n105_));
  oai21  g054(.a(x16), .b(x10), .c(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n56_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n101_), .O(z05));
  nor2   g057(.a(x17), .b(new_n64_), .O(new_n109_));
  nor2   g058(.a(x21), .b(x19), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n103_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n56_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n60_), .O(new_n114_));
  nand4  g063(.a(new_n98_), .b(new_n92_), .c(new_n102_), .d(new_n52_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(x25), .c(x16), .O(new_n116_));
  oai21  g065(.a(x16), .b(x09), .c(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n56_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n114_), .O(z06));
  nand4  g068(.a(new_n111_), .b(new_n103_), .c(new_n74_), .d(new_n65_), .O(new_n120_));
  inv1   g069(.a(x24), .O(new_n121_));
  nor2   g070(.a(x26), .b(x25), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n103_), .c(new_n121_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n75_), .O(new_n124_));
  aoi21  g073(.a(new_n120_), .b(x26), .c(new_n124_), .O(new_n125_));
  inv1   g074(.a(x08), .O(new_n126_));
  aoi21  g075(.a(new_n64_), .b(new_n126_), .c(x18), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n64_), .c(new_n127_), .O(z07));
  nand3  g077(.a(new_n109_), .b(new_n92_), .c(new_n53_), .O(new_n129_));
  inv1   g078(.a(x25), .O(new_n130_));
  nor2   g079(.a(x27), .b(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n98_), .c(new_n130_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n129_), .c(new_n56_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n60_), .O(new_n134_));
  oai21  g083(.a(new_n123_), .b(new_n75_), .c(x27), .O(new_n135_));
  inv1   g084(.a(x07), .O(new_n136_));
  nand2  g085(.a(new_n64_), .b(new_n136_), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n64_), .c(new_n137_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n56_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n134_), .O(z08));
  oai21  g089(.a(new_n132_), .b(new_n93_), .c(x28), .O(new_n141_));
  nor3   g090(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n81_), .d(new_n65_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  inv1   g095(.a(x06), .O(new_n147_));
  aoi21  g096(.a(new_n64_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z09));
  nor3   g098(.a(x26), .b(x25), .c(x24), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n89_), .c(new_n56_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n60_), .O(new_n154_));
  nand3  g103(.a(new_n144_), .b(x29), .c(x16), .O(new_n155_));
  inv1   g104(.a(x05), .O(new_n156_));
  nand2  g105(.a(new_n64_), .b(new_n156_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n56_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n154_), .O(z10));
  nor2   g109(.a(x28), .b(x27), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n161_), .c(new_n122_), .d(new_n121_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n89_), .c(new_n56_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n60_), .O(new_n165_));
  nand4  g114(.a(new_n151_), .b(new_n150_), .c(new_n88_), .d(new_n67_), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(x30), .c(x16), .O(new_n167_));
  inv1   g116(.a(x04), .O(new_n168_));
  nand2  g117(.a(new_n64_), .b(new_n168_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n56_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n165_), .O(z11));
  nor2   g121(.a(x29), .b(x28), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n173_), .c(new_n131_), .d(new_n130_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n99_), .c(new_n56_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n60_), .O(new_n177_));
  oai21  g126(.a(new_n163_), .b(new_n104_), .c(x31), .O(new_n178_));
  inv1   g127(.a(x03), .O(new_n179_));
  nand2  g128(.a(new_n64_), .b(new_n179_), .O(new_n180_));
  oai21  g129(.a(new_n178_), .b(new_n64_), .c(new_n180_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n56_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n177_), .O(z12));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n162_), .c(new_n161_), .d(new_n122_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n99_), .c(new_n56_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n60_), .O(new_n187_));
  oai21  g136(.a(new_n175_), .b(new_n115_), .c(x32), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  nand2  g138(.a(new_n64_), .b(new_n189_), .O(new_n190_));
  oai21  g139(.a(new_n188_), .b(new_n64_), .c(new_n190_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n56_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n187_), .O(z13));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n174_), .c(new_n173_), .d(new_n131_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n112_), .c(new_n56_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n60_), .O(new_n197_));
  oai21  g146(.a(new_n185_), .b(new_n115_), .c(x33), .O(new_n198_));
  inv1   g147(.a(x01), .O(new_n199_));
  nand2  g148(.a(new_n64_), .b(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n198_), .b(new_n64_), .c(new_n200_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n56_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n197_), .O(z14));
  nor3   g152(.a(x34), .b(x33), .c(x32), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n174_), .c(new_n173_), .d(new_n131_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n112_), .c(new_n56_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n60_), .O(new_n207_));
  oai21  g156(.a(new_n195_), .b(new_n120_), .c(x34), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  nand2  g158(.a(new_n64_), .b(new_n209_), .O(new_n210_));
  oai21  g159(.a(new_n208_), .b(new_n64_), .c(new_n210_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n56_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n207_), .O(z15));
endmodule


