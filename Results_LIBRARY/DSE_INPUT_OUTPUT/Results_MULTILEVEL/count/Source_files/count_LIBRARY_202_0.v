// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x31), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x16), .b(x15), .O(new_n55_));
  aoi21  g004(.a(new_n54_), .b(x16), .c(new_n55_), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nor2   g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nand2  g012(.a(x31), .b(new_n52_), .O(new_n64_));
  nor2   g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(new_n69_));
  oai21  g018(.a(new_n61_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  aoi21  g020(.a(new_n70_), .b(x16), .c(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n53_), .c(new_n52_), .O(z02));
  inv1   g022(.a(x16), .O(new_n74_));
  nand2  g023(.a(new_n69_), .b(x22), .O(new_n75_));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n59_), .c(new_n58_), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  oai21  g028(.a(new_n79_), .b(new_n78_), .c(x31), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n52_), .O(z03));
  nor3   g030(.a(x23), .b(x22), .c(x21), .O(new_n82_));
  aoi22  g031(.a(new_n82_), .b(new_n61_), .c(new_n77_), .d(x23), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n74_), .c(new_n85_), .O(z04));
  nand2  g035(.a(new_n82_), .b(new_n61_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x24), .O(new_n88_));
  inv1   g037(.a(x17), .O(new_n89_));
  nor2   g038(.a(x20), .b(x19), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n76_), .c(new_n90_), .d(new_n89_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n88_), .c(new_n74_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x10), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(x31), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n52_), .O(z05));
  nand2  g045(.a(new_n92_), .b(x25), .O(new_n97_));
  nor2   g046(.a(x23), .b(x22), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n98_), .c(new_n68_), .d(new_n59_), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n97_), .c(new_n74_), .O(new_n101_));
  nor2   g050(.a(x16), .b(x09), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n101_), .c(x31), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n52_), .O(z06));
  nand2  g053(.a(new_n100_), .b(x26), .O(new_n105_));
  nor3   g054(.a(x26), .b(x25), .c(x24), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n98_), .c(new_n68_), .d(new_n59_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n105_), .c(new_n74_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x08), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n108_), .c(x31), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n52_), .O(z07));
  inv1   g060(.a(x25), .O(new_n112_));
  nor2   g061(.a(x27), .b(x26), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n91_), .c(new_n112_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n77_), .O(new_n115_));
  aoi21  g064(.a(new_n107_), .b(x27), .c(new_n115_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n64_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n74_), .c(new_n118_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  nor2   g069(.a(new_n115_), .b(new_n120_), .O(new_n121_));
  inv1   g070(.a(x23), .O(new_n122_));
  inv1   g071(.a(x26), .O(new_n123_));
  nor2   g072(.a(x28), .b(x27), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n99_), .c(new_n123_), .d(new_n122_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n77_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n121_), .c(x16), .O(new_n127_));
  nor2   g076(.a(x16), .b(x06), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n64_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z09));
  oai21  g079(.a(new_n125_), .b(new_n77_), .c(x29), .O(new_n131_));
  nor3   g080(.a(x29), .b(x28), .c(x27), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n106_), .c(new_n82_), .d(new_n61_), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n131_), .c(new_n74_), .O(new_n134_));
  nor2   g083(.a(x16), .b(x05), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n134_), .c(x31), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n52_), .O(z10));
  nor2   g086(.a(x30), .b(x29), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n124_), .c(new_n106_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n87_), .O(new_n140_));
  aoi21  g089(.a(new_n133_), .b(x30), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x04), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n64_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n74_), .c(new_n143_), .O(z11));
  inv1   g093(.a(x03), .O(new_n145_));
  nand2  g094(.a(new_n74_), .b(new_n145_), .O(new_n146_));
  oai21  g095(.a(new_n140_), .b(new_n74_), .c(new_n146_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x31), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n52_), .O(z12));
  nand2  g098(.a(x32), .b(x16), .O(new_n150_));
  inv1   g099(.a(x02), .O(new_n151_));
  nand2  g100(.a(new_n74_), .b(new_n151_), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n150_), .c(x31), .d(new_n52_), .O(z13));
  nor2   g102(.a(x16), .b(x01), .O(new_n154_));
  and2   g103(.a(x33), .b(x16), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n154_), .c(x31), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n52_), .O(z14));
  nor2   g106(.a(x16), .b(x00), .O(new_n158_));
  and2   g107(.a(x34), .b(x16), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n158_), .c(x31), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n52_), .O(z15));
endmodule


