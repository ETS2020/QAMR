// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x22), .c(new_n52_), .O(z00));
  inv1   g008(.a(x22), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n55_), .b(x20), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n60_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n52_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  inv1   g018(.a(x17), .O(new_n70_));
  inv1   g019(.a(x19), .O(new_n71_));
  inv1   g020(.a(x20), .O(new_n72_));
  nand4  g021(.a(new_n69_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n73_));
  oai21  g022(.a(new_n63_), .b(new_n69_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x22), .c(new_n52_), .O(z02));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  nor2   g027(.a(x17), .b(new_n61_), .O(new_n79_));
  nor3   g028(.a(x21), .b(x20), .c(x19), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(x22), .c(new_n52_), .O(z03));
  nor2   g031(.a(x23), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n54_), .c(new_n72_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n73_), .b(x23), .c(new_n85_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nand2  g036(.a(new_n60_), .b(new_n52_), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g038(.a(new_n86_), .b(new_n61_), .c(new_n89_), .O(z04));
  nand2  g039(.a(new_n84_), .b(x24), .O(new_n91_));
  nor3   g040(.a(x24), .b(x23), .c(x21), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n63_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n61_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n94_), .c(new_n60_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  nand2  g046(.a(new_n93_), .b(x25), .O(new_n98_));
  nor2   g047(.a(x20), .b(x19), .O(new_n99_));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n83_), .c(new_n99_), .d(new_n70_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n98_), .c(new_n61_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n102_), .c(new_n60_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n52_), .O(z06));
  inv1   g054(.a(x24), .O(new_n106_));
  nor2   g055(.a(x23), .b(x22), .O(new_n107_));
  nor2   g056(.a(x26), .b(x25), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n73_), .O(new_n110_));
  aoi21  g059(.a(new_n101_), .b(x26), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x08), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n61_), .c(new_n113_), .O(z07));
  inv1   g063(.a(x27), .O(new_n115_));
  inv1   g064(.a(new_n73_), .O(new_n116_));
  inv1   g065(.a(x23), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  inv1   g067(.a(x26), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n118_), .c(new_n106_), .d(new_n117_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n116_), .c(new_n115_), .O(new_n122_));
  nor2   g071(.a(x27), .b(x26), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n118_), .c(new_n106_), .d(new_n117_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n73_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n122_), .c(x16), .O(new_n126_));
  nor2   g075(.a(x16), .b(x07), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n88_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z08));
  oai21  g078(.a(new_n124_), .b(new_n73_), .c(x28), .O(new_n130_));
  nor3   g079(.a(x28), .b(x27), .c(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n100_), .c(new_n85_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n130_), .c(new_n61_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x06), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n133_), .c(new_n60_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n52_), .O(z09));
  nand2  g085(.a(new_n132_), .b(x29), .O(new_n137_));
  nor2   g086(.a(x29), .b(x28), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n108_), .c(new_n115_), .d(new_n106_), .O(new_n139_));
  or2    g088(.a(new_n139_), .b(new_n84_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  nor2   g091(.a(x16), .b(x05), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n88_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z10));
  oai21  g094(.a(new_n139_), .b(new_n84_), .c(x30), .O(new_n146_));
  nor3   g095(.a(x27), .b(x26), .c(x25), .O(new_n147_));
  nor3   g096(.a(x30), .b(x29), .c(x28), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n92_), .d(new_n63_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n146_), .c(new_n61_), .O(new_n150_));
  nor2   g099(.a(x16), .b(x04), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n150_), .c(new_n60_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n52_), .O(z11));
  nor2   g102(.a(x31), .b(x30), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n138_), .c(new_n147_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n93_), .O(new_n156_));
  aoi21  g105(.a(new_n149_), .b(x31), .c(new_n156_), .O(new_n157_));
  nor2   g106(.a(x16), .b(x03), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n88_), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n61_), .c(new_n159_), .O(z12));
  oai21  g109(.a(new_n155_), .b(new_n93_), .c(x32), .O(new_n161_));
  nor2   g110(.a(x28), .b(x27), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nor2   g112(.a(x32), .b(x31), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n163_), .c(new_n162_), .d(new_n119_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n101_), .c(new_n161_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x16), .O(new_n167_));
  nor2   g116(.a(x16), .b(x02), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n88_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(z13));
  oai21  g119(.a(new_n165_), .b(new_n101_), .c(x33), .O(new_n171_));
  nand4  g120(.a(new_n118_), .b(new_n106_), .c(new_n117_), .d(new_n69_), .O(new_n172_));
  inv1   g121(.a(new_n172_), .O(new_n173_));
  inv1   g122(.a(x28), .O(new_n174_));
  inv1   g123(.a(x29), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n115_), .d(new_n119_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  inv1   g126(.a(x30), .O(new_n178_));
  inv1   g127(.a(x31), .O(new_n179_));
  inv1   g128(.a(x32), .O(new_n180_));
  inv1   g129(.a(x33), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n180_), .c(new_n179_), .d(new_n178_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n177_), .c(new_n173_), .d(new_n63_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n171_), .c(new_n61_), .O(new_n185_));
  nor2   g134(.a(x16), .b(x01), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n185_), .c(new_n60_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n52_), .O(z14));
  nor2   g137(.a(x33), .b(x32), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n154_), .c(new_n138_), .d(new_n123_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n101_), .c(x34), .O(new_n191_));
  nand4  g140(.a(new_n178_), .b(new_n175_), .c(new_n174_), .d(new_n115_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  inv1   g142(.a(x34), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n181_), .c(new_n180_), .d(new_n179_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n193_), .c(new_n121_), .d(new_n116_), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n191_), .c(new_n61_), .O(new_n198_));
  nor2   g147(.a(x16), .b(x00), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n198_), .c(new_n60_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n52_), .O(z15));
endmodule


