// Benchmark "FAU" written by ABC on Thu Aug 13 14:56:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor3   g003(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  nor3   g005(.a(x18), .b(x16), .c(x15), .O(new_n57_));
  aoi21  g006(.a(x18), .b(x17), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n56_), .O(z00));
  aoi21  g008(.a(x20), .b(x16), .c(x18), .O(new_n60_));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(x20), .b(x19), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  oai21  g017(.a(new_n60_), .b(new_n52_), .c(new_n68_), .O(z01));
  aoi21  g018(.a(x21), .b(x16), .c(x18), .O(new_n70_));
  nand2  g019(.a(new_n64_), .b(new_n53_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x21), .O(new_n72_));
  nor2   g021(.a(x19), .b(x17), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n72_), .c(new_n62_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n76_), .c(new_n61_), .O(new_n78_));
  oai21  g027(.a(new_n70_), .b(new_n52_), .c(new_n78_), .O(z02));
  aoi21  g028(.a(x22), .b(x16), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n74_), .b(new_n53_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x22), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n73_), .c(new_n64_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n62_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(new_n61_), .O(new_n87_));
  oai21  g036(.a(new_n80_), .b(new_n52_), .c(new_n87_), .O(z03));
  aoi21  g037(.a(x23), .b(x16), .c(x18), .O(new_n89_));
  inv1   g038(.a(new_n83_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n71_), .c(x23), .O(new_n91_));
  inv1   g040(.a(x21), .O(new_n92_));
  nor3   g041(.a(x20), .b(x19), .c(x17), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n91_), .c(new_n62_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n61_), .O(new_n98_));
  oai21  g047(.a(new_n89_), .b(new_n52_), .c(new_n98_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n62_), .c(new_n61_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x17), .O(new_n102_));
  nand2  g051(.a(new_n94_), .b(new_n92_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n71_), .c(x24), .O(new_n104_));
  inv1   g053(.a(x22), .O(new_n105_));
  inv1   g054(.a(x23), .O(new_n106_));
  nand4  g055(.a(new_n100_), .b(new_n106_), .c(new_n105_), .d(new_n92_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n93_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n104_), .c(new_n62_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x10), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n110_), .c(new_n61_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n102_), .O(z05));
  aoi21  g062(.a(x25), .b(x16), .c(x18), .O(new_n114_));
  nor2   g063(.a(x24), .b(x23), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n74_), .c(new_n105_), .d(new_n53_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x25), .O(new_n117_));
  nor2   g066(.a(x25), .b(x24), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n94_), .c(new_n74_), .d(new_n73_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n62_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x09), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n120_), .c(new_n61_), .O(new_n122_));
  oai21  g071(.a(new_n114_), .b(new_n52_), .c(new_n122_), .O(z06));
  inv1   g072(.a(x26), .O(new_n124_));
  inv1   g073(.a(new_n119_), .O(new_n125_));
  nand4  g074(.a(new_n74_), .b(new_n53_), .c(new_n61_), .d(new_n52_), .O(new_n126_));
  nor3   g075(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n94_), .O(new_n128_));
  oai22  g077(.a(new_n128_), .b(new_n126_), .c(new_n125_), .d(new_n124_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g079(.a(x08), .O(new_n131_));
  aoi21  g080(.a(new_n62_), .b(new_n131_), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(z07));
  inv1   g082(.a(x27), .O(new_n134_));
  nor2   g083(.a(new_n128_), .b(new_n75_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor3   g085(.a(x27), .b(x26), .c(x25), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n115_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n84_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n136_), .c(x16), .O(new_n140_));
  inv1   g089(.a(x07), .O(new_n141_));
  aoi21  g090(.a(new_n62_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z08));
  inv1   g092(.a(x28), .O(new_n144_));
  nor2   g093(.a(new_n90_), .b(x20), .O(new_n145_));
  nor3   g094(.a(x25), .b(x24), .c(x23), .O(new_n146_));
  nor3   g095(.a(x28), .b(x27), .c(x26), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n73_), .O(new_n148_));
  oai21  g097(.a(new_n139_), .b(new_n144_), .c(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x06), .O(new_n151_));
  aoi21  g100(.a(new_n62_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z09));
  nor2   g102(.a(x29), .b(x28), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n127_), .c(new_n134_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n95_), .O(new_n156_));
  aoi21  g105(.a(new_n148_), .b(x29), .c(new_n156_), .O(new_n157_));
  inv1   g106(.a(x05), .O(new_n158_));
  aoi21  g107(.a(new_n62_), .b(new_n158_), .c(x18), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n62_), .c(new_n159_), .O(z10));
  nor2   g109(.a(new_n103_), .b(new_n65_), .O(new_n161_));
  nand4  g110(.a(new_n154_), .b(new_n127_), .c(new_n161_), .d(new_n134_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n127_), .c(new_n144_), .d(new_n134_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n95_), .O(new_n165_));
  aoi21  g114(.a(new_n162_), .b(x30), .c(new_n165_), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n62_), .b(new_n167_), .c(x18), .O(new_n168_));
  oai21  g117(.a(new_n166_), .b(new_n62_), .c(new_n168_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n62_), .c(new_n61_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x17), .O(new_n172_));
  inv1   g121(.a(x25), .O(new_n173_));
  nor2   g122(.a(x27), .b(x26), .O(new_n174_));
  nand4  g123(.a(new_n163_), .b(new_n174_), .c(new_n144_), .d(new_n173_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n116_), .c(x31), .O(new_n176_));
  inv1   g125(.a(x29), .O(new_n177_));
  inv1   g126(.a(x30), .O(new_n178_));
  nand4  g127(.a(new_n170_), .b(new_n178_), .c(new_n177_), .d(new_n144_), .O(new_n179_));
  inv1   g128(.a(new_n179_), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n137_), .c(new_n108_), .d(new_n93_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n176_), .c(new_n62_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x03), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n182_), .c(new_n61_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n172_), .O(z12));
  inv1   g134(.a(x32), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n62_), .c(new_n61_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x17), .O(new_n188_));
  nor2   g137(.a(x31), .b(x30), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n154_), .c(new_n174_), .d(new_n173_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n116_), .c(x32), .O(new_n191_));
  nand4  g140(.a(new_n144_), .b(new_n134_), .c(new_n124_), .d(new_n173_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand4  g142(.a(new_n186_), .b(new_n170_), .c(new_n178_), .d(new_n177_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n193_), .c(new_n108_), .d(new_n93_), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n191_), .c(new_n62_), .O(new_n197_));
  nor2   g146(.a(x16), .b(x02), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n197_), .c(new_n61_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n188_), .O(z13));
  nand2  g149(.a(new_n196_), .b(x33), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n189_), .c(new_n154_), .d(new_n174_), .O(new_n203_));
  or2    g152(.a(new_n203_), .b(new_n119_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n201_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n62_), .b(new_n207_), .c(x18), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(z14));
  oai21  g158(.a(new_n203_), .b(new_n119_), .c(x34), .O(new_n210_));
  nand2  g159(.a(new_n154_), .b(new_n174_), .O(new_n211_));
  inv1   g160(.a(new_n211_), .O(new_n212_));
  nor3   g161(.a(x34), .b(x33), .c(x32), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n212_), .c(new_n189_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n119_), .c(new_n210_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n62_), .b(new_n217_), .c(x18), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(z15));
endmodule


