// Benchmark "FAU" written by ABC on Thu Aug 13 14:55:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x26), .c(new_n52_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n55_), .b(x20), .c(new_n61_), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  inv1   g012(.a(x26), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai21  g015(.a(new_n62_), .b(new_n60_), .c(new_n66_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  inv1   g017(.a(x17), .O(new_n69_));
  inv1   g018(.a(x19), .O(new_n70_));
  inv1   g019(.a(x20), .O(new_n71_));
  nand4  g020(.a(new_n68_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n72_));
  oai21  g021(.a(new_n61_), .b(new_n68_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(x26), .c(new_n52_), .O(z02));
  nor3   g025(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  aoi22  g026(.a(new_n77_), .b(new_n54_), .c(new_n72_), .d(x22), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n65_), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n60_), .c(new_n80_), .O(z03));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n54_), .c(new_n71_), .O(new_n83_));
  nor2   g032(.a(x20), .b(x19), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n69_), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  aoi21  g037(.a(new_n83_), .b(x23), .c(new_n88_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n60_), .c(new_n91_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n82_), .c(new_n84_), .d(new_n69_), .O(new_n95_));
  oai21  g044(.a(new_n88_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  nor2   g046(.a(x16), .b(x10), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n65_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n97_), .O(z05));
  nand2  g049(.a(new_n95_), .b(x25), .O(new_n101_));
  nor2   g050(.a(x21), .b(x20), .O(new_n102_));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n86_), .c(new_n102_), .d(new_n54_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n101_), .c(new_n60_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x09), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n105_), .c(new_n64_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n52_), .O(z06));
  nand3  g057(.a(new_n84_), .b(new_n69_), .c(x16), .O(new_n109_));
  nor3   g058(.a(x25), .b(x24), .c(x23), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n82_), .O(new_n111_));
  oai22  g060(.a(new_n111_), .b(new_n109_), .c(x16), .d(x08), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n64_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n52_), .O(z07));
  nand2  g063(.a(new_n104_), .b(x27), .O(new_n115_));
  inv1   g064(.a(new_n72_), .O(new_n116_));
  nor3   g065(.a(x27), .b(x25), .c(x24), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n86_), .c(new_n116_), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n115_), .c(new_n60_), .O(new_n119_));
  nor2   g068(.a(x16), .b(x07), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n119_), .c(new_n64_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n52_), .O(z08));
  nor2   g071(.a(x27), .b(x25), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n86_), .c(new_n93_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n72_), .c(x28), .O(new_n125_));
  nor3   g074(.a(x28), .b(x27), .c(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n94_), .c(new_n77_), .d(new_n54_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n60_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x06), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n128_), .c(new_n64_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n52_), .O(z09));
  inv1   g080(.a(x25), .O(new_n132_));
  inv1   g081(.a(x27), .O(new_n133_));
  inv1   g082(.a(x28), .O(new_n134_));
  nand4  g083(.a(new_n94_), .b(new_n134_), .c(new_n133_), .d(new_n132_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n83_), .c(x29), .O(new_n136_));
  nor3   g085(.a(x29), .b(x28), .c(x27), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n110_), .c(new_n77_), .d(new_n54_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n136_), .c(new_n60_), .O(new_n139_));
  nor2   g088(.a(x16), .b(x05), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n139_), .c(new_n64_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n52_), .O(z10));
  nand2  g091(.a(new_n138_), .b(x30), .O(new_n143_));
  nor2   g092(.a(x30), .b(x29), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n117_), .c(new_n88_), .d(new_n134_), .O(new_n145_));
  and2   g094(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nor2   g095(.a(x16), .b(x04), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n65_), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n60_), .c(new_n148_), .O(z11));
  nand3  g098(.a(new_n86_), .b(new_n61_), .c(new_n68_), .O(new_n150_));
  nor2   g099(.a(x29), .b(x28), .O(new_n151_));
  nor2   g100(.a(x31), .b(x30), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n151_), .c(new_n117_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  aoi21  g103(.a(new_n145_), .b(x31), .c(new_n154_), .O(new_n155_));
  nor2   g104(.a(x16), .b(x03), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n65_), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n60_), .c(new_n157_), .O(z12));
  oai21  g107(.a(new_n153_), .b(new_n150_), .c(x32), .O(new_n159_));
  nor2   g108(.a(x32), .b(x31), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n144_), .c(new_n126_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n95_), .c(new_n159_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  nor2   g112(.a(x16), .b(x02), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n65_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z13));
  oai21  g115(.a(new_n161_), .b(new_n95_), .c(x33), .O(new_n167_));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n152_), .c(new_n151_), .d(new_n123_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n95_), .c(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  nor2   g120(.a(x16), .b(x01), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n65_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z14));
  oai21  g123(.a(new_n169_), .b(new_n95_), .c(x34), .O(new_n175_));
  inv1   g124(.a(x22), .O(new_n176_));
  inv1   g125(.a(x23), .O(new_n177_));
  nand4  g126(.a(new_n132_), .b(new_n93_), .c(new_n177_), .d(new_n176_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  inv1   g128(.a(x29), .O(new_n180_));
  inv1   g129(.a(x30), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n180_), .c(new_n134_), .d(new_n133_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  inv1   g132(.a(x31), .O(new_n184_));
  inv1   g133(.a(x32), .O(new_n185_));
  inv1   g134(.a(x33), .O(new_n186_));
  inv1   g135(.a(x34), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n186_), .c(new_n185_), .d(new_n184_), .O(new_n188_));
  inv1   g137(.a(new_n188_), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n183_), .c(new_n179_), .d(new_n116_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n175_), .c(new_n60_), .O(new_n191_));
  nor2   g140(.a(x16), .b(x00), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n191_), .c(new_n64_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n52_), .O(z15));
endmodule


