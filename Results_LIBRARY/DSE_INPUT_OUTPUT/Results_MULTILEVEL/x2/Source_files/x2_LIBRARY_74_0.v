// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x4), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  xnor2a g04(.a(x9), .b(x7), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(z1));
  inv1   g06(.a(x7), .O(new_n24_));
  nand4  g07(.a(new_n18_), .b(new_n21_), .c(new_n24_), .d(x4), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(z2));
  inv1   g09(.a(x0), .O(new_n27_));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n18_), .c(new_n24_), .d(new_n27_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x8), .O(new_n30_));
  oai21  g13(.a(x8), .b(new_n20_), .c(new_n30_), .O(z3));
  nand3  g14(.a(x7), .b(x6), .c(x4), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand3  g16(.a(new_n18_), .b(x7), .c(x6), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x8), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n33_), .O(z4));
  inv1   g19(.a(x1), .O(new_n37_));
  nand2  g20(.a(new_n21_), .b(new_n24_), .O(new_n38_));
  nand2  g21(.a(x9), .b(x8), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n24_), .c(new_n38_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x2), .c(new_n37_), .d(new_n27_), .O(new_n41_));
  nand2  g24(.a(new_n18_), .b(new_n21_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(x7), .c(x6), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x4), .O(new_n44_));
  nand3  g27(.a(new_n18_), .b(x7), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x8), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n20_), .O(new_n47_));
  inv1   g30(.a(x6), .O(new_n48_));
  nand2  g31(.a(new_n38_), .b(new_n48_), .O(new_n49_));
  nand4  g32(.a(x9), .b(x8), .c(new_n24_), .d(x5), .O(new_n50_));
  nand3  g33(.a(new_n18_), .b(new_n21_), .c(x7), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  inv1   g35(.a(new_n52_), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n47_), .c(new_n44_), .d(new_n41_), .O(z5));
  oai21  g37(.a(x8), .b(x4), .c(new_n48_), .O(new_n55_));
  inv1   g38(.a(x5), .O(new_n56_));
  oai22  g39(.a(new_n42_), .b(new_n20_), .c(new_n39_), .d(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n24_), .O(new_n58_));
  nand2  g41(.a(x9), .b(new_n21_), .O(new_n59_));
  nand3  g42(.a(new_n18_), .b(x8), .c(x3), .O(new_n60_));
  aoi21  g43(.a(new_n60_), .b(new_n59_), .c(new_n20_), .O(new_n61_));
  nand2  g44(.a(new_n37_), .b(new_n27_), .O(new_n62_));
  inv1   g45(.a(x2), .O(new_n63_));
  nand3  g46(.a(x9), .b(x8), .c(new_n63_), .O(new_n64_));
  nor2   g47(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n61_), .c(x7), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n58_), .c(new_n55_), .O(z6));
endmodule


