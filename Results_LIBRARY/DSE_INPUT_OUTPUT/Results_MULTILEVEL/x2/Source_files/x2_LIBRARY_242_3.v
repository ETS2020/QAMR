// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_;
  nand2  g00(.a(x5), .b(x3), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand3  g02(.a(new_n19_), .b(x8), .c(x7), .O(new_n20_));
  and2   g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(new_n18_), .O(new_n23_));
  xnor2a g06(.a(x9), .b(x7), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n22_), .c(new_n23_), .O(z1));
  inv1   g08(.a(x7), .O(new_n26_));
  nand4  g09(.a(new_n18_), .b(new_n19_), .c(new_n22_), .d(new_n26_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  nand3  g12(.a(new_n19_), .b(x8), .c(new_n26_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nor2   g14(.a(x2), .b(x1), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(new_n18_), .d(new_n29_), .O(z3));
  aoi21  g16(.a(x9), .b(x8), .c(new_n26_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x6), .c(new_n23_), .O(z4));
  inv1   g18(.a(x1), .O(new_n36_));
  nand3  g19(.a(x9), .b(x8), .c(x7), .O(new_n37_));
  oai21  g20(.a(x8), .b(x7), .c(new_n37_), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(x2), .c(new_n36_), .d(new_n29_), .O(new_n39_));
  nand2  g22(.a(new_n19_), .b(new_n22_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  inv1   g25(.a(x5), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand4  g27(.a(new_n19_), .b(x7), .c(new_n44_), .d(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x6), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  inv1   g30(.a(x3), .O(new_n48_));
  inv1   g31(.a(x6), .O(new_n49_));
  nand2  g32(.a(new_n22_), .b(new_n49_), .O(new_n50_));
  nand3  g33(.a(x9), .b(x8), .c(x5), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n50_), .c(x7), .O(new_n52_));
  aoi21  g35(.a(x9), .b(new_n26_), .c(x6), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n47_), .c(new_n42_), .O(z5));
  inv1   g38(.a(x2), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n36_), .c(new_n29_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x8), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x9), .c(x7), .O(new_n59_));
  nand3  g42(.a(new_n19_), .b(new_n22_), .c(new_n26_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand2  g45(.a(x4), .b(x3), .O(new_n63_));
  oai21  g46(.a(new_n63_), .b(new_n20_), .c(x6), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n43_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n62_), .c(new_n54_), .O(z6));
endmodule


