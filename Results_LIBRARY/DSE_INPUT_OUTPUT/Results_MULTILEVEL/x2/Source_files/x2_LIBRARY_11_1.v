// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x7), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x6), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x8), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(x9), .b(new_n23_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n19_), .c(new_n22_), .O(z1));
  nor2   g08(.a(x9), .b(x8), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n23_), .O(new_n27_));
  oai21  g10(.a(new_n22_), .b(x6), .c(new_n27_), .O(z2));
  nor2   g11(.a(new_n22_), .b(x7), .O(new_n29_));
  inv1   g12(.a(x6), .O(new_n30_));
  nor2   g13(.a(new_n30_), .b(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(new_n29_), .d(new_n18_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x7), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x6), .O(new_n36_));
  nand2  g19(.a(new_n22_), .b(new_n30_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n36_), .O(z4));
  inv1   g21(.a(x0), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand2  g23(.a(new_n22_), .b(new_n23_), .O(new_n41_));
  oai21  g24(.a(new_n34_), .b(new_n23_), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand3  g27(.a(x7), .b(new_n44_), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x8), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nor2   g30(.a(new_n34_), .b(x7), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(x5), .c(new_n30_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n47_), .c(new_n43_), .O(z5));
  nand4  g33(.a(x9), .b(x8), .c(x6), .d(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n26_), .c(new_n23_), .O(new_n53_));
  inv1   g36(.a(x2), .O(new_n54_));
  nand4  g37(.a(x9), .b(new_n54_), .c(new_n40_), .d(new_n39_), .O(new_n55_));
  nand4  g38(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(new_n30_), .O(new_n57_));
  nor2   g40(.a(new_n18_), .b(x8), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n57_), .c(x7), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n53_), .c(new_n37_), .O(z6));
endmodule


