// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n28_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  oai21  g03(.a(new_n19_), .b(x5), .c(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  aoi21  g05(.a(x9), .b(new_n22_), .c(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n21_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  nor2   g09(.a(new_n26_), .b(x9), .O(z2));
  nor3   g10(.a(x2), .b(x1), .c(x0), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n19_), .c(x8), .d(new_n22_), .O(z3));
  inv1   g12(.a(x6), .O(new_n30_));
  nor2   g13(.a(new_n19_), .b(new_n25_), .O(new_n31_));
  oai21  g14(.a(new_n31_), .b(new_n30_), .c(new_n18_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x7), .O(z4));
  inv1   g16(.a(x0), .O(new_n34_));
  inv1   g17(.a(x1), .O(new_n35_));
  nand4  g18(.a(x9), .b(x8), .c(x7), .d(new_n18_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(x2), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  oai21  g21(.a(x9), .b(x8), .c(x6), .O(new_n39_));
  oai21  g22(.a(new_n22_), .b(new_n18_), .c(new_n39_), .O(new_n40_));
  inv1   g23(.a(x3), .O(new_n41_));
  nor2   g24(.a(x4), .b(new_n41_), .O(new_n42_));
  nand3  g25(.a(new_n19_), .b(x7), .c(new_n18_), .O(new_n43_));
  inv1   g26(.a(new_n43_), .O(new_n44_));
  nand4  g27(.a(x9), .b(x8), .c(new_n22_), .d(x5), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  aoi21  g29(.a(new_n44_), .b(new_n42_), .c(new_n46_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n40_), .c(new_n38_), .O(z5));
  oai21  g31(.a(new_n31_), .b(x7), .c(x5), .O(new_n49_));
  xnor2a g32(.a(x9), .b(x7), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  inv1   g34(.a(x2), .O(new_n52_));
  nand4  g35(.a(x9), .b(new_n52_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nand4  g36(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(x7), .O(new_n56_));
  nand4  g39(.a(new_n56_), .b(new_n51_), .c(new_n49_), .d(x6), .O(z6));
endmodule


