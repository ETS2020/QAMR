// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x7), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(new_n18_), .b(new_n21_), .O(new_n22_));
  oai21  g05(.a(new_n20_), .b(new_n18_), .c(new_n22_), .O(z0));
  oai21  g06(.a(x9), .b(x1), .c(new_n18_), .O(new_n24_));
  inv1   g07(.a(x9), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x7), .c(x8), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z1));
  nand4  g10(.a(new_n25_), .b(new_n19_), .c(new_n18_), .d(new_n21_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand3  g14(.a(new_n20_), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(x1), .c(new_n18_), .O(z3));
  nand2  g17(.a(x9), .b(x8), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x7), .c(x6), .O(z4));
  nand2  g19(.a(new_n19_), .b(new_n18_), .O(new_n37_));
  nand2  g20(.a(x7), .b(new_n21_), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n35_), .c(new_n37_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x2), .c(new_n30_), .O(new_n40_));
  inv1   g23(.a(x6), .O(new_n41_));
  oai21  g24(.a(x7), .b(new_n21_), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(x9), .b(x8), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n25_), .b(new_n19_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n43_), .c(new_n21_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  inv1   g29(.a(x3), .O(new_n47_));
  oai21  g30(.a(x4), .b(new_n47_), .c(x8), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n25_), .c(x7), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(new_n46_), .c(new_n42_), .d(new_n40_), .O(z5));
  aoi21  g33(.a(new_n44_), .b(new_n43_), .c(x7), .O(new_n51_));
  nand4  g34(.a(x9), .b(x7), .c(new_n31_), .d(new_n30_), .O(new_n52_));
  inv1   g35(.a(new_n52_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n51_), .c(new_n21_), .O(new_n54_));
  nand4  g37(.a(new_n25_), .b(x8), .c(x4), .d(x3), .O(new_n55_));
  oai21  g38(.a(new_n25_), .b(x8), .c(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x7), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n54_), .c(new_n42_), .O(z6));
endmodule


