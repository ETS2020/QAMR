// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x3), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  aoi21  g04(.a(x9), .b(new_n21_), .c(x8), .O(new_n22_));
  oai21  g05(.a(x8), .b(x7), .c(new_n18_), .O(new_n23_));
  oai21  g06(.a(new_n22_), .b(new_n20_), .c(new_n23_), .O(z1));
  nand2  g07(.a(x9), .b(new_n20_), .O(new_n25_));
  nor2   g08(.a(x9), .b(x8), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n25_), .O(z2));
  nor3   g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(new_n18_), .c(x8), .d(new_n21_), .O(z3));
  inv1   g13(.a(x6), .O(new_n31_));
  oai21  g14(.a(new_n21_), .b(new_n31_), .c(new_n25_), .O(new_n32_));
  inv1   g15(.a(x8), .O(new_n33_));
  nor2   g16(.a(new_n18_), .b(new_n33_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x3), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n32_), .O(z4));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x1), .O(new_n38_));
  nand3  g21(.a(x9), .b(x8), .c(x7), .O(new_n39_));
  oai21  g22(.a(x8), .b(x7), .c(new_n39_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x2), .c(new_n38_), .d(new_n37_), .O(new_n41_));
  nand3  g24(.a(new_n34_), .b(new_n21_), .c(x5), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nor2   g26(.a(x9), .b(new_n21_), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n43_), .c(new_n31_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x3), .O(new_n47_));
  oai21  g30(.a(new_n33_), .b(new_n31_), .c(new_n18_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n47_), .O(z5));
  nand2  g32(.a(new_n25_), .b(new_n31_), .O(new_n50_));
  nand3  g33(.a(new_n34_), .b(x5), .c(x3), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n26_), .c(new_n21_), .O(new_n53_));
  nor2   g36(.a(x2), .b(x1), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n37_), .c(new_n33_), .O(new_n55_));
  nand3  g38(.a(new_n18_), .b(x8), .c(x4), .O(new_n56_));
  oai21  g39(.a(new_n55_), .b(new_n18_), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(x7), .c(x3), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n53_), .c(new_n50_), .O(z6));
endmodule


