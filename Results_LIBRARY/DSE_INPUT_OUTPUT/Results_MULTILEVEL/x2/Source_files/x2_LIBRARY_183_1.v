// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x2), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  oai21  g05(.a(new_n19_), .b(new_n18_), .c(new_n22_), .O(z0));
  oai21  g06(.a(new_n20_), .b(new_n18_), .c(x9), .O(new_n24_));
  nor2   g07(.a(x9), .b(new_n20_), .O(new_n25_));
  nor2   g08(.a(new_n25_), .b(x8), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z1));
  nor2   g10(.a(new_n19_), .b(x2), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(new_n29_));
  nand3  g12(.a(new_n19_), .b(new_n21_), .c(new_n20_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(z2));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  nand4  g16(.a(x8), .b(new_n20_), .c(new_n33_), .d(new_n32_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n19_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n18_), .O(z3));
  inv1   g19(.a(x6), .O(new_n37_));
  oai21  g20(.a(new_n25_), .b(x2), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(x9), .b(x8), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x7), .O(new_n40_));
  aoi22  g23(.a(new_n40_), .b(x2), .c(new_n19_), .d(new_n20_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n38_), .O(z4));
  nand2  g25(.a(new_n21_), .b(new_n20_), .O(new_n43_));
  oai21  g26(.a(new_n39_), .b(new_n20_), .c(new_n43_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n33_), .c(new_n32_), .O(new_n45_));
  nand2  g28(.a(new_n20_), .b(x5), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n39_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x2), .O(new_n48_));
  oai21  g31(.a(new_n19_), .b(x7), .c(new_n37_), .O(new_n49_));
  inv1   g32(.a(x4), .O(new_n50_));
  nand3  g33(.a(x7), .b(new_n50_), .c(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x8), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n19_), .c(new_n28_), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n49_), .c(new_n48_), .d(new_n45_), .O(z5));
  nand2  g37(.a(new_n29_), .b(new_n37_), .O(new_n55_));
  nand3  g38(.a(x8), .b(new_n20_), .c(x5), .O(new_n56_));
  oai21  g39(.a(x8), .b(new_n20_), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(x9), .c(x2), .O(new_n58_));
  nand4  g41(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n43_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n19_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z6));
endmodule


