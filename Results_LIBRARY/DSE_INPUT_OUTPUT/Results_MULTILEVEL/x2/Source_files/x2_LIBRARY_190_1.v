// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x5), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(new_n19_), .O(new_n23_));
  inv1   g06(.a(x9), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x7), .c(x8), .O(new_n25_));
  inv1   g08(.a(x7), .O(new_n26_));
  nand2  g09(.a(x9), .b(new_n26_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n23_), .O(z1));
  nand2  g11(.a(new_n24_), .b(new_n20_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(x7), .c(new_n23_), .O(z2));
  inv1   g13(.a(x1), .O(new_n31_));
  aoi21  g14(.a(new_n20_), .b(x7), .c(new_n24_), .O(new_n32_));
  inv1   g15(.a(x0), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  nand4  g17(.a(x8), .b(new_n26_), .c(new_n34_), .d(new_n33_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n32_), .c(new_n18_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n31_), .O(z3));
  aoi21  g20(.a(x7), .b(new_n18_), .c(x1), .O(new_n38_));
  aoi21  g21(.a(x9), .b(x8), .c(new_n26_), .O(new_n39_));
  oai22  g22(.a(new_n39_), .b(new_n19_), .c(new_n38_), .d(x6), .O(z4));
  nand2  g23(.a(new_n20_), .b(new_n26_), .O(new_n41_));
  nand3  g24(.a(x9), .b(x8), .c(x7), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(new_n34_), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n33_), .c(x5), .O(new_n44_));
  inv1   g27(.a(x4), .O(new_n45_));
  nand3  g28(.a(x7), .b(new_n45_), .c(x3), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(x8), .c(x9), .O(new_n47_));
  nand2  g30(.a(x9), .b(x8), .O(new_n48_));
  nand2  g31(.a(new_n26_), .b(x5), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n48_), .c(x6), .O(new_n50_));
  nor2   g33(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  oai21  g34(.a(new_n44_), .b(x1), .c(new_n51_), .O(z5));
  nand4  g35(.a(x9), .b(x7), .c(new_n34_), .d(new_n33_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  nand4  g38(.a(new_n24_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  oai21  g39(.a(new_n24_), .b(x8), .c(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x7), .O(new_n58_));
  oai21  g41(.a(new_n48_), .b(new_n18_), .c(new_n29_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n26_), .O(new_n60_));
  nand4  g43(.a(new_n60_), .b(new_n58_), .c(new_n55_), .d(x6), .O(z6));
endmodule


