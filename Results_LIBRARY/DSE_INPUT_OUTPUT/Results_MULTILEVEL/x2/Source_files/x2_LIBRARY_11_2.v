// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x7), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x8), .O(new_n20_));
  nor2   g03(.a(x8), .b(x6), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(z0));
  inv1   g06(.a(x7), .O(new_n24_));
  nand2  g07(.a(x9), .b(new_n24_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n21_), .c(new_n19_), .O(z1));
  aoi21  g09(.a(new_n18_), .b(new_n24_), .c(x6), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(x8), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  inv1   g12(.a(x8), .O(new_n30_));
  nor2   g13(.a(new_n30_), .b(x7), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(new_n18_), .d(new_n29_), .O(z3));
  nand2  g16(.a(new_n20_), .b(x6), .O(z4));
  nand3  g17(.a(new_n32_), .b(x9), .c(x2), .O(new_n35_));
  inv1   g18(.a(x4), .O(new_n36_));
  nand3  g19(.a(new_n18_), .b(new_n36_), .c(x3), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x7), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n24_), .d(x5), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(x8), .d(x6), .O(z5));
  inv1   g24(.a(x0), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  nand4  g26(.a(x9), .b(new_n29_), .c(new_n43_), .d(new_n42_), .O(new_n44_));
  nand3  g27(.a(new_n18_), .b(x4), .c(x3), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n24_), .O(new_n46_));
  nand3  g29(.a(x9), .b(new_n24_), .c(x5), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(x8), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x6), .O(z6));
endmodule


