// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  nand2  g04(.a(x9), .b(new_n21_), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(z0), .c(new_n20_), .O(z1));
  aoi21  g06(.a(x8), .b(new_n21_), .c(x9), .O(z2));
  inv1   g07(.a(x2), .O(new_n25_));
  nor2   g08(.a(x1), .b(x0), .O(new_n26_));
  nor2   g09(.a(x9), .b(new_n20_), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(new_n26_), .c(new_n21_), .d(new_n25_), .O(z3));
  nand4  g11(.a(x9), .b(new_n20_), .c(x7), .d(x6), .O(z4));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  xnor2a g14(.a(x8), .b(x7), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n33_));
  nand3  g16(.a(x9), .b(x8), .c(x5), .O(new_n34_));
  oai21  g17(.a(x9), .b(x8), .c(new_n34_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n33_), .c(z0), .d(x6), .O(z5));
  inv1   g20(.a(x6), .O(new_n38_));
  nand2  g21(.a(z0), .b(new_n38_), .O(new_n39_));
  nand3  g22(.a(new_n25_), .b(new_n31_), .c(new_n30_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x8), .c(new_n21_), .O(new_n41_));
  nand3  g24(.a(x8), .b(new_n21_), .c(x5), .O(new_n42_));
  inv1   g25(.a(new_n42_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n41_), .c(x9), .O(new_n44_));
  nand3  g27(.a(new_n18_), .b(new_n20_), .c(new_n21_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n44_), .c(new_n39_), .O(z6));
endmodule


