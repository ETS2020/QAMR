// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x8), .O(new_n19_));
  oai21  g02(.a(x9), .b(x8), .c(new_n19_), .O(z0));
  inv1   g03(.a(x8), .O(new_n21_));
  oai21  g04(.a(x9), .b(new_n18_), .c(new_n21_), .O(z1));
  inv1   g05(.a(z1), .O(z2));
  inv1   g06(.a(x9), .O(new_n24_));
  inv1   g07(.a(x2), .O(new_n25_));
  nor2   g08(.a(x1), .b(x0), .O(new_n26_));
  nand4  g09(.a(new_n26_), .b(x8), .c(new_n18_), .d(new_n25_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  oai21  g11(.a(new_n24_), .b(new_n21_), .c(new_n28_), .O(z3));
  nand3  g12(.a(new_n24_), .b(x7), .c(x6), .O(z4));
  inv1   g13(.a(x6), .O(new_n31_));
  oai21  g14(.a(new_n24_), .b(x8), .c(new_n31_), .O(new_n32_));
  nand3  g15(.a(new_n26_), .b(x9), .c(x2), .O(new_n33_));
  inv1   g16(.a(x4), .O(new_n34_));
  nand3  g17(.a(new_n24_), .b(new_n34_), .c(x3), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x7), .O(new_n37_));
  inv1   g20(.a(x5), .O(new_n38_));
  nor2   g21(.a(x7), .b(new_n38_), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(x9), .c(new_n21_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n37_), .c(new_n32_), .O(z5));
  inv1   g24(.a(x0), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  nand4  g26(.a(x9), .b(new_n25_), .c(new_n43_), .d(new_n42_), .O(new_n44_));
  nand3  g27(.a(new_n24_), .b(x4), .c(x3), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n18_), .O(new_n46_));
  nand3  g29(.a(x9), .b(new_n18_), .c(x5), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(x8), .O(new_n49_));
  nand3  g32(.a(new_n24_), .b(new_n21_), .c(new_n18_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n49_), .c(new_n32_), .O(z6));
endmodule


