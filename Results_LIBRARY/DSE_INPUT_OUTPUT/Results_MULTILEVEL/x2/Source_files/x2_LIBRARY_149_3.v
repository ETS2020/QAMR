// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:09 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(x0), .c(x9), .O(new_n21_));
  aoi21  g04(.a(new_n18_), .b(x7), .c(x8), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(z1));
  nand2  g06(.a(x9), .b(x0), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  nand3  g08(.a(new_n18_), .b(new_n25_), .c(new_n20_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z2));
  nand2  g10(.a(new_n24_), .b(x7), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  oai21  g12(.a(x8), .b(new_n20_), .c(x9), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(x8), .b(new_n32_), .c(new_n31_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n20_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n30_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  nand3  g19(.a(new_n18_), .b(new_n20_), .c(x0), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(new_n28_), .O(z3));
  or2    g21(.a(new_n28_), .b(x6), .O(new_n39_));
  oai21  g22(.a(new_n18_), .b(new_n25_), .c(x7), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n29_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z4));
  nand2  g25(.a(new_n25_), .b(new_n20_), .O(new_n43_));
  nand3  g26(.a(x9), .b(x8), .c(x7), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n43_), .c(new_n32_), .O(new_n45_));
  nand4  g28(.a(x9), .b(x8), .c(new_n20_), .d(x5), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x6), .O(new_n47_));
  aoi21  g30(.a(new_n45_), .b(new_n31_), .c(new_n47_), .O(new_n48_));
  inv1   g31(.a(x4), .O(new_n49_));
  nand3  g32(.a(x7), .b(new_n49_), .c(x3), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(x8), .c(x6), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  oai21  g35(.a(new_n48_), .b(x0), .c(new_n52_), .O(z5));
  nand2  g36(.a(new_n47_), .b(new_n29_), .O(new_n54_));
  xnor2a g37(.a(x9), .b(x7), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n25_), .O(new_n56_));
  nand3  g39(.a(x9), .b(new_n32_), .c(new_n31_), .O(new_n57_));
  nand4  g40(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x7), .O(new_n60_));
  nor2   g43(.a(x9), .b(x6), .O(new_n61_));
  aoi21  g44(.a(x9), .b(x0), .c(new_n61_), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n60_), .c(new_n56_), .d(new_n54_), .O(z6));
endmodule


