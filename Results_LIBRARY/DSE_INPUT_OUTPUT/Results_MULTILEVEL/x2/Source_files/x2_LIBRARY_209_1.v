// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  aoi21  g04(.a(x9), .b(new_n21_), .c(x8), .O(new_n22_));
  nor2   g05(.a(x8), .b(x7), .O(new_n23_));
  oai22  g06(.a(new_n23_), .b(x9), .c(new_n22_), .d(new_n20_), .O(z1));
  nor2   g07(.a(new_n18_), .b(x0), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  inv1   g09(.a(x8), .O(new_n27_));
  nand3  g10(.a(new_n18_), .b(new_n27_), .c(new_n21_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n26_), .O(z2));
  nor3   g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n18_), .c(x8), .d(new_n21_), .O(z3));
  inv1   g14(.a(x6), .O(new_n32_));
  nor2   g15(.a(new_n21_), .b(new_n32_), .O(new_n33_));
  nand3  g16(.a(x9), .b(x8), .c(x0), .O(new_n34_));
  oai21  g17(.a(new_n33_), .b(new_n25_), .c(new_n34_), .O(z4));
  nand4  g18(.a(x9), .b(x8), .c(new_n21_), .d(x5), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x0), .O(new_n38_));
  oai21  g21(.a(new_n18_), .b(x7), .c(new_n32_), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(x7), .b(new_n40_), .c(x3), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x8), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n18_), .c(new_n25_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z5));
  nand2  g27(.a(new_n26_), .b(new_n32_), .O(new_n45_));
  nand3  g28(.a(x8), .b(new_n21_), .c(x5), .O(new_n46_));
  nand2  g29(.a(new_n27_), .b(x7), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(x9), .c(x0), .O(new_n49_));
  nand4  g32(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n23_), .c(new_n18_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z6));
endmodule


