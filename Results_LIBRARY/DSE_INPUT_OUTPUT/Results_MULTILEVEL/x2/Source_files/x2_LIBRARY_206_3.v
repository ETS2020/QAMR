// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x2), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nor2   g04(.a(x9), .b(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x7), .c(new_n20_), .O(z0));
  xnor2a g06(.a(x9), .b(x7), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n21_), .c(new_n20_), .O(z1));
  inv1   g08(.a(new_n20_), .O(new_n26_));
  inv1   g09(.a(x7), .O(new_n27_));
  nor2   g10(.a(x9), .b(x8), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n26_), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  nand2  g14(.a(new_n19_), .b(new_n31_), .O(new_n32_));
  nand2  g15(.a(new_n22_), .b(new_n27_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n32_), .c(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n19_), .b(x1), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(z3));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n26_), .c(x7), .d(x6), .O(z4));
  nand2  g21(.a(new_n21_), .b(new_n27_), .O(new_n39_));
  nand3  g22(.a(x9), .b(x8), .c(x7), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n39_), .c(x0), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(x1), .c(x2), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(x8), .c(x9), .O(new_n45_));
  nand2  g28(.a(new_n27_), .b(x5), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n37_), .c(x6), .O(new_n47_));
  nor2   g30(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n42_), .O(z5));
  nand3  g32(.a(x9), .b(x8), .c(x5), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n28_), .c(new_n27_), .O(new_n52_));
  inv1   g35(.a(x9), .O(new_n53_));
  nand3  g36(.a(new_n19_), .b(new_n18_), .c(new_n31_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x8), .c(new_n53_), .O(new_n55_));
  nand4  g38(.a(new_n53_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(x7), .O(new_n58_));
  inv1   g41(.a(x6), .O(new_n59_));
  nor2   g42(.a(new_n20_), .b(new_n59_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n58_), .c(new_n52_), .O(z6));
endmodule


