// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(x4), .c(new_n21_), .O(z0));
  aoi21  g05(.a(x9), .b(new_n19_), .c(x8), .O(new_n23_));
  oai21  g06(.a(x8), .b(x7), .c(new_n18_), .O(new_n24_));
  oai21  g07(.a(new_n23_), .b(x4), .c(new_n24_), .O(z1));
  nand2  g08(.a(x9), .b(x4), .O(new_n26_));
  nor2   g09(.a(x9), .b(x8), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n19_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n26_), .O(z2));
  inv1   g12(.a(x4), .O(new_n30_));
  nand3  g13(.a(new_n20_), .b(x7), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x9), .O(new_n32_));
  inv1   g15(.a(x0), .O(new_n33_));
  inv1   g16(.a(x1), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  nand4  g18(.a(x8), .b(new_n35_), .c(new_n34_), .d(new_n33_), .O(new_n36_));
  inv1   g19(.a(new_n36_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n32_), .c(new_n19_), .O(z3));
  oai21  g21(.a(x8), .b(x4), .c(x9), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x7), .c(x6), .O(z4));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  oai21  g24(.a(x8), .b(x7), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n34_), .d(new_n33_), .O(new_n43_));
  nor2   g26(.a(new_n18_), .b(new_n20_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n19_), .c(x5), .O(new_n45_));
  inv1   g28(.a(x6), .O(new_n46_));
  nor2   g29(.a(x9), .b(new_n19_), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(x3), .c(new_n46_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n45_), .c(new_n43_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n30_), .O(new_n50_));
  oai21  g33(.a(new_n20_), .b(new_n46_), .c(new_n18_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(z5));
  nand2  g35(.a(new_n26_), .b(new_n46_), .O(new_n53_));
  nand3  g36(.a(new_n44_), .b(x5), .c(new_n30_), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n27_), .c(new_n19_), .O(new_n56_));
  nand3  g39(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x8), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x9), .c(new_n30_), .O(new_n59_));
  nand4  g42(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x7), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n56_), .c(new_n53_), .O(z6));
endmodule


