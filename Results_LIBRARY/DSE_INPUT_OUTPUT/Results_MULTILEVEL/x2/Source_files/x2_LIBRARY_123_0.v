// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x5), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  oai21  g04(.a(x9), .b(new_n20_), .c(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n18_), .b(x7), .c(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  nor2   g08(.a(x7), .b(new_n20_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n18_), .c(new_n25_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  nand3  g12(.a(new_n18_), .b(x8), .c(new_n29_), .O(new_n30_));
  nor3   g13(.a(new_n30_), .b(x1), .c(x0), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n21_), .c(x5), .O(z3));
  inv1   g15(.a(new_n26_), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nor2   g17(.a(new_n18_), .b(new_n25_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n34_), .c(x7), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n33_), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  nand2  g22(.a(x8), .b(x7), .O(new_n40_));
  oai21  g23(.a(x7), .b(new_n20_), .c(new_n40_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(x2), .c(new_n39_), .d(new_n38_), .O(new_n42_));
  nand3  g25(.a(x8), .b(new_n21_), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x9), .O(new_n45_));
  nand3  g28(.a(new_n18_), .b(new_n25_), .c(new_n21_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(x6), .c(new_n20_), .O(new_n47_));
  inv1   g30(.a(x4), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(x3), .c(new_n25_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(x9), .c(x6), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(x7), .c(new_n47_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n45_), .O(z5));
  inv1   g35(.a(new_n47_), .O(new_n53_));
  oai21  g36(.a(x9), .b(x7), .c(new_n34_), .O(new_n54_));
  oai21  g37(.a(new_n35_), .b(new_n20_), .c(new_n21_), .O(new_n55_));
  nand3  g38(.a(new_n29_), .b(new_n39_), .c(new_n38_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(x8), .c(new_n18_), .O(new_n57_));
  nand4  g40(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n57_), .c(x7), .O(new_n60_));
  nand4  g43(.a(new_n60_), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(z6));
endmodule


