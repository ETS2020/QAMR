// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n23_, new_n24_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  oai21  g02(.a(new_n18_), .b(x7), .c(new_n19_), .O(z0));
  nand3  g03(.a(x9), .b(new_n18_), .c(x7), .O(z1));
  inv1   g04(.a(x7), .O(new_n23_));
  nor3   g05(.a(x2), .b(x1), .c(x0), .O(new_n24_));
  nand4  g06(.a(new_n24_), .b(new_n19_), .c(x8), .d(new_n23_), .O(z3));
  nand2  g07(.a(new_n19_), .b(new_n18_), .O(new_n26_));
  nor2   g08(.a(new_n19_), .b(new_n18_), .O(new_n27_));
  inv1   g09(.a(new_n27_), .O(new_n28_));
  nand4  g10(.a(new_n28_), .b(new_n26_), .c(x7), .d(x6), .O(z4));
  inv1   g11(.a(x0), .O(new_n30_));
  inv1   g12(.a(x1), .O(new_n31_));
  nand3  g13(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  oai21  g14(.a(x8), .b(x7), .c(new_n32_), .O(new_n33_));
  nand4  g15(.a(new_n33_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n34_));
  inv1   g16(.a(x4), .O(new_n35_));
  nand3  g17(.a(x7), .b(new_n35_), .c(x3), .O(new_n36_));
  nand2  g18(.a(new_n36_), .b(x8), .O(new_n37_));
  nand2  g19(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  inv1   g20(.a(x6), .O(new_n39_));
  inv1   g21(.a(x5), .O(new_n40_));
  nor2   g22(.a(x7), .b(new_n40_), .O(new_n41_));
  aoi21  g23(.a(new_n41_), .b(new_n27_), .c(new_n39_), .O(new_n42_));
  nand3  g24(.a(new_n42_), .b(new_n38_), .c(new_n34_), .O(z5));
  oai21  g25(.a(new_n19_), .b(x7), .c(new_n18_), .O(new_n44_));
  inv1   g26(.a(x2), .O(new_n45_));
  nand4  g27(.a(x9), .b(new_n45_), .c(new_n31_), .d(new_n30_), .O(new_n46_));
  nand3  g28(.a(new_n19_), .b(x4), .c(x3), .O(new_n47_));
  nand2  g29(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g30(.a(new_n48_), .b(x7), .O(new_n49_));
  nand3  g31(.a(new_n49_), .b(new_n44_), .c(new_n42_), .O(z6));
  zero   g32(.O(z2));
endmodule


