// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x7), .O(new_n18_));
  nor2   g01(.a(x9), .b(new_n18_), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(x4), .c(x8), .O(z0));
  inv1   g03(.a(x8), .O(new_n21_));
  inv1   g04(.a(new_n19_), .O(new_n22_));
  nand2  g05(.a(x9), .b(new_n18_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(z1));
  nor3   g07(.a(x9), .b(x8), .c(x7), .O(z2));
  oai22  g08(.a(x9), .b(x4), .c(x8), .d(new_n18_), .O(new_n26_));
  inv1   g09(.a(x0), .O(new_n27_));
  inv1   g10(.a(x1), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nand4  g12(.a(x8), .b(new_n29_), .c(new_n28_), .d(new_n27_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n26_), .c(new_n18_), .O(z3));
  oai21  g15(.a(x9), .b(x4), .c(x8), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(x7), .c(x6), .O(z4));
  nand3  g17(.a(x9), .b(x8), .c(x7), .O(new_n35_));
  oai21  g18(.a(x8), .b(x7), .c(new_n35_), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(x2), .c(new_n28_), .d(new_n27_), .O(new_n37_));
  oai21  g20(.a(new_n19_), .b(x8), .c(x4), .O(new_n38_));
  inv1   g21(.a(x9), .O(new_n39_));
  nand2  g22(.a(x7), .b(x3), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x8), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  inv1   g25(.a(x6), .O(new_n43_));
  nor2   g26(.a(new_n39_), .b(new_n21_), .O(new_n44_));
  inv1   g27(.a(x5), .O(new_n45_));
  nor2   g28(.a(x7), .b(new_n45_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n42_), .c(new_n38_), .d(new_n37_), .O(z5));
  nand3  g31(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(x8), .c(new_n18_), .O(new_n50_));
  nand3  g33(.a(x8), .b(new_n18_), .c(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n50_), .c(x9), .O(new_n53_));
  nand2  g36(.a(x8), .b(x4), .O(new_n54_));
  nand3  g37(.a(new_n39_), .b(new_n21_), .c(new_n18_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n54_), .c(x6), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n53_), .O(z6));
endmodule


