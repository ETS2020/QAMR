// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  aoi21  g03(.a(x9), .b(new_n20_), .c(x8), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(x8), .c(x7), .O(new_n22_));
  oai21  g05(.a(new_n21_), .b(x6), .c(new_n22_), .O(z1));
  inv1   g06(.a(x6), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  nand2  g08(.a(new_n18_), .b(new_n25_), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n24_), .c(x7), .O(z2));
  nor2   g10(.a(x1), .b(x0), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nand3  g12(.a(new_n18_), .b(x8), .c(new_n29_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nand4  g14(.a(new_n31_), .b(new_n28_), .c(new_n20_), .d(new_n24_), .O(z3));
  nand3  g15(.a(x9), .b(x8), .c(x7), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x6), .O(z4));
  inv1   g17(.a(x3), .O(new_n35_));
  oai21  g18(.a(x4), .b(new_n35_), .c(x8), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  nand3  g20(.a(x9), .b(x8), .c(x2), .O(new_n38_));
  inv1   g21(.a(new_n38_), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(new_n28_), .c(new_n20_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n37_), .c(x6), .O(z5));
  nor3   g24(.a(x2), .b(x1), .c(x0), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n25_), .c(x9), .O(new_n43_));
  nor2   g26(.a(x9), .b(new_n25_), .O(new_n44_));
  and2   g27(.a(x4), .b(x3), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n20_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n43_), .c(x6), .O(z6));
endmodule


