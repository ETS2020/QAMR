// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(x8), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n19_), .O(z0));
  nor3   g05(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g06(.a(z2), .O(z1));
  inv1   g07(.a(x9), .O(new_n25_));
  nor2   g08(.a(x1), .b(x0), .O(new_n26_));
  inv1   g09(.a(x8), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(x2), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n26_), .c(new_n25_), .d(new_n18_), .O(z3));
  nand3  g12(.a(new_n25_), .b(x7), .c(x6), .O(z4));
  inv1   g13(.a(x6), .O(new_n31_));
  oai21  g14(.a(new_n25_), .b(x8), .c(new_n31_), .O(new_n32_));
  inv1   g15(.a(new_n26_), .O(new_n33_));
  nand3  g16(.a(x9), .b(x8), .c(x2), .O(new_n34_));
  inv1   g17(.a(x4), .O(new_n35_));
  nand3  g18(.a(new_n25_), .b(new_n35_), .c(x3), .O(new_n36_));
  oai21  g19(.a(new_n34_), .b(new_n33_), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x7), .O(new_n38_));
  nor2   g21(.a(new_n25_), .b(new_n27_), .O(new_n39_));
  inv1   g22(.a(x5), .O(new_n40_));
  nor2   g23(.a(x7), .b(new_n40_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n39_), .c(new_n20_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n38_), .c(new_n32_), .O(z5));
  inv1   g26(.a(x0), .O(new_n44_));
  inv1   g27(.a(x1), .O(new_n45_));
  inv1   g28(.a(x2), .O(new_n46_));
  nand4  g29(.a(x9), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n47_));
  nand3  g30(.a(new_n25_), .b(x4), .c(x3), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n47_), .c(new_n18_), .O(new_n49_));
  nand3  g32(.a(x9), .b(new_n18_), .c(x5), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n49_), .c(x8), .O(new_n52_));
  nand2  g35(.a(new_n20_), .b(new_n18_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n32_), .O(z6));
endmodule


