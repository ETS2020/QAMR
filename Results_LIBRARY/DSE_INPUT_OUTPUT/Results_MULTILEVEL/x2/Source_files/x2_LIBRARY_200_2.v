// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_;
  inv1   g00(.a(x2), .O(new_n18_));
  nor2   g01(.a(x6), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  xnor2a g05(.a(x9), .b(x7), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n20_), .c(new_n19_), .O(z1));
  inv1   g07(.a(x7), .O(new_n25_));
  nor2   g08(.a(new_n19_), .b(x9), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n20_), .c(new_n25_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g14(.a(new_n21_), .b(new_n25_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(new_n18_), .O(new_n33_));
  nand2  g16(.a(x6), .b(x2), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n33_), .O(z3));
  inv1   g18(.a(x6), .O(new_n36_));
  inv1   g19(.a(x9), .O(new_n37_));
  nor2   g20(.a(new_n37_), .b(new_n20_), .O(new_n38_));
  nor2   g21(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x7), .O(z4));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  oai21  g24(.a(x8), .b(x7), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n30_), .d(new_n29_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand3  g27(.a(x7), .b(new_n44_), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x8), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n37_), .O(new_n47_));
  inv1   g30(.a(x5), .O(new_n48_));
  nor2   g31(.a(x7), .b(new_n48_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n38_), .c(new_n36_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n47_), .c(new_n43_), .O(z5));
  nand3  g34(.a(x9), .b(x8), .c(x5), .O(new_n52_));
  oai21  g35(.a(x9), .b(x8), .c(new_n52_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n25_), .O(new_n54_));
  nand3  g37(.a(new_n18_), .b(new_n30_), .c(new_n29_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(x8), .c(new_n37_), .O(new_n56_));
  nand4  g39(.a(new_n37_), .b(x8), .c(x4), .d(x3), .O(new_n57_));
  inv1   g40(.a(new_n57_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n56_), .c(x7), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n54_), .c(x6), .O(z6));
endmodule


