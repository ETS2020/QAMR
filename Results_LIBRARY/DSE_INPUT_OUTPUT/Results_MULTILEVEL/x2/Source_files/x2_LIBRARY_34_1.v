// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  aoi21  g02(.a(new_n18_), .b(x7), .c(x8), .O(z2));
  inv1   g03(.a(z2), .O(z1));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x2), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  nor2   g07(.a(x1), .b(x0), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(x8), .c(new_n24_), .d(new_n23_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  oai21  g10(.a(new_n18_), .b(new_n22_), .c(new_n27_), .O(z3));
  nand3  g11(.a(new_n18_), .b(x7), .c(x6), .O(z4));
  inv1   g12(.a(x6), .O(new_n30_));
  oai21  g13(.a(new_n18_), .b(x8), .c(new_n30_), .O(new_n31_));
  inv1   g14(.a(new_n25_), .O(new_n32_));
  nand3  g15(.a(x9), .b(x8), .c(x2), .O(new_n33_));
  inv1   g16(.a(x4), .O(new_n34_));
  nand3  g17(.a(new_n18_), .b(new_n34_), .c(x3), .O(new_n35_));
  oai21  g18(.a(new_n33_), .b(new_n32_), .c(new_n35_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x7), .O(new_n37_));
  nor2   g20(.a(new_n18_), .b(new_n22_), .O(new_n38_));
  inv1   g21(.a(x5), .O(new_n39_));
  nor2   g22(.a(x7), .b(new_n39_), .O(new_n40_));
  nor2   g23(.a(x9), .b(x8), .O(new_n41_));
  aoi21  g24(.a(new_n40_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n37_), .c(new_n31_), .O(z5));
  inv1   g26(.a(x0), .O(new_n44_));
  inv1   g27(.a(x1), .O(new_n45_));
  nand4  g28(.a(x9), .b(new_n23_), .c(new_n45_), .d(new_n44_), .O(new_n46_));
  nand3  g29(.a(new_n18_), .b(x4), .c(x3), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n46_), .c(new_n24_), .O(new_n48_));
  nand3  g31(.a(x9), .b(new_n24_), .c(x5), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(x8), .O(new_n51_));
  nand2  g34(.a(new_n41_), .b(new_n24_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n51_), .c(new_n31_), .O(z6));
endmodule


