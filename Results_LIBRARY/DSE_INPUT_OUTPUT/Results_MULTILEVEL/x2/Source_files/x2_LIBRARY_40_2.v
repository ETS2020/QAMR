// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x8), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand2  g03(.a(new_n18_), .b(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(x9), .b(new_n22_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(z1));
  nor3   g07(.a(x9), .b(x8), .c(x7), .O(z2));
  nand3  g08(.a(new_n20_), .b(x7), .c(x6), .O(z4));
  inv1   g09(.a(x0), .O(new_n28_));
  inv1   g10(.a(x1), .O(new_n29_));
  xnor2a g11(.a(x8), .b(x7), .O(new_n30_));
  nand4  g12(.a(new_n30_), .b(x2), .c(new_n29_), .d(new_n28_), .O(new_n31_));
  nand3  g13(.a(x8), .b(new_n22_), .c(x5), .O(new_n32_));
  nand4  g14(.a(new_n32_), .b(new_n31_), .c(x9), .d(x6), .O(z5));
  inv1   g15(.a(x2), .O(new_n34_));
  nand3  g16(.a(new_n34_), .b(new_n29_), .c(new_n28_), .O(new_n35_));
  nand2  g17(.a(new_n35_), .b(x8), .O(new_n36_));
  nand3  g18(.a(new_n36_), .b(x9), .c(x7), .O(new_n37_));
  nand2  g19(.a(x8), .b(x5), .O(new_n38_));
  aoi21  g20(.a(new_n38_), .b(x9), .c(x7), .O(new_n39_));
  oai21  g21(.a(x9), .b(new_n20_), .c(x6), .O(new_n40_));
  nor2   g22(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g23(.a(new_n41_), .b(new_n37_), .O(z6));
  one    g24(.O(z3));
endmodule


