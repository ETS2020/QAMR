// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x3), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  aoi21  g04(.a(x9), .b(new_n21_), .c(x8), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  oai21  g08(.a(new_n22_), .b(new_n20_), .c(new_n25_), .O(z1));
  nor2   g09(.a(new_n18_), .b(x3), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  nor2   g11(.a(x9), .b(x8), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n21_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n28_), .O(z2));
  nor3   g14(.a(x2), .b(x1), .c(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n18_), .c(x8), .d(new_n21_), .O(z3));
  inv1   g16(.a(x6), .O(new_n34_));
  nor2   g17(.a(new_n21_), .b(new_n34_), .O(new_n35_));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  oai22  g19(.a(new_n36_), .b(new_n20_), .c(new_n35_), .d(new_n27_), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  oai21  g22(.a(new_n36_), .b(new_n21_), .c(new_n24_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x2), .c(new_n39_), .d(new_n38_), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(new_n18_), .b(new_n42_), .c(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x7), .O(new_n45_));
  nand2  g28(.a(new_n21_), .b(x5), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n36_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x3), .O(new_n48_));
  aoi21  g31(.a(x8), .b(x6), .c(x9), .O(new_n49_));
  nor2   g32(.a(new_n49_), .b(new_n27_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(new_n48_), .c(new_n45_), .d(new_n41_), .O(z5));
  nand2  g34(.a(new_n28_), .b(new_n34_), .O(new_n52_));
  nand4  g35(.a(x9), .b(x8), .c(x5), .d(x3), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n29_), .c(new_n21_), .O(new_n55_));
  nor2   g38(.a(x2), .b(x1), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n38_), .c(new_n23_), .O(new_n57_));
  nand3  g40(.a(new_n18_), .b(x8), .c(x4), .O(new_n58_));
  oai21  g41(.a(new_n57_), .b(new_n18_), .c(new_n58_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(x7), .c(x3), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n55_), .c(new_n52_), .O(z6));
endmodule


