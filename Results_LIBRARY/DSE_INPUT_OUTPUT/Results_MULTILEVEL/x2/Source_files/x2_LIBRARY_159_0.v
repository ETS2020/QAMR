// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x0), .O(z0));
  inv1   g02(.a(x0), .O(new_n20_));
  aoi21  g03(.a(new_n18_), .b(x7), .c(x8), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  oai21  g05(.a(x8), .b(new_n22_), .c(x9), .O(new_n23_));
  oai21  g06(.a(new_n21_), .b(new_n20_), .c(new_n23_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  nand4  g08(.a(new_n18_), .b(new_n25_), .c(new_n22_), .d(x0), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z2));
  nand2  g10(.a(new_n18_), .b(new_n20_), .O(z3));
  nand2  g11(.a(x9), .b(x8), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(z3), .c(x7), .d(x6), .O(z4));
  inv1   g13(.a(x1), .O(new_n31_));
  xnor2a g14(.a(x8), .b(x7), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(x2), .c(new_n31_), .d(new_n20_), .O(new_n33_));
  nand3  g16(.a(x8), .b(new_n22_), .c(x5), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(x6), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x9), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  inv1   g20(.a(x4), .O(new_n38_));
  nand3  g21(.a(x7), .b(new_n38_), .c(x3), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(x8), .c(x9), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n37_), .c(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n36_), .O(z5));
  nand2  g25(.a(z3), .b(new_n37_), .O(new_n43_));
  nand4  g26(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n44_));
  oai21  g27(.a(x8), .b(x7), .c(new_n44_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n18_), .c(x0), .O(new_n46_));
  inv1   g29(.a(new_n34_), .O(new_n47_));
  inv1   g30(.a(x2), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n31_), .c(new_n20_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(x8), .c(new_n22_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n47_), .c(x9), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n46_), .c(new_n43_), .O(z6));
endmodule


