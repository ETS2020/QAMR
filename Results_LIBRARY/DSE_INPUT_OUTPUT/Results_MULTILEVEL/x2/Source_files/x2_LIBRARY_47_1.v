// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x8), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x2), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x7), .O(z0));
  inv1   g03(.a(x2), .O(new_n21_));
  inv1   g04(.a(x9), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x7), .c(x8), .O(new_n23_));
  nor2   g06(.a(x9), .b(x8), .O(new_n24_));
  oai22  g07(.a(new_n24_), .b(x7), .c(new_n23_), .d(new_n21_), .O(z1));
  inv1   g08(.a(new_n24_), .O(new_n26_));
  nand2  g09(.a(x7), .b(new_n21_), .O(new_n27_));
  oai21  g10(.a(new_n26_), .b(x7), .c(new_n27_), .O(z2));
  inv1   g11(.a(x7), .O(new_n29_));
  nor3   g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n22_), .c(x8), .d(new_n29_), .O(z3));
  inv1   g14(.a(x6), .O(new_n32_));
  aoi21  g15(.a(x9), .b(x8), .c(new_n32_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n21_), .c(x7), .O(z4));
  inv1   g17(.a(x0), .O(new_n35_));
  inv1   g18(.a(x1), .O(new_n36_));
  nand3  g19(.a(new_n18_), .b(new_n29_), .c(x2), .O(new_n37_));
  nand3  g20(.a(x9), .b(x8), .c(x7), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n36_), .c(new_n35_), .O(new_n40_));
  inv1   g23(.a(x4), .O(new_n41_));
  nand3  g24(.a(new_n22_), .b(new_n41_), .c(x3), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x2), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x7), .O(new_n44_));
  nand4  g27(.a(x9), .b(x8), .c(new_n29_), .d(x5), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n26_), .c(x6), .O(new_n46_));
  inv1   g29(.a(new_n46_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n44_), .c(new_n40_), .O(z5));
  nand2  g31(.a(new_n27_), .b(new_n32_), .O(new_n49_));
  nand2  g32(.a(x9), .b(new_n18_), .O(new_n50_));
  nand4  g33(.a(new_n22_), .b(x8), .c(x4), .d(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(x7), .c(x2), .O(new_n53_));
  nand3  g36(.a(x9), .b(x8), .c(x5), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n24_), .c(new_n29_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z6));
endmodule


