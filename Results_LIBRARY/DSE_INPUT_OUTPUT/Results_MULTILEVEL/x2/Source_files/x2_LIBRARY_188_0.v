// Benchmark "FAU" written by ABC on Mon Aug 17 18:23:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  nor2   g00(.a(x5), .b(x4), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  aoi21  g03(.a(new_n20_), .b(x7), .c(new_n18_), .O(z0));
  xnor2a g04(.a(x9), .b(x7), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n19_), .c(new_n18_), .O(z1));
  inv1   g06(.a(new_n18_), .O(new_n24_));
  inv1   g07(.a(x7), .O(new_n25_));
  inv1   g08(.a(x9), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n19_), .c(new_n25_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n24_), .O(z2));
  oai21  g11(.a(x8), .b(new_n25_), .c(x9), .O(new_n29_));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand4  g15(.a(x8), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(x7), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n29_), .c(new_n18_), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(x7), .c(x6), .O(new_n37_));
  and2   g20(.a(new_n37_), .b(new_n24_), .O(z4));
  nand2  g21(.a(new_n19_), .b(new_n25_), .O(new_n39_));
  oai21  g22(.a(new_n36_), .b(new_n25_), .c(new_n39_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n41_));
  nand2  g24(.a(new_n27_), .b(x6), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n24_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand3  g27(.a(new_n26_), .b(x7), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x5), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  inv1   g30(.a(new_n36_), .O(new_n48_));
  oai21  g31(.a(x9), .b(x8), .c(x6), .O(new_n49_));
  inv1   g32(.a(x5), .O(new_n50_));
  nor2   g33(.a(x7), .b(new_n50_), .O(new_n51_));
  aoi22  g34(.a(new_n51_), .b(new_n48_), .c(new_n49_), .d(x7), .O(new_n52_));
  nand4  g35(.a(new_n52_), .b(new_n47_), .c(new_n43_), .d(new_n41_), .O(z5));
  nand3  g36(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x8), .c(new_n26_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(x7), .c(new_n42_), .O(new_n56_));
  nand3  g39(.a(x9), .b(new_n25_), .c(x5), .O(new_n57_));
  nand4  g40(.a(new_n26_), .b(x7), .c(x4), .d(x3), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x8), .O(new_n60_));
  oai21  g43(.a(new_n56_), .b(new_n18_), .c(new_n60_), .O(z6));
endmodule


