// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  oai21  g02(.a(x9), .b(new_n19_), .c(new_n18_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x7), .O(z0));
  inv1   g04(.a(x9), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x7), .c(x8), .O(new_n23_));
  nor2   g06(.a(x9), .b(x8), .O(new_n24_));
  oai22  g07(.a(new_n24_), .b(x7), .c(new_n23_), .d(x4), .O(z1));
  inv1   g08(.a(new_n24_), .O(new_n26_));
  nand2  g09(.a(x7), .b(x4), .O(new_n27_));
  oai21  g10(.a(new_n26_), .b(x7), .c(new_n27_), .O(z2));
  nor2   g11(.a(new_n22_), .b(new_n19_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(x7), .c(new_n18_), .O(new_n30_));
  inv1   g13(.a(x7), .O(new_n31_));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  nand3  g18(.a(new_n22_), .b(x8), .c(new_n35_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n34_), .c(new_n31_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n30_), .O(z3));
  nor2   g21(.a(new_n31_), .b(x6), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n29_), .c(new_n18_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x7), .O(z4));
  nand3  g24(.a(x9), .b(x8), .c(x7), .O(new_n42_));
  oai21  g25(.a(x8), .b(x7), .c(new_n42_), .O(new_n43_));
  nand4  g26(.a(new_n43_), .b(x2), .c(new_n33_), .d(new_n32_), .O(new_n44_));
  inv1   g27(.a(x3), .O(new_n45_));
  oai21  g28(.a(x9), .b(new_n45_), .c(new_n18_), .O(new_n46_));
  nand4  g29(.a(x9), .b(x8), .c(new_n31_), .d(x5), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n26_), .c(x6), .O(new_n48_));
  aoi21  g31(.a(new_n46_), .b(x7), .c(new_n48_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n44_), .O(z5));
  nand3  g33(.a(x9), .b(x8), .c(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n24_), .c(new_n31_), .O(new_n53_));
  nand3  g36(.a(new_n35_), .b(new_n33_), .c(new_n32_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x8), .c(new_n22_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(x4), .c(x7), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n53_), .c(x6), .O(z6));
endmodule


