// Benchmark "FAU" written by ABC on Mon Aug 17 18:22:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x6), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  xnor2a g05(.a(x9), .b(x7), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n20_), .c(new_n19_), .O(z1));
  inv1   g07(.a(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  oai22  g09(.a(new_n26_), .b(x7), .c(new_n18_), .d(x4), .O(z2));
  nor2   g10(.a(new_n20_), .b(x7), .O(new_n28_));
  nor2   g11(.a(x2), .b(x1), .O(new_n29_));
  nor2   g12(.a(new_n19_), .b(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(new_n28_), .d(new_n25_), .O(z3));
  inv1   g14(.a(x7), .O(new_n32_));
  nor2   g15(.a(new_n25_), .b(new_n20_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n32_), .c(x4), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x6), .O(z4));
  inv1   g18(.a(x0), .O(new_n36_));
  inv1   g19(.a(x1), .O(new_n37_));
  nand2  g20(.a(new_n33_), .b(x7), .O(new_n38_));
  oai21  g21(.a(x8), .b(x7), .c(new_n38_), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(x2), .c(new_n37_), .d(new_n36_), .O(new_n40_));
  inv1   g23(.a(x4), .O(new_n41_));
  nand3  g24(.a(x9), .b(x8), .c(x5), .O(new_n42_));
  oai21  g25(.a(new_n26_), .b(new_n41_), .c(new_n42_), .O(new_n43_));
  nand3  g26(.a(new_n25_), .b(new_n20_), .c(x7), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x6), .c(x4), .O(new_n45_));
  aoi21  g28(.a(new_n43_), .b(new_n32_), .c(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n40_), .O(z5));
  aoi21  g30(.a(new_n42_), .b(new_n26_), .c(x7), .O(new_n48_));
  aoi21  g31(.a(new_n29_), .b(new_n36_), .c(new_n20_), .O(new_n49_));
  nand3  g32(.a(new_n25_), .b(x8), .c(x3), .O(new_n50_));
  oai21  g33(.a(new_n49_), .b(new_n25_), .c(new_n50_), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(x7), .c(new_n48_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n41_), .c(x6), .O(z6));
endmodule


