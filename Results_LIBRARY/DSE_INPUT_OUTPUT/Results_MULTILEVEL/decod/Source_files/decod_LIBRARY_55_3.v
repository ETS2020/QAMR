// Benchmark "FAU" written by ABC on Thu Aug 13 21:00:16 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n35_, new_n36_, new_n38_, new_n40_, new_n43_, new_n44_,
    new_n46_, new_n48_, new_n51_, new_n53_, new_n55_;
  nand2  g00(.a(x3), .b(x2), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z00));
  inv1   g02(.a(x2), .O(new_n24_));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(x1), .c(x0), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z01));
  inv1   g06(.a(x4), .O(new_n28_));
  nand4  g07(.a(x3), .b(new_n24_), .c(x1), .d(x0), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n28_), .O(z02));
  nand2  g09(.a(x1), .b(x0), .O(new_n31_));
  nand3  g10(.a(x4), .b(new_n25_), .c(new_n24_), .O(new_n32_));
  oai21  g11(.a(new_n32_), .b(new_n31_), .c(new_n22_), .O(z03));
  inv1   g12(.a(x1), .O(new_n35_));
  nand4  g13(.a(new_n25_), .b(x2), .c(new_n35_), .d(x0), .O(new_n36_));
  nor2   g14(.a(new_n36_), .b(new_n28_), .O(z05));
  nand4  g15(.a(x3), .b(new_n24_), .c(new_n35_), .d(x0), .O(new_n38_));
  nor2   g16(.a(new_n38_), .b(new_n28_), .O(z06));
  nand4  g17(.a(new_n25_), .b(new_n24_), .c(new_n35_), .d(x0), .O(new_n40_));
  nor2   g18(.a(new_n40_), .b(new_n28_), .O(z07));
  inv1   g19(.a(x0), .O(new_n43_));
  nand3  g20(.a(x4), .b(x1), .c(new_n43_), .O(new_n44_));
  aoi21  g21(.a(new_n44_), .b(new_n25_), .c(new_n24_), .O(z09));
  nand4  g22(.a(x3), .b(new_n24_), .c(x1), .d(new_n43_), .O(new_n46_));
  nor2   g23(.a(new_n46_), .b(new_n28_), .O(z10));
  nand4  g24(.a(new_n25_), .b(new_n24_), .c(x1), .d(new_n43_), .O(new_n48_));
  nor2   g25(.a(new_n48_), .b(new_n28_), .O(z11));
  nand3  g26(.a(x4), .b(new_n35_), .c(new_n43_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n25_), .c(new_n24_), .O(z13));
  nand4  g28(.a(x3), .b(new_n24_), .c(new_n35_), .d(new_n43_), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n28_), .O(z14));
  nand2  g30(.a(new_n35_), .b(new_n43_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n32_), .c(new_n22_), .O(z15));
  zero   g32(.O(z04));
  zero   g33(.O(z08));
  zero   g34(.O(z12));
endmodule


