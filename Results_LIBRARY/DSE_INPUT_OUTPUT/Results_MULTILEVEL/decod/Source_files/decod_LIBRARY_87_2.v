// Benchmark "FAU" written by ABC on Thu Aug 13 21:00:25 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n35_, new_n37_, new_n39_, new_n40_, new_n43_, new_n45_,
    new_n47_, new_n49_, new_n50_;
  inv1   g00(.a(x1), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x1), .O(new_n26_));
  inv1   g05(.a(new_n26_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(new_n28_), .c(x0), .O(new_n29_));
  aoi21  g08(.a(new_n29_), .b(x3), .c(new_n22_), .O(z02));
  inv1   g09(.a(x4), .O(new_n31_));
  nand4  g10(.a(x3), .b(x2), .c(new_n22_), .d(x0), .O(new_n32_));
  nor2   g11(.a(new_n32_), .b(new_n31_), .O(z04));
  aoi21  g12(.a(new_n23_), .b(new_n22_), .c(x3), .O(z05));
  nand4  g13(.a(x3), .b(new_n28_), .c(new_n22_), .d(x0), .O(new_n35_));
  nor2   g14(.a(new_n35_), .b(new_n31_), .O(z06));
  nand4  g15(.a(new_n25_), .b(new_n28_), .c(new_n22_), .d(x0), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n31_), .O(z07));
  inv1   g17(.a(x0), .O(new_n39_));
  nand4  g18(.a(x3), .b(x2), .c(x1), .d(new_n39_), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n31_), .O(z08));
  nand3  g20(.a(x4), .b(new_n28_), .c(new_n39_), .O(new_n43_));
  aoi21  g21(.a(new_n43_), .b(x3), .c(new_n22_), .O(z10));
  nand4  g22(.a(x3), .b(x2), .c(new_n22_), .d(new_n39_), .O(new_n45_));
  nor2   g23(.a(new_n45_), .b(new_n31_), .O(z12));
  nand4  g24(.a(new_n25_), .b(x2), .c(new_n22_), .d(new_n39_), .O(new_n47_));
  nor2   g25(.a(new_n47_), .b(new_n31_), .O(z13));
  nand2  g26(.a(new_n22_), .b(new_n39_), .O(new_n49_));
  nand3  g27(.a(x4), .b(x3), .c(new_n28_), .O(new_n50_));
  oai21  g28(.a(new_n50_), .b(new_n49_), .c(new_n26_), .O(z14));
  aoi21  g29(.a(new_n43_), .b(new_n22_), .c(x3), .O(z15));
  zero   g30(.O(z09));
  inv1   g31(.a(new_n26_), .O(z03));
  inv1   g32(.a(new_n26_), .O(z11));
endmodule


