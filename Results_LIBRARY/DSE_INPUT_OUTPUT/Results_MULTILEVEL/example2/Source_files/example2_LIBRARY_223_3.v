// Benchmark "FAU" written by ABC on Thu Aug 13 21:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n199_, new_n200_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n208_, new_n209_, new_n211_, new_n212_,
    new_n214_, new_n215_, new_n217_, new_n218_, new_n220_, new_n221_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n229_,
    new_n230_, new_n232_, new_n233_, new_n245_, new_n246_, new_n247_,
    new_n248_, new_n249_, new_n250_, new_n251_, new_n252_, new_n253_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n259_, new_n260_,
    new_n262_, new_n263_, new_n264_, new_n265_, new_n266_, new_n268_,
    new_n269_, new_n270_, new_n271_, new_n272_, new_n274_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n282_,
    new_n283_, new_n284_, new_n285_, new_n286_, new_n288_, new_n289_,
    new_n290_, new_n291_, new_n292_, new_n294_, new_n295_, new_n296_,
    new_n297_, new_n300_, new_n301_, new_n302_, new_n303_, new_n304_,
    new_n306_, new_n307_, new_n309_, new_n310_, new_n311_, new_n312_,
    new_n313_, new_n315_, new_n316_, new_n317_, new_n319_, new_n320_,
    new_n322_, new_n324_, new_n325_, new_n326_, new_n328_, new_n330_,
    new_n331_, new_n332_, new_n333_, new_n334_, new_n335_, new_n336_,
    new_n337_, new_n338_, new_n340_;
  aoi21  g000(.a(x78), .b(x77), .c(x01), .O(new_n152_));
  oai21  g001(.a(new_n152_), .b(x52), .c(x40), .O(new_n153_));
  inv1   g002(.a(x40), .O(new_n154_));
  nand2  g003(.a(new_n154_), .b(x06), .O(new_n155_));
  nand2  g004(.a(x79), .b(x78), .O(new_n156_));
  nand3  g005(.a(new_n156_), .b(new_n155_), .c(new_n153_), .O(z00));
  inv1   g006(.a(x01), .O(new_n158_));
  nand2  g007(.a(x79), .b(x77), .O(new_n159_));
  nand2  g008(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g009(.a(new_n160_), .b(new_n156_), .O(z01));
  inv1   g010(.a(x78), .O(new_n162_));
  inv1   g011(.a(x79), .O(new_n163_));
  nand3  g012(.a(x77), .b(x66), .c(new_n158_), .O(new_n164_));
  aoi21  g013(.a(new_n164_), .b(new_n162_), .c(new_n163_), .O(z02));
  nand4  g014(.a(new_n163_), .b(x78), .c(x52), .d(new_n158_), .O(new_n166_));
  inv1   g015(.a(new_n166_), .O(z03));
  inv1   g016(.a(x77), .O(new_n168_));
  nand2  g017(.a(new_n163_), .b(new_n168_), .O(new_n169_));
  aoi21  g018(.a(new_n169_), .b(x78), .c(x01), .O(z04));
  nand2  g019(.a(x65), .b(x40), .O(new_n171_));
  nand2  g020(.a(new_n154_), .b(x23), .O(new_n172_));
  nand3  g021(.a(new_n172_), .b(new_n171_), .c(new_n156_), .O(z05));
  inv1   g022(.a(new_n156_), .O(z25));
  nand2  g023(.a(new_n154_), .b(x24), .O(new_n175_));
  nand2  g024(.a(x64), .b(x40), .O(new_n176_));
  aoi21  g025(.a(new_n176_), .b(new_n175_), .c(z25), .O(z06));
  nand2  g026(.a(x63), .b(x40), .O(new_n178_));
  nand2  g027(.a(new_n154_), .b(x25), .O(new_n179_));
  nand3  g028(.a(new_n179_), .b(new_n178_), .c(new_n156_), .O(z07));
  nand2  g029(.a(x62), .b(x40), .O(new_n181_));
  nand2  g030(.a(new_n154_), .b(x26), .O(new_n182_));
  nand3  g031(.a(new_n182_), .b(new_n181_), .c(new_n156_), .O(z08));
  nand2  g032(.a(x61), .b(x40), .O(new_n184_));
  nand2  g033(.a(new_n154_), .b(x27), .O(new_n185_));
  nand3  g034(.a(new_n185_), .b(new_n184_), .c(new_n156_), .O(z09));
  nand2  g035(.a(new_n154_), .b(x28), .O(new_n187_));
  nand2  g036(.a(x60), .b(x40), .O(new_n188_));
  aoi21  g037(.a(new_n188_), .b(new_n187_), .c(z25), .O(z10));
  nand2  g038(.a(new_n154_), .b(x29), .O(new_n190_));
  nand2  g039(.a(x59), .b(x40), .O(new_n191_));
  aoi21  g040(.a(new_n191_), .b(new_n190_), .c(z25), .O(z11));
  nand2  g041(.a(x58), .b(x40), .O(new_n193_));
  nand2  g042(.a(new_n154_), .b(x30), .O(new_n194_));
  nand3  g043(.a(new_n194_), .b(new_n193_), .c(new_n156_), .O(z12));
  nand2  g044(.a(x57), .b(x40), .O(new_n196_));
  nand2  g045(.a(new_n154_), .b(x31), .O(new_n197_));
  nand3  g046(.a(new_n197_), .b(new_n196_), .c(new_n156_), .O(z13));
  nand2  g047(.a(x51), .b(x40), .O(new_n199_));
  nand2  g048(.a(new_n154_), .b(x32), .O(new_n200_));
  nand3  g049(.a(new_n200_), .b(new_n199_), .c(new_n156_), .O(z14));
  nand2  g050(.a(new_n154_), .b(x33), .O(new_n202_));
  nand2  g051(.a(x50), .b(x40), .O(new_n203_));
  aoi21  g052(.a(new_n203_), .b(new_n202_), .c(z25), .O(z15));
  nand2  g053(.a(x49), .b(x40), .O(new_n205_));
  nand2  g054(.a(new_n154_), .b(x34), .O(new_n206_));
  nand3  g055(.a(new_n206_), .b(new_n205_), .c(new_n156_), .O(z16));
  nand2  g056(.a(x48), .b(x40), .O(new_n208_));
  nand2  g057(.a(new_n154_), .b(x35), .O(new_n209_));
  nand3  g058(.a(new_n209_), .b(new_n208_), .c(new_n156_), .O(z17));
  nand2  g059(.a(x47), .b(x40), .O(new_n211_));
  nand2  g060(.a(new_n154_), .b(x36), .O(new_n212_));
  nand3  g061(.a(new_n212_), .b(new_n211_), .c(new_n156_), .O(z18));
  nand2  g062(.a(x46), .b(x40), .O(new_n214_));
  nand2  g063(.a(new_n154_), .b(x37), .O(new_n215_));
  nand3  g064(.a(new_n215_), .b(new_n214_), .c(new_n156_), .O(z19));
  nand2  g065(.a(x45), .b(x40), .O(new_n217_));
  nand2  g066(.a(new_n154_), .b(x38), .O(new_n218_));
  nand3  g067(.a(new_n218_), .b(new_n217_), .c(new_n156_), .O(z20));
  nand2  g068(.a(new_n154_), .b(x39), .O(new_n220_));
  nand2  g069(.a(x44), .b(x40), .O(new_n221_));
  aoi21  g070(.a(new_n221_), .b(new_n220_), .c(z25), .O(z21));
  xnor2a g071(.a(x84), .b(x81), .O(new_n223_));
  nand4  g072(.a(new_n223_), .b(x79), .c(x77), .d(x66), .O(new_n224_));
  nand2  g073(.a(x78), .b(x04), .O(new_n225_));
  oai21  g074(.a(new_n224_), .b(x41), .c(new_n225_), .O(new_n226_));
  nand2  g075(.a(new_n226_), .b(new_n158_), .O(new_n227_));
  nand2  g076(.a(new_n227_), .b(new_n156_), .O(z22));
  inv1   g077(.a(x04), .O(new_n229_));
  nand3  g078(.a(new_n163_), .b(x05), .c(new_n229_), .O(new_n230_));
  nand4  g079(.a(new_n230_), .b(new_n156_), .c(new_n158_), .d(x00), .O(z23));
  nor2   g080(.a(x79), .b(x43), .O(new_n232_));
  nand4  g081(.a(new_n232_), .b(x05), .c(new_n229_), .d(new_n158_), .O(new_n233_));
  nand2  g082(.a(new_n233_), .b(new_n156_), .O(z24));
  nand2  g083(.a(x52), .b(x15), .O(new_n245_));
  inv1   g084(.a(x52), .O(new_n246_));
  nand2  g085(.a(new_n246_), .b(x07), .O(new_n247_));
  aoi21  g086(.a(new_n247_), .b(new_n245_), .c(x79), .O(new_n248_));
  nand4  g087(.a(new_n248_), .b(x78), .c(new_n168_), .d(x04), .O(new_n249_));
  xor2a  g088(.a(x84), .b(x81), .O(new_n250_));
  nor2   g089(.a(x75), .b(x67), .O(new_n251_));
  nor2   g090(.a(new_n251_), .b(new_n250_), .O(new_n252_));
  nand4  g091(.a(new_n252_), .b(x79), .c(new_n162_), .d(x77), .O(new_n253_));
  aoi21  g092(.a(new_n253_), .b(new_n249_), .c(x01), .O(z47));
  nand2  g093(.a(x52), .b(x16), .O(new_n255_));
  nand2  g094(.a(new_n246_), .b(x08), .O(new_n256_));
  aoi21  g095(.a(new_n256_), .b(new_n255_), .c(x79), .O(new_n257_));
  nand4  g096(.a(new_n257_), .b(x78), .c(new_n168_), .d(x04), .O(new_n258_));
  nor2   g097(.a(new_n250_), .b(new_n163_), .O(new_n259_));
  nand4  g098(.a(new_n259_), .b(new_n162_), .c(x77), .d(x68), .O(new_n260_));
  aoi21  g099(.a(new_n260_), .b(new_n258_), .c(x01), .O(z48));
  nand2  g100(.a(x52), .b(x17), .O(new_n262_));
  nand2  g101(.a(new_n246_), .b(x09), .O(new_n263_));
  aoi21  g102(.a(new_n263_), .b(new_n262_), .c(x79), .O(new_n264_));
  nand4  g103(.a(new_n264_), .b(x78), .c(new_n168_), .d(x04), .O(new_n265_));
  nand4  g104(.a(new_n259_), .b(new_n162_), .c(x77), .d(x69), .O(new_n266_));
  aoi21  g105(.a(new_n266_), .b(new_n265_), .c(x01), .O(z49));
  nand2  g106(.a(x52), .b(x18), .O(new_n268_));
  nand2  g107(.a(new_n246_), .b(x10), .O(new_n269_));
  aoi21  g108(.a(new_n269_), .b(new_n268_), .c(x79), .O(new_n270_));
  nand4  g109(.a(new_n270_), .b(x78), .c(new_n168_), .d(x04), .O(new_n271_));
  nand4  g110(.a(new_n259_), .b(new_n162_), .c(x77), .d(x70), .O(new_n272_));
  aoi21  g111(.a(new_n272_), .b(new_n271_), .c(x01), .O(z50));
  inv1   g112(.a(x11), .O(new_n274_));
  nand2  g113(.a(x52), .b(x19), .O(new_n275_));
  oai21  g114(.a(x52), .b(new_n274_), .c(new_n275_), .O(new_n276_));
  nand4  g115(.a(new_n276_), .b(x78), .c(new_n168_), .d(x04), .O(new_n277_));
  nand3  g116(.a(new_n259_), .b(x77), .c(x71), .O(new_n278_));
  nand2  g117(.a(new_n278_), .b(new_n277_), .O(new_n279_));
  nand2  g118(.a(new_n279_), .b(new_n158_), .O(new_n280_));
  nand2  g119(.a(new_n280_), .b(new_n156_), .O(z51));
  nand2  g120(.a(x52), .b(x20), .O(new_n282_));
  nand2  g121(.a(new_n246_), .b(x12), .O(new_n283_));
  aoi21  g122(.a(new_n283_), .b(new_n282_), .c(x79), .O(new_n284_));
  nand4  g123(.a(new_n284_), .b(x78), .c(new_n168_), .d(x04), .O(new_n285_));
  nand4  g124(.a(new_n259_), .b(new_n162_), .c(x77), .d(x72), .O(new_n286_));
  aoi21  g125(.a(new_n286_), .b(new_n285_), .c(x01), .O(z52));
  nand2  g126(.a(x52), .b(x21), .O(new_n288_));
  nand2  g127(.a(new_n246_), .b(x13), .O(new_n289_));
  aoi21  g128(.a(new_n289_), .b(new_n288_), .c(x79), .O(new_n290_));
  nand4  g129(.a(new_n290_), .b(x78), .c(new_n168_), .d(x04), .O(new_n291_));
  nand4  g130(.a(new_n259_), .b(new_n162_), .c(x77), .d(x73), .O(new_n292_));
  aoi21  g131(.a(new_n292_), .b(new_n291_), .c(x01), .O(z53));
  nand2  g132(.a(x52), .b(x22), .O(new_n294_));
  nand2  g133(.a(new_n246_), .b(x14), .O(new_n295_));
  aoi21  g134(.a(new_n295_), .b(new_n294_), .c(x79), .O(new_n296_));
  nand4  g135(.a(new_n296_), .b(x78), .c(new_n168_), .d(x04), .O(new_n297_));
  nor2   g136(.a(new_n297_), .b(x01), .O(z54));
  inv1   g137(.a(x76), .O(new_n300_));
  nand2  g138(.a(new_n162_), .b(new_n300_), .O(new_n301_));
  xor2a  g139(.a(x84), .b(x81), .O(new_n302_));
  oai21  g140(.a(new_n302_), .b(new_n301_), .c(x79), .O(new_n303_));
  aoi21  g141(.a(new_n162_), .b(new_n168_), .c(x01), .O(new_n304_));
  nand3  g142(.a(new_n304_), .b(new_n303_), .c(x00), .O(z56));
  inv1   g143(.a(x00), .O(new_n306_));
  nand2  g144(.a(new_n156_), .b(x03), .O(new_n307_));
  nor4   g145(.a(new_n307_), .b(x02), .c(x01), .d(new_n306_), .O(z57));
  nand2  g146(.a(x78), .b(new_n168_), .O(new_n309_));
  inv1   g147(.a(x42), .O(new_n310_));
  nand4  g148(.a(new_n162_), .b(x77), .c(new_n310_), .d(x40), .O(new_n311_));
  nand3  g149(.a(new_n311_), .b(new_n309_), .c(x04), .O(new_n312_));
  nand3  g150(.a(new_n312_), .b(new_n163_), .c(new_n158_), .O(new_n313_));
  inv1   g151(.a(new_n313_), .O(z58));
  oai21  g152(.a(x78), .b(x40), .c(x77), .O(new_n315_));
  nand2  g153(.a(new_n315_), .b(x04), .O(new_n316_));
  nand3  g154(.a(new_n316_), .b(new_n163_), .c(new_n158_), .O(new_n317_));
  nand2  g155(.a(new_n317_), .b(new_n156_), .O(z59));
  nand4  g156(.a(new_n302_), .b(x79), .c(new_n162_), .d(x77), .O(new_n319_));
  oai21  g157(.a(x78), .b(new_n229_), .c(new_n163_), .O(new_n320_));
  aoi21  g158(.a(new_n320_), .b(new_n319_), .c(x01), .O(z60));
  nand4  g159(.a(new_n223_), .b(x80), .c(x79), .d(new_n162_), .O(new_n322_));
  nor3   g160(.a(new_n322_), .b(new_n168_), .c(x01), .O(z61));
  nand2  g161(.a(x84), .b(x81), .O(new_n324_));
  oai21  g162(.a(new_n324_), .b(new_n159_), .c(new_n225_), .O(new_n325_));
  nand2  g163(.a(new_n325_), .b(new_n158_), .O(new_n326_));
  nand2  g164(.a(new_n326_), .b(new_n156_), .O(z62));
  nand4  g165(.a(new_n223_), .b(x82), .c(x79), .d(new_n162_), .O(new_n328_));
  nor3   g166(.a(new_n328_), .b(new_n168_), .c(x01), .O(z63));
  inv1   g167(.a(x81), .O(new_n330_));
  inv1   g168(.a(x84), .O(new_n331_));
  nand2  g169(.a(new_n331_), .b(new_n330_), .O(new_n332_));
  nand3  g170(.a(x84), .b(x81), .c(new_n162_), .O(new_n333_));
  nand2  g171(.a(new_n333_), .b(new_n332_), .O(new_n334_));
  nand4  g172(.a(new_n334_), .b(x83), .c(x79), .d(x77), .O(new_n335_));
  nand3  g173(.a(x78), .b(new_n168_), .c(x04), .O(new_n336_));
  nand2  g174(.a(new_n336_), .b(new_n335_), .O(new_n337_));
  nand2  g175(.a(new_n337_), .b(new_n158_), .O(new_n338_));
  nand2  g176(.a(new_n338_), .b(new_n156_), .O(z64));
  nand3  g177(.a(new_n162_), .b(x77), .c(new_n158_), .O(new_n340_));
  nor3   g178(.a(new_n340_), .b(new_n324_), .c(new_n163_), .O(z65));
  zero   g179(.O(z26));
  zero   g180(.O(z27));
  zero   g181(.O(z28));
  zero   g182(.O(z29));
  zero   g183(.O(z33));
  zero   g184(.O(z34));
  zero   g185(.O(z36));
  zero   g186(.O(z38));
  zero   g187(.O(z42));
  zero   g188(.O(z45));
  zero   g189(.O(z55));
  inv1   g190(.a(new_n156_), .O(z30));
  inv1   g191(.a(new_n156_), .O(z31));
  inv1   g192(.a(new_n156_), .O(z32));
  inv1   g193(.a(new_n156_), .O(z35));
  inv1   g194(.a(new_n156_), .O(z37));
  inv1   g195(.a(new_n156_), .O(z39));
  inv1   g196(.a(new_n156_), .O(z40));
  inv1   g197(.a(new_n156_), .O(z41));
  inv1   g198(.a(new_n156_), .O(z43));
  inv1   g199(.a(new_n156_), .O(z44));
  inv1   g200(.a(new_n156_), .O(z46));
endmodule


