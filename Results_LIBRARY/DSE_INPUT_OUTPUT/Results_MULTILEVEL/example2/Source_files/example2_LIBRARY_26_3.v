// Benchmark "FAU" written by ABC on Thu Aug 13 21:57:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n191_, new_n192_, new_n194_, new_n195_,
    new_n197_, new_n198_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n206_, new_n207_, new_n209_, new_n210_, new_n212_, new_n213_,
    new_n215_, new_n216_, new_n218_, new_n219_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n227_, new_n228_, new_n229_,
    new_n230_, new_n232_, new_n246_, new_n247_, new_n248_, new_n249_,
    new_n250_, new_n251_, new_n252_, new_n253_, new_n254_, new_n256_,
    new_n257_, new_n258_, new_n259_, new_n260_, new_n261_, new_n262_,
    new_n264_, new_n265_, new_n266_, new_n267_, new_n268_, new_n269_,
    new_n271_, new_n272_, new_n273_, new_n274_, new_n275_, new_n276_,
    new_n277_, new_n279_, new_n280_, new_n281_, new_n282_, new_n283_,
    new_n285_, new_n286_, new_n287_, new_n288_, new_n289_, new_n290_,
    new_n291_, new_n293_, new_n294_, new_n295_, new_n296_, new_n297_,
    new_n299_, new_n300_, new_n301_, new_n302_, new_n305_, new_n306_,
    new_n307_, new_n308_, new_n310_, new_n312_, new_n313_, new_n314_,
    new_n315_, new_n316_, new_n317_, new_n319_, new_n320_, new_n322_,
    new_n323_, new_n324_, new_n325_, new_n327_, new_n329_, new_n330_,
    new_n331_, new_n333_, new_n335_, new_n336_, new_n337_, new_n338_,
    new_n340_;
  inv1   g000(.a(x40), .O(new_n152_));
  inv1   g001(.a(x77), .O(new_n153_));
  inv1   g002(.a(x79), .O(new_n154_));
  nand2  g003(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  aoi21  g004(.a(new_n155_), .b(x78), .c(x01), .O(z04));
  nand2  g005(.a(x79), .b(x78), .O(new_n157_));
  aoi21  g006(.a(new_n157_), .b(x52), .c(z04), .O(new_n158_));
  nand3  g007(.a(new_n157_), .b(new_n152_), .c(x06), .O(new_n159_));
  oai21  g008(.a(new_n158_), .b(new_n152_), .c(new_n159_), .O(z00));
  inv1   g009(.a(x78), .O(new_n161_));
  nand2  g010(.a(new_n161_), .b(new_n153_), .O(new_n162_));
  aoi21  g011(.a(new_n162_), .b(x79), .c(x01), .O(z01));
  inv1   g012(.a(x01), .O(new_n164_));
  nand4  g013(.a(new_n161_), .b(x77), .c(x66), .d(new_n164_), .O(new_n165_));
  nor2   g014(.a(new_n165_), .b(new_n154_), .O(z02));
  nand2  g015(.a(x52), .b(new_n164_), .O(new_n167_));
  aoi21  g016(.a(new_n167_), .b(new_n154_), .c(new_n161_), .O(z03));
  nand2  g017(.a(x65), .b(x40), .O(new_n169_));
  nand2  g018(.a(new_n152_), .b(x23), .O(new_n170_));
  nand3  g019(.a(new_n170_), .b(new_n169_), .c(new_n157_), .O(z05));
  nand2  g020(.a(x64), .b(x40), .O(new_n172_));
  nand2  g021(.a(new_n152_), .b(x24), .O(new_n173_));
  nand3  g022(.a(new_n173_), .b(new_n172_), .c(new_n157_), .O(z06));
  inv1   g023(.a(new_n157_), .O(z27));
  nand2  g024(.a(new_n152_), .b(x25), .O(new_n176_));
  nand2  g025(.a(x63), .b(x40), .O(new_n177_));
  aoi21  g026(.a(new_n177_), .b(new_n176_), .c(z27), .O(z07));
  nand2  g027(.a(x62), .b(x40), .O(new_n179_));
  nand2  g028(.a(new_n152_), .b(x26), .O(new_n180_));
  nand3  g029(.a(new_n180_), .b(new_n179_), .c(new_n157_), .O(z08));
  nand2  g030(.a(x61), .b(x40), .O(new_n182_));
  nand2  g031(.a(new_n152_), .b(x27), .O(new_n183_));
  nand3  g032(.a(new_n183_), .b(new_n182_), .c(new_n157_), .O(z09));
  nand2  g033(.a(new_n152_), .b(x28), .O(new_n185_));
  nand2  g034(.a(x60), .b(x40), .O(new_n186_));
  aoi21  g035(.a(new_n186_), .b(new_n185_), .c(z27), .O(z10));
  nand2  g036(.a(x59), .b(x40), .O(new_n188_));
  nand2  g037(.a(new_n152_), .b(x29), .O(new_n189_));
  nand3  g038(.a(new_n189_), .b(new_n188_), .c(new_n157_), .O(z11));
  nand2  g039(.a(x58), .b(x40), .O(new_n191_));
  nand2  g040(.a(new_n152_), .b(x30), .O(new_n192_));
  nand3  g041(.a(new_n192_), .b(new_n191_), .c(new_n157_), .O(z12));
  nand2  g042(.a(x57), .b(x40), .O(new_n194_));
  nand2  g043(.a(new_n152_), .b(x31), .O(new_n195_));
  nand3  g044(.a(new_n195_), .b(new_n194_), .c(new_n157_), .O(z13));
  nand2  g045(.a(x51), .b(x40), .O(new_n197_));
  nand2  g046(.a(new_n152_), .b(x32), .O(new_n198_));
  nand3  g047(.a(new_n198_), .b(new_n197_), .c(new_n157_), .O(z14));
  nand2  g048(.a(new_n152_), .b(x33), .O(new_n200_));
  nand2  g049(.a(x50), .b(x40), .O(new_n201_));
  aoi21  g050(.a(new_n201_), .b(new_n200_), .c(z27), .O(z15));
  nand2  g051(.a(new_n152_), .b(x34), .O(new_n203_));
  nand2  g052(.a(x49), .b(x40), .O(new_n204_));
  aoi21  g053(.a(new_n204_), .b(new_n203_), .c(z27), .O(z16));
  nand2  g054(.a(new_n152_), .b(x35), .O(new_n206_));
  nand2  g055(.a(x48), .b(x40), .O(new_n207_));
  aoi21  g056(.a(new_n207_), .b(new_n206_), .c(z27), .O(z17));
  nand2  g057(.a(new_n152_), .b(x36), .O(new_n209_));
  nand2  g058(.a(x47), .b(x40), .O(new_n210_));
  aoi21  g059(.a(new_n210_), .b(new_n209_), .c(z27), .O(z18));
  nand2  g060(.a(x46), .b(x40), .O(new_n212_));
  nand2  g061(.a(new_n152_), .b(x37), .O(new_n213_));
  nand3  g062(.a(new_n213_), .b(new_n212_), .c(new_n157_), .O(z19));
  nand2  g063(.a(new_n152_), .b(x38), .O(new_n215_));
  nand2  g064(.a(x45), .b(x40), .O(new_n216_));
  aoi21  g065(.a(new_n216_), .b(new_n215_), .c(z27), .O(z20));
  nand2  g066(.a(new_n152_), .b(x39), .O(new_n218_));
  nand2  g067(.a(x44), .b(x40), .O(new_n219_));
  aoi21  g068(.a(new_n219_), .b(new_n218_), .c(z27), .O(z21));
  xnor2a g069(.a(x84), .b(x81), .O(new_n221_));
  nand4  g070(.a(new_n221_), .b(x79), .c(x77), .d(x66), .O(new_n222_));
  nand2  g071(.a(x78), .b(x04), .O(new_n223_));
  oai21  g072(.a(new_n222_), .b(x41), .c(new_n223_), .O(new_n224_));
  nand2  g073(.a(new_n224_), .b(new_n164_), .O(new_n225_));
  nand2  g074(.a(new_n225_), .b(new_n157_), .O(z22));
  inv1   g075(.a(x00), .O(new_n227_));
  oai21  g076(.a(x01), .b(new_n227_), .c(new_n157_), .O(new_n228_));
  inv1   g077(.a(x04), .O(new_n229_));
  nand3  g078(.a(new_n154_), .b(x05), .c(new_n229_), .O(new_n230_));
  nand2  g079(.a(new_n230_), .b(new_n228_), .O(z23));
  nand3  g080(.a(x05), .b(new_n229_), .c(new_n164_), .O(new_n232_));
  nor3   g081(.a(new_n232_), .b(x79), .c(x43), .O(z24));
  nand2  g082(.a(x52), .b(x15), .O(new_n246_));
  inv1   g083(.a(x52), .O(new_n247_));
  nand2  g084(.a(new_n247_), .b(x07), .O(new_n248_));
  aoi21  g085(.a(new_n248_), .b(new_n246_), .c(x79), .O(new_n249_));
  nand4  g086(.a(new_n249_), .b(x78), .c(new_n153_), .d(x04), .O(new_n250_));
  xor2a  g087(.a(x84), .b(x81), .O(new_n251_));
  nor2   g088(.a(x75), .b(x67), .O(new_n252_));
  nor2   g089(.a(new_n252_), .b(new_n251_), .O(new_n253_));
  nand4  g090(.a(new_n253_), .b(x79), .c(new_n161_), .d(x77), .O(new_n254_));
  aoi21  g091(.a(new_n254_), .b(new_n250_), .c(x01), .O(z47));
  inv1   g092(.a(x08), .O(new_n256_));
  nand2  g093(.a(x52), .b(x16), .O(new_n257_));
  oai21  g094(.a(x52), .b(new_n256_), .c(new_n257_), .O(new_n258_));
  nand4  g095(.a(new_n258_), .b(x78), .c(new_n153_), .d(x04), .O(new_n259_));
  nand4  g096(.a(new_n221_), .b(x79), .c(x77), .d(x68), .O(new_n260_));
  nand2  g097(.a(new_n260_), .b(new_n259_), .O(new_n261_));
  nand2  g098(.a(new_n261_), .b(new_n164_), .O(new_n262_));
  nand2  g099(.a(new_n262_), .b(new_n157_), .O(z48));
  nand2  g100(.a(x52), .b(x17), .O(new_n264_));
  nand2  g101(.a(new_n247_), .b(x09), .O(new_n265_));
  aoi21  g102(.a(new_n265_), .b(new_n264_), .c(x79), .O(new_n266_));
  nand4  g103(.a(new_n266_), .b(x78), .c(new_n153_), .d(x04), .O(new_n267_));
  nor2   g104(.a(new_n251_), .b(new_n154_), .O(new_n268_));
  nand4  g105(.a(new_n268_), .b(new_n161_), .c(x77), .d(x69), .O(new_n269_));
  aoi21  g106(.a(new_n269_), .b(new_n267_), .c(x01), .O(z49));
  inv1   g107(.a(x10), .O(new_n271_));
  nand2  g108(.a(x52), .b(x18), .O(new_n272_));
  oai21  g109(.a(x52), .b(new_n271_), .c(new_n272_), .O(new_n273_));
  nand4  g110(.a(new_n273_), .b(x78), .c(new_n153_), .d(x04), .O(new_n274_));
  nand4  g111(.a(new_n221_), .b(x79), .c(x77), .d(x70), .O(new_n275_));
  nand2  g112(.a(new_n275_), .b(new_n274_), .O(new_n276_));
  nand2  g113(.a(new_n276_), .b(new_n164_), .O(new_n277_));
  nand2  g114(.a(new_n277_), .b(new_n157_), .O(z50));
  nand2  g115(.a(x52), .b(x19), .O(new_n279_));
  nand2  g116(.a(new_n247_), .b(x11), .O(new_n280_));
  aoi21  g117(.a(new_n280_), .b(new_n279_), .c(x79), .O(new_n281_));
  nand4  g118(.a(new_n281_), .b(x78), .c(new_n153_), .d(x04), .O(new_n282_));
  nand4  g119(.a(new_n268_), .b(new_n161_), .c(x77), .d(x71), .O(new_n283_));
  aoi21  g120(.a(new_n283_), .b(new_n282_), .c(x01), .O(z51));
  inv1   g121(.a(x12), .O(new_n285_));
  nand2  g122(.a(x52), .b(x20), .O(new_n286_));
  oai21  g123(.a(x52), .b(new_n285_), .c(new_n286_), .O(new_n287_));
  nand4  g124(.a(new_n287_), .b(x78), .c(new_n153_), .d(x04), .O(new_n288_));
  nand4  g125(.a(new_n221_), .b(x79), .c(x77), .d(x72), .O(new_n289_));
  nand2  g126(.a(new_n289_), .b(new_n288_), .O(new_n290_));
  nand2  g127(.a(new_n290_), .b(new_n164_), .O(new_n291_));
  nand2  g128(.a(new_n291_), .b(new_n157_), .O(z52));
  nand2  g129(.a(x52), .b(x21), .O(new_n293_));
  nand2  g130(.a(new_n247_), .b(x13), .O(new_n294_));
  aoi21  g131(.a(new_n294_), .b(new_n293_), .c(x79), .O(new_n295_));
  nand4  g132(.a(new_n295_), .b(x78), .c(new_n153_), .d(x04), .O(new_n296_));
  nand4  g133(.a(new_n268_), .b(new_n161_), .c(x77), .d(x73), .O(new_n297_));
  aoi21  g134(.a(new_n297_), .b(new_n296_), .c(x01), .O(z53));
  inv1   g135(.a(x14), .O(new_n299_));
  nand2  g136(.a(x52), .b(x22), .O(new_n300_));
  oai21  g137(.a(x52), .b(new_n299_), .c(new_n300_), .O(new_n301_));
  nand4  g138(.a(new_n301_), .b(new_n153_), .c(x04), .d(new_n164_), .O(new_n302_));
  aoi21  g139(.a(new_n302_), .b(new_n154_), .c(new_n161_), .O(z54));
  inv1   g140(.a(x76), .O(new_n305_));
  nand2  g141(.a(new_n161_), .b(new_n305_), .O(new_n306_));
  xor2a  g142(.a(x84), .b(x81), .O(new_n307_));
  oai21  g143(.a(new_n307_), .b(new_n306_), .c(x79), .O(new_n308_));
  nand4  g144(.a(new_n308_), .b(new_n162_), .c(new_n164_), .d(x00), .O(z56));
  nand2  g145(.a(new_n157_), .b(x03), .O(new_n310_));
  nor4   g146(.a(new_n310_), .b(x02), .c(x01), .d(new_n227_), .O(z57));
  nor2   g147(.a(x77), .b(x01), .O(new_n312_));
  oai21  g148(.a(new_n312_), .b(x79), .c(x78), .O(new_n313_));
  inv1   g149(.a(x42), .O(new_n314_));
  nand4  g150(.a(new_n161_), .b(x77), .c(new_n314_), .d(x40), .O(new_n315_));
  nand2  g151(.a(new_n315_), .b(x04), .O(new_n316_));
  nand3  g152(.a(new_n316_), .b(new_n154_), .c(new_n164_), .O(new_n317_));
  nand2  g153(.a(new_n317_), .b(new_n313_), .O(z58));
  oai21  g154(.a(x79), .b(new_n152_), .c(new_n161_), .O(new_n319_));
  aoi22  g155(.a(new_n319_), .b(x77), .c(new_n154_), .d(new_n229_), .O(new_n320_));
  oai21  g156(.a(new_n320_), .b(x01), .c(new_n157_), .O(z59));
  nand3  g157(.a(new_n307_), .b(x79), .c(x77), .O(new_n322_));
  aoi21  g158(.a(new_n154_), .b(new_n229_), .c(x78), .O(new_n323_));
  nand2  g159(.a(new_n323_), .b(new_n322_), .O(new_n324_));
  nand2  g160(.a(new_n324_), .b(new_n164_), .O(new_n325_));
  nand2  g161(.a(new_n325_), .b(new_n157_), .O(z60));
  nand4  g162(.a(new_n221_), .b(x80), .c(x79), .d(new_n161_), .O(new_n327_));
  nor3   g163(.a(new_n327_), .b(new_n153_), .c(x01), .O(z61));
  nand3  g164(.a(new_n154_), .b(x78), .c(x04), .O(new_n329_));
  and2   g165(.a(x84), .b(x81), .O(new_n330_));
  nand4  g166(.a(new_n330_), .b(x79), .c(new_n161_), .d(x77), .O(new_n331_));
  aoi21  g167(.a(new_n331_), .b(new_n329_), .c(x01), .O(z62));
  nand4  g168(.a(new_n221_), .b(x82), .c(x79), .d(new_n161_), .O(new_n333_));
  nor3   g169(.a(new_n333_), .b(new_n153_), .c(x01), .O(z63));
  nand4  g170(.a(new_n221_), .b(x83), .c(x79), .d(x77), .O(new_n335_));
  nand3  g171(.a(x78), .b(new_n153_), .c(x04), .O(new_n336_));
  nand2  g172(.a(new_n336_), .b(new_n335_), .O(new_n337_));
  nand2  g173(.a(new_n337_), .b(new_n164_), .O(new_n338_));
  nand2  g174(.a(new_n338_), .b(new_n157_), .O(z64));
  nand3  g175(.a(new_n330_), .b(x77), .c(new_n164_), .O(new_n340_));
  aoi21  g176(.a(new_n340_), .b(new_n161_), .c(new_n154_), .O(z65));
  zero   g177(.O(z25));
  zero   g178(.O(z26));
  zero   g179(.O(z28));
  zero   g180(.O(z29));
  zero   g181(.O(z31));
  zero   g182(.O(z32));
  zero   g183(.O(z34));
  zero   g184(.O(z35));
  zero   g185(.O(z39));
  zero   g186(.O(z40));
  zero   g187(.O(z42));
  zero   g188(.O(z43));
  zero   g189(.O(z55));
  inv1   g190(.a(new_n157_), .O(z30));
  inv1   g191(.a(new_n157_), .O(z33));
  inv1   g192(.a(new_n157_), .O(z36));
  inv1   g193(.a(new_n157_), .O(z37));
  inv1   g194(.a(new_n157_), .O(z38));
  inv1   g195(.a(new_n157_), .O(z41));
  inv1   g196(.a(new_n157_), .O(z44));
  inv1   g197(.a(new_n157_), .O(z45));
  inv1   g198(.a(new_n157_), .O(z46));
endmodule


