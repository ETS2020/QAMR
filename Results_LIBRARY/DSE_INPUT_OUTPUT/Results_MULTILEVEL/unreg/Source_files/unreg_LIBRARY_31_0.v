// Benchmark "FAU" written by ABC on Fri Aug 14 02:04:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(x03), .O(new_n55_));
  oai21  g003(.a(x21), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand3  g004(.a(new_n56_), .b(x19), .c(new_n53_), .O(new_n57_));
  nor2   g005(.a(x20), .b(x19), .O(new_n58_));
  nor2   g006(.a(x32), .b(x26), .O(new_n59_));
  nor2   g007(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n57_), .O(z00));
  nand2  g009(.a(new_n54_), .b(x02), .O(new_n62_));
  inv1   g010(.a(x22), .O(new_n63_));
  nand2  g011(.a(new_n63_), .b(x18), .O(new_n64_));
  nand2  g012(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand3  g013(.a(new_n65_), .b(x19), .c(new_n53_), .O(new_n66_));
  inv1   g014(.a(x19), .O(new_n67_));
  inv1   g015(.a(x21), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g017(.a(new_n69_), .b(new_n66_), .c(new_n59_), .O(z01));
  nand2  g018(.a(new_n54_), .b(x01), .O(new_n71_));
  oai21  g019(.a(x23), .b(new_n54_), .c(new_n71_), .O(new_n72_));
  nand3  g020(.a(new_n72_), .b(x19), .c(new_n53_), .O(new_n73_));
  aoi21  g021(.a(new_n63_), .b(new_n67_), .c(new_n59_), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(new_n73_), .O(z02));
  inv1   g023(.a(x00), .O(new_n76_));
  nand2  g024(.a(x18), .b(x16), .O(new_n77_));
  oai21  g025(.a(x18), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  nand3  g026(.a(new_n78_), .b(x19), .c(new_n53_), .O(new_n79_));
  inv1   g027(.a(x23), .O(new_n80_));
  nand2  g028(.a(new_n80_), .b(new_n67_), .O(new_n81_));
  aoi21  g029(.a(new_n81_), .b(new_n79_), .c(new_n59_), .O(z03));
  nand2  g030(.a(new_n54_), .b(x07), .O(new_n83_));
  inv1   g031(.a(x25), .O(new_n84_));
  nand2  g032(.a(new_n84_), .b(x18), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand3  g034(.a(new_n86_), .b(x19), .c(new_n53_), .O(new_n87_));
  inv1   g035(.a(x24), .O(new_n88_));
  nand2  g036(.a(new_n88_), .b(new_n67_), .O(new_n89_));
  aoi21  g037(.a(new_n89_), .b(new_n87_), .c(new_n59_), .O(z04));
  nand2  g038(.a(new_n54_), .b(x06), .O(new_n91_));
  oai21  g039(.a(x26), .b(new_n54_), .c(new_n91_), .O(new_n92_));
  nand3  g040(.a(new_n92_), .b(x19), .c(new_n53_), .O(new_n93_));
  aoi21  g041(.a(new_n84_), .b(new_n67_), .c(new_n59_), .O(new_n94_));
  nand2  g042(.a(new_n94_), .b(new_n93_), .O(z05));
  inv1   g043(.a(new_n59_), .O(new_n96_));
  nand2  g044(.a(new_n54_), .b(x05), .O(new_n97_));
  inv1   g045(.a(x27), .O(new_n98_));
  nand2  g046(.a(new_n98_), .b(x18), .O(new_n99_));
  nand2  g047(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand4  g048(.a(new_n100_), .b(new_n96_), .c(x19), .d(new_n53_), .O(new_n101_));
  inv1   g049(.a(x26), .O(new_n102_));
  nand3  g050(.a(x32), .b(new_n102_), .c(new_n67_), .O(new_n103_));
  nand2  g051(.a(new_n103_), .b(new_n101_), .O(z06));
  nand2  g052(.a(new_n54_), .b(x04), .O(new_n105_));
  oai21  g053(.a(x20), .b(new_n54_), .c(new_n105_), .O(new_n106_));
  nand3  g054(.a(new_n106_), .b(x19), .c(new_n53_), .O(new_n107_));
  inv1   g055(.a(x32), .O(new_n108_));
  aoi22  g056(.a(new_n108_), .b(new_n102_), .c(new_n98_), .d(new_n67_), .O(new_n109_));
  nand2  g057(.a(new_n109_), .b(new_n107_), .O(z07));
  nand2  g058(.a(new_n54_), .b(x11), .O(new_n111_));
  inv1   g059(.a(x29), .O(new_n112_));
  nand2  g060(.a(new_n112_), .b(x18), .O(new_n113_));
  nand2  g061(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand3  g062(.a(new_n114_), .b(x19), .c(new_n53_), .O(new_n115_));
  inv1   g063(.a(x28), .O(new_n116_));
  nand2  g064(.a(new_n116_), .b(new_n67_), .O(new_n117_));
  aoi21  g065(.a(new_n117_), .b(new_n115_), .c(new_n59_), .O(z08));
  nand2  g066(.a(new_n54_), .b(x10), .O(new_n119_));
  oai21  g067(.a(x30), .b(new_n54_), .c(new_n119_), .O(new_n120_));
  nand3  g068(.a(new_n120_), .b(x19), .c(new_n53_), .O(new_n121_));
  aoi21  g069(.a(new_n112_), .b(new_n67_), .c(new_n59_), .O(new_n122_));
  nand2  g070(.a(new_n122_), .b(new_n121_), .O(z09));
  nand2  g071(.a(new_n54_), .b(x09), .O(new_n124_));
  inv1   g072(.a(x31), .O(new_n125_));
  nand2  g073(.a(new_n125_), .b(x18), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g075(.a(new_n127_), .b(x19), .c(new_n53_), .O(new_n128_));
  inv1   g076(.a(x30), .O(new_n129_));
  nand2  g077(.a(new_n129_), .b(new_n67_), .O(new_n130_));
  aoi21  g078(.a(new_n130_), .b(new_n128_), .c(new_n59_), .O(z10));
  nand2  g079(.a(new_n54_), .b(x08), .O(new_n132_));
  nand2  g080(.a(new_n88_), .b(x18), .O(new_n133_));
  nand2  g081(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g082(.a(new_n134_), .b(x19), .c(new_n53_), .O(new_n135_));
  nand2  g083(.a(new_n125_), .b(new_n67_), .O(new_n136_));
  aoi21  g084(.a(new_n136_), .b(new_n135_), .c(new_n59_), .O(z11));
  nand2  g085(.a(new_n54_), .b(x15), .O(new_n138_));
  inv1   g086(.a(x33), .O(new_n139_));
  nand2  g087(.a(new_n139_), .b(x18), .O(new_n140_));
  nand2  g088(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand4  g089(.a(new_n141_), .b(new_n96_), .c(x19), .d(new_n53_), .O(new_n142_));
  nand3  g090(.a(new_n108_), .b(x26), .c(new_n67_), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n142_), .O(z12));
  nand2  g092(.a(new_n54_), .b(x14), .O(new_n145_));
  inv1   g093(.a(x34), .O(new_n146_));
  nand2  g094(.a(new_n146_), .b(x18), .O(new_n147_));
  nand2  g095(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand4  g096(.a(new_n148_), .b(new_n96_), .c(x19), .d(new_n53_), .O(new_n149_));
  nand2  g097(.a(new_n108_), .b(new_n102_), .O(new_n150_));
  nand3  g098(.a(new_n150_), .b(new_n139_), .c(new_n67_), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n149_), .O(z13));
  nand2  g100(.a(new_n54_), .b(x13), .O(new_n153_));
  inv1   g101(.a(x35), .O(new_n154_));
  nand2  g102(.a(new_n154_), .b(x18), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand3  g104(.a(new_n156_), .b(x19), .c(new_n53_), .O(new_n157_));
  nand2  g105(.a(new_n146_), .b(new_n67_), .O(new_n158_));
  aoi21  g106(.a(new_n158_), .b(new_n157_), .c(new_n59_), .O(z14));
  nand2  g107(.a(new_n54_), .b(x12), .O(new_n160_));
  oai21  g108(.a(x28), .b(new_n54_), .c(new_n160_), .O(new_n161_));
  nand3  g109(.a(new_n161_), .b(x19), .c(new_n53_), .O(new_n162_));
  aoi21  g110(.a(new_n154_), .b(new_n67_), .c(new_n59_), .O(new_n163_));
  nand2  g111(.a(new_n163_), .b(new_n162_), .O(z15));
endmodule


