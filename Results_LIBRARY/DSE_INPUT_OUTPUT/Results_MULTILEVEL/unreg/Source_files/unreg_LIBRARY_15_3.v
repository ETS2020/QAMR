// Benchmark "FAU" written by ABC on Fri Aug 14 02:04:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(x03), .O(new_n55_));
  oai21  g003(.a(x21), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand3  g004(.a(new_n56_), .b(x19), .c(new_n53_), .O(new_n57_));
  nor2   g005(.a(x20), .b(x19), .O(new_n58_));
  inv1   g006(.a(x21), .O(new_n59_));
  inv1   g007(.a(x32), .O(new_n60_));
  nor2   g008(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g009(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n57_), .O(z00));
  nand2  g011(.a(new_n54_), .b(x02), .O(new_n64_));
  oai21  g012(.a(x22), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  nand3  g013(.a(new_n65_), .b(x19), .c(new_n53_), .O(new_n66_));
  aoi21  g014(.a(new_n60_), .b(x21), .c(x19), .O(new_n67_));
  nor2   g015(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n66_), .O(z01));
  nand2  g017(.a(new_n54_), .b(x01), .O(new_n70_));
  inv1   g018(.a(x23), .O(new_n71_));
  nand2  g019(.a(new_n71_), .b(x18), .O(new_n72_));
  nand2  g020(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g021(.a(new_n73_), .b(x19), .c(new_n53_), .O(new_n74_));
  inv1   g022(.a(x19), .O(new_n75_));
  inv1   g023(.a(x22), .O(new_n76_));
  nand2  g024(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g025(.a(new_n77_), .b(new_n74_), .c(new_n61_), .O(z02));
  inv1   g026(.a(x00), .O(new_n79_));
  nand2  g027(.a(x18), .b(x16), .O(new_n80_));
  oai21  g028(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand3  g029(.a(new_n81_), .b(x19), .c(new_n53_), .O(new_n82_));
  aoi21  g030(.a(new_n71_), .b(new_n75_), .c(new_n61_), .O(new_n83_));
  nand2  g031(.a(new_n83_), .b(new_n82_), .O(z03));
  nand2  g032(.a(new_n54_), .b(x07), .O(new_n85_));
  inv1   g033(.a(x25), .O(new_n86_));
  nand2  g034(.a(new_n86_), .b(x18), .O(new_n87_));
  nand2  g035(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g036(.a(new_n88_), .b(x19), .c(new_n53_), .O(new_n89_));
  inv1   g037(.a(x24), .O(new_n90_));
  nand2  g038(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  aoi21  g039(.a(new_n91_), .b(new_n89_), .c(new_n61_), .O(z04));
  nand2  g040(.a(new_n54_), .b(x06), .O(new_n93_));
  inv1   g041(.a(x26), .O(new_n94_));
  nand2  g042(.a(new_n94_), .b(x18), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g044(.a(new_n96_), .b(x19), .c(new_n53_), .O(new_n97_));
  nand2  g045(.a(new_n86_), .b(new_n75_), .O(new_n98_));
  aoi21  g046(.a(new_n98_), .b(new_n97_), .c(new_n61_), .O(z05));
  nand2  g047(.a(new_n54_), .b(x05), .O(new_n100_));
  oai21  g048(.a(x27), .b(new_n54_), .c(new_n100_), .O(new_n101_));
  nand3  g049(.a(new_n101_), .b(x19), .c(new_n53_), .O(new_n102_));
  aoi21  g050(.a(new_n94_), .b(new_n75_), .c(new_n61_), .O(new_n103_));
  nand2  g051(.a(new_n103_), .b(new_n102_), .O(z06));
  nand2  g052(.a(new_n54_), .b(x04), .O(new_n105_));
  oai21  g053(.a(x20), .b(new_n54_), .c(new_n105_), .O(new_n106_));
  nand3  g054(.a(new_n106_), .b(x19), .c(new_n53_), .O(new_n107_));
  nor2   g055(.a(x27), .b(x19), .O(new_n108_));
  nor2   g056(.a(new_n108_), .b(new_n61_), .O(new_n109_));
  nand2  g057(.a(new_n109_), .b(new_n107_), .O(z07));
  nand2  g058(.a(new_n54_), .b(x11), .O(new_n111_));
  oai21  g059(.a(x29), .b(new_n54_), .c(new_n111_), .O(new_n112_));
  nand3  g060(.a(new_n112_), .b(x19), .c(new_n53_), .O(new_n113_));
  inv1   g061(.a(x28), .O(new_n114_));
  aoi21  g062(.a(new_n114_), .b(new_n75_), .c(new_n61_), .O(new_n115_));
  nand2  g063(.a(new_n115_), .b(new_n113_), .O(z08));
  nand2  g064(.a(new_n54_), .b(x10), .O(new_n117_));
  inv1   g065(.a(x30), .O(new_n118_));
  nand2  g066(.a(new_n118_), .b(x18), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand3  g068(.a(new_n120_), .b(x19), .c(new_n53_), .O(new_n121_));
  inv1   g069(.a(x29), .O(new_n122_));
  nand2  g070(.a(new_n122_), .b(new_n75_), .O(new_n123_));
  aoi21  g071(.a(new_n123_), .b(new_n121_), .c(new_n61_), .O(z09));
  nand2  g072(.a(new_n54_), .b(x09), .O(new_n125_));
  oai21  g073(.a(x31), .b(new_n54_), .c(new_n125_), .O(new_n126_));
  nand3  g074(.a(new_n126_), .b(x19), .c(new_n53_), .O(new_n127_));
  aoi21  g075(.a(new_n118_), .b(new_n75_), .c(new_n61_), .O(new_n128_));
  nand2  g076(.a(new_n128_), .b(new_n127_), .O(z10));
  nand2  g077(.a(new_n54_), .b(x08), .O(new_n130_));
  nand2  g078(.a(new_n90_), .b(x18), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g080(.a(new_n132_), .b(x19), .c(new_n53_), .O(new_n133_));
  inv1   g081(.a(x31), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n75_), .O(new_n135_));
  aoi21  g083(.a(new_n135_), .b(new_n133_), .c(new_n61_), .O(z11));
  nand2  g084(.a(new_n54_), .b(x15), .O(new_n137_));
  oai21  g085(.a(x33), .b(new_n54_), .c(new_n137_), .O(new_n138_));
  nand3  g086(.a(new_n138_), .b(x19), .c(new_n53_), .O(new_n139_));
  aoi21  g087(.a(x32), .b(new_n59_), .c(x19), .O(new_n140_));
  nor2   g088(.a(new_n140_), .b(new_n61_), .O(new_n141_));
  nand2  g089(.a(new_n141_), .b(new_n139_), .O(z12));
  nand2  g090(.a(new_n54_), .b(x14), .O(new_n143_));
  inv1   g091(.a(x34), .O(new_n144_));
  nand2  g092(.a(new_n144_), .b(x18), .O(new_n145_));
  nand2  g093(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand3  g094(.a(new_n146_), .b(x19), .c(new_n53_), .O(new_n147_));
  inv1   g095(.a(x33), .O(new_n148_));
  nand2  g096(.a(new_n148_), .b(new_n75_), .O(new_n149_));
  aoi21  g097(.a(new_n149_), .b(new_n147_), .c(new_n61_), .O(z13));
  nand2  g098(.a(new_n54_), .b(x13), .O(new_n151_));
  oai21  g099(.a(x35), .b(new_n54_), .c(new_n151_), .O(new_n152_));
  nand3  g100(.a(new_n152_), .b(x19), .c(new_n53_), .O(new_n153_));
  aoi21  g101(.a(new_n144_), .b(new_n75_), .c(new_n61_), .O(new_n154_));
  nand2  g102(.a(new_n154_), .b(new_n153_), .O(z14));
  nand2  g103(.a(new_n54_), .b(x12), .O(new_n156_));
  nand2  g104(.a(new_n114_), .b(x18), .O(new_n157_));
  nand2  g105(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand3  g106(.a(new_n158_), .b(x19), .c(new_n53_), .O(new_n159_));
  inv1   g107(.a(x35), .O(new_n160_));
  nand2  g108(.a(new_n160_), .b(new_n75_), .O(new_n161_));
  aoi21  g109(.a(new_n161_), .b(new_n159_), .c(new_n61_), .O(z15));
endmodule


