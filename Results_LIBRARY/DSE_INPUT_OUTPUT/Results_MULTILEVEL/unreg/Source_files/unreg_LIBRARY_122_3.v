// Benchmark "FAU" written by ABC on Fri Aug 14 02:05:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  oai21  g002(.a(x18), .b(x11), .c(x21), .O(new_n55_));
  nand2  g003(.a(new_n55_), .b(x03), .O(new_n56_));
  oai21  g004(.a(x21), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g005(.a(new_n57_), .b(x19), .c(new_n53_), .O(new_n58_));
  inv1   g006(.a(x19), .O(new_n59_));
  inv1   g007(.a(x20), .O(new_n60_));
  nand2  g008(.a(x21), .b(x11), .O(new_n61_));
  nand3  g009(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n58_), .O(z00));
  oai21  g011(.a(new_n59_), .b(x11), .c(x21), .O(new_n64_));
  nand3  g012(.a(new_n64_), .b(new_n54_), .c(x02), .O(new_n65_));
  inv1   g013(.a(x22), .O(new_n66_));
  nand4  g014(.a(new_n61_), .b(new_n66_), .c(x19), .d(x18), .O(new_n67_));
  nand2  g015(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  inv1   g017(.a(x21), .O(new_n70_));
  nand2  g018(.a(new_n70_), .b(new_n59_), .O(new_n71_));
  nand2  g019(.a(new_n71_), .b(new_n69_), .O(z01));
  nand2  g020(.a(new_n54_), .b(x01), .O(new_n73_));
  oai21  g021(.a(x23), .b(new_n54_), .c(new_n73_), .O(new_n74_));
  nand3  g022(.a(new_n74_), .b(x19), .c(new_n53_), .O(new_n75_));
  inv1   g023(.a(new_n61_), .O(new_n76_));
  aoi21  g024(.a(new_n66_), .b(new_n59_), .c(new_n76_), .O(new_n77_));
  nand2  g025(.a(new_n77_), .b(new_n75_), .O(z02));
  inv1   g026(.a(x00), .O(new_n79_));
  nand2  g027(.a(x18), .b(x16), .O(new_n80_));
  oai21  g028(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand3  g029(.a(new_n81_), .b(x19), .c(new_n53_), .O(new_n82_));
  nor2   g030(.a(x23), .b(x19), .O(new_n83_));
  nor2   g031(.a(new_n83_), .b(new_n76_), .O(new_n84_));
  nand2  g032(.a(new_n84_), .b(new_n82_), .O(z03));
  nand2  g033(.a(new_n54_), .b(x07), .O(new_n86_));
  oai21  g034(.a(x25), .b(new_n54_), .c(new_n86_), .O(new_n87_));
  nand3  g035(.a(new_n87_), .b(x19), .c(new_n53_), .O(new_n88_));
  nor2   g036(.a(x24), .b(x19), .O(new_n89_));
  nor2   g037(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  nand2  g038(.a(new_n90_), .b(new_n88_), .O(z04));
  nand2  g039(.a(new_n54_), .b(x06), .O(new_n92_));
  oai21  g040(.a(x26), .b(new_n54_), .c(new_n92_), .O(new_n93_));
  nand3  g041(.a(new_n93_), .b(x19), .c(new_n53_), .O(new_n94_));
  inv1   g042(.a(x25), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  aoi21  g044(.a(new_n96_), .b(new_n94_), .c(new_n76_), .O(z05));
  nand2  g045(.a(new_n54_), .b(x05), .O(new_n98_));
  oai21  g046(.a(x27), .b(new_n54_), .c(new_n98_), .O(new_n99_));
  nand3  g047(.a(new_n99_), .b(x19), .c(new_n53_), .O(new_n100_));
  inv1   g048(.a(x26), .O(new_n101_));
  nand2  g049(.a(new_n101_), .b(new_n59_), .O(new_n102_));
  aoi21  g050(.a(new_n102_), .b(new_n100_), .c(new_n76_), .O(z06));
  nand2  g051(.a(new_n54_), .b(x04), .O(new_n104_));
  oai21  g052(.a(x20), .b(new_n54_), .c(new_n104_), .O(new_n105_));
  nand3  g053(.a(new_n105_), .b(x19), .c(new_n53_), .O(new_n106_));
  nor2   g054(.a(x27), .b(x19), .O(new_n107_));
  nor2   g055(.a(new_n107_), .b(new_n76_), .O(new_n108_));
  nand2  g056(.a(new_n108_), .b(new_n106_), .O(z07));
  nand2  g057(.a(x29), .b(x18), .O(new_n110_));
  nand3  g058(.a(new_n110_), .b(new_n70_), .c(x11), .O(new_n111_));
  inv1   g059(.a(x29), .O(new_n112_));
  nand2  g060(.a(new_n112_), .b(x18), .O(new_n113_));
  oai21  g061(.a(new_n113_), .b(x11), .c(new_n111_), .O(new_n114_));
  nand3  g062(.a(new_n114_), .b(x19), .c(new_n53_), .O(new_n115_));
  inv1   g063(.a(x28), .O(new_n116_));
  nand3  g064(.a(new_n61_), .b(new_n116_), .c(new_n59_), .O(new_n117_));
  nand2  g065(.a(new_n117_), .b(new_n115_), .O(z08));
  nand2  g066(.a(new_n54_), .b(x10), .O(new_n119_));
  oai21  g067(.a(x30), .b(new_n54_), .c(new_n119_), .O(new_n120_));
  nand3  g068(.a(new_n120_), .b(x19), .c(new_n53_), .O(new_n121_));
  aoi21  g069(.a(new_n112_), .b(new_n59_), .c(new_n76_), .O(new_n122_));
  nand2  g070(.a(new_n122_), .b(new_n121_), .O(z09));
  nand2  g071(.a(new_n54_), .b(x09), .O(new_n124_));
  oai21  g072(.a(x31), .b(new_n54_), .c(new_n124_), .O(new_n125_));
  nand3  g073(.a(new_n125_), .b(x19), .c(new_n53_), .O(new_n126_));
  inv1   g074(.a(x30), .O(new_n127_));
  nand2  g075(.a(new_n127_), .b(new_n59_), .O(new_n128_));
  aoi21  g076(.a(new_n128_), .b(new_n126_), .c(new_n76_), .O(z10));
  nand2  g077(.a(new_n54_), .b(x08), .O(new_n130_));
  oai21  g078(.a(x24), .b(new_n54_), .c(new_n130_), .O(new_n131_));
  nand3  g079(.a(new_n131_), .b(x19), .c(new_n53_), .O(new_n132_));
  nor2   g080(.a(x31), .b(x19), .O(new_n133_));
  nor2   g081(.a(new_n133_), .b(new_n76_), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n132_), .O(z11));
  nand2  g083(.a(new_n54_), .b(x15), .O(new_n136_));
  oai21  g084(.a(x33), .b(new_n54_), .c(new_n136_), .O(new_n137_));
  nand3  g085(.a(new_n137_), .b(x19), .c(new_n53_), .O(new_n138_));
  inv1   g086(.a(x32), .O(new_n139_));
  nand2  g087(.a(new_n139_), .b(new_n59_), .O(new_n140_));
  aoi21  g088(.a(new_n140_), .b(new_n138_), .c(new_n76_), .O(z12));
  nand2  g089(.a(new_n54_), .b(x14), .O(new_n142_));
  oai21  g090(.a(x34), .b(new_n54_), .c(new_n142_), .O(new_n143_));
  nand3  g091(.a(new_n143_), .b(x19), .c(new_n53_), .O(new_n144_));
  nor2   g092(.a(x33), .b(x19), .O(new_n145_));
  nor2   g093(.a(new_n145_), .b(new_n76_), .O(new_n146_));
  nand2  g094(.a(new_n146_), .b(new_n144_), .O(z13));
  nand2  g095(.a(new_n54_), .b(x13), .O(new_n148_));
  oai21  g096(.a(x35), .b(new_n54_), .c(new_n148_), .O(new_n149_));
  nand3  g097(.a(new_n149_), .b(x19), .c(new_n53_), .O(new_n150_));
  inv1   g098(.a(x34), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n59_), .O(new_n152_));
  aoi21  g100(.a(new_n152_), .b(new_n150_), .c(new_n76_), .O(z14));
  nand2  g101(.a(new_n54_), .b(x12), .O(new_n154_));
  oai21  g102(.a(x28), .b(new_n54_), .c(new_n154_), .O(new_n155_));
  nand3  g103(.a(new_n155_), .b(x19), .c(new_n53_), .O(new_n156_));
  inv1   g104(.a(x35), .O(new_n157_));
  nand2  g105(.a(new_n157_), .b(new_n59_), .O(new_n158_));
  aoi21  g106(.a(new_n158_), .b(new_n156_), .c(new_n76_), .O(z15));
endmodule


