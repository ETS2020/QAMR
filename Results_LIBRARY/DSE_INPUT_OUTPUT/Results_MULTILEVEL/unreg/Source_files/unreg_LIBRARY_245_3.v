// Benchmark "FAU" written by ABC on Fri Aug 14 02:05:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x18), .O(new_n53_));
  nand2  g001(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g002(.a(x21), .O(new_n55_));
  nand2  g003(.a(new_n55_), .b(x18), .O(new_n56_));
  aoi21  g004(.a(new_n56_), .b(new_n54_), .c(x17), .O(new_n57_));
  oai21  g005(.a(new_n57_), .b(x29), .c(x19), .O(new_n58_));
  inv1   g006(.a(x19), .O(new_n59_));
  inv1   g007(.a(x20), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g009(.a(new_n61_), .b(new_n58_), .O(z00));
  nand2  g010(.a(new_n53_), .b(x02), .O(new_n63_));
  inv1   g011(.a(x22), .O(new_n64_));
  nand2  g012(.a(new_n64_), .b(x18), .O(new_n65_));
  aoi21  g013(.a(new_n65_), .b(new_n63_), .c(x17), .O(new_n66_));
  oai21  g014(.a(new_n66_), .b(x29), .c(x19), .O(new_n67_));
  nand2  g015(.a(new_n55_), .b(new_n59_), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n67_), .O(z01));
  inv1   g017(.a(x17), .O(new_n70_));
  inv1   g018(.a(x29), .O(new_n71_));
  nand2  g019(.a(new_n53_), .b(x01), .O(new_n72_));
  oai21  g020(.a(x23), .b(new_n53_), .c(new_n72_), .O(new_n73_));
  nand4  g021(.a(new_n73_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n74_));
  nand2  g022(.a(new_n64_), .b(new_n59_), .O(new_n75_));
  nand2  g023(.a(new_n75_), .b(new_n74_), .O(z02));
  inv1   g024(.a(x00), .O(new_n77_));
  nand2  g025(.a(x18), .b(x16), .O(new_n78_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand4  g027(.a(new_n79_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n80_));
  oai21  g028(.a(x23), .b(x19), .c(new_n80_), .O(z03));
  nand2  g029(.a(new_n53_), .b(x07), .O(new_n82_));
  oai21  g030(.a(x25), .b(new_n53_), .c(new_n82_), .O(new_n83_));
  nand4  g031(.a(new_n83_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n84_));
  inv1   g032(.a(x24), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(new_n59_), .O(new_n86_));
  nand2  g034(.a(new_n86_), .b(new_n84_), .O(z04));
  nand2  g035(.a(new_n53_), .b(x06), .O(new_n88_));
  oai21  g036(.a(x26), .b(new_n53_), .c(new_n88_), .O(new_n89_));
  nand4  g037(.a(new_n89_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n90_));
  inv1   g038(.a(x25), .O(new_n91_));
  nand2  g039(.a(new_n91_), .b(new_n59_), .O(new_n92_));
  nand2  g040(.a(new_n92_), .b(new_n90_), .O(z05));
  nand2  g041(.a(new_n53_), .b(x05), .O(new_n94_));
  inv1   g042(.a(x27), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(x18), .O(new_n96_));
  aoi21  g044(.a(new_n96_), .b(new_n94_), .c(x17), .O(new_n97_));
  oai21  g045(.a(new_n97_), .b(x29), .c(x19), .O(new_n98_));
  inv1   g046(.a(x26), .O(new_n99_));
  nand2  g047(.a(new_n99_), .b(new_n59_), .O(new_n100_));
  nand2  g048(.a(new_n100_), .b(new_n98_), .O(z06));
  nand2  g049(.a(new_n53_), .b(x04), .O(new_n102_));
  oai21  g050(.a(x20), .b(new_n53_), .c(new_n102_), .O(new_n103_));
  nand4  g051(.a(new_n103_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n104_));
  nand2  g052(.a(new_n95_), .b(new_n59_), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n104_), .O(z07));
  nor2   g054(.a(x18), .b(x11), .O(new_n107_));
  oai21  g055(.a(new_n107_), .b(x17), .c(new_n71_), .O(new_n108_));
  nand2  g056(.a(new_n108_), .b(x19), .O(new_n109_));
  oai21  g057(.a(x28), .b(x19), .c(new_n109_), .O(z08));
  nand2  g058(.a(new_n53_), .b(x10), .O(new_n111_));
  inv1   g059(.a(x30), .O(new_n112_));
  nand2  g060(.a(new_n112_), .b(x18), .O(new_n113_));
  aoi21  g061(.a(new_n113_), .b(new_n111_), .c(x17), .O(new_n114_));
  oai21  g062(.a(new_n114_), .b(x29), .c(x19), .O(new_n115_));
  nand2  g063(.a(new_n71_), .b(new_n59_), .O(new_n116_));
  nand2  g064(.a(new_n116_), .b(new_n115_), .O(z09));
  nand2  g065(.a(new_n53_), .b(x09), .O(new_n118_));
  inv1   g066(.a(x31), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(x18), .O(new_n120_));
  aoi21  g068(.a(new_n120_), .b(new_n118_), .c(x17), .O(new_n121_));
  oai21  g069(.a(new_n121_), .b(x29), .c(x19), .O(new_n122_));
  nand2  g070(.a(new_n112_), .b(new_n59_), .O(new_n123_));
  nand2  g071(.a(new_n123_), .b(new_n122_), .O(z10));
  nand2  g072(.a(new_n53_), .b(x08), .O(new_n125_));
  oai21  g073(.a(x24), .b(new_n53_), .c(new_n125_), .O(new_n126_));
  nand4  g074(.a(new_n126_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n127_));
  nand2  g075(.a(new_n119_), .b(new_n59_), .O(new_n128_));
  nand2  g076(.a(new_n128_), .b(new_n127_), .O(z11));
  nand2  g077(.a(new_n53_), .b(x15), .O(new_n130_));
  inv1   g078(.a(x33), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(x18), .O(new_n132_));
  aoi21  g080(.a(new_n132_), .b(new_n130_), .c(x17), .O(new_n133_));
  oai21  g081(.a(new_n133_), .b(x29), .c(x19), .O(new_n134_));
  inv1   g082(.a(x32), .O(new_n135_));
  nand2  g083(.a(new_n135_), .b(new_n59_), .O(new_n136_));
  nand2  g084(.a(new_n136_), .b(new_n134_), .O(z12));
  nand2  g085(.a(new_n53_), .b(x14), .O(new_n138_));
  inv1   g086(.a(x34), .O(new_n139_));
  nand2  g087(.a(new_n139_), .b(x18), .O(new_n140_));
  aoi21  g088(.a(new_n140_), .b(new_n138_), .c(x17), .O(new_n141_));
  oai21  g089(.a(new_n141_), .b(x29), .c(x19), .O(new_n142_));
  nand2  g090(.a(new_n131_), .b(new_n59_), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n142_), .O(z13));
  nand2  g092(.a(new_n53_), .b(x13), .O(new_n145_));
  oai21  g093(.a(x35), .b(new_n53_), .c(new_n145_), .O(new_n146_));
  nand4  g094(.a(new_n146_), .b(new_n71_), .c(x19), .d(new_n70_), .O(new_n147_));
  nand2  g095(.a(new_n139_), .b(new_n59_), .O(new_n148_));
  nand2  g096(.a(new_n148_), .b(new_n147_), .O(z14));
  nand2  g097(.a(new_n53_), .b(x12), .O(new_n150_));
  inv1   g098(.a(x28), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(x18), .O(new_n152_));
  aoi21  g100(.a(new_n152_), .b(new_n150_), .c(x17), .O(new_n153_));
  oai21  g101(.a(new_n153_), .b(x29), .c(x19), .O(new_n154_));
  inv1   g102(.a(x35), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n59_), .O(new_n156_));
  nand2  g104(.a(new_n156_), .b(new_n154_), .O(z15));
endmodule


