// Benchmark "FAU" written by ABC on Fri Aug 14 02:04:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(x03), .O(new_n55_));
  inv1   g003(.a(x21), .O(new_n56_));
  inv1   g004(.a(x27), .O(new_n57_));
  nand3  g005(.a(new_n57_), .b(new_n56_), .c(x18), .O(new_n58_));
  nand2  g006(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand3  g007(.a(new_n59_), .b(x19), .c(new_n53_), .O(new_n60_));
  inv1   g008(.a(x19), .O(new_n61_));
  inv1   g009(.a(x20), .O(new_n62_));
  nand2  g010(.a(x27), .b(x18), .O(new_n63_));
  nand3  g011(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g012(.a(new_n64_), .b(new_n60_), .O(z00));
  nand2  g013(.a(new_n54_), .b(x02), .O(new_n66_));
  inv1   g014(.a(x22), .O(new_n67_));
  nand3  g015(.a(new_n57_), .b(new_n67_), .c(x18), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand3  g017(.a(new_n69_), .b(x19), .c(new_n53_), .O(new_n70_));
  nand3  g018(.a(new_n63_), .b(new_n56_), .c(new_n61_), .O(new_n71_));
  nand2  g019(.a(new_n71_), .b(new_n70_), .O(z01));
  nand2  g020(.a(new_n54_), .b(x01), .O(new_n73_));
  inv1   g021(.a(x23), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(x18), .O(new_n75_));
  nand2  g023(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand3  g024(.a(new_n76_), .b(x19), .c(new_n53_), .O(new_n77_));
  inv1   g025(.a(new_n63_), .O(new_n78_));
  aoi21  g026(.a(new_n67_), .b(new_n61_), .c(new_n78_), .O(new_n79_));
  nand2  g027(.a(new_n79_), .b(new_n77_), .O(z02));
  inv1   g028(.a(x00), .O(new_n81_));
  nand2  g029(.a(x18), .b(x16), .O(new_n82_));
  oai21  g030(.a(x18), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  nand3  g031(.a(new_n83_), .b(x19), .c(new_n53_), .O(new_n84_));
  aoi21  g032(.a(new_n74_), .b(new_n61_), .c(new_n78_), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(new_n84_), .O(z03));
  nand2  g034(.a(new_n54_), .b(x07), .O(new_n87_));
  inv1   g035(.a(x25), .O(new_n88_));
  nand2  g036(.a(new_n88_), .b(x18), .O(new_n89_));
  nand2  g037(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g038(.a(new_n90_), .b(x19), .c(new_n53_), .O(new_n91_));
  inv1   g039(.a(x24), .O(new_n92_));
  aoi21  g040(.a(new_n92_), .b(new_n61_), .c(new_n78_), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(new_n91_), .O(z04));
  nand2  g042(.a(new_n54_), .b(x06), .O(new_n95_));
  inv1   g043(.a(x26), .O(new_n96_));
  nand3  g044(.a(new_n57_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g046(.a(new_n98_), .b(x19), .c(new_n53_), .O(new_n99_));
  nand3  g047(.a(new_n63_), .b(new_n88_), .c(new_n61_), .O(new_n100_));
  nand2  g048(.a(new_n100_), .b(new_n99_), .O(z05));
  inv1   g049(.a(x05), .O(new_n102_));
  nand2  g050(.a(new_n54_), .b(new_n102_), .O(new_n103_));
  nand3  g051(.a(new_n103_), .b(x19), .c(new_n53_), .O(new_n104_));
  nand2  g052(.a(new_n96_), .b(new_n61_), .O(new_n105_));
  nand3  g053(.a(new_n105_), .b(new_n104_), .c(new_n63_), .O(z06));
  nand2  g054(.a(new_n54_), .b(x04), .O(new_n107_));
  nand3  g055(.a(new_n57_), .b(new_n62_), .c(x18), .O(new_n108_));
  nand2  g056(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g057(.a(new_n109_), .b(x19), .c(new_n53_), .O(new_n110_));
  nand2  g058(.a(new_n57_), .b(new_n61_), .O(new_n111_));
  nand2  g059(.a(new_n111_), .b(new_n110_), .O(z07));
  nand2  g060(.a(new_n54_), .b(x11), .O(new_n113_));
  inv1   g061(.a(x29), .O(new_n114_));
  nand2  g062(.a(new_n114_), .b(x18), .O(new_n115_));
  nand2  g063(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand3  g064(.a(new_n116_), .b(x19), .c(new_n53_), .O(new_n117_));
  inv1   g065(.a(x28), .O(new_n118_));
  aoi21  g066(.a(new_n118_), .b(new_n61_), .c(new_n78_), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n117_), .O(z08));
  nand2  g068(.a(new_n54_), .b(x10), .O(new_n121_));
  inv1   g069(.a(x30), .O(new_n122_));
  nand3  g070(.a(new_n122_), .b(new_n57_), .c(x18), .O(new_n123_));
  nand2  g071(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand3  g072(.a(new_n124_), .b(x19), .c(new_n53_), .O(new_n125_));
  nand3  g073(.a(new_n63_), .b(new_n114_), .c(new_n61_), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n125_), .O(z09));
  nand2  g075(.a(new_n54_), .b(x09), .O(new_n128_));
  inv1   g076(.a(x31), .O(new_n129_));
  nand2  g077(.a(new_n129_), .b(x18), .O(new_n130_));
  nand2  g078(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand3  g079(.a(new_n131_), .b(x19), .c(new_n53_), .O(new_n132_));
  aoi21  g080(.a(new_n122_), .b(new_n61_), .c(new_n78_), .O(new_n133_));
  nand2  g081(.a(new_n133_), .b(new_n132_), .O(z10));
  nand2  g082(.a(new_n54_), .b(x08), .O(new_n135_));
  nand3  g083(.a(new_n57_), .b(new_n92_), .c(x18), .O(new_n136_));
  nand2  g084(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand3  g085(.a(new_n137_), .b(x19), .c(new_n53_), .O(new_n138_));
  nand3  g086(.a(new_n63_), .b(new_n129_), .c(new_n61_), .O(new_n139_));
  nand2  g087(.a(new_n139_), .b(new_n138_), .O(z11));
  nand2  g088(.a(new_n54_), .b(x15), .O(new_n141_));
  inv1   g089(.a(x33), .O(new_n142_));
  nand3  g090(.a(new_n142_), .b(new_n57_), .c(x18), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g092(.a(new_n144_), .b(x19), .c(new_n53_), .O(new_n145_));
  inv1   g093(.a(x32), .O(new_n146_));
  nand3  g094(.a(new_n63_), .b(new_n146_), .c(new_n61_), .O(new_n147_));
  nand2  g095(.a(new_n147_), .b(new_n145_), .O(z12));
  nand2  g096(.a(new_n54_), .b(x14), .O(new_n149_));
  inv1   g097(.a(x34), .O(new_n150_));
  nand2  g098(.a(new_n150_), .b(x18), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand3  g100(.a(new_n152_), .b(x19), .c(new_n53_), .O(new_n153_));
  aoi21  g101(.a(new_n142_), .b(new_n61_), .c(new_n78_), .O(new_n154_));
  nand2  g102(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g103(.a(new_n54_), .b(x13), .O(new_n156_));
  inv1   g104(.a(x35), .O(new_n157_));
  nand3  g105(.a(new_n157_), .b(new_n57_), .c(x18), .O(new_n158_));
  nand2  g106(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand3  g107(.a(new_n159_), .b(x19), .c(new_n53_), .O(new_n160_));
  nand3  g108(.a(new_n63_), .b(new_n150_), .c(new_n61_), .O(new_n161_));
  nand2  g109(.a(new_n161_), .b(new_n160_), .O(z14));
  nand2  g110(.a(new_n54_), .b(x12), .O(new_n163_));
  nand3  g111(.a(new_n118_), .b(new_n57_), .c(x18), .O(new_n164_));
  nand2  g112(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand3  g113(.a(new_n165_), .b(x19), .c(new_n53_), .O(new_n166_));
  nand3  g114(.a(new_n63_), .b(new_n157_), .c(new_n61_), .O(new_n167_));
  nand2  g115(.a(new_n167_), .b(new_n166_), .O(z15));
endmodule


