// Benchmark "FAU" written by ABC on Fri Aug 14 02:05:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(x03), .O(new_n55_));
  oai21  g003(.a(x21), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand3  g004(.a(new_n56_), .b(x19), .c(new_n53_), .O(new_n57_));
  inv1   g005(.a(x19), .O(new_n58_));
  inv1   g006(.a(x20), .O(new_n59_));
  inv1   g007(.a(x24), .O(new_n60_));
  nand2  g008(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g009(.a(new_n61_), .O(new_n62_));
  aoi21  g010(.a(new_n59_), .b(new_n58_), .c(new_n62_), .O(new_n63_));
  nand2  g011(.a(new_n63_), .b(new_n57_), .O(z00));
  nand2  g012(.a(new_n54_), .b(x02), .O(new_n65_));
  oai21  g013(.a(x22), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand3  g014(.a(new_n66_), .b(x19), .c(new_n53_), .O(new_n67_));
  inv1   g015(.a(x21), .O(new_n68_));
  nand2  g016(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  aoi21  g017(.a(new_n69_), .b(new_n67_), .c(new_n62_), .O(z01));
  nand2  g018(.a(new_n54_), .b(x01), .O(new_n71_));
  oai21  g019(.a(x23), .b(new_n54_), .c(new_n71_), .O(new_n72_));
  nand3  g020(.a(new_n72_), .b(x19), .c(new_n53_), .O(new_n73_));
  inv1   g021(.a(x22), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(new_n58_), .O(new_n75_));
  aoi21  g023(.a(new_n75_), .b(new_n73_), .c(new_n62_), .O(z02));
  inv1   g024(.a(x00), .O(new_n77_));
  nand2  g025(.a(x18), .b(x16), .O(new_n78_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand3  g027(.a(new_n79_), .b(x19), .c(new_n53_), .O(new_n80_));
  inv1   g028(.a(x23), .O(new_n81_));
  nand2  g029(.a(new_n81_), .b(new_n58_), .O(new_n82_));
  nand3  g030(.a(new_n82_), .b(new_n80_), .c(new_n61_), .O(z03));
  nand2  g031(.a(new_n54_), .b(x07), .O(new_n84_));
  oai21  g032(.a(x25), .b(new_n54_), .c(new_n84_), .O(new_n85_));
  nand3  g033(.a(new_n85_), .b(x19), .c(new_n53_), .O(new_n86_));
  oai21  g034(.a(x27), .b(new_n58_), .c(new_n60_), .O(new_n87_));
  nand2  g035(.a(new_n87_), .b(new_n86_), .O(z04));
  nand2  g036(.a(new_n54_), .b(x06), .O(new_n89_));
  oai21  g037(.a(x26), .b(new_n54_), .c(new_n89_), .O(new_n90_));
  nand3  g038(.a(new_n90_), .b(x19), .c(new_n53_), .O(new_n91_));
  nor2   g039(.a(x25), .b(x19), .O(new_n92_));
  nor2   g040(.a(new_n92_), .b(new_n62_), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(new_n91_), .O(z05));
  nand2  g042(.a(new_n54_), .b(x05), .O(new_n95_));
  oai21  g043(.a(x27), .b(new_n54_), .c(new_n95_), .O(new_n96_));
  nand3  g044(.a(new_n96_), .b(x19), .c(new_n53_), .O(new_n97_));
  nor2   g045(.a(x26), .b(x19), .O(new_n98_));
  nor2   g046(.a(new_n98_), .b(new_n62_), .O(new_n99_));
  nand2  g047(.a(new_n99_), .b(new_n97_), .O(z06));
  nand2  g048(.a(x24), .b(x19), .O(new_n101_));
  nand2  g049(.a(new_n101_), .b(x27), .O(new_n102_));
  nand3  g050(.a(new_n102_), .b(new_n54_), .c(x04), .O(new_n103_));
  nand4  g051(.a(new_n61_), .b(new_n59_), .c(x19), .d(x18), .O(new_n104_));
  nand2  g052(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n53_), .O(new_n106_));
  inv1   g054(.a(x27), .O(new_n107_));
  nand2  g055(.a(new_n107_), .b(new_n58_), .O(new_n108_));
  nand2  g056(.a(new_n108_), .b(new_n106_), .O(z07));
  nand2  g057(.a(new_n54_), .b(x11), .O(new_n110_));
  oai21  g058(.a(x29), .b(new_n54_), .c(new_n110_), .O(new_n111_));
  nand3  g059(.a(new_n111_), .b(x19), .c(new_n53_), .O(new_n112_));
  nor2   g060(.a(x28), .b(x19), .O(new_n113_));
  nor2   g061(.a(new_n113_), .b(new_n62_), .O(new_n114_));
  nand2  g062(.a(new_n114_), .b(new_n112_), .O(z08));
  nand2  g063(.a(new_n54_), .b(x10), .O(new_n116_));
  oai21  g064(.a(x30), .b(new_n54_), .c(new_n116_), .O(new_n117_));
  nand3  g065(.a(new_n117_), .b(x19), .c(new_n53_), .O(new_n118_));
  nor2   g066(.a(x29), .b(x19), .O(new_n119_));
  nor2   g067(.a(new_n119_), .b(new_n62_), .O(new_n120_));
  nand2  g068(.a(new_n120_), .b(new_n118_), .O(z09));
  nand4  g069(.a(x19), .b(new_n54_), .c(new_n53_), .d(x09), .O(new_n122_));
  oai21  g070(.a(x30), .b(x19), .c(new_n122_), .O(new_n123_));
  nand2  g071(.a(new_n123_), .b(new_n61_), .O(new_n124_));
  aoi21  g072(.a(x27), .b(new_n60_), .c(x31), .O(new_n125_));
  nand4  g073(.a(new_n125_), .b(x19), .c(x18), .d(new_n53_), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n124_), .O(z10));
  nand4  g075(.a(x19), .b(new_n54_), .c(new_n53_), .d(x08), .O(new_n128_));
  oai21  g076(.a(x31), .b(x19), .c(new_n128_), .O(new_n129_));
  nand2  g077(.a(new_n129_), .b(new_n61_), .O(new_n130_));
  nor2   g078(.a(x27), .b(x24), .O(new_n131_));
  nand4  g079(.a(new_n131_), .b(x19), .c(x18), .d(new_n53_), .O(new_n132_));
  nand2  g080(.a(new_n132_), .b(new_n130_), .O(z11));
  nand2  g081(.a(new_n54_), .b(x15), .O(new_n134_));
  oai21  g082(.a(x33), .b(new_n54_), .c(new_n134_), .O(new_n135_));
  nand3  g083(.a(new_n135_), .b(x19), .c(new_n53_), .O(new_n136_));
  nor2   g084(.a(x32), .b(x19), .O(new_n137_));
  nor2   g085(.a(new_n137_), .b(new_n62_), .O(new_n138_));
  nand2  g086(.a(new_n138_), .b(new_n136_), .O(z12));
  nand2  g087(.a(new_n54_), .b(x14), .O(new_n140_));
  oai21  g088(.a(x34), .b(new_n54_), .c(new_n140_), .O(new_n141_));
  nand3  g089(.a(new_n141_), .b(x19), .c(new_n53_), .O(new_n142_));
  inv1   g090(.a(x33), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n58_), .O(new_n144_));
  aoi21  g092(.a(new_n144_), .b(new_n142_), .c(new_n62_), .O(z13));
  nand2  g093(.a(new_n54_), .b(x13), .O(new_n146_));
  oai21  g094(.a(x35), .b(new_n54_), .c(new_n146_), .O(new_n147_));
  nand3  g095(.a(new_n147_), .b(x19), .c(new_n53_), .O(new_n148_));
  inv1   g096(.a(x34), .O(new_n149_));
  nand2  g097(.a(new_n149_), .b(new_n58_), .O(new_n150_));
  aoi21  g098(.a(new_n150_), .b(new_n148_), .c(new_n62_), .O(z14));
  nand2  g099(.a(new_n54_), .b(x12), .O(new_n152_));
  oai21  g100(.a(x28), .b(new_n54_), .c(new_n152_), .O(new_n153_));
  nand3  g101(.a(new_n153_), .b(x19), .c(new_n53_), .O(new_n154_));
  inv1   g102(.a(x35), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n58_), .O(new_n156_));
  aoi21  g104(.a(new_n156_), .b(new_n154_), .c(new_n62_), .O(z15));
endmodule


