// Benchmark "FAU" written by ABC on Fri Aug 14 02:05:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_;
  inv1   g000(.a(x03), .O(new_n53_));
  nor2   g001(.a(x18), .b(new_n53_), .O(new_n54_));
  inv1   g002(.a(x18), .O(new_n55_));
  nor2   g003(.a(x21), .b(new_n55_), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n54_), .c(x19), .O(new_n57_));
  inv1   g005(.a(x19), .O(new_n58_));
  inv1   g006(.a(x20), .O(new_n59_));
  nand2  g007(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g008(.a(new_n60_), .b(new_n57_), .c(x17), .O(z00));
  inv1   g009(.a(x02), .O(new_n62_));
  nor2   g010(.a(x18), .b(new_n62_), .O(new_n63_));
  nor2   g011(.a(x22), .b(new_n55_), .O(new_n64_));
  oai21  g012(.a(new_n64_), .b(new_n63_), .c(x19), .O(new_n65_));
  inv1   g013(.a(x21), .O(new_n66_));
  nand2  g014(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  aoi21  g015(.a(new_n67_), .b(new_n65_), .c(x17), .O(z01));
  inv1   g016(.a(x01), .O(new_n69_));
  nor2   g017(.a(x18), .b(new_n69_), .O(new_n70_));
  nor2   g018(.a(x23), .b(new_n55_), .O(new_n71_));
  oai21  g019(.a(new_n71_), .b(new_n70_), .c(x19), .O(new_n72_));
  inv1   g020(.a(x22), .O(new_n73_));
  nand2  g021(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  aoi21  g022(.a(new_n74_), .b(new_n72_), .c(x17), .O(z02));
  inv1   g023(.a(x17), .O(new_n76_));
  inv1   g024(.a(x00), .O(new_n77_));
  nand2  g025(.a(x18), .b(x16), .O(new_n78_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand3  g027(.a(new_n79_), .b(x19), .c(new_n76_), .O(new_n80_));
  inv1   g028(.a(x23), .O(new_n81_));
  oai21  g029(.a(new_n81_), .b(x17), .c(new_n58_), .O(new_n82_));
  nand2  g030(.a(new_n82_), .b(new_n80_), .O(z03));
  nand2  g031(.a(new_n55_), .b(x07), .O(new_n84_));
  inv1   g032(.a(x25), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(x18), .O(new_n86_));
  nand2  g034(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g035(.a(new_n87_), .b(x19), .c(new_n76_), .O(new_n88_));
  inv1   g036(.a(x24), .O(new_n89_));
  oai21  g037(.a(new_n89_), .b(x17), .c(new_n58_), .O(new_n90_));
  nand2  g038(.a(new_n90_), .b(new_n88_), .O(z04));
  inv1   g039(.a(x06), .O(new_n92_));
  nor2   g040(.a(x18), .b(new_n92_), .O(new_n93_));
  nor2   g041(.a(x26), .b(new_n55_), .O(new_n94_));
  oai21  g042(.a(new_n94_), .b(new_n93_), .c(x19), .O(new_n95_));
  nand2  g043(.a(new_n85_), .b(new_n58_), .O(new_n96_));
  aoi21  g044(.a(new_n96_), .b(new_n95_), .c(x17), .O(z05));
  nand2  g045(.a(new_n55_), .b(x05), .O(new_n98_));
  inv1   g046(.a(x27), .O(new_n99_));
  nand2  g047(.a(new_n99_), .b(x18), .O(new_n100_));
  nand2  g048(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand3  g049(.a(new_n101_), .b(x19), .c(new_n76_), .O(new_n102_));
  inv1   g050(.a(x26), .O(new_n103_));
  oai21  g051(.a(new_n103_), .b(x17), .c(new_n58_), .O(new_n104_));
  nand2  g052(.a(new_n104_), .b(new_n102_), .O(z06));
  inv1   g053(.a(x04), .O(new_n106_));
  nor2   g054(.a(x18), .b(new_n106_), .O(new_n107_));
  nor2   g055(.a(x20), .b(new_n55_), .O(new_n108_));
  oai21  g056(.a(new_n108_), .b(new_n107_), .c(x19), .O(new_n109_));
  nand2  g057(.a(new_n99_), .b(new_n58_), .O(new_n110_));
  aoi21  g058(.a(new_n110_), .b(new_n109_), .c(x17), .O(z07));
  nand2  g059(.a(new_n55_), .b(x11), .O(new_n112_));
  inv1   g060(.a(x29), .O(new_n113_));
  nand2  g061(.a(new_n113_), .b(x18), .O(new_n114_));
  nand2  g062(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand3  g063(.a(new_n115_), .b(x19), .c(new_n76_), .O(new_n116_));
  inv1   g064(.a(x28), .O(new_n117_));
  oai21  g065(.a(new_n117_), .b(x17), .c(new_n58_), .O(new_n118_));
  nand2  g066(.a(new_n118_), .b(new_n116_), .O(z08));
  inv1   g067(.a(x10), .O(new_n120_));
  nor2   g068(.a(x18), .b(new_n120_), .O(new_n121_));
  nor2   g069(.a(x30), .b(new_n55_), .O(new_n122_));
  oai21  g070(.a(new_n122_), .b(new_n121_), .c(x19), .O(new_n123_));
  nand2  g071(.a(new_n113_), .b(new_n58_), .O(new_n124_));
  aoi21  g072(.a(new_n124_), .b(new_n123_), .c(x17), .O(z09));
  inv1   g073(.a(x09), .O(new_n126_));
  nor2   g074(.a(x18), .b(new_n126_), .O(new_n127_));
  nor2   g075(.a(x31), .b(new_n55_), .O(new_n128_));
  oai21  g076(.a(new_n128_), .b(new_n127_), .c(x19), .O(new_n129_));
  inv1   g077(.a(x30), .O(new_n130_));
  nand2  g078(.a(new_n130_), .b(new_n58_), .O(new_n131_));
  aoi21  g079(.a(new_n131_), .b(new_n129_), .c(x17), .O(z10));
  nand2  g080(.a(new_n55_), .b(x08), .O(new_n133_));
  nand2  g081(.a(new_n89_), .b(x18), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g083(.a(new_n135_), .b(x19), .c(new_n76_), .O(new_n136_));
  inv1   g084(.a(x31), .O(new_n137_));
  oai21  g085(.a(new_n137_), .b(x17), .c(new_n58_), .O(new_n138_));
  nand2  g086(.a(new_n138_), .b(new_n136_), .O(z11));
  inv1   g087(.a(x15), .O(new_n140_));
  nor2   g088(.a(x18), .b(new_n140_), .O(new_n141_));
  nor2   g089(.a(x33), .b(new_n55_), .O(new_n142_));
  oai21  g090(.a(new_n142_), .b(new_n141_), .c(x19), .O(new_n143_));
  inv1   g091(.a(x32), .O(new_n144_));
  nand2  g092(.a(new_n144_), .b(new_n58_), .O(new_n145_));
  aoi21  g093(.a(new_n145_), .b(new_n143_), .c(x17), .O(z12));
  inv1   g094(.a(x14), .O(new_n147_));
  nor2   g095(.a(x18), .b(new_n147_), .O(new_n148_));
  nor2   g096(.a(x34), .b(new_n55_), .O(new_n149_));
  oai21  g097(.a(new_n149_), .b(new_n148_), .c(x19), .O(new_n150_));
  inv1   g098(.a(x33), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n58_), .O(new_n152_));
  aoi21  g100(.a(new_n152_), .b(new_n150_), .c(x17), .O(z13));
  nand2  g101(.a(new_n55_), .b(x13), .O(new_n154_));
  inv1   g102(.a(x35), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(x18), .O(new_n156_));
  nand2  g104(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  nand3  g105(.a(new_n157_), .b(x19), .c(new_n76_), .O(new_n158_));
  inv1   g106(.a(x34), .O(new_n159_));
  oai21  g107(.a(new_n159_), .b(x17), .c(new_n58_), .O(new_n160_));
  nand2  g108(.a(new_n160_), .b(new_n158_), .O(z14));
  inv1   g109(.a(x12), .O(new_n162_));
  nor2   g110(.a(x18), .b(new_n162_), .O(new_n163_));
  nor2   g111(.a(x28), .b(new_n55_), .O(new_n164_));
  oai21  g112(.a(new_n164_), .b(new_n163_), .c(x19), .O(new_n165_));
  nand2  g113(.a(new_n155_), .b(new_n58_), .O(new_n166_));
  aoi21  g114(.a(new_n166_), .b(new_n165_), .c(x17), .O(z15));
endmodule


