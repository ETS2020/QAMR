// Benchmark "FAU" written by ABC on Fri Aug 14 02:05:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x27), .O(new_n53_));
  nand2  g001(.a(x35), .b(new_n53_), .O(new_n54_));
  inv1   g002(.a(new_n54_), .O(new_n55_));
  inv1   g003(.a(x17), .O(new_n56_));
  inv1   g004(.a(x18), .O(new_n57_));
  nand2  g005(.a(new_n57_), .b(x03), .O(new_n58_));
  oai21  g006(.a(x21), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  nand3  g007(.a(new_n59_), .b(x19), .c(new_n56_), .O(new_n60_));
  inv1   g008(.a(x19), .O(new_n61_));
  inv1   g009(.a(x20), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g011(.a(new_n63_), .b(new_n60_), .c(new_n55_), .O(z00));
  nand2  g012(.a(new_n57_), .b(x02), .O(new_n65_));
  oai21  g013(.a(x22), .b(new_n57_), .c(new_n65_), .O(new_n66_));
  nand3  g014(.a(new_n66_), .b(x19), .c(new_n56_), .O(new_n67_));
  nor2   g015(.a(x21), .b(x19), .O(new_n68_));
  nor2   g016(.a(new_n68_), .b(new_n55_), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n67_), .O(z01));
  nand2  g018(.a(new_n57_), .b(x01), .O(new_n71_));
  oai21  g019(.a(x23), .b(new_n57_), .c(new_n71_), .O(new_n72_));
  nand3  g020(.a(new_n72_), .b(x19), .c(new_n56_), .O(new_n73_));
  nor2   g021(.a(x22), .b(x19), .O(new_n74_));
  nor2   g022(.a(new_n74_), .b(new_n55_), .O(new_n75_));
  nand2  g023(.a(new_n75_), .b(new_n73_), .O(z02));
  inv1   g024(.a(x00), .O(new_n77_));
  nand2  g025(.a(x18), .b(x16), .O(new_n78_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand3  g027(.a(new_n79_), .b(x19), .c(new_n56_), .O(new_n80_));
  inv1   g028(.a(x23), .O(new_n81_));
  nand2  g029(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  aoi21  g030(.a(new_n82_), .b(new_n80_), .c(new_n55_), .O(z03));
  nand2  g031(.a(new_n57_), .b(x07), .O(new_n84_));
  oai21  g032(.a(x25), .b(new_n57_), .c(new_n84_), .O(new_n85_));
  nand3  g033(.a(new_n85_), .b(x19), .c(new_n56_), .O(new_n86_));
  nor2   g034(.a(x24), .b(x19), .O(new_n87_));
  nor2   g035(.a(new_n87_), .b(new_n55_), .O(new_n88_));
  nand2  g036(.a(new_n88_), .b(new_n86_), .O(z04));
  nand2  g037(.a(new_n57_), .b(x06), .O(new_n90_));
  oai21  g038(.a(x26), .b(new_n57_), .c(new_n90_), .O(new_n91_));
  nand3  g039(.a(new_n91_), .b(x19), .c(new_n56_), .O(new_n92_));
  inv1   g040(.a(x25), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  aoi21  g042(.a(new_n94_), .b(new_n92_), .c(new_n55_), .O(z05));
  nand4  g043(.a(x19), .b(new_n57_), .c(new_n56_), .d(x05), .O(new_n96_));
  oai21  g044(.a(x26), .b(x19), .c(new_n96_), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(new_n54_), .O(new_n98_));
  nor2   g046(.a(x35), .b(x27), .O(new_n99_));
  nand4  g047(.a(new_n99_), .b(x19), .c(x18), .d(new_n56_), .O(new_n100_));
  nand2  g048(.a(new_n100_), .b(new_n98_), .O(z06));
  nand2  g049(.a(new_n57_), .b(x04), .O(new_n102_));
  oai21  g050(.a(x20), .b(new_n57_), .c(new_n102_), .O(new_n103_));
  nand3  g051(.a(new_n103_), .b(x19), .c(new_n56_), .O(new_n104_));
  oai21  g052(.a(x35), .b(new_n61_), .c(new_n53_), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n104_), .O(z07));
  nand2  g054(.a(new_n57_), .b(x11), .O(new_n107_));
  oai21  g055(.a(x29), .b(new_n57_), .c(new_n107_), .O(new_n108_));
  nand3  g056(.a(new_n108_), .b(x19), .c(new_n56_), .O(new_n109_));
  inv1   g057(.a(x28), .O(new_n110_));
  aoi21  g058(.a(new_n110_), .b(new_n61_), .c(new_n55_), .O(new_n111_));
  nand2  g059(.a(new_n111_), .b(new_n109_), .O(z08));
  nand2  g060(.a(new_n57_), .b(x10), .O(new_n113_));
  oai21  g061(.a(x30), .b(new_n57_), .c(new_n113_), .O(new_n114_));
  nand3  g062(.a(new_n114_), .b(x19), .c(new_n56_), .O(new_n115_));
  nor2   g063(.a(x29), .b(x19), .O(new_n116_));
  nor2   g064(.a(new_n116_), .b(new_n55_), .O(new_n117_));
  nand2  g065(.a(new_n117_), .b(new_n115_), .O(z09));
  nand2  g066(.a(new_n57_), .b(x09), .O(new_n119_));
  oai21  g067(.a(x31), .b(new_n57_), .c(new_n119_), .O(new_n120_));
  nand3  g068(.a(new_n120_), .b(x19), .c(new_n56_), .O(new_n121_));
  nor2   g069(.a(x30), .b(x19), .O(new_n122_));
  nor2   g070(.a(new_n122_), .b(new_n55_), .O(new_n123_));
  nand2  g071(.a(new_n123_), .b(new_n121_), .O(z10));
  nand2  g072(.a(new_n57_), .b(x08), .O(new_n125_));
  oai21  g073(.a(x24), .b(new_n57_), .c(new_n125_), .O(new_n126_));
  nand3  g074(.a(new_n126_), .b(x19), .c(new_n56_), .O(new_n127_));
  inv1   g075(.a(x31), .O(new_n128_));
  nand2  g076(.a(new_n128_), .b(new_n61_), .O(new_n129_));
  aoi21  g077(.a(new_n129_), .b(new_n127_), .c(new_n55_), .O(z11));
  nand2  g078(.a(new_n57_), .b(x15), .O(new_n131_));
  oai21  g079(.a(x33), .b(new_n57_), .c(new_n131_), .O(new_n132_));
  nand3  g080(.a(new_n132_), .b(x19), .c(new_n56_), .O(new_n133_));
  inv1   g081(.a(x32), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n61_), .O(new_n135_));
  aoi21  g083(.a(new_n135_), .b(new_n133_), .c(new_n55_), .O(z12));
  nand2  g084(.a(new_n57_), .b(x14), .O(new_n137_));
  oai21  g085(.a(x34), .b(new_n57_), .c(new_n137_), .O(new_n138_));
  nand3  g086(.a(new_n138_), .b(x19), .c(new_n56_), .O(new_n139_));
  inv1   g087(.a(x33), .O(new_n140_));
  nand2  g088(.a(new_n140_), .b(new_n61_), .O(new_n141_));
  aoi21  g089(.a(new_n141_), .b(new_n139_), .c(new_n55_), .O(z13));
  nand2  g090(.a(new_n57_), .b(x13), .O(new_n143_));
  oai21  g091(.a(x35), .b(new_n57_), .c(new_n143_), .O(new_n144_));
  nand3  g092(.a(new_n144_), .b(x19), .c(new_n56_), .O(new_n145_));
  nor2   g093(.a(x34), .b(x19), .O(new_n146_));
  nor2   g094(.a(new_n146_), .b(new_n55_), .O(new_n147_));
  nand2  g095(.a(new_n147_), .b(new_n145_), .O(z14));
  nand2  g096(.a(x27), .b(x19), .O(new_n149_));
  nand2  g097(.a(new_n149_), .b(x35), .O(new_n150_));
  nand3  g098(.a(new_n150_), .b(new_n57_), .c(x12), .O(new_n151_));
  nand4  g099(.a(new_n54_), .b(new_n110_), .c(x19), .d(x18), .O(new_n152_));
  nand2  g100(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand2  g101(.a(new_n153_), .b(new_n56_), .O(new_n154_));
  inv1   g102(.a(x35), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n61_), .O(new_n156_));
  nand2  g104(.a(new_n156_), .b(new_n154_), .O(z15));
endmodule


