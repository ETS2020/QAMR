// Benchmark "FAU" written by ABC on Fri Aug 14 02:04:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_,
    new_n164_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(x03), .O(new_n55_));
  oai21  g003(.a(x21), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand3  g004(.a(new_n56_), .b(x19), .c(new_n53_), .O(new_n57_));
  inv1   g005(.a(x19), .O(new_n58_));
  inv1   g006(.a(x20), .O(new_n59_));
  inv1   g007(.a(x16), .O(new_n60_));
  nor2   g008(.a(x35), .b(new_n60_), .O(new_n61_));
  aoi21  g009(.a(new_n59_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n57_), .O(z00));
  nand2  g011(.a(new_n54_), .b(x02), .O(new_n64_));
  inv1   g012(.a(x22), .O(new_n65_));
  nand2  g013(.a(new_n65_), .b(x18), .O(new_n66_));
  nand2  g014(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g015(.a(new_n67_), .b(x19), .c(new_n53_), .O(new_n68_));
  inv1   g016(.a(x21), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  aoi21  g018(.a(new_n70_), .b(new_n68_), .c(new_n61_), .O(z01));
  inv1   g019(.a(new_n61_), .O(new_n72_));
  nand4  g020(.a(x19), .b(new_n54_), .c(new_n53_), .d(x01), .O(new_n73_));
  nand2  g021(.a(new_n65_), .b(new_n58_), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g023(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  inv1   g024(.a(x35), .O(new_n77_));
  aoi21  g025(.a(new_n77_), .b(x16), .c(x23), .O(new_n78_));
  nand4  g026(.a(new_n78_), .b(x19), .c(x18), .d(new_n53_), .O(new_n79_));
  nand2  g027(.a(new_n79_), .b(new_n76_), .O(z02));
  nand4  g028(.a(x19), .b(new_n54_), .c(new_n53_), .d(x00), .O(new_n81_));
  inv1   g029(.a(x23), .O(new_n82_));
  nand2  g030(.a(new_n82_), .b(new_n58_), .O(new_n83_));
  nand2  g031(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g032(.a(new_n84_), .b(new_n72_), .O(new_n85_));
  nor2   g033(.a(x17), .b(new_n60_), .O(new_n86_));
  nand4  g034(.a(new_n86_), .b(x35), .c(x19), .d(x18), .O(new_n87_));
  nand2  g035(.a(new_n87_), .b(new_n85_), .O(z03));
  nand2  g036(.a(new_n54_), .b(x07), .O(new_n89_));
  oai21  g037(.a(x25), .b(new_n54_), .c(new_n89_), .O(new_n90_));
  nand3  g038(.a(new_n90_), .b(x19), .c(new_n53_), .O(new_n91_));
  inv1   g039(.a(x24), .O(new_n92_));
  aoi21  g040(.a(new_n92_), .b(new_n58_), .c(new_n61_), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(new_n91_), .O(z04));
  nand2  g042(.a(new_n54_), .b(x06), .O(new_n95_));
  oai21  g043(.a(x26), .b(new_n54_), .c(new_n95_), .O(new_n96_));
  nand3  g044(.a(new_n96_), .b(x19), .c(new_n53_), .O(new_n97_));
  nor2   g045(.a(x25), .b(x19), .O(new_n98_));
  nor2   g046(.a(new_n98_), .b(new_n61_), .O(new_n99_));
  nand2  g047(.a(new_n99_), .b(new_n97_), .O(z05));
  nand2  g048(.a(new_n54_), .b(x05), .O(new_n101_));
  inv1   g049(.a(x27), .O(new_n102_));
  nand2  g050(.a(new_n102_), .b(x18), .O(new_n103_));
  nand2  g051(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand3  g052(.a(new_n104_), .b(x19), .c(new_n53_), .O(new_n105_));
  inv1   g053(.a(x26), .O(new_n106_));
  nand2  g054(.a(new_n106_), .b(new_n58_), .O(new_n107_));
  aoi21  g055(.a(new_n107_), .b(new_n105_), .c(new_n61_), .O(z06));
  nand2  g056(.a(new_n54_), .b(x04), .O(new_n109_));
  nand2  g057(.a(new_n59_), .b(x18), .O(new_n110_));
  nand2  g058(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g059(.a(new_n111_), .b(x19), .c(new_n53_), .O(new_n112_));
  nand2  g060(.a(new_n102_), .b(new_n58_), .O(new_n113_));
  aoi21  g061(.a(new_n113_), .b(new_n112_), .c(new_n61_), .O(z07));
  nand2  g062(.a(new_n54_), .b(x11), .O(new_n115_));
  oai21  g063(.a(x29), .b(new_n54_), .c(new_n115_), .O(new_n116_));
  nand3  g064(.a(new_n116_), .b(x19), .c(new_n53_), .O(new_n117_));
  nor2   g065(.a(x28), .b(x19), .O(new_n118_));
  nor2   g066(.a(new_n118_), .b(new_n61_), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n117_), .O(z08));
  nand2  g068(.a(new_n54_), .b(x10), .O(new_n121_));
  oai21  g069(.a(x30), .b(new_n54_), .c(new_n121_), .O(new_n122_));
  nand3  g070(.a(new_n122_), .b(x19), .c(new_n53_), .O(new_n123_));
  nor2   g071(.a(x29), .b(x19), .O(new_n124_));
  nor2   g072(.a(new_n124_), .b(new_n61_), .O(new_n125_));
  nand2  g073(.a(new_n125_), .b(new_n123_), .O(z09));
  nand2  g074(.a(new_n54_), .b(x09), .O(new_n127_));
  oai21  g075(.a(x31), .b(new_n54_), .c(new_n127_), .O(new_n128_));
  nand3  g076(.a(new_n128_), .b(x19), .c(new_n53_), .O(new_n129_));
  nor2   g077(.a(x30), .b(x19), .O(new_n130_));
  nor2   g078(.a(new_n130_), .b(new_n61_), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(new_n129_), .O(z10));
  nand2  g080(.a(new_n54_), .b(x08), .O(new_n133_));
  nand2  g081(.a(new_n92_), .b(x18), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g083(.a(new_n135_), .b(x19), .c(new_n53_), .O(new_n136_));
  inv1   g084(.a(x31), .O(new_n137_));
  nand2  g085(.a(new_n137_), .b(new_n58_), .O(new_n138_));
  aoi21  g086(.a(new_n138_), .b(new_n136_), .c(new_n61_), .O(z11));
  nand2  g087(.a(new_n54_), .b(x15), .O(new_n140_));
  oai21  g088(.a(x33), .b(new_n54_), .c(new_n140_), .O(new_n141_));
  nand3  g089(.a(new_n141_), .b(x19), .c(new_n53_), .O(new_n142_));
  nor2   g090(.a(x32), .b(x19), .O(new_n143_));
  nor2   g091(.a(new_n143_), .b(new_n61_), .O(new_n144_));
  nand2  g092(.a(new_n144_), .b(new_n142_), .O(z12));
  nand2  g093(.a(new_n54_), .b(x14), .O(new_n146_));
  inv1   g094(.a(x34), .O(new_n147_));
  nand2  g095(.a(new_n147_), .b(x18), .O(new_n148_));
  nand2  g096(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand3  g097(.a(new_n149_), .b(x19), .c(new_n53_), .O(new_n150_));
  inv1   g098(.a(x33), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n58_), .O(new_n152_));
  aoi21  g100(.a(new_n152_), .b(new_n150_), .c(new_n61_), .O(z13));
  nand4  g101(.a(x19), .b(new_n54_), .c(new_n53_), .d(x13), .O(new_n154_));
  nand2  g102(.a(new_n147_), .b(new_n58_), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g104(.a(new_n156_), .b(new_n72_), .O(new_n157_));
  nor2   g105(.a(x17), .b(x16), .O(new_n158_));
  nand4  g106(.a(new_n158_), .b(new_n77_), .c(x19), .d(x18), .O(new_n159_));
  nand2  g107(.a(new_n159_), .b(new_n157_), .O(z14));
  nand2  g108(.a(new_n54_), .b(x12), .O(new_n161_));
  oai21  g109(.a(x28), .b(new_n54_), .c(new_n161_), .O(new_n162_));
  nand3  g110(.a(new_n162_), .b(x19), .c(new_n53_), .O(new_n163_));
  oai21  g111(.a(new_n58_), .b(x16), .c(new_n77_), .O(new_n164_));
  nand2  g112(.a(new_n164_), .b(new_n163_), .O(z15));
endmodule


