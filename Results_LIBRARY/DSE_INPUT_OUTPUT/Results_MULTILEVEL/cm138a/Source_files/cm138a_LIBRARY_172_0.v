// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_;
  inv1   g00(.a(x4), .O(new_n15_));
  inv1   g01(.a(x5), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(new_n17_));
  oai21  g03(.a(new_n17_), .b(x2), .c(x3), .O(new_n18_));
  inv1   g04(.a(x0), .O(new_n19_));
  inv1   g05(.a(x1), .O(new_n20_));
  nand3  g06(.a(x3), .b(new_n20_), .c(new_n19_), .O(new_n21_));
  inv1   g07(.a(new_n21_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n18_), .O(z0));
  inv1   g09(.a(x2), .O(new_n24_));
  nor2   g10(.a(x5), .b(x4), .O(new_n25_));
  nand3  g11(.a(new_n25_), .b(new_n24_), .c(x0), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(x3), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(new_n20_), .O(z1));
  nand3  g14(.a(new_n16_), .b(new_n15_), .c(x3), .O(new_n29_));
  inv1   g15(.a(new_n29_), .O(new_n30_));
  nand4  g16(.a(new_n30_), .b(new_n24_), .c(x1), .d(new_n19_), .O(z2));
  nand4  g17(.a(new_n30_), .b(new_n24_), .c(x1), .d(x0), .O(z3));
  oai21  g18(.a(new_n17_), .b(new_n24_), .c(x3), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n22_), .O(z4));
  nand3  g20(.a(new_n25_), .b(x2), .c(x0), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(x3), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(new_n20_), .O(z5));
  nand2  g23(.a(x3), .b(new_n20_), .O(new_n38_));
  nand2  g24(.a(x2), .b(new_n19_), .O(new_n39_));
  nand3  g25(.a(new_n16_), .b(new_n15_), .c(x3), .O(new_n40_));
  oai21  g26(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand2  g27(.a(new_n41_), .b(new_n38_), .O(z6));
  and2   g28(.a(x3), .b(x2), .O(new_n43_));
  nand4  g29(.a(new_n43_), .b(new_n25_), .c(x1), .d(x0), .O(z7));
endmodule


