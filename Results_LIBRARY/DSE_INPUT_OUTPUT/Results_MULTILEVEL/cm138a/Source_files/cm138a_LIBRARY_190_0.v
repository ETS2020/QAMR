// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_;
  inv1   g00(.a(x2), .O(new_n17_));
  inv1   g01(.a(x5), .O(new_n18_));
  inv1   g02(.a(x3), .O(new_n19_));
  nor2   g03(.a(x4), .b(new_n19_), .O(new_n20_));
  inv1   g04(.a(x1), .O(new_n21_));
  nor2   g05(.a(new_n21_), .b(x0), .O(new_n22_));
  nand4  g06(.a(new_n22_), .b(new_n20_), .c(new_n18_), .d(new_n17_), .O(z2));
  inv1   g07(.a(x4), .O(new_n24_));
  nand3  g08(.a(new_n18_), .b(new_n24_), .c(x3), .O(new_n25_));
  inv1   g09(.a(new_n25_), .O(new_n26_));
  nand4  g10(.a(new_n26_), .b(new_n17_), .c(x1), .d(x0), .O(z3));
  inv1   g11(.a(x0), .O(new_n28_));
  nor2   g12(.a(x5), .b(x4), .O(new_n29_));
  nand3  g13(.a(new_n29_), .b(x3), .c(new_n28_), .O(new_n30_));
  nand2  g14(.a(new_n30_), .b(x2), .O(new_n31_));
  nand2  g15(.a(new_n31_), .b(new_n21_), .O(z4));
  nand3  g16(.a(new_n29_), .b(x3), .c(x0), .O(new_n33_));
  nand2  g17(.a(new_n33_), .b(x2), .O(new_n34_));
  nand2  g18(.a(new_n34_), .b(new_n21_), .O(z5));
  nand2  g19(.a(x2), .b(new_n28_), .O(new_n36_));
  oai21  g20(.a(new_n36_), .b(new_n25_), .c(x1), .O(new_n37_));
  nand2  g21(.a(x2), .b(new_n21_), .O(new_n38_));
  nand2  g22(.a(new_n38_), .b(new_n37_), .O(z6));
  nand2  g23(.a(x2), .b(x0), .O(new_n40_));
  oai21  g24(.a(new_n40_), .b(new_n25_), .c(x1), .O(new_n41_));
  nand2  g25(.a(new_n41_), .b(new_n38_), .O(z7));
  one    g26(.O(z0));
  one    g27(.O(z1));
endmodule


