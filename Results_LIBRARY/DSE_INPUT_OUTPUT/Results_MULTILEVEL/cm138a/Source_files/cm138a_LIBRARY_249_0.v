// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n38_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  inv1   g03(.a(x5), .O(new_n19_));
  nand4  g04(.a(new_n19_), .b(new_n18_), .c(x3), .d(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(z1));
  inv1   g07(.a(x0), .O(new_n23_));
  nand2  g08(.a(new_n20_), .b(x1), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(z2));
  nand2  g10(.a(new_n16_), .b(new_n23_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  xnor2a g12(.a(x1), .b(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z3));
  nor2   g14(.a(x1), .b(new_n23_), .O(new_n31_));
  inv1   g15(.a(x3), .O(new_n32_));
  nor2   g16(.a(x4), .b(new_n32_), .O(new_n33_));
  nand4  g17(.a(new_n33_), .b(new_n31_), .c(new_n19_), .d(x2), .O(z5));
  nand4  g18(.a(new_n19_), .b(new_n18_), .c(x3), .d(x2), .O(new_n35_));
  inv1   g19(.a(new_n35_), .O(new_n36_));
  nand3  g20(.a(new_n36_), .b(x1), .c(new_n23_), .O(z6));
  nand2  g21(.a(new_n35_), .b(new_n26_), .O(new_n38_));
  nand2  g22(.a(new_n38_), .b(new_n28_), .O(z7));
  one    g23(.O(z0));
  one    g24(.O(z4));
endmodule


