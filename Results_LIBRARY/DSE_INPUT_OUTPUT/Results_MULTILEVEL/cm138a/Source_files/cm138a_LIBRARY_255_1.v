// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n40_, new_n41_,
    new_n43_, new_n44_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  inv1   g02(.a(x1), .O(new_n17_));
  nor2   g03(.a(x5), .b(x4), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(x3), .c(new_n17_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n15_), .O(z0));
  nand2  g07(.a(x2), .b(new_n15_), .O(z6));
  nand2  g08(.a(new_n18_), .b(x3), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(z6), .O(new_n24_));
  oai21  g10(.a(x2), .b(x1), .c(x0), .O(new_n25_));
  nand2  g11(.a(new_n16_), .b(new_n15_), .O(new_n26_));
  nand3  g12(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z1));
  inv1   g13(.a(x4), .O(new_n28_));
  inv1   g14(.a(x5), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  oai21  g16(.a(x1), .b(x0), .c(x3), .O(new_n31_));
  oai21  g17(.a(new_n31_), .b(new_n30_), .c(new_n16_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n15_), .O(z2));
  nand2  g19(.a(z6), .b(new_n17_), .O(new_n34_));
  nand2  g20(.a(x2), .b(x1), .O(new_n35_));
  nand4  g21(.a(new_n35_), .b(new_n29_), .c(new_n28_), .d(x3), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(x0), .O(new_n37_));
  nand3  g23(.a(new_n37_), .b(new_n34_), .c(new_n26_), .O(z3));
  oai21  g24(.a(new_n17_), .b(new_n15_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n19_), .b(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z5));
  nand3  g27(.a(new_n18_), .b(x3), .c(x1), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(x2), .O(z7));
  one    g30(.O(z4));
endmodule


