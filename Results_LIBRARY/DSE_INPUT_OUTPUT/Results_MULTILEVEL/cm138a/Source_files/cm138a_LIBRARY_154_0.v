// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n43_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  nor3   g02(.a(x5), .b(x4), .c(x2), .O(new_n17_));
  nand4  g03(.a(new_n17_), .b(x3), .c(new_n16_), .d(new_n15_), .O(z0));
  inv1   g04(.a(x2), .O(new_n19_));
  nor2   g05(.a(x5), .b(x4), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(x3), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n16_), .O(z1));
  nand2  g09(.a(x3), .b(new_n16_), .O(new_n24_));
  nand2  g10(.a(new_n19_), .b(new_n15_), .O(new_n25_));
  inv1   g11(.a(x4), .O(new_n26_));
  inv1   g12(.a(x5), .O(new_n27_));
  nand3  g13(.a(new_n27_), .b(new_n26_), .c(x3), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(new_n25_), .c(x1), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n24_), .O(z2));
  inv1   g16(.a(x3), .O(new_n31_));
  nor2   g17(.a(new_n31_), .b(x2), .O(new_n32_));
  nand4  g18(.a(new_n32_), .b(new_n20_), .c(x1), .d(x0), .O(z3));
  nand3  g19(.a(new_n20_), .b(x2), .c(new_n15_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x3), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n16_), .O(z4));
  nand3  g22(.a(new_n20_), .b(x2), .c(x0), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(x3), .O(new_n38_));
  nand2  g24(.a(new_n38_), .b(new_n16_), .O(z5));
  nand2  g25(.a(x2), .b(new_n15_), .O(new_n40_));
  oai21  g26(.a(new_n40_), .b(new_n28_), .c(x1), .O(new_n41_));
  nand2  g27(.a(new_n41_), .b(new_n24_), .O(z6));
  nor2   g28(.a(new_n31_), .b(new_n19_), .O(new_n43_));
  nand4  g29(.a(new_n43_), .b(new_n20_), .c(x1), .d(x0), .O(z7));
endmodule


