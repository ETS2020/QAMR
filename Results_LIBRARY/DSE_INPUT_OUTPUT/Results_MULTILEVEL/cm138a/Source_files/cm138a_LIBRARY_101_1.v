// Benchmark "FAU" written by ABC on Thu Aug 13 15:14:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_;
  inv1   g00(.a(x2), .O(new_n15_));
  inv1   g01(.a(x4), .O(new_n16_));
  inv1   g02(.a(x5), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(x3), .O(new_n18_));
  inv1   g04(.a(new_n18_), .O(new_n19_));
  nor2   g05(.a(x1), .b(x0), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n19_), .c(new_n15_), .O(z0));
  inv1   g07(.a(x0), .O(new_n22_));
  nor2   g08(.a(x1), .b(new_n22_), .O(new_n23_));
  nand3  g09(.a(new_n23_), .b(new_n19_), .c(new_n15_), .O(z1));
  nand3  g10(.a(new_n17_), .b(x3), .c(x1), .O(new_n25_));
  inv1   g11(.a(new_n25_), .O(new_n26_));
  nand4  g12(.a(new_n26_), .b(new_n16_), .c(new_n15_), .d(new_n22_), .O(z2));
  nand2  g13(.a(new_n17_), .b(x3), .O(new_n28_));
  nand2  g14(.a(x1), .b(x0), .O(new_n29_));
  oai21  g15(.a(new_n29_), .b(new_n28_), .c(new_n16_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n15_), .O(z3));
  nand2  g17(.a(new_n16_), .b(new_n15_), .O(new_n32_));
  nor2   g18(.a(x5), .b(x4), .O(new_n33_));
  nand3  g19(.a(new_n33_), .b(new_n20_), .c(x3), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x2), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n32_), .O(z4));
  inv1   g22(.a(x3), .O(new_n37_));
  nor2   g23(.a(new_n37_), .b(x1), .O(new_n38_));
  nand4  g24(.a(new_n38_), .b(new_n33_), .c(x2), .d(x0), .O(z5));
  nand4  g25(.a(new_n19_), .b(x2), .c(x1), .d(new_n22_), .O(z6));
  nand4  g26(.a(new_n19_), .b(x2), .c(x1), .d(x0), .O(z7));
endmodule


