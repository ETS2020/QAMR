// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x2), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand2  g03(.a(x3), .b(x1), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x4), .d(new_n19_), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  nand3  g06(.a(x6), .b(new_n23_), .c(x3), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n22_), .c(new_n18_), .O(new_n25_));
  oai21  g08(.a(x1), .b(x0), .c(x6), .O(new_n26_));
  nor2   g09(.a(new_n26_), .b(x4), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(x5), .O(new_n28_));
  inv1   g11(.a(x5), .O(new_n29_));
  nor2   g12(.a(x6), .b(x2), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n29_), .c(x4), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n28_), .O(z0));
  nor2   g16(.a(x6), .b(x4), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(new_n35_));
  nor2   g18(.a(new_n20_), .b(new_n18_), .O(new_n36_));
  aoi21  g19(.a(new_n35_), .b(x1), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(x6), .b(x4), .O(new_n38_));
  oai21  g21(.a(new_n37_), .b(new_n19_), .c(new_n38_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n29_), .O(new_n40_));
  nand4  g23(.a(new_n21_), .b(new_n20_), .c(x4), .d(x2), .O(new_n41_));
  inv1   g24(.a(x1), .O(new_n42_));
  nand2  g25(.a(x3), .b(x2), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n23_), .c(new_n42_), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n41_), .c(x0), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n34_), .c(x5), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n40_), .c(new_n31_), .O(z1));
  inv1   g30(.a(x3), .O(new_n48_));
  oai21  g31(.a(new_n29_), .b(new_n48_), .c(new_n19_), .O(new_n49_));
  oai22  g32(.a(new_n38_), .b(new_n18_), .c(new_n35_), .d(new_n42_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g34(.a(x5), .b(x1), .c(x0), .O(new_n52_));
  nand2  g35(.a(x5), .b(x1), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n52_), .c(new_n20_), .O(new_n54_));
  nand2  g37(.a(x5), .b(x0), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n42_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n49_), .c(x6), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n54_), .c(x4), .O(new_n58_));
  oai21  g41(.a(new_n20_), .b(x3), .c(x2), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  nand2  g43(.a(new_n29_), .b(new_n18_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n60_), .c(x1), .O(new_n62_));
  nand3  g45(.a(x6), .b(new_n29_), .c(new_n19_), .O(new_n63_));
  nand2  g46(.a(new_n20_), .b(x5), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n19_), .c(new_n63_), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n62_), .c(new_n23_), .O(new_n66_));
  nand4  g49(.a(new_n66_), .b(new_n58_), .c(new_n51_), .d(new_n31_), .O(z2));
  nand3  g50(.a(new_n29_), .b(x3), .c(new_n19_), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n55_), .O(new_n69_));
  oai21  g52(.a(new_n36_), .b(x1), .c(new_n69_), .O(new_n70_));
  xnor2a g53(.a(x5), .b(x0), .O(new_n71_));
  nand3  g54(.a(x5), .b(new_n48_), .c(new_n19_), .O(new_n72_));
  oai21  g55(.a(new_n71_), .b(new_n36_), .c(new_n72_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n42_), .O(new_n74_));
  nand2  g57(.a(new_n20_), .b(x2), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n29_), .c(x1), .O(new_n76_));
  oai21  g59(.a(new_n64_), .b(x3), .c(new_n76_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n19_), .O(new_n78_));
  nand4  g61(.a(new_n78_), .b(new_n74_), .c(new_n70_), .d(new_n31_), .O(z3));
  nand2  g62(.a(x6), .b(new_n42_), .O(new_n80_));
  oai21  g63(.a(new_n75_), .b(new_n42_), .c(new_n80_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n48_), .c(new_n19_), .O(new_n82_));
  xnor2a g65(.a(x6), .b(x1), .O(new_n83_));
  nand2  g66(.a(new_n48_), .b(new_n19_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n83_), .c(x2), .O(new_n85_));
  nor2   g68(.a(x2), .b(x1), .O(new_n86_));
  inv1   g69(.a(new_n86_), .O(new_n87_));
  oai21  g70(.a(new_n21_), .b(new_n19_), .c(new_n87_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(x6), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n85_), .c(new_n82_), .O(z4));
  nor2   g73(.a(x3), .b(new_n18_), .O(new_n91_));
  inv1   g74(.a(new_n91_), .O(new_n92_));
  nor2   g75(.a(new_n20_), .b(new_n48_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n18_), .c(x1), .O(new_n94_));
  aoi21  g77(.a(new_n94_), .b(new_n92_), .c(x0), .O(new_n95_));
  nand2  g78(.a(x3), .b(x1), .O(new_n96_));
  nand3  g79(.a(new_n96_), .b(x6), .c(new_n18_), .O(new_n97_));
  aoi21  g80(.a(new_n97_), .b(new_n43_), .c(new_n19_), .O(new_n98_));
  or2    g81(.a(new_n98_), .b(new_n95_), .O(z5));
  inv1   g82(.a(new_n93_), .O(new_n100_));
  aoi21  g83(.a(x6), .b(new_n48_), .c(x2), .O(new_n101_));
  oai22  g84(.a(new_n101_), .b(new_n42_), .c(new_n100_), .d(new_n87_), .O(z6));
  oai21  g85(.a(new_n100_), .b(x2), .c(new_n92_), .O(z7));
  nor2   g86(.a(new_n30_), .b(x3), .O(z8));
  inv1   g87(.a(new_n21_), .O(new_n105_));
  oai21  g88(.a(new_n105_), .b(x0), .c(x2), .O(new_n106_));
  nand2  g89(.a(new_n106_), .b(new_n20_), .O(new_n107_));
  nand3  g90(.a(new_n107_), .b(x5), .c(x4), .O(new_n108_));
  inv1   g91(.a(new_n108_), .O(z9));
endmodule


