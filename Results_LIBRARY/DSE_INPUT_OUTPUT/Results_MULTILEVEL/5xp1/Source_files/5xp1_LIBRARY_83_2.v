// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n110_, new_n111_;
  nand2  g00(.a(x5), .b(x4), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x6), .O(new_n23_));
  inv1   g06(.a(x0), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  nand3  g08(.a(x3), .b(x2), .c(x1), .O(new_n26_));
  nand4  g09(.a(new_n26_), .b(new_n25_), .c(x4), .d(new_n24_), .O(new_n27_));
  oai21  g10(.a(new_n23_), .b(x4), .c(new_n27_), .O(new_n28_));
  inv1   g11(.a(x4), .O(new_n29_));
  nor2   g12(.a(x5), .b(new_n29_), .O(new_n30_));
  aoi21  g13(.a(new_n28_), .b(x5), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n19_), .O(z0));
  inv1   g15(.a(x5), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  inv1   g17(.a(x3), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(x1), .c(x0), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n25_), .O(new_n38_));
  nand4  g21(.a(new_n26_), .b(new_n25_), .c(x5), .d(new_n24_), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  aoi21  g23(.a(new_n38_), .b(new_n33_), .c(new_n40_), .O(new_n41_));
  nand4  g24(.a(new_n23_), .b(x5), .c(new_n29_), .d(new_n24_), .O(new_n42_));
  oai21  g25(.a(new_n41_), .b(new_n29_), .c(new_n42_), .O(z1));
  nand2  g26(.a(x6), .b(x4), .O(new_n44_));
  nor2   g27(.a(x6), .b(x4), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(x3), .c(x2), .d(new_n24_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n44_), .c(new_n20_), .O(new_n47_));
  nand2  g30(.a(new_n21_), .b(new_n24_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x6), .O(new_n49_));
  nor2   g32(.a(new_n49_), .b(new_n29_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n47_), .c(x5), .O(new_n51_));
  oai21  g34(.a(x2), .b(x1), .c(x6), .O(new_n52_));
  nor2   g35(.a(new_n52_), .b(new_n24_), .O(new_n53_));
  oai21  g36(.a(new_n33_), .b(new_n24_), .c(new_n20_), .O(new_n54_));
  oai21  g37(.a(x5), .b(x3), .c(x0), .O(new_n55_));
  aoi21  g38(.a(x5), .b(x3), .c(x0), .O(new_n56_));
  aoi21  g39(.a(new_n55_), .b(new_n34_), .c(new_n56_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n54_), .c(x6), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n53_), .c(x4), .O(new_n59_));
  nand2  g42(.a(new_n22_), .b(x5), .O(new_n60_));
  nand4  g43(.a(new_n60_), .b(x6), .c(new_n29_), .d(new_n24_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n59_), .c(new_n51_), .O(z2));
  nand2  g45(.a(new_n25_), .b(new_n20_), .O(new_n63_));
  nand3  g46(.a(new_n33_), .b(x3), .c(new_n24_), .O(new_n64_));
  oai21  g47(.a(new_n18_), .b(new_n24_), .c(new_n64_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n63_), .c(x2), .O(new_n66_));
  nand2  g49(.a(new_n25_), .b(new_n35_), .O(new_n67_));
  nand4  g50(.a(new_n67_), .b(x5), .c(x4), .d(x0), .O(new_n68_));
  nand3  g51(.a(x6), .b(new_n33_), .c(new_n24_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x1), .O(new_n71_));
  nand2  g54(.a(x5), .b(new_n24_), .O(new_n72_));
  nand3  g55(.a(new_n33_), .b(x4), .c(x0), .O(new_n73_));
  aoi22  g56(.a(new_n73_), .b(new_n72_), .c(x6), .d(x2), .O(new_n74_));
  nand3  g57(.a(x5), .b(new_n35_), .c(new_n24_), .O(new_n75_));
  inv1   g58(.a(new_n75_), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(new_n74_), .c(new_n20_), .O(new_n77_));
  nand3  g60(.a(new_n21_), .b(x5), .c(new_n24_), .O(new_n78_));
  nand2  g61(.a(new_n34_), .b(x0), .O(new_n79_));
  nand2  g62(.a(new_n30_), .b(new_n35_), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n25_), .O(new_n82_));
  nand4  g65(.a(new_n82_), .b(new_n77_), .c(new_n71_), .d(new_n66_), .O(z3));
  nand2  g66(.a(new_n36_), .b(x0), .O(new_n84_));
  aoi21  g67(.a(new_n84_), .b(new_n21_), .c(new_n25_), .O(new_n85_));
  aoi21  g68(.a(new_n48_), .b(new_n36_), .c(x6), .O(new_n86_));
  oai21  g69(.a(new_n86_), .b(new_n85_), .c(x1), .O(new_n87_));
  nand3  g70(.a(new_n25_), .b(x2), .c(new_n20_), .O(new_n88_));
  aoi21  g71(.a(new_n88_), .b(x4), .c(new_n24_), .O(new_n89_));
  aoi21  g72(.a(new_n35_), .b(new_n24_), .c(new_n34_), .O(new_n90_));
  nand3  g73(.a(new_n25_), .b(x3), .c(x2), .O(new_n91_));
  oai21  g74(.a(new_n90_), .b(new_n25_), .c(new_n91_), .O(new_n92_));
  aoi21  g75(.a(new_n92_), .b(new_n20_), .c(new_n89_), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(new_n87_), .O(z4));
  nand2  g77(.a(x3), .b(new_n34_), .O(new_n95_));
  nand2  g78(.a(new_n35_), .b(x2), .O(new_n96_));
  oai21  g79(.a(new_n95_), .b(new_n20_), .c(new_n96_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n24_), .O(new_n98_));
  aoi21  g81(.a(x3), .b(x1), .c(x2), .O(new_n99_));
  nand2  g82(.a(new_n21_), .b(x4), .O(new_n100_));
  oai21  g83(.a(new_n100_), .b(new_n99_), .c(x0), .O(new_n101_));
  nand2  g84(.a(new_n101_), .b(new_n98_), .O(z5));
  nor2   g85(.a(x4), .b(new_n24_), .O(new_n103_));
  nand2  g86(.a(new_n95_), .b(x1), .O(new_n104_));
  nand3  g87(.a(x3), .b(new_n34_), .c(new_n20_), .O(new_n105_));
  aoi21  g88(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(z6));
  inv1   g89(.a(new_n103_), .O(new_n107_));
  nand3  g90(.a(new_n107_), .b(new_n96_), .c(new_n95_), .O(z7));
  nor2   g91(.a(new_n103_), .b(x3), .O(z8));
  nand2  g92(.a(new_n26_), .b(new_n25_), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x5), .c(x4), .O(new_n111_));
  oai21  g94(.a(new_n30_), .b(new_n24_), .c(new_n111_), .O(z9));
endmodule


