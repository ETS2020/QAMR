// Benchmark "FAU" written by ABC on Thu Aug 13 17:56:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n119_;
  inv1   g000(.a(x4), .O(new_n18_));
  nand3  g001(.a(x6), .b(x5), .c(x2), .O(new_n19_));
  nand2  g002(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g003(.a(new_n20_), .b(x3), .O(new_n21_));
  oai21  g004(.a(x6), .b(x0), .c(x5), .O(new_n22_));
  nand2  g005(.a(new_n22_), .b(x4), .O(new_n23_));
  inv1   g006(.a(x0), .O(new_n24_));
  inv1   g007(.a(x1), .O(new_n25_));
  nand2  g008(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand4  g009(.a(new_n26_), .b(x6), .c(x5), .d(new_n18_), .O(new_n27_));
  nand3  g010(.a(new_n27_), .b(new_n23_), .c(new_n21_), .O(z0));
  inv1   g011(.a(x5), .O(new_n29_));
  inv1   g012(.a(x2), .O(new_n30_));
  inv1   g013(.a(x6), .O(new_n31_));
  aoi21  g014(.a(x4), .b(x2), .c(x6), .O(new_n32_));
  oai22  g015(.a(new_n32_), .b(new_n25_), .c(new_n31_), .d(new_n30_), .O(new_n33_));
  nand2  g016(.a(new_n33_), .b(x0), .O(new_n34_));
  nand2  g017(.a(x6), .b(x4), .O(new_n35_));
  nand2  g018(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(new_n29_), .O(new_n37_));
  nand2  g020(.a(x3), .b(x2), .O(new_n38_));
  nand3  g021(.a(new_n38_), .b(new_n18_), .c(new_n25_), .O(new_n39_));
  nand2  g022(.a(new_n31_), .b(x4), .O(new_n40_));
  aoi21  g023(.a(new_n40_), .b(new_n39_), .c(x0), .O(new_n41_));
  nor2   g024(.a(x6), .b(x4), .O(new_n42_));
  oai21  g025(.a(new_n42_), .b(new_n41_), .c(x5), .O(new_n43_));
  nand2  g026(.a(x4), .b(x3), .O(new_n44_));
  nand3  g027(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(z1));
  nand2  g028(.a(new_n42_), .b(x1), .O(new_n46_));
  aoi21  g029(.a(new_n46_), .b(new_n35_), .c(new_n30_), .O(new_n47_));
  nand2  g030(.a(new_n31_), .b(x3), .O(new_n48_));
  nand2  g031(.a(new_n48_), .b(new_n35_), .O(new_n49_));
  nand2  g032(.a(new_n49_), .b(x1), .O(new_n50_));
  inv1   g033(.a(new_n35_), .O(new_n51_));
  oai21  g034(.a(new_n42_), .b(new_n51_), .c(x5), .O(new_n52_));
  nand2  g035(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  oai21  g036(.a(new_n53_), .b(new_n47_), .c(x0), .O(new_n54_));
  oai21  g037(.a(new_n48_), .b(new_n30_), .c(new_n35_), .O(new_n55_));
  nand3  g038(.a(new_n55_), .b(x5), .c(x1), .O(new_n56_));
  aoi21  g039(.a(x3), .b(x2), .c(x0), .O(new_n57_));
  nor2   g040(.a(x5), .b(x2), .O(new_n58_));
  oai21  g041(.a(new_n58_), .b(new_n57_), .c(x6), .O(new_n59_));
  nand3  g042(.a(new_n31_), .b(new_n29_), .c(x4), .O(new_n60_));
  oai21  g043(.a(new_n59_), .b(x4), .c(new_n60_), .O(new_n61_));
  nand2  g044(.a(new_n61_), .b(new_n25_), .O(new_n62_));
  nand3  g045(.a(x6), .b(new_n29_), .c(new_n18_), .O(new_n63_));
  aoi21  g046(.a(new_n63_), .b(new_n40_), .c(x0), .O(new_n64_));
  inv1   g047(.a(x3), .O(new_n65_));
  nand3  g048(.a(new_n31_), .b(new_n29_), .c(new_n30_), .O(new_n66_));
  aoi21  g049(.a(new_n66_), .b(new_n65_), .c(new_n18_), .O(new_n67_));
  nor2   g050(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand4  g051(.a(new_n68_), .b(new_n62_), .c(new_n56_), .d(new_n54_), .O(z2));
  nand2  g052(.a(new_n31_), .b(new_n25_), .O(new_n70_));
  nand2  g053(.a(new_n70_), .b(x2), .O(new_n71_));
  oai21  g054(.a(x6), .b(x3), .c(x1), .O(new_n72_));
  aoi21  g055(.a(new_n72_), .b(new_n71_), .c(new_n24_), .O(new_n73_));
  oai21  g056(.a(new_n31_), .b(new_n25_), .c(new_n38_), .O(new_n74_));
  aoi21  g057(.a(new_n74_), .b(new_n70_), .c(x0), .O(new_n75_));
  oai21  g058(.a(new_n75_), .b(new_n73_), .c(x5), .O(new_n76_));
  aoi21  g059(.a(new_n38_), .b(new_n31_), .c(new_n25_), .O(new_n77_));
  nand3  g060(.a(x6), .b(x3), .c(x2), .O(new_n78_));
  inv1   g061(.a(new_n78_), .O(new_n79_));
  oai21  g062(.a(new_n79_), .b(new_n77_), .c(new_n24_), .O(new_n80_));
  aoi21  g063(.a(x6), .b(x2), .c(x1), .O(new_n81_));
  nor3   g064(.a(x6), .b(x3), .c(x2), .O(new_n82_));
  oai21  g065(.a(new_n82_), .b(new_n81_), .c(x0), .O(new_n83_));
  nand2  g066(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand2  g067(.a(new_n84_), .b(new_n29_), .O(new_n85_));
  nand3  g068(.a(new_n85_), .b(new_n76_), .c(new_n44_), .O(z3));
  nor2   g069(.a(x3), .b(new_n30_), .O(new_n87_));
  nor2   g070(.a(x4), .b(new_n65_), .O(new_n88_));
  oai21  g071(.a(new_n88_), .b(new_n87_), .c(x0), .O(new_n89_));
  nand2  g072(.a(new_n88_), .b(x2), .O(new_n90_));
  aoi21  g073(.a(new_n90_), .b(new_n89_), .c(new_n31_), .O(new_n91_));
  oai21  g074(.a(x4), .b(x2), .c(x3), .O(new_n92_));
  nand2  g075(.a(new_n92_), .b(new_n24_), .O(new_n93_));
  nand2  g076(.a(new_n65_), .b(new_n30_), .O(new_n94_));
  aoi21  g077(.a(new_n94_), .b(new_n93_), .c(x6), .O(new_n95_));
  oai21  g078(.a(new_n95_), .b(new_n91_), .c(x1), .O(new_n96_));
  nand2  g079(.a(new_n18_), .b(x3), .O(z8));
  oai21  g080(.a(x3), .b(new_n24_), .c(z8), .O(new_n98_));
  nand3  g081(.a(new_n98_), .b(new_n31_), .c(x2), .O(new_n99_));
  aoi21  g082(.a(x4), .b(x3), .c(x2), .O(new_n100_));
  nor2   g083(.a(x3), .b(x0), .O(new_n101_));
  oai21  g084(.a(new_n101_), .b(new_n100_), .c(x6), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand2  g086(.a(new_n103_), .b(new_n25_), .O(new_n104_));
  nand2  g087(.a(new_n104_), .b(new_n96_), .O(z4));
  nor2   g088(.a(new_n65_), .b(new_n24_), .O(new_n106_));
  oai21  g089(.a(new_n106_), .b(new_n101_), .c(x2), .O(new_n107_));
  oai21  g090(.a(x2), .b(new_n25_), .c(new_n18_), .O(new_n108_));
  nand3  g091(.a(new_n108_), .b(x3), .c(new_n24_), .O(new_n109_));
  nand2  g092(.a(new_n88_), .b(x1), .O(new_n110_));
  nand3  g093(.a(new_n110_), .b(new_n30_), .c(x0), .O(new_n111_));
  nand3  g094(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(z5));
  nor2   g095(.a(x2), .b(x1), .O(new_n113_));
  oai21  g096(.a(new_n113_), .b(x4), .c(x3), .O(new_n114_));
  nand2  g097(.a(new_n88_), .b(new_n30_), .O(new_n115_));
  nand2  g098(.a(new_n115_), .b(x1), .O(new_n116_));
  nand2  g099(.a(new_n116_), .b(new_n114_), .O(z6));
  oai21  g100(.a(x3), .b(new_n30_), .c(new_n115_), .O(z7));
  oai21  g101(.a(x6), .b(x0), .c(x5), .O(new_n119_));
  nor3   g102(.a(new_n119_), .b(new_n18_), .c(x3), .O(z9));
endmodule


