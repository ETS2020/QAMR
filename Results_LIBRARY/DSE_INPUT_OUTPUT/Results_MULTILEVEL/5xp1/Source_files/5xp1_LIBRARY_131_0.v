// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n104_, new_n106_, new_n108_, new_n109_,
    new_n110_;
  inv1   g00(.a(x3), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand2  g03(.a(x2), .b(x1), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x4), .d(new_n19_), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  nand3  g06(.a(x6), .b(new_n23_), .c(x2), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n22_), .c(new_n18_), .O(new_n25_));
  oai21  g08(.a(x1), .b(x0), .c(x6), .O(new_n26_));
  nor2   g09(.a(new_n26_), .b(x4), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(x5), .O(new_n28_));
  inv1   g11(.a(x5), .O(new_n29_));
  nor2   g12(.a(x6), .b(x3), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n29_), .c(x4), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n28_), .O(z0));
  oai21  g16(.a(x6), .b(x4), .c(x1), .O(new_n34_));
  nand2  g17(.a(x6), .b(x2), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n34_), .c(new_n19_), .O(new_n36_));
  nor2   g19(.a(new_n20_), .b(new_n23_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n36_), .c(new_n29_), .O(new_n38_));
  nor2   g21(.a(x6), .b(x4), .O(new_n39_));
  inv1   g22(.a(x2), .O(new_n40_));
  oai21  g23(.a(x4), .b(x1), .c(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  oai21  g26(.a(x4), .b(x3), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n42_), .c(x0), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n39_), .c(x5), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n38_), .c(new_n31_), .O(z1));
  oai21  g31(.a(x5), .b(x1), .c(x0), .O(new_n49_));
  nand3  g32(.a(x5), .b(x2), .c(x1), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n23_), .O(new_n52_));
  aoi21  g35(.a(x5), .b(x0), .c(x1), .O(new_n53_));
  aoi21  g36(.a(x5), .b(x2), .c(x0), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n53_), .c(x4), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n52_), .c(x6), .O(new_n56_));
  nand4  g39(.a(x6), .b(x5), .c(x4), .d(x2), .O(new_n57_));
  inv1   g40(.a(new_n57_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n56_), .c(x3), .O(new_n59_));
  nand2  g42(.a(new_n29_), .b(new_n19_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x1), .O(new_n61_));
  oai21  g44(.a(x5), .b(x2), .c(x0), .O(new_n62_));
  aoi21  g45(.a(new_n62_), .b(new_n61_), .c(new_n23_), .O(new_n63_));
  nor2   g46(.a(x5), .b(x2), .O(new_n64_));
  aoi21  g47(.a(x3), .b(x2), .c(x0), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n64_), .c(new_n43_), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(new_n60_), .c(x4), .O(new_n67_));
  oai21  g50(.a(new_n67_), .b(new_n63_), .c(x6), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n59_), .O(z2));
  oai21  g52(.a(new_n20_), .b(new_n18_), .c(new_n43_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x2), .O(new_n71_));
  oai21  g54(.a(x6), .b(new_n18_), .c(x1), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n71_), .c(x5), .O(new_n73_));
  oai21  g56(.a(new_n20_), .b(new_n43_), .c(new_n40_), .O(new_n74_));
  aoi21  g57(.a(new_n74_), .b(new_n70_), .c(new_n29_), .O(new_n75_));
  oai21  g58(.a(new_n75_), .b(new_n73_), .c(new_n19_), .O(new_n76_));
  nand3  g59(.a(new_n35_), .b(new_n29_), .c(new_n43_), .O(new_n77_));
  aoi21  g60(.a(x6), .b(x2), .c(x1), .O(new_n78_));
  oai21  g61(.a(new_n78_), .b(new_n29_), .c(new_n77_), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(x0), .c(new_n30_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n76_), .O(z3));
  nand2  g64(.a(x3), .b(x2), .O(new_n82_));
  oai21  g65(.a(x3), .b(x2), .c(x0), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n82_), .c(new_n43_), .O(new_n84_));
  nand2  g67(.a(new_n18_), .b(new_n19_), .O(new_n85_));
  aoi21  g68(.a(new_n85_), .b(x2), .c(x1), .O(new_n86_));
  oai21  g69(.a(new_n86_), .b(new_n84_), .c(x6), .O(new_n87_));
  nand3  g70(.a(new_n40_), .b(x1), .c(new_n19_), .O(new_n88_));
  oai21  g71(.a(new_n40_), .b(x1), .c(new_n88_), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n20_), .c(x3), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n87_), .O(z4));
  oai21  g74(.a(new_n40_), .b(new_n19_), .c(new_n88_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(x3), .O(new_n93_));
  aoi21  g76(.a(x6), .b(new_n40_), .c(x0), .O(new_n94_));
  nor2   g77(.a(x2), .b(new_n19_), .O(new_n95_));
  oai21  g78(.a(new_n95_), .b(new_n94_), .c(new_n18_), .O(new_n96_));
  nor2   g79(.a(x2), .b(x1), .O(new_n97_));
  nor2   g80(.a(x6), .b(new_n40_), .O(new_n98_));
  oai21  g81(.a(new_n98_), .b(new_n97_), .c(x0), .O(new_n99_));
  nand3  g82(.a(new_n99_), .b(new_n96_), .c(new_n93_), .O(z5));
  inv1   g83(.a(new_n82_), .O(new_n101_));
  nor2   g84(.a(new_n20_), .b(x3), .O(z8));
  oai21  g85(.a(z8), .b(new_n101_), .c(x1), .O(new_n103_));
  nand3  g86(.a(x3), .b(new_n40_), .c(new_n43_), .O(new_n104_));
  nand2  g87(.a(new_n104_), .b(new_n103_), .O(z6));
  nand2  g88(.a(new_n18_), .b(x2), .O(new_n106_));
  oai21  g89(.a(z8), .b(x2), .c(new_n106_), .O(z7));
  aoi21  g90(.a(x2), .b(x1), .c(x0), .O(new_n108_));
  oai21  g91(.a(new_n108_), .b(new_n18_), .c(new_n20_), .O(new_n109_));
  nand3  g92(.a(new_n109_), .b(x5), .c(x4), .O(new_n110_));
  inv1   g93(.a(new_n110_), .O(z9));
endmodule


