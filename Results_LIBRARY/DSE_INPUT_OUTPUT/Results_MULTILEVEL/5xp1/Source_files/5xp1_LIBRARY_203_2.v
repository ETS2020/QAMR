// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n90_, new_n91_;
  inv1   g00(.a(x4), .O(new_n18_));
  nand2  g01(.a(x5), .b(new_n18_), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x6), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x1), .O(new_n21_));
  oai21  g04(.a(x6), .b(x0), .c(x5), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x4), .O(new_n23_));
  inv1   g06(.a(x6), .O(new_n24_));
  aoi21  g07(.a(x3), .b(x2), .c(x0), .O(new_n25_));
  nor2   g08(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x5), .c(new_n18_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n23_), .c(new_n21_), .O(z0));
  inv1   g11(.a(x5), .O(new_n29_));
  inv1   g12(.a(x0), .O(new_n30_));
  aoi21  g13(.a(x6), .b(x2), .c(x1), .O(new_n31_));
  oai22  g14(.a(new_n31_), .b(new_n30_), .c(new_n24_), .d(new_n18_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  inv1   g16(.a(x1), .O(new_n34_));
  nand2  g17(.a(x3), .b(x2), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n18_), .c(new_n34_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x6), .c(x0), .O(new_n37_));
  nor2   g20(.a(x6), .b(x4), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n37_), .c(x5), .O(new_n39_));
  nand2  g22(.a(new_n24_), .b(x1), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n39_), .c(new_n33_), .O(z1));
  oai21  g24(.a(x5), .b(x0), .c(x1), .O(new_n42_));
  oai21  g25(.a(x5), .b(x2), .c(x0), .O(new_n43_));
  nand3  g26(.a(x5), .b(x3), .c(x2), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  and2   g28(.a(new_n45_), .b(x4), .O(new_n46_));
  nand2  g29(.a(new_n29_), .b(new_n30_), .O(new_n47_));
  nor2   g30(.a(x5), .b(x2), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n25_), .c(new_n34_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n47_), .c(x4), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n46_), .c(x6), .O(new_n51_));
  nor2   g34(.a(new_n29_), .b(new_n30_), .O(new_n52_));
  oai22  g35(.a(new_n52_), .b(new_n18_), .c(new_n19_), .d(new_n30_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n24_), .c(new_n34_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n51_), .O(z2));
  inv1   g38(.a(new_n52_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x1), .O(new_n58_));
  nand3  g41(.a(new_n29_), .b(x3), .c(new_n30_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(x6), .c(x2), .O(new_n61_));
  oai21  g44(.a(x2), .b(x1), .c(x6), .O(new_n62_));
  xor2a  g45(.a(x5), .b(x0), .O(new_n63_));
  nor2   g46(.a(x1), .b(x0), .O(new_n64_));
  nor2   g47(.a(new_n29_), .b(x3), .O(new_n65_));
  aoi22  g48(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n61_), .c(new_n58_), .O(z3));
  xnor2a g50(.a(x6), .b(x1), .O(new_n68_));
  inv1   g51(.a(x3), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n30_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n68_), .c(x2), .O(new_n71_));
  nand2  g54(.a(new_n70_), .b(x2), .O(new_n72_));
  nor2   g55(.a(new_n69_), .b(new_n34_), .O(new_n73_));
  aoi22  g56(.a(new_n73_), .b(x0), .c(new_n72_), .d(new_n34_), .O(new_n74_));
  oai21  g57(.a(new_n74_), .b(new_n24_), .c(new_n71_), .O(z4));
  inv1   g58(.a(x2), .O(new_n76_));
  nand2  g59(.a(x3), .b(new_n76_), .O(new_n77_));
  nand2  g60(.a(new_n69_), .b(x2), .O(new_n78_));
  oai21  g61(.a(new_n77_), .b(new_n34_), .c(new_n78_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n30_), .O(new_n80_));
  oai21  g63(.a(x2), .b(new_n30_), .c(new_n34_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n24_), .O(new_n82_));
  oai21  g65(.a(new_n73_), .b(x2), .c(new_n35_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(x0), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n82_), .c(new_n80_), .O(z5));
  nand3  g68(.a(new_n77_), .b(x6), .c(x1), .O(new_n86_));
  oai21  g69(.a(new_n77_), .b(x1), .c(new_n86_), .O(z6));
  nand3  g70(.a(new_n78_), .b(new_n77_), .c(new_n40_), .O(z7));
  nand2  g71(.a(new_n40_), .b(x3), .O(z8));
  oai21  g72(.a(x1), .b(new_n30_), .c(new_n24_), .O(new_n90_));
  nand3  g73(.a(new_n90_), .b(x5), .c(x4), .O(new_n91_));
  inv1   g74(.a(new_n91_), .O(z9));
endmodule


