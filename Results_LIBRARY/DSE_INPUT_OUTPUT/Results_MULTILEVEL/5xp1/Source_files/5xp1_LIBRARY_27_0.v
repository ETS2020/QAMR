// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand4  g05(.a(new_n22_), .b(x6), .c(x5), .d(new_n18_), .O(new_n23_));
  inv1   g06(.a(x5), .O(new_n24_));
  nor2   g07(.a(x6), .b(x0), .O(new_n25_));
  aoi21  g08(.a(new_n24_), .b(x4), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n23_), .O(z0));
  inv1   g10(.a(x6), .O(new_n28_));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(new_n18_), .c(new_n28_), .O(new_n30_));
  oai21  g13(.a(x3), .b(x2), .c(x1), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(x0), .c(new_n18_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n30_), .c(new_n24_), .O(new_n33_));
  nand4  g16(.a(new_n21_), .b(x5), .c(new_n18_), .d(new_n20_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x6), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n19_), .O(new_n36_));
  nand3  g19(.a(new_n28_), .b(x5), .c(new_n18_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(new_n33_), .O(z1));
  nand4  g21(.a(x6), .b(x5), .c(x4), .d(x2), .O(new_n39_));
  nor2   g22(.a(x6), .b(x4), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x1), .c(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x3), .O(new_n43_));
  nand2  g26(.a(x6), .b(x4), .O(new_n44_));
  nand2  g27(.a(new_n40_), .b(x2), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x1), .O(new_n47_));
  inv1   g30(.a(new_n44_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n40_), .c(x5), .O(new_n49_));
  nand3  g32(.a(new_n31_), .b(new_n28_), .c(new_n24_), .O(new_n50_));
  nand2  g33(.a(x6), .b(x2), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x4), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(new_n47_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x0), .O(new_n55_));
  aoi21  g38(.a(x3), .b(x2), .c(x0), .O(new_n56_));
  nor2   g39(.a(x5), .b(x2), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n56_), .c(new_n20_), .O(new_n58_));
  nand2  g41(.a(new_n24_), .b(new_n19_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n58_), .c(x4), .O(new_n60_));
  nand3  g43(.a(x5), .b(x4), .c(x1), .O(new_n61_));
  inv1   g44(.a(new_n61_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n60_), .c(x6), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n55_), .c(new_n43_), .O(z2));
  nand3  g47(.a(x5), .b(x1), .c(x0), .O(new_n65_));
  nand3  g48(.a(new_n24_), .b(x2), .c(new_n19_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(x3), .O(new_n68_));
  inv1   g51(.a(new_n22_), .O(new_n69_));
  oai21  g52(.a(x6), .b(x2), .c(x1), .O(new_n70_));
  aoi21  g53(.a(new_n70_), .b(new_n51_), .c(new_n19_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n69_), .c(x5), .O(new_n72_));
  oai21  g55(.a(x5), .b(new_n20_), .c(x6), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n19_), .O(new_n74_));
  inv1   g57(.a(x2), .O(new_n75_));
  aoi21  g58(.a(new_n75_), .b(x0), .c(new_n28_), .O(new_n76_));
  inv1   g59(.a(x3), .O(new_n77_));
  nand3  g60(.a(new_n28_), .b(new_n77_), .c(new_n75_), .O(new_n78_));
  oai21  g61(.a(new_n76_), .b(x1), .c(new_n78_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n24_), .O(new_n80_));
  nand4  g63(.a(new_n80_), .b(new_n74_), .c(new_n72_), .d(new_n68_), .O(z3));
  nand2  g64(.a(new_n77_), .b(new_n75_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x0), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n21_), .c(new_n28_), .O(new_n84_));
  aoi21  g67(.a(new_n82_), .b(x0), .c(x6), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n84_), .c(x1), .O(new_n86_));
  xor2a  g69(.a(x6), .b(x2), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n56_), .c(new_n20_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n86_), .O(z4));
  nand2  g72(.a(x3), .b(new_n75_), .O(new_n90_));
  nand2  g73(.a(new_n77_), .b(x2), .O(new_n91_));
  oai21  g74(.a(new_n90_), .b(new_n20_), .c(new_n91_), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(x6), .c(new_n19_), .O(new_n93_));
  inv1   g76(.a(new_n21_), .O(new_n94_));
  aoi21  g77(.a(x3), .b(x1), .c(x2), .O(new_n95_));
  oai21  g78(.a(new_n95_), .b(new_n94_), .c(x0), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n93_), .O(z5));
  inv1   g80(.a(new_n25_), .O(new_n98_));
  nand2  g81(.a(new_n90_), .b(x1), .O(new_n99_));
  nand3  g82(.a(x3), .b(new_n75_), .c(new_n20_), .O(new_n100_));
  nand3  g83(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z6));
  nand3  g84(.a(new_n91_), .b(new_n90_), .c(new_n98_), .O(z7));
  nand2  g85(.a(new_n98_), .b(x3), .O(z8));
  nor3   g86(.a(new_n25_), .b(new_n24_), .c(new_n18_), .O(z9));
endmodule


