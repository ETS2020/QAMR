// Benchmark "FAU" written by ABC on Thu Aug 13 17:56:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_;
  oai21  g000(.a(x1), .b(x0), .c(x6), .O(new_n18_));
  inv1   g001(.a(x0), .O(new_n19_));
  inv1   g002(.a(x6), .O(new_n20_));
  nand3  g003(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand4  g004(.a(new_n21_), .b(new_n20_), .c(x4), .d(new_n19_), .O(new_n22_));
  oai21  g005(.a(new_n18_), .b(x4), .c(new_n22_), .O(new_n23_));
  nand2  g006(.a(new_n23_), .b(x5), .O(new_n24_));
  inv1   g007(.a(x5), .O(new_n25_));
  nand2  g008(.a(new_n25_), .b(x4), .O(new_n26_));
  nand2  g009(.a(x6), .b(x3), .O(new_n27_));
  nand3  g010(.a(new_n27_), .b(new_n26_), .c(new_n24_), .O(z0));
  inv1   g011(.a(x1), .O(new_n29_));
  inv1   g012(.a(x4), .O(new_n30_));
  oai21  g013(.a(new_n20_), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand2  g014(.a(x3), .b(x2), .O(new_n32_));
  nand3  g015(.a(new_n32_), .b(new_n20_), .c(x4), .O(new_n33_));
  aoi21  g016(.a(new_n33_), .b(new_n31_), .c(x0), .O(new_n34_));
  nor2   g017(.a(x6), .b(x4), .O(new_n35_));
  oai21  g018(.a(new_n35_), .b(new_n34_), .c(x5), .O(new_n36_));
  inv1   g019(.a(x2), .O(new_n37_));
  nand2  g020(.a(x4), .b(x1), .O(new_n38_));
  aoi21  g021(.a(new_n38_), .b(new_n20_), .c(new_n37_), .O(new_n39_));
  nand2  g022(.a(x4), .b(x3), .O(new_n40_));
  aoi21  g023(.a(new_n40_), .b(new_n20_), .c(new_n29_), .O(new_n41_));
  oai21  g024(.a(new_n41_), .b(new_n39_), .c(x0), .O(new_n42_));
  nand2  g025(.a(x6), .b(x4), .O(new_n43_));
  nand2  g026(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g027(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand3  g028(.a(new_n45_), .b(new_n36_), .c(new_n27_), .O(z1));
  nand2  g029(.a(x5), .b(x0), .O(new_n47_));
  nand2  g030(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  oai21  g031(.a(x5), .b(x3), .c(x0), .O(new_n49_));
  aoi21  g032(.a(x5), .b(x3), .c(x0), .O(new_n50_));
  aoi21  g033(.a(new_n49_), .b(new_n37_), .c(new_n50_), .O(new_n51_));
  aoi21  g034(.a(new_n51_), .b(new_n48_), .c(new_n30_), .O(new_n52_));
  oai21  g035(.a(x3), .b(x2), .c(x0), .O(new_n53_));
  nand3  g036(.a(x5), .b(x3), .c(x2), .O(new_n54_));
  nand2  g037(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g038(.a(new_n55_), .b(x1), .O(new_n56_));
  aoi21  g039(.a(new_n56_), .b(new_n47_), .c(x4), .O(new_n57_));
  oai21  g040(.a(new_n57_), .b(new_n52_), .c(new_n20_), .O(new_n58_));
  inv1   g041(.a(x3), .O(new_n59_));
  aoi21  g042(.a(x5), .b(x1), .c(x0), .O(new_n60_));
  nor3   g043(.a(x5), .b(x2), .c(x1), .O(new_n61_));
  oai21  g044(.a(new_n61_), .b(new_n60_), .c(new_n30_), .O(new_n62_));
  nor2   g045(.a(x5), .b(x2), .O(new_n63_));
  oai21  g046(.a(x5), .b(x0), .c(x1), .O(new_n64_));
  oai21  g047(.a(new_n63_), .b(new_n19_), .c(new_n64_), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(x4), .O(new_n66_));
  nand2  g049(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  nand3  g050(.a(new_n67_), .b(x6), .c(new_n59_), .O(new_n68_));
  nand2  g051(.a(new_n68_), .b(new_n58_), .O(z2));
  oai21  g052(.a(x3), .b(x2), .c(x1), .O(new_n70_));
  nand3  g053(.a(new_n70_), .b(new_n25_), .c(x0), .O(new_n71_));
  nand3  g054(.a(new_n32_), .b(x5), .c(new_n19_), .O(new_n72_));
  nand2  g055(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g056(.a(new_n73_), .b(new_n20_), .O(new_n74_));
  nand2  g057(.a(new_n63_), .b(x0), .O(new_n75_));
  oai21  g058(.a(new_n25_), .b(x0), .c(new_n75_), .O(new_n76_));
  nand2  g059(.a(new_n76_), .b(new_n29_), .O(new_n77_));
  nand2  g060(.a(new_n32_), .b(new_n20_), .O(new_n78_));
  nand3  g061(.a(new_n78_), .b(new_n25_), .c(new_n19_), .O(new_n79_));
  nand3  g062(.a(new_n20_), .b(new_n59_), .c(new_n37_), .O(new_n80_));
  nand3  g063(.a(new_n80_), .b(x5), .c(x0), .O(new_n81_));
  nand2  g064(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(x1), .O(new_n83_));
  nand3  g066(.a(x5), .b(x2), .c(x0), .O(new_n84_));
  nand2  g067(.a(new_n84_), .b(new_n59_), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(x6), .O(new_n86_));
  nand4  g069(.a(new_n86_), .b(new_n83_), .c(new_n77_), .d(new_n74_), .O(z3));
  nand3  g070(.a(x6), .b(new_n59_), .c(x1), .O(new_n88_));
  nand2  g071(.a(new_n20_), .b(new_n29_), .O(new_n89_));
  aoi21  g072(.a(new_n89_), .b(new_n88_), .c(new_n19_), .O(new_n90_));
  nand3  g073(.a(new_n20_), .b(x3), .c(new_n29_), .O(new_n91_));
  inv1   g074(.a(new_n91_), .O(new_n92_));
  oai21  g075(.a(new_n92_), .b(new_n90_), .c(x2), .O(new_n93_));
  xor2a  g076(.a(x6), .b(x1), .O(new_n94_));
  nand2  g077(.a(x2), .b(x0), .O(new_n95_));
  nand3  g078(.a(new_n95_), .b(new_n94_), .c(new_n59_), .O(new_n96_));
  nand4  g079(.a(new_n20_), .b(new_n37_), .c(x1), .d(new_n19_), .O(new_n97_));
  nand3  g080(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(z4));
  nand2  g081(.a(new_n20_), .b(x3), .O(z8));
  nand2  g082(.a(new_n37_), .b(x1), .O(new_n100_));
  oai22  g083(.a(new_n100_), .b(z8), .c(x3), .d(new_n37_), .O(new_n101_));
  nand2  g084(.a(new_n101_), .b(new_n19_), .O(new_n102_));
  nor2   g085(.a(x3), .b(x2), .O(new_n103_));
  aoi21  g086(.a(new_n37_), .b(x1), .c(x6), .O(new_n104_));
  aoi21  g087(.a(new_n104_), .b(x3), .c(new_n103_), .O(new_n105_));
  oai21  g088(.a(new_n105_), .b(new_n19_), .c(new_n102_), .O(z5));
  nor2   g089(.a(x2), .b(x1), .O(new_n107_));
  oai21  g090(.a(new_n107_), .b(x6), .c(x3), .O(new_n108_));
  oai21  g091(.a(z8), .b(x2), .c(x1), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(new_n108_), .O(z6));
  nand2  g093(.a(z8), .b(x2), .O(new_n111_));
  oai21  g094(.a(new_n59_), .b(x2), .c(new_n111_), .O(z7));
  nand4  g095(.a(x5), .b(x4), .c(x2), .d(x1), .O(new_n113_));
  nand2  g096(.a(new_n113_), .b(new_n20_), .O(new_n114_));
  nand2  g097(.a(new_n114_), .b(x3), .O(new_n115_));
  nand2  g098(.a(new_n20_), .b(new_n19_), .O(new_n116_));
  nand3  g099(.a(new_n116_), .b(x5), .c(x4), .O(new_n117_));
  nand2  g100(.a(new_n117_), .b(new_n115_), .O(z9));
endmodule


