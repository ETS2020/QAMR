// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n101_, new_n102_;
  inv1   g00(.a(x5), .O(new_n18_));
  oai21  g01(.a(x6), .b(x4), .c(new_n18_), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x6), .O(new_n21_));
  nand3  g04(.a(x3), .b(x2), .c(x1), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(x5), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(x4), .c(new_n20_), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  nand2  g09(.a(x6), .b(new_n26_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n19_), .O(z0));
  oai21  g11(.a(x3), .b(x2), .c(x1), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n20_), .c(new_n21_), .O(new_n30_));
  nand4  g13(.a(new_n22_), .b(new_n21_), .c(x5), .d(new_n20_), .O(new_n31_));
  inv1   g14(.a(new_n31_), .O(new_n32_));
  aoi21  g15(.a(new_n30_), .b(new_n18_), .c(new_n32_), .O(new_n33_));
  nand3  g16(.a(new_n21_), .b(x5), .c(new_n26_), .O(new_n34_));
  oai21  g17(.a(new_n33_), .b(new_n26_), .c(new_n34_), .O(z1));
  inv1   g18(.a(x1), .O(new_n36_));
  nand2  g19(.a(x5), .b(x0), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g21(.a(x2), .O(new_n39_));
  oai21  g22(.a(x5), .b(x3), .c(x0), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  inv1   g24(.a(x3), .O(new_n42_));
  oai21  g25(.a(new_n18_), .b(new_n42_), .c(new_n20_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n21_), .c(x4), .O(new_n45_));
  oai21  g28(.a(x3), .b(x2), .c(x0), .O(new_n46_));
  nand3  g29(.a(x5), .b(x3), .c(x2), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n46_), .c(x4), .O(new_n48_));
  aoi21  g31(.a(new_n18_), .b(new_n20_), .c(new_n21_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n48_), .c(x1), .O(new_n50_));
  oai21  g33(.a(x5), .b(x2), .c(x0), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n47_), .c(x4), .O(new_n52_));
  nor2   g35(.a(new_n18_), .b(x4), .O(new_n53_));
  aoi22  g36(.a(new_n53_), .b(x0), .c(new_n52_), .d(x6), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z2));
  nand2  g38(.a(new_n21_), .b(new_n36_), .O(new_n56_));
  nand3  g39(.a(new_n18_), .b(x3), .c(new_n20_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n37_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n56_), .c(x2), .O(new_n59_));
  oai21  g42(.a(x6), .b(x3), .c(x1), .O(new_n60_));
  nor2   g43(.a(new_n60_), .b(new_n20_), .O(new_n61_));
  nand2  g44(.a(x3), .b(x2), .O(new_n62_));
  nand2  g45(.a(x6), .b(x1), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(new_n56_), .c(x0), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n61_), .c(x5), .O(new_n66_));
  nand2  g49(.a(x6), .b(x2), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n36_), .O(new_n68_));
  nand3  g51(.a(new_n21_), .b(new_n42_), .c(new_n39_), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(new_n68_), .c(new_n20_), .O(new_n70_));
  nor2   g53(.a(new_n63_), .b(x0), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n70_), .c(new_n18_), .O(new_n72_));
  nand4  g55(.a(new_n72_), .b(new_n66_), .c(new_n59_), .d(new_n27_), .O(z3));
  nand2  g56(.a(new_n46_), .b(new_n62_), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(x6), .c(x4), .O(new_n75_));
  nor2   g58(.a(x3), .b(x2), .O(new_n76_));
  aoi21  g59(.a(x3), .b(x2), .c(x0), .O(new_n77_));
  oai21  g60(.a(new_n77_), .b(new_n76_), .c(new_n21_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x1), .O(new_n80_));
  nand2  g63(.a(new_n42_), .b(new_n20_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n21_), .c(x2), .O(new_n82_));
  oai21  g65(.a(x3), .b(x0), .c(x2), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(x6), .c(x4), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n36_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n80_), .O(z4));
  nand2  g70(.a(x3), .b(new_n39_), .O(new_n88_));
  nand2  g71(.a(new_n42_), .b(x2), .O(new_n89_));
  oai21  g72(.a(new_n88_), .b(new_n36_), .c(new_n89_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n20_), .O(new_n91_));
  inv1   g74(.a(new_n62_), .O(new_n92_));
  aoi21  g75(.a(x3), .b(x1), .c(x2), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n92_), .c(x0), .O(new_n94_));
  nand3  g77(.a(new_n94_), .b(new_n91_), .c(new_n27_), .O(z5));
  nand2  g78(.a(new_n88_), .b(x1), .O(new_n96_));
  nand3  g79(.a(x3), .b(new_n39_), .c(new_n36_), .O(new_n97_));
  aoi22  g80(.a(new_n97_), .b(new_n96_), .c(x6), .d(new_n26_), .O(z6));
  nand3  g81(.a(new_n89_), .b(new_n88_), .c(new_n27_), .O(z7));
  nand2  g82(.a(new_n27_), .b(x3), .O(z8));
  aoi21  g83(.a(new_n22_), .b(new_n20_), .c(new_n26_), .O(new_n101_));
  oai21  g84(.a(new_n101_), .b(x6), .c(x5), .O(new_n102_));
  nand2  g85(.a(new_n102_), .b(new_n27_), .O(z9));
endmodule


