// Benchmark "FAU" written by ABC on Thu Aug 13 17:56:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n111_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x1), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n19_), .c(new_n18_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  nor2   g07(.a(new_n21_), .b(x1), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(new_n18_), .c(x4), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n24_), .c(x5), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  nor2   g11(.a(x6), .b(x4), .O(new_n29_));
  aoi21  g12(.a(new_n28_), .b(x4), .c(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n27_), .O(z0));
  inv1   g14(.a(x4), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(x1), .c(x2), .O(new_n33_));
  oai21  g16(.a(new_n32_), .b(x3), .c(x1), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n33_), .c(new_n18_), .O(new_n35_));
  nor2   g18(.a(new_n19_), .b(new_n32_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n35_), .c(new_n28_), .O(new_n37_));
  oai21  g20(.a(x4), .b(x1), .c(x6), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n20_), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand2  g23(.a(new_n19_), .b(new_n40_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n39_), .c(new_n28_), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n18_), .c(new_n29_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n37_), .O(z1));
  oai21  g27(.a(x5), .b(x0), .c(x1), .O(new_n45_));
  oai21  g28(.a(x5), .b(x2), .c(x0), .O(new_n46_));
  nand3  g29(.a(x5), .b(x3), .c(x2), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  and2   g31(.a(new_n48_), .b(x6), .O(new_n49_));
  nand2  g32(.a(x5), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n40_), .O(new_n51_));
  inv1   g34(.a(x2), .O(new_n52_));
  oai21  g35(.a(x5), .b(x3), .c(x0), .O(new_n53_));
  aoi21  g36(.a(x5), .b(x3), .c(x0), .O(new_n54_));
  aoi21  g37(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n51_), .c(x6), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n49_), .c(x4), .O(new_n57_));
  aoi22  g40(.a(new_n20_), .b(new_n18_), .c(new_n28_), .d(new_n52_), .O(new_n58_));
  oai22  g41(.a(new_n58_), .b(x1), .c(x5), .d(x0), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(x6), .c(new_n32_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n57_), .O(z2));
  oai21  g44(.a(new_n32_), .b(new_n40_), .c(new_n19_), .O(new_n62_));
  nand3  g45(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n62_), .c(x2), .O(new_n65_));
  aoi21  g48(.a(x4), .b(x3), .c(x6), .O(new_n66_));
  nor3   g49(.a(new_n66_), .b(new_n40_), .c(new_n18_), .O(new_n67_));
  nor2   g50(.a(new_n19_), .b(x1), .O(new_n68_));
  nor2   g51(.a(x6), .b(new_n32_), .O(new_n69_));
  oai21  g52(.a(new_n69_), .b(new_n68_), .c(new_n20_), .O(new_n70_));
  nand2  g53(.a(new_n69_), .b(new_n40_), .O(new_n71_));
  aoi21  g54(.a(new_n71_), .b(new_n70_), .c(x0), .O(new_n72_));
  oai21  g55(.a(new_n72_), .b(new_n67_), .c(x5), .O(new_n73_));
  nand2  g56(.a(new_n19_), .b(x4), .O(new_n74_));
  nand2  g57(.a(x6), .b(new_n52_), .O(new_n75_));
  aoi21  g58(.a(new_n75_), .b(new_n74_), .c(x1), .O(new_n76_));
  inv1   g59(.a(x3), .O(new_n77_));
  nand4  g60(.a(new_n19_), .b(x4), .c(new_n77_), .d(new_n52_), .O(new_n78_));
  inv1   g61(.a(new_n78_), .O(new_n79_));
  oai21  g62(.a(new_n79_), .b(new_n76_), .c(x0), .O(new_n80_));
  nand3  g63(.a(x6), .b(x1), .c(new_n18_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n28_), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n73_), .c(new_n65_), .O(z3));
  nand2  g67(.a(new_n77_), .b(new_n52_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x0), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n20_), .O(new_n87_));
  nand2  g70(.a(new_n20_), .b(new_n18_), .O(new_n88_));
  aoi21  g71(.a(new_n85_), .b(new_n88_), .c(x6), .O(new_n89_));
  aoi22  g72(.a(new_n89_), .b(x4), .c(new_n87_), .d(x6), .O(new_n90_));
  nor2   g73(.a(x3), .b(x0), .O(new_n91_));
  nor2   g74(.a(new_n91_), .b(x6), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(x4), .c(x2), .O(new_n93_));
  oai21  g76(.a(new_n91_), .b(new_n52_), .c(x6), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g78(.a(new_n95_), .b(new_n40_), .O(new_n96_));
  oai21  g79(.a(new_n90_), .b(new_n40_), .c(new_n96_), .O(z4));
  inv1   g80(.a(new_n29_), .O(new_n98_));
  nand2  g81(.a(x3), .b(new_n52_), .O(new_n99_));
  nand2  g82(.a(new_n77_), .b(x2), .O(new_n100_));
  oai21  g83(.a(new_n99_), .b(new_n40_), .c(new_n100_), .O(new_n101_));
  nand2  g84(.a(new_n101_), .b(new_n18_), .O(new_n102_));
  aoi21  g85(.a(x3), .b(x1), .c(x2), .O(new_n103_));
  oai21  g86(.a(new_n103_), .b(new_n21_), .c(x0), .O(new_n104_));
  nand3  g87(.a(new_n104_), .b(new_n102_), .c(new_n98_), .O(z5));
  nand2  g88(.a(new_n99_), .b(x1), .O(new_n106_));
  nand3  g89(.a(x3), .b(new_n52_), .c(new_n40_), .O(new_n107_));
  aoi21  g90(.a(new_n107_), .b(new_n106_), .c(new_n29_), .O(z6));
  nand3  g91(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z7));
  nor2   g92(.a(new_n29_), .b(x3), .O(z8));
  nand3  g93(.a(new_n23_), .b(x5), .c(x4), .O(new_n111_));
  nand2  g94(.a(new_n111_), .b(new_n98_), .O(z9));
endmodule


