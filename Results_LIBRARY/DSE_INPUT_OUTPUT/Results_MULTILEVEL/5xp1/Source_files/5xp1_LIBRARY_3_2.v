// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n88_, new_n89_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  oai21  g03(.a(x6), .b(x0), .c(x5), .O(new_n21_));
  aoi22  g04(.a(new_n21_), .b(new_n19_), .c(x6), .d(new_n20_), .O(new_n22_));
  inv1   g05(.a(x0), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n19_), .c(new_n23_), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(x6), .c(x5), .d(new_n18_), .O(new_n26_));
  oai21  g09(.a(new_n22_), .b(new_n18_), .c(new_n26_), .O(z0));
  oai21  g10(.a(x2), .b(x1), .c(x0), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n18_), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(x6), .c(new_n20_), .O(new_n30_));
  inv1   g13(.a(x6), .O(new_n31_));
  oai21  g14(.a(new_n18_), .b(new_n23_), .c(new_n31_), .O(new_n32_));
  nand3  g15(.a(new_n24_), .b(new_n18_), .c(new_n23_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x5), .c(new_n19_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n30_), .O(z1));
  nand2  g19(.a(new_n20_), .b(new_n23_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x1), .O(new_n38_));
  nor2   g21(.a(x5), .b(x2), .O(new_n39_));
  nand3  g22(.a(x5), .b(x3), .c(x2), .O(new_n40_));
  oai21  g23(.a(new_n39_), .b(new_n23_), .c(new_n40_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x6), .O(new_n42_));
  nand2  g25(.a(x5), .b(x0), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n31_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x4), .O(new_n46_));
  aoi21  g29(.a(x3), .b(x2), .c(x0), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n39_), .c(new_n19_), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n37_), .c(new_n31_), .O(new_n49_));
  nand3  g32(.a(new_n31_), .b(x5), .c(x0), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n49_), .c(new_n18_), .O(new_n52_));
  nand2  g35(.a(new_n31_), .b(x1), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n46_), .O(z2));
  nand2  g37(.a(new_n43_), .b(new_n37_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(x1), .O(new_n56_));
  nand3  g39(.a(new_n20_), .b(x3), .c(new_n23_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n43_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x6), .c(x2), .O(new_n59_));
  oai21  g42(.a(x2), .b(x1), .c(x6), .O(new_n60_));
  xor2a  g43(.a(x5), .b(x0), .O(new_n61_));
  nor2   g44(.a(x1), .b(x0), .O(new_n62_));
  nor2   g45(.a(new_n20_), .b(x3), .O(new_n63_));
  aoi22  g46(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n59_), .c(new_n56_), .O(z3));
  xnor2a g48(.a(x6), .b(x1), .O(new_n66_));
  inv1   g49(.a(x3), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n23_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n66_), .c(x2), .O(new_n69_));
  nand2  g52(.a(new_n68_), .b(x2), .O(new_n70_));
  nor2   g53(.a(new_n67_), .b(new_n19_), .O(new_n71_));
  aoi22  g54(.a(new_n71_), .b(x0), .c(new_n70_), .d(new_n19_), .O(new_n72_));
  oai21  g55(.a(new_n72_), .b(new_n31_), .c(new_n69_), .O(z4));
  inv1   g56(.a(x2), .O(new_n74_));
  nand2  g57(.a(x3), .b(new_n74_), .O(new_n75_));
  nand2  g58(.a(new_n67_), .b(x2), .O(new_n76_));
  oai21  g59(.a(new_n75_), .b(new_n19_), .c(new_n76_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n23_), .O(new_n78_));
  oai21  g61(.a(x2), .b(new_n23_), .c(new_n19_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n31_), .O(new_n80_));
  oai21  g63(.a(new_n71_), .b(x2), .c(new_n24_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(x0), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(z5));
  nand3  g66(.a(new_n75_), .b(x6), .c(x1), .O(new_n84_));
  oai21  g67(.a(new_n75_), .b(x1), .c(new_n84_), .O(z6));
  nand3  g68(.a(new_n76_), .b(new_n75_), .c(new_n53_), .O(z7));
  nand2  g69(.a(new_n53_), .b(x3), .O(z8));
  oai21  g70(.a(x1), .b(new_n23_), .c(new_n31_), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(x5), .c(x4), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z9));
endmodule


