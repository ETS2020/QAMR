// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n105_, new_n106_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x1), .c(x6), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n19_), .c(new_n20_), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(new_n18_), .O(z0));
  inv1   g08(.a(x1), .O(new_n26_));
  inv1   g09(.a(x2), .O(new_n27_));
  aoi21  g10(.a(x4), .b(x1), .c(x6), .O(new_n28_));
  aoi21  g11(.a(x4), .b(x3), .c(x6), .O(new_n29_));
  oai22  g12(.a(new_n29_), .b(new_n26_), .c(new_n28_), .d(new_n27_), .O(new_n30_));
  aoi22  g13(.a(new_n30_), .b(x0), .c(x6), .d(x4), .O(new_n31_));
  nand4  g14(.a(new_n23_), .b(x5), .c(x4), .d(new_n19_), .O(new_n32_));
  oai21  g15(.a(new_n31_), .b(x5), .c(new_n32_), .O(z1));
  nand4  g16(.a(x6), .b(x5), .c(x4), .d(x2), .O(new_n34_));
  nand2  g17(.a(x1), .b(x0), .O(new_n35_));
  nor2   g18(.a(x6), .b(x5), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x3), .O(new_n39_));
  nand2  g22(.a(x6), .b(x4), .O(new_n40_));
  nand3  g23(.a(new_n36_), .b(new_n18_), .c(x2), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(new_n26_), .O(new_n42_));
  oai21  g25(.a(x5), .b(x2), .c(x6), .O(new_n43_));
  nor2   g26(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n42_), .c(x0), .O(new_n45_));
  nand2  g28(.a(x5), .b(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n26_), .O(new_n47_));
  oai21  g30(.a(x5), .b(x3), .c(x0), .O(new_n48_));
  aoi21  g31(.a(x5), .b(x3), .c(x0), .O(new_n49_));
  aoi21  g32(.a(new_n48_), .b(new_n27_), .c(new_n49_), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n47_), .c(x6), .O(new_n51_));
  nand3  g34(.a(x6), .b(x5), .c(x1), .O(new_n52_));
  inv1   g35(.a(new_n52_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n51_), .c(x4), .O(new_n54_));
  oai21  g37(.a(x2), .b(x1), .c(x0), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(x6), .c(new_n20_), .d(new_n18_), .O(new_n56_));
  nand4  g39(.a(new_n56_), .b(new_n54_), .c(new_n45_), .d(new_n39_), .O(z2));
  inv1   g40(.a(x6), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  nand3  g42(.a(new_n20_), .b(x3), .c(new_n19_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n46_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n59_), .c(x2), .O(new_n62_));
  oai21  g45(.a(x6), .b(x3), .c(x5), .O(new_n63_));
  nand3  g46(.a(x6), .b(new_n20_), .c(new_n19_), .O(new_n64_));
  oai21  g47(.a(new_n63_), .b(new_n19_), .c(new_n64_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(x1), .O(new_n66_));
  nor2   g49(.a(new_n58_), .b(new_n27_), .O(new_n67_));
  xnor2a g50(.a(x5), .b(x0), .O(new_n68_));
  inv1   g51(.a(x3), .O(new_n69_));
  nand3  g52(.a(x5), .b(new_n69_), .c(new_n19_), .O(new_n70_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n26_), .O(new_n72_));
  nand3  g55(.a(new_n21_), .b(new_n58_), .c(new_n19_), .O(new_n73_));
  aoi21  g56(.a(new_n73_), .b(x4), .c(new_n20_), .O(new_n74_));
  nand2  g57(.a(new_n36_), .b(new_n69_), .O(new_n75_));
  nor3   g58(.a(new_n75_), .b(x2), .c(new_n19_), .O(new_n76_));
  nor2   g59(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand4  g60(.a(new_n77_), .b(new_n72_), .c(new_n66_), .d(new_n62_), .O(z3));
  nor2   g61(.a(new_n20_), .b(x4), .O(new_n79_));
  nand2  g62(.a(new_n69_), .b(new_n27_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(x0), .O(new_n81_));
  aoi21  g64(.a(new_n81_), .b(new_n21_), .c(new_n58_), .O(new_n82_));
  nand2  g65(.a(new_n21_), .b(new_n19_), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n80_), .c(x6), .O(new_n84_));
  oai21  g67(.a(new_n84_), .b(new_n82_), .c(x1), .O(new_n85_));
  nor2   g68(.a(x3), .b(x0), .O(new_n86_));
  nor3   g69(.a(new_n86_), .b(x6), .c(new_n27_), .O(new_n87_));
  nor2   g70(.a(new_n86_), .b(new_n27_), .O(new_n88_));
  nor2   g71(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  oai21  g72(.a(new_n89_), .b(new_n87_), .c(new_n26_), .O(new_n90_));
  aoi21  g73(.a(new_n90_), .b(new_n85_), .c(new_n79_), .O(z4));
  nand2  g74(.a(x3), .b(new_n27_), .O(new_n92_));
  nand2  g75(.a(new_n69_), .b(x2), .O(new_n93_));
  oai21  g76(.a(new_n92_), .b(new_n26_), .c(new_n93_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n19_), .O(new_n95_));
  aoi21  g78(.a(x3), .b(x1), .c(x2), .O(new_n96_));
  oai21  g79(.a(new_n96_), .b(new_n22_), .c(x0), .O(new_n97_));
  aoi21  g80(.a(new_n97_), .b(new_n95_), .c(new_n79_), .O(z5));
  nand2  g81(.a(new_n92_), .b(x1), .O(new_n99_));
  nand3  g82(.a(x3), .b(new_n27_), .c(new_n26_), .O(new_n100_));
  aoi21  g83(.a(new_n100_), .b(new_n99_), .c(new_n79_), .O(z6));
  inv1   g84(.a(new_n79_), .O(new_n102_));
  nand3  g85(.a(new_n93_), .b(new_n92_), .c(new_n102_), .O(z7));
  nand2  g86(.a(new_n102_), .b(x3), .O(z8));
  nand2  g87(.a(new_n23_), .b(new_n19_), .O(new_n105_));
  nand3  g88(.a(new_n105_), .b(x5), .c(x4), .O(new_n106_));
  inv1   g89(.a(new_n106_), .O(z9));
endmodule


