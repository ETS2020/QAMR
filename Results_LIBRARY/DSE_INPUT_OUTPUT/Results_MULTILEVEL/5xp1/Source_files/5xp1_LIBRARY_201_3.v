// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n112_;
  inv1   g00(.a(x3), .O(z8));
  oai21  g01(.a(x6), .b(x0), .c(x5), .O(new_n19_));
  nand3  g02(.a(new_n19_), .b(x4), .c(z8), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(x6), .c(x5), .d(new_n21_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n20_), .O(z0));
  inv1   g10(.a(x5), .O(new_n28_));
  nand3  g11(.a(x4), .b(z8), .c(x2), .O(new_n29_));
  nand2  g12(.a(x6), .b(new_n21_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n29_), .c(new_n23_), .O(new_n31_));
  nand3  g14(.a(x6), .b(new_n21_), .c(x2), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n31_), .c(x0), .O(new_n34_));
  nand3  g17(.a(x6), .b(x4), .c(z8), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n28_), .O(new_n37_));
  nand3  g20(.a(new_n24_), .b(new_n21_), .c(new_n23_), .O(new_n38_));
  inv1   g21(.a(x6), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x4), .c(z8), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n38_), .c(x0), .O(new_n41_));
  nor2   g24(.a(x6), .b(x4), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n41_), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n37_), .O(z1));
  nor2   g27(.a(x3), .b(x2), .O(new_n45_));
  nand3  g28(.a(x5), .b(x3), .c(x2), .O(new_n46_));
  oai21  g29(.a(new_n45_), .b(new_n22_), .c(new_n46_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x1), .O(new_n48_));
  nand2  g31(.a(x5), .b(x0), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n48_), .c(x6), .O(new_n50_));
  aoi21  g33(.a(x3), .b(x2), .c(x0), .O(new_n51_));
  nor2   g34(.a(x5), .b(x2), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n51_), .c(new_n23_), .O(new_n53_));
  nand2  g36(.a(new_n28_), .b(new_n22_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n53_), .c(new_n39_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n50_), .c(new_n21_), .O(new_n56_));
  nor2   g39(.a(new_n52_), .b(new_n22_), .O(new_n57_));
  aoi21  g40(.a(new_n54_), .b(x1), .c(new_n57_), .O(new_n58_));
  aoi21  g41(.a(x2), .b(x1), .c(x5), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n22_), .c(new_n39_), .O(new_n60_));
  oai21  g43(.a(new_n58_), .b(new_n39_), .c(new_n60_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(x4), .c(z8), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n56_), .O(z2));
  nand2  g46(.a(new_n39_), .b(new_n23_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(x2), .O(new_n65_));
  oai21  g48(.a(x6), .b(x3), .c(x1), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(new_n65_), .c(new_n22_), .O(new_n67_));
  oai21  g50(.a(new_n39_), .b(new_n23_), .c(new_n24_), .O(new_n68_));
  aoi21  g51(.a(new_n68_), .b(new_n64_), .c(x0), .O(new_n69_));
  oai21  g52(.a(new_n69_), .b(new_n67_), .c(x5), .O(new_n70_));
  aoi21  g53(.a(new_n24_), .b(new_n39_), .c(new_n23_), .O(new_n71_));
  nand3  g54(.a(x6), .b(x3), .c(x2), .O(new_n72_));
  inv1   g55(.a(new_n72_), .O(new_n73_));
  oai21  g56(.a(new_n73_), .b(new_n71_), .c(new_n22_), .O(new_n74_));
  aoi21  g57(.a(x6), .b(x2), .c(x1), .O(new_n75_));
  nor3   g58(.a(x6), .b(x3), .c(x2), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(new_n75_), .c(x0), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n28_), .O(new_n79_));
  nand2  g62(.a(x4), .b(x3), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n79_), .c(new_n70_), .O(z3));
  inv1   g64(.a(x2), .O(new_n82_));
  nor2   g65(.a(x3), .b(new_n82_), .O(new_n83_));
  nor2   g66(.a(x4), .b(z8), .O(new_n84_));
  oai21  g67(.a(new_n84_), .b(new_n83_), .c(x0), .O(new_n85_));
  nand2  g68(.a(new_n84_), .b(x2), .O(new_n86_));
  aoi21  g69(.a(new_n86_), .b(new_n85_), .c(new_n39_), .O(new_n87_));
  oai21  g70(.a(x4), .b(x2), .c(x3), .O(new_n88_));
  aoi21  g71(.a(new_n88_), .b(new_n22_), .c(new_n45_), .O(new_n89_));
  nor2   g72(.a(new_n89_), .b(x6), .O(new_n90_));
  oai21  g73(.a(new_n90_), .b(new_n87_), .c(x1), .O(new_n91_));
  nand2  g74(.a(new_n21_), .b(x3), .O(new_n92_));
  oai21  g75(.a(x3), .b(new_n22_), .c(new_n92_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n39_), .c(x2), .O(new_n94_));
  aoi22  g77(.a(new_n80_), .b(new_n82_), .c(z8), .d(new_n22_), .O(new_n95_));
  oai21  g78(.a(new_n95_), .b(new_n39_), .c(new_n94_), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n23_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n91_), .O(z4));
  inv1   g81(.a(new_n83_), .O(new_n99_));
  nand3  g82(.a(new_n84_), .b(new_n82_), .c(x1), .O(new_n100_));
  nand2  g83(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g84(.a(new_n101_), .b(new_n22_), .O(new_n102_));
  aoi21  g85(.a(new_n82_), .b(x1), .c(x4), .O(new_n103_));
  aoi21  g86(.a(new_n103_), .b(x3), .c(new_n45_), .O(new_n104_));
  oai21  g87(.a(new_n104_), .b(new_n22_), .c(new_n102_), .O(z5));
  nor2   g88(.a(x2), .b(x1), .O(new_n106_));
  oai21  g89(.a(new_n106_), .b(x4), .c(x3), .O(new_n107_));
  nand2  g90(.a(new_n84_), .b(new_n82_), .O(new_n108_));
  nand2  g91(.a(new_n108_), .b(x1), .O(new_n109_));
  nand2  g92(.a(new_n109_), .b(new_n107_), .O(z6));
  nand2  g93(.a(new_n108_), .b(new_n99_), .O(z7));
  oai21  g94(.a(x6), .b(x0), .c(x5), .O(new_n112_));
  nor3   g95(.a(new_n112_), .b(new_n21_), .c(x3), .O(z9));
endmodule


