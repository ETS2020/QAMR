// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n126_, new_n127_,
    new_n128_;
  inv1   g000(.a(x3), .O(z8));
  inv1   g001(.a(x5), .O(new_n19_));
  inv1   g002(.a(x6), .O(new_n20_));
  inv1   g003(.a(x0), .O(new_n21_));
  nand2  g004(.a(x4), .b(new_n21_), .O(new_n22_));
  oai21  g005(.a(new_n22_), .b(new_n19_), .c(new_n20_), .O(new_n23_));
  nand2  g006(.a(new_n23_), .b(z8), .O(new_n24_));
  nand2  g007(.a(x2), .b(x1), .O(new_n25_));
  nand4  g008(.a(new_n25_), .b(new_n20_), .c(x4), .d(new_n21_), .O(new_n26_));
  inv1   g009(.a(x4), .O(new_n27_));
  nor2   g010(.a(x2), .b(x1), .O(new_n28_));
  nand2  g011(.a(new_n28_), .b(new_n21_), .O(new_n29_));
  nand3  g012(.a(new_n29_), .b(x6), .c(new_n27_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g014(.a(new_n31_), .b(x5), .O(new_n32_));
  nand2  g015(.a(new_n19_), .b(x4), .O(new_n33_));
  nand3  g016(.a(new_n33_), .b(new_n32_), .c(new_n24_), .O(z0));
  inv1   g017(.a(x2), .O(new_n35_));
  nand3  g018(.a(new_n27_), .b(x3), .c(new_n35_), .O(new_n36_));
  nand2  g019(.a(new_n20_), .b(x4), .O(new_n37_));
  aoi21  g020(.a(new_n37_), .b(new_n36_), .c(x1), .O(new_n38_));
  nand2  g021(.a(x3), .b(x2), .O(new_n39_));
  nand3  g022(.a(new_n39_), .b(new_n20_), .c(x4), .O(new_n40_));
  inv1   g023(.a(new_n40_), .O(new_n41_));
  oai21  g024(.a(new_n41_), .b(new_n38_), .c(new_n21_), .O(new_n42_));
  nand2  g025(.a(new_n20_), .b(new_n27_), .O(new_n43_));
  nand2  g026(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g027(.a(new_n44_), .b(x5), .O(new_n45_));
  nand2  g028(.a(x6), .b(x3), .O(new_n46_));
  nand3  g029(.a(new_n20_), .b(x4), .c(x1), .O(new_n47_));
  nand2  g030(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g031(.a(new_n48_), .b(x2), .O(new_n49_));
  nand3  g032(.a(new_n43_), .b(x3), .c(x1), .O(new_n50_));
  aoi21  g033(.a(new_n50_), .b(new_n49_), .c(new_n21_), .O(new_n51_));
  nand3  g034(.a(x6), .b(x4), .c(x3), .O(new_n52_));
  inv1   g035(.a(new_n52_), .O(new_n53_));
  oai21  g036(.a(new_n53_), .b(new_n51_), .c(new_n19_), .O(new_n54_));
  nand2  g037(.a(new_n54_), .b(new_n45_), .O(z1));
  nor2   g038(.a(x5), .b(x0), .O(new_n56_));
  nand2  g039(.a(x5), .b(x0), .O(new_n57_));
  oai21  g040(.a(new_n56_), .b(new_n28_), .c(new_n57_), .O(new_n58_));
  nand3  g041(.a(new_n58_), .b(x6), .c(x3), .O(new_n59_));
  inv1   g042(.a(new_n59_), .O(new_n60_));
  inv1   g043(.a(x1), .O(new_n61_));
  nand2  g044(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  oai21  g045(.a(x5), .b(x3), .c(x0), .O(new_n63_));
  aoi21  g046(.a(x5), .b(x3), .c(x0), .O(new_n64_));
  aoi21  g047(.a(new_n63_), .b(new_n35_), .c(new_n64_), .O(new_n65_));
  aoi21  g048(.a(new_n65_), .b(new_n62_), .c(x6), .O(new_n66_));
  oai21  g049(.a(new_n66_), .b(new_n60_), .c(x4), .O(new_n67_));
  oai21  g050(.a(x2), .b(x1), .c(x5), .O(new_n68_));
  nor2   g051(.a(x5), .b(x2), .O(new_n69_));
  aoi22  g052(.a(new_n69_), .b(new_n61_), .c(new_n68_), .d(new_n21_), .O(new_n70_));
  oai21  g053(.a(new_n19_), .b(new_n35_), .c(new_n21_), .O(new_n71_));
  nand3  g054(.a(new_n71_), .b(new_n20_), .c(x1), .O(new_n72_));
  oai21  g055(.a(new_n70_), .b(new_n20_), .c(new_n72_), .O(new_n73_));
  aoi21  g056(.a(x2), .b(x1), .c(x5), .O(new_n74_));
  nor3   g057(.a(new_n74_), .b(x6), .c(new_n21_), .O(new_n75_));
  aoi21  g058(.a(new_n73_), .b(x3), .c(new_n75_), .O(new_n76_));
  oai21  g059(.a(new_n76_), .b(x4), .c(new_n67_), .O(z2));
  nand2  g060(.a(z8), .b(new_n35_), .O(new_n78_));
  nand3  g061(.a(new_n78_), .b(x5), .c(x0), .O(new_n79_));
  nand2  g062(.a(new_n39_), .b(new_n20_), .O(new_n80_));
  nand3  g063(.a(new_n80_), .b(new_n19_), .c(new_n21_), .O(new_n81_));
  nand2  g064(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(x1), .O(new_n83_));
  inv1   g066(.a(new_n57_), .O(new_n84_));
  oai21  g067(.a(new_n84_), .b(new_n56_), .c(x2), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(x3), .O(new_n86_));
  nand2  g069(.a(new_n86_), .b(x6), .O(new_n87_));
  nand2  g070(.a(x5), .b(new_n21_), .O(new_n88_));
  nand2  g071(.a(new_n69_), .b(x0), .O(new_n89_));
  aoi21  g072(.a(new_n89_), .b(new_n88_), .c(x3), .O(new_n90_));
  nand2  g073(.a(x6), .b(x2), .O(new_n91_));
  nand2  g074(.a(new_n19_), .b(x0), .O(new_n92_));
  nand2  g075(.a(new_n92_), .b(new_n88_), .O(new_n93_));
  nand3  g076(.a(new_n93_), .b(new_n91_), .c(new_n61_), .O(new_n94_));
  nand4  g077(.a(new_n20_), .b(x5), .c(new_n35_), .d(new_n21_), .O(new_n95_));
  nand2  g078(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g079(.a(new_n96_), .b(new_n90_), .O(new_n97_));
  nand3  g080(.a(new_n97_), .b(new_n87_), .c(new_n83_), .O(z3));
  nand3  g081(.a(new_n20_), .b(x2), .c(new_n61_), .O(new_n99_));
  oai21  g082(.a(new_n20_), .b(new_n61_), .c(new_n99_), .O(new_n100_));
  nand2  g083(.a(new_n100_), .b(x0), .O(new_n101_));
  oai21  g084(.a(x6), .b(x2), .c(x3), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(new_n21_), .O(new_n103_));
  nand3  g086(.a(new_n103_), .b(new_n91_), .c(new_n78_), .O(new_n104_));
  nand2  g087(.a(new_n104_), .b(x1), .O(new_n105_));
  nand3  g088(.a(new_n20_), .b(x3), .c(x2), .O(new_n106_));
  oai21  g089(.a(new_n20_), .b(x2), .c(new_n106_), .O(new_n107_));
  aoi22  g090(.a(new_n107_), .b(new_n61_), .c(x6), .d(z8), .O(new_n108_));
  nand3  g091(.a(new_n108_), .b(new_n105_), .c(new_n101_), .O(z4));
  nand3  g092(.a(new_n35_), .b(x1), .c(new_n21_), .O(new_n110_));
  oai21  g093(.a(new_n35_), .b(new_n21_), .c(new_n110_), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(x3), .O(new_n112_));
  aoi21  g095(.a(new_n20_), .b(new_n35_), .c(x0), .O(new_n113_));
  nor2   g096(.a(x2), .b(new_n21_), .O(new_n114_));
  oai21  g097(.a(new_n114_), .b(new_n113_), .c(z8), .O(new_n115_));
  inv1   g098(.a(new_n91_), .O(new_n116_));
  oai21  g099(.a(new_n116_), .b(new_n28_), .c(x0), .O(new_n117_));
  nand3  g100(.a(new_n117_), .b(new_n115_), .c(new_n112_), .O(z5));
  inv1   g101(.a(new_n39_), .O(new_n119_));
  nor2   g102(.a(x6), .b(x3), .O(new_n120_));
  oai21  g103(.a(new_n120_), .b(new_n119_), .c(x1), .O(new_n121_));
  nand2  g104(.a(x3), .b(new_n35_), .O(new_n122_));
  oai21  g105(.a(new_n122_), .b(x1), .c(new_n121_), .O(z6));
  nand2  g106(.a(new_n120_), .b(x2), .O(new_n124_));
  nand2  g107(.a(new_n124_), .b(new_n122_), .O(z7));
  aoi21  g108(.a(x5), .b(x4), .c(z8), .O(new_n126_));
  oai21  g109(.a(new_n39_), .b(new_n61_), .c(new_n21_), .O(new_n127_));
  nand3  g110(.a(new_n127_), .b(x5), .c(x4), .O(new_n128_));
  oai21  g111(.a(new_n126_), .b(new_n20_), .c(new_n128_), .O(z9));
endmodule


