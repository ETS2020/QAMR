// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_;
  inv1   g00(.a(x4), .O(new_n18_));
  nor2   g01(.a(x5), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  inv1   g03(.a(x6), .O(new_n21_));
  nor3   g04(.a(new_n21_), .b(new_n20_), .c(x4), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n19_), .c(x0), .O(new_n23_));
  aoi21  g06(.a(x3), .b(x2), .c(x1), .O(new_n24_));
  nor3   g07(.a(new_n24_), .b(new_n20_), .c(x4), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n19_), .c(x6), .O(new_n26_));
  nor2   g09(.a(x6), .b(x0), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(new_n26_), .c(new_n23_), .O(z0));
  oai21  g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n18_), .c(x5), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  nand2  g15(.a(x3), .b(x2), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(x5), .c(new_n18_), .d(new_n32_), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x0), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n31_), .c(x6), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  inv1   g20(.a(x3), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(new_n20_), .c(x4), .d(x1), .O(new_n40_));
  nand3  g23(.a(new_n21_), .b(x5), .c(new_n18_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n36_), .O(z1));
  nand4  g27(.a(x6), .b(x5), .c(x4), .d(x2), .O(new_n45_));
  nor2   g28(.a(x6), .b(x4), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(x1), .c(x0), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x3), .O(new_n49_));
  nand2  g32(.a(x6), .b(x4), .O(new_n50_));
  nand2  g33(.a(new_n46_), .b(x2), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x1), .O(new_n53_));
  inv1   g36(.a(new_n50_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n46_), .c(x5), .O(new_n55_));
  oai21  g38(.a(x3), .b(x2), .c(x1), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n21_), .c(new_n20_), .O(new_n57_));
  nand2  g40(.a(x6), .b(x2), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x4), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n55_), .c(new_n53_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x0), .O(new_n62_));
  aoi21  g45(.a(x3), .b(x2), .c(x0), .O(new_n63_));
  nor2   g46(.a(x5), .b(x2), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n63_), .c(new_n32_), .O(new_n65_));
  inv1   g48(.a(x0), .O(new_n66_));
  nand2  g49(.a(new_n20_), .b(new_n66_), .O(new_n67_));
  aoi21  g50(.a(new_n67_), .b(new_n65_), .c(x4), .O(new_n68_));
  nand3  g51(.a(x5), .b(x4), .c(x1), .O(new_n69_));
  inv1   g52(.a(new_n69_), .O(new_n70_));
  oai21  g53(.a(new_n70_), .b(new_n68_), .c(x6), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n62_), .c(new_n49_), .O(z2));
  nand3  g55(.a(x5), .b(x1), .c(x0), .O(new_n73_));
  nand3  g56(.a(new_n20_), .b(x2), .c(new_n66_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x3), .O(new_n76_));
  nand2  g59(.a(new_n24_), .b(new_n66_), .O(new_n77_));
  inv1   g60(.a(new_n77_), .O(new_n78_));
  oai21  g61(.a(x6), .b(x2), .c(x1), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(new_n58_), .c(new_n66_), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n78_), .c(x5), .O(new_n81_));
  oai21  g64(.a(x5), .b(new_n32_), .c(x6), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n66_), .O(new_n83_));
  aoi21  g66(.a(new_n37_), .b(x0), .c(new_n21_), .O(new_n84_));
  nand3  g67(.a(new_n21_), .b(new_n38_), .c(new_n37_), .O(new_n85_));
  oai21  g68(.a(new_n84_), .b(x1), .c(new_n85_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n20_), .O(new_n87_));
  nand4  g70(.a(new_n87_), .b(new_n83_), .c(new_n81_), .d(new_n76_), .O(z3));
  nand2  g71(.a(new_n39_), .b(x0), .O(new_n89_));
  aoi21  g72(.a(new_n89_), .b(new_n33_), .c(new_n21_), .O(new_n90_));
  aoi21  g73(.a(new_n39_), .b(x0), .c(x6), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n90_), .c(x1), .O(new_n92_));
  xor2a  g75(.a(x6), .b(x2), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n63_), .c(new_n32_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n92_), .O(z4));
  xor2a  g78(.a(x3), .b(x0), .O(new_n96_));
  nand2  g79(.a(x3), .b(x1), .O(new_n97_));
  xor2a  g80(.a(new_n97_), .b(new_n66_), .O(new_n98_));
  aoi21  g81(.a(new_n98_), .b(new_n37_), .c(new_n27_), .O(new_n99_));
  oai21  g82(.a(new_n96_), .b(new_n37_), .c(new_n99_), .O(z5));
  nand2  g83(.a(x3), .b(new_n37_), .O(new_n101_));
  nand2  g84(.a(new_n101_), .b(x1), .O(new_n102_));
  nand3  g85(.a(x3), .b(new_n37_), .c(new_n32_), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(new_n102_), .c(new_n28_), .O(z6));
  nand2  g87(.a(new_n38_), .b(x2), .O(new_n105_));
  aoi21  g88(.a(new_n105_), .b(new_n101_), .c(new_n27_), .O(z7));
  nor2   g89(.a(new_n27_), .b(x3), .O(z8));
  oai21  g90(.a(new_n20_), .b(new_n18_), .c(new_n28_), .O(z9));
endmodule


