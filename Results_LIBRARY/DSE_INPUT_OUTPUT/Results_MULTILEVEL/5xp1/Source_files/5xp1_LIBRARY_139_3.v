// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n113_, new_n114_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x1), .O(new_n22_));
  nand4  g05(.a(new_n22_), .b(new_n19_), .c(x4), .d(new_n18_), .O(new_n23_));
  inv1   g06(.a(x4), .O(new_n24_));
  inv1   g07(.a(x1), .O(new_n25_));
  nand3  g08(.a(new_n20_), .b(new_n25_), .c(new_n18_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x6), .c(new_n24_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x5), .O(new_n29_));
  inv1   g12(.a(x5), .O(new_n30_));
  nand3  g13(.a(new_n19_), .b(new_n30_), .c(x4), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n29_), .O(z0));
  inv1   g15(.a(x2), .O(new_n33_));
  aoi21  g16(.a(x4), .b(x1), .c(x6), .O(new_n34_));
  aoi21  g17(.a(x4), .b(x3), .c(x6), .O(new_n35_));
  oai22  g18(.a(new_n35_), .b(new_n25_), .c(new_n34_), .d(new_n33_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n30_), .c(x0), .O(new_n37_));
  nor2   g20(.a(x6), .b(new_n24_), .O(new_n38_));
  nor2   g21(.a(x4), .b(x1), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n38_), .c(new_n20_), .O(new_n40_));
  nand2  g23(.a(new_n38_), .b(new_n25_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(x0), .O(new_n42_));
  nor2   g25(.a(x6), .b(x4), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n42_), .c(x5), .O(new_n44_));
  nor2   g27(.a(new_n19_), .b(new_n24_), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z1));
  oai21  g30(.a(x3), .b(x2), .c(x0), .O(new_n48_));
  nand3  g31(.a(x5), .b(x3), .c(x2), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x1), .O(new_n51_));
  nand2  g34(.a(x5), .b(x0), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n19_), .c(new_n24_), .O(new_n54_));
  nand2  g37(.a(x6), .b(new_n25_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n24_), .c(new_n21_), .O(new_n56_));
  oai21  g39(.a(new_n30_), .b(new_n25_), .c(x4), .O(new_n57_));
  oai21  g40(.a(new_n19_), .b(x5), .c(new_n57_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n56_), .c(new_n18_), .O(new_n59_));
  aoi21  g42(.a(x6), .b(new_n33_), .c(x4), .O(new_n60_));
  inv1   g43(.a(x3), .O(new_n61_));
  nand3  g44(.a(x4), .b(new_n61_), .c(new_n33_), .O(new_n62_));
  oai21  g45(.a(new_n60_), .b(x1), .c(new_n62_), .O(new_n63_));
  aoi21  g46(.a(new_n63_), .b(new_n30_), .c(new_n45_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n59_), .c(new_n54_), .O(z2));
  nand2  g48(.a(x6), .b(new_n24_), .O(new_n66_));
  oai21  g49(.a(x6), .b(new_n25_), .c(new_n66_), .O(new_n67_));
  nand3  g50(.a(new_n30_), .b(x3), .c(new_n18_), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n67_), .c(x2), .O(new_n70_));
  nand3  g53(.a(new_n30_), .b(new_n33_), .c(new_n25_), .O(new_n71_));
  nand3  g54(.a(x6), .b(x5), .c(x1), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n71_), .c(new_n18_), .O(new_n73_));
  nand3  g56(.a(new_n20_), .b(x5), .c(new_n25_), .O(new_n74_));
  nand3  g57(.a(x6), .b(new_n30_), .c(x1), .O(new_n75_));
  aoi21  g58(.a(new_n75_), .b(new_n74_), .c(x0), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(new_n73_), .c(new_n24_), .O(new_n77_));
  xor2a  g60(.a(x5), .b(x0), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n25_), .O(new_n79_));
  nand3  g62(.a(x5), .b(x3), .c(x1), .O(new_n80_));
  inv1   g63(.a(new_n80_), .O(new_n81_));
  nor3   g64(.a(x5), .b(x3), .c(x2), .O(new_n82_));
  oai21  g65(.a(new_n82_), .b(new_n81_), .c(x0), .O(new_n83_));
  nand3  g66(.a(new_n20_), .b(x5), .c(new_n18_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n83_), .c(new_n79_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n19_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n77_), .c(new_n70_), .O(z3));
  nand2  g70(.a(new_n48_), .b(new_n20_), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(x6), .c(new_n24_), .O(new_n89_));
  nor2   g72(.a(x3), .b(x2), .O(new_n90_));
  aoi21  g73(.a(new_n20_), .b(new_n18_), .c(new_n90_), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(x6), .c(new_n89_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(x1), .O(new_n93_));
  nand2  g76(.a(new_n61_), .b(new_n18_), .O(new_n94_));
  nand3  g77(.a(new_n94_), .b(new_n19_), .c(x2), .O(new_n95_));
  nand2  g78(.a(new_n94_), .b(x2), .O(new_n96_));
  nand3  g79(.a(new_n96_), .b(x6), .c(new_n24_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n25_), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(new_n93_), .O(z4));
  nand2  g83(.a(x3), .b(new_n33_), .O(new_n101_));
  nand2  g84(.a(new_n61_), .b(x2), .O(new_n102_));
  oai21  g85(.a(new_n101_), .b(new_n25_), .c(new_n102_), .O(new_n103_));
  nand2  g86(.a(new_n103_), .b(new_n18_), .O(new_n104_));
  aoi21  g87(.a(x3), .b(x1), .c(x2), .O(new_n105_));
  oai21  g88(.a(new_n105_), .b(new_n21_), .c(x0), .O(new_n106_));
  aoi21  g89(.a(new_n106_), .b(new_n104_), .c(new_n45_), .O(z5));
  nand2  g90(.a(new_n101_), .b(x1), .O(new_n108_));
  nand3  g91(.a(x3), .b(new_n33_), .c(new_n25_), .O(new_n109_));
  nand3  g92(.a(new_n109_), .b(new_n108_), .c(new_n46_), .O(z6));
  nand3  g93(.a(new_n102_), .b(new_n101_), .c(new_n46_), .O(z7));
  nor2   g94(.a(new_n45_), .b(x3), .O(z8));
  nand2  g95(.a(new_n22_), .b(new_n18_), .O(new_n113_));
  nand2  g96(.a(new_n113_), .b(x5), .O(new_n114_));
  aoi21  g97(.a(new_n114_), .b(new_n19_), .c(new_n24_), .O(z9));
endmodule


