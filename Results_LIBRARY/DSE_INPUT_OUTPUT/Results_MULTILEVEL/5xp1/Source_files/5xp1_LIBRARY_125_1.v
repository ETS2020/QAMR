// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n106_;
  nand2  g00(.a(x5), .b(x0), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x4), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand4  g07(.a(new_n24_), .b(x6), .c(x5), .d(new_n20_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n19_), .O(z0));
  nand2  g09(.a(new_n24_), .b(x6), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(x5), .c(new_n20_), .O(new_n28_));
  inv1   g11(.a(x5), .O(new_n29_));
  inv1   g12(.a(x6), .O(new_n30_));
  oai21  g13(.a(new_n20_), .b(new_n22_), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x2), .O(new_n32_));
  inv1   g15(.a(x3), .O(new_n33_));
  oai21  g16(.a(new_n20_), .b(new_n33_), .c(new_n30_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x1), .O(new_n35_));
  nand2  g18(.a(x6), .b(x4), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n29_), .c(x0), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n28_), .O(z1));
  oai21  g22(.a(x3), .b(x2), .c(x0), .O(new_n40_));
  nand3  g23(.a(x5), .b(x3), .c(x2), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x1), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n18_), .c(x4), .O(new_n44_));
  inv1   g27(.a(x2), .O(new_n45_));
  nand2  g28(.a(new_n33_), .b(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x1), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n29_), .c(x4), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n44_), .c(new_n30_), .O(new_n50_));
  aoi21  g33(.a(x3), .b(x2), .c(x0), .O(new_n51_));
  nor3   g34(.a(x5), .b(x4), .c(x2), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n51_), .c(new_n22_), .O(new_n53_));
  nand3  g36(.a(new_n29_), .b(new_n45_), .c(new_n22_), .O(new_n54_));
  aoi22  g37(.a(new_n54_), .b(x4), .c(new_n29_), .d(new_n21_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g39(.a(new_n20_), .b(x0), .O(new_n57_));
  aoi21  g40(.a(new_n56_), .b(x6), .c(new_n57_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n50_), .O(z2));
  nand2  g42(.a(new_n30_), .b(new_n22_), .O(new_n60_));
  nand3  g43(.a(new_n29_), .b(x3), .c(new_n21_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n60_), .c(x2), .O(new_n63_));
  oai21  g46(.a(x6), .b(x3), .c(x5), .O(new_n64_));
  nand3  g47(.a(x6), .b(new_n29_), .c(new_n21_), .O(new_n65_));
  oai21  g48(.a(new_n64_), .b(new_n21_), .c(new_n65_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x1), .O(new_n67_));
  nor2   g50(.a(new_n30_), .b(new_n45_), .O(new_n68_));
  xnor2a g51(.a(x5), .b(x0), .O(new_n69_));
  nand3  g52(.a(x5), .b(new_n33_), .c(new_n21_), .O(new_n70_));
  oai21  g53(.a(new_n69_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n22_), .O(new_n72_));
  nand3  g55(.a(new_n23_), .b(x5), .c(new_n21_), .O(new_n73_));
  nand4  g56(.a(new_n29_), .b(new_n33_), .c(new_n45_), .d(x0), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g58(.a(new_n75_), .b(new_n30_), .c(new_n57_), .O(new_n76_));
  nand4  g59(.a(new_n76_), .b(new_n72_), .c(new_n67_), .d(new_n63_), .O(z3));
  nand3  g60(.a(new_n23_), .b(new_n30_), .c(new_n21_), .O(new_n78_));
  nand3  g61(.a(x6), .b(x3), .c(x2), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(new_n78_), .c(x4), .O(new_n80_));
  nand2  g63(.a(new_n46_), .b(x6), .O(new_n81_));
  nand3  g64(.a(new_n30_), .b(new_n33_), .c(new_n45_), .O(new_n82_));
  aoi21  g65(.a(new_n82_), .b(new_n81_), .c(new_n21_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n80_), .c(x1), .O(new_n84_));
  xnor2a g67(.a(x6), .b(x2), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n21_), .O(new_n86_));
  oai21  g69(.a(x3), .b(x0), .c(x2), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(x6), .O(new_n88_));
  nand3  g71(.a(new_n30_), .b(x3), .c(x2), .O(new_n89_));
  aoi21  g72(.a(new_n89_), .b(new_n88_), .c(x4), .O(new_n90_));
  oai21  g73(.a(new_n90_), .b(new_n86_), .c(new_n22_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n84_), .O(z4));
  xor2a  g75(.a(x3), .b(x0), .O(new_n93_));
  nand2  g76(.a(x3), .b(x1), .O(new_n94_));
  xor2a  g77(.a(new_n94_), .b(new_n21_), .O(new_n95_));
  aoi21  g78(.a(new_n95_), .b(new_n45_), .c(new_n57_), .O(new_n96_));
  oai21  g79(.a(new_n93_), .b(new_n45_), .c(new_n96_), .O(z5));
  inv1   g80(.a(new_n57_), .O(new_n98_));
  nand2  g81(.a(x3), .b(new_n45_), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(x1), .O(new_n100_));
  nand3  g83(.a(x3), .b(new_n45_), .c(new_n22_), .O(new_n101_));
  nand3  g84(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(z6));
  nand2  g85(.a(new_n33_), .b(x2), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(new_n99_), .c(new_n98_), .O(z7));
  nand2  g87(.a(new_n98_), .b(x3), .O(z8));
  nand3  g88(.a(x5), .b(x4), .c(x0), .O(new_n106_));
  inv1   g89(.a(new_n106_), .O(z9));
endmodule


