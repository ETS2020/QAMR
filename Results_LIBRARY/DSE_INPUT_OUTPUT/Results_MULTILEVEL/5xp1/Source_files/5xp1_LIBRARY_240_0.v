// Benchmark "FAU" written by ABC on Thu Aug 13 17:56:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n109_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand3  g02(.a(x3), .b(x2), .c(x1), .O(new_n20_));
  nand4  g03(.a(new_n20_), .b(new_n19_), .c(x4), .d(new_n18_), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n18_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x6), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(x4), .c(new_n21_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x5), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x4), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n27_), .O(z0));
  inv1   g13(.a(x4), .O(new_n31_));
  oai21  g14(.a(x3), .b(x2), .c(x1), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n18_), .c(new_n19_), .O(new_n33_));
  nand4  g16(.a(new_n20_), .b(new_n19_), .c(x5), .d(new_n18_), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(new_n35_));
  aoi21  g18(.a(new_n33_), .b(new_n28_), .c(new_n35_), .O(new_n36_));
  nand3  g19(.a(new_n25_), .b(x5), .c(new_n31_), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n31_), .c(new_n37_), .O(z1));
  inv1   g21(.a(x3), .O(new_n39_));
  nand3  g22(.a(x6), .b(x5), .c(x4), .O(new_n40_));
  nand3  g23(.a(new_n19_), .b(new_n31_), .c(x1), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand3  g25(.a(x6), .b(x4), .c(x0), .O(new_n43_));
  inv1   g26(.a(new_n43_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n42_), .c(x2), .O(new_n45_));
  oai21  g28(.a(x5), .b(x1), .c(x0), .O(new_n46_));
  nand2  g29(.a(x5), .b(x1), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n46_), .c(new_n19_), .O(new_n48_));
  nand3  g31(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(x4), .O(new_n51_));
  inv1   g34(.a(x2), .O(new_n52_));
  nand4  g35(.a(x6), .b(new_n31_), .c(new_n22_), .d(new_n18_), .O(new_n53_));
  nand2  g36(.a(new_n19_), .b(new_n28_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(x3), .c(new_n53_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand4  g39(.a(x6), .b(new_n31_), .c(new_n39_), .d(new_n22_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n54_), .c(x0), .O(new_n58_));
  aoi21  g41(.a(new_n19_), .b(x0), .c(new_n28_), .O(new_n59_));
  nand3  g42(.a(new_n19_), .b(new_n28_), .c(new_n22_), .O(new_n60_));
  oai21  g43(.a(new_n59_), .b(x4), .c(new_n60_), .O(new_n61_));
  nor2   g44(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n56_), .c(new_n51_), .d(new_n45_), .O(z2));
  nand2  g46(.a(new_n19_), .b(new_n22_), .O(new_n64_));
  nand3  g47(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n65_));
  oai21  g48(.a(new_n28_), .b(new_n18_), .c(new_n65_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n64_), .c(x2), .O(new_n67_));
  oai21  g50(.a(x6), .b(x3), .c(x5), .O(new_n68_));
  nand3  g51(.a(x6), .b(new_n28_), .c(new_n18_), .O(new_n69_));
  oai21  g52(.a(new_n68_), .b(new_n18_), .c(new_n69_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x1), .O(new_n71_));
  nor2   g54(.a(new_n19_), .b(new_n52_), .O(new_n72_));
  xnor2a g55(.a(x5), .b(x0), .O(new_n73_));
  nand3  g56(.a(x5), .b(new_n39_), .c(new_n18_), .O(new_n74_));
  oai21  g57(.a(new_n73_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n22_), .O(new_n76_));
  nand3  g59(.a(new_n23_), .b(x5), .c(new_n18_), .O(new_n77_));
  nand4  g60(.a(new_n28_), .b(new_n39_), .c(new_n52_), .d(x0), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g62(.a(x5), .b(x4), .O(new_n80_));
  aoi21  g63(.a(new_n79_), .b(new_n19_), .c(new_n80_), .O(new_n81_));
  nand4  g64(.a(new_n81_), .b(new_n76_), .c(new_n71_), .d(new_n67_), .O(z3));
  inv1   g65(.a(new_n80_), .O(new_n83_));
  oai21  g66(.a(x3), .b(x2), .c(x0), .O(new_n84_));
  aoi21  g67(.a(new_n84_), .b(new_n23_), .c(new_n19_), .O(new_n85_));
  nand2  g68(.a(new_n39_), .b(new_n52_), .O(new_n86_));
  nand2  g69(.a(new_n23_), .b(new_n18_), .O(new_n87_));
  aoi21  g70(.a(new_n87_), .b(new_n86_), .c(x6), .O(new_n88_));
  oai21  g71(.a(new_n88_), .b(new_n85_), .c(x1), .O(new_n89_));
  nor2   g72(.a(x3), .b(x0), .O(new_n90_));
  nor3   g73(.a(new_n90_), .b(x6), .c(new_n52_), .O(new_n91_));
  nor2   g74(.a(new_n90_), .b(new_n52_), .O(new_n92_));
  nor2   g75(.a(new_n92_), .b(new_n19_), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n91_), .c(new_n22_), .O(new_n94_));
  nand3  g77(.a(new_n94_), .b(new_n89_), .c(new_n83_), .O(z4));
  nand2  g78(.a(x3), .b(new_n52_), .O(new_n96_));
  nand2  g79(.a(new_n39_), .b(x2), .O(new_n97_));
  oai21  g80(.a(new_n96_), .b(new_n22_), .c(new_n97_), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n18_), .O(new_n99_));
  inv1   g82(.a(new_n23_), .O(new_n100_));
  aoi21  g83(.a(x3), .b(x1), .c(x2), .O(new_n101_));
  oai21  g84(.a(new_n101_), .b(new_n100_), .c(x0), .O(new_n102_));
  nand3  g85(.a(new_n102_), .b(new_n99_), .c(new_n83_), .O(z5));
  nand2  g86(.a(new_n96_), .b(x1), .O(new_n104_));
  nand3  g87(.a(x3), .b(new_n52_), .c(new_n22_), .O(new_n105_));
  aoi21  g88(.a(new_n105_), .b(new_n104_), .c(new_n80_), .O(z6));
  nand3  g89(.a(new_n97_), .b(new_n96_), .c(new_n83_), .O(z7));
  nand2  g90(.a(new_n83_), .b(x3), .O(z8));
  nand3  g91(.a(new_n49_), .b(x5), .c(x4), .O(new_n109_));
  nand2  g92(.a(new_n109_), .b(new_n83_), .O(z9));
endmodule


