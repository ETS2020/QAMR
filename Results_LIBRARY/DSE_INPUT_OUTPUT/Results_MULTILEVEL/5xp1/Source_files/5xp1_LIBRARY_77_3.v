// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_;
  oai21  g00(.a(x1), .b(x0), .c(x6), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x4), .d(new_n19_), .O(new_n22_));
  oai21  g05(.a(new_n18_), .b(x4), .c(new_n22_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x5), .O(new_n24_));
  inv1   g07(.a(x5), .O(new_n25_));
  aoi22  g08(.a(x6), .b(x2), .c(new_n25_), .d(x4), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z0));
  nor2   g10(.a(x4), .b(x2), .O(new_n28_));
  aoi21  g11(.a(new_n20_), .b(x4), .c(new_n28_), .O(new_n29_));
  nand2  g12(.a(x3), .b(x2), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n20_), .c(x4), .O(new_n31_));
  oai21  g14(.a(new_n29_), .b(x1), .c(new_n31_), .O(new_n32_));
  nor2   g15(.a(x6), .b(x4), .O(new_n33_));
  aoi21  g16(.a(new_n32_), .b(new_n19_), .c(new_n33_), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  inv1   g18(.a(x1), .O(new_n36_));
  inv1   g19(.a(x4), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n19_), .c(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(x6), .c(new_n35_), .O(new_n39_));
  inv1   g22(.a(x3), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n20_), .c(x4), .d(x1), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n19_), .c(new_n39_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  oai21  g27(.a(new_n34_), .b(new_n25_), .c(new_n44_), .O(z1));
  nand2  g28(.a(x5), .b(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n36_), .O(new_n47_));
  oai21  g30(.a(x5), .b(x3), .c(x0), .O(new_n48_));
  aoi21  g31(.a(x5), .b(x3), .c(x0), .O(new_n49_));
  aoi21  g32(.a(new_n48_), .b(new_n35_), .c(new_n49_), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n47_), .c(new_n37_), .O(new_n51_));
  oai21  g34(.a(x3), .b(x2), .c(x0), .O(new_n52_));
  nand3  g35(.a(x5), .b(x3), .c(x2), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x1), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n46_), .c(x4), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n51_), .c(new_n20_), .O(new_n57_));
  nor2   g40(.a(x5), .b(x1), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  nor2   g42(.a(new_n25_), .b(new_n36_), .O(new_n60_));
  aoi21  g43(.a(new_n59_), .b(x0), .c(new_n60_), .O(new_n61_));
  aoi21  g44(.a(x5), .b(x1), .c(x0), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n58_), .c(new_n37_), .O(new_n63_));
  oai21  g46(.a(new_n61_), .b(new_n37_), .c(new_n63_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(x6), .c(new_n35_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n57_), .O(z2));
  nand3  g49(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(x2), .c(x1), .O(new_n69_));
  nand2  g52(.a(new_n41_), .b(x1), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n25_), .c(x0), .O(new_n71_));
  nand3  g54(.a(new_n21_), .b(x5), .c(new_n19_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n71_), .c(new_n69_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n20_), .O(new_n74_));
  nand2  g57(.a(x5), .b(new_n36_), .O(new_n75_));
  nand3  g58(.a(x6), .b(new_n25_), .c(x1), .O(new_n76_));
  aoi21  g59(.a(new_n76_), .b(new_n75_), .c(x0), .O(new_n77_));
  nand2  g60(.a(new_n20_), .b(new_n40_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(x5), .c(x1), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(new_n59_), .c(new_n19_), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n77_), .c(new_n35_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n74_), .O(z3));
  nand3  g65(.a(x6), .b(x3), .c(new_n35_), .O(new_n83_));
  nand3  g66(.a(new_n20_), .b(x2), .c(new_n36_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x0), .O(new_n86_));
  nand2  g69(.a(new_n30_), .b(new_n19_), .O(new_n87_));
  aoi21  g70(.a(new_n87_), .b(new_n41_), .c(new_n36_), .O(new_n88_));
  nor2   g71(.a(new_n30_), .b(x1), .O(new_n89_));
  oai21  g72(.a(new_n89_), .b(new_n88_), .c(new_n20_), .O(new_n90_));
  nand3  g73(.a(x6), .b(new_n35_), .c(new_n36_), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(z4));
  nor2   g75(.a(new_n40_), .b(x2), .O(new_n93_));
  nor2   g76(.a(x3), .b(new_n35_), .O(new_n94_));
  aoi21  g77(.a(new_n93_), .b(x1), .c(new_n94_), .O(new_n95_));
  oai21  g78(.a(new_n40_), .b(new_n19_), .c(new_n20_), .O(new_n96_));
  aoi21  g79(.a(x3), .b(x1), .c(x2), .O(new_n97_));
  aoi22  g80(.a(new_n97_), .b(x0), .c(new_n96_), .d(x2), .O(new_n98_));
  oai21  g81(.a(new_n95_), .b(x0), .c(new_n98_), .O(z5));
  inv1   g82(.a(new_n41_), .O(new_n100_));
  aoi21  g83(.a(new_n20_), .b(x2), .c(new_n100_), .O(new_n101_));
  nand2  g84(.a(new_n93_), .b(new_n36_), .O(new_n102_));
  oai21  g85(.a(new_n101_), .b(new_n36_), .c(new_n102_), .O(z6));
  oai21  g86(.a(x6), .b(new_n40_), .c(x2), .O(new_n104_));
  oai21  g87(.a(new_n40_), .b(x2), .c(new_n104_), .O(z7));
  aoi21  g88(.a(x6), .b(x2), .c(x3), .O(z8));
  nand4  g89(.a(x5), .b(x4), .c(x3), .d(x1), .O(new_n107_));
  nand2  g90(.a(new_n107_), .b(new_n20_), .O(new_n108_));
  nand2  g91(.a(new_n108_), .b(x2), .O(new_n109_));
  nand2  g92(.a(new_n20_), .b(new_n19_), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x5), .c(x4), .O(new_n111_));
  nand2  g94(.a(new_n111_), .b(new_n109_), .O(z9));
endmodule


