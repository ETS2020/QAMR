// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  oai21  g02(.a(x3), .b(x2), .c(x0), .O(new_n20_));
  oai21  g03(.a(new_n19_), .b(x1), .c(new_n18_), .O(new_n22_));
  inv1   g04(.a(x0), .O(new_n23_));
  nand2  g05(.a(x3), .b(x2), .O(new_n24_));
  inv1   g06(.a(new_n24_), .O(new_n25_));
  nand2  g07(.a(new_n25_), .b(x1), .O(new_n26_));
  nand4  g08(.a(new_n26_), .b(new_n19_), .c(x5), .d(new_n23_), .O(new_n27_));
  nand2  g09(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  nand2  g10(.a(new_n28_), .b(x4), .O(new_n29_));
  inv1   g11(.a(x4), .O(new_n30_));
  nor2   g12(.a(new_n19_), .b(new_n18_), .O(new_n31_));
  nand3  g13(.a(new_n31_), .b(new_n30_), .c(x1), .O(new_n32_));
  nand2  g14(.a(new_n32_), .b(new_n29_), .O(z0));
  aoi21  g15(.a(new_n19_), .b(new_n23_), .c(new_n30_), .O(new_n34_));
  aoi21  g16(.a(new_n24_), .b(new_n23_), .c(new_n30_), .O(new_n35_));
  oai22  g17(.a(new_n35_), .b(x6), .c(new_n34_), .d(x1), .O(new_n36_));
  nand2  g18(.a(new_n36_), .b(x5), .O(new_n37_));
  aoi21  g19(.a(x4), .b(x1), .c(x0), .O(new_n38_));
  nor2   g20(.a(x3), .b(x2), .O(new_n39_));
  inv1   g21(.a(new_n39_), .O(new_n40_));
  nand4  g22(.a(new_n40_), .b(x4), .c(x1), .d(x0), .O(new_n41_));
  oai21  g23(.a(new_n38_), .b(new_n19_), .c(new_n41_), .O(new_n42_));
  nand2  g24(.a(new_n42_), .b(new_n18_), .O(new_n43_));
  nor2   g25(.a(new_n19_), .b(x1), .O(new_n44_));
  inv1   g26(.a(new_n44_), .O(new_n45_));
  nand3  g27(.a(new_n45_), .b(new_n43_), .c(new_n37_), .O(z1));
  inv1   g28(.a(x1), .O(new_n47_));
  nand2  g29(.a(x5), .b(x0), .O(new_n48_));
  nand2  g30(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g31(.a(x2), .O(new_n50_));
  oai21  g32(.a(x5), .b(x3), .c(x0), .O(new_n51_));
  aoi21  g33(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  aoi21  g34(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  aoi21  g35(.a(new_n53_), .b(new_n49_), .c(new_n30_), .O(new_n54_));
  nand3  g36(.a(x5), .b(x3), .c(x2), .O(new_n55_));
  nand2  g37(.a(new_n55_), .b(new_n20_), .O(new_n56_));
  nand2  g38(.a(new_n56_), .b(x1), .O(new_n57_));
  aoi21  g39(.a(new_n57_), .b(new_n48_), .c(x4), .O(new_n58_));
  oai21  g40(.a(new_n58_), .b(new_n54_), .c(new_n19_), .O(new_n59_));
  oai21  g41(.a(x5), .b(x0), .c(x4), .O(new_n60_));
  nand3  g42(.a(new_n18_), .b(new_n30_), .c(new_n23_), .O(new_n61_));
  nand2  g43(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g44(.a(new_n62_), .b(x6), .c(x1), .O(new_n63_));
  nand2  g45(.a(new_n63_), .b(new_n59_), .O(z2));
  xor2a  g46(.a(x5), .b(x0), .O(new_n65_));
  nand2  g47(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nor3   g48(.a(new_n39_), .b(new_n18_), .c(new_n23_), .O(new_n67_));
  nand4  g49(.a(new_n18_), .b(x3), .c(x2), .d(new_n23_), .O(new_n68_));
  inv1   g50(.a(new_n68_), .O(new_n69_));
  oai21  g51(.a(new_n69_), .b(new_n67_), .c(x1), .O(new_n70_));
  nand3  g52(.a(new_n39_), .b(new_n19_), .c(new_n18_), .O(new_n71_));
  inv1   g53(.a(new_n71_), .O(new_n72_));
  oai21  g54(.a(new_n72_), .b(new_n31_), .c(x0), .O(new_n73_));
  nand3  g55(.a(new_n24_), .b(new_n19_), .c(x5), .O(new_n74_));
  oai21  g56(.a(new_n19_), .b(x5), .c(new_n74_), .O(new_n75_));
  nand2  g57(.a(new_n75_), .b(new_n23_), .O(new_n76_));
  nand4  g58(.a(new_n76_), .b(new_n73_), .c(new_n70_), .d(new_n66_), .O(z3));
  aoi21  g59(.a(new_n24_), .b(new_n20_), .c(new_n19_), .O(new_n78_));
  nand2  g60(.a(new_n24_), .b(new_n23_), .O(new_n79_));
  aoi21  g61(.a(new_n79_), .b(new_n40_), .c(x6), .O(new_n80_));
  oai21  g62(.a(new_n80_), .b(new_n78_), .c(x1), .O(new_n81_));
  inv1   g63(.a(x3), .O(new_n82_));
  nand2  g64(.a(new_n82_), .b(new_n23_), .O(new_n83_));
  nand4  g65(.a(new_n83_), .b(new_n19_), .c(x2), .d(new_n47_), .O(new_n84_));
  nand2  g66(.a(new_n84_), .b(new_n81_), .O(z4));
  nand2  g67(.a(x3), .b(new_n50_), .O(new_n86_));
  nand2  g68(.a(new_n82_), .b(x2), .O(new_n87_));
  oai21  g69(.a(new_n86_), .b(new_n47_), .c(new_n87_), .O(new_n88_));
  nand2  g70(.a(new_n88_), .b(new_n23_), .O(new_n89_));
  oai21  g71(.a(x2), .b(new_n23_), .c(new_n19_), .O(new_n90_));
  nand2  g72(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  oai21  g73(.a(new_n25_), .b(new_n39_), .c(x0), .O(new_n92_));
  nand3  g74(.a(new_n92_), .b(new_n91_), .c(new_n89_), .O(z5));
  nand2  g75(.a(new_n86_), .b(x1), .O(new_n94_));
  aoi21  g76(.a(x3), .b(new_n50_), .c(x6), .O(new_n95_));
  oai21  g77(.a(new_n95_), .b(x1), .c(new_n94_), .O(z6));
  nand3  g78(.a(new_n87_), .b(new_n86_), .c(new_n45_), .O(z7));
  nor2   g79(.a(new_n44_), .b(x3), .O(z8));
  aoi21  g80(.a(x5), .b(x4), .c(new_n47_), .O(new_n99_));
  nand2  g81(.a(new_n26_), .b(new_n23_), .O(new_n100_));
  nand3  g82(.a(new_n100_), .b(x5), .c(x4), .O(new_n101_));
  oai21  g83(.a(new_n99_), .b(new_n19_), .c(new_n101_), .O(z9));
endmodule


