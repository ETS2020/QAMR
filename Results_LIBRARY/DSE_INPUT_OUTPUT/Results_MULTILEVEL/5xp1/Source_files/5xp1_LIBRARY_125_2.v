// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n105_, new_n106_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x6), .O(new_n24_));
  inv1   g07(.a(x0), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand3  g09(.a(x3), .b(x2), .c(x1), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(new_n26_), .c(x4), .d(new_n25_), .O(new_n28_));
  oai21  g11(.a(new_n24_), .b(x4), .c(new_n28_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x5), .O(new_n30_));
  nand2  g13(.a(new_n19_), .b(x4), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n20_), .O(z0));
  inv1   g15(.a(x2), .O(new_n33_));
  inv1   g16(.a(x3), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x1), .c(x0), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  nand4  g20(.a(new_n27_), .b(new_n26_), .c(x5), .d(new_n25_), .O(new_n38_));
  inv1   g21(.a(new_n38_), .O(new_n39_));
  aoi21  g22(.a(new_n37_), .b(new_n19_), .c(new_n39_), .O(new_n40_));
  nand4  g23(.a(new_n24_), .b(x5), .c(new_n18_), .d(new_n25_), .O(new_n41_));
  oai21  g24(.a(new_n40_), .b(new_n18_), .c(new_n41_), .O(z1));
  nand2  g25(.a(x6), .b(x4), .O(new_n43_));
  nand4  g26(.a(new_n26_), .b(new_n18_), .c(x3), .d(x2), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n43_), .c(new_n21_), .O(new_n45_));
  nand3  g28(.a(x4), .b(x3), .c(x2), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n25_), .c(new_n26_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n45_), .c(x5), .O(new_n48_));
  aoi21  g31(.a(new_n33_), .b(new_n21_), .c(new_n25_), .O(new_n49_));
  aoi21  g32(.a(new_n23_), .b(x5), .c(x4), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n49_), .c(x6), .O(new_n51_));
  nand2  g34(.a(x5), .b(x0), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  oai21  g36(.a(x5), .b(x3), .c(x0), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n33_), .O(new_n55_));
  oai21  g38(.a(new_n19_), .b(new_n34_), .c(new_n25_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n26_), .c(x4), .O(new_n58_));
  nand2  g41(.a(new_n18_), .b(x0), .O(new_n59_));
  nand4  g42(.a(new_n59_), .b(new_n58_), .c(new_n51_), .d(new_n48_), .O(z2));
  nand2  g43(.a(new_n26_), .b(new_n21_), .O(new_n61_));
  nand3  g44(.a(new_n19_), .b(x3), .c(new_n25_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n52_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n61_), .c(x2), .O(new_n64_));
  oai21  g47(.a(x6), .b(x3), .c(x5), .O(new_n65_));
  nand3  g48(.a(x6), .b(new_n19_), .c(new_n25_), .O(new_n66_));
  oai21  g49(.a(new_n65_), .b(new_n25_), .c(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(x1), .O(new_n68_));
  oai21  g51(.a(new_n26_), .b(new_n21_), .c(new_n22_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n61_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(x5), .c(new_n25_), .O(new_n71_));
  oai21  g54(.a(new_n26_), .b(new_n33_), .c(new_n21_), .O(new_n72_));
  nand3  g55(.a(new_n26_), .b(new_n34_), .c(new_n33_), .O(new_n73_));
  aoi21  g56(.a(new_n73_), .b(new_n72_), .c(x5), .O(new_n74_));
  oai21  g57(.a(new_n74_), .b(new_n18_), .c(x0), .O(new_n75_));
  nand4  g58(.a(new_n75_), .b(new_n71_), .c(new_n68_), .d(new_n64_), .O(z3));
  nand3  g59(.a(new_n35_), .b(x6), .c(x0), .O(new_n77_));
  aoi21  g60(.a(new_n77_), .b(new_n73_), .c(new_n18_), .O(new_n78_));
  nand2  g61(.a(new_n22_), .b(new_n26_), .O(new_n79_));
  nand3  g62(.a(x6), .b(x3), .c(x2), .O(new_n80_));
  aoi21  g63(.a(new_n80_), .b(new_n79_), .c(x0), .O(new_n81_));
  oai21  g64(.a(new_n81_), .b(new_n78_), .c(x1), .O(new_n82_));
  nand2  g65(.a(x4), .b(x0), .O(new_n83_));
  oai21  g66(.a(new_n34_), .b(x0), .c(new_n83_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n26_), .c(x2), .O(new_n85_));
  aoi21  g68(.a(x3), .b(x2), .c(x0), .O(new_n86_));
  nor2   g69(.a(new_n18_), .b(x2), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n86_), .c(x6), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n21_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n82_), .O(z4));
  nand2  g74(.a(x3), .b(new_n33_), .O(new_n92_));
  nand2  g75(.a(new_n34_), .b(x2), .O(new_n93_));
  oai21  g76(.a(new_n92_), .b(new_n21_), .c(new_n93_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n25_), .O(new_n95_));
  aoi21  g78(.a(x3), .b(x1), .c(x2), .O(new_n96_));
  nand2  g79(.a(new_n22_), .b(x4), .O(new_n97_));
  oai21  g80(.a(new_n97_), .b(new_n96_), .c(x0), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n95_), .O(z5));
  nand2  g82(.a(new_n92_), .b(x1), .O(new_n100_));
  nand3  g83(.a(x3), .b(new_n33_), .c(new_n21_), .O(new_n101_));
  nand3  g84(.a(new_n101_), .b(new_n100_), .c(new_n59_), .O(z6));
  nand3  g85(.a(new_n93_), .b(new_n92_), .c(new_n59_), .O(z7));
  nand2  g86(.a(new_n59_), .b(x3), .O(z8));
  nand3  g87(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n105_));
  nand3  g88(.a(new_n105_), .b(x5), .c(x4), .O(new_n106_));
  inv1   g89(.a(new_n106_), .O(z9));
endmodule


