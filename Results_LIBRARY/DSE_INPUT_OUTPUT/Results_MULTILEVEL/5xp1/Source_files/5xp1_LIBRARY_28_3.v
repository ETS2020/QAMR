// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g000(.a(x5), .O(new_n18_));
  inv1   g001(.a(x3), .O(new_n19_));
  oai21  g002(.a(x1), .b(x0), .c(x6), .O(new_n20_));
  inv1   g003(.a(x0), .O(new_n21_));
  inv1   g004(.a(x4), .O(new_n22_));
  nor2   g005(.a(x6), .b(new_n22_), .O(new_n23_));
  nand2  g006(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  oai21  g007(.a(new_n20_), .b(x4), .c(new_n24_), .O(new_n25_));
  inv1   g008(.a(x6), .O(new_n26_));
  nand2  g009(.a(x2), .b(x1), .O(new_n27_));
  nand4  g010(.a(new_n27_), .b(new_n26_), .c(x4), .d(new_n21_), .O(new_n28_));
  inv1   g011(.a(new_n28_), .O(new_n29_));
  aoi21  g012(.a(new_n25_), .b(new_n19_), .c(new_n29_), .O(new_n30_));
  nand2  g013(.a(x6), .b(x3), .O(new_n31_));
  nand3  g014(.a(new_n31_), .b(new_n18_), .c(x4), .O(new_n32_));
  oai21  g015(.a(new_n30_), .b(new_n18_), .c(new_n32_), .O(z0));
  inv1   g016(.a(x1), .O(new_n34_));
  nor2   g017(.a(x4), .b(x3), .O(new_n35_));
  oai21  g018(.a(new_n35_), .b(new_n23_), .c(new_n34_), .O(new_n36_));
  nand2  g019(.a(x3), .b(x2), .O(new_n37_));
  nand3  g020(.a(new_n37_), .b(new_n26_), .c(x4), .O(new_n38_));
  aoi21  g021(.a(new_n38_), .b(new_n36_), .c(x0), .O(new_n39_));
  nor2   g022(.a(x6), .b(x4), .O(new_n40_));
  oai21  g023(.a(new_n40_), .b(new_n39_), .c(x5), .O(new_n41_));
  inv1   g024(.a(x2), .O(new_n42_));
  nand2  g025(.a(x6), .b(new_n19_), .O(new_n43_));
  nand3  g026(.a(new_n26_), .b(x4), .c(x1), .O(new_n44_));
  aoi21  g027(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nand3  g028(.a(new_n26_), .b(x4), .c(x3), .O(new_n46_));
  aoi21  g029(.a(new_n46_), .b(new_n43_), .c(new_n34_), .O(new_n47_));
  oai21  g030(.a(new_n47_), .b(new_n45_), .c(x0), .O(new_n48_));
  nand3  g031(.a(x6), .b(x4), .c(new_n19_), .O(new_n49_));
  nand2  g032(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g033(.a(new_n50_), .b(new_n18_), .O(new_n51_));
  nand2  g034(.a(new_n51_), .b(new_n41_), .O(z1));
  nand2  g035(.a(x5), .b(x0), .O(new_n53_));
  nand2  g036(.a(new_n53_), .b(new_n34_), .O(new_n54_));
  oai21  g037(.a(x5), .b(x3), .c(x0), .O(new_n55_));
  aoi21  g038(.a(x5), .b(x3), .c(x0), .O(new_n56_));
  aoi21  g039(.a(new_n55_), .b(new_n42_), .c(new_n56_), .O(new_n57_));
  aoi21  g040(.a(new_n57_), .b(new_n54_), .c(new_n22_), .O(new_n58_));
  oai21  g041(.a(x3), .b(x2), .c(x0), .O(new_n59_));
  nand3  g042(.a(x5), .b(x3), .c(x2), .O(new_n60_));
  nand2  g043(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g044(.a(new_n61_), .b(x1), .O(new_n62_));
  aoi21  g045(.a(new_n62_), .b(new_n53_), .c(x4), .O(new_n63_));
  oai21  g046(.a(new_n63_), .b(new_n58_), .c(new_n26_), .O(new_n64_));
  aoi21  g047(.a(x5), .b(x1), .c(x0), .O(new_n65_));
  nor3   g048(.a(x5), .b(x2), .c(x1), .O(new_n66_));
  oai21  g049(.a(new_n66_), .b(new_n65_), .c(new_n22_), .O(new_n67_));
  aoi21  g050(.a(new_n18_), .b(new_n21_), .c(new_n34_), .O(new_n68_));
  aoi21  g051(.a(new_n18_), .b(new_n42_), .c(new_n21_), .O(new_n69_));
  oai21  g052(.a(new_n69_), .b(new_n68_), .c(x4), .O(new_n70_));
  nand2  g053(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  nand3  g054(.a(new_n71_), .b(x6), .c(new_n19_), .O(new_n72_));
  nand2  g055(.a(new_n72_), .b(new_n64_), .O(z2));
  oai21  g056(.a(x3), .b(x2), .c(x1), .O(new_n74_));
  nand3  g057(.a(new_n74_), .b(new_n18_), .c(x0), .O(new_n75_));
  nand3  g058(.a(new_n37_), .b(x5), .c(new_n21_), .O(new_n76_));
  nand2  g059(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g060(.a(new_n77_), .b(new_n26_), .O(new_n78_));
  nand4  g061(.a(x6), .b(new_n18_), .c(new_n42_), .d(x0), .O(new_n79_));
  oai21  g062(.a(new_n18_), .b(x0), .c(new_n79_), .O(new_n80_));
  nand2  g063(.a(new_n80_), .b(new_n34_), .O(new_n81_));
  nand2  g064(.a(new_n37_), .b(new_n26_), .O(new_n82_));
  nand3  g065(.a(new_n82_), .b(new_n18_), .c(new_n21_), .O(new_n83_));
  nand3  g066(.a(new_n26_), .b(new_n19_), .c(new_n42_), .O(new_n84_));
  nand3  g067(.a(new_n84_), .b(x5), .c(x0), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g069(.a(new_n86_), .b(x1), .O(new_n87_));
  nand3  g070(.a(x5), .b(x2), .c(x0), .O(new_n88_));
  nand2  g071(.a(new_n88_), .b(new_n19_), .O(new_n89_));
  nand2  g072(.a(new_n89_), .b(x6), .O(new_n90_));
  nand4  g073(.a(new_n90_), .b(new_n87_), .c(new_n81_), .d(new_n78_), .O(z3));
  xor2a  g074(.a(x6), .b(x1), .O(new_n92_));
  oai22  g075(.a(new_n92_), .b(new_n21_), .c(new_n19_), .d(x1), .O(new_n93_));
  nand2  g076(.a(new_n93_), .b(x2), .O(new_n94_));
  nand3  g077(.a(new_n26_), .b(new_n19_), .c(x1), .O(new_n95_));
  oai21  g078(.a(new_n26_), .b(x1), .c(new_n95_), .O(new_n96_));
  oai21  g079(.a(new_n42_), .b(new_n21_), .c(new_n96_), .O(new_n97_));
  nor2   g080(.a(new_n34_), .b(x0), .O(new_n98_));
  nor2   g081(.a(x6), .b(x2), .O(new_n99_));
  aoi22  g082(.a(new_n99_), .b(new_n98_), .c(x6), .d(x3), .O(new_n100_));
  nand3  g083(.a(new_n100_), .b(new_n97_), .c(new_n94_), .O(z4));
  xnor2a g084(.a(x3), .b(x0), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(x2), .O(new_n103_));
  oai21  g086(.a(x2), .b(new_n34_), .c(new_n26_), .O(new_n104_));
  nand3  g087(.a(new_n104_), .b(x3), .c(new_n21_), .O(new_n105_));
  nor2   g088(.a(x6), .b(new_n19_), .O(new_n106_));
  nand2  g089(.a(new_n106_), .b(x1), .O(new_n107_));
  nand3  g090(.a(new_n107_), .b(new_n42_), .c(x0), .O(new_n108_));
  nand3  g091(.a(new_n108_), .b(new_n105_), .c(new_n103_), .O(z5));
  aoi21  g092(.a(new_n26_), .b(x2), .c(new_n19_), .O(new_n110_));
  nand3  g093(.a(new_n106_), .b(new_n42_), .c(new_n34_), .O(new_n111_));
  oai21  g094(.a(new_n110_), .b(new_n34_), .c(new_n111_), .O(z6));
  inv1   g095(.a(new_n106_), .O(z8));
  nand2  g096(.a(new_n19_), .b(x2), .O(new_n114_));
  oai21  g097(.a(z8), .b(x2), .c(new_n114_), .O(z7));
  nand2  g098(.a(x5), .b(x4), .O(new_n116_));
  oai21  g099(.a(new_n116_), .b(new_n27_), .c(new_n26_), .O(new_n117_));
  nand2  g100(.a(new_n117_), .b(x3), .O(new_n118_));
  nand2  g101(.a(new_n26_), .b(new_n21_), .O(new_n119_));
  nand3  g102(.a(new_n119_), .b(x5), .c(x4), .O(new_n120_));
  nand2  g103(.a(new_n120_), .b(new_n118_), .O(z9));
endmodule


