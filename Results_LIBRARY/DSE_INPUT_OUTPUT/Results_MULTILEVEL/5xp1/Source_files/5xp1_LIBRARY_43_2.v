// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n95_, new_n97_, new_n100_, new_n101_, new_n102_,
    new_n103_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand2  g02(.a(x3), .b(x1), .O(new_n20_));
  nand4  g03(.a(new_n20_), .b(new_n19_), .c(x4), .d(new_n18_), .O(new_n21_));
  inv1   g04(.a(x4), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n18_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(x6), .c(new_n22_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(x5), .O(new_n28_));
  inv1   g11(.a(x5), .O(new_n29_));
  nor2   g12(.a(x2), .b(new_n23_), .O(new_n30_));
  aoi21  g13(.a(new_n29_), .b(x4), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n28_), .O(z0));
  inv1   g15(.a(new_n30_), .O(new_n33_));
  nand2  g16(.a(x4), .b(x1), .O(new_n34_));
  nand2  g17(.a(x6), .b(x2), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n34_), .c(new_n18_), .O(new_n36_));
  nand2  g19(.a(x6), .b(x4), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n36_), .c(new_n29_), .O(new_n39_));
  nor2   g22(.a(x4), .b(x3), .O(new_n40_));
  nor2   g23(.a(x6), .b(new_n22_), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n40_), .c(new_n23_), .O(new_n42_));
  inv1   g25(.a(x3), .O(new_n43_));
  nor2   g26(.a(x4), .b(x2), .O(new_n44_));
  aoi21  g27(.a(new_n41_), .b(new_n43_), .c(new_n44_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n42_), .c(x0), .O(new_n46_));
  nor2   g29(.a(x6), .b(x4), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n46_), .c(x5), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n39_), .c(new_n33_), .O(z1));
  inv1   g32(.a(new_n47_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(x5), .c(x0), .O(new_n52_));
  nand2  g35(.a(x5), .b(x0), .O(new_n53_));
  inv1   g36(.a(x2), .O(new_n54_));
  nand3  g37(.a(x6), .b(new_n22_), .c(new_n54_), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n41_), .c(new_n53_), .O(new_n57_));
  nand4  g40(.a(x6), .b(new_n22_), .c(new_n43_), .d(new_n18_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n57_), .c(new_n52_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n23_), .O(new_n60_));
  oai21  g43(.a(new_n50_), .b(new_n23_), .c(new_n37_), .O(new_n61_));
  nand2  g44(.a(x5), .b(x3), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n18_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand3  g47(.a(new_n62_), .b(new_n19_), .c(x4), .O(new_n65_));
  nand3  g48(.a(x6), .b(new_n29_), .c(new_n22_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n18_), .O(new_n68_));
  nand4  g51(.a(x6), .b(x5), .c(x4), .d(x1), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x2), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n60_), .O(z2));
  nand3  g55(.a(new_n29_), .b(x3), .c(new_n18_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  nand2  g57(.a(new_n35_), .b(new_n23_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai21  g59(.a(x6), .b(x1), .c(x2), .O(new_n77_));
  xor2a  g60(.a(x5), .b(x0), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g62(.a(new_n19_), .b(x0), .c(x2), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n29_), .c(x1), .O(new_n81_));
  nand2  g64(.a(x6), .b(x1), .O(new_n82_));
  nand4  g65(.a(new_n82_), .b(x5), .c(new_n43_), .d(new_n18_), .O(new_n83_));
  nand4  g66(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n76_), .O(z3));
  nand3  g67(.a(new_n19_), .b(x2), .c(new_n23_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  oai21  g69(.a(x3), .b(x0), .c(new_n86_), .O(new_n87_));
  xor2a  g70(.a(x6), .b(x1), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n43_), .c(new_n18_), .O(new_n89_));
  oai21  g72(.a(x6), .b(x1), .c(new_n54_), .O(new_n90_));
  nand3  g73(.a(new_n90_), .b(new_n89_), .c(new_n87_), .O(z4));
  nor2   g74(.a(x3), .b(new_n54_), .O(new_n92_));
  aoi21  g75(.a(new_n92_), .b(new_n18_), .c(new_n30_), .O(new_n93_));
  oai21  g76(.a(new_n92_), .b(new_n18_), .c(new_n93_), .O(z5));
  nand3  g77(.a(x3), .b(new_n54_), .c(new_n23_), .O(new_n95_));
  oai21  g78(.a(new_n54_), .b(new_n23_), .c(new_n95_), .O(z6));
  oai21  g79(.a(x3), .b(x1), .c(new_n54_), .O(new_n97_));
  oai21  g80(.a(x3), .b(new_n54_), .c(new_n97_), .O(z7));
  nor2   g81(.a(new_n30_), .b(x3), .O(z8));
  aoi21  g82(.a(new_n19_), .b(new_n18_), .c(new_n30_), .O(new_n100_));
  nand3  g83(.a(x3), .b(x2), .c(x1), .O(new_n101_));
  inv1   g84(.a(new_n101_), .O(new_n102_));
  oai21  g85(.a(new_n102_), .b(new_n100_), .c(x5), .O(new_n103_));
  nor2   g86(.a(new_n103_), .b(new_n22_), .O(z9));
endmodule


