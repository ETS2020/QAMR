// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n107_;
  inv1   g00(.a(x2), .O(new_n18_));
  oai21  g01(.a(x6), .b(x0), .c(x5), .O(new_n19_));
  nand3  g02(.a(new_n19_), .b(x4), .c(new_n18_), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(x6), .c(x5), .d(new_n21_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n20_), .O(z0));
  inv1   g10(.a(x5), .O(new_n28_));
  inv1   g11(.a(x6), .O(new_n29_));
  nand2  g12(.a(x4), .b(x3), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n29_), .c(new_n23_), .O(new_n31_));
  nor2   g14(.a(new_n29_), .b(new_n18_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(x0), .O(new_n33_));
  nand2  g16(.a(x6), .b(x4), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n28_), .O(new_n36_));
  nand3  g19(.a(new_n24_), .b(new_n21_), .c(new_n23_), .O(new_n37_));
  nand3  g20(.a(new_n29_), .b(x4), .c(new_n18_), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(new_n37_), .c(x0), .O(new_n39_));
  nor2   g22(.a(x6), .b(x4), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n39_), .c(x5), .O(new_n41_));
  nand2  g24(.a(x4), .b(x2), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n41_), .c(new_n36_), .O(z1));
  oai21  g26(.a(x5), .b(x1), .c(x0), .O(new_n44_));
  nand2  g27(.a(x5), .b(x1), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n21_), .O(new_n46_));
  aoi21  g29(.a(x3), .b(x2), .c(x0), .O(new_n47_));
  nor2   g30(.a(x5), .b(x2), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n47_), .c(new_n23_), .O(new_n49_));
  nand2  g32(.a(new_n28_), .b(new_n22_), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n49_), .c(x4), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n46_), .c(x6), .O(new_n52_));
  inv1   g35(.a(x3), .O(new_n53_));
  oai21  g36(.a(x4), .b(new_n53_), .c(new_n18_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x0), .O(new_n55_));
  nand3  g38(.a(x5), .b(x3), .c(x2), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(new_n23_), .O(new_n57_));
  nand2  g40(.a(x3), .b(x1), .O(new_n58_));
  aoi21  g41(.a(new_n58_), .b(new_n28_), .c(new_n22_), .O(new_n59_));
  nand3  g42(.a(x5), .b(new_n21_), .c(x0), .O(new_n60_));
  oai21  g43(.a(new_n59_), .b(new_n21_), .c(new_n60_), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n57_), .c(new_n29_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n52_), .c(new_n42_), .O(z2));
  nand2  g46(.a(x5), .b(x0), .O(new_n64_));
  nand3  g47(.a(new_n28_), .b(x3), .c(new_n22_), .O(new_n65_));
  aoi22  g48(.a(new_n65_), .b(new_n64_), .c(new_n29_), .d(new_n23_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(x4), .c(x2), .O(new_n67_));
  oai21  g50(.a(x6), .b(x3), .c(x5), .O(new_n68_));
  nand3  g51(.a(x6), .b(new_n28_), .c(new_n22_), .O(new_n69_));
  oai21  g52(.a(new_n68_), .b(new_n22_), .c(new_n69_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x1), .O(new_n71_));
  xnor2a g54(.a(x5), .b(x0), .O(new_n72_));
  nand3  g55(.a(x5), .b(new_n53_), .c(new_n22_), .O(new_n73_));
  oai21  g56(.a(new_n72_), .b(new_n32_), .c(new_n73_), .O(new_n74_));
  nand3  g57(.a(new_n24_), .b(x5), .c(new_n22_), .O(new_n75_));
  nand4  g58(.a(new_n28_), .b(new_n53_), .c(new_n18_), .d(x0), .O(new_n76_));
  aoi21  g59(.a(new_n76_), .b(new_n75_), .c(x6), .O(new_n77_));
  aoi21  g60(.a(new_n74_), .b(new_n23_), .c(new_n77_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n71_), .c(new_n67_), .O(z3));
  xnor2a g62(.a(x6), .b(x1), .O(new_n80_));
  nand2  g63(.a(new_n53_), .b(new_n22_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n80_), .c(x2), .O(new_n82_));
  xor2a  g65(.a(x6), .b(x1), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n53_), .c(new_n22_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n21_), .O(new_n86_));
  nand2  g69(.a(x3), .b(x0), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n29_), .c(x1), .O(new_n88_));
  inv1   g71(.a(new_n88_), .O(new_n89_));
  aoi21  g72(.a(new_n87_), .b(x1), .c(new_n29_), .O(new_n90_));
  oai21  g73(.a(new_n90_), .b(new_n89_), .c(new_n18_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n86_), .O(z4));
  nor2   g75(.a(new_n53_), .b(x2), .O(new_n93_));
  nor2   g76(.a(x3), .b(new_n18_), .O(new_n94_));
  aoi21  g77(.a(new_n93_), .b(x1), .c(new_n94_), .O(new_n95_));
  nand2  g78(.a(new_n87_), .b(new_n21_), .O(new_n96_));
  aoi21  g79(.a(x3), .b(x1), .c(x2), .O(new_n97_));
  aoi22  g80(.a(new_n97_), .b(x0), .c(new_n96_), .d(x2), .O(new_n98_));
  oai21  g81(.a(new_n95_), .b(x0), .c(new_n98_), .O(z5));
  nor2   g82(.a(x3), .b(x2), .O(new_n100_));
  aoi21  g83(.a(new_n21_), .b(x2), .c(new_n100_), .O(new_n101_));
  nand2  g84(.a(new_n93_), .b(new_n23_), .O(new_n102_));
  oai21  g85(.a(new_n101_), .b(new_n23_), .c(new_n102_), .O(z6));
  oai21  g86(.a(x4), .b(new_n53_), .c(x2), .O(new_n104_));
  oai21  g87(.a(new_n53_), .b(x2), .c(new_n104_), .O(z7));
  nand2  g88(.a(new_n42_), .b(x3), .O(z8));
  oai21  g89(.a(x6), .b(x0), .c(x5), .O(new_n107_));
  aoi21  g90(.a(new_n107_), .b(new_n18_), .c(new_n21_), .O(z9));
endmodule


