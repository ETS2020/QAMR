// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x2), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(x1), .c(x3), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n19_), .c(x4), .d(new_n18_), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  nor2   g06(.a(x1), .b(x0), .O(new_n24_));
  nand2  g07(.a(x3), .b(x2), .O(z8));
  oai21  g08(.a(new_n24_), .b(x3), .c(z8), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x6), .c(new_n23_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x5), .O(new_n29_));
  inv1   g12(.a(x5), .O(new_n30_));
  nand2  g13(.a(x3), .b(new_n20_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(x4), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(z0));
  oai22  g16(.a(new_n19_), .b(x3), .c(new_n23_), .d(new_n20_), .O(new_n34_));
  nor2   g17(.a(new_n19_), .b(new_n20_), .O(new_n35_));
  aoi21  g18(.a(new_n34_), .b(x1), .c(new_n35_), .O(new_n36_));
  nand3  g19(.a(new_n31_), .b(x6), .c(x4), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n18_), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  inv1   g22(.a(x3), .O(new_n40_));
  nand2  g23(.a(new_n23_), .b(new_n40_), .O(new_n41_));
  nand3  g24(.a(new_n19_), .b(x4), .c(x2), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(x1), .O(new_n43_));
  nand3  g26(.a(new_n19_), .b(x4), .c(new_n40_), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(new_n18_), .O(new_n46_));
  nand3  g29(.a(new_n31_), .b(new_n19_), .c(new_n23_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x5), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n39_), .O(z1));
  nor2   g33(.a(x5), .b(x1), .O(new_n51_));
  nand2  g34(.a(x5), .b(x1), .O(new_n52_));
  oai21  g35(.a(new_n51_), .b(new_n18_), .c(new_n52_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(x4), .O(new_n54_));
  aoi21  g37(.a(x5), .b(x1), .c(x0), .O(new_n55_));
  nor3   g38(.a(x5), .b(x2), .c(x1), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n55_), .c(new_n23_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n54_), .c(new_n19_), .O(new_n58_));
  nand2  g41(.a(new_n30_), .b(new_n20_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x0), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x4), .O(new_n61_));
  nand3  g44(.a(x5), .b(new_n23_), .c(x0), .O(new_n62_));
  aoi21  g45(.a(new_n62_), .b(new_n61_), .c(x6), .O(new_n63_));
  oai21  g46(.a(new_n63_), .b(new_n58_), .c(new_n40_), .O(new_n64_));
  oai21  g47(.a(new_n30_), .b(new_n40_), .c(new_n18_), .O(new_n65_));
  nand3  g48(.a(new_n19_), .b(new_n23_), .c(x1), .O(new_n66_));
  oai21  g49(.a(new_n19_), .b(new_n23_), .c(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  inv1   g51(.a(new_n62_), .O(new_n69_));
  inv1   g52(.a(x1), .O(new_n70_));
  nand2  g53(.a(new_n30_), .b(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n52_), .b(new_n18_), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n71_), .c(new_n23_), .O(new_n73_));
  oai21  g56(.a(new_n73_), .b(new_n69_), .c(new_n19_), .O(new_n74_));
  nand4  g57(.a(x6), .b(new_n30_), .c(new_n23_), .d(new_n18_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n74_), .c(new_n68_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(x2), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n64_), .O(z2));
  nand2  g61(.a(new_n19_), .b(new_n70_), .O(new_n79_));
  nand3  g62(.a(x5), .b(x2), .c(x0), .O(new_n80_));
  nand3  g63(.a(new_n30_), .b(x3), .c(new_n18_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  xnor2a g66(.a(x5), .b(x0), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(x6), .c(x1), .O(new_n85_));
  nand2  g68(.a(x6), .b(x1), .O(new_n86_));
  nand3  g69(.a(x5), .b(new_n40_), .c(new_n18_), .O(new_n87_));
  oai21  g70(.a(new_n59_), .b(new_n18_), .c(new_n87_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  xor2a  g72(.a(x5), .b(x0), .O(new_n90_));
  nand3  g73(.a(new_n90_), .b(new_n19_), .c(new_n70_), .O(new_n91_));
  and2   g74(.a(new_n91_), .b(new_n31_), .O(new_n92_));
  nand4  g75(.a(new_n92_), .b(new_n89_), .c(new_n85_), .d(new_n83_), .O(z3));
  nand2  g76(.a(new_n86_), .b(new_n79_), .O(new_n94_));
  nand2  g77(.a(new_n40_), .b(new_n18_), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(new_n94_), .c(x2), .O(new_n96_));
  xor2a  g79(.a(x6), .b(x1), .O(new_n97_));
  nand2  g80(.a(x2), .b(x0), .O(new_n98_));
  nand3  g81(.a(new_n98_), .b(new_n97_), .c(new_n40_), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(new_n96_), .O(z4));
  xor2a  g83(.a(x3), .b(x2), .O(new_n101_));
  nand3  g84(.a(new_n40_), .b(x2), .c(new_n18_), .O(new_n102_));
  oai21  g85(.a(new_n101_), .b(new_n18_), .c(new_n102_), .O(z5));
  nand2  g86(.a(new_n31_), .b(new_n70_), .O(z6));
  nand2  g87(.a(new_n40_), .b(x2), .O(new_n105_));
  nand2  g88(.a(new_n105_), .b(new_n31_), .O(z7));
  nand3  g89(.a(x5), .b(x4), .c(x1), .O(new_n107_));
  nand2  g90(.a(new_n107_), .b(x2), .O(new_n108_));
  nand2  g91(.a(new_n108_), .b(x3), .O(new_n109_));
  nand2  g92(.a(new_n19_), .b(new_n18_), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x5), .c(x4), .O(new_n111_));
  nand2  g94(.a(new_n111_), .b(new_n109_), .O(z9));
endmodule


