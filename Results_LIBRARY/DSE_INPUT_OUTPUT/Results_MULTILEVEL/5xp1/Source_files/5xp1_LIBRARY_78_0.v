// Benchmark "FAU" written by ABC on Thu Aug 13 17:55:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n107_, new_n108_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x4), .d(new_n19_), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n19_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(x6), .c(new_n18_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n22_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(x5), .O(new_n28_));
  oai21  g11(.a(x5), .b(new_n18_), .c(new_n28_), .O(z0));
  inv1   g12(.a(x5), .O(new_n30_));
  oai21  g13(.a(new_n18_), .b(x1), .c(x2), .O(new_n31_));
  oai21  g14(.a(new_n18_), .b(x3), .c(x1), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n31_), .c(new_n19_), .O(new_n33_));
  nor2   g16(.a(new_n20_), .b(new_n18_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n33_), .c(new_n30_), .O(new_n35_));
  inv1   g18(.a(new_n24_), .O(new_n36_));
  nor2   g19(.a(x4), .b(x1), .O(new_n37_));
  aoi21  g20(.a(new_n20_), .b(x4), .c(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n20_), .b(x4), .c(new_n23_), .O(new_n39_));
  oai21  g22(.a(new_n38_), .b(new_n36_), .c(new_n39_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x5), .c(new_n19_), .O(new_n41_));
  nor2   g24(.a(x6), .b(x4), .O(new_n42_));
  inv1   g25(.a(new_n42_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(new_n35_), .O(z1));
  oai21  g27(.a(x5), .b(x0), .c(x1), .O(new_n45_));
  oai21  g28(.a(x5), .b(x2), .c(x0), .O(new_n46_));
  nand3  g29(.a(x5), .b(x3), .c(x2), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  and2   g31(.a(new_n48_), .b(x6), .O(new_n49_));
  nand2  g32(.a(x5), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n23_), .O(new_n51_));
  inv1   g34(.a(x2), .O(new_n52_));
  oai21  g35(.a(x5), .b(x3), .c(x0), .O(new_n53_));
  aoi21  g36(.a(x5), .b(x3), .c(x0), .O(new_n54_));
  aoi21  g37(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n51_), .c(x6), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n49_), .c(x4), .O(new_n57_));
  nor2   g40(.a(x5), .b(x2), .O(new_n58_));
  aoi21  g41(.a(x3), .b(x2), .c(x0), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n58_), .c(new_n23_), .O(new_n60_));
  oai21  g43(.a(x5), .b(x0), .c(new_n60_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(x6), .c(new_n18_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n57_), .O(z2));
  nand2  g46(.a(new_n20_), .b(new_n23_), .O(new_n64_));
  nand3  g47(.a(new_n30_), .b(x3), .c(new_n19_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n64_), .c(x2), .O(new_n67_));
  oai21  g50(.a(x6), .b(x3), .c(x5), .O(new_n68_));
  nand3  g51(.a(x6), .b(new_n30_), .c(new_n19_), .O(new_n69_));
  oai21  g52(.a(new_n68_), .b(new_n19_), .c(new_n69_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(x1), .O(new_n71_));
  nor2   g54(.a(new_n20_), .b(new_n52_), .O(new_n72_));
  xnor2a g55(.a(x5), .b(x0), .O(new_n73_));
  inv1   g56(.a(x3), .O(new_n74_));
  nand3  g57(.a(x5), .b(new_n74_), .c(new_n19_), .O(new_n75_));
  oai21  g58(.a(new_n73_), .b(new_n72_), .c(new_n75_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n23_), .O(new_n77_));
  nand3  g60(.a(new_n24_), .b(x5), .c(new_n19_), .O(new_n78_));
  nand4  g61(.a(new_n30_), .b(new_n74_), .c(new_n52_), .d(x0), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n78_), .c(x4), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n20_), .O(new_n81_));
  nand4  g64(.a(new_n81_), .b(new_n77_), .c(new_n71_), .d(new_n67_), .O(z3));
  oai21  g65(.a(x3), .b(x2), .c(x0), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n24_), .c(new_n20_), .O(new_n84_));
  inv1   g67(.a(new_n59_), .O(new_n85_));
  nand2  g68(.a(new_n74_), .b(new_n52_), .O(new_n86_));
  aoi21  g69(.a(new_n86_), .b(new_n85_), .c(x6), .O(new_n87_));
  aoi21  g70(.a(new_n87_), .b(x4), .c(new_n84_), .O(new_n88_));
  nand2  g71(.a(new_n74_), .b(new_n19_), .O(new_n89_));
  nand4  g72(.a(new_n89_), .b(new_n20_), .c(x4), .d(x2), .O(new_n90_));
  inv1   g73(.a(new_n90_), .O(new_n91_));
  aoi21  g74(.a(new_n89_), .b(x2), .c(new_n20_), .O(new_n92_));
  oai21  g75(.a(new_n92_), .b(new_n91_), .c(new_n23_), .O(new_n93_));
  oai21  g76(.a(new_n88_), .b(new_n23_), .c(new_n93_), .O(z4));
  nand2  g77(.a(x3), .b(new_n52_), .O(new_n95_));
  nand2  g78(.a(new_n74_), .b(x2), .O(new_n96_));
  oai21  g79(.a(new_n95_), .b(new_n23_), .c(new_n96_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n19_), .O(new_n98_));
  aoi21  g81(.a(x3), .b(x1), .c(x2), .O(new_n99_));
  oai21  g82(.a(new_n99_), .b(new_n36_), .c(x0), .O(new_n100_));
  aoi21  g83(.a(new_n100_), .b(new_n98_), .c(new_n42_), .O(z5));
  nand2  g84(.a(new_n95_), .b(x1), .O(new_n102_));
  nand3  g85(.a(x3), .b(new_n52_), .c(new_n23_), .O(new_n103_));
  aoi21  g86(.a(new_n103_), .b(new_n102_), .c(new_n42_), .O(z6));
  aoi21  g87(.a(new_n96_), .b(new_n95_), .c(new_n42_), .O(z7));
  nand2  g88(.a(new_n43_), .b(x3), .O(z8));
  nand3  g89(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n107_));
  nand3  g90(.a(new_n107_), .b(x5), .c(x4), .O(new_n108_));
  inv1   g91(.a(new_n108_), .O(z9));
endmodule


