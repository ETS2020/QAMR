// Benchmark "FAU" written by ABC on Thu Aug 13 17:54:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n109_,
    new_n110_, new_n111_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(x6), .c(new_n18_), .O(new_n22_));
  inv1   g05(.a(x6), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(x4), .c(new_n19_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n22_), .c(x1), .O(new_n25_));
  nand4  g08(.a(new_n20_), .b(new_n23_), .c(x4), .d(new_n19_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(x5), .O(new_n28_));
  inv1   g11(.a(x5), .O(new_n29_));
  nand2  g12(.a(x6), .b(x1), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n29_), .c(x4), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n28_), .O(z0));
  inv1   g15(.a(x2), .O(new_n33_));
  inv1   g16(.a(x3), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n29_), .c(x1), .d(x0), .O(new_n36_));
  nand3  g19(.a(x3), .b(x2), .c(x1), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(x5), .c(new_n19_), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(new_n36_), .c(x6), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand3  g23(.a(x6), .b(new_n29_), .c(new_n40_), .O(new_n41_));
  inv1   g24(.a(new_n41_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n39_), .c(x4), .O(new_n43_));
  nand4  g26(.a(new_n20_), .b(x5), .c(new_n18_), .d(new_n19_), .O(new_n44_));
  nand4  g27(.a(x6), .b(new_n29_), .c(x2), .d(x0), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor2   g29(.a(x6), .b(new_n29_), .O(new_n47_));
  aoi22  g30(.a(new_n47_), .b(new_n18_), .c(new_n46_), .d(new_n40_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n43_), .O(z1));
  nand2  g32(.a(x5), .b(x0), .O(new_n50_));
  nand3  g33(.a(x6), .b(new_n18_), .c(new_n33_), .O(new_n51_));
  nand3  g34(.a(new_n23_), .b(x4), .c(new_n40_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  oai21  g37(.a(x5), .b(x2), .c(x0), .O(new_n55_));
  nand3  g38(.a(x5), .b(x3), .c(x2), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(new_n23_), .O(new_n57_));
  oai21  g40(.a(x5), .b(x3), .c(x0), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n33_), .O(new_n59_));
  nand2  g42(.a(x5), .b(x3), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n19_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n59_), .c(x6), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n57_), .c(x4), .O(new_n63_));
  nand3  g46(.a(new_n60_), .b(x6), .c(new_n19_), .O(new_n64_));
  nor2   g47(.a(x3), .b(x2), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n19_), .c(new_n56_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x1), .O(new_n67_));
  nand2  g50(.a(new_n47_), .b(x0), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n18_), .O(new_n70_));
  nand2  g53(.a(x6), .b(x1), .O(new_n71_));
  nand4  g54(.a(new_n71_), .b(new_n70_), .c(new_n63_), .d(new_n54_), .O(z2));
  nand2  g55(.a(new_n23_), .b(new_n40_), .O(new_n73_));
  nand3  g56(.a(new_n29_), .b(x3), .c(new_n19_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n50_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n73_), .c(x2), .O(new_n76_));
  oai21  g59(.a(new_n23_), .b(new_n40_), .c(new_n20_), .O(new_n77_));
  aoi21  g60(.a(new_n77_), .b(new_n73_), .c(x0), .O(new_n78_));
  nand3  g61(.a(x3), .b(x1), .c(x0), .O(new_n79_));
  inv1   g62(.a(new_n79_), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n78_), .c(x5), .O(new_n81_));
  nand2  g64(.a(new_n33_), .b(x0), .O(new_n82_));
  nand3  g65(.a(new_n23_), .b(new_n29_), .c(new_n34_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n82_), .c(new_n23_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x1), .O(new_n85_));
  nand2  g68(.a(x6), .b(x2), .O(new_n86_));
  nand4  g69(.a(new_n86_), .b(new_n29_), .c(new_n40_), .d(x0), .O(new_n87_));
  nand4  g70(.a(new_n87_), .b(new_n85_), .c(new_n81_), .d(new_n76_), .O(z3));
  aoi21  g71(.a(new_n35_), .b(new_n21_), .c(new_n40_), .O(new_n89_));
  oai21  g72(.a(x3), .b(x0), .c(x2), .O(new_n90_));
  nor2   g73(.a(new_n90_), .b(x1), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n89_), .c(new_n23_), .O(new_n92_));
  nand3  g75(.a(new_n90_), .b(x6), .c(new_n40_), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(new_n92_), .O(z4));
  nand2  g77(.a(x3), .b(new_n33_), .O(new_n95_));
  nand2  g78(.a(new_n34_), .b(x2), .O(new_n96_));
  oai21  g79(.a(new_n95_), .b(new_n40_), .c(new_n96_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n19_), .O(new_n98_));
  nand2  g81(.a(new_n82_), .b(new_n40_), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(x6), .O(new_n100_));
  inv1   g83(.a(new_n20_), .O(new_n101_));
  aoi21  g84(.a(x3), .b(x1), .c(x2), .O(new_n102_));
  oai21  g85(.a(new_n102_), .b(new_n101_), .c(x0), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(new_n100_), .c(new_n98_), .O(z5));
  nand3  g87(.a(new_n95_), .b(new_n23_), .c(x1), .O(new_n105_));
  oai21  g88(.a(new_n95_), .b(x1), .c(new_n105_), .O(z6));
  nand3  g89(.a(new_n96_), .b(new_n95_), .c(new_n71_), .O(z7));
  nand2  g90(.a(new_n71_), .b(x3), .O(z8));
  nor2   g91(.a(new_n23_), .b(x1), .O(new_n109_));
  aoi21  g92(.a(new_n37_), .b(new_n19_), .c(x6), .O(new_n110_));
  oai21  g93(.a(new_n110_), .b(new_n109_), .c(x5), .O(new_n111_));
  nor2   g94(.a(new_n111_), .b(new_n18_), .O(z9));
endmodule


