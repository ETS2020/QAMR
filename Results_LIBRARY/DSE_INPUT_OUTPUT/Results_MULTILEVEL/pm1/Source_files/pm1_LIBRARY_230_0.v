// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n41_,
    new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(x07), .b(x06), .c(x05), .O(new_n33_));
  nand3  g04(.a(x12), .b(x09), .c(x08), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  oai21  g06(.a(new_n32_), .b(x11), .c(new_n35_), .O(z02));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g09(.a(new_n30_), .b(x14), .O(z04));
  nor2   g10(.a(new_n30_), .b(x13), .O(z05));
  aoi21  g11(.a(x09), .b(x01), .c(new_n32_), .O(new_n41_));
  nand3  g12(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(x09), .c(x01), .O(new_n43_));
  oai21  g14(.a(new_n41_), .b(x11), .c(new_n43_), .O(z06));
  nor2   g15(.a(new_n30_), .b(x15), .O(z07));
  inv1   g16(.a(x00), .O(new_n46_));
  oai21  g17(.a(x10), .b(new_n46_), .c(x11), .O(new_n47_));
  nand4  g18(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x11), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n47_), .O(z08));
  inv1   g22(.a(x10), .O(new_n52_));
  nand4  g23(.a(x12), .b(x11), .c(new_n52_), .d(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z09));
  inv1   g25(.a(x09), .O(new_n55_));
  nand4  g26(.a(new_n42_), .b(x12), .c(x11), .d(new_n52_), .O(new_n56_));
  nor3   g27(.a(new_n56_), .b(new_n55_), .c(new_n46_), .O(z10));
  inv1   g28(.a(x01), .O(new_n58_));
  nand3  g29(.a(new_n52_), .b(new_n58_), .c(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x11), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n32_), .O(new_n61_));
  inv1   g32(.a(new_n56_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x09), .c(x01), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n61_), .O(z11));
  nand4  g35(.a(x11), .b(new_n52_), .c(new_n55_), .d(x00), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n32_), .O(z12));
  buf    g37(.a(x11), .O(z01));
endmodule


