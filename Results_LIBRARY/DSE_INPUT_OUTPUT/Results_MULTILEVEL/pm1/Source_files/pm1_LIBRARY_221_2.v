// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_;
  inv1   g00(.a(x15), .O(z07));
  nand2  g01(.a(z07), .b(x04), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  inv1   g04(.a(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(new_n32_), .O(z00));
  nand2  g08(.a(x12), .b(new_n33_), .O(new_n38_));
  and2   g09(.a(new_n38_), .b(new_n31_), .O(z01));
  nand2  g10(.a(x06), .b(x05), .O(new_n40_));
  nand3  g11(.a(x09), .b(x08), .c(x07), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n40_), .c(x11), .O(new_n42_));
  nand2  g13(.a(x12), .b(x11), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  aoi21  g15(.a(new_n44_), .b(new_n42_), .c(new_n32_), .O(z02));
  nand4  g16(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n46_));
  and2   g17(.a(new_n46_), .b(x09), .O(new_n47_));
  aoi21  g18(.a(new_n47_), .b(new_n44_), .c(new_n32_), .O(z03));
  nor2   g19(.a(new_n32_), .b(x14), .O(z04));
  nor2   g20(.a(new_n32_), .b(x13), .O(z05));
  nand2  g21(.a(x03), .b(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n38_), .c(x04), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n31_), .O(z06));
  inv1   g26(.a(x10), .O(new_n56_));
  nand2  g27(.a(x12), .b(x09), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n51_), .c(x15), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x04), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x11), .c(new_n56_), .d(x00), .O(z08));
  nand2  g31(.a(new_n43_), .b(new_n35_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n31_), .c(new_n56_), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z09));
  inv1   g34(.a(x00), .O(new_n64_));
  nand2  g35(.a(new_n51_), .b(x15), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x04), .c(new_n34_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x11), .c(new_n56_), .d(x09), .O(new_n67_));
  nor2   g38(.a(new_n67_), .b(new_n64_), .O(z10));
  nand3  g39(.a(x04), .b(x03), .c(x02), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x12), .c(x09), .d(x01), .O(new_n70_));
  inv1   g41(.a(x01), .O(new_n71_));
  nand2  g42(.a(new_n34_), .b(new_n71_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n56_), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n31_), .O(z11));
  inv1   g46(.a(x09), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(x00), .O(new_n77_));
  nand2  g48(.a(new_n44_), .b(new_n56_), .O(new_n78_));
  oai21  g49(.a(new_n78_), .b(new_n77_), .c(new_n31_), .O(z12));
endmodule


