// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n30_), .c(x12), .O(z01));
  nor2   g06(.a(new_n31_), .b(new_n34_), .O(new_n36_));
  nand4  g07(.a(x07), .b(x06), .c(x05), .d(x00), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x09), .d(x08), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n36_), .c(x09), .d(x00), .O(z03));
  nand2  g12(.a(x12), .b(new_n30_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  aoi21  g14(.a(x12), .b(new_n30_), .c(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  oai21  g16(.a(new_n31_), .b(x00), .c(new_n45_), .O(new_n46_));
  nand3  g17(.a(x12), .b(new_n34_), .c(x00), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(z06));
  nand2  g21(.a(new_n42_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  inv1   g23(.a(x09), .O(new_n53_));
  nor2   g24(.a(new_n31_), .b(new_n53_), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x04), .c(x03), .d(x02), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(x11), .c(new_n52_), .d(x00), .O(z08));
  xnor2a g27(.a(x12), .b(x11), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n52_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z09));
  nand4  g30(.a(new_n45_), .b(x12), .c(x11), .d(new_n52_), .O(new_n60_));
  nor3   g31(.a(new_n60_), .b(new_n53_), .c(new_n30_), .O(z10));
  nand4  g32(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n62_));
  inv1   g33(.a(x01), .O(new_n63_));
  nand2  g34(.a(new_n31_), .b(new_n63_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x11), .c(new_n52_), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z11));
  nand4  g38(.a(x11), .b(new_n52_), .c(new_n53_), .d(x00), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n31_), .O(z12));
endmodule


