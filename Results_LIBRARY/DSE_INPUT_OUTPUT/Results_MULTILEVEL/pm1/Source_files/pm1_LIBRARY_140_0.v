// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n45_, new_n46_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x15), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nor2   g06(.a(x15), .b(x01), .O(new_n36_));
  aoi21  g07(.a(x12), .b(new_n31_), .c(new_n36_), .O(z01));
  nand2  g08(.a(x12), .b(x11), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  inv1   g10(.a(x05), .O(new_n40_));
  nand2  g11(.a(x07), .b(x06), .O(new_n41_));
  nor3   g12(.a(new_n41_), .b(new_n36_), .c(new_n40_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n39_), .c(x09), .d(x08), .O(z02));
  inv1   g14(.a(x15), .O(z07));
  nand2  g15(.a(z07), .b(new_n30_), .O(new_n45_));
  nand4  g16(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(new_n39_), .c(new_n45_), .d(x09), .O(z03));
  nand2  g18(.a(new_n45_), .b(x14), .O(z04));
  nand2  g19(.a(new_n45_), .b(x13), .O(z05));
  nand2  g20(.a(x12), .b(new_n31_), .O(new_n50_));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n45_), .O(z06));
  nor2   g25(.a(new_n31_), .b(x10), .O(new_n55_));
  nand4  g26(.a(new_n51_), .b(x12), .c(x09), .d(x04), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n55_), .c(new_n45_), .d(x00), .O(z08));
  inv1   g28(.a(x10), .O(new_n58_));
  nand2  g29(.a(new_n38_), .b(new_n33_), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n45_), .c(new_n58_), .d(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z09));
  nand3  g32(.a(x04), .b(x03), .c(x02), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n45_), .c(x12), .d(x11), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n58_), .c(x09), .d(x00), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z10));
  nand4  g37(.a(new_n62_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  nand3  g38(.a(x15), .b(new_n32_), .c(new_n30_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n58_), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z11));
  inv1   g42(.a(x00), .O(new_n72_));
  nand4  g43(.a(new_n45_), .b(x12), .c(x11), .d(new_n58_), .O(new_n73_));
  nor3   g44(.a(new_n73_), .b(x09), .c(new_n72_), .O(z12));
endmodule


