// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x09), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nand3  g06(.a(x12), .b(new_n30_), .c(x09), .O(z01));
  nand2  g07(.a(x12), .b(x11), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x09), .O(new_n38_));
  nand2  g09(.a(x06), .b(x05), .O(new_n39_));
  nand3  g10(.a(x09), .b(x08), .c(x07), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(x11), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n38_), .O(z02));
  nand4  g13(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n37_), .c(x09), .O(new_n45_));
  oai21  g16(.a(new_n30_), .b(x09), .c(new_n45_), .O(z03));
  inv1   g17(.a(x09), .O(new_n47_));
  aoi21  g18(.a(new_n30_), .b(new_n47_), .c(x14), .O(z04));
  nand2  g19(.a(new_n30_), .b(new_n47_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x13), .O(z05));
  aoi21  g21(.a(x12), .b(x01), .c(new_n47_), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  oai21  g24(.a(new_n51_), .b(x11), .c(new_n53_), .O(z06));
  nand2  g25(.a(new_n49_), .b(x15), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  nor2   g27(.a(new_n32_), .b(new_n47_), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x04), .c(x03), .d(x02), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(x11), .c(new_n56_), .d(x00), .O(z08));
  nand3  g30(.a(new_n32_), .b(new_n30_), .c(x09), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n37_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n56_), .c(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z09));
  nand4  g34(.a(new_n52_), .b(x12), .c(x11), .d(new_n56_), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x09), .c(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n49_), .O(z10));
  nand4  g38(.a(new_n52_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n56_), .d(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n49_), .O(z11));
  nand4  g42(.a(x11), .b(new_n56_), .c(new_n47_), .d(x00), .O(new_n72_));
  nor2   g43(.a(new_n72_), .b(new_n32_), .O(z12));
endmodule


