// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x13), .O(z05));
  nand4  g04(.a(z05), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(z05), .b(new_n32_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n31_), .O(z01));
  nor2   g07(.a(z05), .b(x11), .O(new_n37_));
  nand2  g08(.a(x06), .b(x05), .O(new_n38_));
  nand3  g09(.a(x09), .b(x08), .c(x07), .O(new_n39_));
  nor3   g10(.a(new_n39_), .b(new_n38_), .c(new_n31_), .O(new_n40_));
  oai21  g11(.a(new_n37_), .b(x12), .c(new_n40_), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand2  g13(.a(x12), .b(x09), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x11), .O(new_n46_));
  oai21  g17(.a(x13), .b(x11), .c(new_n46_), .O(z03));
  nor2   g18(.a(new_n37_), .b(x14), .O(z04));
  aoi21  g19(.a(new_n44_), .b(x01), .c(x13), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  oai21  g22(.a(new_n49_), .b(x11), .c(new_n51_), .O(z06));
  inv1   g23(.a(new_n37_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand4  g26(.a(new_n44_), .b(x04), .c(x03), .d(x02), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n55_), .d(x00), .O(z08));
  nand2  g28(.a(x12), .b(x11), .O(new_n58_));
  oai21  g29(.a(new_n35_), .b(x11), .c(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n55_), .c(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z09));
  nand4  g32(.a(new_n50_), .b(x12), .c(x11), .d(new_n55_), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(x09), .c(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n53_), .O(z10));
  nand4  g36(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n32_), .b(new_n30_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n55_), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n53_), .O(z11));
  inv1   g41(.a(x09), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(x00), .O(new_n72_));
  nand3  g43(.a(x12), .b(x11), .c(new_n55_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n72_), .c(new_n53_), .O(z12));
endmodule


