// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n38_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  oai21  g04(.a(new_n31_), .b(x05), .c(x12), .O(z01));
  inv1   g05(.a(x05), .O(new_n36_));
  nand4  g06(.a(x12), .b(x11), .c(x09), .d(new_n36_), .O(z03));
  nor2   g07(.a(new_n32_), .b(new_n36_), .O(new_n38_));
  nor2   g08(.a(new_n38_), .b(x14), .O(z04));
  nor2   g09(.a(new_n38_), .b(x13), .O(z05));
  nand3  g10(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  inv1   g11(.a(new_n41_), .O(new_n42_));
  nand3  g12(.a(x12), .b(new_n31_), .c(new_n36_), .O(new_n43_));
  oai21  g13(.a(new_n42_), .b(new_n38_), .c(new_n43_), .O(new_n44_));
  nand3  g14(.a(new_n44_), .b(x09), .c(x01), .O(new_n45_));
  inv1   g15(.a(new_n45_), .O(z06));
  nor2   g16(.a(new_n38_), .b(x15), .O(z07));
  inv1   g17(.a(x10), .O(new_n48_));
  nand3  g18(.a(x11), .b(new_n48_), .c(x00), .O(new_n49_));
  inv1   g19(.a(new_n49_), .O(new_n50_));
  nand3  g20(.a(x12), .b(x09), .c(new_n36_), .O(new_n51_));
  oai22  g21(.a(new_n51_), .b(new_n41_), .c(new_n50_), .d(new_n38_), .O(z08));
  nand3  g22(.a(x12), .b(x11), .c(new_n36_), .O(new_n53_));
  oai21  g23(.a(x12), .b(x11), .c(new_n53_), .O(new_n54_));
  nand3  g24(.a(new_n54_), .b(new_n48_), .c(x00), .O(new_n55_));
  inv1   g25(.a(new_n55_), .O(z09));
  nand4  g26(.a(new_n41_), .b(x12), .c(x11), .d(new_n48_), .O(new_n57_));
  inv1   g27(.a(new_n57_), .O(new_n58_));
  nand4  g28(.a(new_n58_), .b(x09), .c(new_n36_), .d(x00), .O(new_n59_));
  inv1   g29(.a(new_n59_), .O(z10));
  nand4  g30(.a(new_n41_), .b(x12), .c(x09), .d(new_n36_), .O(new_n61_));
  nand2  g31(.a(new_n32_), .b(new_n30_), .O(new_n62_));
  oai21  g32(.a(new_n61_), .b(new_n30_), .c(new_n62_), .O(new_n63_));
  nand4  g33(.a(new_n63_), .b(x11), .c(new_n48_), .d(x00), .O(new_n64_));
  inv1   g34(.a(new_n64_), .O(z11));
  inv1   g35(.a(x09), .O(new_n66_));
  nand4  g36(.a(new_n48_), .b(new_n66_), .c(new_n36_), .d(x00), .O(new_n67_));
  nor3   g37(.a(new_n67_), .b(new_n32_), .c(new_n31_), .O(z12));
  one    g38(.O(z02));
endmodule


