// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_;
  nor2   g00(.a(x15), .b(x04), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x01), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n30_), .O(z00));
  aoi21  g06(.a(x12), .b(new_n31_), .c(new_n30_), .O(z01));
  nand2  g07(.a(x06), .b(x05), .O(new_n37_));
  nand3  g08(.a(x09), .b(x08), .c(x07), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x11), .O(new_n39_));
  nand2  g10(.a(x12), .b(x11), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  aoi21  g12(.a(new_n41_), .b(new_n39_), .c(new_n30_), .O(z02));
  nand4  g13(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n43_));
  and2   g14(.a(new_n43_), .b(x09), .O(new_n44_));
  aoi21  g15(.a(new_n44_), .b(new_n41_), .c(new_n30_), .O(z03));
  nor2   g16(.a(new_n30_), .b(x14), .O(z04));
  nor2   g17(.a(new_n30_), .b(x13), .O(z05));
  inv1   g18(.a(x04), .O(new_n48_));
  inv1   g19(.a(x15), .O(z07));
  and2   g20(.a(x09), .b(x01), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(z07), .c(new_n48_), .O(new_n51_));
  nand2  g22(.a(x12), .b(new_n31_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x03), .c(x02), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n51_), .O(z06));
  inv1   g26(.a(new_n30_), .O(new_n56_));
  nor2   g27(.a(new_n31_), .b(x10), .O(new_n57_));
  inv1   g28(.a(x09), .O(new_n58_));
  nor2   g29(.a(new_n32_), .b(new_n58_), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x04), .c(x03), .d(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n57_), .c(new_n56_), .d(x00), .O(z08));
  inv1   g32(.a(x10), .O(new_n62_));
  nand2  g33(.a(new_n40_), .b(new_n33_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n56_), .c(new_n62_), .d(x00), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(z09));
  inv1   g36(.a(x00), .O(new_n66_));
  nand2  g37(.a(x03), .b(x02), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g39(.a(x15), .b(new_n48_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n68_), .c(new_n32_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x11), .c(new_n62_), .d(x09), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n66_), .O(z10));
  nand3  g43(.a(x09), .b(x01), .c(x00), .O(new_n73_));
  nand3  g44(.a(x12), .b(x11), .c(new_n62_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n73_), .c(x15), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nand4  g47(.a(new_n67_), .b(x12), .c(x09), .d(x01), .O(new_n77_));
  oai21  g48(.a(x12), .b(x01), .c(new_n77_), .O(new_n78_));
  nand4  g49(.a(new_n78_), .b(x11), .c(new_n62_), .d(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n76_), .O(z11));
  nand2  g51(.a(new_n58_), .b(x00), .O(new_n81_));
  oai21  g52(.a(new_n81_), .b(new_n74_), .c(new_n56_), .O(z12));
endmodule


