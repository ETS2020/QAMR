// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  nand3  g02(.a(x07), .b(x06), .c(x05), .O(new_n32_));
  nand3  g03(.a(x12), .b(x09), .c(x08), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  inv1   g05(.a(x11), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n35_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n34_), .O(z02));
  inv1   g08(.a(x09), .O(new_n38_));
  inv1   g09(.a(x12), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x11), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n36_), .O(z03));
  nor2   g15(.a(new_n30_), .b(x14), .O(z04));
  nor2   g16(.a(new_n30_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x11), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n36_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(z06));
  nand2  g22(.a(z00), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand4  g24(.a(new_n40_), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  nand4  g26(.a(x12), .b(x11), .c(new_n53_), .d(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(z00), .O(z09));
  inv1   g28(.a(x00), .O(new_n58_));
  nand4  g29(.a(new_n47_), .b(x12), .c(x11), .d(new_n53_), .O(new_n59_));
  nor3   g30(.a(new_n59_), .b(new_n38_), .c(new_n58_), .O(z10));
  nand4  g31(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n61_));
  inv1   g32(.a(x01), .O(new_n62_));
  nand2  g33(.a(new_n39_), .b(new_n62_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(x11), .c(new_n53_), .d(x00), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z11));
  nand2  g37(.a(new_n38_), .b(x00), .O(new_n67_));
  nand3  g38(.a(x12), .b(x11), .c(new_n53_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n67_), .c(z00), .O(z12));
  buf    g40(.a(x11), .O(z01));
endmodule


