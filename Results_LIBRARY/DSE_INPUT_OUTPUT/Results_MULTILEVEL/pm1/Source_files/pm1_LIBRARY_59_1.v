// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n39_,
    new_n41_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x09), .O(new_n34_));
  oai21  g05(.a(new_n31_), .b(new_n34_), .c(x12), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  oai21  g09(.a(new_n37_), .b(new_n31_), .c(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z03));
  nand2  g11(.a(x12), .b(new_n34_), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(z12));
  nor2   g13(.a(z12), .b(x14), .O(z04));
  nand2  g14(.a(new_n41_), .b(x13), .O(z05));
  aoi21  g15(.a(new_n31_), .b(x01), .c(new_n34_), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  oai21  g18(.a(new_n45_), .b(new_n32_), .c(new_n47_), .O(z06));
  nand2  g19(.a(new_n41_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nand2  g23(.a(x03), .b(x02), .O(new_n53_));
  nand3  g24(.a(x12), .b(x09), .c(x04), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z08));
  xnor2a g26(.a(x12), .b(x11), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n50_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n41_), .O(z09));
  nand4  g29(.a(new_n46_), .b(x11), .c(new_n50_), .d(x00), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(x09), .c(new_n32_), .O(z10));
  nand3  g31(.a(new_n46_), .b(x12), .c(x01), .O(new_n61_));
  nand2  g32(.a(new_n32_), .b(new_n30_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x11), .c(new_n50_), .d(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n41_), .O(z11));
endmodule


