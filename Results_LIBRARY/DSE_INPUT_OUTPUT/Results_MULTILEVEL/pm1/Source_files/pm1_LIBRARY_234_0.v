// Benchmark "FAU" written by ABC on Fri Aug 14 01:54:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x11), .O(new_n31_));
  nand2  g01(.a(x12), .b(new_n31_), .O(z01));
  nand4  g02(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  inv1   g03(.a(new_n33_), .O(new_n34_));
  nand4  g04(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g05(.a(x09), .O(new_n36_));
  inv1   g06(.a(x12), .O(new_n37_));
  nor2   g07(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g08(.a(new_n38_), .b(new_n33_), .O(new_n39_));
  nand2  g09(.a(new_n39_), .b(x11), .O(new_n40_));
  nand2  g10(.a(new_n40_), .b(z01), .O(z03));
  nor2   g11(.a(x12), .b(x11), .O(new_n42_));
  nor2   g12(.a(new_n42_), .b(x14), .O(z04));
  nor2   g13(.a(new_n42_), .b(x13), .O(z05));
  aoi21  g14(.a(x09), .b(x01), .c(new_n37_), .O(new_n45_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand3  g16(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  oai21  g17(.a(new_n45_), .b(x11), .c(new_n47_), .O(z06));
  inv1   g18(.a(new_n42_), .O(new_n49_));
  nand2  g19(.a(new_n49_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n51_));
  nand4  g21(.a(new_n38_), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand4  g22(.a(new_n52_), .b(x11), .c(new_n51_), .d(x00), .O(z08));
  nand4  g23(.a(x12), .b(x11), .c(new_n51_), .d(x00), .O(new_n54_));
  nand2  g24(.a(new_n54_), .b(new_n49_), .O(z09));
  nand4  g25(.a(new_n46_), .b(x12), .c(x11), .d(new_n51_), .O(new_n56_));
  inv1   g26(.a(new_n56_), .O(new_n57_));
  nand3  g27(.a(new_n57_), .b(x09), .c(x00), .O(new_n58_));
  nand2  g28(.a(new_n58_), .b(new_n49_), .O(z10));
  inv1   g29(.a(x01), .O(new_n60_));
  nand3  g30(.a(new_n51_), .b(new_n60_), .c(x00), .O(new_n61_));
  nand2  g31(.a(new_n61_), .b(x11), .O(new_n62_));
  nand2  g32(.a(new_n62_), .b(new_n37_), .O(new_n63_));
  nand4  g33(.a(new_n57_), .b(x09), .c(x01), .d(x00), .O(new_n64_));
  nand2  g34(.a(new_n64_), .b(new_n63_), .O(z11));
  nand2  g35(.a(new_n36_), .b(x00), .O(new_n66_));
  nand3  g36(.a(x12), .b(x11), .c(new_n51_), .O(new_n67_));
  oai21  g37(.a(new_n67_), .b(new_n66_), .c(new_n49_), .O(z12));
  one    g38(.O(z00));
endmodule


