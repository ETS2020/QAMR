// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n39_, new_n40_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n31_), .c(x09), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n31_), .c(x09), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x12), .O(z02));
  nand4  g08(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g09(.a(x09), .O(new_n39_));
  nand2  g10(.a(x12), .b(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x14), .O(z04));
  nand2  g12(.a(new_n40_), .b(x13), .O(z05));
  aoi21  g13(.a(new_n31_), .b(x01), .c(new_n39_), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x09), .c(x01), .O(new_n45_));
  oai21  g16(.a(new_n43_), .b(new_n32_), .c(new_n45_), .O(z06));
  aoi21  g17(.a(x12), .b(new_n39_), .c(x15), .O(z07));
  inv1   g18(.a(x10), .O(new_n48_));
  nand3  g19(.a(x11), .b(new_n48_), .c(x00), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n40_), .O(new_n50_));
  nand2  g21(.a(x03), .b(x02), .O(new_n51_));
  nand3  g22(.a(x12), .b(x09), .c(x04), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z08));
  xnor2a g24(.a(x12), .b(x11), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n48_), .c(x00), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n40_), .O(z09));
  nand4  g27(.a(new_n44_), .b(x11), .c(new_n48_), .d(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(x09), .c(new_n32_), .O(z10));
  nand4  g29(.a(new_n44_), .b(x12), .c(x09), .d(x01), .O(new_n59_));
  nand2  g30(.a(new_n32_), .b(new_n30_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x11), .c(new_n48_), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z11));
  zero   g34(.O(z12));
endmodule


