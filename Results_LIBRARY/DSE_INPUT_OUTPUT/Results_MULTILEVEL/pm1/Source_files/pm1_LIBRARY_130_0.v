// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_;
  nor2   g00(.a(x12), .b(x01), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  inv1   g02(.a(x11), .O(new_n32_));
  nand2  g03(.a(x12), .b(new_n32_), .O(z01));
  inv1   g04(.a(x12), .O(new_n34_));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  nor3   g07(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(new_n37_));
  nand2  g08(.a(new_n34_), .b(x01), .O(new_n38_));
  oai21  g09(.a(new_n37_), .b(new_n34_), .c(new_n38_), .O(z02));
  nor2   g10(.a(new_n34_), .b(new_n32_), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(z03));
  nor2   g13(.a(new_n30_), .b(x14), .O(z04));
  nor2   g14(.a(new_n30_), .b(x13), .O(z05));
  and2   g15(.a(x03), .b(x02), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(z01), .c(x04), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(z00), .O(z06));
  nand2  g19(.a(z00), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(z00), .O(new_n52_));
  nand4  g23(.a(new_n45_), .b(x12), .c(x09), .d(x04), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n52_), .O(z08));
  nand3  g25(.a(new_n34_), .b(new_n32_), .c(x01), .O(new_n55_));
  oai21  g26(.a(new_n34_), .b(new_n32_), .c(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n50_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(z09));
  nand3  g29(.a(x04), .b(x03), .c(x02), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x12), .c(x11), .d(new_n50_), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x09), .c(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(z00), .O(z10));
  nand4  g34(.a(new_n61_), .b(x09), .c(x01), .d(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(z00), .O(z11));
  inv1   g36(.a(x09), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x00), .O(new_n67_));
  nand2  g38(.a(new_n40_), .b(new_n50_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n67_), .c(z00), .O(z12));
endmodule


