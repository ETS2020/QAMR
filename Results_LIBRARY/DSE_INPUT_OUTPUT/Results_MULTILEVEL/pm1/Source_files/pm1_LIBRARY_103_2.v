// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x07), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x03), .O(new_n34_));
  nand4  g05(.a(new_n34_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g06(.a(new_n34_), .b(x12), .c(new_n31_), .O(z01));
  oai21  g07(.a(x11), .b(x03), .c(new_n33_), .O(new_n37_));
  nand4  g08(.a(x09), .b(x08), .c(x06), .d(x05), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x11), .O(new_n39_));
  nor2   g10(.a(new_n32_), .b(new_n31_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n40_), .c(new_n34_), .d(x09), .O(z03));
  nand2  g14(.a(new_n34_), .b(x14), .O(z04));
  nand2  g15(.a(new_n34_), .b(x13), .O(z05));
  nand2  g16(.a(x12), .b(new_n31_), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(x04), .c(x03), .d(x02), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n34_), .O(z06));
  nand2  g20(.a(new_n34_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x09), .c(x07), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z08));
  xnor2a g27(.a(x12), .b(x11), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n51_), .c(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n34_), .O(z09));
  inv1   g30(.a(x00), .O(new_n60_));
  nand2  g31(.a(x04), .b(x02), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x07), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x03), .c(new_n32_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x11), .c(new_n51_), .d(x09), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n60_), .O(z10));
  nand4  g36(.a(new_n54_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n32_), .b(new_n30_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n51_), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n34_), .O(z11));
  inv1   g41(.a(x09), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(x00), .O(new_n72_));
  nand2  g43(.a(new_n40_), .b(new_n51_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n72_), .c(new_n34_), .O(z12));
endmodule


