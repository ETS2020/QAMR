// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n36_, new_n38_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x08), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x08), .c(x12), .O(z01));
  inv1   g05(.a(x08), .O(new_n36_));
  nand4  g06(.a(x12), .b(x11), .c(x09), .d(new_n36_), .O(z03));
  nor2   g07(.a(new_n30_), .b(new_n36_), .O(new_n38_));
  nor2   g08(.a(new_n38_), .b(x14), .O(z04));
  inv1   g09(.a(new_n38_), .O(new_n40_));
  nand2  g10(.a(new_n40_), .b(x13), .O(z05));
  nand3  g11(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  inv1   g12(.a(new_n42_), .O(new_n43_));
  nand3  g13(.a(x12), .b(new_n33_), .c(new_n36_), .O(new_n44_));
  oai21  g14(.a(new_n43_), .b(new_n38_), .c(new_n44_), .O(new_n45_));
  nand3  g15(.a(new_n45_), .b(x09), .c(x01), .O(new_n46_));
  inv1   g16(.a(new_n46_), .O(z06));
  nor2   g17(.a(new_n38_), .b(x15), .O(z07));
  inv1   g18(.a(x10), .O(new_n49_));
  nand3  g19(.a(x11), .b(new_n49_), .c(x00), .O(new_n50_));
  inv1   g20(.a(new_n50_), .O(new_n51_));
  nand3  g21(.a(x12), .b(x09), .c(new_n36_), .O(new_n52_));
  oai22  g22(.a(new_n52_), .b(new_n42_), .c(new_n51_), .d(new_n38_), .O(z08));
  xnor2a g23(.a(x12), .b(x11), .O(new_n54_));
  nand3  g24(.a(new_n54_), .b(new_n49_), .c(x00), .O(new_n55_));
  nand2  g25(.a(new_n55_), .b(new_n40_), .O(z09));
  nand3  g26(.a(new_n42_), .b(x11), .c(new_n49_), .O(new_n57_));
  inv1   g27(.a(new_n57_), .O(new_n58_));
  nand3  g28(.a(new_n58_), .b(x09), .c(x00), .O(new_n59_));
  aoi21  g29(.a(new_n59_), .b(new_n36_), .c(new_n30_), .O(z10));
  nand4  g30(.a(new_n42_), .b(x12), .c(x09), .d(x01), .O(new_n61_));
  inv1   g31(.a(x01), .O(new_n62_));
  nand2  g32(.a(new_n30_), .b(new_n62_), .O(new_n63_));
  nand2  g33(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand4  g34(.a(new_n64_), .b(x11), .c(new_n49_), .d(x00), .O(new_n65_));
  nand2  g35(.a(new_n65_), .b(new_n40_), .O(z11));
  inv1   g36(.a(x09), .O(new_n67_));
  nand4  g37(.a(new_n49_), .b(new_n67_), .c(new_n36_), .d(x00), .O(new_n68_));
  nor3   g38(.a(new_n68_), .b(new_n30_), .c(new_n33_), .O(z12));
  one    g39(.O(z02));
endmodule


