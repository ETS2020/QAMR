// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n42_, new_n43_, new_n44_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  nand3  g02(.a(x07), .b(x06), .c(x05), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  inv1   g04(.a(x09), .O(new_n34_));
  inv1   g05(.a(x12), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(new_n33_), .c(x11), .d(x08), .O(z02));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g10(.a(new_n30_), .b(x14), .O(z04));
  nand2  g11(.a(z00), .b(x13), .O(z05));
  aoi21  g12(.a(x09), .b(x01), .c(new_n35_), .O(new_n42_));
  nand3  g13(.a(x04), .b(x03), .c(x02), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(x09), .c(x01), .O(new_n44_));
  oai21  g15(.a(new_n42_), .b(x11), .c(new_n44_), .O(z06));
  nor2   g16(.a(new_n30_), .b(x15), .O(z07));
  inv1   g17(.a(x10), .O(new_n47_));
  nand4  g18(.a(new_n36_), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(x11), .c(new_n47_), .d(x00), .O(z08));
  nand4  g20(.a(x12), .b(x11), .c(new_n47_), .d(x00), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(z00), .O(z09));
  inv1   g22(.a(x00), .O(new_n52_));
  nand4  g23(.a(new_n43_), .b(x12), .c(x11), .d(new_n47_), .O(new_n53_));
  nor3   g24(.a(new_n53_), .b(new_n34_), .c(new_n52_), .O(z10));
  inv1   g25(.a(x01), .O(new_n55_));
  nand3  g26(.a(new_n47_), .b(new_n55_), .c(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n35_), .O(new_n58_));
  inv1   g29(.a(new_n53_), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x09), .c(x01), .d(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n58_), .O(z11));
  nand4  g32(.a(x11), .b(new_n47_), .c(new_n34_), .d(x00), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n35_), .O(z12));
  buf    g34(.a(x11), .O(z01));
endmodule


