// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x09), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  oai21  g06(.a(new_n31_), .b(new_n30_), .c(new_n35_), .O(z00));
  nor2   g07(.a(new_n31_), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(x12), .c(new_n31_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  aoi21  g10(.a(new_n39_), .b(x11), .c(new_n33_), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n30_), .c(x11), .O(z02));
  nand2  g12(.a(new_n39_), .b(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x11), .O(z03));
  nor2   g15(.a(new_n37_), .b(x14), .O(z04));
  inv1   g16(.a(new_n37_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x12), .b(new_n31_), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(x04), .c(x03), .d(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n46_), .O(z06));
  nand2  g22(.a(new_n46_), .b(x15), .O(z07));
  nor2   g23(.a(new_n31_), .b(x10), .O(new_n53_));
  nand4  g24(.a(x12), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n53_), .c(x09), .d(x00), .O(z08));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x12), .b(x11), .c(x09), .O(new_n57_));
  oai21  g28(.a(x12), .b(x11), .c(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z09));
  nand3  g31(.a(x04), .b(x03), .c(x02), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x12), .c(new_n56_), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x09), .c(new_n31_), .O(z10));
  nand3  g34(.a(new_n61_), .b(x12), .c(x01), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n56_), .c(x00), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(x09), .c(new_n31_), .O(z11));
  zero   g38(.O(z12));
endmodule


