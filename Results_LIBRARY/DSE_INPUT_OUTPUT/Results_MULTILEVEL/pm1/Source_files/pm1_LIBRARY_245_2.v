// Benchmark "FAU" written by ABC on Fri Aug 14 01:54:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n41_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x00), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  nand3  g09(.a(new_n35_), .b(x08), .c(x07), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g11(.a(new_n30_), .b(x00), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x14), .O(z04));
  nand2  g13(.a(new_n41_), .b(x13), .O(z05));
  nand4  g14(.a(z01), .b(x04), .c(x03), .d(x02), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x09), .c(x01), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n41_), .O(z06));
  aoi21  g17(.a(new_n30_), .b(x00), .c(x15), .O(z07));
  nand4  g18(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  inv1   g20(.a(x10), .O(new_n50_));
  nand2  g21(.a(x11), .b(new_n50_), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n49_), .c(x12), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x00), .O(z08));
  inv1   g24(.a(x00), .O(new_n54_));
  aoi21  g25(.a(new_n51_), .b(x12), .c(new_n54_), .O(z09));
  inv1   g26(.a(x09), .O(new_n56_));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x12), .c(x11), .d(new_n50_), .O(new_n58_));
  nor3   g29(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z10));
  nand3  g30(.a(new_n57_), .b(x11), .c(new_n50_), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x09), .c(x01), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x12), .c(new_n54_), .O(z11));
  nand4  g34(.a(x11), .b(new_n50_), .c(new_n56_), .d(x00), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n30_), .O(z12));
endmodule


