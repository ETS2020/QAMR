// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n31_), .c(x09), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  nor2   g07(.a(new_n32_), .b(new_n31_), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n36_), .c(x09), .O(z02));
  nand3  g09(.a(new_n37_), .b(new_n35_), .c(x09), .O(z03));
  nor2   g10(.a(new_n32_), .b(x09), .O(z12));
  nor2   g11(.a(z12), .b(x14), .O(z04));
  nor2   g12(.a(z12), .b(x13), .O(z05));
  and2   g13(.a(x03), .b(x02), .O(new_n43_));
  nand2  g14(.a(x12), .b(new_n31_), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n43_), .c(x04), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(x09), .c(x01), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(z06));
  inv1   g18(.a(z12), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand4  g23(.a(new_n43_), .b(x12), .c(x09), .d(x04), .O(new_n53_));
  oai21  g24(.a(new_n52_), .b(z12), .c(new_n53_), .O(z08));
  xnor2a g25(.a(x12), .b(x11), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n50_), .c(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n48_), .O(z09));
  inv1   g28(.a(x00), .O(new_n58_));
  inv1   g29(.a(x09), .O(new_n59_));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x12), .c(x11), .d(new_n50_), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(z10));
  nand4  g33(.a(new_n60_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  nand2  g34(.a(new_n32_), .b(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x11), .c(new_n50_), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z11));
endmodule


