// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x14), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n31_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(x14), .b(new_n35_), .c(x12), .O(z01));
  inv1   g07(.a(new_n33_), .O(new_n37_));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(new_n37_), .c(x11), .d(x09), .O(z02));
  nand4  g11(.a(new_n38_), .b(new_n37_), .c(x11), .d(x09), .O(z03));
  nand2  g12(.a(x14), .b(new_n30_), .O(z04));
  aoi21  g13(.a(x14), .b(x12), .c(x13), .O(z05));
  nand2  g14(.a(x14), .b(x12), .O(new_n44_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g17(.a(new_n33_), .b(x11), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(z06));
  nand2  g20(.a(new_n44_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand4  g22(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n32_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x11), .c(new_n51_), .d(x00), .O(z08));
  xnor2a g26(.a(x12), .b(x11), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n51_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n44_), .O(z09));
  nand4  g29(.a(new_n45_), .b(new_n32_), .c(x12), .d(x11), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n51_), .c(x09), .d(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z10));
  nand4  g33(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  inv1   g34(.a(x01), .O(new_n64_));
  nand2  g35(.a(new_n30_), .b(new_n64_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x11), .c(new_n51_), .d(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n44_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(x11), .b(new_n51_), .c(new_n69_), .d(x00), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(x14), .c(new_n30_), .O(z12));
endmodule


