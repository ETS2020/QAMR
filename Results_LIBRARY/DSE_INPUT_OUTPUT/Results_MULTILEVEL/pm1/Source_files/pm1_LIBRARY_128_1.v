// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n31_), .c(x03), .O(z01));
  nand3  g05(.a(x07), .b(x06), .c(x05), .O(new_n35_));
  nand3  g06(.a(x11), .b(x09), .c(x08), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x03), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z02));
  nor2   g09(.a(new_n32_), .b(new_n31_), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n39_), .c(x09), .d(x03), .O(z03));
  nor2   g12(.a(new_n32_), .b(x03), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x14), .O(z04));
  nor2   g14(.a(new_n42_), .b(x13), .O(z05));
  inv1   g15(.a(x09), .O(new_n45_));
  nand2  g16(.a(x12), .b(new_n31_), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  nor2   g18(.a(x12), .b(x03), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(x03), .c(new_n48_), .O(new_n49_));
  nor3   g20(.a(new_n49_), .b(new_n45_), .c(new_n30_), .O(z06));
  nor2   g21(.a(new_n42_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x09), .b(x04), .c(x02), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x03), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(x11), .c(new_n52_), .d(x00), .O(z08));
  inv1   g27(.a(new_n42_), .O(new_n57_));
  xnor2a g28(.a(x12), .b(x11), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n52_), .c(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(z09));
  aoi21  g31(.a(x04), .b(x02), .c(new_n31_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n52_), .c(x09), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x03), .c(new_n32_), .O(z10));
  nand2  g34(.a(x04), .b(x02), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  oai21  g36(.a(x12), .b(x01), .c(new_n65_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x11), .c(new_n52_), .d(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n57_), .O(z11));
  nand4  g39(.a(x11), .b(new_n52_), .c(new_n45_), .d(x00), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(x03), .c(new_n32_), .O(z12));
endmodule


