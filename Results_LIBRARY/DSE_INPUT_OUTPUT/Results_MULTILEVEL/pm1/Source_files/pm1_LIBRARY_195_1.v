// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n38_, new_n41_, new_n42_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  oai21  g02(.a(x12), .b(new_n30_), .c(new_n31_), .O(z00));
  nand4  g03(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g04(.a(new_n34_), .O(new_n35_));
  nand4  g05(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g06(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g07(.a(x12), .b(new_n31_), .O(new_n38_));
  nand2  g08(.a(new_n38_), .b(x14), .O(z04));
  aoi21  g09(.a(x12), .b(new_n31_), .c(x13), .O(z05));
  nand3  g10(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  nand3  g11(.a(new_n41_), .b(x09), .c(x01), .O(new_n42_));
  nand2  g12(.a(new_n42_), .b(new_n38_), .O(z06));
  nand2  g13(.a(new_n38_), .b(x15), .O(z07));
  inv1   g14(.a(x00), .O(new_n45_));
  inv1   g15(.a(x10), .O(new_n46_));
  nand2  g16(.a(x03), .b(x02), .O(new_n47_));
  nand3  g17(.a(x12), .b(x09), .c(x04), .O(new_n48_));
  oai21  g18(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g19(.a(new_n49_), .b(new_n45_), .c(x11), .O(new_n50_));
  inv1   g20(.a(x12), .O(new_n51_));
  nand2  g21(.a(new_n51_), .b(new_n31_), .O(new_n52_));
  nand2  g22(.a(new_n52_), .b(new_n50_), .O(z08));
  nand2  g23(.a(new_n51_), .b(x11), .O(new_n54_));
  nand3  g24(.a(new_n54_), .b(new_n46_), .c(x00), .O(new_n55_));
  nand2  g25(.a(new_n55_), .b(new_n38_), .O(z09));
  inv1   g26(.a(x09), .O(new_n57_));
  nand4  g27(.a(new_n41_), .b(x12), .c(x11), .d(new_n46_), .O(new_n58_));
  nor3   g28(.a(new_n58_), .b(new_n57_), .c(new_n45_), .O(z10));
  nand4  g29(.a(new_n41_), .b(x12), .c(x09), .d(x01), .O(new_n60_));
  nand3  g30(.a(new_n51_), .b(x11), .c(new_n30_), .O(new_n61_));
  nand2  g31(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g32(.a(new_n62_), .b(new_n46_), .c(x00), .O(new_n63_));
  nand2  g33(.a(new_n63_), .b(new_n38_), .O(z11));
  nand3  g34(.a(new_n46_), .b(new_n57_), .c(x00), .O(new_n65_));
  aoi21  g35(.a(new_n65_), .b(x11), .c(new_n51_), .O(z12));
  one    g36(.O(z01));
endmodule


