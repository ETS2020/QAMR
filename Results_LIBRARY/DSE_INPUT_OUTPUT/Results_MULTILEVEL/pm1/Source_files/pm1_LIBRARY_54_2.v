// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x04), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  nand3  g09(.a(new_n35_), .b(x08), .c(x07), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(x11), .c(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(new_n41_));
  oai21  g12(.a(x12), .b(x04), .c(new_n41_), .O(z03));
  nand2  g13(.a(new_n30_), .b(x04), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  aoi21  g15(.a(new_n30_), .b(x04), .c(x13), .O(z05));
  inv1   g16(.a(x01), .O(new_n46_));
  inv1   g17(.a(x09), .O(new_n47_));
  inv1   g18(.a(x04), .O(new_n48_));
  nand3  g19(.a(x11), .b(x03), .c(x02), .O(new_n49_));
  aoi21  g20(.a(new_n49_), .b(x12), .c(new_n48_), .O(new_n50_));
  nor3   g21(.a(new_n50_), .b(new_n47_), .c(new_n46_), .O(z06));
  nand2  g22(.a(new_n43_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x09), .b(x03), .c(x02), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x04), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  nand3  g28(.a(new_n30_), .b(new_n33_), .c(new_n48_), .O(new_n58_));
  oai21  g29(.a(new_n30_), .b(new_n33_), .c(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n53_), .c(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z09));
  inv1   g32(.a(x00), .O(new_n62_));
  nand3  g33(.a(x04), .b(x03), .c(x02), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x12), .c(x11), .d(new_n53_), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n47_), .c(new_n62_), .O(z10));
  nand3  g36(.a(x12), .b(x09), .c(x01), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nor2   g38(.a(x12), .b(x01), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n67_), .c(new_n48_), .O(new_n69_));
  nand2  g40(.a(x03), .b(x02), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x12), .c(x09), .d(x01), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n53_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z11));
  nand4  g45(.a(x11), .b(new_n53_), .c(new_n47_), .d(x00), .O(new_n75_));
  nor2   g46(.a(new_n75_), .b(new_n30_), .O(z12));
endmodule


