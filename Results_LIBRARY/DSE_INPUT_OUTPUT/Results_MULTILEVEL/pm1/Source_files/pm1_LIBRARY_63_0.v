// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x04), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x04), .O(new_n33_));
  nand2  g04(.a(x12), .b(x11), .O(new_n34_));
  oai21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  and2   g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x12), .d(x11), .O(z02));
  inv1   g10(.a(new_n34_), .O(new_n40_));
  nand3  g11(.a(new_n36_), .b(x08), .c(x07), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(z03));
  nand2  g13(.a(new_n30_), .b(new_n33_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(new_n43_), .b(x13), .O(z05));
  aoi21  g16(.a(x09), .b(x01), .c(new_n30_), .O(new_n46_));
  inv1   g17(.a(x11), .O(new_n47_));
  nand2  g18(.a(x12), .b(new_n47_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  oai21  g21(.a(new_n46_), .b(x04), .c(new_n50_), .O(z06));
  aoi21  g22(.a(new_n30_), .b(new_n33_), .c(x15), .O(z07));
  nor2   g23(.a(new_n47_), .b(x10), .O(new_n53_));
  nand2  g24(.a(x03), .b(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x09), .c(x04), .O(new_n55_));
  or2    g26(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n53_), .c(new_n43_), .d(x00), .O(z08));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(new_n30_), .b(new_n47_), .c(x04), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n34_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand4  g33(.a(x11), .b(new_n58_), .c(x09), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(x12), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  nand4  g36(.a(new_n54_), .b(x12), .c(x11), .d(new_n58_), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x09), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n65_), .O(z10));
  nand3  g40(.a(x04), .b(x03), .c(x02), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x12), .c(x09), .d(x01), .O(new_n71_));
  inv1   g42(.a(x01), .O(new_n72_));
  nand3  g43(.a(new_n30_), .b(x04), .c(new_n72_), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x11), .c(new_n58_), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z11));
  inv1   g47(.a(x09), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(x00), .O(new_n78_));
  nand2  g49(.a(new_n40_), .b(new_n58_), .O(new_n79_));
  oai21  g50(.a(new_n79_), .b(new_n78_), .c(new_n43_), .O(z12));
endmodule


