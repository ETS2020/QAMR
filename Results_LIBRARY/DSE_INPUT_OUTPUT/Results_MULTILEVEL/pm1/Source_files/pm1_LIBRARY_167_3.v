// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x10), .c(new_n31_), .O(z00));
  inv1   g03(.a(x10), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n34_), .c(new_n33_), .O(z01));
  nand3  g06(.a(x07), .b(x06), .c(x05), .O(new_n36_));
  nand3  g07(.a(x11), .b(x09), .c(x08), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(new_n33_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n33_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nand2  g14(.a(x12), .b(x10), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  inv1   g16(.a(new_n44_), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand3  g20(.a(x12), .b(new_n34_), .c(new_n33_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  nor2   g24(.a(new_n46_), .b(x15), .O(z07));
  inv1   g25(.a(x00), .O(new_n55_));
  nand2  g26(.a(x03), .b(x02), .O(new_n56_));
  nand3  g27(.a(x12), .b(x09), .c(x04), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n56_), .c(x11), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n55_), .c(new_n33_), .O(new_n59_));
  nand2  g30(.a(new_n30_), .b(x10), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n59_), .O(z08));
  nor2   g32(.a(new_n30_), .b(new_n34_), .O(new_n62_));
  nor3   g33(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n62_), .c(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n44_), .O(z09));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(new_n48_), .b(x12), .c(x11), .d(new_n33_), .O(new_n67_));
  nor3   g38(.a(new_n67_), .b(new_n66_), .c(new_n55_), .O(z10));
  nand4  g39(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n69_));
  inv1   g40(.a(x01), .O(new_n70_));
  nand3  g41(.a(new_n30_), .b(new_n33_), .c(new_n70_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x11), .c(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n44_), .O(z11));
  nand4  g45(.a(x11), .b(new_n33_), .c(new_n66_), .d(x00), .O(new_n75_));
  nor2   g46(.a(new_n75_), .b(new_n30_), .O(z12));
endmodule


