// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x05), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  inv1   g05(.a(x05), .O(new_n35_));
  nand2  g06(.a(z01), .b(new_n35_), .O(new_n36_));
  nand4  g07(.a(x09), .b(x08), .c(x07), .d(x06), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n33_), .c(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n36_), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g12(.a(x12), .b(new_n35_), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x14), .O(z04));
  nor2   g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  oai21  g16(.a(x12), .b(new_n35_), .c(new_n45_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(z01), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(z06));
  oai21  g20(.a(x12), .b(new_n35_), .c(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand2  g24(.a(x03), .b(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x09), .c(x04), .O(new_n55_));
  oai22  g26(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n42_), .O(z08));
  nand3  g27(.a(new_n30_), .b(new_n33_), .c(new_n35_), .O(new_n57_));
  oai21  g28(.a(new_n30_), .b(new_n33_), .c(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n51_), .c(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z09));
  inv1   g31(.a(x00), .O(new_n61_));
  inv1   g32(.a(x09), .O(new_n62_));
  nand4  g33(.a(new_n45_), .b(x12), .c(x11), .d(new_n51_), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z10));
  nand4  g35(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  inv1   g36(.a(x01), .O(new_n66_));
  nand3  g37(.a(new_n30_), .b(new_n35_), .c(new_n66_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n51_), .d(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z11));
  nand4  g41(.a(x11), .b(new_n51_), .c(new_n62_), .d(x00), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n30_), .O(z12));
endmodule


