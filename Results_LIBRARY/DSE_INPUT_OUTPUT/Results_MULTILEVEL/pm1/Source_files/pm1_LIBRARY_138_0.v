// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x03), .d(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  nand2  g09(.a(x12), .b(x11), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n35_), .b(x08), .c(x07), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(z03));
  inv1   g13(.a(x03), .O(new_n43_));
  nand2  g14(.a(new_n32_), .b(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  aoi21  g17(.a(x09), .b(x01), .c(new_n32_), .O(new_n47_));
  nand3  g18(.a(z01), .b(x04), .c(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  oai21  g20(.a(new_n47_), .b(x03), .c(new_n49_), .O(z06));
  aoi21  g21(.a(new_n32_), .b(new_n43_), .c(x15), .O(z07));
  nor2   g22(.a(new_n31_), .b(x10), .O(new_n52_));
  inv1   g23(.a(x09), .O(new_n53_));
  nor2   g24(.a(new_n32_), .b(new_n53_), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x04), .c(x03), .d(x02), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(new_n52_), .c(new_n44_), .d(x00), .O(z08));
  inv1   g27(.a(x10), .O(new_n57_));
  nand4  g28(.a(x11), .b(new_n57_), .c(x09), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x12), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n43_), .O(new_n60_));
  nand2  g31(.a(new_n32_), .b(new_n31_), .O(new_n61_));
  nand2  g32(.a(new_n39_), .b(new_n61_), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n57_), .c(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n60_), .O(z09));
  nand2  g35(.a(x04), .b(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x12), .c(x11), .d(new_n57_), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x09), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n60_), .O(z10));
  nand2  g40(.a(x01), .b(x00), .O(new_n70_));
  nand3  g41(.a(x11), .b(new_n57_), .c(x09), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n70_), .c(x12), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nand4  g44(.a(new_n65_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  nand2  g45(.a(new_n32_), .b(new_n30_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x11), .c(new_n57_), .d(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n73_), .O(z11));
  nand4  g49(.a(x11), .b(new_n57_), .c(new_n53_), .d(x00), .O(new_n79_));
  nor2   g50(.a(new_n79_), .b(new_n32_), .O(z12));
endmodule


