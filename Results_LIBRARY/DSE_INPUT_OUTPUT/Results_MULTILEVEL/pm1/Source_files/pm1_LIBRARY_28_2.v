// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n32_), .O(z01));
  nand2  g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  nor3   g08(.a(new_n37_), .b(new_n36_), .c(new_n32_), .O(new_n38_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(new_n39_));
  oai21  g10(.a(new_n38_), .b(new_n33_), .c(new_n39_), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g13(.a(x12), .b(new_n31_), .O(new_n43_));
  nor2   g14(.a(new_n43_), .b(x14), .O(z04));
  inv1   g15(.a(new_n43_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  and2   g17(.a(x03), .b(x02), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(z01), .c(x04), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n45_), .O(z06));
  nor2   g21(.a(new_n43_), .b(x15), .O(z07));
  nand4  g22(.a(new_n47_), .b(x12), .c(x09), .d(x04), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(x11), .c(new_n31_), .d(x00), .O(z08));
  nand3  g24(.a(x12), .b(x11), .c(new_n31_), .O(new_n54_));
  oai21  g25(.a(x12), .b(x11), .c(new_n54_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n45_), .O(z09));
  inv1   g28(.a(x00), .O(new_n58_));
  inv1   g29(.a(x09), .O(new_n59_));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x12), .c(x11), .d(new_n31_), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(z10));
  nand3  g33(.a(x11), .b(new_n30_), .c(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n31_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  inv1   g36(.a(new_n61_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x09), .c(x01), .d(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(z11));
  nand2  g39(.a(new_n59_), .b(x00), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n54_), .c(new_n45_), .O(z12));
endmodule


