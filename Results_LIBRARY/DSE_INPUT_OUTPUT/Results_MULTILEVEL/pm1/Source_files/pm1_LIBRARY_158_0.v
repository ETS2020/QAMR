// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x03), .d(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n31_), .c(x03), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  nand2  g09(.a(x12), .b(x11), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n35_), .b(x08), .c(x07), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(z03));
  nor2   g13(.a(x11), .b(x03), .O(new_n43_));
  nor2   g14(.a(new_n43_), .b(x14), .O(z04));
  inv1   g15(.a(new_n43_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  inv1   g17(.a(x09), .O(new_n47_));
  nand2  g18(.a(x12), .b(new_n31_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x04), .c(x02), .O(new_n49_));
  nor2   g20(.a(new_n31_), .b(x03), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(x03), .c(new_n50_), .O(new_n51_));
  nor3   g22(.a(new_n51_), .b(new_n47_), .c(new_n30_), .O(z06));
  nand2  g23(.a(new_n45_), .b(x15), .O(z07));
  nand4  g24(.a(x12), .b(x09), .c(x04), .d(x02), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x03), .O(new_n56_));
  inv1   g27(.a(x00), .O(new_n57_));
  oai21  g28(.a(x10), .b(new_n57_), .c(x11), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n56_), .O(z08));
  inv1   g30(.a(x10), .O(new_n60_));
  nand2  g31(.a(new_n32_), .b(new_n31_), .O(new_n61_));
  nand2  g32(.a(new_n39_), .b(new_n61_), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n45_), .O(z09));
  nand3  g35(.a(x04), .b(x03), .c(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x12), .c(x11), .d(new_n60_), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n47_), .c(new_n57_), .O(z10));
  nand4  g38(.a(new_n65_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  oai21  g39(.a(x12), .b(x01), .c(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n60_), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z11));
  nand4  g42(.a(x11), .b(new_n60_), .c(new_n47_), .d(x00), .O(new_n72_));
  nor2   g43(.a(new_n72_), .b(new_n32_), .O(z12));
endmodule


