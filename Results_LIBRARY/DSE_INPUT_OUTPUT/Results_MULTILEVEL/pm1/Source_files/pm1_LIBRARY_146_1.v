// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n30_), .c(x12), .O(z01));
  nor2   g06(.a(new_n31_), .b(new_n34_), .O(new_n36_));
  nand4  g07(.a(x07), .b(x06), .c(x05), .d(x00), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x09), .d(x08), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x00), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nand2  g14(.a(x12), .b(new_n30_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  aoi21  g16(.a(x12), .b(new_n30_), .c(x13), .O(z05));
  nand3  g17(.a(new_n34_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x00), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n49_), .O(z06));
  nand2  g23(.a(new_n44_), .b(x15), .O(z07));
  nand2  g24(.a(x03), .b(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x09), .c(x04), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(x11), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(x10), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n31_), .b(new_n30_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n57_), .O(z08));
  inv1   g30(.a(x10), .O(new_n60_));
  nand3  g31(.a(new_n31_), .b(new_n34_), .c(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n36_), .c(new_n60_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n44_), .O(z09));
  nand4  g35(.a(new_n50_), .b(x11), .c(new_n60_), .d(x09), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x00), .c(new_n31_), .O(z10));
  nand4  g37(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  inv1   g38(.a(x01), .O(new_n68_));
  nand3  g39(.a(new_n31_), .b(new_n68_), .c(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(x11), .c(new_n60_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n44_), .O(z11));
  inv1   g43(.a(x09), .O(new_n73_));
  nand3  g44(.a(x11), .b(new_n60_), .c(new_n73_), .O(new_n74_));
  aoi21  g45(.a(new_n74_), .b(x00), .c(new_n31_), .O(z12));
endmodule


