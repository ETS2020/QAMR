// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x13), .O(z05));
  nand4  g04(.a(z05), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n31_), .O(z01));
  and2   g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x12), .d(x11), .O(z02));
  nand2  g10(.a(z05), .b(new_n32_), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n40_), .O(z03));
  aoi21  g15(.a(x13), .b(new_n32_), .c(x14), .O(z04));
  nand2  g16(.a(x13), .b(new_n32_), .O(new_n46_));
  and2   g17(.a(x03), .b(x02), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(z01), .c(x04), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(z06));
  nand2  g21(.a(new_n46_), .b(x15), .O(z07));
  nor2   g22(.a(new_n31_), .b(x10), .O(new_n52_));
  nand4  g23(.a(new_n47_), .b(x12), .c(x09), .d(x04), .O(new_n53_));
  nand4  g24(.a(new_n53_), .b(new_n52_), .c(new_n46_), .d(x00), .O(z08));
  inv1   g25(.a(x10), .O(new_n55_));
  nand2  g26(.a(x12), .b(x11), .O(new_n56_));
  oai21  g27(.a(new_n40_), .b(x11), .c(new_n56_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z09));
  inv1   g30(.a(x00), .O(new_n60_));
  inv1   g31(.a(x09), .O(new_n61_));
  nand3  g32(.a(x04), .b(x03), .c(x02), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x12), .c(x11), .d(new_n55_), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z10));
  nand4  g35(.a(x11), .b(new_n55_), .c(new_n30_), .d(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(z05), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n32_), .O(new_n67_));
  inv1   g38(.a(new_n63_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x09), .c(x01), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n67_), .O(z11));
  nand2  g41(.a(new_n61_), .b(x00), .O(new_n71_));
  nand3  g42(.a(x12), .b(x11), .c(new_n55_), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(new_n71_), .c(new_n46_), .O(z12));
endmodule


