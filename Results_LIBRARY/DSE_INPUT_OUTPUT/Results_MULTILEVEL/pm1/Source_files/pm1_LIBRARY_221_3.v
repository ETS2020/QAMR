// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_;
  nand2  g00(.a(x15), .b(x04), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x01), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n31_), .O(z00));
  aoi21  g07(.a(x12), .b(new_n32_), .c(new_n31_), .O(z01));
  nand2  g08(.a(x06), .b(x05), .O(new_n38_));
  nand3  g09(.a(x09), .b(x08), .c(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x11), .O(new_n40_));
  nand2  g11(.a(x12), .b(x11), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  aoi21  g13(.a(new_n42_), .b(new_n40_), .c(new_n31_), .O(z02));
  nand4  g14(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n44_));
  and2   g15(.a(new_n44_), .b(x09), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(new_n42_), .c(new_n31_), .O(z03));
  nor2   g17(.a(new_n31_), .b(x14), .O(z04));
  nor2   g18(.a(new_n31_), .b(x13), .O(z05));
  nand2  g19(.a(x12), .b(new_n32_), .O(new_n49_));
  nand2  g20(.a(x03), .b(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n49_), .c(x04), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n30_), .O(z06));
  inv1   g25(.a(x04), .O(new_n55_));
  nand2  g26(.a(x15), .b(new_n55_), .O(z07));
  inv1   g27(.a(x10), .O(new_n57_));
  inv1   g28(.a(x15), .O(new_n58_));
  nand2  g29(.a(x12), .b(x09), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n50_), .c(new_n58_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x04), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x11), .c(new_n57_), .d(x00), .O(z08));
  nand2  g33(.a(new_n41_), .b(new_n34_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n30_), .c(new_n57_), .d(x00), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(z09));
  inv1   g36(.a(x00), .O(new_n66_));
  nand2  g37(.a(new_n50_), .b(new_n58_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(x04), .c(new_n33_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n57_), .d(x09), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n66_), .O(z10));
  nand3  g41(.a(x04), .b(x03), .c(x02), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  inv1   g43(.a(x01), .O(new_n73_));
  nand2  g44(.a(new_n33_), .b(new_n73_), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x11), .c(new_n57_), .d(x00), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n30_), .O(z11));
  inv1   g48(.a(x09), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(x00), .O(new_n79_));
  nand2  g50(.a(new_n42_), .b(new_n57_), .O(new_n80_));
  oai21  g51(.a(new_n80_), .b(new_n79_), .c(new_n30_), .O(z12));
endmodule


