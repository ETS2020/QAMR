// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_;
  nor2   g00(.a(x11), .b(x01), .O(new_n30_));
  nand2  g01(.a(x15), .b(x12), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x12), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  inv1   g04(.a(x15), .O(z07));
  oai21  g05(.a(z07), .b(new_n33_), .c(x12), .O(z01));
  inv1   g06(.a(new_n31_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x11), .d(x09), .O(z02));
  nand3  g10(.a(new_n37_), .b(x11), .c(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x15), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  aoi21  g13(.a(z07), .b(x12), .c(x14), .O(z04));
  aoi21  g14(.a(z07), .b(x12), .c(x13), .O(z05));
  nand3  g15(.a(new_n33_), .b(x09), .c(x01), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x15), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n47_), .O(z06));
  inv1   g21(.a(x10), .O(new_n51_));
  nand4  g22(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x15), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x11), .c(new_n51_), .d(x00), .O(z08));
  inv1   g26(.a(x12), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  oai21  g28(.a(new_n31_), .b(new_n33_), .c(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n51_), .c(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z09));
  nand4  g31(.a(new_n48_), .b(x15), .c(x12), .d(x11), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n51_), .c(x09), .d(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z10));
  inv1   g35(.a(x01), .O(new_n65_));
  nand4  g36(.a(new_n48_), .b(x15), .c(x12), .d(x09), .O(new_n66_));
  nand2  g37(.a(new_n56_), .b(new_n65_), .O(new_n67_));
  oai21  g38(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n51_), .d(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z11));
  inv1   g41(.a(x09), .O(new_n71_));
  nand4  g42(.a(x11), .b(new_n51_), .c(new_n71_), .d(x00), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(z07), .c(new_n56_), .O(z12));
endmodule


