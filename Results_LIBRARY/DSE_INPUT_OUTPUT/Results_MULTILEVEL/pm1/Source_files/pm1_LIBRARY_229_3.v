// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x08), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x08), .c(x12), .O(z01));
  inv1   g05(.a(x08), .O(new_n36_));
  inv1   g06(.a(x09), .O(new_n37_));
  oai21  g07(.a(new_n33_), .b(new_n37_), .c(new_n36_), .O(new_n38_));
  nand2  g08(.a(new_n38_), .b(x12), .O(z03));
  nor2   g09(.a(new_n30_), .b(new_n36_), .O(new_n40_));
  inv1   g10(.a(new_n40_), .O(new_n41_));
  nand2  g11(.a(new_n41_), .b(x14), .O(z04));
  nor2   g12(.a(new_n40_), .b(x13), .O(z05));
  nand3  g13(.a(new_n33_), .b(x09), .c(x01), .O(new_n44_));
  nand2  g14(.a(new_n44_), .b(new_n36_), .O(new_n45_));
  nand2  g15(.a(new_n45_), .b(x12), .O(new_n46_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand3  g17(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  nand2  g18(.a(new_n48_), .b(new_n46_), .O(z06));
  nor2   g19(.a(new_n40_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n51_));
  nand4  g21(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand2  g22(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand2  g23(.a(new_n53_), .b(x12), .O(new_n54_));
  nand4  g24(.a(new_n54_), .b(x11), .c(new_n51_), .d(x00), .O(z08));
  nand3  g25(.a(x12), .b(x11), .c(new_n36_), .O(new_n56_));
  oai21  g26(.a(x12), .b(x11), .c(new_n56_), .O(new_n57_));
  nand3  g27(.a(new_n57_), .b(new_n51_), .c(x00), .O(new_n58_));
  inv1   g28(.a(new_n58_), .O(z09));
  nand4  g29(.a(new_n47_), .b(x12), .c(x11), .d(new_n51_), .O(new_n60_));
  inv1   g30(.a(new_n60_), .O(new_n61_));
  nand4  g31(.a(new_n61_), .b(x09), .c(new_n36_), .d(x00), .O(new_n62_));
  inv1   g32(.a(new_n62_), .O(z10));
  nand4  g33(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  inv1   g34(.a(x01), .O(new_n65_));
  nand2  g35(.a(new_n30_), .b(new_n65_), .O(new_n66_));
  nand2  g36(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand4  g37(.a(new_n67_), .b(x11), .c(new_n51_), .d(x00), .O(new_n68_));
  nand2  g38(.a(new_n68_), .b(new_n41_), .O(z11));
  nand4  g39(.a(x11), .b(new_n51_), .c(new_n37_), .d(x00), .O(new_n70_));
  aoi21  g40(.a(new_n70_), .b(new_n36_), .c(new_n30_), .O(z12));
  one    g41(.O(z02));
endmodule


