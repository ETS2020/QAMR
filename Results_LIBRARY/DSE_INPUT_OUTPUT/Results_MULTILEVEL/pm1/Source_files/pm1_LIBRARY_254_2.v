// Benchmark "FAU" written by ABC on Fri Aug 14 01:54:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  or2    g02(.a(x12), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n30_), .O(z00));
  nand3  g05(.a(x12), .b(new_n30_), .c(new_n31_), .O(z01));
  and2   g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x12), .d(x11), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x12), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(new_n42_));
  oai21  g13(.a(x11), .b(x10), .c(new_n42_), .O(z03));
  nand2  g14(.a(new_n30_), .b(x10), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand3  g19(.a(x12), .b(new_n30_), .c(new_n31_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(z06));
  aoi21  g23(.a(new_n30_), .b(x10), .c(x15), .O(z07));
  inv1   g24(.a(x00), .O(new_n54_));
  nand2  g25(.a(x03), .b(x02), .O(new_n55_));
  nand3  g26(.a(x12), .b(x09), .c(x04), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n55_), .c(x11), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n54_), .c(new_n31_), .O(new_n58_));
  oai21  g29(.a(new_n30_), .b(new_n31_), .c(new_n58_), .O(z08));
  nand3  g30(.a(x12), .b(x11), .c(new_n31_), .O(new_n60_));
  oai21  g31(.a(x12), .b(x11), .c(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n44_), .O(z09));
  inv1   g34(.a(x09), .O(new_n64_));
  nand4  g35(.a(new_n47_), .b(x12), .c(x11), .d(new_n31_), .O(new_n65_));
  nor3   g36(.a(new_n65_), .b(new_n64_), .c(new_n54_), .O(z10));
  nand4  g37(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n32_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n31_), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n44_), .O(z11));
  nand2  g41(.a(new_n64_), .b(x00), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n60_), .c(new_n44_), .O(z12));
endmodule


