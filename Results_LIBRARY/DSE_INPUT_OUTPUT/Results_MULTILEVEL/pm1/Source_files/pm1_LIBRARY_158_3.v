// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  nor3   g07(.a(new_n36_), .b(new_n35_), .c(x03), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(x12), .c(x11), .O(z02));
  inv1   g09(.a(x03), .O(new_n39_));
  nand2  g10(.a(x12), .b(x11), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n41_), .c(x09), .d(new_n39_), .O(z03));
  aoi21  g14(.a(x11), .b(x03), .c(x14), .O(z04));
  nand2  g15(.a(x11), .b(x03), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  inv1   g17(.a(x09), .O(new_n47_));
  nand3  g18(.a(new_n32_), .b(x04), .c(x02), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(new_n31_), .c(new_n39_), .O(new_n49_));
  nor3   g20(.a(new_n49_), .b(new_n47_), .c(new_n30_), .O(z06));
  nand2  g21(.a(new_n45_), .b(x15), .O(z07));
  inv1   g22(.a(x00), .O(new_n52_));
  oai21  g23(.a(x10), .b(new_n52_), .c(new_n39_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x11), .O(z08));
  inv1   g25(.a(x10), .O(new_n55_));
  nand2  g26(.a(new_n32_), .b(new_n31_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n40_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n45_), .O(z09));
  nand3  g30(.a(x09), .b(new_n39_), .c(x00), .O(new_n60_));
  nor3   g31(.a(new_n60_), .b(new_n40_), .c(x10), .O(z10));
  nand3  g32(.a(x12), .b(x09), .c(x01), .O(new_n62_));
  oai21  g33(.a(x12), .b(x01), .c(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x11), .c(new_n55_), .d(new_n39_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n52_), .O(z11));
  nand4  g36(.a(new_n55_), .b(new_n47_), .c(new_n39_), .d(x00), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n32_), .c(new_n31_), .O(z12));
endmodule


