// Benchmark "FAU" written by ABC on Fri Aug 14 01:54:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x11), .O(new_n30_));
  or2    g01(.a(x12), .b(x01), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(x10), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n30_), .c(x10), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  inv1   g09(.a(x10), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x12), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(new_n42_));
  oai21  g13(.a(x11), .b(new_n39_), .c(new_n42_), .O(z03));
  nor2   g14(.a(x11), .b(x10), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n45_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand3  g20(.a(x12), .b(new_n30_), .c(x10), .O(new_n50_));
  oai21  g21(.a(new_n49_), .b(new_n44_), .c(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  nor2   g24(.a(new_n44_), .b(x15), .O(z07));
  nand2  g25(.a(x03), .b(x02), .O(new_n55_));
  nand3  g26(.a(x12), .b(x09), .c(x04), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n55_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x11), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n39_), .O(z08));
  nand2  g30(.a(x12), .b(x00), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x11), .c(x10), .O(z09));
  inv1   g32(.a(x00), .O(new_n62_));
  inv1   g33(.a(x09), .O(new_n63_));
  nand4  g34(.a(new_n48_), .b(x12), .c(x11), .d(new_n39_), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z10));
  nand4  g36(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n31_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x00), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(x11), .c(x10), .O(z11));
  nand3  g40(.a(x12), .b(new_n63_), .c(x00), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(x11), .c(x10), .O(z12));
endmodule


