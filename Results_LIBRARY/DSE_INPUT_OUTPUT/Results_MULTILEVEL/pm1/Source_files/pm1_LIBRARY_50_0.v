// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_;
  inv1   g00(.a(x11), .O(new_n31_));
  nand2  g01(.a(x12), .b(new_n31_), .O(z01));
  nand3  g02(.a(x07), .b(x06), .c(x05), .O(new_n33_));
  nand3  g03(.a(x12), .b(x09), .c(x08), .O(new_n34_));
  oai21  g04(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  nand2  g05(.a(new_n35_), .b(z01), .O(z02));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand3  g07(.a(new_n37_), .b(x12), .c(x09), .O(new_n38_));
  nand2  g08(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g09(.a(new_n39_), .b(z01), .O(z03));
  inv1   g10(.a(x12), .O(new_n41_));
  aoi21  g11(.a(new_n41_), .b(new_n31_), .c(x14), .O(z04));
  nand2  g12(.a(new_n41_), .b(new_n31_), .O(new_n43_));
  nand2  g13(.a(new_n43_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  nand2  g15(.a(new_n45_), .b(x11), .O(new_n46_));
  nand2  g16(.a(new_n46_), .b(z01), .O(new_n47_));
  nand3  g17(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  inv1   g18(.a(new_n48_), .O(z06));
  nand2  g19(.a(new_n43_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n51_));
  aoi21  g21(.a(new_n51_), .b(x00), .c(new_n31_), .O(new_n52_));
  nand4  g22(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n53_));
  aoi21  g23(.a(new_n53_), .b(x11), .c(new_n41_), .O(new_n54_));
  or2    g24(.a(new_n54_), .b(new_n52_), .O(z08));
  nand4  g25(.a(x12), .b(x11), .c(new_n51_), .d(x00), .O(new_n56_));
  inv1   g26(.a(new_n56_), .O(z09));
  nand4  g27(.a(new_n45_), .b(x12), .c(x11), .d(new_n51_), .O(new_n58_));
  inv1   g28(.a(new_n58_), .O(new_n59_));
  nand3  g29(.a(new_n59_), .b(x09), .c(x00), .O(new_n60_));
  nand2  g30(.a(new_n60_), .b(new_n43_), .O(z10));
  nand4  g31(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n62_));
  inv1   g32(.a(x01), .O(new_n63_));
  nand2  g33(.a(new_n41_), .b(new_n63_), .O(new_n64_));
  nand2  g34(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand4  g35(.a(new_n65_), .b(x11), .c(new_n51_), .d(x00), .O(new_n66_));
  inv1   g36(.a(new_n66_), .O(z11));
  inv1   g37(.a(x09), .O(new_n68_));
  nand2  g38(.a(new_n68_), .b(x00), .O(new_n69_));
  nand3  g39(.a(x12), .b(x11), .c(new_n51_), .O(new_n70_));
  oai21  g40(.a(new_n70_), .b(new_n69_), .c(new_n43_), .O(z12));
  one    g41(.O(z00));
endmodule


