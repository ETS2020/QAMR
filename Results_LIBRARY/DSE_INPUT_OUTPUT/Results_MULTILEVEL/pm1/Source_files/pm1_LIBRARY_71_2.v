// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n41_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x09), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  and2   g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(x12), .d(x11), .O(z02));
  nand3  g09(.a(new_n35_), .b(x08), .c(x07), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g11(.a(new_n30_), .b(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x14), .O(z04));
  aoi21  g13(.a(new_n30_), .b(x09), .c(x13), .O(z05));
  nand4  g14(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(x12), .c(x09), .d(x01), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(z06));
  nand2  g17(.a(new_n41_), .b(x15), .O(z07));
  inv1   g18(.a(x10), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(x11), .c(new_n48_), .d(x00), .O(z08));
  inv1   g23(.a(x09), .O(new_n53_));
  nand3  g24(.a(new_n33_), .b(new_n48_), .c(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand2  g27(.a(new_n49_), .b(x09), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n30_), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(x11), .c(new_n48_), .d(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n56_), .O(z09));
  nand4  g31(.a(new_n49_), .b(x11), .c(new_n48_), .d(x00), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(x12), .c(new_n53_), .O(z10));
  inv1   g33(.a(x01), .O(new_n63_));
  nand4  g34(.a(x11), .b(new_n48_), .c(new_n63_), .d(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand3  g37(.a(new_n49_), .b(x11), .c(new_n48_), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x09), .c(x01), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n66_), .O(z11));
  nand4  g41(.a(x11), .b(new_n48_), .c(new_n53_), .d(x00), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n30_), .O(z12));
endmodule


