// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n36_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x06), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x06), .c(x12), .O(z01));
  nand2  g05(.a(x12), .b(x06), .O(z02));
  inv1   g06(.a(x06), .O(new_n36_));
  nand4  g07(.a(x12), .b(x11), .c(x09), .d(new_n36_), .O(z03));
  nand2  g08(.a(z02), .b(x14), .O(z04));
  nand2  g09(.a(z02), .b(x13), .O(z05));
  nand3  g10(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(z02), .O(new_n41_));
  nand3  g12(.a(x12), .b(new_n33_), .c(new_n36_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(x09), .c(x01), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(z06));
  nand2  g16(.a(z02), .b(x15), .O(z07));
  inv1   g17(.a(x10), .O(new_n47_));
  nand4  g18(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n36_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand4  g21(.a(new_n50_), .b(x11), .c(new_n47_), .d(x00), .O(z08));
  xnor2a g22(.a(x12), .b(x11), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n47_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(z02), .O(z09));
  nand3  g25(.a(new_n40_), .b(x11), .c(new_n47_), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n36_), .c(new_n30_), .O(z10));
  inv1   g29(.a(x01), .O(new_n59_));
  nand4  g30(.a(new_n40_), .b(x12), .c(x09), .d(new_n36_), .O(new_n60_));
  nand2  g31(.a(new_n30_), .b(new_n59_), .O(new_n61_));
  oai21  g32(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x11), .c(new_n47_), .d(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z11));
  inv1   g35(.a(x09), .O(new_n65_));
  nand4  g36(.a(new_n47_), .b(new_n65_), .c(new_n36_), .d(x00), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n30_), .c(new_n33_), .O(z12));
endmodule


