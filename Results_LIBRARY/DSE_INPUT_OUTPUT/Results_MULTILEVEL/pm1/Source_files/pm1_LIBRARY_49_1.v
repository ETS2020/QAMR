// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x03), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n34_), .c(x03), .O(z01));
  nand3  g06(.a(x07), .b(x06), .c(x05), .O(new_n36_));
  nand3  g07(.a(x11), .b(x09), .c(x08), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x03), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x03), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nand2  g14(.a(x12), .b(new_n30_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  inv1   g17(.a(x01), .O(new_n47_));
  inv1   g18(.a(x09), .O(new_n48_));
  nand2  g19(.a(x12), .b(new_n34_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x04), .c(x02), .O(new_n50_));
  nor2   g21(.a(x12), .b(x03), .O(new_n51_));
  aoi21  g22(.a(new_n50_), .b(x03), .c(new_n51_), .O(new_n52_));
  nor3   g23(.a(new_n52_), .b(new_n48_), .c(new_n47_), .O(z06));
  nand2  g24(.a(new_n44_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x09), .b(x04), .c(x02), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x03), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x12), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(x11), .c(new_n55_), .d(x00), .O(z08));
  xnor2a g30(.a(x12), .b(x11), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n55_), .c(x00), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n44_), .O(z09));
  aoi21  g33(.a(x04), .b(x02), .c(new_n34_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n55_), .c(x09), .d(x00), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(x03), .c(new_n31_), .O(z10));
  nand2  g36(.a(x04), .b(x02), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x09), .d(x03), .O(new_n67_));
  nand2  g38(.a(new_n31_), .b(new_n47_), .O(new_n68_));
  oai21  g39(.a(new_n67_), .b(new_n47_), .c(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n55_), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z11));
  nand4  g42(.a(new_n55_), .b(new_n48_), .c(x03), .d(x00), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(new_n31_), .c(new_n34_), .O(z12));
endmodule


