// Benchmark "FAU" written by ABC on Fri Aug 14 01:54:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  oai21  g05(.a(new_n30_), .b(x10), .c(new_n34_), .O(z00));
  nand2  g06(.a(x12), .b(new_n30_), .O(z01));
  nand3  g07(.a(x07), .b(x06), .c(x05), .O(new_n37_));
  inv1   g08(.a(x10), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(x09), .c(x08), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x12), .c(x11), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(x12), .c(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n38_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x11), .O(z03));
  nand2  g16(.a(x11), .b(x10), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(new_n46_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(z01), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  aoi21  g24(.a(x11), .b(x10), .c(x15), .O(z07));
  nand2  g25(.a(x03), .b(x02), .O(new_n55_));
  nand3  g26(.a(x12), .b(x09), .c(x04), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n55_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x11), .O(z08));
  nor2   g30(.a(new_n32_), .b(new_n30_), .O(new_n60_));
  nor3   g31(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n60_), .c(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n46_), .O(z09));
  inv1   g34(.a(x00), .O(new_n64_));
  inv1   g35(.a(x09), .O(new_n65_));
  nand4  g36(.a(new_n49_), .b(x12), .c(x11), .d(new_n38_), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z10));
  nand4  g38(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(x00), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(new_n38_), .c(new_n30_), .O(z11));
  nand3  g42(.a(x12), .b(new_n65_), .c(x00), .O(new_n72_));
  aoi21  g43(.a(new_n72_), .b(new_n38_), .c(new_n30_), .O(z12));
endmodule


