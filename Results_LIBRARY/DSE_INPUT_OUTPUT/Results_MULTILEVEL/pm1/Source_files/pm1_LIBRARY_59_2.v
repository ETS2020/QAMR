// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x09), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  oai21  g05(.a(new_n33_), .b(x11), .c(new_n31_), .O(new_n35_));
  oai21  g06(.a(new_n33_), .b(new_n32_), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  oai21  g10(.a(new_n38_), .b(new_n32_), .c(x12), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x09), .O(z03));
  aoi21  g12(.a(new_n33_), .b(x09), .c(x14), .O(z04));
  nand2  g13(.a(new_n33_), .b(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x13), .O(z05));
  nand2  g15(.a(x03), .b(x02), .O(new_n45_));
  nand2  g16(.a(x11), .b(x04), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(new_n45_), .c(x01), .O(new_n47_));
  aoi21  g18(.a(new_n47_), .b(x12), .c(new_n31_), .O(z06));
  nand2  g19(.a(new_n43_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n43_), .O(new_n52_));
  nand3  g23(.a(x12), .b(x09), .c(x04), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n45_), .c(new_n52_), .O(z08));
  nand3  g25(.a(new_n32_), .b(new_n50_), .c(x00), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n31_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x09), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n33_), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x11), .c(new_n50_), .d(x00), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n57_), .O(z09));
  nand4  g33(.a(new_n58_), .b(x11), .c(new_n50_), .d(x00), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(x12), .c(new_n31_), .O(z10));
  nand4  g35(.a(x11), .b(new_n50_), .c(new_n30_), .d(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n31_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  nand3  g38(.a(new_n58_), .b(x11), .c(new_n50_), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x09), .c(x01), .d(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n67_), .O(z11));
  nand2  g42(.a(new_n31_), .b(x00), .O(new_n72_));
  nand3  g43(.a(x12), .b(x11), .c(new_n50_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n72_), .c(new_n43_), .O(z12));
endmodule


