// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n46_, new_n47_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x03), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nand2  g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x11), .O(new_n38_));
  nor2   g09(.a(new_n32_), .b(new_n31_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n38_), .c(new_n34_), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n39_), .c(x09), .d(new_n34_), .O(z03));
  nand2  g13(.a(x12), .b(x03), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(new_n43_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n43_), .O(z06));
  aoi21  g19(.a(x12), .b(x03), .c(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand4  g21(.a(new_n43_), .b(x11), .c(new_n50_), .d(x00), .O(z08));
  xnor2a g22(.a(x12), .b(x11), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n50_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n43_), .O(z09));
  nand4  g25(.a(x11), .b(new_n50_), .c(x09), .d(x00), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand3  g27(.a(x12), .b(x09), .c(x01), .O(new_n57_));
  oai21  g28(.a(x12), .b(x01), .c(new_n57_), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(x11), .c(new_n50_), .d(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n43_), .O(z11));
  inv1   g31(.a(x09), .O(new_n61_));
  nand4  g32(.a(new_n50_), .b(new_n61_), .c(new_n34_), .d(x00), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n32_), .c(new_n31_), .O(z12));
endmodule


