// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n46_, new_n47_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x04), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x04), .c(x12), .O(z01));
  nand2  g05(.a(x12), .b(x11), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  inv1   g07(.a(x04), .O(new_n37_));
  nand4  g08(.a(x07), .b(x06), .c(x05), .d(new_n37_), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(new_n36_), .c(x09), .d(x08), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n36_), .c(x09), .d(new_n37_), .O(z03));
  nand2  g13(.a(x12), .b(x04), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(new_n43_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n43_), .O(z06));
  aoi21  g19(.a(x12), .b(x04), .c(x15), .O(z07));
  nor2   g20(.a(new_n33_), .b(x10), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n43_), .c(x00), .O(z08));
  inv1   g22(.a(x10), .O(new_n52_));
  nand2  g23(.a(new_n30_), .b(new_n33_), .O(new_n53_));
  oai21  g24(.a(new_n35_), .b(x04), .c(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(z09));
  nand3  g27(.a(new_n50_), .b(x09), .c(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n37_), .c(new_n30_), .O(z10));
  nand4  g29(.a(x12), .b(x09), .c(new_n37_), .d(x01), .O(new_n59_));
  inv1   g30(.a(x01), .O(new_n60_));
  nand2  g31(.a(new_n30_), .b(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x11), .c(new_n52_), .d(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z11));
  inv1   g35(.a(x09), .O(new_n65_));
  nand3  g36(.a(new_n50_), .b(new_n65_), .c(x00), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n37_), .c(new_n30_), .O(z12));
endmodule


