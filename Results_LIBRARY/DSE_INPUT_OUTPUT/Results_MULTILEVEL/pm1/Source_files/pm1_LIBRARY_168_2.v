// Benchmark "FAU" written by ABC on Fri Aug 14 01:53:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x00), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(x09), .b(x08), .c(x07), .O(new_n36_));
  nor3   g07(.a(new_n36_), .b(new_n35_), .c(new_n33_), .O(new_n37_));
  inv1   g08(.a(x00), .O(new_n38_));
  nand2  g09(.a(new_n30_), .b(new_n38_), .O(new_n39_));
  oai21  g10(.a(new_n37_), .b(new_n30_), .c(new_n39_), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g13(.a(new_n30_), .b(x00), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  aoi21  g15(.a(new_n30_), .b(x00), .c(x13), .O(z05));
  nand4  g16(.a(z01), .b(x04), .c(x03), .d(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n43_), .O(z06));
  nand2  g19(.a(new_n43_), .b(x15), .O(z07));
  nand4  g20(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand2  g23(.a(x11), .b(new_n52_), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n51_), .c(x12), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x00), .O(z08));
  aoi21  g26(.a(new_n53_), .b(x12), .c(new_n38_), .O(z09));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x11), .c(new_n52_), .d(x09), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(x12), .c(new_n38_), .O(z10));
  nand4  g30(.a(new_n57_), .b(x12), .c(x11), .d(new_n52_), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x09), .c(x01), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z11));
  inv1   g34(.a(x09), .O(new_n64_));
  nand3  g35(.a(x11), .b(new_n52_), .c(new_n64_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x12), .c(new_n38_), .O(z12));
endmodule


