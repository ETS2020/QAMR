// Benchmark "FAU" written by ABC on Fri Aug 14 01:52:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nand3  g06(.a(x07), .b(x06), .c(x05), .O(new_n36_));
  nand3  g07(.a(x11), .b(x09), .c(x08), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nor2   g10(.a(new_n32_), .b(new_n31_), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n40_), .c(new_n34_), .d(x09), .O(z03));
  nor2   g13(.a(new_n32_), .b(new_n34_), .O(new_n43_));
  nor2   g14(.a(new_n43_), .b(x14), .O(z04));
  inv1   g15(.a(new_n43_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  nand3  g17(.a(new_n31_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n49_), .O(z06));
  nor2   g23(.a(new_n43_), .b(x15), .O(z07));
  inv1   g24(.a(x09), .O(new_n54_));
  nor2   g25(.a(new_n32_), .b(new_n54_), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(x04), .c(x03), .d(x02), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n34_), .d(x00), .O(z08));
  nor3   g28(.a(x12), .b(x11), .c(x10), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n40_), .c(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n45_), .O(z09));
  inv1   g31(.a(x00), .O(new_n61_));
  nand4  g32(.a(new_n50_), .b(x12), .c(x11), .d(new_n34_), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n54_), .c(new_n61_), .O(z10));
  nand4  g34(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nand3  g35(.a(new_n32_), .b(new_n34_), .c(new_n30_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(x11), .c(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n45_), .O(z11));
  nand3  g39(.a(x11), .b(new_n54_), .c(x00), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n34_), .c(new_n32_), .O(z12));
endmodule


