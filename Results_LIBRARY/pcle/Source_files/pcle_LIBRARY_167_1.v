// Benchmark "FAU" written by ABC on Thu Jun 25 16:30:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  nand4  g00(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  nor4   g02(.a(new_n30_), .b(new_n29_), .c(x10), .d(x08), .O(z0));
  nand2  g03(.a(x08), .b(x00), .O(new_n32_));
  inv1   g04(.a(x11), .O(new_n33_));
  nor2   g05(.a(x10), .b(x08), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n33_), .c(x09), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n32_), .O(z1));
  xnor2a g08(.a(x12), .b(x11), .O(new_n37_));
  nand2  g09(.a(new_n34_), .b(x09), .O(new_n38_));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  oai21  g11(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(z2));
  nand2  g12(.a(x12), .b(x11), .O(new_n41_));
  xor2a  g13(.a(new_n41_), .b(x13), .O(new_n42_));
  nand2  g14(.a(x08), .b(x02), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(new_n38_), .c(new_n43_), .O(z3));
  inv1   g16(.a(new_n41_), .O(new_n45_));
  nand3  g17(.a(x13), .b(x12), .c(x11), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  nor2   g19(.a(x14), .b(new_n47_), .O(new_n48_));
  aoi22  g20(.a(new_n48_), .b(new_n45_), .c(new_n46_), .d(x14), .O(new_n49_));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n38_), .c(new_n50_), .O(z4));
  nand2  g23(.a(x14), .b(x13), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x15), .O(new_n53_));
  aoi22  g25(.a(new_n53_), .b(new_n45_), .c(new_n30_), .d(x15), .O(new_n54_));
  nand2  g26(.a(x08), .b(x04), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n38_), .c(new_n55_), .O(z5));
  inv1   g28(.a(new_n38_), .O(new_n57_));
  inv1   g29(.a(x16), .O(new_n58_));
  nand2  g30(.a(x15), .b(x14), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  inv1   g33(.a(new_n61_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  nor3   g35(.a(new_n59_), .b(new_n46_), .c(x16), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n63_), .c(new_n57_), .O(new_n65_));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(z6));
  nand3  g39(.a(x16), .b(x15), .c(x14), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n46_), .c(x17), .O(new_n69_));
  inv1   g41(.a(x17), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(x16), .c(x15), .d(x14), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n46_), .c(new_n69_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n57_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x06), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z7));
  nand4  g47(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n46_), .c(x18), .O(new_n77_));
  nand2  g49(.a(x17), .b(x16), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(x18), .O(new_n79_));
  nand4  g51(.a(x13), .b(x12), .c(x11), .d(x09), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n79_), .c(new_n60_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n34_), .O(new_n84_));
  nand2  g56(.a(x08), .b(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


