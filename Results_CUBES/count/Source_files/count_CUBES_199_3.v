// Benchmark "FAU" written by ABC on Mon Jul  6 14:18:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x17), .O(new_n60_));
  nor2   g009(.a(x20), .b(x19), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n59_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n52_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  aoi22  g017(.a(new_n68_), .b(new_n58_), .c(new_n62_), .d(x21), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n52_), .c(new_n71_), .O(z02));
  nand2  g021(.a(x22), .b(x21), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n61_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n73_), .c(x17), .O(new_n76_));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n63_), .b(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n52_), .b(new_n80_), .c(x18), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(z03));
  nand3  g031(.a(new_n74_), .b(new_n61_), .c(new_n60_), .O(new_n83_));
  inv1   g032(.a(x21), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  aoi21  g036(.a(new_n83_), .b(x23), .c(new_n87_), .O(new_n88_));
  inv1   g037(.a(x11), .O(new_n89_));
  aoi21  g038(.a(new_n52_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n52_), .c(new_n90_), .O(z04));
  inv1   g040(.a(new_n86_), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n61_), .c(new_n60_), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n74_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi22  g045(.a(new_n96_), .b(new_n63_), .c(new_n93_), .d(x24), .O(new_n97_));
  inv1   g046(.a(x10), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n52_), .c(new_n99_), .O(z05));
  nand3  g049(.a(new_n96_), .b(new_n61_), .c(new_n60_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x25), .O(new_n102_));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n85_), .c(new_n68_), .d(new_n58_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g055(.a(x09), .O(new_n107_));
  aoi21  g056(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z06));
  inv1   g058(.a(x23), .O(new_n110_));
  nand3  g059(.a(new_n103_), .b(new_n74_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n60_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n63_), .O(new_n113_));
  nor3   g062(.a(x26), .b(x25), .c(x24), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nand3  g064(.a(new_n85_), .b(new_n68_), .c(new_n58_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  aoi21  g066(.a(new_n113_), .b(x26), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x08), .O(new_n119_));
  aoi21  g068(.a(new_n52_), .b(new_n119_), .c(x18), .O(new_n120_));
  oai21  g069(.a(new_n118_), .b(new_n52_), .c(new_n120_), .O(z07));
  nand4  g070(.a(new_n114_), .b(new_n92_), .c(new_n61_), .d(new_n60_), .O(new_n122_));
  inv1   g071(.a(x25), .O(new_n123_));
  nor2   g072(.a(x27), .b(x26), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g074(.a(new_n58_), .b(new_n57_), .O(new_n126_));
  nor3   g075(.a(new_n126_), .b(new_n125_), .c(new_n95_), .O(new_n127_));
  aoi21  g076(.a(new_n122_), .b(x27), .c(new_n127_), .O(new_n128_));
  inv1   g077(.a(x07), .O(new_n129_));
  aoi21  g078(.a(new_n52_), .b(new_n129_), .c(x18), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n52_), .c(new_n130_), .O(z08));
  nand3  g080(.a(new_n103_), .b(new_n85_), .c(new_n68_), .O(new_n132_));
  inv1   g081(.a(x28), .O(new_n133_));
  inv1   g082(.a(x29), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x19), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n124_), .c(new_n133_), .O(new_n136_));
  nand4  g085(.a(new_n124_), .b(new_n103_), .c(new_n85_), .d(new_n84_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x28), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n132_), .c(new_n138_), .O(new_n139_));
  nor2   g088(.a(new_n63_), .b(new_n133_), .O(new_n140_));
  aoi21  g089(.a(new_n139_), .b(new_n60_), .c(new_n140_), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n52_), .b(new_n142_), .c(x18), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n52_), .c(new_n143_), .O(z09));
  nor3   g093(.a(x28), .b(x27), .c(x26), .O(new_n145_));
  nor2   g094(.a(x20), .b(x17), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n103_), .d(new_n92_), .O(new_n147_));
  inv1   g096(.a(x27), .O(new_n148_));
  nor2   g097(.a(x29), .b(x28), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n115_), .O(new_n151_));
  aoi22  g100(.a(new_n151_), .b(new_n87_), .c(new_n147_), .d(x29), .O(new_n152_));
  inv1   g101(.a(x05), .O(new_n153_));
  aoi21  g102(.a(new_n52_), .b(new_n153_), .c(x18), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n52_), .c(new_n154_), .O(z10));
  inv1   g104(.a(x30), .O(new_n156_));
  nor2   g105(.a(x26), .b(x25), .O(new_n157_));
  nor2   g106(.a(x28), .b(x27), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n95_), .c(new_n60_), .O(new_n160_));
  nand3  g109(.a(new_n58_), .b(new_n134_), .c(new_n57_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n160_), .c(new_n156_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n158_), .c(new_n114_), .O(new_n165_));
  nor3   g114(.a(new_n165_), .b(new_n86_), .c(new_n62_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  inv1   g116(.a(x04), .O(new_n168_));
  aoi21  g117(.a(new_n52_), .b(new_n168_), .c(x18), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(z11));
  inv1   g119(.a(x31), .O(new_n171_));
  nand2  g120(.a(new_n158_), .b(new_n156_), .O(new_n172_));
  nand3  g121(.a(new_n157_), .b(new_n94_), .c(new_n74_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n172_), .c(new_n60_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n162_), .c(new_n171_), .O(new_n175_));
  nand2  g124(.a(new_n96_), .b(new_n63_), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n149_), .c(new_n124_), .d(new_n123_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n175_), .c(x16), .O(new_n180_));
  inv1   g129(.a(x03), .O(new_n181_));
  aoi21  g130(.a(new_n52_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z12));
  inv1   g132(.a(x32), .O(new_n184_));
  nand3  g133(.a(new_n177_), .b(new_n124_), .c(new_n133_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n111_), .c(new_n60_), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n162_), .c(new_n184_), .O(new_n187_));
  nor2   g136(.a(x32), .b(x31), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n164_), .c(new_n158_), .d(new_n157_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n176_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n187_), .c(x16), .O(new_n191_));
  inv1   g140(.a(x02), .O(new_n192_));
  aoi21  g141(.a(new_n52_), .b(new_n192_), .c(x18), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n191_), .O(z13));
  inv1   g143(.a(x33), .O(new_n195_));
  inv1   g144(.a(x26), .O(new_n196_));
  nand4  g145(.a(new_n188_), .b(new_n158_), .c(new_n156_), .d(new_n196_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n111_), .c(new_n60_), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n162_), .c(new_n195_), .O(new_n199_));
  nand2  g148(.a(new_n149_), .b(new_n124_), .O(new_n200_));
  nor2   g149(.a(x33), .b(x32), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n177_), .O(new_n202_));
  nor3   g151(.a(new_n202_), .b(new_n200_), .c(new_n104_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n199_), .c(x16), .O(new_n204_));
  inv1   g153(.a(x01), .O(new_n205_));
  aoi21  g154(.a(new_n52_), .b(new_n205_), .c(x18), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n204_), .O(z14));
  inv1   g156(.a(x34), .O(new_n208_));
  inv1   g157(.a(x24), .O(new_n209_));
  nand4  g158(.a(new_n157_), .b(new_n85_), .c(new_n209_), .d(new_n84_), .O(new_n210_));
  nand4  g159(.a(new_n201_), .b(new_n158_), .c(new_n171_), .d(new_n156_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n210_), .c(new_n60_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n162_), .c(new_n208_), .O(new_n213_));
  nand3  g162(.a(new_n201_), .b(new_n177_), .c(new_n208_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n200_), .c(new_n104_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n213_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n52_), .b(new_n217_), .c(x18), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(z15));
endmodule


