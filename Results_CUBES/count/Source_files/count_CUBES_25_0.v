// Benchmark "FAU" written by ABC on Mon Jul  6 14:17:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g013(.a(new_n58_), .b(new_n57_), .O(new_n65_));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  aoi22  g015(.a(new_n66_), .b(new_n58_), .c(new_n65_), .d(x21), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  inv1   g019(.a(x22), .O(new_n71_));
  aoi21  g020(.a(new_n66_), .b(new_n58_), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n58_), .c(new_n57_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n72_), .c(x16), .O(new_n76_));
  inv1   g025(.a(x12), .O(new_n77_));
  aoi21  g026(.a(new_n52_), .b(new_n77_), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n76_), .O(z03));
  nor3   g028(.a(x23), .b(x22), .c(x21), .O(new_n80_));
  aoi22  g029(.a(new_n80_), .b(new_n60_), .c(new_n74_), .d(x23), .O(new_n81_));
  inv1   g030(.a(x11), .O(new_n82_));
  aoi21  g031(.a(new_n52_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n52_), .c(new_n83_), .O(z04));
  nand2  g033(.a(new_n80_), .b(new_n60_), .O(new_n85_));
  nor2   g034(.a(x24), .b(x23), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n73_), .c(new_n58_), .d(new_n57_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n85_), .b(x24), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x10), .O(new_n90_));
  aoi21  g039(.a(new_n52_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n52_), .c(new_n91_), .O(z05));
  nand2  g041(.a(new_n87_), .b(x25), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n94_), .c(new_n66_), .d(new_n58_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x16), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n52_), .b(new_n99_), .c(x18), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n98_), .O(z06));
  nand2  g050(.a(new_n96_), .b(x26), .O(new_n102_));
  nor3   g051(.a(x26), .b(x25), .c(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n94_), .c(new_n66_), .d(new_n58_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g055(.a(x08), .O(new_n107_));
  aoi21  g056(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z07));
  inv1   g058(.a(x25), .O(new_n110_));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n86_), .c(new_n110_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n74_), .O(new_n113_));
  aoi21  g062(.a(new_n104_), .b(x27), .c(new_n113_), .O(new_n114_));
  inv1   g063(.a(x07), .O(new_n115_));
  aoi21  g064(.a(new_n52_), .b(new_n115_), .c(x18), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n52_), .c(new_n116_), .O(z08));
  aoi21  g066(.a(new_n103_), .b(new_n80_), .c(x19), .O(new_n118_));
  inv1   g067(.a(x27), .O(new_n119_));
  nand3  g068(.a(new_n58_), .b(new_n119_), .c(new_n57_), .O(new_n120_));
  inv1   g069(.a(x28), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nor3   g072(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  nor3   g073(.a(x28), .b(x27), .c(x26), .O(new_n125_));
  and2   g074(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  inv1   g075(.a(x18), .O(new_n127_));
  oai21  g076(.a(x16), .b(x06), .c(new_n127_), .O(new_n128_));
  aoi21  g077(.a(new_n126_), .b(new_n75_), .c(new_n128_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n123_), .O(z09));
  nand2  g079(.a(x29), .b(x27), .O(new_n131_));
  inv1   g080(.a(x26), .O(new_n132_));
  nor2   g081(.a(x29), .b(x27), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n124_), .c(new_n132_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n74_), .c(new_n131_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n121_), .O(new_n136_));
  nor3   g085(.a(x28), .b(x26), .c(x25), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n86_), .c(new_n75_), .O(new_n138_));
  and2   g087(.a(x29), .b(x16), .O(new_n139_));
  oai21  g088(.a(x16), .b(x05), .c(new_n127_), .O(new_n140_));
  aoi21  g089(.a(new_n139_), .b(new_n138_), .c(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n136_), .O(z10));
  nand2  g091(.a(x30), .b(x27), .O(new_n143_));
  nor2   g092(.a(x30), .b(x29), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n103_), .c(new_n119_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n85_), .c(new_n143_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n121_), .O(new_n147_));
  nor3   g096(.a(x29), .b(x28), .c(x26), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n124_), .c(new_n75_), .O(new_n149_));
  and2   g098(.a(x30), .b(x16), .O(new_n150_));
  oai21  g099(.a(x16), .b(x04), .c(new_n127_), .O(new_n151_));
  aoi21  g100(.a(new_n150_), .b(new_n149_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n147_), .O(z11));
  nand2  g102(.a(x31), .b(x27), .O(new_n154_));
  nor2   g103(.a(x31), .b(x30), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n133_), .c(new_n103_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n85_), .c(new_n154_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n121_), .O(new_n158_));
  nor3   g107(.a(x30), .b(x29), .c(x28), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n103_), .c(new_n80_), .d(new_n60_), .O(new_n160_));
  and2   g109(.a(x31), .b(x16), .O(new_n161_));
  oai21  g110(.a(x16), .b(x03), .c(new_n127_), .O(new_n162_));
  aoi21  g111(.a(new_n161_), .b(new_n160_), .c(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n158_), .O(z12));
  inv1   g113(.a(x32), .O(new_n165_));
  nor2   g114(.a(x32), .b(x31), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n144_), .c(new_n111_), .d(new_n110_), .O(new_n167_));
  oai22  g116(.a(new_n167_), .b(new_n87_), .c(new_n165_), .d(new_n119_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n121_), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nand3  g119(.a(new_n155_), .b(new_n170_), .c(new_n103_), .O(new_n171_));
  nor2   g120(.a(new_n165_), .b(new_n52_), .O(new_n172_));
  oai21  g121(.a(new_n171_), .b(new_n85_), .c(new_n172_), .O(new_n173_));
  inv1   g122(.a(x02), .O(new_n174_));
  aoi21  g123(.a(new_n52_), .b(new_n174_), .c(x18), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n173_), .c(new_n169_), .O(z13));
  inv1   g125(.a(x33), .O(new_n177_));
  nor2   g126(.a(x26), .b(x25), .O(new_n178_));
  nor2   g127(.a(x33), .b(x32), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n155_), .c(new_n133_), .d(new_n178_), .O(new_n180_));
  oai22  g129(.a(new_n180_), .b(new_n87_), .c(new_n177_), .d(new_n119_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n121_), .O(new_n182_));
  nand3  g131(.a(new_n166_), .b(new_n144_), .c(new_n137_), .O(new_n183_));
  nor2   g132(.a(new_n177_), .b(new_n52_), .O(new_n184_));
  oai21  g133(.a(new_n183_), .b(new_n87_), .c(new_n184_), .O(new_n185_));
  inv1   g134(.a(x01), .O(new_n186_));
  aoi21  g135(.a(new_n52_), .b(new_n186_), .c(x18), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n185_), .c(new_n182_), .O(z14));
  inv1   g137(.a(x34), .O(new_n189_));
  nor2   g138(.a(x34), .b(x33), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n166_), .c(new_n144_), .d(new_n111_), .O(new_n191_));
  oai22  g140(.a(new_n191_), .b(new_n96_), .c(new_n189_), .d(new_n119_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n121_), .O(new_n193_));
  nand4  g142(.a(new_n179_), .b(new_n155_), .c(new_n170_), .d(new_n178_), .O(new_n194_));
  nor2   g143(.a(new_n189_), .b(new_n52_), .O(new_n195_));
  oai21  g144(.a(new_n194_), .b(new_n87_), .c(new_n195_), .O(new_n196_));
  inv1   g145(.a(x00), .O(new_n197_));
  aoi21  g146(.a(new_n52_), .b(new_n197_), .c(x18), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n196_), .c(new_n193_), .O(z15));
endmodule


