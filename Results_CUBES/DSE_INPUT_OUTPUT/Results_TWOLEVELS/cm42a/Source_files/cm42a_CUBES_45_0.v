// Benchmark "FAU" written by ABC on Thu Aug 20 10:56:22 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n17_, new_n18_, new_n20_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n32_;
  assign z0 = x2 | x3;
  assign new_n17_ = ~x1 & ~x3;
  assign new_n18_ = ~x0 & z0;
  assign z4 = ~new_n17_ | ~new_n18_;
  assign new_n20_ = x0 & z0;
  assign z5 = ~new_n17_ | ~new_n20_;
  assign new_n22_ = ~x0 & x1;
  assign new_n23_ = x2 & ~new_n22_;
  assign z6 = x3 | new_n23_;
  assign new_n25_ = x0 & x1;
  assign new_n26_ = x2 & ~new_n25_;
  assign z7 = x3 | new_n26_;
  assign new_n28_ = ~x0 & ~x1;
  assign new_n29_ = ~x1 & x2;
  assign new_n30_ = z0 & ~new_n29_;
  assign z8 = ~new_n28_ | ~new_n30_;
  assign new_n32_ = x0 & ~x1;
  assign z9 = ~new_n30_ | ~new_n32_;
  assign z1 = 1'b1;
  assign z2 = 1'b1;
  assign z3 = z0;
endmodule


