// Benchmark "FAU" written by ABC on Wed Jul  8 19:31:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n76_, new_n77_, new_n78_, new_n81_, new_n82_, new_n84_, new_n85_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x25), .O(new_n48_));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g09(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(x23), .c(x24), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  xor2a  g16(.a(x11), .b(x02), .O(new_n64_));
  xor2a  g17(.a(x12), .b(x03), .O(new_n65_));
  nor2   g18(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g19(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g20(.a(x10), .b(x01), .O(new_n68_));
  nor2   g21(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g22(.a(new_n69_), .b(new_n66_), .c(x08), .O(z04));
  inv1   g23(.a(x14), .O(new_n72_));
  nor2   g24(.a(new_n72_), .b(x08), .O(z06));
  nand2  g25(.a(new_n46_), .b(x06), .O(z07));
  inv1   g26(.a(x07), .O(new_n76_));
  inv1   g27(.a(x15), .O(new_n77_));
  nand3  g28(.a(new_n59_), .b(new_n77_), .c(new_n76_), .O(new_n78_));
  inv1   g29(.a(new_n78_), .O(z09));
  xnor2a g30(.a(x18), .b(x17), .O(new_n81_));
  nand3  g31(.a(new_n58_), .b(new_n77_), .c(new_n76_), .O(new_n82_));
  nor2   g32(.a(new_n82_), .b(new_n81_), .O(z11));
  oai21  g33(.a(new_n51_), .b(new_n50_), .c(x19), .O(new_n84_));
  nand3  g34(.a(new_n52_), .b(x18), .c(x17), .O(new_n85_));
  aoi21  g35(.a(new_n85_), .b(new_n84_), .c(new_n82_), .O(z12));
  zero   g36(.O(z02));
  zero   g37(.O(z03));
  zero   g38(.O(z05));
  zero   g39(.O(z08));
  zero   g40(.O(z10));
  zero   g41(.O(z13));
  zero   g42(.O(z14));
  zero   g43(.O(z15));
  zero   g44(.O(z16));
  zero   g45(.O(z17));
  zero   g46(.O(z18));
endmodule


