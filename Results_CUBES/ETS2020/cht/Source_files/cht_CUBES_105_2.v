// Benchmark "FAU" written by ABC on Mon Jul  6 19:22:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n95_, new_n96_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n108_, new_n109_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n122_, new_n123_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n138_, new_n139_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n162_, new_n163_;
  nand2  g00(.a(x07), .b(x04), .O(new_n84_));
  inv1   g01(.a(x07), .O(new_n85_));
  nand2  g02(.a(x11), .b(new_n85_), .O(new_n86_));
  aoi21  g03(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g04(.a(x07), .b(x05), .O(new_n88_));
  nand2  g05(.a(x12), .b(new_n85_), .O(new_n89_));
  aoi21  g06(.a(new_n89_), .b(new_n88_), .c(x10), .O(z01));
  nand2  g07(.a(x07), .b(x06), .O(new_n91_));
  nand2  g08(.a(x13), .b(new_n85_), .O(new_n92_));
  aoi21  g09(.a(new_n92_), .b(new_n91_), .c(x10), .O(z02));
  nand2  g10(.a(x07), .b(x02), .O(new_n95_));
  nand2  g11(.a(x15), .b(new_n85_), .O(new_n96_));
  aoi21  g12(.a(new_n96_), .b(new_n95_), .c(x10), .O(z04));
  inv1   g13(.a(x08), .O(new_n100_));
  nand2  g14(.a(x18), .b(new_n100_), .O(new_n101_));
  nand2  g15(.a(x19), .b(x08), .O(new_n102_));
  aoi21  g16(.a(new_n102_), .b(new_n101_), .c(x10), .O(z07));
  nand2  g17(.a(x19), .b(new_n100_), .O(new_n104_));
  nand2  g18(.a(x20), .b(x08), .O(new_n105_));
  aoi21  g19(.a(new_n105_), .b(new_n104_), .c(x10), .O(z08));
  nand2  g20(.a(x21), .b(new_n100_), .O(new_n108_));
  nand2  g21(.a(x22), .b(x08), .O(new_n109_));
  aoi21  g22(.a(new_n109_), .b(new_n108_), .c(x10), .O(z10));
  nand2  g23(.a(x25), .b(new_n100_), .O(new_n114_));
  nand2  g24(.a(x26), .b(x08), .O(new_n115_));
  aoi21  g25(.a(new_n115_), .b(new_n114_), .c(x10), .O(z14));
  nand2  g26(.a(x27), .b(new_n100_), .O(new_n118_));
  nand2  g27(.a(x28), .b(x08), .O(new_n119_));
  aoi21  g28(.a(new_n119_), .b(new_n118_), .c(x10), .O(z16));
  nand2  g29(.a(x29), .b(new_n100_), .O(new_n122_));
  nand2  g30(.a(x30), .b(x08), .O(new_n123_));
  aoi21  g31(.a(new_n123_), .b(new_n122_), .c(x10), .O(z18));
  inv1   g32(.a(x09), .O(new_n126_));
  nand2  g33(.a(x31), .b(new_n126_), .O(new_n127_));
  nand2  g34(.a(x32), .b(x09), .O(new_n128_));
  aoi21  g35(.a(new_n128_), .b(new_n127_), .c(x10), .O(z20));
  nand2  g36(.a(x32), .b(new_n126_), .O(new_n130_));
  nand2  g37(.a(x33), .b(x09), .O(new_n131_));
  aoi21  g38(.a(new_n131_), .b(new_n130_), .c(x10), .O(z21));
  nand2  g39(.a(x33), .b(new_n126_), .O(new_n133_));
  nand2  g40(.a(x34), .b(x09), .O(new_n134_));
  aoi21  g41(.a(new_n134_), .b(new_n133_), .c(x10), .O(z22));
  nand2  g42(.a(x36), .b(new_n126_), .O(new_n138_));
  nand2  g43(.a(x37), .b(x09), .O(new_n139_));
  aoi21  g44(.a(new_n139_), .b(new_n138_), .c(x10), .O(z25));
  inv1   g45(.a(x39), .O(new_n142_));
  nand2  g46(.a(x14), .b(x00), .O(new_n143_));
  oai21  g47(.a(new_n142_), .b(x14), .c(new_n143_), .O(new_n144_));
  nand2  g48(.a(new_n144_), .b(x09), .O(new_n145_));
  nand2  g49(.a(x38), .b(new_n126_), .O(new_n146_));
  aoi21  g50(.a(new_n146_), .b(new_n145_), .c(x10), .O(z27));
  inv1   g51(.a(x14), .O(new_n149_));
  nand2  g52(.a(new_n149_), .b(x09), .O(new_n150_));
  nand2  g53(.a(new_n150_), .b(x40), .O(new_n151_));
  nor2   g54(.a(x14), .b(new_n126_), .O(new_n152_));
  nand2  g55(.a(new_n152_), .b(x41), .O(new_n153_));
  aoi21  g56(.a(new_n153_), .b(new_n151_), .c(x10), .O(z29));
  nand2  g57(.a(new_n150_), .b(x41), .O(new_n155_));
  nand2  g58(.a(new_n152_), .b(x42), .O(new_n156_));
  aoi21  g59(.a(new_n156_), .b(new_n155_), .c(x10), .O(z30));
  nand2  g60(.a(new_n150_), .b(x46), .O(new_n162_));
  nand2  g61(.a(new_n152_), .b(x00), .O(new_n163_));
  aoi21  g62(.a(new_n163_), .b(new_n162_), .c(x10), .O(z35));
  zero   g63(.O(z03));
  zero   g64(.O(z05));
  zero   g65(.O(z06));
  zero   g66(.O(z09));
  zero   g67(.O(z11));
  zero   g68(.O(z12));
  zero   g69(.O(z13));
  zero   g70(.O(z15));
  zero   g71(.O(z17));
  zero   g72(.O(z19));
  zero   g73(.O(z23));
  zero   g74(.O(z24));
  zero   g75(.O(z26));
  zero   g76(.O(z28));
  zero   g77(.O(z31));
  zero   g78(.O(z32));
  zero   g79(.O(z33));
  zero   g80(.O(z34));
endmodule


