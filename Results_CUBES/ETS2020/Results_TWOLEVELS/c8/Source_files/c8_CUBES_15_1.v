// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x11), .O(new_n53_));
  nand2  g05(.a(x27), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(x27), .b(x22), .c(new_n54_), .O(z03));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n48_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n48_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x14), .O(new_n62_));
  nand2  g14(.a(x27), .b(new_n62_), .O(new_n63_));
  oai21  g15(.a(x27), .b(x25), .c(new_n63_), .O(z06));
  inv1   g16(.a(x15), .O(new_n65_));
  nand2  g17(.a(x27), .b(new_n65_), .O(new_n66_));
  oai21  g18(.a(x27), .b(x26), .c(new_n66_), .O(z07));
  and2   g19(.a(x19), .b(x17), .O(new_n69_));
  nor2   g20(.a(x19), .b(x17), .O(new_n70_));
  oai21  g21(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  inv1   g22(.a(x08), .O(new_n72_));
  nor2   g23(.a(x18), .b(new_n72_), .O(new_n73_));
  aoi21  g24(.a(x18), .b(x00), .c(new_n73_), .O(new_n74_));
  oai21  g25(.a(new_n74_), .b(x16), .c(new_n71_), .O(z09));
  inv1   g26(.a(x09), .O(new_n76_));
  nor2   g27(.a(x18), .b(new_n76_), .O(new_n77_));
  aoi21  g28(.a(x18), .b(x01), .c(new_n77_), .O(new_n78_));
  nor2   g29(.a(new_n70_), .b(new_n49_), .O(new_n79_));
  nor3   g30(.a(x20), .b(x19), .c(x17), .O(new_n80_));
  oai21  g31(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  oai21  g32(.a(new_n78_), .b(x16), .c(new_n81_), .O(z10));
  inv1   g33(.a(x16), .O(new_n83_));
  inv1   g34(.a(x10), .O(new_n84_));
  nand2  g35(.a(x18), .b(x02), .O(new_n85_));
  oai21  g36(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g37(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand2  g38(.a(new_n70_), .b(new_n49_), .O(new_n88_));
  nor2   g39(.a(x21), .b(x20), .O(new_n89_));
  aoi22  g40(.a(new_n89_), .b(new_n70_), .c(new_n88_), .d(x21), .O(new_n90_));
  oai21  g41(.a(new_n90_), .b(new_n83_), .c(new_n87_), .O(z11));
  nand2  g42(.a(x18), .b(x03), .O(new_n92_));
  oai21  g43(.a(x18), .b(new_n53_), .c(new_n92_), .O(new_n93_));
  nand2  g44(.a(new_n93_), .b(new_n83_), .O(new_n94_));
  nand2  g45(.a(new_n89_), .b(new_n70_), .O(new_n95_));
  nor2   g46(.a(x22), .b(x21), .O(new_n96_));
  aoi22  g47(.a(new_n96_), .b(new_n80_), .c(new_n95_), .d(x22), .O(new_n97_));
  oai21  g48(.a(new_n97_), .b(new_n83_), .c(new_n94_), .O(z12));
  nand2  g49(.a(x18), .b(x06), .O(new_n101_));
  oai21  g50(.a(x18), .b(new_n62_), .c(new_n101_), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  nor2   g52(.a(x24), .b(x23), .O(new_n104_));
  nand4  g53(.a(new_n104_), .b(new_n96_), .c(new_n70_), .d(new_n49_), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(x25), .O(new_n106_));
  nor2   g55(.a(x23), .b(x22), .O(new_n107_));
  nor2   g56(.a(x25), .b(x24), .O(new_n108_));
  nand4  g57(.a(new_n108_), .b(new_n107_), .c(new_n89_), .d(new_n70_), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(x16), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(new_n103_), .O(z15));
  nand2  g61(.a(x18), .b(x07), .O(new_n113_));
  oai21  g62(.a(x18), .b(new_n65_), .c(new_n113_), .O(new_n114_));
  nand2  g63(.a(new_n114_), .b(new_n83_), .O(new_n115_));
  nand2  g64(.a(new_n109_), .b(x26), .O(new_n116_));
  nor3   g65(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand4  g66(.a(new_n117_), .b(new_n107_), .c(new_n89_), .d(new_n70_), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(x16), .O(new_n120_));
  nand2  g69(.a(new_n120_), .b(new_n115_), .O(z16));
  inv1   g70(.a(x19), .O(new_n122_));
  nor2   g71(.a(new_n122_), .b(x17), .O(new_n123_));
  nand4  g72(.a(new_n123_), .b(new_n117_), .c(new_n107_), .d(new_n89_), .O(new_n124_));
  nand2  g73(.a(x27), .b(x17), .O(new_n125_));
  aoi21  g74(.a(new_n125_), .b(new_n124_), .c(new_n83_), .O(z17));
  zero   g75(.O(z00));
  zero   g76(.O(z02));
  zero   g77(.O(z08));
  zero   g78(.O(z13));
  zero   g79(.O(z14));
endmodule


