// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x10), .O(new_n52_));
  nand2  g05(.a(x27), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(x27), .b(x21), .c(new_n53_), .O(z02));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n47_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  inv1   g13(.a(x09), .O(new_n65_));
  nor2   g14(.a(x18), .b(new_n65_), .O(new_n66_));
  aoi21  g15(.a(x18), .b(x01), .c(new_n66_), .O(new_n67_));
  inv1   g16(.a(x20), .O(new_n68_));
  nor2   g17(.a(x19), .b(x17), .O(new_n69_));
  nor2   g18(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor3   g19(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g20(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  oai21  g21(.a(new_n67_), .b(x16), .c(new_n72_), .O(z10));
  inv1   g22(.a(x16), .O(new_n74_));
  nand2  g23(.a(x18), .b(x02), .O(new_n75_));
  oai21  g24(.a(x18), .b(new_n52_), .c(new_n75_), .O(new_n76_));
  nand2  g25(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g26(.a(new_n69_), .b(new_n68_), .O(new_n78_));
  nor2   g27(.a(x21), .b(x20), .O(new_n79_));
  aoi22  g28(.a(new_n79_), .b(new_n69_), .c(new_n78_), .d(x21), .O(new_n80_));
  oai21  g29(.a(new_n80_), .b(new_n74_), .c(new_n77_), .O(z11));
  inv1   g30(.a(x12), .O(new_n83_));
  nand2  g31(.a(x18), .b(x04), .O(new_n84_));
  oai21  g32(.a(x18), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nand2  g33(.a(new_n85_), .b(new_n74_), .O(new_n86_));
  inv1   g34(.a(x22), .O(new_n87_));
  nand3  g35(.a(new_n79_), .b(new_n69_), .c(new_n87_), .O(new_n88_));
  nor3   g36(.a(x23), .b(x22), .c(x21), .O(new_n89_));
  aoi22  g37(.a(new_n89_), .b(new_n71_), .c(new_n88_), .d(x23), .O(new_n90_));
  oai21  g38(.a(new_n90_), .b(new_n74_), .c(new_n86_), .O(z13));
  nand2  g39(.a(x18), .b(x07), .O(new_n94_));
  oai21  g40(.a(x18), .b(new_n61_), .c(new_n94_), .O(new_n95_));
  nand2  g41(.a(new_n95_), .b(new_n74_), .O(new_n96_));
  nor2   g42(.a(x23), .b(x22), .O(new_n97_));
  nor2   g43(.a(x25), .b(x24), .O(new_n98_));
  nand4  g44(.a(new_n98_), .b(new_n97_), .c(new_n79_), .d(new_n69_), .O(new_n99_));
  nand2  g45(.a(new_n99_), .b(x26), .O(new_n100_));
  nor3   g46(.a(x26), .b(x25), .c(x24), .O(new_n101_));
  nand4  g47(.a(new_n101_), .b(new_n97_), .c(new_n79_), .d(new_n69_), .O(new_n102_));
  nand2  g48(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g49(.a(new_n103_), .b(x16), .O(new_n104_));
  nand2  g50(.a(new_n104_), .b(new_n96_), .O(z16));
  zero   g51(.O(z01));
  zero   g52(.O(z03));
  zero   g53(.O(z04));
  zero   g54(.O(z06));
  zero   g55(.O(z09));
  zero   g56(.O(z12));
  zero   g57(.O(z14));
  zero   g58(.O(z15));
  zero   g59(.O(z17));
  buf    g60(.a(x27), .O(z08));
endmodule


