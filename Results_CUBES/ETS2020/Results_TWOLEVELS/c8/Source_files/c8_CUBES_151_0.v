// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n50_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n50_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x11), .O(new_n55_));
  nand2  g08(.a(x27), .b(new_n55_), .O(new_n56_));
  oai21  g09(.a(x27), .b(x22), .c(new_n56_), .O(z03));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n50_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n50_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x25), .O(new_n62_));
  nand2  g14(.a(new_n50_), .b(new_n62_), .O(new_n63_));
  oai21  g15(.a(new_n50_), .b(x14), .c(new_n63_), .O(z06));
  and2   g16(.a(x19), .b(x17), .O(new_n67_));
  nor2   g17(.a(x19), .b(x17), .O(new_n68_));
  oai21  g18(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  nor2   g19(.a(x18), .b(new_n47_), .O(new_n70_));
  aoi21  g20(.a(x18), .b(x00), .c(new_n70_), .O(new_n71_));
  oai21  g21(.a(new_n71_), .b(x16), .c(new_n69_), .O(z09));
  inv1   g22(.a(x16), .O(new_n75_));
  nand2  g23(.a(x18), .b(x03), .O(new_n76_));
  oai21  g24(.a(x18), .b(new_n55_), .c(new_n76_), .O(new_n77_));
  nand2  g25(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nor2   g26(.a(x21), .b(x20), .O(new_n79_));
  nand2  g27(.a(new_n79_), .b(new_n68_), .O(new_n80_));
  nor3   g28(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  aoi22  g29(.a(new_n81_), .b(new_n68_), .c(new_n80_), .d(x22), .O(new_n82_));
  oai21  g30(.a(new_n82_), .b(new_n75_), .c(new_n78_), .O(z12));
  inv1   g31(.a(x15), .O(new_n87_));
  nand2  g32(.a(x18), .b(x07), .O(new_n88_));
  oai21  g33(.a(x18), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g34(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  nor2   g35(.a(x23), .b(x22), .O(new_n91_));
  nor2   g36(.a(x25), .b(x24), .O(new_n92_));
  nand4  g37(.a(new_n92_), .b(new_n91_), .c(new_n79_), .d(new_n68_), .O(new_n93_));
  nand2  g38(.a(new_n93_), .b(x26), .O(new_n94_));
  nor3   g39(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  nand4  g40(.a(new_n95_), .b(new_n91_), .c(new_n79_), .d(new_n68_), .O(new_n96_));
  nand2  g41(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand2  g42(.a(new_n97_), .b(x16), .O(new_n98_));
  nand2  g43(.a(new_n98_), .b(new_n90_), .O(z16));
  zero   g44(.O(z02));
  zero   g45(.O(z04));
  zero   g46(.O(z07));
  zero   g47(.O(z08));
  zero   g48(.O(z10));
  zero   g49(.O(z11));
  zero   g50(.O(z13));
  zero   g51(.O(z14));
  zero   g52(.O(z15));
  zero   g53(.O(z17));
endmodule


