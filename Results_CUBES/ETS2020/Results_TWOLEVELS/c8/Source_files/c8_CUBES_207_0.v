// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n65_, new_n66_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x21), .O(new_n54_));
  nand2  g08(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  oai21  g09(.a(new_n47_), .b(x10), .c(new_n55_), .O(z02));
  inv1   g10(.a(x12), .O(new_n58_));
  nand2  g11(.a(x27), .b(new_n58_), .O(new_n59_));
  oai21  g12(.a(x27), .b(x23), .c(new_n59_), .O(z04));
  inv1   g13(.a(x24), .O(new_n61_));
  nand2  g14(.a(new_n47_), .b(new_n61_), .O(new_n62_));
  oai21  g15(.a(new_n47_), .b(x13), .c(new_n62_), .O(z05));
  inv1   g16(.a(x15), .O(new_n65_));
  nand2  g17(.a(x27), .b(new_n65_), .O(new_n66_));
  oai21  g18(.a(x27), .b(x26), .c(new_n66_), .O(z07));
  inv1   g19(.a(x16), .O(new_n71_));
  inv1   g20(.a(x11), .O(new_n72_));
  nand2  g21(.a(x18), .b(x03), .O(new_n73_));
  oai21  g22(.a(x18), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g23(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nor2   g24(.a(x19), .b(x17), .O(new_n76_));
  nor2   g25(.a(x21), .b(x20), .O(new_n77_));
  nand2  g26(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g27(.a(x22), .b(x21), .O(new_n79_));
  nor3   g28(.a(x20), .b(x19), .c(x17), .O(new_n80_));
  aoi22  g29(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(x22), .O(new_n81_));
  oai21  g30(.a(new_n81_), .b(new_n71_), .c(new_n75_), .O(z12));
  nand2  g31(.a(x18), .b(x04), .O(new_n83_));
  oai21  g32(.a(x18), .b(new_n58_), .c(new_n83_), .O(new_n84_));
  nand2  g33(.a(new_n84_), .b(new_n71_), .O(new_n85_));
  nand3  g34(.a(new_n79_), .b(new_n76_), .c(new_n51_), .O(new_n86_));
  nor3   g35(.a(x23), .b(x22), .c(x21), .O(new_n87_));
  aoi22  g36(.a(new_n87_), .b(new_n80_), .c(new_n86_), .d(x23), .O(new_n88_));
  oai21  g37(.a(new_n88_), .b(new_n71_), .c(new_n85_), .O(z13));
  inv1   g38(.a(x13), .O(new_n90_));
  nand2  g39(.a(x18), .b(x05), .O(new_n91_));
  oai21  g40(.a(x18), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand2  g41(.a(new_n92_), .b(new_n71_), .O(new_n93_));
  aoi21  g42(.a(new_n87_), .b(new_n80_), .c(new_n61_), .O(new_n94_));
  nor2   g43(.a(x24), .b(x23), .O(new_n95_));
  nand4  g44(.a(new_n95_), .b(new_n79_), .c(new_n76_), .d(new_n51_), .O(new_n96_));
  inv1   g45(.a(new_n96_), .O(new_n97_));
  oai21  g46(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n93_), .O(z14));
  inv1   g48(.a(x14), .O(new_n100_));
  nand2  g49(.a(x18), .b(x06), .O(new_n101_));
  oai21  g50(.a(x18), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n71_), .O(new_n103_));
  nand2  g52(.a(new_n96_), .b(x25), .O(new_n104_));
  nor2   g53(.a(x23), .b(x22), .O(new_n105_));
  nor2   g54(.a(x25), .b(x24), .O(new_n106_));
  nand4  g55(.a(new_n106_), .b(new_n105_), .c(new_n77_), .d(new_n76_), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand2  g57(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n103_), .O(z15));
  nand2  g59(.a(x18), .b(x07), .O(new_n111_));
  oai21  g60(.a(x18), .b(new_n65_), .c(new_n111_), .O(new_n112_));
  nand2  g61(.a(new_n112_), .b(new_n71_), .O(new_n113_));
  nand2  g62(.a(new_n107_), .b(x26), .O(new_n114_));
  nor3   g63(.a(x26), .b(x25), .c(x24), .O(new_n115_));
  nand4  g64(.a(new_n115_), .b(new_n105_), .c(new_n77_), .d(new_n76_), .O(new_n116_));
  nand2  g65(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g66(.a(new_n117_), .b(x16), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(new_n113_), .O(z16));
  zero   g68(.O(z03));
  zero   g69(.O(z06));
  zero   g70(.O(z09));
  zero   g71(.O(z10));
  zero   g72(.O(z11));
  zero   g73(.O(z17));
  buf    g74(.a(x27), .O(z08));
endmodule


