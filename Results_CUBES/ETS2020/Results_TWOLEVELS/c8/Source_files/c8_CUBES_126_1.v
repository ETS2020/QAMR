// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n56_, new_n57_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x21), .O(new_n52_));
  nand2  g05(.a(new_n48_), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(new_n48_), .b(x10), .c(new_n53_), .O(z02));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n48_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g10(.a(x25), .O(new_n60_));
  nand2  g11(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  oai21  g12(.a(new_n48_), .b(x14), .c(new_n61_), .O(z06));
  inv1   g13(.a(x26), .O(new_n63_));
  nand2  g14(.a(new_n48_), .b(new_n63_), .O(new_n64_));
  oai21  g15(.a(new_n48_), .b(x15), .c(new_n64_), .O(z07));
  inv1   g16(.a(x09), .O(new_n67_));
  nor2   g17(.a(x18), .b(new_n67_), .O(new_n68_));
  aoi21  g18(.a(x18), .b(x01), .c(new_n68_), .O(new_n69_));
  nor2   g19(.a(x19), .b(x17), .O(new_n70_));
  nor2   g20(.a(new_n70_), .b(new_n49_), .O(new_n71_));
  nor3   g21(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  oai21  g22(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  oai21  g23(.a(new_n69_), .b(x16), .c(new_n73_), .O(z10));
  inv1   g24(.a(x16), .O(new_n77_));
  inv1   g25(.a(x12), .O(new_n78_));
  nand2  g26(.a(x18), .b(x04), .O(new_n79_));
  oai21  g27(.a(x18), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand2  g28(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nor2   g29(.a(x22), .b(x21), .O(new_n82_));
  nand3  g30(.a(new_n82_), .b(new_n70_), .c(new_n49_), .O(new_n83_));
  nor3   g31(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  aoi22  g32(.a(new_n84_), .b(new_n72_), .c(new_n83_), .d(x23), .O(new_n85_));
  oai21  g33(.a(new_n85_), .b(new_n77_), .c(new_n81_), .O(z13));
  inv1   g34(.a(x14), .O(new_n88_));
  nand2  g35(.a(x18), .b(x06), .O(new_n89_));
  oai21  g36(.a(x18), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand2  g37(.a(new_n90_), .b(new_n77_), .O(new_n91_));
  nor2   g38(.a(x24), .b(x23), .O(new_n92_));
  nand4  g39(.a(new_n92_), .b(new_n82_), .c(new_n70_), .d(new_n49_), .O(new_n93_));
  inv1   g40(.a(x17), .O(new_n94_));
  inv1   g41(.a(x19), .O(new_n95_));
  inv1   g42(.a(x24), .O(new_n96_));
  nand4  g43(.a(new_n60_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g44(.a(new_n97_), .O(new_n98_));
  inv1   g45(.a(x22), .O(new_n99_));
  nand4  g46(.a(new_n56_), .b(new_n99_), .c(new_n52_), .d(new_n49_), .O(new_n100_));
  inv1   g47(.a(new_n100_), .O(new_n101_));
  aoi22  g48(.a(new_n101_), .b(new_n98_), .c(new_n93_), .d(x25), .O(new_n102_));
  oai21  g49(.a(new_n102_), .b(new_n77_), .c(new_n91_), .O(z15));
  nor2   g50(.a(x23), .b(x22), .O(new_n105_));
  nor2   g51(.a(x21), .b(x20), .O(new_n106_));
  nor3   g52(.a(x26), .b(x25), .c(x24), .O(new_n107_));
  nor2   g53(.a(new_n95_), .b(x17), .O(new_n108_));
  nand4  g54(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n105_), .O(new_n109_));
  nand2  g55(.a(x27), .b(x17), .O(new_n110_));
  aoi21  g56(.a(new_n110_), .b(new_n109_), .c(new_n77_), .O(z17));
  zero   g57(.O(z00));
  zero   g58(.O(z03));
  zero   g59(.O(z05));
  zero   g60(.O(z09));
  zero   g61(.O(z11));
  zero   g62(.O(z12));
  zero   g63(.O(z14));
  zero   g64(.O(z16));
  buf    g65(.a(x27), .O(z08));
endmodule


