// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x21), .O(new_n52_));
  nand2  g05(.a(new_n48_), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(new_n48_), .b(x10), .c(new_n53_), .O(z02));
  inv1   g07(.a(x22), .O(new_n55_));
  nand2  g08(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  oai21  g09(.a(new_n48_), .b(x11), .c(new_n56_), .O(z03));
  inv1   g10(.a(x25), .O(new_n60_));
  nand2  g11(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  oai21  g12(.a(new_n48_), .b(x14), .c(new_n61_), .O(z06));
  inv1   g13(.a(x26), .O(new_n63_));
  nand2  g14(.a(new_n48_), .b(new_n63_), .O(new_n64_));
  oai21  g15(.a(new_n48_), .b(x15), .c(new_n64_), .O(z07));
  inv1   g16(.a(x10), .O(new_n68_));
  nor2   g17(.a(x18), .b(new_n68_), .O(new_n69_));
  aoi21  g18(.a(x18), .b(x02), .c(new_n69_), .O(new_n70_));
  nor2   g19(.a(x19), .b(x17), .O(new_n71_));
  aoi21  g20(.a(new_n71_), .b(new_n49_), .c(new_n52_), .O(new_n72_));
  inv1   g21(.a(x17), .O(new_n73_));
  inv1   g22(.a(x19), .O(new_n74_));
  nand4  g23(.a(new_n52_), .b(new_n49_), .c(new_n74_), .d(new_n73_), .O(new_n75_));
  inv1   g24(.a(new_n75_), .O(new_n76_));
  oai21  g25(.a(new_n76_), .b(new_n72_), .c(x16), .O(new_n77_));
  oai21  g26(.a(new_n70_), .b(x16), .c(new_n77_), .O(z11));
  inv1   g27(.a(x16), .O(new_n82_));
  inv1   g28(.a(x14), .O(new_n83_));
  nand2  g29(.a(x18), .b(x06), .O(new_n84_));
  oai21  g30(.a(x18), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nand2  g31(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nor2   g32(.a(x22), .b(x21), .O(new_n87_));
  nor2   g33(.a(x24), .b(x23), .O(new_n88_));
  nand4  g34(.a(new_n88_), .b(new_n87_), .c(new_n71_), .d(new_n49_), .O(new_n89_));
  inv1   g35(.a(x23), .O(new_n90_));
  inv1   g36(.a(x24), .O(new_n91_));
  nand4  g37(.a(new_n60_), .b(new_n91_), .c(new_n90_), .d(new_n55_), .O(new_n92_));
  inv1   g38(.a(new_n92_), .O(new_n93_));
  aoi22  g39(.a(new_n93_), .b(new_n76_), .c(new_n89_), .d(x25), .O(new_n94_));
  oai21  g40(.a(new_n94_), .b(new_n82_), .c(new_n86_), .O(z15));
  nor2   g41(.a(x21), .b(x20), .O(new_n97_));
  nor2   g42(.a(x23), .b(x22), .O(new_n98_));
  nor3   g43(.a(x26), .b(x25), .c(x24), .O(new_n99_));
  nor2   g44(.a(new_n74_), .b(x17), .O(new_n100_));
  nand4  g45(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n101_));
  nand2  g46(.a(x27), .b(x17), .O(new_n102_));
  aoi21  g47(.a(new_n102_), .b(new_n101_), .c(new_n82_), .O(z17));
  zero   g48(.O(z00));
  zero   g49(.O(z04));
  zero   g50(.O(z05));
  zero   g51(.O(z09));
  zero   g52(.O(z10));
  zero   g53(.O(z12));
  zero   g54(.O(z13));
  zero   g55(.O(z14));
  zero   g56(.O(z16));
  buf    g57(.a(x27), .O(z08));
endmodule


