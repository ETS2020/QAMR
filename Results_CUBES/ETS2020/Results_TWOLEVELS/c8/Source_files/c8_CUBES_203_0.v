// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n56_, new_n57_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n50_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n50_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n50_), .b(x12), .c(new_n57_), .O(z04));
  and2   g10(.a(x19), .b(x17), .O(new_n63_));
  nor2   g11(.a(x19), .b(x17), .O(new_n64_));
  oai21  g12(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g13(.a(x18), .b(new_n47_), .O(new_n66_));
  aoi21  g14(.a(x18), .b(x00), .c(new_n66_), .O(new_n67_));
  oai21  g15(.a(new_n67_), .b(x16), .c(new_n65_), .O(z09));
  inv1   g16(.a(x09), .O(new_n69_));
  nor2   g17(.a(x18), .b(new_n69_), .O(new_n70_));
  aoi21  g18(.a(x18), .b(x01), .c(new_n70_), .O(new_n71_));
  nor2   g19(.a(new_n64_), .b(new_n51_), .O(new_n72_));
  nor3   g20(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g21(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  oai21  g22(.a(new_n71_), .b(x16), .c(new_n74_), .O(z10));
  inv1   g23(.a(x11), .O(new_n77_));
  nor2   g24(.a(x18), .b(new_n77_), .O(new_n78_));
  aoi21  g25(.a(x18), .b(x03), .c(new_n78_), .O(new_n79_));
  inv1   g26(.a(x22), .O(new_n80_));
  nor2   g27(.a(x21), .b(x20), .O(new_n81_));
  aoi21  g28(.a(new_n81_), .b(new_n64_), .c(new_n80_), .O(new_n82_));
  nor2   g29(.a(x22), .b(x21), .O(new_n83_));
  nand3  g30(.a(new_n83_), .b(new_n64_), .c(new_n51_), .O(new_n84_));
  inv1   g31(.a(new_n84_), .O(new_n85_));
  oai21  g32(.a(new_n85_), .b(new_n82_), .c(x16), .O(new_n86_));
  oai21  g33(.a(new_n79_), .b(x16), .c(new_n86_), .O(z12));
  inv1   g34(.a(x16), .O(new_n88_));
  inv1   g35(.a(x12), .O(new_n89_));
  nand2  g36(.a(x18), .b(x04), .O(new_n90_));
  oai21  g37(.a(x18), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand2  g38(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nor3   g39(.a(x23), .b(x22), .c(x21), .O(new_n93_));
  aoi22  g40(.a(new_n93_), .b(new_n73_), .c(new_n84_), .d(x23), .O(new_n94_));
  oai21  g41(.a(new_n94_), .b(new_n88_), .c(new_n92_), .O(z13));
  inv1   g42(.a(x13), .O(new_n96_));
  nand2  g43(.a(x18), .b(x05), .O(new_n97_));
  oai21  g44(.a(x18), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g45(.a(new_n98_), .b(new_n88_), .O(new_n99_));
  inv1   g46(.a(x24), .O(new_n100_));
  aoi21  g47(.a(new_n93_), .b(new_n73_), .c(new_n100_), .O(new_n101_));
  nand2  g48(.a(new_n64_), .b(new_n51_), .O(new_n102_));
  nand3  g49(.a(new_n83_), .b(new_n100_), .c(new_n56_), .O(new_n103_));
  nor2   g50(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g51(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand2  g52(.a(new_n105_), .b(new_n99_), .O(z14));
  zero   g53(.O(z02));
  zero   g54(.O(z03));
  zero   g55(.O(z05));
  zero   g56(.O(z06));
  zero   g57(.O(z07));
  zero   g58(.O(z08));
  zero   g59(.O(z11));
  zero   g60(.O(z15));
  zero   g61(.O(z16));
  zero   g62(.O(z17));
endmodule


