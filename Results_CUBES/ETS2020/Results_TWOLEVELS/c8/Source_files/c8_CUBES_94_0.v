// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x10), .O(new_n52_));
  nand2  g05(.a(x27), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(x27), .b(x21), .c(new_n53_), .O(z02));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n48_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n48_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  and2   g13(.a(x19), .b(x17), .O(new_n64_));
  nor2   g14(.a(x19), .b(x17), .O(new_n65_));
  oai21  g15(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g16(.a(x08), .O(new_n67_));
  nor2   g17(.a(x18), .b(new_n67_), .O(new_n68_));
  aoi21  g18(.a(x18), .b(x00), .c(new_n68_), .O(new_n69_));
  oai21  g19(.a(new_n69_), .b(x16), .c(new_n66_), .O(z09));
  inv1   g20(.a(x09), .O(new_n71_));
  nor2   g21(.a(x18), .b(new_n71_), .O(new_n72_));
  aoi21  g22(.a(x18), .b(x01), .c(new_n72_), .O(new_n73_));
  nor2   g23(.a(new_n65_), .b(new_n49_), .O(new_n74_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g25(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  oai21  g26(.a(new_n73_), .b(x16), .c(new_n76_), .O(z10));
  inv1   g27(.a(x16), .O(new_n78_));
  nand2  g28(.a(x18), .b(x02), .O(new_n79_));
  oai21  g29(.a(x18), .b(new_n52_), .c(new_n79_), .O(new_n80_));
  nand2  g30(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g31(.a(new_n65_), .b(new_n49_), .O(new_n82_));
  nor2   g32(.a(x21), .b(x20), .O(new_n83_));
  aoi22  g33(.a(new_n83_), .b(new_n65_), .c(new_n82_), .d(x21), .O(new_n84_));
  oai21  g34(.a(new_n84_), .b(new_n78_), .c(new_n81_), .O(z11));
  inv1   g35(.a(x11), .O(new_n86_));
  nand2  g36(.a(x18), .b(x03), .O(new_n87_));
  oai21  g37(.a(x18), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nand2  g38(.a(new_n88_), .b(new_n78_), .O(new_n89_));
  nand2  g39(.a(new_n83_), .b(new_n65_), .O(new_n90_));
  nor2   g40(.a(x22), .b(x21), .O(new_n91_));
  aoi22  g41(.a(new_n91_), .b(new_n75_), .c(new_n90_), .d(x22), .O(new_n92_));
  oai21  g42(.a(new_n92_), .b(new_n78_), .c(new_n89_), .O(z12));
  inv1   g43(.a(x12), .O(new_n94_));
  nand2  g44(.a(x18), .b(x04), .O(new_n95_));
  oai21  g45(.a(x18), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g46(.a(new_n96_), .b(new_n78_), .O(new_n97_));
  nand3  g47(.a(new_n91_), .b(new_n65_), .c(new_n49_), .O(new_n98_));
  nor3   g48(.a(x23), .b(x22), .c(x21), .O(new_n99_));
  aoi22  g49(.a(new_n99_), .b(new_n75_), .c(new_n98_), .d(x23), .O(new_n100_));
  oai21  g50(.a(new_n100_), .b(new_n78_), .c(new_n97_), .O(z13));
  inv1   g51(.a(x13), .O(new_n102_));
  nand2  g52(.a(x18), .b(x05), .O(new_n103_));
  oai21  g53(.a(x18), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  nand2  g54(.a(new_n104_), .b(new_n78_), .O(new_n105_));
  aoi21  g55(.a(new_n99_), .b(new_n75_), .c(new_n57_), .O(new_n106_));
  nor2   g56(.a(x24), .b(x23), .O(new_n107_));
  nand2  g57(.a(new_n107_), .b(new_n91_), .O(new_n108_));
  nor2   g58(.a(new_n108_), .b(new_n82_), .O(new_n109_));
  oai21  g59(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nand2  g60(.a(new_n110_), .b(new_n105_), .O(z14));
  nand2  g61(.a(x18), .b(x07), .O(new_n113_));
  oai21  g62(.a(x18), .b(new_n61_), .c(new_n113_), .O(new_n114_));
  nand2  g63(.a(new_n114_), .b(new_n78_), .O(new_n115_));
  nor2   g64(.a(x23), .b(x22), .O(new_n116_));
  nor2   g65(.a(x25), .b(x24), .O(new_n117_));
  nand4  g66(.a(new_n117_), .b(new_n116_), .c(new_n83_), .d(new_n65_), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(x26), .O(new_n119_));
  nor3   g68(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand4  g69(.a(new_n120_), .b(new_n116_), .c(new_n83_), .d(new_n65_), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g71(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g72(.a(new_n123_), .b(new_n115_), .O(z16));
  zero   g73(.O(z00));
  zero   g74(.O(z03));
  zero   g75(.O(z04));
  zero   g76(.O(z06));
  zero   g77(.O(z15));
  zero   g78(.O(z17));
  buf    g79(.a(x27), .O(z08));
endmodule


