// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n51_));
  inv1   g04(.a(x21), .O(new_n52_));
  nand2  g05(.a(new_n51_), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(new_n51_), .b(x10), .c(new_n53_), .O(z02));
  inv1   g07(.a(x22), .O(new_n55_));
  nand2  g08(.a(new_n51_), .b(new_n55_), .O(new_n56_));
  oai21  g09(.a(new_n51_), .b(x11), .c(new_n56_), .O(z03));
  inv1   g10(.a(x23), .O(new_n58_));
  nand2  g11(.a(new_n51_), .b(new_n58_), .O(new_n59_));
  oai21  g12(.a(new_n51_), .b(x12), .c(new_n59_), .O(z04));
  inv1   g13(.a(x24), .O(new_n61_));
  nand2  g14(.a(new_n51_), .b(new_n61_), .O(new_n62_));
  oai21  g15(.a(new_n51_), .b(x13), .c(new_n62_), .O(z05));
  inv1   g16(.a(x25), .O(new_n64_));
  nand2  g17(.a(new_n51_), .b(new_n64_), .O(new_n65_));
  oai21  g18(.a(new_n51_), .b(x14), .c(new_n65_), .O(z06));
  and2   g19(.a(x19), .b(x17), .O(new_n69_));
  nor2   g20(.a(x19), .b(x17), .O(new_n70_));
  oai21  g21(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  nor2   g22(.a(x18), .b(new_n47_), .O(new_n72_));
  aoi21  g23(.a(x18), .b(x00), .c(new_n72_), .O(new_n73_));
  oai21  g24(.a(new_n73_), .b(x16), .c(new_n71_), .O(z09));
  inv1   g25(.a(x09), .O(new_n75_));
  nor2   g26(.a(x18), .b(new_n75_), .O(new_n76_));
  aoi21  g27(.a(x18), .b(x01), .c(new_n76_), .O(new_n77_));
  inv1   g28(.a(x20), .O(new_n78_));
  nor2   g29(.a(new_n70_), .b(new_n78_), .O(new_n79_));
  nor3   g30(.a(x20), .b(x19), .c(x17), .O(new_n80_));
  oai21  g31(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  oai21  g32(.a(new_n77_), .b(x16), .c(new_n81_), .O(z10));
  inv1   g33(.a(x11), .O(new_n84_));
  nor2   g34(.a(x18), .b(new_n84_), .O(new_n85_));
  aoi21  g35(.a(x18), .b(x03), .c(new_n85_), .O(new_n86_));
  nor2   g36(.a(x21), .b(x20), .O(new_n87_));
  aoi21  g37(.a(new_n87_), .b(new_n70_), .c(new_n55_), .O(new_n88_));
  nor2   g38(.a(x22), .b(x21), .O(new_n89_));
  nand3  g39(.a(new_n89_), .b(new_n70_), .c(new_n78_), .O(new_n90_));
  inv1   g40(.a(new_n90_), .O(new_n91_));
  oai21  g41(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  oai21  g42(.a(new_n86_), .b(x16), .c(new_n92_), .O(z12));
  inv1   g43(.a(x16), .O(new_n94_));
  inv1   g44(.a(x12), .O(new_n95_));
  nand2  g45(.a(x18), .b(x04), .O(new_n96_));
  oai21  g46(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g47(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nor3   g48(.a(x23), .b(x22), .c(x21), .O(new_n99_));
  aoi22  g49(.a(new_n99_), .b(new_n80_), .c(new_n90_), .d(x23), .O(new_n100_));
  oai21  g50(.a(new_n100_), .b(new_n94_), .c(new_n98_), .O(z13));
  inv1   g51(.a(x13), .O(new_n102_));
  nand2  g52(.a(x18), .b(x05), .O(new_n103_));
  oai21  g53(.a(x18), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  nand2  g54(.a(new_n104_), .b(new_n94_), .O(new_n105_));
  aoi21  g55(.a(new_n99_), .b(new_n80_), .c(new_n61_), .O(new_n106_));
  nand2  g56(.a(new_n70_), .b(new_n78_), .O(new_n107_));
  nand3  g57(.a(new_n89_), .b(new_n61_), .c(new_n58_), .O(new_n108_));
  nor2   g58(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g59(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nand2  g60(.a(new_n110_), .b(new_n105_), .O(z14));
  zero   g61(.O(z01));
  zero   g62(.O(z07));
  zero   g63(.O(z08));
  zero   g64(.O(z11));
  zero   g65(.O(z15));
  zero   g66(.O(z16));
  zero   g67(.O(z17));
endmodule


