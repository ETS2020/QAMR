// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n50_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n50_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n50_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n50_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x25), .O(new_n60_));
  nand2  g11(.a(new_n50_), .b(new_n60_), .O(new_n61_));
  oai21  g12(.a(new_n50_), .b(x14), .c(new_n61_), .O(z06));
  and2   g13(.a(x19), .b(x17), .O(new_n64_));
  nor2   g14(.a(x19), .b(x17), .O(new_n65_));
  oai21  g15(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g16(.a(x18), .b(new_n47_), .O(new_n67_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n67_), .O(new_n68_));
  oai21  g18(.a(new_n68_), .b(x16), .c(new_n66_), .O(z09));
  inv1   g19(.a(x11), .O(new_n72_));
  nor2   g20(.a(x18), .b(new_n72_), .O(new_n73_));
  aoi21  g21(.a(x18), .b(x03), .c(new_n73_), .O(new_n74_));
  inv1   g22(.a(x22), .O(new_n75_));
  nor2   g23(.a(x21), .b(x20), .O(new_n76_));
  aoi21  g24(.a(new_n76_), .b(new_n65_), .c(new_n75_), .O(new_n77_));
  nor2   g25(.a(x22), .b(x21), .O(new_n78_));
  nand3  g26(.a(new_n78_), .b(new_n65_), .c(new_n51_), .O(new_n79_));
  inv1   g27(.a(new_n79_), .O(new_n80_));
  oai21  g28(.a(new_n80_), .b(new_n77_), .c(x16), .O(new_n81_));
  oai21  g29(.a(new_n74_), .b(x16), .c(new_n81_), .O(z12));
  inv1   g30(.a(x16), .O(new_n83_));
  inv1   g31(.a(x12), .O(new_n84_));
  nand2  g32(.a(x18), .b(x04), .O(new_n85_));
  oai21  g33(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g34(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nor3   g35(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  nor3   g36(.a(x23), .b(x22), .c(x21), .O(new_n89_));
  aoi22  g37(.a(new_n89_), .b(new_n88_), .c(new_n79_), .d(x23), .O(new_n90_));
  oai21  g38(.a(new_n90_), .b(new_n83_), .c(new_n87_), .O(z13));
  inv1   g39(.a(x13), .O(new_n92_));
  nand2  g40(.a(x18), .b(x05), .O(new_n93_));
  oai21  g41(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g42(.a(new_n94_), .b(new_n83_), .O(new_n95_));
  aoi21  g43(.a(new_n89_), .b(new_n88_), .c(new_n57_), .O(new_n96_));
  nand2  g44(.a(new_n65_), .b(new_n51_), .O(new_n97_));
  nor2   g45(.a(x24), .b(x23), .O(new_n98_));
  nand2  g46(.a(new_n98_), .b(new_n78_), .O(new_n99_));
  nor2   g47(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  oai21  g48(.a(new_n100_), .b(new_n96_), .c(x16), .O(new_n101_));
  nand2  g49(.a(new_n101_), .b(new_n95_), .O(z14));
  zero   g50(.O(z02));
  zero   g51(.O(z03));
  zero   g52(.O(z04));
  zero   g53(.O(z07));
  zero   g54(.O(z10));
  zero   g55(.O(z11));
  zero   g56(.O(z15));
  zero   g57(.O(z16));
  zero   g58(.O(z17));
  buf    g59(.a(x27), .O(z08));
endmodule


