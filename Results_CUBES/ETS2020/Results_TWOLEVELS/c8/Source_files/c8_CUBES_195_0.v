// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x22), .O(new_n53_));
  nand2  g05(.a(new_n48_), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(new_n48_), .b(x11), .c(new_n54_), .O(z03));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n48_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n48_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x25), .O(new_n60_));
  nand2  g11(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  oai21  g12(.a(new_n48_), .b(x14), .c(new_n61_), .O(z06));
  and2   g13(.a(x19), .b(x17), .O(new_n64_));
  nor2   g14(.a(x19), .b(x17), .O(new_n65_));
  oai21  g15(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g16(.a(x08), .O(new_n67_));
  nor2   g17(.a(x18), .b(new_n67_), .O(new_n68_));
  aoi21  g18(.a(x18), .b(x00), .c(new_n68_), .O(new_n69_));
  oai21  g19(.a(new_n69_), .b(x16), .c(new_n66_), .O(z09));
  inv1   g20(.a(x09), .O(new_n71_));
  nor2   g21(.a(x18), .b(new_n71_), .O(new_n72_));
  aoi21  g22(.a(x18), .b(x01), .c(new_n72_), .O(new_n73_));
  nor2   g23(.a(new_n65_), .b(new_n49_), .O(new_n74_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g25(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  oai21  g26(.a(new_n73_), .b(x16), .c(new_n76_), .O(z10));
  inv1   g27(.a(x16), .O(new_n78_));
  inv1   g28(.a(x10), .O(new_n79_));
  nand2  g29(.a(x18), .b(x02), .O(new_n80_));
  oai21  g30(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand2  g31(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g32(.a(new_n65_), .b(new_n49_), .O(new_n83_));
  nor2   g33(.a(x21), .b(x20), .O(new_n84_));
  aoi22  g34(.a(new_n84_), .b(new_n65_), .c(new_n83_), .d(x21), .O(new_n85_));
  oai21  g35(.a(new_n85_), .b(new_n78_), .c(new_n82_), .O(z11));
  inv1   g36(.a(x12), .O(new_n88_));
  nand2  g37(.a(x18), .b(x04), .O(new_n89_));
  oai21  g38(.a(x18), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand2  g39(.a(new_n90_), .b(new_n78_), .O(new_n91_));
  nor2   g40(.a(x22), .b(x21), .O(new_n92_));
  nand3  g41(.a(new_n92_), .b(new_n65_), .c(new_n49_), .O(new_n93_));
  nor3   g42(.a(x23), .b(x22), .c(x21), .O(new_n94_));
  aoi22  g43(.a(new_n94_), .b(new_n75_), .c(new_n93_), .d(x23), .O(new_n95_));
  oai21  g44(.a(new_n95_), .b(new_n78_), .c(new_n91_), .O(z13));
  inv1   g45(.a(x13), .O(new_n97_));
  nand2  g46(.a(x18), .b(x05), .O(new_n98_));
  oai21  g47(.a(x18), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g48(.a(new_n99_), .b(new_n78_), .O(new_n100_));
  aoi21  g49(.a(new_n94_), .b(new_n75_), .c(new_n57_), .O(new_n101_));
  nor2   g50(.a(x24), .b(x23), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n92_), .O(new_n103_));
  nor2   g52(.a(new_n103_), .b(new_n83_), .O(new_n104_));
  oai21  g53(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(new_n100_), .O(z14));
  zero   g55(.O(z00));
  zero   g56(.O(z02));
  zero   g57(.O(z04));
  zero   g58(.O(z07));
  zero   g59(.O(z12));
  zero   g60(.O(z15));
  zero   g61(.O(z16));
  zero   g62(.O(z17));
  buf    g63(.a(x27), .O(z08));
endmodule


