// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n55_, new_n56_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x09), .O(new_n50_));
  nand2  g04(.a(x27), .b(new_n50_), .O(new_n51_));
  oai21  g05(.a(x27), .b(x20), .c(new_n51_), .O(z01));
  inv1   g06(.a(x12), .O(new_n55_));
  nand2  g07(.a(x27), .b(new_n55_), .O(new_n56_));
  oai21  g08(.a(x27), .b(x23), .c(new_n56_), .O(z04));
  and2   g09(.a(x19), .b(x17), .O(new_n62_));
  nor2   g10(.a(x19), .b(x17), .O(new_n63_));
  oai21  g11(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g12(.a(x18), .b(new_n47_), .O(new_n65_));
  aoi21  g13(.a(x18), .b(x00), .c(new_n65_), .O(new_n66_));
  oai21  g14(.a(new_n66_), .b(x16), .c(new_n64_), .O(z09));
  inv1   g15(.a(x16), .O(new_n70_));
  inv1   g16(.a(x11), .O(new_n71_));
  nand2  g17(.a(x18), .b(x03), .O(new_n72_));
  oai21  g18(.a(x18), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nand2  g19(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nor2   g20(.a(x21), .b(x20), .O(new_n75_));
  nand2  g21(.a(new_n75_), .b(new_n63_), .O(new_n76_));
  nor3   g22(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  aoi22  g23(.a(new_n77_), .b(new_n63_), .c(new_n76_), .d(x22), .O(new_n78_));
  oai21  g24(.a(new_n78_), .b(new_n70_), .c(new_n74_), .O(z12));
  nand2  g25(.a(x18), .b(x04), .O(new_n80_));
  oai21  g26(.a(x18), .b(new_n55_), .c(new_n80_), .O(new_n81_));
  nand2  g27(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  inv1   g28(.a(x22), .O(new_n83_));
  nand3  g29(.a(new_n75_), .b(new_n63_), .c(new_n83_), .O(new_n84_));
  nand2  g30(.a(new_n84_), .b(x23), .O(new_n85_));
  nor2   g31(.a(x23), .b(x22), .O(new_n86_));
  nand3  g32(.a(new_n86_), .b(new_n75_), .c(new_n63_), .O(new_n87_));
  nand2  g33(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g34(.a(new_n88_), .b(x16), .O(new_n89_));
  nand2  g35(.a(new_n89_), .b(new_n82_), .O(z13));
  inv1   g36(.a(x15), .O(new_n93_));
  nand2  g37(.a(x18), .b(x07), .O(new_n94_));
  oai21  g38(.a(x18), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g39(.a(new_n95_), .b(new_n70_), .O(new_n96_));
  nor2   g40(.a(x25), .b(x24), .O(new_n97_));
  nand4  g41(.a(new_n97_), .b(new_n86_), .c(new_n75_), .d(new_n63_), .O(new_n98_));
  nand2  g42(.a(new_n98_), .b(x26), .O(new_n99_));
  nor3   g43(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  nand4  g44(.a(new_n100_), .b(new_n86_), .c(new_n75_), .d(new_n63_), .O(new_n101_));
  nand2  g45(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g46(.a(new_n102_), .b(x16), .O(new_n103_));
  nand2  g47(.a(new_n103_), .b(new_n96_), .O(z16));
  zero   g48(.O(z02));
  zero   g49(.O(z03));
  zero   g50(.O(z05));
  zero   g51(.O(z06));
  zero   g52(.O(z07));
  zero   g53(.O(z08));
  zero   g54(.O(z10));
  zero   g55(.O(z11));
  zero   g56(.O(z14));
  zero   g57(.O(z15));
  zero   g58(.O(z17));
endmodule


