// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n58_, new_n59_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x25), .O(new_n58_));
  nand2  g08(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  oai21  g09(.a(new_n47_), .b(x14), .c(new_n59_), .O(z06));
  inv1   g10(.a(x09), .O(new_n64_));
  nor2   g11(.a(x18), .b(new_n64_), .O(new_n65_));
  aoi21  g12(.a(x18), .b(x01), .c(new_n65_), .O(new_n66_));
  nor2   g13(.a(x19), .b(x17), .O(new_n67_));
  nor2   g14(.a(new_n67_), .b(new_n51_), .O(new_n68_));
  nor3   g15(.a(x20), .b(x19), .c(x17), .O(new_n69_));
  oai21  g16(.a(new_n69_), .b(new_n68_), .c(x16), .O(new_n70_));
  oai21  g17(.a(new_n66_), .b(x16), .c(new_n70_), .O(z10));
  inv1   g18(.a(x16), .O(new_n73_));
  inv1   g19(.a(x11), .O(new_n74_));
  nand2  g20(.a(x18), .b(x03), .O(new_n75_));
  oai21  g21(.a(x18), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  nand2  g22(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nor2   g23(.a(x21), .b(x20), .O(new_n78_));
  nand2  g24(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nor2   g25(.a(x22), .b(x21), .O(new_n80_));
  aoi22  g26(.a(new_n80_), .b(new_n69_), .c(new_n79_), .d(x22), .O(new_n81_));
  oai21  g27(.a(new_n81_), .b(new_n73_), .c(new_n77_), .O(z12));
  inv1   g28(.a(x13), .O(new_n84_));
  nand2  g29(.a(x18), .b(x05), .O(new_n85_));
  oai21  g30(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g31(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  inv1   g32(.a(x24), .O(new_n88_));
  nor3   g33(.a(x23), .b(x22), .c(x21), .O(new_n89_));
  aoi21  g34(.a(new_n89_), .b(new_n69_), .c(new_n88_), .O(new_n90_));
  nand2  g35(.a(new_n67_), .b(new_n51_), .O(new_n91_));
  nor2   g36(.a(x24), .b(x23), .O(new_n92_));
  nand2  g37(.a(new_n92_), .b(new_n80_), .O(new_n93_));
  nor2   g38(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  oai21  g39(.a(new_n94_), .b(new_n90_), .c(x16), .O(new_n95_));
  nand2  g40(.a(new_n95_), .b(new_n87_), .O(z14));
  inv1   g41(.a(x15), .O(new_n98_));
  nand2  g42(.a(x18), .b(x07), .O(new_n99_));
  oai21  g43(.a(x18), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  nand2  g44(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  nor2   g45(.a(x23), .b(x22), .O(new_n102_));
  nor2   g46(.a(x25), .b(x24), .O(new_n103_));
  nand4  g47(.a(new_n103_), .b(new_n102_), .c(new_n78_), .d(new_n67_), .O(new_n104_));
  nand2  g48(.a(new_n104_), .b(x26), .O(new_n105_));
  nor3   g49(.a(x26), .b(x25), .c(x24), .O(new_n106_));
  nand4  g50(.a(new_n106_), .b(new_n102_), .c(new_n78_), .d(new_n67_), .O(new_n107_));
  nand2  g51(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g52(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g53(.a(new_n109_), .b(new_n101_), .O(z16));
  zero   g54(.O(z02));
  zero   g55(.O(z03));
  zero   g56(.O(z04));
  zero   g57(.O(z05));
  zero   g58(.O(z07));
  zero   g59(.O(z08));
  zero   g60(.O(z09));
  zero   g61(.O(z11));
  zero   g62(.O(z13));
  zero   g63(.O(z15));
  zero   g64(.O(z17));
endmodule


