// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x24), .O(new_n55_));
  nand2  g05(.a(new_n47_), .b(new_n55_), .O(new_n56_));
  oai21  g06(.a(new_n47_), .b(x13), .c(new_n56_), .O(z05));
  inv1   g07(.a(x26), .O(new_n59_));
  nand2  g08(.a(new_n47_), .b(new_n59_), .O(new_n60_));
  oai21  g09(.a(new_n47_), .b(x15), .c(new_n60_), .O(z07));
  inv1   g10(.a(x09), .O(new_n63_));
  nor2   g11(.a(x18), .b(new_n63_), .O(new_n64_));
  aoi21  g12(.a(x18), .b(x01), .c(new_n64_), .O(new_n65_));
  inv1   g13(.a(x20), .O(new_n66_));
  nor2   g14(.a(x19), .b(x17), .O(new_n67_));
  nor2   g15(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor3   g16(.a(x20), .b(x19), .c(x17), .O(new_n69_));
  oai21  g17(.a(new_n69_), .b(new_n68_), .c(x16), .O(new_n70_));
  oai21  g18(.a(new_n65_), .b(x16), .c(new_n70_), .O(z10));
  inv1   g19(.a(x16), .O(new_n72_));
  inv1   g20(.a(x10), .O(new_n73_));
  nand2  g21(.a(x18), .b(x02), .O(new_n74_));
  oai21  g22(.a(x18), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g23(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand2  g24(.a(new_n67_), .b(new_n66_), .O(new_n77_));
  inv1   g25(.a(x17), .O(new_n78_));
  inv1   g26(.a(x21), .O(new_n79_));
  nand4  g27(.a(new_n79_), .b(new_n66_), .c(new_n48_), .d(new_n78_), .O(new_n80_));
  inv1   g28(.a(new_n80_), .O(new_n81_));
  aoi21  g29(.a(new_n77_), .b(x21), .c(new_n81_), .O(new_n82_));
  oai21  g30(.a(new_n82_), .b(new_n72_), .c(new_n76_), .O(z11));
  inv1   g31(.a(x11), .O(new_n84_));
  nand2  g32(.a(x18), .b(x03), .O(new_n85_));
  oai21  g33(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g34(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nor2   g35(.a(x22), .b(x21), .O(new_n88_));
  aoi22  g36(.a(new_n88_), .b(new_n69_), .c(new_n80_), .d(x22), .O(new_n89_));
  oai21  g37(.a(new_n89_), .b(new_n72_), .c(new_n87_), .O(z12));
  inv1   g38(.a(x13), .O(new_n92_));
  nand2  g39(.a(x18), .b(x05), .O(new_n93_));
  oai21  g40(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g41(.a(new_n94_), .b(new_n72_), .O(new_n95_));
  nor3   g42(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g43(.a(new_n96_), .b(new_n69_), .c(new_n55_), .O(new_n97_));
  nor2   g44(.a(x24), .b(x23), .O(new_n98_));
  nand4  g45(.a(new_n98_), .b(new_n88_), .c(new_n67_), .d(new_n66_), .O(new_n99_));
  inv1   g46(.a(new_n99_), .O(new_n100_));
  oai21  g47(.a(new_n100_), .b(new_n97_), .c(x16), .O(new_n101_));
  nand2  g48(.a(new_n101_), .b(new_n95_), .O(z14));
  inv1   g49(.a(x14), .O(new_n103_));
  nand2  g50(.a(x18), .b(x06), .O(new_n104_));
  oai21  g51(.a(x18), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g52(.a(new_n105_), .b(new_n72_), .O(new_n106_));
  inv1   g53(.a(x22), .O(new_n107_));
  inv1   g54(.a(x23), .O(new_n108_));
  inv1   g55(.a(x25), .O(new_n109_));
  nand4  g56(.a(new_n109_), .b(new_n55_), .c(new_n108_), .d(new_n107_), .O(new_n110_));
  inv1   g57(.a(new_n110_), .O(new_n111_));
  aoi22  g58(.a(new_n111_), .b(new_n81_), .c(new_n99_), .d(x25), .O(new_n112_));
  oai21  g59(.a(new_n112_), .b(new_n72_), .c(new_n106_), .O(z15));
  zero   g60(.O(z01));
  zero   g61(.O(z02));
  zero   g62(.O(z03));
  zero   g63(.O(z04));
  zero   g64(.O(z06));
  zero   g65(.O(z09));
  zero   g66(.O(z13));
  zero   g67(.O(z16));
  zero   g68(.O(z17));
  buf    g69(.a(x27), .O(z08));
endmodule


