// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x10), .O(new_n54_));
  nand2  g08(.a(x27), .b(new_n54_), .O(new_n55_));
  oai21  g09(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g10(.a(x22), .O(new_n57_));
  nand2  g11(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  oai21  g12(.a(new_n47_), .b(x11), .c(new_n58_), .O(z03));
  inv1   g13(.a(x24), .O(new_n61_));
  nand2  g14(.a(new_n47_), .b(new_n61_), .O(new_n62_));
  oai21  g15(.a(new_n47_), .b(x13), .c(new_n62_), .O(z05));
  inv1   g16(.a(x14), .O(new_n64_));
  nand2  g17(.a(x27), .b(new_n64_), .O(new_n65_));
  oai21  g18(.a(x27), .b(x25), .c(new_n65_), .O(z06));
  inv1   g19(.a(x15), .O(new_n67_));
  nand2  g20(.a(x27), .b(new_n67_), .O(new_n68_));
  oai21  g21(.a(x27), .b(x26), .c(new_n68_), .O(z07));
  inv1   g22(.a(x16), .O(new_n71_));
  inv1   g23(.a(x09), .O(new_n72_));
  nand2  g24(.a(x18), .b(x01), .O(new_n73_));
  oai21  g25(.a(x18), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g26(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nor2   g27(.a(x19), .b(x17), .O(new_n76_));
  xor2a  g28(.a(new_n76_), .b(new_n51_), .O(new_n77_));
  oai21  g29(.a(new_n77_), .b(new_n71_), .c(new_n75_), .O(z10));
  nand2  g30(.a(x18), .b(x02), .O(new_n79_));
  oai21  g31(.a(x18), .b(new_n54_), .c(new_n79_), .O(new_n80_));
  nand2  g32(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  nand2  g33(.a(new_n76_), .b(new_n51_), .O(new_n82_));
  nor2   g34(.a(x21), .b(x20), .O(new_n83_));
  aoi22  g35(.a(new_n83_), .b(new_n76_), .c(new_n82_), .d(x21), .O(new_n84_));
  oai21  g36(.a(new_n84_), .b(new_n71_), .c(new_n81_), .O(z11));
  nand2  g37(.a(x18), .b(x06), .O(new_n89_));
  oai21  g38(.a(x18), .b(new_n64_), .c(new_n89_), .O(new_n90_));
  nand2  g39(.a(new_n90_), .b(new_n71_), .O(new_n91_));
  nor2   g40(.a(x22), .b(x21), .O(new_n92_));
  nor2   g41(.a(x24), .b(x23), .O(new_n93_));
  nand4  g42(.a(new_n93_), .b(new_n92_), .c(new_n76_), .d(new_n51_), .O(new_n94_));
  nand2  g43(.a(new_n94_), .b(x25), .O(new_n95_));
  nor2   g44(.a(x23), .b(x22), .O(new_n96_));
  nor2   g45(.a(x25), .b(x24), .O(new_n97_));
  nand4  g46(.a(new_n97_), .b(new_n96_), .c(new_n83_), .d(new_n76_), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  nand2  g48(.a(new_n99_), .b(x16), .O(new_n100_));
  nand2  g49(.a(new_n100_), .b(new_n91_), .O(z15));
  nand2  g50(.a(x18), .b(x07), .O(new_n102_));
  oai21  g51(.a(x18), .b(new_n67_), .c(new_n102_), .O(new_n103_));
  nand2  g52(.a(new_n103_), .b(new_n71_), .O(new_n104_));
  nand2  g53(.a(new_n98_), .b(x26), .O(new_n105_));
  nor3   g54(.a(x26), .b(x25), .c(x24), .O(new_n106_));
  nand4  g55(.a(new_n106_), .b(new_n96_), .c(new_n83_), .d(new_n76_), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g57(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n104_), .O(z16));
  nor2   g59(.a(new_n48_), .b(x17), .O(new_n111_));
  nand4  g60(.a(new_n111_), .b(new_n106_), .c(new_n96_), .d(new_n83_), .O(new_n112_));
  nand2  g61(.a(x27), .b(x17), .O(new_n113_));
  aoi21  g62(.a(new_n113_), .b(new_n112_), .c(new_n71_), .O(z17));
  zero   g63(.O(z04));
  zero   g64(.O(z09));
  zero   g65(.O(z12));
  zero   g66(.O(z13));
  zero   g67(.O(z14));
  buf    g68(.a(x27), .O(z08));
endmodule


