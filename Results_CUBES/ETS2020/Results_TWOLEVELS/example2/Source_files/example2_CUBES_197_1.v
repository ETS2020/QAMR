// Benchmark "FAU" written by ABC on Fri Jul 10 18:23:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n193_, new_n194_, new_n197_, new_n198_,
    new_n200_, new_n201_, new_n203_, new_n204_, new_n207_, new_n208_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n223_, new_n224_, new_n226_, new_n227_, new_n228_, new_n229_,
    new_n230_, new_n233_, new_n234_, new_n235_, new_n236_, new_n237_,
    new_n238_, new_n239_, new_n240_, new_n241_, new_n242_, new_n244_,
    new_n246_, new_n248_, new_n250_, new_n252_, new_n254_, new_n256_,
    new_n257_, new_n258_, new_n259_, new_n260_, new_n261_, new_n262_,
    new_n263_, new_n264_, new_n265_, new_n266_, new_n267_, new_n270_,
    new_n271_, new_n272_, new_n273_, new_n274_, new_n275_, new_n276_,
    new_n277_, new_n278_, new_n279_, new_n281_, new_n283_, new_n285_,
    new_n287_, new_n289_, new_n295_, new_n297_, new_n300_, new_n301_,
    new_n302_, new_n303_, new_n304_, new_n305_, new_n306_, new_n307_,
    new_n308_, new_n309_, new_n310_, new_n313_, new_n314_, new_n315_,
    new_n316_, new_n317_, new_n319_, new_n320_, new_n321_, new_n322_,
    new_n323_, new_n327_, new_n328_, new_n329_, new_n330_, new_n331_,
    new_n333_, new_n334_, new_n335_, new_n336_, new_n338_, new_n339_,
    new_n340_, new_n341_, new_n342_, new_n343_, new_n344_, new_n347_,
    new_n348_, new_n349_, new_n350_, new_n351_, new_n352_, new_n353_,
    new_n354_, new_n356_, new_n357_, new_n358_, new_n359_, new_n360_,
    new_n361_, new_n362_, new_n364_, new_n366_, new_n367_, new_n368_,
    new_n370_, new_n371_, new_n372_, new_n373_, new_n374_, new_n375_,
    new_n376_, new_n377_, new_n378_, new_n379_, new_n380_, new_n383_,
    new_n384_, new_n385_, new_n387_, new_n388_, new_n389_, new_n390_,
    new_n391_;
  inv1   g000(.a(x40), .O(new_n152_));
  inv1   g001(.a(x01), .O(new_n153_));
  inv1   g002(.a(x79), .O(new_n154_));
  nand3  g003(.a(new_n154_), .b(x78), .c(x77), .O(new_n155_));
  aoi21  g004(.a(new_n155_), .b(new_n153_), .c(x52), .O(new_n156_));
  nand2  g005(.a(new_n152_), .b(x06), .O(new_n157_));
  oai21  g006(.a(new_n156_), .b(new_n152_), .c(new_n157_), .O(z00));
  inv1   g007(.a(x66), .O(new_n160_));
  inv1   g008(.a(x75), .O(new_n161_));
  inv1   g009(.a(x77), .O(new_n162_));
  nand2  g010(.a(x78), .b(new_n162_), .O(new_n163_));
  inv1   g011(.a(x78), .O(new_n164_));
  nand2  g012(.a(new_n164_), .b(x77), .O(new_n165_));
  oai22  g013(.a(new_n165_), .b(new_n160_), .c(new_n163_), .d(new_n161_), .O(new_n166_));
  nor2   g014(.a(new_n154_), .b(x01), .O(new_n167_));
  and2   g015(.a(new_n167_), .b(new_n166_), .O(z02));
  nor2   g016(.a(x79), .b(new_n164_), .O(new_n169_));
  nand3  g017(.a(new_n169_), .b(x52), .c(new_n153_), .O(new_n170_));
  inv1   g018(.a(new_n170_), .O(z03));
  aoi21  g019(.a(new_n169_), .b(x77), .c(x01), .O(z04));
  inv1   g020(.a(x62), .O(new_n176_));
  nand2  g021(.a(new_n152_), .b(x26), .O(new_n177_));
  oai21  g022(.a(new_n176_), .b(new_n152_), .c(new_n177_), .O(z08));
  inv1   g023(.a(x27), .O(new_n179_));
  nand2  g024(.a(x61), .b(x40), .O(new_n180_));
  oai21  g025(.a(x40), .b(new_n179_), .c(new_n180_), .O(z09));
  inv1   g026(.a(x28), .O(new_n182_));
  nand2  g027(.a(x60), .b(x40), .O(new_n183_));
  oai21  g028(.a(x40), .b(new_n182_), .c(new_n183_), .O(z10));
  inv1   g029(.a(x59), .O(new_n185_));
  nand2  g030(.a(new_n152_), .b(x29), .O(new_n186_));
  oai21  g031(.a(new_n185_), .b(new_n152_), .c(new_n186_), .O(z11));
  inv1   g032(.a(x58), .O(new_n188_));
  nand2  g033(.a(new_n152_), .b(x30), .O(new_n189_));
  oai21  g034(.a(new_n188_), .b(new_n152_), .c(new_n189_), .O(z12));
  inv1   g035(.a(x33), .O(new_n193_));
  nand2  g036(.a(x50), .b(x40), .O(new_n194_));
  oai21  g037(.a(x40), .b(new_n193_), .c(new_n194_), .O(z15));
  inv1   g038(.a(x35), .O(new_n197_));
  nand2  g039(.a(x48), .b(x40), .O(new_n198_));
  oai21  g040(.a(x40), .b(new_n197_), .c(new_n198_), .O(z17));
  inv1   g041(.a(x36), .O(new_n200_));
  nand2  g042(.a(x47), .b(x40), .O(new_n201_));
  oai21  g043(.a(x40), .b(new_n200_), .c(new_n201_), .O(z18));
  inv1   g044(.a(x37), .O(new_n203_));
  nand2  g045(.a(x46), .b(x40), .O(new_n204_));
  oai21  g046(.a(x40), .b(new_n203_), .c(new_n204_), .O(z19));
  inv1   g047(.a(x39), .O(new_n207_));
  nand2  g048(.a(x44), .b(x40), .O(new_n208_));
  oai21  g049(.a(x40), .b(new_n207_), .c(new_n208_), .O(z21));
  xnor2a g050(.a(x84), .b(x81), .O(new_n210_));
  nor2   g051(.a(new_n154_), .b(x41), .O(new_n211_));
  nand3  g052(.a(new_n211_), .b(new_n210_), .c(new_n166_), .O(new_n212_));
  inv1   g053(.a(x83), .O(new_n213_));
  nand4  g054(.a(x84), .b(new_n213_), .c(x82), .d(x81), .O(new_n214_));
  inv1   g055(.a(x74), .O(new_n215_));
  nand3  g056(.a(x80), .b(new_n215_), .c(x43), .O(new_n216_));
  nor2   g057(.a(new_n216_), .b(new_n214_), .O(new_n217_));
  nor3   g058(.a(new_n217_), .b(new_n162_), .c(x42), .O(new_n218_));
  nand2  g059(.a(x78), .b(x04), .O(new_n219_));
  inv1   g060(.a(new_n219_), .O(new_n220_));
  oai21  g061(.a(new_n218_), .b(new_n154_), .c(new_n220_), .O(new_n221_));
  aoi21  g062(.a(new_n221_), .b(new_n212_), .c(x01), .O(z22));
  inv1   g063(.a(x04), .O(new_n223_));
  nand3  g064(.a(new_n154_), .b(x05), .c(new_n223_), .O(new_n224_));
  nand3  g065(.a(new_n224_), .b(new_n153_), .c(x00), .O(z23));
  nor2   g066(.a(new_n164_), .b(new_n162_), .O(new_n226_));
  inv1   g067(.a(new_n226_), .O(new_n227_));
  inv1   g068(.a(x43), .O(new_n228_));
  nor2   g069(.a(x04), .b(x01), .O(new_n229_));
  nand3  g070(.a(new_n229_), .b(new_n228_), .c(x05), .O(new_n230_));
  aoi21  g071(.a(new_n227_), .b(x79), .c(new_n230_), .O(z24));
  inv1   g072(.a(x42), .O(new_n233_));
  xor2a  g073(.a(x84), .b(x82), .O(new_n234_));
  inv1   g074(.a(new_n234_), .O(new_n235_));
  nand2  g075(.a(new_n235_), .b(x81), .O(new_n236_));
  inv1   g076(.a(x81), .O(new_n237_));
  xor2a  g077(.a(x84), .b(x82), .O(new_n238_));
  nand2  g078(.a(new_n238_), .b(new_n237_), .O(new_n239_));
  nand2  g079(.a(new_n226_), .b(x79), .O(new_n240_));
  aoi21  g080(.a(new_n239_), .b(new_n236_), .c(new_n240_), .O(new_n241_));
  nand4  g081(.a(new_n241_), .b(new_n229_), .c(x44), .d(new_n233_), .O(new_n242_));
  inv1   g082(.a(new_n242_), .O(z26));
  nand4  g083(.a(new_n241_), .b(new_n229_), .c(x45), .d(new_n233_), .O(new_n244_));
  inv1   g084(.a(new_n244_), .O(z27));
  nand4  g085(.a(new_n241_), .b(new_n229_), .c(x46), .d(new_n233_), .O(new_n246_));
  inv1   g086(.a(new_n246_), .O(z28));
  nand4  g087(.a(new_n241_), .b(new_n229_), .c(x47), .d(new_n233_), .O(new_n248_));
  inv1   g088(.a(new_n248_), .O(z29));
  nand4  g089(.a(new_n241_), .b(new_n229_), .c(x48), .d(new_n233_), .O(new_n250_));
  inv1   g090(.a(new_n250_), .O(z30));
  nand4  g091(.a(new_n241_), .b(new_n229_), .c(x49), .d(new_n233_), .O(new_n252_));
  inv1   g092(.a(new_n252_), .O(z31));
  nand4  g093(.a(new_n241_), .b(new_n229_), .c(x50), .d(new_n233_), .O(new_n254_));
  inv1   g094(.a(new_n254_), .O(z32));
  nor2   g095(.a(new_n213_), .b(x81), .O(new_n256_));
  nor2   g096(.a(x83), .b(new_n237_), .O(new_n257_));
  nor2   g097(.a(new_n257_), .b(new_n256_), .O(new_n258_));
  nand2  g098(.a(x42), .b(x05), .O(new_n259_));
  nand2  g099(.a(x51), .b(new_n233_), .O(new_n260_));
  oai22  g100(.a(new_n260_), .b(new_n237_), .c(new_n259_), .d(new_n258_), .O(new_n261_));
  nand2  g101(.a(new_n261_), .b(new_n235_), .O(new_n262_));
  xor2a  g102(.a(x83), .b(x81), .O(new_n263_));
  oai22  g103(.a(new_n263_), .b(new_n259_), .c(new_n260_), .d(x81), .O(new_n264_));
  nand2  g104(.a(new_n264_), .b(new_n238_), .O(new_n265_));
  inv1   g105(.a(new_n240_), .O(new_n266_));
  nand2  g106(.a(new_n266_), .b(new_n229_), .O(new_n267_));
  aoi21  g107(.a(new_n265_), .b(new_n262_), .c(new_n267_), .O(z33));
  nand2  g108(.a(x83), .b(x42), .O(new_n270_));
  nand2  g109(.a(new_n270_), .b(new_n237_), .O(new_n271_));
  nand3  g110(.a(x83), .b(x81), .c(x42), .O(new_n272_));
  nand2  g111(.a(new_n272_), .b(new_n271_), .O(new_n273_));
  and2   g112(.a(new_n273_), .b(new_n238_), .O(new_n274_));
  nand2  g113(.a(new_n270_), .b(x81), .O(new_n275_));
  nand2  g114(.a(new_n256_), .b(x42), .O(new_n276_));
  aoi21  g115(.a(new_n276_), .b(new_n275_), .c(new_n234_), .O(new_n277_));
  oai21  g116(.a(new_n277_), .b(new_n274_), .c(new_n266_), .O(new_n278_));
  nand2  g117(.a(new_n229_), .b(x53), .O(new_n279_));
  nor2   g118(.a(new_n279_), .b(new_n278_), .O(z35));
  nand2  g119(.a(new_n229_), .b(x54), .O(new_n281_));
  nor2   g120(.a(new_n281_), .b(new_n278_), .O(z36));
  nand2  g121(.a(new_n229_), .b(x55), .O(new_n283_));
  nor2   g122(.a(new_n283_), .b(new_n278_), .O(z37));
  nand2  g123(.a(new_n229_), .b(x56), .O(new_n285_));
  nor2   g124(.a(new_n285_), .b(new_n278_), .O(z38));
  nand2  g125(.a(new_n229_), .b(x57), .O(new_n287_));
  nor2   g126(.a(new_n287_), .b(new_n278_), .O(z39));
  inv1   g127(.a(new_n229_), .O(new_n289_));
  nor3   g128(.a(new_n278_), .b(new_n289_), .c(new_n188_), .O(z40));
  nor3   g129(.a(new_n278_), .b(new_n289_), .c(new_n185_), .O(z41));
  nor3   g130(.a(new_n278_), .b(new_n289_), .c(new_n176_), .O(z44));
  nand2  g131(.a(new_n229_), .b(x63), .O(new_n295_));
  nor2   g132(.a(new_n295_), .b(new_n278_), .O(z45));
  nand2  g133(.a(new_n229_), .b(x64), .O(new_n297_));
  nor2   g134(.a(new_n297_), .b(new_n278_), .O(z46));
  inv1   g135(.a(x08), .O(new_n300_));
  nand2  g136(.a(x52), .b(x16), .O(new_n301_));
  oai21  g137(.a(x52), .b(new_n300_), .c(new_n301_), .O(new_n302_));
  nand3  g138(.a(new_n220_), .b(new_n154_), .c(new_n162_), .O(new_n303_));
  inv1   g139(.a(new_n303_), .O(new_n304_));
  nand2  g140(.a(new_n304_), .b(new_n302_), .O(new_n305_));
  inv1   g141(.a(new_n165_), .O(new_n306_));
  nand2  g142(.a(new_n306_), .b(x79), .O(new_n307_));
  inv1   g143(.a(new_n307_), .O(new_n308_));
  and2   g144(.a(new_n308_), .b(new_n210_), .O(new_n309_));
  nand2  g145(.a(new_n309_), .b(x68), .O(new_n310_));
  aoi21  g146(.a(new_n310_), .b(new_n305_), .c(x01), .O(z48));
  inv1   g147(.a(x10), .O(new_n313_));
  nand2  g148(.a(x52), .b(x18), .O(new_n314_));
  oai21  g149(.a(x52), .b(new_n313_), .c(new_n314_), .O(new_n315_));
  nand2  g150(.a(new_n315_), .b(new_n304_), .O(new_n316_));
  nand2  g151(.a(new_n309_), .b(x70), .O(new_n317_));
  aoi21  g152(.a(new_n317_), .b(new_n316_), .c(x01), .O(z50));
  inv1   g153(.a(x11), .O(new_n319_));
  nand2  g154(.a(x52), .b(x19), .O(new_n320_));
  oai21  g155(.a(x52), .b(new_n319_), .c(new_n320_), .O(new_n321_));
  nand2  g156(.a(new_n321_), .b(new_n304_), .O(new_n322_));
  nand2  g157(.a(new_n309_), .b(x71), .O(new_n323_));
  aoi21  g158(.a(new_n323_), .b(new_n322_), .c(x01), .O(z51));
  inv1   g159(.a(x14), .O(new_n327_));
  nor2   g160(.a(x52), .b(new_n327_), .O(new_n328_));
  aoi21  g161(.a(x52), .b(x22), .c(new_n328_), .O(new_n329_));
  inv1   g162(.a(new_n163_), .O(new_n330_));
  nand4  g163(.a(new_n330_), .b(new_n154_), .c(x04), .d(new_n153_), .O(new_n331_));
  nor2   g164(.a(new_n331_), .b(new_n329_), .O(z54));
  inv1   g165(.a(x82), .O(new_n333_));
  nand3  g166(.a(new_n256_), .b(x84), .c(new_n333_), .O(new_n334_));
  inv1   g167(.a(x80), .O(new_n335_));
  nand4  g168(.a(new_n229_), .b(new_n226_), .c(new_n335_), .d(x79), .O(new_n336_));
  nor2   g169(.a(new_n336_), .b(new_n334_), .O(z55));
  nand2  g170(.a(new_n227_), .b(x76), .O(new_n338_));
  xnor2a g171(.a(x84), .b(x81), .O(new_n339_));
  aoi21  g172(.a(new_n165_), .b(new_n163_), .c(new_n339_), .O(new_n340_));
  nand2  g173(.a(new_n340_), .b(new_n153_), .O(new_n341_));
  nand2  g174(.a(new_n341_), .b(new_n338_), .O(new_n342_));
  nand2  g175(.a(new_n342_), .b(x79), .O(new_n343_));
  nand2  g176(.a(new_n164_), .b(new_n162_), .O(new_n344_));
  nand4  g177(.a(new_n344_), .b(new_n343_), .c(new_n153_), .d(x00), .O(z56));
  nand4  g178(.a(x80), .b(new_n215_), .c(x43), .d(new_n233_), .O(new_n347_));
  oai22  g179(.a(new_n347_), .b(new_n214_), .c(new_n233_), .d(x40), .O(new_n348_));
  nand3  g180(.a(new_n348_), .b(new_n220_), .c(x79), .O(new_n349_));
  nor2   g181(.a(x79), .b(x78), .O(new_n350_));
  nand3  g182(.a(new_n350_), .b(new_n233_), .c(x40), .O(new_n351_));
  nand2  g183(.a(new_n351_), .b(new_n349_), .O(new_n352_));
  nand2  g184(.a(new_n352_), .b(x77), .O(new_n353_));
  oai21  g185(.a(new_n330_), .b(new_n223_), .c(new_n154_), .O(new_n354_));
  aoi21  g186(.a(new_n354_), .b(new_n353_), .c(x01), .O(z58));
  inv1   g187(.a(new_n350_), .O(new_n356_));
  aoi21  g188(.a(new_n219_), .b(new_n356_), .c(new_n152_), .O(new_n357_));
  oai21  g189(.a(new_n216_), .b(new_n214_), .c(new_n233_), .O(new_n358_));
  aoi21  g190(.a(new_n358_), .b(x79), .c(new_n219_), .O(new_n359_));
  oai21  g191(.a(new_n359_), .b(new_n357_), .c(x77), .O(new_n360_));
  nor2   g192(.a(x79), .b(x04), .O(new_n361_));
  inv1   g193(.a(new_n361_), .O(new_n362_));
  aoi21  g194(.a(new_n362_), .b(new_n360_), .c(x01), .O(z59));
  aoi21  g195(.a(new_n340_), .b(x79), .c(new_n361_), .O(new_n364_));
  aoi21  g196(.a(new_n364_), .b(new_n221_), .c(x01), .O(z60));
  inv1   g197(.a(new_n167_), .O(new_n366_));
  nand2  g198(.a(new_n165_), .b(new_n163_), .O(new_n367_));
  aoi22  g199(.a(new_n367_), .b(new_n210_), .c(new_n226_), .d(new_n223_), .O(new_n368_));
  nor3   g200(.a(new_n368_), .b(new_n366_), .c(new_n335_), .O(z61));
  nand3  g201(.a(x84), .b(x81), .c(x79), .O(new_n370_));
  oai21  g202(.a(x79), .b(new_n223_), .c(new_n370_), .O(new_n371_));
  nand2  g203(.a(new_n371_), .b(new_n162_), .O(new_n372_));
  nand2  g204(.a(new_n358_), .b(x79), .O(new_n373_));
  nand3  g205(.a(x81), .b(x79), .c(new_n223_), .O(new_n374_));
  inv1   g206(.a(new_n374_), .O(new_n375_));
  aoi21  g207(.a(new_n373_), .b(x04), .c(new_n375_), .O(new_n376_));
  oai21  g208(.a(new_n376_), .b(new_n162_), .c(new_n372_), .O(new_n377_));
  nand2  g209(.a(new_n377_), .b(x78), .O(new_n378_));
  inv1   g210(.a(new_n370_), .O(new_n379_));
  nand2  g211(.a(new_n379_), .b(new_n306_), .O(new_n380_));
  aoi21  g212(.a(new_n380_), .b(new_n378_), .c(x01), .O(z62));
  nor3   g213(.a(new_n368_), .b(new_n366_), .c(new_n333_), .O(z63));
  nand2  g214(.a(x83), .b(x79), .O(new_n383_));
  or2    g215(.a(new_n383_), .b(new_n368_), .O(new_n384_));
  nand3  g216(.a(new_n169_), .b(new_n162_), .c(x04), .O(new_n385_));
  aoi21  g217(.a(new_n385_), .b(new_n384_), .c(x01), .O(z64));
  nor2   g218(.a(new_n164_), .b(x04), .O(new_n387_));
  nor2   g219(.a(new_n237_), .b(x78), .O(new_n388_));
  oai21  g220(.a(new_n388_), .b(new_n387_), .c(x77), .O(new_n389_));
  nand2  g221(.a(new_n330_), .b(x81), .O(new_n390_));
  nand2  g222(.a(new_n167_), .b(x84), .O(new_n391_));
  aoi21  g223(.a(new_n390_), .b(new_n389_), .c(new_n391_), .O(z65));
  zero   g224(.O(z01));
  zero   g225(.O(z05));
  zero   g226(.O(z06));
  zero   g227(.O(z07));
  zero   g228(.O(z13));
  zero   g229(.O(z14));
  zero   g230(.O(z16));
  zero   g231(.O(z20));
  zero   g232(.O(z25));
  zero   g233(.O(z34));
  zero   g234(.O(z42));
  zero   g235(.O(z43));
  zero   g236(.O(z47));
  zero   g237(.O(z49));
  zero   g238(.O(z52));
  zero   g239(.O(z53));
  zero   g240(.O(z57));
endmodule


