// Benchmark "FAU" written by ABC on Fri Jul 10 18:11:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n185_, new_n186_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n195_, new_n196_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n205_, new_n206_,
    new_n209_, new_n210_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n229_, new_n230_, new_n231_, new_n234_,
    new_n235_, new_n236_, new_n239_, new_n240_, new_n241_, new_n244_,
    new_n245_, new_n246_, new_n247_, new_n248_, new_n249_, new_n250_,
    new_n251_, new_n252_, new_n253_, new_n255_, new_n256_, new_n257_,
    new_n258_, new_n259_, new_n260_, new_n261_, new_n262_, new_n263_,
    new_n264_, new_n266_, new_n268_, new_n272_, new_n274_, new_n276_,
    new_n279_, new_n281_, new_n283_, new_n285_, new_n287_, new_n288_,
    new_n289_, new_n290_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n298_, new_n299_, new_n300_, new_n301_, new_n302_,
    new_n304_, new_n305_, new_n306_, new_n307_, new_n308_, new_n312_,
    new_n313_, new_n314_, new_n315_, new_n316_, new_n318_, new_n319_,
    new_n320_, new_n321_, new_n323_, new_n324_, new_n325_, new_n326_,
    new_n327_, new_n330_, new_n331_, new_n336_, new_n337_, new_n338_,
    new_n339_;
  inv1   g000(.a(x77), .O(new_n153_));
  inv1   g001(.a(x78), .O(new_n154_));
  nand2  g002(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  inv1   g003(.a(x79), .O(new_n156_));
  nor2   g004(.a(new_n154_), .b(new_n153_), .O(new_n157_));
  nor2   g005(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  aoi21  g006(.a(new_n158_), .b(new_n155_), .c(x01), .O(z01));
  nor2   g007(.a(new_n154_), .b(x77), .O(new_n160_));
  nand2  g008(.a(new_n160_), .b(x75), .O(new_n161_));
  nor2   g009(.a(x78), .b(new_n153_), .O(new_n162_));
  nand2  g010(.a(new_n162_), .b(x66), .O(new_n163_));
  inv1   g011(.a(x01), .O(new_n164_));
  nand2  g012(.a(x79), .b(new_n164_), .O(new_n165_));
  aoi21  g013(.a(new_n163_), .b(new_n161_), .c(new_n165_), .O(z02));
  inv1   g014(.a(x23), .O(new_n169_));
  nand2  g015(.a(x65), .b(x40), .O(new_n170_));
  oai21  g016(.a(x40), .b(new_n169_), .c(new_n170_), .O(z05));
  inv1   g017(.a(x24), .O(new_n172_));
  nand2  g018(.a(x64), .b(x40), .O(new_n173_));
  oai21  g019(.a(x40), .b(new_n172_), .c(new_n173_), .O(z06));
  inv1   g020(.a(x25), .O(new_n175_));
  nand2  g021(.a(x63), .b(x40), .O(new_n176_));
  oai21  g022(.a(x40), .b(new_n175_), .c(new_n176_), .O(z07));
  inv1   g023(.a(x26), .O(new_n178_));
  nand2  g024(.a(x62), .b(x40), .O(new_n179_));
  oai21  g025(.a(x40), .b(new_n178_), .c(new_n179_), .O(z08));
  inv1   g026(.a(x27), .O(new_n181_));
  nand2  g027(.a(x61), .b(x40), .O(new_n182_));
  oai21  g028(.a(x40), .b(new_n181_), .c(new_n182_), .O(z09));
  inv1   g029(.a(x29), .O(new_n185_));
  nand2  g030(.a(x59), .b(x40), .O(new_n186_));
  oai21  g031(.a(x40), .b(new_n185_), .c(new_n186_), .O(z11));
  inv1   g032(.a(x31), .O(new_n189_));
  nand2  g033(.a(x57), .b(x40), .O(new_n190_));
  oai21  g034(.a(x40), .b(new_n189_), .c(new_n190_), .O(z13));
  inv1   g035(.a(x32), .O(new_n192_));
  nand2  g036(.a(x51), .b(x40), .O(new_n193_));
  oai21  g037(.a(x40), .b(new_n192_), .c(new_n193_), .O(z14));
  inv1   g038(.a(x33), .O(new_n195_));
  nand2  g039(.a(x50), .b(x40), .O(new_n196_));
  oai21  g040(.a(x40), .b(new_n195_), .c(new_n196_), .O(z15));
  inv1   g041(.a(x34), .O(new_n198_));
  nand2  g042(.a(x49), .b(x40), .O(new_n199_));
  oai21  g043(.a(x40), .b(new_n198_), .c(new_n199_), .O(z16));
  inv1   g044(.a(x35), .O(new_n201_));
  nand2  g045(.a(x48), .b(x40), .O(new_n202_));
  oai21  g046(.a(x40), .b(new_n201_), .c(new_n202_), .O(z17));
  inv1   g047(.a(x37), .O(new_n205_));
  nand2  g048(.a(x46), .b(x40), .O(new_n206_));
  oai21  g049(.a(x40), .b(new_n205_), .c(new_n206_), .O(z19));
  inv1   g050(.a(x39), .O(new_n209_));
  nand2  g051(.a(x44), .b(x40), .O(new_n210_));
  oai21  g052(.a(x40), .b(new_n209_), .c(new_n210_), .O(z21));
  inv1   g053(.a(x81), .O(new_n215_));
  xor2a  g054(.a(x84), .b(x82), .O(new_n216_));
  xor2a  g055(.a(x84), .b(x82), .O(new_n217_));
  nand2  g056(.a(new_n217_), .b(new_n215_), .O(new_n218_));
  oai21  g057(.a(new_n216_), .b(new_n215_), .c(new_n218_), .O(new_n219_));
  nand3  g058(.a(x79), .b(x78), .c(x77), .O(new_n220_));
  inv1   g059(.a(new_n220_), .O(new_n221_));
  inv1   g060(.a(x42), .O(new_n222_));
  nor2   g061(.a(x04), .b(x01), .O(new_n223_));
  nand3  g062(.a(new_n223_), .b(new_n222_), .c(x05), .O(new_n224_));
  inv1   g063(.a(new_n224_), .O(new_n225_));
  nand3  g064(.a(new_n225_), .b(new_n221_), .c(new_n219_), .O(new_n226_));
  inv1   g065(.a(new_n226_), .O(z25));
  nand3  g066(.a(new_n223_), .b(x45), .c(new_n222_), .O(new_n229_));
  inv1   g067(.a(new_n229_), .O(new_n230_));
  nand3  g068(.a(new_n230_), .b(new_n221_), .c(new_n219_), .O(new_n231_));
  inv1   g069(.a(new_n231_), .O(z27));
  nand3  g070(.a(new_n223_), .b(x47), .c(new_n222_), .O(new_n234_));
  inv1   g071(.a(new_n234_), .O(new_n235_));
  nand3  g072(.a(new_n235_), .b(new_n221_), .c(new_n219_), .O(new_n236_));
  inv1   g073(.a(new_n236_), .O(z29));
  nand3  g074(.a(new_n223_), .b(x49), .c(new_n222_), .O(new_n239_));
  inv1   g075(.a(new_n239_), .O(new_n240_));
  nand3  g076(.a(new_n240_), .b(new_n221_), .c(new_n219_), .O(new_n241_));
  inv1   g077(.a(new_n241_), .O(z31));
  inv1   g078(.a(new_n216_), .O(new_n244_));
  xnor2a g079(.a(x83), .b(x81), .O(new_n245_));
  nand2  g080(.a(x42), .b(x05), .O(new_n246_));
  nand2  g081(.a(x51), .b(new_n222_), .O(new_n247_));
  oai22  g082(.a(new_n247_), .b(new_n215_), .c(new_n246_), .d(new_n245_), .O(new_n248_));
  nand2  g083(.a(new_n248_), .b(new_n244_), .O(new_n249_));
  xor2a  g084(.a(x83), .b(x81), .O(new_n250_));
  oai22  g085(.a(new_n250_), .b(new_n246_), .c(new_n247_), .d(x81), .O(new_n251_));
  nand2  g086(.a(new_n251_), .b(new_n217_), .O(new_n252_));
  nand2  g087(.a(new_n223_), .b(new_n221_), .O(new_n253_));
  aoi21  g088(.a(new_n252_), .b(new_n249_), .c(new_n253_), .O(z33));
  xnor2a g089(.a(x84), .b(x82), .O(new_n255_));
  nand2  g090(.a(x83), .b(x42), .O(new_n256_));
  nand2  g091(.a(new_n256_), .b(new_n215_), .O(new_n257_));
  nand3  g092(.a(x83), .b(x81), .c(x42), .O(new_n258_));
  aoi21  g093(.a(new_n258_), .b(new_n257_), .c(new_n255_), .O(new_n259_));
  nand2  g094(.a(new_n256_), .b(x81), .O(new_n260_));
  nand3  g095(.a(x83), .b(new_n215_), .c(x42), .O(new_n261_));
  aoi21  g096(.a(new_n261_), .b(new_n260_), .c(new_n216_), .O(new_n262_));
  oai21  g097(.a(new_n262_), .b(new_n259_), .c(new_n221_), .O(new_n263_));
  nand2  g098(.a(new_n223_), .b(x52), .O(new_n264_));
  nor2   g099(.a(new_n264_), .b(new_n263_), .O(z34));
  nand2  g100(.a(new_n223_), .b(x53), .O(new_n266_));
  nor2   g101(.a(new_n266_), .b(new_n263_), .O(z35));
  nand2  g102(.a(new_n223_), .b(x54), .O(new_n268_));
  nor2   g103(.a(new_n268_), .b(new_n263_), .O(z36));
  nand2  g104(.a(new_n223_), .b(x57), .O(new_n272_));
  nor2   g105(.a(new_n272_), .b(new_n263_), .O(z39));
  nand2  g106(.a(new_n223_), .b(x58), .O(new_n274_));
  nor2   g107(.a(new_n274_), .b(new_n263_), .O(z40));
  nand2  g108(.a(new_n223_), .b(x59), .O(new_n276_));
  nor2   g109(.a(new_n276_), .b(new_n263_), .O(z41));
  nand2  g110(.a(new_n223_), .b(x61), .O(new_n279_));
  nor2   g111(.a(new_n279_), .b(new_n263_), .O(z43));
  nand2  g112(.a(new_n223_), .b(x62), .O(new_n281_));
  nor2   g113(.a(new_n281_), .b(new_n263_), .O(z44));
  nand2  g114(.a(new_n223_), .b(x63), .O(new_n283_));
  nor2   g115(.a(new_n283_), .b(new_n263_), .O(z45));
  nand2  g116(.a(new_n223_), .b(x64), .O(new_n285_));
  nor2   g117(.a(new_n285_), .b(new_n263_), .O(z46));
  inv1   g118(.a(x07), .O(new_n287_));
  nand2  g119(.a(x52), .b(x15), .O(new_n288_));
  oai21  g120(.a(x52), .b(new_n287_), .c(new_n288_), .O(new_n289_));
  nand4  g121(.a(new_n156_), .b(x78), .c(new_n153_), .d(x04), .O(new_n290_));
  inv1   g122(.a(new_n290_), .O(new_n291_));
  nand2  g123(.a(new_n291_), .b(new_n289_), .O(new_n292_));
  xnor2a g124(.a(x84), .b(x81), .O(new_n293_));
  or2    g125(.a(x75), .b(x67), .O(new_n294_));
  nand4  g126(.a(new_n294_), .b(new_n293_), .c(new_n162_), .d(x79), .O(new_n295_));
  aoi21  g127(.a(new_n295_), .b(new_n292_), .c(x01), .O(z47));
  inv1   g128(.a(x09), .O(new_n298_));
  nand2  g129(.a(x52), .b(x17), .O(new_n299_));
  oai21  g130(.a(x52), .b(new_n298_), .c(new_n299_), .O(new_n300_));
  nand2  g131(.a(new_n300_), .b(new_n291_), .O(new_n301_));
  nand4  g132(.a(new_n293_), .b(new_n162_), .c(x79), .d(x69), .O(new_n302_));
  aoi21  g133(.a(new_n302_), .b(new_n301_), .c(x01), .O(z49));
  inv1   g134(.a(x10), .O(new_n304_));
  nand2  g135(.a(x52), .b(x18), .O(new_n305_));
  oai21  g136(.a(x52), .b(new_n304_), .c(new_n305_), .O(new_n306_));
  nand2  g137(.a(new_n306_), .b(new_n291_), .O(new_n307_));
  nand4  g138(.a(new_n293_), .b(new_n162_), .c(x79), .d(x70), .O(new_n308_));
  aoi21  g139(.a(new_n308_), .b(new_n307_), .c(x01), .O(z50));
  inv1   g140(.a(x13), .O(new_n312_));
  nand2  g141(.a(x52), .b(x21), .O(new_n313_));
  oai21  g142(.a(x52), .b(new_n312_), .c(new_n313_), .O(new_n314_));
  nand2  g143(.a(new_n314_), .b(new_n291_), .O(new_n315_));
  nand4  g144(.a(new_n293_), .b(new_n162_), .c(x79), .d(x73), .O(new_n316_));
  aoi21  g145(.a(new_n316_), .b(new_n315_), .c(x01), .O(z53));
  inv1   g146(.a(x14), .O(new_n318_));
  nor2   g147(.a(x52), .b(new_n318_), .O(new_n319_));
  aoi21  g148(.a(x52), .b(x22), .c(new_n319_), .O(new_n320_));
  nand4  g149(.a(new_n160_), .b(new_n156_), .c(x04), .d(new_n164_), .O(new_n321_));
  nor2   g150(.a(new_n321_), .b(new_n320_), .O(z54));
  inv1   g151(.a(x82), .O(new_n323_));
  nand2  g152(.a(x84), .b(new_n323_), .O(new_n324_));
  nand2  g153(.a(x83), .b(new_n215_), .O(new_n325_));
  nor2   g154(.a(x80), .b(new_n156_), .O(new_n326_));
  nand3  g155(.a(new_n326_), .b(new_n223_), .c(new_n157_), .O(new_n327_));
  nor3   g156(.a(new_n327_), .b(new_n325_), .c(new_n324_), .O(z55));
  inv1   g157(.a(x02), .O(new_n330_));
  nand4  g158(.a(x03), .b(new_n330_), .c(new_n164_), .d(x00), .O(new_n331_));
  inv1   g159(.a(new_n331_), .O(z57));
  oai21  g160(.a(new_n162_), .b(new_n160_), .c(new_n293_), .O(new_n336_));
  inv1   g161(.a(x04), .O(new_n337_));
  nand2  g162(.a(new_n157_), .b(new_n337_), .O(new_n338_));
  nand3  g163(.a(x80), .b(x79), .c(new_n164_), .O(new_n339_));
  aoi21  g164(.a(new_n338_), .b(new_n336_), .c(new_n339_), .O(z61));
  zero   g165(.O(z00));
  zero   g166(.O(z03));
  zero   g167(.O(z04));
  zero   g168(.O(z10));
  zero   g169(.O(z12));
  zero   g170(.O(z18));
  zero   g171(.O(z20));
  zero   g172(.O(z22));
  zero   g173(.O(z23));
  zero   g174(.O(z24));
  zero   g175(.O(z26));
  zero   g176(.O(z28));
  zero   g177(.O(z30));
  zero   g178(.O(z32));
  zero   g179(.O(z37));
  zero   g180(.O(z38));
  zero   g181(.O(z42));
  zero   g182(.O(z48));
  zero   g183(.O(z51));
  zero   g184(.O(z52));
  zero   g185(.O(z56));
  zero   g186(.O(z58));
  zero   g187(.O(z59));
  zero   g188(.O(z60));
  zero   g189(.O(z62));
  zero   g190(.O(z63));
  zero   g191(.O(z64));
  zero   g192(.O(z65));
endmodule


