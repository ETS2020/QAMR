// Benchmark "FAU" written by ABC on Fri Jul 10 18:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n186_, new_n187_, new_n189_, new_n190_,
    new_n193_, new_n194_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n208_, new_n209_, new_n210_, new_n211_,
    new_n212_, new_n213_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n227_, new_n228_, new_n229_, new_n231_, new_n232_,
    new_n233_, new_n235_, new_n236_, new_n237_, new_n240_, new_n241_,
    new_n242_, new_n244_, new_n245_, new_n246_, new_n249_, new_n250_,
    new_n251_, new_n252_, new_n253_, new_n254_, new_n255_, new_n256_,
    new_n257_, new_n258_, new_n261_, new_n262_, new_n263_, new_n264_,
    new_n265_, new_n266_, new_n267_, new_n268_, new_n269_, new_n270_,
    new_n272_, new_n275_, new_n277_, new_n279_, new_n282_, new_n284_,
    new_n286_, new_n288_, new_n290_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n297_, new_n298_, new_n299_, new_n300_,
    new_n301_, new_n303_, new_n304_, new_n305_, new_n306_, new_n307_,
    new_n309_, new_n310_, new_n311_, new_n312_, new_n313_, new_n316_,
    new_n317_, new_n318_, new_n319_, new_n320_, new_n322_, new_n323_,
    new_n324_, new_n325_, new_n326_, new_n328_, new_n329_, new_n330_,
    new_n331_, new_n332_, new_n334_, new_n335_, new_n336_, new_n337_,
    new_n339_, new_n340_, new_n341_, new_n342_, new_n343_, new_n346_,
    new_n347_, new_n354_, new_n355_, new_n356_;
  inv1   g000(.a(x78), .O(new_n154_));
  nor2   g001(.a(new_n154_), .b(x77), .O(new_n155_));
  nand2  g002(.a(new_n155_), .b(x75), .O(new_n156_));
  inv1   g003(.a(x77), .O(new_n157_));
  nor2   g004(.a(x78), .b(new_n157_), .O(new_n158_));
  nand2  g005(.a(new_n158_), .b(x66), .O(new_n159_));
  inv1   g006(.a(x01), .O(new_n160_));
  nand2  g007(.a(x79), .b(new_n160_), .O(new_n161_));
  aoi21  g008(.a(new_n159_), .b(new_n156_), .c(new_n161_), .O(z02));
  inv1   g009(.a(x23), .O(new_n165_));
  nand2  g010(.a(x65), .b(x40), .O(new_n166_));
  oai21  g011(.a(x40), .b(new_n165_), .c(new_n166_), .O(z05));
  inv1   g012(.a(x24), .O(new_n168_));
  nand2  g013(.a(x64), .b(x40), .O(new_n169_));
  oai21  g014(.a(x40), .b(new_n168_), .c(new_n169_), .O(z06));
  inv1   g015(.a(x25), .O(new_n171_));
  nand2  g016(.a(x63), .b(x40), .O(new_n172_));
  oai21  g017(.a(x40), .b(new_n171_), .c(new_n172_), .O(z07));
  inv1   g018(.a(x26), .O(new_n174_));
  nand2  g019(.a(x62), .b(x40), .O(new_n175_));
  oai21  g020(.a(x40), .b(new_n174_), .c(new_n175_), .O(z08));
  inv1   g021(.a(x29), .O(new_n179_));
  nand2  g022(.a(x59), .b(x40), .O(new_n180_));
  oai21  g023(.a(x40), .b(new_n179_), .c(new_n180_), .O(z11));
  inv1   g024(.a(x30), .O(new_n182_));
  nand2  g025(.a(x58), .b(x40), .O(new_n183_));
  oai21  g026(.a(x40), .b(new_n182_), .c(new_n183_), .O(z12));
  inv1   g027(.a(x32), .O(new_n186_));
  nand2  g028(.a(x51), .b(x40), .O(new_n187_));
  oai21  g029(.a(x40), .b(new_n186_), .c(new_n187_), .O(z14));
  inv1   g030(.a(x33), .O(new_n189_));
  nand2  g031(.a(x50), .b(x40), .O(new_n190_));
  oai21  g032(.a(x40), .b(new_n189_), .c(new_n190_), .O(z15));
  inv1   g033(.a(x35), .O(new_n193_));
  nand2  g034(.a(x48), .b(x40), .O(new_n194_));
  oai21  g035(.a(x40), .b(new_n193_), .c(new_n194_), .O(z17));
  inv1   g036(.a(x37), .O(new_n197_));
  nand2  g037(.a(x46), .b(x40), .O(new_n198_));
  oai21  g038(.a(x40), .b(new_n197_), .c(new_n198_), .O(z19));
  inv1   g039(.a(x38), .O(new_n200_));
  nand2  g040(.a(x45), .b(x40), .O(new_n201_));
  oai21  g041(.a(x40), .b(new_n200_), .c(new_n201_), .O(z20));
  inv1   g042(.a(x39), .O(new_n203_));
  nand2  g043(.a(x44), .b(x40), .O(new_n204_));
  oai21  g044(.a(x40), .b(new_n203_), .c(new_n204_), .O(z21));
  inv1   g045(.a(x79), .O(new_n208_));
  nor2   g046(.a(new_n154_), .b(new_n157_), .O(new_n209_));
  nor2   g047(.a(new_n209_), .b(new_n208_), .O(new_n210_));
  inv1   g048(.a(x43), .O(new_n211_));
  nor2   g049(.a(x04), .b(x01), .O(new_n212_));
  nand3  g050(.a(new_n212_), .b(new_n211_), .c(x05), .O(new_n213_));
  nor2   g051(.a(new_n213_), .b(new_n210_), .O(z24));
  inv1   g052(.a(x81), .O(new_n215_));
  xor2a  g053(.a(x84), .b(x82), .O(new_n216_));
  xor2a  g054(.a(x84), .b(x82), .O(new_n217_));
  nand2  g055(.a(new_n217_), .b(new_n215_), .O(new_n218_));
  oai21  g056(.a(new_n216_), .b(new_n215_), .c(new_n218_), .O(new_n219_));
  nand3  g057(.a(x79), .b(x78), .c(x77), .O(new_n220_));
  inv1   g058(.a(new_n220_), .O(new_n221_));
  inv1   g059(.a(x42), .O(new_n222_));
  nand3  g060(.a(new_n212_), .b(new_n222_), .c(x05), .O(new_n223_));
  inv1   g061(.a(new_n223_), .O(new_n224_));
  nand3  g062(.a(new_n224_), .b(new_n221_), .c(new_n219_), .O(new_n225_));
  inv1   g063(.a(new_n225_), .O(z25));
  nand3  g064(.a(new_n212_), .b(x44), .c(new_n222_), .O(new_n227_));
  inv1   g065(.a(new_n227_), .O(new_n228_));
  nand3  g066(.a(new_n228_), .b(new_n221_), .c(new_n219_), .O(new_n229_));
  inv1   g067(.a(new_n229_), .O(z26));
  nand3  g068(.a(new_n212_), .b(x45), .c(new_n222_), .O(new_n231_));
  inv1   g069(.a(new_n231_), .O(new_n232_));
  nand3  g070(.a(new_n232_), .b(new_n221_), .c(new_n219_), .O(new_n233_));
  inv1   g071(.a(new_n233_), .O(z27));
  nand3  g072(.a(new_n212_), .b(x46), .c(new_n222_), .O(new_n235_));
  inv1   g073(.a(new_n235_), .O(new_n236_));
  nand3  g074(.a(new_n236_), .b(new_n221_), .c(new_n219_), .O(new_n237_));
  inv1   g075(.a(new_n237_), .O(z28));
  nand3  g076(.a(new_n212_), .b(x48), .c(new_n222_), .O(new_n240_));
  inv1   g077(.a(new_n240_), .O(new_n241_));
  nand3  g078(.a(new_n241_), .b(new_n221_), .c(new_n219_), .O(new_n242_));
  inv1   g079(.a(new_n242_), .O(z30));
  nand3  g080(.a(new_n212_), .b(x49), .c(new_n222_), .O(new_n244_));
  inv1   g081(.a(new_n244_), .O(new_n245_));
  nand3  g082(.a(new_n245_), .b(new_n221_), .c(new_n219_), .O(new_n246_));
  inv1   g083(.a(new_n246_), .O(z31));
  inv1   g084(.a(new_n216_), .O(new_n249_));
  xnor2a g085(.a(x83), .b(x81), .O(new_n250_));
  nand2  g086(.a(x42), .b(x05), .O(new_n251_));
  nand2  g087(.a(x51), .b(new_n222_), .O(new_n252_));
  oai22  g088(.a(new_n252_), .b(new_n215_), .c(new_n251_), .d(new_n250_), .O(new_n253_));
  nand2  g089(.a(new_n253_), .b(new_n249_), .O(new_n254_));
  xor2a  g090(.a(x83), .b(x81), .O(new_n255_));
  oai22  g091(.a(new_n255_), .b(new_n251_), .c(new_n252_), .d(x81), .O(new_n256_));
  nand2  g092(.a(new_n256_), .b(new_n217_), .O(new_n257_));
  nand2  g093(.a(new_n221_), .b(new_n212_), .O(new_n258_));
  aoi21  g094(.a(new_n257_), .b(new_n254_), .c(new_n258_), .O(z33));
  xnor2a g095(.a(x84), .b(x82), .O(new_n261_));
  nand2  g096(.a(x83), .b(x42), .O(new_n262_));
  nand2  g097(.a(new_n262_), .b(new_n215_), .O(new_n263_));
  nand3  g098(.a(x83), .b(x81), .c(x42), .O(new_n264_));
  aoi21  g099(.a(new_n264_), .b(new_n263_), .c(new_n261_), .O(new_n265_));
  nand2  g100(.a(new_n262_), .b(x81), .O(new_n266_));
  nand3  g101(.a(x83), .b(new_n215_), .c(x42), .O(new_n267_));
  aoi21  g102(.a(new_n267_), .b(new_n266_), .c(new_n216_), .O(new_n268_));
  oai21  g103(.a(new_n268_), .b(new_n265_), .c(new_n221_), .O(new_n269_));
  nand2  g104(.a(new_n212_), .b(x53), .O(new_n270_));
  nor2   g105(.a(new_n270_), .b(new_n269_), .O(z35));
  nand2  g106(.a(new_n212_), .b(x54), .O(new_n272_));
  nor2   g107(.a(new_n272_), .b(new_n269_), .O(z36));
  nand2  g108(.a(new_n212_), .b(x56), .O(new_n275_));
  nor2   g109(.a(new_n275_), .b(new_n269_), .O(z38));
  nand2  g110(.a(new_n212_), .b(x57), .O(new_n277_));
  nor2   g111(.a(new_n277_), .b(new_n269_), .O(z39));
  nand2  g112(.a(new_n212_), .b(x58), .O(new_n279_));
  nor2   g113(.a(new_n279_), .b(new_n269_), .O(z40));
  nand2  g114(.a(new_n212_), .b(x60), .O(new_n282_));
  nor2   g115(.a(new_n282_), .b(new_n269_), .O(z42));
  nand2  g116(.a(new_n212_), .b(x61), .O(new_n284_));
  nor2   g117(.a(new_n284_), .b(new_n269_), .O(z43));
  nand2  g118(.a(new_n212_), .b(x62), .O(new_n286_));
  nor2   g119(.a(new_n286_), .b(new_n269_), .O(z44));
  nand2  g120(.a(new_n212_), .b(x63), .O(new_n288_));
  nor2   g121(.a(new_n288_), .b(new_n269_), .O(z45));
  nand2  g122(.a(new_n212_), .b(x64), .O(new_n290_));
  nor2   g123(.a(new_n290_), .b(new_n269_), .O(z46));
  inv1   g124(.a(x07), .O(new_n292_));
  nand2  g125(.a(x52), .b(x15), .O(new_n293_));
  oai21  g126(.a(x52), .b(new_n292_), .c(new_n293_), .O(new_n294_));
  inv1   g127(.a(x04), .O(new_n295_));
  nor2   g128(.a(x77), .b(new_n295_), .O(new_n296_));
  nor2   g129(.a(x79), .b(new_n154_), .O(new_n297_));
  nand3  g130(.a(new_n297_), .b(new_n296_), .c(new_n294_), .O(new_n298_));
  xnor2a g131(.a(x84), .b(x81), .O(new_n299_));
  or2    g132(.a(x75), .b(x67), .O(new_n300_));
  nand4  g133(.a(new_n300_), .b(new_n299_), .c(new_n158_), .d(x79), .O(new_n301_));
  aoi21  g134(.a(new_n301_), .b(new_n298_), .c(x01), .O(z47));
  inv1   g135(.a(x08), .O(new_n303_));
  nand2  g136(.a(x52), .b(x16), .O(new_n304_));
  oai21  g137(.a(x52), .b(new_n303_), .c(new_n304_), .O(new_n305_));
  nand3  g138(.a(new_n305_), .b(new_n297_), .c(new_n296_), .O(new_n306_));
  nand4  g139(.a(new_n299_), .b(new_n158_), .c(x79), .d(x68), .O(new_n307_));
  aoi21  g140(.a(new_n307_), .b(new_n306_), .c(x01), .O(z48));
  inv1   g141(.a(x09), .O(new_n309_));
  nand2  g142(.a(x52), .b(x17), .O(new_n310_));
  oai21  g143(.a(x52), .b(new_n309_), .c(new_n310_), .O(new_n311_));
  nand3  g144(.a(new_n311_), .b(new_n297_), .c(new_n296_), .O(new_n312_));
  nand4  g145(.a(new_n299_), .b(new_n158_), .c(x79), .d(x69), .O(new_n313_));
  aoi21  g146(.a(new_n313_), .b(new_n312_), .c(x01), .O(z49));
  inv1   g147(.a(x11), .O(new_n316_));
  nand2  g148(.a(x52), .b(x19), .O(new_n317_));
  oai21  g149(.a(x52), .b(new_n316_), .c(new_n317_), .O(new_n318_));
  nand3  g150(.a(new_n318_), .b(new_n297_), .c(new_n296_), .O(new_n319_));
  nand4  g151(.a(new_n299_), .b(new_n158_), .c(x79), .d(x71), .O(new_n320_));
  aoi21  g152(.a(new_n320_), .b(new_n319_), .c(x01), .O(z51));
  inv1   g153(.a(x12), .O(new_n322_));
  nand2  g154(.a(x52), .b(x20), .O(new_n323_));
  oai21  g155(.a(x52), .b(new_n322_), .c(new_n323_), .O(new_n324_));
  nand3  g156(.a(new_n324_), .b(new_n297_), .c(new_n296_), .O(new_n325_));
  nand4  g157(.a(new_n299_), .b(new_n158_), .c(x79), .d(x72), .O(new_n326_));
  aoi21  g158(.a(new_n326_), .b(new_n325_), .c(x01), .O(z52));
  inv1   g159(.a(x13), .O(new_n328_));
  nand2  g160(.a(x52), .b(x21), .O(new_n329_));
  oai21  g161(.a(x52), .b(new_n328_), .c(new_n329_), .O(new_n330_));
  nand3  g162(.a(new_n330_), .b(new_n297_), .c(new_n296_), .O(new_n331_));
  nand4  g163(.a(new_n299_), .b(new_n158_), .c(x79), .d(x73), .O(new_n332_));
  aoi21  g164(.a(new_n332_), .b(new_n331_), .c(x01), .O(z53));
  inv1   g165(.a(x14), .O(new_n334_));
  nor2   g166(.a(x52), .b(new_n334_), .O(new_n335_));
  aoi21  g167(.a(x52), .b(x22), .c(new_n335_), .O(new_n336_));
  nand4  g168(.a(new_n155_), .b(new_n208_), .c(x04), .d(new_n160_), .O(new_n337_));
  nor2   g169(.a(new_n337_), .b(new_n336_), .O(z54));
  inv1   g170(.a(x82), .O(new_n339_));
  nand2  g171(.a(x84), .b(new_n339_), .O(new_n340_));
  nand2  g172(.a(x83), .b(new_n215_), .O(new_n341_));
  nor2   g173(.a(x80), .b(new_n208_), .O(new_n342_));
  nand3  g174(.a(new_n342_), .b(new_n212_), .c(new_n209_), .O(new_n343_));
  nor3   g175(.a(new_n343_), .b(new_n341_), .c(new_n340_), .O(z55));
  inv1   g176(.a(x02), .O(new_n346_));
  nand4  g177(.a(x03), .b(new_n346_), .c(new_n160_), .d(x00), .O(new_n347_));
  inv1   g178(.a(new_n347_), .O(z57));
  oai21  g179(.a(new_n158_), .b(new_n155_), .c(new_n299_), .O(new_n354_));
  nand2  g180(.a(new_n209_), .b(new_n295_), .O(new_n355_));
  nand3  g181(.a(x82), .b(x79), .c(new_n160_), .O(new_n356_));
  aoi21  g182(.a(new_n355_), .b(new_n354_), .c(new_n356_), .O(z63));
  zero   g183(.O(z00));
  zero   g184(.O(z01));
  zero   g185(.O(z03));
  zero   g186(.O(z04));
  zero   g187(.O(z09));
  zero   g188(.O(z10));
  zero   g189(.O(z13));
  zero   g190(.O(z16));
  zero   g191(.O(z18));
  zero   g192(.O(z22));
  zero   g193(.O(z23));
  zero   g194(.O(z29));
  zero   g195(.O(z32));
  zero   g196(.O(z34));
  zero   g197(.O(z37));
  zero   g198(.O(z41));
  zero   g199(.O(z50));
  zero   g200(.O(z56));
  zero   g201(.O(z58));
  zero   g202(.O(z59));
  zero   g203(.O(z60));
  zero   g204(.O(z61));
  zero   g205(.O(z62));
  zero   g206(.O(z64));
  zero   g207(.O(z65));
endmodule


