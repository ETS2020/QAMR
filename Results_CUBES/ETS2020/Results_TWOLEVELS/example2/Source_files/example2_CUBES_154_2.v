// Benchmark "FAU" written by ABC on Fri Jul 10 18:20:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n199_, new_n200_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n219_, new_n220_,
    new_n221_, new_n224_, new_n225_, new_n226_, new_n228_, new_n229_,
    new_n230_, new_n232_, new_n233_, new_n234_, new_n237_, new_n238_,
    new_n239_, new_n240_, new_n241_, new_n242_, new_n243_, new_n244_,
    new_n245_, new_n246_, new_n248_, new_n249_, new_n250_, new_n251_,
    new_n252_, new_n253_, new_n254_, new_n255_, new_n256_, new_n257_,
    new_n259_, new_n262_, new_n265_, new_n268_, new_n270_, new_n272_,
    new_n274_, new_n276_, new_n279_, new_n280_, new_n281_, new_n282_,
    new_n283_, new_n284_, new_n285_, new_n286_, new_n287_, new_n288_,
    new_n291_, new_n292_, new_n293_, new_n294_, new_n295_, new_n297_,
    new_n298_, new_n299_, new_n300_, new_n301_, new_n303_, new_n304_,
    new_n305_, new_n306_, new_n307_, new_n309_, new_n310_, new_n311_,
    new_n312_, new_n313_, new_n315_, new_n316_, new_n317_, new_n318_,
    new_n319_, new_n322_, new_n323_, new_n324_, new_n325_, new_n326_,
    new_n327_, new_n330_, new_n331_, new_n336_, new_n337_, new_n338_,
    new_n339_, new_n342_;
  inv1   g000(.a(x78), .O(new_n154_));
  nor2   g001(.a(new_n154_), .b(x77), .O(new_n155_));
  nand2  g002(.a(new_n155_), .b(x75), .O(new_n156_));
  inv1   g003(.a(x77), .O(new_n157_));
  nor2   g004(.a(x78), .b(new_n157_), .O(new_n158_));
  nand2  g005(.a(new_n158_), .b(x66), .O(new_n159_));
  inv1   g006(.a(x01), .O(new_n160_));
  nand2  g007(.a(x79), .b(new_n160_), .O(new_n161_));
  aoi21  g008(.a(new_n159_), .b(new_n156_), .c(new_n161_), .O(z02));
  inv1   g009(.a(x23), .O(new_n165_));
  nand2  g010(.a(x65), .b(x40), .O(new_n166_));
  oai21  g011(.a(x40), .b(new_n165_), .c(new_n166_), .O(z05));
  inv1   g012(.a(x24), .O(new_n168_));
  nand2  g013(.a(x64), .b(x40), .O(new_n169_));
  oai21  g014(.a(x40), .b(new_n168_), .c(new_n169_), .O(z06));
  inv1   g015(.a(x25), .O(new_n171_));
  nand2  g016(.a(x63), .b(x40), .O(new_n172_));
  oai21  g017(.a(x40), .b(new_n171_), .c(new_n172_), .O(z07));
  inv1   g018(.a(x26), .O(new_n174_));
  nand2  g019(.a(x62), .b(x40), .O(new_n175_));
  oai21  g020(.a(x40), .b(new_n174_), .c(new_n175_), .O(z08));
  inv1   g021(.a(x27), .O(new_n177_));
  nand2  g022(.a(x61), .b(x40), .O(new_n178_));
  oai21  g023(.a(x40), .b(new_n177_), .c(new_n178_), .O(z09));
  inv1   g024(.a(x34), .O(new_n186_));
  nand2  g025(.a(x49), .b(x40), .O(new_n187_));
  oai21  g026(.a(x40), .b(new_n186_), .c(new_n187_), .O(z16));
  inv1   g027(.a(x35), .O(new_n189_));
  nand2  g028(.a(x48), .b(x40), .O(new_n190_));
  oai21  g029(.a(x40), .b(new_n189_), .c(new_n190_), .O(z17));
  inv1   g030(.a(x37), .O(new_n193_));
  nand2  g031(.a(x46), .b(x40), .O(new_n194_));
  oai21  g032(.a(x40), .b(new_n193_), .c(new_n194_), .O(z19));
  inv1   g033(.a(x38), .O(new_n196_));
  nand2  g034(.a(x45), .b(x40), .O(new_n197_));
  oai21  g035(.a(x40), .b(new_n196_), .c(new_n197_), .O(z20));
  inv1   g036(.a(x39), .O(new_n199_));
  nand2  g037(.a(x44), .b(x40), .O(new_n200_));
  oai21  g038(.a(x40), .b(new_n199_), .c(new_n200_), .O(z21));
  inv1   g039(.a(x81), .O(new_n205_));
  xor2a  g040(.a(x84), .b(x82), .O(new_n206_));
  xor2a  g041(.a(x84), .b(x82), .O(new_n207_));
  nand2  g042(.a(new_n207_), .b(new_n205_), .O(new_n208_));
  oai21  g043(.a(new_n206_), .b(new_n205_), .c(new_n208_), .O(new_n209_));
  nand3  g044(.a(x79), .b(x78), .c(x77), .O(new_n210_));
  inv1   g045(.a(new_n210_), .O(new_n211_));
  inv1   g046(.a(x42), .O(new_n212_));
  nor2   g047(.a(x04), .b(x01), .O(new_n213_));
  nand3  g048(.a(new_n213_), .b(new_n212_), .c(x05), .O(new_n214_));
  inv1   g049(.a(new_n214_), .O(new_n215_));
  nand3  g050(.a(new_n215_), .b(new_n211_), .c(new_n209_), .O(new_n216_));
  inv1   g051(.a(new_n216_), .O(z25));
  nand3  g052(.a(new_n213_), .b(x45), .c(new_n212_), .O(new_n219_));
  inv1   g053(.a(new_n219_), .O(new_n220_));
  nand3  g054(.a(new_n220_), .b(new_n211_), .c(new_n209_), .O(new_n221_));
  inv1   g055(.a(new_n221_), .O(z27));
  nand3  g056(.a(new_n213_), .b(x47), .c(new_n212_), .O(new_n224_));
  inv1   g057(.a(new_n224_), .O(new_n225_));
  nand3  g058(.a(new_n225_), .b(new_n211_), .c(new_n209_), .O(new_n226_));
  inv1   g059(.a(new_n226_), .O(z29));
  nand3  g060(.a(new_n213_), .b(x48), .c(new_n212_), .O(new_n228_));
  inv1   g061(.a(new_n228_), .O(new_n229_));
  nand3  g062(.a(new_n229_), .b(new_n211_), .c(new_n209_), .O(new_n230_));
  inv1   g063(.a(new_n230_), .O(z30));
  nand3  g064(.a(new_n213_), .b(x49), .c(new_n212_), .O(new_n232_));
  inv1   g065(.a(new_n232_), .O(new_n233_));
  nand3  g066(.a(new_n233_), .b(new_n211_), .c(new_n209_), .O(new_n234_));
  inv1   g067(.a(new_n234_), .O(z31));
  inv1   g068(.a(new_n206_), .O(new_n237_));
  xnor2a g069(.a(x83), .b(x81), .O(new_n238_));
  nand2  g070(.a(x42), .b(x05), .O(new_n239_));
  nand2  g071(.a(x51), .b(new_n212_), .O(new_n240_));
  oai22  g072(.a(new_n240_), .b(new_n205_), .c(new_n239_), .d(new_n238_), .O(new_n241_));
  nand2  g073(.a(new_n241_), .b(new_n237_), .O(new_n242_));
  xor2a  g074(.a(x83), .b(x81), .O(new_n243_));
  oai22  g075(.a(new_n243_), .b(new_n239_), .c(new_n240_), .d(x81), .O(new_n244_));
  nand2  g076(.a(new_n244_), .b(new_n207_), .O(new_n245_));
  nand2  g077(.a(new_n213_), .b(new_n211_), .O(new_n246_));
  aoi21  g078(.a(new_n245_), .b(new_n242_), .c(new_n246_), .O(z33));
  xnor2a g079(.a(x84), .b(x82), .O(new_n248_));
  nand2  g080(.a(x83), .b(x42), .O(new_n249_));
  nand2  g081(.a(new_n249_), .b(new_n205_), .O(new_n250_));
  nand3  g082(.a(x83), .b(x81), .c(x42), .O(new_n251_));
  aoi21  g083(.a(new_n251_), .b(new_n250_), .c(new_n248_), .O(new_n252_));
  nand2  g084(.a(new_n249_), .b(x81), .O(new_n253_));
  nand3  g085(.a(x83), .b(new_n205_), .c(x42), .O(new_n254_));
  aoi21  g086(.a(new_n254_), .b(new_n253_), .c(new_n206_), .O(new_n255_));
  oai21  g087(.a(new_n255_), .b(new_n252_), .c(new_n211_), .O(new_n256_));
  nand2  g088(.a(new_n213_), .b(x52), .O(new_n257_));
  nor2   g089(.a(new_n257_), .b(new_n256_), .O(z34));
  nand2  g090(.a(new_n213_), .b(x53), .O(new_n259_));
  nor2   g091(.a(new_n259_), .b(new_n256_), .O(z35));
  nand2  g092(.a(new_n213_), .b(x55), .O(new_n262_));
  nor2   g093(.a(new_n262_), .b(new_n256_), .O(z37));
  nand2  g094(.a(new_n213_), .b(x57), .O(new_n265_));
  nor2   g095(.a(new_n265_), .b(new_n256_), .O(z39));
  nand2  g096(.a(new_n213_), .b(x59), .O(new_n268_));
  nor2   g097(.a(new_n268_), .b(new_n256_), .O(z41));
  nand2  g098(.a(new_n213_), .b(x60), .O(new_n270_));
  nor2   g099(.a(new_n270_), .b(new_n256_), .O(z42));
  nand2  g100(.a(new_n213_), .b(x61), .O(new_n272_));
  nor2   g101(.a(new_n272_), .b(new_n256_), .O(z43));
  nand2  g102(.a(new_n213_), .b(x62), .O(new_n274_));
  nor2   g103(.a(new_n274_), .b(new_n256_), .O(z44));
  nand2  g104(.a(new_n213_), .b(x63), .O(new_n276_));
  nor2   g105(.a(new_n276_), .b(new_n256_), .O(z45));
  inv1   g106(.a(x07), .O(new_n279_));
  nand2  g107(.a(x52), .b(x15), .O(new_n280_));
  oai21  g108(.a(x52), .b(new_n279_), .c(new_n280_), .O(new_n281_));
  inv1   g109(.a(x04), .O(new_n282_));
  nor2   g110(.a(x77), .b(new_n282_), .O(new_n283_));
  nor2   g111(.a(x79), .b(new_n154_), .O(new_n284_));
  nand3  g112(.a(new_n284_), .b(new_n283_), .c(new_n281_), .O(new_n285_));
  xnor2a g113(.a(x84), .b(x81), .O(new_n286_));
  or2    g114(.a(x75), .b(x67), .O(new_n287_));
  nand4  g115(.a(new_n287_), .b(new_n286_), .c(new_n158_), .d(x79), .O(new_n288_));
  aoi21  g116(.a(new_n288_), .b(new_n285_), .c(x01), .O(z47));
  inv1   g117(.a(x09), .O(new_n291_));
  nand2  g118(.a(x52), .b(x17), .O(new_n292_));
  oai21  g119(.a(x52), .b(new_n291_), .c(new_n292_), .O(new_n293_));
  nand3  g120(.a(new_n293_), .b(new_n284_), .c(new_n283_), .O(new_n294_));
  nand4  g121(.a(new_n286_), .b(new_n158_), .c(x79), .d(x69), .O(new_n295_));
  aoi21  g122(.a(new_n295_), .b(new_n294_), .c(x01), .O(z49));
  inv1   g123(.a(x10), .O(new_n297_));
  nand2  g124(.a(x52), .b(x18), .O(new_n298_));
  oai21  g125(.a(x52), .b(new_n297_), .c(new_n298_), .O(new_n299_));
  nand3  g126(.a(new_n299_), .b(new_n284_), .c(new_n283_), .O(new_n300_));
  nand4  g127(.a(new_n286_), .b(new_n158_), .c(x79), .d(x70), .O(new_n301_));
  aoi21  g128(.a(new_n301_), .b(new_n300_), .c(x01), .O(z50));
  inv1   g129(.a(x11), .O(new_n303_));
  nand2  g130(.a(x52), .b(x19), .O(new_n304_));
  oai21  g131(.a(x52), .b(new_n303_), .c(new_n304_), .O(new_n305_));
  nand3  g132(.a(new_n305_), .b(new_n284_), .c(new_n283_), .O(new_n306_));
  nand4  g133(.a(new_n286_), .b(new_n158_), .c(x79), .d(x71), .O(new_n307_));
  aoi21  g134(.a(new_n307_), .b(new_n306_), .c(x01), .O(z51));
  inv1   g135(.a(x12), .O(new_n309_));
  nand2  g136(.a(x52), .b(x20), .O(new_n310_));
  oai21  g137(.a(x52), .b(new_n309_), .c(new_n310_), .O(new_n311_));
  nand3  g138(.a(new_n311_), .b(new_n284_), .c(new_n283_), .O(new_n312_));
  nand4  g139(.a(new_n286_), .b(new_n158_), .c(x79), .d(x72), .O(new_n313_));
  aoi21  g140(.a(new_n313_), .b(new_n312_), .c(x01), .O(z52));
  inv1   g141(.a(x13), .O(new_n315_));
  nand2  g142(.a(x52), .b(x21), .O(new_n316_));
  oai21  g143(.a(x52), .b(new_n315_), .c(new_n316_), .O(new_n317_));
  nand3  g144(.a(new_n317_), .b(new_n284_), .c(new_n283_), .O(new_n318_));
  nand4  g145(.a(new_n286_), .b(new_n158_), .c(x79), .d(x73), .O(new_n319_));
  aoi21  g146(.a(new_n319_), .b(new_n318_), .c(x01), .O(z53));
  inv1   g147(.a(x82), .O(new_n322_));
  nand2  g148(.a(x84), .b(new_n322_), .O(new_n323_));
  inv1   g149(.a(new_n213_), .O(new_n324_));
  nand2  g150(.a(x83), .b(new_n205_), .O(new_n325_));
  inv1   g151(.a(x80), .O(new_n326_));
  nand4  g152(.a(new_n326_), .b(x79), .c(x78), .d(x77), .O(new_n327_));
  nor4   g153(.a(new_n327_), .b(new_n325_), .c(new_n324_), .d(new_n323_), .O(z55));
  inv1   g154(.a(x02), .O(new_n330_));
  nand4  g155(.a(x03), .b(new_n330_), .c(new_n160_), .d(x00), .O(new_n331_));
  inv1   g156(.a(new_n331_), .O(z57));
  oai21  g157(.a(new_n158_), .b(new_n155_), .c(new_n286_), .O(new_n336_));
  nand3  g158(.a(x78), .b(x77), .c(new_n282_), .O(new_n337_));
  inv1   g159(.a(new_n161_), .O(new_n338_));
  nand2  g160(.a(new_n338_), .b(x80), .O(new_n339_));
  aoi21  g161(.a(new_n337_), .b(new_n336_), .c(new_n339_), .O(z61));
  nand2  g162(.a(new_n338_), .b(x82), .O(new_n342_));
  aoi21  g163(.a(new_n337_), .b(new_n336_), .c(new_n342_), .O(z63));
  zero   g164(.O(z00));
  zero   g165(.O(z01));
  zero   g166(.O(z03));
  zero   g167(.O(z04));
  zero   g168(.O(z10));
  zero   g169(.O(z11));
  zero   g170(.O(z12));
  zero   g171(.O(z13));
  zero   g172(.O(z14));
  zero   g173(.O(z15));
  zero   g174(.O(z18));
  zero   g175(.O(z22));
  zero   g176(.O(z23));
  zero   g177(.O(z24));
  zero   g178(.O(z26));
  zero   g179(.O(z28));
  zero   g180(.O(z32));
  zero   g181(.O(z36));
  zero   g182(.O(z38));
  zero   g183(.O(z40));
  zero   g184(.O(z46));
  zero   g185(.O(z48));
  zero   g186(.O(z54));
  zero   g187(.O(z56));
  zero   g188(.O(z58));
  zero   g189(.O(z59));
  zero   g190(.O(z60));
  zero   g191(.O(z62));
  zero   g192(.O(z64));
  zero   g193(.O(z65));
endmodule


