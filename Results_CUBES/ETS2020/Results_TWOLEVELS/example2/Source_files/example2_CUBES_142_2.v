// Benchmark "FAU" written by ABC on Fri Jul 10 18:19:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n167_, new_n168_,
    new_n171_, new_n172_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n199_, new_n200_,
    new_n202_, new_n203_, new_n205_, new_n206_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n224_, new_n225_,
    new_n226_, new_n228_, new_n229_, new_n230_, new_n232_, new_n233_,
    new_n234_, new_n236_, new_n237_, new_n238_, new_n241_, new_n242_,
    new_n243_, new_n245_, new_n246_, new_n247_, new_n249_, new_n250_,
    new_n251_, new_n252_, new_n253_, new_n254_, new_n255_, new_n256_,
    new_n257_, new_n258_, new_n262_, new_n263_, new_n264_, new_n265_,
    new_n266_, new_n267_, new_n268_, new_n269_, new_n270_, new_n271_,
    new_n273_, new_n275_, new_n277_, new_n279_, new_n281_, new_n283_,
    new_n286_, new_n288_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n297_, new_n298_, new_n299_, new_n301_,
    new_n302_, new_n303_, new_n304_, new_n305_, new_n307_, new_n308_,
    new_n309_, new_n310_, new_n311_, new_n313_, new_n314_, new_n315_,
    new_n316_, new_n317_, new_n319_, new_n320_, new_n321_, new_n322_,
    new_n323_, new_n325_, new_n326_, new_n327_, new_n328_, new_n329_,
    new_n331_, new_n332_, new_n333_, new_n334_, new_n335_, new_n337_,
    new_n338_, new_n339_, new_n340_, new_n341_, new_n343_, new_n344_,
    new_n345_, new_n346_, new_n347_, new_n348_, new_n351_, new_n352_,
    new_n357_, new_n358_, new_n359_;
  inv1   g000(.a(x78), .O(new_n154_));
  nor2   g001(.a(new_n154_), .b(x77), .O(new_n155_));
  nand2  g002(.a(new_n155_), .b(x75), .O(new_n156_));
  inv1   g003(.a(x77), .O(new_n157_));
  nor2   g004(.a(x78), .b(new_n157_), .O(new_n158_));
  nand2  g005(.a(new_n158_), .b(x66), .O(new_n159_));
  inv1   g006(.a(x01), .O(new_n160_));
  nand2  g007(.a(x79), .b(new_n160_), .O(new_n161_));
  aoi21  g008(.a(new_n159_), .b(new_n156_), .c(new_n161_), .O(z02));
  nor2   g009(.a(x79), .b(new_n154_), .O(new_n163_));
  nand3  g010(.a(new_n163_), .b(x52), .c(new_n160_), .O(new_n164_));
  inv1   g011(.a(new_n164_), .O(z03));
  inv1   g012(.a(x23), .O(new_n167_));
  nand2  g013(.a(x65), .b(x40), .O(new_n168_));
  oai21  g014(.a(x40), .b(new_n167_), .c(new_n168_), .O(z05));
  inv1   g015(.a(x25), .O(new_n171_));
  nand2  g016(.a(x63), .b(x40), .O(new_n172_));
  oai21  g017(.a(x40), .b(new_n171_), .c(new_n172_), .O(z07));
  inv1   g018(.a(x30), .O(new_n178_));
  nand2  g019(.a(x58), .b(x40), .O(new_n179_));
  oai21  g020(.a(x40), .b(new_n178_), .c(new_n179_), .O(z12));
  inv1   g021(.a(x31), .O(new_n181_));
  nand2  g022(.a(x57), .b(x40), .O(new_n182_));
  oai21  g023(.a(x40), .b(new_n181_), .c(new_n182_), .O(z13));
  inv1   g024(.a(x32), .O(new_n184_));
  nand2  g025(.a(x51), .b(x40), .O(new_n185_));
  oai21  g026(.a(x40), .b(new_n184_), .c(new_n185_), .O(z14));
  inv1   g027(.a(x33), .O(new_n187_));
  nand2  g028(.a(x50), .b(x40), .O(new_n188_));
  oai21  g029(.a(x40), .b(new_n187_), .c(new_n188_), .O(z15));
  inv1   g030(.a(x34), .O(new_n190_));
  nand2  g031(.a(x49), .b(x40), .O(new_n191_));
  oai21  g032(.a(x40), .b(new_n190_), .c(new_n191_), .O(z16));
  inv1   g033(.a(x35), .O(new_n193_));
  nand2  g034(.a(x48), .b(x40), .O(new_n194_));
  oai21  g035(.a(x40), .b(new_n193_), .c(new_n194_), .O(z17));
  inv1   g036(.a(x36), .O(new_n196_));
  nand2  g037(.a(x47), .b(x40), .O(new_n197_));
  oai21  g038(.a(x40), .b(new_n196_), .c(new_n197_), .O(z18));
  inv1   g039(.a(x37), .O(new_n199_));
  nand2  g040(.a(x46), .b(x40), .O(new_n200_));
  oai21  g041(.a(x40), .b(new_n199_), .c(new_n200_), .O(z19));
  inv1   g042(.a(x38), .O(new_n202_));
  nand2  g043(.a(x45), .b(x40), .O(new_n203_));
  oai21  g044(.a(x40), .b(new_n202_), .c(new_n203_), .O(z20));
  inv1   g045(.a(x39), .O(new_n205_));
  nand2  g046(.a(x44), .b(x40), .O(new_n206_));
  oai21  g047(.a(x40), .b(new_n205_), .c(new_n206_), .O(z21));
  inv1   g048(.a(x81), .O(new_n211_));
  xor2a  g049(.a(x84), .b(x82), .O(new_n212_));
  xor2a  g050(.a(x84), .b(x82), .O(new_n213_));
  nand2  g051(.a(new_n213_), .b(new_n211_), .O(new_n214_));
  oai21  g052(.a(new_n212_), .b(new_n211_), .c(new_n214_), .O(new_n215_));
  nand3  g053(.a(x79), .b(x78), .c(x77), .O(new_n216_));
  inv1   g054(.a(new_n216_), .O(new_n217_));
  inv1   g055(.a(x42), .O(new_n218_));
  nor2   g056(.a(x04), .b(x01), .O(new_n219_));
  nand3  g057(.a(new_n219_), .b(new_n218_), .c(x05), .O(new_n220_));
  inv1   g058(.a(new_n220_), .O(new_n221_));
  nand3  g059(.a(new_n221_), .b(new_n217_), .c(new_n215_), .O(new_n222_));
  inv1   g060(.a(new_n222_), .O(z25));
  nand3  g061(.a(new_n219_), .b(x44), .c(new_n218_), .O(new_n224_));
  inv1   g062(.a(new_n224_), .O(new_n225_));
  nand3  g063(.a(new_n225_), .b(new_n217_), .c(new_n215_), .O(new_n226_));
  inv1   g064(.a(new_n226_), .O(z26));
  nand3  g065(.a(new_n219_), .b(x45), .c(new_n218_), .O(new_n228_));
  inv1   g066(.a(new_n228_), .O(new_n229_));
  nand3  g067(.a(new_n229_), .b(new_n217_), .c(new_n215_), .O(new_n230_));
  inv1   g068(.a(new_n230_), .O(z27));
  nand3  g069(.a(new_n219_), .b(x46), .c(new_n218_), .O(new_n232_));
  inv1   g070(.a(new_n232_), .O(new_n233_));
  nand3  g071(.a(new_n233_), .b(new_n217_), .c(new_n215_), .O(new_n234_));
  inv1   g072(.a(new_n234_), .O(z28));
  nand3  g073(.a(new_n219_), .b(x47), .c(new_n218_), .O(new_n236_));
  inv1   g074(.a(new_n236_), .O(new_n237_));
  nand3  g075(.a(new_n237_), .b(new_n217_), .c(new_n215_), .O(new_n238_));
  inv1   g076(.a(new_n238_), .O(z29));
  nand3  g077(.a(new_n219_), .b(x49), .c(new_n218_), .O(new_n241_));
  inv1   g078(.a(new_n241_), .O(new_n242_));
  nand3  g079(.a(new_n242_), .b(new_n217_), .c(new_n215_), .O(new_n243_));
  inv1   g080(.a(new_n243_), .O(z31));
  nand3  g081(.a(new_n219_), .b(x50), .c(new_n218_), .O(new_n245_));
  inv1   g082(.a(new_n245_), .O(new_n246_));
  nand3  g083(.a(new_n246_), .b(new_n217_), .c(new_n215_), .O(new_n247_));
  inv1   g084(.a(new_n247_), .O(z32));
  inv1   g085(.a(new_n212_), .O(new_n249_));
  xnor2a g086(.a(x83), .b(x81), .O(new_n250_));
  nand2  g087(.a(x42), .b(x05), .O(new_n251_));
  nand2  g088(.a(x51), .b(new_n218_), .O(new_n252_));
  oai22  g089(.a(new_n252_), .b(new_n211_), .c(new_n251_), .d(new_n250_), .O(new_n253_));
  nand2  g090(.a(new_n253_), .b(new_n249_), .O(new_n254_));
  xor2a  g091(.a(x83), .b(x81), .O(new_n255_));
  oai22  g092(.a(new_n255_), .b(new_n251_), .c(new_n252_), .d(x81), .O(new_n256_));
  nand2  g093(.a(new_n256_), .b(new_n213_), .O(new_n257_));
  nand2  g094(.a(new_n219_), .b(new_n217_), .O(new_n258_));
  aoi21  g095(.a(new_n257_), .b(new_n254_), .c(new_n258_), .O(z33));
  xnor2a g096(.a(x84), .b(x82), .O(new_n262_));
  nand2  g097(.a(x83), .b(x42), .O(new_n263_));
  nand2  g098(.a(new_n263_), .b(new_n211_), .O(new_n264_));
  nand3  g099(.a(x83), .b(x81), .c(x42), .O(new_n265_));
  aoi21  g100(.a(new_n265_), .b(new_n264_), .c(new_n262_), .O(new_n266_));
  nand2  g101(.a(new_n263_), .b(x81), .O(new_n267_));
  nand3  g102(.a(x83), .b(new_n211_), .c(x42), .O(new_n268_));
  aoi21  g103(.a(new_n268_), .b(new_n267_), .c(new_n212_), .O(new_n269_));
  oai21  g104(.a(new_n269_), .b(new_n266_), .c(new_n217_), .O(new_n270_));
  nand2  g105(.a(new_n219_), .b(x54), .O(new_n271_));
  nor2   g106(.a(new_n271_), .b(new_n270_), .O(z36));
  nand2  g107(.a(new_n219_), .b(x55), .O(new_n273_));
  nor2   g108(.a(new_n273_), .b(new_n270_), .O(z37));
  nand2  g109(.a(new_n219_), .b(x56), .O(new_n275_));
  nor2   g110(.a(new_n275_), .b(new_n270_), .O(z38));
  nand2  g111(.a(new_n219_), .b(x57), .O(new_n277_));
  nor2   g112(.a(new_n277_), .b(new_n270_), .O(z39));
  nand2  g113(.a(new_n219_), .b(x58), .O(new_n279_));
  nor2   g114(.a(new_n279_), .b(new_n270_), .O(z40));
  nand2  g115(.a(new_n219_), .b(x59), .O(new_n281_));
  nor2   g116(.a(new_n281_), .b(new_n270_), .O(z41));
  nand2  g117(.a(new_n219_), .b(x60), .O(new_n283_));
  nor2   g118(.a(new_n283_), .b(new_n270_), .O(z42));
  nand2  g119(.a(new_n219_), .b(x62), .O(new_n286_));
  nor2   g120(.a(new_n286_), .b(new_n270_), .O(z44));
  nand2  g121(.a(new_n219_), .b(x63), .O(new_n288_));
  nor2   g122(.a(new_n288_), .b(new_n270_), .O(z45));
  inv1   g123(.a(x07), .O(new_n291_));
  nand2  g124(.a(x52), .b(x15), .O(new_n292_));
  oai21  g125(.a(x52), .b(new_n291_), .c(new_n292_), .O(new_n293_));
  inv1   g126(.a(x04), .O(new_n294_));
  nor2   g127(.a(x77), .b(new_n294_), .O(new_n295_));
  nand3  g128(.a(new_n295_), .b(new_n293_), .c(new_n163_), .O(new_n296_));
  xnor2a g129(.a(x84), .b(x81), .O(new_n297_));
  or2    g130(.a(x75), .b(x67), .O(new_n298_));
  nand4  g131(.a(new_n298_), .b(new_n297_), .c(new_n158_), .d(x79), .O(new_n299_));
  aoi21  g132(.a(new_n299_), .b(new_n296_), .c(x01), .O(z47));
  inv1   g133(.a(x08), .O(new_n301_));
  nand2  g134(.a(x52), .b(x16), .O(new_n302_));
  oai21  g135(.a(x52), .b(new_n301_), .c(new_n302_), .O(new_n303_));
  nand3  g136(.a(new_n303_), .b(new_n295_), .c(new_n163_), .O(new_n304_));
  nand4  g137(.a(new_n297_), .b(new_n158_), .c(x79), .d(x68), .O(new_n305_));
  aoi21  g138(.a(new_n305_), .b(new_n304_), .c(x01), .O(z48));
  inv1   g139(.a(x09), .O(new_n307_));
  nand2  g140(.a(x52), .b(x17), .O(new_n308_));
  oai21  g141(.a(x52), .b(new_n307_), .c(new_n308_), .O(new_n309_));
  nand3  g142(.a(new_n309_), .b(new_n295_), .c(new_n163_), .O(new_n310_));
  nand4  g143(.a(new_n297_), .b(new_n158_), .c(x79), .d(x69), .O(new_n311_));
  aoi21  g144(.a(new_n311_), .b(new_n310_), .c(x01), .O(z49));
  inv1   g145(.a(x10), .O(new_n313_));
  nand2  g146(.a(x52), .b(x18), .O(new_n314_));
  oai21  g147(.a(x52), .b(new_n313_), .c(new_n314_), .O(new_n315_));
  nand3  g148(.a(new_n315_), .b(new_n295_), .c(new_n163_), .O(new_n316_));
  nand4  g149(.a(new_n297_), .b(new_n158_), .c(x79), .d(x70), .O(new_n317_));
  aoi21  g150(.a(new_n317_), .b(new_n316_), .c(x01), .O(z50));
  inv1   g151(.a(x11), .O(new_n319_));
  nand2  g152(.a(x52), .b(x19), .O(new_n320_));
  oai21  g153(.a(x52), .b(new_n319_), .c(new_n320_), .O(new_n321_));
  nand3  g154(.a(new_n321_), .b(new_n295_), .c(new_n163_), .O(new_n322_));
  nand4  g155(.a(new_n297_), .b(new_n158_), .c(x79), .d(x71), .O(new_n323_));
  aoi21  g156(.a(new_n323_), .b(new_n322_), .c(x01), .O(z51));
  inv1   g157(.a(x12), .O(new_n325_));
  nand2  g158(.a(x52), .b(x20), .O(new_n326_));
  oai21  g159(.a(x52), .b(new_n325_), .c(new_n326_), .O(new_n327_));
  nand3  g160(.a(new_n327_), .b(new_n295_), .c(new_n163_), .O(new_n328_));
  nand4  g161(.a(new_n297_), .b(new_n158_), .c(x79), .d(x72), .O(new_n329_));
  aoi21  g162(.a(new_n329_), .b(new_n328_), .c(x01), .O(z52));
  inv1   g163(.a(x13), .O(new_n331_));
  nand2  g164(.a(x52), .b(x21), .O(new_n332_));
  oai21  g165(.a(x52), .b(new_n331_), .c(new_n332_), .O(new_n333_));
  nand3  g166(.a(new_n333_), .b(new_n295_), .c(new_n163_), .O(new_n334_));
  nand4  g167(.a(new_n297_), .b(new_n158_), .c(x79), .d(x73), .O(new_n335_));
  aoi21  g168(.a(new_n335_), .b(new_n334_), .c(x01), .O(z53));
  inv1   g169(.a(x14), .O(new_n337_));
  nor2   g170(.a(x52), .b(new_n337_), .O(new_n338_));
  aoi21  g171(.a(x52), .b(x22), .c(new_n338_), .O(new_n339_));
  inv1   g172(.a(x79), .O(new_n340_));
  nand4  g173(.a(new_n155_), .b(new_n340_), .c(x04), .d(new_n160_), .O(new_n341_));
  nor2   g174(.a(new_n341_), .b(new_n339_), .O(z54));
  inv1   g175(.a(x82), .O(new_n343_));
  nand2  g176(.a(x84), .b(new_n343_), .O(new_n344_));
  inv1   g177(.a(new_n219_), .O(new_n345_));
  nand2  g178(.a(x83), .b(new_n211_), .O(new_n346_));
  inv1   g179(.a(x80), .O(new_n347_));
  nand4  g180(.a(new_n347_), .b(x79), .c(x78), .d(x77), .O(new_n348_));
  nor4   g181(.a(new_n348_), .b(new_n346_), .c(new_n345_), .d(new_n344_), .O(z55));
  inv1   g182(.a(x02), .O(new_n351_));
  nand4  g183(.a(x03), .b(new_n351_), .c(new_n160_), .d(x00), .O(new_n352_));
  inv1   g184(.a(new_n352_), .O(z57));
  oai21  g185(.a(new_n158_), .b(new_n155_), .c(new_n297_), .O(new_n357_));
  nand3  g186(.a(x78), .b(x77), .c(new_n294_), .O(new_n358_));
  nand3  g187(.a(x80), .b(x79), .c(new_n160_), .O(new_n359_));
  aoi21  g188(.a(new_n358_), .b(new_n357_), .c(new_n359_), .O(z61));
  zero   g189(.O(z00));
  zero   g190(.O(z01));
  zero   g191(.O(z04));
  zero   g192(.O(z06));
  zero   g193(.O(z08));
  zero   g194(.O(z09));
  zero   g195(.O(z10));
  zero   g196(.O(z11));
  zero   g197(.O(z22));
  zero   g198(.O(z23));
  zero   g199(.O(z24));
  zero   g200(.O(z30));
  zero   g201(.O(z34));
  zero   g202(.O(z35));
  zero   g203(.O(z43));
  zero   g204(.O(z46));
  zero   g205(.O(z56));
  zero   g206(.O(z58));
  zero   g207(.O(z59));
  zero   g208(.O(z60));
  zero   g209(.O(z62));
  zero   g210(.O(z63));
  zero   g211(.O(z64));
  zero   g212(.O(z65));
endmodule


