// Benchmark "FAU" written by ABC on Mon Jul  6 19:22:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n97_, new_n102_, new_n103_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n169_;
  nand2  g00(.a(x07), .b(x04), .O(new_n84_));
  inv1   g01(.a(x07), .O(new_n85_));
  nand2  g02(.a(x11), .b(new_n85_), .O(new_n86_));
  aoi21  g03(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g04(.a(x07), .b(x03), .O(new_n92_));
  nand2  g05(.a(x16), .b(new_n85_), .O(new_n93_));
  aoi21  g06(.a(new_n93_), .b(new_n92_), .c(x10), .O(z05));
  inv1   g07(.a(x08), .O(new_n95_));
  nand2  g08(.a(x17), .b(new_n95_), .O(new_n96_));
  nand2  g09(.a(x18), .b(x08), .O(new_n97_));
  aoi21  g10(.a(new_n97_), .b(new_n96_), .c(x10), .O(z06));
  nand2  g11(.a(x21), .b(new_n95_), .O(new_n102_));
  nand2  g12(.a(x22), .b(x08), .O(new_n103_));
  aoi21  g13(.a(new_n103_), .b(new_n102_), .c(x10), .O(z10));
  nand2  g14(.a(x24), .b(new_n95_), .O(new_n107_));
  nand2  g15(.a(x25), .b(x08), .O(new_n108_));
  aoi21  g16(.a(new_n108_), .b(new_n107_), .c(x10), .O(z13));
  nand2  g17(.a(x26), .b(new_n95_), .O(new_n111_));
  nand2  g18(.a(x27), .b(x08), .O(new_n112_));
  aoi21  g19(.a(new_n112_), .b(new_n111_), .c(x10), .O(z15));
  nand2  g20(.a(x08), .b(x00), .O(new_n117_));
  nand2  g21(.a(x30), .b(new_n95_), .O(new_n118_));
  aoi21  g22(.a(new_n118_), .b(new_n117_), .c(x10), .O(z19));
  inv1   g23(.a(x09), .O(new_n120_));
  nand2  g24(.a(x31), .b(new_n120_), .O(new_n121_));
  nand2  g25(.a(x32), .b(x09), .O(new_n122_));
  aoi21  g26(.a(new_n122_), .b(new_n121_), .c(x10), .O(z20));
  nand2  g27(.a(x32), .b(new_n120_), .O(new_n124_));
  nand2  g28(.a(x33), .b(x09), .O(new_n125_));
  aoi21  g29(.a(new_n125_), .b(new_n124_), .c(x10), .O(z21));
  nand2  g30(.a(x33), .b(new_n120_), .O(new_n127_));
  nand2  g31(.a(x34), .b(x09), .O(new_n128_));
  aoi21  g32(.a(new_n128_), .b(new_n127_), .c(x10), .O(z22));
  nand2  g33(.a(x35), .b(new_n120_), .O(new_n131_));
  nand2  g34(.a(x36), .b(x09), .O(new_n132_));
  aoi21  g35(.a(new_n132_), .b(new_n131_), .c(x10), .O(z24));
  nand2  g36(.a(x36), .b(new_n120_), .O(new_n134_));
  nand2  g37(.a(x37), .b(x09), .O(new_n135_));
  aoi21  g38(.a(new_n135_), .b(new_n134_), .c(x10), .O(z25));
  nand2  g39(.a(x37), .b(new_n120_), .O(new_n137_));
  nand2  g40(.a(x38), .b(x09), .O(new_n138_));
  aoi21  g41(.a(new_n138_), .b(new_n137_), .c(x10), .O(z26));
  inv1   g42(.a(x39), .O(new_n140_));
  nand2  g43(.a(x14), .b(x00), .O(new_n141_));
  oai21  g44(.a(new_n140_), .b(x14), .c(new_n141_), .O(new_n142_));
  nand2  g45(.a(new_n142_), .b(x09), .O(new_n143_));
  nand2  g46(.a(x38), .b(new_n120_), .O(new_n144_));
  aoi21  g47(.a(new_n144_), .b(new_n143_), .c(x10), .O(z27));
  inv1   g48(.a(x14), .O(new_n147_));
  nand2  g49(.a(new_n147_), .b(x09), .O(new_n148_));
  nand2  g50(.a(new_n148_), .b(x40), .O(new_n149_));
  nor2   g51(.a(x14), .b(new_n120_), .O(new_n150_));
  nand2  g52(.a(new_n150_), .b(x41), .O(new_n151_));
  aoi21  g53(.a(new_n151_), .b(new_n149_), .c(x10), .O(z29));
  nand2  g54(.a(new_n148_), .b(x41), .O(new_n153_));
  nand2  g55(.a(new_n150_), .b(x42), .O(new_n154_));
  aoi21  g56(.a(new_n154_), .b(new_n153_), .c(x10), .O(z30));
  nand2  g57(.a(new_n148_), .b(x42), .O(new_n156_));
  nand2  g58(.a(new_n150_), .b(x43), .O(new_n157_));
  aoi21  g59(.a(new_n157_), .b(new_n156_), .c(x10), .O(z31));
  nand2  g60(.a(new_n148_), .b(x43), .O(new_n159_));
  nand2  g61(.a(new_n150_), .b(x44), .O(new_n160_));
  aoi21  g62(.a(new_n160_), .b(new_n159_), .c(x10), .O(z32));
  nand2  g63(.a(new_n148_), .b(x44), .O(new_n162_));
  nand2  g64(.a(new_n150_), .b(x45), .O(new_n163_));
  aoi21  g65(.a(new_n163_), .b(new_n162_), .c(x10), .O(z33));
  nand2  g66(.a(new_n148_), .b(x45), .O(new_n165_));
  nand2  g67(.a(new_n150_), .b(x46), .O(new_n166_));
  aoi21  g68(.a(new_n166_), .b(new_n165_), .c(x10), .O(z34));
  nand2  g69(.a(new_n148_), .b(x46), .O(new_n168_));
  nand2  g70(.a(new_n150_), .b(x00), .O(new_n169_));
  aoi21  g71(.a(new_n169_), .b(new_n168_), .c(x10), .O(z35));
  zero   g72(.O(z01));
  zero   g73(.O(z02));
  zero   g74(.O(z03));
  zero   g75(.O(z04));
  zero   g76(.O(z07));
  zero   g77(.O(z08));
  zero   g78(.O(z09));
  zero   g79(.O(z11));
  zero   g80(.O(z12));
  zero   g81(.O(z14));
  zero   g82(.O(z16));
  zero   g83(.O(z17));
  zero   g84(.O(z18));
  zero   g85(.O(z23));
  zero   g86(.O(z28));
endmodule


