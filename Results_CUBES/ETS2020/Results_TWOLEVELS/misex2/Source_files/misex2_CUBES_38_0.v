// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n72_, new_n74_, new_n75_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n120_, new_n121_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(x10), .c(new_n47_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n46_), .O(z00));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n48_), .b(new_n52_), .c(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n46_), .O(z02));
  inv1   g10(.a(x19), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x18), .O(new_n56_));
  inv1   g12(.a(x17), .O(new_n57_));
  nand3  g13(.a(new_n45_), .b(new_n57_), .c(new_n44_), .O(new_n58_));
  inv1   g14(.a(x11), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x12), .O(new_n61_));
  inv1   g17(.a(x00), .O(new_n62_));
  inv1   g18(.a(x01), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g20(.a(new_n64_), .b(new_n47_), .c(x02), .O(new_n65_));
  oai22  g21(.a(new_n65_), .b(new_n61_), .c(new_n58_), .d(new_n56_), .O(z03));
  nand2  g22(.a(new_n64_), .b(x02), .O(new_n67_));
  inv1   g23(.a(x12), .O(new_n68_));
  nand4  g24(.a(new_n68_), .b(new_n59_), .c(x10), .d(new_n47_), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n67_), .O(z04));
  nor3   g26(.a(new_n67_), .b(new_n52_), .c(new_n47_), .O(z05));
  nand3  g27(.a(new_n60_), .b(new_n47_), .c(x02), .O(new_n72_));
  nor3   g28(.a(new_n72_), .b(new_n63_), .c(new_n62_), .O(z06));
  nand4  g29(.a(new_n60_), .b(new_n68_), .c(new_n47_), .d(x01), .O(new_n74_));
  nand2  g30(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g32(.a(x15), .O(new_n78_));
  inv1   g33(.a(x16), .O(new_n79_));
  nand3  g34(.a(x20), .b(new_n79_), .c(new_n78_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(new_n81_));
  nand2  g36(.a(x02), .b(new_n63_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  nor2   g38(.a(new_n68_), .b(x11), .O(new_n84_));
  nor2   g39(.a(x14), .b(x13), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n81_), .O(new_n86_));
  nor2   g41(.a(x20), .b(x19), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x18), .c(x01), .O(new_n88_));
  inv1   g43(.a(x21), .O(new_n89_));
  inv1   g44(.a(x22), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n62_), .O(new_n91_));
  aoi21  g46(.a(new_n88_), .b(new_n86_), .c(new_n91_), .O(z09));
  nor2   g47(.a(x20), .b(new_n63_), .O(new_n93_));
  nor2   g48(.a(new_n90_), .b(new_n89_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n93_), .c(new_n55_), .d(x18), .O(new_n95_));
  inv1   g50(.a(x13), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(x12), .c(new_n59_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n82_), .O(new_n98_));
  nor2   g53(.a(new_n78_), .b(x14), .O(new_n99_));
  nand4  g54(.a(new_n90_), .b(new_n89_), .c(x20), .d(x16), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n95_), .c(x00), .O(z10));
  nand4  g58(.a(new_n87_), .b(x21), .c(x18), .d(x01), .O(new_n104_));
  nand3  g59(.a(new_n89_), .b(x20), .c(new_n79_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n99_), .c(new_n98_), .O(new_n107_));
  nand2  g62(.a(new_n90_), .b(new_n62_), .O(new_n108_));
  aoi21  g63(.a(new_n107_), .b(new_n104_), .c(new_n108_), .O(z11));
  oai21  g64(.a(new_n52_), .b(new_n44_), .c(new_n64_), .O(new_n110_));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g66(.a(new_n55_), .b(x17), .c(new_n44_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n45_), .O(new_n114_));
  inv1   g69(.a(x24), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g71(.a(new_n114_), .b(new_n110_), .c(new_n116_), .O(z12));
  nor3   g72(.a(new_n46_), .b(x19), .c(new_n57_), .O(z13));
  aoi21  g73(.a(new_n52_), .b(x01), .c(new_n44_), .O(new_n120_));
  nand3  g74(.a(x19), .b(new_n44_), .c(new_n63_), .O(new_n121_));
  oai21  g75(.a(new_n120_), .b(new_n62_), .c(new_n121_), .O(z15));
  zero   g76(.O(z01));
  zero   g77(.O(z08));
  zero   g78(.O(z14));
  zero   g79(.O(z16));
  zero   g80(.O(z17));
endmodule


