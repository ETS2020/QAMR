// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n113_, new_n114_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n58_));
  inv1   g14(.a(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x02), .O(new_n61_));
  inv1   g17(.a(x11), .O(new_n62_));
  inv1   g18(.a(x12), .O(new_n63_));
  nand4  g19(.a(new_n63_), .b(new_n62_), .c(x10), .d(new_n44_), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n61_), .O(z04));
  nor3   g21(.a(new_n61_), .b(new_n54_), .c(new_n44_), .O(z05));
  inv1   g22(.a(x08), .O(new_n69_));
  nand4  g23(.a(x19), .b(new_n48_), .c(x17), .d(new_n69_), .O(new_n70_));
  inv1   g24(.a(x05), .O(new_n71_));
  nor2   g25(.a(x07), .b(x06), .O(new_n72_));
  nand3  g26(.a(new_n72_), .b(new_n71_), .c(x04), .O(new_n73_));
  inv1   g27(.a(x03), .O(new_n74_));
  nor2   g28(.a(x01), .b(x00), .O(new_n75_));
  nand3  g29(.a(new_n75_), .b(new_n74_), .c(x02), .O(new_n76_));
  nor3   g30(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z08));
  nor2   g31(.a(new_n48_), .b(new_n59_), .O(new_n79_));
  nor2   g32(.a(x20), .b(x19), .O(new_n80_));
  nand4  g33(.a(new_n80_), .b(new_n79_), .c(x22), .d(x21), .O(new_n81_));
  nand2  g34(.a(x02), .b(new_n59_), .O(new_n82_));
  inv1   g35(.a(x13), .O(new_n83_));
  nand3  g36(.a(new_n83_), .b(x12), .c(new_n62_), .O(new_n84_));
  nor2   g37(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  inv1   g38(.a(x15), .O(new_n86_));
  nor2   g39(.a(new_n86_), .b(x14), .O(new_n87_));
  inv1   g40(.a(x21), .O(new_n88_));
  inv1   g41(.a(x22), .O(new_n89_));
  nand4  g42(.a(new_n89_), .b(new_n88_), .c(x20), .d(x16), .O(new_n90_));
  inv1   g43(.a(new_n90_), .O(new_n91_));
  nand3  g44(.a(new_n91_), .b(new_n87_), .c(new_n85_), .O(new_n92_));
  aoi21  g45(.a(new_n92_), .b(new_n81_), .c(x00), .O(z10));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(x21), .O(new_n94_));
  inv1   g47(.a(x16), .O(new_n95_));
  nand3  g48(.a(new_n88_), .b(x20), .c(new_n95_), .O(new_n96_));
  inv1   g49(.a(new_n96_), .O(new_n97_));
  nand3  g50(.a(new_n97_), .b(new_n87_), .c(new_n85_), .O(new_n98_));
  nand2  g51(.a(new_n89_), .b(new_n58_), .O(new_n99_));
  aoi21  g52(.a(new_n98_), .b(new_n94_), .c(new_n99_), .O(z11));
  inv1   g53(.a(x02), .O(new_n101_));
  oai21  g54(.a(new_n54_), .b(new_n101_), .c(new_n60_), .O(new_n102_));
  oai21  g55(.a(x19), .b(x02), .c(x23), .O(new_n103_));
  nand3  g56(.a(new_n49_), .b(x17), .c(new_n101_), .O(new_n104_));
  nand2  g57(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g58(.a(new_n105_), .b(new_n75_), .O(new_n106_));
  inv1   g59(.a(x24), .O(new_n107_));
  nand2  g60(.a(new_n107_), .b(x09), .O(new_n108_));
  aoi21  g61(.a(new_n106_), .b(new_n102_), .c(new_n108_), .O(z12));
  nor2   g62(.a(x19), .b(new_n47_), .O(new_n110_));
  and2   g63(.a(new_n110_), .b(new_n45_), .O(z13));
  nor2   g64(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g65(.a(new_n54_), .b(x01), .c(new_n101_), .O(new_n113_));
  nand3  g66(.a(x19), .b(new_n101_), .c(new_n59_), .O(new_n114_));
  oai21  g67(.a(new_n113_), .b(new_n58_), .c(new_n114_), .O(z15));
  nor2   g68(.a(new_n59_), .b(x00), .O(z16));
  zero   g69(.O(z03));
  zero   g70(.O(z06));
  zero   g71(.O(z07));
  zero   g72(.O(z09));
  zero   g73(.O(z17));
endmodule


