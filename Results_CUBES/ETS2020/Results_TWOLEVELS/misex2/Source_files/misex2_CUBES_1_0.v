// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n68_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n123_, new_n124_;
  inv1   g00(.a(x19), .O(new_n47_));
  nand2  g01(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g02(.a(x02), .O(new_n49_));
  inv1   g03(.a(x17), .O(new_n50_));
  nor2   g04(.a(x01), .b(x00), .O(new_n51_));
  nand3  g05(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  inv1   g06(.a(x10), .O(new_n53_));
  inv1   g07(.a(x11), .O(new_n54_));
  nor2   g08(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g09(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g10(.a(x09), .O(new_n57_));
  inv1   g11(.a(x00), .O(new_n58_));
  inv1   g12(.a(x01), .O(new_n59_));
  nor2   g13(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g14(.a(new_n60_), .b(new_n57_), .c(x02), .O(new_n61_));
  oai22  g15(.a(new_n61_), .b(new_n56_), .c(new_n52_), .d(new_n48_), .O(z03));
  nand2  g16(.a(new_n60_), .b(x02), .O(new_n63_));
  inv1   g17(.a(x12), .O(new_n64_));
  nand4  g18(.a(new_n64_), .b(new_n54_), .c(x10), .d(new_n57_), .O(new_n65_));
  nor2   g19(.a(new_n65_), .b(new_n63_), .O(z04));
  nor3   g20(.a(new_n63_), .b(new_n53_), .c(new_n57_), .O(z05));
  nand3  g21(.a(new_n55_), .b(new_n57_), .c(x02), .O(new_n68_));
  nor3   g22(.a(new_n68_), .b(new_n59_), .c(new_n58_), .O(z06));
  nand4  g23(.a(new_n55_), .b(new_n64_), .c(new_n57_), .d(x01), .O(new_n70_));
  nand2  g24(.a(x02), .b(x00), .O(new_n71_));
  aoi21  g25(.a(new_n70_), .b(x01), .c(new_n71_), .O(z07));
  inv1   g26(.a(x03), .O(new_n73_));
  nor2   g27(.a(x06), .b(x05), .O(new_n74_));
  nor2   g28(.a(x08), .b(x07), .O(new_n75_));
  nand4  g29(.a(new_n75_), .b(new_n74_), .c(x04), .d(new_n73_), .O(new_n76_));
  nand2  g30(.a(new_n51_), .b(x02), .O(new_n77_));
  inv1   g31(.a(x18), .O(new_n78_));
  nand3  g32(.a(x19), .b(new_n78_), .c(x17), .O(new_n79_));
  nor3   g33(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z08));
  inv1   g34(.a(x15), .O(new_n81_));
  inv1   g35(.a(x16), .O(new_n82_));
  nand3  g36(.a(x20), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  nand2  g38(.a(x02), .b(new_n59_), .O(new_n85_));
  inv1   g39(.a(new_n85_), .O(new_n86_));
  nor2   g40(.a(new_n64_), .b(x11), .O(new_n87_));
  nor2   g41(.a(x14), .b(x13), .O(new_n88_));
  nand4  g42(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n84_), .O(new_n89_));
  nor2   g43(.a(x20), .b(x19), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(x18), .c(x01), .O(new_n91_));
  inv1   g45(.a(x21), .O(new_n92_));
  inv1   g46(.a(x22), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n92_), .c(new_n58_), .O(new_n94_));
  aoi21  g48(.a(new_n91_), .b(new_n89_), .c(new_n94_), .O(z09));
  nor2   g49(.a(x20), .b(new_n59_), .O(new_n96_));
  nor2   g50(.a(new_n93_), .b(new_n92_), .O(new_n97_));
  nand4  g51(.a(new_n97_), .b(new_n96_), .c(new_n47_), .d(x18), .O(new_n98_));
  inv1   g52(.a(x13), .O(new_n99_));
  nand3  g53(.a(new_n99_), .b(x12), .c(new_n54_), .O(new_n100_));
  nor2   g54(.a(new_n100_), .b(new_n85_), .O(new_n101_));
  nor2   g55(.a(new_n81_), .b(x14), .O(new_n102_));
  nand4  g56(.a(new_n93_), .b(new_n92_), .c(x20), .d(x16), .O(new_n103_));
  inv1   g57(.a(new_n103_), .O(new_n104_));
  nand3  g58(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  aoi21  g59(.a(new_n105_), .b(new_n98_), .c(x00), .O(z10));
  nand4  g60(.a(new_n90_), .b(x21), .c(x18), .d(x01), .O(new_n107_));
  nand3  g61(.a(new_n92_), .b(x20), .c(new_n82_), .O(new_n108_));
  inv1   g62(.a(new_n108_), .O(new_n109_));
  nand3  g63(.a(new_n109_), .b(new_n102_), .c(new_n101_), .O(new_n110_));
  nand2  g64(.a(new_n93_), .b(new_n58_), .O(new_n111_));
  aoi21  g65(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(z11));
  oai21  g66(.a(new_n53_), .b(new_n49_), .c(new_n60_), .O(new_n113_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g68(.a(new_n47_), .b(x17), .c(new_n49_), .O(new_n115_));
  nand2  g69(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n51_), .O(new_n117_));
  inv1   g71(.a(x24), .O(new_n118_));
  nand2  g72(.a(new_n118_), .b(x09), .O(new_n119_));
  aoi21  g73(.a(new_n117_), .b(new_n113_), .c(new_n119_), .O(z12));
  nor3   g74(.a(new_n115_), .b(x01), .c(x00), .O(z13));
  aoi21  g75(.a(new_n53_), .b(x01), .c(new_n49_), .O(new_n123_));
  nand3  g76(.a(x19), .b(new_n49_), .c(new_n59_), .O(new_n124_));
  oai21  g77(.a(new_n123_), .b(new_n58_), .c(new_n124_), .O(z15));
  nor2   g78(.a(new_n59_), .b(x00), .O(z16));
  inv1   g79(.a(new_n77_), .O(z17));
  zero   g80(.O(z00));
  zero   g81(.O(z01));
  zero   g82(.O(z02));
  zero   g83(.O(z14));
endmodule


