// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n58_, new_n59_, new_n60_, new_n63_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n121_, new_n122_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(new_n59_));
  nand3  g15(.a(new_n59_), .b(x10), .c(new_n44_), .O(new_n60_));
  nor3   g16(.a(new_n60_), .b(x12), .c(x11), .O(z04));
  nor3   g17(.a(new_n58_), .b(new_n54_), .c(new_n44_), .O(z05));
  inv1   g18(.a(x11), .O(new_n63_));
  nor2   g19(.a(new_n60_), .b(new_n63_), .O(z06));
  inv1   g20(.a(x03), .O(new_n66_));
  nor2   g21(.a(x06), .b(x05), .O(new_n67_));
  nor2   g22(.a(x08), .b(x07), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(x04), .d(new_n66_), .O(new_n69_));
  nor2   g24(.a(x01), .b(x00), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x02), .O(new_n71_));
  nand3  g26(.a(x19), .b(new_n48_), .c(x17), .O(new_n72_));
  nor3   g27(.a(new_n72_), .b(new_n71_), .c(new_n69_), .O(z08));
  inv1   g28(.a(x15), .O(new_n74_));
  inv1   g29(.a(x16), .O(new_n75_));
  nand3  g30(.a(x20), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(new_n77_));
  inv1   g32(.a(x01), .O(new_n78_));
  nand2  g33(.a(x02), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(new_n80_));
  inv1   g35(.a(x13), .O(new_n81_));
  inv1   g36(.a(x14), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(new_n81_), .c(x12), .d(new_n63_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n80_), .c(new_n77_), .O(new_n85_));
  nor2   g40(.a(new_n48_), .b(new_n78_), .O(new_n86_));
  nor2   g41(.a(x20), .b(x19), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g43(.a(x00), .O(new_n89_));
  inv1   g44(.a(x21), .O(new_n90_));
  inv1   g45(.a(x22), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  aoi21  g47(.a(new_n88_), .b(new_n85_), .c(new_n92_), .O(z09));
  nand4  g48(.a(new_n87_), .b(new_n86_), .c(x22), .d(x21), .O(new_n94_));
  nand3  g49(.a(new_n81_), .b(x12), .c(new_n63_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nor2   g51(.a(new_n74_), .b(x14), .O(new_n97_));
  nand4  g52(.a(new_n91_), .b(new_n90_), .c(x20), .d(x16), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n96_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n94_), .c(x00), .O(z10));
  nand3  g56(.a(new_n87_), .b(new_n86_), .c(x21), .O(new_n102_));
  nand3  g57(.a(new_n90_), .b(x20), .c(new_n75_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n97_), .c(new_n96_), .O(new_n105_));
  nand2  g60(.a(new_n91_), .b(new_n89_), .O(new_n106_));
  aoi21  g61(.a(new_n105_), .b(new_n102_), .c(new_n106_), .O(z11));
  nand2  g62(.a(x10), .b(x02), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(x01), .c(x00), .O(new_n109_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  inv1   g65(.a(x02), .O(new_n111_));
  nand3  g66(.a(new_n49_), .b(x17), .c(new_n111_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n70_), .O(new_n114_));
  inv1   g69(.a(x24), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g71(.a(new_n114_), .b(new_n109_), .c(new_n116_), .O(z12));
  nor2   g72(.a(x19), .b(new_n47_), .O(new_n118_));
  and2   g73(.a(new_n118_), .b(new_n45_), .O(z13));
  nor2   g74(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g75(.a(new_n54_), .b(x01), .c(new_n111_), .O(new_n121_));
  nand3  g76(.a(x19), .b(new_n111_), .c(new_n78_), .O(new_n122_));
  oai21  g77(.a(new_n121_), .b(new_n89_), .c(new_n122_), .O(z15));
  nor2   g78(.a(new_n78_), .b(x00), .O(z16));
  inv1   g79(.a(new_n71_), .O(z17));
  zero   g80(.O(z03));
  zero   g81(.O(z07));
endmodule


