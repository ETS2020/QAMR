// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n88_, new_n89_, new_n92_;
  inv1   g00(.a(x09), .O(new_n45_));
  inv1   g01(.a(x00), .O(new_n46_));
  inv1   g02(.a(x01), .O(new_n47_));
  inv1   g03(.a(x02), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  or2    g05(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g06(.a(x17), .O(new_n51_));
  inv1   g07(.a(x18), .O(new_n52_));
  inv1   g08(.a(x19), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n50_), .O(z01));
  inv1   g11(.a(x10), .O(new_n56_));
  nand4  g12(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n56_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n50_), .O(z02));
  inv1   g14(.a(x11), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x10), .O(new_n61_));
  nand4  g16(.a(new_n45_), .b(x02), .c(x01), .d(x00), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n61_), .c(x12), .O(z04));
  nor2   g18(.a(x20), .b(x19), .O(new_n70_));
  nand4  g19(.a(new_n70_), .b(x21), .c(x18), .d(x01), .O(new_n71_));
  inv1   g20(.a(x16), .O(new_n72_));
  inv1   g21(.a(x21), .O(new_n73_));
  nand4  g22(.a(new_n73_), .b(x20), .c(new_n72_), .d(x15), .O(new_n74_));
  inv1   g23(.a(new_n74_), .O(new_n75_));
  nor2   g24(.a(new_n48_), .b(x01), .O(new_n76_));
  inv1   g25(.a(x13), .O(new_n77_));
  inv1   g26(.a(x14), .O(new_n78_));
  nand4  g27(.a(new_n78_), .b(new_n77_), .c(x12), .d(new_n60_), .O(new_n79_));
  inv1   g28(.a(new_n79_), .O(new_n80_));
  nand3  g29(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n81_));
  inv1   g30(.a(x22), .O(new_n82_));
  nand2  g31(.a(new_n82_), .b(new_n46_), .O(new_n83_));
  aoi21  g32(.a(new_n81_), .b(new_n71_), .c(new_n83_), .O(z11));
  nor3   g33(.a(new_n49_), .b(x19), .c(new_n51_), .O(z13));
  aoi21  g34(.a(new_n56_), .b(x01), .c(new_n48_), .O(new_n88_));
  nand3  g35(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n89_));
  oai21  g36(.a(new_n88_), .b(new_n46_), .c(new_n89_), .O(z15));
  nor2   g37(.a(new_n47_), .b(x00), .O(z16));
  nand3  g38(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n92_));
  inv1   g39(.a(new_n92_), .O(z17));
  zero   g40(.O(z00));
  zero   g41(.O(z03));
  zero   g42(.O(z05));
  zero   g43(.O(z06));
  zero   g44(.O(z07));
  zero   g45(.O(z08));
  zero   g46(.O(z09));
  zero   g47(.O(z10));
  zero   g48(.O(z12));
  zero   g49(.O(z14));
endmodule


