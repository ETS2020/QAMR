// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n53_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n106_,
    new_n107_, new_n108_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x09), .O(new_n48_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand3  g08(.a(new_n49_), .b(new_n47_), .c(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n46_), .O(z02));
  inv1   g10(.a(x03), .O(new_n60_));
  nor2   g11(.a(x06), .b(x05), .O(new_n61_));
  nor2   g12(.a(x08), .b(x07), .O(new_n62_));
  nand4  g13(.a(new_n62_), .b(new_n61_), .c(x04), .d(new_n60_), .O(new_n63_));
  nand2  g14(.a(new_n45_), .b(x02), .O(new_n64_));
  inv1   g15(.a(x18), .O(new_n65_));
  nand3  g16(.a(x19), .b(new_n65_), .c(x17), .O(new_n66_));
  nor3   g17(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(z08));
  inv1   g18(.a(x15), .O(new_n68_));
  inv1   g19(.a(x16), .O(new_n69_));
  nand3  g20(.a(x20), .b(new_n69_), .c(new_n68_), .O(new_n70_));
  inv1   g21(.a(new_n70_), .O(new_n71_));
  nor2   g22(.a(new_n44_), .b(x01), .O(new_n72_));
  inv1   g23(.a(x11), .O(new_n73_));
  inv1   g24(.a(x13), .O(new_n74_));
  inv1   g25(.a(x14), .O(new_n75_));
  nand4  g26(.a(new_n75_), .b(new_n74_), .c(x12), .d(new_n73_), .O(new_n76_));
  inv1   g27(.a(new_n76_), .O(new_n77_));
  nand3  g28(.a(new_n77_), .b(new_n72_), .c(new_n71_), .O(new_n78_));
  nor2   g29(.a(x20), .b(x19), .O(new_n79_));
  nand3  g30(.a(new_n79_), .b(x18), .c(x01), .O(new_n80_));
  inv1   g31(.a(x00), .O(new_n81_));
  inv1   g32(.a(x21), .O(new_n82_));
  inv1   g33(.a(x22), .O(new_n83_));
  nand3  g34(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  aoi21  g35(.a(new_n80_), .b(new_n78_), .c(new_n84_), .O(z09));
  nand4  g36(.a(new_n79_), .b(x21), .c(x18), .d(x01), .O(new_n87_));
  nand4  g37(.a(new_n82_), .b(x20), .c(new_n69_), .d(x15), .O(new_n88_));
  inv1   g38(.a(new_n88_), .O(new_n89_));
  nand3  g39(.a(new_n89_), .b(new_n77_), .c(new_n72_), .O(new_n90_));
  nand2  g40(.a(new_n83_), .b(new_n81_), .O(new_n91_));
  aoi21  g41(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(z11));
  nand2  g42(.a(x10), .b(x02), .O(new_n93_));
  nand3  g43(.a(new_n93_), .b(x01), .c(x00), .O(new_n94_));
  oai21  g44(.a(x19), .b(x02), .c(x23), .O(new_n95_));
  inv1   g45(.a(x19), .O(new_n96_));
  nand3  g46(.a(new_n96_), .b(x17), .c(new_n44_), .O(new_n97_));
  nand2  g47(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g48(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  inv1   g49(.a(x24), .O(new_n100_));
  nand2  g50(.a(new_n100_), .b(x09), .O(new_n101_));
  aoi21  g51(.a(new_n99_), .b(new_n94_), .c(new_n101_), .O(z12));
  nand2  g52(.a(new_n96_), .b(x17), .O(new_n103_));
  nor2   g53(.a(new_n103_), .b(new_n46_), .O(z13));
  aoi21  g54(.a(new_n47_), .b(x01), .c(new_n44_), .O(new_n106_));
  inv1   g55(.a(x01), .O(new_n107_));
  nand3  g56(.a(x19), .b(new_n44_), .c(new_n107_), .O(new_n108_));
  oai21  g57(.a(new_n106_), .b(new_n81_), .c(new_n108_), .O(z15));
  nor2   g58(.a(new_n107_), .b(x00), .O(z16));
  inv1   g59(.a(new_n64_), .O(z17));
  zero   g60(.O(z01));
  zero   g61(.O(z03));
  zero   g62(.O(z04));
  zero   g63(.O(z05));
  zero   g64(.O(z06));
  zero   g65(.O(z07));
  zero   g66(.O(z10));
  zero   g67(.O(z14));
endmodule


