// Benchmark "FAU" written by ABC on Tue Jul  7 19:18:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x20), .O(new_n53_));
  nor2   g001(.a(x19), .b(x17), .O(new_n54_));
  nor2   g002(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g003(.a(x20), .b(x19), .c(x17), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g005(.a(x14), .O(new_n58_));
  inv1   g006(.a(x16), .O(new_n59_));
  aoi21  g007(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g009(.a(new_n56_), .O(new_n62_));
  inv1   g010(.a(x17), .O(new_n63_));
  inv1   g011(.a(x19), .O(new_n64_));
  inv1   g012(.a(x21), .O(new_n65_));
  nand4  g013(.a(new_n65_), .b(new_n53_), .c(new_n64_), .d(new_n63_), .O(new_n66_));
  inv1   g014(.a(new_n66_), .O(new_n67_));
  aoi21  g015(.a(new_n62_), .b(x21), .c(new_n67_), .O(new_n68_));
  inv1   g016(.a(x13), .O(new_n69_));
  aoi21  g017(.a(new_n59_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g018(.a(new_n68_), .b(new_n59_), .c(new_n70_), .O(z02));
  nor2   g019(.a(x22), .b(x21), .O(new_n73_));
  nand3  g020(.a(new_n73_), .b(new_n54_), .c(new_n53_), .O(new_n74_));
  nor3   g021(.a(x23), .b(x22), .c(x21), .O(new_n75_));
  aoi22  g022(.a(new_n75_), .b(new_n56_), .c(new_n74_), .d(x23), .O(new_n76_));
  inv1   g023(.a(x11), .O(new_n77_));
  aoi21  g024(.a(new_n59_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g025(.a(new_n76_), .b(new_n59_), .c(new_n78_), .O(z04));
  nor2   g026(.a(x24), .b(x23), .O(new_n81_));
  nand4  g027(.a(new_n81_), .b(new_n73_), .c(new_n54_), .d(new_n53_), .O(new_n82_));
  inv1   g028(.a(x22), .O(new_n83_));
  inv1   g029(.a(x23), .O(new_n84_));
  inv1   g030(.a(x24), .O(new_n85_));
  inv1   g031(.a(x25), .O(new_n86_));
  nand4  g032(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  nor2   g033(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  aoi21  g034(.a(new_n82_), .b(x25), .c(new_n88_), .O(new_n89_));
  inv1   g035(.a(x09), .O(new_n90_));
  aoi21  g036(.a(new_n59_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g037(.a(new_n89_), .b(new_n59_), .c(new_n91_), .O(z06));
  nor2   g038(.a(x21), .b(x20), .O(new_n93_));
  nor2   g039(.a(x23), .b(x22), .O(new_n94_));
  nor2   g040(.a(x25), .b(x24), .O(new_n95_));
  nand4  g041(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n54_), .O(new_n96_));
  nand2  g042(.a(new_n96_), .b(x26), .O(new_n97_));
  nor3   g043(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  nand3  g044(.a(new_n98_), .b(new_n94_), .c(new_n67_), .O(new_n99_));
  nand2  g045(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g046(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g047(.a(x08), .O(new_n102_));
  aoi21  g048(.a(new_n59_), .b(new_n102_), .c(x18), .O(new_n103_));
  nand2  g049(.a(new_n103_), .b(new_n101_), .O(z07));
  nor3   g050(.a(x27), .b(x26), .c(x25), .O(new_n106_));
  nand4  g051(.a(new_n106_), .b(new_n81_), .c(new_n73_), .d(new_n56_), .O(new_n107_));
  nor2   g052(.a(x28), .b(x27), .O(new_n108_));
  nor2   g053(.a(x26), .b(x23), .O(new_n109_));
  nand3  g054(.a(new_n109_), .b(new_n108_), .c(new_n95_), .O(new_n110_));
  nor2   g055(.a(new_n110_), .b(new_n74_), .O(new_n111_));
  aoi21  g056(.a(new_n107_), .b(x28), .c(new_n111_), .O(new_n112_));
  inv1   g057(.a(x06), .O(new_n113_));
  aoi21  g058(.a(new_n59_), .b(new_n113_), .c(x18), .O(new_n114_));
  oai21  g059(.a(new_n112_), .b(new_n59_), .c(new_n114_), .O(z09));
  inv1   g060(.a(x29), .O(new_n116_));
  nor3   g061(.a(x29), .b(x28), .c(x27), .O(new_n117_));
  nand4  g062(.a(new_n117_), .b(new_n98_), .c(new_n75_), .d(new_n56_), .O(new_n118_));
  oai21  g063(.a(new_n111_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g064(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g065(.a(x05), .O(new_n121_));
  aoi21  g066(.a(new_n59_), .b(new_n121_), .c(x18), .O(new_n122_));
  nand2  g067(.a(new_n122_), .b(new_n120_), .O(z10));
  nand4  g068(.a(new_n94_), .b(new_n54_), .c(new_n65_), .d(new_n53_), .O(new_n125_));
  nor2   g069(.a(x26), .b(x25), .O(new_n126_));
  nor2   g070(.a(x30), .b(x29), .O(new_n127_));
  nand4  g071(.a(new_n127_), .b(new_n108_), .c(new_n126_), .d(new_n85_), .O(new_n128_));
  oai21  g072(.a(new_n128_), .b(new_n125_), .c(x31), .O(new_n129_));
  nand2  g073(.a(new_n81_), .b(new_n73_), .O(new_n130_));
  inv1   g074(.a(new_n130_), .O(new_n131_));
  nor2   g075(.a(x29), .b(x28), .O(new_n132_));
  nor2   g076(.a(x31), .b(x30), .O(new_n133_));
  nand2  g077(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g078(.a(new_n134_), .O(new_n135_));
  nand4  g079(.a(new_n135_), .b(new_n106_), .c(new_n131_), .d(new_n56_), .O(new_n136_));
  nand2  g080(.a(new_n136_), .b(new_n129_), .O(new_n137_));
  nand2  g081(.a(new_n137_), .b(x16), .O(new_n138_));
  inv1   g082(.a(x03), .O(new_n139_));
  aoi21  g083(.a(new_n59_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g084(.a(new_n140_), .b(new_n138_), .O(z12));
  nor2   g085(.a(x27), .b(x26), .O(new_n142_));
  nand4  g086(.a(new_n133_), .b(new_n132_), .c(new_n142_), .d(new_n86_), .O(new_n143_));
  oai21  g087(.a(new_n143_), .b(new_n82_), .c(x32), .O(new_n144_));
  nand2  g088(.a(new_n127_), .b(new_n108_), .O(new_n145_));
  inv1   g089(.a(new_n145_), .O(new_n146_));
  nor2   g090(.a(x32), .b(x31), .O(new_n147_));
  nand2  g091(.a(new_n147_), .b(new_n126_), .O(new_n148_));
  inv1   g092(.a(new_n148_), .O(new_n149_));
  nand4  g093(.a(new_n149_), .b(new_n146_), .c(new_n131_), .d(new_n56_), .O(new_n150_));
  nand2  g094(.a(new_n150_), .b(new_n144_), .O(new_n151_));
  nand2  g095(.a(new_n151_), .b(x16), .O(new_n152_));
  inv1   g096(.a(x02), .O(new_n153_));
  aoi21  g097(.a(new_n59_), .b(new_n153_), .c(x18), .O(new_n154_));
  nand2  g098(.a(new_n154_), .b(new_n152_), .O(z13));
  nand4  g099(.a(new_n147_), .b(new_n127_), .c(new_n108_), .d(new_n126_), .O(new_n156_));
  oai21  g100(.a(new_n156_), .b(new_n82_), .c(x33), .O(new_n157_));
  nor2   g101(.a(x33), .b(x32), .O(new_n158_));
  nand4  g102(.a(new_n158_), .b(new_n133_), .c(new_n132_), .d(new_n142_), .O(new_n159_));
  inv1   g103(.a(new_n159_), .O(new_n160_));
  nand2  g104(.a(new_n160_), .b(new_n88_), .O(new_n161_));
  nand2  g105(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g106(.a(new_n162_), .b(x16), .O(new_n163_));
  inv1   g107(.a(x01), .O(new_n164_));
  aoi21  g108(.a(new_n59_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g109(.a(new_n165_), .b(new_n163_), .O(z14));
  oai21  g110(.a(new_n159_), .b(new_n96_), .c(x34), .O(new_n167_));
  nand2  g111(.a(new_n132_), .b(new_n142_), .O(new_n168_));
  inv1   g112(.a(new_n168_), .O(new_n169_));
  inv1   g113(.a(x30), .O(new_n170_));
  inv1   g114(.a(x31), .O(new_n171_));
  nand2  g115(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  inv1   g116(.a(x32), .O(new_n173_));
  inv1   g117(.a(x33), .O(new_n174_));
  inv1   g118(.a(x34), .O(new_n175_));
  nand3  g119(.a(new_n175_), .b(new_n174_), .c(new_n173_), .O(new_n176_));
  nor2   g120(.a(new_n176_), .b(new_n172_), .O(new_n177_));
  nand3  g121(.a(new_n177_), .b(new_n169_), .c(new_n88_), .O(new_n178_));
  nand2  g122(.a(new_n178_), .b(new_n167_), .O(new_n179_));
  nand2  g123(.a(new_n179_), .b(x16), .O(new_n180_));
  inv1   g124(.a(x00), .O(new_n181_));
  aoi21  g125(.a(new_n59_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g126(.a(new_n182_), .b(new_n180_), .O(z15));
  zero   g127(.O(z00));
  zero   g128(.O(z03));
  zero   g129(.O(z05));
  zero   g130(.O(z08));
  zero   g131(.O(z11));
endmodule


