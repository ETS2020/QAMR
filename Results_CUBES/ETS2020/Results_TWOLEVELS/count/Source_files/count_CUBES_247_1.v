// Benchmark "FAU" written by ABC on Tue Jul  7 19:21:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  nand2  g016(.a(new_n55_), .b(new_n61_), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nand4  g018(.a(new_n69_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n68_), .b(x21), .c(new_n71_), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n58_), .c(new_n74_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n68_), .O(new_n78_));
  aoi21  g027(.a(new_n70_), .b(x22), .c(new_n78_), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n58_), .b(new_n80_), .c(x18), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n58_), .c(new_n81_), .O(z03));
  nand2  g031(.a(new_n76_), .b(new_n63_), .O(new_n83_));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n63_), .c(new_n83_), .d(x23), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n58_), .c(new_n87_), .O(z04));
  inv1   g037(.a(x22), .O(new_n91_));
  inv1   g038(.a(x23), .O(new_n92_));
  inv1   g039(.a(x24), .O(new_n93_));
  inv1   g040(.a(x25), .O(new_n94_));
  nand4  g041(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n95_));
  oai21  g042(.a(new_n95_), .b(new_n70_), .c(x26), .O(new_n96_));
  nor2   g043(.a(x23), .b(x22), .O(new_n97_));
  nor3   g044(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  nand3  g045(.a(new_n98_), .b(new_n97_), .c(new_n71_), .O(new_n99_));
  nand2  g046(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g047(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g048(.a(x08), .O(new_n102_));
  aoi21  g049(.a(new_n58_), .b(new_n102_), .c(x18), .O(new_n103_));
  nand2  g050(.a(new_n103_), .b(new_n101_), .O(z07));
  nor2   g051(.a(x24), .b(x23), .O(new_n106_));
  nor3   g052(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  nand4  g053(.a(new_n107_), .b(new_n106_), .c(new_n76_), .d(new_n63_), .O(new_n108_));
  inv1   g054(.a(x26), .O(new_n109_));
  nor2   g055(.a(x25), .b(x24), .O(new_n110_));
  nor2   g056(.a(x28), .b(x27), .O(new_n111_));
  nand4  g057(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n92_), .O(new_n112_));
  inv1   g058(.a(new_n112_), .O(new_n113_));
  aoi22  g059(.a(new_n113_), .b(new_n78_), .c(new_n108_), .d(x28), .O(new_n114_));
  inv1   g060(.a(x06), .O(new_n115_));
  aoi21  g061(.a(new_n58_), .b(new_n115_), .c(x18), .O(new_n116_));
  oai21  g062(.a(new_n114_), .b(new_n58_), .c(new_n116_), .O(z09));
  nor3   g063(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand4  g064(.a(new_n119_), .b(new_n98_), .c(new_n84_), .d(new_n63_), .O(new_n120_));
  nand4  g065(.a(new_n97_), .b(new_n55_), .c(new_n69_), .d(new_n61_), .O(new_n121_));
  nor2   g066(.a(x26), .b(x25), .O(new_n122_));
  nor2   g067(.a(x30), .b(x29), .O(new_n123_));
  nand4  g068(.a(new_n123_), .b(new_n111_), .c(new_n122_), .d(new_n93_), .O(new_n124_));
  nor2   g069(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  aoi21  g070(.a(new_n120_), .b(x30), .c(new_n125_), .O(new_n126_));
  inv1   g071(.a(x04), .O(new_n127_));
  aoi21  g072(.a(new_n58_), .b(new_n127_), .c(x18), .O(new_n128_));
  oai21  g073(.a(new_n126_), .b(new_n58_), .c(new_n128_), .O(z11));
  oai21  g074(.a(new_n124_), .b(new_n121_), .c(x31), .O(new_n130_));
  nand4  g075(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n69_), .O(new_n131_));
  inv1   g076(.a(new_n131_), .O(new_n132_));
  inv1   g077(.a(x28), .O(new_n133_));
  inv1   g078(.a(x29), .O(new_n134_));
  inv1   g079(.a(x30), .O(new_n135_));
  inv1   g080(.a(x31), .O(new_n136_));
  nand4  g081(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n133_), .O(new_n137_));
  inv1   g082(.a(new_n137_), .O(new_n138_));
  nand4  g083(.a(new_n138_), .b(new_n132_), .c(new_n107_), .d(new_n63_), .O(new_n139_));
  nand2  g084(.a(new_n139_), .b(new_n130_), .O(new_n140_));
  nand2  g085(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g086(.a(x03), .O(new_n142_));
  aoi21  g087(.a(new_n58_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g088(.a(new_n143_), .b(new_n141_), .O(z12));
  nand4  g089(.a(new_n106_), .b(new_n76_), .c(new_n55_), .d(new_n61_), .O(new_n146_));
  nor2   g090(.a(x32), .b(x31), .O(new_n147_));
  nand4  g091(.a(new_n147_), .b(new_n123_), .c(new_n111_), .d(new_n122_), .O(new_n148_));
  oai21  g092(.a(new_n148_), .b(new_n146_), .c(x33), .O(new_n149_));
  inv1   g093(.a(new_n95_), .O(new_n150_));
  inv1   g094(.a(x27), .O(new_n151_));
  inv1   g095(.a(x32), .O(new_n152_));
  inv1   g096(.a(x33), .O(new_n153_));
  nand4  g097(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n109_), .O(new_n154_));
  inv1   g098(.a(new_n154_), .O(new_n155_));
  nand4  g099(.a(new_n155_), .b(new_n138_), .c(new_n150_), .d(new_n71_), .O(new_n156_));
  nand2  g100(.a(new_n156_), .b(new_n149_), .O(new_n157_));
  nand2  g101(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g102(.a(x01), .O(new_n159_));
  aoi21  g103(.a(new_n58_), .b(new_n159_), .c(x18), .O(new_n160_));
  nand2  g104(.a(new_n160_), .b(new_n158_), .O(z14));
  zero   g105(.O(z05));
  zero   g106(.O(z06));
  zero   g107(.O(z08));
  zero   g108(.O(z10));
  zero   g109(.O(z13));
  zero   g110(.O(z15));
endmodule


