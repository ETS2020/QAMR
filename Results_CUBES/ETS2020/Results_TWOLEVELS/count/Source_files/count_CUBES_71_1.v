// Benchmark "FAU" written by ABC on Tue Jul  7 19:18:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  inv1   g000(.a(x16), .O(new_n54_));
  inv1   g001(.a(x20), .O(new_n55_));
  nor2   g002(.a(x19), .b(x17), .O(new_n56_));
  nand2  g003(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g004(.a(x17), .O(new_n58_));
  inv1   g005(.a(x19), .O(new_n59_));
  inv1   g006(.a(x21), .O(new_n60_));
  nand4  g007(.a(new_n60_), .b(new_n55_), .c(new_n59_), .d(new_n58_), .O(new_n61_));
  inv1   g008(.a(new_n61_), .O(new_n62_));
  aoi21  g009(.a(new_n57_), .b(x21), .c(new_n62_), .O(new_n63_));
  inv1   g010(.a(x13), .O(new_n64_));
  aoi21  g011(.a(new_n54_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g012(.a(new_n63_), .b(new_n54_), .c(new_n65_), .O(z02));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  nor2   g014(.a(x22), .b(x21), .O(new_n68_));
  aoi22  g015(.a(new_n68_), .b(new_n67_), .c(new_n61_), .d(x22), .O(new_n69_));
  inv1   g016(.a(x12), .O(new_n70_));
  aoi21  g017(.a(new_n54_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g018(.a(new_n69_), .b(new_n54_), .c(new_n71_), .O(z03));
  nand3  g019(.a(new_n68_), .b(new_n56_), .c(new_n55_), .O(new_n73_));
  nand2  g020(.a(new_n73_), .b(x23), .O(new_n74_));
  nor2   g021(.a(x23), .b(x22), .O(new_n75_));
  nand4  g022(.a(new_n75_), .b(new_n56_), .c(new_n60_), .d(new_n55_), .O(new_n76_));
  and2   g023(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  inv1   g024(.a(x11), .O(new_n78_));
  aoi21  g025(.a(new_n54_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g026(.a(new_n77_), .b(new_n54_), .c(new_n79_), .O(z04));
  nor2   g027(.a(x24), .b(x23), .O(new_n82_));
  nand4  g028(.a(new_n82_), .b(new_n68_), .c(new_n56_), .d(new_n55_), .O(new_n83_));
  inv1   g029(.a(x22), .O(new_n84_));
  inv1   g030(.a(x23), .O(new_n85_));
  inv1   g031(.a(x24), .O(new_n86_));
  inv1   g032(.a(x25), .O(new_n87_));
  nand4  g033(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n88_));
  nor2   g034(.a(new_n88_), .b(new_n61_), .O(new_n89_));
  aoi21  g035(.a(new_n83_), .b(x25), .c(new_n89_), .O(new_n90_));
  inv1   g036(.a(x09), .O(new_n91_));
  aoi21  g037(.a(new_n54_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g038(.a(new_n90_), .b(new_n54_), .c(new_n92_), .O(z06));
  nor2   g039(.a(x21), .b(x20), .O(new_n94_));
  nor2   g040(.a(x25), .b(x24), .O(new_n95_));
  nand4  g041(.a(new_n95_), .b(new_n75_), .c(new_n94_), .d(new_n56_), .O(new_n96_));
  nand2  g042(.a(new_n96_), .b(x26), .O(new_n97_));
  nor2   g043(.a(x26), .b(x25), .O(new_n98_));
  nand4  g044(.a(new_n98_), .b(new_n75_), .c(new_n62_), .d(new_n86_), .O(new_n99_));
  nand2  g045(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g046(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g047(.a(x08), .O(new_n102_));
  aoi21  g048(.a(new_n54_), .b(new_n102_), .c(x18), .O(new_n103_));
  nand2  g049(.a(new_n103_), .b(new_n101_), .O(z07));
  nor2   g050(.a(x27), .b(x26), .O(new_n105_));
  nand3  g051(.a(new_n105_), .b(new_n82_), .c(new_n87_), .O(new_n106_));
  nor2   g052(.a(new_n106_), .b(new_n73_), .O(new_n107_));
  aoi21  g053(.a(new_n99_), .b(x27), .c(new_n107_), .O(new_n108_));
  inv1   g054(.a(x07), .O(new_n109_));
  aoi21  g055(.a(new_n54_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g056(.a(new_n108_), .b(new_n54_), .c(new_n110_), .O(z08));
  nor3   g057(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  nand4  g058(.a(new_n112_), .b(new_n82_), .c(new_n68_), .d(new_n67_), .O(new_n113_));
  nor2   g059(.a(x28), .b(x27), .O(new_n114_));
  nor2   g060(.a(x26), .b(x23), .O(new_n115_));
  nand3  g061(.a(new_n115_), .b(new_n114_), .c(new_n95_), .O(new_n116_));
  nor2   g062(.a(new_n116_), .b(new_n73_), .O(new_n117_));
  aoi21  g063(.a(new_n113_), .b(x28), .c(new_n117_), .O(new_n118_));
  inv1   g064(.a(x06), .O(new_n119_));
  aoi21  g065(.a(new_n54_), .b(new_n119_), .c(x18), .O(new_n120_));
  oai21  g066(.a(new_n118_), .b(new_n54_), .c(new_n120_), .O(z09));
  nor2   g067(.a(x30), .b(x29), .O(new_n124_));
  nand4  g068(.a(new_n124_), .b(new_n114_), .c(new_n98_), .d(new_n86_), .O(new_n125_));
  oai21  g069(.a(new_n125_), .b(new_n76_), .c(x31), .O(new_n126_));
  nand2  g070(.a(new_n82_), .b(new_n68_), .O(new_n127_));
  inv1   g071(.a(new_n127_), .O(new_n128_));
  nor2   g072(.a(x29), .b(x28), .O(new_n129_));
  nor2   g073(.a(x31), .b(x30), .O(new_n130_));
  nand2  g074(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g075(.a(new_n131_), .O(new_n132_));
  nand4  g076(.a(new_n132_), .b(new_n112_), .c(new_n128_), .d(new_n67_), .O(new_n133_));
  nand2  g077(.a(new_n133_), .b(new_n126_), .O(new_n134_));
  nand2  g078(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g079(.a(x03), .O(new_n136_));
  aoi21  g080(.a(new_n54_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g081(.a(new_n137_), .b(new_n135_), .O(z12));
  nand4  g082(.a(new_n130_), .b(new_n129_), .c(new_n105_), .d(new_n87_), .O(new_n139_));
  oai21  g083(.a(new_n139_), .b(new_n83_), .c(x32), .O(new_n140_));
  nand2  g084(.a(new_n124_), .b(new_n114_), .O(new_n141_));
  inv1   g085(.a(new_n141_), .O(new_n142_));
  nor2   g086(.a(x32), .b(x31), .O(new_n143_));
  nand2  g087(.a(new_n143_), .b(new_n98_), .O(new_n144_));
  inv1   g088(.a(new_n144_), .O(new_n145_));
  nand4  g089(.a(new_n145_), .b(new_n142_), .c(new_n128_), .d(new_n67_), .O(new_n146_));
  nand2  g090(.a(new_n146_), .b(new_n140_), .O(new_n147_));
  nand2  g091(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g092(.a(x02), .O(new_n149_));
  aoi21  g093(.a(new_n54_), .b(new_n149_), .c(x18), .O(new_n150_));
  nand2  g094(.a(new_n150_), .b(new_n148_), .O(z13));
  nand4  g095(.a(new_n143_), .b(new_n124_), .c(new_n114_), .d(new_n98_), .O(new_n152_));
  oai21  g096(.a(new_n152_), .b(new_n83_), .c(x33), .O(new_n153_));
  nor2   g097(.a(x33), .b(x32), .O(new_n154_));
  nand4  g098(.a(new_n154_), .b(new_n130_), .c(new_n129_), .d(new_n105_), .O(new_n155_));
  inv1   g099(.a(new_n155_), .O(new_n156_));
  nand2  g100(.a(new_n156_), .b(new_n89_), .O(new_n157_));
  nand2  g101(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g102(.a(new_n158_), .b(x16), .O(new_n159_));
  inv1   g103(.a(x01), .O(new_n160_));
  aoi21  g104(.a(new_n54_), .b(new_n160_), .c(x18), .O(new_n161_));
  nand2  g105(.a(new_n161_), .b(new_n159_), .O(z14));
  oai21  g106(.a(new_n155_), .b(new_n96_), .c(x34), .O(new_n163_));
  nand2  g107(.a(new_n129_), .b(new_n105_), .O(new_n164_));
  inv1   g108(.a(new_n164_), .O(new_n165_));
  inv1   g109(.a(x30), .O(new_n166_));
  inv1   g110(.a(x31), .O(new_n167_));
  nand2  g111(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  inv1   g112(.a(x32), .O(new_n169_));
  inv1   g113(.a(x33), .O(new_n170_));
  inv1   g114(.a(x34), .O(new_n171_));
  nand3  g115(.a(new_n171_), .b(new_n170_), .c(new_n169_), .O(new_n172_));
  nor2   g116(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand3  g117(.a(new_n173_), .b(new_n165_), .c(new_n89_), .O(new_n174_));
  nand2  g118(.a(new_n174_), .b(new_n163_), .O(new_n175_));
  nand2  g119(.a(new_n175_), .b(x16), .O(new_n176_));
  inv1   g120(.a(x00), .O(new_n177_));
  aoi21  g121(.a(new_n54_), .b(new_n177_), .c(x18), .O(new_n178_));
  nand2  g122(.a(new_n178_), .b(new_n176_), .O(z15));
  zero   g123(.O(z00));
  zero   g124(.O(z01));
  zero   g125(.O(z05));
  zero   g126(.O(z10));
  zero   g127(.O(z11));
endmodule


