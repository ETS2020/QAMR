// Benchmark "FAU" written by ABC on Tue Jul  7 19:19:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x20), .O(new_n53_));
  nor2   g001(.a(x19), .b(x17), .O(new_n54_));
  nor2   g002(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g003(.a(x20), .b(x19), .c(x17), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g005(.a(x14), .O(new_n58_));
  inv1   g006(.a(x16), .O(new_n59_));
  aoi21  g007(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g009(.a(new_n56_), .O(new_n62_));
  inv1   g010(.a(x17), .O(new_n63_));
  inv1   g011(.a(x19), .O(new_n64_));
  inv1   g012(.a(x21), .O(new_n65_));
  nand4  g013(.a(new_n65_), .b(new_n53_), .c(new_n64_), .d(new_n63_), .O(new_n66_));
  inv1   g014(.a(new_n66_), .O(new_n67_));
  aoi21  g015(.a(new_n62_), .b(x21), .c(new_n67_), .O(new_n68_));
  inv1   g016(.a(x13), .O(new_n69_));
  aoi21  g017(.a(new_n59_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g018(.a(new_n68_), .b(new_n59_), .c(new_n70_), .O(z02));
  nor2   g019(.a(x22), .b(x21), .O(new_n72_));
  aoi22  g020(.a(new_n72_), .b(new_n56_), .c(new_n66_), .d(x22), .O(new_n73_));
  inv1   g021(.a(x12), .O(new_n74_));
  aoi21  g022(.a(new_n59_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g023(.a(new_n73_), .b(new_n59_), .c(new_n75_), .O(z03));
  nand3  g024(.a(new_n72_), .b(new_n54_), .c(new_n53_), .O(new_n77_));
  nand2  g025(.a(new_n77_), .b(x23), .O(new_n78_));
  nor2   g026(.a(x23), .b(x22), .O(new_n79_));
  nand4  g027(.a(new_n79_), .b(new_n54_), .c(new_n65_), .d(new_n53_), .O(new_n80_));
  and2   g028(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  inv1   g029(.a(x11), .O(new_n82_));
  aoi21  g030(.a(new_n59_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g031(.a(new_n81_), .b(new_n59_), .c(new_n83_), .O(z04));
  and2   g032(.a(new_n80_), .b(x24), .O(new_n85_));
  nor2   g033(.a(x24), .b(x23), .O(new_n86_));
  nand4  g034(.a(new_n86_), .b(new_n72_), .c(new_n54_), .d(new_n53_), .O(new_n87_));
  inv1   g035(.a(new_n87_), .O(new_n88_));
  oai21  g036(.a(new_n88_), .b(new_n85_), .c(x16), .O(new_n89_));
  inv1   g037(.a(x10), .O(new_n90_));
  aoi21  g038(.a(new_n59_), .b(new_n90_), .c(x18), .O(new_n91_));
  nand2  g039(.a(new_n91_), .b(new_n89_), .O(z05));
  nor3   g040(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  nand3  g041(.a(new_n95_), .b(new_n79_), .c(new_n67_), .O(new_n96_));
  inv1   g042(.a(x25), .O(new_n97_));
  inv1   g043(.a(x26), .O(new_n98_));
  inv1   g044(.a(x27), .O(new_n99_));
  nand4  g045(.a(new_n86_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  nor2   g046(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  aoi21  g047(.a(new_n96_), .b(x27), .c(new_n101_), .O(new_n102_));
  inv1   g048(.a(x07), .O(new_n103_));
  aoi21  g049(.a(new_n59_), .b(new_n103_), .c(x18), .O(new_n104_));
  oai21  g050(.a(new_n102_), .b(new_n59_), .c(new_n104_), .O(z08));
  nor3   g051(.a(x27), .b(x26), .c(x25), .O(new_n106_));
  nand4  g052(.a(new_n106_), .b(new_n86_), .c(new_n72_), .d(new_n56_), .O(new_n107_));
  inv1   g053(.a(x23), .O(new_n108_));
  nor2   g054(.a(x28), .b(x27), .O(new_n109_));
  nor2   g055(.a(x25), .b(x24), .O(new_n110_));
  nand4  g056(.a(new_n110_), .b(new_n109_), .c(new_n98_), .d(new_n108_), .O(new_n111_));
  nor2   g057(.a(new_n111_), .b(new_n77_), .O(new_n112_));
  aoi21  g058(.a(new_n107_), .b(x28), .c(new_n112_), .O(new_n113_));
  inv1   g059(.a(x06), .O(new_n114_));
  aoi21  g060(.a(new_n59_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g061(.a(new_n113_), .b(new_n59_), .c(new_n115_), .O(z09));
  inv1   g062(.a(x24), .O(new_n119_));
  nor2   g063(.a(x26), .b(x25), .O(new_n120_));
  nor2   g064(.a(x30), .b(x29), .O(new_n121_));
  nand4  g065(.a(new_n121_), .b(new_n109_), .c(new_n120_), .d(new_n119_), .O(new_n122_));
  oai21  g066(.a(new_n122_), .b(new_n80_), .c(x31), .O(new_n123_));
  inv1   g067(.a(x22), .O(new_n124_));
  nand4  g068(.a(new_n119_), .b(new_n108_), .c(new_n124_), .d(new_n65_), .O(new_n125_));
  inv1   g069(.a(new_n125_), .O(new_n126_));
  inv1   g070(.a(x28), .O(new_n127_));
  inv1   g071(.a(x29), .O(new_n128_));
  inv1   g072(.a(x30), .O(new_n129_));
  inv1   g073(.a(x31), .O(new_n130_));
  nand4  g074(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(new_n127_), .O(new_n131_));
  inv1   g075(.a(new_n131_), .O(new_n132_));
  nand4  g076(.a(new_n132_), .b(new_n106_), .c(new_n126_), .d(new_n56_), .O(new_n133_));
  nand2  g077(.a(new_n133_), .b(new_n123_), .O(new_n134_));
  nand2  g078(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g079(.a(x03), .O(new_n136_));
  aoi21  g080(.a(new_n59_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g081(.a(new_n137_), .b(new_n135_), .O(z12));
  nor2   g082(.a(x32), .b(x31), .O(new_n140_));
  nand4  g083(.a(new_n140_), .b(new_n121_), .c(new_n109_), .d(new_n120_), .O(new_n141_));
  oai21  g084(.a(new_n141_), .b(new_n87_), .c(x33), .O(new_n142_));
  nand4  g085(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n119_), .O(new_n143_));
  inv1   g086(.a(new_n143_), .O(new_n144_));
  inv1   g087(.a(x32), .O(new_n145_));
  inv1   g088(.a(x33), .O(new_n146_));
  nand4  g089(.a(new_n146_), .b(new_n145_), .c(new_n108_), .d(new_n124_), .O(new_n147_));
  inv1   g090(.a(new_n147_), .O(new_n148_));
  nand4  g091(.a(new_n148_), .b(new_n144_), .c(new_n132_), .d(new_n67_), .O(new_n149_));
  nand2  g092(.a(new_n149_), .b(new_n142_), .O(new_n150_));
  nand2  g093(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g094(.a(x01), .O(new_n152_));
  aoi21  g095(.a(new_n59_), .b(new_n152_), .c(x18), .O(new_n153_));
  nand2  g096(.a(new_n153_), .b(new_n151_), .O(z14));
  zero   g097(.O(z00));
  zero   g098(.O(z06));
  zero   g099(.O(z07));
  zero   g100(.O(z10));
  zero   g101(.O(z11));
  zero   g102(.O(z13));
  zero   g103(.O(z15));
endmodule


