// Benchmark "FAU" written by ABC on Tue Jul  7 19:20:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  nand2  g016(.a(new_n55_), .b(new_n61_), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nand4  g018(.a(new_n69_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n68_), .b(x21), .c(new_n71_), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n58_), .c(new_n74_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n68_), .O(new_n78_));
  aoi21  g027(.a(new_n70_), .b(x22), .c(new_n78_), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n58_), .b(new_n80_), .c(x18), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n58_), .c(new_n81_), .O(z03));
  nand2  g031(.a(new_n76_), .b(new_n63_), .O(new_n83_));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  aoi22  g033(.a(new_n84_), .b(new_n63_), .c(new_n83_), .d(x23), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n58_), .c(new_n87_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n90_));
  nand4  g038(.a(new_n90_), .b(new_n76_), .c(new_n55_), .d(new_n61_), .O(new_n91_));
  nand2  g039(.a(new_n91_), .b(x25), .O(new_n92_));
  nor2   g040(.a(x21), .b(x20), .O(new_n93_));
  nor2   g041(.a(x23), .b(x22), .O(new_n94_));
  nor2   g042(.a(x25), .b(x24), .O(new_n95_));
  nand4  g043(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n55_), .O(new_n96_));
  nand2  g044(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(x16), .O(new_n98_));
  inv1   g046(.a(x09), .O(new_n99_));
  aoi21  g047(.a(new_n58_), .b(new_n99_), .c(x18), .O(new_n100_));
  nand2  g048(.a(new_n100_), .b(new_n98_), .O(z06));
  nand2  g049(.a(new_n96_), .b(x26), .O(new_n102_));
  inv1   g050(.a(x24), .O(new_n103_));
  nor2   g051(.a(x26), .b(x25), .O(new_n104_));
  nand4  g052(.a(new_n104_), .b(new_n94_), .c(new_n71_), .d(new_n103_), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nand2  g054(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g055(.a(x08), .O(new_n108_));
  aoi21  g056(.a(new_n58_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand2  g057(.a(new_n109_), .b(new_n107_), .O(z07));
  nor3   g058(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  nand4  g059(.a(new_n112_), .b(new_n90_), .c(new_n76_), .d(new_n63_), .O(new_n113_));
  inv1   g060(.a(x23), .O(new_n114_));
  inv1   g061(.a(x26), .O(new_n115_));
  nor2   g062(.a(x28), .b(x27), .O(new_n116_));
  nand4  g063(.a(new_n116_), .b(new_n95_), .c(new_n115_), .d(new_n114_), .O(new_n117_));
  inv1   g064(.a(new_n117_), .O(new_n118_));
  aoi22  g065(.a(new_n118_), .b(new_n78_), .c(new_n113_), .d(x28), .O(new_n119_));
  inv1   g066(.a(x06), .O(new_n120_));
  aoi21  g067(.a(new_n58_), .b(new_n120_), .c(x18), .O(new_n121_));
  oai21  g068(.a(new_n119_), .b(new_n58_), .c(new_n121_), .O(z09));
  nand4  g069(.a(new_n94_), .b(new_n55_), .c(new_n69_), .d(new_n61_), .O(new_n125_));
  nor2   g070(.a(x30), .b(x29), .O(new_n126_));
  nand4  g071(.a(new_n126_), .b(new_n116_), .c(new_n104_), .d(new_n103_), .O(new_n127_));
  oai21  g072(.a(new_n127_), .b(new_n125_), .c(x31), .O(new_n128_));
  inv1   g073(.a(x22), .O(new_n129_));
  nand4  g074(.a(new_n103_), .b(new_n114_), .c(new_n129_), .d(new_n69_), .O(new_n130_));
  inv1   g075(.a(new_n130_), .O(new_n131_));
  inv1   g076(.a(x28), .O(new_n132_));
  inv1   g077(.a(x29), .O(new_n133_));
  inv1   g078(.a(x30), .O(new_n134_));
  inv1   g079(.a(x31), .O(new_n135_));
  nand4  g080(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n132_), .O(new_n136_));
  inv1   g081(.a(new_n136_), .O(new_n137_));
  nand4  g082(.a(new_n137_), .b(new_n112_), .c(new_n131_), .d(new_n63_), .O(new_n138_));
  nand2  g083(.a(new_n138_), .b(new_n128_), .O(new_n139_));
  nand2  g084(.a(new_n139_), .b(x16), .O(new_n140_));
  inv1   g085(.a(x03), .O(new_n141_));
  aoi21  g086(.a(new_n58_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand2  g087(.a(new_n142_), .b(new_n140_), .O(z12));
  nor2   g088(.a(x32), .b(x31), .O(new_n145_));
  nand4  g089(.a(new_n145_), .b(new_n126_), .c(new_n116_), .d(new_n104_), .O(new_n146_));
  oai21  g090(.a(new_n146_), .b(new_n91_), .c(x33), .O(new_n147_));
  inv1   g091(.a(x25), .O(new_n148_));
  nand4  g092(.a(new_n148_), .b(new_n103_), .c(new_n114_), .d(new_n129_), .O(new_n149_));
  inv1   g093(.a(new_n149_), .O(new_n150_));
  inv1   g094(.a(x27), .O(new_n151_));
  inv1   g095(.a(x32), .O(new_n152_));
  inv1   g096(.a(x33), .O(new_n153_));
  nand4  g097(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n115_), .O(new_n154_));
  inv1   g098(.a(new_n154_), .O(new_n155_));
  nand4  g099(.a(new_n155_), .b(new_n137_), .c(new_n150_), .d(new_n71_), .O(new_n156_));
  nand2  g100(.a(new_n156_), .b(new_n147_), .O(new_n157_));
  nand2  g101(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g102(.a(x01), .O(new_n159_));
  aoi21  g103(.a(new_n58_), .b(new_n159_), .c(x18), .O(new_n160_));
  nand2  g104(.a(new_n160_), .b(new_n158_), .O(z14));
  zero   g105(.O(z05));
  zero   g106(.O(z08));
  zero   g107(.O(z10));
  zero   g108(.O(z11));
  zero   g109(.O(z13));
  zero   g110(.O(z15));
endmodule


