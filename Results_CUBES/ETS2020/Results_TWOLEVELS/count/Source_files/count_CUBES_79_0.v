// Benchmark "FAU" written by ABC on Tue Jul  7 19:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x20), .O(new_n53_));
  nor2   g001(.a(x19), .b(x17), .O(new_n54_));
  nor2   g002(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g003(.a(x20), .b(x19), .c(x17), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g005(.a(x14), .O(new_n58_));
  inv1   g006(.a(x16), .O(new_n59_));
  aoi21  g007(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n57_), .O(z01));
  nand2  g009(.a(new_n54_), .b(new_n53_), .O(new_n62_));
  nor2   g010(.a(x21), .b(x20), .O(new_n63_));
  aoi22  g011(.a(new_n63_), .b(new_n54_), .c(new_n62_), .d(x21), .O(new_n64_));
  inv1   g012(.a(x13), .O(new_n65_));
  aoi21  g013(.a(new_n59_), .b(new_n65_), .c(x18), .O(new_n66_));
  oai21  g014(.a(new_n64_), .b(new_n59_), .c(new_n66_), .O(z02));
  inv1   g015(.a(x17), .O(new_n68_));
  inv1   g016(.a(x19), .O(new_n69_));
  inv1   g017(.a(x21), .O(new_n70_));
  nand4  g018(.a(new_n70_), .b(new_n53_), .c(new_n69_), .d(new_n68_), .O(new_n71_));
  nor2   g019(.a(x22), .b(x21), .O(new_n72_));
  aoi22  g020(.a(new_n72_), .b(new_n56_), .c(new_n71_), .d(x22), .O(new_n73_));
  inv1   g021(.a(x12), .O(new_n74_));
  aoi21  g022(.a(new_n59_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g023(.a(new_n73_), .b(new_n59_), .c(new_n75_), .O(z03));
  nand3  g024(.a(new_n72_), .b(new_n54_), .c(new_n53_), .O(new_n77_));
  nor2   g025(.a(x23), .b(x22), .O(new_n78_));
  nand2  g026(.a(new_n78_), .b(new_n70_), .O(new_n79_));
  nor2   g027(.a(new_n79_), .b(new_n62_), .O(new_n80_));
  aoi21  g028(.a(new_n77_), .b(x23), .c(new_n80_), .O(new_n81_));
  inv1   g029(.a(x11), .O(new_n82_));
  aoi21  g030(.a(new_n59_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g031(.a(new_n81_), .b(new_n59_), .c(new_n83_), .O(z04));
  nand3  g032(.a(new_n78_), .b(new_n56_), .c(new_n70_), .O(new_n85_));
  nor2   g033(.a(x24), .b(x23), .O(new_n86_));
  nand4  g034(.a(new_n86_), .b(new_n72_), .c(new_n54_), .d(new_n53_), .O(new_n87_));
  inv1   g035(.a(new_n87_), .O(new_n88_));
  aoi21  g036(.a(new_n85_), .b(x24), .c(new_n88_), .O(new_n89_));
  inv1   g037(.a(x10), .O(new_n90_));
  aoi21  g038(.a(new_n59_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g039(.a(new_n89_), .b(new_n59_), .c(new_n91_), .O(z05));
  inv1   g040(.a(x22), .O(new_n93_));
  inv1   g041(.a(x23), .O(new_n94_));
  inv1   g042(.a(x24), .O(new_n95_));
  inv1   g043(.a(x25), .O(new_n96_));
  nand4  g044(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n97_));
  nor2   g045(.a(new_n97_), .b(new_n71_), .O(new_n98_));
  aoi21  g046(.a(new_n87_), .b(x25), .c(new_n98_), .O(new_n99_));
  inv1   g047(.a(x09), .O(new_n100_));
  aoi21  g048(.a(new_n59_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g049(.a(new_n99_), .b(new_n59_), .c(new_n101_), .O(z06));
  nor3   g050(.a(x27), .b(x26), .c(x25), .O(new_n105_));
  nand4  g051(.a(new_n105_), .b(new_n86_), .c(new_n72_), .d(new_n56_), .O(new_n106_));
  nor2   g052(.a(x25), .b(x24), .O(new_n107_));
  nor2   g053(.a(x28), .b(x27), .O(new_n108_));
  nor2   g054(.a(x26), .b(x23), .O(new_n109_));
  nand3  g055(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  nor2   g056(.a(new_n110_), .b(new_n77_), .O(new_n111_));
  aoi21  g057(.a(new_n106_), .b(x28), .c(new_n111_), .O(new_n112_));
  inv1   g058(.a(x06), .O(new_n113_));
  aoi21  g059(.a(new_n59_), .b(new_n113_), .c(x18), .O(new_n114_));
  oai21  g060(.a(new_n112_), .b(new_n59_), .c(new_n114_), .O(z09));
  oai21  g061(.a(new_n110_), .b(new_n77_), .c(x29), .O(new_n116_));
  nor2   g062(.a(x29), .b(x28), .O(new_n117_));
  nor2   g063(.a(x26), .b(x25), .O(new_n118_));
  nor2   g064(.a(x27), .b(x24), .O(new_n119_));
  nand3  g065(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  oai21  g066(.a(new_n120_), .b(new_n85_), .c(new_n116_), .O(new_n121_));
  nand2  g067(.a(new_n121_), .b(x16), .O(new_n122_));
  inv1   g068(.a(x05), .O(new_n123_));
  aoi21  g069(.a(new_n59_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand2  g070(.a(new_n124_), .b(new_n122_), .O(z10));
  nor2   g071(.a(x27), .b(x26), .O(new_n128_));
  nor2   g072(.a(x31), .b(x30), .O(new_n129_));
  nand4  g073(.a(new_n129_), .b(new_n117_), .c(new_n128_), .d(new_n96_), .O(new_n130_));
  oai21  g074(.a(new_n130_), .b(new_n87_), .c(x32), .O(new_n131_));
  nor2   g075(.a(x30), .b(x29), .O(new_n132_));
  nor2   g076(.a(x32), .b(x31), .O(new_n133_));
  nand4  g077(.a(new_n133_), .b(new_n132_), .c(new_n118_), .d(new_n108_), .O(new_n134_));
  inv1   g078(.a(new_n134_), .O(new_n135_));
  nand2  g079(.a(new_n135_), .b(new_n88_), .O(new_n136_));
  nand2  g080(.a(new_n136_), .b(new_n131_), .O(new_n137_));
  nand2  g081(.a(new_n137_), .b(x16), .O(new_n138_));
  inv1   g082(.a(x02), .O(new_n139_));
  aoi21  g083(.a(new_n59_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g084(.a(new_n140_), .b(new_n138_), .O(z13));
  oai21  g085(.a(new_n134_), .b(new_n87_), .c(x33), .O(new_n142_));
  nor2   g086(.a(x33), .b(x32), .O(new_n143_));
  nand4  g087(.a(new_n143_), .b(new_n129_), .c(new_n117_), .d(new_n128_), .O(new_n144_));
  inv1   g088(.a(new_n144_), .O(new_n145_));
  nand2  g089(.a(new_n145_), .b(new_n98_), .O(new_n146_));
  nand2  g090(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand2  g091(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g092(.a(x01), .O(new_n149_));
  aoi21  g093(.a(new_n59_), .b(new_n149_), .c(x18), .O(new_n150_));
  nand2  g094(.a(new_n150_), .b(new_n148_), .O(z14));
  nand4  g095(.a(new_n107_), .b(new_n78_), .c(new_n63_), .d(new_n54_), .O(new_n152_));
  oai21  g096(.a(new_n144_), .b(new_n152_), .c(x34), .O(new_n153_));
  nand2  g097(.a(new_n117_), .b(new_n128_), .O(new_n154_));
  inv1   g098(.a(new_n154_), .O(new_n155_));
  inv1   g099(.a(x30), .O(new_n156_));
  inv1   g100(.a(x31), .O(new_n157_));
  nand2  g101(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  inv1   g102(.a(x32), .O(new_n159_));
  inv1   g103(.a(x33), .O(new_n160_));
  inv1   g104(.a(x34), .O(new_n161_));
  nand3  g105(.a(new_n161_), .b(new_n160_), .c(new_n159_), .O(new_n162_));
  nor2   g106(.a(new_n162_), .b(new_n158_), .O(new_n163_));
  nand3  g107(.a(new_n163_), .b(new_n155_), .c(new_n98_), .O(new_n164_));
  nand2  g108(.a(new_n164_), .b(new_n153_), .O(new_n165_));
  nand2  g109(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g110(.a(x00), .O(new_n167_));
  aoi21  g111(.a(new_n59_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g112(.a(new_n168_), .b(new_n166_), .O(z15));
  zero   g113(.O(z00));
  zero   g114(.O(z07));
  zero   g115(.O(z08));
  zero   g116(.O(z11));
  zero   g117(.O(z12));
endmodule


