// Benchmark "FAU" written by ABC on Tue Jul  7 19:20:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  inv1   g000(.a(x20), .O(new_n53_));
  nor2   g001(.a(x19), .b(x17), .O(new_n54_));
  nor2   g002(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g003(.a(x20), .b(x19), .c(x17), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g005(.a(x14), .O(new_n58_));
  inv1   g006(.a(x16), .O(new_n59_));
  aoi21  g007(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g009(.a(x17), .O(new_n63_));
  inv1   g010(.a(x19), .O(new_n64_));
  inv1   g011(.a(x21), .O(new_n65_));
  nand4  g012(.a(new_n65_), .b(new_n53_), .c(new_n64_), .d(new_n63_), .O(new_n66_));
  nor2   g013(.a(x22), .b(x21), .O(new_n67_));
  aoi22  g014(.a(new_n67_), .b(new_n56_), .c(new_n66_), .d(x22), .O(new_n68_));
  inv1   g015(.a(x12), .O(new_n69_));
  aoi21  g016(.a(new_n59_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g017(.a(new_n68_), .b(new_n59_), .c(new_n70_), .O(z03));
  nor2   g018(.a(x24), .b(x23), .O(new_n74_));
  nand4  g019(.a(new_n74_), .b(new_n67_), .c(new_n54_), .d(new_n53_), .O(new_n75_));
  inv1   g020(.a(x22), .O(new_n76_));
  inv1   g021(.a(x23), .O(new_n77_));
  inv1   g022(.a(x24), .O(new_n78_));
  inv1   g023(.a(x25), .O(new_n79_));
  nand4  g024(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n80_));
  nor2   g025(.a(new_n80_), .b(new_n66_), .O(new_n81_));
  aoi21  g026(.a(new_n75_), .b(x25), .c(new_n81_), .O(new_n82_));
  inv1   g027(.a(x09), .O(new_n83_));
  aoi21  g028(.a(new_n59_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g029(.a(new_n82_), .b(new_n59_), .c(new_n84_), .O(z06));
  inv1   g030(.a(x26), .O(new_n86_));
  nor2   g031(.a(x21), .b(x20), .O(new_n87_));
  nor2   g032(.a(x23), .b(x22), .O(new_n88_));
  nor3   g033(.a(x26), .b(x25), .c(x24), .O(new_n89_));
  nand4  g034(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n54_), .O(new_n90_));
  oai21  g035(.a(new_n81_), .b(new_n86_), .c(new_n90_), .O(new_n91_));
  nand2  g036(.a(new_n91_), .b(x16), .O(new_n92_));
  inv1   g037(.a(x08), .O(new_n93_));
  aoi21  g038(.a(new_n59_), .b(new_n93_), .c(x18), .O(new_n94_));
  nand2  g039(.a(new_n94_), .b(new_n92_), .O(z07));
  nand3  g040(.a(new_n67_), .b(new_n54_), .c(new_n53_), .O(new_n96_));
  nor2   g041(.a(x27), .b(x26), .O(new_n97_));
  nand3  g042(.a(new_n97_), .b(new_n74_), .c(new_n79_), .O(new_n98_));
  nor2   g043(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  aoi21  g044(.a(new_n90_), .b(x27), .c(new_n99_), .O(new_n100_));
  inv1   g045(.a(x07), .O(new_n101_));
  aoi21  g046(.a(new_n59_), .b(new_n101_), .c(x18), .O(new_n102_));
  oai21  g047(.a(new_n100_), .b(new_n59_), .c(new_n102_), .O(z08));
  nor3   g048(.a(x27), .b(x26), .c(x25), .O(new_n104_));
  nand4  g049(.a(new_n104_), .b(new_n74_), .c(new_n67_), .d(new_n56_), .O(new_n105_));
  nor2   g050(.a(x25), .b(x24), .O(new_n106_));
  nor2   g051(.a(x28), .b(x27), .O(new_n107_));
  nand4  g052(.a(new_n107_), .b(new_n106_), .c(new_n86_), .d(new_n77_), .O(new_n108_));
  nor2   g053(.a(new_n108_), .b(new_n96_), .O(new_n109_));
  aoi21  g054(.a(new_n105_), .b(x28), .c(new_n109_), .O(new_n110_));
  inv1   g055(.a(x06), .O(new_n111_));
  aoi21  g056(.a(new_n59_), .b(new_n111_), .c(x18), .O(new_n112_));
  oai21  g057(.a(new_n110_), .b(new_n59_), .c(new_n112_), .O(z09));
  inv1   g058(.a(x29), .O(new_n114_));
  nor3   g059(.a(x23), .b(x22), .c(x21), .O(new_n115_));
  nor3   g060(.a(x29), .b(x28), .c(x27), .O(new_n116_));
  nand4  g061(.a(new_n116_), .b(new_n115_), .c(new_n89_), .d(new_n56_), .O(new_n117_));
  oai21  g062(.a(new_n109_), .b(new_n114_), .c(new_n117_), .O(new_n118_));
  nand2  g063(.a(new_n118_), .b(x16), .O(new_n119_));
  inv1   g064(.a(x05), .O(new_n120_));
  aoi21  g065(.a(new_n59_), .b(new_n120_), .c(x18), .O(new_n121_));
  nand2  g066(.a(new_n121_), .b(new_n119_), .O(z10));
  nand4  g067(.a(new_n88_), .b(new_n54_), .c(new_n65_), .d(new_n53_), .O(new_n123_));
  nor2   g068(.a(x26), .b(x25), .O(new_n124_));
  nor2   g069(.a(x30), .b(x29), .O(new_n125_));
  nand4  g070(.a(new_n125_), .b(new_n107_), .c(new_n124_), .d(new_n78_), .O(new_n126_));
  nor2   g071(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  aoi21  g072(.a(new_n117_), .b(x30), .c(new_n127_), .O(new_n128_));
  inv1   g073(.a(x04), .O(new_n129_));
  aoi21  g074(.a(new_n59_), .b(new_n129_), .c(x18), .O(new_n130_));
  oai21  g075(.a(new_n128_), .b(new_n59_), .c(new_n130_), .O(z11));
  oai21  g076(.a(new_n126_), .b(new_n123_), .c(x31), .O(new_n132_));
  nand2  g077(.a(new_n74_), .b(new_n67_), .O(new_n133_));
  inv1   g078(.a(new_n133_), .O(new_n134_));
  nor2   g079(.a(x29), .b(x28), .O(new_n135_));
  nor2   g080(.a(x31), .b(x30), .O(new_n136_));
  nand2  g081(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  inv1   g082(.a(new_n137_), .O(new_n138_));
  nand4  g083(.a(new_n138_), .b(new_n104_), .c(new_n134_), .d(new_n56_), .O(new_n139_));
  nand2  g084(.a(new_n139_), .b(new_n132_), .O(new_n140_));
  nand2  g085(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g086(.a(x03), .O(new_n142_));
  aoi21  g087(.a(new_n59_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g088(.a(new_n143_), .b(new_n141_), .O(z12));
  nand4  g089(.a(new_n136_), .b(new_n135_), .c(new_n97_), .d(new_n79_), .O(new_n145_));
  oai21  g090(.a(new_n145_), .b(new_n75_), .c(x32), .O(new_n146_));
  nand2  g091(.a(new_n125_), .b(new_n107_), .O(new_n147_));
  inv1   g092(.a(new_n147_), .O(new_n148_));
  nor2   g093(.a(x32), .b(x31), .O(new_n149_));
  nand2  g094(.a(new_n149_), .b(new_n124_), .O(new_n150_));
  inv1   g095(.a(new_n150_), .O(new_n151_));
  nand4  g096(.a(new_n151_), .b(new_n148_), .c(new_n134_), .d(new_n56_), .O(new_n152_));
  nand2  g097(.a(new_n152_), .b(new_n146_), .O(new_n153_));
  nand2  g098(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g099(.a(x02), .O(new_n155_));
  aoi21  g100(.a(new_n59_), .b(new_n155_), .c(x18), .O(new_n156_));
  nand2  g101(.a(new_n156_), .b(new_n154_), .O(z13));
  nand4  g102(.a(new_n149_), .b(new_n125_), .c(new_n107_), .d(new_n124_), .O(new_n158_));
  oai21  g103(.a(new_n158_), .b(new_n75_), .c(x33), .O(new_n159_));
  nor2   g104(.a(x33), .b(x32), .O(new_n160_));
  nand4  g105(.a(new_n160_), .b(new_n136_), .c(new_n135_), .d(new_n97_), .O(new_n161_));
  inv1   g106(.a(new_n161_), .O(new_n162_));
  nand2  g107(.a(new_n162_), .b(new_n81_), .O(new_n163_));
  nand2  g108(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g109(.a(new_n164_), .b(x16), .O(new_n165_));
  inv1   g110(.a(x01), .O(new_n166_));
  aoi21  g111(.a(new_n59_), .b(new_n166_), .c(x18), .O(new_n167_));
  nand2  g112(.a(new_n167_), .b(new_n165_), .O(z14));
  nand4  g113(.a(new_n106_), .b(new_n88_), .c(new_n87_), .d(new_n54_), .O(new_n169_));
  oai21  g114(.a(new_n161_), .b(new_n169_), .c(x34), .O(new_n170_));
  nand2  g115(.a(new_n135_), .b(new_n97_), .O(new_n171_));
  inv1   g116(.a(new_n171_), .O(new_n172_));
  inv1   g117(.a(x30), .O(new_n173_));
  inv1   g118(.a(x31), .O(new_n174_));
  nand2  g119(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  inv1   g120(.a(x32), .O(new_n176_));
  inv1   g121(.a(x33), .O(new_n177_));
  inv1   g122(.a(x34), .O(new_n178_));
  nand3  g123(.a(new_n178_), .b(new_n177_), .c(new_n176_), .O(new_n179_));
  nor2   g124(.a(new_n179_), .b(new_n175_), .O(new_n180_));
  nand3  g125(.a(new_n180_), .b(new_n172_), .c(new_n81_), .O(new_n181_));
  nand2  g126(.a(new_n181_), .b(new_n170_), .O(new_n182_));
  nand2  g127(.a(new_n182_), .b(x16), .O(new_n183_));
  inv1   g128(.a(x00), .O(new_n184_));
  aoi21  g129(.a(new_n59_), .b(new_n184_), .c(x18), .O(new_n185_));
  nand2  g130(.a(new_n185_), .b(new_n183_), .O(z15));
  zero   g131(.O(z00));
  zero   g132(.O(z02));
  zero   g133(.O(z04));
  zero   g134(.O(z05));
endmodule


