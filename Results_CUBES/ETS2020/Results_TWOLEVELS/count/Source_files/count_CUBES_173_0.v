// Benchmark "FAU" written by ABC on Tue Jul  7 19:19:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  nand4  g018(.a(new_n68_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n69_), .c(x16), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n72_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n79_));
  nor2   g025(.a(x24), .b(x23), .O(new_n80_));
  nand4  g026(.a(new_n80_), .b(new_n79_), .c(new_n55_), .d(new_n61_), .O(new_n81_));
  inv1   g027(.a(x22), .O(new_n82_));
  inv1   g028(.a(x23), .O(new_n83_));
  inv1   g029(.a(x24), .O(new_n84_));
  inv1   g030(.a(x25), .O(new_n85_));
  nand4  g031(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n86_));
  nor2   g032(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  aoi21  g033(.a(new_n81_), .b(x25), .c(new_n87_), .O(new_n88_));
  inv1   g034(.a(x09), .O(new_n89_));
  aoi21  g035(.a(new_n58_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g036(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(z06));
  nor2   g037(.a(x21), .b(x20), .O(new_n92_));
  nor2   g038(.a(x23), .b(x22), .O(new_n93_));
  nor2   g039(.a(x25), .b(x24), .O(new_n94_));
  nand4  g040(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n55_), .O(new_n95_));
  nand2  g041(.a(new_n95_), .b(x26), .O(new_n96_));
  nor3   g042(.a(x26), .b(x25), .c(x24), .O(new_n97_));
  nand3  g043(.a(new_n97_), .b(new_n93_), .c(new_n71_), .O(new_n98_));
  nand2  g044(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g045(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g046(.a(x08), .O(new_n101_));
  aoi21  g047(.a(new_n58_), .b(new_n101_), .c(x18), .O(new_n102_));
  nand2  g048(.a(new_n102_), .b(new_n100_), .O(z07));
  nand3  g049(.a(new_n79_), .b(new_n55_), .c(new_n61_), .O(new_n104_));
  nor2   g050(.a(x27), .b(x26), .O(new_n105_));
  nand3  g051(.a(new_n105_), .b(new_n80_), .c(new_n85_), .O(new_n106_));
  nor2   g052(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g053(.a(new_n98_), .b(x27), .c(new_n107_), .O(new_n108_));
  inv1   g054(.a(x07), .O(new_n109_));
  aoi21  g055(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g056(.a(new_n108_), .b(new_n58_), .c(new_n110_), .O(z08));
  nor3   g057(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  nand4  g058(.a(new_n112_), .b(new_n80_), .c(new_n79_), .d(new_n63_), .O(new_n113_));
  inv1   g059(.a(x26), .O(new_n114_));
  nor2   g060(.a(x28), .b(x27), .O(new_n115_));
  nand4  g061(.a(new_n115_), .b(new_n94_), .c(new_n114_), .d(new_n83_), .O(new_n116_));
  nor2   g062(.a(new_n116_), .b(new_n104_), .O(new_n117_));
  aoi21  g063(.a(new_n113_), .b(x28), .c(new_n117_), .O(new_n118_));
  inv1   g064(.a(x06), .O(new_n119_));
  aoi21  g065(.a(new_n58_), .b(new_n119_), .c(x18), .O(new_n120_));
  oai21  g066(.a(new_n118_), .b(new_n58_), .c(new_n120_), .O(z09));
  inv1   g067(.a(x29), .O(new_n122_));
  nor3   g068(.a(x23), .b(x22), .c(x21), .O(new_n123_));
  nor3   g069(.a(x29), .b(x28), .c(x27), .O(new_n124_));
  nand4  g070(.a(new_n124_), .b(new_n123_), .c(new_n97_), .d(new_n63_), .O(new_n125_));
  oai21  g071(.a(new_n117_), .b(new_n122_), .c(new_n125_), .O(new_n126_));
  nand2  g072(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g073(.a(x05), .O(new_n128_));
  aoi21  g074(.a(new_n58_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand2  g075(.a(new_n129_), .b(new_n127_), .O(z10));
  nand4  g076(.a(new_n93_), .b(new_n55_), .c(new_n68_), .d(new_n61_), .O(new_n131_));
  nor2   g077(.a(x26), .b(x25), .O(new_n132_));
  nor2   g078(.a(x30), .b(x29), .O(new_n133_));
  nand4  g079(.a(new_n133_), .b(new_n115_), .c(new_n132_), .d(new_n84_), .O(new_n134_));
  nor2   g080(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  aoi21  g081(.a(new_n125_), .b(x30), .c(new_n135_), .O(new_n136_));
  inv1   g082(.a(x04), .O(new_n137_));
  aoi21  g083(.a(new_n58_), .b(new_n137_), .c(x18), .O(new_n138_));
  oai21  g084(.a(new_n136_), .b(new_n58_), .c(new_n138_), .O(z11));
  nor2   g085(.a(x29), .b(x28), .O(new_n141_));
  nor2   g086(.a(x31), .b(x30), .O(new_n142_));
  nand4  g087(.a(new_n142_), .b(new_n141_), .c(new_n105_), .d(new_n85_), .O(new_n143_));
  oai21  g088(.a(new_n143_), .b(new_n81_), .c(x32), .O(new_n144_));
  nand2  g089(.a(new_n80_), .b(new_n79_), .O(new_n145_));
  inv1   g090(.a(new_n145_), .O(new_n146_));
  nand2  g091(.a(new_n133_), .b(new_n115_), .O(new_n147_));
  inv1   g092(.a(new_n147_), .O(new_n148_));
  inv1   g093(.a(x31), .O(new_n149_));
  inv1   g094(.a(x32), .O(new_n150_));
  nand4  g095(.a(new_n150_), .b(new_n149_), .c(new_n114_), .d(new_n85_), .O(new_n151_));
  inv1   g096(.a(new_n151_), .O(new_n152_));
  nand4  g097(.a(new_n152_), .b(new_n148_), .c(new_n146_), .d(new_n63_), .O(new_n153_));
  nand2  g098(.a(new_n153_), .b(new_n144_), .O(new_n154_));
  nand2  g099(.a(new_n154_), .b(x16), .O(new_n155_));
  inv1   g100(.a(x02), .O(new_n156_));
  aoi21  g101(.a(new_n58_), .b(new_n156_), .c(x18), .O(new_n157_));
  nand2  g102(.a(new_n157_), .b(new_n155_), .O(z13));
  nor2   g103(.a(x33), .b(x32), .O(new_n160_));
  nand4  g104(.a(new_n160_), .b(new_n142_), .c(new_n141_), .d(new_n105_), .O(new_n161_));
  oai21  g105(.a(new_n161_), .b(new_n95_), .c(x34), .O(new_n162_));
  nand2  g106(.a(new_n141_), .b(new_n105_), .O(new_n163_));
  inv1   g107(.a(new_n163_), .O(new_n164_));
  inv1   g108(.a(x30), .O(new_n165_));
  nand2  g109(.a(new_n149_), .b(new_n165_), .O(new_n166_));
  inv1   g110(.a(x33), .O(new_n167_));
  inv1   g111(.a(x34), .O(new_n168_));
  nand3  g112(.a(new_n168_), .b(new_n167_), .c(new_n150_), .O(new_n169_));
  nor2   g113(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nand3  g114(.a(new_n170_), .b(new_n164_), .c(new_n87_), .O(new_n171_));
  nand2  g115(.a(new_n171_), .b(new_n162_), .O(new_n172_));
  nand2  g116(.a(new_n172_), .b(x16), .O(new_n173_));
  inv1   g117(.a(x00), .O(new_n174_));
  aoi21  g118(.a(new_n58_), .b(new_n174_), .c(x18), .O(new_n175_));
  nand2  g119(.a(new_n175_), .b(new_n173_), .O(z15));
  zero   g120(.O(z03));
  zero   g121(.O(z04));
  zero   g122(.O(z05));
  zero   g123(.O(z12));
  zero   g124(.O(z14));
endmodule


