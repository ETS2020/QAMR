// Benchmark "FAU" written by ABC on Tue Jul  7 11:32:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  nand4  g05(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g06(.a(x14), .O(z04));
  inv1   g07(.a(x13), .O(z05));
  and2   g08(.a(x03), .b(x02), .O(new_n40_));
  inv1   g09(.a(x04), .O(new_n41_));
  aoi21  g10(.a(x12), .b(new_n31_), .c(new_n41_), .O(new_n42_));
  nand2  g11(.a(x09), .b(x01), .O(new_n43_));
  aoi21  g12(.a(new_n42_), .b(new_n40_), .c(new_n43_), .O(z06));
  inv1   g13(.a(x15), .O(z07));
  nand2  g14(.a(x12), .b(new_n31_), .O(new_n46_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g16(.a(new_n47_), .O(new_n48_));
  nand3  g17(.a(x12), .b(x11), .c(x09), .O(new_n49_));
  inv1   g18(.a(new_n49_), .O(new_n50_));
  nand2  g19(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g20(.a(new_n32_), .b(new_n31_), .O(new_n52_));
  inv1   g21(.a(x10), .O(new_n53_));
  nand2  g22(.a(new_n53_), .b(x00), .O(new_n54_));
  inv1   g23(.a(new_n54_), .O(new_n55_));
  nand4  g24(.a(new_n55_), .b(new_n52_), .c(new_n51_), .d(new_n46_), .O(z08));
  nand2  g25(.a(x12), .b(x11), .O(new_n57_));
  aoi21  g26(.a(new_n57_), .b(new_n52_), .c(new_n54_), .O(z09));
  nand2  g27(.a(new_n47_), .b(x12), .O(new_n59_));
  nand4  g28(.a(x11), .b(new_n53_), .c(x09), .d(x00), .O(new_n60_));
  nor2   g29(.a(new_n60_), .b(new_n59_), .O(z10));
  nand4  g30(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n62_));
  nand2  g31(.a(new_n32_), .b(new_n30_), .O(new_n63_));
  nand3  g32(.a(x11), .b(new_n53_), .c(x00), .O(new_n64_));
  aoi21  g33(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(z11));
  inv1   g34(.a(x09), .O(new_n66_));
  nand3  g35(.a(x12), .b(x11), .c(new_n66_), .O(new_n67_));
  nor2   g36(.a(new_n67_), .b(new_n54_), .O(z12));
  zero   g37(.O(z01));
  zero   g38(.O(z02));
endmodule


