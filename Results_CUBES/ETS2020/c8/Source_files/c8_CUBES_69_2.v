// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n52_, new_n54_, new_n55_, new_n59_,
    new_n60_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x10), .O(new_n51_));
  nand2  g04(.a(x27), .b(new_n51_), .O(new_n52_));
  oai21  g05(.a(x27), .b(x21), .c(new_n52_), .O(z02));
  inv1   g06(.a(x11), .O(new_n54_));
  nand2  g07(.a(x27), .b(new_n54_), .O(new_n55_));
  oai21  g08(.a(x27), .b(x22), .c(new_n55_), .O(z03));
  inv1   g09(.a(x14), .O(new_n59_));
  nand2  g10(.a(x27), .b(new_n59_), .O(new_n60_));
  oai21  g11(.a(x27), .b(x25), .c(new_n60_), .O(z06));
  and2   g12(.a(x19), .b(x17), .O(new_n63_));
  nor2   g13(.a(x19), .b(x17), .O(new_n64_));
  oai21  g14(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g15(.a(x18), .b(new_n47_), .O(new_n66_));
  aoi21  g16(.a(x18), .b(x00), .c(new_n66_), .O(new_n67_));
  oai21  g17(.a(new_n67_), .b(x16), .c(new_n65_), .O(z09));
  inv1   g18(.a(x09), .O(new_n69_));
  nor2   g19(.a(x18), .b(new_n69_), .O(new_n70_));
  aoi21  g20(.a(x18), .b(x01), .c(new_n70_), .O(new_n71_));
  inv1   g21(.a(x20), .O(new_n72_));
  nor2   g22(.a(new_n64_), .b(new_n72_), .O(new_n73_));
  nor3   g23(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g24(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  oai21  g25(.a(new_n71_), .b(x16), .c(new_n75_), .O(z10));
  inv1   g26(.a(x16), .O(new_n77_));
  nand2  g27(.a(x18), .b(x02), .O(new_n78_));
  oai21  g28(.a(x18), .b(new_n51_), .c(new_n78_), .O(new_n79_));
  nand2  g29(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g30(.a(new_n64_), .b(new_n72_), .O(new_n81_));
  nor2   g31(.a(x21), .b(x20), .O(new_n82_));
  aoi22  g32(.a(new_n82_), .b(new_n64_), .c(new_n81_), .d(x21), .O(new_n83_));
  oai21  g33(.a(new_n83_), .b(new_n77_), .c(new_n80_), .O(z11));
  nand2  g34(.a(x18), .b(x03), .O(new_n85_));
  oai21  g35(.a(x18), .b(new_n54_), .c(new_n85_), .O(new_n86_));
  nand2  g36(.a(new_n86_), .b(new_n77_), .O(new_n87_));
  nand2  g37(.a(new_n82_), .b(new_n64_), .O(new_n88_));
  nor2   g38(.a(x22), .b(x21), .O(new_n89_));
  aoi22  g39(.a(new_n89_), .b(new_n74_), .c(new_n88_), .d(x22), .O(new_n90_));
  oai21  g40(.a(new_n90_), .b(new_n77_), .c(new_n87_), .O(z12));
  inv1   g41(.a(x12), .O(new_n92_));
  nand2  g42(.a(x18), .b(x04), .O(new_n93_));
  oai21  g43(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g44(.a(new_n94_), .b(new_n77_), .O(new_n95_));
  nand3  g45(.a(new_n89_), .b(new_n64_), .c(new_n72_), .O(new_n96_));
  nor3   g46(.a(x23), .b(x22), .c(x21), .O(new_n97_));
  aoi22  g47(.a(new_n97_), .b(new_n74_), .c(new_n96_), .d(x23), .O(new_n98_));
  oai21  g48(.a(new_n98_), .b(new_n77_), .c(new_n95_), .O(z13));
  nand2  g49(.a(x18), .b(x06), .O(new_n101_));
  oai21  g50(.a(x18), .b(new_n59_), .c(new_n101_), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n77_), .O(new_n103_));
  nor2   g52(.a(x24), .b(x23), .O(new_n104_));
  nand4  g53(.a(new_n104_), .b(new_n89_), .c(new_n64_), .d(new_n72_), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(x25), .O(new_n106_));
  nor2   g55(.a(x23), .b(x22), .O(new_n107_));
  nor2   g56(.a(x25), .b(x24), .O(new_n108_));
  nand4  g57(.a(new_n108_), .b(new_n107_), .c(new_n82_), .d(new_n64_), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(x16), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(new_n103_), .O(z15));
  inv1   g61(.a(x15), .O(new_n113_));
  nand2  g62(.a(x18), .b(x07), .O(new_n114_));
  oai21  g63(.a(x18), .b(new_n113_), .c(new_n114_), .O(new_n115_));
  nand2  g64(.a(new_n115_), .b(new_n77_), .O(new_n116_));
  nand2  g65(.a(new_n109_), .b(x26), .O(new_n117_));
  nor3   g66(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand4  g67(.a(new_n118_), .b(new_n107_), .c(new_n82_), .d(new_n64_), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g69(.a(new_n120_), .b(x16), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(new_n116_), .O(z16));
  inv1   g71(.a(x19), .O(new_n123_));
  nor2   g72(.a(new_n123_), .b(x17), .O(new_n124_));
  nand4  g73(.a(new_n124_), .b(new_n118_), .c(new_n107_), .d(new_n82_), .O(new_n125_));
  nand2  g74(.a(x27), .b(x17), .O(new_n126_));
  aoi21  g75(.a(new_n126_), .b(new_n125_), .c(new_n77_), .O(z17));
  zero   g76(.O(z01));
  zero   g77(.O(z04));
  zero   g78(.O(z05));
  zero   g79(.O(z07));
  zero   g80(.O(z14));
  buf    g81(.a(x27), .O(z08));
endmodule


