// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x11), .O(new_n53_));
  nand2  g05(.a(x27), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(x27), .b(x22), .c(new_n54_), .O(z03));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n47_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  inv1   g13(.a(x09), .O(new_n65_));
  nor2   g14(.a(x18), .b(new_n65_), .O(new_n66_));
  aoi21  g15(.a(x18), .b(x01), .c(new_n66_), .O(new_n67_));
  inv1   g16(.a(x20), .O(new_n68_));
  nor2   g17(.a(x19), .b(x17), .O(new_n69_));
  nor2   g18(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor3   g19(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g20(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  oai21  g21(.a(new_n67_), .b(x16), .c(new_n72_), .O(z10));
  inv1   g22(.a(x16), .O(new_n75_));
  nand2  g23(.a(x18), .b(x03), .O(new_n76_));
  oai21  g24(.a(x18), .b(new_n53_), .c(new_n76_), .O(new_n77_));
  nand2  g25(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nor2   g26(.a(x21), .b(x20), .O(new_n79_));
  nand2  g27(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  nor2   g28(.a(x22), .b(x21), .O(new_n81_));
  aoi22  g29(.a(new_n81_), .b(new_n71_), .c(new_n80_), .d(x22), .O(new_n82_));
  oai21  g30(.a(new_n82_), .b(new_n75_), .c(new_n78_), .O(z12));
  inv1   g31(.a(x13), .O(new_n85_));
  nand2  g32(.a(x18), .b(x05), .O(new_n86_));
  oai21  g33(.a(x18), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  nand2  g34(.a(new_n87_), .b(new_n75_), .O(new_n88_));
  inv1   g35(.a(x24), .O(new_n89_));
  nor3   g36(.a(x23), .b(x22), .c(x21), .O(new_n90_));
  aoi21  g37(.a(new_n90_), .b(new_n71_), .c(new_n89_), .O(new_n91_));
  nor2   g38(.a(x24), .b(x23), .O(new_n92_));
  nand4  g39(.a(new_n92_), .b(new_n81_), .c(new_n69_), .d(new_n68_), .O(new_n93_));
  inv1   g40(.a(new_n93_), .O(new_n94_));
  oai21  g41(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  nand2  g42(.a(new_n95_), .b(new_n88_), .O(z14));
  inv1   g43(.a(x14), .O(new_n97_));
  nand2  g44(.a(x18), .b(x06), .O(new_n98_));
  oai21  g45(.a(x18), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g46(.a(new_n99_), .b(new_n75_), .O(new_n100_));
  nand2  g47(.a(new_n93_), .b(x25), .O(new_n101_));
  nor2   g48(.a(x23), .b(x22), .O(new_n102_));
  nor2   g49(.a(x25), .b(x24), .O(new_n103_));
  nand4  g50(.a(new_n103_), .b(new_n102_), .c(new_n79_), .d(new_n69_), .O(new_n104_));
  nand2  g51(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand2  g52(.a(new_n105_), .b(x16), .O(new_n106_));
  nand2  g53(.a(new_n106_), .b(new_n100_), .O(z15));
  nand2  g54(.a(x18), .b(x07), .O(new_n108_));
  oai21  g55(.a(x18), .b(new_n61_), .c(new_n108_), .O(new_n109_));
  nand2  g56(.a(new_n109_), .b(new_n75_), .O(new_n110_));
  nand2  g57(.a(new_n104_), .b(x26), .O(new_n111_));
  nor3   g58(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand4  g59(.a(new_n112_), .b(new_n102_), .c(new_n79_), .d(new_n69_), .O(new_n113_));
  nand2  g60(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g61(.a(new_n114_), .b(x16), .O(new_n115_));
  nand2  g62(.a(new_n115_), .b(new_n110_), .O(z16));
  nor2   g63(.a(new_n48_), .b(x17), .O(new_n117_));
  nand4  g64(.a(new_n117_), .b(new_n112_), .c(new_n102_), .d(new_n79_), .O(new_n118_));
  nand2  g65(.a(x27), .b(x17), .O(new_n119_));
  aoi21  g66(.a(new_n119_), .b(new_n118_), .c(new_n75_), .O(z17));
  zero   g67(.O(z01));
  zero   g68(.O(z02));
  zero   g69(.O(z05));
  zero   g70(.O(z06));
  zero   g71(.O(z09));
  zero   g72(.O(z11));
  zero   g73(.O(z13));
  buf    g74(.a(x27), .O(z08));
endmodule


