// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n54_));
  inv1   g04(.a(x24), .O(new_n55_));
  nand2  g05(.a(new_n54_), .b(new_n55_), .O(new_n56_));
  oai21  g06(.a(new_n54_), .b(x13), .c(new_n56_), .O(z05));
  inv1   g07(.a(x25), .O(new_n58_));
  nand2  g08(.a(new_n54_), .b(new_n58_), .O(new_n59_));
  oai21  g09(.a(new_n54_), .b(x14), .c(new_n59_), .O(z06));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  and2   g13(.a(x19), .b(x17), .O(new_n64_));
  nor2   g14(.a(x19), .b(x17), .O(new_n65_));
  oai21  g15(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g16(.a(x18), .b(new_n47_), .O(new_n67_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n67_), .O(new_n68_));
  oai21  g18(.a(new_n68_), .b(x16), .c(new_n66_), .O(z09));
  inv1   g19(.a(x09), .O(new_n70_));
  nor2   g20(.a(x18), .b(new_n70_), .O(new_n71_));
  aoi21  g21(.a(x18), .b(x01), .c(new_n71_), .O(new_n72_));
  inv1   g22(.a(x20), .O(new_n73_));
  nor2   g23(.a(new_n65_), .b(new_n73_), .O(new_n74_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g25(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  oai21  g26(.a(new_n72_), .b(x16), .c(new_n76_), .O(z10));
  inv1   g27(.a(x16), .O(new_n78_));
  inv1   g28(.a(x10), .O(new_n79_));
  nand2  g29(.a(x18), .b(x02), .O(new_n80_));
  oai21  g30(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand2  g31(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g32(.a(new_n65_), .b(new_n73_), .O(new_n83_));
  nor2   g33(.a(x21), .b(x20), .O(new_n84_));
  aoi22  g34(.a(new_n84_), .b(new_n65_), .c(new_n83_), .d(x21), .O(new_n85_));
  oai21  g35(.a(new_n85_), .b(new_n78_), .c(new_n82_), .O(z11));
  inv1   g36(.a(x11), .O(new_n87_));
  nand2  g37(.a(x18), .b(x03), .O(new_n88_));
  oai21  g38(.a(x18), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g39(.a(new_n89_), .b(new_n78_), .O(new_n90_));
  nand2  g40(.a(new_n84_), .b(new_n65_), .O(new_n91_));
  nor2   g41(.a(x22), .b(x21), .O(new_n92_));
  aoi22  g42(.a(new_n92_), .b(new_n75_), .c(new_n91_), .d(x22), .O(new_n93_));
  oai21  g43(.a(new_n93_), .b(new_n78_), .c(new_n90_), .O(z12));
  inv1   g44(.a(x12), .O(new_n95_));
  nand2  g45(.a(x18), .b(x04), .O(new_n96_));
  oai21  g46(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g47(.a(new_n97_), .b(new_n78_), .O(new_n98_));
  nand3  g48(.a(new_n92_), .b(new_n65_), .c(new_n73_), .O(new_n99_));
  nor3   g49(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  aoi22  g50(.a(new_n100_), .b(new_n75_), .c(new_n99_), .d(x23), .O(new_n101_));
  oai21  g51(.a(new_n101_), .b(new_n78_), .c(new_n98_), .O(z13));
  inv1   g52(.a(x13), .O(new_n103_));
  nand2  g53(.a(x18), .b(x05), .O(new_n104_));
  oai21  g54(.a(x18), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g55(.a(new_n105_), .b(new_n78_), .O(new_n106_));
  aoi21  g56(.a(new_n100_), .b(new_n75_), .c(new_n55_), .O(new_n107_));
  nor2   g57(.a(x24), .b(x23), .O(new_n108_));
  nand2  g58(.a(new_n108_), .b(new_n92_), .O(new_n109_));
  nor2   g59(.a(new_n109_), .b(new_n83_), .O(new_n110_));
  oai21  g60(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  nand2  g61(.a(new_n111_), .b(new_n106_), .O(z14));
  nand2  g62(.a(x18), .b(x07), .O(new_n114_));
  oai21  g63(.a(x18), .b(new_n61_), .c(new_n114_), .O(new_n115_));
  nand2  g64(.a(new_n115_), .b(new_n78_), .O(new_n116_));
  nor2   g65(.a(x23), .b(x22), .O(new_n117_));
  nor2   g66(.a(x25), .b(x24), .O(new_n118_));
  nand4  g67(.a(new_n118_), .b(new_n117_), .c(new_n84_), .d(new_n65_), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(x26), .O(new_n120_));
  nor3   g69(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand4  g70(.a(new_n121_), .b(new_n117_), .c(new_n84_), .d(new_n65_), .O(new_n122_));
  nand2  g71(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g72(.a(new_n123_), .b(x16), .O(new_n124_));
  nand2  g73(.a(new_n124_), .b(new_n116_), .O(z16));
  zero   g74(.O(z01));
  zero   g75(.O(z02));
  zero   g76(.O(z03));
  zero   g77(.O(z04));
  zero   g78(.O(z15));
  zero   g79(.O(z17));
  buf    g80(.a(x27), .O(z08));
endmodule


