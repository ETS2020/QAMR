// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x12), .O(new_n54_));
  nand2  g05(.a(x27), .b(new_n54_), .O(new_n55_));
  oai21  g06(.a(x27), .b(x23), .c(new_n55_), .O(z04));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n47_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x14), .O(new_n60_));
  nand2  g11(.a(x27), .b(new_n60_), .O(new_n61_));
  oai21  g12(.a(x27), .b(x25), .c(new_n61_), .O(z06));
  inv1   g13(.a(x15), .O(new_n63_));
  nand2  g14(.a(x27), .b(new_n63_), .O(new_n64_));
  oai21  g15(.a(x27), .b(x26), .c(new_n64_), .O(z07));
  inv1   g16(.a(x16), .O(new_n68_));
  inv1   g17(.a(x10), .O(new_n69_));
  nand2  g18(.a(x18), .b(x02), .O(new_n70_));
  oai21  g19(.a(x18), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  nand2  g20(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  nor2   g21(.a(x19), .b(x17), .O(new_n73_));
  inv1   g22(.a(x20), .O(new_n74_));
  nand2  g23(.a(new_n73_), .b(new_n74_), .O(new_n75_));
  nor2   g24(.a(x21), .b(x20), .O(new_n76_));
  aoi22  g25(.a(new_n76_), .b(new_n73_), .c(new_n75_), .d(x21), .O(new_n77_));
  oai21  g26(.a(new_n77_), .b(new_n68_), .c(new_n72_), .O(z11));
  inv1   g27(.a(x11), .O(new_n79_));
  nand2  g28(.a(x18), .b(x03), .O(new_n80_));
  oai21  g29(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand2  g30(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  nor3   g31(.a(x20), .b(x19), .c(x17), .O(new_n83_));
  nand2  g32(.a(new_n76_), .b(new_n73_), .O(new_n84_));
  nor2   g33(.a(x22), .b(x21), .O(new_n85_));
  aoi22  g34(.a(new_n85_), .b(new_n83_), .c(new_n84_), .d(x22), .O(new_n86_));
  oai21  g35(.a(new_n86_), .b(new_n68_), .c(new_n82_), .O(z12));
  nand2  g36(.a(x18), .b(x04), .O(new_n88_));
  oai21  g37(.a(x18), .b(new_n54_), .c(new_n88_), .O(new_n89_));
  nand2  g38(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  nand3  g39(.a(new_n85_), .b(new_n73_), .c(new_n74_), .O(new_n91_));
  nor3   g40(.a(x23), .b(x22), .c(x21), .O(new_n92_));
  aoi22  g41(.a(new_n92_), .b(new_n83_), .c(new_n91_), .d(x23), .O(new_n93_));
  oai21  g42(.a(new_n93_), .b(new_n68_), .c(new_n90_), .O(z13));
  inv1   g43(.a(x13), .O(new_n95_));
  nand2  g44(.a(x18), .b(x05), .O(new_n96_));
  oai21  g45(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g46(.a(new_n97_), .b(new_n68_), .O(new_n98_));
  aoi21  g47(.a(new_n92_), .b(new_n83_), .c(new_n57_), .O(new_n99_));
  nor2   g48(.a(x24), .b(x23), .O(new_n100_));
  nand4  g49(.a(new_n100_), .b(new_n85_), .c(new_n73_), .d(new_n74_), .O(new_n101_));
  inv1   g50(.a(new_n101_), .O(new_n102_));
  oai21  g51(.a(new_n102_), .b(new_n99_), .c(x16), .O(new_n103_));
  nand2  g52(.a(new_n103_), .b(new_n98_), .O(z14));
  nand2  g53(.a(x18), .b(x06), .O(new_n105_));
  oai21  g54(.a(x18), .b(new_n60_), .c(new_n105_), .O(new_n106_));
  nand2  g55(.a(new_n106_), .b(new_n68_), .O(new_n107_));
  nand2  g56(.a(new_n101_), .b(x25), .O(new_n108_));
  nor2   g57(.a(x23), .b(x22), .O(new_n109_));
  nor2   g58(.a(x25), .b(x24), .O(new_n110_));
  nand4  g59(.a(new_n110_), .b(new_n109_), .c(new_n76_), .d(new_n73_), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  nand2  g61(.a(new_n112_), .b(x16), .O(new_n113_));
  nand2  g62(.a(new_n113_), .b(new_n107_), .O(z15));
  nand2  g63(.a(x18), .b(x07), .O(new_n115_));
  oai21  g64(.a(x18), .b(new_n63_), .c(new_n115_), .O(new_n116_));
  nand2  g65(.a(new_n116_), .b(new_n68_), .O(new_n117_));
  nand2  g66(.a(new_n111_), .b(x26), .O(new_n118_));
  nor3   g67(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand4  g68(.a(new_n119_), .b(new_n109_), .c(new_n76_), .d(new_n73_), .O(new_n120_));
  nand2  g69(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(x16), .O(new_n122_));
  nand2  g71(.a(new_n122_), .b(new_n117_), .O(z16));
  nor2   g72(.a(new_n48_), .b(x17), .O(new_n124_));
  nand4  g73(.a(new_n124_), .b(new_n119_), .c(new_n109_), .d(new_n76_), .O(new_n125_));
  nand2  g74(.a(x27), .b(x17), .O(new_n126_));
  aoi21  g75(.a(new_n126_), .b(new_n125_), .c(new_n68_), .O(z17));
  zero   g76(.O(z01));
  zero   g77(.O(z02));
  zero   g78(.O(z03));
  zero   g79(.O(z09));
  zero   g80(.O(z10));
  buf    g81(.a(x27), .O(z08));
endmodule


