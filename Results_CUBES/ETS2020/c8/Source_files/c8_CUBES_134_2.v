// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x11), .O(new_n53_));
  nand2  g05(.a(x27), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(x27), .b(x22), .c(new_n54_), .O(z03));
  inv1   g07(.a(x12), .O(new_n56_));
  nand2  g08(.a(x27), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(x27), .b(x23), .c(new_n57_), .O(z04));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n48_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x15), .O(new_n63_));
  nand2  g14(.a(x27), .b(new_n63_), .O(new_n64_));
  oai21  g15(.a(x27), .b(x26), .c(new_n64_), .O(z07));
  and2   g16(.a(x19), .b(x17), .O(new_n66_));
  nor2   g17(.a(x19), .b(x17), .O(new_n67_));
  oai21  g18(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  inv1   g19(.a(x08), .O(new_n69_));
  nor2   g20(.a(x18), .b(new_n69_), .O(new_n70_));
  aoi21  g21(.a(x18), .b(x00), .c(new_n70_), .O(new_n71_));
  oai21  g22(.a(new_n71_), .b(x16), .c(new_n68_), .O(z09));
  inv1   g23(.a(x16), .O(new_n74_));
  inv1   g24(.a(x10), .O(new_n75_));
  nand2  g25(.a(x18), .b(x02), .O(new_n76_));
  oai21  g26(.a(x18), .b(new_n75_), .c(new_n76_), .O(new_n77_));
  nand2  g27(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g28(.a(new_n67_), .b(new_n49_), .O(new_n79_));
  nor2   g29(.a(x21), .b(x20), .O(new_n80_));
  aoi22  g30(.a(new_n80_), .b(new_n67_), .c(new_n79_), .d(x21), .O(new_n81_));
  oai21  g31(.a(new_n81_), .b(new_n74_), .c(new_n78_), .O(z11));
  nand2  g32(.a(x18), .b(x03), .O(new_n83_));
  oai21  g33(.a(x18), .b(new_n53_), .c(new_n83_), .O(new_n84_));
  nand2  g34(.a(new_n84_), .b(new_n74_), .O(new_n85_));
  nor3   g35(.a(x20), .b(x19), .c(x17), .O(new_n86_));
  nand2  g36(.a(new_n80_), .b(new_n67_), .O(new_n87_));
  nor2   g37(.a(x22), .b(x21), .O(new_n88_));
  aoi22  g38(.a(new_n88_), .b(new_n86_), .c(new_n87_), .d(x22), .O(new_n89_));
  oai21  g39(.a(new_n89_), .b(new_n74_), .c(new_n85_), .O(z12));
  nand2  g40(.a(x18), .b(x04), .O(new_n91_));
  oai21  g41(.a(x18), .b(new_n56_), .c(new_n91_), .O(new_n92_));
  nand2  g42(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  nand3  g43(.a(new_n88_), .b(new_n67_), .c(new_n49_), .O(new_n94_));
  nor3   g44(.a(x23), .b(x22), .c(x21), .O(new_n95_));
  aoi22  g45(.a(new_n95_), .b(new_n86_), .c(new_n94_), .d(x23), .O(new_n96_));
  oai21  g46(.a(new_n96_), .b(new_n74_), .c(new_n93_), .O(z13));
  inv1   g47(.a(x14), .O(new_n99_));
  nand2  g48(.a(x18), .b(x06), .O(new_n100_));
  oai21  g49(.a(x18), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand2  g50(.a(new_n101_), .b(new_n74_), .O(new_n102_));
  nor2   g51(.a(x24), .b(x23), .O(new_n103_));
  nand4  g52(.a(new_n103_), .b(new_n88_), .c(new_n67_), .d(new_n49_), .O(new_n104_));
  nand2  g53(.a(new_n104_), .b(x25), .O(new_n105_));
  nor2   g54(.a(x23), .b(x22), .O(new_n106_));
  nor2   g55(.a(x25), .b(x24), .O(new_n107_));
  nand4  g56(.a(new_n107_), .b(new_n106_), .c(new_n80_), .d(new_n67_), .O(new_n108_));
  nand2  g57(.a(new_n108_), .b(new_n105_), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(x16), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(new_n102_), .O(z15));
  nand2  g60(.a(x18), .b(x07), .O(new_n112_));
  oai21  g61(.a(x18), .b(new_n63_), .c(new_n112_), .O(new_n113_));
  nand2  g62(.a(new_n113_), .b(new_n74_), .O(new_n114_));
  nand2  g63(.a(new_n108_), .b(x26), .O(new_n115_));
  nor3   g64(.a(x26), .b(x25), .c(x24), .O(new_n116_));
  nand4  g65(.a(new_n116_), .b(new_n106_), .c(new_n80_), .d(new_n67_), .O(new_n117_));
  nand2  g66(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(x16), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n114_), .O(z16));
  inv1   g69(.a(x19), .O(new_n121_));
  nor2   g70(.a(new_n121_), .b(x17), .O(new_n122_));
  nand4  g71(.a(new_n122_), .b(new_n116_), .c(new_n106_), .d(new_n80_), .O(new_n123_));
  nand2  g72(.a(x27), .b(x17), .O(new_n124_));
  aoi21  g73(.a(new_n124_), .b(new_n123_), .c(new_n74_), .O(z17));
  zero   g74(.O(z00));
  zero   g75(.O(z02));
  zero   g76(.O(z06));
  zero   g77(.O(z10));
  zero   g78(.O(z14));
  buf    g79(.a(x27), .O(z08));
endmodule


