// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x10), .O(new_n52_));
  nand2  g05(.a(x27), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(x27), .b(x21), .c(new_n53_), .O(z02));
  inv1   g07(.a(x22), .O(new_n55_));
  nand2  g08(.a(new_n47_), .b(new_n55_), .O(new_n56_));
  oai21  g09(.a(new_n47_), .b(x11), .c(new_n56_), .O(z03));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n47_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n47_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x14), .O(new_n62_));
  nand2  g14(.a(x27), .b(new_n62_), .O(new_n63_));
  oai21  g15(.a(x27), .b(x25), .c(new_n63_), .O(z06));
  and2   g16(.a(x19), .b(x17), .O(new_n66_));
  nor2   g17(.a(x19), .b(x17), .O(new_n67_));
  oai21  g18(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  inv1   g19(.a(x08), .O(new_n69_));
  nor2   g20(.a(x18), .b(new_n69_), .O(new_n70_));
  aoi21  g21(.a(x18), .b(x00), .c(new_n70_), .O(new_n71_));
  oai21  g22(.a(new_n71_), .b(x16), .c(new_n68_), .O(z09));
  inv1   g23(.a(x16), .O(new_n74_));
  nand2  g24(.a(x18), .b(x02), .O(new_n75_));
  oai21  g25(.a(x18), .b(new_n52_), .c(new_n75_), .O(new_n76_));
  nand2  g26(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  inv1   g27(.a(x20), .O(new_n78_));
  nand2  g28(.a(new_n67_), .b(new_n78_), .O(new_n79_));
  nor2   g29(.a(x21), .b(x20), .O(new_n80_));
  aoi22  g30(.a(new_n80_), .b(new_n67_), .c(new_n79_), .d(x21), .O(new_n81_));
  oai21  g31(.a(new_n81_), .b(new_n74_), .c(new_n77_), .O(z11));
  inv1   g32(.a(x12), .O(new_n84_));
  nand2  g33(.a(x18), .b(x04), .O(new_n85_));
  oai21  g34(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g35(.a(new_n86_), .b(new_n74_), .O(new_n87_));
  nor3   g36(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  nor2   g37(.a(x22), .b(x21), .O(new_n89_));
  nand3  g38(.a(new_n89_), .b(new_n67_), .c(new_n78_), .O(new_n90_));
  nor3   g39(.a(x23), .b(x22), .c(x21), .O(new_n91_));
  aoi22  g40(.a(new_n91_), .b(new_n88_), .c(new_n90_), .d(x23), .O(new_n92_));
  oai21  g41(.a(new_n92_), .b(new_n74_), .c(new_n87_), .O(z13));
  inv1   g42(.a(x13), .O(new_n94_));
  nand2  g43(.a(x18), .b(x05), .O(new_n95_));
  oai21  g44(.a(x18), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g45(.a(new_n96_), .b(new_n74_), .O(new_n97_));
  aoi21  g46(.a(new_n91_), .b(new_n88_), .c(new_n59_), .O(new_n98_));
  nor2   g47(.a(x24), .b(x23), .O(new_n99_));
  nand4  g48(.a(new_n99_), .b(new_n89_), .c(new_n67_), .d(new_n78_), .O(new_n100_));
  inv1   g49(.a(new_n100_), .O(new_n101_));
  oai21  g50(.a(new_n101_), .b(new_n98_), .c(x16), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n97_), .O(z14));
  nand2  g52(.a(x18), .b(x06), .O(new_n104_));
  oai21  g53(.a(x18), .b(new_n62_), .c(new_n104_), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(new_n74_), .O(new_n106_));
  nand2  g55(.a(new_n100_), .b(x25), .O(new_n107_));
  nor2   g56(.a(x23), .b(x22), .O(new_n108_));
  nor2   g57(.a(x25), .b(x24), .O(new_n109_));
  nand4  g58(.a(new_n109_), .b(new_n108_), .c(new_n80_), .d(new_n67_), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(x16), .O(new_n112_));
  nand2  g61(.a(new_n112_), .b(new_n106_), .O(z15));
  inv1   g62(.a(x15), .O(new_n114_));
  nand2  g63(.a(x18), .b(x07), .O(new_n115_));
  oai21  g64(.a(x18), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  nand2  g65(.a(new_n116_), .b(new_n74_), .O(new_n117_));
  nand2  g66(.a(new_n110_), .b(x26), .O(new_n118_));
  nor3   g67(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand4  g68(.a(new_n119_), .b(new_n108_), .c(new_n80_), .d(new_n67_), .O(new_n120_));
  nand2  g69(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(x16), .O(new_n122_));
  nand2  g71(.a(new_n122_), .b(new_n117_), .O(z16));
  nor2   g72(.a(new_n48_), .b(x17), .O(new_n124_));
  nand4  g73(.a(new_n124_), .b(new_n119_), .c(new_n108_), .d(new_n80_), .O(new_n125_));
  nand2  g74(.a(x27), .b(x17), .O(new_n126_));
  aoi21  g75(.a(new_n126_), .b(new_n125_), .c(new_n74_), .O(z17));
  zero   g76(.O(z01));
  zero   g77(.O(z04));
  zero   g78(.O(z07));
  zero   g79(.O(z10));
  zero   g80(.O(z12));
  buf    g81(.a(x27), .O(z08));
endmodule


