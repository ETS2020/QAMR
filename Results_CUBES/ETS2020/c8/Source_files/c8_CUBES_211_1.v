// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x22), .O(new_n53_));
  nand2  g05(.a(new_n48_), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(new_n48_), .b(x11), .c(new_n54_), .O(z03));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n48_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n48_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x25), .O(new_n62_));
  nand2  g14(.a(new_n48_), .b(new_n62_), .O(new_n63_));
  oai21  g15(.a(new_n48_), .b(x14), .c(new_n63_), .O(z06));
  inv1   g16(.a(x26), .O(new_n65_));
  nand2  g17(.a(new_n48_), .b(new_n65_), .O(new_n66_));
  oai21  g18(.a(new_n48_), .b(x15), .c(new_n66_), .O(z07));
  inv1   g19(.a(x16), .O(new_n74_));
  inv1   g20(.a(x13), .O(new_n75_));
  nand2  g21(.a(x18), .b(x05), .O(new_n76_));
  oai21  g22(.a(x18), .b(new_n75_), .c(new_n76_), .O(new_n77_));
  nand2  g23(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n79_));
  nor3   g25(.a(x23), .b(x22), .c(x21), .O(new_n80_));
  aoi21  g26(.a(new_n80_), .b(new_n79_), .c(new_n59_), .O(new_n81_));
  nor2   g27(.a(x19), .b(x17), .O(new_n82_));
  nor2   g28(.a(x22), .b(x21), .O(new_n83_));
  nor2   g29(.a(x24), .b(x23), .O(new_n84_));
  nand4  g30(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n49_), .O(new_n85_));
  inv1   g31(.a(new_n85_), .O(new_n86_));
  oai21  g32(.a(new_n86_), .b(new_n81_), .c(x16), .O(new_n87_));
  nand2  g33(.a(new_n87_), .b(new_n78_), .O(z14));
  inv1   g34(.a(x14), .O(new_n89_));
  nand2  g35(.a(x18), .b(x06), .O(new_n90_));
  oai21  g36(.a(x18), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand2  g37(.a(new_n91_), .b(new_n74_), .O(new_n92_));
  inv1   g38(.a(x17), .O(new_n93_));
  inv1   g39(.a(x19), .O(new_n94_));
  nand4  g40(.a(new_n62_), .b(new_n59_), .c(new_n94_), .d(new_n93_), .O(new_n95_));
  inv1   g41(.a(new_n95_), .O(new_n96_));
  inv1   g42(.a(x21), .O(new_n97_));
  nand4  g43(.a(new_n56_), .b(new_n53_), .c(new_n97_), .d(new_n49_), .O(new_n98_));
  inv1   g44(.a(new_n98_), .O(new_n99_));
  aoi22  g45(.a(new_n99_), .b(new_n96_), .c(new_n85_), .d(x25), .O(new_n100_));
  oai21  g46(.a(new_n100_), .b(new_n74_), .c(new_n92_), .O(z15));
  nor2   g47(.a(x23), .b(x22), .O(new_n103_));
  nor2   g48(.a(x21), .b(x20), .O(new_n104_));
  nor3   g49(.a(x26), .b(x25), .c(x24), .O(new_n105_));
  nor2   g50(.a(new_n94_), .b(x17), .O(new_n106_));
  nand4  g51(.a(new_n106_), .b(new_n105_), .c(new_n104_), .d(new_n103_), .O(new_n107_));
  nand2  g52(.a(x27), .b(x17), .O(new_n108_));
  aoi21  g53(.a(new_n108_), .b(new_n107_), .c(new_n74_), .O(z17));
  zero   g54(.O(z00));
  zero   g55(.O(z02));
  zero   g56(.O(z08));
  zero   g57(.O(z09));
  zero   g58(.O(z10));
  zero   g59(.O(z11));
  zero   g60(.O(z12));
  zero   g61(.O(z13));
  zero   g62(.O(z16));
endmodule


