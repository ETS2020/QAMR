// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n56_, new_n57_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand2  g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x20), .O(new_n51_));
  nand2  g05(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g06(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g07(.a(x23), .O(new_n56_));
  nand2  g08(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(new_n47_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  inv1   g13(.a(x16), .O(new_n66_));
  inv1   g14(.a(x10), .O(new_n67_));
  nand2  g15(.a(x18), .b(x02), .O(new_n68_));
  oai21  g16(.a(x18), .b(new_n67_), .c(new_n68_), .O(new_n69_));
  nand2  g17(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nor2   g18(.a(x19), .b(x17), .O(new_n71_));
  nand2  g19(.a(new_n71_), .b(new_n51_), .O(new_n72_));
  inv1   g20(.a(x17), .O(new_n73_));
  inv1   g21(.a(x21), .O(new_n74_));
  nand4  g22(.a(new_n74_), .b(new_n51_), .c(new_n48_), .d(new_n73_), .O(new_n75_));
  inv1   g23(.a(new_n75_), .O(new_n76_));
  aoi21  g24(.a(new_n72_), .b(x21), .c(new_n76_), .O(new_n77_));
  oai21  g25(.a(new_n77_), .b(new_n66_), .c(new_n70_), .O(z11));
  inv1   g26(.a(x11), .O(new_n79_));
  nand2  g27(.a(x18), .b(x03), .O(new_n80_));
  oai21  g28(.a(x18), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  nand2  g29(.a(new_n81_), .b(new_n66_), .O(new_n82_));
  nor3   g30(.a(x20), .b(x19), .c(x17), .O(new_n83_));
  nor2   g31(.a(x22), .b(x21), .O(new_n84_));
  aoi22  g32(.a(new_n84_), .b(new_n83_), .c(new_n75_), .d(x22), .O(new_n85_));
  oai21  g33(.a(new_n85_), .b(new_n66_), .c(new_n82_), .O(z12));
  inv1   g34(.a(x12), .O(new_n87_));
  nand2  g35(.a(x18), .b(x04), .O(new_n88_));
  oai21  g36(.a(x18), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g37(.a(new_n89_), .b(new_n66_), .O(new_n90_));
  nand3  g38(.a(new_n84_), .b(new_n71_), .c(new_n51_), .O(new_n91_));
  nor3   g39(.a(x23), .b(x22), .c(x21), .O(new_n92_));
  aoi22  g40(.a(new_n92_), .b(new_n83_), .c(new_n91_), .d(x23), .O(new_n93_));
  oai21  g41(.a(new_n93_), .b(new_n66_), .c(new_n90_), .O(z13));
  inv1   g42(.a(x13), .O(new_n95_));
  nand2  g43(.a(x18), .b(x05), .O(new_n96_));
  oai21  g44(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g45(.a(new_n97_), .b(new_n66_), .O(new_n98_));
  inv1   g46(.a(x24), .O(new_n99_));
  aoi21  g47(.a(new_n92_), .b(new_n83_), .c(new_n99_), .O(new_n100_));
  nor2   g48(.a(x24), .b(x23), .O(new_n101_));
  nand4  g49(.a(new_n101_), .b(new_n84_), .c(new_n71_), .d(new_n51_), .O(new_n102_));
  inv1   g50(.a(new_n102_), .O(new_n103_));
  oai21  g51(.a(new_n103_), .b(new_n100_), .c(x16), .O(new_n104_));
  nand2  g52(.a(new_n104_), .b(new_n98_), .O(z14));
  inv1   g53(.a(x14), .O(new_n106_));
  nand2  g54(.a(x18), .b(x06), .O(new_n107_));
  oai21  g55(.a(x18), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand2  g56(.a(new_n108_), .b(new_n66_), .O(new_n109_));
  inv1   g57(.a(x22), .O(new_n110_));
  inv1   g58(.a(x25), .O(new_n111_));
  nand4  g59(.a(new_n111_), .b(new_n99_), .c(new_n56_), .d(new_n110_), .O(new_n112_));
  inv1   g60(.a(new_n112_), .O(new_n113_));
  aoi22  g61(.a(new_n113_), .b(new_n76_), .c(new_n102_), .d(x25), .O(new_n114_));
  oai21  g62(.a(new_n114_), .b(new_n66_), .c(new_n109_), .O(z15));
  nand2  g63(.a(x18), .b(x07), .O(new_n116_));
  oai21  g64(.a(x18), .b(new_n61_), .c(new_n116_), .O(new_n117_));
  nand2  g65(.a(new_n117_), .b(new_n66_), .O(new_n118_));
  nor2   g66(.a(x21), .b(x20), .O(new_n119_));
  nor2   g67(.a(x23), .b(x22), .O(new_n120_));
  nor2   g68(.a(x25), .b(x24), .O(new_n121_));
  nand4  g69(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(new_n71_), .O(new_n122_));
  nand2  g70(.a(new_n122_), .b(x26), .O(new_n123_));
  nor3   g71(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand4  g72(.a(new_n124_), .b(new_n120_), .c(new_n119_), .d(new_n71_), .O(new_n125_));
  nand2  g73(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g74(.a(new_n126_), .b(x16), .O(new_n127_));
  nand2  g75(.a(new_n127_), .b(new_n118_), .O(z16));
  zero   g76(.O(z02));
  zero   g77(.O(z03));
  zero   g78(.O(z05));
  zero   g79(.O(z06));
  zero   g80(.O(z09));
  zero   g81(.O(z10));
  zero   g82(.O(z17));
  buf    g83(.a(x27), .O(z08));
endmodule


