// Benchmark "FAU" written by ABC on Mon Jul  6 18:50:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_;
  inv1   g00(.a(x08), .O(new_n47_));
  nand2  g01(.a(x27), .b(new_n47_), .O(new_n48_));
  oai21  g02(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g03(.a(x27), .O(new_n52_));
  inv1   g04(.a(x22), .O(new_n53_));
  nand2  g05(.a(new_n52_), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(new_n52_), .b(x11), .c(new_n54_), .O(z03));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n52_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  and2   g13(.a(x19), .b(x17), .O(new_n64_));
  nor2   g14(.a(x19), .b(x17), .O(new_n65_));
  oai21  g15(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g16(.a(x18), .b(new_n47_), .O(new_n67_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n67_), .O(new_n68_));
  oai21  g18(.a(new_n68_), .b(x16), .c(new_n66_), .O(z09));
  inv1   g19(.a(x09), .O(new_n70_));
  nor2   g20(.a(x18), .b(new_n70_), .O(new_n71_));
  aoi21  g21(.a(x18), .b(x01), .c(new_n71_), .O(new_n72_));
  inv1   g22(.a(x20), .O(new_n73_));
  nor2   g23(.a(new_n65_), .b(new_n73_), .O(new_n74_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g25(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  oai21  g26(.a(new_n72_), .b(x16), .c(new_n76_), .O(z10));
  inv1   g27(.a(x16), .O(new_n80_));
  inv1   g28(.a(x12), .O(new_n81_));
  nand2  g29(.a(x18), .b(x04), .O(new_n82_));
  oai21  g30(.a(x18), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  nand2  g31(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nor2   g32(.a(x21), .b(x20), .O(new_n85_));
  nand3  g33(.a(new_n85_), .b(new_n65_), .c(new_n53_), .O(new_n86_));
  nor3   g34(.a(x23), .b(x22), .c(x21), .O(new_n87_));
  aoi22  g35(.a(new_n87_), .b(new_n75_), .c(new_n86_), .d(x23), .O(new_n88_));
  oai21  g36(.a(new_n88_), .b(new_n80_), .c(new_n84_), .O(z13));
  nand2  g37(.a(x18), .b(x07), .O(new_n92_));
  oai21  g38(.a(x18), .b(new_n61_), .c(new_n92_), .O(new_n93_));
  nand2  g39(.a(new_n93_), .b(new_n80_), .O(new_n94_));
  nor2   g40(.a(x23), .b(x22), .O(new_n95_));
  nor2   g41(.a(x25), .b(x24), .O(new_n96_));
  nand4  g42(.a(new_n96_), .b(new_n95_), .c(new_n85_), .d(new_n65_), .O(new_n97_));
  nand2  g43(.a(new_n97_), .b(x26), .O(new_n98_));
  nor3   g44(.a(x26), .b(x25), .c(x24), .O(new_n99_));
  nand4  g45(.a(new_n99_), .b(new_n95_), .c(new_n85_), .d(new_n65_), .O(new_n100_));
  nand2  g46(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g47(.a(new_n101_), .b(x16), .O(new_n102_));
  nand2  g48(.a(new_n102_), .b(new_n94_), .O(z16));
  zero   g49(.O(z01));
  zero   g50(.O(z02));
  zero   g51(.O(z04));
  zero   g52(.O(z06));
  zero   g53(.O(z11));
  zero   g54(.O(z12));
  zero   g55(.O(z14));
  zero   g56(.O(z15));
  zero   g57(.O(z17));
  buf    g58(.a(x27), .O(z08));
endmodule


