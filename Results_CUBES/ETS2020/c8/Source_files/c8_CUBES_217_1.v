// Benchmark "FAU" written by ABC on Mon Jul  6 18:51:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x22), .O(new_n53_));
  nand2  g05(.a(new_n48_), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(new_n48_), .b(x11), .c(new_n54_), .O(z03));
  inv1   g07(.a(x24), .O(new_n57_));
  nand2  g08(.a(new_n48_), .b(new_n57_), .O(new_n58_));
  oai21  g09(.a(new_n48_), .b(x13), .c(new_n58_), .O(z05));
  inv1   g10(.a(x15), .O(new_n61_));
  nand2  g11(.a(x27), .b(new_n61_), .O(new_n62_));
  oai21  g12(.a(x27), .b(x26), .c(new_n62_), .O(z07));
  and2   g13(.a(x19), .b(x17), .O(new_n65_));
  nor2   g14(.a(x19), .b(x17), .O(new_n66_));
  oai21  g15(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  inv1   g16(.a(x08), .O(new_n68_));
  nor2   g17(.a(x18), .b(new_n68_), .O(new_n69_));
  aoi21  g18(.a(x18), .b(x00), .c(new_n69_), .O(new_n70_));
  oai21  g19(.a(new_n70_), .b(x16), .c(new_n67_), .O(z09));
  inv1   g20(.a(x09), .O(new_n72_));
  nor2   g21(.a(x18), .b(new_n72_), .O(new_n73_));
  aoi21  g22(.a(x18), .b(x01), .c(new_n73_), .O(new_n74_));
  nor2   g23(.a(new_n66_), .b(new_n49_), .O(new_n75_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n76_));
  oai21  g25(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  oai21  g26(.a(new_n74_), .b(x16), .c(new_n77_), .O(z10));
  inv1   g27(.a(x16), .O(new_n79_));
  inv1   g28(.a(x10), .O(new_n80_));
  nand2  g29(.a(x18), .b(x02), .O(new_n81_));
  oai21  g30(.a(x18), .b(new_n80_), .c(new_n81_), .O(new_n82_));
  nand2  g31(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g32(.a(new_n66_), .b(new_n49_), .O(new_n84_));
  nor2   g33(.a(x21), .b(x20), .O(new_n85_));
  aoi22  g34(.a(new_n85_), .b(new_n66_), .c(new_n84_), .d(x21), .O(new_n86_));
  oai21  g35(.a(new_n86_), .b(new_n79_), .c(new_n83_), .O(z11));
  inv1   g36(.a(x13), .O(new_n90_));
  nand2  g37(.a(x18), .b(x05), .O(new_n91_));
  oai21  g38(.a(x18), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand2  g39(.a(new_n92_), .b(new_n79_), .O(new_n93_));
  nor3   g40(.a(x23), .b(x22), .c(x21), .O(new_n94_));
  aoi21  g41(.a(new_n94_), .b(new_n76_), .c(new_n57_), .O(new_n95_));
  inv1   g42(.a(x21), .O(new_n96_));
  inv1   g43(.a(x23), .O(new_n97_));
  nand4  g44(.a(new_n57_), .b(new_n97_), .c(new_n53_), .d(new_n96_), .O(new_n98_));
  nor2   g45(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  oai21  g46(.a(new_n99_), .b(new_n95_), .c(x16), .O(new_n100_));
  nand2  g47(.a(new_n100_), .b(new_n93_), .O(z14));
  nand2  g48(.a(x18), .b(x07), .O(new_n103_));
  oai21  g49(.a(x18), .b(new_n61_), .c(new_n103_), .O(new_n104_));
  nand2  g50(.a(new_n104_), .b(new_n79_), .O(new_n105_));
  nor2   g51(.a(x23), .b(x22), .O(new_n106_));
  nor2   g52(.a(x25), .b(x24), .O(new_n107_));
  nand4  g53(.a(new_n107_), .b(new_n106_), .c(new_n85_), .d(new_n66_), .O(new_n108_));
  nand2  g54(.a(new_n108_), .b(x26), .O(new_n109_));
  nor3   g55(.a(x26), .b(x25), .c(x24), .O(new_n110_));
  nand4  g56(.a(new_n110_), .b(new_n106_), .c(new_n85_), .d(new_n66_), .O(new_n111_));
  nand2  g57(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g58(.a(new_n112_), .b(x16), .O(new_n113_));
  nand2  g59(.a(new_n113_), .b(new_n105_), .O(z16));
  inv1   g60(.a(x19), .O(new_n115_));
  nor2   g61(.a(new_n115_), .b(x17), .O(new_n116_));
  nand4  g62(.a(new_n116_), .b(new_n110_), .c(new_n106_), .d(new_n85_), .O(new_n117_));
  nand2  g63(.a(x27), .b(x17), .O(new_n118_));
  aoi21  g64(.a(new_n118_), .b(new_n117_), .c(new_n79_), .O(z17));
  zero   g65(.O(z00));
  zero   g66(.O(z02));
  zero   g67(.O(z04));
  zero   g68(.O(z06));
  zero   g69(.O(z08));
  zero   g70(.O(z12));
  zero   g71(.O(z13));
  zero   g72(.O(z15));
endmodule


