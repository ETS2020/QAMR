// Benchmark "FAU" written by ABC on Mon Jul  6 18:49:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x21), .O(new_n52_));
  nand2  g05(.a(new_n48_), .b(new_n52_), .O(new_n53_));
  oai21  g06(.a(new_n48_), .b(x10), .c(new_n53_), .O(z02));
  inv1   g07(.a(x22), .O(new_n55_));
  nand2  g08(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  oai21  g09(.a(new_n48_), .b(x11), .c(new_n56_), .O(z03));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n48_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x25), .O(new_n62_));
  nand2  g14(.a(new_n48_), .b(new_n62_), .O(new_n63_));
  oai21  g15(.a(new_n48_), .b(x14), .c(new_n63_), .O(z06));
  inv1   g16(.a(x26), .O(new_n65_));
  nand2  g17(.a(new_n48_), .b(new_n65_), .O(new_n66_));
  oai21  g18(.a(new_n48_), .b(x15), .c(new_n66_), .O(z07));
  inv1   g19(.a(x17), .O(new_n69_));
  inv1   g20(.a(x19), .O(new_n70_));
  nor2   g21(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g22(.a(x19), .b(x17), .O(new_n72_));
  oai21  g23(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g24(.a(x08), .O(new_n74_));
  nor2   g25(.a(x18), .b(new_n74_), .O(new_n75_));
  aoi21  g26(.a(x18), .b(x00), .c(new_n75_), .O(new_n76_));
  oai21  g27(.a(new_n76_), .b(x16), .c(new_n73_), .O(z09));
  inv1   g28(.a(x09), .O(new_n78_));
  nor2   g29(.a(x18), .b(new_n78_), .O(new_n79_));
  aoi21  g30(.a(x18), .b(x01), .c(new_n79_), .O(new_n80_));
  nor2   g31(.a(new_n72_), .b(new_n49_), .O(new_n81_));
  nor3   g32(.a(x20), .b(x19), .c(x17), .O(new_n82_));
  oai21  g33(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  oai21  g34(.a(new_n80_), .b(x16), .c(new_n83_), .O(z10));
  inv1   g35(.a(x16), .O(new_n86_));
  inv1   g36(.a(x11), .O(new_n87_));
  nand2  g37(.a(x18), .b(x03), .O(new_n88_));
  oai21  g38(.a(x18), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g39(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand4  g40(.a(new_n52_), .b(new_n49_), .c(new_n70_), .d(new_n69_), .O(new_n91_));
  nor2   g41(.a(x22), .b(x21), .O(new_n92_));
  aoi22  g42(.a(new_n92_), .b(new_n82_), .c(new_n91_), .d(x22), .O(new_n93_));
  oai21  g43(.a(new_n93_), .b(new_n86_), .c(new_n90_), .O(z12));
  inv1   g44(.a(x13), .O(new_n96_));
  nand2  g45(.a(x18), .b(x05), .O(new_n97_));
  oai21  g46(.a(x18), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  nor3   g48(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  aoi21  g49(.a(new_n100_), .b(new_n82_), .c(new_n59_), .O(new_n101_));
  nor2   g50(.a(x24), .b(x23), .O(new_n102_));
  nand4  g51(.a(new_n102_), .b(new_n92_), .c(new_n72_), .d(new_n49_), .O(new_n103_));
  inv1   g52(.a(new_n103_), .O(new_n104_));
  oai21  g53(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(new_n99_), .O(z14));
  inv1   g55(.a(x14), .O(new_n107_));
  nand2  g56(.a(x18), .b(x06), .O(new_n108_));
  oai21  g57(.a(x18), .b(new_n107_), .c(new_n108_), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n86_), .O(new_n110_));
  inv1   g59(.a(new_n91_), .O(new_n111_));
  inv1   g60(.a(x23), .O(new_n112_));
  nand4  g61(.a(new_n62_), .b(new_n59_), .c(new_n112_), .d(new_n55_), .O(new_n113_));
  inv1   g62(.a(new_n113_), .O(new_n114_));
  aoi22  g63(.a(new_n114_), .b(new_n111_), .c(new_n103_), .d(x25), .O(new_n115_));
  oai21  g64(.a(new_n115_), .b(new_n86_), .c(new_n110_), .O(z15));
  zero   g65(.O(z00));
  zero   g66(.O(z04));
  zero   g67(.O(z08));
  zero   g68(.O(z11));
  zero   g69(.O(z13));
  zero   g70(.O(z16));
  zero   g71(.O(z17));
endmodule


