// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n112_, new_n114_, new_n115_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x01), .b(x00), .O(new_n46_));
  nand2  g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x10), .c(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n47_), .O(z01));
  inv1   g06(.a(x19), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(x18), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  nand3  g09(.a(new_n46_), .b(new_n54_), .c(new_n45_), .O(new_n55_));
  inv1   g10(.a(x10), .O(new_n56_));
  inv1   g11(.a(x11), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(x12), .O(new_n59_));
  inv1   g14(.a(x09), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  oai22  g16(.a(new_n61_), .b(new_n59_), .c(new_n55_), .d(new_n53_), .O(z03));
  nand2  g17(.a(x01), .b(x00), .O(new_n64_));
  nor4   g18(.a(new_n64_), .b(new_n56_), .c(new_n60_), .d(new_n45_), .O(z05));
  nand3  g19(.a(new_n58_), .b(new_n60_), .c(x02), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n64_), .O(z06));
  inv1   g21(.a(x12), .O(new_n68_));
  nand4  g22(.a(new_n58_), .b(new_n68_), .c(new_n60_), .d(x01), .O(new_n69_));
  nand2  g23(.a(x02), .b(x00), .O(new_n70_));
  aoi21  g24(.a(new_n69_), .b(x01), .c(new_n70_), .O(z07));
  inv1   g25(.a(x03), .O(new_n72_));
  nor2   g26(.a(x06), .b(x05), .O(new_n73_));
  nor2   g27(.a(x08), .b(x07), .O(new_n74_));
  nand4  g28(.a(new_n74_), .b(new_n73_), .c(x04), .d(new_n72_), .O(new_n75_));
  nand2  g29(.a(new_n46_), .b(x02), .O(new_n76_));
  inv1   g30(.a(x18), .O(new_n77_));
  nand3  g31(.a(x19), .b(new_n77_), .c(x17), .O(new_n78_));
  nor3   g32(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(z08));
  inv1   g33(.a(x15), .O(new_n80_));
  inv1   g34(.a(x16), .O(new_n81_));
  nand3  g35(.a(x20), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  nor2   g37(.a(new_n45_), .b(x01), .O(new_n84_));
  inv1   g38(.a(x13), .O(new_n85_));
  inv1   g39(.a(x14), .O(new_n86_));
  nand4  g40(.a(new_n86_), .b(new_n85_), .c(x12), .d(new_n57_), .O(new_n87_));
  inv1   g41(.a(new_n87_), .O(new_n88_));
  nand3  g42(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(new_n89_));
  inv1   g43(.a(x20), .O(new_n90_));
  nand4  g44(.a(new_n90_), .b(new_n52_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g45(.a(x00), .O(new_n92_));
  inv1   g46(.a(x21), .O(new_n93_));
  inv1   g47(.a(x22), .O(new_n94_));
  nand3  g48(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  aoi21  g49(.a(new_n91_), .b(new_n89_), .c(new_n95_), .O(z09));
  inv1   g50(.a(x01), .O(new_n97_));
  nor2   g51(.a(x20), .b(new_n97_), .O(new_n98_));
  nor2   g52(.a(new_n94_), .b(new_n93_), .O(new_n99_));
  nand4  g53(.a(new_n99_), .b(new_n98_), .c(new_n52_), .d(x18), .O(new_n100_));
  nand3  g54(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n101_));
  inv1   g55(.a(new_n101_), .O(new_n102_));
  nand4  g56(.a(x16), .b(x15), .c(new_n86_), .d(new_n85_), .O(new_n103_));
  inv1   g57(.a(new_n103_), .O(new_n104_));
  nand4  g58(.a(x12), .b(new_n57_), .c(x02), .d(new_n97_), .O(new_n105_));
  inv1   g59(.a(new_n105_), .O(new_n106_));
  nand3  g60(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  aoi21  g61(.a(new_n107_), .b(new_n100_), .c(x00), .O(z10));
  nor3   g62(.a(new_n47_), .b(x19), .c(new_n54_), .O(z13));
  nand3  g63(.a(new_n48_), .b(new_n56_), .c(new_n60_), .O(new_n112_));
  nor2   g64(.a(new_n112_), .b(new_n47_), .O(z14));
  aoi21  g65(.a(new_n56_), .b(x01), .c(new_n45_), .O(new_n114_));
  nand3  g66(.a(x19), .b(new_n45_), .c(new_n97_), .O(new_n115_));
  oai21  g67(.a(new_n114_), .b(new_n92_), .c(new_n115_), .O(z15));
  nor2   g68(.a(new_n97_), .b(x00), .O(z16));
  inv1   g69(.a(new_n76_), .O(z17));
  zero   g70(.O(z00));
  zero   g71(.O(z02));
  zero   g72(.O(z04));
  zero   g73(.O(z11));
  zero   g74(.O(z12));
endmodule


