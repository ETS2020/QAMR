// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n105_, new_n108_, new_n109_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x01), .b(x00), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(x17), .O(new_n48_));
  inv1   g04(.a(x18), .O(new_n49_));
  inv1   g05(.a(x19), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g08(.a(x10), .O(new_n53_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n53_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n47_), .O(z02));
  nor2   g11(.a(x19), .b(new_n49_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand3  g13(.a(new_n46_), .b(new_n48_), .c(new_n45_), .O(new_n58_));
  inv1   g14(.a(x11), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x12), .O(new_n61_));
  inv1   g17(.a(x09), .O(new_n62_));
  nand4  g18(.a(new_n62_), .b(x02), .c(x01), .d(x00), .O(new_n63_));
  oai22  g19(.a(new_n63_), .b(new_n61_), .c(new_n58_), .d(new_n57_), .O(z03));
  nor4   g20(.a(new_n63_), .b(x12), .c(x11), .d(new_n53_), .O(z04));
  inv1   g21(.a(x00), .O(new_n67_));
  inv1   g22(.a(x01), .O(new_n68_));
  nand3  g23(.a(new_n60_), .b(new_n62_), .c(x02), .O(new_n69_));
  nor3   g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z06));
  inv1   g25(.a(x12), .O(new_n71_));
  nand4  g26(.a(new_n60_), .b(new_n71_), .c(new_n62_), .d(x01), .O(new_n72_));
  nand2  g27(.a(x02), .b(x00), .O(new_n73_));
  aoi21  g28(.a(new_n72_), .b(x01), .c(new_n73_), .O(z07));
  inv1   g29(.a(x03), .O(new_n75_));
  nor2   g30(.a(x06), .b(x05), .O(new_n76_));
  nor2   g31(.a(x08), .b(x07), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(x04), .d(new_n75_), .O(new_n78_));
  nand2  g33(.a(new_n46_), .b(x02), .O(new_n79_));
  nand3  g34(.a(x19), .b(new_n49_), .c(x17), .O(new_n80_));
  nor3   g35(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(z08));
  nor2   g36(.a(x20), .b(new_n68_), .O(new_n83_));
  nand4  g37(.a(new_n83_), .b(new_n56_), .c(x22), .d(x21), .O(new_n84_));
  nand2  g38(.a(x02), .b(new_n68_), .O(new_n85_));
  inv1   g39(.a(x13), .O(new_n86_));
  nand3  g40(.a(new_n86_), .b(x12), .c(new_n59_), .O(new_n87_));
  nor2   g41(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  inv1   g42(.a(x15), .O(new_n89_));
  nor2   g43(.a(new_n89_), .b(x14), .O(new_n90_));
  inv1   g44(.a(x21), .O(new_n91_));
  inv1   g45(.a(x22), .O(new_n92_));
  nand4  g46(.a(new_n92_), .b(new_n91_), .c(x20), .d(x16), .O(new_n93_));
  inv1   g47(.a(new_n93_), .O(new_n94_));
  nand3  g48(.a(new_n94_), .b(new_n90_), .c(new_n88_), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n84_), .c(x00), .O(z10));
  nand3  g50(.a(new_n83_), .b(new_n56_), .c(x21), .O(new_n97_));
  inv1   g51(.a(x16), .O(new_n98_));
  nand3  g52(.a(new_n91_), .b(x20), .c(new_n98_), .O(new_n99_));
  inv1   g53(.a(new_n99_), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n90_), .c(new_n88_), .O(new_n101_));
  nand2  g55(.a(new_n92_), .b(new_n67_), .O(new_n102_));
  aoi21  g56(.a(new_n101_), .b(new_n97_), .c(new_n102_), .O(z11));
  nand2  g57(.a(new_n46_), .b(new_n45_), .O(new_n105_));
  nor3   g58(.a(new_n105_), .b(x19), .c(new_n48_), .O(z13));
  aoi21  g59(.a(new_n53_), .b(x01), .c(new_n45_), .O(new_n108_));
  nand3  g60(.a(x19), .b(new_n45_), .c(new_n68_), .O(new_n109_));
  oai21  g61(.a(new_n108_), .b(new_n67_), .c(new_n109_), .O(z15));
  inv1   g62(.a(new_n79_), .O(z17));
  zero   g63(.O(z00));
  zero   g64(.O(z05));
  zero   g65(.O(z09));
  zero   g66(.O(z12));
  zero   g67(.O(z14));
  zero   g68(.O(z16));
endmodule


