// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n108_, new_n110_, new_n111_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z00));
  nor2   g09(.a(x19), .b(new_n49_), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  nand3  g11(.a(new_n46_), .b(new_n48_), .c(new_n44_), .O(new_n57_));
  inv1   g12(.a(x10), .O(new_n58_));
  inv1   g13(.a(x11), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x12), .O(new_n61_));
  inv1   g16(.a(x00), .O(new_n62_));
  inv1   g17(.a(x01), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n45_), .c(x02), .O(new_n65_));
  oai22  g20(.a(new_n65_), .b(new_n61_), .c(new_n57_), .d(new_n56_), .O(z03));
  nand3  g21(.a(new_n60_), .b(new_n45_), .c(x02), .O(new_n69_));
  nor3   g22(.a(new_n69_), .b(new_n63_), .c(new_n62_), .O(z06));
  inv1   g23(.a(x12), .O(new_n71_));
  nand4  g24(.a(new_n60_), .b(new_n71_), .c(new_n45_), .d(x01), .O(new_n72_));
  nand2  g25(.a(x02), .b(x00), .O(new_n73_));
  aoi21  g26(.a(new_n72_), .b(x01), .c(new_n73_), .O(z07));
  inv1   g27(.a(x03), .O(new_n75_));
  nor2   g28(.a(x06), .b(x05), .O(new_n76_));
  nor2   g29(.a(x08), .b(x07), .O(new_n77_));
  nand4  g30(.a(new_n77_), .b(new_n76_), .c(x04), .d(new_n75_), .O(new_n78_));
  nand2  g31(.a(new_n46_), .b(x02), .O(new_n79_));
  nand3  g32(.a(x19), .b(new_n49_), .c(x17), .O(new_n80_));
  nor3   g33(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(z08));
  nor2   g34(.a(x20), .b(new_n63_), .O(new_n83_));
  nand4  g35(.a(new_n83_), .b(new_n55_), .c(x22), .d(x21), .O(new_n84_));
  inv1   g36(.a(x21), .O(new_n85_));
  inv1   g37(.a(x22), .O(new_n86_));
  nand3  g38(.a(new_n86_), .b(new_n85_), .c(x20), .O(new_n87_));
  inv1   g39(.a(new_n87_), .O(new_n88_));
  inv1   g40(.a(x13), .O(new_n89_));
  inv1   g41(.a(x14), .O(new_n90_));
  nand4  g42(.a(x16), .b(x15), .c(new_n90_), .d(new_n89_), .O(new_n91_));
  inv1   g43(.a(new_n91_), .O(new_n92_));
  nand4  g44(.a(x12), .b(new_n59_), .c(x02), .d(new_n63_), .O(new_n93_));
  inv1   g45(.a(new_n93_), .O(new_n94_));
  nand3  g46(.a(new_n94_), .b(new_n92_), .c(new_n88_), .O(new_n95_));
  aoi21  g47(.a(new_n95_), .b(new_n84_), .c(x00), .O(z10));
  oai21  g48(.a(new_n58_), .b(new_n44_), .c(new_n64_), .O(new_n98_));
  oai21  g49(.a(x19), .b(x02), .c(x23), .O(new_n99_));
  nand3  g50(.a(new_n50_), .b(x17), .c(new_n44_), .O(new_n100_));
  nand2  g51(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g52(.a(new_n101_), .b(new_n46_), .O(new_n102_));
  inv1   g53(.a(x24), .O(new_n103_));
  nand2  g54(.a(new_n103_), .b(x09), .O(new_n104_));
  aoi21  g55(.a(new_n102_), .b(new_n98_), .c(new_n104_), .O(z12));
  nand2  g56(.a(new_n46_), .b(new_n44_), .O(new_n106_));
  nor3   g57(.a(new_n106_), .b(x19), .c(new_n48_), .O(z13));
  nand4  g58(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n58_), .O(new_n108_));
  nor2   g59(.a(new_n108_), .b(new_n47_), .O(z14));
  aoi21  g60(.a(new_n58_), .b(x01), .c(new_n44_), .O(new_n110_));
  nand3  g61(.a(x19), .b(new_n44_), .c(new_n63_), .O(new_n111_));
  oai21  g62(.a(new_n110_), .b(new_n62_), .c(new_n111_), .O(z15));
  inv1   g63(.a(new_n79_), .O(z17));
  zero   g64(.O(z01));
  zero   g65(.O(z02));
  zero   g66(.O(z04));
  zero   g67(.O(z05));
  zero   g68(.O(z09));
  zero   g69(.O(z11));
  zero   g70(.O(z16));
endmodule


