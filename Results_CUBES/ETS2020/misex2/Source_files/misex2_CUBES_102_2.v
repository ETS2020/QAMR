// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n99_, new_n100_;
  inv1   g00(.a(x00), .O(new_n45_));
  inv1   g01(.a(x01), .O(new_n46_));
  inv1   g02(.a(x02), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nand2  g04(.a(x10), .b(x09), .O(new_n49_));
  inv1   g05(.a(x17), .O(new_n50_));
  inv1   g06(.a(x18), .O(new_n51_));
  inv1   g07(.a(x19), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n49_), .c(new_n48_), .O(z01));
  inv1   g10(.a(x10), .O(new_n57_));
  nand3  g11(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  inv1   g12(.a(x11), .O(new_n59_));
  inv1   g13(.a(x12), .O(new_n60_));
  nand2  g14(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor4   g15(.a(new_n61_), .b(new_n58_), .c(new_n57_), .d(x09), .O(z04));
  nor2   g16(.a(new_n58_), .b(new_n49_), .O(z05));
  inv1   g17(.a(x03), .O(new_n66_));
  nor2   g18(.a(x06), .b(x05), .O(new_n67_));
  nor2   g19(.a(x08), .b(x07), .O(new_n68_));
  nand4  g20(.a(new_n68_), .b(new_n67_), .c(x04), .d(new_n66_), .O(new_n69_));
  nand3  g21(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n70_));
  nand3  g22(.a(x19), .b(new_n51_), .c(x17), .O(new_n71_));
  nor3   g23(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z08));
  nor2   g24(.a(new_n51_), .b(new_n46_), .O(new_n74_));
  nor2   g25(.a(x20), .b(x19), .O(new_n75_));
  nand4  g26(.a(new_n75_), .b(new_n74_), .c(x22), .d(x21), .O(new_n76_));
  nand2  g27(.a(x02), .b(new_n46_), .O(new_n77_));
  inv1   g28(.a(x13), .O(new_n78_));
  nand3  g29(.a(new_n78_), .b(x12), .c(new_n59_), .O(new_n79_));
  nor2   g30(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  inv1   g31(.a(x15), .O(new_n81_));
  nor2   g32(.a(new_n81_), .b(x14), .O(new_n82_));
  inv1   g33(.a(x21), .O(new_n83_));
  inv1   g34(.a(x22), .O(new_n84_));
  nand4  g35(.a(new_n84_), .b(new_n83_), .c(x20), .d(x16), .O(new_n85_));
  inv1   g36(.a(new_n85_), .O(new_n86_));
  nand3  g37(.a(new_n86_), .b(new_n82_), .c(new_n80_), .O(new_n87_));
  aoi21  g38(.a(new_n87_), .b(new_n76_), .c(x00), .O(z10));
  nand3  g39(.a(new_n75_), .b(new_n74_), .c(x21), .O(new_n89_));
  inv1   g40(.a(x16), .O(new_n90_));
  nand3  g41(.a(new_n83_), .b(x20), .c(new_n90_), .O(new_n91_));
  inv1   g42(.a(new_n91_), .O(new_n92_));
  nand3  g43(.a(new_n92_), .b(new_n82_), .c(new_n80_), .O(new_n93_));
  nand2  g44(.a(new_n84_), .b(new_n45_), .O(new_n94_));
  aoi21  g45(.a(new_n93_), .b(new_n89_), .c(new_n94_), .O(z11));
  nor3   g46(.a(new_n48_), .b(x19), .c(new_n50_), .O(z13));
  nor4   g47(.a(new_n53_), .b(new_n48_), .c(x10), .d(x09), .O(z14));
  aoi21  g48(.a(new_n57_), .b(x01), .c(new_n47_), .O(new_n99_));
  nand3  g49(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n100_));
  oai21  g50(.a(new_n99_), .b(new_n45_), .c(new_n100_), .O(z15));
  nor2   g51(.a(new_n46_), .b(x00), .O(z16));
  inv1   g52(.a(new_n70_), .O(z17));
  zero   g53(.O(z00));
  zero   g54(.O(z02));
  zero   g55(.O(z03));
  zero   g56(.O(z06));
  zero   g57(.O(z07));
  zero   g58(.O(z09));
  zero   g59(.O(z12));
endmodule


