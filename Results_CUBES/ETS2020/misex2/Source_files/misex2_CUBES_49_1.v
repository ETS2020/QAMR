// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x01), .b(x00), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(x17), .O(new_n48_));
  inv1   g04(.a(x18), .O(new_n49_));
  inv1   g05(.a(x19), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g08(.a(x10), .O(new_n53_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n53_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n47_), .O(z02));
  nor2   g11(.a(x19), .b(new_n49_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand3  g13(.a(new_n46_), .b(new_n48_), .c(new_n45_), .O(new_n58_));
  nand3  g14(.a(x12), .b(x11), .c(x02), .O(new_n59_));
  inv1   g15(.a(x09), .O(new_n60_));
  nand4  g16(.a(x10), .b(new_n60_), .c(x01), .d(x00), .O(new_n61_));
  oai22  g17(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(z03));
  inv1   g18(.a(x00), .O(new_n64_));
  inv1   g19(.a(x01), .O(new_n65_));
  nand3  g20(.a(x10), .b(x09), .c(x02), .O(new_n66_));
  nor3   g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z05));
  inv1   g22(.a(x12), .O(new_n69_));
  nand4  g23(.a(new_n69_), .b(x11), .c(x10), .d(new_n60_), .O(new_n70_));
  nand2  g24(.a(x02), .b(x00), .O(new_n71_));
  aoi21  g25(.a(new_n70_), .b(x01), .c(new_n71_), .O(z07));
  inv1   g26(.a(x03), .O(new_n73_));
  nor2   g27(.a(x06), .b(x05), .O(new_n74_));
  nor2   g28(.a(x08), .b(x07), .O(new_n75_));
  nand4  g29(.a(new_n75_), .b(new_n74_), .c(x04), .d(new_n73_), .O(new_n76_));
  nand2  g30(.a(new_n46_), .b(x02), .O(new_n77_));
  nand3  g31(.a(x19), .b(new_n49_), .c(x17), .O(new_n78_));
  nor3   g32(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(z08));
  inv1   g33(.a(x15), .O(new_n80_));
  inv1   g34(.a(x16), .O(new_n81_));
  nand3  g35(.a(x20), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  nand2  g37(.a(x02), .b(new_n65_), .O(new_n84_));
  inv1   g38(.a(new_n84_), .O(new_n85_));
  nor2   g39(.a(new_n69_), .b(x11), .O(new_n86_));
  nor2   g40(.a(x14), .b(x13), .O(new_n87_));
  nand4  g41(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(new_n83_), .O(new_n88_));
  nor2   g42(.a(x20), .b(x19), .O(new_n89_));
  nand3  g43(.a(new_n89_), .b(x18), .c(x01), .O(new_n90_));
  inv1   g44(.a(x21), .O(new_n91_));
  inv1   g45(.a(x22), .O(new_n92_));
  nand3  g46(.a(new_n92_), .b(new_n91_), .c(new_n64_), .O(new_n93_));
  aoi21  g47(.a(new_n90_), .b(new_n88_), .c(new_n93_), .O(z09));
  nor2   g48(.a(x20), .b(new_n65_), .O(new_n95_));
  nand4  g49(.a(new_n95_), .b(new_n56_), .c(x22), .d(x21), .O(new_n96_));
  inv1   g50(.a(x11), .O(new_n97_));
  inv1   g51(.a(x13), .O(new_n98_));
  nand3  g52(.a(new_n98_), .b(x12), .c(new_n97_), .O(new_n99_));
  nor2   g53(.a(new_n99_), .b(new_n84_), .O(new_n100_));
  nor2   g54(.a(new_n80_), .b(x14), .O(new_n101_));
  nand4  g55(.a(new_n92_), .b(new_n91_), .c(x20), .d(x16), .O(new_n102_));
  inv1   g56(.a(new_n102_), .O(new_n103_));
  nand3  g57(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  aoi21  g58(.a(new_n104_), .b(new_n96_), .c(x00), .O(z10));
  nand4  g59(.a(new_n89_), .b(x21), .c(x18), .d(x01), .O(new_n106_));
  nand3  g60(.a(new_n91_), .b(x20), .c(new_n81_), .O(new_n107_));
  inv1   g61(.a(new_n107_), .O(new_n108_));
  nand3  g62(.a(new_n108_), .b(new_n101_), .c(new_n100_), .O(new_n109_));
  nand2  g63(.a(new_n92_), .b(new_n64_), .O(new_n110_));
  aoi21  g64(.a(new_n109_), .b(new_n106_), .c(new_n110_), .O(z11));
  nand2  g65(.a(new_n46_), .b(new_n45_), .O(new_n113_));
  nor3   g66(.a(new_n113_), .b(x19), .c(new_n48_), .O(z13));
  nor2   g67(.a(x10), .b(x09), .O(new_n115_));
  nand4  g68(.a(new_n115_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n116_));
  nor2   g69(.a(new_n116_), .b(new_n113_), .O(z14));
  aoi21  g70(.a(new_n53_), .b(x01), .c(new_n45_), .O(new_n118_));
  nand3  g71(.a(x19), .b(new_n45_), .c(new_n65_), .O(new_n119_));
  oai21  g72(.a(new_n118_), .b(new_n64_), .c(new_n119_), .O(z15));
  nor2   g73(.a(new_n65_), .b(x00), .O(z16));
  inv1   g74(.a(new_n77_), .O(z17));
  zero   g75(.O(z00));
  zero   g76(.O(z04));
  zero   g77(.O(z06));
  zero   g78(.O(z12));
endmodule


