// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n114_, new_n115_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nand2  g04(.a(x10), .b(new_n47_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(new_n48_), .c(new_n46_), .O(z00));
  nand2  g10(.a(x10), .b(x09), .O(new_n54_));
  nor3   g11(.a(new_n54_), .b(new_n52_), .c(new_n46_), .O(z01));
  inv1   g12(.a(new_n45_), .O(new_n57_));
  nand4  g13(.a(new_n51_), .b(x18), .c(new_n49_), .d(new_n44_), .O(new_n58_));
  nand3  g14(.a(x12), .b(x11), .c(x10), .O(new_n59_));
  inv1   g15(.a(x00), .O(new_n60_));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g18(.a(new_n62_), .b(new_n47_), .c(x02), .O(new_n63_));
  oai22  g19(.a(new_n63_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(z03));
  nand2  g20(.a(new_n62_), .b(x02), .O(new_n65_));
  inv1   g21(.a(x11), .O(new_n66_));
  inv1   g22(.a(x12), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor3   g24(.a(new_n68_), .b(new_n65_), .c(new_n48_), .O(z04));
  nor2   g25(.a(new_n65_), .b(new_n54_), .O(z05));
  inv1   g26(.a(x03), .O(new_n73_));
  nor2   g27(.a(x06), .b(x05), .O(new_n74_));
  nor2   g28(.a(x08), .b(x07), .O(new_n75_));
  nand4  g29(.a(new_n75_), .b(new_n74_), .c(x04), .d(new_n73_), .O(new_n76_));
  nand2  g30(.a(new_n45_), .b(x02), .O(new_n77_));
  nand3  g31(.a(x19), .b(new_n50_), .c(x17), .O(new_n78_));
  nor3   g32(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(z08));
  inv1   g33(.a(x15), .O(new_n80_));
  inv1   g34(.a(x16), .O(new_n81_));
  nand3  g35(.a(x20), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  nor2   g37(.a(new_n44_), .b(x01), .O(new_n84_));
  inv1   g38(.a(x13), .O(new_n85_));
  inv1   g39(.a(x14), .O(new_n86_));
  nand4  g40(.a(new_n86_), .b(new_n85_), .c(x12), .d(new_n66_), .O(new_n87_));
  inv1   g41(.a(new_n87_), .O(new_n88_));
  nand3  g42(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(new_n89_));
  nor2   g43(.a(x20), .b(x19), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(x18), .c(x01), .O(new_n91_));
  inv1   g45(.a(x21), .O(new_n92_));
  inv1   g46(.a(x22), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n92_), .c(new_n60_), .O(new_n94_));
  aoi21  g48(.a(new_n91_), .b(new_n89_), .c(new_n94_), .O(z09));
  nand4  g49(.a(new_n90_), .b(x21), .c(x18), .d(x01), .O(new_n97_));
  nand4  g50(.a(new_n92_), .b(x20), .c(new_n81_), .d(x15), .O(new_n98_));
  inv1   g51(.a(new_n98_), .O(new_n99_));
  nand3  g52(.a(new_n99_), .b(new_n88_), .c(new_n84_), .O(new_n100_));
  nand2  g53(.a(new_n93_), .b(new_n60_), .O(new_n101_));
  aoi21  g54(.a(new_n100_), .b(new_n97_), .c(new_n101_), .O(z11));
  inv1   g55(.a(x10), .O(new_n103_));
  oai21  g56(.a(new_n103_), .b(new_n44_), .c(new_n62_), .O(new_n104_));
  oai21  g57(.a(x19), .b(x02), .c(x23), .O(new_n105_));
  nand3  g58(.a(new_n51_), .b(x17), .c(new_n44_), .O(new_n106_));
  nand2  g59(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g60(.a(new_n107_), .b(new_n45_), .O(new_n108_));
  inv1   g61(.a(x24), .O(new_n109_));
  nand2  g62(.a(new_n109_), .b(x09), .O(new_n110_));
  aoi21  g63(.a(new_n108_), .b(new_n104_), .c(new_n110_), .O(z12));
  nor3   g64(.a(new_n46_), .b(x19), .c(new_n49_), .O(z13));
  aoi21  g65(.a(new_n103_), .b(x01), .c(new_n44_), .O(new_n114_));
  nand3  g66(.a(x19), .b(new_n44_), .c(new_n61_), .O(new_n115_));
  oai21  g67(.a(new_n114_), .b(new_n60_), .c(new_n115_), .O(z15));
  nor2   g68(.a(new_n61_), .b(x00), .O(z16));
  inv1   g69(.a(new_n77_), .O(z17));
  zero   g70(.O(z02));
  zero   g71(.O(z06));
  zero   g72(.O(z07));
  zero   g73(.O(z10));
  zero   g74(.O(z14));
endmodule


