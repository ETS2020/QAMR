// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_;
  inv1   g00(.a(x02), .O(new_n46_));
  nor2   g01(.a(x01), .b(x00), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(x10), .O(new_n49_));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  nand4  g06(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x09), .O(new_n52_));
  nor2   g07(.a(new_n52_), .b(new_n48_), .O(z02));
  nand2  g08(.a(new_n50_), .b(x18), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  nand3  g10(.a(new_n47_), .b(new_n55_), .c(new_n46_), .O(new_n56_));
  inv1   g11(.a(x11), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(x12), .O(new_n59_));
  inv1   g14(.a(x09), .O(new_n60_));
  inv1   g15(.a(x00), .O(new_n61_));
  inv1   g16(.a(x01), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n60_), .c(x02), .O(new_n64_));
  oai22  g19(.a(new_n64_), .b(new_n59_), .c(new_n56_), .d(new_n54_), .O(z03));
  inv1   g20(.a(x12), .O(new_n66_));
  nand3  g21(.a(new_n66_), .b(new_n57_), .c(x10), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n64_), .O(z04));
  nand3  g23(.a(new_n58_), .b(new_n60_), .c(x02), .O(new_n70_));
  nor3   g24(.a(new_n70_), .b(new_n62_), .c(new_n61_), .O(z06));
  nand4  g25(.a(new_n58_), .b(new_n66_), .c(new_n60_), .d(x01), .O(new_n72_));
  nand2  g26(.a(x02), .b(x00), .O(new_n73_));
  aoi21  g27(.a(new_n72_), .b(x01), .c(new_n73_), .O(z07));
  inv1   g28(.a(x03), .O(new_n75_));
  nor2   g29(.a(x06), .b(x05), .O(new_n76_));
  nor2   g30(.a(x08), .b(x07), .O(new_n77_));
  nand4  g31(.a(new_n77_), .b(new_n76_), .c(x04), .d(new_n75_), .O(new_n78_));
  nand2  g32(.a(new_n47_), .b(x02), .O(new_n79_));
  inv1   g33(.a(x18), .O(new_n80_));
  nand3  g34(.a(x19), .b(new_n80_), .c(x17), .O(new_n81_));
  nor3   g35(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(z08));
  inv1   g36(.a(x15), .O(new_n83_));
  inv1   g37(.a(x16), .O(new_n84_));
  nand3  g38(.a(x20), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g39(.a(new_n85_), .O(new_n86_));
  nor2   g40(.a(new_n46_), .b(x01), .O(new_n87_));
  inv1   g41(.a(x13), .O(new_n88_));
  inv1   g42(.a(x14), .O(new_n89_));
  nand4  g43(.a(new_n89_), .b(new_n88_), .c(x12), .d(new_n57_), .O(new_n90_));
  inv1   g44(.a(new_n90_), .O(new_n91_));
  nand3  g45(.a(new_n91_), .b(new_n87_), .c(new_n86_), .O(new_n92_));
  inv1   g46(.a(x20), .O(new_n93_));
  nand4  g47(.a(new_n93_), .b(new_n50_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g48(.a(x21), .O(new_n95_));
  inv1   g49(.a(x22), .O(new_n96_));
  nand3  g50(.a(new_n96_), .b(new_n95_), .c(new_n61_), .O(new_n97_));
  aoi21  g51(.a(new_n94_), .b(new_n92_), .c(new_n97_), .O(z09));
  nor2   g52(.a(x20), .b(new_n62_), .O(new_n99_));
  nor2   g53(.a(new_n96_), .b(new_n95_), .O(new_n100_));
  nand4  g54(.a(new_n100_), .b(new_n99_), .c(new_n50_), .d(x18), .O(new_n101_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n102_));
  inv1   g56(.a(new_n102_), .O(new_n103_));
  nand4  g57(.a(x16), .b(x15), .c(new_n89_), .d(new_n88_), .O(new_n104_));
  inv1   g58(.a(new_n104_), .O(new_n105_));
  nand4  g59(.a(x12), .b(new_n57_), .c(x02), .d(new_n62_), .O(new_n106_));
  inv1   g60(.a(new_n106_), .O(new_n107_));
  nand3  g61(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  aoi21  g62(.a(new_n108_), .b(new_n101_), .c(x00), .O(z10));
  oai21  g63(.a(new_n49_), .b(new_n46_), .c(new_n63_), .O(new_n111_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g65(.a(new_n50_), .b(x17), .c(new_n46_), .O(new_n113_));
  nand2  g66(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g67(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  inv1   g68(.a(x24), .O(new_n116_));
  nand2  g69(.a(new_n116_), .b(x09), .O(new_n117_));
  aoi21  g70(.a(new_n115_), .b(new_n111_), .c(new_n117_), .O(z12));
  nor3   g71(.a(new_n48_), .b(x19), .c(new_n55_), .O(z13));
  nor2   g72(.a(new_n62_), .b(x00), .O(z16));
  inv1   g73(.a(new_n79_), .O(z17));
  zero   g74(.O(z00));
  zero   g75(.O(z01));
  zero   g76(.O(z05));
  zero   g77(.O(z11));
  zero   g78(.O(z14));
  zero   g79(.O(z15));
endmodule


