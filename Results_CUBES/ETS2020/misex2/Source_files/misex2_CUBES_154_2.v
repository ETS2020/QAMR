// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n115_, new_n116_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  inv1   g14(.a(x11), .O(new_n59_));
  inv1   g15(.a(x12), .O(new_n60_));
  nand4  g16(.a(new_n60_), .b(new_n59_), .c(x10), .d(new_n44_), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n58_), .O(z04));
  nor3   g18(.a(new_n58_), .b(new_n54_), .c(new_n44_), .O(z05));
  nand4  g19(.a(new_n60_), .b(x11), .c(x10), .d(new_n44_), .O(new_n65_));
  nand2  g20(.a(x02), .b(x00), .O(new_n66_));
  aoi21  g21(.a(new_n65_), .b(x01), .c(new_n66_), .O(z07));
  inv1   g22(.a(x08), .O(new_n68_));
  nand4  g23(.a(x19), .b(new_n48_), .c(x17), .d(new_n68_), .O(new_n69_));
  inv1   g24(.a(x05), .O(new_n70_));
  nor2   g25(.a(x07), .b(x06), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n70_), .c(x04), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  nor2   g28(.a(x01), .b(x00), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n73_), .c(x02), .O(new_n75_));
  nor3   g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z08));
  inv1   g31(.a(x01), .O(new_n78_));
  nor2   g32(.a(new_n48_), .b(new_n78_), .O(new_n79_));
  nor2   g33(.a(x20), .b(x19), .O(new_n80_));
  nand4  g34(.a(new_n80_), .b(new_n79_), .c(x22), .d(x21), .O(new_n81_));
  nand2  g35(.a(x02), .b(new_n78_), .O(new_n82_));
  inv1   g36(.a(x13), .O(new_n83_));
  nand3  g37(.a(new_n83_), .b(x12), .c(new_n59_), .O(new_n84_));
  nor2   g38(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  inv1   g39(.a(x15), .O(new_n86_));
  nor2   g40(.a(new_n86_), .b(x14), .O(new_n87_));
  inv1   g41(.a(x21), .O(new_n88_));
  inv1   g42(.a(x22), .O(new_n89_));
  nand4  g43(.a(new_n89_), .b(new_n88_), .c(x20), .d(x16), .O(new_n90_));
  inv1   g44(.a(new_n90_), .O(new_n91_));
  nand3  g45(.a(new_n91_), .b(new_n87_), .c(new_n85_), .O(new_n92_));
  aoi21  g46(.a(new_n92_), .b(new_n81_), .c(x00), .O(z10));
  nand3  g47(.a(new_n80_), .b(new_n79_), .c(x21), .O(new_n94_));
  inv1   g48(.a(x16), .O(new_n95_));
  nand3  g49(.a(new_n88_), .b(x20), .c(new_n95_), .O(new_n96_));
  inv1   g50(.a(new_n96_), .O(new_n97_));
  nand3  g51(.a(new_n97_), .b(new_n87_), .c(new_n85_), .O(new_n98_));
  inv1   g52(.a(x00), .O(new_n99_));
  nand2  g53(.a(new_n89_), .b(new_n99_), .O(new_n100_));
  aoi21  g54(.a(new_n98_), .b(new_n94_), .c(new_n100_), .O(z11));
  nand2  g55(.a(x10), .b(x02), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(x01), .c(x00), .O(new_n103_));
  oai21  g57(.a(x19), .b(x02), .c(x23), .O(new_n104_));
  inv1   g58(.a(x02), .O(new_n105_));
  nand3  g59(.a(new_n49_), .b(x17), .c(new_n105_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n74_), .O(new_n108_));
  inv1   g62(.a(x24), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(x09), .O(new_n110_));
  aoi21  g64(.a(new_n108_), .b(new_n103_), .c(new_n110_), .O(z12));
  nor2   g65(.a(x19), .b(new_n47_), .O(new_n112_));
  and2   g66(.a(new_n112_), .b(new_n45_), .O(z13));
  nor2   g67(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g68(.a(new_n54_), .b(x01), .c(new_n105_), .O(new_n115_));
  nand3  g69(.a(x19), .b(new_n105_), .c(new_n78_), .O(new_n116_));
  oai21  g70(.a(new_n115_), .b(new_n99_), .c(new_n116_), .O(z15));
  nor2   g71(.a(new_n78_), .b(x00), .O(z16));
  zero   g72(.O(z03));
  zero   g73(.O(z06));
  zero   g74(.O(z09));
  zero   g75(.O(z17));
endmodule


