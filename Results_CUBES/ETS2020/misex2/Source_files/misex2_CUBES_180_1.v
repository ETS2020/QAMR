// Benchmark "FAU" written by ABC on Tue Jul  7 11:01:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n71_,
    new_n72_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n105_, new_n107_, new_n108_, new_n112_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x01), .b(x00), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(x17), .O(new_n48_));
  inv1   g04(.a(x18), .O(new_n49_));
  inv1   g05(.a(x19), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g08(.a(x10), .O(new_n53_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n53_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n47_), .O(z02));
  nor2   g11(.a(x19), .b(new_n49_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand3  g13(.a(new_n46_), .b(new_n48_), .c(new_n45_), .O(new_n58_));
  nand3  g14(.a(x12), .b(x11), .c(x02), .O(new_n59_));
  nor2   g15(.a(new_n53_), .b(x09), .O(new_n60_));
  nand3  g16(.a(new_n60_), .b(x01), .c(x00), .O(new_n61_));
  oai22  g17(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(z03));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g19(.a(x11), .O(new_n64_));
  inv1   g20(.a(x12), .O(new_n65_));
  nand3  g21(.a(new_n60_), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n63_), .O(z04));
  nand2  g23(.a(x10), .b(x09), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n63_), .O(z05));
  nand3  g25(.a(new_n60_), .b(new_n65_), .c(x11), .O(new_n71_));
  nand2  g26(.a(x02), .b(x00), .O(new_n72_));
  aoi21  g27(.a(new_n71_), .b(x01), .c(new_n72_), .O(z07));
  inv1   g28(.a(x15), .O(new_n75_));
  inv1   g29(.a(x16), .O(new_n76_));
  nand3  g30(.a(x20), .b(new_n76_), .c(new_n75_), .O(new_n77_));
  inv1   g31(.a(new_n77_), .O(new_n78_));
  nor2   g32(.a(new_n45_), .b(x01), .O(new_n79_));
  inv1   g33(.a(x13), .O(new_n80_));
  inv1   g34(.a(x14), .O(new_n81_));
  nand4  g35(.a(new_n81_), .b(new_n80_), .c(x12), .d(new_n64_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  nand3  g37(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  inv1   g38(.a(x20), .O(new_n85_));
  nand4  g39(.a(new_n85_), .b(new_n50_), .c(x18), .d(x01), .O(new_n86_));
  inv1   g40(.a(x00), .O(new_n87_));
  inv1   g41(.a(x21), .O(new_n88_));
  inv1   g42(.a(x22), .O(new_n89_));
  nand3  g43(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  aoi21  g44(.a(new_n86_), .b(new_n84_), .c(new_n90_), .O(z09));
  nor2   g45(.a(new_n89_), .b(new_n88_), .O(new_n92_));
  nand4  g46(.a(new_n92_), .b(new_n56_), .c(new_n85_), .d(x01), .O(new_n93_));
  nand3  g47(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n94_));
  inv1   g48(.a(new_n94_), .O(new_n95_));
  nand4  g49(.a(x16), .b(x15), .c(new_n81_), .d(new_n80_), .O(new_n96_));
  inv1   g50(.a(new_n96_), .O(new_n97_));
  inv1   g51(.a(x01), .O(new_n98_));
  nand4  g52(.a(x12), .b(new_n64_), .c(x02), .d(new_n98_), .O(new_n99_));
  inv1   g53(.a(new_n99_), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n97_), .c(new_n95_), .O(new_n101_));
  aoi21  g55(.a(new_n101_), .b(new_n93_), .c(x00), .O(z10));
  nand2  g56(.a(new_n46_), .b(new_n45_), .O(new_n105_));
  nor3   g57(.a(new_n105_), .b(x19), .c(new_n48_), .O(z13));
  nor2   g58(.a(x10), .b(x09), .O(new_n107_));
  nand4  g59(.a(new_n107_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n108_));
  nor2   g60(.a(new_n108_), .b(new_n105_), .O(z14));
  nand2  g61(.a(new_n46_), .b(x02), .O(new_n112_));
  inv1   g62(.a(new_n112_), .O(z17));
  zero   g63(.O(z00));
  zero   g64(.O(z06));
  zero   g65(.O(z08));
  zero   g66(.O(z11));
  zero   g67(.O(z12));
  zero   g68(.O(z15));
  zero   g69(.O(z16));
endmodule


