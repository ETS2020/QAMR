// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n101_, new_n103_, new_n104_;
  inv1   g00(.a(x02), .O(new_n46_));
  nor2   g01(.a(x01), .b(x00), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(x10), .O(new_n49_));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n49_), .c(x09), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(new_n48_), .O(z02));
  inv1   g07(.a(x18), .O(new_n53_));
  nor2   g08(.a(x19), .b(new_n53_), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  nand3  g11(.a(new_n47_), .b(new_n56_), .c(new_n46_), .O(new_n57_));
  inv1   g12(.a(x11), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n49_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(x12), .O(new_n60_));
  inv1   g15(.a(x09), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(x02), .c(x01), .d(x00), .O(new_n62_));
  oai22  g17(.a(new_n62_), .b(new_n60_), .c(new_n57_), .d(new_n55_), .O(z03));
  inv1   g18(.a(x12), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n58_), .c(x10), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n62_), .O(z04));
  inv1   g21(.a(x00), .O(new_n68_));
  inv1   g22(.a(x01), .O(new_n69_));
  nand3  g23(.a(new_n59_), .b(new_n61_), .c(x02), .O(new_n70_));
  nor3   g24(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z06));
  nand4  g25(.a(new_n59_), .b(new_n64_), .c(new_n61_), .d(x01), .O(new_n72_));
  nand2  g26(.a(x02), .b(x00), .O(new_n73_));
  aoi21  g27(.a(new_n72_), .b(x01), .c(new_n73_), .O(z07));
  inv1   g28(.a(x08), .O(new_n75_));
  nand4  g29(.a(x19), .b(new_n53_), .c(x17), .d(new_n75_), .O(new_n76_));
  inv1   g30(.a(x05), .O(new_n77_));
  nor2   g31(.a(x07), .b(x06), .O(new_n78_));
  nand3  g32(.a(new_n78_), .b(new_n77_), .c(x04), .O(new_n79_));
  inv1   g33(.a(x03), .O(new_n80_));
  nand3  g34(.a(new_n47_), .b(new_n80_), .c(x02), .O(new_n81_));
  nor3   g35(.a(new_n81_), .b(new_n79_), .c(new_n76_), .O(z08));
  nor2   g36(.a(x20), .b(new_n69_), .O(new_n84_));
  nand4  g37(.a(new_n84_), .b(new_n54_), .c(x22), .d(x21), .O(new_n85_));
  inv1   g38(.a(x21), .O(new_n86_));
  inv1   g39(.a(x22), .O(new_n87_));
  nand3  g40(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  inv1   g41(.a(new_n88_), .O(new_n89_));
  inv1   g42(.a(x13), .O(new_n90_));
  inv1   g43(.a(x14), .O(new_n91_));
  nand4  g44(.a(x16), .b(x15), .c(new_n91_), .d(new_n90_), .O(new_n92_));
  inv1   g45(.a(new_n92_), .O(new_n93_));
  nand4  g46(.a(x12), .b(new_n58_), .c(x02), .d(new_n69_), .O(new_n94_));
  inv1   g47(.a(new_n94_), .O(new_n95_));
  nand3  g48(.a(new_n95_), .b(new_n93_), .c(new_n89_), .O(new_n96_));
  aoi21  g49(.a(new_n96_), .b(new_n85_), .c(x00), .O(z10));
  nor3   g50(.a(new_n48_), .b(x19), .c(new_n56_), .O(z13));
  nand3  g51(.a(new_n50_), .b(new_n49_), .c(new_n61_), .O(new_n101_));
  nor2   g52(.a(new_n101_), .b(new_n48_), .O(z14));
  aoi21  g53(.a(new_n49_), .b(x01), .c(new_n46_), .O(new_n103_));
  nand3  g54(.a(x19), .b(new_n46_), .c(new_n69_), .O(new_n104_));
  oai21  g55(.a(new_n103_), .b(new_n68_), .c(new_n104_), .O(z15));
  zero   g56(.O(z00));
  zero   g57(.O(z01));
  zero   g58(.O(z05));
  zero   g59(.O(z09));
  zero   g60(.O(z11));
  zero   g61(.O(z12));
  zero   g62(.O(z16));
  zero   g63(.O(z17));
endmodule


