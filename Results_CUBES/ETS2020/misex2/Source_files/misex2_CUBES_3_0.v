// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n95_, new_n96_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x01), .b(x00), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(x17), .O(new_n48_));
  inv1   g04(.a(x18), .O(new_n49_));
  inv1   g05(.a(x19), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g08(.a(x10), .O(new_n53_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n53_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n47_), .O(z02));
  inv1   g11(.a(new_n46_), .O(new_n56_));
  nand4  g12(.a(new_n50_), .b(x18), .c(new_n48_), .d(new_n45_), .O(new_n57_));
  and2   g13(.a(x11), .b(x10), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x12), .O(new_n59_));
  inv1   g15(.a(x09), .O(new_n60_));
  inv1   g16(.a(x00), .O(new_n61_));
  inv1   g17(.a(x01), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g19(.a(new_n63_), .b(new_n60_), .c(x02), .O(new_n64_));
  oai22  g20(.a(new_n64_), .b(new_n59_), .c(new_n57_), .d(new_n56_), .O(z03));
  nand3  g21(.a(new_n58_), .b(new_n60_), .c(x02), .O(new_n68_));
  nor3   g22(.a(new_n68_), .b(new_n62_), .c(new_n61_), .O(z06));
  inv1   g23(.a(x12), .O(new_n70_));
  nand4  g24(.a(new_n58_), .b(new_n70_), .c(new_n60_), .d(x01), .O(new_n71_));
  nand2  g25(.a(x02), .b(x00), .O(new_n72_));
  aoi21  g26(.a(new_n71_), .b(x01), .c(new_n72_), .O(z07));
  inv1   g27(.a(x08), .O(new_n74_));
  nand4  g28(.a(x19), .b(new_n49_), .c(x17), .d(new_n74_), .O(new_n75_));
  inv1   g29(.a(x05), .O(new_n76_));
  nor2   g30(.a(x07), .b(x06), .O(new_n77_));
  nand3  g31(.a(new_n77_), .b(new_n76_), .c(x04), .O(new_n78_));
  inv1   g32(.a(x03), .O(new_n79_));
  nand3  g33(.a(new_n46_), .b(new_n79_), .c(x02), .O(new_n80_));
  nor3   g34(.a(new_n80_), .b(new_n78_), .c(new_n75_), .O(z08));
  oai21  g35(.a(new_n53_), .b(new_n45_), .c(new_n63_), .O(new_n85_));
  oai21  g36(.a(x19), .b(x02), .c(x23), .O(new_n86_));
  nand3  g37(.a(new_n50_), .b(x17), .c(new_n45_), .O(new_n87_));
  nand2  g38(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g39(.a(new_n88_), .b(new_n46_), .O(new_n89_));
  inv1   g40(.a(x24), .O(new_n90_));
  nand2  g41(.a(new_n90_), .b(x09), .O(new_n91_));
  aoi21  g42(.a(new_n89_), .b(new_n85_), .c(new_n91_), .O(z12));
  aoi21  g43(.a(new_n53_), .b(x01), .c(new_n45_), .O(new_n95_));
  nand3  g44(.a(x19), .b(new_n45_), .c(new_n62_), .O(new_n96_));
  oai21  g45(.a(new_n95_), .b(new_n61_), .c(new_n96_), .O(z15));
  zero   g46(.O(z00));
  zero   g47(.O(z04));
  zero   g48(.O(z05));
  zero   g49(.O(z09));
  zero   g50(.O(z10));
  zero   g51(.O(z11));
  zero   g52(.O(z13));
  zero   g53(.O(z14));
  zero   g54(.O(z16));
  zero   g55(.O(z17));
endmodule


