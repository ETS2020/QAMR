// Benchmark "FAU" written by ABC on Tue Jul  7 11:00:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n80_, new_n83_, new_n84_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x01), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g16(.a(x02), .O(new_n60_));
  nand4  g17(.a(new_n49_), .b(x18), .c(new_n47_), .d(new_n60_), .O(new_n61_));
  and2   g18(.a(x11), .b(x10), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x12), .O(new_n63_));
  nand4  g20(.a(new_n44_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n61_), .d(new_n59_), .O(z03));
  nand2  g22(.a(x01), .b(x00), .O(new_n67_));
  nor4   g23(.a(new_n67_), .b(new_n54_), .c(new_n44_), .d(new_n60_), .O(z05));
  nand3  g24(.a(new_n62_), .b(new_n44_), .c(x02), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n67_), .O(z06));
  inv1   g26(.a(x12), .O(new_n71_));
  nand4  g27(.a(new_n62_), .b(new_n71_), .c(new_n44_), .d(x01), .O(new_n72_));
  nand2  g28(.a(x02), .b(x00), .O(new_n73_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n73_), .O(z07));
  nor2   g30(.a(x19), .b(new_n47_), .O(new_n80_));
  and2   g31(.a(new_n80_), .b(new_n45_), .O(z13));
  nor2   g32(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g33(.a(new_n54_), .b(x01), .c(new_n60_), .O(new_n83_));
  nand3  g34(.a(x19), .b(new_n60_), .c(new_n58_), .O(new_n84_));
  oai21  g35(.a(new_n83_), .b(new_n57_), .c(new_n84_), .O(z15));
  nor2   g36(.a(new_n58_), .b(x00), .O(z16));
  zero   g37(.O(z04));
  zero   g38(.O(z08));
  zero   g39(.O(z09));
  zero   g40(.O(z10));
  zero   g41(.O(z11));
  zero   g42(.O(z12));
  zero   g43(.O(z17));
endmodule


