// Benchmark "FAU" written by ABC on Fri Jul 10 18:27:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n176_, new_n177_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n198_, new_n199_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n213_, new_n214_, new_n217_, new_n218_, new_n220_,
    new_n221_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n235_, new_n237_, new_n239_, new_n240_, new_n241_,
    new_n242_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n254_, new_n256_,
    new_n258_, new_n260_, new_n263_, new_n265_, new_n268_, new_n269_,
    new_n270_, new_n271_, new_n272_, new_n273_, new_n274_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n281_, new_n283_, new_n285_,
    new_n287_, new_n289_, new_n291_, new_n295_, new_n297_, new_n300_,
    new_n301_, new_n302_, new_n303_, new_n304_, new_n305_, new_n306_,
    new_n307_, new_n308_, new_n309_, new_n310_, new_n311_, new_n312_,
    new_n314_, new_n315_, new_n316_, new_n317_, new_n318_, new_n319_,
    new_n321_, new_n322_, new_n323_, new_n324_, new_n325_, new_n327_,
    new_n328_, new_n329_, new_n330_, new_n331_, new_n333_, new_n334_,
    new_n335_, new_n336_, new_n337_, new_n339_, new_n340_, new_n341_,
    new_n342_, new_n343_, new_n347_, new_n348_, new_n349_, new_n350_,
    new_n352_, new_n353_, new_n354_, new_n355_, new_n356_, new_n359_,
    new_n360_, new_n361_, new_n362_, new_n363_, new_n364_, new_n365_,
    new_n366_, new_n367_, new_n369_, new_n370_, new_n371_, new_n372_,
    new_n373_, new_n374_, new_n375_, new_n376_, new_n378_, new_n380_,
    new_n381_, new_n382_, new_n386_, new_n387_, new_n389_, new_n390_,
    new_n391_, new_n392_, new_n393_;
  inv1   g000(.a(x40), .O(new_n152_));
  inv1   g001(.a(x01), .O(new_n153_));
  inv1   g002(.a(x79), .O(new_n154_));
  nand3  g003(.a(new_n154_), .b(x78), .c(x77), .O(new_n155_));
  aoi21  g004(.a(new_n155_), .b(new_n153_), .c(x52), .O(new_n156_));
  nand2  g005(.a(new_n152_), .b(x06), .O(new_n157_));
  oai21  g006(.a(new_n156_), .b(new_n152_), .c(new_n157_), .O(z00));
  inv1   g007(.a(x77), .O(new_n159_));
  inv1   g008(.a(x78), .O(new_n160_));
  nor2   g009(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g010(.a(new_n161_), .b(x79), .O(new_n162_));
  nor2   g011(.a(x78), .b(x77), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(new_n154_), .O(new_n164_));
  aoi21  g013(.a(new_n164_), .b(new_n162_), .c(x01), .O(z01));
  inv1   g014(.a(x66), .O(new_n166_));
  inv1   g015(.a(x75), .O(new_n167_));
  nand2  g016(.a(x78), .b(new_n159_), .O(new_n168_));
  nand2  g017(.a(new_n160_), .b(x77), .O(new_n169_));
  oai22  g018(.a(new_n169_), .b(new_n166_), .c(new_n168_), .d(new_n167_), .O(new_n170_));
  nor2   g019(.a(new_n154_), .b(x01), .O(new_n171_));
  and2   g020(.a(new_n171_), .b(new_n170_), .O(z02));
  nand4  g021(.a(new_n154_), .b(x78), .c(x52), .d(new_n153_), .O(new_n173_));
  inv1   g022(.a(new_n173_), .O(z03));
  aoi21  g023(.a(new_n161_), .b(new_n154_), .c(x01), .O(z04));
  inv1   g024(.a(x23), .O(new_n176_));
  nand2  g025(.a(x65), .b(x40), .O(new_n177_));
  oai21  g026(.a(x40), .b(new_n176_), .c(new_n177_), .O(z05));
  inv1   g027(.a(x25), .O(new_n180_));
  nand2  g028(.a(x63), .b(x40), .O(new_n181_));
  oai21  g029(.a(x40), .b(new_n180_), .c(new_n181_), .O(z07));
  inv1   g030(.a(x26), .O(new_n183_));
  nand2  g031(.a(x62), .b(x40), .O(new_n184_));
  oai21  g032(.a(x40), .b(new_n183_), .c(new_n184_), .O(z08));
  inv1   g033(.a(x27), .O(new_n186_));
  nand2  g034(.a(x61), .b(x40), .O(new_n187_));
  oai21  g035(.a(x40), .b(new_n186_), .c(new_n187_), .O(z09));
  inv1   g036(.a(x28), .O(new_n189_));
  nand2  g037(.a(x60), .b(x40), .O(new_n190_));
  oai21  g038(.a(x40), .b(new_n189_), .c(new_n190_), .O(z10));
  inv1   g039(.a(x29), .O(new_n192_));
  nand2  g040(.a(x59), .b(x40), .O(new_n193_));
  oai21  g041(.a(x40), .b(new_n192_), .c(new_n193_), .O(z11));
  inv1   g042(.a(x30), .O(new_n195_));
  nand2  g043(.a(x58), .b(x40), .O(new_n196_));
  oai21  g044(.a(x40), .b(new_n195_), .c(new_n196_), .O(z12));
  inv1   g045(.a(x31), .O(new_n198_));
  nand2  g046(.a(x57), .b(x40), .O(new_n199_));
  oai21  g047(.a(x40), .b(new_n198_), .c(new_n199_), .O(z13));
  inv1   g048(.a(x32), .O(new_n201_));
  nand2  g049(.a(x51), .b(x40), .O(new_n202_));
  oai21  g050(.a(x40), .b(new_n201_), .c(new_n202_), .O(z14));
  inv1   g051(.a(x33), .O(new_n204_));
  nand2  g052(.a(x50), .b(x40), .O(new_n205_));
  oai21  g053(.a(x40), .b(new_n204_), .c(new_n205_), .O(z15));
  inv1   g054(.a(x34), .O(new_n207_));
  nand2  g055(.a(x49), .b(x40), .O(new_n208_));
  oai21  g056(.a(x40), .b(new_n207_), .c(new_n208_), .O(z16));
  inv1   g057(.a(x35), .O(new_n210_));
  nand2  g058(.a(x48), .b(x40), .O(new_n211_));
  oai21  g059(.a(x40), .b(new_n210_), .c(new_n211_), .O(z17));
  inv1   g060(.a(x36), .O(new_n213_));
  nand2  g061(.a(x47), .b(x40), .O(new_n214_));
  oai21  g062(.a(x40), .b(new_n213_), .c(new_n214_), .O(z18));
  inv1   g063(.a(x38), .O(new_n217_));
  nand2  g064(.a(x45), .b(x40), .O(new_n218_));
  oai21  g065(.a(x40), .b(new_n217_), .c(new_n218_), .O(z20));
  inv1   g066(.a(x39), .O(new_n220_));
  nand2  g067(.a(x44), .b(x40), .O(new_n221_));
  oai21  g068(.a(x40), .b(new_n220_), .c(new_n221_), .O(z21));
  xnor2a g069(.a(x84), .b(x81), .O(new_n223_));
  nor2   g070(.a(new_n154_), .b(x41), .O(new_n224_));
  nand3  g071(.a(new_n224_), .b(new_n223_), .c(new_n170_), .O(new_n225_));
  inv1   g072(.a(x83), .O(new_n226_));
  nand4  g073(.a(x84), .b(new_n226_), .c(x82), .d(x81), .O(new_n227_));
  inv1   g074(.a(x74), .O(new_n228_));
  nand3  g075(.a(x80), .b(new_n228_), .c(x43), .O(new_n229_));
  nor2   g076(.a(new_n159_), .b(x42), .O(new_n230_));
  oai21  g077(.a(new_n229_), .b(new_n227_), .c(new_n230_), .O(new_n231_));
  nand2  g078(.a(new_n231_), .b(x79), .O(new_n232_));
  inv1   g079(.a(x04), .O(new_n233_));
  nor2   g080(.a(new_n160_), .b(new_n233_), .O(new_n234_));
  nand2  g081(.a(new_n234_), .b(new_n232_), .O(new_n235_));
  aoi21  g082(.a(new_n235_), .b(new_n225_), .c(x01), .O(z22));
  nand3  g083(.a(new_n154_), .b(x05), .c(new_n233_), .O(new_n237_));
  nand3  g084(.a(new_n237_), .b(new_n153_), .c(x00), .O(z23));
  inv1   g085(.a(new_n161_), .O(new_n239_));
  inv1   g086(.a(x43), .O(new_n240_));
  nor2   g087(.a(x04), .b(x01), .O(new_n241_));
  nand3  g088(.a(new_n241_), .b(new_n240_), .c(x05), .O(new_n242_));
  aoi21  g089(.a(new_n239_), .b(x79), .c(new_n242_), .O(z24));
  inv1   g090(.a(x42), .O(new_n244_));
  inv1   g091(.a(x81), .O(new_n245_));
  xor2a  g092(.a(x84), .b(x82), .O(new_n246_));
  nor2   g093(.a(new_n246_), .b(new_n245_), .O(new_n247_));
  xnor2a g094(.a(x84), .b(x82), .O(new_n248_));
  nor2   g095(.a(new_n248_), .b(x81), .O(new_n249_));
  nor2   g096(.a(new_n249_), .b(new_n247_), .O(new_n250_));
  nor2   g097(.a(new_n250_), .b(new_n162_), .O(new_n251_));
  nand4  g098(.a(new_n251_), .b(new_n241_), .c(new_n244_), .d(x05), .O(new_n252_));
  inv1   g099(.a(new_n252_), .O(z25));
  nand4  g100(.a(new_n251_), .b(new_n241_), .c(x44), .d(new_n244_), .O(new_n254_));
  inv1   g101(.a(new_n254_), .O(z26));
  nand4  g102(.a(new_n251_), .b(new_n241_), .c(x45), .d(new_n244_), .O(new_n256_));
  inv1   g103(.a(new_n256_), .O(z27));
  nand4  g104(.a(new_n251_), .b(new_n241_), .c(x46), .d(new_n244_), .O(new_n258_));
  inv1   g105(.a(new_n258_), .O(z28));
  nand4  g106(.a(new_n251_), .b(new_n241_), .c(x47), .d(new_n244_), .O(new_n260_));
  inv1   g107(.a(new_n260_), .O(z29));
  nand4  g108(.a(new_n251_), .b(new_n241_), .c(x49), .d(new_n244_), .O(new_n263_));
  inv1   g109(.a(new_n263_), .O(z31));
  nand4  g110(.a(new_n251_), .b(new_n241_), .c(x50), .d(new_n244_), .O(new_n265_));
  inv1   g111(.a(new_n265_), .O(z32));
  inv1   g112(.a(new_n162_), .O(new_n268_));
  nand2  g113(.a(x83), .b(x42), .O(new_n269_));
  nand2  g114(.a(new_n269_), .b(new_n245_), .O(new_n270_));
  inv1   g115(.a(new_n269_), .O(new_n271_));
  nand2  g116(.a(new_n271_), .b(x81), .O(new_n272_));
  aoi21  g117(.a(new_n272_), .b(new_n270_), .c(new_n248_), .O(new_n273_));
  nand2  g118(.a(new_n269_), .b(x81), .O(new_n274_));
  nand2  g119(.a(new_n271_), .b(new_n245_), .O(new_n275_));
  aoi21  g120(.a(new_n275_), .b(new_n274_), .c(new_n246_), .O(new_n276_));
  oai21  g121(.a(new_n276_), .b(new_n273_), .c(new_n268_), .O(new_n277_));
  nand2  g122(.a(new_n241_), .b(x52), .O(new_n278_));
  nor2   g123(.a(new_n278_), .b(new_n277_), .O(z34));
  nand2  g124(.a(new_n241_), .b(x54), .O(new_n281_));
  nor2   g125(.a(new_n281_), .b(new_n277_), .O(z36));
  nand2  g126(.a(new_n241_), .b(x55), .O(new_n283_));
  nor2   g127(.a(new_n283_), .b(new_n277_), .O(z37));
  nand2  g128(.a(new_n241_), .b(x56), .O(new_n285_));
  nor2   g129(.a(new_n285_), .b(new_n277_), .O(z38));
  nand2  g130(.a(new_n241_), .b(x57), .O(new_n287_));
  nor2   g131(.a(new_n287_), .b(new_n277_), .O(z39));
  nand2  g132(.a(new_n241_), .b(x58), .O(new_n289_));
  nor2   g133(.a(new_n289_), .b(new_n277_), .O(z40));
  nand2  g134(.a(new_n241_), .b(x59), .O(new_n291_));
  nor2   g135(.a(new_n291_), .b(new_n277_), .O(z41));
  nand2  g136(.a(new_n241_), .b(x62), .O(new_n295_));
  nor2   g137(.a(new_n295_), .b(new_n277_), .O(z44));
  nand2  g138(.a(new_n241_), .b(x63), .O(new_n297_));
  nor2   g139(.a(new_n297_), .b(new_n277_), .O(z45));
  inv1   g140(.a(x07), .O(new_n300_));
  nand2  g141(.a(x52), .b(x15), .O(new_n301_));
  oai21  g142(.a(x52), .b(new_n300_), .c(new_n301_), .O(new_n302_));
  nor2   g143(.a(x79), .b(x77), .O(new_n303_));
  nand2  g144(.a(new_n303_), .b(new_n234_), .O(new_n304_));
  inv1   g145(.a(new_n304_), .O(new_n305_));
  nand2  g146(.a(new_n305_), .b(new_n302_), .O(new_n306_));
  inv1   g147(.a(x67), .O(new_n307_));
  nand2  g148(.a(new_n167_), .b(new_n307_), .O(new_n308_));
  nor2   g149(.a(x78), .b(new_n159_), .O(new_n309_));
  nand2  g150(.a(new_n309_), .b(x79), .O(new_n310_));
  inv1   g151(.a(new_n310_), .O(new_n311_));
  nand3  g152(.a(new_n311_), .b(new_n308_), .c(new_n223_), .O(new_n312_));
  aoi21  g153(.a(new_n312_), .b(new_n306_), .c(x01), .O(z47));
  inv1   g154(.a(x08), .O(new_n314_));
  nand2  g155(.a(x52), .b(x16), .O(new_n315_));
  oai21  g156(.a(x52), .b(new_n314_), .c(new_n315_), .O(new_n316_));
  nand2  g157(.a(new_n316_), .b(new_n305_), .O(new_n317_));
  and2   g158(.a(new_n311_), .b(new_n223_), .O(new_n318_));
  nand2  g159(.a(new_n318_), .b(x68), .O(new_n319_));
  aoi21  g160(.a(new_n319_), .b(new_n317_), .c(x01), .O(z48));
  inv1   g161(.a(x09), .O(new_n321_));
  nand2  g162(.a(x52), .b(x17), .O(new_n322_));
  oai21  g163(.a(x52), .b(new_n321_), .c(new_n322_), .O(new_n323_));
  nand2  g164(.a(new_n323_), .b(new_n305_), .O(new_n324_));
  nand2  g165(.a(new_n318_), .b(x69), .O(new_n325_));
  aoi21  g166(.a(new_n325_), .b(new_n324_), .c(x01), .O(z49));
  inv1   g167(.a(x10), .O(new_n327_));
  nand2  g168(.a(x52), .b(x18), .O(new_n328_));
  oai21  g169(.a(x52), .b(new_n327_), .c(new_n328_), .O(new_n329_));
  nand2  g170(.a(new_n329_), .b(new_n305_), .O(new_n330_));
  nand2  g171(.a(new_n318_), .b(x70), .O(new_n331_));
  aoi21  g172(.a(new_n331_), .b(new_n330_), .c(x01), .O(z50));
  inv1   g173(.a(x11), .O(new_n333_));
  nand2  g174(.a(x52), .b(x19), .O(new_n334_));
  oai21  g175(.a(x52), .b(new_n333_), .c(new_n334_), .O(new_n335_));
  nand2  g176(.a(new_n335_), .b(new_n305_), .O(new_n336_));
  nand2  g177(.a(new_n318_), .b(x71), .O(new_n337_));
  aoi21  g178(.a(new_n337_), .b(new_n336_), .c(x01), .O(z51));
  inv1   g179(.a(x12), .O(new_n339_));
  nand2  g180(.a(x52), .b(x20), .O(new_n340_));
  oai21  g181(.a(x52), .b(new_n339_), .c(new_n340_), .O(new_n341_));
  nand2  g182(.a(new_n341_), .b(new_n305_), .O(new_n342_));
  nand2  g183(.a(new_n318_), .b(x72), .O(new_n343_));
  aoi21  g184(.a(new_n343_), .b(new_n342_), .c(x01), .O(z52));
  inv1   g185(.a(x82), .O(new_n347_));
  nand4  g186(.a(x84), .b(x83), .c(new_n347_), .d(new_n245_), .O(new_n348_));
  inv1   g187(.a(x80), .O(new_n349_));
  nand4  g188(.a(new_n241_), .b(new_n161_), .c(new_n349_), .d(x79), .O(new_n350_));
  nor2   g189(.a(new_n350_), .b(new_n348_), .O(z55));
  xnor2a g190(.a(x84), .b(x81), .O(new_n352_));
  aoi21  g191(.a(new_n169_), .b(new_n168_), .c(new_n352_), .O(new_n353_));
  aoi22  g192(.a(new_n353_), .b(new_n153_), .c(new_n239_), .d(x76), .O(new_n354_));
  inv1   g193(.a(x00), .O(new_n355_));
  nor3   g194(.a(new_n163_), .b(x01), .c(new_n355_), .O(new_n356_));
  oai21  g195(.a(new_n354_), .b(new_n154_), .c(new_n356_), .O(z56));
  nand2  g196(.a(x42), .b(new_n152_), .O(new_n359_));
  nand4  g197(.a(x80), .b(new_n228_), .c(x43), .d(new_n244_), .O(new_n360_));
  oai21  g198(.a(new_n360_), .b(new_n227_), .c(new_n359_), .O(new_n361_));
  nand3  g199(.a(new_n361_), .b(new_n234_), .c(x79), .O(new_n362_));
  nand4  g200(.a(new_n154_), .b(new_n160_), .c(new_n244_), .d(x40), .O(new_n363_));
  nand2  g201(.a(new_n363_), .b(new_n362_), .O(new_n364_));
  nand2  g202(.a(new_n364_), .b(x77), .O(new_n365_));
  inv1   g203(.a(new_n168_), .O(new_n366_));
  oai21  g204(.a(new_n366_), .b(new_n233_), .c(new_n154_), .O(new_n367_));
  aoi21  g205(.a(new_n367_), .b(new_n365_), .c(x01), .O(z58));
  nand2  g206(.a(new_n154_), .b(new_n160_), .O(new_n369_));
  inv1   g207(.a(new_n234_), .O(new_n370_));
  aoi21  g208(.a(new_n370_), .b(new_n369_), .c(new_n152_), .O(new_n371_));
  oai21  g209(.a(new_n229_), .b(new_n227_), .c(new_n244_), .O(new_n372_));
  aoi21  g210(.a(new_n372_), .b(x79), .c(new_n370_), .O(new_n373_));
  oai21  g211(.a(new_n373_), .b(new_n371_), .c(x77), .O(new_n374_));
  nor2   g212(.a(x79), .b(x04), .O(new_n375_));
  inv1   g213(.a(new_n375_), .O(new_n376_));
  aoi21  g214(.a(new_n376_), .b(new_n374_), .c(x01), .O(z59));
  aoi21  g215(.a(new_n353_), .b(x79), .c(new_n375_), .O(new_n378_));
  aoi21  g216(.a(new_n378_), .b(new_n235_), .c(x01), .O(z60));
  inv1   g217(.a(new_n171_), .O(new_n380_));
  nand2  g218(.a(new_n169_), .b(new_n168_), .O(new_n381_));
  aoi22  g219(.a(new_n381_), .b(new_n223_), .c(new_n161_), .d(new_n233_), .O(new_n382_));
  nor3   g220(.a(new_n382_), .b(new_n380_), .c(new_n349_), .O(z61));
  nor3   g221(.a(new_n382_), .b(new_n380_), .c(new_n347_), .O(z63));
  inv1   g222(.a(new_n382_), .O(new_n386_));
  nand3  g223(.a(new_n386_), .b(x83), .c(x79), .O(new_n387_));
  aoi21  g224(.a(new_n387_), .b(new_n304_), .c(x01), .O(z64));
  nor2   g225(.a(new_n160_), .b(x04), .O(new_n389_));
  nor2   g226(.a(new_n245_), .b(x78), .O(new_n390_));
  oai21  g227(.a(new_n390_), .b(new_n389_), .c(x77), .O(new_n391_));
  nand2  g228(.a(new_n366_), .b(x81), .O(new_n392_));
  nand2  g229(.a(new_n171_), .b(x84), .O(new_n393_));
  aoi21  g230(.a(new_n392_), .b(new_n391_), .c(new_n393_), .O(z65));
  zero   g231(.O(z06));
  zero   g232(.O(z19));
  zero   g233(.O(z30));
  zero   g234(.O(z33));
  zero   g235(.O(z35));
  zero   g236(.O(z42));
  zero   g237(.O(z43));
  zero   g238(.O(z46));
  zero   g239(.O(z53));
  zero   g240(.O(z54));
  zero   g241(.O(z57));
  zero   g242(.O(z62));
endmodule


