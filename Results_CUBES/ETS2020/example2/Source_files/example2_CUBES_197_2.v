// Benchmark "FAU" written by ABC on Fri Jul 10 18:23:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n190_, new_n191_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n201_, new_n202_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n226_, new_n227_, new_n228_, new_n230_, new_n231_, new_n232_,
    new_n234_, new_n235_, new_n236_, new_n241_, new_n242_, new_n243_,
    new_n245_, new_n246_, new_n247_, new_n248_, new_n249_, new_n250_,
    new_n251_, new_n252_, new_n253_, new_n254_, new_n256_, new_n257_,
    new_n258_, new_n259_, new_n260_, new_n261_, new_n262_, new_n263_,
    new_n264_, new_n265_, new_n267_, new_n271_, new_n273_, new_n275_,
    new_n278_, new_n280_, new_n282_, new_n284_, new_n286_, new_n288_,
    new_n289_, new_n290_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n298_, new_n299_, new_n300_, new_n301_,
    new_n302_, new_n304_, new_n305_, new_n306_, new_n307_, new_n308_,
    new_n310_, new_n311_, new_n312_, new_n313_, new_n314_, new_n316_,
    new_n317_, new_n318_, new_n319_, new_n320_, new_n322_, new_n323_,
    new_n324_, new_n325_, new_n326_, new_n328_, new_n329_, new_n330_,
    new_n331_, new_n332_, new_n334_, new_n335_, new_n336_, new_n337_,
    new_n341_, new_n342_, new_n347_, new_n348_, new_n349_;
  inv1   g000(.a(x77), .O(new_n153_));
  inv1   g001(.a(x78), .O(new_n154_));
  nand2  g002(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  inv1   g003(.a(x79), .O(new_n156_));
  nor2   g004(.a(new_n154_), .b(new_n153_), .O(new_n157_));
  nor2   g005(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  aoi21  g006(.a(new_n158_), .b(new_n155_), .c(x01), .O(z01));
  nor2   g007(.a(new_n154_), .b(x77), .O(new_n160_));
  nand2  g008(.a(new_n160_), .b(x75), .O(new_n161_));
  nor2   g009(.a(x78), .b(new_n153_), .O(new_n162_));
  nand2  g010(.a(new_n162_), .b(x66), .O(new_n163_));
  inv1   g011(.a(x01), .O(new_n164_));
  nand2  g012(.a(x79), .b(new_n164_), .O(new_n165_));
  aoi21  g013(.a(new_n163_), .b(new_n161_), .c(new_n165_), .O(z02));
  nor2   g014(.a(x79), .b(new_n154_), .O(new_n167_));
  nand3  g015(.a(new_n167_), .b(x52), .c(new_n164_), .O(new_n168_));
  inv1   g016(.a(new_n168_), .O(z03));
  inv1   g017(.a(x23), .O(new_n171_));
  nand2  g018(.a(x65), .b(x40), .O(new_n172_));
  oai21  g019(.a(x40), .b(new_n171_), .c(new_n172_), .O(z05));
  inv1   g020(.a(x24), .O(new_n174_));
  nand2  g021(.a(x64), .b(x40), .O(new_n175_));
  oai21  g022(.a(x40), .b(new_n174_), .c(new_n175_), .O(z06));
  inv1   g023(.a(x25), .O(new_n177_));
  nand2  g024(.a(x63), .b(x40), .O(new_n178_));
  oai21  g025(.a(x40), .b(new_n177_), .c(new_n178_), .O(z07));
  inv1   g026(.a(x30), .O(new_n184_));
  nand2  g027(.a(x58), .b(x40), .O(new_n185_));
  oai21  g028(.a(x40), .b(new_n184_), .c(new_n185_), .O(z12));
  inv1   g029(.a(x31), .O(new_n187_));
  nand2  g030(.a(x57), .b(x40), .O(new_n188_));
  oai21  g031(.a(x40), .b(new_n187_), .c(new_n188_), .O(z13));
  inv1   g032(.a(x32), .O(new_n190_));
  nand2  g033(.a(x51), .b(x40), .O(new_n191_));
  oai21  g034(.a(x40), .b(new_n190_), .c(new_n191_), .O(z14));
  inv1   g035(.a(x34), .O(new_n194_));
  nand2  g036(.a(x49), .b(x40), .O(new_n195_));
  oai21  g037(.a(x40), .b(new_n194_), .c(new_n195_), .O(z16));
  inv1   g038(.a(x35), .O(new_n197_));
  nand2  g039(.a(x48), .b(x40), .O(new_n198_));
  oai21  g040(.a(x40), .b(new_n197_), .c(new_n198_), .O(z17));
  inv1   g041(.a(x37), .O(new_n201_));
  nand2  g042(.a(x46), .b(x40), .O(new_n202_));
  oai21  g043(.a(x40), .b(new_n201_), .c(new_n202_), .O(z19));
  inv1   g044(.a(x38), .O(new_n204_));
  nand2  g045(.a(x45), .b(x40), .O(new_n205_));
  oai21  g046(.a(x40), .b(new_n204_), .c(new_n205_), .O(z20));
  inv1   g047(.a(x39), .O(new_n207_));
  nand2  g048(.a(x44), .b(x40), .O(new_n208_));
  oai21  g049(.a(x40), .b(new_n207_), .c(new_n208_), .O(z21));
  inv1   g050(.a(x81), .O(new_n213_));
  xor2a  g051(.a(x84), .b(x82), .O(new_n214_));
  xor2a  g052(.a(x84), .b(x82), .O(new_n215_));
  nand2  g053(.a(new_n215_), .b(new_n213_), .O(new_n216_));
  oai21  g054(.a(new_n214_), .b(new_n213_), .c(new_n216_), .O(new_n217_));
  nand3  g055(.a(x79), .b(x78), .c(x77), .O(new_n218_));
  inv1   g056(.a(new_n218_), .O(new_n219_));
  inv1   g057(.a(x42), .O(new_n220_));
  nor2   g058(.a(x04), .b(x01), .O(new_n221_));
  nand3  g059(.a(new_n221_), .b(new_n220_), .c(x05), .O(new_n222_));
  inv1   g060(.a(new_n222_), .O(new_n223_));
  nand3  g061(.a(new_n223_), .b(new_n219_), .c(new_n217_), .O(new_n224_));
  inv1   g062(.a(new_n224_), .O(z25));
  nand3  g063(.a(new_n221_), .b(x44), .c(new_n220_), .O(new_n226_));
  inv1   g064(.a(new_n226_), .O(new_n227_));
  nand3  g065(.a(new_n227_), .b(new_n219_), .c(new_n217_), .O(new_n228_));
  inv1   g066(.a(new_n228_), .O(z26));
  nand3  g067(.a(new_n221_), .b(x45), .c(new_n220_), .O(new_n230_));
  inv1   g068(.a(new_n230_), .O(new_n231_));
  nand3  g069(.a(new_n231_), .b(new_n219_), .c(new_n217_), .O(new_n232_));
  inv1   g070(.a(new_n232_), .O(z27));
  nand3  g071(.a(new_n221_), .b(x46), .c(new_n220_), .O(new_n234_));
  inv1   g072(.a(new_n234_), .O(new_n235_));
  nand3  g073(.a(new_n235_), .b(new_n219_), .c(new_n217_), .O(new_n236_));
  inv1   g074(.a(new_n236_), .O(z28));
  nand3  g075(.a(new_n221_), .b(x50), .c(new_n220_), .O(new_n241_));
  inv1   g076(.a(new_n241_), .O(new_n242_));
  nand3  g077(.a(new_n242_), .b(new_n219_), .c(new_n217_), .O(new_n243_));
  inv1   g078(.a(new_n243_), .O(z32));
  inv1   g079(.a(new_n214_), .O(new_n245_));
  xnor2a g080(.a(x83), .b(x81), .O(new_n246_));
  nand2  g081(.a(x42), .b(x05), .O(new_n247_));
  nand2  g082(.a(x51), .b(new_n220_), .O(new_n248_));
  oai22  g083(.a(new_n248_), .b(new_n213_), .c(new_n247_), .d(new_n246_), .O(new_n249_));
  nand2  g084(.a(new_n249_), .b(new_n245_), .O(new_n250_));
  xor2a  g085(.a(x83), .b(x81), .O(new_n251_));
  oai22  g086(.a(new_n251_), .b(new_n247_), .c(new_n248_), .d(x81), .O(new_n252_));
  nand2  g087(.a(new_n252_), .b(new_n215_), .O(new_n253_));
  nand2  g088(.a(new_n221_), .b(new_n219_), .O(new_n254_));
  aoi21  g089(.a(new_n253_), .b(new_n250_), .c(new_n254_), .O(z33));
  xnor2a g090(.a(x84), .b(x82), .O(new_n256_));
  nand2  g091(.a(x83), .b(x42), .O(new_n257_));
  nand2  g092(.a(new_n257_), .b(new_n213_), .O(new_n258_));
  nand3  g093(.a(x83), .b(x81), .c(x42), .O(new_n259_));
  aoi21  g094(.a(new_n259_), .b(new_n258_), .c(new_n256_), .O(new_n260_));
  nand2  g095(.a(new_n257_), .b(x81), .O(new_n261_));
  nand3  g096(.a(x83), .b(new_n213_), .c(x42), .O(new_n262_));
  aoi21  g097(.a(new_n262_), .b(new_n261_), .c(new_n214_), .O(new_n263_));
  oai21  g098(.a(new_n263_), .b(new_n260_), .c(new_n219_), .O(new_n264_));
  nand2  g099(.a(new_n221_), .b(x52), .O(new_n265_));
  nor2   g100(.a(new_n265_), .b(new_n264_), .O(z34));
  nand2  g101(.a(new_n221_), .b(x53), .O(new_n267_));
  nor2   g102(.a(new_n267_), .b(new_n264_), .O(z35));
  nand2  g103(.a(new_n221_), .b(x56), .O(new_n271_));
  nor2   g104(.a(new_n271_), .b(new_n264_), .O(z38));
  nand2  g105(.a(new_n221_), .b(x57), .O(new_n273_));
  nor2   g106(.a(new_n273_), .b(new_n264_), .O(z39));
  nand2  g107(.a(new_n221_), .b(x58), .O(new_n275_));
  nor2   g108(.a(new_n275_), .b(new_n264_), .O(z40));
  nand2  g109(.a(new_n221_), .b(x60), .O(new_n278_));
  nor2   g110(.a(new_n278_), .b(new_n264_), .O(z42));
  nand2  g111(.a(new_n221_), .b(x61), .O(new_n280_));
  nor2   g112(.a(new_n280_), .b(new_n264_), .O(z43));
  nand2  g113(.a(new_n221_), .b(x62), .O(new_n282_));
  nor2   g114(.a(new_n282_), .b(new_n264_), .O(z44));
  nand2  g115(.a(new_n221_), .b(x63), .O(new_n284_));
  nor2   g116(.a(new_n284_), .b(new_n264_), .O(z45));
  nand2  g117(.a(new_n221_), .b(x64), .O(new_n286_));
  nor2   g118(.a(new_n286_), .b(new_n264_), .O(z46));
  inv1   g119(.a(x07), .O(new_n288_));
  nand2  g120(.a(x52), .b(x15), .O(new_n289_));
  oai21  g121(.a(x52), .b(new_n288_), .c(new_n289_), .O(new_n290_));
  inv1   g122(.a(x04), .O(new_n291_));
  nor2   g123(.a(x77), .b(new_n291_), .O(new_n292_));
  nand3  g124(.a(new_n292_), .b(new_n290_), .c(new_n167_), .O(new_n293_));
  xnor2a g125(.a(x84), .b(x81), .O(new_n294_));
  or2    g126(.a(x75), .b(x67), .O(new_n295_));
  nand4  g127(.a(new_n295_), .b(new_n294_), .c(new_n162_), .d(x79), .O(new_n296_));
  aoi21  g128(.a(new_n296_), .b(new_n293_), .c(x01), .O(z47));
  inv1   g129(.a(x08), .O(new_n298_));
  nand2  g130(.a(x52), .b(x16), .O(new_n299_));
  oai21  g131(.a(x52), .b(new_n298_), .c(new_n299_), .O(new_n300_));
  nand3  g132(.a(new_n300_), .b(new_n292_), .c(new_n167_), .O(new_n301_));
  nand4  g133(.a(new_n294_), .b(new_n162_), .c(x79), .d(x68), .O(new_n302_));
  aoi21  g134(.a(new_n302_), .b(new_n301_), .c(x01), .O(z48));
  inv1   g135(.a(x09), .O(new_n304_));
  nand2  g136(.a(x52), .b(x17), .O(new_n305_));
  oai21  g137(.a(x52), .b(new_n304_), .c(new_n305_), .O(new_n306_));
  nand3  g138(.a(new_n306_), .b(new_n292_), .c(new_n167_), .O(new_n307_));
  nand4  g139(.a(new_n294_), .b(new_n162_), .c(x79), .d(x69), .O(new_n308_));
  aoi21  g140(.a(new_n308_), .b(new_n307_), .c(x01), .O(z49));
  inv1   g141(.a(x10), .O(new_n310_));
  nand2  g142(.a(x52), .b(x18), .O(new_n311_));
  oai21  g143(.a(x52), .b(new_n310_), .c(new_n311_), .O(new_n312_));
  nand3  g144(.a(new_n312_), .b(new_n292_), .c(new_n167_), .O(new_n313_));
  nand4  g145(.a(new_n294_), .b(new_n162_), .c(x79), .d(x70), .O(new_n314_));
  aoi21  g146(.a(new_n314_), .b(new_n313_), .c(x01), .O(z50));
  inv1   g147(.a(x11), .O(new_n316_));
  nand2  g148(.a(x52), .b(x19), .O(new_n317_));
  oai21  g149(.a(x52), .b(new_n316_), .c(new_n317_), .O(new_n318_));
  nand3  g150(.a(new_n318_), .b(new_n292_), .c(new_n167_), .O(new_n319_));
  nand4  g151(.a(new_n294_), .b(new_n162_), .c(x79), .d(x71), .O(new_n320_));
  aoi21  g152(.a(new_n320_), .b(new_n319_), .c(x01), .O(z51));
  inv1   g153(.a(x12), .O(new_n322_));
  nand2  g154(.a(x52), .b(x20), .O(new_n323_));
  oai21  g155(.a(x52), .b(new_n322_), .c(new_n323_), .O(new_n324_));
  nand3  g156(.a(new_n324_), .b(new_n292_), .c(new_n167_), .O(new_n325_));
  nand4  g157(.a(new_n294_), .b(new_n162_), .c(x79), .d(x72), .O(new_n326_));
  aoi21  g158(.a(new_n326_), .b(new_n325_), .c(x01), .O(z52));
  inv1   g159(.a(x13), .O(new_n328_));
  nand2  g160(.a(x52), .b(x21), .O(new_n329_));
  oai21  g161(.a(x52), .b(new_n328_), .c(new_n329_), .O(new_n330_));
  nand3  g162(.a(new_n330_), .b(new_n292_), .c(new_n167_), .O(new_n331_));
  nand4  g163(.a(new_n294_), .b(new_n162_), .c(x79), .d(x73), .O(new_n332_));
  aoi21  g164(.a(new_n332_), .b(new_n331_), .c(x01), .O(z53));
  inv1   g165(.a(x14), .O(new_n334_));
  nor2   g166(.a(x52), .b(new_n334_), .O(new_n335_));
  aoi21  g167(.a(x52), .b(x22), .c(new_n335_), .O(new_n336_));
  nand4  g168(.a(new_n160_), .b(new_n156_), .c(x04), .d(new_n164_), .O(new_n337_));
  nor2   g169(.a(new_n337_), .b(new_n336_), .O(z54));
  inv1   g170(.a(x02), .O(new_n341_));
  nand4  g171(.a(x03), .b(new_n341_), .c(new_n164_), .d(x00), .O(new_n342_));
  inv1   g172(.a(new_n342_), .O(z57));
  oai21  g173(.a(new_n162_), .b(new_n160_), .c(new_n294_), .O(new_n347_));
  nand2  g174(.a(new_n157_), .b(new_n291_), .O(new_n348_));
  nand3  g175(.a(x80), .b(x79), .c(new_n164_), .O(new_n349_));
  aoi21  g176(.a(new_n348_), .b(new_n347_), .c(new_n349_), .O(z61));
  zero   g177(.O(z00));
  zero   g178(.O(z04));
  zero   g179(.O(z08));
  zero   g180(.O(z09));
  zero   g181(.O(z10));
  zero   g182(.O(z11));
  zero   g183(.O(z15));
  zero   g184(.O(z18));
  zero   g185(.O(z22));
  zero   g186(.O(z23));
  zero   g187(.O(z24));
  zero   g188(.O(z29));
  zero   g189(.O(z30));
  zero   g190(.O(z31));
  zero   g191(.O(z36));
  zero   g192(.O(z37));
  zero   g193(.O(z41));
  zero   g194(.O(z55));
  zero   g195(.O(z56));
  zero   g196(.O(z58));
  zero   g197(.O(z59));
  zero   g198(.O(z60));
  zero   g199(.O(z62));
  zero   g200(.O(z63));
  zero   g201(.O(z64));
  zero   g202(.O(z65));
endmodule


