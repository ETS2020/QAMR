// Benchmark "FAU" written by ABC on Fri Jul 10 18:17:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n177_, new_n178_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n196_, new_n197_, new_n199_, new_n200_, new_n202_,
    new_n203_, new_n207_, new_n208_, new_n211_, new_n212_, new_n214_,
    new_n215_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n230_, new_n231_, new_n232_, new_n233_, new_n237_,
    new_n238_, new_n239_, new_n240_, new_n241_, new_n242_, new_n243_,
    new_n244_, new_n245_, new_n246_, new_n248_, new_n250_, new_n253_,
    new_n255_, new_n257_, new_n259_, new_n260_, new_n261_, new_n262_,
    new_n263_, new_n264_, new_n265_, new_n266_, new_n267_, new_n268_,
    new_n270_, new_n271_, new_n272_, new_n273_, new_n274_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n282_,
    new_n284_, new_n286_, new_n288_, new_n298_, new_n299_, new_n300_,
    new_n301_, new_n302_, new_n303_, new_n304_, new_n305_, new_n306_,
    new_n307_, new_n308_, new_n310_, new_n311_, new_n312_, new_n313_,
    new_n314_, new_n315_, new_n318_, new_n319_, new_n320_, new_n321_,
    new_n322_, new_n324_, new_n325_, new_n326_, new_n327_, new_n328_,
    new_n332_, new_n333_, new_n334_, new_n337_, new_n338_, new_n339_,
    new_n340_, new_n341_, new_n342_, new_n343_, new_n344_, new_n345_,
    new_n347_, new_n348_, new_n351_, new_n352_, new_n353_, new_n354_,
    new_n355_, new_n356_, new_n357_, new_n359_, new_n362_, new_n363_,
    new_n364_, new_n365_, new_n366_, new_n367_, new_n368_, new_n369_,
    new_n370_, new_n371_, new_n372_, new_n374_, new_n375_, new_n376_,
    new_n377_, new_n379_, new_n381_, new_n382_, new_n383_, new_n384_,
    new_n385_;
  inv1   g000(.a(x40), .O(new_n152_));
  inv1   g001(.a(x01), .O(new_n153_));
  inv1   g002(.a(x79), .O(new_n154_));
  nand3  g003(.a(new_n154_), .b(x78), .c(x77), .O(new_n155_));
  aoi21  g004(.a(new_n155_), .b(new_n153_), .c(x52), .O(new_n156_));
  nand2  g005(.a(new_n152_), .b(x06), .O(new_n157_));
  oai21  g006(.a(new_n156_), .b(new_n152_), .c(new_n157_), .O(z00));
  inv1   g007(.a(x77), .O(new_n159_));
  inv1   g008(.a(x78), .O(new_n160_));
  nor2   g009(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g010(.a(new_n161_), .b(x79), .O(new_n162_));
  aoi21  g011(.a(new_n160_), .b(new_n159_), .c(new_n154_), .O(new_n163_));
  aoi21  g012(.a(new_n163_), .b(new_n162_), .c(x01), .O(z01));
  inv1   g013(.a(x75), .O(new_n165_));
  nor2   g014(.a(new_n160_), .b(x77), .O(new_n166_));
  inv1   g015(.a(new_n166_), .O(new_n167_));
  nor2   g016(.a(x78), .b(new_n159_), .O(new_n168_));
  nand2  g017(.a(new_n168_), .b(x66), .O(new_n169_));
  oai21  g018(.a(new_n167_), .b(new_n165_), .c(new_n169_), .O(new_n170_));
  nor2   g019(.a(new_n154_), .b(x01), .O(new_n171_));
  and2   g020(.a(new_n171_), .b(new_n170_), .O(z02));
  nand4  g021(.a(new_n154_), .b(x78), .c(x52), .d(new_n153_), .O(new_n173_));
  inv1   g022(.a(new_n173_), .O(z03));
  aoi21  g023(.a(new_n161_), .b(new_n154_), .c(x01), .O(z04));
  inv1   g024(.a(x64), .O(new_n177_));
  nand2  g025(.a(new_n152_), .b(x24), .O(new_n178_));
  oai21  g026(.a(new_n177_), .b(new_n152_), .c(new_n178_), .O(z06));
  inv1   g027(.a(x25), .O(new_n180_));
  nand2  g028(.a(x63), .b(x40), .O(new_n181_));
  oai21  g029(.a(x40), .b(new_n180_), .c(new_n181_), .O(z07));
  inv1   g030(.a(x26), .O(new_n183_));
  nand2  g031(.a(x62), .b(x40), .O(new_n184_));
  oai21  g032(.a(x40), .b(new_n183_), .c(new_n184_), .O(z08));
  inv1   g033(.a(x27), .O(new_n186_));
  nand2  g034(.a(x61), .b(x40), .O(new_n187_));
  oai21  g035(.a(x40), .b(new_n186_), .c(new_n187_), .O(z09));
  inv1   g036(.a(x60), .O(new_n189_));
  nand2  g037(.a(new_n152_), .b(x28), .O(new_n190_));
  oai21  g038(.a(new_n189_), .b(new_n152_), .c(new_n190_), .O(z10));
  inv1   g039(.a(x59), .O(new_n192_));
  nand2  g040(.a(new_n152_), .b(x29), .O(new_n193_));
  oai21  g041(.a(new_n192_), .b(new_n152_), .c(new_n193_), .O(z11));
  inv1   g042(.a(x57), .O(new_n196_));
  nand2  g043(.a(new_n152_), .b(x31), .O(new_n197_));
  oai21  g044(.a(new_n196_), .b(new_n152_), .c(new_n197_), .O(z13));
  inv1   g045(.a(x32), .O(new_n199_));
  nand2  g046(.a(x51), .b(x40), .O(new_n200_));
  oai21  g047(.a(x40), .b(new_n199_), .c(new_n200_), .O(z14));
  inv1   g048(.a(x33), .O(new_n202_));
  nand2  g049(.a(x50), .b(x40), .O(new_n203_));
  oai21  g050(.a(x40), .b(new_n202_), .c(new_n203_), .O(z15));
  inv1   g051(.a(x36), .O(new_n207_));
  nand2  g052(.a(x47), .b(x40), .O(new_n208_));
  oai21  g053(.a(x40), .b(new_n207_), .c(new_n208_), .O(z18));
  inv1   g054(.a(x38), .O(new_n211_));
  nand2  g055(.a(x45), .b(x40), .O(new_n212_));
  oai21  g056(.a(x40), .b(new_n211_), .c(new_n212_), .O(z20));
  inv1   g057(.a(x39), .O(new_n214_));
  nand2  g058(.a(x44), .b(x40), .O(new_n215_));
  oai21  g059(.a(x40), .b(new_n214_), .c(new_n215_), .O(z21));
  xor2a  g060(.a(x84), .b(x81), .O(new_n217_));
  nor3   g061(.a(new_n217_), .b(new_n154_), .c(x41), .O(new_n218_));
  nand2  g062(.a(new_n218_), .b(new_n170_), .O(new_n219_));
  inv1   g063(.a(x83), .O(new_n220_));
  nand3  g064(.a(x84), .b(new_n220_), .c(x82), .O(new_n221_));
  inv1   g065(.a(x74), .O(new_n222_));
  nand4  g066(.a(x81), .b(x80), .c(new_n222_), .d(x43), .O(new_n223_));
  nor2   g067(.a(new_n223_), .b(new_n221_), .O(new_n224_));
  nor3   g068(.a(new_n224_), .b(new_n159_), .c(x42), .O(new_n225_));
  nand2  g069(.a(x78), .b(x04), .O(new_n226_));
  inv1   g070(.a(new_n226_), .O(new_n227_));
  oai21  g071(.a(new_n225_), .b(new_n154_), .c(new_n227_), .O(new_n228_));
  aoi21  g072(.a(new_n228_), .b(new_n219_), .c(x01), .O(z22));
  inv1   g073(.a(x04), .O(new_n230_));
  nand3  g074(.a(new_n154_), .b(x05), .c(new_n230_), .O(new_n231_));
  inv1   g075(.a(x00), .O(new_n232_));
  nor2   g076(.a(x01), .b(new_n232_), .O(new_n233_));
  nand2  g077(.a(new_n233_), .b(new_n231_), .O(z23));
  inv1   g078(.a(x42), .O(new_n237_));
  xor2a  g079(.a(x84), .b(x82), .O(new_n238_));
  inv1   g080(.a(new_n238_), .O(new_n239_));
  nand2  g081(.a(new_n239_), .b(x81), .O(new_n240_));
  inv1   g082(.a(x81), .O(new_n241_));
  xor2a  g083(.a(x84), .b(x82), .O(new_n242_));
  nand2  g084(.a(new_n242_), .b(new_n241_), .O(new_n243_));
  aoi21  g085(.a(new_n243_), .b(new_n240_), .c(new_n162_), .O(new_n244_));
  nor2   g086(.a(x04), .b(x01), .O(new_n245_));
  nand4  g087(.a(new_n245_), .b(new_n244_), .c(x44), .d(new_n237_), .O(new_n246_));
  inv1   g088(.a(new_n246_), .O(z26));
  nand4  g089(.a(new_n245_), .b(new_n244_), .c(x45), .d(new_n237_), .O(new_n248_));
  inv1   g090(.a(new_n248_), .O(z27));
  nand4  g091(.a(new_n245_), .b(new_n244_), .c(x46), .d(new_n237_), .O(new_n250_));
  inv1   g092(.a(new_n250_), .O(z28));
  nand4  g093(.a(new_n245_), .b(new_n244_), .c(x48), .d(new_n237_), .O(new_n253_));
  inv1   g094(.a(new_n253_), .O(z30));
  nand4  g095(.a(new_n245_), .b(new_n244_), .c(x49), .d(new_n237_), .O(new_n255_));
  inv1   g096(.a(new_n255_), .O(z31));
  nand4  g097(.a(new_n245_), .b(new_n244_), .c(x50), .d(new_n237_), .O(new_n257_));
  inv1   g098(.a(new_n257_), .O(z32));
  xnor2a g099(.a(x83), .b(x81), .O(new_n259_));
  nand2  g100(.a(x42), .b(x05), .O(new_n260_));
  nand2  g101(.a(x51), .b(new_n237_), .O(new_n261_));
  oai22  g102(.a(new_n261_), .b(new_n241_), .c(new_n260_), .d(new_n259_), .O(new_n262_));
  nand2  g103(.a(new_n262_), .b(new_n239_), .O(new_n263_));
  xor2a  g104(.a(x83), .b(x81), .O(new_n264_));
  oai22  g105(.a(new_n264_), .b(new_n260_), .c(new_n261_), .d(x81), .O(new_n265_));
  nand2  g106(.a(new_n265_), .b(new_n242_), .O(new_n266_));
  inv1   g107(.a(new_n162_), .O(new_n267_));
  nand2  g108(.a(new_n245_), .b(new_n267_), .O(new_n268_));
  aoi21  g109(.a(new_n266_), .b(new_n263_), .c(new_n268_), .O(z33));
  inv1   g110(.a(x52), .O(new_n270_));
  inv1   g111(.a(new_n245_), .O(new_n271_));
  nand2  g112(.a(x83), .b(x42), .O(new_n272_));
  nand2  g113(.a(new_n272_), .b(new_n241_), .O(new_n273_));
  nand3  g114(.a(x83), .b(x81), .c(x42), .O(new_n274_));
  nand2  g115(.a(new_n274_), .b(new_n273_), .O(new_n275_));
  and2   g116(.a(new_n275_), .b(new_n242_), .O(new_n276_));
  nand2  g117(.a(new_n272_), .b(x81), .O(new_n277_));
  nand3  g118(.a(x83), .b(new_n241_), .c(x42), .O(new_n278_));
  aoi21  g119(.a(new_n278_), .b(new_n277_), .c(new_n238_), .O(new_n279_));
  oai21  g120(.a(new_n279_), .b(new_n276_), .c(new_n267_), .O(new_n280_));
  nor3   g121(.a(new_n280_), .b(new_n271_), .c(new_n270_), .O(z34));
  nand2  g122(.a(new_n245_), .b(x53), .O(new_n282_));
  nor2   g123(.a(new_n282_), .b(new_n280_), .O(z35));
  nand2  g124(.a(new_n245_), .b(x54), .O(new_n284_));
  nor2   g125(.a(new_n284_), .b(new_n280_), .O(z36));
  nand2  g126(.a(new_n245_), .b(x55), .O(new_n286_));
  nor2   g127(.a(new_n286_), .b(new_n280_), .O(z37));
  nand2  g128(.a(new_n245_), .b(x56), .O(new_n288_));
  nor2   g129(.a(new_n288_), .b(new_n280_), .O(z38));
  nor3   g130(.a(new_n280_), .b(new_n271_), .c(new_n196_), .O(z39));
  nor3   g131(.a(new_n280_), .b(new_n271_), .c(new_n192_), .O(z41));
  nor3   g132(.a(new_n280_), .b(new_n271_), .c(new_n189_), .O(z42));
  nor3   g133(.a(new_n280_), .b(new_n271_), .c(new_n177_), .O(z46));
  inv1   g134(.a(x07), .O(new_n298_));
  nand2  g135(.a(x52), .b(x15), .O(new_n299_));
  oai21  g136(.a(x52), .b(new_n298_), .c(new_n299_), .O(new_n300_));
  nand3  g137(.a(new_n227_), .b(new_n154_), .c(new_n159_), .O(new_n301_));
  inv1   g138(.a(new_n301_), .O(new_n302_));
  nand2  g139(.a(new_n302_), .b(new_n300_), .O(new_n303_));
  inv1   g140(.a(new_n217_), .O(new_n304_));
  nor2   g141(.a(x75), .b(x67), .O(new_n305_));
  nand2  g142(.a(new_n168_), .b(x79), .O(new_n306_));
  nor2   g143(.a(new_n306_), .b(new_n305_), .O(new_n307_));
  nand2  g144(.a(new_n307_), .b(new_n304_), .O(new_n308_));
  aoi21  g145(.a(new_n308_), .b(new_n303_), .c(x01), .O(z47));
  inv1   g146(.a(x08), .O(new_n310_));
  nand2  g147(.a(x52), .b(x16), .O(new_n311_));
  oai21  g148(.a(x52), .b(new_n310_), .c(new_n311_), .O(new_n312_));
  nand2  g149(.a(new_n312_), .b(new_n302_), .O(new_n313_));
  nor2   g150(.a(new_n306_), .b(new_n217_), .O(new_n314_));
  nand2  g151(.a(new_n314_), .b(x68), .O(new_n315_));
  aoi21  g152(.a(new_n315_), .b(new_n313_), .c(x01), .O(z48));
  inv1   g153(.a(x10), .O(new_n318_));
  nand2  g154(.a(x52), .b(x18), .O(new_n319_));
  oai21  g155(.a(x52), .b(new_n318_), .c(new_n319_), .O(new_n320_));
  nand2  g156(.a(new_n320_), .b(new_n302_), .O(new_n321_));
  nand2  g157(.a(new_n314_), .b(x70), .O(new_n322_));
  aoi21  g158(.a(new_n322_), .b(new_n321_), .c(x01), .O(z50));
  inv1   g159(.a(x11), .O(new_n324_));
  nand2  g160(.a(x52), .b(x19), .O(new_n325_));
  oai21  g161(.a(x52), .b(new_n324_), .c(new_n325_), .O(new_n326_));
  nand2  g162(.a(new_n326_), .b(new_n302_), .O(new_n327_));
  nand2  g163(.a(new_n314_), .b(x71), .O(new_n328_));
  aoi21  g164(.a(new_n328_), .b(new_n327_), .c(x01), .O(z51));
  nand2  g165(.a(x52), .b(x22), .O(new_n332_));
  nand2  g166(.a(new_n270_), .b(x14), .O(new_n333_));
  nand4  g167(.a(new_n166_), .b(new_n154_), .c(x04), .d(new_n153_), .O(new_n334_));
  aoi21  g168(.a(new_n333_), .b(new_n332_), .c(new_n334_), .O(z54));
  inv1   g169(.a(new_n161_), .O(new_n337_));
  nand2  g170(.a(new_n337_), .b(x76), .O(new_n338_));
  inv1   g171(.a(new_n168_), .O(new_n339_));
  xnor2a g172(.a(x84), .b(x81), .O(new_n340_));
  aoi21  g173(.a(new_n339_), .b(new_n167_), .c(new_n340_), .O(new_n341_));
  nand2  g174(.a(new_n341_), .b(new_n153_), .O(new_n342_));
  nand2  g175(.a(new_n342_), .b(new_n338_), .O(new_n343_));
  nand2  g176(.a(new_n343_), .b(x79), .O(new_n344_));
  nand3  g177(.a(new_n160_), .b(new_n159_), .c(new_n153_), .O(new_n345_));
  nand3  g178(.a(new_n345_), .b(new_n344_), .c(new_n233_), .O(z56));
  inv1   g179(.a(x02), .O(new_n347_));
  nand3  g180(.a(new_n233_), .b(x03), .c(new_n347_), .O(new_n348_));
  inv1   g181(.a(new_n348_), .O(z57));
  nand2  g182(.a(new_n154_), .b(new_n160_), .O(new_n351_));
  aoi21  g183(.a(new_n226_), .b(new_n351_), .c(new_n152_), .O(new_n352_));
  oai21  g184(.a(new_n223_), .b(new_n221_), .c(new_n237_), .O(new_n353_));
  aoi21  g185(.a(new_n353_), .b(x79), .c(new_n226_), .O(new_n354_));
  oai21  g186(.a(new_n354_), .b(new_n352_), .c(x77), .O(new_n355_));
  nor2   g187(.a(x79), .b(x04), .O(new_n356_));
  inv1   g188(.a(new_n356_), .O(new_n357_));
  aoi21  g189(.a(new_n357_), .b(new_n355_), .c(x01), .O(z59));
  aoi21  g190(.a(new_n341_), .b(x79), .c(new_n356_), .O(new_n359_));
  aoi21  g191(.a(new_n359_), .b(new_n228_), .c(x01), .O(z60));
  nand3  g192(.a(x84), .b(x81), .c(x79), .O(new_n362_));
  oai21  g193(.a(x79), .b(new_n230_), .c(new_n362_), .O(new_n363_));
  nand2  g194(.a(new_n363_), .b(new_n159_), .O(new_n364_));
  nand2  g195(.a(new_n353_), .b(x79), .O(new_n365_));
  nand3  g196(.a(x81), .b(x79), .c(new_n230_), .O(new_n366_));
  inv1   g197(.a(new_n366_), .O(new_n367_));
  aoi21  g198(.a(new_n365_), .b(x04), .c(new_n367_), .O(new_n368_));
  oai21  g199(.a(new_n368_), .b(new_n159_), .c(new_n364_), .O(new_n369_));
  nand2  g200(.a(new_n369_), .b(x78), .O(new_n370_));
  inv1   g201(.a(new_n362_), .O(new_n371_));
  nand2  g202(.a(new_n371_), .b(new_n168_), .O(new_n372_));
  aoi21  g203(.a(new_n372_), .b(new_n370_), .c(x01), .O(z62));
  nor2   g204(.a(new_n168_), .b(new_n166_), .O(new_n374_));
  oai22  g205(.a(new_n374_), .b(new_n217_), .c(new_n337_), .d(x04), .O(new_n375_));
  nand2  g206(.a(new_n171_), .b(x82), .O(new_n376_));
  inv1   g207(.a(new_n376_), .O(new_n377_));
  and2   g208(.a(new_n377_), .b(new_n375_), .O(z63));
  nand3  g209(.a(new_n375_), .b(x83), .c(x79), .O(new_n379_));
  aoi21  g210(.a(new_n379_), .b(new_n301_), .c(x01), .O(z64));
  nor2   g211(.a(new_n160_), .b(x04), .O(new_n381_));
  nor2   g212(.a(new_n241_), .b(x78), .O(new_n382_));
  oai21  g213(.a(new_n382_), .b(new_n381_), .c(x77), .O(new_n383_));
  nand2  g214(.a(new_n166_), .b(x81), .O(new_n384_));
  nand2  g215(.a(new_n171_), .b(x84), .O(new_n385_));
  aoi21  g216(.a(new_n384_), .b(new_n383_), .c(new_n385_), .O(z65));
  zero   g217(.O(z05));
  zero   g218(.O(z12));
  zero   g219(.O(z16));
  zero   g220(.O(z17));
  zero   g221(.O(z19));
  zero   g222(.O(z24));
  zero   g223(.O(z25));
  zero   g224(.O(z29));
  zero   g225(.O(z40));
  zero   g226(.O(z43));
  zero   g227(.O(z44));
  zero   g228(.O(z45));
  zero   g229(.O(z49));
  zero   g230(.O(z52));
  zero   g231(.O(z53));
  zero   g232(.O(z55));
  zero   g233(.O(z58));
  zero   g234(.O(z61));
endmodule


