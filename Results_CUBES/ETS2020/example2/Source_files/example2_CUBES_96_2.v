// Benchmark "FAU" written by ABC on Fri Jul 10 18:16:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n166_, new_n167_, new_n170_, new_n171_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n199_, new_n200_, new_n204_, new_n205_,
    new_n206_, new_n207_, new_n208_, new_n209_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n223_, new_n224_, new_n225_,
    new_n227_, new_n228_, new_n229_, new_n231_, new_n232_, new_n233_,
    new_n235_, new_n236_, new_n237_, new_n240_, new_n241_, new_n242_,
    new_n250_, new_n251_, new_n252_, new_n253_, new_n254_, new_n255_,
    new_n256_, new_n257_, new_n258_, new_n259_, new_n261_, new_n263_,
    new_n267_, new_n269_, new_n271_, new_n274_, new_n275_, new_n276_,
    new_n277_, new_n278_, new_n279_, new_n280_, new_n281_, new_n282_,
    new_n283_, new_n286_, new_n287_, new_n288_, new_n289_, new_n290_,
    new_n292_, new_n293_, new_n294_, new_n295_, new_n296_, new_n298_,
    new_n299_, new_n300_, new_n301_, new_n302_, new_n304_, new_n305_,
    new_n306_, new_n307_, new_n308_, new_n310_, new_n311_, new_n312_,
    new_n313_, new_n314_, new_n316_, new_n317_, new_n318_, new_n319_,
    new_n323_, new_n324_, new_n331_, new_n332_, new_n333_;
  inv1   g000(.a(x78), .O(new_n154_));
  nor2   g001(.a(new_n154_), .b(x77), .O(new_n155_));
  nand2  g002(.a(new_n155_), .b(x75), .O(new_n156_));
  inv1   g003(.a(x77), .O(new_n157_));
  nor2   g004(.a(x78), .b(new_n157_), .O(new_n158_));
  nand2  g005(.a(new_n158_), .b(x66), .O(new_n159_));
  inv1   g006(.a(x01), .O(new_n160_));
  nand2  g007(.a(x79), .b(new_n160_), .O(new_n161_));
  aoi21  g008(.a(new_n159_), .b(new_n156_), .c(new_n161_), .O(z02));
  inv1   g009(.a(x24), .O(new_n166_));
  nand2  g010(.a(x64), .b(x40), .O(new_n167_));
  oai21  g011(.a(x40), .b(new_n166_), .c(new_n167_), .O(z06));
  inv1   g012(.a(x26), .O(new_n170_));
  nand2  g013(.a(x62), .b(x40), .O(new_n171_));
  oai21  g014(.a(x40), .b(new_n170_), .c(new_n171_), .O(z08));
  inv1   g015(.a(x31), .O(new_n177_));
  nand2  g016(.a(x57), .b(x40), .O(new_n178_));
  oai21  g017(.a(x40), .b(new_n177_), .c(new_n178_), .O(z13));
  inv1   g018(.a(x32), .O(new_n180_));
  nand2  g019(.a(x51), .b(x40), .O(new_n181_));
  oai21  g020(.a(x40), .b(new_n180_), .c(new_n181_), .O(z14));
  inv1   g021(.a(x34), .O(new_n184_));
  nand2  g022(.a(x49), .b(x40), .O(new_n185_));
  oai21  g023(.a(x40), .b(new_n184_), .c(new_n185_), .O(z16));
  inv1   g024(.a(x35), .O(new_n187_));
  nand2  g025(.a(x48), .b(x40), .O(new_n188_));
  oai21  g026(.a(x40), .b(new_n187_), .c(new_n188_), .O(z17));
  inv1   g027(.a(x36), .O(new_n190_));
  nand2  g028(.a(x47), .b(x40), .O(new_n191_));
  oai21  g029(.a(x40), .b(new_n190_), .c(new_n191_), .O(z18));
  inv1   g030(.a(x37), .O(new_n193_));
  nand2  g031(.a(x46), .b(x40), .O(new_n194_));
  oai21  g032(.a(x40), .b(new_n193_), .c(new_n194_), .O(z19));
  inv1   g033(.a(x38), .O(new_n196_));
  nand2  g034(.a(x45), .b(x40), .O(new_n197_));
  oai21  g035(.a(x40), .b(new_n196_), .c(new_n197_), .O(z20));
  inv1   g036(.a(x39), .O(new_n199_));
  nand2  g037(.a(x44), .b(x40), .O(new_n200_));
  oai21  g038(.a(x40), .b(new_n199_), .c(new_n200_), .O(z21));
  inv1   g039(.a(x79), .O(new_n204_));
  nor2   g040(.a(new_n154_), .b(new_n157_), .O(new_n205_));
  nor2   g041(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  inv1   g042(.a(x43), .O(new_n207_));
  nor2   g043(.a(x04), .b(x01), .O(new_n208_));
  nand3  g044(.a(new_n208_), .b(new_n207_), .c(x05), .O(new_n209_));
  nor2   g045(.a(new_n209_), .b(new_n206_), .O(z24));
  inv1   g046(.a(x81), .O(new_n211_));
  xor2a  g047(.a(x84), .b(x82), .O(new_n212_));
  xor2a  g048(.a(x84), .b(x82), .O(new_n213_));
  nand2  g049(.a(new_n213_), .b(new_n211_), .O(new_n214_));
  oai21  g050(.a(new_n212_), .b(new_n211_), .c(new_n214_), .O(new_n215_));
  nand3  g051(.a(x79), .b(x78), .c(x77), .O(new_n216_));
  inv1   g052(.a(new_n216_), .O(new_n217_));
  inv1   g053(.a(x42), .O(new_n218_));
  nand3  g054(.a(new_n208_), .b(new_n218_), .c(x05), .O(new_n219_));
  inv1   g055(.a(new_n219_), .O(new_n220_));
  nand3  g056(.a(new_n220_), .b(new_n217_), .c(new_n215_), .O(new_n221_));
  inv1   g057(.a(new_n221_), .O(z25));
  nand3  g058(.a(new_n208_), .b(x44), .c(new_n218_), .O(new_n223_));
  inv1   g059(.a(new_n223_), .O(new_n224_));
  nand3  g060(.a(new_n224_), .b(new_n217_), .c(new_n215_), .O(new_n225_));
  inv1   g061(.a(new_n225_), .O(z26));
  nand3  g062(.a(new_n208_), .b(x45), .c(new_n218_), .O(new_n227_));
  inv1   g063(.a(new_n227_), .O(new_n228_));
  nand3  g064(.a(new_n228_), .b(new_n217_), .c(new_n215_), .O(new_n229_));
  inv1   g065(.a(new_n229_), .O(z27));
  nand3  g066(.a(new_n208_), .b(x46), .c(new_n218_), .O(new_n231_));
  inv1   g067(.a(new_n231_), .O(new_n232_));
  nand3  g068(.a(new_n232_), .b(new_n217_), .c(new_n215_), .O(new_n233_));
  inv1   g069(.a(new_n233_), .O(z28));
  nand3  g070(.a(new_n208_), .b(x47), .c(new_n218_), .O(new_n235_));
  inv1   g071(.a(new_n235_), .O(new_n236_));
  nand3  g072(.a(new_n236_), .b(new_n217_), .c(new_n215_), .O(new_n237_));
  inv1   g073(.a(new_n237_), .O(z29));
  nand3  g074(.a(new_n208_), .b(x49), .c(new_n218_), .O(new_n240_));
  inv1   g075(.a(new_n240_), .O(new_n241_));
  nand3  g076(.a(new_n241_), .b(new_n217_), .c(new_n215_), .O(new_n242_));
  inv1   g077(.a(new_n242_), .O(z31));
  xnor2a g078(.a(x84), .b(x82), .O(new_n250_));
  nand2  g079(.a(x83), .b(x42), .O(new_n251_));
  nand2  g080(.a(new_n251_), .b(new_n211_), .O(new_n252_));
  nand3  g081(.a(x83), .b(x81), .c(x42), .O(new_n253_));
  aoi21  g082(.a(new_n253_), .b(new_n252_), .c(new_n250_), .O(new_n254_));
  nand2  g083(.a(new_n251_), .b(x81), .O(new_n255_));
  nand3  g084(.a(x83), .b(new_n211_), .c(x42), .O(new_n256_));
  aoi21  g085(.a(new_n256_), .b(new_n255_), .c(new_n212_), .O(new_n257_));
  oai21  g086(.a(new_n257_), .b(new_n254_), .c(new_n217_), .O(new_n258_));
  nand2  g087(.a(new_n208_), .b(x56), .O(new_n259_));
  nor2   g088(.a(new_n259_), .b(new_n258_), .O(z38));
  nand2  g089(.a(new_n208_), .b(x57), .O(new_n261_));
  nor2   g090(.a(new_n261_), .b(new_n258_), .O(z39));
  nand2  g091(.a(new_n208_), .b(x58), .O(new_n263_));
  nor2   g092(.a(new_n263_), .b(new_n258_), .O(z40));
  nand2  g093(.a(new_n208_), .b(x61), .O(new_n267_));
  nor2   g094(.a(new_n267_), .b(new_n258_), .O(z43));
  nand2  g095(.a(new_n208_), .b(x62), .O(new_n269_));
  nor2   g096(.a(new_n269_), .b(new_n258_), .O(z44));
  nand2  g097(.a(new_n208_), .b(x63), .O(new_n271_));
  nor2   g098(.a(new_n271_), .b(new_n258_), .O(z45));
  inv1   g099(.a(x07), .O(new_n274_));
  nand2  g100(.a(x52), .b(x15), .O(new_n275_));
  oai21  g101(.a(x52), .b(new_n274_), .c(new_n275_), .O(new_n276_));
  inv1   g102(.a(x04), .O(new_n277_));
  nor2   g103(.a(x77), .b(new_n277_), .O(new_n278_));
  nor2   g104(.a(x79), .b(new_n154_), .O(new_n279_));
  nand3  g105(.a(new_n279_), .b(new_n278_), .c(new_n276_), .O(new_n280_));
  xnor2a g106(.a(x84), .b(x81), .O(new_n281_));
  or2    g107(.a(x75), .b(x67), .O(new_n282_));
  nand4  g108(.a(new_n282_), .b(new_n281_), .c(new_n158_), .d(x79), .O(new_n283_));
  aoi21  g109(.a(new_n283_), .b(new_n280_), .c(x01), .O(z47));
  inv1   g110(.a(x09), .O(new_n286_));
  nand2  g111(.a(x52), .b(x17), .O(new_n287_));
  oai21  g112(.a(x52), .b(new_n286_), .c(new_n287_), .O(new_n288_));
  nand3  g113(.a(new_n288_), .b(new_n279_), .c(new_n278_), .O(new_n289_));
  nand4  g114(.a(new_n281_), .b(new_n158_), .c(x79), .d(x69), .O(new_n290_));
  aoi21  g115(.a(new_n290_), .b(new_n289_), .c(x01), .O(z49));
  inv1   g116(.a(x10), .O(new_n292_));
  nand2  g117(.a(x52), .b(x18), .O(new_n293_));
  oai21  g118(.a(x52), .b(new_n292_), .c(new_n293_), .O(new_n294_));
  nand3  g119(.a(new_n294_), .b(new_n279_), .c(new_n278_), .O(new_n295_));
  nand4  g120(.a(new_n281_), .b(new_n158_), .c(x79), .d(x70), .O(new_n296_));
  aoi21  g121(.a(new_n296_), .b(new_n295_), .c(x01), .O(z50));
  inv1   g122(.a(x11), .O(new_n298_));
  nand2  g123(.a(x52), .b(x19), .O(new_n299_));
  oai21  g124(.a(x52), .b(new_n298_), .c(new_n299_), .O(new_n300_));
  nand3  g125(.a(new_n300_), .b(new_n279_), .c(new_n278_), .O(new_n301_));
  nand4  g126(.a(new_n281_), .b(new_n158_), .c(x79), .d(x71), .O(new_n302_));
  aoi21  g127(.a(new_n302_), .b(new_n301_), .c(x01), .O(z51));
  inv1   g128(.a(x12), .O(new_n304_));
  nand2  g129(.a(x52), .b(x20), .O(new_n305_));
  oai21  g130(.a(x52), .b(new_n304_), .c(new_n305_), .O(new_n306_));
  nand3  g131(.a(new_n306_), .b(new_n279_), .c(new_n278_), .O(new_n307_));
  nand4  g132(.a(new_n281_), .b(new_n158_), .c(x79), .d(x72), .O(new_n308_));
  aoi21  g133(.a(new_n308_), .b(new_n307_), .c(x01), .O(z52));
  inv1   g134(.a(x13), .O(new_n310_));
  nand2  g135(.a(x52), .b(x21), .O(new_n311_));
  oai21  g136(.a(x52), .b(new_n310_), .c(new_n311_), .O(new_n312_));
  nand3  g137(.a(new_n312_), .b(new_n279_), .c(new_n278_), .O(new_n313_));
  nand4  g138(.a(new_n281_), .b(new_n158_), .c(x79), .d(x73), .O(new_n314_));
  aoi21  g139(.a(new_n314_), .b(new_n313_), .c(x01), .O(z53));
  inv1   g140(.a(x14), .O(new_n316_));
  nor2   g141(.a(x52), .b(new_n316_), .O(new_n317_));
  aoi21  g142(.a(x52), .b(x22), .c(new_n317_), .O(new_n318_));
  nand4  g143(.a(new_n155_), .b(new_n204_), .c(x04), .d(new_n160_), .O(new_n319_));
  nor2   g144(.a(new_n319_), .b(new_n318_), .O(z54));
  inv1   g145(.a(x02), .O(new_n323_));
  nand4  g146(.a(x03), .b(new_n323_), .c(new_n160_), .d(x00), .O(new_n324_));
  inv1   g147(.a(new_n324_), .O(z57));
  oai21  g148(.a(new_n158_), .b(new_n155_), .c(new_n281_), .O(new_n331_));
  nand2  g149(.a(new_n205_), .b(new_n277_), .O(new_n332_));
  nand3  g150(.a(x82), .b(x79), .c(new_n160_), .O(new_n333_));
  aoi21  g151(.a(new_n332_), .b(new_n331_), .c(new_n333_), .O(z63));
  zero   g152(.O(z00));
  zero   g153(.O(z01));
  zero   g154(.O(z03));
  zero   g155(.O(z04));
  zero   g156(.O(z05));
  zero   g157(.O(z07));
  zero   g158(.O(z09));
  zero   g159(.O(z10));
  zero   g160(.O(z11));
  zero   g161(.O(z12));
  zero   g162(.O(z15));
  zero   g163(.O(z22));
  zero   g164(.O(z23));
  zero   g165(.O(z30));
  zero   g166(.O(z32));
  zero   g167(.O(z33));
  zero   g168(.O(z34));
  zero   g169(.O(z35));
  zero   g170(.O(z36));
  zero   g171(.O(z37));
  zero   g172(.O(z41));
  zero   g173(.O(z42));
  zero   g174(.O(z46));
  zero   g175(.O(z48));
  zero   g176(.O(z55));
  zero   g177(.O(z56));
  zero   g178(.O(z58));
  zero   g179(.O(z59));
  zero   g180(.O(z60));
  zero   g181(.O(z61));
  zero   g182(.O(z62));
  zero   g183(.O(z64));
  zero   g184(.O(z65));
endmodule


