// Benchmark "FAU" written by ABC on Fri Jul 10 18:13:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n157_, new_n158_, new_n160_, new_n161_, new_n164_, new_n165_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n191_, new_n192_, new_n195_, new_n196_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n206_, new_n207_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n221_, new_n222_, new_n223_, new_n225_, new_n226_,
    new_n227_, new_n231_, new_n232_, new_n233_, new_n235_, new_n236_,
    new_n237_, new_n240_, new_n241_, new_n242_, new_n243_, new_n244_,
    new_n245_, new_n246_, new_n247_, new_n248_, new_n249_, new_n251_,
    new_n252_, new_n253_, new_n254_, new_n255_, new_n256_, new_n257_,
    new_n258_, new_n259_, new_n260_, new_n262_, new_n264_, new_n267_,
    new_n269_, new_n271_, new_n274_, new_n276_, new_n278_, new_n280_,
    new_n282_, new_n284_, new_n285_, new_n286_, new_n287_, new_n288_,
    new_n289_, new_n290_, new_n291_, new_n292_, new_n293_, new_n296_,
    new_n297_, new_n298_, new_n299_, new_n300_, new_n303_, new_n304_,
    new_n305_, new_n306_, new_n307_, new_n309_, new_n310_, new_n311_,
    new_n312_, new_n313_, new_n315_, new_n316_, new_n317_, new_n318_,
    new_n319_, new_n322_, new_n323_, new_n324_, new_n325_, new_n326_,
    new_n329_, new_n330_, new_n331_, new_n336_, new_n337_, new_n338_,
    new_n339_, new_n340_;
  inv1   g000(.a(x23), .O(new_n157_));
  nand2  g001(.a(x65), .b(x40), .O(new_n158_));
  oai21  g002(.a(x40), .b(new_n157_), .c(new_n158_), .O(z05));
  inv1   g003(.a(x24), .O(new_n160_));
  nand2  g004(.a(x64), .b(x40), .O(new_n161_));
  oai21  g005(.a(x40), .b(new_n160_), .c(new_n161_), .O(z06));
  inv1   g006(.a(x26), .O(new_n164_));
  nand2  g007(.a(x62), .b(x40), .O(new_n165_));
  oai21  g008(.a(x40), .b(new_n164_), .c(new_n165_), .O(z08));
  inv1   g009(.a(x28), .O(new_n168_));
  nand2  g010(.a(x60), .b(x40), .O(new_n169_));
  oai21  g011(.a(x40), .b(new_n168_), .c(new_n169_), .O(z10));
  inv1   g012(.a(x29), .O(new_n171_));
  nand2  g013(.a(x59), .b(x40), .O(new_n172_));
  oai21  g014(.a(x40), .b(new_n171_), .c(new_n172_), .O(z11));
  inv1   g015(.a(x31), .O(new_n175_));
  nand2  g016(.a(x57), .b(x40), .O(new_n176_));
  oai21  g017(.a(x40), .b(new_n175_), .c(new_n176_), .O(z13));
  inv1   g018(.a(x32), .O(new_n178_));
  nand2  g019(.a(x51), .b(x40), .O(new_n179_));
  oai21  g020(.a(x40), .b(new_n178_), .c(new_n179_), .O(z14));
  inv1   g021(.a(x33), .O(new_n181_));
  nand2  g022(.a(x50), .b(x40), .O(new_n182_));
  oai21  g023(.a(x40), .b(new_n181_), .c(new_n182_), .O(z15));
  inv1   g024(.a(x34), .O(new_n184_));
  nand2  g025(.a(x49), .b(x40), .O(new_n185_));
  oai21  g026(.a(x40), .b(new_n184_), .c(new_n185_), .O(z16));
  inv1   g027(.a(x35), .O(new_n187_));
  nand2  g028(.a(x48), .b(x40), .O(new_n188_));
  oai21  g029(.a(x40), .b(new_n187_), .c(new_n188_), .O(z17));
  inv1   g030(.a(x37), .O(new_n191_));
  nand2  g031(.a(x46), .b(x40), .O(new_n192_));
  oai21  g032(.a(x40), .b(new_n191_), .c(new_n192_), .O(z19));
  inv1   g033(.a(x39), .O(new_n195_));
  nand2  g034(.a(x44), .b(x40), .O(new_n196_));
  oai21  g035(.a(x40), .b(new_n195_), .c(new_n196_), .O(z21));
  inv1   g036(.a(x79), .O(new_n200_));
  inv1   g037(.a(x77), .O(new_n201_));
  inv1   g038(.a(x78), .O(new_n202_));
  nor2   g039(.a(new_n202_), .b(new_n201_), .O(new_n203_));
  nor2   g040(.a(new_n203_), .b(new_n200_), .O(new_n204_));
  inv1   g041(.a(x43), .O(new_n205_));
  nor2   g042(.a(x04), .b(x01), .O(new_n206_));
  nand3  g043(.a(new_n206_), .b(new_n205_), .c(x05), .O(new_n207_));
  nor2   g044(.a(new_n207_), .b(new_n204_), .O(z24));
  inv1   g045(.a(x81), .O(new_n209_));
  xor2a  g046(.a(x84), .b(x82), .O(new_n210_));
  xor2a  g047(.a(x84), .b(x82), .O(new_n211_));
  nand2  g048(.a(new_n211_), .b(new_n209_), .O(new_n212_));
  oai21  g049(.a(new_n210_), .b(new_n209_), .c(new_n212_), .O(new_n213_));
  nand3  g050(.a(x79), .b(x78), .c(x77), .O(new_n214_));
  inv1   g051(.a(new_n214_), .O(new_n215_));
  inv1   g052(.a(x42), .O(new_n216_));
  nand3  g053(.a(new_n206_), .b(new_n216_), .c(x05), .O(new_n217_));
  inv1   g054(.a(new_n217_), .O(new_n218_));
  nand3  g055(.a(new_n218_), .b(new_n215_), .c(new_n213_), .O(new_n219_));
  inv1   g056(.a(new_n219_), .O(z25));
  nand3  g057(.a(new_n206_), .b(x44), .c(new_n216_), .O(new_n221_));
  inv1   g058(.a(new_n221_), .O(new_n222_));
  nand3  g059(.a(new_n222_), .b(new_n215_), .c(new_n213_), .O(new_n223_));
  inv1   g060(.a(new_n223_), .O(z26));
  nand3  g061(.a(new_n206_), .b(x45), .c(new_n216_), .O(new_n225_));
  inv1   g062(.a(new_n225_), .O(new_n226_));
  nand3  g063(.a(new_n226_), .b(new_n215_), .c(new_n213_), .O(new_n227_));
  inv1   g064(.a(new_n227_), .O(z27));
  nand3  g065(.a(new_n206_), .b(x48), .c(new_n216_), .O(new_n231_));
  inv1   g066(.a(new_n231_), .O(new_n232_));
  nand3  g067(.a(new_n232_), .b(new_n215_), .c(new_n213_), .O(new_n233_));
  inv1   g068(.a(new_n233_), .O(z30));
  nand3  g069(.a(new_n206_), .b(x49), .c(new_n216_), .O(new_n235_));
  inv1   g070(.a(new_n235_), .O(new_n236_));
  nand3  g071(.a(new_n236_), .b(new_n215_), .c(new_n213_), .O(new_n237_));
  inv1   g072(.a(new_n237_), .O(z31));
  inv1   g073(.a(new_n210_), .O(new_n240_));
  xnor2a g074(.a(x83), .b(x81), .O(new_n241_));
  nand2  g075(.a(x42), .b(x05), .O(new_n242_));
  nand2  g076(.a(x51), .b(new_n216_), .O(new_n243_));
  oai22  g077(.a(new_n243_), .b(new_n209_), .c(new_n242_), .d(new_n241_), .O(new_n244_));
  nand2  g078(.a(new_n244_), .b(new_n240_), .O(new_n245_));
  xor2a  g079(.a(x83), .b(x81), .O(new_n246_));
  oai22  g080(.a(new_n246_), .b(new_n242_), .c(new_n243_), .d(x81), .O(new_n247_));
  nand2  g081(.a(new_n247_), .b(new_n211_), .O(new_n248_));
  nand2  g082(.a(new_n215_), .b(new_n206_), .O(new_n249_));
  aoi21  g083(.a(new_n248_), .b(new_n245_), .c(new_n249_), .O(z33));
  xnor2a g084(.a(x84), .b(x82), .O(new_n251_));
  nand2  g085(.a(x83), .b(x42), .O(new_n252_));
  nand2  g086(.a(new_n252_), .b(new_n209_), .O(new_n253_));
  nand3  g087(.a(x83), .b(x81), .c(x42), .O(new_n254_));
  aoi21  g088(.a(new_n254_), .b(new_n253_), .c(new_n251_), .O(new_n255_));
  nand2  g089(.a(new_n252_), .b(x81), .O(new_n256_));
  nand3  g090(.a(x83), .b(new_n209_), .c(x42), .O(new_n257_));
  aoi21  g091(.a(new_n257_), .b(new_n256_), .c(new_n210_), .O(new_n258_));
  oai21  g092(.a(new_n258_), .b(new_n255_), .c(new_n215_), .O(new_n259_));
  nand2  g093(.a(new_n206_), .b(x52), .O(new_n260_));
  nor2   g094(.a(new_n260_), .b(new_n259_), .O(z34));
  nand2  g095(.a(new_n206_), .b(x53), .O(new_n262_));
  nor2   g096(.a(new_n262_), .b(new_n259_), .O(z35));
  nand2  g097(.a(new_n206_), .b(x54), .O(new_n264_));
  nor2   g098(.a(new_n264_), .b(new_n259_), .O(z36));
  nand2  g099(.a(new_n206_), .b(x56), .O(new_n267_));
  nor2   g100(.a(new_n267_), .b(new_n259_), .O(z38));
  nand2  g101(.a(new_n206_), .b(x57), .O(new_n269_));
  nor2   g102(.a(new_n269_), .b(new_n259_), .O(z39));
  nand2  g103(.a(new_n206_), .b(x58), .O(new_n271_));
  nor2   g104(.a(new_n271_), .b(new_n259_), .O(z40));
  nand2  g105(.a(new_n206_), .b(x60), .O(new_n274_));
  nor2   g106(.a(new_n274_), .b(new_n259_), .O(z42));
  nand2  g107(.a(new_n206_), .b(x61), .O(new_n276_));
  nor2   g108(.a(new_n276_), .b(new_n259_), .O(z43));
  nand2  g109(.a(new_n206_), .b(x62), .O(new_n278_));
  nor2   g110(.a(new_n278_), .b(new_n259_), .O(z44));
  nand2  g111(.a(new_n206_), .b(x63), .O(new_n280_));
  nor2   g112(.a(new_n280_), .b(new_n259_), .O(z45));
  nand2  g113(.a(new_n206_), .b(x64), .O(new_n282_));
  nor2   g114(.a(new_n282_), .b(new_n259_), .O(z46));
  inv1   g115(.a(x07), .O(new_n284_));
  nand2  g116(.a(x52), .b(x15), .O(new_n285_));
  oai21  g117(.a(x52), .b(new_n284_), .c(new_n285_), .O(new_n286_));
  nand4  g118(.a(new_n200_), .b(x78), .c(new_n201_), .d(x04), .O(new_n287_));
  inv1   g119(.a(new_n287_), .O(new_n288_));
  nand2  g120(.a(new_n288_), .b(new_n286_), .O(new_n289_));
  xnor2a g121(.a(x84), .b(x81), .O(new_n290_));
  or2    g122(.a(x75), .b(x67), .O(new_n291_));
  nor2   g123(.a(x78), .b(new_n201_), .O(new_n292_));
  nand4  g124(.a(new_n292_), .b(new_n291_), .c(new_n290_), .d(x79), .O(new_n293_));
  aoi21  g125(.a(new_n293_), .b(new_n289_), .c(x01), .O(z47));
  inv1   g126(.a(x09), .O(new_n296_));
  nand2  g127(.a(x52), .b(x17), .O(new_n297_));
  oai21  g128(.a(x52), .b(new_n296_), .c(new_n297_), .O(new_n298_));
  nand2  g129(.a(new_n298_), .b(new_n288_), .O(new_n299_));
  nand4  g130(.a(new_n292_), .b(new_n290_), .c(x79), .d(x69), .O(new_n300_));
  aoi21  g131(.a(new_n300_), .b(new_n299_), .c(x01), .O(z49));
  inv1   g132(.a(x11), .O(new_n303_));
  nand2  g133(.a(x52), .b(x19), .O(new_n304_));
  oai21  g134(.a(x52), .b(new_n303_), .c(new_n304_), .O(new_n305_));
  nand2  g135(.a(new_n305_), .b(new_n288_), .O(new_n306_));
  nand4  g136(.a(new_n292_), .b(new_n290_), .c(x79), .d(x71), .O(new_n307_));
  aoi21  g137(.a(new_n307_), .b(new_n306_), .c(x01), .O(z51));
  inv1   g138(.a(x12), .O(new_n309_));
  nand2  g139(.a(x52), .b(x20), .O(new_n310_));
  oai21  g140(.a(x52), .b(new_n309_), .c(new_n310_), .O(new_n311_));
  nand2  g141(.a(new_n311_), .b(new_n288_), .O(new_n312_));
  nand4  g142(.a(new_n292_), .b(new_n290_), .c(x79), .d(x72), .O(new_n313_));
  aoi21  g143(.a(new_n313_), .b(new_n312_), .c(x01), .O(z52));
  inv1   g144(.a(x13), .O(new_n315_));
  nand2  g145(.a(x52), .b(x21), .O(new_n316_));
  oai21  g146(.a(x52), .b(new_n315_), .c(new_n316_), .O(new_n317_));
  nand2  g147(.a(new_n317_), .b(new_n288_), .O(new_n318_));
  nand4  g148(.a(new_n292_), .b(new_n290_), .c(x79), .d(x73), .O(new_n319_));
  aoi21  g149(.a(new_n319_), .b(new_n318_), .c(x01), .O(z53));
  inv1   g150(.a(x82), .O(new_n322_));
  nand2  g151(.a(x84), .b(new_n322_), .O(new_n323_));
  nand2  g152(.a(x83), .b(new_n209_), .O(new_n324_));
  nor2   g153(.a(x80), .b(new_n200_), .O(new_n325_));
  nand3  g154(.a(new_n325_), .b(new_n206_), .c(new_n203_), .O(new_n326_));
  nor3   g155(.a(new_n326_), .b(new_n324_), .c(new_n323_), .O(z55));
  inv1   g156(.a(x01), .O(new_n329_));
  inv1   g157(.a(x02), .O(new_n330_));
  nand4  g158(.a(x03), .b(new_n330_), .c(new_n329_), .d(x00), .O(new_n331_));
  inv1   g159(.a(new_n331_), .O(z57));
  nor2   g160(.a(new_n202_), .b(x77), .O(new_n336_));
  oai21  g161(.a(new_n336_), .b(new_n292_), .c(new_n290_), .O(new_n337_));
  inv1   g162(.a(x04), .O(new_n338_));
  nand2  g163(.a(new_n203_), .b(new_n338_), .O(new_n339_));
  nand3  g164(.a(x80), .b(x79), .c(new_n329_), .O(new_n340_));
  aoi21  g165(.a(new_n339_), .b(new_n337_), .c(new_n340_), .O(z61));
  zero   g166(.O(z00));
  zero   g167(.O(z01));
  zero   g168(.O(z02));
  zero   g169(.O(z03));
  zero   g170(.O(z04));
  zero   g171(.O(z07));
  zero   g172(.O(z09));
  zero   g173(.O(z12));
  zero   g174(.O(z18));
  zero   g175(.O(z20));
  zero   g176(.O(z22));
  zero   g177(.O(z23));
  zero   g178(.O(z28));
  zero   g179(.O(z29));
  zero   g180(.O(z32));
  zero   g181(.O(z37));
  zero   g182(.O(z41));
  zero   g183(.O(z48));
  zero   g184(.O(z50));
  zero   g185(.O(z54));
  zero   g186(.O(z56));
  zero   g187(.O(z58));
  zero   g188(.O(z59));
  zero   g189(.O(z60));
  zero   g190(.O(z62));
  zero   g191(.O(z63));
  zero   g192(.O(z64));
  zero   g193(.O(z65));
endmodule


