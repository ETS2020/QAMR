// Benchmark "FAU" written by ABC on Fri Jul 10 18:09:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n182_, new_n183_, new_n185_, new_n186_, new_n188_, new_n189_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n200_, new_n201_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n226_, new_n227_, new_n228_, new_n230_, new_n231_, new_n232_,
    new_n234_, new_n235_, new_n236_, new_n238_, new_n239_, new_n240_,
    new_n242_, new_n243_, new_n244_, new_n248_, new_n249_, new_n250_,
    new_n251_, new_n252_, new_n253_, new_n254_, new_n255_, new_n256_,
    new_n257_, new_n259_, new_n260_, new_n261_, new_n262_, new_n263_,
    new_n264_, new_n265_, new_n266_, new_n267_, new_n268_, new_n271_,
    new_n274_, new_n276_, new_n280_, new_n282_, new_n284_, new_n286_,
    new_n288_, new_n290_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n297_, new_n298_, new_n299_, new_n301_,
    new_n302_, new_n303_, new_n304_, new_n305_, new_n308_, new_n309_,
    new_n310_, new_n311_, new_n312_, new_n314_, new_n315_, new_n316_,
    new_n317_, new_n318_, new_n320_, new_n321_, new_n322_, new_n323_,
    new_n324_, new_n326_, new_n327_, new_n328_, new_n329_, new_n330_,
    new_n332_, new_n333_, new_n334_, new_n335_, new_n336_, new_n337_,
    new_n341_, new_n342_;
  inv1   g000(.a(x77), .O(new_n154_));
  nand3  g001(.a(x78), .b(new_n154_), .c(x75), .O(new_n155_));
  inv1   g002(.a(x78), .O(new_n156_));
  nand3  g003(.a(new_n156_), .b(x77), .c(x66), .O(new_n157_));
  inv1   g004(.a(x01), .O(new_n158_));
  nand2  g005(.a(x79), .b(new_n158_), .O(new_n159_));
  aoi21  g006(.a(new_n157_), .b(new_n155_), .c(new_n159_), .O(z02));
  nor2   g007(.a(x79), .b(new_n156_), .O(new_n161_));
  nand3  g008(.a(new_n161_), .b(x52), .c(new_n158_), .O(new_n162_));
  inv1   g009(.a(new_n162_), .O(z03));
  inv1   g010(.a(x23), .O(new_n165_));
  nand2  g011(.a(x65), .b(x40), .O(new_n166_));
  oai21  g012(.a(x40), .b(new_n165_), .c(new_n166_), .O(z05));
  inv1   g013(.a(x24), .O(new_n168_));
  nand2  g014(.a(x64), .b(x40), .O(new_n169_));
  oai21  g015(.a(x40), .b(new_n168_), .c(new_n169_), .O(z06));
  inv1   g016(.a(x25), .O(new_n171_));
  nand2  g017(.a(x63), .b(x40), .O(new_n172_));
  oai21  g018(.a(x40), .b(new_n171_), .c(new_n172_), .O(z07));
  inv1   g019(.a(x27), .O(new_n175_));
  nand2  g020(.a(x61), .b(x40), .O(new_n176_));
  oai21  g021(.a(x40), .b(new_n175_), .c(new_n176_), .O(z09));
  inv1   g022(.a(x28), .O(new_n178_));
  nand2  g023(.a(x60), .b(x40), .O(new_n179_));
  oai21  g024(.a(x40), .b(new_n178_), .c(new_n179_), .O(z10));
  inv1   g025(.a(x30), .O(new_n182_));
  nand2  g026(.a(x58), .b(x40), .O(new_n183_));
  oai21  g027(.a(x40), .b(new_n182_), .c(new_n183_), .O(z12));
  inv1   g028(.a(x31), .O(new_n185_));
  nand2  g029(.a(x57), .b(x40), .O(new_n186_));
  oai21  g030(.a(x40), .b(new_n185_), .c(new_n186_), .O(z13));
  inv1   g031(.a(x32), .O(new_n188_));
  nand2  g032(.a(x51), .b(x40), .O(new_n189_));
  oai21  g033(.a(x40), .b(new_n188_), .c(new_n189_), .O(z14));
  inv1   g034(.a(x33), .O(new_n191_));
  nand2  g035(.a(x50), .b(x40), .O(new_n192_));
  oai21  g036(.a(x40), .b(new_n191_), .c(new_n192_), .O(z15));
  inv1   g037(.a(x34), .O(new_n194_));
  nand2  g038(.a(x49), .b(x40), .O(new_n195_));
  oai21  g039(.a(x40), .b(new_n194_), .c(new_n195_), .O(z16));
  inv1   g040(.a(x35), .O(new_n197_));
  nand2  g041(.a(x48), .b(x40), .O(new_n198_));
  oai21  g042(.a(x40), .b(new_n197_), .c(new_n198_), .O(z17));
  inv1   g043(.a(x36), .O(new_n200_));
  nand2  g044(.a(x47), .b(x40), .O(new_n201_));
  oai21  g045(.a(x40), .b(new_n200_), .c(new_n201_), .O(z18));
  inv1   g046(.a(x38), .O(new_n204_));
  nand2  g047(.a(x45), .b(x40), .O(new_n205_));
  oai21  g048(.a(x40), .b(new_n204_), .c(new_n205_), .O(z20));
  inv1   g049(.a(x39), .O(new_n207_));
  nand2  g050(.a(x44), .b(x40), .O(new_n208_));
  oai21  g051(.a(x40), .b(new_n207_), .c(new_n208_), .O(z21));
  inv1   g052(.a(x81), .O(new_n213_));
  xor2a  g053(.a(x84), .b(x82), .O(new_n214_));
  xor2a  g054(.a(x84), .b(x82), .O(new_n215_));
  nand2  g055(.a(new_n215_), .b(new_n213_), .O(new_n216_));
  oai21  g056(.a(new_n214_), .b(new_n213_), .c(new_n216_), .O(new_n217_));
  nand3  g057(.a(x79), .b(x78), .c(x77), .O(new_n218_));
  inv1   g058(.a(new_n218_), .O(new_n219_));
  inv1   g059(.a(x42), .O(new_n220_));
  nor2   g060(.a(x04), .b(x01), .O(new_n221_));
  nand3  g061(.a(new_n221_), .b(new_n220_), .c(x05), .O(new_n222_));
  inv1   g062(.a(new_n222_), .O(new_n223_));
  nand3  g063(.a(new_n223_), .b(new_n219_), .c(new_n217_), .O(new_n224_));
  inv1   g064(.a(new_n224_), .O(z25));
  nand3  g065(.a(new_n221_), .b(x44), .c(new_n220_), .O(new_n226_));
  inv1   g066(.a(new_n226_), .O(new_n227_));
  nand3  g067(.a(new_n227_), .b(new_n219_), .c(new_n217_), .O(new_n228_));
  inv1   g068(.a(new_n228_), .O(z26));
  nand3  g069(.a(new_n221_), .b(x45), .c(new_n220_), .O(new_n230_));
  inv1   g070(.a(new_n230_), .O(new_n231_));
  nand3  g071(.a(new_n231_), .b(new_n219_), .c(new_n217_), .O(new_n232_));
  inv1   g072(.a(new_n232_), .O(z27));
  nand3  g073(.a(new_n221_), .b(x46), .c(new_n220_), .O(new_n234_));
  inv1   g074(.a(new_n234_), .O(new_n235_));
  nand3  g075(.a(new_n235_), .b(new_n219_), .c(new_n217_), .O(new_n236_));
  inv1   g076(.a(new_n236_), .O(z28));
  nand3  g077(.a(new_n221_), .b(x47), .c(new_n220_), .O(new_n238_));
  inv1   g078(.a(new_n238_), .O(new_n239_));
  nand3  g079(.a(new_n239_), .b(new_n219_), .c(new_n217_), .O(new_n240_));
  inv1   g080(.a(new_n240_), .O(z29));
  nand3  g081(.a(new_n221_), .b(x48), .c(new_n220_), .O(new_n242_));
  inv1   g082(.a(new_n242_), .O(new_n243_));
  nand3  g083(.a(new_n243_), .b(new_n219_), .c(new_n217_), .O(new_n244_));
  inv1   g084(.a(new_n244_), .O(z30));
  inv1   g085(.a(new_n214_), .O(new_n248_));
  xnor2a g086(.a(x83), .b(x81), .O(new_n249_));
  nand2  g087(.a(x42), .b(x05), .O(new_n250_));
  nand2  g088(.a(x51), .b(new_n220_), .O(new_n251_));
  oai22  g089(.a(new_n251_), .b(new_n213_), .c(new_n250_), .d(new_n249_), .O(new_n252_));
  nand2  g090(.a(new_n252_), .b(new_n248_), .O(new_n253_));
  xor2a  g091(.a(x83), .b(x81), .O(new_n254_));
  oai22  g092(.a(new_n254_), .b(new_n250_), .c(new_n251_), .d(x81), .O(new_n255_));
  nand2  g093(.a(new_n255_), .b(new_n215_), .O(new_n256_));
  nand2  g094(.a(new_n221_), .b(new_n219_), .O(new_n257_));
  aoi21  g095(.a(new_n256_), .b(new_n253_), .c(new_n257_), .O(z33));
  xnor2a g096(.a(x84), .b(x82), .O(new_n259_));
  nand2  g097(.a(x83), .b(x42), .O(new_n260_));
  nand2  g098(.a(new_n260_), .b(new_n213_), .O(new_n261_));
  nand3  g099(.a(x83), .b(x81), .c(x42), .O(new_n262_));
  aoi21  g100(.a(new_n262_), .b(new_n261_), .c(new_n259_), .O(new_n263_));
  nand2  g101(.a(new_n260_), .b(x81), .O(new_n264_));
  nand3  g102(.a(x83), .b(new_n213_), .c(x42), .O(new_n265_));
  aoi21  g103(.a(new_n265_), .b(new_n264_), .c(new_n214_), .O(new_n266_));
  oai21  g104(.a(new_n266_), .b(new_n263_), .c(new_n219_), .O(new_n267_));
  nand2  g105(.a(new_n221_), .b(x52), .O(new_n268_));
  nor2   g106(.a(new_n268_), .b(new_n267_), .O(z34));
  nand2  g107(.a(new_n221_), .b(x54), .O(new_n271_));
  nor2   g108(.a(new_n271_), .b(new_n267_), .O(z36));
  nand2  g109(.a(new_n221_), .b(x56), .O(new_n274_));
  nor2   g110(.a(new_n274_), .b(new_n267_), .O(z38));
  nand2  g111(.a(new_n221_), .b(x57), .O(new_n276_));
  nor2   g112(.a(new_n276_), .b(new_n267_), .O(z39));
  nand2  g113(.a(new_n221_), .b(x60), .O(new_n280_));
  nor2   g114(.a(new_n280_), .b(new_n267_), .O(z42));
  nand2  g115(.a(new_n221_), .b(x61), .O(new_n282_));
  nor2   g116(.a(new_n282_), .b(new_n267_), .O(z43));
  nand2  g117(.a(new_n221_), .b(x62), .O(new_n284_));
  nor2   g118(.a(new_n284_), .b(new_n267_), .O(z44));
  nand2  g119(.a(new_n221_), .b(x63), .O(new_n286_));
  nor2   g120(.a(new_n286_), .b(new_n267_), .O(z45));
  nand2  g121(.a(new_n221_), .b(x64), .O(new_n288_));
  nor2   g122(.a(new_n288_), .b(new_n267_), .O(z46));
  inv1   g123(.a(x07), .O(new_n290_));
  nand2  g124(.a(x52), .b(x15), .O(new_n291_));
  oai21  g125(.a(x52), .b(new_n290_), .c(new_n291_), .O(new_n292_));
  inv1   g126(.a(x04), .O(new_n293_));
  nor2   g127(.a(x77), .b(new_n293_), .O(new_n294_));
  nand3  g128(.a(new_n294_), .b(new_n292_), .c(new_n161_), .O(new_n295_));
  xnor2a g129(.a(x84), .b(x81), .O(new_n296_));
  or2    g130(.a(x75), .b(x67), .O(new_n297_));
  nor2   g131(.a(x78), .b(new_n154_), .O(new_n298_));
  nand4  g132(.a(new_n298_), .b(new_n297_), .c(new_n296_), .d(x79), .O(new_n299_));
  aoi21  g133(.a(new_n299_), .b(new_n295_), .c(x01), .O(z47));
  inv1   g134(.a(x08), .O(new_n301_));
  nand2  g135(.a(x52), .b(x16), .O(new_n302_));
  oai21  g136(.a(x52), .b(new_n301_), .c(new_n302_), .O(new_n303_));
  nand3  g137(.a(new_n303_), .b(new_n294_), .c(new_n161_), .O(new_n304_));
  nand4  g138(.a(new_n298_), .b(new_n296_), .c(x79), .d(x68), .O(new_n305_));
  aoi21  g139(.a(new_n305_), .b(new_n304_), .c(x01), .O(z48));
  inv1   g140(.a(x10), .O(new_n308_));
  nand2  g141(.a(x52), .b(x18), .O(new_n309_));
  oai21  g142(.a(x52), .b(new_n308_), .c(new_n309_), .O(new_n310_));
  nand3  g143(.a(new_n310_), .b(new_n294_), .c(new_n161_), .O(new_n311_));
  nand4  g144(.a(new_n298_), .b(new_n296_), .c(x79), .d(x70), .O(new_n312_));
  aoi21  g145(.a(new_n312_), .b(new_n311_), .c(x01), .O(z50));
  inv1   g146(.a(x11), .O(new_n314_));
  nand2  g147(.a(x52), .b(x19), .O(new_n315_));
  oai21  g148(.a(x52), .b(new_n314_), .c(new_n315_), .O(new_n316_));
  nand3  g149(.a(new_n316_), .b(new_n294_), .c(new_n161_), .O(new_n317_));
  nand4  g150(.a(new_n298_), .b(new_n296_), .c(x79), .d(x71), .O(new_n318_));
  aoi21  g151(.a(new_n318_), .b(new_n317_), .c(x01), .O(z51));
  inv1   g152(.a(x12), .O(new_n320_));
  nand2  g153(.a(x52), .b(x20), .O(new_n321_));
  oai21  g154(.a(x52), .b(new_n320_), .c(new_n321_), .O(new_n322_));
  nand3  g155(.a(new_n322_), .b(new_n294_), .c(new_n161_), .O(new_n323_));
  nand4  g156(.a(new_n298_), .b(new_n296_), .c(x79), .d(x72), .O(new_n324_));
  aoi21  g157(.a(new_n324_), .b(new_n323_), .c(x01), .O(z52));
  inv1   g158(.a(x13), .O(new_n326_));
  nand2  g159(.a(x52), .b(x21), .O(new_n327_));
  oai21  g160(.a(x52), .b(new_n326_), .c(new_n327_), .O(new_n328_));
  nand3  g161(.a(new_n328_), .b(new_n294_), .c(new_n161_), .O(new_n329_));
  nand4  g162(.a(new_n298_), .b(new_n296_), .c(x79), .d(x73), .O(new_n330_));
  aoi21  g163(.a(new_n330_), .b(new_n329_), .c(x01), .O(z53));
  inv1   g164(.a(x14), .O(new_n332_));
  nor2   g165(.a(x52), .b(new_n332_), .O(new_n333_));
  aoi21  g166(.a(x52), .b(x22), .c(new_n333_), .O(new_n334_));
  inv1   g167(.a(x79), .O(new_n335_));
  nor2   g168(.a(new_n293_), .b(x01), .O(new_n336_));
  nand4  g169(.a(new_n336_), .b(new_n335_), .c(x78), .d(new_n154_), .O(new_n337_));
  nor2   g170(.a(new_n337_), .b(new_n334_), .O(z54));
  inv1   g171(.a(x02), .O(new_n341_));
  nand4  g172(.a(x03), .b(new_n341_), .c(new_n158_), .d(x00), .O(new_n342_));
  inv1   g173(.a(new_n342_), .O(z57));
  zero   g174(.O(z00));
  zero   g175(.O(z01));
  zero   g176(.O(z04));
  zero   g177(.O(z08));
  zero   g178(.O(z11));
  zero   g179(.O(z19));
  zero   g180(.O(z22));
  zero   g181(.O(z23));
  zero   g182(.O(z24));
  zero   g183(.O(z31));
  zero   g184(.O(z32));
  zero   g185(.O(z35));
  zero   g186(.O(z37));
  zero   g187(.O(z40));
  zero   g188(.O(z41));
  zero   g189(.O(z49));
  zero   g190(.O(z55));
  zero   g191(.O(z56));
  zero   g192(.O(z58));
  zero   g193(.O(z59));
  zero   g194(.O(z60));
  zero   g195(.O(z61));
  zero   g196(.O(z62));
  zero   g197(.O(z63));
  zero   g198(.O(z64));
  zero   g199(.O(z65));
endmodule


