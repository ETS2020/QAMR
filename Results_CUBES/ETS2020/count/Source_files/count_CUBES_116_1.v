// Benchmark "FAU" written by ABC on Tue Jul  7 19:18:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x16), .O(new_n54_));
  inv1   g001(.a(x20), .O(new_n55_));
  nor2   g002(.a(x19), .b(x17), .O(new_n56_));
  nand2  g003(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g004(.a(x17), .O(new_n58_));
  inv1   g005(.a(x19), .O(new_n59_));
  inv1   g006(.a(x21), .O(new_n60_));
  nand4  g007(.a(new_n60_), .b(new_n55_), .c(new_n59_), .d(new_n58_), .O(new_n61_));
  inv1   g008(.a(new_n61_), .O(new_n62_));
  aoi21  g009(.a(new_n57_), .b(x21), .c(new_n62_), .O(new_n63_));
  inv1   g010(.a(x13), .O(new_n64_));
  aoi21  g011(.a(new_n54_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g012(.a(new_n63_), .b(new_n54_), .c(new_n65_), .O(z02));
  inv1   g013(.a(new_n57_), .O(new_n67_));
  nor2   g014(.a(x22), .b(x21), .O(new_n68_));
  aoi22  g015(.a(new_n68_), .b(new_n67_), .c(new_n61_), .d(x22), .O(new_n69_));
  inv1   g016(.a(x12), .O(new_n70_));
  aoi21  g017(.a(new_n54_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g018(.a(new_n69_), .b(new_n54_), .c(new_n71_), .O(z03));
  nand3  g019(.a(new_n68_), .b(new_n56_), .c(new_n55_), .O(new_n73_));
  nor3   g020(.a(x23), .b(x22), .c(x21), .O(new_n74_));
  aoi22  g021(.a(new_n74_), .b(new_n67_), .c(new_n73_), .d(x23), .O(new_n75_));
  inv1   g022(.a(x11), .O(new_n76_));
  aoi21  g023(.a(new_n54_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g024(.a(new_n75_), .b(new_n54_), .c(new_n77_), .O(z04));
  nor2   g025(.a(x23), .b(x22), .O(new_n79_));
  nand4  g026(.a(new_n79_), .b(new_n56_), .c(new_n60_), .d(new_n55_), .O(new_n80_));
  and2   g027(.a(new_n80_), .b(x24), .O(new_n81_));
  nor2   g028(.a(x24), .b(x23), .O(new_n82_));
  nand4  g029(.a(new_n82_), .b(new_n68_), .c(new_n56_), .d(new_n55_), .O(new_n83_));
  inv1   g030(.a(new_n83_), .O(new_n84_));
  oai21  g031(.a(new_n84_), .b(new_n81_), .c(x16), .O(new_n85_));
  inv1   g032(.a(x10), .O(new_n86_));
  aoi21  g033(.a(new_n54_), .b(new_n86_), .c(x18), .O(new_n87_));
  nand2  g034(.a(new_n87_), .b(new_n85_), .O(z05));
  inv1   g035(.a(x22), .O(new_n89_));
  inv1   g036(.a(x23), .O(new_n90_));
  inv1   g037(.a(x24), .O(new_n91_));
  inv1   g038(.a(x25), .O(new_n92_));
  nand4  g039(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n93_));
  nor2   g040(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  aoi21  g041(.a(new_n83_), .b(x25), .c(new_n94_), .O(new_n95_));
  inv1   g042(.a(x09), .O(new_n96_));
  aoi21  g043(.a(new_n54_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g044(.a(new_n95_), .b(new_n54_), .c(new_n97_), .O(z06));
  nor2   g045(.a(x21), .b(x20), .O(new_n99_));
  nor2   g046(.a(x25), .b(x24), .O(new_n100_));
  nand4  g047(.a(new_n100_), .b(new_n79_), .c(new_n99_), .d(new_n56_), .O(new_n101_));
  nand2  g048(.a(new_n101_), .b(x26), .O(new_n102_));
  nor3   g049(.a(x26), .b(x25), .c(x24), .O(new_n103_));
  nand3  g050(.a(new_n103_), .b(new_n79_), .c(new_n62_), .O(new_n104_));
  nand2  g051(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g052(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g053(.a(x08), .O(new_n107_));
  aoi21  g054(.a(new_n54_), .b(new_n107_), .c(x18), .O(new_n108_));
  nand2  g055(.a(new_n108_), .b(new_n106_), .O(z07));
  nor2   g056(.a(x27), .b(x26), .O(new_n110_));
  nand3  g057(.a(new_n110_), .b(new_n82_), .c(new_n92_), .O(new_n111_));
  nor2   g058(.a(new_n111_), .b(new_n73_), .O(new_n112_));
  aoi21  g059(.a(new_n104_), .b(x27), .c(new_n112_), .O(new_n113_));
  inv1   g060(.a(x07), .O(new_n114_));
  aoi21  g061(.a(new_n54_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g062(.a(new_n113_), .b(new_n54_), .c(new_n115_), .O(z08));
  nor2   g063(.a(x28), .b(x27), .O(new_n118_));
  nor2   g064(.a(x26), .b(x23), .O(new_n119_));
  nand3  g065(.a(new_n119_), .b(new_n118_), .c(new_n100_), .O(new_n120_));
  oai21  g066(.a(new_n120_), .b(new_n73_), .c(x29), .O(new_n121_));
  inv1   g067(.a(x27), .O(new_n122_));
  nor2   g068(.a(x29), .b(x28), .O(new_n123_));
  nand3  g069(.a(new_n123_), .b(new_n103_), .c(new_n122_), .O(new_n124_));
  oai21  g070(.a(new_n124_), .b(new_n80_), .c(new_n121_), .O(new_n125_));
  nand2  g071(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g072(.a(x05), .O(new_n127_));
  aoi21  g073(.a(new_n54_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g074(.a(new_n128_), .b(new_n126_), .O(z10));
  nor2   g075(.a(x26), .b(x25), .O(new_n131_));
  nor2   g076(.a(x30), .b(x29), .O(new_n132_));
  nand4  g077(.a(new_n132_), .b(new_n118_), .c(new_n131_), .d(new_n91_), .O(new_n133_));
  oai21  g078(.a(new_n133_), .b(new_n80_), .c(x31), .O(new_n134_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nor2   g080(.a(x31), .b(x30), .O(new_n136_));
  nand2  g081(.a(new_n136_), .b(new_n123_), .O(new_n137_));
  inv1   g082(.a(new_n137_), .O(new_n138_));
  nand3  g083(.a(new_n138_), .b(new_n135_), .c(new_n84_), .O(new_n139_));
  nand2  g084(.a(new_n139_), .b(new_n134_), .O(new_n140_));
  nand2  g085(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g086(.a(x03), .O(new_n142_));
  aoi21  g087(.a(new_n54_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g088(.a(new_n143_), .b(new_n141_), .O(z12));
  nor2   g089(.a(x32), .b(x31), .O(new_n146_));
  nand4  g090(.a(new_n146_), .b(new_n132_), .c(new_n118_), .d(new_n131_), .O(new_n147_));
  oai21  g091(.a(new_n147_), .b(new_n83_), .c(x33), .O(new_n148_));
  nor2   g092(.a(x33), .b(x32), .O(new_n149_));
  nand4  g093(.a(new_n149_), .b(new_n136_), .c(new_n123_), .d(new_n110_), .O(new_n150_));
  inv1   g094(.a(new_n150_), .O(new_n151_));
  nand2  g095(.a(new_n151_), .b(new_n94_), .O(new_n152_));
  nand2  g096(.a(new_n152_), .b(new_n148_), .O(new_n153_));
  nand2  g097(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g098(.a(x01), .O(new_n155_));
  aoi21  g099(.a(new_n54_), .b(new_n155_), .c(x18), .O(new_n156_));
  nand2  g100(.a(new_n156_), .b(new_n154_), .O(z14));
  oai21  g101(.a(new_n150_), .b(new_n101_), .c(x34), .O(new_n158_));
  nand2  g102(.a(new_n123_), .b(new_n110_), .O(new_n159_));
  inv1   g103(.a(new_n159_), .O(new_n160_));
  inv1   g104(.a(x30), .O(new_n161_));
  inv1   g105(.a(x31), .O(new_n162_));
  nand2  g106(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  inv1   g107(.a(x32), .O(new_n164_));
  inv1   g108(.a(x33), .O(new_n165_));
  inv1   g109(.a(x34), .O(new_n166_));
  nand3  g110(.a(new_n166_), .b(new_n165_), .c(new_n164_), .O(new_n167_));
  nor2   g111(.a(new_n167_), .b(new_n163_), .O(new_n168_));
  nand3  g112(.a(new_n168_), .b(new_n160_), .c(new_n94_), .O(new_n169_));
  nand2  g113(.a(new_n169_), .b(new_n158_), .O(new_n170_));
  nand2  g114(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g115(.a(x00), .O(new_n172_));
  aoi21  g116(.a(new_n54_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g117(.a(new_n173_), .b(new_n171_), .O(z15));
  zero   g118(.O(z00));
  zero   g119(.O(z01));
  zero   g120(.O(z09));
  zero   g121(.O(z11));
  zero   g122(.O(z13));
endmodule


