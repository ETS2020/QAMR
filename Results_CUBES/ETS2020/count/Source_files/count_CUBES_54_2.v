// Benchmark "FAU" written by ABC on Tue Jul  7 19:18:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  inv1   g000(.a(x16), .O(new_n55_));
  inv1   g001(.a(x17), .O(new_n56_));
  inv1   g002(.a(x19), .O(new_n57_));
  inv1   g003(.a(x20), .O(new_n58_));
  inv1   g004(.a(x21), .O(new_n59_));
  nand4  g005(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  nor2   g006(.a(x22), .b(x21), .O(new_n61_));
  nor3   g007(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  aoi22  g008(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(x22), .O(new_n63_));
  inv1   g009(.a(x12), .O(new_n64_));
  aoi21  g010(.a(new_n55_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g011(.a(new_n63_), .b(new_n55_), .c(new_n65_), .O(z03));
  nor2   g012(.a(x19), .b(x17), .O(new_n67_));
  nand3  g013(.a(new_n61_), .b(new_n67_), .c(new_n58_), .O(new_n68_));
  nor3   g014(.a(x23), .b(x22), .c(x21), .O(new_n69_));
  aoi22  g015(.a(new_n69_), .b(new_n62_), .c(new_n68_), .d(x23), .O(new_n70_));
  inv1   g016(.a(x11), .O(new_n71_));
  aoi21  g017(.a(new_n55_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g018(.a(new_n70_), .b(new_n55_), .c(new_n72_), .O(z04));
  inv1   g019(.a(x24), .O(new_n74_));
  aoi21  g020(.a(new_n69_), .b(new_n62_), .c(new_n74_), .O(new_n75_));
  nor2   g021(.a(x24), .b(x23), .O(new_n76_));
  nand4  g022(.a(new_n76_), .b(new_n61_), .c(new_n67_), .d(new_n58_), .O(new_n77_));
  inv1   g023(.a(new_n77_), .O(new_n78_));
  oai21  g024(.a(new_n78_), .b(new_n75_), .c(x16), .O(new_n79_));
  inv1   g025(.a(x10), .O(new_n80_));
  aoi21  g026(.a(new_n55_), .b(new_n80_), .c(x18), .O(new_n81_));
  nand2  g027(.a(new_n81_), .b(new_n79_), .O(z05));
  inv1   g028(.a(x22), .O(new_n83_));
  inv1   g029(.a(x23), .O(new_n84_));
  inv1   g030(.a(x25), .O(new_n85_));
  nand4  g031(.a(new_n85_), .b(new_n74_), .c(new_n84_), .d(new_n83_), .O(new_n86_));
  nor2   g032(.a(new_n86_), .b(new_n60_), .O(new_n87_));
  aoi21  g033(.a(new_n77_), .b(x25), .c(new_n87_), .O(new_n88_));
  inv1   g034(.a(x09), .O(new_n89_));
  aoi21  g035(.a(new_n55_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g036(.a(new_n88_), .b(new_n55_), .c(new_n90_), .O(z06));
  nor2   g037(.a(x21), .b(x20), .O(new_n93_));
  nor2   g038(.a(x23), .b(x22), .O(new_n94_));
  nor3   g039(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  nand4  g040(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n67_), .O(new_n96_));
  nor2   g041(.a(x27), .b(x26), .O(new_n97_));
  nand3  g042(.a(new_n97_), .b(new_n76_), .c(new_n85_), .O(new_n98_));
  nor2   g043(.a(new_n98_), .b(new_n68_), .O(new_n99_));
  aoi21  g044(.a(new_n96_), .b(x27), .c(new_n99_), .O(new_n100_));
  inv1   g045(.a(x07), .O(new_n101_));
  aoi21  g046(.a(new_n55_), .b(new_n101_), .c(x18), .O(new_n102_));
  oai21  g047(.a(new_n100_), .b(new_n55_), .c(new_n102_), .O(z08));
  nor3   g048(.a(x27), .b(x26), .c(x25), .O(new_n104_));
  nand4  g049(.a(new_n104_), .b(new_n76_), .c(new_n62_), .d(new_n61_), .O(new_n105_));
  nor2   g050(.a(x25), .b(x24), .O(new_n106_));
  nor2   g051(.a(x28), .b(x27), .O(new_n107_));
  nor2   g052(.a(x26), .b(x23), .O(new_n108_));
  nand3  g053(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  nor2   g054(.a(new_n109_), .b(new_n68_), .O(new_n110_));
  aoi21  g055(.a(new_n105_), .b(x28), .c(new_n110_), .O(new_n111_));
  inv1   g056(.a(x06), .O(new_n112_));
  aoi21  g057(.a(new_n55_), .b(new_n112_), .c(x18), .O(new_n113_));
  oai21  g058(.a(new_n111_), .b(new_n55_), .c(new_n113_), .O(z09));
  inv1   g059(.a(x29), .O(new_n115_));
  nor3   g060(.a(x29), .b(x28), .c(x27), .O(new_n116_));
  nand4  g061(.a(new_n116_), .b(new_n95_), .c(new_n69_), .d(new_n62_), .O(new_n117_));
  oai21  g062(.a(new_n110_), .b(new_n115_), .c(new_n117_), .O(new_n118_));
  nand2  g063(.a(new_n118_), .b(x16), .O(new_n119_));
  inv1   g064(.a(x05), .O(new_n120_));
  aoi21  g065(.a(new_n55_), .b(new_n120_), .c(x18), .O(new_n121_));
  nand2  g066(.a(new_n121_), .b(new_n119_), .O(z10));
  nand4  g067(.a(new_n94_), .b(new_n67_), .c(new_n59_), .d(new_n58_), .O(new_n124_));
  nor2   g068(.a(x26), .b(x25), .O(new_n125_));
  nor2   g069(.a(x30), .b(x29), .O(new_n126_));
  nand4  g070(.a(new_n126_), .b(new_n107_), .c(new_n125_), .d(new_n74_), .O(new_n127_));
  oai21  g071(.a(new_n127_), .b(new_n124_), .c(x31), .O(new_n128_));
  nor2   g072(.a(x29), .b(x28), .O(new_n129_));
  nor2   g073(.a(x31), .b(x30), .O(new_n130_));
  nand2  g074(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g075(.a(new_n131_), .O(new_n132_));
  nand3  g076(.a(new_n132_), .b(new_n104_), .c(new_n78_), .O(new_n133_));
  nand2  g077(.a(new_n133_), .b(new_n128_), .O(new_n134_));
  nand2  g078(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g079(.a(x03), .O(new_n136_));
  aoi21  g080(.a(new_n55_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g081(.a(new_n137_), .b(new_n135_), .O(z12));
  nand4  g082(.a(new_n130_), .b(new_n129_), .c(new_n97_), .d(new_n85_), .O(new_n139_));
  oai21  g083(.a(new_n139_), .b(new_n77_), .c(x32), .O(new_n140_));
  nor2   g084(.a(x32), .b(x31), .O(new_n141_));
  nand4  g085(.a(new_n141_), .b(new_n126_), .c(new_n107_), .d(new_n125_), .O(new_n142_));
  inv1   g086(.a(new_n142_), .O(new_n143_));
  nand2  g087(.a(new_n143_), .b(new_n78_), .O(new_n144_));
  nand2  g088(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  nand2  g089(.a(new_n145_), .b(x16), .O(new_n146_));
  inv1   g090(.a(x02), .O(new_n147_));
  aoi21  g091(.a(new_n55_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand2  g092(.a(new_n148_), .b(new_n146_), .O(z13));
  oai21  g093(.a(new_n142_), .b(new_n77_), .c(x33), .O(new_n150_));
  nor2   g094(.a(x33), .b(x32), .O(new_n151_));
  nand4  g095(.a(new_n151_), .b(new_n130_), .c(new_n129_), .d(new_n97_), .O(new_n152_));
  inv1   g096(.a(new_n152_), .O(new_n153_));
  nand2  g097(.a(new_n153_), .b(new_n87_), .O(new_n154_));
  nand2  g098(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  nand2  g099(.a(new_n155_), .b(x16), .O(new_n156_));
  inv1   g100(.a(x01), .O(new_n157_));
  aoi21  g101(.a(new_n55_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand2  g102(.a(new_n158_), .b(new_n156_), .O(z14));
  nand4  g103(.a(new_n106_), .b(new_n94_), .c(new_n93_), .d(new_n67_), .O(new_n160_));
  oai21  g104(.a(new_n152_), .b(new_n160_), .c(x34), .O(new_n161_));
  nand2  g105(.a(new_n129_), .b(new_n97_), .O(new_n162_));
  inv1   g106(.a(new_n162_), .O(new_n163_));
  inv1   g107(.a(x30), .O(new_n164_));
  inv1   g108(.a(x31), .O(new_n165_));
  nand2  g109(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  inv1   g110(.a(x32), .O(new_n167_));
  inv1   g111(.a(x33), .O(new_n168_));
  inv1   g112(.a(x34), .O(new_n169_));
  nand3  g113(.a(new_n169_), .b(new_n168_), .c(new_n167_), .O(new_n170_));
  nor2   g114(.a(new_n170_), .b(new_n166_), .O(new_n171_));
  nand3  g115(.a(new_n171_), .b(new_n163_), .c(new_n87_), .O(new_n172_));
  nand2  g116(.a(new_n172_), .b(new_n161_), .O(new_n173_));
  nand2  g117(.a(new_n173_), .b(x16), .O(new_n174_));
  inv1   g118(.a(x00), .O(new_n175_));
  aoi21  g119(.a(new_n55_), .b(new_n175_), .c(x18), .O(new_n176_));
  nand2  g120(.a(new_n176_), .b(new_n174_), .O(z15));
  zero   g121(.O(z00));
  zero   g122(.O(z01));
  zero   g123(.O(z02));
  zero   g124(.O(z07));
  zero   g125(.O(z11));
endmodule


