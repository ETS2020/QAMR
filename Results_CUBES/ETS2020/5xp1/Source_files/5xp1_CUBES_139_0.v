// Benchmark "FAU" written by ABC on Mon Jul  6 15:57:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n96_,
    new_n97_, new_n98_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  inv1   g04(.a(x5), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g09(.a(x1), .b(x0), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n19_), .c(x5), .O(new_n28_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n28_), .O(new_n29_));
  nand3  g12(.a(new_n24_), .b(new_n21_), .c(new_n20_), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(x6), .c(x5), .d(new_n18_), .O(new_n31_));
  oai21  g14(.a(new_n29_), .b(new_n18_), .c(new_n31_), .O(z0));
  inv1   g15(.a(x2), .O(new_n33_));
  nand2  g16(.a(x4), .b(x1), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n19_), .c(new_n33_), .O(new_n35_));
  nand2  g18(.a(x4), .b(x3), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n19_), .c(new_n21_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n35_), .c(x0), .O(new_n38_));
  oai21  g21(.a(new_n19_), .b(new_n18_), .c(new_n38_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n22_), .O(new_n40_));
  nand3  g23(.a(x6), .b(new_n18_), .c(new_n21_), .O(new_n41_));
  nand2  g24(.a(new_n19_), .b(x4), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g26(.a(new_n19_), .b(x4), .c(new_n21_), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  aoi21  g28(.a(new_n43_), .b(new_n24_), .c(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n19_), .b(new_n18_), .O(new_n47_));
  oai21  g30(.a(new_n46_), .b(x0), .c(new_n47_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x5), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n40_), .O(z1));
  nand2  g33(.a(new_n22_), .b(new_n33_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x0), .O(new_n52_));
  nand3  g35(.a(x5), .b(x3), .c(x2), .O(new_n53_));
  oai21  g36(.a(x5), .b(x0), .c(x1), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(x4), .O(new_n56_));
  nor2   g39(.a(x5), .b(x0), .O(new_n57_));
  nand3  g40(.a(new_n24_), .b(x5), .c(new_n20_), .O(new_n58_));
  aoi21  g41(.a(new_n58_), .b(new_n51_), .c(x1), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n57_), .c(new_n18_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x6), .O(new_n62_));
  nand2  g45(.a(new_n26_), .b(x4), .O(new_n63_));
  oai21  g46(.a(x3), .b(x2), .c(x0), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(new_n53_), .c(new_n21_), .O(new_n65_));
  nor2   g48(.a(new_n22_), .b(new_n20_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n65_), .c(new_n18_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n19_), .O(new_n69_));
  inv1   g52(.a(new_n27_), .O(new_n70_));
  inv1   g53(.a(x3), .O(z8));
  nand2  g54(.a(z8), .b(new_n33_), .O(new_n72_));
  inv1   g55(.a(new_n72_), .O(new_n73_));
  nand4  g56(.a(new_n73_), .b(new_n70_), .c(new_n22_), .d(x4), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n69_), .c(new_n62_), .O(z2));
  aoi21  g58(.a(new_n64_), .b(new_n24_), .c(new_n19_), .O(new_n77_));
  nand2  g59(.a(new_n24_), .b(new_n20_), .O(new_n78_));
  aoi21  g60(.a(new_n78_), .b(new_n72_), .c(x6), .O(new_n79_));
  oai21  g61(.a(new_n79_), .b(new_n77_), .c(x1), .O(new_n80_));
  nand2  g62(.a(z8), .b(new_n20_), .O(new_n81_));
  nor2   g63(.a(x6), .b(new_n33_), .O(new_n82_));
  nand2  g64(.a(new_n81_), .b(x2), .O(new_n83_));
  aoi22  g65(.a(new_n83_), .b(x6), .c(new_n82_), .d(new_n81_), .O(new_n84_));
  oai21  g66(.a(new_n84_), .b(x1), .c(new_n80_), .O(z4));
  nand2  g67(.a(x3), .b(new_n33_), .O(new_n86_));
  nand2  g68(.a(z8), .b(x2), .O(new_n87_));
  oai21  g69(.a(new_n86_), .b(new_n21_), .c(new_n87_), .O(new_n88_));
  nand2  g70(.a(new_n88_), .b(new_n20_), .O(new_n89_));
  inv1   g71(.a(new_n24_), .O(new_n90_));
  aoi21  g72(.a(x3), .b(x1), .c(x2), .O(new_n91_));
  oai21  g73(.a(new_n91_), .b(new_n90_), .c(x0), .O(new_n92_));
  nand2  g74(.a(new_n92_), .b(new_n89_), .O(z5));
  xor2a  g75(.a(new_n86_), .b(new_n21_), .O(z6));
  nand2  g76(.a(new_n87_), .b(new_n86_), .O(z7));
  nand2  g77(.a(new_n90_), .b(x1), .O(new_n96_));
  nor2   g78(.a(x6), .b(x0), .O(new_n97_));
  nand2  g79(.a(x5), .b(x4), .O(new_n98_));
  aoi21  g80(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(z9));
  zero   g81(.O(z3));
endmodule


