// Benchmark "FAU" written by ABC on Mon Jul  6 14:34:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x03), .O(new_n43_));
  inv1   g09(.a(x16), .O(new_n44_));
  aoi21  g10(.a(new_n43_), .b(x02), .c(new_n44_), .O(new_n45_));
  nor3   g11(.a(new_n45_), .b(x06), .c(new_n40_), .O(z02));
  xor2a  g12(.a(x07), .b(x06), .O(new_n47_));
  nor2   g13(.a(new_n45_), .b(new_n40_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z03));
  inv1   g16(.a(x08), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(x07), .c(new_n43_), .d(x02), .O(new_n52_));
  oai21  g18(.a(x16), .b(x07), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x06), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(x08), .b(new_n43_), .c(x02), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nor2   g23(.a(x16), .b(x06), .O(new_n58_));
  aoi21  g24(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(new_n54_), .c(new_n40_), .O(z04));
  inv1   g26(.a(new_n55_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x08), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nand2  g29(.a(x04), .b(new_n35_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n43_), .c(new_n44_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n61_), .b(new_n66_), .c(x08), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n63_), .O(z05));
  nor2   g36(.a(new_n65_), .b(new_n40_), .O(new_n71_));
  nand2  g37(.a(new_n67_), .b(x10), .O(new_n72_));
  inv1   g38(.a(new_n62_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(z06));
  inv1   g42(.a(new_n74_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n62_), .c(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n73_), .c(new_n66_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n71_), .O(z07));
  nand2  g47(.a(new_n79_), .b(new_n66_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n62_), .c(x12), .O(new_n83_));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n74_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n83_), .c(new_n71_), .O(z08));
  nand2  g54(.a(new_n56_), .b(x16), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  inv1   g59(.a(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n66_), .c(x07), .d(x06), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g62(.a(new_n44_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(new_n89_), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n85_), .b(new_n62_), .c(x13), .O(new_n99_));
  nand2  g65(.a(x16), .b(x03), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x04), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand3  g68(.a(new_n79_), .b(new_n92_), .c(new_n91_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n89_), .c(new_n61_), .d(new_n66_), .O(new_n105_));
  nand2  g71(.a(new_n45_), .b(x14), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n40_), .O(z10));
  nor2   g73(.a(new_n97_), .b(x03), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n40_), .O(z12));
  and2   g75(.a(x17), .b(x04), .O(z14));
  buf    g76(.a(x02), .O(z11));
  buf    g77(.a(x04), .O(z13));
endmodule


