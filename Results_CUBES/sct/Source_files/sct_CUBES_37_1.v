// Benchmark "FAU" written by ABC on Mon Jul  6 14:34:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  nor2   g08(.a(x06), .b(new_n40_), .O(z02));
  inv1   g09(.a(x07), .O(new_n44_));
  inv1   g10(.a(x16), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g13(.a(new_n45_), .b(x06), .O(new_n48_));
  oai21  g14(.a(new_n47_), .b(new_n45_), .c(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  inv1   g16(.a(x06), .O(new_n51_));
  nand2  g17(.a(x07), .b(new_n51_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n50_), .c(new_n40_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand3  g20(.a(x16), .b(new_n54_), .c(x07), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(x07), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n46_), .c(x02), .O(new_n57_));
  oai21  g23(.a(x16), .b(x07), .c(x06), .O(new_n58_));
  nor2   g24(.a(new_n44_), .b(new_n51_), .O(new_n59_));
  nor2   g25(.a(x16), .b(x08), .O(new_n60_));
  aoi22  g26(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(x08), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n57_), .c(new_n40_), .O(z04));
  nand2  g28(.a(new_n59_), .b(x08), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  oai21  g30(.a(new_n40_), .b(x02), .c(new_n46_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x16), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n59_), .b(new_n67_), .c(x08), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n66_), .c(new_n64_), .d(x04), .O(z05));
  aoi21  g35(.a(new_n65_), .b(x16), .c(new_n40_), .O(new_n70_));
  nand2  g36(.a(new_n68_), .b(x10), .O(new_n71_));
  inv1   g37(.a(new_n63_), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(z06));
  inv1   g41(.a(new_n73_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n63_), .c(x11), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n72_), .c(new_n67_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n70_), .O(z07));
  nand2  g46(.a(new_n78_), .b(new_n67_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n63_), .c(x12), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n73_), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(new_n70_), .O(z08));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g57(.a(x10), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n67_), .c(x08), .d(x07), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nor2   g60(.a(new_n45_), .b(x02), .O(new_n95_));
  aoi21  g61(.a(new_n94_), .b(new_n49_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n84_), .b(new_n63_), .c(x13), .O(new_n97_));
  nand2  g63(.a(x16), .b(x03), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(x04), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand3  g66(.a(new_n78_), .b(new_n90_), .c(new_n89_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor3   g68(.a(x09), .b(new_n54_), .c(new_n44_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n102_), .c(new_n49_), .O(new_n104_));
  nand3  g70(.a(new_n47_), .b(x16), .c(x14), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n40_), .O(z10));
  nor2   g72(.a(new_n95_), .b(x03), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(new_n40_), .O(z12));
  and2   g74(.a(x17), .b(x04), .O(z14));
  buf    g75(.a(x02), .O(z11));
  buf    g76(.a(x04), .O(z13));
endmodule


