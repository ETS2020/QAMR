// Benchmark "FAU" written by ABC on Mon Jul  6 14:22:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  and2   g02(.a(x21), .b(x20), .O(new_n47_));
  nand2  g03(.a(x23), .b(x22), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  nand2  g08(.a(x09), .b(new_n52_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  inv1   g10(.a(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(x10), .O(new_n56_));
  nand4  g12(.a(new_n56_), .b(new_n54_), .c(new_n51_), .d(new_n46_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z00));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n52_), .b(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  inv1   g30(.a(x26), .O(new_n75_));
  nand4  g31(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n76_));
  nand3  g32(.a(x21), .b(x20), .c(x11), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nor2   g34(.a(new_n75_), .b(x10), .O(new_n79_));
  aoi22  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .d(new_n55_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n53_), .c(new_n59_), .O(z09));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nand4  g38(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(x19), .O(new_n84_));
  nor2   g40(.a(x20), .b(new_n55_), .O(new_n85_));
  aoi21  g41(.a(new_n84_), .b(x20), .c(new_n85_), .O(new_n86_));
  inv1   g42(.a(x10), .O(new_n87_));
  nand2  g43(.a(new_n54_), .b(new_n87_), .O(new_n88_));
  oai22  g44(.a(new_n88_), .b(new_n86_), .c(new_n52_), .d(new_n61_), .O(z10));
  nand3  g45(.a(x26), .b(x25), .c(x24), .O(new_n90_));
  nand3  g46(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  nand2  g47(.a(x20), .b(x19), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(new_n94_));
  nor2   g50(.a(new_n92_), .b(x21), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(x21), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n88_), .c(new_n63_), .O(z11));
  nand2  g53(.a(x23), .b(x14), .O(new_n98_));
  nand3  g54(.a(x21), .b(x20), .c(x19), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n98_), .b(new_n90_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(x21), .c(x20), .d(x19), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  aoi21  g60(.a(new_n101_), .b(x22), .c(new_n104_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n88_), .c(new_n65_), .O(z12));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand4  g63(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g66(.a(x22), .b(x21), .O(new_n111_));
  nor2   g67(.a(new_n111_), .b(x23), .O(new_n112_));
  aoi22  g68(.a(new_n112_), .b(new_n93_), .c(new_n110_), .d(x23), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n88_), .c(new_n67_), .O(z13));
  nand3  g70(.a(x26), .b(x25), .c(x16), .O(new_n115_));
  nand3  g71(.a(x21), .b(x20), .c(x19), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n49_), .O(new_n118_));
  nor3   g74(.a(new_n116_), .b(new_n48_), .c(x24), .O(new_n119_));
  aoi21  g75(.a(new_n118_), .b(x24), .c(new_n119_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n88_), .c(new_n69_), .O(z14));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand3  g78(.a(x24), .b(x23), .c(x22), .O(new_n123_));
  inv1   g79(.a(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n117_), .O(new_n125_));
  inv1   g81(.a(x25), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(x24), .c(x23), .d(x22), .O(new_n127_));
  inv1   g83(.a(new_n127_), .O(new_n128_));
  aoi22  g84(.a(new_n128_), .b(new_n117_), .c(new_n125_), .d(x25), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n88_), .c(new_n71_), .O(z15));
  inv1   g86(.a(new_n76_), .O(new_n131_));
  nor2   g87(.a(new_n55_), .b(x18), .O(new_n132_));
  nand3  g88(.a(new_n132_), .b(new_n131_), .c(new_n47_), .O(new_n133_));
  nand4  g89(.a(new_n75_), .b(x25), .c(x22), .d(x21), .O(new_n134_));
  nand4  g90(.a(x24), .b(x23), .c(x20), .d(x19), .O(new_n135_));
  nor2   g91(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  aoi21  g92(.a(new_n133_), .b(new_n79_), .c(new_n136_), .O(new_n137_));
  oai21  g93(.a(new_n137_), .b(new_n53_), .c(new_n73_), .O(z16));
endmodule


