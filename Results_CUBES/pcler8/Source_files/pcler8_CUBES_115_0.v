// Benchmark "FAU" written by ABC on Mon Jul  6 14:22:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x21), .c(x20), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x19), .c(new_n48_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x26), .c(x25), .d(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  inv1   g21(.a(x05), .O(new_n66_));
  nor2   g22(.a(new_n56_), .b(new_n66_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  and2   g27(.a(x24), .b(x23), .O(new_n72_));
  inv1   g28(.a(x25), .O(new_n73_));
  inv1   g29(.a(x26), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(new_n56_), .b(new_n55_), .c(new_n82_), .O(z09));
  nand2  g39(.a(new_n77_), .b(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n76_), .c(x19), .O(new_n85_));
  inv1   g41(.a(x19), .O(new_n86_));
  nor2   g42(.a(x20), .b(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n85_), .b(x20), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n80_), .c(new_n58_), .O(z10));
  nand2  g45(.a(new_n75_), .b(x24), .O(new_n90_));
  nand2  g46(.a(new_n46_), .b(x13), .O(new_n91_));
  nand2  g47(.a(x20), .b(x19), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(new_n94_));
  nor2   g50(.a(new_n92_), .b(x21), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(x21), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n80_), .c(new_n60_), .O(z11));
  nand2  g53(.a(x23), .b(x14), .O(new_n98_));
  nand3  g54(.a(x21), .b(x20), .c(x19), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n98_), .b(new_n90_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand3  g58(.a(new_n93_), .b(new_n102_), .c(x21), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  aoi21  g60(.a(new_n101_), .b(x22), .c(new_n104_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n80_), .c(new_n62_), .O(z12));
  inv1   g62(.a(x23), .O(new_n107_));
  nand3  g63(.a(new_n75_), .b(x24), .c(x15), .O(new_n108_));
  nand2  g64(.a(new_n93_), .b(new_n77_), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  nand2  g67(.a(new_n77_), .b(new_n107_), .O(new_n112_));
  nor2   g68(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n111_), .c(new_n81_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n64_), .O(z13));
  nand2  g71(.a(new_n75_), .b(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n100_), .c(new_n46_), .O(new_n117_));
  nor3   g73(.a(new_n99_), .b(new_n45_), .c(x24), .O(new_n118_));
  aoi21  g74(.a(new_n117_), .b(x24), .c(new_n118_), .O(new_n119_));
  oai22  g75(.a(new_n119_), .b(new_n80_), .c(new_n56_), .d(new_n66_), .O(z14));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(new_n100_), .c(new_n46_), .d(x24), .O(new_n122_));
  nand3  g78(.a(new_n46_), .b(new_n73_), .c(x24), .O(new_n123_));
  inv1   g79(.a(new_n123_), .O(new_n124_));
  aoi22  g80(.a(new_n124_), .b(new_n100_), .c(new_n122_), .d(x25), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n80_), .c(new_n68_), .O(z15));
  nand3  g82(.a(x25), .b(x24), .c(x21), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand4  g84(.a(x23), .b(x22), .c(x19), .d(new_n128_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n127_), .c(x26), .O(new_n130_));
  nor2   g86(.a(x26), .b(new_n73_), .O(new_n131_));
  nand4  g87(.a(new_n131_), .b(new_n93_), .c(new_n77_), .d(new_n72_), .O(new_n132_));
  aoi21  g88(.a(new_n132_), .b(new_n130_), .c(x08), .O(new_n133_));
  nor2   g89(.a(new_n74_), .b(x20), .O(new_n134_));
  nor2   g90(.a(x10), .b(new_n49_), .O(new_n135_));
  oai21  g91(.a(new_n134_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n70_), .O(z16));
endmodule


