// Benchmark "FAU" written by ABC on Mon Jul  6 14:21:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nor2   g01(.a(x10), .b(x08), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor3   g04(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(z00));
  nand2  g05(.a(x08), .b(x00), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(z01));
  inv1   g07(.a(x01), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(z02));
  nand2  g10(.a(x08), .b(x02), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z03));
  nand2  g12(.a(x08), .b(x03), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z04));
  nand2  g14(.a(x08), .b(x04), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z05));
  inv1   g16(.a(x05), .O(new_n61_));
  nor2   g17(.a(new_n53_), .b(new_n61_), .O(z06));
  nand2  g18(.a(x08), .b(x06), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z07));
  nand2  g20(.a(x08), .b(x07), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z08));
  inv1   g22(.a(x09), .O(new_n67_));
  and2   g23(.a(x22), .b(x21), .O(new_n68_));
  nand3  g24(.a(new_n68_), .b(x20), .c(x11), .O(new_n69_));
  oai22  g25(.a(new_n69_), .b(new_n45_), .c(x19), .d(new_n67_), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n50_), .O(z09));
  inv1   g28(.a(new_n45_), .O(new_n73_));
  xor2a  g29(.a(x20), .b(x19), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x12), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  aoi22  g32(.a(new_n76_), .b(new_n73_), .c(new_n74_), .d(x09), .O(new_n77_));
  oai22  g33(.a(new_n77_), .b(new_n47_), .c(new_n53_), .d(new_n52_), .O(z10));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  xor2a  g35(.a(new_n79_), .b(x21), .O(new_n80_));
  nand2  g36(.a(new_n68_), .b(x13), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  aoi22  g38(.a(new_n82_), .b(new_n73_), .c(new_n80_), .d(x09), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n47_), .c(new_n55_), .O(z11));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x22), .O(new_n86_));
  inv1   g42(.a(x22), .O(new_n87_));
  nand3  g43(.a(new_n79_), .b(new_n87_), .c(x21), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n67_), .O(new_n89_));
  inv1   g45(.a(x14), .O(new_n90_));
  inv1   g46(.a(x24), .O(new_n91_));
  nand2  g47(.a(x26), .b(x25), .O(new_n92_));
  nand2  g48(.a(x23), .b(x22), .O(new_n93_));
  nor4   g49(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n89_), .c(new_n46_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n57_), .O(z12));
  nand2  g52(.a(new_n48_), .b(x23), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  nand3  g54(.a(new_n68_), .b(new_n79_), .c(new_n98_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n97_), .c(new_n67_), .O(new_n100_));
  nand2  g56(.a(x23), .b(x15), .O(new_n101_));
  nor3   g57(.a(new_n101_), .b(new_n92_), .c(new_n91_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n46_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n59_), .O(z13));
  inv1   g60(.a(new_n85_), .O(new_n105_));
  inv1   g61(.a(new_n93_), .O(new_n106_));
  nand3  g62(.a(x26), .b(x25), .c(x16), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  nand3  g64(.a(new_n91_), .b(x23), .c(x09), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  aoi21  g66(.a(new_n108_), .b(x24), .c(new_n110_), .O(new_n111_));
  oai22  g67(.a(new_n111_), .b(new_n47_), .c(new_n53_), .d(new_n61_), .O(z14));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(x23), .c(x22), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n85_), .c(x25), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand3  g72(.a(new_n106_), .b(new_n105_), .c(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n91_), .O(new_n118_));
  nand3  g74(.a(x25), .b(new_n91_), .c(x09), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n118_), .c(new_n46_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n63_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand3  g80(.a(x25), .b(x24), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(x09), .O(new_n126_));
  oai21  g82(.a(new_n93_), .b(new_n85_), .c(x24), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n126_), .c(new_n123_), .O(new_n128_));
  nand4  g84(.a(new_n123_), .b(x25), .c(x24), .d(x23), .O(new_n129_));
  nor2   g85(.a(new_n129_), .b(new_n48_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n128_), .c(new_n46_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n65_), .O(z16));
endmodule


