// Benchmark "FAU" written by ABC on Mon Jul  6 14:21:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x21), .c(x20), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x19), .c(new_n48_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x26), .c(x25), .d(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  inv1   g13(.a(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  nor2   g16(.a(new_n58_), .b(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z05));
  inv1   g21(.a(x05), .O(new_n66_));
  nor2   g22(.a(new_n58_), .b(new_n66_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  inv1   g27(.a(x19), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand2  g32(.a(new_n50_), .b(new_n48_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(new_n55_), .O(z09));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n73_), .c(x19), .O(new_n80_));
  nor2   g36(.a(x20), .b(new_n72_), .O(new_n81_));
  aoi21  g37(.a(new_n80_), .b(x20), .c(new_n81_), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(new_n77_), .c(new_n58_), .d(new_n57_), .O(z10));
  nand3  g39(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  nand3  g40(.a(x23), .b(x22), .c(x13), .O(new_n85_));
  nand2  g41(.a(x20), .b(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  oai21  g43(.a(new_n85_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nor2   g44(.a(new_n86_), .b(x21), .O(new_n89_));
  aoi21  g45(.a(new_n88_), .b(x21), .c(new_n89_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(new_n77_), .c(new_n58_), .d(new_n60_), .O(z11));
  nand2  g47(.a(x23), .b(x14), .O(new_n92_));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n92_), .b(new_n84_), .c(new_n94_), .O(new_n95_));
  inv1   g51(.a(x21), .O(new_n96_));
  nor2   g52(.a(x22), .b(new_n96_), .O(new_n97_));
  aoi22  g53(.a(new_n97_), .b(new_n87_), .c(new_n95_), .d(x22), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n77_), .c(new_n62_), .O(z12));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n100_));
  nand4  g56(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  inv1   g59(.a(x22), .O(new_n104_));
  nor3   g60(.a(x23), .b(new_n104_), .c(new_n96_), .O(new_n105_));
  aoi22  g61(.a(new_n105_), .b(new_n87_), .c(new_n103_), .d(x23), .O(new_n106_));
  oai22  g62(.a(new_n106_), .b(new_n77_), .c(new_n58_), .d(new_n64_), .O(z13));
  nand3  g63(.a(x26), .b(x25), .c(x16), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n94_), .c(new_n46_), .O(new_n109_));
  nor3   g65(.a(new_n93_), .b(new_n45_), .c(x24), .O(new_n110_));
  aoi21  g66(.a(new_n109_), .b(x24), .c(new_n110_), .O(new_n111_));
  oai22  g67(.a(new_n111_), .b(new_n77_), .c(new_n58_), .d(new_n66_), .O(z14));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n94_), .c(new_n46_), .d(x24), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n116_));
  nor3   g72(.a(new_n116_), .b(new_n93_), .c(new_n45_), .O(new_n117_));
  aoi21  g73(.a(new_n114_), .b(x25), .c(new_n117_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n77_), .c(new_n68_), .O(z15));
  nand2  g75(.a(x26), .b(new_n104_), .O(new_n120_));
  inv1   g76(.a(x26), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(x25), .c(x24), .d(x22), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n93_), .c(new_n120_), .O(new_n123_));
  nor2   g79(.a(new_n96_), .b(x18), .O(new_n124_));
  and2   g80(.a(x25), .b(x23), .O(new_n125_));
  inv1   g81(.a(x24), .O(new_n126_));
  aoi21  g82(.a(x25), .b(new_n126_), .c(new_n86_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(new_n128_));
  aoi22  g84(.a(new_n128_), .b(x26), .c(new_n123_), .d(x23), .O(new_n129_));
  nand2  g85(.a(new_n48_), .b(x09), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n129_), .c(new_n70_), .O(z16));
endmodule


