// Benchmark "FAU" written by ABC on Mon Jul  6 13:30:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n122_, new_n124_,
    new_n125_;
  inv1   g00(.a(x02), .O(new_n46_));
  nor2   g01(.a(x01), .b(x00), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(x17), .O(new_n49_));
  inv1   g04(.a(x18), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nand4  g06(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x09), .O(new_n52_));
  nor2   g07(.a(new_n52_), .b(new_n48_), .O(z02));
  nor2   g08(.a(x19), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  nand3  g10(.a(new_n47_), .b(new_n49_), .c(new_n46_), .O(new_n56_));
  inv1   g11(.a(x01), .O(new_n57_));
  nor2   g12(.a(x09), .b(new_n57_), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nand4  g14(.a(x12), .b(x11), .c(x10), .d(x00), .O(new_n60_));
  oai22  g15(.a(new_n60_), .b(new_n59_), .c(new_n56_), .d(new_n55_), .O(z03));
  nand2  g16(.a(x01), .b(x00), .O(new_n62_));
  inv1   g17(.a(x09), .O(new_n63_));
  inv1   g18(.a(x11), .O(new_n64_));
  inv1   g19(.a(x12), .O(new_n65_));
  nand4  g20(.a(new_n65_), .b(new_n64_), .c(x10), .d(new_n63_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n62_), .O(z04));
  nand4  g22(.a(x10), .b(x09), .c(x01), .d(x00), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z05));
  inv1   g24(.a(x10), .O(new_n70_));
  nor4   g25(.a(new_n62_), .b(new_n64_), .c(new_n70_), .d(x09), .O(z06));
  inv1   g26(.a(x00), .O(new_n72_));
  nand2  g27(.a(x02), .b(new_n57_), .O(new_n73_));
  nand4  g28(.a(new_n58_), .b(new_n65_), .c(x11), .d(x10), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(z07));
  inv1   g30(.a(x03), .O(new_n76_));
  nor2   g31(.a(x06), .b(x05), .O(new_n77_));
  nor2   g32(.a(x08), .b(x07), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(x04), .d(new_n76_), .O(new_n79_));
  nand2  g34(.a(new_n47_), .b(x02), .O(new_n80_));
  nand3  g35(.a(x19), .b(new_n50_), .c(x17), .O(new_n81_));
  nor3   g36(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(z08));
  inv1   g37(.a(new_n73_), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  inv1   g39(.a(x16), .O(new_n85_));
  nand3  g40(.a(x20), .b(new_n85_), .c(new_n84_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nor2   g42(.a(new_n65_), .b(x11), .O(new_n88_));
  nor2   g43(.a(x14), .b(x13), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n83_), .O(new_n90_));
  nor2   g45(.a(x20), .b(x19), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(x18), .c(x01), .O(new_n92_));
  inv1   g47(.a(x21), .O(new_n93_));
  inv1   g48(.a(x22), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n72_), .O(new_n95_));
  aoi21  g50(.a(new_n92_), .b(new_n90_), .c(new_n95_), .O(z09));
  nor2   g51(.a(x20), .b(new_n57_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n54_), .c(x22), .d(x21), .O(new_n98_));
  inv1   g53(.a(x13), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(x12), .c(new_n64_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  nor2   g56(.a(new_n84_), .b(x14), .O(new_n102_));
  nand4  g57(.a(new_n94_), .b(new_n93_), .c(x20), .d(x16), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n98_), .c(x00), .O(z10));
  nand4  g61(.a(new_n91_), .b(x21), .c(x18), .d(x01), .O(new_n107_));
  nand3  g62(.a(new_n93_), .b(x20), .c(new_n85_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n102_), .c(new_n101_), .O(new_n110_));
  nand2  g65(.a(new_n94_), .b(new_n72_), .O(new_n111_));
  aoi21  g66(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(z11));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n113_));
  nand3  g68(.a(new_n51_), .b(x17), .c(new_n46_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n47_), .O(new_n116_));
  nand3  g71(.a(new_n70_), .b(x01), .c(x00), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x09), .O(new_n119_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n119_), .O(z12));
  nor3   g75(.a(new_n48_), .b(x19), .c(new_n49_), .O(z13));
  nand4  g76(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n63_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n48_), .O(z14));
  aoi21  g78(.a(new_n70_), .b(x01), .c(new_n46_), .O(new_n124_));
  nand3  g79(.a(x19), .b(new_n46_), .c(new_n57_), .O(new_n125_));
  oai21  g80(.a(new_n124_), .b(new_n72_), .c(new_n125_), .O(z15));
  nor2   g81(.a(new_n57_), .b(x00), .O(z16));
  inv1   g82(.a(new_n80_), .O(z17));
  zero   g83(.O(z00));
  zero   g84(.O(z01));
endmodule


