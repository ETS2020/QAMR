// Benchmark "FAU" written by ABC on Mon Jul  6 13:26:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n55_, new_n56_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(x03), .b(new_n28_), .O(new_n29_));
  inv1   g05(.a(x00), .O(new_n30_));
  nand2  g06(.a(x01), .b(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n29_), .b(new_n27_), .c(new_n31_), .O(z0));
  nand3  g08(.a(x06), .b(x05), .c(x01), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n28_), .c(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g12(.a(x01), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x10), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand4  g17(.a(new_n41_), .b(new_n26_), .c(new_n38_), .d(new_n37_), .O(new_n42_));
  inv1   g18(.a(x03), .O(new_n43_));
  oai21  g19(.a(x10), .b(x07), .c(new_n43_), .O(new_n44_));
  nand3  g20(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi22  g22(.a(new_n46_), .b(new_n37_), .c(new_n42_), .d(x02), .O(new_n47_));
  nand2  g23(.a(new_n40_), .b(x09), .O(new_n48_));
  nand2  g24(.a(x10), .b(new_n43_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n48_), .c(x01), .O(new_n50_));
  nor2   g26(.a(x08), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  oai21  g28(.a(new_n47_), .b(x00), .c(new_n52_), .O(z2));
  aoi22  g29(.a(x12), .b(new_n30_), .c(x11), .d(x07), .O(z3));
  nand3  g30(.a(x09), .b(x08), .c(x00), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n39_), .O(new_n56_));
  oai21  g32(.a(x11), .b(new_n39_), .c(new_n56_), .O(z4));
  nor2   g33(.a(x12), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n40_), .d(new_n39_), .O(z5));
  inv1   g35(.a(x08), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x03), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n28_), .c(new_n30_), .O(new_n63_));
  nand2  g39(.a(new_n28_), .b(x01), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n43_), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(x01), .c(new_n64_), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n63_), .c(new_n60_), .O(new_n67_));
  aoi21  g43(.a(new_n65_), .b(new_n28_), .c(x01), .O(new_n68_));
  nand2  g44(.a(x14), .b(x02), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x01), .O(new_n70_));
  oai21  g46(.a(new_n62_), .b(x02), .c(new_n70_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n68_), .c(new_n30_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n67_), .c(new_n41_), .O(z6));
  nand3  g49(.a(x09), .b(x03), .c(new_n37_), .O(new_n74_));
  nand4  g50(.a(new_n40_), .b(new_n60_), .c(new_n39_), .d(x01), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n28_), .O(new_n77_));
  oai21  g53(.a(x07), .b(new_n28_), .c(x03), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n60_), .c(new_n41_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n77_), .O(z7));
  inv1   g56(.a(x12), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(x03), .c(new_n28_), .d(new_n37_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(x09), .O(new_n83_));
  aoi21  g59(.a(new_n61_), .b(new_n30_), .c(new_n41_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


