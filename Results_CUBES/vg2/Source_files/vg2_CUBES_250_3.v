// Benchmark "FAU" written by ABC on Mon Jul  6 14:58:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  nor2   g009(.a(x24), .b(x10), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(x00), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n41_), .c(new_n40_), .d(new_n38_), .O(z5));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x20), .b(x14), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  and2   g018(.a(new_n51_), .b(z5), .O(z0));
  nand3  g019(.a(x11), .b(x09), .c(x04), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  nand4  g021(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  nand4  g023(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n56_), .c(new_n54_), .O(new_n60_));
  inv1   g027(.a(x08), .O(new_n61_));
  inv1   g028(.a(x09), .O(new_n62_));
  inv1   g029(.a(x22), .O(new_n63_));
  inv1   g030(.a(x23), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  nor2   g033(.a(x20), .b(x17), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(x07), .d(new_n46_), .O(new_n68_));
  nor3   g035(.a(x04), .b(x03), .c(x01), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  inv1   g037(.a(x14), .O(new_n71_));
  inv1   g038(.a(x16), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n47_), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  oai21  g042(.a(new_n75_), .b(new_n68_), .c(new_n60_), .O(new_n76_));
  nor2   g043(.a(new_n39_), .b(new_n35_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand4  g045(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n79_));
  nand4  g046(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n56_), .c(new_n54_), .O(new_n82_));
  nor2   g049(.a(x23), .b(x22), .O(new_n83_));
  nor2   g050(.a(x08), .b(x06), .O(new_n84_));
  nor2   g051(.a(x11), .b(x09), .O(new_n85_));
  inv1   g052(.a(x19), .O(new_n86_));
  nor2   g053(.a(x20), .b(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n88_));
  nor2   g055(.a(x17), .b(x16), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n69_), .c(new_n71_), .d(new_n70_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n88_), .c(new_n82_), .O(new_n91_));
  inv1   g058(.a(new_n39_), .O(new_n92_));
  nand2  g059(.a(x13), .b(x05), .O(new_n93_));
  oai21  g060(.a(new_n93_), .b(x24), .c(new_n92_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g062(.a(x01), .O(new_n96_));
  inv1   g063(.a(x03), .O(new_n97_));
  nand3  g064(.a(new_n67_), .b(new_n97_), .c(new_n42_), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  inv1   g066(.a(x04), .O(new_n100_));
  nand3  g067(.a(new_n43_), .b(new_n46_), .c(new_n100_), .O(new_n101_));
  nor3   g068(.a(new_n101_), .b(new_n73_), .c(new_n65_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n99_), .c(new_n96_), .d(x00), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n95_), .c(new_n78_), .O(z1));
  inv1   g071(.a(x18), .O(new_n105_));
  nor2   g072(.a(new_n35_), .b(new_n105_), .O(new_n106_));
  nand2  g073(.a(x15), .b(x13), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(x24), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n106_), .c(x05), .O(new_n109_));
  aoi22  g076(.a(new_n106_), .b(x13), .c(new_n39_), .d(x15), .O(new_n110_));
  nor2   g077(.a(new_n97_), .b(new_n96_), .O(new_n111_));
  inv1   g078(.a(x20), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n71_), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(new_n111_), .c(x11), .d(x06), .O(new_n114_));
  aoi21  g081(.a(new_n110_), .b(new_n109_), .c(new_n114_), .O(z2));
  nand4  g082(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n116_));
  nand3  g083(.a(new_n49_), .b(new_n47_), .c(new_n61_), .O(new_n117_));
  nand3  g084(.a(new_n48_), .b(x07), .c(new_n46_), .O(new_n118_));
  oai22  g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n55_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n77_), .O(new_n120_));
  nand4  g087(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n121_));
  nand3  g088(.a(new_n48_), .b(new_n71_), .c(new_n47_), .O(new_n122_));
  nand2  g089(.a(new_n87_), .b(new_n84_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n55_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n94_), .O(new_n125_));
  nand2  g092(.a(new_n84_), .b(new_n43_), .O(new_n126_));
  nand4  g093(.a(new_n97_), .b(new_n42_), .c(new_n96_), .d(x00), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n49_), .c(new_n47_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n125_), .c(new_n120_), .O(z3));
  inv1   g097(.a(x17), .O(new_n131_));
  oai21  g098(.a(x23), .b(new_n100_), .c(new_n131_), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(new_n63_), .c(x09), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(x16), .c(new_n61_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(z5), .O(new_n135_));
  oai21  g102(.a(new_n64_), .b(x04), .c(x17), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x22), .c(new_n62_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n72_), .c(x08), .O(new_n138_));
  nor2   g105(.a(x10), .b(x02), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n35_), .c(x21), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n110_), .c(new_n109_), .O(z7));
  nand2  g108(.a(z7), .b(new_n138_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n135_), .O(z4));
  inv1   g110(.a(new_n93_), .O(new_n144_));
  aoi21  g111(.a(new_n112_), .b(x14), .c(new_n46_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n47_), .c(x03), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n144_), .c(x15), .O(new_n147_));
  nand2  g114(.a(new_n139_), .b(x00), .O(new_n148_));
  oai21  g115(.a(new_n93_), .b(new_n86_), .c(new_n148_), .O(new_n149_));
  aoi21  g116(.a(x20), .b(new_n71_), .c(x06), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x11), .c(new_n97_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g119(.a(new_n47_), .b(x03), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n139_), .c(x21), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n152_), .c(new_n147_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n35_), .O(new_n156_));
  oai21  g123(.a(x20), .b(new_n71_), .c(x06), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(x11), .c(new_n97_), .O(new_n158_));
  nor2   g125(.a(new_n158_), .b(new_n105_), .O(new_n159_));
  oai21  g126(.a(new_n112_), .b(x14), .c(new_n46_), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(new_n47_), .c(x03), .O(new_n161_));
  nor2   g128(.a(new_n161_), .b(new_n34_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n159_), .c(new_n77_), .O(new_n163_));
  inv1   g130(.a(x15), .O(new_n164_));
  nor2   g131(.a(new_n158_), .b(new_n164_), .O(new_n165_));
  nor2   g132(.a(new_n161_), .b(new_n86_), .O(new_n166_));
  oai21  g133(.a(new_n166_), .b(new_n165_), .c(new_n39_), .O(new_n167_));
  nand3  g134(.a(new_n167_), .b(new_n163_), .c(new_n156_), .O(z6));
endmodule


