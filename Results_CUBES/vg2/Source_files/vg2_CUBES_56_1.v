// Benchmark "FAU" written by ABC on Mon Jul  6 14:57:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nor2   g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g003(.a(x19), .b(x13), .O(new_n37_));
  nor2   g004(.a(new_n37_), .b(x24), .O(new_n38_));
  oai21  g005(.a(new_n38_), .b(new_n36_), .c(x05), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  inv1   g007(.a(x13), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x19), .O(new_n44_));
  nand2  g011(.a(new_n36_), .b(x13), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  nand4  g014(.a(new_n35_), .b(new_n47_), .c(new_n46_), .d(x00), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n45_), .c(new_n44_), .d(new_n39_), .O(z5));
  inv1   g016(.a(x06), .O(new_n50_));
  inv1   g017(.a(x11), .O(new_n51_));
  nor2   g018(.a(x03), .b(x01), .O(new_n52_));
  nor2   g019(.a(x20), .b(x14), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  and2   g022(.a(new_n55_), .b(z5), .O(z0));
  nand2  g023(.a(x15), .b(x05), .O(new_n57_));
  nor2   g024(.a(x10), .b(x02), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(x21), .O(new_n59_));
  nand2  g026(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  and2   g027(.a(x03), .b(x01), .O(new_n61_));
  nand4  g028(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  nand4  g030(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n64_));
  nand4  g031(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n63_), .c(new_n61_), .d(new_n60_), .O(new_n67_));
  inv1   g034(.a(x00), .O(new_n68_));
  inv1   g035(.a(x19), .O(new_n69_));
  nand2  g036(.a(new_n47_), .b(new_n46_), .O(new_n70_));
  nand2  g037(.a(x13), .b(x05), .O(new_n71_));
  oai22  g038(.a(new_n71_), .b(new_n69_), .c(new_n70_), .d(new_n68_), .O(new_n72_));
  inv1   g039(.a(x04), .O(new_n73_));
  nor2   g040(.a(x09), .b(x08), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n52_), .c(new_n50_), .d(new_n73_), .O(new_n75_));
  inv1   g042(.a(x16), .O(new_n76_));
  inv1   g043(.a(x17), .O(new_n77_));
  inv1   g044(.a(x22), .O(new_n78_));
  inv1   g045(.a(x23), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n80_));
  nor2   g047(.a(x12), .b(x11), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  nor3   g049(.a(new_n82_), .b(new_n80_), .c(new_n75_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n72_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n35_), .O(new_n86_));
  nand3  g053(.a(new_n42_), .b(new_n36_), .c(new_n76_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n44_), .O(new_n88_));
  nand3  g055(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n89_));
  nor3   g056(.a(new_n89_), .b(new_n82_), .c(new_n75_), .O(new_n90_));
  nand4  g057(.a(x12), .b(x11), .c(x09), .d(x08), .O(new_n91_));
  nand3  g058(.a(new_n61_), .b(x06), .c(x04), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g060(.a(x24), .b(x23), .c(x22), .d(x18), .O(new_n94_));
  nand4  g061(.a(x20), .b(x17), .c(x16), .d(x14), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi22  g063(.a(new_n96_), .b(new_n93_), .c(new_n90_), .d(new_n88_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n86_), .O(z1));
  nand2  g065(.a(x24), .b(x18), .O(new_n99_));
  nand3  g066(.a(new_n35_), .b(x15), .c(x13), .O(new_n100_));
  aoi21  g067(.a(new_n100_), .b(new_n99_), .c(new_n40_), .O(new_n101_));
  nand3  g068(.a(new_n58_), .b(new_n35_), .c(x21), .O(new_n102_));
  inv1   g069(.a(new_n99_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(x13), .O(new_n104_));
  nand2  g071(.a(new_n43_), .b(x15), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n101_), .O(new_n107_));
  nor2   g074(.a(new_n51_), .b(new_n50_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(new_n61_), .c(x20), .d(x14), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n107_), .O(z2));
  inv1   g077(.a(x15), .O(new_n111_));
  oai21  g078(.a(new_n71_), .b(new_n111_), .c(new_n59_), .O(new_n112_));
  nand3  g079(.a(x20), .b(x14), .c(x08), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(new_n112_), .c(new_n108_), .d(new_n61_), .O(new_n115_));
  inv1   g082(.a(x20), .O(new_n116_));
  nor2   g083(.a(x14), .b(x08), .O(new_n117_));
  nand3  g084(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(new_n117_), .c(new_n72_), .d(new_n116_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n115_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n35_), .O(new_n122_));
  nand3  g089(.a(new_n61_), .b(x08), .c(x06), .O(new_n123_));
  nand4  g090(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n124_));
  nand3  g091(.a(new_n117_), .b(new_n116_), .c(x07), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n118_), .c(new_n124_), .d(new_n123_), .O(new_n126_));
  nor2   g093(.a(new_n43_), .b(new_n35_), .O(new_n127_));
  nand4  g094(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n128_));
  nand3  g095(.a(new_n117_), .b(new_n116_), .c(x19), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n118_), .c(new_n128_), .d(new_n123_), .O(new_n130_));
  aoi22  g097(.a(new_n130_), .b(new_n43_), .c(new_n127_), .d(new_n126_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n122_), .O(z3));
  inv1   g099(.a(x09), .O(new_n133_));
  aoi21  g100(.a(new_n79_), .b(x04), .c(x17), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(x22), .c(new_n133_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n76_), .c(x08), .O(new_n136_));
  inv1   g103(.a(x08), .O(new_n137_));
  nand2  g104(.a(x18), .b(new_n137_), .O(new_n138_));
  oai21  g105(.a(new_n136_), .b(new_n34_), .c(new_n138_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n127_), .O(new_n140_));
  inv1   g107(.a(new_n135_), .O(new_n141_));
  nand3  g108(.a(new_n72_), .b(new_n35_), .c(new_n76_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n44_), .c(new_n141_), .O(new_n143_));
  aoi21  g110(.a(new_n60_), .b(new_n35_), .c(new_n103_), .O(new_n144_));
  aoi21  g111(.a(x23), .b(new_n73_), .c(new_n77_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n78_), .c(x09), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(x16), .O(new_n147_));
  nand3  g114(.a(new_n35_), .b(x13), .c(x05), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n42_), .O(new_n149_));
  nand2  g116(.a(x19), .b(x08), .O(new_n150_));
  oai21  g117(.a(new_n111_), .b(x08), .c(new_n150_), .O(new_n151_));
  inv1   g118(.a(x21), .O(new_n152_));
  nand2  g119(.a(x08), .b(x00), .O(new_n153_));
  oai21  g120(.a(new_n152_), .b(x08), .c(new_n153_), .O(new_n154_));
  nor2   g121(.a(new_n70_), .b(x24), .O(new_n155_));
  aoi22  g122(.a(new_n155_), .b(new_n154_), .c(new_n151_), .d(new_n149_), .O(new_n156_));
  oai21  g123(.a(new_n147_), .b(new_n144_), .c(new_n156_), .O(new_n157_));
  nor2   g124(.a(new_n157_), .b(new_n143_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n140_), .O(z4));
  inv1   g126(.a(x14), .O(new_n160_));
  nand2  g127(.a(x20), .b(new_n160_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n50_), .c(x11), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x03), .c(z5), .O(new_n163_));
  inv1   g130(.a(x03), .O(new_n164_));
  nand2  g131(.a(new_n116_), .b(x14), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(x06), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(x11), .c(new_n164_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n107_), .c(new_n163_), .O(z6));
  inv1   g135(.a(new_n107_), .O(z7));
endmodule


