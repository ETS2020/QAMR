// Benchmark "FAU" written by ABC on Mon Jul  6 14:56:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x24), .O(new_n34_));
  nor2   g001(.a(x13), .b(x05), .O(new_n35_));
  nand2  g002(.a(x24), .b(x07), .O(new_n36_));
  nor2   g003(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nor2   g004(.a(x10), .b(x02), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x00), .O(new_n39_));
  nand2  g006(.a(x19), .b(x13), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n34_), .c(new_n37_), .O(new_n42_));
  inv1   g009(.a(x14), .O(new_n43_));
  inv1   g010(.a(x20), .O(new_n44_));
  nor2   g011(.a(x03), .b(x01), .O(new_n45_));
  nor2   g012(.a(x11), .b(x06), .O(new_n46_));
  nand4  g013(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  nor2   g014(.a(new_n47_), .b(new_n42_), .O(z0));
  nand3  g015(.a(x15), .b(x13), .c(x05), .O(new_n49_));
  nand2  g016(.a(new_n38_), .b(x21), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g018(.a(x03), .b(x01), .O(new_n52_));
  nand4  g019(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g021(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n55_));
  nand4  g022(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n54_), .c(new_n51_), .O(new_n58_));
  inv1   g025(.a(x04), .O(new_n59_));
  inv1   g026(.a(x06), .O(new_n60_));
  nor2   g027(.a(x09), .b(x08), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n45_), .c(new_n60_), .d(new_n59_), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x16), .O(new_n64_));
  inv1   g031(.a(x17), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  inv1   g035(.a(x11), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  nand4  g037(.a(new_n44_), .b(new_n43_), .c(new_n70_), .d(new_n69_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  nand3  g039(.a(new_n72_), .b(new_n63_), .c(new_n41_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n34_), .O(new_n75_));
  nand4  g042(.a(x23), .b(x22), .c(x03), .d(x01), .O(new_n76_));
  nand4  g043(.a(x20), .b(x18), .c(x17), .d(x16), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand4  g045(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n79_));
  inv1   g046(.a(new_n79_), .O(new_n80_));
  nor2   g047(.a(new_n70_), .b(new_n59_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n80_), .c(new_n78_), .d(x14), .O(new_n82_));
  nor2   g049(.a(x08), .b(x04), .O(new_n83_));
  nor2   g050(.a(x11), .b(x09), .O(new_n84_));
  inv1   g051(.a(x07), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n84_), .c(new_n83_), .d(new_n45_), .O(new_n87_));
  nor3   g054(.a(x23), .b(x22), .c(x20), .O(new_n88_));
  nor2   g055(.a(x14), .b(x12), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n65_), .d(new_n64_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n87_), .c(new_n82_), .O(new_n91_));
  nor2   g058(.a(new_n35_), .b(new_n34_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor3   g060(.a(new_n76_), .b(new_n65_), .c(new_n64_), .O(new_n94_));
  nand3  g061(.a(x20), .b(x15), .c(x14), .O(new_n95_));
  nand2  g062(.a(new_n81_), .b(new_n35_), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n94_), .c(new_n80_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n93_), .c(new_n75_), .O(z1));
  inv1   g066(.a(x05), .O(new_n100_));
  nand2  g067(.a(x24), .b(x18), .O(new_n101_));
  nand3  g068(.a(new_n34_), .b(x15), .c(x13), .O(new_n102_));
  aoi21  g069(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nand3  g070(.a(new_n38_), .b(new_n34_), .c(x21), .O(new_n104_));
  nand3  g071(.a(x24), .b(x18), .c(x13), .O(new_n105_));
  nand2  g072(.a(new_n35_), .b(x15), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  inv1   g075(.a(new_n52_), .O(new_n109_));
  nor2   g076(.a(new_n44_), .b(new_n43_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(new_n109_), .c(x11), .d(x06), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n108_), .O(z2));
  inv1   g079(.a(x08), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n60_), .O(new_n114_));
  nand3  g081(.a(x20), .b(x14), .c(x11), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(new_n114_), .c(new_n109_), .d(new_n51_), .O(new_n117_));
  nor3   g084(.a(x20), .b(x14), .c(x08), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(new_n46_), .c(new_n45_), .d(new_n41_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n34_), .O(new_n121_));
  nand2  g088(.a(new_n86_), .b(new_n45_), .O(new_n122_));
  nor2   g089(.a(new_n43_), .b(new_n69_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n109_), .O(new_n124_));
  nand4  g091(.a(x20), .b(x18), .c(x08), .d(x06), .O(new_n125_));
  nand4  g092(.a(new_n44_), .b(new_n43_), .c(new_n69_), .d(new_n113_), .O(new_n126_));
  oai22  g093(.a(new_n126_), .b(new_n122_), .c(new_n125_), .d(new_n124_), .O(new_n127_));
  nand3  g094(.a(new_n123_), .b(x20), .c(x15), .O(new_n128_));
  nand3  g095(.a(new_n114_), .b(new_n109_), .c(new_n35_), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  aoi21  g097(.a(new_n127_), .b(new_n92_), .c(new_n130_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n121_), .O(z3));
  inv1   g099(.a(x09), .O(new_n133_));
  aoi21  g100(.a(new_n67_), .b(x04), .c(x17), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(x22), .c(new_n133_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n64_), .c(x08), .O(new_n136_));
  aoi21  g103(.a(x23), .b(new_n59_), .c(new_n65_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n66_), .c(x09), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(x16), .c(new_n113_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n108_), .c(new_n136_), .d(new_n42_), .O(z4));
  inv1   g107(.a(new_n42_), .O(z5));
  oai21  g108(.a(new_n44_), .b(x14), .c(new_n60_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n69_), .c(x03), .O(new_n143_));
  inv1   g110(.a(x03), .O(new_n144_));
  oai21  g111(.a(x20), .b(new_n43_), .c(x06), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x11), .c(new_n144_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n108_), .c(new_n143_), .d(new_n42_), .O(z6));
  inv1   g114(.a(new_n108_), .O(z7));
endmodule


