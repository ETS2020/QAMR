// Benchmark "FAU" written by ABC on Mon Jul  6 14:58:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  nor2   g009(.a(x24), .b(x10), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(x00), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nor2   g013(.a(x03), .b(x01), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  inv1   g016(.a(x11), .O(new_n50_));
  inv1   g017(.a(x14), .O(new_n51_));
  inv1   g018(.a(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(new_n48_), .c(new_n46_), .O(z0));
  nand3  g021(.a(x11), .b(x09), .c(x04), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  nand4  g023(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand4  g025(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(new_n58_), .c(new_n56_), .O(new_n62_));
  nor2   g029(.a(x23), .b(x22), .O(new_n63_));
  nor2   g030(.a(x09), .b(x08), .O(new_n64_));
  inv1   g031(.a(x07), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(x06), .O(new_n66_));
  nor2   g033(.a(x20), .b(x17), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(new_n63_), .O(new_n68_));
  nor2   g035(.a(new_n48_), .b(x04), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n51_), .c(new_n70_), .d(new_n50_), .O(new_n72_));
  inv1   g039(.a(new_n72_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  oai21  g041(.a(new_n74_), .b(new_n68_), .c(new_n62_), .O(new_n75_));
  nor2   g042(.a(new_n39_), .b(new_n36_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n78_));
  nand4  g045(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n58_), .c(new_n56_), .O(new_n81_));
  nor2   g048(.a(x08), .b(x06), .O(new_n82_));
  nor2   g049(.a(x11), .b(x09), .O(new_n83_));
  inv1   g050(.a(x19), .O(new_n84_));
  nor2   g051(.a(x20), .b(new_n84_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n63_), .O(new_n86_));
  nor2   g053(.a(x17), .b(x16), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n69_), .c(new_n51_), .d(new_n70_), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(new_n86_), .c(new_n81_), .O(new_n89_));
  inv1   g056(.a(new_n39_), .O(new_n90_));
  nand3  g057(.a(new_n36_), .b(x13), .c(x05), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  inv1   g060(.a(x00), .O(new_n94_));
  nor2   g061(.a(x01), .b(new_n94_), .O(new_n95_));
  nor2   g062(.a(x03), .b(x02), .O(new_n96_));
  nand2  g063(.a(new_n63_), .b(new_n36_), .O(new_n97_));
  inv1   g064(.a(x04), .O(new_n98_));
  inv1   g065(.a(x09), .O(new_n99_));
  inv1   g066(.a(x10), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n49_), .d(new_n98_), .O(new_n101_));
  nor3   g068(.a(new_n101_), .b(new_n97_), .c(new_n72_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n96_), .c(new_n95_), .d(new_n67_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n93_), .c(new_n77_), .O(z1));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  nand3  g072(.a(new_n36_), .b(x15), .c(x13), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(x05), .O(new_n108_));
  nand4  g075(.a(new_n36_), .b(x21), .c(new_n100_), .d(new_n42_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n39_), .b(x15), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n108_), .O(z7));
  nand2  g079(.a(x03), .b(x01), .O(new_n113_));
  nand4  g080(.a(x20), .b(x14), .c(x11), .d(x06), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  and2   g082(.a(new_n115_), .b(z7), .O(z2));
  nand4  g083(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n117_));
  inv1   g084(.a(x08), .O(new_n118_));
  nand4  g085(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n118_), .O(new_n119_));
  nand2  g086(.a(new_n66_), .b(new_n47_), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n119_), .c(new_n117_), .d(new_n57_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n76_), .O(new_n122_));
  nand4  g089(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n123_));
  nand3  g090(.a(new_n47_), .b(new_n51_), .c(new_n50_), .O(new_n124_));
  nand2  g091(.a(new_n85_), .b(new_n82_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n57_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n92_), .O(new_n127_));
  inv1   g094(.a(new_n53_), .O(new_n128_));
  nand4  g095(.a(new_n96_), .b(new_n95_), .c(new_n128_), .d(new_n43_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n127_), .c(new_n122_), .O(z3));
  inv1   g097(.a(x15), .O(new_n131_));
  inv1   g098(.a(x22), .O(new_n132_));
  inv1   g099(.a(x17), .O(new_n133_));
  aoi21  g100(.a(x23), .b(new_n98_), .c(new_n133_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n132_), .c(x09), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(x16), .c(new_n118_), .O(new_n136_));
  nand2  g103(.a(x19), .b(x08), .O(new_n137_));
  oai21  g104(.a(new_n136_), .b(new_n131_), .c(new_n137_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n92_), .O(new_n139_));
  inv1   g106(.a(x18), .O(new_n140_));
  nand2  g107(.a(x08), .b(x07), .O(new_n141_));
  oai21  g108(.a(new_n136_), .b(new_n140_), .c(new_n141_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n76_), .O(new_n143_));
  inv1   g110(.a(new_n46_), .O(z5));
  inv1   g111(.a(new_n109_), .O(new_n145_));
  oai21  g112(.a(x23), .b(new_n98_), .c(new_n133_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n132_), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(new_n99_), .c(x16), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(z5), .c(new_n145_), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n143_), .c(new_n139_), .O(z4));
  nand2  g117(.a(x20), .b(new_n51_), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(new_n49_), .c(x11), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(x03), .c(z5), .O(new_n153_));
  inv1   g120(.a(x03), .O(new_n154_));
  nand2  g121(.a(new_n52_), .b(x14), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(x06), .c(new_n50_), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(new_n154_), .c(z7), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n153_), .O(z6));
endmodule


