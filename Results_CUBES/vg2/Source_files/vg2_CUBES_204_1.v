// Benchmark "FAU" written by ABC on Mon Jul  6 14:58:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  nand2  g001(.a(x19), .b(x13), .O(new_n35_));
  oai21  g002(.a(new_n35_), .b(x24), .c(new_n34_), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(x05), .O(new_n37_));
  nor2   g004(.a(x13), .b(x05), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x19), .O(new_n39_));
  nand3  g006(.a(x24), .b(x13), .c(x07), .O(new_n40_));
  inv1   g007(.a(x02), .O(new_n41_));
  inv1   g008(.a(x10), .O(new_n42_));
  inv1   g009(.a(x24), .O(new_n43_));
  nand4  g010(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(x00), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n40_), .c(new_n39_), .d(new_n37_), .O(z5));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x20), .b(x14), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  and2   g018(.a(new_n51_), .b(z5), .O(z0));
  nand2  g019(.a(x13), .b(x05), .O(new_n53_));
  nor2   g020(.a(x10), .b(x02), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(x21), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g023(.a(x03), .b(x01), .O(new_n57_));
  nand4  g024(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n60_));
  nand4  g027(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(new_n63_));
  inv1   g030(.a(x00), .O(new_n64_));
  inv1   g031(.a(x19), .O(new_n65_));
  inv1   g032(.a(new_n54_), .O(new_n66_));
  oai22  g033(.a(new_n66_), .b(new_n64_), .c(new_n53_), .d(new_n65_), .O(new_n67_));
  nor2   g034(.a(x06), .b(x04), .O(new_n68_));
  nor2   g035(.a(x09), .b(x08), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n68_), .c(new_n48_), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  inv1   g040(.a(x23), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nor2   g042(.a(x12), .b(x11), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n49_), .O(new_n77_));
  nor3   g044(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n43_), .O(new_n81_));
  inv1   g048(.a(new_n38_), .O(new_n82_));
  nand2  g049(.a(x24), .b(x18), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g051(.a(new_n84_), .b(new_n62_), .c(new_n59_), .O(new_n85_));
  oai21  g052(.a(new_n38_), .b(new_n34_), .c(new_n39_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n78_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n85_), .c(new_n81_), .O(z1));
  inv1   g055(.a(x05), .O(new_n89_));
  nand3  g056(.a(new_n43_), .b(x15), .c(x13), .O(new_n90_));
  aoi21  g057(.a(new_n90_), .b(new_n83_), .c(new_n89_), .O(new_n91_));
  nand3  g058(.a(new_n54_), .b(new_n43_), .c(x21), .O(new_n92_));
  nand3  g059(.a(x24), .b(x18), .c(x13), .O(new_n93_));
  nand2  g060(.a(new_n38_), .b(x15), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  inv1   g063(.a(new_n57_), .O(new_n97_));
  nor2   g064(.a(new_n47_), .b(new_n46_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n97_), .c(x20), .d(x14), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n96_), .O(z2));
  inv1   g067(.a(x15), .O(new_n101_));
  oai21  g068(.a(new_n53_), .b(new_n101_), .c(new_n55_), .O(new_n102_));
  nand3  g069(.a(x20), .b(x14), .c(x08), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n102_), .c(new_n98_), .d(new_n97_), .O(new_n105_));
  inv1   g072(.a(x20), .O(new_n106_));
  nor2   g073(.a(x14), .b(x08), .O(new_n107_));
  nand3  g074(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n107_), .c(new_n67_), .d(new_n106_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n105_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n43_), .O(new_n112_));
  nand3  g079(.a(new_n97_), .b(x08), .c(x06), .O(new_n113_));
  nand4  g080(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n114_));
  nand3  g081(.a(new_n107_), .b(new_n106_), .c(x07), .O(new_n115_));
  oai22  g082(.a(new_n115_), .b(new_n108_), .c(new_n114_), .d(new_n113_), .O(new_n116_));
  nor2   g083(.a(new_n38_), .b(new_n43_), .O(new_n117_));
  nand4  g084(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n118_));
  nand3  g085(.a(new_n107_), .b(new_n106_), .c(x19), .O(new_n119_));
  oai22  g086(.a(new_n119_), .b(new_n108_), .c(new_n118_), .d(new_n113_), .O(new_n120_));
  aoi22  g087(.a(new_n120_), .b(new_n38_), .c(new_n117_), .d(new_n116_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n112_), .O(z3));
  inv1   g089(.a(x09), .O(new_n123_));
  aoi21  g090(.a(new_n74_), .b(x04), .c(x17), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(x22), .c(new_n123_), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(new_n71_), .c(x08), .O(new_n126_));
  inv1   g093(.a(x08), .O(new_n127_));
  nand2  g094(.a(x21), .b(new_n127_), .O(new_n128_));
  oai21  g095(.a(new_n126_), .b(new_n64_), .c(new_n128_), .O(new_n129_));
  nor2   g096(.a(new_n66_), .b(x24), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g098(.a(x07), .O(new_n132_));
  nand2  g099(.a(x18), .b(new_n127_), .O(new_n133_));
  oai21  g100(.a(new_n126_), .b(new_n132_), .c(new_n133_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n117_), .O(new_n135_));
  oai21  g102(.a(new_n53_), .b(x24), .c(new_n82_), .O(new_n136_));
  inv1   g103(.a(x04), .O(new_n137_));
  aoi21  g104(.a(x23), .b(new_n137_), .c(new_n72_), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n73_), .c(x09), .O(new_n139_));
  oai21  g106(.a(new_n124_), .b(x22), .c(new_n69_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(x19), .O(new_n141_));
  nand2  g108(.a(new_n139_), .b(x16), .O(new_n142_));
  nand2  g109(.a(x15), .b(new_n127_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .O(new_n144_));
  aoi21  g111(.a(new_n92_), .b(new_n83_), .c(new_n71_), .O(new_n145_));
  aoi22  g112(.a(new_n145_), .b(new_n139_), .c(new_n144_), .d(new_n136_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n135_), .c(new_n131_), .O(z4));
  inv1   g114(.a(x14), .O(new_n148_));
  nand2  g115(.a(x20), .b(new_n148_), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n46_), .c(x11), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x03), .c(z5), .O(new_n151_));
  inv1   g118(.a(x03), .O(new_n152_));
  nand2  g119(.a(new_n106_), .b(x14), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(x06), .O(new_n154_));
  aoi21  g121(.a(new_n154_), .b(x11), .c(new_n152_), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(new_n96_), .c(new_n151_), .O(z6));
  inv1   g123(.a(new_n96_), .O(z7));
endmodule


