// Benchmark "FAU" written by ABC on Mon Jul  6 14:57:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  nand2  g001(.a(x19), .b(x13), .O(new_n35_));
  oai21  g002(.a(new_n35_), .b(x24), .c(new_n34_), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(x05), .O(new_n37_));
  nor2   g004(.a(x13), .b(x05), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x19), .O(new_n39_));
  inv1   g006(.a(new_n34_), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x13), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  inv1   g010(.a(x24), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n45_));
  nand4  g012(.a(new_n45_), .b(new_n41_), .c(new_n39_), .d(new_n37_), .O(z5));
  inv1   g013(.a(x06), .O(new_n47_));
  inv1   g014(.a(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nor2   g016(.a(x20), .b(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  and2   g019(.a(new_n52_), .b(z5), .O(z0));
  inv1   g020(.a(x15), .O(new_n54_));
  inv1   g021(.a(x21), .O(new_n55_));
  nand2  g022(.a(x13), .b(x05), .O(new_n56_));
  nor2   g023(.a(x10), .b(x02), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  oai22  g025(.a(new_n58_), .b(new_n55_), .c(new_n56_), .d(new_n54_), .O(new_n59_));
  and2   g026(.a(x03), .b(x01), .O(new_n60_));
  nand4  g027(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand4  g029(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n63_));
  nand4  g030(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n62_), .c(new_n60_), .O(new_n66_));
  inv1   g033(.a(new_n66_), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  nand2  g035(.a(new_n57_), .b(x00), .O(new_n69_));
  nand2  g036(.a(new_n69_), .b(new_n56_), .O(new_n70_));
  inv1   g037(.a(x04), .O(new_n71_));
  nor2   g038(.a(x09), .b(x08), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n49_), .c(new_n47_), .d(new_n71_), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  inv1   g043(.a(x22), .O(new_n77_));
  inv1   g044(.a(x23), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  inv1   g046(.a(x12), .O(new_n80_));
  nand3  g047(.a(new_n50_), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n74_), .c(new_n70_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n68_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n44_), .O(new_n85_));
  nor2   g052(.a(new_n38_), .b(new_n40_), .O(new_n86_));
  nor3   g053(.a(new_n86_), .b(new_n81_), .c(new_n79_), .O(new_n87_));
  nand2  g054(.a(x24), .b(x18), .O(new_n88_));
  nand2  g055(.a(new_n38_), .b(x15), .O(new_n89_));
  oai21  g056(.a(new_n88_), .b(new_n38_), .c(new_n89_), .O(new_n90_));
  aoi22  g057(.a(new_n90_), .b(new_n67_), .c(new_n87_), .d(new_n74_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n85_), .O(z1));
  inv1   g059(.a(x05), .O(new_n93_));
  nand3  g060(.a(new_n44_), .b(x15), .c(x13), .O(new_n94_));
  aoi21  g061(.a(new_n94_), .b(new_n88_), .c(new_n93_), .O(new_n95_));
  nand3  g062(.a(new_n57_), .b(new_n44_), .c(x21), .O(new_n96_));
  nand3  g063(.a(x24), .b(x18), .c(x13), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n96_), .c(new_n89_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  nor2   g066(.a(new_n48_), .b(new_n47_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n60_), .c(x20), .d(x14), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n99_), .O(z2));
  nand3  g069(.a(x20), .b(x14), .c(x08), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n100_), .c(new_n60_), .d(new_n59_), .O(new_n105_));
  inv1   g072(.a(x20), .O(new_n106_));
  inv1   g073(.a(x19), .O(new_n107_));
  oai21  g074(.a(new_n56_), .b(new_n107_), .c(new_n69_), .O(new_n108_));
  nor2   g075(.a(x14), .b(x08), .O(new_n109_));
  nand3  g076(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n110_));
  inv1   g077(.a(new_n110_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n109_), .c(new_n108_), .d(new_n106_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n105_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n44_), .O(new_n114_));
  nand3  g081(.a(new_n60_), .b(x08), .c(x06), .O(new_n115_));
  nand4  g082(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n116_));
  nand3  g083(.a(new_n109_), .b(new_n106_), .c(x07), .O(new_n117_));
  oai22  g084(.a(new_n117_), .b(new_n110_), .c(new_n116_), .d(new_n115_), .O(new_n118_));
  nor2   g085(.a(new_n38_), .b(new_n44_), .O(new_n119_));
  nand4  g086(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n120_));
  nand3  g087(.a(new_n109_), .b(new_n106_), .c(x19), .O(new_n121_));
  oai22  g088(.a(new_n121_), .b(new_n110_), .c(new_n120_), .d(new_n115_), .O(new_n122_));
  aoi22  g089(.a(new_n122_), .b(new_n38_), .c(new_n119_), .d(new_n118_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n114_), .O(z3));
  inv1   g091(.a(x08), .O(new_n125_));
  aoi21  g092(.a(x23), .b(new_n71_), .c(new_n76_), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(new_n77_), .c(x09), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(x16), .c(new_n125_), .O(new_n128_));
  nand2  g095(.a(x08), .b(x00), .O(new_n129_));
  oai21  g096(.a(new_n128_), .b(new_n55_), .c(new_n129_), .O(new_n130_));
  nor2   g097(.a(new_n58_), .b(x24), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g099(.a(x18), .O(new_n133_));
  nand2  g100(.a(x08), .b(x07), .O(new_n134_));
  oai21  g101(.a(new_n128_), .b(new_n133_), .c(new_n134_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n119_), .O(new_n136_));
  nor2   g103(.a(new_n56_), .b(x24), .O(new_n137_));
  or2    g104(.a(new_n137_), .b(new_n38_), .O(new_n138_));
  inv1   g105(.a(x09), .O(new_n139_));
  aoi21  g106(.a(new_n78_), .b(x04), .c(x17), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x22), .c(new_n139_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n75_), .O(new_n142_));
  nand2  g109(.a(x09), .b(x08), .O(new_n143_));
  nor2   g110(.a(new_n126_), .b(new_n77_), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(new_n143_), .c(x15), .O(new_n145_));
  nand2  g112(.a(x19), .b(x08), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n145_), .c(new_n142_), .O(new_n147_));
  aoi21  g114(.a(new_n45_), .b(new_n34_), .c(x16), .O(new_n148_));
  and2   g115(.a(new_n148_), .b(new_n141_), .O(new_n149_));
  aoi21  g116(.a(new_n147_), .b(new_n138_), .c(new_n149_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n136_), .c(new_n132_), .O(z4));
  inv1   g118(.a(x14), .O(new_n152_));
  nand2  g119(.a(x20), .b(new_n152_), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n47_), .c(x11), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(x03), .c(z5), .O(new_n155_));
  inv1   g122(.a(x03), .O(new_n156_));
  nand2  g123(.a(new_n106_), .b(x14), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(x06), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(x11), .c(new_n156_), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(new_n99_), .c(new_n155_), .O(z6));
  inv1   g127(.a(new_n99_), .O(z7));
endmodule


