// Benchmark "FAU" written by ABC on Mon Jul  6 14:40:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x24), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  aoi21  g12(.a(x24), .b(new_n56_), .c(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n55_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n48_), .c(new_n49_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g19(.a(new_n60_), .O(z03));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(x08), .O(z04));
  nor2   g27(.a(x13), .b(x08), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x08), .O(z06));
  nand2  g30(.a(new_n46_), .b(x06), .O(z07));
  nand2  g31(.a(x22), .b(x21), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  inv1   g33(.a(x23), .O(new_n79_));
  oai21  g34(.a(new_n77_), .b(new_n50_), .c(new_n79_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n78_), .c(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z08));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n62_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z09));
  nand3  g40(.a(new_n83_), .b(new_n61_), .c(new_n51_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(new_n83_), .b(new_n61_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n88_), .O(z11));
  oai21  g45(.a(new_n52_), .b(new_n51_), .c(x19), .O(new_n91_));
  nor2   g46(.a(new_n52_), .b(new_n51_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n53_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n91_), .c(new_n89_), .O(z12));
  nand2  g49(.a(new_n92_), .b(x19), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n92_), .b(new_n50_), .c(x19), .O(new_n97_));
  nand2  g52(.a(new_n83_), .b(new_n61_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n96_), .O(z13));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n97_), .b(x21), .c(new_n98_), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n95_), .c(new_n103_), .O(z14));
  oai21  g59(.a(new_n102_), .b(new_n95_), .c(x22), .O(new_n105_));
  inv1   g60(.a(new_n95_), .O(new_n106_));
  nand3  g61(.a(new_n101_), .b(new_n106_), .c(new_n57_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n99_), .O(z15));
  nand2  g63(.a(x24), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n57_), .c(new_n56_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n95_), .c(x23), .O(new_n111_));
  nand4  g66(.a(new_n101_), .b(new_n106_), .c(new_n79_), .d(new_n57_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n99_), .O(z16));
  nand4  g68(.a(new_n79_), .b(new_n57_), .c(new_n56_), .d(new_n50_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n95_), .c(x24), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n99_), .O(z17));
  nand3  g71(.a(new_n83_), .b(new_n61_), .c(new_n49_), .O(z18));
  buf    g72(.a(x16), .O(z02));
endmodule


