// Benchmark "FAU" written by ABC on Mon Jul  6 13:06:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n42_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n64_, new_n66_, new_n68_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g04(.a(x05), .b(x04), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n30_), .c(new_n29_), .d(new_n26_), .O(z00));
  nand3  g08(.a(x05), .b(new_n31_), .c(new_n27_), .O(new_n34_));
  nand4  g09(.a(new_n28_), .b(x04), .c(new_n26_), .d(x02), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(z01));
  inv1   g11(.a(x13), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n31_), .d(new_n27_), .O(new_n38_));
  nor3   g13(.a(new_n38_), .b(x01), .c(x00), .O(z02));
  inv1   g14(.a(x00), .O(new_n40_));
  nor3   g15(.a(new_n38_), .b(x01), .c(new_n40_), .O(z03));
  inv1   g16(.a(x01), .O(new_n42_));
  nor3   g17(.a(new_n38_), .b(new_n42_), .c(x00), .O(z04));
  nor3   g18(.a(new_n38_), .b(new_n42_), .c(new_n40_), .O(z05));
  inv1   g19(.a(x09), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(new_n42_), .O(new_n46_));
  inv1   g21(.a(x11), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x01), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n46_), .c(new_n40_), .O(new_n49_));
  inv1   g24(.a(x08), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(new_n42_), .O(new_n51_));
  inv1   g26(.a(x10), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  nand2  g29(.a(x05), .b(new_n27_), .O(new_n55_));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand3  g32(.a(x13), .b(new_n57_), .c(new_n56_), .O(new_n58_));
  nor2   g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  oai21  g34(.a(new_n54_), .b(new_n49_), .c(new_n59_), .O(new_n60_));
  nand4  g35(.a(new_n37_), .b(new_n28_), .c(new_n26_), .d(x02), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n60_), .c(new_n31_), .O(z06));
  inv1   g37(.a(new_n38_), .O(z07));
  nand4  g38(.a(x13), .b(new_n57_), .c(x05), .d(new_n27_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n61_), .c(new_n31_), .O(z08));
  inv1   g40(.a(x06), .O(new_n66_));
  nor2   g41(.a(new_n37_), .b(new_n66_), .O(z09));
  nand2  g42(.a(new_n28_), .b(new_n26_), .O(new_n68_));
  aoi21  g43(.a(new_n68_), .b(new_n55_), .c(new_n66_), .O(z10));
endmodule


