// Benchmark "FAU" written by ABC on Mon Jul  6 15:00:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n178_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(x15), .c(x16), .O(z00));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  oai21  g011(.a(new_n72_), .b(x28), .c(new_n73_), .O(new_n74_));
  and2   g012(.a(new_n74_), .b(x27), .O(new_n75_));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(x04), .O(z01));
  aoi21  g017(.a(x29), .b(x08), .c(x02), .O(new_n80_));
  aoi21  g018(.a(x28), .b(x04), .c(new_n80_), .O(new_n81_));
  oai21  g019(.a(x27), .b(x08), .c(x35), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(new_n83_));
  nand4  g021(.a(new_n83_), .b(new_n81_), .c(x40), .d(x39), .O(z02));
  nand2  g022(.a(x35), .b(x28), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  inv1   g024(.a(x27), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n86_), .c(x21), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  nand3  g029(.a(new_n89_), .b(new_n86_), .c(new_n91_), .O(z04));
  aoi21  g030(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g031(.a(z06), .O(z05));
  inv1   g032(.a(x03), .O(new_n95_));
  inv1   g033(.a(x00), .O(new_n96_));
  oai21  g034(.a(x25), .b(new_n96_), .c(x38), .O(new_n97_));
  inv1   g035(.a(x15), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n98_), .O(new_n99_));
  nor2   g037(.a(x33), .b(x31), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand3  g041(.a(x38), .b(new_n103_), .c(new_n96_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n95_), .O(z07));
  nand2  g044(.a(x40), .b(x39), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z08));
  nand4  g046(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z09));
  inv1   g048(.a(x05), .O(new_n111_));
  inv1   g049(.a(x07), .O(new_n112_));
  oai21  g050(.a(new_n107_), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n74_), .c(x27), .O(new_n114_));
  nand3  g052(.a(new_n78_), .b(z08), .c(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(new_n117_));
  nand3  g055(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(z10));
  nand2  g057(.a(z08), .b(x29), .O(new_n120_));
  nor2   g058(.a(new_n72_), .b(x28), .O(new_n121_));
  oai21  g059(.a(new_n87_), .b(new_n64_), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  inv1   g061(.a(x08), .O(new_n124_));
  inv1   g062(.a(x28), .O(new_n125_));
  nand2  g063(.a(x40), .b(x04), .O(new_n126_));
  nand4  g064(.a(new_n126_), .b(x35), .c(new_n125_), .d(x27), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  inv1   g066(.a(x09), .O(new_n129_));
  nand2  g067(.a(new_n76_), .b(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n128_), .b(new_n123_), .c(new_n130_), .O(z11));
  nor3   g069(.a(x37), .b(x36), .c(x35), .O(new_n132_));
  inv1   g070(.a(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(x27), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g073(.a(z08), .b(new_n64_), .O(new_n136_));
  nand2  g074(.a(new_n126_), .b(z08), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(new_n87_), .c(new_n136_), .d(new_n66_), .O(new_n138_));
  oai21  g076(.a(new_n135_), .b(x10), .c(new_n138_), .O(z12));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nor2   g079(.a(new_n133_), .b(x13), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n141_), .c(x39), .d(new_n64_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand2  g083(.a(new_n85_), .b(new_n88_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x20), .c(new_n145_), .d(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n78_), .c(z08), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z13));
  nand3  g090(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n153_));
  nand4  g091(.a(new_n140_), .b(new_n77_), .c(new_n76_), .d(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n150_), .c(x40), .d(x39), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  oai21  g094(.a(new_n66_), .b(new_n87_), .c(new_n107_), .O(new_n157_));
  nand3  g095(.a(new_n88_), .b(new_n72_), .c(x28), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n77_), .c(new_n76_), .O(new_n160_));
  oai22  g098(.a(new_n134_), .b(new_n66_), .c(x13), .d(x04), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n160_), .c(new_n157_), .d(new_n156_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(x23), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(x23), .c(x22), .d(x01), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  nand2  g107(.a(new_n121_), .b(new_n87_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n120_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x08), .O(new_n172_));
  nor2   g110(.a(new_n87_), .b(x04), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n121_), .c(new_n130_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n172_), .O(z18));
  oai21  g113(.a(new_n72_), .b(new_n87_), .c(new_n124_), .O(new_n176_));
  oai21  g114(.a(new_n125_), .b(x27), .c(x35), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n120_), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n176_), .c(new_n130_), .O(z20));
  aoi21  g117(.a(new_n128_), .b(new_n123_), .c(new_n130_), .O(z19));
endmodule


