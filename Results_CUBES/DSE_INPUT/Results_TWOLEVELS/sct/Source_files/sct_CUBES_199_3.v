// Benchmark "FAU" written by ABC on Mon Jul  6 14:34:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x03), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x02), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n44_), .b(x16), .c(new_n46_), .O(z02));
  oai21  g13(.a(x16), .b(x07), .c(new_n44_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x06), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(x07), .c(new_n45_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(new_n49_), .c(new_n40_), .O(z03));
  and2   g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(x16), .c(new_n44_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x08), .O(new_n55_));
  nor2   g21(.a(x16), .b(x08), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  aoi21  g23(.a(new_n57_), .b(new_n55_), .c(new_n40_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  nand2  g26(.a(x04), .b(new_n35_), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n43_), .c(new_n50_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x04), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n60_), .O(z05));
  nor2   g33(.a(new_n62_), .b(new_n40_), .O(new_n68_));
  nand2  g34(.a(new_n64_), .b(x10), .O(new_n69_));
  inv1   g35(.a(new_n59_), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(z06));
  nand2  g39(.a(new_n72_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n70_), .c(new_n63_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n68_), .O(z07));
  nand2  g43(.a(new_n76_), .b(x12), .O(new_n78_));
  nor2   g44(.a(x12), .b(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n71_), .c(new_n70_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n68_), .O(z08));
  nand2  g47(.a(x16), .b(new_n35_), .O(new_n82_));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n75_), .c(new_n50_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n64_), .c(new_n82_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  oai21  g52(.a(new_n50_), .b(new_n43_), .c(x04), .O(new_n87_));
  aoi21  g53(.a(new_n80_), .b(x13), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n83_), .b(new_n75_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g58(.a(new_n50_), .b(new_n63_), .c(x08), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n92_), .c(new_n53_), .O(new_n95_));
  nand3  g61(.a(new_n44_), .b(x16), .c(x14), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n95_), .c(new_n40_), .O(z10));
  aoi21  g63(.a(new_n82_), .b(new_n43_), .c(new_n40_), .O(z12));
  and2   g64(.a(x17), .b(x04), .O(z14));
  buf    g65(.a(x02), .O(z11));
  buf    g66(.a(x04), .O(z13));
endmodule


