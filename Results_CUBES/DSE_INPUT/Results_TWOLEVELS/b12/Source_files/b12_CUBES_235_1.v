// Benchmark "FAU" written by ABC on Mon Jul  6 13:27:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  oai21  g03(.a(x02), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n26_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  aoi21  g10(.a(new_n26_), .b(new_n34_), .c(x01), .O(new_n35_));
  aoi22  g11(.a(x06), .b(x05), .c(new_n26_), .d(new_n34_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n35_), .c(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n30_), .c(x01), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  nor2   g16(.a(x04), .b(new_n34_), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n41_), .c(new_n25_), .O(new_n43_));
  nor2   g19(.a(x04), .b(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(new_n30_), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nor2   g23(.a(new_n47_), .b(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(new_n49_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n25_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n45_), .c(new_n40_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand2  g30(.a(new_n51_), .b(new_n30_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n46_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(z2));
  aoi22  g33(.a(x12), .b(new_n40_), .c(x11), .d(x07), .O(z3));
  nand2  g34(.a(x08), .b(x00), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n49_), .c(new_n46_), .O(new_n60_));
  oai21  g36(.a(x11), .b(new_n46_), .c(new_n60_), .O(z4));
  nor2   g37(.a(x12), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n47_), .d(new_n46_), .O(z5));
  aoi21  g39(.a(x09), .b(new_n34_), .c(x01), .O(new_n64_));
  oai21  g40(.a(x09), .b(new_n34_), .c(new_n64_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n59_), .c(new_n30_), .O(new_n66_));
  aoi21  g42(.a(x14), .b(x01), .c(x00), .O(new_n67_));
  nor2   g43(.a(x08), .b(new_n40_), .O(new_n68_));
  nor2   g44(.a(x07), .b(new_n30_), .O(new_n69_));
  oai21  g45(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  aoi21  g46(.a(new_n70_), .b(new_n66_), .c(x10), .O(z6));
  nand3  g47(.a(x09), .b(x03), .c(new_n25_), .O(new_n72_));
  nand3  g48(.a(new_n47_), .b(new_n54_), .c(x01), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n30_), .O(new_n75_));
  oai21  g51(.a(x07), .b(new_n30_), .c(x03), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n54_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n42_), .O(z7));
  inv1   g54(.a(x12), .O(new_n79_));
  nand4  g55(.a(new_n79_), .b(x03), .c(new_n30_), .d(new_n25_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x09), .O(new_n81_));
  nand2  g57(.a(new_n49_), .b(new_n40_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n81_), .c(new_n42_), .O(z8));
endmodule


