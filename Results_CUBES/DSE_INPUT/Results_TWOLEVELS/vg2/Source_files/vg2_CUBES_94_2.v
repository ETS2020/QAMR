// Benchmark "FAU" written by ABC on Mon Jul  6 14:57:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  inv1   g007(.a(new_n35_), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x13), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n36_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n42_), .c(new_n40_), .O(new_n46_));
  nor2   g013(.a(new_n46_), .b(new_n38_), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x11), .b(x06), .O(new_n49_));
  nor2   g016(.a(x20), .b(x14), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n47_), .O(z0));
  nand3  g019(.a(x15), .b(x13), .c(x05), .O(new_n53_));
  nand2  g020(.a(new_n44_), .b(new_n43_), .O(new_n54_));
  nand2  g021(.a(x22), .b(x21), .O(new_n55_));
  oai21  g022(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g023(.a(x03), .b(x01), .O(new_n57_));
  nand4  g024(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(x23), .b(x20), .c(x17), .O(new_n60_));
  nand4  g027(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  and2   g029(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  and2   g030(.a(new_n63_), .b(new_n56_), .O(new_n64_));
  nor2   g031(.a(x06), .b(x04), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n48_), .c(new_n43_), .d(x00), .O(new_n66_));
  inv1   g033(.a(x20), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nor2   g037(.a(x14), .b(x12), .O(new_n71_));
  nor2   g038(.a(x17), .b(x16), .O(new_n72_));
  nor2   g039(.a(x09), .b(x08), .O(new_n73_));
  nor2   g040(.a(x11), .b(x10), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nor3   g042(.a(new_n75_), .b(new_n70_), .c(new_n66_), .O(new_n76_));
  oai21  g043(.a(new_n76_), .b(new_n64_), .c(new_n36_), .O(new_n77_));
  nand2  g044(.a(x19), .b(new_n34_), .O(new_n78_));
  nand2  g045(.a(new_n41_), .b(x05), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g047(.a(new_n73_), .b(new_n65_), .c(new_n48_), .O(new_n81_));
  nor2   g048(.a(x23), .b(x22), .O(new_n82_));
  nor2   g049(.a(x12), .b(x11), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n82_), .c(new_n72_), .d(new_n50_), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  aoi21  g052(.a(x22), .b(x05), .c(x13), .O(new_n86_));
  nand2  g053(.a(x24), .b(x18), .O(new_n87_));
  nand3  g054(.a(new_n39_), .b(x22), .c(x15), .O(new_n88_));
  oai21  g055(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  aoi22  g056(.a(new_n89_), .b(new_n63_), .c(new_n85_), .d(new_n80_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n77_), .O(z1));
  nand3  g058(.a(new_n36_), .b(x15), .c(x13), .O(new_n92_));
  aoi21  g059(.a(new_n92_), .b(new_n87_), .c(new_n34_), .O(new_n93_));
  inv1   g060(.a(new_n54_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n36_), .c(x21), .O(new_n95_));
  nand3  g062(.a(x24), .b(x18), .c(x13), .O(new_n96_));
  nand2  g063(.a(new_n39_), .b(x15), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(new_n93_), .O(new_n99_));
  inv1   g066(.a(new_n57_), .O(new_n100_));
  inv1   g067(.a(x06), .O(new_n101_));
  inv1   g068(.a(x11), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n100_), .c(x20), .d(x14), .O(new_n104_));
  nor2   g071(.a(new_n104_), .b(new_n99_), .O(z2));
  nand2  g072(.a(new_n94_), .b(x21), .O(new_n106_));
  nand3  g073(.a(x20), .b(x14), .c(x08), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n103_), .c(new_n100_), .O(new_n109_));
  aoi21  g076(.a(new_n106_), .b(new_n53_), .c(new_n109_), .O(new_n110_));
  nand2  g077(.a(new_n94_), .b(x00), .O(new_n111_));
  nand3  g078(.a(x19), .b(x13), .c(x05), .O(new_n112_));
  nor2   g079(.a(x14), .b(x08), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(new_n49_), .c(new_n48_), .d(new_n67_), .O(new_n114_));
  aoi21  g081(.a(new_n112_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n110_), .c(new_n36_), .O(new_n116_));
  nand2  g083(.a(new_n49_), .b(new_n48_), .O(new_n117_));
  nand3  g084(.a(new_n100_), .b(x08), .c(x06), .O(new_n118_));
  nand4  g085(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n119_));
  nand3  g086(.a(new_n113_), .b(new_n67_), .c(x07), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n117_), .c(new_n119_), .d(new_n118_), .O(new_n121_));
  nor2   g088(.a(new_n39_), .b(new_n36_), .O(new_n122_));
  nand4  g089(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n123_));
  nand3  g090(.a(new_n113_), .b(new_n67_), .c(x19), .O(new_n124_));
  oai22  g091(.a(new_n124_), .b(new_n117_), .c(new_n123_), .d(new_n118_), .O(new_n125_));
  aoi22  g092(.a(new_n125_), .b(new_n39_), .c(new_n122_), .d(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n116_), .O(z3));
  inv1   g094(.a(x18), .O(new_n128_));
  inv1   g095(.a(x04), .O(new_n129_));
  inv1   g096(.a(x17), .O(new_n130_));
  aoi21  g097(.a(x23), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  inv1   g098(.a(x09), .O(new_n132_));
  oai21  g099(.a(x13), .b(x05), .c(new_n132_), .O(new_n133_));
  oai21  g100(.a(new_n131_), .b(new_n86_), .c(new_n133_), .O(new_n134_));
  nor2   g101(.a(new_n39_), .b(x08), .O(new_n135_));
  aoi21  g102(.a(new_n134_), .b(x16), .c(new_n135_), .O(new_n136_));
  inv1   g103(.a(new_n39_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(x08), .c(x07), .O(new_n138_));
  oai21  g105(.a(new_n136_), .b(new_n128_), .c(new_n138_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(x24), .O(new_n140_));
  inv1   g107(.a(new_n131_), .O(new_n141_));
  inv1   g108(.a(x15), .O(new_n142_));
  nand3  g109(.a(new_n36_), .b(x13), .c(x05), .O(new_n143_));
  nand2  g110(.a(new_n39_), .b(x22), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n143_), .c(new_n142_), .O(new_n145_));
  nor3   g112(.a(new_n55_), .b(new_n54_), .c(x24), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n145_), .c(new_n141_), .O(new_n147_));
  nand4  g114(.a(new_n94_), .b(new_n36_), .c(x21), .d(new_n132_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g117(.a(x16), .O(new_n151_));
  oai21  g118(.a(new_n46_), .b(new_n38_), .c(x09), .O(new_n152_));
  nand3  g119(.a(new_n79_), .b(new_n78_), .c(new_n45_), .O(new_n153_));
  oai21  g120(.a(x23), .b(new_n129_), .c(new_n130_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n153_), .c(new_n68_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  and2   g123(.a(new_n143_), .b(new_n137_), .O(new_n157_));
  oai21  g124(.a(new_n151_), .b(x09), .c(x08), .O(new_n158_));
  aoi22  g125(.a(new_n158_), .b(x15), .c(x19), .d(x08), .O(new_n159_));
  inv1   g126(.a(x21), .O(new_n160_));
  nand2  g127(.a(x08), .b(x00), .O(new_n161_));
  oai21  g128(.a(new_n160_), .b(x08), .c(new_n161_), .O(new_n162_));
  nand3  g129(.a(new_n162_), .b(new_n94_), .c(new_n36_), .O(new_n163_));
  oai21  g130(.a(new_n159_), .b(new_n157_), .c(new_n163_), .O(new_n164_));
  aoi21  g131(.a(new_n156_), .b(new_n151_), .c(new_n164_), .O(new_n165_));
  nand3  g132(.a(new_n165_), .b(new_n150_), .c(new_n140_), .O(z4));
  inv1   g133(.a(new_n47_), .O(z5));
  inv1   g134(.a(x14), .O(new_n168_));
  nand2  g135(.a(x20), .b(new_n168_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n101_), .O(new_n170_));
  aoi21  g137(.a(new_n170_), .b(new_n102_), .c(x03), .O(new_n171_));
  inv1   g138(.a(x03), .O(new_n172_));
  nand2  g139(.a(new_n67_), .b(x14), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(x06), .O(new_n174_));
  aoi21  g141(.a(new_n174_), .b(x11), .c(new_n172_), .O(new_n175_));
  oai22  g142(.a(new_n175_), .b(new_n99_), .c(new_n171_), .d(new_n47_), .O(z6));
  inv1   g143(.a(new_n99_), .O(z7));
endmodule


