// Benchmark "FAU" written by ABC on Mon Jul  6 14:58:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n35_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n41_), .c(new_n40_), .d(new_n38_), .O(z5));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x20), .b(x14), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  and2   g018(.a(new_n51_), .b(z5), .O(z0));
  inv1   g019(.a(x00), .O(new_n53_));
  inv1   g020(.a(x19), .O(new_n54_));
  nand2  g021(.a(new_n43_), .b(new_n42_), .O(new_n55_));
  nand2  g022(.a(x13), .b(x05), .O(new_n56_));
  oai22  g023(.a(new_n56_), .b(new_n54_), .c(new_n55_), .d(new_n53_), .O(new_n57_));
  inv1   g024(.a(x04), .O(new_n58_));
  nor2   g025(.a(x09), .b(x08), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n48_), .c(new_n46_), .d(new_n58_), .O(new_n60_));
  inv1   g027(.a(x16), .O(new_n61_));
  inv1   g028(.a(x17), .O(new_n62_));
  nor2   g029(.a(x23), .b(x22), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nor2   g031(.a(x12), .b(x11), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n49_), .O(new_n66_));
  nor3   g033(.a(new_n66_), .b(new_n64_), .c(new_n60_), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  inv1   g035(.a(x22), .O(new_n69_));
  inv1   g036(.a(x23), .O(new_n70_));
  nand4  g037(.a(x08), .b(x06), .c(x04), .d(x03), .O(new_n71_));
  nor3   g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  inv1   g039(.a(x20), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n62_), .O(new_n74_));
  nand4  g041(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n75_));
  nand4  g042(.a(new_n43_), .b(x09), .c(new_n42_), .d(x01), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n74_), .c(new_n72_), .d(x21), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand3  g049(.a(x17), .b(x16), .c(x14), .O(new_n83_));
  nand4  g050(.a(x12), .b(x11), .c(x09), .d(x01), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nor2   g053(.a(new_n34_), .b(x06), .O(new_n87_));
  nor2   g054(.a(x16), .b(x14), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n63_), .d(new_n48_), .O(new_n89_));
  inv1   g056(.a(x08), .O(new_n90_));
  nor3   g057(.a(x20), .b(x17), .c(x12), .O(new_n91_));
  nor2   g058(.a(x11), .b(x09), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n58_), .O(new_n93_));
  oai21  g060(.a(new_n93_), .b(new_n89_), .c(new_n86_), .O(new_n94_));
  nor2   g061(.a(new_n39_), .b(new_n35_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand4  g063(.a(new_n88_), .b(new_n63_), .c(new_n90_), .d(new_n46_), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  inv1   g065(.a(x13), .O(new_n99_));
  nand3  g066(.a(new_n92_), .b(x19), .c(new_n99_), .O(new_n100_));
  inv1   g067(.a(x05), .O(new_n101_));
  nand3  g068(.a(new_n48_), .b(new_n101_), .c(new_n58_), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n98_), .c(new_n91_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n96_), .c(new_n80_), .O(z1));
  nand2  g072(.a(x24), .b(x18), .O(new_n106_));
  nand3  g073(.a(new_n35_), .b(x15), .c(x13), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n106_), .c(new_n101_), .O(new_n108_));
  nand4  g075(.a(new_n35_), .b(x21), .c(new_n43_), .d(new_n42_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n39_), .b(x15), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  nor2   g080(.a(new_n47_), .b(new_n46_), .O(new_n114_));
  inv1   g081(.a(x14), .O(new_n115_));
  nor2   g082(.a(new_n73_), .b(new_n115_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(new_n114_), .c(x03), .d(x01), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n113_), .O(z2));
  inv1   g085(.a(x15), .O(new_n119_));
  inv1   g086(.a(x21), .O(new_n120_));
  oai22  g087(.a(new_n56_), .b(new_n119_), .c(new_n55_), .d(new_n120_), .O(new_n121_));
  nor2   g088(.a(new_n115_), .b(new_n47_), .O(new_n122_));
  nand4  g089(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n122_), .c(new_n121_), .d(x20), .O(new_n125_));
  nor3   g092(.a(x20), .b(x14), .c(x11), .O(new_n126_));
  nand3  g093(.a(new_n48_), .b(new_n90_), .c(new_n46_), .O(new_n127_));
  inv1   g094(.a(new_n127_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n126_), .c(new_n57_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n35_), .O(new_n131_));
  nand3  g098(.a(new_n122_), .b(x20), .c(x18), .O(new_n132_));
  nand3  g099(.a(new_n49_), .b(new_n47_), .c(new_n90_), .O(new_n133_));
  nand2  g100(.a(new_n87_), .b(new_n48_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n123_), .O(new_n135_));
  nand3  g102(.a(new_n122_), .b(x20), .c(x15), .O(new_n136_));
  nand4  g103(.a(new_n73_), .b(x19), .c(new_n115_), .d(new_n47_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n127_), .c(new_n136_), .d(new_n123_), .O(new_n138_));
  aoi22  g105(.a(new_n138_), .b(new_n39_), .c(new_n135_), .d(new_n95_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n131_), .O(z3));
  aoi21  g107(.a(x23), .b(new_n58_), .c(new_n62_), .O(new_n141_));
  oai21  g108(.a(new_n141_), .b(new_n69_), .c(x09), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(x16), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(x08), .c(new_n120_), .O(new_n144_));
  inv1   g111(.a(x09), .O(new_n145_));
  aoi21  g112(.a(new_n70_), .b(x04), .c(x17), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x22), .c(new_n145_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n61_), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(new_n90_), .c(new_n53_), .O(new_n149_));
  nor2   g116(.a(new_n55_), .b(x24), .O(new_n150_));
  oai21  g117(.a(new_n149_), .b(new_n144_), .c(new_n150_), .O(new_n151_));
  inv1   g118(.a(new_n39_), .O(new_n152_));
  oai21  g119(.a(new_n56_), .b(x24), .c(new_n152_), .O(new_n153_));
  aoi21  g120(.a(new_n147_), .b(new_n61_), .c(x08), .O(new_n154_));
  nand2  g121(.a(x15), .b(new_n90_), .O(new_n155_));
  oai21  g122(.a(new_n154_), .b(new_n54_), .c(new_n155_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  inv1   g124(.a(x18), .O(new_n158_));
  aoi21  g125(.a(new_n143_), .b(x08), .c(new_n158_), .O(new_n159_));
  aoi21  g126(.a(new_n148_), .b(new_n90_), .c(new_n34_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n159_), .c(new_n95_), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n157_), .c(new_n151_), .O(z4));
  nand2  g129(.a(x20), .b(new_n115_), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(new_n46_), .c(x11), .O(new_n164_));
  oai21  g131(.a(new_n164_), .b(x03), .c(z5), .O(new_n165_));
  inv1   g132(.a(x03), .O(new_n166_));
  inv1   g133(.a(new_n113_), .O(z7));
  nand2  g134(.a(new_n73_), .b(x14), .O(new_n168_));
  aoi21  g135(.a(new_n168_), .b(x06), .c(new_n47_), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(new_n166_), .c(z7), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n165_), .O(z6));
endmodule


