// Benchmark "FAU" written by ABC on Mon Jul  6 13:30:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n126_, new_n127_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n46_), .O(z00));
  nand2  g07(.a(new_n45_), .b(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n49_), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n48_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n51_), .O(z02));
  inv1   g12(.a(x00), .O(new_n56_));
  inv1   g13(.a(x01), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(x02), .O(new_n59_));
  nand2  g16(.a(x10), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(x12), .b(x11), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z03));
  inv1   g19(.a(x11), .O(new_n63_));
  inv1   g20(.a(x12), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor3   g22(.a(new_n65_), .b(new_n60_), .c(new_n59_), .O(z04));
  nor3   g23(.a(new_n59_), .b(new_n53_), .c(new_n44_), .O(z05));
  nor2   g24(.a(new_n63_), .b(new_n53_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n44_), .c(x02), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(new_n57_), .c(new_n56_), .O(z06));
  nand4  g27(.a(new_n68_), .b(new_n64_), .c(new_n44_), .d(x01), .O(new_n71_));
  nand2  g28(.a(x02), .b(x00), .O(new_n72_));
  aoi21  g29(.a(new_n71_), .b(x01), .c(new_n72_), .O(z07));
  inv1   g30(.a(x03), .O(new_n74_));
  nor2   g31(.a(x06), .b(x05), .O(new_n75_));
  nor2   g32(.a(x08), .b(x07), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(new_n75_), .c(x04), .d(new_n74_), .O(new_n77_));
  nor2   g34(.a(x01), .b(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x02), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n80_), .c(x17), .O(new_n81_));
  nor3   g38(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(z08));
  inv1   g39(.a(x15), .O(new_n83_));
  inv1   g40(.a(x16), .O(new_n84_));
  nand3  g41(.a(x20), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand2  g43(.a(x02), .b(new_n57_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nor2   g45(.a(new_n64_), .b(x11), .O(new_n89_));
  nor2   g46(.a(x14), .b(x13), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n86_), .O(new_n91_));
  nor2   g48(.a(new_n80_), .b(new_n57_), .O(new_n92_));
  nor2   g49(.a(x20), .b(x19), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n56_), .O(new_n97_));
  aoi21  g54(.a(new_n94_), .b(new_n91_), .c(new_n97_), .O(z09));
  nand4  g55(.a(new_n93_), .b(new_n92_), .c(x22), .d(x21), .O(new_n99_));
  inv1   g56(.a(x13), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(x12), .c(new_n63_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  nor2   g59(.a(new_n83_), .b(x14), .O(new_n103_));
  nand4  g60(.a(new_n96_), .b(new_n95_), .c(x20), .d(x16), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n99_), .c(x00), .O(z10));
  nand3  g64(.a(new_n93_), .b(new_n92_), .c(x21), .O(new_n108_));
  nand3  g65(.a(new_n95_), .b(x20), .c(new_n84_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n103_), .c(new_n102_), .O(new_n111_));
  nand2  g68(.a(new_n96_), .b(new_n56_), .O(new_n112_));
  aoi21  g69(.a(new_n111_), .b(new_n108_), .c(new_n112_), .O(z11));
  inv1   g70(.a(x02), .O(new_n114_));
  oai21  g71(.a(new_n53_), .b(new_n114_), .c(new_n58_), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g73(.a(new_n48_), .b(x17), .c(new_n114_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n78_), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n115_), .c(new_n121_), .O(z12));
  nor2   g79(.a(x19), .b(new_n47_), .O(new_n123_));
  and2   g80(.a(new_n123_), .b(new_n45_), .O(z13));
  nor2   g81(.a(new_n54_), .b(new_n46_), .O(z14));
  aoi21  g82(.a(new_n53_), .b(x01), .c(new_n114_), .O(new_n126_));
  nand3  g83(.a(x19), .b(new_n114_), .c(new_n57_), .O(new_n127_));
  oai21  g84(.a(new_n126_), .b(new_n56_), .c(new_n127_), .O(z15));
  nor2   g85(.a(new_n57_), .b(x00), .O(z16));
  inv1   g86(.a(new_n79_), .O(z17));
endmodule


