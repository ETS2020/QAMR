// Benchmark "FAU" written by ABC on Mon Jul  6 13:31:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nor2   g05(.a(x18), .b(x17), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(x10), .d(new_n47_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  inv1   g08(.a(x17), .O(new_n52_));
  inv1   g09(.a(x18), .O(new_n53_));
  nand4  g10(.a(new_n48_), .b(new_n53_), .c(new_n52_), .d(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n46_), .O(z01));
  nor2   g12(.a(x19), .b(new_n53_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nand3  g14(.a(new_n45_), .b(new_n52_), .c(new_n44_), .O(new_n59_));
  inv1   g15(.a(x10), .O(new_n60_));
  inv1   g16(.a(x11), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(x12), .O(new_n63_));
  nand2  g19(.a(x01), .b(x00), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(new_n65_));
  nand3  g21(.a(new_n65_), .b(new_n47_), .c(x02), .O(new_n66_));
  oai22  g22(.a(new_n66_), .b(new_n63_), .c(new_n59_), .d(new_n58_), .O(z03));
  inv1   g23(.a(x12), .O(new_n68_));
  nand4  g24(.a(new_n68_), .b(new_n61_), .c(x10), .d(new_n47_), .O(new_n69_));
  nor3   g25(.a(new_n69_), .b(new_n64_), .c(new_n44_), .O(z04));
  nand3  g26(.a(new_n65_), .b(x09), .c(x02), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z05));
  nand3  g28(.a(new_n62_), .b(new_n47_), .c(x02), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n64_), .O(z06));
  nand4  g30(.a(new_n62_), .b(new_n68_), .c(new_n47_), .d(x01), .O(new_n75_));
  nand2  g31(.a(x02), .b(x00), .O(new_n76_));
  aoi21  g32(.a(new_n75_), .b(x01), .c(new_n76_), .O(z07));
  inv1   g33(.a(x03), .O(new_n78_));
  nor2   g34(.a(x06), .b(x05), .O(new_n79_));
  nor2   g35(.a(x08), .b(x07), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nand2  g37(.a(new_n45_), .b(x02), .O(new_n82_));
  nand3  g38(.a(x19), .b(new_n53_), .c(x17), .O(new_n83_));
  nor3   g39(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(z08));
  inv1   g40(.a(x15), .O(new_n85_));
  inv1   g41(.a(x16), .O(new_n86_));
  nand3  g42(.a(x20), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  inv1   g44(.a(x01), .O(new_n89_));
  nand2  g45(.a(x02), .b(new_n89_), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  nor2   g47(.a(new_n68_), .b(x11), .O(new_n92_));
  nor2   g48(.a(x14), .b(x13), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n88_), .O(new_n94_));
  nor2   g50(.a(x20), .b(x19), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(x18), .c(x01), .O(new_n96_));
  inv1   g52(.a(x00), .O(new_n97_));
  inv1   g53(.a(x21), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  aoi21  g56(.a(new_n96_), .b(new_n94_), .c(new_n100_), .O(z09));
  nor2   g57(.a(x20), .b(new_n89_), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n57_), .c(x22), .d(x21), .O(new_n103_));
  inv1   g59(.a(x13), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(x12), .c(new_n61_), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n90_), .O(new_n106_));
  nor2   g62(.a(new_n85_), .b(x14), .O(new_n107_));
  nand4  g63(.a(new_n99_), .b(new_n98_), .c(x20), .d(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n103_), .c(x00), .O(z10));
  nand4  g67(.a(new_n95_), .b(x21), .c(x18), .d(x01), .O(new_n112_));
  nand3  g68(.a(new_n98_), .b(x20), .c(new_n86_), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n107_), .c(new_n106_), .O(new_n115_));
  nand2  g71(.a(new_n99_), .b(new_n97_), .O(new_n116_));
  aoi21  g72(.a(new_n115_), .b(new_n112_), .c(new_n116_), .O(z11));
  nand3  g73(.a(new_n45_), .b(new_n48_), .c(x17), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n64_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  nand2  g76(.a(new_n48_), .b(new_n44_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n45_), .c(x23), .O(new_n122_));
  inv1   g78(.a(x24), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g80(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z12));
  nor3   g81(.a(new_n46_), .b(x19), .c(new_n52_), .O(z13));
  nand4  g82(.a(new_n48_), .b(new_n53_), .c(new_n52_), .d(new_n60_), .O(new_n127_));
  nor2   g83(.a(new_n127_), .b(new_n46_), .O(z14));
  aoi21  g84(.a(new_n60_), .b(x01), .c(new_n44_), .O(new_n129_));
  nand3  g85(.a(x19), .b(new_n44_), .c(new_n89_), .O(new_n130_));
  oai21  g86(.a(new_n129_), .b(new_n97_), .c(new_n130_), .O(z15));
  nor2   g87(.a(new_n89_), .b(x00), .O(z16));
  inv1   g88(.a(new_n82_), .O(z17));
  zero   g89(.O(z02));
endmodule


