// Benchmark "FAU" written by ABC on Mon Jul  6 20:11:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  inv1   g000(.a(x32), .O(z0));
  nor2   g001(.a(x32), .b(x03), .O(new_n46_));
  inv1   g002(.a(x03), .O(new_n47_));
  nor2   g003(.a(x33), .b(new_n47_), .O(new_n48_));
  oai21  g004(.a(new_n48_), .b(new_n46_), .c(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  nor2   g006(.a(x32), .b(new_n47_), .O(new_n51_));
  nor2   g007(.a(x33), .b(x03), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n49_), .O(z1));
  xnor2a g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x01), .O(new_n56_));
  nor2   g012(.a(new_n47_), .b(new_n50_), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand2  g014(.a(new_n47_), .b(new_n50_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n47_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x06), .c(new_n50_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n50_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x05), .c(new_n47_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n62_), .c(x01), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n60_), .c(new_n55_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  xnor2a g023(.a(x03), .b(x02), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n56_), .O(new_n69_));
  nand2  g025(.a(new_n63_), .b(new_n61_), .O(new_n70_));
  nand4  g026(.a(x06), .b(x05), .c(new_n47_), .d(new_n50_), .O(new_n71_));
  aoi21  g027(.a(new_n71_), .b(new_n70_), .c(x01), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n67_), .O(new_n73_));
  inv1   g029(.a(x09), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x08), .O(new_n75_));
  aoi21  g031(.a(new_n73_), .b(new_n66_), .c(new_n75_), .O(z2));
  inv1   g032(.a(x26), .O(new_n77_));
  inv1   g033(.a(x27), .O(new_n78_));
  nor2   g034(.a(x23), .b(x18), .O(new_n79_));
  nor2   g035(.a(x24), .b(x19), .O(new_n80_));
  nor2   g036(.a(x20), .b(x15), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  inv1   g038(.a(x16), .O(new_n83_));
  inv1   g039(.a(x21), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nor2   g042(.a(x22), .b(x17), .O(new_n87_));
  nor4   g043(.a(new_n87_), .b(new_n86_), .c(new_n80_), .d(new_n79_), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n57_), .O(new_n89_));
  nand2  g045(.a(x25), .b(x01), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n78_), .d(new_n77_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z3));
  xor2a  g049(.a(x28), .b(x27), .O(new_n94_));
  nor2   g050(.a(new_n90_), .b(x26), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z4));
  inv1   g053(.a(x29), .O(new_n98_));
  inv1   g054(.a(x28), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n78_), .O(new_n100_));
  nand3  g056(.a(new_n98_), .b(x28), .c(x27), .O(new_n101_));
  oai21  g057(.a(new_n100_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n95_), .c(new_n58_), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n88_), .O(z5));
  nand2  g060(.a(new_n95_), .b(new_n58_), .O(new_n105_));
  inv1   g061(.a(new_n85_), .O(new_n106_));
  oai21  g062(.a(new_n98_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  inv1   g063(.a(new_n79_), .O(new_n108_));
  inv1   g064(.a(new_n80_), .O(new_n109_));
  inv1   g065(.a(new_n87_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n82_), .c(new_n109_), .d(new_n108_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  inv1   g068(.a(x30), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x29), .c(x28), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(new_n107_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x27), .O(new_n116_));
  nand2  g072(.a(new_n100_), .b(x29), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nor2   g074(.a(new_n118_), .b(new_n113_), .O(new_n119_));
  aoi22  g075(.a(new_n119_), .b(new_n111_), .c(new_n106_), .d(new_n78_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n116_), .c(new_n105_), .O(z6));
  inv1   g077(.a(new_n86_), .O(new_n122_));
  nand4  g078(.a(new_n110_), .b(new_n122_), .c(new_n109_), .d(new_n108_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nand3  g080(.a(new_n118_), .b(new_n124_), .c(x30), .O(new_n125_));
  nand3  g081(.a(new_n100_), .b(x30), .c(x29), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(x31), .c(new_n105_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n125_), .c(new_n123_), .O(z7));
  nand2  g084(.a(new_n99_), .b(new_n78_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x29), .O(new_n130_));
  inv1   g086(.a(x17), .O(new_n131_));
  inv1   g087(.a(x18), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n132_), .c(x13), .O(new_n133_));
  inv1   g089(.a(x19), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x18), .c(x14), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n131_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n131_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nor2   g094(.a(new_n113_), .b(new_n83_), .O(new_n139_));
  oai21  g095(.a(new_n138_), .b(new_n136_), .c(new_n139_), .O(new_n140_));
  nand3  g096(.a(x19), .b(x18), .c(x17), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n83_), .c(x11), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g100(.a(new_n139_), .b(x19), .O(new_n145_));
  nand2  g101(.a(x18), .b(x17), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  inv1   g103(.a(x10), .O(new_n148_));
  nor2   g104(.a(x15), .b(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nor2   g106(.a(new_n150_), .b(new_n145_), .O(new_n151_));
  aoi21  g107(.a(new_n144_), .b(x15), .c(new_n151_), .O(new_n152_));
  nand2  g108(.a(z0), .b(x31), .O(new_n153_));
  nand2  g109(.a(x32), .b(new_n113_), .O(new_n154_));
  oai21  g110(.a(new_n153_), .b(new_n152_), .c(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n130_), .O(new_n156_));
  nand2  g112(.a(new_n135_), .b(new_n133_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x17), .O(new_n158_));
  nand2  g114(.a(new_n137_), .b(new_n158_), .O(new_n159_));
  aoi22  g115(.a(new_n149_), .b(new_n142_), .c(new_n159_), .d(x15), .O(new_n160_));
  nand4  g116(.a(new_n129_), .b(z0), .c(x31), .d(new_n113_), .O(new_n161_));
  nand2  g117(.a(new_n99_), .b(new_n78_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(x32), .c(x30), .O(new_n163_));
  oai21  g119(.a(new_n161_), .b(new_n160_), .c(new_n163_), .O(new_n164_));
  inv1   g120(.a(x15), .O(new_n165_));
  nand2  g121(.a(x16), .b(x10), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n141_), .c(new_n165_), .O(new_n167_));
  nand2  g123(.a(x19), .b(x18), .O(new_n168_));
  nand2  g124(.a(x17), .b(x11), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n168_), .c(new_n83_), .O(new_n170_));
  inv1   g126(.a(x13), .O(new_n171_));
  oai21  g127(.a(new_n134_), .b(new_n171_), .c(new_n132_), .O(new_n172_));
  inv1   g128(.a(x14), .O(new_n173_));
  aoi21  g129(.a(new_n134_), .b(new_n173_), .c(new_n124_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n172_), .c(new_n170_), .O(new_n175_));
  inv1   g131(.a(x12), .O(new_n176_));
  oai21  g132(.a(new_n168_), .b(new_n176_), .c(new_n131_), .O(new_n177_));
  nand2  g133(.a(x16), .b(x15), .O(new_n178_));
  oai21  g134(.a(new_n178_), .b(new_n141_), .c(new_n177_), .O(new_n179_));
  nor2   g135(.a(new_n179_), .b(new_n175_), .O(new_n180_));
  aoi21  g136(.a(new_n180_), .b(new_n167_), .c(z0), .O(new_n181_));
  aoi21  g137(.a(new_n164_), .b(x29), .c(new_n181_), .O(new_n182_));
  nand2  g138(.a(new_n77_), .b(x00), .O(new_n183_));
  aoi21  g139(.a(new_n182_), .b(new_n156_), .c(new_n183_), .O(z8));
  inv1   g140(.a(new_n130_), .O(new_n185_));
  inv1   g141(.a(x33), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(x31), .O(new_n187_));
  nand2  g143(.a(x33), .b(new_n124_), .O(new_n188_));
  oai21  g144(.a(new_n187_), .b(new_n152_), .c(new_n188_), .O(new_n189_));
  nand2  g145(.a(new_n189_), .b(new_n185_), .O(new_n190_));
  nand3  g146(.a(new_n186_), .b(new_n124_), .c(new_n113_), .O(new_n191_));
  nand2  g147(.a(x33), .b(x31), .O(new_n192_));
  oai21  g148(.a(new_n191_), .b(new_n160_), .c(new_n192_), .O(new_n193_));
  nand2  g149(.a(new_n124_), .b(x30), .O(new_n194_));
  nand2  g150(.a(x31), .b(new_n113_), .O(new_n195_));
  oai21  g151(.a(new_n132_), .b(new_n176_), .c(new_n131_), .O(new_n196_));
  nand2  g152(.a(new_n132_), .b(new_n171_), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(new_n196_), .c(new_n195_), .d(new_n194_), .O(new_n198_));
  nand2  g154(.a(new_n134_), .b(x14), .O(new_n199_));
  nand4  g155(.a(new_n199_), .b(new_n147_), .c(x16), .d(x15), .O(new_n200_));
  oai21  g156(.a(new_n178_), .b(new_n146_), .c(new_n134_), .O(new_n201_));
  oai21  g157(.a(new_n166_), .b(new_n146_), .c(new_n165_), .O(new_n202_));
  inv1   g158(.a(x11), .O(new_n203_));
  oai21  g159(.a(new_n146_), .b(new_n203_), .c(new_n83_), .O(new_n204_));
  nand4  g160(.a(new_n204_), .b(new_n202_), .c(new_n201_), .d(new_n200_), .O(new_n205_));
  nor2   g161(.a(new_n205_), .b(new_n198_), .O(new_n206_));
  nor2   g162(.a(new_n206_), .b(new_n186_), .O(new_n207_));
  aoi21  g163(.a(new_n193_), .b(new_n130_), .c(new_n207_), .O(new_n208_));
  aoi21  g164(.a(new_n208_), .b(new_n190_), .c(new_n183_), .O(z9));
endmodule


