// Benchmark "FAU" written by ABC on Mon Jul  6 13:04:32 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n23_, new_n26_, new_n27_, new_n30_, new_n31_, new_n34_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n45_, new_n47_,
    new_n49_, new_n51_;
  nand4  g00(.a(x4), .b(x2), .c(x1), .d(x0), .O(new_n23_));
  inv1   g01(.a(new_n23_), .O(z01));
  inv1   g02(.a(x2), .O(new_n26_));
  nand4  g03(.a(x4), .b(new_n26_), .c(x1), .d(x0), .O(new_n27_));
  inv1   g04(.a(new_n27_), .O(z03));
  inv1   g05(.a(x1), .O(new_n30_));
  nand4  g06(.a(x4), .b(x2), .c(new_n30_), .d(x0), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(z05));
  nand4  g08(.a(x4), .b(new_n26_), .c(new_n30_), .d(x0), .O(new_n34_));
  inv1   g09(.a(new_n34_), .O(z07));
  nand4  g10(.a(x4), .b(x3), .c(x2), .d(x1), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z08));
  inv1   g12(.a(x0), .O(new_n38_));
  nand2  g13(.a(x1), .b(new_n38_), .O(new_n39_));
  inv1   g14(.a(x3), .O(new_n40_));
  nand3  g15(.a(x4), .b(new_n40_), .c(x2), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n39_), .O(z09));
  nand4  g17(.a(x4), .b(x3), .c(new_n26_), .d(x1), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(z10));
  nand3  g19(.a(x4), .b(new_n40_), .c(new_n26_), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n39_), .O(z11));
  nand4  g21(.a(x4), .b(x3), .c(x2), .d(new_n30_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z12));
  nand2  g23(.a(new_n30_), .b(new_n38_), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(new_n41_), .O(z13));
  nand4  g25(.a(x4), .b(x3), .c(new_n26_), .d(new_n30_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(z14));
  nor2   g27(.a(new_n49_), .b(new_n45_), .O(z15));
  zero   g28(.O(z00));
  zero   g29(.O(z02));
  zero   g30(.O(z04));
  zero   g31(.O(z06));
endmodule


