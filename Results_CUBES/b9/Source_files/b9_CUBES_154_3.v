// Benchmark "FAU" written by ABC on Mon Jul  6 15:00:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n171_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  aoi21  g002(.a(x36), .b(x27), .c(x10), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  nand3  g004(.a(x40), .b(x39), .c(new_n66_), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  oai22  g006(.a(new_n68_), .b(new_n64_), .c(new_n67_), .d(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(z00));
  inv1   g009(.a(x36), .O(new_n72_));
  inv1   g010(.a(x28), .O(new_n73_));
  nand2  g011(.a(x35), .b(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n64_), .O(new_n75_));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(x04), .O(z01));
  inv1   g017(.a(new_n67_), .O(new_n80_));
  inv1   g018(.a(x08), .O(new_n81_));
  inv1   g019(.a(x29), .O(new_n82_));
  inv1   g020(.a(x40), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x02), .O(new_n84_));
  oai21  g022(.a(new_n82_), .b(new_n81_), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n80_), .O(z02));
  oai21  g024(.a(x40), .b(x28), .c(x35), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n64_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n88_), .c(new_n92_), .O(z04));
  aoi21  g031(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g032(.a(z06), .O(z05));
  inv1   g033(.a(x03), .O(new_n96_));
  inv1   g034(.a(x00), .O(new_n97_));
  oai21  g035(.a(x25), .b(new_n97_), .c(x38), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n63_), .O(new_n99_));
  nor2   g037(.a(x33), .b(x31), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand3  g041(.a(x38), .b(new_n103_), .c(new_n97_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n96_), .O(z07));
  nand2  g044(.a(x40), .b(x39), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z08));
  nand4  g046(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z09));
  nand2  g048(.a(x36), .b(x27), .O(new_n111_));
  nor2   g049(.a(x32), .b(x30), .O(new_n112_));
  nand3  g050(.a(x40), .b(x39), .c(x05), .O(new_n113_));
  aoi21  g051(.a(new_n112_), .b(new_n111_), .c(new_n113_), .O(new_n114_));
  nand2  g052(.a(new_n107_), .b(x36), .O(new_n115_));
  nand2  g053(.a(x27), .b(x07), .O(new_n116_));
  aoi21  g054(.a(new_n115_), .b(new_n74_), .c(new_n116_), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(new_n114_), .c(new_n66_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(z10));
  oai21  g058(.a(new_n64_), .b(new_n66_), .c(x35), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n83_), .O(new_n122_));
  nand2  g060(.a(x35), .b(x28), .O(new_n123_));
  inv1   g061(.a(x35), .O(new_n124_));
  nand2  g062(.a(x39), .b(x29), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n64_), .c(new_n81_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n123_), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  inv1   g067(.a(x09), .O(new_n130_));
  nand2  g068(.a(new_n76_), .b(new_n130_), .O(new_n131_));
  aoi21  g069(.a(new_n129_), .b(new_n122_), .c(new_n131_), .O(z11));
  inv1   g070(.a(x10), .O(new_n133_));
  oai21  g071(.a(x37), .b(x36), .c(x27), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x40), .c(new_n133_), .O(new_n135_));
  oai21  g073(.a(x37), .b(x35), .c(x27), .O(new_n136_));
  nand2  g074(.a(z08), .b(new_n66_), .O(new_n137_));
  nor3   g075(.a(x40), .b(x37), .c(x28), .O(new_n138_));
  aoi21  g076(.a(new_n137_), .b(new_n136_), .c(new_n138_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n135_), .O(z12));
  inv1   g078(.a(x18), .O(new_n141_));
  inv1   g079(.a(x19), .O(new_n142_));
  nand3  g080(.a(x20), .b(new_n142_), .c(new_n141_), .O(new_n143_));
  inv1   g081(.a(x13), .O(new_n144_));
  nand3  g082(.a(new_n73_), .b(new_n144_), .c(new_n66_), .O(new_n145_));
  oai22  g083(.a(new_n145_), .b(new_n107_), .c(new_n143_), .d(new_n68_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand4  g085(.a(z08), .b(new_n78_), .c(new_n144_), .d(new_n66_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z13));
  oai21  g087(.a(new_n143_), .b(new_n136_), .c(new_n137_), .O(new_n150_));
  oai21  g088(.a(new_n78_), .b(new_n73_), .c(new_n144_), .O(new_n151_));
  nor2   g089(.a(new_n83_), .b(x37), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n143_), .c(new_n151_), .O(new_n153_));
  nand2  g091(.a(new_n78_), .b(new_n144_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n64_), .c(new_n138_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n153_), .c(new_n150_), .O(z14));
  nand4  g094(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(x23), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x23), .c(x22), .d(x01), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z17));
  nor2   g101(.a(new_n124_), .b(x28), .O(new_n164_));
  aoi22  g102(.a(z08), .b(x29), .c(new_n164_), .d(new_n64_), .O(new_n165_));
  nor2   g103(.a(new_n64_), .b(x04), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n164_), .c(new_n131_), .O(new_n167_));
  oai21  g105(.a(new_n165_), .b(new_n81_), .c(new_n167_), .O(z18));
  oai21  g106(.a(new_n73_), .b(x27), .c(x35), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n83_), .O(new_n170_));
  and2   g108(.a(new_n127_), .b(new_n126_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n170_), .c(new_n131_), .O(z20));
  aoi21  g110(.a(new_n129_), .b(new_n122_), .c(new_n131_), .O(z19));
endmodule


