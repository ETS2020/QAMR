// Benchmark "FAU" written by ABC on Mon Jul  6 15:00:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  inv1   g005(.a(x39), .O(new_n68_));
  inv1   g006(.a(x40), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(z08));
  inv1   g008(.a(x10), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x04), .O(new_n72_));
  aoi22  g010(.a(new_n72_), .b(z08), .c(new_n67_), .d(x27), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(x15), .c(x16), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  and2   g015(.a(new_n77_), .b(x27), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n78_), .c(x04), .O(z01));
  nor2   g020(.a(new_n75_), .b(x28), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  aoi21  g022(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g023(.a(new_n84_), .b(x04), .c(new_n85_), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n86_), .c(x40), .d(x39), .O(z02));
  inv1   g028(.a(x28), .O(new_n91_));
  oai21  g029(.a(new_n75_), .b(new_n91_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n88_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x03), .O(new_n100_));
  inv1   g038(.a(x00), .O(new_n101_));
  oai21  g039(.a(x25), .b(new_n101_), .c(x38), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n101_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  aoi21  g048(.a(new_n110_), .b(new_n106_), .c(new_n100_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  inv1   g051(.a(x05), .O(new_n114_));
  nand3  g052(.a(new_n77_), .b(x27), .c(x07), .O(new_n115_));
  nand2  g053(.a(new_n81_), .b(z08), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  nand2  g055(.a(x40), .b(new_n68_), .O(new_n118_));
  nand2  g056(.a(new_n69_), .b(x07), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n77_), .c(x27), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n117_), .c(new_n64_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(z10));
  nand2  g063(.a(z08), .b(x29), .O(new_n126_));
  oai21  g064(.a(new_n88_), .b(new_n64_), .c(new_n83_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g066(.a(new_n83_), .b(x27), .c(new_n64_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n87_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n128_), .c(new_n132_), .O(z11));
  aoi22  g071(.a(new_n63_), .b(new_n93_), .c(x40), .d(new_n88_), .O(new_n134_));
  nand2  g072(.a(z08), .b(new_n64_), .O(new_n135_));
  oai21  g073(.a(new_n66_), .b(new_n88_), .c(new_n135_), .O(new_n136_));
  oai21  g074(.a(new_n134_), .b(x10), .c(new_n136_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nor2   g076(.a(new_n68_), .b(x04), .O(new_n139_));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(new_n139_), .c(x40), .d(new_n138_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x20), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n66_), .c(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n81_), .c(z08), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(z13));
  nand4  g087(.a(new_n140_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nand2  g089(.a(new_n147_), .b(z08), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(new_n144_), .O(new_n153_));
  nor2   g091(.a(new_n69_), .b(new_n138_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n65_), .c(new_n88_), .O(new_n155_));
  nand3  g093(.a(new_n93_), .b(new_n75_), .c(x28), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n80_), .c(new_n79_), .O(new_n158_));
  nand2  g096(.a(new_n152_), .b(new_n66_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n158_), .c(new_n155_), .d(new_n153_), .O(z14));
  nand4  g098(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(x23), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x23), .c(x22), .d(x01), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z17));
  oai21  g105(.a(new_n84_), .b(x27), .c(new_n126_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x08), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n131_), .c(new_n129_), .O(z18));
  oai21  g108(.a(new_n75_), .b(new_n88_), .c(new_n87_), .O(new_n171_));
  oai21  g109(.a(new_n91_), .b(x27), .c(x35), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n126_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n171_), .c(new_n132_), .O(z20));
  aoi21  g112(.a(new_n130_), .b(new_n128_), .c(new_n132_), .O(z19));
endmodule


