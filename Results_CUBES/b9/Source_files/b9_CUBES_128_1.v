// Benchmark "FAU" written by ABC on Mon Jul  6 15:00:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n160_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(z08));
  nand3  g007(.a(z08), .b(x10), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n67_), .b(x27), .c(new_n71_), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(x15), .c(x16), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n78_), .c(x04), .O(z01));
  oai21  g020(.a(x08), .b(new_n64_), .c(x40), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n74_), .O(new_n84_));
  inv1   g022(.a(x40), .O(new_n85_));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  oai22  g024(.a(new_n86_), .b(x02), .c(new_n85_), .d(x39), .O(new_n87_));
  aoi21  g025(.a(new_n77_), .b(x04), .c(new_n87_), .O(new_n88_));
  nor2   g026(.a(new_n66_), .b(new_n74_), .O(new_n89_));
  inv1   g027(.a(x18), .O(new_n90_));
  inv1   g028(.a(x19), .O(new_n91_));
  nand3  g029(.a(x20), .b(new_n91_), .c(new_n90_), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  inv1   g032(.a(new_n66_), .O(new_n95_));
  nand2  g033(.a(new_n93_), .b(new_n95_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n85_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n94_), .c(new_n88_), .d(new_n84_), .O(z02));
  inv1   g036(.a(x35), .O(new_n99_));
  oai21  g037(.a(new_n99_), .b(new_n76_), .c(x27), .O(new_n100_));
  inv1   g038(.a(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n74_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n100_), .c(new_n104_), .O(z04));
  aoi21  g043(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g044(.a(z06), .O(z05));
  inv1   g045(.a(x03), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x15), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n111_), .O(new_n112_));
  nor2   g050(.a(x33), .b(x31), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n109_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n114_), .c(new_n108_), .O(z07));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z09));
  nand2  g059(.a(new_n68_), .b(x07), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  oai21  g063(.a(x32), .b(x30), .c(x40), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  aoi22  g065(.a(new_n127_), .b(x05), .c(new_n124_), .d(new_n78_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(x04), .c(new_n129_), .O(z10));
  nand2  g068(.a(z08), .b(x29), .O(new_n131_));
  nor2   g069(.a(new_n74_), .b(new_n64_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n77_), .c(new_n131_), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  inv1   g072(.a(new_n77_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x27), .c(new_n64_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nor2   g075(.a(x30), .b(x09), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n133_), .c(new_n139_), .O(z11));
  aoi21  g078(.a(new_n63_), .b(new_n101_), .c(new_n74_), .O(new_n141_));
  nor2   g079(.a(new_n68_), .b(x04), .O(new_n142_));
  oai22  g080(.a(new_n142_), .b(new_n89_), .c(new_n141_), .d(x10), .O(z12));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nor2   g083(.a(new_n85_), .b(x13), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n145_), .c(x39), .d(new_n64_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n96_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n127_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z13));
  nand4  g090(.a(new_n144_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n150_), .c(x39), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x40), .O(new_n155_));
  nand2  g093(.a(new_n126_), .b(new_n74_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n155_), .c(new_n97_), .O(z14));
  nand4  g095(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x23), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x23), .c(x22), .d(x01), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  oai21  g102(.a(new_n77_), .b(x27), .c(new_n131_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x08), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n138_), .c(new_n136_), .O(z18));
  oai21  g105(.a(new_n99_), .b(new_n74_), .c(new_n134_), .O(new_n168_));
  oai21  g106(.a(new_n76_), .b(x27), .c(x35), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n131_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n168_), .c(new_n139_), .O(z20));
  aoi21  g109(.a(new_n137_), .b(new_n133_), .c(new_n139_), .O(z19));
endmodule


