// Benchmark "FAU" written by ABC on Mon Jul  6 14:40:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  nand2  g02(.a(x05), .b(x04), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  aoi21  g05(.a(new_n50_), .b(new_n49_), .c(x07), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n48_), .O(z01));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(x24), .c(x25), .O(z03));
  xor2a  g14(.a(x11), .b(x02), .O(new_n60_));
  xor2a  g15(.a(x12), .b(x03), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  xor2a  g17(.a(x09), .b(x00), .O(new_n63_));
  xor2a  g18(.a(x10), .b(x01), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n62_), .c(x08), .O(z04));
  nor2   g21(.a(x13), .b(x08), .O(z05));
  inv1   g22(.a(x14), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(x08), .O(z06));
  nand2  g24(.a(new_n46_), .b(x06), .O(z07));
  inv1   g25(.a(x20), .O(new_n71_));
  inv1   g26(.a(x17), .O(new_n72_));
  inv1   g27(.a(x18), .O(new_n73_));
  nand3  g28(.a(new_n54_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n57_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x23), .c(x24), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z08));
  nor2   g32(.a(x15), .b(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x05), .c(x04), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  nand3  g35(.a(new_n78_), .b(new_n48_), .c(new_n72_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z10));
  xnor2a g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(new_n78_), .b(new_n48_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n83_), .O(z11));
  oai21  g40(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n86_));
  nand3  g41(.a(new_n54_), .b(x18), .c(x17), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n86_), .c(new_n84_), .O(z12));
  nand2  g43(.a(new_n78_), .b(new_n48_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor2   g45(.a(new_n73_), .b(new_n72_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n71_), .c(x19), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x20), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n92_), .c(new_n90_), .O(z13));
  nand2  g50(.a(new_n92_), .b(x21), .O(new_n96_));
  inv1   g51(.a(x21), .O(new_n97_));
  nand4  g52(.a(new_n91_), .b(new_n97_), .c(new_n71_), .d(x19), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n90_), .O(z14));
  nand2  g54(.a(new_n98_), .b(x22), .O(new_n100_));
  inv1   g55(.a(x22), .O(new_n101_));
  inv1   g56(.a(new_n93_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n97_), .d(new_n71_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n100_), .c(new_n90_), .O(z15));
  nand4  g59(.a(new_n48_), .b(x19), .c(x18), .d(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n78_), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(new_n97_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n91_), .c(new_n71_), .d(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n78_), .d(new_n71_), .O(z16));
  nand2  g64(.a(x24), .b(x23), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n78_), .c(new_n48_), .O(z17));
  nand3  g66(.a(new_n78_), .b(new_n48_), .c(new_n50_), .O(z18));
  buf    g67(.a(x16), .O(z02));
endmodule


