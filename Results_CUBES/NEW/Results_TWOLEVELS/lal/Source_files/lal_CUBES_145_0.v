// Benchmark "FAU" written by ABC on Mon Jul  6 14:41:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x25), .O(new_n48_));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g09(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(x23), .c(x24), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g16(.a(new_n57_), .O(z03));
  xor2a  g17(.a(x11), .b(x02), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  xor2a  g20(.a(x09), .b(x00), .O(new_n66_));
  xor2a  g21(.a(x10), .b(x01), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n65_), .c(x08), .O(z04));
  nor2   g24(.a(x13), .b(x08), .O(z05));
  inv1   g25(.a(x14), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x08), .O(z06));
  nand2  g27(.a(new_n46_), .b(x06), .O(z07));
  nand3  g28(.a(new_n49_), .b(x19), .c(x17), .O(new_n74_));
  inv1   g29(.a(x21), .O(new_n75_));
  inv1   g30(.a(x22), .O(new_n76_));
  nor2   g31(.a(x24), .b(x23), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  inv1   g34(.a(x24), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n76_), .c(new_n75_), .d(x19), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n79_), .c(x25), .O(new_n82_));
  nand2  g37(.a(x24), .b(x22), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n75_), .c(new_n48_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x20), .O(new_n85_));
  nand2  g40(.a(new_n52_), .b(new_n51_), .O(new_n86_));
  oai21  g41(.a(new_n83_), .b(new_n86_), .c(new_n48_), .O(new_n87_));
  nand2  g42(.a(new_n48_), .b(new_n80_), .O(new_n88_));
  aoi22  g43(.a(new_n88_), .b(x23), .c(new_n87_), .d(new_n50_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n85_), .c(new_n82_), .O(z08));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n59_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand2  g48(.a(new_n91_), .b(new_n50_), .O(new_n94_));
  oai21  g49(.a(x21), .b(x18), .c(new_n58_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n94_), .O(z10));
  nand2  g51(.a(x18), .b(new_n50_), .O(new_n97_));
  nand2  g52(.a(new_n51_), .b(x17), .O(new_n98_));
  nand2  g53(.a(new_n91_), .b(new_n58_), .O(new_n99_));
  aoi21  g54(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(z11));
  nor2   g55(.a(new_n51_), .b(new_n50_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x19), .O(new_n103_));
  nand2  g58(.a(new_n101_), .b(new_n52_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n99_), .O(z12));
  oai21  g60(.a(new_n102_), .b(new_n52_), .c(x20), .O(new_n106_));
  nand3  g61(.a(new_n101_), .b(new_n49_), .c(x19), .O(new_n107_));
  nand2  g62(.a(new_n91_), .b(new_n58_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(z13));
  nand2  g65(.a(x21), .b(new_n50_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n98_), .c(new_n58_), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nor2   g68(.a(new_n52_), .b(new_n50_), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  oai21  g71(.a(x20), .b(new_n52_), .c(x21), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(new_n113_), .d(new_n91_), .O(z14));
  nand4  g73(.a(new_n114_), .b(new_n76_), .c(new_n75_), .d(new_n49_), .O(new_n119_));
  nand2  g74(.a(new_n116_), .b(x22), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n109_), .O(z15));
  nand2  g76(.a(new_n119_), .b(x23), .O(new_n122_));
  inv1   g77(.a(new_n74_), .O(new_n123_));
  nor2   g78(.a(x23), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n75_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n122_), .c(new_n109_), .O(z16));
  nand2  g81(.a(new_n125_), .b(x24), .O(new_n127_));
  nand4  g82(.a(new_n115_), .b(new_n114_), .c(new_n77_), .d(new_n76_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n109_), .O(z17));
  oai21  g84(.a(new_n78_), .b(new_n74_), .c(x25), .O(new_n130_));
  inv1   g85(.a(new_n116_), .O(new_n131_));
  nand4  g86(.a(new_n124_), .b(new_n131_), .c(new_n48_), .d(new_n80_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n130_), .c(new_n109_), .O(z18));
  buf    g88(.a(x16), .O(z02));
endmodule


