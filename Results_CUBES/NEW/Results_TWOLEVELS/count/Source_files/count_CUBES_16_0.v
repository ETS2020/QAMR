// Benchmark "FAU" written by ABC on Mon Jul  6 14:17:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n116_, new_n118_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n128_;
  nor2   g00(.a(x19), .b(x17), .O(new_n52_));
  and2   g01(.a(x19), .b(x17), .O(new_n53_));
  oai21  g02(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g03(.a(x15), .O(new_n55_));
  inv1   g04(.a(x16), .O(new_n56_));
  aoi21  g05(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g06(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g07(.a(x20), .O(new_n59_));
  nor2   g08(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nand2  g09(.a(new_n52_), .b(new_n59_), .O(new_n61_));
  inv1   g10(.a(new_n61_), .O(new_n62_));
  oai21  g11(.a(new_n62_), .b(new_n60_), .c(x16), .O(new_n63_));
  inv1   g12(.a(x14), .O(new_n64_));
  aoi21  g13(.a(new_n56_), .b(new_n64_), .c(x18), .O(new_n65_));
  nand2  g14(.a(new_n65_), .b(new_n63_), .O(z01));
  nor2   g15(.a(x21), .b(x20), .O(new_n67_));
  nand2  g16(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  inv1   g17(.a(new_n68_), .O(new_n69_));
  aoi21  g18(.a(new_n61_), .b(x21), .c(new_n69_), .O(new_n70_));
  inv1   g19(.a(x13), .O(new_n71_));
  aoi21  g20(.a(new_n56_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g21(.a(new_n70_), .b(new_n56_), .c(new_n72_), .O(z02));
  nor3   g22(.a(x22), .b(x21), .c(x20), .O(new_n74_));
  aoi22  g23(.a(new_n74_), .b(new_n52_), .c(new_n68_), .d(x22), .O(new_n75_));
  inv1   g24(.a(x12), .O(new_n76_));
  aoi21  g25(.a(new_n56_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g26(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g27(.a(x21), .O(new_n79_));
  inv1   g28(.a(x22), .O(new_n80_));
  inv1   g29(.a(x23), .O(new_n81_));
  nand3  g30(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  nor2   g31(.a(new_n82_), .b(new_n61_), .O(new_n83_));
  aoi21  g32(.a(new_n74_), .b(new_n52_), .c(new_n81_), .O(new_n84_));
  oai21  g33(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g34(.a(x11), .O(new_n86_));
  aoi21  g35(.a(new_n56_), .b(new_n86_), .c(x18), .O(new_n87_));
  nand2  g36(.a(new_n87_), .b(new_n85_), .O(z04));
  nor2   g37(.a(x24), .b(x23), .O(new_n89_));
  nand4  g38(.a(new_n89_), .b(new_n67_), .c(new_n52_), .d(new_n80_), .O(new_n90_));
  oai21  g39(.a(new_n82_), .b(new_n61_), .c(x24), .O(new_n91_));
  nand2  g40(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g41(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g42(.a(x10), .O(new_n94_));
  aoi21  g43(.a(new_n56_), .b(new_n94_), .c(x18), .O(new_n95_));
  nand2  g44(.a(new_n95_), .b(new_n93_), .O(z05));
  nor2   g45(.a(x22), .b(x21), .O(new_n97_));
  nand4  g46(.a(new_n89_), .b(new_n97_), .c(new_n52_), .d(new_n59_), .O(new_n98_));
  nor2   g47(.a(x25), .b(x24), .O(new_n99_));
  nand3  g48(.a(new_n99_), .b(new_n81_), .c(new_n80_), .O(new_n100_));
  inv1   g49(.a(new_n100_), .O(new_n101_));
  aoi22  g50(.a(new_n101_), .b(new_n69_), .c(new_n98_), .d(x25), .O(new_n102_));
  inv1   g51(.a(x09), .O(new_n103_));
  aoi21  g52(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  oai21  g53(.a(new_n102_), .b(new_n56_), .c(new_n104_), .O(z06));
  nor3   g54(.a(x25), .b(x24), .c(x23), .O(new_n106_));
  nand4  g55(.a(new_n106_), .b(new_n80_), .c(new_n79_), .d(new_n59_), .O(new_n107_));
  inv1   g56(.a(x26), .O(new_n108_));
  nor2   g57(.a(new_n52_), .b(new_n108_), .O(new_n109_));
  aoi21  g58(.a(new_n107_), .b(new_n52_), .c(new_n109_), .O(new_n110_));
  inv1   g59(.a(x08), .O(new_n111_));
  aoi21  g60(.a(new_n56_), .b(new_n111_), .c(x18), .O(new_n112_));
  oai21  g61(.a(new_n110_), .b(new_n56_), .c(new_n112_), .O(z07));
  aoi21  g62(.a(x27), .b(x16), .c(x18), .O(new_n114_));
  oai21  g63(.a(x16), .b(x07), .c(new_n114_), .O(z08));
  aoi21  g64(.a(x28), .b(x16), .c(x18), .O(new_n116_));
  oai21  g65(.a(x16), .b(x06), .c(new_n116_), .O(z09));
  aoi21  g66(.a(x29), .b(x16), .c(x18), .O(new_n118_));
  oai21  g67(.a(x16), .b(x05), .c(new_n118_), .O(z10));
  aoi21  g68(.a(x30), .b(x16), .c(x18), .O(new_n120_));
  oai21  g69(.a(x16), .b(x04), .c(new_n120_), .O(z11));
  aoi21  g70(.a(x31), .b(x16), .c(x18), .O(new_n122_));
  oai21  g71(.a(x16), .b(x03), .c(new_n122_), .O(z12));
  aoi21  g72(.a(x32), .b(x16), .c(x18), .O(new_n124_));
  oai21  g73(.a(x16), .b(x02), .c(new_n124_), .O(z13));
  aoi21  g74(.a(x33), .b(x16), .c(x18), .O(new_n126_));
  oai21  g75(.a(x16), .b(x01), .c(new_n126_), .O(z14));
  aoi21  g76(.a(x34), .b(x16), .c(x18), .O(new_n128_));
  oai21  g77(.a(x16), .b(x00), .c(new_n128_), .O(z15));
endmodule


