// Benchmark "FAU" written by ABC on Mon Jul  6 14:22:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_;
  and2   g00(.a(x25), .b(x24), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x26), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand4  g03(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  inv1   g07(.a(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x10), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  inv1   g12(.a(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nor2   g15(.a(new_n57_), .b(new_n59_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n57_), .b(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n52_), .O(new_n76_));
  inv1   g32(.a(x10), .O(new_n77_));
  nand2  g33(.a(new_n51_), .b(new_n77_), .O(new_n78_));
  oai22  g34(.a(new_n78_), .b(new_n76_), .c(new_n57_), .d(new_n56_), .O(z09));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n73_), .c(x19), .O(new_n81_));
  nor2   g37(.a(x20), .b(new_n52_), .O(new_n82_));
  aoi21  g38(.a(new_n81_), .b(x20), .c(new_n82_), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n78_), .c(new_n57_), .d(new_n59_), .O(z10));
  nand3  g40(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  nand3  g41(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n85_), .c(x19), .O(new_n87_));
  nand2  g43(.a(x20), .b(x19), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(x21), .O(new_n89_));
  aoi21  g45(.a(new_n87_), .b(x21), .c(new_n89_), .O(new_n90_));
  inv1   g46(.a(x20), .O(new_n91_));
  nand2  g47(.a(x21), .b(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n90_), .b(x08), .c(new_n92_), .O(new_n93_));
  nor2   g49(.a(x10), .b(new_n50_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n61_), .O(z11));
  inv1   g52(.a(new_n88_), .O(new_n97_));
  inv1   g53(.a(new_n85_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(x23), .c(x14), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n97_), .c(x21), .O(new_n100_));
  inv1   g56(.a(x22), .O(new_n101_));
  nand3  g57(.a(new_n97_), .b(new_n101_), .c(x21), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  aoi21  g59(.a(new_n100_), .b(x22), .c(new_n103_), .O(new_n104_));
  oai22  g60(.a(new_n104_), .b(new_n78_), .c(new_n57_), .d(new_n63_), .O(z12));
  inv1   g61(.a(new_n78_), .O(new_n106_));
  inv1   g62(.a(x23), .O(new_n107_));
  inv1   g63(.a(x25), .O(new_n108_));
  inv1   g64(.a(x26), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(x24), .c(x15), .O(new_n111_));
  nand4  g67(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(new_n107_), .O(new_n114_));
  nand3  g70(.a(new_n107_), .b(x22), .c(x21), .O(new_n115_));
  nor2   g71(.a(new_n115_), .b(new_n88_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n114_), .c(new_n106_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n65_), .O(z13));
  nand2  g74(.a(x23), .b(x22), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n110_), .b(x16), .O(new_n121_));
  nand3  g77(.a(x21), .b(x20), .c(x19), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  nor3   g80(.a(new_n122_), .b(new_n119_), .c(x24), .O(new_n125_));
  aoi21  g81(.a(new_n124_), .b(x24), .c(new_n125_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n78_), .c(new_n67_), .O(z14));
  nand2  g83(.a(x26), .b(x17), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n123_), .c(new_n120_), .d(x24), .O(new_n129_));
  nand3  g85(.a(new_n120_), .b(new_n108_), .c(x24), .O(new_n130_));
  inv1   g86(.a(new_n130_), .O(new_n131_));
  aoi22  g87(.a(new_n131_), .b(new_n123_), .c(new_n129_), .d(x25), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n78_), .c(new_n69_), .O(z15));
  nor2   g89(.a(new_n52_), .b(x18), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n49_), .c(new_n45_), .O(new_n135_));
  nand4  g91(.a(new_n109_), .b(x25), .c(x22), .d(x21), .O(new_n136_));
  nand3  g92(.a(new_n97_), .b(x24), .c(x23), .O(new_n137_));
  nor2   g93(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  aoi21  g94(.a(new_n135_), .b(x26), .c(new_n138_), .O(new_n139_));
  oai21  g95(.a(new_n139_), .b(new_n78_), .c(new_n71_), .O(z16));
endmodule


