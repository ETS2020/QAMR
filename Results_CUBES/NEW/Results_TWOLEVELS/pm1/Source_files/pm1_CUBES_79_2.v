// Benchmark "FAU" written by ABC on Mon Jul  6 14:06:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x08), .O(new_n31_));
  oai21  g02(.a(x10), .b(new_n31_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n32_), .c(new_n30_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x12), .O(z01));
  nand3  g07(.a(x12), .b(x09), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(x11), .b(new_n31_), .O(new_n39_));
  and2   g10(.a(x06), .b(x05), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n34_), .O(z02));
  nand3  g12(.a(new_n40_), .b(x08), .c(x07), .O(new_n42_));
  and2   g13(.a(x12), .b(x09), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n42_), .c(new_n34_), .O(z03));
  inv1   g15(.a(x14), .O(z04));
  inv1   g16(.a(x13), .O(z05));
  and2   g17(.a(x03), .b(x02), .O(new_n47_));
  inv1   g18(.a(x04), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n33_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z06));
  inv1   g22(.a(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  and2   g24(.a(x04), .b(x00), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n47_), .c(new_n43_), .d(new_n53_), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  xor2a  g27(.a(x12), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n53_), .b(x00), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n57_), .O(z09));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x12), .O(new_n61_));
  nand2  g32(.a(x11), .b(x09), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(z10));
  nand4  g34(.a(new_n60_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  inv1   g35(.a(x12), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand3  g37(.a(x11), .b(new_n53_), .c(x00), .O(new_n67_));
  aoi21  g38(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand3  g40(.a(x12), .b(x11), .c(new_n69_), .O(new_n70_));
  nor2   g41(.a(new_n70_), .b(new_n58_), .O(z12));
endmodule


