// Benchmark "FAU" written by ABC on Mon Jul  6 13:21:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nand2  g002(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x20), .O(new_n51_));
  nand2  g005(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g006(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g007(.a(x21), .O(new_n54_));
  nand2  g008(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  oai21  g009(.a(new_n47_), .b(x10), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  inv1   g013(.a(x23), .O(new_n60_));
  nand2  g014(.a(new_n47_), .b(new_n60_), .O(new_n61_));
  oai21  g015(.a(new_n47_), .b(x12), .c(new_n61_), .O(z04));
  inv1   g016(.a(x24), .O(new_n63_));
  nand2  g017(.a(new_n47_), .b(new_n63_), .O(new_n64_));
  oai21  g018(.a(new_n47_), .b(x13), .c(new_n64_), .O(z05));
  inv1   g019(.a(x25), .O(new_n66_));
  nand2  g020(.a(new_n47_), .b(new_n66_), .O(new_n67_));
  oai21  g021(.a(new_n47_), .b(x14), .c(new_n67_), .O(z06));
  inv1   g022(.a(x15), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x26), .c(new_n70_), .O(z07));
  inv1   g025(.a(x08), .O(new_n72_));
  nor2   g026(.a(x18), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(x18), .b(x00), .c(new_n73_), .O(new_n74_));
  inv1   g028(.a(x17), .O(new_n75_));
  nor2   g029(.a(new_n48_), .b(new_n75_), .O(new_n76_));
  nor2   g030(.a(x19), .b(x17), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(x16), .c(new_n76_), .O(new_n78_));
  oai21  g032(.a(new_n74_), .b(x16), .c(new_n78_), .O(z09));
  nor2   g033(.a(new_n51_), .b(new_n75_), .O(new_n80_));
  nor3   g034(.a(x20), .b(x19), .c(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g036(.a(x16), .O(new_n83_));
  inv1   g037(.a(x09), .O(new_n84_));
  nand2  g038(.a(x18), .b(x01), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand2  g041(.a(x20), .b(x19), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n87_), .c(new_n82_), .O(z10));
  nor2   g043(.a(x20), .b(x17), .O(new_n90_));
  nor2   g044(.a(x21), .b(x20), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  oai21  g046(.a(new_n90_), .b(new_n54_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g048(.a(x10), .O(new_n95_));
  nand2  g049(.a(x18), .b(x02), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  aoi22  g051(.a(new_n97_), .b(new_n83_), .c(x21), .d(x19), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n94_), .O(z11));
  nand2  g053(.a(new_n91_), .b(new_n75_), .O(new_n100_));
  nor2   g054(.a(x22), .b(x21), .O(new_n101_));
  aoi22  g055(.a(new_n101_), .b(new_n81_), .c(new_n100_), .d(x22), .O(new_n102_));
  nand2  g056(.a(x18), .b(x03), .O(new_n103_));
  oai21  g057(.a(x18), .b(new_n57_), .c(new_n103_), .O(new_n104_));
  aoi22  g058(.a(new_n104_), .b(new_n83_), .c(x22), .d(x19), .O(new_n105_));
  oai21  g059(.a(new_n102_), .b(new_n83_), .c(new_n105_), .O(z12));
  nand2  g060(.a(new_n101_), .b(new_n90_), .O(new_n107_));
  nor3   g061(.a(x23), .b(x22), .c(x21), .O(new_n108_));
  aoi22  g062(.a(new_n108_), .b(new_n81_), .c(new_n107_), .d(x23), .O(new_n109_));
  inv1   g063(.a(x12), .O(new_n110_));
  nand2  g064(.a(x18), .b(x04), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  aoi22  g066(.a(new_n112_), .b(new_n83_), .c(x23), .d(x19), .O(new_n113_));
  oai21  g067(.a(new_n109_), .b(new_n83_), .c(new_n113_), .O(z13));
  aoi21  g068(.a(new_n108_), .b(new_n90_), .c(new_n63_), .O(new_n115_));
  nand2  g069(.a(new_n77_), .b(new_n51_), .O(new_n116_));
  nand3  g070(.a(new_n101_), .b(new_n63_), .c(new_n60_), .O(new_n117_));
  nor2   g071(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  inv1   g073(.a(x13), .O(new_n120_));
  nand2  g074(.a(x18), .b(x05), .O(new_n121_));
  oai21  g075(.a(x18), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  aoi22  g076(.a(new_n122_), .b(new_n83_), .c(x24), .d(x19), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n119_), .O(z14));
  nor3   g078(.a(x21), .b(x20), .c(x17), .O(new_n125_));
  nor3   g079(.a(x24), .b(x23), .c(x22), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(new_n125_), .c(new_n66_), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nor2   g082(.a(x25), .b(x24), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g084(.a(new_n130_), .b(new_n92_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n127_), .c(x16), .O(new_n132_));
  inv1   g086(.a(x14), .O(new_n133_));
  nand2  g087(.a(x18), .b(x06), .O(new_n134_));
  oai21  g088(.a(x18), .b(new_n133_), .c(new_n134_), .O(new_n135_));
  aoi22  g089(.a(new_n135_), .b(new_n83_), .c(x25), .d(x19), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n132_), .O(z15));
  nand4  g091(.a(new_n129_), .b(new_n128_), .c(new_n91_), .d(new_n75_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x26), .O(new_n139_));
  nor3   g093(.a(x26), .b(x25), .c(x24), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n128_), .c(new_n91_), .d(new_n77_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x16), .O(new_n143_));
  nand2  g097(.a(x18), .b(x07), .O(new_n144_));
  oai21  g098(.a(x18), .b(new_n69_), .c(new_n144_), .O(new_n145_));
  aoi22  g099(.a(new_n145_), .b(new_n83_), .c(x26), .d(x19), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n143_), .O(z16));
  nand2  g101(.a(new_n140_), .b(new_n128_), .O(new_n148_));
  nand3  g102(.a(x27), .b(x17), .c(x16), .O(new_n149_));
  nand3  g103(.a(new_n91_), .b(x19), .c(new_n75_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n148_), .c(new_n149_), .O(z17));
  buf    g105(.a(x27), .O(z08));
endmodule


