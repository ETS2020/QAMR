// Benchmark "FAU" written by ABC on Mon Jul  6 14:59:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n90_, new_n91_, new_n93_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x27), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  aoi21  g003(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  nand3  g009(.a(x40), .b(x39), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n70_), .c(new_n66_), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(x15), .c(x16), .O(z00));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n69_), .c(new_n63_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n78_), .c(x04), .O(z01));
  nand2  g020(.a(new_n77_), .b(x04), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(x39), .c(new_n84_), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  inv1   g025(.a(new_n87_), .O(z08));
  nand3  g026(.a(z08), .b(new_n86_), .c(new_n83_), .O(z02));
  nand2  g027(.a(new_n65_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n64_), .b(new_n63_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n90_), .c(x21), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  nand3  g031(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(z04));
  aoi21  g032(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g033(.a(z06), .O(z05));
  inv1   g034(.a(x03), .O(new_n97_));
  inv1   g035(.a(x00), .O(new_n98_));
  oai21  g036(.a(x25), .b(new_n98_), .c(x38), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n98_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n103_), .c(new_n97_), .O(z07));
  nand4  g046(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z09));
  nand2  g048(.a(new_n77_), .b(new_n69_), .O(new_n111_));
  nand3  g049(.a(new_n87_), .b(x27), .c(x07), .O(new_n112_));
  nand3  g050(.a(x40), .b(x39), .c(x05), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  aoi21  g053(.a(new_n80_), .b(new_n79_), .c(new_n115_), .O(new_n116_));
  aoi21  g054(.a(new_n114_), .b(new_n111_), .c(new_n116_), .O(new_n117_));
  nand3  g055(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  oai21  g056(.a(new_n117_), .b(x04), .c(new_n118_), .O(z10));
  nand2  g057(.a(z08), .b(x29), .O(new_n120_));
  nor2   g058(.a(new_n63_), .b(new_n71_), .O(new_n121_));
  oai21  g059(.a(new_n121_), .b(new_n77_), .c(new_n120_), .O(new_n122_));
  inv1   g060(.a(x08), .O(new_n123_));
  nand2  g061(.a(x39), .b(x04), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(x35), .c(new_n76_), .d(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  inv1   g064(.a(x09), .O(new_n127_));
  nand2  g065(.a(new_n79_), .b(new_n127_), .O(new_n128_));
  aoi21  g066(.a(new_n126_), .b(new_n122_), .c(new_n128_), .O(z11));
  inv1   g067(.a(new_n70_), .O(new_n130_));
  nand2  g068(.a(z08), .b(new_n71_), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n65_), .c(new_n130_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(x37), .c(x27), .O(z12));
  nor2   g071(.a(x19), .b(x18), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n66_), .c(x20), .O(new_n135_));
  nand2  g073(.a(x36), .b(x35), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n137_));
  inv1   g075(.a(x40), .O(new_n138_));
  nor2   g076(.a(new_n138_), .b(x13), .O(new_n139_));
  nand4  g077(.a(new_n139_), .b(new_n137_), .c(x39), .d(new_n71_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n135_), .O(z13));
  inv1   g079(.a(new_n65_), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(z08), .c(new_n142_), .O(new_n144_));
  nand4  g082(.a(new_n68_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n144_), .c(new_n64_), .O(new_n147_));
  nand2  g085(.a(new_n134_), .b(x20), .O(new_n148_));
  oai21  g086(.a(x13), .b(x04), .c(x39), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n137_), .c(z08), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n148_), .c(new_n63_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(z14));
  nand4  g090(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(x23), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x23), .c(x22), .d(x01), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z17));
  nor2   g097(.a(new_n68_), .b(x28), .O(new_n160_));
  aoi22  g098(.a(z08), .b(x29), .c(new_n160_), .d(new_n63_), .O(new_n161_));
  nor2   g099(.a(new_n63_), .b(x04), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n128_), .O(new_n163_));
  oai21  g101(.a(new_n161_), .b(new_n123_), .c(new_n163_), .O(z18));
  nand2  g102(.a(new_n120_), .b(new_n68_), .O(new_n165_));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  aoi22  g104(.a(new_n166_), .b(new_n123_), .c(x28), .d(new_n63_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n165_), .c(new_n128_), .O(z20));
  aoi21  g106(.a(new_n126_), .b(new_n122_), .c(new_n128_), .O(z19));
endmodule


