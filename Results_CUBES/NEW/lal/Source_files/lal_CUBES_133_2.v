// Benchmark "FAU" written by ABC on Mon Jul  6 14:41:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x20), .O(new_n48_));
  inv1   g03(.a(x17), .O(new_n49_));
  inv1   g04(.a(x18), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nand3  g06(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand3  g07(.a(x24), .b(x22), .c(x21), .O(new_n53_));
  aoi21  g08(.a(new_n52_), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(x07), .O(new_n57_));
  oai21  g12(.a(new_n54_), .b(x25), .c(new_n57_), .O(z01));
  nor2   g13(.a(new_n54_), .b(x25), .O(z03));
  xor2a  g14(.a(x11), .b(x02), .O(new_n60_));
  xor2a  g15(.a(x12), .b(x03), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  xor2a  g17(.a(x09), .b(x00), .O(new_n63_));
  xor2a  g18(.a(x10), .b(x01), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n62_), .c(x08), .O(z04));
  nor2   g21(.a(x13), .b(x08), .O(z05));
  inv1   g22(.a(x14), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(x08), .O(z06));
  nand2  g24(.a(new_n46_), .b(x06), .O(z07));
  inv1   g25(.a(x25), .O(new_n71_));
  nand2  g26(.a(new_n51_), .b(new_n50_), .O(new_n72_));
  nand3  g27(.a(x24), .b(x22), .c(x21), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  inv1   g30(.a(x22), .O(new_n76_));
  inv1   g31(.a(x24), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g33(.a(x21), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x19), .c(x18), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n78_), .c(x25), .O(new_n81_));
  inv1   g36(.a(x23), .O(new_n82_));
  oai21  g37(.a(new_n71_), .b(x24), .c(new_n82_), .O(new_n83_));
  nor2   g38(.a(new_n50_), .b(new_n49_), .O(new_n84_));
  nor2   g39(.a(x20), .b(new_n51_), .O(new_n85_));
  nor2   g40(.a(x22), .b(x21), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  nand2  g42(.a(new_n73_), .b(new_n71_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x20), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n87_), .c(new_n81_), .d(new_n75_), .O(z08));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n56_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand3  g48(.a(new_n91_), .b(new_n55_), .c(new_n49_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xnor2a g50(.a(x18), .b(x17), .O(new_n96_));
  nand2  g51(.a(new_n91_), .b(new_n55_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n96_), .O(z11));
  oai21  g53(.a(new_n50_), .b(new_n49_), .c(x19), .O(new_n99_));
  nand2  g54(.a(new_n84_), .b(new_n51_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n85_), .b(new_n84_), .O(new_n104_));
  nand2  g59(.a(new_n91_), .b(new_n55_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(z13));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n84_), .c(x19), .O(new_n109_));
  nand2  g64(.a(new_n104_), .b(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(z14));
  nand2  g66(.a(new_n109_), .b(x22), .O(new_n112_));
  nand4  g67(.a(new_n108_), .b(new_n84_), .c(new_n76_), .d(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n106_), .O(z15));
  nand2  g69(.a(new_n108_), .b(new_n76_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n102_), .c(x23), .O(new_n116_));
  inv1   g71(.a(new_n102_), .O(new_n117_));
  nand4  g72(.a(new_n108_), .b(new_n117_), .c(new_n82_), .d(new_n76_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n106_), .O(z16));
  nand2  g74(.a(new_n118_), .b(x24), .O(new_n120_));
  nand2  g75(.a(x25), .b(x23), .O(new_n121_));
  nor3   g76(.a(x24), .b(x22), .c(x21), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n121_), .c(new_n85_), .d(new_n84_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n120_), .c(new_n106_), .O(z17));
  nand3  g79(.a(new_n108_), .b(new_n77_), .c(new_n76_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n102_), .c(x25), .O(new_n126_));
  oai21  g81(.a(x25), .b(x24), .c(new_n82_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n126_), .c(new_n106_), .O(z18));
  buf    g84(.a(x16), .O(z02));
endmodule


