// Benchmark "FAU" written by ABC on Mon Jul  6 20:08:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x32), .O(z0));
  nor2   g001(.a(x32), .b(x03), .O(new_n46_));
  inv1   g002(.a(x03), .O(new_n47_));
  nor2   g003(.a(x33), .b(new_n47_), .O(new_n48_));
  oai21  g004(.a(new_n48_), .b(new_n46_), .c(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  nor2   g006(.a(x32), .b(new_n47_), .O(new_n51_));
  nor2   g007(.a(x33), .b(x03), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n49_), .O(z1));
  xnor2a g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x01), .O(new_n56_));
  nand2  g012(.a(x03), .b(x02), .O(new_n57_));
  nand2  g013(.a(new_n47_), .b(new_n50_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n47_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n47_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n61_), .c(x01), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n59_), .c(new_n55_), .O(new_n65_));
  xor2a  g021(.a(x07), .b(x04), .O(new_n66_));
  nor2   g022(.a(new_n47_), .b(x02), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(new_n68_));
  nand2  g024(.a(new_n47_), .b(x02), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n68_), .c(new_n56_), .O(new_n70_));
  nand2  g026(.a(new_n62_), .b(new_n60_), .O(new_n71_));
  nand4  g027(.a(x06), .b(x05), .c(new_n47_), .d(new_n50_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n66_), .O(new_n74_));
  inv1   g030(.a(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  aoi21  g032(.a(new_n74_), .b(new_n65_), .c(new_n76_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  nor2   g035(.a(x23), .b(x18), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nor2   g037(.a(x24), .b(x19), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nor2   g039(.a(x20), .b(x15), .O(new_n84_));
  nor2   g040(.a(x21), .b(x16), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x17), .O(new_n87_));
  inv1   g043(.a(x22), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n86_), .c(new_n83_), .d(new_n81_), .O(new_n90_));
  and2   g046(.a(new_n90_), .b(new_n57_), .O(new_n91_));
  nand2  g047(.a(x25), .b(x01), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n91_), .c(new_n79_), .d(new_n78_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z3));
  xor2a  g051(.a(x28), .b(x27), .O(new_n96_));
  nor2   g052(.a(new_n92_), .b(x26), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n96_), .c(new_n91_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z4));
  inv1   g055(.a(new_n97_), .O(new_n100_));
  inv1   g056(.a(x29), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n79_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  oai21  g060(.a(new_n101_), .b(x27), .c(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n91_), .O(new_n106_));
  nand2  g062(.a(new_n83_), .b(new_n81_), .O(new_n107_));
  oai21  g063(.a(x20), .b(x15), .c(new_n89_), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n107_), .c(new_n57_), .O(new_n109_));
  inv1   g065(.a(x16), .O(new_n110_));
  nor2   g066(.a(x21), .b(x03), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n67_), .c(new_n110_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x29), .c(new_n102_), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n106_), .c(new_n100_), .O(z5));
  aoi21  g071(.a(new_n112_), .b(new_n109_), .c(new_n103_), .O(new_n116_));
  nand2  g072(.a(new_n91_), .b(new_n101_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x30), .O(new_n119_));
  inv1   g075(.a(x30), .O(new_n120_));
  nand4  g076(.a(new_n103_), .b(new_n91_), .c(new_n120_), .d(x29), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n119_), .c(new_n100_), .O(z6));
  nand4  g078(.a(new_n89_), .b(new_n86_), .c(new_n83_), .d(new_n81_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nand4  g080(.a(new_n103_), .b(new_n124_), .c(x30), .d(x29), .O(new_n125_));
  nor2   g081(.a(new_n120_), .b(new_n101_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n103_), .O(new_n127_));
  nand2  g083(.a(new_n97_), .b(new_n57_), .O(new_n128_));
  aoi21  g084(.a(new_n127_), .b(x31), .c(new_n128_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n125_), .c(new_n123_), .O(z7));
  inv1   g086(.a(x15), .O(new_n131_));
  inv1   g087(.a(x18), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n132_), .c(x13), .O(new_n133_));
  inv1   g089(.a(x19), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x18), .c(x14), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n87_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n87_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  nand3  g095(.a(x19), .b(x18), .c(x17), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n110_), .c(x11), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n139_), .c(new_n131_), .O(new_n143_));
  nand3  g099(.a(x16), .b(new_n131_), .c(x10), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  nand2  g101(.a(new_n102_), .b(new_n79_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n120_), .c(x29), .O(new_n147_));
  nand2  g103(.a(new_n146_), .b(x29), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x30), .O(new_n149_));
  nand2  g105(.a(z0), .b(x31), .O(new_n150_));
  aoi21  g106(.a(new_n149_), .b(new_n147_), .c(new_n150_), .O(new_n151_));
  oai21  g107(.a(new_n145_), .b(new_n143_), .c(new_n151_), .O(new_n152_));
  nor2   g108(.a(new_n110_), .b(new_n131_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n141_), .O(new_n154_));
  nor2   g110(.a(new_n134_), .b(new_n132_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(x17), .c(x11), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n110_), .O(new_n157_));
  nand2  g113(.a(new_n155_), .b(x12), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n87_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n157_), .c(new_n154_), .O(new_n160_));
  nand2  g116(.a(x16), .b(x10), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n140_), .c(new_n131_), .O(new_n162_));
  nand2  g118(.a(new_n126_), .b(x27), .O(new_n163_));
  oai21  g119(.a(x30), .b(x27), .c(new_n163_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n102_), .O(new_n165_));
  oai21  g121(.a(x19), .b(x14), .c(x31), .O(new_n166_));
  aoi21  g122(.a(new_n120_), .b(new_n101_), .c(new_n166_), .O(new_n167_));
  aoi21  g123(.a(x19), .b(x13), .c(x18), .O(new_n168_));
  aoi21  g124(.a(new_n126_), .b(x28), .c(new_n168_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n167_), .c(new_n165_), .d(new_n162_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n160_), .c(x32), .O(new_n171_));
  nand2  g127(.a(new_n78_), .b(x00), .O(new_n172_));
  aoi21  g128(.a(new_n171_), .b(new_n152_), .c(new_n172_), .O(z8));
  inv1   g129(.a(x33), .O(new_n174_));
  oai21  g130(.a(new_n145_), .b(new_n143_), .c(new_n174_), .O(new_n175_));
  nand2  g131(.a(new_n124_), .b(new_n120_), .O(new_n176_));
  oai22  g132(.a(new_n176_), .b(new_n175_), .c(new_n174_), .d(new_n124_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n148_), .O(new_n178_));
  inv1   g134(.a(new_n148_), .O(new_n179_));
  nand2  g135(.a(x31), .b(x30), .O(new_n180_));
  nand2  g136(.a(x33), .b(new_n124_), .O(new_n181_));
  oai21  g137(.a(new_n180_), .b(new_n175_), .c(new_n181_), .O(new_n182_));
  aoi21  g138(.a(x18), .b(x12), .c(x17), .O(new_n183_));
  nand2  g139(.a(new_n124_), .b(x30), .O(new_n184_));
  nand2  g140(.a(x31), .b(new_n120_), .O(new_n185_));
  inv1   g141(.a(x13), .O(new_n186_));
  nand2  g142(.a(new_n132_), .b(new_n186_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n185_), .c(new_n184_), .O(new_n188_));
  nand2  g144(.a(new_n134_), .b(x14), .O(new_n189_));
  nor2   g145(.a(new_n132_), .b(new_n87_), .O(new_n190_));
  nand3  g146(.a(new_n190_), .b(new_n189_), .c(new_n153_), .O(new_n191_));
  nand2  g147(.a(new_n190_), .b(new_n153_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n134_), .O(new_n193_));
  inv1   g149(.a(new_n190_), .O(new_n194_));
  oai21  g150(.a(new_n194_), .b(new_n161_), .c(new_n131_), .O(new_n195_));
  nand2  g151(.a(new_n190_), .b(x11), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n110_), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(new_n195_), .c(new_n193_), .d(new_n191_), .O(new_n198_));
  nor3   g154(.a(new_n198_), .b(new_n188_), .c(new_n183_), .O(new_n199_));
  nor2   g155(.a(new_n199_), .b(new_n174_), .O(new_n200_));
  aoi21  g156(.a(new_n182_), .b(new_n179_), .c(new_n200_), .O(new_n201_));
  aoi21  g157(.a(new_n201_), .b(new_n178_), .c(new_n172_), .O(z9));
endmodule


