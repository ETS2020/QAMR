// Benchmark "FAU" written by ABC on Mon Jul  6 13:56:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand2  g00(.a(x15), .b(x14), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x13), .c(x12), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x11), .c(new_n32_), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n31_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x18), .c(x17), .d(x16), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  nor2   g11(.a(x11), .b(x10), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n34_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n39_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(new_n34_), .b(new_n32_), .O(new_n44_));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  oai21  g17(.a(new_n44_), .b(new_n43_), .c(new_n45_), .O(z2));
  and2   g18(.a(x12), .b(x11), .O(new_n47_));
  xnor2a g19(.a(new_n47_), .b(x13), .O(new_n48_));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(new_n44_), .c(new_n49_), .O(z3));
  inv1   g22(.a(new_n44_), .O(new_n51_));
  inv1   g23(.a(x14), .O(new_n52_));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n47_), .b(new_n52_), .c(x13), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n51_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n57_), .O(z4));
  nand4  g31(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  nand2  g32(.a(x14), .b(x13), .O(new_n61_));
  nor2   g33(.a(new_n61_), .b(x15), .O(new_n62_));
  aoi22  g34(.a(new_n62_), .b(new_n47_), .c(new_n60_), .d(x15), .O(new_n63_));
  nand2  g35(.a(x08), .b(x04), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n44_), .c(new_n64_), .O(z5));
  inv1   g37(.a(x16), .O(new_n66_));
  aoi21  g38(.a(new_n54_), .b(new_n30_), .c(new_n66_), .O(new_n67_));
  nor3   g39(.a(new_n53_), .b(new_n29_), .c(x16), .O(new_n68_));
  nor2   g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n44_), .c(new_n70_), .O(z6));
  nand3  g43(.a(x16), .b(x15), .c(x14), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n53_), .c(x17), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nand3  g46(.a(new_n30_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n53_), .c(new_n73_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n51_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x06), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n77_), .O(z7));
  nand4  g51(.a(x17), .b(x16), .c(x15), .d(x11), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n61_), .c(x18), .O(new_n81_));
  inv1   g53(.a(new_n60_), .O(new_n82_));
  and2   g54(.a(x16), .b(x15), .O(new_n83_));
  nor2   g55(.a(x18), .b(new_n74_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n81_), .c(x08), .O(new_n86_));
  inv1   g58(.a(x18), .O(new_n87_));
  nor2   g59(.a(new_n87_), .b(x12), .O(new_n88_));
  nor2   g60(.a(x10), .b(new_n33_), .O(new_n89_));
  oai21  g61(.a(new_n88_), .b(new_n86_), .c(new_n89_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z8));
endmodule


