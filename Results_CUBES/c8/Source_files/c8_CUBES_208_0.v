// Benchmark "FAU" written by ABC on Mon Jul  6 13:22:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nand2  g002(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x20), .O(new_n51_));
  nand2  g005(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g006(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g007(.a(x21), .O(new_n54_));
  nand2  g008(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  oai21  g009(.a(new_n47_), .b(x10), .c(new_n55_), .O(z02));
  inv1   g010(.a(x22), .O(new_n57_));
  nand2  g011(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  oai21  g012(.a(new_n47_), .b(x11), .c(new_n58_), .O(z03));
  inv1   g013(.a(x23), .O(new_n60_));
  nand2  g014(.a(new_n47_), .b(new_n60_), .O(new_n61_));
  oai21  g015(.a(new_n47_), .b(x12), .c(new_n61_), .O(z04));
  inv1   g016(.a(x24), .O(new_n63_));
  nand2  g017(.a(new_n47_), .b(new_n63_), .O(new_n64_));
  oai21  g018(.a(new_n47_), .b(x13), .c(new_n64_), .O(z05));
  inv1   g019(.a(x25), .O(new_n66_));
  nand2  g020(.a(new_n47_), .b(new_n66_), .O(new_n67_));
  oai21  g021(.a(new_n47_), .b(x14), .c(new_n67_), .O(z06));
  inv1   g022(.a(x26), .O(new_n69_));
  nand2  g023(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n47_), .b(x15), .c(new_n70_), .O(z07));
  inv1   g025(.a(x16), .O(new_n72_));
  inv1   g026(.a(x08), .O(new_n73_));
  nand2  g027(.a(x18), .b(x00), .O(new_n74_));
  oai21  g028(.a(x18), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand3  g030(.a(x19), .b(x17), .c(x16), .O(new_n77_));
  inv1   g031(.a(x17), .O(new_n78_));
  nand2  g032(.a(new_n48_), .b(new_n78_), .O(new_n79_));
  nand3  g033(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z09));
  nor2   g034(.a(new_n51_), .b(new_n78_), .O(new_n81_));
  nor2   g035(.a(x20), .b(x17), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n81_), .c(new_n48_), .O(new_n83_));
  inv1   g037(.a(x09), .O(new_n84_));
  nand2  g038(.a(x18), .b(x01), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nand3  g041(.a(x20), .b(x19), .c(x16), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z10));
  nor2   g043(.a(x20), .b(x19), .O(new_n90_));
  oai22  g044(.a(new_n90_), .b(new_n72_), .c(x19), .d(new_n78_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x21), .O(new_n92_));
  inv1   g046(.a(x10), .O(new_n93_));
  nand2  g047(.a(x18), .b(x02), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n72_), .O(new_n96_));
  nor2   g050(.a(x21), .b(x20), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n48_), .c(new_n78_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n96_), .c(new_n92_), .O(z11));
  nand2  g053(.a(x22), .b(x17), .O(new_n100_));
  nor2   g054(.a(x22), .b(x21), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n82_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  inv1   g058(.a(x11), .O(new_n105_));
  nand2  g059(.a(x18), .b(x03), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n105_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n72_), .O(new_n108_));
  nand2  g062(.a(new_n97_), .b(new_n48_), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(x22), .c(x16), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n108_), .c(new_n104_), .O(z12));
  nand2  g065(.a(x23), .b(x17), .O(new_n112_));
  nor2   g066(.a(x23), .b(x22), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n82_), .c(new_n54_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n48_), .O(new_n116_));
  inv1   g070(.a(x12), .O(new_n117_));
  nand2  g071(.a(x18), .b(x04), .O(new_n118_));
  oai21  g072(.a(x18), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(x23), .b(x16), .O(new_n120_));
  aoi21  g074(.a(new_n101_), .b(new_n90_), .c(new_n120_), .O(new_n121_));
  aoi21  g075(.a(new_n119_), .b(new_n72_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n116_), .O(z13));
  nand2  g077(.a(x24), .b(x17), .O(new_n124_));
  nand3  g078(.a(new_n54_), .b(new_n51_), .c(new_n78_), .O(new_n125_));
  nand3  g079(.a(new_n63_), .b(new_n60_), .c(new_n57_), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n48_), .O(new_n128_));
  inv1   g082(.a(x13), .O(new_n129_));
  nand2  g083(.a(x18), .b(x05), .O(new_n130_));
  oai21  g084(.a(x18), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  nand3  g085(.a(new_n113_), .b(new_n90_), .c(new_n54_), .O(new_n132_));
  nor2   g086(.a(new_n63_), .b(new_n72_), .O(new_n133_));
  aoi22  g087(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n72_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n128_), .O(z14));
  nand2  g089(.a(x25), .b(x17), .O(new_n136_));
  nor2   g090(.a(x25), .b(x24), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n113_), .c(new_n97_), .d(new_n78_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n48_), .O(new_n140_));
  inv1   g094(.a(x14), .O(new_n141_));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n141_), .c(new_n142_), .O(new_n143_));
  nor2   g097(.a(x24), .b(x23), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n97_), .c(new_n57_), .d(new_n48_), .O(new_n145_));
  nor2   g099(.a(new_n66_), .b(new_n72_), .O(new_n146_));
  aoi22  g100(.a(new_n146_), .b(new_n145_), .c(new_n143_), .d(new_n72_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n140_), .O(z15));
  nand2  g102(.a(x26), .b(x17), .O(new_n149_));
  nor2   g103(.a(x26), .b(x25), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n144_), .c(new_n101_), .d(new_n82_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n48_), .O(new_n153_));
  inv1   g107(.a(x15), .O(new_n154_));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n154_), .c(new_n155_), .O(new_n156_));
  nand4  g110(.a(new_n137_), .b(new_n113_), .c(new_n97_), .d(new_n48_), .O(new_n157_));
  nor2   g111(.a(new_n69_), .b(new_n72_), .O(new_n158_));
  aoi22  g112(.a(new_n158_), .b(new_n157_), .c(new_n156_), .d(new_n72_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n153_), .O(z16));
  nand3  g114(.a(new_n63_), .b(x19), .c(new_n78_), .O(new_n161_));
  inv1   g115(.a(new_n161_), .O(new_n162_));
  nand4  g116(.a(new_n60_), .b(new_n57_), .c(new_n54_), .d(new_n51_), .O(new_n163_));
  inv1   g117(.a(new_n163_), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n162_), .c(new_n150_), .O(new_n165_));
  nand2  g119(.a(x27), .b(x17), .O(new_n166_));
  aoi21  g120(.a(new_n166_), .b(new_n165_), .c(new_n72_), .O(z17));
  buf    g121(.a(x27), .O(z08));
endmodule


