// Benchmark "FAU" written by ABC on Tue Jun 23 01:12:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x01), .O(new_n55_));
  nor2   g01(.a(x18), .b(new_n55_), .O(new_n56_));
  inv1   g02(.a(x18), .O(new_n57_));
  nor2   g03(.a(x23), .b(new_n57_), .O(new_n58_));
  inv1   g04(.a(x19), .O(new_n59_));
  nor2   g05(.a(new_n59_), .b(x17), .O(new_n60_));
  oai21  g06(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  inv1   g07(.a(x22), .O(new_n62_));
  nand2  g08(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand2  g09(.a(new_n63_), .b(new_n61_), .O(z02));
  inv1   g10(.a(x07), .O(new_n66_));
  nor2   g11(.a(x18), .b(new_n66_), .O(new_n67_));
  nor2   g12(.a(x25), .b(new_n57_), .O(new_n68_));
  oai21  g13(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  inv1   g14(.a(x24), .O(new_n70_));
  nand2  g15(.a(new_n70_), .b(new_n59_), .O(new_n71_));
  nand2  g16(.a(new_n71_), .b(new_n69_), .O(z04));
  inv1   g17(.a(x04), .O(new_n75_));
  nor2   g18(.a(x18), .b(new_n75_), .O(new_n76_));
  nor2   g19(.a(x20), .b(new_n57_), .O(new_n77_));
  oai21  g20(.a(new_n77_), .b(new_n76_), .c(new_n60_), .O(new_n78_));
  inv1   g21(.a(x27), .O(new_n79_));
  nand2  g22(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand2  g23(.a(new_n80_), .b(new_n78_), .O(z07));
  inv1   g24(.a(x08), .O(new_n85_));
  nor2   g25(.a(x18), .b(new_n85_), .O(new_n86_));
  nor2   g26(.a(x24), .b(new_n57_), .O(new_n87_));
  oai21  g27(.a(new_n87_), .b(new_n86_), .c(new_n60_), .O(new_n88_));
  inv1   g28(.a(x31), .O(new_n89_));
  nand2  g29(.a(new_n89_), .b(new_n59_), .O(new_n90_));
  nand2  g30(.a(new_n90_), .b(new_n88_), .O(z11));
  inv1   g31(.a(x15), .O(new_n92_));
  nor2   g32(.a(x18), .b(new_n92_), .O(new_n93_));
  nor2   g33(.a(x33), .b(new_n57_), .O(new_n94_));
  oai21  g34(.a(new_n94_), .b(new_n93_), .c(new_n60_), .O(new_n95_));
  inv1   g35(.a(x32), .O(new_n96_));
  nand2  g36(.a(new_n96_), .b(new_n59_), .O(new_n97_));
  nand2  g37(.a(new_n97_), .b(new_n95_), .O(z12));
  inv1   g38(.a(x12), .O(new_n101_));
  nor2   g39(.a(x18), .b(new_n101_), .O(new_n102_));
  nor2   g40(.a(x28), .b(new_n57_), .O(new_n103_));
  oai21  g41(.a(new_n103_), .b(new_n102_), .c(new_n60_), .O(new_n104_));
  inv1   g42(.a(x35), .O(new_n105_));
  nand2  g43(.a(new_n105_), .b(new_n59_), .O(new_n106_));
  nand2  g44(.a(new_n106_), .b(new_n104_), .O(z15));
  zero   g45(.O(z00));
  zero   g46(.O(z01));
  zero   g47(.O(z03));
  zero   g48(.O(z05));
  zero   g49(.O(z06));
  zero   g50(.O(z08));
  zero   g51(.O(z09));
  zero   g52(.O(z10));
  zero   g53(.O(z13));
  zero   g54(.O(z14));
endmodule


