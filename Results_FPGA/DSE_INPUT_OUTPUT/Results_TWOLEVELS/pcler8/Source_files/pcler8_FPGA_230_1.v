// Benchmark "FAU" written by ABC on Mon Aug 24 16:59:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n59_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_;
  assign z00 = x19 & (~x16 | (new_n47_ & new_n50_ & ~x10 & x16 & x20));
  assign new_n47_ = new_n49_ & new_n48_ & x21;
  assign new_n48_ = x22 & x23;
  assign new_n49_ = x24 & x25 & x26;
  assign new_n50_ = ~x08 & x09;
  assign z01 = new_n52_ & x00;
  assign new_n52_ = x08 & (x16 | ~x19);
  assign z02 = new_n52_ & x01;
  assign z03 = (x02 & x08) | (~x16 & x19);
  assign z04 = (x03 & x08) | (~x16 & x19);
  assign z05 = new_n52_ & x04;
  assign z06 = new_n52_ & x05;
  assign z07 = new_n59_ | (~x16 & x19);
  assign new_n59_ = new_n52_ & x06;
  assign z08 = new_n52_ & x07;
  assign z09 = z01 | (~x08 & x09 & ~new_n62_ & ~x10);
  assign new_n62_ = x19 & (~new_n48_ | ~new_n49_ | ~new_n63_ | ~x11 | ~x16);
  assign new_n63_ = x20 & x21;
  assign z10 = z02 | (~x08 & new_n65_ & x09);
  assign new_n65_ = ~x10 & ((x16 & x19 & (~x20 | (new_n66_ & new_n67_))) | (~x19 & x20));
  assign new_n66_ = x12 & x21 & x22;
  assign new_n67_ = x23 & x24 & x25 & x26;
  assign z11 = new_n69_ | (x02 & x08) | (~x08 & x09 & new_n70_ & ~x10);
  assign new_n69_ = x19 & (~x16 | (new_n50_ & ~x10 & x20 & ~x21));
  assign new_n70_ = x21 & (~x19 | ~x20 | (new_n49_ & new_n48_ & x13));
  assign z12 = (x19 & (new_n74_ | ~x16)) | (x03 & x08) | (new_n72_ & ~x08);
  assign new_n72_ = x09 & ~x10 & x22 & (~new_n73_ | ~x19 | ~x20);
  assign new_n73_ = x21 & (~x14 | ~x23 | ~x24 | ~x25 | ~x26);
  assign new_n74_ = ~x08 & x09 & ~x10 & x20 & x21 & ~x22;
  assign z13 = (x04 & x08 & (x16 | ~x19)) | (x19 & (new_n76_ | ~x16)) | (new_n77_ & ~x08);
  assign new_n76_ = ~x08 & x09 & ~x10 & new_n63_ & x22 & ~x23;
  assign new_n77_ = x09 & ~x10 & x23 & (~new_n78_ | ~x19 | ~x20);
  assign new_n78_ = x21 & x22 & (~x25 | ~x26 | ~x15 | ~x24);
  assign z14 = z06 | (~x08 & x09 & ~new_n80_ & ~x10);
  assign new_n80_ = (~x16 | ((~x19 | ~new_n81_ | ~x20) & (new_n82_ | ~x24))) & (x19 | ~x24);
  assign new_n81_ = x21 & x22 & x23 & (~x24 | (x24 & x25 & x26));
  assign new_n82_ = x20 & x21 & x22 & x23;
  assign z15 = new_n59_ | (~x08 & x09 & ~x10 & (new_n84_ | new_n89_));
  assign new_n84_ = x16 & ((x25 & (~new_n85_ | ~new_n86_)) | (new_n87_ & new_n88_));
  assign new_n85_ = x20 & x21 & x22;
  assign new_n86_ = x23 & x24 & (~x17 | ~x26);
  assign new_n87_ = x19 & x20 & x21;
  assign new_n88_ = x22 & x23 & x24 & ~x25;
  assign new_n89_ = ~x19 & x25;
  assign z16 = z08 | (~new_n91_ & x19) | (~x08 & new_n93_ & x09);
  assign new_n91_ = x16 & (~new_n92_ | ~new_n48_ | ~x24 | ~x25 | x26);
  assign new_n92_ = ~x08 & x09 & ~x10 & x20 & x21;
  assign new_n93_ = ~x10 & x26 & (~new_n94_ | ~new_n48_ | ~x24 | ~x25);
  assign new_n94_ = ~x18 & x19 & x20 & x21;
endmodule


