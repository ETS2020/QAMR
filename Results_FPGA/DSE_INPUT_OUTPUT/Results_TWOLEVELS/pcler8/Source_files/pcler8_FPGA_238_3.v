// Benchmark "FAU" written by ABC on Mon Aug 24 16:59:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n47_, new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_;
  assign z01 = x00 & ~new_n47_ & x08;
  assign new_n47_ = x19 & x26;
  assign z02 = x01 & ~new_n47_ & x08;
  assign z03 = x02 & ~new_n47_ & x08;
  assign z04 = x03 & ~new_n47_ & x08;
  assign z05 = x04 & ~new_n47_ & x08;
  assign z06 = x05 & ~new_n47_ & x08;
  assign z07 = x06 & ~new_n47_ & x08;
  assign z08 = x07 & ~new_n47_ & x08;
  assign z09 = (x00 & x08 & (~x19 | ~x26)) | (x19 & x26) | (~x08 & x09 & ~x10 & ~x19);
  assign z10 = (x01 & ~new_n47_ & x08) | new_n47_ | (new_n57_ & ~x08);
  assign new_n57_ = x09 & ~x10 & (x19 ^ x20);
  assign z11 = (x02 & x08 & (~x19 | ~x26)) | (new_n60_ & ~x08) | (x19 & (new_n59_ | x26));
  assign new_n59_ = ~x08 & x09 & ~x10 & x20 & ~x21;
  assign new_n60_ = x09 & ~x10 & x21 & (~x19 | ~x20);
  assign z12 = z04 | (~x08 & new_n62_ & x09);
  assign new_n62_ = ~x10 & ((x22 & (~x19 | (~x26 & (~x20 | ~x21)))) | (x19 & x20 & x21 & ~x22 & ~x26));
  assign z13 = z05 | new_n64_ | (~x08 & new_n66_ & x09);
  assign new_n64_ = x19 & (x26 | (new_n65_ & ~x08 & x09 & ~x10));
  assign new_n65_ = x20 & x21 & x22 & ~x23;
  assign new_n66_ = ~x10 & x23 & (~x19 | ~x20 | ~x21 | ~x22);
  assign z14 = z06 | (~x08 & x09 & ~new_n68_ & ~x10);
  assign new_n68_ = (~x24 | (x19 & (x26 | (new_n69_ & x22 & x23)))) & (~new_n69_ | ~x19 | ~x22 | ~x23 | x24 | x26);
  assign new_n69_ = x20 & x21;
  assign z15 = z07 | new_n71_ | (x19 & (x26 | (new_n73_ & new_n74_)));
  assign new_n71_ = ~x08 & x09 & ~x10 & ~new_n72_ & x25;
  assign new_n72_ = x19 & x20 & x21 & x22 & x23 & x24;
  assign new_n73_ = ~x10 & x20 & ~x08 & x09;
  assign new_n74_ = x21 & x22 & x23 & x24 & ~x25;
  assign z16 = (x07 & ~new_n47_ & x08) | new_n47_ | (~x08 & x09 & ~new_n76_ & ~x10);
  assign new_n76_ = ~x26 & (~new_n77_ | ~new_n69_ | ~x19);
  assign new_n77_ = x22 & x23 & x24 & x25;
  assign z00 = 1'b0;
endmodule


