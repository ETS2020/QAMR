// Benchmark "FAU" written by ABC on Fri Aug 21 20:16:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_;
  assign z0 = (~x02 & (x04 | (~x00 & x01 & x03))) | (~x00 & x02 & (x01 ? (~x04 | x05) : (x03 & ~x04)));
  assign z1 = (~x00 & ((~x01 & (x04 | (x02 & x03 & ~x04))) | (~new_n28_ & (x04 | (x02 & x03))) | (x01 & ~x02 & x03))) | (~x02 & x04);
  assign new_n28_ = x05 & x06;
  assign z2 = new_n30_ | (~x02 & x04) | (~x07 & ~x08 & (new_n33_ | x02));
  assign new_n30_ = ~x00 & ((~new_n31_ & x02) | new_n32_ | (x04 & ~x05));
  assign new_n31_ = x01 ? (x04 & ~x05) : ((~x03 | x04) & (x07 | x10));
  assign new_n32_ = ~x01 & ((~x03 & (x07 | x10)) | x04 | (~x07 & x09 & ~x10));
  assign new_n33_ = ~x01 & (x10 ? ~x03 : x09);
  assign z3 = (((~x04 & (~x07 | ~x11)) | (x02 & ~x11)) & (x00 | ~x12)) | (x02 & ~x07 & (x00 | (~x00 & ~x12)));
  assign z4 = (~new_n36_ & (x02 | ~x04)) | (~x07 & ((~x04 & (~x00 | ~x09)) | (x02 & (~x00 | (x00 & ~x09)))));
  assign new_n36_ = x07 ? x11 : x08;
  assign z5 = (x07 & (x02 | ~x04)) | (~new_n38_ & (x02 | (~x04 & ~x07))) | (~x07 & ((~x04 & (x00 | x12)) | (x02 & (x00 | (~x00 & x12)))));
  assign new_n38_ = ~x10 & x13;
  assign z6 = (~x02 & x04) | (~x07 & ~x10 & (new_n40_ | new_n41_));
  assign new_n40_ = ~x08 & ((x00 & (x02 | (x03 & ~x09))) | (x01 & ~x02) | (~x01 & ~x03 & x09));
  assign new_n41_ = ~x00 & ((~x01 & (x02 | (~x03 & x09))) | (~x02 & (x01 | (x03 & ~x09))) | (x02 & ~x14));
  assign z7 = (~x04 & (x07 | (~x07 & x10))) | new_n43_ | (x02 & (x07 | x10)) | (~x02 & (new_n44_ | x04));
  assign new_n43_ = ~x08 & (~x03 | (~x07 & (x02 | (x01 & ~x02 & ~x10))));
  assign new_n44_ = ~x01 & x03 & x09;
  assign z8 = ~new_n46_ | (~x00 & (~x09 | (x02 & ~x07 & ~x12)));
  assign new_n46_ = (x04 | (~x07 & (x07 | ~x10))) & (~x02 | (~x07 & ~new_n47_ & ~x10)) & ~new_n48_ & (x02 | ~x04);
  assign new_n47_ = x00 & ~x07 & x09;
  assign new_n48_ = x09 & (x01 | ~x03 | x12);
endmodule


