...
module FAU ( 
  ...
  assign z2 = (~new_n31_ & ~x00) | (new_n34_ & ~x07) | (x10 & ~x11);
  assign new_n31_ = (~x02 | (~new_n32_ & (x01 | x07 | x10))) & (new_n33_ | x01);
  assign new_n32_ = (~x10 | x11) & ((x01 & (~x04 | x05)) | (x04 & ~x05) | (~x01 & (x04 | (x03 & ~x04))));
  assign new_n33_ = (x03 | (~x07 & ~x10)) & (x07 | ~x09 | x10);
  assign new_n34_ = ~x08 & (x02 | (~x01 & (x10 ? ~x03 : x09)));
  assign z3 = (x11 ? ~x07 : ~x10) & (x00 | ~x12);
  assign z4 = (~x11 & (x10 | (x07 & ~x10))) | (~x07 & (~x00 | ~x08 | ~x09));
  assign z5 = (x00 & (x11 ? ~x07 : ~x10)) | (x07 & (x11 | (~x10 & ~x11))) | (x10 & x11) | (~x07 & ~x10 & (x12 | ~x13));
  ...
  assign new_n44_ = (~x07 | (~x11 & (x10 | x11))) & ~x10 & (x08 | (~x01 & x03));
  assign z8 = (x07 & (x11 | (~x10 & ~x11))) | (~new_n46_ & ~x10) | (x10 & x11);
  assign new_n46_ = x09 ? (~x01 & ~x02 & x03 & ~x12) : x00;
endmodule
