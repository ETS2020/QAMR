// Benchmark "FAU" written by ABC on Mon Aug 24 18:24:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n49_, new_n50_, new_n54_, new_n55_, new_n60_, new_n65_, new_n66_,
    new_n68_, new_n70_, new_n72_, new_n74_, new_n76_;
  assign z00 = (x08 & ~x16) | (x04 & ~x15);
  assign z01 = ((~x04 | x15) & (x07 | (~new_n49_ & ~x25))) | (x04 & x05 & x15);
  assign new_n49_ = x24 & (new_n50_ | x23);
  assign new_n50_ = (x20 | (~x17 & ~x18 & ~x19)) & x21 & x22;
  assign z02 = x16 | (x04 & ~x15);
  assign z03 = ~x25 & ~new_n49_ & (~x04 | x15);
  assign z04 = new_n54_ | (x04 & ~x15);
  assign new_n54_ = ~x08 & (~new_n55_ | (x00 & ~x09) | (~x00 & x09) | (~x01 & x10) | (x01 & ~x10));
  assign new_n55_ = (~x02 | x11) & (x02 | ~x11) & (~x03 | x12) & (x03 | ~x12);
  assign z05 = (~x08 & ~x13) | (x04 & ~x15);
  assign z06 = (x04 & ~x15) | (~x08 & x14);
  assign z07 = (~x06 | x08) & (~x04 | x15);
  assign z08 = (x24 & (new_n60_ | x23)) | x25 | (x04 & ~x15);
  assign new_n60_ = x21 & x22 & (x20 | (~x17 & ~x18 & ~x19));
  assign z10 = ~x15 & (x04 | (~x07 & ~x17));
  assign z11 = ~x15 & (x04 | (~x07 & (x17 ^ x18)));
  assign z12 = ~x15 & (x04 | (~x07 & ((x19 & (~x17 | ~x18)) | (x17 & x18 & ~x19))));
  assign z13 = x15 | (~x04 & (new_n66_ | x07 | (~new_n65_ & x20)));
  assign new_n65_ = x17 & x18 & x19;
  assign new_n66_ = x17 & x18 & x19 & ~x20;
  assign z14 = (~new_n66_ & x21) | x15 | (~x04 & x07) | new_n68_ | (x04 & ~x15);
  assign new_n68_ = x17 & x18 & x19 & ~x20 & ~x21;
  assign z15 = (~new_n68_ & x22) | x15 | (~x04 & x07) | new_n70_ | (x04 & ~x15);
  assign new_n70_ = new_n65_ & ~x20 & ~x21 & ~x22;
  assign z16 = x15 | (~x04 & (new_n72_ | x07 | (~new_n70_ & x23)));
  assign new_n72_ = new_n65_ & ~x20 & ~x21 & ~x22 & ~x23;
  assign z17 = x15 | (~x04 & (new_n74_ | x07 | (~new_n72_ & x24)));
  assign new_n74_ = new_n66_ & ~x21 & ~x22 & ~x23 & ~x24;
  assign z18 = (~new_n74_ & x25) | x15 | (~x04 & x07) | new_n76_ | (x04 & ~x15);
  assign new_n76_ = new_n66_ & ~x21 & ~x22 & ~x23 & ~x24 & ~x25;
  assign z09 = 1'b0;
endmodule


