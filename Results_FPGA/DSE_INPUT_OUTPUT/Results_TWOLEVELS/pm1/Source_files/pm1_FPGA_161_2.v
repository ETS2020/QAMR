// Benchmark "FAU" written by ABC on Mon Aug 24 16:54:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n32_, new_n35_, new_n37_, new_n40_, new_n42_, new_n45_, new_n47_;
  assign z00 = (~new_n32_ & x11) | x01 | new_n32_ | (~x11 & x12);
  assign new_n32_ = x14 & ~x15;
  assign z01 = ~new_n32_ & (x11 | ~x12);
  assign z02 = (~new_n32_ & (~x09 | ~x12)) | ~new_n35_ | ~x08 | new_n32_ | (~x11 & x12);
  assign new_n35_ = x05 & x06 & x07;
  assign z03 = ~new_n32_ & (~new_n37_ | ~x11 | ~x12);
  assign new_n37_ = x09 & (~x07 | ~x08 | ~x05 | ~x06);
  assign z05 = ~new_n32_ & ~x13;
  assign z06 = new_n32_ | (new_n40_ & x01);
  assign new_n40_ = x09 & (~x02 | ~x03 | ~x04 | (~x11 & x12));
  assign z08 = (~new_n32_ & ~x11) | new_n32_ | new_n42_ | ~x00 | x10;
  assign new_n42_ = x02 & x03 & x04 & x09 & x12;
  assign z09 = x00 & ~x10 & ~new_n32_ & (~x11 ^ x12);
  assign z10 = x00 & x09 & ~x10 & new_n45_ & x11;
  assign new_n45_ = x12 & (~x02 | ~x03 | ~x04) & (~x14 | x15);
  assign z11 = x00 & ~x10 & x11 & ~new_n32_ & ~new_n47_;
  assign new_n47_ = x01 ? (~x09 | ~x12 | (x02 & x03 & x04)) : x12;
  assign z12 = x00 & ~x09 & ~x10 & x11 & ~new_n32_ & x12;
  assign z04 = ~x14;
  assign z07 = ~x15;
endmodule


