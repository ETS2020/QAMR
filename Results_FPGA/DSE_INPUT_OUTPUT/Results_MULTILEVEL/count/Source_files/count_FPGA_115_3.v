// Benchmark "FAU" written by ABC on Mon Aug 17 20:11:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n54_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_;
  assign z00 = ~new_n54_ & ((~x16 & (~x15 | x18)) | (x17 & (x18 | (x16 & x19))) | (x16 & ~x17 & (~x19 | (x18 & x19))));
  assign new_n54_ = x33 & x34;
  assign z01 = ~new_n56_ | (~x16 & (~x14 | x18));
  assign new_n56_ = (~x17 | (~x18 & (~x16 | ~x20))) & (x19 | (~x18 & (~x16 | x17 | x20))) & ~new_n54_ & (~x16 | ~x19 | (~x20 & (x17 | new_n54_ | ~x18)));
  assign z02 = (x16 & (new_n60_ | (~new_n59_ & x21))) | ~new_n58_ | (~x13 & ~x16);
  assign new_n58_ = ~new_n54_ & ~x18;
  assign new_n59_ = ~x17 & ~x19 & ~x20;
  assign new_n60_ = ~x17 & ~x19 & ~x20 & ~x21;
  assign z03 = (x16 & (new_n62_ | (~new_n60_ & x22))) | ~new_n58_ | (~x12 & ~x16);
  assign new_n62_ = ~x17 & ~x19 & ~x20 & ~x21 & ~x22;
  assign z04 = (x16 & (new_n64_ | (~new_n62_ & x23))) | ~new_n58_ | (~x11 & ~x16);
  assign new_n64_ = ~x17 & ~x19 & ~x20 & ~x21 & ~x22 & ~x23;
  assign z05 = ~new_n54_ & ((~new_n66_ & x16) | x18 | (~x10 & ~x16));
  assign new_n66_ = (new_n64_ | ~x24) & (~new_n59_ | ~new_n67_);
  assign new_n67_ = ~x21 & ~x22 & ~x23 & ~x24;
  assign z06 = ~new_n58_ | (~x09 & ~x16) | (x16 & (new_n69_ | new_n70_));
  assign new_n69_ = x25 & (~new_n59_ | ~new_n67_);
  assign new_n70_ = new_n60_ & new_n71_;
  assign new_n71_ = ~x22 & ~x23 & ~x24 & ~x25;
  assign z07 = ~new_n54_ & ((~new_n73_ & x16) | x18 | (~x08 & ~x16));
  assign new_n73_ = (~x26 | (new_n60_ & ~x24 & ~x25 & ~x22 & ~x23)) & (~new_n60_ | x22 | x23 | x24 | x25 | x26);
  assign z08 = ~new_n54_ & ((x16 & (new_n75_ | new_n77_)) | x18 | (~x07 & ~x16));
  assign new_n75_ = x27 & (~new_n60_ | ~new_n76_ | x22 | x23);
  assign new_n76_ = ~x24 & ~x25 & ~x26;
  assign new_n77_ = new_n62_ & ~x23 & ~x24 & ~x25 & ~x26 & ~x27;
  assign z09 = ~new_n54_ & (~new_n81_ | (x16 & (new_n79_ | (~new_n77_ & x28))));
  assign new_n79_ = new_n62_ & new_n80_;
  assign new_n80_ = ~x23 & ~x24 & ~x25 & ~x26 & ~x27 & ~x28;
  assign new_n81_ = ~x18 & (x06 | x16);
  assign z10 = ~new_n54_ & (~new_n84_ | (x16 & (new_n83_ | (~new_n79_ & x29))));
  assign new_n83_ = new_n64_ & new_n76_ & ~x27 & ~x28 & ~x29;
  assign new_n84_ = ~x18 & (x05 | x16);
  assign z11 = (~new_n86_ & x16) | ~new_n58_ | (~x04 & ~x16);
  assign new_n86_ = (~x30 | (new_n64_ & new_n76_ & ~x27 & ~x28 & ~x29)) & (~new_n64_ | ~new_n76_ | x27 | x28 | x29 | x30);
  assign z12 = ~new_n54_ & (~new_n91_ | (x16 & (new_n89_ | (~new_n88_ & x31))));
  assign new_n88_ = new_n64_ & new_n76_ & ~x27 & ~x28 & ~x29 & ~x30;
  assign new_n89_ = new_n59_ & new_n67_ & new_n90_ & ~x25 & ~x26 & ~x27;
  assign new_n90_ = ~x28 & ~x29 & ~x30 & ~x31;
  assign new_n91_ = ~x18 & (x03 | x16);
  assign z13 = ~new_n54_ & (~new_n96_ | (x16 & (new_n93_ | (~new_n89_ & x32))));
  assign new_n93_ = new_n59_ & new_n67_ & new_n94_ & new_n95_;
  assign new_n94_ = ~x25 & ~x26 & ~x27 & ~x28;
  assign new_n95_ = ~x29 & ~x30 & ~x31 & ~x32;
  assign new_n96_ = ~x18 & (x02 | x16);
  assign z14 = ~new_n100_ | (x16 & ((~new_n93_ & x33) | (new_n70_ & new_n98_)));
  assign new_n98_ = new_n99_ & ~x32 & ~x33 & ~x30 & ~x31;
  assign new_n99_ = ~x26 & ~x27 & ~x28 & ~x29;
  assign new_n100_ = (x01 | x16) & ~new_n54_ & ~x18;
  assign z15 = (~new_n104_ & (~x33 | ~x34)) | (x16 & ~x33 & (new_n102_ | (~new_n93_ & x34)));
  assign new_n102_ = new_n60_ & new_n71_ & new_n99_ & new_n103_;
  assign new_n103_ = ~x30 & ~x31 & ~x32 & ~x34;
  assign new_n104_ = ~x18 & (x00 | x16);
endmodule


