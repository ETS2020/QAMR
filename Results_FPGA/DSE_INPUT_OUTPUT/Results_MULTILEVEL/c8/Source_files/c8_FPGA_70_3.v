// Benchmark "FAU" written by ABC on Mon Aug 17 20:02:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n59_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_;
  assign z00 = (~x17 | ~x18) & (x27 ? ~x08 : ~x19);
  assign z01 = (~x17 | ~x18) & (x27 ? ~x09 : ~x20);
  assign z02 = (~x21 & ~x27) | (x17 & x18) | (~x10 & x27);
  assign z03 = (~x17 | ~x18) & (x27 ? ~x11 : ~x22);
  assign z04 = (~x17 | ~x18) & (x27 ? ~x12 : ~x23);
  assign z05 = (~x24 & ~x27) | (x17 & x18) | (~x13 & x27);
  assign z06 = (~x17 | ~x18) & (x27 ? ~x14 : ~x25);
  assign z07 = (~x17 | ~x18) & (x27 ? ~x15 : ~x26);
  assign z08 = x27 & (~x17 | ~x18);
  assign z09 = x16 ? (x17 ? (~x18 & x19) : ~x19) : ((x00 & ~x17 & x18) | (x08 & ~x18));
  assign z10 = (~new_n59_ & ~x16) | (x17 & (x18 | (x16 & x20))) | (x16 & ((x19 & x20) | (~x17 & ~x19 & ~x20)));
  assign new_n59_ = x18 ? ~x01 : ~x09;
  assign z11 = ~new_n61_ | (~x16 & (x18 ? x02 : x10));
  assign new_n61_ = (~x17 | (~x18 & (~x16 | ~x21))) & (~x16 | (((~x19 & ~x20) | ~x21) & (x20 | x21 | x17 | x19)));
  assign z12 = (~new_n63_ & ~x16) | (x17 & (x18 | (x16 & x22))) | (~new_n64_ & x16);
  assign new_n63_ = x18 ? ~x03 : ~x11;
  assign new_n64_ = (~x22 | (~x19 & ~x20 & ~x21)) & (x17 | x19 | x20 | x21 | x22);
  assign z13 = (~new_n66_ & ~x16) | (x17 & (x18 | (x16 & x23))) | (~new_n67_ & x16);
  assign new_n66_ = x18 ? ~x04 : ~x12;
  assign new_n67_ = (~x23 | (~x21 & ~x22 & ~x19 & ~x20)) & (x17 | x19 | x20 | x21 | x22 | x23);
  assign z14 = x16 ? (x17 ? (~x18 & x24) : ~new_n70_) : ~new_n69_;
  assign new_n69_ = (~x05 | x17 | ~x18) & (~x13 | x18);
  assign new_n70_ = (x19 | x20 | x21 | x22 | x23 | x24) & (~x24 | (~x19 & ~x20 & ~x21 & ~x22 & ~x23));
  assign z15 = x16 ? (x17 ? (~x18 & x25) : ~new_n72_) : ~new_n74_;
  assign new_n72_ = (~x25 | (new_n73_ & ~x22 & ~x23 & ~x24)) & (~new_n73_ | x24 | x25 | x22 | x23);
  assign new_n73_ = ~x19 & ~x20 & ~x21;
  assign new_n74_ = (~x06 | x17 | ~x18) & (~x14 | x18);
  assign z16 = (~new_n79_ & ~x16) | (x17 & (x18 | (x16 & x26))) | (~new_n76_ & x16);
  assign new_n76_ = (~x26 | (new_n73_ & new_n78_ & ~x24 & ~x25)) & (~new_n77_ | ~new_n78_ | x24 | x25 | x26);
  assign new_n77_ = ~x17 & ~x19 & ~x20 & ~x21;
  assign new_n78_ = ~x22 & ~x23;
  assign new_n79_ = (~x15 | (~x17 & x18)) & (~x07 | ~x18);
  assign z17 = (x17 & x18) | (~new_n81_ & x16);
  assign new_n81_ = (~x17 | ~x27) & (~new_n82_ | x20 | x21 | x17 | ~x19);
  assign new_n82_ = ~x22 & ~x23 & ~x24 & ~x25 & ~x26;
endmodule


