// Benchmark "FAU" written by ABC on Tue Aug 18 17:01:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  assign z03 = z06 & x12;
  assign z06 = x11 & x10 & ~x09 & x00 & x01 & x02;
  assign z04 = (~x01 & ~x17) | (new_n48_ & x00 & x01 & x02);
  assign new_n48_ = ~x09 & x10 & ~x11 & ~x12;
  assign z05 = (~x01 & ~x17) | (x00 & x01 & x02 & x09 & x10);
  assign z07 = (~x01 & ~x17) | (x00 & x02 & (new_n51_ | ~x01));
  assign new_n51_ = ~x09 & x10 & x11 & ~x12;
  assign z08 = ~x01 & (new_n53_ | ~x17);
  assign new_n53_ = new_n54_ & ~x00 & x02 & ~x03 & x04 & ~x05;
  assign new_n54_ = ~x06 & ~x07 & ~x08 & ~x18 & x19;
  assign z09 = z14 | (~x00 & ~x21 & ~new_n56_ & ~x22);
  assign new_n56_ = ~new_n58_ & (~new_n57_ | x01 | ~x02 | x11 | ~x12);
  assign new_n57_ = ~x13 & ~x14 & ~x15 & ~x16 & x20;
  assign new_n58_ = x01 & x18 & ~x19 & ~x20;
  assign z14 = ~x01 & ~x17;
  assign z10 = ~x00 & (new_n61_ | (new_n62_ & new_n63_));
  assign new_n61_ = x01 & x18 & ~x19 & ~x20 & x21 & x22;
  assign new_n62_ = ~x01 & x02 & ~x11 & x12 & ~x13 & ~x14;
  assign new_n63_ = x20 & ~x21 & ~x22 & x15 & x16 & x17;
  assign z11 = z14 | (~x00 & ~x22 & (new_n65_ | new_n67_));
  assign new_n65_ = new_n66_ & ~x01 & x02 & ~x11 & x12 & ~x13;
  assign new_n66_ = ~x14 & x15 & ~x16 & x20 & ~x21;
  assign new_n67_ = x01 & x18 & ~x19 & ~x20 & x21;
  assign z12 = new_n69_ & x09;
  assign new_n69_ = ~x24 & (x00 ? (x01 & (~x02 | ~x10)) : (new_n70_ & ~x01));
  assign new_n70_ = x17 & (x23 | (~x02 & ~x19));
  assign z13 = ~x19 & x17 & ~x02 & ~x00 & ~x01;
  assign z15 = (x00 & (~x02 | (x01 & ~x10))) | (~x01 & (~x17 | (~x02 & x19)));
  assign z16 = x01 ? ~x00 : ~x17;
  assign z17 = ~x01 & (~x17 | (~x00 & x02));
  assign z00 = 1'b0;
  assign z01 = 1'b0;
  assign z02 = 1'b0;
endmodule


