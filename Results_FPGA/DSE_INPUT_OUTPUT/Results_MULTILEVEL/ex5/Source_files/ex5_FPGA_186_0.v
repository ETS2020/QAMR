// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n81_, new_n83_, new_n85_, new_n86_, new_n88_, new_n91_,
    new_n93_, new_n95_, new_n96_, new_n105_, new_n108_, new_n112_,
    new_n114_, new_n117_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n125_, new_n126_, new_n128_, new_n130_, new_n132_, new_n134_,
    new_n135_, new_n137_, new_n139_, new_n140_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n161_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n181_,
    new_n183_, new_n185_, new_n187_, new_n188_, new_n190_, new_n192_;
  assign z00 = ~x6 & ~x5 & x2 & ~x4;
  assign z01 = ~x7 & ~x6 & x2 & ~x5;
  assign z02 = ~x6 & (~x2 | (~x3 & ~x4 & x5 & ~x7));
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & x2 & x3;
  assign z04 = (~x2 & ~x6) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = (~x2 & ~x6) | (new_n79_ & x6 & ~x7);
  assign new_n79_ = ~x4 & x5;
  assign z06 = ~x6 & ~x5 & ~x4 & x3 & new_n81_ & x2;
  assign new_n81_ = ~x0 & ~x1;
  assign z07 = x7 & new_n83_ & x6;
  assign new_n83_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = (~x2 & ~x6) | (new_n85_ & x0 & x1 & x2 & ~x3);
  assign new_n85_ = new_n79_ & new_n86_;
  assign new_n86_ = x6 & x7;
  assign z09 = (~x2 & ~x6) | (new_n88_ & new_n81_ & x2 & ~x3);
  assign new_n88_ = new_n86_ & ~x4 & ~x5;
  assign z10 = (~x2 & ~x6) | (new_n85_ & ~x0 & x1 & x2);
  assign z11 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = ~x2 & (~x6 | (new_n96_ & new_n95_ & ~x0));
  assign new_n95_ = x1 & x3;
  assign new_n96_ = ~x4 & x5 & x7;
  assign z14 = ~x2 & (~x6 | (new_n96_ & x0 & ~x1 & x3));
  assign z15 = (~x2 & ~x6) | (new_n85_ & x2 & x3 & ~x0 & x1);
  assign z16 = ~x2 & (~x6 | (new_n96_ & new_n95_ & x0));
  assign z17 = x6 & ~x5 & x4 & ~x2 & x0 & ~x1;
  assign z18 = ~x5 & x4 & x3 & new_n81_ & x2;
  assign z19 = x6 & x4 & ~x3 & new_n81_ & ~x2;
  assign z21 = ~x2 & ~x6;
  assign z22 = new_n105_ & x7;
  assign new_n105_ = x6 & ~x4 & ~x2 & x0 & ~x1 & ~x5;
  assign z23 = x6 & x5 & x3 & new_n81_ & ~x2;
  assign z24 = ~x2 & (~x6 | (new_n108_ & ~x0 & ~x1 & ~x3));
  assign new_n108_ = ~x4 & ~x5 & ~x7;
  assign z25 = ~x2 & (~x6 | (new_n108_ & ~x0 & x1 & ~x3));
  assign z26 = (~x2 & ~x6) | (new_n88_ & x0 & x2 & ~x3);
  assign z27 = (~x2 & ~x6) | (new_n112_ & x2 & ~x3 & ~x0 & x1);
  assign new_n112_ = x6 & ~x7 & ~x4 & ~x5;
  assign z28 = z21 | (new_n88_ & new_n114_);
  assign new_n114_ = x0 & ~x1 & x2 & x3;
  assign z30 = (~x2 & ~x6) | (new_n88_ & x0 & x1 & x2 & ~x3);
  assign z31 = (~new_n117_ & x2) | (~x2 & (new_n117_ | ~x6)) | x1 | ~x4 | ~x5;
  assign new_n117_ = ~x0 & x3;
  assign z32 = ~new_n119_ | (x0 & (new_n121_ | x2));
  assign new_n119_ = (~x1 | (~x2 & ~x6)) & (new_n120_ | ~x6) & (~x2 | (x3 & x4));
  assign new_n120_ = (x2 | ~x4 | (x0 & x5)) & (x4 | (~x3 & ~x5 & ~x7));
  assign new_n121_ = ~x4 & x6;
  assign z33 = ~new_n123_ | ~new_n121_ | ~x7;
  assign new_n123_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = ~new_n125_ | ((~x2 | ~x3) & (x1 | x5 | ~x6));
  assign new_n125_ = x2 ? ((~x3 | (x5 & ~x6)) & new_n126_ & (~x0 | x3)) : (~new_n126_ & x0);
  assign new_n126_ = ~x4 & ~x7;
  assign z35 = (~x2 & (~x6 | (~x0 & x3))) | (~x5 & (x0 | x2)) | ~new_n128_ | (x0 & (x2 | ~x6));
  assign new_n128_ = ~x1 & x4 & (~x2 | x3);
  assign z36 = ~new_n130_ | (~z21 & (x1 | x5));
  assign new_n130_ = x2 ? (~x4 & x6 & ~x7 & ~x0 & ~x3) : (~x6 | (x0 & x4));
  assign z37 = (~new_n132_ & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n126_)) | ~x6 | (~x1 & ~x3);
  assign new_n132_ = x0 & ~x2;
  assign z38 = (~x4 & (x0 | x2)) | (x0 & (x2 | ~x6)) | ~new_n135_ | (~x2 & (~x6 | (~new_n134_ & ~x0)));
  assign new_n134_ = ~x3 & ~x4 & ~x5 & ~x7;
  assign new_n135_ = ~x1 & (~x2 | x3);
  assign z39 = ~new_n137_ | (x2 & (~x3 | ~x5 | x6 | x7));
  assign new_n137_ = ~x4 & (x2 | (x0 & ~x1 & new_n86_ & ~x5));
  assign z40 = (x1 & (~x0 | ~x2)) | ~new_n139_ | (x3 & (x0 ^ ~x2));
  assign new_n139_ = (~x5 | (x0 ? ~x2 : x4)) & new_n140_ & (x6 | (~x0 & x2));
  assign new_n140_ = x0 ? (x2 ? (~x4 & x7) : (x4 & x5)) : ((x4 | ~x7) & (~x2 | (x3 & x4)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~new_n132_ | ~x6 | (x1 & x3);
  assign z42 = (x5 & (~x2 | x6 | x7)) | x4 | (~new_n143_ & ~x5);
  assign new_n143_ = x0 & ~x1 & new_n86_ & (~x2 | x3);
  assign z43 = new_n149_ | ~new_n145_ | (x1 & (~x2 | (~x0 & x4)));
  assign new_n145_ = (new_n146_ | x2) & ~new_n147_ & ~new_n148_ & (~x2 | x3 | ~x4);
  assign new_n146_ = x6 & (x0 | x4 | ~x5);
  assign new_n147_ = x0 & ((x4 & (x2 | ~x6)) | (x6 & (x2 | ~x4) & (x5 | ~x7)) | (~x6 & (~x5 | x7)));
  assign new_n148_ = ~x0 & ~x4 & (x7 | (x2 & (~x5 | x6)));
  assign new_n149_ = x3 & (x0 ? (x1 & x6) : ~x2);
  assign z44 = (~x0 & ((x1 & x4) | (~x2 & x3))) | (~x2 & (x1 | ~x6)) | x0 | x2 | ~x4;
  assign z45 = ~new_n152_ | (~z21 & x0);
  assign new_n152_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (~x6 | (~x1 & ~x4 & ~x5 & x7));
  assign z46 = (~x4 & (x5 | ~x7)) | ~new_n154_ | x0 | ~x1;
  assign new_n154_ = ~x2 & ~x3 & x6;
  assign z47 = ~new_n156_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n156_ = (x2 | (~x0 & ~x1)) & (new_n157_ | (~x0 & x1)) & (~x0 | (x1 & x3 & x5)) & (x1 | (~x2 & ~x5));
  assign new_n157_ = ~x4 & x6 & x7;
  assign z48 = (~new_n159_ & ~x4) | ~new_n81_ | x2 | ~x3 | ~x6;
  assign new_n159_ = x5 & x7;
  assign z49 = (~new_n161_ & ~x4) | (x2 & (x0 | x1)) | ~x2 | (x3 & x4) | (x0 & (~x1 | ~x3));
  assign new_n161_ = ~x5 & (~x2 | ~x6);
  assign z50 = ~new_n163_ | (~new_n95_ & x0);
  assign new_n163_ = (~x4 | (x2 & ~x3)) & (~x5 | (x2 & x4)) & ~x2 & (new_n86_ | x2);
  assign z51 = (~new_n167_ & ~x0) | ~new_n165_ | (~x2 & (~x6 | (x0 & x3)));
  assign new_n165_ = (~x0 | (x1 & (x4 | ~x5 | x6))) & (x4 | new_n166_ | ~x6);
  assign new_n166_ = ~x2 & x5 & x7;
  assign new_n167_ = (~x1 | (~x2 & ~x6)) & (x2 | (x4 & (x3 | ~x6))) & (x4 | (~x6 & (~x2 | ~x5))) & (~x2 | (x3 & ~x4));
  assign z52 = ~new_n169_ | (x0 & ((x2 & x3) | (~x1 & ~x2 & x6)));
  assign new_n169_ = ~new_n171_ & ~new_n170_ & (x0 | (~new_n154_ & (z21 | ~x1)));
  assign new_n170_ = ~x4 & (x6 | (x2 & x5));
  assign new_n171_ = x3 & ((x1 & x6) | (~x0 & x2 & x4));
  assign z53 = (~new_n96_ & (new_n154_ | (~x1 & x2))) | new_n173_ | (~new_n175_ & x2);
  assign new_n173_ = ~new_n174_ & x6;
  assign new_n174_ = (~x1 | (x3 ? (x4 | x5) : ~x0)) & (x2 | ~x3 | (x1 & x4)) & (~x2 | x3 | x4);
  assign new_n175_ = (~x0 | (x1 & x3)) & ((x3 & x6) | (~new_n79_ & x1)) & ~new_n176_ & (x0 | ~x1 | ~x3);
  assign new_n176_ = ~x4 & x5 & ~x7;
  assign z54 = (x2 & (x3 ? ~x1 : x4)) | (~x1 & (x0 | (~x3 & x4))) | ~new_n178_ | (x0 & (x3 | x4));
  assign new_n178_ = (new_n179_ | x4) & (x6 | (x2 & x3)) & (x2 | ~x3 | (new_n159_ & ~x4));
  assign new_n179_ = (~x5 | (x6 & x7)) & (x5 | (x3 & ~x6)) & (x3 | (x7 & (x0 | x2)));
  assign z55 = ~new_n181_ | (~x4 & (~x2 | (~x0 & (x5 | x6))));
  assign new_n181_ = (x2 | (x6 & (~x0 | x3))) & x1 & (~x0 | (x6 & (new_n96_ | ~x2)));
  assign z56 = (x0 & (x2 | x6)) | ~new_n183_ | (~x1 & (x2 ? x3 : x6));
  assign new_n183_ = (~x6 | ((x4 | x7) & (x2 | (x3 & (x4 | ~x5))))) & (~x2 | (~x4 & x5 & x6 & x7));
  assign z57 = (x0 & (x2 | (~x2 & ~x3 & x6))) | ~new_n185_ | (~x1 & (x2 ? ~x3 : x6));
  assign new_n185_ = x2 ? (~x4 & x5 & x6 & x7) : (~x6 | (~new_n117_ & (x4 | (~x5 & x7))));
  assign z58 = (~new_n187_ & x2) | (x6 & (~x3 | (~new_n188_ & ~x2)));
  assign new_n187_ = (x0 | x4 | (~x5 & ~x6)) & new_n95_ & (~x0 | (~x4 & x5 & x6 & x7));
  assign new_n188_ = new_n81_ & ~x4 & ~x5 & x7;
  assign z59 = (x2 & ((~x0 & (x1 | x3)) | (x1 & (new_n121_ | x3)))) | ~new_n190_ | (x0 & ((~x2 & (~x1 | ~x3)) | (~x1 & (new_n121_ | ~x3))));
  assign new_n190_ = (x4 | ~x5) & ((new_n86_ & ~x4 & ~x5) | (x1 ? x2 : x0));
  assign z60 = (~new_n192_ & (x2 | x6)) | (~x2 & x3 & x6) | (~x0 & x2 & (~x3 | ~x6));
  assign new_n192_ = x0 ? (x1 & ~x3 & x4) : (new_n159_ & ~x1 & ~x4);
  assign z61 = ~new_n114_ | (~x4 & (x5 | x6));
  assign z62 = new_n170_ | (~z21 & (~x0 | ~x1 | x3));
  assign z20 = 1'b0;
  assign z29 = 1'b0;
endmodule


