// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n80_, new_n82_, new_n84_, new_n87_, new_n88_, new_n90_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n104_,
    new_n107_, new_n114_, new_n116_, new_n118_, new_n120_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n176_, new_n177_, new_n179_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n210_, new_n211_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n219_, new_n221_;
  assign z00 = ~x6 & (new_n74_ | x7);
  assign new_n74_ = ~x4 & ~x5;
  assign z01 = ~x6 & (~x5 | x7);
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = (~x6 & x7) | (new_n80_ & x6 & ~x7);
  assign new_n80_ = ~x4 & x5;
  assign z06 = ~x6 & (new_n82_ | x7);
  assign new_n82_ = ~x0 & ~x1 & x2 & new_n74_ & x3;
  assign z07 = x7 & (~x6 | (new_n84_ & ~x0 & x1 & ~x2));
  assign new_n84_ = new_n80_ & ~x3;
  assign z08 = x7 & (~x6 | (new_n84_ & x0 & x1 & x2));
  assign z09 = x7 & new_n87_ & x6;
  assign new_n87_ = ~x5 & ~x4 & ~x3 & new_n88_ & x2;
  assign new_n88_ = ~x0 & ~x1;
  assign z10 = x7 & x6 & x5 & ~x4 & new_n90_ & x2;
  assign new_n90_ = ~x0 & x1;
  assign z11 = x7 & (~x6 | (new_n84_ & x0 & x1 & ~x2));
  assign z12 = x7 & (~x6 | (new_n84_ & x0 & ~x1 & x2));
  assign z13 = x7 & x6 & x5 & ~x4 & new_n94_ & x3;
  assign new_n94_ = new_n90_ & ~x2;
  assign z14 = x7 & x6 & new_n96_ & x5;
  assign new_n96_ = ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z15 = x7 & (new_n98_ | ~x6);
  assign new_n98_ = new_n80_ & x3 & ~x0 & x1 & x2;
  assign z16 = x7 & new_n100_ & x6;
  assign new_n100_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = new_n102_ | (x0 & ~x1 & ~x2 & x4 & ~x5);
  assign new_n102_ = ~x6 & x7;
  assign z18 = new_n102_ | new_n104_;
  assign new_n104_ = ~x0 & ~x1 & x2 & x3 & x4 & ~x5;
  assign z19 = ~x0 & ~x1 & ~x2 & ~x3 & ~new_n102_ & x4;
  assign z20 = ~x7 & new_n107_ & ~x6;
  assign new_n107_ = ~x5 & ~x4 & ~x3 & ~x2 & x0 & ~x1;
  assign z21 = ~x7 & ~x6 & new_n96_ & ~x5;
  assign z22 = x7 & (~x6 | (x0 & ~x1 & new_n74_ & ~x2));
  assign z23 = new_n102_ | (new_n88_ & ~x2 & x3 & x5);
  assign z24 = (~x6 & x7) | (new_n88_ & ~x2 & ~x3 & new_n74_ & x6 & ~x7);
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n94_ & ~x3;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n114_ & ~x3;
  assign new_n114_ = x0 & x2;
  assign z27 = ~x7 & new_n116_ & x6;
  assign new_n116_ = ~x5 & ~x4 & ~x3 & new_n90_ & x2;
  assign z28 = x7 & (new_n118_ | ~x6);
  assign new_n118_ = new_n74_ & x3 & x0 & ~x1 & x2;
  assign z30 = x7 & new_n120_ & x6;
  assign new_n120_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = (x0 & (x2 | (~x4 & x6))) | ~new_n122_ | (~x0 & (~x4 | (~x2 & x3)));
  assign new_n122_ = (x5 | (~x2 & (x2 | ~x4))) & (~x2 | (x3 & x4)) & new_n123_ & (x4 | (~x5 & ~x7));
  assign new_n123_ = ~x1 & (x6 | ~x7);
  assign z32 = (~x3 & (x2 | (x0 & ~x4))) | ~new_n125_ | (x0 & (x2 | (~x4 & x6)));
  assign new_n125_ = ~new_n126_ & new_n127_ & (x6 | (~x7 & (x0 | x4)));
  assign new_n126_ = ~x2 & ((x4 & ~x5) | (~x0 & (x3 | x4)));
  assign new_n127_ = ~x1 & (x4 | (~x2 & ~x5 & ~x7));
  assign z33 = ~new_n129_ | ~x7 | x4 | ~x6;
  assign new_n129_ = new_n114_ & (~x1 | ~x3 | x5) & (x1 | ~x5);
  assign z34 = new_n133_ | (~new_n131_ & x6);
  assign new_n131_ = (~x0 | (~new_n132_ & ~x2)) & (x0 | (new_n132_ & x2 & ~x3)) & ~x1 & ~x5;
  assign new_n132_ = ~x4 & ~x7;
  assign new_n133_ = ~x7 & (new_n134_ | (x4 & (~x0 | x1 | x5)));
  assign new_n134_ = ~x6 & (x4 ? x2 : (~x3 | ~x5));
  assign z35 = ~new_n102_ & ~new_n136_;
  assign new_n136_ = (~x0 | (~x2 & x5)) & (~x2 | (x3 & x5)) & ~x1 & x4 & (x0 | x2 | ~x3);
  assign z36 = (x0 & (x2 | ~x4)) | ~new_n138_ | (~x6 & (~x0 | x7));
  assign new_n138_ = (x0 | (new_n132_ & x2 & ~x3)) & ~x1 & ~x5;
  assign z37 = (~new_n140_ & (x6 | ~x7)) | (x3 & ~x5 & (x6 ? (x4 | x7) : ~x7));
  assign new_n140_ = ((x0 & ~x2) | (x3 & ~x5)) & (x1 | x3) & (~x1 | ~x3 | ~x5);
  assign z38 = new_n142_ | new_n144_ | new_n145_ | (~new_n143_ & x6);
  assign new_n142_ = x5 & (x0 ? new_n132_ : (~x2 & x6));
  assign new_n143_ = (~x2 | (~x0 & x3)) & ~x1 & (x0 | x2 | (new_n132_ & ~x3));
  assign new_n144_ = (x0 | x2) & ((x1 & ~x7) | (~x4 & x6));
  assign new_n145_ = ~x7 & ((x0 & (x2 | (~x3 & ~x4))) | (x2 & (~x3 | ~x4)) | (~x0 & ~x2 & ~x6));
  assign z39 = (x5 & (~x3 | x6 | x7)) | x4 | (~new_n147_ & ~x5);
  assign new_n147_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = (x3 & (~x0 ^ x2)) | ~new_n149_ | (x1 & (~x0 | ~x2 | x4));
  assign new_n149_ = ~new_n150_ & new_n151_ & (x0 | (~new_n102_ & (~x2 | x3)));
  assign new_n150_ = ~x4 & ((~x2 & ((x0 & x6) | x7)) | x5 | (~x0 & (x2 | ~x6 | x7)));
  assign new_n151_ = (x6 | ((~x0 | ~x2) & (~x4 | ~x7))) & (~x0 | ((~x4 | x5) & (~x2 | (~x4 & x7))));
  assign z41 = ~new_n102_ & ((~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3));
  assign z42 = (x4 & (x6 | ~x7)) | (~x7 & (~x5 | x6)) | (~new_n154_ & x6);
  assign new_n154_ = x0 & ~x1 & ~x5 & (~x2 | x3);
  assign z43 = (~new_n160_ & x2) | (~new_n158_ & x6) | (~new_n156_ & ~x7);
  assign new_n156_ = (~x0 | ((x4 | ~x6) & (~x1 | x5 | x6))) & (~x4 | ((x0 | (~new_n157_ & ~x1)) & (~x1 | x6))) & (x5 | x6 | x0 | x4);
  assign new_n157_ = ~x2 & x3;
  assign new_n158_ = new_n159_ & (x2 | (~x1 & (x0 | ~x3)));
  assign new_n159_ = (x0 | (~x1 & (x4 | ~x7))) & (x4 | ~x5) & (~x1 | ~x3);
  assign new_n160_ = (~x6 | (x0 ? (~x4 & ~x5 & x7) : (x3 & x4))) & (x7 | ((~x0 | (~x4 & x5)) & (x3 | ~x4) & (x4 | x5)));
  assign z44 = new_n168_ | (~x7 & (~new_n162_ | new_n167_));
  assign new_n162_ = ~new_n163_ & ~new_n164_ & ~new_n165_ & new_n166_;
  assign new_n163_ = ~x4 & (~x0 | (x2 & ~x5));
  assign new_n164_ = ~x0 & (x2 | (x1 & x4));
  assign new_n165_ = x1 & ~x6 & (x4 | (x0 & ~x5));
  assign new_n166_ = (~x2 | ((x3 | ~x4) & (~x0 | x5))) & (~x0 | (~x4 & ~x5));
  assign new_n167_ = x3 & (x0 | (~x0 & ~x2 & x4));
  assign new_n168_ = x6 & ((~x0 & (x1 | (~x2 & x3))) | x0 | x2 | ~x4 | (x1 & (~x2 | x3)));
  assign z45 = ~new_n170_ | (~new_n102_ & x0);
  assign new_n170_ = (~x6 | (x1 ? (x2 & x4) : (~x2 & ~x4 & ~x5))) & (x7 | (x1 & x2 & (x4 | ~x5)));
  assign z46 = ((~new_n172_ | ~new_n90_) & (x6 | ~x7)) | (~x4 & x6 & ~x7);
  assign new_n172_ = ~x2 & ~new_n80_ & ~x3;
  assign z47 = (~new_n174_ & x6) | (~x7 & (~new_n90_ | (~x6 & (new_n80_ | ~x2))));
  assign new_n174_ = (~x1 | (x2 & (x0 | x4))) & (~x4 | (~x0 & x1)) & (x1 | (~x0 & ~x2 & ~x5)) & (~x0 | (x2 & x3 & x5));
  assign z48 = new_n176_ | ~new_n88_ | ~new_n177_ | x2;
  assign new_n176_ = ~x4 & (x6 ? (~x5 | ~x7) : x5);
  assign new_n177_ = x3 & (x6 | ~x7);
  assign z49 = (x7 & (~x4 | ~x6)) | ~new_n88_ | ~new_n179_ | (~x4 & (x5 | x6));
  assign new_n179_ = x2 & (~x3 | ~x4);
  assign z50 = ~x7 | (~new_n181_ & x6);
  assign new_n181_ = new_n74_ & ~x2 & (~x0 | (x1 & x3));
  assign z51 = new_n183_ | new_n186_ | (~new_n184_ & ~x0);
  assign new_n183_ = ~new_n102_ & (x0 ? (~x1 | (~x2 & x3)) : (~x2 & (x1 | ~x3)));
  assign new_n184_ = (~x2 | (~x6 & (~x4 | x7))) & (x4 | (~x6 & (new_n185_ | x7)));
  assign new_n185_ = ~x1 & x3;
  assign new_n186_ = ~x4 & ((~x7 & (x5 | x6)) | (x6 & (x2 | ~x5)));
  assign z52 = new_n188_ | ~new_n189_ | (~new_n191_ & ~x0);
  assign new_n188_ = x2 & (new_n90_ | x3) & x4;
  assign new_n189_ = (x4 | (~x6 & (~x5 | (~x0 & x7)))) & (new_n190_ | ~x0) & (x6 | ~x7);
  assign new_n190_ = ~x3 & (x1 | x2);
  assign new_n191_ = (x2 | (~x6 & x7) | (~x1 & x3)) & (x4 | (~x7 & (~x1 | x7)));
  assign z53 = (~new_n196_ & (~x7 | (x1 & x6))) | (~new_n193_ & x6) | (~x7 & (~new_n195_ | ~x1));
  assign new_n193_ = new_n194_ & (~x0 | (x3 ? x1 : ~x2));
  assign new_n194_ = ((x1 & x4) | (x3 ? (x2 & x5) : ~x2)) & (x2 | x3 | (~x4 & x5)) & (~x3 | (x4 ? x1 : x7));
  assign new_n195_ = ~new_n80_ & (x2 | x3);
  assign new_n196_ = x0 ? x3 : (~x2 | ~x3);
  assign z54 = (~new_n198_ & x6) | (~x7 & (~new_n200_ | (x0 & (x3 | ~x6))));
  assign new_n198_ = (~x2 | (x3 ? x1 : ~x4)) & (x1 | (~x0 & (x3 | ~x4))) & new_n199_ & (~x0 | (~x3 & ~x4));
  assign new_n199_ = (x2 | ((~x3 | (~x4 & x5)) & (x0 | x3 | x4))) & (x4 | (x5 & x7));
  assign new_n200_ = (x6 | (x1 & (~x2 | x3))) & ~new_n80_ & (x2 | ~x3);
  assign z55 = ~new_n202_ | (x0 & ~new_n157_ & (x4 | ~x6));
  assign new_n202_ = (x6 | ((x4 | ~x5) & ~x7)) & x1 & (x4 | ~x6 | (new_n114_ & x5 & x7));
  assign z56 = (x0 & (x6 | ~x7)) | ~new_n205_ | (~new_n204_ & (~x7 | (~x2 & x6)));
  assign new_n204_ = x1 & ~new_n80_ & x3;
  assign new_n205_ = (x7 | (~x2 & (x4 | ~x6))) & (~x6 | (~new_n185_ & (~x2 | (~x4 & x5))));
  assign z57 = ((x0 | ~x1) & ~x3) | ~new_n207_ | ((new_n80_ | ~x1) & (x0 | ~x2));
  assign new_n207_ = (x6 | (~x2 & ~x7)) & new_n208_ & (x7 | (~x2 & (x4 | ~x6)));
  assign new_n208_ = (x0 | x2 | ~x3) & (~x2 | (~x0 & ~x4 & x5));
  assign z58 = ~new_n210_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n210_ = (x2 | (~x0 & ~x1)) & (new_n211_ | (~x0 & x1)) & new_n177_ & (x1 | (~x2 & ~x5)) & (~x0 | (x1 & x5));
  assign new_n211_ = ~x4 & x6 & x7;
  assign z59 = (~new_n217_ & x0) | ~new_n213_ | (x5 & (~x4 | (~x0 & ~x1)));
  assign new_n213_ = ~new_n214_ & (new_n215_ | ~x1) & ~new_n216_ & (~new_n102_ | x1);
  assign new_n214_ = (~x6 | ~x7) & (x1 ? (~x2 & ~x4) : ~x0);
  assign new_n215_ = (~x3 | (~x2 & ~x4)) & (~x4 | (x2 & (x6 | ~x7))) & (~x2 | (x0 & (x4 | (~x6 & ~x7))));
  assign new_n216_ = ~x0 & (x4 | (x2 & x3));
  assign new_n217_ = (x2 | (x1 & (~x1 | x3 | x4))) & (x1 | (x3 & (x4 | ~x6)));
  assign z60 = (~x0 & (x4 | (x1 & x2))) | (x2 & (x1 ^ ~x3)) | ~new_n219_ | (x3 & (x1 ? x4 : ~x2));
  assign new_n219_ = x1 ? (x4 & (~x4 | x6 | ~x7)) : (~x0 & ~x4 & x5 & x6 & x7);
  assign z61 = (~new_n221_ & ~x4) | ~x0 | x1 | ~new_n177_ | ~x2;
  assign new_n221_ = ~x5 & ~x6;
  assign z62 = (~new_n221_ & ~x4) | ~x0 | ~x1 | new_n102_ | x3;
  assign z29 = 1'b0;
endmodule


