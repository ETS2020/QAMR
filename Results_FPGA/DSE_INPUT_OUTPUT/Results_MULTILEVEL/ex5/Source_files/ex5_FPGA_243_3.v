// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n80_, new_n83_, new_n85_, new_n87_, new_n89_, new_n92_,
    new_n94_, new_n96_, new_n99_, new_n101_, new_n105_, new_n108_,
    new_n111_, new_n113_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n130_, new_n132_,
    new_n134_, new_n135_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n146_, new_n147_, new_n148_, new_n150_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n158_, new_n160_, new_n164_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n180_, new_n181_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = z06 | (~x5 & ~x6 & ~x7);
  assign z02 = (x2 & x3) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = x3 & (x2 | (~x6 & ~x7 & ~x4 & x5));
  assign z04 = x3 & (x2 | (new_n79_ & new_n80_));
  assign new_n79_ = ~x4 & ~x5;
  assign new_n80_ = x6 & ~x7;
  assign z05 = z06 | (new_n80_ & ~x4 & x5);
  assign z07 = (x2 & x3) | (new_n83_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n83_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n85_ & x5;
  assign new_n85_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x2 & (x3 | (new_n87_ & ~x0 & ~x1 & ~x4));
  assign new_n87_ = ~x5 & x6 & x7;
  assign z10 = x7 & new_n89_ & x6;
  assign new_n89_ = x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = (x2 & x3) | (new_n83_ & ~x2 & ~x3 & x0 & x1);
  assign z12 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x3 & (x2 | (new_n96_ & x0 & ~x1 & ~x4));
  assign new_n96_ = x5 & x6 & x7;
  assign z16 = x3 & (x2 | (new_n96_ & x0 & x1 & ~x4));
  assign z17 = (x2 & x3) | (new_n99_ & ~x2 & x4 & ~x5);
  assign new_n99_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & new_n101_ & ~x2;
  assign new_n101_ = ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n99_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n99_ & ~x2;
  assign z22 = (x2 & x3) | (new_n105_ & x0 & ~x1 & ~x2);
  assign new_n105_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x5 & x3 & new_n101_ & ~x2;
  assign z24 = ~x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x2 & x3) | (new_n79_ & new_n80_ & ~x2 & ~x3 & ~x0 & x1);
  assign z26 = new_n111_ & x7;
  assign new_n111_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = x2 & (new_n113_ | x3);
  assign new_n113_ = ~x0 & x1 & ~x4 & ~x5 & x6 & ~x7;
  assign z29 = (x2 & x3) | (new_n101_ & ~x2 & ~x3 & new_n79_ & ~x6 & x7);
  assign z30 = x7 & x6 & new_n85_ & ~x5;
  assign z31 = ~new_n117_ | (~x0 & (x3 | ~x4));
  assign new_n117_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (~new_n119_ & ~x4) | (x2 & ~x3) | (~x2 & (~new_n121_ | (~new_n120_ & x4)));
  assign new_n119_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | ~x5);
  assign new_n120_ = x0 & x5;
  assign new_n121_ = ~x1 & (x0 | (~x3 & ~x5 & x6 & ~x7));
  assign z33 = ~new_n123_ | ~x7 | x4 | ~x6;
  assign new_n123_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n126_ & ~x5) | ~new_n127_ | (~new_n125_ & (x2 | x5));
  assign new_n125_ = ~x4 & ~x7;
  assign new_n126_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n127_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n130_ & (~x0 | x2)) | x1 | x5 | (~x0 & ~x2);
  assign new_n130_ = ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n132_)) | x2 | (~x1 & ~x3);
  assign new_n132_ = ~x4 & x6 & ~x7;
  assign z38 = new_n134_ | new_n135_ | x1 | x2;
  assign new_n134_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n135_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~new_n137_ & ~x2) | (~x3 & (x2 | x5));
  assign new_n137_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = x2 ? new_n142_ : (new_n139_ | new_n140_ | ~new_n141_);
  assign new_n139_ = x0 & (x4 ? ~x5 : x6);
  assign new_n140_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n141_ = ~x1 & (x4 | ~x5);
  assign new_n142_ = ~x3 & (x5 | ~x6 | ~x7 | ~x0 | x4);
  assign z41 = (~x1 & (~x3 | (~x2 & ~x5))) | (x2 & ~x3) | (~x2 & (~x0 | (x1 & x3)));
  assign z42 = ~new_n137_ | (x2 & (x3 | ~x5));
  assign z43 = x2 ? new_n148_ : (new_n146_ | new_n147_);
  assign new_n146_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n147_ = ~x4 & ((~x0 & ((~x5 & ~x6) | x7)) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n148_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = (~new_n150_ & ~x2) | (~x3 & (x2 | (~x4 & x5)));
  assign new_n150_ = (~x3 | (~x0 & ~x4)) & (x4 | (x0 & ~x5)) & ~x1 & (~x0 | (~x4 & ~x5 & ~x6));
  assign z45 = (x0 & (~x2 | (x2 & ~x3))) | (~new_n152_ & ~x2) | (~new_n153_ & ~x3);
  assign new_n152_ = ~x1 & ~x4 & (x4 | (~x5 & (x0 | (x6 & x7))));
  assign new_n153_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n155_ | ~x1 | x3));
  assign new_n155_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | (~new_n158_ & ~x2) | (x2 & ~x3 & (new_n157_ | ~x1));
  assign new_n157_ = ~x4 & (x5 | x6);
  assign new_n158_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~x3 | (~new_n160_ & ~x2);
  assign new_n160_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = new_n157_ | ~new_n101_ | ~x2 | x3;
  assign z50 = (x0 & (~x1 | ~x3)) | ~new_n87_ | x2 | x4;
  assign z51 = (~x2 & (~new_n164_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | ~x1 | (new_n157_ & x2)));
  assign new_n164_ = (x0 | ~x1) & (x4 | ((x5 | ~x6) & (~x5 | (x6 & x7)) & (x0 | (~x5 & ~x6))));
  assign z52 = (x0 & ((~x1 & ~x2) | x3)) | (~x0 & (x1 | (~x2 & ~x3))) | new_n157_ | (x2 & x3);
  assign z53 = (~x3 & ((x0 & (x1 | x2)) | (x2 & (new_n157_ | ~x1)) | (~new_n83_ & ~x2))) | (~x2 & x3 & (new_n157_ | ~x1));
  assign z54 = (~x3 & (new_n99_ | new_n169_ | (~new_n168_ & ~x2))) | (~x2 & ~new_n170_ & x3);
  assign new_n168_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n169_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n170_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n172_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n173_ & ~x3);
  assign new_n172_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n173_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n175_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n176_ & x2);
  assign new_n175_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n176_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | ~x3)) | (~x2 & (new_n155_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n83_ & x2)));
  assign z58 = ~x3 | (~x2 & (~new_n87_ | x0 | x1 | x4));
  assign z59 = (~new_n180_ & x0) | (~new_n181_ & ~x3) | (~new_n105_ & (x3 ? ~x2 : ~x0));
  assign new_n180_ = (x1 | (x2 & x3)) & (x3 | (x2 & (x4 | ~x6)));
  assign new_n181_ = (x0 | ~x1 | ~x2) & (x4 | ~x5);
  assign z60 = x3 ? ~x2 : (x0 ? (~x1 | ~x4) : (~new_n96_ | x1 | x2 | x4));
  assign z62 = x3 ? ~x2 : (~x0 | (x0 & (new_n157_ | ~x1)));
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = 1'b1;
endmodule


