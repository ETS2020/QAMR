// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n92_,
    new_n94_, new_n97_, new_n101_, new_n103_, new_n105_, new_n111_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n128_, new_n130_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n149_, new_n150_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n159_, new_n161_, new_n162_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n174_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n184_;
  assign z00 = ~x4 & ~x5 & ~z13 & ~x6;
  assign z13 = ~x2 & x3;
  assign z01 = z13 | (~x5 & ~x6 & ~x7);
  assign z02 = (~x2 & x3) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & x2 & x3;
  assign z04 = x3 & (new_n79_ | ~x2);
  assign new_n79_ = ~x4 & ~x5 & x6 & ~x7;
  assign z05 = z13 | (x6 & ~x7 & ~x4 & x5);
  assign z06 = ~x6 & ~x5 & ~x4 & x3 & new_n82_ & x2;
  assign new_n82_ = ~x0 & ~x1;
  assign z07 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & new_n86_ & x6;
  assign new_n86_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = (~x2 & x3) | (new_n88_ & new_n82_ & x2 & ~x3);
  assign new_n88_ = ~x4 & ~x5 & x6 & x7;
  assign z10 = x7 & x6 & x5 & new_n90_ & ~x4;
  assign new_n90_ = x2 & ~x0 & x1;
  assign z11 = ~x2 & (x3 | (new_n92_ & x0 & x1 & ~x4));
  assign new_n92_ = x5 & x6 & x7;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n94_ & ~x3;
  assign new_n94_ = x2 & x0 & ~x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n90_ & x3;
  assign z17 = ~x5 & x4 & ~x3 & new_n97_ & ~x2;
  assign new_n97_ = x0 & ~x1;
  assign z18 = x3 & (~x2 | (new_n82_ & x4 & ~x5));
  assign z19 = ~x2 & (x3 | (~x0 & ~x1 & x4));
  assign z20 = ~x2 & (x3 | (new_n97_ & new_n101_ & ~x4));
  assign new_n101_ = ~x5 & ~x6;
  assign z22 = x7 & new_n103_ & x6;
  assign new_n103_ = ~x5 & ~x4 & ~x3 & ~x2 & x0 & ~x1;
  assign z24 = ~x2 & (new_n105_ | x3);
  assign new_n105_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & ~x7;
  assign z25 = ~x7 & x6 & new_n84_ & ~x5;
  assign z26 = (~x2 & x3) | (new_n88_ & x0 & x2 & ~x3);
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n90_ & ~x3;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n94_ & x3;
  assign z29 = x7 & new_n111_ & ~x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = (~x2 & x3) | (new_n88_ & x2 & ~x3 & x0 & x1);
  assign z31 = ~new_n114_ | (~z13 & x1);
  assign new_n114_ = (x5 | (~x2 & (x3 | ~x4))) & (~x2 | (~x0 & x3 & x4)) & (x3 | x4 | (x0 & ~x5 & ~x6));
  assign z32 = (x1 & (x2 | ~x3)) | new_n116_ | new_n117_ | (~x3 & (new_n118_ | x2));
  assign new_n116_ = ~x4 & (x2 | (x0 & ~x3));
  assign new_n117_ = x0 & (x2 | (~x3 & ~x5));
  assign new_n118_ = ~x0 & (x4 | x5 | ~x6 | x7);
  assign z33 = ~new_n120_ | ~x7 | x4 | ~x6;
  assign new_n120_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n122_ | new_n123_ | new_n124_ | (x3 & (~x5 | x6));
  assign new_n122_ = (x1 | x5) & (~x2 | ~x3);
  assign new_n123_ = x2 & ((~x3 & (x0 | ~x6)) | x4 | x7);
  assign new_n124_ = ~x2 & (~x0 | x3 | (~x4 & (~x6 | ~x7)));
  assign z35 = (x0 & (x2 | (~x3 & ~x5))) | ((x2 | ~x3) & (x1 | ~x4)) | (x2 & (~x3 | ~x5));
  assign z36 = (x1 & (~x2 | (x2 & ~x3))) | (~new_n127_ & ~x1) | (~new_n128_ & x2) | x3 | (~x0 & ~x2);
  assign new_n127_ = ~x5 & (x2 | x4);
  assign new_n128_ = ~x5 & x6 & ~x7 & ~x4 & (~x0 | x3);
  assign z37 = (~x1 & (~x2 | x5)) | (~x2 & (~x0 | x3)) | (~new_n130_ & x2);
  assign new_n130_ = x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z38 = (x3 & (x0 | ~x2)) | (x0 & (x2 | ~x4)) | (x2 & (~x3 | ~x4)) | x1 | (~x0 & ~new_n79_ & ~x2);
  assign z39 = (~new_n133_ & x2) | x4 | (~new_n134_ & ~x2);
  assign new_n133_ = ~x6 & ~x7 & x3 & x5;
  assign new_n134_ = x0 & ~x1 & ~x3 & ~x5 & x6 & x7;
  assign z40 = (x1 & (~x0 | ~x2)) | (x3 & (x0 | ~x2)) | new_n136_ | (~new_n137_ & x0) | (~x0 & x2 & ~x3);
  assign new_n136_ = ~x4 & ((~x2 & (x5 | (x0 & x6))) | (~x0 & (x2 | ~x6 | x7)));
  assign new_n137_ = (~x4 | (~x2 & x5)) & (~x2 | (~x5 & x6 & x7));
  assign z41 = x2 | (~x3 & (~x0 | ~x1));
  assign z42 = (~new_n140_ & (x2 | ~x3)) | (x2 & ~x3 & ~x5);
  assign new_n140_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = new_n142_ | new_n143_ | new_n144_ | new_n145_ | new_n146_;
  assign new_n142_ = x0 & ((x2 & x4) | (~x2 & ~x4 & x6 & ~x7));
  assign new_n143_ = x3 & (~x2 | (x1 & ~x5));
  assign new_n144_ = x1 & (x4 | (~x2 & ~x5));
  assign new_n145_ = ~x4 & ((x5 & (x6 | x7)) | (~x5 & ((~x0 & (x2 | ~x6)) | (x2 & (~x6 | ~x7)))) | (~x0 & ~x2 & x7));
  assign new_n146_ = x2 & ~x3 & x4;
  assign z44 = (x0 & (~new_n101_ | x4)) | x1 | x2 | x3 | (~x0 & ~x4);
  assign z45 = (x0 & (x2 | ~x3)) | (x2 & (new_n149_ | ~x1)) | (~x2 & ~new_n150_ & ~x3);
  assign new_n149_ = ~x4 & (x5 | x6);
  assign new_n150_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = ~new_n152_ | (~x4 & (x5 | (x6 & ~x7)));
  assign new_n152_ = ~x0 & x1 & ~x2 & ~x3;
  assign z47 = (new_n154_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n156_ | (~new_n155_ & (x0 | ~x1));
  assign new_n154_ = ~x4 & (x5 | (x1 & x6));
  assign new_n155_ = ~x4 & x6 & x7;
  assign new_n156_ = (~x0 | (x1 & x3 & x5)) & (x1 | (~x2 & ~x3 & ~x5));
  assign z49 = (~new_n101_ & ~x4) | ~new_n82_ | ~x2 | (x3 & x4);
  assign z50 = x2 | (~x3 & (~new_n159_ | x5 | x0 | x4));
  assign new_n159_ = x6 & x7;
  assign z51 = (~x1 & (~x3 | (x0 & x2))) | (new_n161_ & x0) | (~x0 & (~x3 | (~new_n162_ & x2)));
  assign new_n161_ = ~x4 & ((x2 & (x5 | x6)) | (~x3 & (x5 ? (~x6 | ~x7) : x6)));
  assign new_n162_ = ~x5 & ~x6 & ~x1 & ~x4;
  assign z52 = (~x0 & (x1 | ~x2)) | (x3 & (x0 | x4)) | (~x1 & ~x2) | (~new_n101_ & ~x4);
  assign z53 = (x1 & (x0 ? ~x3 : (x2 & x3))) | (~x2 & ~new_n166_ & ~x3) | (x2 & (~new_n165_ | (x0 & (~x3 | (~x1 & x3)))));
  assign new_n165_ = ((x1 & (x4 | ~x6)) | (x3 & (~x3 | x5))) & (~x3 | ((x1 | ~x4) & ((x6 & x7) | (x1 & (x4 | ~x5))))) & (x3 | x4 | ~x5);
  assign new_n166_ = ~x4 & x5 & x6 & x7;
  assign z54 = (~x3 & (new_n97_ | new_n169_ | (~new_n168_ & ~x2))) | (x2 & ~new_n170_ & x3);
  assign new_n168_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n169_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n170_ = ~x0 & x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z55 = (new_n149_ & ~x0) | (~x2 & (x0 | x3)) | ~x1 | (~new_n166_ & x0);
  assign z56 = x2 ? (~new_n92_ | x0 | x4 | (~x1 & x3)) : ~x3;
  assign z57 = (x0 & (x2 | ~x3)) | (~new_n166_ & x2) | (~new_n174_ & ~x3);
  assign new_n174_ = x1 & (x4 | ((~x6 | x7) & (x2 | ~x5)));
  assign z58 = (new_n149_ & ~x0) | (~new_n166_ & x0) | ~x1 | ~x2 | ~x3;
  assign z59 = (x2 & (new_n178_ | (~new_n177_ & x3))) | ((new_n179_ | new_n180_) & ~x3);
  assign new_n177_ = x0 & ~x1 & (~x0 | x4 | (~x5 & ~x6));
  assign new_n178_ = ~x0 & (x1 | x4 | ~x6 | ~x7);
  assign new_n179_ = x0 & (~x1 | ~x2 | (~x4 & (x5 | x6)));
  assign new_n180_ = ~x0 & (x4 | x5 | ~x6 | ~x7);
  assign z60 = ~new_n184_ | (~x0 & (new_n182_ | ~new_n183_));
  assign new_n182_ = x1 & (x2 | (~x3 & x5));
  assign new_n183_ = ((~x2 & x3) | (~x4 & x6 & x7)) & (x3 | x5) & (~x2 | (~x3 ^ x5));
  assign new_n184_ = (~x2 | ~x3 | (~x0 & ~x1)) & (~x0 | x3 | (x1 & x4));
  assign z61 = (~x1 & (~x2 | ~x3)) | new_n149_ | ~x0 | x1;
  assign z62 = new_n149_ | ~x0 | ~x1 | (x1 & x3);
  assign z14 = 1'b0;
  assign z21 = 1'b0;
  assign z23 = 1'b0;
  assign z48 = 1'b1;
  assign z16 = z13;
endmodule


