// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n82_, new_n84_, new_n86_, new_n88_, new_n93_, new_n95_,
    new_n99_, new_n101_, new_n103_, new_n105_, new_n108_, new_n111_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n131_, new_n132_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n142_, new_n144_,
    new_n149_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n167_, new_n168_,
    new_n169_;
  assign z00 = ~x4 & ~x5 & ~z13 & ~x6;
  assign z13 = ~x0 & ~x2;
  assign z01 = z13 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = z13 | (~x3 & ~x4 & new_n76_ & x5);
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z13 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z13 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z13 & ~x7;
  assign z06 = new_n82_ & ~x6;
  assign new_n82_ = ~x5 & ~x4 & x3 & x2 & ~x0 & ~x1;
  assign z08 = (~x0 & ~x2) | (new_n84_ & x0 & x1 & x2 & ~x3);
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z09 = ~x0 & (new_n86_ | ~x2);
  assign new_n86_ = ~x1 & ~x3 & ~x4 & ~x5 & x6 & x7;
  assign z10 = ~x0 & (~x2 | (new_n88_ & x1 & ~x4));
  assign new_n88_ = x5 & x6 & x7;
  assign z11 = ~x2 & (~x0 | (new_n88_ & x1 & ~x3 & ~x4));
  assign z12 = (~x0 & ~x2) | (new_n84_ & x0 & ~x1 & x2 & ~x3);
  assign z14 = ~x2 & (~x0 | (new_n88_ & ~x1 & x3 & ~x4));
  assign z15 = x7 & x6 & x5 & ~x4 & new_n93_ & x3;
  assign new_n93_ = x2 & ~x0 & x1;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x2 & (~x0 | (~x1 & x4 & ~x5));
  assign z18 = ~x5 & x4 & x3 & x2 & ~x0 & ~x1;
  assign z20 = ~x2 & (~x0 | (new_n99_ & ~x1 & ~x3));
  assign new_n99_ = ~x4 & ~x5 & ~x6;
  assign z21 = new_n101_ & ~x6;
  assign new_n101_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = ~x2 & (~x0 | (new_n103_ & ~x1 & ~x4));
  assign new_n103_ = ~x5 & x6 & x7;
  assign z26 = (~x0 & ~x2) | (new_n105_ & x0 & x2 & ~x3);
  assign new_n105_ = ~x4 & ~x5 & x6 & x7;
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n93_ & ~x3;
  assign z28 = x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & x3 & x2 & x0 & ~x1;
  assign z30 = (~x0 & ~x2) | (new_n105_ & x0 & x1 & x2 & ~x3);
  assign z31 = ~new_n111_ | (~z13 & x1);
  assign new_n111_ = (x5 | (~x2 & (~x0 | ~x4))) & (~x0 | (~x2 & (x4 | (~x5 & ~x6)))) & (~x2 | (x3 & x4));
  assign z32 = (x0 & (x2 | (x4 & ~x5))) | (~x3 & (~x0 | ~x4)) | ~new_n113_ | (~x0 & (~x2 | ~x4));
  assign new_n113_ = ~x1 & (x4 | (~x5 & ~x6));
  assign z33 = ~new_n115_ | ~x7 | x4 | ~x6;
  assign new_n115_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = (~x5 & (new_n117_ | ~new_n118_)) | new_n119_ | ~new_n120_;
  assign new_n117_ = x0 & (x2 | (~x4 & ~x7));
  assign new_n118_ = (x0 | (~x3 & x6)) & ~x1 & (x4 | x6);
  assign new_n119_ = (x4 | x7) & (~x0 | x5);
  assign new_n120_ = (x0 | x2) & (~x5 | (x3 & ~x6));
  assign z35 = ((x0 | x2) & (x1 | ~x4 | ~x5)) | (x2 & (x0 | ~x3));
  assign z36 = ((x0 | x2) & (x1 | x5)) | (~new_n123_ & x2) | (x0 & (x2 | ~x4));
  assign new_n123_ = ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x3 & (x2 | (x0 & ~x1))) | (~new_n125_ & (x0 ? (x3 & ~x5) : x2)) | (x5 & (x2 | (x0 & x1 & x3)));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (x0 | x2)) | (~x3 & (x2 | (x0 & ~x4))) | (x2 & (x0 | ~x4)) | (x0 & ~new_n127_ & ~x4);
  assign new_n127_ = ~x5 & ~x6;
  assign z39 = (~x0 & (~x2 | ~x5)) | ~new_n129_ | (x5 & (~new_n76_ | ~x3));
  assign new_n129_ = ~x4 & (x5 | (~x1 & ~x2 & x6 & x7));
  assign z40 = (x1 & (~x0 ^ ~x2)) | (~new_n132_ & x0) | (~new_n131_ & x2);
  assign new_n131_ = x0 ? (~x3 & ~x4 & ~x5 & x6 & x7) : (x3 & x4);
  assign new_n132_ = x4 ? x5 : (~x5 & (x2 | ~x6));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (x0 | x2)) | (~new_n76_ & (x0 ? x5 : x2)) | (~new_n135_ & ~x5);
  assign new_n135_ = (~x2 | (x0 & x3)) & (~x0 | (~x1 & x6 & x7));
  assign z43 = (~new_n137_ & ~x4) | (~new_n138_ & x0) | (x2 & ~new_n139_ & x4);
  assign new_n137_ = ((~x6 & ~x7) | (x0 ? ~x5 : ~x2)) & (x0 | ~x2 | x5) & (~x0 | ~x6 | x7);
  assign new_n138_ = ((~x1 & ~x2) | (~x4 & (x5 | x6))) & (~x1 | (x2 & ~x3) | x5);
  assign new_n139_ = ~x1 & x3;
  assign z44 = (~new_n127_ & (x0 | ~x4)) | ~x0 | (x0 & (x1 | x2 | x3 | x4));
  assign z45 = new_n142_ | x0 | (~x0 & (~x1 | ~x2));
  assign new_n142_ = ~x4 & (x5 | x6);
  assign z46 = x2 | (~new_n144_ & x0);
  assign new_n144_ = x5 & x6 & x7 & x2 & x3 & ~x4;
  assign z47 = (~new_n144_ & x0) | (x2 & (~x1 | (new_n142_ & ~x0)));
  assign z49 = x0 | (x2 & ((~new_n127_ & ~x4) | x1 | (x3 & x4)));
  assign z50 = ~x0 | x2 | (x0 & (~new_n103_ | ~x1 | ~x3 | x4));
  assign z51 = x0 ? (new_n149_ | ~x1 | (~x2 & x3)) : (x2 & (~new_n99_ | x1 | ~x3));
  assign new_n149_ = ~x4 & ((x2 & (x5 | x6)) | (~x5 & x6) | (x5 & (~x6 | ~x7)));
  assign z52 = (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x1 | ~x2)) | (~new_n127_ & ~x4) | (x3 & x4);
  assign z53 = new_n152_ | (~new_n153_ & ~x4) | (~new_n84_ & ~new_n154_) | ~new_n155_;
  assign new_n152_ = x1 & (x0 ? ~x3 : (x2 & x3));
  assign new_n153_ = ((~x5 & ~x6) | ((~x2 | x3) & (~x0 | x2 | ~x3))) & (~x0 | (x5 ? (x6 & x7) : ~x6));
  assign new_n154_ = (x1 | ~x2) & (~x0 | x3);
  assign new_n155_ = (~x0 | (x3 ? x1 : ~x2)) & (x1 | ~x2 | x3);
  assign z54 = (~x1 & (x0 | (x2 & x3))) | (~new_n84_ & (x2 ? ~x3 : x0)) | (new_n157_ & x2) | (x0 & x3);
  assign new_n157_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z55 = new_n159_ | (~x2 & (~x0 | ~x3)) | ~x1 | (x0 & ~new_n84_ & x2);
  assign new_n159_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = ~new_n161_ | ~x7 | ~x5 | ~x6;
  assign new_n161_ = x2 & ~x4 & ~x0 & (x1 | ~x3);
  assign z57 = (~x1 & (x0 | ~x3)) | new_n163_ | (~new_n164_ & ~x0) | (x0 & (x2 | ~x3));
  assign new_n163_ = ~x4 & ((x6 & ~x7) | (x0 & x5));
  assign new_n164_ = x2 & ~x4 & x5 & x6 & x7;
  assign z58 = (~new_n164_ & x0) | (x2 & ((new_n142_ & ~x0) | ~x1 | ~x3));
  assign z59 = x0 ? (new_n167_ | new_n168_ | ~new_n169_) : (~new_n86_ & x2);
  assign new_n167_ = (~x1 | ~x3) & (~x2 | (~x4 & x6));
  assign new_n168_ = x5 & (~x4 | (x1 & x3));
  assign new_n169_ = x1 ? (~x3 | (~x2 & ~x4 & x6 & x7)) : x3;
  assign z60 = x0 ? (x3 | ~x4 | (~x1 & (~x2 | ~x3))) : (x2 & (~new_n88_ | x1 | ~x3 | x4));
  assign z61 = new_n142_ | ~x0 | x1 | (~x1 & (~x2 | ~x3));
  assign z62 = new_n142_ | ~x0 | ~x1 | (x1 & x3);
  assign z07 = 1'b0;
  assign z19 = 1'b0;
  assign z23 = 1'b0;
  assign z48 = 1'b1;
  assign z24 = z13;
  assign z25 = z13;
  assign z29 = z13;
endmodule


