// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n81_, new_n84_, new_n85_, new_n87_, new_n89_, new_n92_,
    new_n94_, new_n96_, new_n98_, new_n103_, new_n105_, new_n108_,
    new_n112_, new_n114_, new_n115_, new_n123_, new_n125_, new_n126_,
    new_n128_, new_n130_, new_n132_, new_n134_, new_n136_, new_n138_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n147_, new_n149_,
    new_n151_, new_n152_, new_n155_, new_n156_, new_n157_;
  assign z00 = z23 | (new_n75_ & ~x4);
  assign z23 = ~x1 & ~x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z23 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z23 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z23 & ~x7;
  assign z04 = z23 | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z23 | (new_n81_ & x6 & ~x7);
  assign new_n81_ = ~x4 & x5;
  assign z06 = ~x1 & (~x2 | (new_n75_ & ~x4 & ~x0 & x3));
  assign z07 = ~x2 & (~x1 | (new_n84_ & new_n85_ & x5));
  assign new_n84_ = ~x0 & ~x3 & ~x4;
  assign new_n85_ = x6 & x7;
  assign z08 = (~x1 & ~x2) | (new_n87_ & x0 & x1 & x2 & ~x3);
  assign new_n87_ = new_n81_ & new_n85_;
  assign z09 = ~x1 & (~x2 | (new_n89_ & new_n84_));
  assign new_n89_ = new_n85_ & ~x5;
  assign z10 = (~x1 & ~x2) | (new_n87_ & ~x0 & x1 & x2);
  assign z11 = x7 & x6 & x5 & ~x4 & new_n92_ & ~x3;
  assign new_n92_ = ~x2 & x0 & x1;
  assign z12 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n96_ & x3;
  assign new_n96_ = ~x2 & ~x0 & x1;
  assign z15 = x7 & new_n98_ & x6;
  assign new_n98_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n92_ & x3;
  assign z18 = ~x1 & (~x2 | (x4 & ~x5 & ~x0 & x3));
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n96_ & ~x3;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n103_ & ~x3;
  assign new_n103_ = x0 & x2;
  assign z27 = (~x1 & ~x2) | (new_n105_ & x2 & ~x3 & ~x0 & x1);
  assign new_n105_ = ~x4 & ~x5 & x6 & ~x7;
  assign z28 = ~x1 & (~x2 | (new_n89_ & x0 & x3 & ~x4));
  assign z30 = (~x1 & ~x2) | (new_n108_ & x0 & x1 & x2 & ~x3);
  assign new_n108_ = new_n85_ & ~x4 & ~x5;
  assign z31 = (x2 & (x0 | ~x3 | ~x4)) | x1 | ~x2 | ~x5;
  assign z32 = x1 | (x2 & (x0 | ~x3 | ~x4));
  assign z33 = ~new_n112_ | ~x7 | x4 | ~x6;
  assign new_n112_ = new_n103_ & (~x1 | ~x3 | x5) & (x1 | ~x5);
  assign z34 = ~new_n114_ | (~x2 & (~x1 | ~x3));
  assign new_n114_ = (~x3 | (x5 & ~x6)) & new_n115_ & (x3 | (~x0 & ~x1 & ~x5 & x6));
  assign new_n115_ = ~x4 & ~x7;
  assign z35 = x1 | ~x2 | ~x3 | (x2 & (x0 | (x3 & (~x4 | ~x5))));
  assign z36 = x1 | (x2 & ((~new_n105_ & ~x3) | x0 | x3));
  assign z37 = (~x3 & (x2 | (~x0 & x1))) | (~new_n105_ & (x1 ? x3 : x2));
  assign z39 = x4 | z23 | ~x3 | x7 | ~x5 | x6;
  assign z40 = (~x0 & (x1 | ~x3 | ~x4)) | ~x2 | (x0 & (~new_n89_ | x3 | x4));
  assign z41 = x2 | (x1 & (~x0 | x3));
  assign z42 = (~x2 & (~x1 | ~x5)) | ~new_n123_ | (x5 & (x6 | x7));
  assign new_n123_ = ~x4 & (x5 | (x0 & ~x1 & new_n85_ & x3));
  assign z43 = (x1 & (x4 | (x3 & ~x5))) | ~new_n126_ | (~new_n125_ & ~x4);
  assign new_n125_ = x5 ? (~x6 & ~x7) : (x0 & x6 & x7);
  assign new_n126_ = (x2 | (x1 & ~x4 & x5)) & (~x4 | (~x0 & x3));
  assign z45 = new_n128_ | x0 | ~x1 | ~x2;
  assign new_n128_ = ~x4 & (x5 | x6);
  assign z46 = x2 | (x1 & ~new_n130_ & ~x2);
  assign new_n130_ = ~x0 & ~x3 & (x4 | (~x5 & (~x6 | x7)));
  assign z47 = x2 ? ((new_n128_ & ~x0) | ~x1 | (~new_n132_ & x0)) : x1;
  assign new_n132_ = x3 & ~x4 & new_n85_ & x5;
  assign z49 = (x0 & (~x1 | ~x3)) | (~new_n134_ & ~x1) | x1 | (x3 & x4) | (~x4 & x5);
  assign new_n134_ = x2 & (x4 | ~x6);
  assign z50 = ((x1 | x3) & x4) | (x5 & (x1 | ~x4)) | (~new_n136_ & x1) | ~x1 | (x0 & ~x3);
  assign new_n136_ = new_n85_ & ~x2;
  assign z51 = (~x1 & (x0 | ~x2)) | (~new_n138_ & ~x4) | (~x2 & (~x0 | x3)) | (~x0 & (x1 | ~x3 | x4));
  assign new_n138_ = ((~x5 & ~x6) | (x0 & ~x2)) & (x5 | ~x6) & (~x5 | (x6 & x7));
  assign z52 = (~new_n75_ & ~x4) | (~x0 & (x1 | ~x2 | (x3 & x4))) | (x0 & x3) | (~x2 & (~x1 | x3));
  assign z53 = ~new_n141_ | (~new_n87_ & (x1 ? (~x2 & ~x3) : x2));
  assign new_n141_ = (~x2 | ((~x0 | (x1 & x3)) & (x3 | (~new_n128_ & x1)) & (x0 | ~x1 | ~x3))) & (~x1 | (x3 ? ~new_n142_ : ~x0));
  assign new_n142_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (x1 & ((x0 & x3) | (~new_n144_ & ~x2))) | (x2 & (~new_n145_ | (x0 & (~x1 | x3))));
  assign new_n144_ = (x0 | ~new_n128_ | x3) & ((new_n81_ & new_n85_) | (~x0 & ~x3));
  assign new_n145_ = ((x6 & x7) | (x3 & (x4 | ~x5))) & (x3 | (~x4 & x5)) & (x1 | ~x3) & (x4 | x5 | ~x6);
  assign z55 = new_n147_ | ~x1 | (x0 & (x2 ? ~new_n87_ : ~x3));
  assign new_n147_ = ~x4 & ~new_n75_ & ~new_n103_;
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n149_ | (~x2 & (new_n81_ | ~x3));
  assign new_n149_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x3 & (x0 | ~x1)) | ~new_n151_ | ((new_n81_ | ~x1) & (x0 | ~x2));
  assign new_n151_ = new_n152_ & (x7 | (~x2 & (x4 | ~x6)));
  assign new_n152_ = (~x2 | (x5 & x6 & ~x0 & ~x4)) & (x0 | x2 | ~x3);
  assign z58 = (new_n128_ & ~x0) | (~new_n87_ & x0) | ~x1 | ~x2 | ~x3;
  assign z59 = (~new_n155_ & x2) | (x1 & ((x5 & (~x2 | ~x4)) | (~new_n157_ & ~x2)));
  assign new_n155_ = (~x0 | ((x1 | x3) & (x4 | ~x6))) & ((x0 & ~x1) | ~x3) & new_n156_ & (~x1 | (x0 & (x4 | ~x6)));
  assign new_n156_ = (~x5 | (x0 & x4)) & (x0 | (~x4 & x6 & x7));
  assign new_n157_ = new_n85_ & (~x0 | x3) & ~x4;
  assign z60 = x0 ? (~x1 | x3 | ~x4) : (~new_n87_ | x1 | ~x2 | ~x3);
  assign z61 = new_n128_ | ~x0 | x1 | ~x2 | ~x3;
  assign z62 = x1 ? (new_n128_ | ~x0 | x3) : x2;
  assign z14 = 1'b0;
  assign z17 = 1'b0;
  assign z19 = 1'b0;
  assign z20 = 1'b0;
  assign z21 = 1'b0;
  assign z22 = 1'b0;
  assign z29 = 1'b0;
  assign z44 = 1'b1;
  assign z48 = 1'b1;
  assign z24 = z23;
  assign z38 = z32;
endmodule


