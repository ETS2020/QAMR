// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n89_, new_n90_, new_n92_, new_n94_,
    new_n99_, new_n103_, new_n105_, new_n109_, new_n111_, new_n113_,
    new_n114_, new_n116_, new_n118_, new_n122_, new_n124_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n149_, new_n151_, new_n152_,
    new_n154_, new_n158_, new_n160_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n179_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = ~x1 & x2;
  assign z01 = z06 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z06 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (~x1 & x2) | (new_n81_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n81_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z10 = new_n85_ & x7;
  assign new_n85_ = x6 & x5 & ~x4 & x2 & ~x0 & x1;
  assign z11 = (~x1 & x2) | (new_n81_ & ~x2 & ~x3 & x0 & x1);
  assign z13 = (~x1 & x2) | (new_n81_ & ~x0 & x1 & ~x2 & x3);
  assign z14 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign new_n89_ = new_n90_ & ~x2;
  assign new_n90_ = x0 & ~x1;
  assign z15 = x2 & (~x1 | (new_n92_ & ~x0 & x3 & ~x4));
  assign new_n92_ = x5 & x6 & x7;
  assign z16 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & new_n89_ & x4;
  assign z19 = ~x1 & (x2 | (~x0 & ~x3 & x4));
  assign z20 = ~x6 & ~x5 & ~x4 & new_n89_ & ~x3;
  assign z21 = ~x1 & (x2 | (x0 & x3 & new_n99_ & ~x4));
  assign new_n99_ = ~x5 & ~x6;
  assign z22 = x7 & x6 & ~x5 & new_n89_ & ~x4;
  assign z23 = ~x1 & (x2 | (~x0 & x3 & x5));
  assign z24 = ~x1 & (new_n103_ | x2);
  assign new_n103_ = ~x0 & ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z25 = ~x7 & new_n105_ & x6;
  assign new_n105_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = x7 & x6 & new_n83_ & ~x5;
  assign z27 = x2 & (new_n103_ | ~x1);
  assign z29 = x7 & new_n109_ & ~x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z31 = new_n111_ | x1;
  assign new_n111_ = ~x2 & ((~x0 & (x3 | ~x4)) | (~x4 & (x5 | (x0 & x6))) | (x4 & ~x5));
  assign z32 = ~new_n114_ | (~new_n113_ & ~x4);
  assign new_n113_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n114_ = (x0 | (~x4 & (x2 | ~x3))) & ~x1 & ~x2 & (x2 | ~x4 | x5);
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = x1 & x2 & x0 & (~x3 | x5);
  assign z34 = ~new_n118_ | (x2 & (~x1 | ~x5));
  assign new_n118_ = x5 ? (~x6 & ~x7 & x3 & ~x4) : (new_n90_ & (x4 | (x6 & x7)));
  assign z35 = x1 | (~x2 & ((~x0 & x3) | ~x4 | (x0 & ~x5)));
  assign z36 = ~new_n89_ | ~x4 | x5;
  assign z37 = (~x0 & (x1 ? ~x3 : (~x2 & x5))) | (~x3 & (x1 ^ ~x2)) | (~new_n122_ & (x1 ? x3 : (~x2 & ~x5))) | (x1 & x3 & x5);
  assign new_n122_ = ~x4 & x6 & ~x7;
  assign z38 = x1 | (~new_n124_ & ~x2);
  assign new_n124_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x3 & ~x4 & ~x5 & x6 & ~x7));
  assign z39 = ~new_n126_ | (x2 & (~x1 | ~x5));
  assign new_n126_ = (~x5 | (x3 & ~x6 & ~x7)) & ~x4 & (x5 | (new_n90_ & x6 & x7));
  assign z40 = (~new_n130_ & x0) | (x2 & (~x0 | ~x1)) | ~new_n128_ | (x3 & (~x0 | x1));
  assign new_n128_ = new_n129_ & (~x5 | (~x1 & x4));
  assign new_n129_ = (x0 | (~x1 & (x4 | (x6 & ~x7)))) & (~x1 | (x6 & x7 & x2 & ~x4));
  assign new_n130_ = (~x4 | x5) & (x1 | x4 | ~x6);
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n133_ | (x2 & (~x1 | ~x5));
  assign new_n133_ = ~x4 & (x5 | (new_n90_ & x6 & x7)) & (~x5 | (~x6 & ~x7));
  assign z43 = new_n139_ | new_n135_ | ~new_n137_;
  assign new_n135_ = ~x5 & (~new_n136_ | (x3 & (x1 | (~x0 & ~x2))));
  assign new_n136_ = (x0 | (~x1 & (x2 | x4 | x6))) & (~x1 | (x2 & x6 & x7));
  assign new_n137_ = (new_n138_ | (~x6 & ~x7)) & (x2 | x4 | (x0 ? (~x6 | x7) : ~x7));
  assign new_n138_ = (~x1 | ~x2 | (x0 & ~x5)) & (x2 | x4 | ~x5);
  assign new_n139_ = x4 & (x1 | (~x0 & ~x2 & x3));
  assign z44 = ~new_n141_ | (x6 & (x0 | (~x2 & ~x4 & x5)));
  assign new_n141_ = ~new_n142_ & ~new_n143_ & ~new_n139_ & ~x2 & (new_n144_ | x5);
  assign new_n142_ = ~x4 & (~x0 | (~x2 & x5 & x7));
  assign new_n143_ = x0 & (x3 | x4 | x5);
  assign new_n144_ = (~x1 | (x6 & x7 & x2 & ~x3)) & (x0 | (~x1 & (x2 | ~x3)));
  assign z45 = (x0 & (x1 | ~x2)) | (x1 & (~x2 | (new_n146_ & x2))) | (~new_n147_ & ~x2);
  assign new_n146_ = ~new_n99_ & ~x4;
  assign new_n147_ = x6 & x7 & ~x4 & ~x5;
  assign z46 = (x0 & (x1 | ~x2)) | (x1 & x2) | (~x2 & (new_n149_ | ~x1 | x3));
  assign new_n149_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & ((new_n146_ & ~x0) | ~x2 | (~new_n151_ & x0))) | (~new_n152_ & ~x2);
  assign new_n151_ = new_n92_ & x3 & ~x4;
  assign new_n152_ = ~x0 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~new_n154_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n154_ = ~x2 & x3 & ~x0 & ~x1;
  assign z49 = ~x2 | (x1 & x2);
  assign z50 = x2 ? x1 : (~new_n147_ | (x0 & (~x1 | ~x3)));
  assign z51 = new_n158_ | (x1 & (~x0 | (~x2 & x3))) | (~x1 & ~x2 & (x0 | ~x3));
  assign new_n158_ = ~x4 & (((x5 | x6) & (~x1 ^ x2)) | (x1 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (x2 & (~x1 | (x1 & ~x4 & x6))) | ~new_n160_ | (~x4 & (x1 ? (x5 | (~x5 & x6)) : (~x2 & (x5 | x6))));
  assign new_n160_ = x1 ? (x0 & ~x3) : (x2 | (~x0 & x3));
  assign z53 = (~x3 & (~new_n162_ | (x0 & (x1 | x2)))) | (~x1 & (x2 | x3)) | (x3 & (new_n163_ | (~x0 & x2)));
  assign new_n162_ = x2 ? (new_n99_ | x4) : new_n81_;
  assign new_n163_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n165_ & ~x2) | (x1 & (new_n166_ | ~new_n167_));
  assign new_n165_ = (x3 | (x1 & (x0 | new_n99_ | x4))) & (~x0 | ~x3) & (new_n81_ | (~x0 & ~x3));
  assign new_n166_ = (~x6 | ~x7) & (x3 ? (~x4 & x5) : x2);
  assign new_n167_ = (~x2 | x3 | (~x4 & x5)) & (~x3 | (~x0 & (x4 | x5 | ~x6)));
  assign z55 = new_n169_ | (~x2 & (~x1 | (x0 & ~x3))) | (x0 & x1 & ~new_n81_ & x2);
  assign new_n169_ = ~x4 & ~new_n99_ & (~x2 | (~x0 & x1));
  assign z56 = new_n171_ | (~x2 & (~x1 | ~x3)) | (x1 & (x0 | (~new_n81_ & x2)));
  assign new_n171_ = ~x4 & ((~x2 & x5) | (x1 & x6 & ~x7));
  assign z57 = ~new_n173_ | (~x2 & ((~x0 & x3) | (~x4 & x5)));
  assign new_n173_ = ~new_n174_ & (~x0 | (~x2 & x3)) & x1 & (new_n81_ | ~x2);
  assign new_n174_ = ~x4 & ((x6 & ~x7) | (x0 & x5));
  assign z58 = ~new_n176_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n176_ = (x2 | (~x0 & ~x1)) & (new_n177_ | (~x0 & x1)) & (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign new_n177_ = ~x4 & x6 & x7;
  assign z59 = (x0 & (~x1 | (~x2 & ~x3))) | new_n179_ | (~new_n147_ & ~x2) | (x2 & (~x0 | ~x1 | x3));
  assign new_n179_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x3 & (x1 | ~x2)) | (x1 & (~x0 | ~x4)) | (~x1 & ~x2 & (~new_n92_ | x0 | x4));
  assign z62 = new_n146_ | ~x0 | ~x1 | x3;
  assign z09 = 1'b0;
  assign z12 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = ~z06;
  assign z18 = z06;
  assign z30 = z26;
endmodule


