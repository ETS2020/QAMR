// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n80_, new_n82_, new_n84_, new_n86_, new_n89_, new_n94_,
    new_n96_, new_n101_, new_n104_, new_n107_, new_n109_, new_n111_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n129_, new_n131_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n145_, new_n146_, new_n147_, new_n148_, new_n151_,
    new_n152_, new_n154_, new_n156_, new_n158_, new_n161_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n171_, new_n174_,
    new_n177_;
  assign z00 = (x2 & x3) | (new_n74_ & ~x4);
  assign new_n74_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~x7 & (~x2 | ~x3);
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = (x2 & x3) | (new_n80_ & x6 & ~x7);
  assign new_n80_ = ~x4 & x5;
  assign z07 = x7 & x6 & x5 & ~x4 & new_n82_ & ~x3;
  assign new_n82_ = ~x2 & ~x0 & x1;
  assign z08 = x2 & (x3 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x7 & new_n86_ & x6;
  assign new_n86_ = ~x4 & ~x3 & x2 & ~x0 & ~x1 & ~x5;
  assign z10 = x2 & (x3 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z11 = (x2 & x3) | (new_n89_ & x0 & x1 & ~x2 & ~x3);
  assign new_n89_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (x3 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z13 = x7 & x6 & x5 & ~x4 & new_n82_ & x3;
  assign z14 = x3 & (x2 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z16 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & new_n96_ & x4;
  assign new_n96_ = ~x2 & x0 & ~x1;
  assign z19 = (x2 & x3) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = ~x6 & ~x5 & ~x4 & new_n96_ & ~x3;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n96_ & x3;
  assign z22 = (x2 & x3) | (new_n101_ & x0 & ~x1 & ~x2);
  assign new_n101_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = (x2 & x3) | (new_n104_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n104_ = ~x4 & ~x5 & x6 & ~x7;
  assign z25 = (x2 & x3) | (new_n104_ & ~x2 & ~x3 & ~x0 & x1);
  assign z26 = x2 & (x3 | (new_n107_ & x0 & ~x4));
  assign new_n107_ = ~x5 & x6 & x7;
  assign z27 = x2 & (x3 | (new_n109_ & ~x0 & x1 & ~x4));
  assign new_n109_ = ~x5 & x6 & ~x7;
  assign z29 = x7 & new_n111_ & ~x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = x2 ? ~x3 : ~new_n115_;
  assign new_n115_ = (x0 | (~x3 & x4)) & (x4 | (~x5 & ~x6)) & ~x1 & (~x4 | x5);
  assign z32 = (~new_n117_ & ~x4) | (x2 & ~x3) | (~x2 & (~new_n119_ | (~new_n118_ & x4)));
  assign new_n117_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | ~x5);
  assign new_n118_ = x0 & x5;
  assign new_n119_ = ~x1 & (x0 | (~x3 & ~x5 & x6 & ~x7));
  assign z33 = ~new_n121_ | ~x7 | x4 | ~x6;
  assign new_n121_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = new_n123_ | new_n124_ | new_n125_ | new_n126_;
  assign new_n123_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n124_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n125_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n126_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = x2 | ~x4 | x1 | (~x0 & x3) | (x0 & ~x5);
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n129_ & ~x0) | x1 | x5;
  assign new_n129_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n131_)) | x2 | (~x1 & ~x3);
  assign new_n131_ = ~x4 & x6 & ~x7;
  assign z38 = new_n133_ | new_n134_ | x1 | x2;
  assign new_n133_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n134_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~new_n136_ | ~x3)) | ~new_n138_ | (~new_n137_ & ~x5);
  assign new_n136_ = ~x6 & ~x7;
  assign new_n137_ = x0 & ~x1 & x6 & x7;
  assign new_n138_ = ~x2 & ~x4;
  assign z40 = (~new_n140_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n141_ | (x3 & (~x0 | x2));
  assign new_n140_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n141_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | (~x3 & x5))) | (x5 & ~new_n136_ & (~x2 | ~x3)) | (~x5 & (x2 ? ~x3 : ~new_n137_));
  assign z43 = new_n145_ | (~new_n146_ & ~x4) | ~new_n148_ | (~new_n147_ & x4);
  assign new_n145_ = x1 & (x4 | (~x2 & ~x5));
  assign new_n146_ = (x0 | ((x5 | x6) & (x2 | ~x7))) & (~x5 | (~x6 & ~x7)) & (~x6 | x7 | ~x0 | x2);
  assign new_n147_ = ~x2 & (x0 | ~x3);
  assign new_n148_ = (x5 | ((x0 | (~x2 & ~x3)) & (~x2 | (x6 & x7)))) & (~x2 | (~x3 & (~x5 | (~x6 & ~x7))));
  assign z44 = x2 ? ~x3 : ((x0 & (~new_n74_ | x4)) | x1 | x3 | (~x0 & ~x4));
  assign z45 = new_n152_ | (~new_n151_ & x1) | x0 | (~x1 & (~new_n107_ | ~new_n138_));
  assign new_n151_ = x2 & ~x3;
  assign new_n152_ = ~x4 & (x5 | (x1 & x6));
  assign z46 = new_n154_ | x2 | x3 | x0 | ~x1;
  assign new_n154_ = ~x4 & (x5 | (x6 & ~x7));
  assign z48 = ~x3 | (~new_n156_ & ~x2);
  assign new_n156_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = new_n158_ | ~new_n151_ | x0 | x1;
  assign new_n158_ = ~x4 & (x5 | x6);
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (x2 & ~x3) | (~new_n101_ & ~x2);
  assign z51 = (x0 & (~x1 | x3)) | (x2 & (new_n80_ | ~x0)) | new_n161_ | (~x0 & (x1 | ~x3));
  assign new_n161_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x0 & (x5 | x6)));
  assign z52 = (~x3 & (new_n158_ | (~x1 & ~x2) | (~x0 & (x1 | ~x2)))) | (~x2 & (new_n158_ | (x3 & (x0 | x1))));
  assign z53 = (~x3 & ((x0 & (x1 | x2)) | (x2 & (new_n158_ | ~x1)) | (~new_n89_ & ~x2))) | (~x2 & x3 & (new_n158_ | ~x1));
  assign z54 = (~x3 & ((~new_n166_ & ~x2) | new_n165_ | new_n167_)) | (~x2 & ~new_n168_ & x3);
  assign new_n165_ = x0 & ~x1;
  assign new_n166_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n167_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n168_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n170_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n171_ & ~x3);
  assign new_n170_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n171_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n89_ & ~x3) | (~x2 & (new_n154_ | ~x1 | ~x3));
  assign z57 = new_n174_ | (x0 & (x2 | ~x3)) | (~new_n89_ & x2) | ~x1 | (~x0 & x3);
  assign new_n174_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~x3 | (~x2 & (~new_n107_ | x0 | x1 | x4));
  assign z59 = (~new_n101_ & (~x0 | ~x2)) | new_n177_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n177_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = x3 ? ~x2 : (x0 ? (~x1 | ~x4) : (~new_n84_ | x1 | x2 | x4));
  assign z62 = (x0 & ~x3 & (new_n158_ | ~x1)) | ~x0 | x3;
  assign z06 = 1'b0;
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = 1'b1;
  assign z47 = new_n152_ | (~new_n151_ & x1) | x0 | (~x1 & (~new_n107_ | ~new_n138_));
endmodule


