// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n92_, new_n94_,
    new_n97_, new_n99_, new_n101_, new_n104_, new_n107_, new_n110_,
    new_n113_, new_n115_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n131_,
    new_n133_, new_n135_, new_n136_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n154_, new_n155_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n164_, new_n167_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n181_, new_n184_,
    new_n185_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = x7 & x6 & new_n81_ & x5;
  assign new_n81_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x2 & (x3 | (new_n83_ & x0 & x1 & ~x4));
  assign new_n83_ = x5 & x6 & x7;
  assign z09 = x2 & (x3 | (new_n85_ & ~x0 & ~x1 & ~x4));
  assign new_n85_ = ~x5 & x6 & x7;
  assign z10 = x7 & x6 & new_n87_ & x5;
  assign new_n87_ = ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = (x2 & x3) | (new_n89_ & x0 & x1 & ~x2 & ~x3);
  assign new_n89_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (x3 | (new_n83_ & x0 & ~x1 & ~x4));
  assign z13 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x3 & (x2 | (new_n83_ & x0 & x1 & ~x4));
  assign z17 = (x2 & x3) | (new_n97_ & ~x2 & x4 & ~x5);
  assign new_n97_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & new_n99_ & ~x2;
  assign new_n99_ = ~x0 & ~x1;
  assign z20 = (x2 & x3) | (x0 & ~x1 & ~x2 & new_n101_ & ~x3 & ~x4);
  assign new_n101_ = ~x5 & ~x6;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n97_ & ~x2;
  assign z22 = (x2 & x3) | (new_n104_ & x0 & ~x1 & ~x2);
  assign new_n104_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x5 & x3 & new_n99_ & ~x2;
  assign z24 = (x2 & x3) | (new_n99_ & ~x2 & ~x3 & new_n107_ & ~x4 & ~x5);
  assign new_n107_ = x6 & ~x7;
  assign z25 = ~x7 & x6 & new_n81_ & ~x5;
  assign z26 = new_n110_ & x7;
  assign new_n110_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & x6 & new_n87_ & ~x5;
  assign z29 = x7 & new_n113_ & ~x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & new_n115_ & x6;
  assign new_n115_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n117_ | (~x0 & (x3 | ~x4));
  assign new_n117_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = x2 ? ~x3 : (new_n119_ | new_n120_ | ~new_n121_);
  assign new_n119_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n120_ = x4 & (~x0 | ~x5);
  assign new_n121_ = ~x1 & (x0 | (~x3 & ~x5 & x6 & ~x7));
  assign z33 = ~x2 | (x2 & ~new_n123_ & ~x3);
  assign new_n123_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = new_n125_ | new_n126_ | new_n127_ | new_n128_;
  assign new_n125_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n126_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n127_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n128_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = x2 | ~x4 | x1 | (~x0 & x3) | (x0 & ~x5);
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n131_ & ~x0) | x1 | x5;
  assign new_n131_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n133_)) | x2 | (~x1 & ~x3);
  assign new_n133_ = ~x4 & x6 & ~x7;
  assign z38 = new_n135_ | (x2 & ~x3) | (~x2 & (new_n136_ | x1));
  assign new_n135_ = ~x4 & ((~x2 & x5) | (x0 & (~x3 | (~x2 & x6))));
  assign new_n136_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~x3 | x6 | x7)) | ~new_n138_ | (~x5 & (~new_n97_ | ~x6 | ~x7));
  assign new_n138_ = ~x2 & ~x4;
  assign z40 = x2 ? new_n143_ : (new_n140_ | new_n141_ | ~new_n142_);
  assign new_n140_ = x0 & (x4 ? ~x5 : x6);
  assign new_n141_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n142_ = ~x1 & (x4 | ~x5);
  assign new_n143_ = ~x3 & (~x0 | x4 | x5 | ~x6 | ~x7);
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n146_ | (x2 & (x3 | ~x5));
  assign new_n146_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = x2 ? new_n150_ : (new_n148_ | new_n149_);
  assign new_n148_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n149_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n150_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = (~new_n152_ & ~x2) | (~x3 & (x2 | (~x4 & x5)));
  assign new_n152_ = (~x3 | (~x0 & ~x4)) & (x4 | (x0 & ~x5)) & ~x1 & (~x0 | (~x4 & ~x5 & ~x6));
  assign z45 = (x0 & (~x2 | (x2 & ~x3))) | (~new_n154_ & ~x2) | (~new_n155_ & ~x3);
  assign new_n154_ = ~x1 & ~x4 & (x4 | (~x5 & (x0 | (x6 & x7))));
  assign new_n155_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign z46 = ~new_n157_ | (~x4 & (new_n107_ | x5));
  assign new_n157_ = ~x0 & x1 & ~x2 & ~x3;
  assign z47 = new_n160_ | (~new_n159_ & x1) | x0 | (~x1 & (~new_n85_ | ~new_n138_));
  assign new_n159_ = x2 & ~x3;
  assign new_n160_ = ~x4 & (x5 | (x1 & x6));
  assign z48 = ~new_n162_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n162_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = new_n164_ | ~new_n99_ | ~new_n159_;
  assign new_n164_ = ~x4 & (x5 | x6);
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n104_ & ~x2) | (x2 & ~x3);
  assign z51 = (~x2 & (~new_n167_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | ~x1 | (new_n164_ & x2)));
  assign new_n167_ = (x0 | ~x1) & (x4 | ((x5 | ~x6) & (~x5 | (x6 & x7)) & (x0 | (~x5 & ~x6))));
  assign z52 = (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x1 | (~x2 & ~x3))) | new_n164_ | (x2 & x3);
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n164_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n89_ & ~x3);
  assign z54 = (~x3 & (new_n97_ | new_n172_ | (~new_n171_ & ~x2))) | (~x2 & ~new_n173_ & x3);
  assign new_n171_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n172_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n173_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n175_ | ~x1 | (x2 & x3) | (x0 & ~new_n176_ & ~x3);
  assign new_n175_ = ~x4 & (x5 | x6) & (~x0 | x3);
  assign new_n176_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n178_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n179_ & x2);
  assign new_n178_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n179_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = new_n181_ | (x0 & (x2 | ~x3)) | (~new_n89_ & x2) | ~x1 | (~x0 & x3);
  assign new_n181_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~x3 | (~x2 & (~new_n85_ | x0 | x1 | x4));
  assign z59 = (~new_n184_ & x0) | (~new_n185_ & ~x3) | (~new_n104_ & (x3 ? ~x2 : ~x0));
  assign new_n184_ = (x1 | (x2 & x3)) & (x3 | (x2 & (x4 | ~x6)));
  assign new_n185_ = (x0 | ~x1 | ~x2) & (x4 | ~x5);
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n83_ | x1 | x2 | x4));
  assign z62 = ~x0 | x3 | (x0 & (new_n164_ | ~x1));
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z61 = 1'b1;
  assign z28 = z06;
endmodule


