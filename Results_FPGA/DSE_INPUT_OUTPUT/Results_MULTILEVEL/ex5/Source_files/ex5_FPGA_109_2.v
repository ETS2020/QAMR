// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n80_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n96_, new_n101_, new_n111_, new_n114_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n162_, new_n164_, new_n166_, new_n168_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n188_, new_n189_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n202_, new_n203_, new_n204_, new_n206_,
    new_n209_;
  assign z00 = ~x6 & (new_n74_ | x2);
  assign new_n74_ = ~x4 & ~x5;
  assign z01 = ~x7 & ~x6 & ~x2 & ~x5;
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = ~x7 & new_n80_ & x6;
  assign new_n80_ = ~x4 & x5;
  assign z06 = x2 & ~x6;
  assign z07 = x7 & x6 & x5 & ~x4 & new_n83_ & ~x3;
  assign new_n83_ = ~x2 & ~x0 & x1;
  assign z08 = x2 & (~x6 | (new_n85_ & x0 & x1 & ~x3));
  assign new_n85_ = ~x4 & x5 & x7;
  assign z09 = x2 & (~x6 | (new_n87_ & ~x0 & ~x1 & ~x3));
  assign new_n87_ = ~x4 & ~x5 & x7;
  assign z10 = x2 & (~x6 | (new_n89_ & new_n85_));
  assign new_n89_ = ~x0 & x1;
  assign z11 = (x2 & ~x6) | (new_n91_ & x0 & x1 & ~x2 & ~x3);
  assign new_n91_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n93_ & ~x3;
  assign new_n93_ = x2 & x0 & ~x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n83_ & x3;
  assign z14 = (x2 & ~x6) | (new_n91_ & new_n96_ & ~x2 & x3);
  assign new_n96_ = x0 & ~x1;
  assign z15 = x2 & (~x6 | (new_n85_ & ~x0 & x1 & x3));
  assign z16 = (x2 & ~x6) | (new_n91_ & x0 & x1 & ~x2 & x3);
  assign z17 = (x2 & ~x6) | (new_n96_ & ~x2 & x4 & ~x5);
  assign z18 = x6 & ~x5 & x4 & x3 & new_n101_ & x2;
  assign new_n101_ = ~x0 & ~x1;
  assign z19 = (x2 & ~x6) | (new_n101_ & ~x2 & ~x3 & x4);
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n96_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n96_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n96_ & ~x2;
  assign z23 = (x2 & ~x6) | (new_n101_ & ~x2 & x3 & x5);
  assign z24 = (x2 & ~x6) | (new_n101_ & ~x2 & ~x3 & new_n74_ & x6 & ~x7);
  assign z25 = (x2 & ~x6) | (new_n89_ & ~x2 & ~x3 & new_n74_ & x6 & ~x7);
  assign z26 = x2 & (~x6 | (new_n87_ & x0 & ~x3));
  assign z27 = ~x7 & new_n111_ & x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n93_ & x3;
  assign z29 = x7 & new_n114_ & ~x6;
  assign new_n114_ = ~x5 & ~x3 & ~x2 & ~x0 & ~x1 & ~x4;
  assign z30 = x2 & (~x6 | (new_n87_ & x0 & x1 & ~x3));
  assign z31 = new_n118_ | (~new_n117_ & x6);
  assign new_n117_ = (~x0 | (~x2 & (x2 | x4))) & ~x1 & (~x2 | (x3 & x4 & x5));
  assign new_n118_ = ~x2 & ((~x0 & (x3 | ~x4)) | x1 | (x4 & ~x5) | (~x4 & x5));
  assign z32 = new_n120_ | new_n121_ | (~new_n122_ & x2) | x1 | (~new_n123_ & ~x2);
  assign new_n120_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n121_ = x0 & (x2 | (~x2 & ~x4 & x6));
  assign new_n122_ = x4 & x6;
  assign new_n123_ = (~x4 | (x0 & x5)) & (x4 | ~x5) & (x0 | (~x3 & (x4 | (x6 & ~x7))));
  assign z33 = ~x2 | (~new_n125_ & x6);
  assign new_n125_ = x0 & ~x4 & (x1 | ~x5) & x7 & (~x1 | ~x3 | x5);
  assign z34 = new_n127_ | new_n128_ | new_n129_ | (~new_n130_ & ~x2);
  assign new_n127_ = x1 & (x2 ? x6 : ~x5);
  assign new_n128_ = (x2 ? x6 : x5) & (x4 | x7);
  assign new_n129_ = x6 & (x5 | (x2 & (x0 | x3)));
  assign new_n130_ = x5 ? x3 : (x0 & (x4 | (x6 & x7)));
  assign z35 = (x0 & (x2 | ~x5)) | ~new_n132_ | (x2 & (~x3 | ~x5 | ~x6));
  assign new_n132_ = ~x1 & x4 & (x0 | x2 | ~x3);
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n134_ & ~x0) | x1 | x5;
  assign new_n134_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = new_n136_ | new_n137_ | new_n138_;
  assign new_n136_ = (~x3 | x5) & (x2 ? x6 : ~x0);
  assign new_n137_ = (x4 | x7) & (x2 ? x6 : (x3 & ~x5));
  assign new_n138_ = ~x2 & (x3 ? (x5 ? x1 : ~x6) : ~x1);
  assign z38 = new_n120_ | new_n140_ | (new_n141_ & x0) | x1 | (~new_n142_ & ~x0);
  assign new_n140_ = x2 & (x0 | ~x4);
  assign new_n141_ = ~x4 & (x5 | x6);
  assign new_n142_ = x6 & (x2 | (~x5 & ~x7 & ~x3 & ~x4));
  assign z39 = (x4 & (~x2 | x6)) | (~new_n145_ & x6) | (~x2 & ~new_n144_ & ~x6);
  assign new_n144_ = x3 & x5 & ~x7;
  assign new_n145_ = x0 & ~x1 & ~x2 & ~x5 & x7;
  assign z40 = new_n147_ | new_n148_ | new_n149_ | new_n150_ | (new_n80_ & ~x2);
  assign new_n147_ = x1 & (~x2 | (~x0 & x6));
  assign new_n148_ = x3 & (x0 ? (x2 & x6) : ~x2);
  assign new_n149_ = x0 & (x2 ? (x6 & (x4 | x5 | ~x7)) : (x4 ? ~x5 : x6));
  assign new_n150_ = ~x0 & (x2 ? (x6 & (~x3 | ~x4)) : (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | x6)) | (~x2 & ~x6 & (~x5 | x7)) | (~new_n153_ & x6);
  assign new_n153_ = x0 & ~x1 & (~x2 | x3) & ~x5 & x7;
  assign z43 = ((new_n155_ | new_n156_) & ~x2) | (x6 & (new_n158_ | (~new_n157_ & x2)));
  assign new_n155_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n156_ = ~x4 & ((x5 & x7) | (~x0 & (x7 | (~x5 & ~x6))));
  assign new_n157_ = (~x1 | (x0 & ~x3)) & (x0 | (x3 & x4)) & (~x0 | (~x4 & ~x5 & x7));
  assign new_n158_ = ~x4 & (x5 | (x0 & ~x7));
  assign z44 = (~new_n160_ & ~x2) | (x6 & (x0 | new_n80_ | x2));
  assign new_n160_ = ((~x4 & x5) | (~x1 & (x0 | ~x3))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x1 & (~x2 | ~x4)) | x0 | ~x6 | (~x1 & (~new_n162_ | x2 | x4));
  assign new_n162_ = ~x5 & x7;
  assign z46 = new_n164_ | ~new_n89_ | x2 | x3;
  assign new_n164_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (~x2 | (~x0 & ~x4))) | ~new_n166_ | ((x0 | ~x1) & (x4 | ~x7));
  assign new_n166_ = (x1 | (~x0 & ~x2 & ~x5)) & x6 & (~x0 | (x2 & x3 & x5));
  assign z48 = ~new_n168_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n168_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (x6 & (~new_n101_ | x3 | ~x4));
  assign z50 = ~new_n171_ | (x0 & (~x1 | ~x3));
  assign new_n171_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z51 = ~new_n174_ | (~x2 & ((x0 & (~x1 | x3)) | new_n173_ | (~x0 & (x1 | ~x3))));
  assign new_n173_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x0 & (x5 | x6)));
  assign new_n174_ = (x0 | (~x2 & (~x1 | ~x6))) & (~x2 | (x1 & x4 & x6));
  assign z52 = (~new_n176_ & ~x2) | (x6 & (new_n89_ | (x2 & (x3 | ~x4))));
  assign new_n176_ = (~x0 | (x1 & ~x3)) & (x0 | (~x1 & x3)) & (x4 | (~x5 & (~x6 | (x0 & x5))));
  assign z53 = new_n178_ | (~new_n181_ & ~x2) | (x6 & (new_n179_ | ~new_n180_));
  assign new_n178_ = x1 & ((x0 & ~x2 & ~x3) | (~x0 & x2 & x3 & x6));
  assign new_n179_ = x0 & (x3 ? ~x1 : x2);
  assign new_n180_ = ((x1 & x4) | (x3 ? (x5 & x7) : ~x2)) & (~x3 | (x4 ? x1 : x2));
  assign new_n181_ = x3 ? (x1 & (x4 | ~x5)) : (~x4 & x5 & x6 & x7);
  assign z54 = (~x2 & (new_n184_ | ~new_n185_)) | (x6 & (new_n183_ | ~new_n186_));
  assign new_n183_ = x0 & (~x1 | x3);
  assign new_n184_ = ~x3 & (~x1 | (~x0 & ~x4 & (x5 | x6)));
  assign new_n185_ = (~x0 | ~x3) & ((~x0 & ~x3) | (~x4 & x5 & x6 & x7));
  assign new_n186_ = ((x5 & x7) | (x3 ? x4 : ~x2)) & (~x2 | (x3 ? x1 : ~x4));
  assign z55 = (~x1 & (~x2 | x6)) | (~new_n188_ & x6) | (~new_n189_ & ~x2);
  assign new_n188_ = (x4 | (x0 & x2)) & (~x0 | ~x2 | (~x4 & x5 & x7));
  assign new_n189_ = (~x0 | x3) & (x4 | ~x5);
  assign z56 = (x0 & (~x2 | x6)) | ~new_n191_ | (~x1 & (~x2 | (x3 & x6)));
  assign new_n191_ = x2 ? (~x6 | (~x4 & x5 & x7)) : (x3 & (x4 | (~x5 & (~x6 | x7))));
  assign z57 = new_n193_ | new_n194_ | ~new_n195_ | (~new_n89_ & ~x3);
  assign new_n193_ = (x0 | ~x2) & (~x1 | (~x4 & x5));
  assign new_n194_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n195_ = (x0 | x2 | ~x3) & (~x2 | (x5 & x6 & ~x0 & ~x4));
  assign z58 = (x6 & (~new_n198_ | (~new_n197_ & x2))) | (~x2 & (~new_n199_ | ~new_n200_));
  assign new_n197_ = x1 & (x0 | x4);
  assign new_n198_ = x3 & (~x0 | (~x4 & x5 & x7));
  assign new_n199_ = ~x0 & ~x1 & x3;
  assign new_n200_ = ~x4 & ~x5 & x6 & x7;
  assign z59 = (~new_n202_ & x0) | new_n204_ | (x6 & (new_n203_ | (~new_n87_ & ~x0)));
  assign new_n202_ = (x1 | (x2 & (x3 | ~x6))) & (x2 | x3) & (~x2 | x4 | ~x6);
  assign new_n203_ = x2 & ((x1 & (x3 | ~x4)) | (~x0 & (x1 | x3)));
  assign new_n204_ = ~x2 & (x4 | x5 | ~x6 | ~x7);
  assign z60 = (x2 & (~x6 | (~x0 & ~x3))) | ~new_n206_ | (x3 & (x0 | ~x2));
  assign new_n206_ = x0 ? (x1 & x4) : (x5 & x6 & x7 & ~x1 & ~x4);
  assign z61 = ~x2 | (x6 & (~x0 | x1 | ~x4 | (~x1 & ~x3)));
  assign z62 = ~new_n209_ | (~z06 & (~x0 | ~x1));
  assign new_n209_ = (x2 | (~x3 & (x4 | ~x5))) & (~x6 | (x4 & (~x1 | ~x3)));
endmodule


