// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n80_, new_n82_, new_n84_, new_n85_, new_n87_, new_n89_,
    new_n90_, new_n93_, new_n101_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n114_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n146_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n160_, new_n162_,
    new_n164_, new_n166_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n186_, new_n188_, new_n190_,
    new_n192_, new_n194_;
  assign z00 = new_n74_ | (~x2 & ~x7);
  assign new_n74_ = ~x4 & ~x5 & ~x6;
  assign z01 = ~x7 & (~x2 | (~x5 & ~x6));
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & x2 & x3;
  assign z04 = ~x7 & (~x2 | (x3 & ~x4 & ~x5 & x6));
  assign z05 = ~x7 & (new_n80_ | ~x2);
  assign new_n80_ = ~x4 & x5 & x6;
  assign z06 = (~x2 & ~x7) | (new_n82_ & ~x0 & ~x1 & x2);
  assign new_n82_ = x3 & ~x4 & ~x5 & ~x6;
  assign z07 = x7 & new_n84_ & x6;
  assign new_n84_ = x5 & ~x4 & ~x3 & new_n85_ & ~x2;
  assign new_n85_ = ~x0 & x1;
  assign z08 = (~x2 & ~x7) | (new_n87_ & x0 & x1 & x2 & ~x3);
  assign new_n87_ = ~x4 & x5 & x6 & x7;
  assign z09 = (~x2 & ~x7) | (new_n90_ & new_n89_ & x2 & ~x3);
  assign new_n89_ = ~x0 & ~x1;
  assign new_n90_ = x6 & x7 & ~x4 & ~x5;
  assign z10 = x7 & x6 & x5 & ~x4 & new_n85_ & x2;
  assign z11 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = (~x2 & ~x7) | (new_n87_ & x0 & ~x1 & x2 & ~x3);
  assign z13 = ~x2 & (~x7 | (new_n80_ & ~x0 & x1 & x3));
  assign z14 = ~x2 & (~x7 | (new_n80_ & x0 & ~x1 & x3));
  assign z15 = (~x2 & ~x7) | (new_n87_ & new_n85_ & x2 & x3);
  assign z16 = ~x2 & (~x7 | (new_n80_ & x0 & x1 & x3));
  assign z17 = ~x2 & (~x7 | (x0 & ~x1 & x4 & ~x5));
  assign z18 = (~x2 & ~x7) | (new_n101_ & ~x0 & ~x1 & x2);
  assign new_n101_ = x3 & x4 & ~x5;
  assign z19 = x7 & x4 & ~x3 & new_n89_ & ~x2;
  assign z20 = ~x2 & (~x7 | (new_n74_ & x0 & ~x1 & ~x3));
  assign z21 = x7 & new_n105_ & ~x6;
  assign new_n105_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = ~x2 & (~x7 | (new_n107_ & x0 & ~x1));
  assign new_n107_ = ~x4 & ~x5 & x6;
  assign z23 = ~x2 & (~x7 | (new_n89_ & new_n109_));
  assign new_n109_ = x3 & x5;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n111_ & ~x3;
  assign new_n111_ = x0 & x2;
  assign z27 = ~x7 & (~x2 | (new_n107_ & ~x0 & x1 & ~x3));
  assign z28 = x7 & new_n114_ & x6;
  assign new_n114_ = ~x5 & ~x4 & x3 & x2 & x0 & ~x1;
  assign z29 = ~x2 & (~x7 | (new_n74_ & ~x0 & ~x1 & ~x3));
  assign z30 = (~x2 & ~x7) | (new_n90_ & x0 & x1 & x2 & ~x3);
  assign z31 = new_n118_ | (x2 & (~new_n89_ | ~x3 | ~x4 | ~x5));
  assign new_n118_ = x7 & (~new_n119_ | (~x0 & (~x4 | (~x2 & x3))));
  assign new_n119_ = (x4 | (~x5 & ~x6)) & ~x1 & (x2 | ~x4 | x5);
  assign z32 = (x7 & (~new_n122_ | (~x0 & (~x2 | ~x4)))) | (~new_n121_ & x2);
  assign new_n121_ = new_n89_ & x3 & x4;
  assign new_n122_ = ~x1 & (x2 | ~x4 | x5) & (x4 | (x3 & ~x5 & ~x6));
  assign z33 = ~new_n124_ | ~x7 | ~new_n111_ | (~x1 & x5) | (x1 & x3 & ~x5);
  assign new_n124_ = ~x4 & x6;
  assign z34 = ~new_n126_ | ((x7 | (x2 & ~x3)) & (x1 | x5));
  assign new_n126_ = (new_n127_ | ~x2) & (~x7 | (x0 & (x4 | x6)));
  assign new_n127_ = (~x3 | (x5 & ~x6)) & (x3 | (~x0 & x6)) & ~x4 & ~x7;
  assign z35 = (~x2 & (~x7 | (~x0 & x3))) | ((x0 | x2) & ~x5) | ~new_n129_ | (x0 & (x2 | ~x7));
  assign new_n129_ = ~x1 & x4 & (~x2 | x3);
  assign z36 = ~new_n131_ | ((x2 | x7) & (x1 | x5));
  assign new_n131_ = (~x7 | (x0 & ~x2 & x4)) & (~x2 | (~x4 & x6 & ~x0 & ~x3));
  assign z37 = (~x1 & (~x3 | (~x2 & ~x5))) | (~new_n134_ & x2) | (~new_n133_ & ~x2);
  assign new_n133_ = x0 & x7 & (~x1 | ~x3);
  assign new_n134_ = x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z38 = (x0 & (x2 | ~x7)) | (~x3 & (~x0 | ~x4)) | ~new_n136_ | (~x0 & (~x2 | ~x4));
  assign new_n136_ = ~new_n137_ & ~x1;
  assign new_n137_ = ~x4 & (x5 | x6);
  assign z39 = (x4 & (x2 | x7)) | new_n139_ | (x2 & (~new_n109_ | x6 | x7));
  assign new_n139_ = x7 & (~x0 | x1 | x5 | ~x6);
  assign z40 = ~new_n141_ | (x1 & (x2 ? ~x0 : x7));
  assign new_n141_ = (~x3 | (x0 ? ~x2 : (x2 | ~x7))) & ~new_n142_ & (new_n143_ | ~x0) & (x0 | ~x2 | x3);
  assign new_n142_ = ~x4 & ((~x0 & (x2 | x7)) | (~x2 & x7 & (x5 | x6)));
  assign new_n143_ = (~x4 | (~x2 & (x5 | ~x7))) & (~x2 | (~x5 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x7 | (x1 & x3) | ~x0 | x2;
  assign z42 = (x4 & (x2 | x7)) | new_n139_ | (x2 & (x7 ? ~x3 : ~new_n146_));
  assign new_n146_ = x5 & ~x6;
  assign z43 = ~new_n148_ | (x3 & ((~x0 & ~x2) | (x1 & ~x5)));
  assign new_n148_ = (new_n150_ | x4) & (~x1 | (x2 & ~x4)) & (x2 | x7) & (~x2 | new_n149_ | ~x4);
  assign new_n149_ = ~x0 & x3;
  assign new_n150_ = (x0 | (x2 & x5)) & (x5 | (x7 & (~x2 | x6))) & (~x5 | (x2 & ~x6 & ~x7));
  assign z44 = ~new_n152_ | (x0 & (x4 | x6));
  assign new_n152_ = (new_n153_ | x4) & (~x1 | (x2 & ~x4)) & ~x2 & ~x3 & (x2 | x7);
  assign new_n153_ = (x0 | (x2 & x5)) & (x5 | x7) & (~x5 | (x2 & ~x6 & ~x7));
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | ~new_n155_ | (x5 & (~x1 | ~x4));
  assign new_n155_ = ~x0 & (x1 | (~x2 & ~x4 & x6 & x7));
  assign z46 = ~x7 | x2 | x3 | ~x1 | new_n157_ | x0;
  assign new_n157_ = ~x4 & x5;
  assign z47 = ~new_n159_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n159_ = ((~x0 & ~x1) | x2) & (new_n160_ | (~x0 & x1)) & (~x0 | (x1 & x3 & x5)) & (x1 | (~x2 & ~x5));
  assign new_n160_ = ~x4 & x6 & x7;
  assign z48 = x2 | (~new_n162_ & x7);
  assign new_n162_ = ~x0 & ~x1 & x3 & (x4 | (~x5 ^ x6));
  assign z49 = (~new_n164_ & ~x4) | ((x0 | x1) & x2) | (x0 & (~x1 | ~x3)) | ~x2 | (x3 & x4);
  assign new_n164_ = ~x5 & (~x2 | ~x6);
  assign z50 = ~new_n166_ | (x0 & (~x1 | ~x3));
  assign new_n166_ = (~x4 | (x2 & ~x3)) & (~x5 | (x2 & x4)) & ~x2 & (x2 | (x6 & x7));
  assign z51 = ~new_n169_ | (~x0 & (~new_n168_ | (x1 & (x2 | x7))));
  assign new_n168_ = (~x2 | (x3 & ~x4)) & ((~new_n137_ & (x2 | x3)) | ~x7);
  assign new_n169_ = ~new_n170_ & (~x0 | ((x1 | (~x2 & (x2 | ~x7))) & (x2 | ~x3 | ~x7)));
  assign new_n170_ = ~x4 & (x2 ? (x5 | x6) : (x7 & (x5 ^ x6)));
  assign z52 = ((new_n85_ | new_n157_) & (x2 | x7)) | (~new_n173_ & x2) | (~new_n172_ & x7);
  assign new_n172_ = (~new_n124_ | x0) & (x2 | ((~x0 | (x1 & ~x3)) & ~new_n107_ & (x0 | x3)));
  assign new_n173_ = (x4 | ~x6) & (~x3 | (~x0 & (x0 | ~x4)));
  assign z53 = ~new_n175_ | (x1 & (x0 ? (~x3 & x7) : (x2 & x3)));
  assign new_n175_ = x2 ? (new_n176_ & (~x0 | (x1 & x3))) : ~new_n177_;
  assign new_n176_ = ((x3 & x5) | (x1 & (x4 | ~x6))) & ((x6 & x7) | (x1 & (x4 | ~x5))) & (x3 | x4 | ~x5) & (x1 | ~x4);
  assign new_n177_ = x7 & (x3 ? (~x1 | (~x4 & (x5 | x6))) : (x4 | ~x5 | ~x6));
  assign z54 = ~new_n180_ | (~x2 & ((~new_n179_ & ~x3) | ~x7 | (~new_n80_ & x3)));
  assign new_n179_ = x1 & (~new_n137_ | x0);
  assign new_n180_ = (x1 | (~x0 & (~x2 | ~x3))) & new_n181_ & (new_n80_ | (~x0 & (~x2 | x3)));
  assign new_n181_ = (x4 | ((~x5 | x7) & (~x3 | (~x5 ^ x6)))) & (~x0 | ~x3) & (x3 | x7);
  assign z55 = new_n184_ | (~new_n183_ & x0) | (~x1 & (x2 | (~x2 & x7)));
  assign new_n183_ = x2 ? new_n87_ : (x3 | ~x7);
  assign new_n184_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : x7);
  assign z56 = (x0 & (x2 | x7)) | ~new_n186_ | (~x1 & (x2 ? x3 : x7));
  assign new_n186_ = x2 ? new_n87_ : (~x7 | (~new_n157_ & x3));
  assign z57 = (x0 & (x2 | (~x2 & ~x3 & x7))) | ~new_n188_ | (~x1 & (x2 ? ~x3 : x7));
  assign new_n188_ = x2 ? new_n87_ : (~x7 | (~new_n157_ & ~new_n149_));
  assign z58 = (~new_n190_ & x2) | (x7 & (~x3 | (~x2 & (~new_n89_ | ~new_n107_))));
  assign new_n190_ = (~new_n137_ | x0) & x1 & x3 & (new_n87_ | ~x0);
  assign z59 = (x2 & ((~x0 & (x1 | x3)) | (x1 & (new_n124_ | x3)))) | ~new_n192_ | (x0 & ((~x2 & (~x1 | ~x3)) | (~x1 & (new_n124_ | ~x3))));
  assign new_n192_ = ~new_n157_ & (new_n90_ | (x1 ? x2 : x0));
  assign z60 = (~new_n194_ & (x2 | x7)) | (~x2 & x3 & x7) | (~x0 & x2 & (~x3 | ~x7));
  assign new_n194_ = x0 ? (x1 & ~x3 & x4) : (x5 & x6 & ~x1 & ~x4);
  assign z61 = (x1 & (x2 | x7)) | (~x1 & (x2 ? ~x3 : x7)) | (x2 & (new_n137_ | ~x0));
  assign z62 = ((new_n137_ | ~x0) & (x2 | (x1 & x7))) | (~x1 & (x2 | (~x2 & x7))) | (x1 & x3 & (x2 | x7));
  assign z24 = 1'b0;
  assign z25 = 1'b0;
endmodule


