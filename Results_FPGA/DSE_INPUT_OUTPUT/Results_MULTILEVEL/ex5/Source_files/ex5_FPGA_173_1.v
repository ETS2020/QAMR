// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n93_, new_n98_,
    new_n101_, new_n104_, new_n107_, new_n110_, new_n112_, new_n114_,
    new_n116_, new_n117_, new_n121_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n145_,
    new_n147_, new_n149_, new_n152_, new_n155_, new_n157_, new_n159_,
    new_n164_;
  assign z00 = ~x4 & ~x5 & ~z09 & ~x6;
  assign z09 = ~x0 & x2;
  assign z01 = ~x5 & ~x6 & ~z09 & ~x7;
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z09 & ~x7;
  assign z03 = z09 | (x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z09 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z09 & ~x7;
  assign z07 = x7 & x6 & new_n81_ & x5;
  assign new_n81_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & new_n83_ & x6;
  assign new_n83_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z11 = (~x0 & x2) | (new_n85_ & x0 & x1 & ~x2 & ~x3);
  assign new_n85_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & new_n87_ & x6;
  assign new_n87_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = ~x0 & (x2 | (new_n89_ & x1 & x3 & ~x4));
  assign new_n89_ = x5 & x6 & x7;
  assign z14 = (~x0 & x2) | (new_n85_ & x0 & ~x1 & ~x2 & x3);
  assign z16 = (~x0 & x2) | (new_n85_ & x0 & x1 & ~x2 & x3);
  assign z17 = ~x5 & new_n93_ & x4;
  assign new_n93_ = ~x2 & x0 & ~x1;
  assign z19 = ~x0 & (x2 | (~x1 & ~x3 & x4));
  assign z20 = ~x6 & ~x5 & ~x4 & new_n93_ & ~x3;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n93_ & x3;
  assign z22 = (~x0 & x2) | (new_n98_ & x0 & ~x1 & ~x2);
  assign new_n98_ = x6 & x7 & ~x4 & ~x5;
  assign z23 = ~x0 & (x2 | (~x1 & x3 & x5));
  assign z24 = ~x0 & (x2 | (new_n101_ & ~x1 & ~x3 & ~x4));
  assign new_n101_ = ~x5 & x6 & ~x7;
  assign z25 = ~x7 & x6 & new_n81_ & ~x5;
  assign z26 = x2 & (~x0 | (new_n104_ & ~x3 & ~x4));
  assign new_n104_ = ~x5 & x6 & x7;
  assign z28 = x2 & (~x0 | (new_n104_ & ~x1 & x3 & ~x4));
  assign z29 = x7 & new_n107_ & ~x6;
  assign new_n107_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x2 & (~x0 | (new_n104_ & x1 & ~x3 & ~x4));
  assign z31 = ~new_n110_ | (~x0 & (x3 | ~x4));
  assign new_n110_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (x1 & (x0 | (~x0 & ~x2))) | (x4 & (x0 ? ~x5 : ~x2)) | ~new_n112_ | (x5 & (x0 ? ~x4 : ~x2));
  assign new_n112_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (~x3 & x6 & ~x7));
  assign z33 = ~x2 | (~new_n114_ & x0);
  assign new_n114_ = ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = (~new_n116_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n117_ : ~x2));
  assign new_n116_ = x3 & ~x4 & ~x6 & ~x7;
  assign new_n117_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (~x2 & (x1 | ~x4 | (~x0 & x3)));
  assign z36 = ~new_n93_ | ~x4 | x5;
  assign z37 = (~x0 & (x2 | ~x3 | x5)) | (x2 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n121_)) | (~x1 & ~x3);
  assign new_n121_ = ~x4 & x6 & ~x7;
  assign z38 = (~new_n124_ & ~x4) | ~new_n123_ | (~x0 & (~new_n101_ | x3 | x4));
  assign new_n123_ = ~x1 & ~x2;
  assign new_n124_ = ~x5 & (~x0 | (x3 & ~x6));
  assign z39 = (~new_n126_ & (x0 ? x5 : ~x2)) | (~x2 & (x4 | (~x0 & ~x5))) | (x0 & (x4 | (~new_n127_ & ~x5)));
  assign new_n126_ = x3 & ~x6 & ~x7;
  assign new_n127_ = x6 & x7 & ~x1 & ~x2;
  assign z40 = (x3 & (~x0 ^ x2)) | (~new_n129_ & x0) | (~x2 & (x1 | (new_n130_ & ~x0)));
  assign new_n129_ = (~x4 | (~x2 & x5)) & (x4 | (~x5 & (x2 | ~x6))) & (~x2 | (~x5 & x6 & x7));
  assign new_n130_ = ~x4 & (x5 | ~x6 | x7);
  assign z41 = x0 ? ((~x1 & (~x3 | ~x5)) | x2 | (x1 & x3)) : ~x2;
  assign z42 = new_n133_ | (x5 & (x6 | x7)) | x4 | (~x5 & (~new_n134_ | ~x6 | ~x7));
  assign new_n133_ = ~x0 & (x2 | ~x5);
  assign new_n134_ = ~x1 & (~x2 | x3);
  assign z43 = (~x5 & (new_n136_ | new_n137_)) | new_n138_ | new_n139_ | new_n140_;
  assign new_n136_ = ~x2 & (x1 | (~x0 & (x3 | (~x4 & ~x6))));
  assign new_n137_ = x0 & ((x1 & x3) | (x2 & (~x6 | ~x7)));
  assign new_n138_ = (x0 ? x5 : ~x2) & (x4 ? x1 : x7);
  assign new_n139_ = ~x2 & ((~x0 & x3 & x4) | (~x4 & x5 & x6));
  assign new_n140_ = x0 & (x4 ? x2 : (x6 & (x5 | ~x7)));
  assign z44 = (x0 & (~new_n142_ | x4)) | ~new_n123_ | x3 | (~x0 & ~x4);
  assign new_n142_ = ~x5 & ~x6;
  assign z45 = x0 | (~x2 & (~new_n104_ | x1 | x4));
  assign z46 = x0 | (~x2 & (new_n145_ | ~x1 | x3));
  assign new_n145_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~x0 & (x1 | x2 | x5)) | ~new_n147_ | (x0 & (~x1 | ~x2 | ~x3 | ~x5));
  assign new_n147_ = ~x4 & x6 & x7;
  assign z48 = ~new_n149_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n149_ = ~x0 & ~x1 & ~x2 & x3;
  assign z50 = (~new_n98_ & ~x2) | (x0 & (~x1 | x2 | ~x3));
  assign z51 = new_n152_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n152_ = ~x4 & (((x5 | x6) & (~x0 ^ x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n142_ & (x0 | (~x0 & ~x2))) | (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x2 | (~x2 & (x1 | ~x3))));
  assign z53 = x3 ? (new_n155_ | (~x1 & (x0 | ~x2))) : ((~new_n85_ & ~x2) | (x0 & (x1 | x2)));
  assign new_n155_ = ~x4 & ((x0 & (x5 ? (~x6 | ~x7) : x6)) | (~x2 & (x5 | x6)));
  assign z54 = (~x1 & (x0 | ~x3)) | (~x0 & (x2 | (new_n157_ & ~x3))) | (x0 & x3) | (~new_n85_ & (x0 | x3));
  assign new_n157_ = ~x4 & (x5 | x6);
  assign z55 = (~x2 & (new_n157_ | (x0 & ~x3))) | ~x1 | (~new_n159_ & x2);
  assign new_n159_ = x0 & ~x4 & x5 & x6 & x7;
  assign z56 = x0 | (~x2 & (new_n145_ | ~x1 | ~x3));
  assign z57 = (x0 & (x2 | ~x3)) | (~x2 & (new_n145_ | ~x1 | (~x0 & x3)));
  assign z58 = (~x3 & (x0 | ~x2)) | (~new_n147_ & (~x2 | (x0 & x2))) | (x0 & (~x2 | (x2 & (~x1 | ~x5)))) | (~x2 & (x1 | x5));
  assign z59 = (~x2 & (~new_n98_ | (x0 & (~x1 | ~x3)))) | new_n164_ | (x2 & (~x0 | (~x1 & ~x3) | (x1 & x3)));
  assign new_n164_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n89_ | x1 | x2 | x4));
  assign z61 = (~x1 & (~x2 | (x0 & ~x3))) | (~x0 & ~x2) | (x0 & (new_n157_ | x1));
  assign z62 = (~x1 & (x0 | ~x2)) | (~x0 & ~x2) | (x0 & (new_n157_ | (x1 & x3)));
  assign z06 = 1'b0;
  assign z10 = 1'b0;
  assign z18 = 1'b0;
  assign z27 = 1'b0;
  assign z49 = 1'b1;
  assign z15 = z09;
endmodule


