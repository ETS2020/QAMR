// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n78_, new_n83_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n96_, new_n98_, new_n101_, new_n104_,
    new_n109_, new_n111_, new_n113_, new_n115_, new_n117_, new_n120_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n134_, new_n136_, new_n138_, new_n140_,
    new_n142_, new_n143_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n154_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n163_, new_n165_, new_n167_, new_n169_, new_n173_, new_n174_,
    new_n177_, new_n181_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = z06 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = z06 | (new_n78_ & new_n76_ & x5);
  assign new_n78_ = ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (x2 & x3) | (new_n84_ & new_n83_ & ~x2 & ~x3);
  assign new_n83_ = ~x0 & x1;
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x2 & (x3 | (new_n88_ & ~x0 & ~x1 & ~x4));
  assign new_n88_ = ~x5 & x6 & x7;
  assign z10 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = x7 & x6 & x5 & ~x4 & new_n92_ & ~x3;
  assign new_n92_ = ~x2 & x0 & x1;
  assign z12 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x3 & (new_n98_ | x2);
  assign new_n98_ = x0 & ~x1 & ~x4 & x5 & x6 & x7;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n92_ & x3;
  assign z17 = (x2 & x3) | (new_n101_ & ~x2 & x4 & ~x5);
  assign new_n101_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = z06 | (new_n104_ & x0 & new_n78_ & ~x5 & ~x6);
  assign new_n104_ = ~x1 & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n101_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n101_ & ~x2;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n109_ & x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x2 & x3) | (new_n83_ & ~x2 & ~x3 & new_n111_ & ~x4 & ~x5);
  assign new_n111_ = x6 & ~x7;
  assign z26 = new_n113_ & x7;
  assign new_n113_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = x2 & (new_n115_ | x3);
  assign new_n115_ = ~x0 & x1 & ~x4 & ~x5 & x6 & ~x7;
  assign z29 = (x2 & x3) | (new_n117_ & ~x0 & ~x1 & ~x2 & ~x3);
  assign new_n117_ = ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x7 & x6 & new_n86_ & ~x5;
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | x2 | (~new_n120_ & ~x2);
  assign new_n120_ = (x4 | (~x5 & (~x0 | ~x6))) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n123_ | (~new_n122_ & ~x4);
  assign new_n122_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n123_ = x2 ? x3 : (~x1 & (~x4 | x5) & (x0 | (~x3 & ~x4)));
  assign z33 = ~new_n126_ | ~x7 | ~new_n125_ | ~x0 | (~x1 & x5);
  assign new_n125_ = x2 & ~x3;
  assign new_n126_ = ~x4 & x6;
  assign z34 = new_n128_ | new_n129_ | new_n130_ | new_n131_;
  assign new_n128_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n129_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n130_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n131_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = x2 ? ~x3 : (x1 | (~x0 & x3) | ~x4 | (x0 & ~x5));
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n134_ & ~x0) | x1 | x5;
  assign new_n134_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n136_)) | x2 | (~x1 & ~x3);
  assign new_n136_ = ~x4 & x6 & ~x7;
  assign z38 = new_n138_ | ~new_n104_ | (~x0 & (~new_n78_ | ~new_n111_ | x5));
  assign new_n138_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign z39 = (x5 & (~x3 | x6 | x7)) | ~new_n140_ | (~x5 & (~new_n101_ | ~x6 | ~x7));
  assign new_n140_ = ~x2 & ~x4;
  assign z40 = (~new_n142_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n143_ | (x3 & (~x0 | x2));
  assign new_n142_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n143_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = (~x1 & (~x3 | (~x2 & ~x5))) | (x2 & ~x3) | (~x2 & (~x0 | (x1 & x3)));
  assign z42 = (x4 & (~x2 | (~x3 & x5))) | (x5 & ~new_n76_ & (~x2 | ~x3)) | (~new_n146_ & ~x5);
  assign new_n146_ = x2 ? x3 : (x0 & ~x1 & x6 & x7);
  assign z43 = x2 ? ~new_n150_ : (new_n148_ | new_n149_);
  assign new_n148_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n149_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n150_ = ~x3 & (x3 | ((~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & x6 & x7))));
  assign z44 = x2 ? ~x3 : (new_n148_ | ~new_n152_);
  assign new_n152_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x0 & (~x2 | ~x3)) | (x2 & ~x3 & (new_n154_ | ~x1)) | (~new_n155_ & ~x2);
  assign new_n154_ = ~x4 & (x5 | x6);
  assign new_n155_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n157_ | ~x1 | x3));
  assign new_n157_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = new_n159_ | (~new_n125_ & x1) | x0 | (~x1 & (~new_n88_ | ~new_n140_));
  assign new_n159_ = ~x4 & (x5 | (x1 & x6));
  assign z48 = ~new_n161_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n161_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (~x3 & (new_n163_ | x0 | (x2 & (new_n126_ | x1))));
  assign new_n163_ = ~x4 & x5;
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n165_ & ~x2) | (~x3 & (new_n163_ | x2));
  assign new_n165_ = ~x4 & ~x5 & x6 & x7;
  assign z51 = (~x2 & (~new_n167_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | (x2 & (new_n154_ | ~x1))));
  assign new_n167_ = (x0 | ~x1) & (x4 | ((x0 | (~x5 & ~x6)) & (x5 | ~x6) & (~x5 | (x6 & x7))));
  assign z52 = (~new_n169_ & ~x2) | new_n83_ | new_n163_ | (x2 & (new_n126_ | x3));
  assign new_n169_ = (~x0 | (x1 & ~x3)) & (x0 | (x3 & (x4 | ~x6))) & (x4 | x5 | ~x6);
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n154_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n84_ & ~x3);
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n154_ & ~x0))) | (~new_n84_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n173_ | ~x1 | (x2 & x3) | (x0 & ~new_n174_ & ~x3);
  assign new_n173_ = ~x4 & (~x0 | x3) & (x5 | x6);
  assign new_n174_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n84_ & ~x3) | (~x2 & (new_n157_ | ~x1 | ~x3));
  assign z57 = new_n177_ | (x0 & (x2 | ~x3)) | (~new_n84_ & x2) | ~x1 | (~x0 & x3);
  assign new_n177_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~new_n161_ | ~new_n165_;
  assign z59 = (x0 & ((~x1 & (~x2 | ~x3)) | (~x3 & (new_n154_ | ~x2)))) | (~new_n165_ & (x3 ? ~x2 : ~x0)) | (x2 & ~x3 & ~x0 & x1);
  assign z60 = (~x3 & (x0 ? (~x1 | ~x4) : ~new_n181_)) | (~x2 & ((x0 & ~x1) | x3));
  assign new_n181_ = (~x1 | (~x2 & ~x5)) & x5 & x6 & x7 & ~x4 & (~x2 | ~x5);
  assign z62 = new_n154_ | ~x0 | ~x1 | x3;
  assign z18 = 1'b0;
  assign z61 = ~z06;
  assign z15 = z06;
  assign z28 = z06;
endmodule


