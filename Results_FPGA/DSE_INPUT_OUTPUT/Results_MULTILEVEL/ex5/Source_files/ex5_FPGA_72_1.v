// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n80_, new_n81_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n93_, new_n94_, new_n96_, new_n98_, new_n104_, new_n106_,
    new_n109_, new_n113_, new_n116_, new_n118_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n131_, new_n133_, new_n134_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n152_, new_n153_,
    new_n155_, new_n158_, new_n159_, new_n162_, new_n164_, new_n165_,
    new_n167_, new_n170_, new_n173_, new_n174_, new_n176_;
  assign z00 = z13 | (new_n75_ & ~x4);
  assign z13 = ~x2 & x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z13 | (~x5 & ~x6 & ~x7);
  assign z02 = (~x2 & x3) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & x2 & x3;
  assign z04 = x3 & (new_n80_ | ~x2);
  assign new_n80_ = new_n81_ & ~x4 & ~x5;
  assign new_n81_ = x6 & ~x7;
  assign z05 = z13 | (new_n81_ & ~x4 & x5);
  assign z06 = x3 & (~x2 | (~x0 & ~x1 & new_n75_ & ~x4));
  assign z07 = x7 & x6 & new_n85_ & x5;
  assign new_n85_ = ~x4 & ~x3 & new_n86_ & ~x2;
  assign new_n86_ = ~x0 & x1;
  assign z08 = (~x2 & x3) | (new_n88_ & x0 & x1 & x2 & ~x3);
  assign new_n88_ = ~x4 & x5 & x6 & x7;
  assign z09 = x7 & new_n90_ & x6;
  assign new_n90_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x7 & x6 & x5 & ~x4 & new_n86_ & x2;
  assign z11 = ~x2 & (x3 | (new_n93_ & x0 & x1 & ~x4));
  assign new_n93_ = new_n94_ & x5;
  assign new_n94_ = x6 & x7;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n96_ & ~x3;
  assign new_n96_ = x2 & x0 & ~x1;
  assign z15 = x7 & new_n98_ & x6;
  assign new_n98_ = x5 & ~x4 & x3 & new_n86_ & x2;
  assign z17 = ~x2 & (x3 | (x0 & ~x1 & x4 & ~x5));
  assign z18 = x3 & (~x2 | (~x0 & ~x1 & x4 & ~x5));
  assign z19 = ~x2 & (x3 | (~x0 & ~x1 & x4));
  assign z20 = ~x2 & (x3 | (x0 & ~x1 & new_n75_ & ~x4));
  assign z22 = ~x2 & (x3 | (new_n104_ & x0 & ~x1 & ~x4));
  assign new_n104_ = ~x5 & x6 & x7;
  assign z24 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n85_ & ~x5;
  assign z26 = (~x2 & x3) | (new_n109_ & x0 & x2 & ~x3);
  assign new_n109_ = new_n94_ & ~x4 & ~x5;
  assign z27 = (~x2 & x3) | (new_n80_ & new_n86_ & x2 & ~x3);
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n96_ & x3;
  assign z29 = ~x2 & (new_n113_ | x3);
  assign new_n113_ = ~x0 & ~x1 & ~x4 & ~x5 & ~x6 & x7;
  assign z30 = (~x2 & x3) | (new_n109_ & x0 & x1 & x2 & ~x3);
  assign z31 = (~x5 & (x2 | x4)) | ~new_n116_ | (x2 & (x0 | ~x3 | ~x4));
  assign new_n116_ = (x4 | (new_n75_ & x0)) & ~z13 & ~x1;
  assign z32 = ~new_n118_ | (~z13 & x1);
  assign new_n118_ = (x4 | (~x2 & (~x0 | x3))) & (~x0 | (~x2 & (x3 | x5))) & (x3 | (~x2 & (x0 | (new_n81_ & ~x4 & ~x5))));
  assign z33 = x2 ? ~new_n120_ : ~x3;
  assign new_n120_ = x0 & ~x4 & (x1 | ~x5) & new_n94_ & (~x1 | ~x3 | x5);
  assign z34 = (~new_n124_ & (~x2 | ~x3)) | ~new_n122_ | (~new_n125_ & x2);
  assign new_n122_ = (x2 | (~new_n123_ & x0 & ~x3)) & (~x3 | (x5 & ~x6));
  assign new_n123_ = ~x4 & (~x6 | ~x7);
  assign new_n124_ = ~x1 & ~x5;
  assign new_n125_ = ~x4 & ~x7 & (x3 | (~x0 & x6));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x2 & x3);
  assign z36 = (~new_n124_ & (x2 | ~x3)) | (~x2 & ~x3 & (~x0 | ~x4)) | (x2 & (~new_n81_ | x4 | x0 | x3));
  assign z37 = (~new_n80_ & x2) | (~x3 & (~x0 | ~x1 | x2));
  assign z38 = (x0 & (x2 | (~x3 & ~x4))) | (x1 & (x2 | ~x3)) | (x2 & (~x3 | ~x4)) | (~x0 & ~new_n80_ & ~x3);
  assign z39 = ~new_n131_ | (x2 & (x6 | x7 | ~x3 | ~x5));
  assign new_n131_ = ~x4 & (x2 | (new_n104_ & x0 & ~x1 & ~x3));
  assign z40 = (x1 & (~x0 | ~x2)) | (x3 & (x0 | ~x2)) | new_n134_ | (~new_n133_ & x0) | (~x0 & x2 & ~x3);
  assign new_n133_ = (~x4 | (~x2 & x5)) & (~x2 | (new_n94_ & ~x5));
  assign new_n134_ = ~x4 & ((~x2 & (x5 | (x0 & x6))) | (~x0 & (x2 | ~x6 | x7)));
  assign z41 = x2 | x3 | ~x0 | ~x1;
  assign z42 = (~new_n137_ & (x2 | ~x3)) | (x2 & ~x3 & ~x5);
  assign new_n137_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = (~new_n139_ & ~x3) | (~new_n141_ & x2);
  assign new_n139_ = ((~x1 & ~x2) | (~x4 & (x0 | x5))) & (new_n140_ | x4) & (~x1 | x2 | x5);
  assign new_n140_ = (x0 | ((x5 | x6) & ~x7)) & (~x5 | (~x6 & ~x7)) & (~x0 | x2 | ~x6 | x7);
  assign new_n141_ = new_n142_ & (~x1 | (~x4 & (~x0 | ~x3 | x5)));
  assign new_n142_ = x0 ? ((~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x6 & x7))) : (x4 | (x5 & ~x7 & (~x3 | ~x6)));
  assign z44 = (~new_n144_ & ~x3) | (~new_n147_ & x2);
  assign new_n144_ = ~new_n145_ & (~x6 | (~x0 & (x4 | ~x5))) & new_n146_ & (~x0 | (~x4 & ~x5));
  assign new_n145_ = (x1 | x2) & (x4 | (~x0 & ~x5));
  assign new_n146_ = (~x1 | x2 | x5) & (x4 | (x0 & (~x5 | ~x7)));
  assign new_n147_ = (x0 | x4 | (x5 & ~x7)) & ~x0 & ~x3 & (~x1 | ~x4);
  assign z45 = ~new_n149_ | (~z13 & x0);
  assign new_n149_ = x2 ? (x1 & (new_n75_ | x4)) : (x3 | (new_n104_ & ~x1 & ~x4));
  assign z46 = ~new_n86_ | x2 | x3 | (~x4 & (new_n81_ | x5));
  assign z47 = new_n152_ | (~x3 & (new_n153_ | x0));
  assign new_n152_ = x2 & ((~x0 & ~new_n75_ & ~x4) | ~x1 | (~new_n88_ & x0));
  assign new_n153_ = ~x2 & (~new_n104_ | x1 | x4);
  assign z49 = (x2 & (x1 | (~x4 & x6))) | ~new_n155_ | x0 | ~x2;
  assign new_n155_ = x4 ? ~x3 : ~x5;
  assign z50 = (x3 & (~x2 | x4)) | (x5 & (~x2 | ~x4)) | x0 | x2 | (~x2 & (~new_n94_ | x4));
  assign z51 = (~x1 & (~x3 | (x0 & x2))) | (new_n159_ & x0) | (~x0 & (~x3 | (~new_n158_ & x2)));
  assign new_n158_ = new_n75_ & ~x1 & ~x4;
  assign new_n159_ = ~x4 & ((x2 & (x5 | x6)) | (~x3 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x0 & (x1 | ~x2)) | (x3 & (x0 | x4)) | (~x1 & ~x2) | (~new_n75_ & ~x4);
  assign z53 = (x1 & (x0 ? ~x3 : (x2 & x3))) | (x2 & (~new_n162_ | (x0 & (~x3 | (~x1 & x3))))) | (~x2 & ~new_n88_ & ~x3);
  assign new_n162_ = ((x1 & (x4 | ~x6)) | (x3 & (~x3 | x5))) & (~x3 | ((x1 | ~x4) & ((x1 & (x4 | ~x5)) | (x6 & x7)))) & (x3 | x4 | ~x5);
  assign z54 = (~new_n164_ & ~x3) | (x2 & ~new_n165_ & x3);
  assign new_n164_ = (x2 | (x1 & (x0 | new_n75_ | x4))) & (~x0 | x1) & (new_n88_ | (~x0 & ~x2));
  assign new_n165_ = new_n86_ & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z55 = new_n167_ | (~x1 & (x2 | (~x2 & ~x3))) | (x0 & (x2 ? ~new_n88_ : ~x3));
  assign new_n167_ = ~x4 & ~new_n75_ & (x2 ? ~x0 : ~x3);
  assign z56 = x2 ? (~new_n93_ | x0 | x4 | (~x1 & x3)) : ~x3;
  assign z57 = (~x1 & (~x2 | ~x3)) | ~new_n170_ | (~x2 & (x3 | (~x4 & x5)));
  assign new_n170_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z58 = new_n152_ | ~x3;
  assign z59 = (x1 & (x3 | (~x0 & x2 & ~x3))) | (~x2 & (x0 | x3)) | new_n173_ | (~new_n174_ & ~x0) | (x0 & ~x1 & ~x3);
  assign new_n173_ = ~x4 & ~new_n75_ & (x3 | (x0 & ~x3));
  assign new_n174_ = ~x3 & ~x5 & (x3 | (new_n94_ & ~x4));
  assign z60 = ~new_n176_ | (~new_n88_ & (x3 ? x2 : ~x0));
  assign new_n176_ = (~x2 | ((x0 | x3 | (~x1 & ~x5)) & ((~x0 & ~x1) | ~x3))) & (x3 | (x0 ? (x1 & x4) : (~x1 | ~x5)));
  assign z61 = (~x1 & (~x2 | ~x3)) | ~x0 | x1 | (~new_n75_ & ~x4);
  assign z62 = (~new_n75_ & ~x4) | ~x0 | ~x1 | (x1 & x3);
  assign z23 = 1'b0;
  assign z48 = ~z13;
  assign z14 = z13;
  assign z16 = z13;
  assign z21 = z13;
endmodule


