// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_,
    new_n95_, new_n98_, new_n99_, new_n103_, new_n105_, new_n107_,
    new_n112_, new_n114_, new_n115_, new_n124_, new_n126_, new_n128_,
    new_n130_, new_n132_, new_n135_, new_n137_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n146_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n163_;
  assign z00 = ~x4 & ~x5 & ~z14 & ~x6;
  assign z14 = ~x1 & ~x2;
  assign z01 = z14 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z14 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z14 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z14 & ~x7;
  assign z05 = z14 | (~x4 & x5 & x6 & ~x7);
  assign z06 = ~x1 & (~x2 | (~x0 & x3 & new_n81_ & ~x4));
  assign new_n81_ = ~x5 & ~x6;
  assign z07 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & x6 & new_n85_ & x5;
  assign new_n85_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n87_ & x6;
  assign new_n87_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x7 & x6 & x5 & new_n89_ & ~x4;
  assign new_n89_ = x2 & ~x0 & x1;
  assign z11 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z16 = ~x2 & (~x1 | (new_n98_ & x0 & new_n99_ & x5));
  assign new_n98_ = x3 & ~x4;
  assign new_n99_ = x6 & x7;
  assign z18 = ~x1 & (~x2 | (~x0 & x3 & x4 & ~x5));
  assign z25 = ~x7 & x6 & new_n83_ & ~x5;
  assign z26 = (~x1 & ~x2) | (new_n103_ & x0 & x2 & ~x3);
  assign new_n103_ = ~x4 & ~x5 & x6 & x7;
  assign z27 = (~x1 & ~x2) | (new_n105_ & ~x0 & x1 & x2 & ~x3);
  assign new_n105_ = ~x4 & ~x5 & x6 & ~x7;
  assign z28 = ~x1 & (~x2 | (new_n107_ & new_n98_ & x0));
  assign new_n107_ = ~x5 & x6 & x7;
  assign z30 = x7 & x6 & new_n85_ & ~x5;
  assign z31 = ~x2 | x0 | x1 | ~x5 | ~x3 | ~x4;
  assign z32 = ~x2 | x0 | x1 | ~x3 | ~x4;
  assign z33 = (x1 & (~x2 | (x3 & ~x5))) | (~new_n112_ & x2);
  assign new_n112_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = new_n114_ | new_n115_ | (~z14 & (x4 | x7));
  assign new_n114_ = (~x5 | x6) & (x2 ? x3 : x1);
  assign new_n115_ = ~x3 & (x1 | (x2 & (x0 | x5 | ~x6)));
  assign z35 = x1 | (x2 & (x0 | ~x3 | ~x4 | ~x5));
  assign z36 = ~new_n105_ | ~x2 | x3 | x0 | x1;
  assign z37 = (~x3 & (x2 | (~x0 & x1))) | (~new_n105_ & (x1 ? x3 : x2));
  assign z38 = x1 | (x2 & (x0 | ~x3 | ~x4));
  assign z39 = ~z14 & (~new_n98_ | ~x5 | x6 | x7);
  assign z40 = (x1 & (~x0 | ~x2)) | (x2 & (x0 ? (~new_n107_ | x3 | x4) : (~x3 | ~x4)));
  assign z41 = x2 | x3 | ~x0 | ~x1;
  assign z42 = (~x2 & (~x1 | ~x5)) | new_n124_ | x4 | (~x5 & (x1 | ~x3));
  assign new_n124_ = ~x4 & (x5 ? (x6 | x7) : (~x0 | ~x6 | ~x7));
  assign z43 = new_n124_ | ~new_n126_ | (x1 & (x4 | (x3 & ~x5)));
  assign new_n126_ = (x2 | (x1 & ~x4 & x5)) & (~x4 | (~x0 & x3));
  assign z45 = x1 ? (new_n128_ | x0 | ~x2) : x2;
  assign new_n128_ = ~x4 & (x5 | x6);
  assign z46 = x2 | (~new_n130_ & x1);
  assign new_n130_ = ~x0 & ~x3 & (x4 | (~x5 & (~x6 | x7)));
  assign z47 = (new_n128_ & ~x0) | (~new_n132_ & x0) | ~x1 | ~x2;
  assign new_n132_ = x3 & ~x4 & x5 & x6 & x7;
  assign z49 = x1 | (x2 & ((~new_n81_ & ~x4) | x0 | (x3 & x4)));
  assign z50 = ~x7 | x5 | ~x6 | ~new_n135_ | x2 | x4;
  assign new_n135_ = x1 & (~x0 | x3);
  assign z51 = (~x1 & (x0 | ~x2)) | (~new_n137_ & ~x4) | (~x2 & (~x0 | x3)) | (~x0 & (x1 | ~x3 | x4));
  assign new_n137_ = ((~x5 & ~x6) | (x0 & ~x2)) & (x5 | ~x6) & (~x5 | (x6 & x7));
  assign z52 = (~new_n81_ & ~x4) | (~x0 & (x1 | ~x2 | (x3 & x4))) | (~x2 & (~x1 | x3)) | (x0 & x3);
  assign z53 = (x0 & (~x1 | ~x3)) | (~new_n140_ & (~x1 | (~x2 & ~x3))) | (~x1 & (~x2 | ~x3)) | new_n141_ | (x2 & x3 & ~x0 & x1);
  assign new_n140_ = ~x4 & x5 & x6 & x7;
  assign new_n141_ = ~x4 & (((x5 | x6) & (x2 ^ x3)) | (x3 & (x5 ? (~x6 | ~x7) : x6)));
  assign z54 = (x1 & ((x0 & x3) | (~new_n143_ & ~x2))) | (x2 & (~new_n144_ | (x0 & (~x1 | x3))));
  assign new_n143_ = (x0 | x3 | x4 | (~x5 & ~x6)) & ((~x0 & ~x3) | (x6 & x7 & ~x4 & x5));
  assign new_n144_ = ((x6 & x7) | (x3 & (x4 | ~x5))) & (x3 | (~x4 & x5)) & (x1 | ~x3) & (x4 | x5 | ~x6);
  assign z55 = (~new_n146_ & x1) | (x2 & (~x1 | (~new_n140_ & x0)));
  assign new_n146_ = (~x0 | x2 | x3) & (x4 | (x0 & x2) | (~x5 & ~x6));
  assign z56 = (x0 & (x1 | (~x1 & x2))) | (new_n148_ & x1) | (~x1 & (~x2 | (x2 & x3))) | (~x2 & ~x3) | (~new_n140_ & x2);
  assign new_n148_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign z57 = new_n150_ | new_n151_ | ~new_n152_ | (~x3 & (x0 | ~x1));
  assign new_n150_ = (x0 | ~x2) & (~x1 | (~x4 & x5));
  assign new_n151_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n152_ = (x0 | x2 | ~x3) & (~x2 | (~x0 & ~x4 & x5 & x6));
  assign z58 = (new_n128_ & ~x0) | (~new_n140_ & x0) | ~x1 | ~x2 | ~x3;
  assign z59 = new_n155_ | new_n156_ | new_n158_ | (~new_n157_ & x1);
  assign new_n155_ = x0 & ((x2 & ((~x4 & x6) | (~x1 & ~x3))) | (~x3 & ~x4 & x1 & ~x2));
  assign new_n156_ = (~x6 | ~x7) & ((~x0 & x2) | (x1 & ~x2 & ~x4));
  assign new_n157_ = ((x0 & ~x3) | (~x2 & ~x4)) & (x2 | ~x4) & (x4 | (~x5 & (~x2 | ~x6)));
  assign new_n158_ = x2 & ((x5 & (~x0 | ~x4)) | (~x0 & (x3 | x4)));
  assign z60 = new_n163_ | (x2 & (new_n160_ | new_n135_ | new_n161_ | new_n162_));
  assign new_n160_ = x0 & (~x1 | (~x4 & x6));
  assign new_n161_ = (~x0 | ~x1) & (x4 | ~x6 | ~x7);
  assign new_n162_ = ~x1 & (~x3 | ~x5);
  assign new_n163_ = x1 & (~x4 | (x4 & (~x0 | x3)));
  assign z61 = x1 | (x2 & (new_n128_ | ~x0 | ~x3));
  assign z62 = new_n128_ | ~x0 | ~x1 | x3;
  assign z19 = 1'b0;
  assign z20 = 1'b0;
  assign z21 = 1'b0;
  assign z22 = 1'b0;
  assign z29 = 1'b0;
  assign z44 = 1'b1;
  assign z48 = ~z14;
  assign z17 = z14;
  assign z23 = z14;
  assign z24 = z14;
endmodule


