// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n77_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n96_,
    new_n101_, new_n104_, new_n106_, new_n107_, new_n109_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n120_, new_n121_,
    new_n125_, new_n128_, new_n129_, new_n131_, new_n132_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n144_, new_n146_,
    new_n148_, new_n150_, new_n152_, new_n154_, new_n156_, new_n157_,
    new_n159_, new_n161_, new_n166_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = ~x0 & x2;
  assign z01 = z06 | (~x5 & ~x6 & ~x7);
  assign z02 = z06 | (new_n77_ & x5 & ~x6 & ~x7);
  assign new_n77_ = ~x3 & ~x4;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z06 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = ~x0 & (new_n82_ | x2);
  assign new_n82_ = x1 & ~x3 & ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z11 = (~x0 & x2) | (new_n86_ & ~x2 & ~x3 & x0 & x1);
  assign new_n86_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (~x0 | (new_n88_ & new_n77_ & ~x1));
  assign new_n88_ = x5 & x6 & x7;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n90_ & x3;
  assign new_n90_ = ~x2 & ~x0 & x1;
  assign z14 = (~x0 & x2) | (new_n86_ & x0 & ~x1 & ~x2 & x3);
  assign z16 = (~x0 & x2) | (new_n86_ & x0 & x1 & ~x2 & x3);
  assign z17 = (~x0 & x2) | (x0 & ~x1 & ~x2 & x4 & ~x5);
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n96_ & ~x3;
  assign new_n96_ = ~x2 & x0 & ~x1;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n96_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n96_ & ~x4;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x0 & (x2 | (new_n77_ & ~x1 & new_n101_ & ~x5));
  assign new_n101_ = x6 & ~x7;
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n90_ & ~x3;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n104_ & ~x3;
  assign new_n104_ = x0 & x2;
  assign z28 = x2 & (~x0 | (new_n107_ & ~x1 & new_n106_ & ~x5));
  assign new_n106_ = x6 & x7;
  assign new_n107_ = x3 & ~x4;
  assign z29 = x7 & new_n109_ & ~x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & x6 & new_n84_ & ~x5;
  assign z31 = ~new_n112_ | (~x0 & (x3 | ~x4));
  assign new_n112_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = new_n114_ | (x4 & (~x0 | ~x5)) | ~new_n116_ | (~new_n115_ & ~x0);
  assign new_n114_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n115_ = ~x3 & ~x5 & x6 & ~x7;
  assign new_n116_ = ~x1 & ~x2;
  assign z33 = ~new_n118_ | ~x7 | x4 | ~x6;
  assign new_n118_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = (~new_n120_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n121_ : ~x2));
  assign new_n120_ = ~x6 & ~x7 & x3 & ~x4;
  assign new_n121_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (~x2 & (x1 | ~x4 | (~x0 & x3)));
  assign z36 = ~new_n96_ | ~x4 | x5;
  assign z37 = ((~x3 | x5) & (x0 ^ ~x2)) | (~new_n125_ & (x0 ? (x3 & ~x5) : ~x2)) | (x0 & (x1 ? (x3 & x5) : ~x3));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = new_n114_ | ~new_n116_ | (~x0 & (~new_n77_ | ~new_n101_ | x5));
  assign z39 = new_n128_ | (~x2 & (x4 | (~x0 & ~x5))) | (x0 & (x4 | (~new_n129_ & ~x5)));
  assign new_n128_ = (x0 ? x5 : ~x2) & (~x3 | x6 | x7);
  assign new_n129_ = ~x1 & ~x2 & x6 & x7;
  assign z40 = (~new_n131_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n132_ | (x3 & (~x0 | x2));
  assign new_n131_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n132_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n135_ | (x2 & (~x0 | (~x3 & ~x5)));
  assign new_n135_ = (x0 | (x5 & (x4 | ~x7))) & (x4 | ((~x5 | (~x6 & ~x7)) & (~x0 | ~x6 | x7))) & ~x4 & (x5 | (~x1 & x6 & x7));
  assign z43 = new_n137_ | new_n138_ | new_n139_ | new_n140_;
  assign new_n137_ = ~x4 & ((x5 & (x6 | x7)) | (x0 & x6 & ~x7) | (~x0 & ((~x5 & ~x6) | x7)));
  assign new_n138_ = ~x0 & (x2 | ((x1 | x3) & (x4 | ~x5)));
  assign new_n139_ = x4 & (x2 | (x1 & x5));
  assign new_n140_ = ~x5 & ((x1 & (~x2 | x3)) | (x2 & (~x6 | ~x7)));
  assign z44 = (x3 & (x0 | x4)) | (~x4 & (~x0 | x5)) | ~new_n116_ | (x0 & (x4 | x5 | x6));
  assign z45 = (~x4 & ((~new_n106_ & ~x0) | x5)) | x2 | x4 | x0 | x1;
  assign z46 = new_n144_ | x2 | x3 | x0 | ~x1;
  assign new_n144_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~x0 & (x1 | x2 | x5)) | ~new_n146_ | (x0 & (~x3 | ~x5 | ~x1 | ~x2));
  assign new_n146_ = ~x4 & x6 & x7;
  assign z48 = ~new_n148_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n148_ = ~x0 & ~x1 & ~x2 & x3;
  assign z50 = (~new_n150_ & ~x2) | (x0 & (~x1 | x2 | ~x3));
  assign new_n150_ = ~x4 & ~x5 & x6 & x7;
  assign z51 = new_n152_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n152_ = ~x4 & (((x5 | x6) & (x0 ^ ~x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n154_ & (x0 | (~x0 & ~x2))) | (~x0 & ~x2 & (x1 | ~x3)) | (x0 & ((~x1 & ~x2) | x3));
  assign new_n154_ = ~x5 & ~x6;
  assign z53 = (x2 & (~x0 | ~x3)) | new_n156_ | (~new_n157_ & ~x3) | (~x1 & x3);
  assign new_n156_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & x3 & (x5 | x6)));
  assign new_n157_ = x5 & x6 & x7 & ~x4 & (~x0 | ~x1);
  assign z54 = (~new_n82_ & x0) | (~x2 & (x3 ? ~new_n86_ : ~new_n159_));
  assign new_n159_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign z55 = (~x1 & (x0 | ~x2)) | (~x2 & (new_n161_ | (x0 & ~x3))) | (x0 & ~new_n86_ & x2);
  assign new_n161_ = ~x4 & (x5 | x6);
  assign z56 = new_n144_ | x2 | ~x3 | x0 | ~x1;
  assign z57 = new_n144_ | ~x1 | x2 | (x0 & ~x3) | (~x0 & x3);
  assign z58 = (x0 & (~x1 | ~x2 | ~x5)) | ~new_n106_ | ~new_n107_ | (~x0 & (x1 | x2 | x5));
  assign z59 = (~x2 & (~new_n150_ | (x0 & (~x1 | ~x3)))) | new_n166_ | (x2 & (~x0 | (x1 & x3) | (~x1 & ~x3)));
  assign new_n166_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & ~x2 & (~new_n88_ | x1 | x4));
  assign z61 = (~x1 & (~x2 | (x0 & ~x3))) | (~x0 & ~x2) | (x0 & (new_n161_ | x1));
  assign z62 = (~x1 & (x0 | ~x2)) | (~x0 & ~x2) | (x0 & (new_n161_ | (x1 & x3)));
  assign z10 = 1'b0;
  assign z49 = 1'b1;
  assign z09 = z06;
  assign z15 = z06;
  assign z18 = z06;
  assign z27 = z06;
endmodule


