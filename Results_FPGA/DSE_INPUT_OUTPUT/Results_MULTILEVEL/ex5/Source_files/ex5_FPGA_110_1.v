// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n77_, new_n81_, new_n83_, new_n85_, new_n87_, new_n88_,
    new_n91_, new_n93_, new_n95_, new_n97_, new_n108_, new_n111_,
    new_n114_, new_n116_, new_n118_, new_n120_, new_n121_, new_n125_,
    new_n127_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n146_, new_n148_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_,
    new_n183_, new_n184_;
  assign z00 = z06 | (new_n75_ & ~x4);
  assign z06 = ~x1 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z06 | (new_n77_ & ~x5);
  assign new_n77_ = ~x6 & ~x7;
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z04 = z06 | (new_n81_ & x3 & ~x4);
  assign new_n81_ = ~x5 & x6 & ~x7;
  assign z05 = z06 | (new_n83_ & x6 & ~x7);
  assign new_n83_ = ~x4 & x5;
  assign z07 = (~x1 & x2) | (new_n83_ & new_n85_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n85_ = x6 & x7;
  assign z08 = x2 & (~x1 | (new_n88_ & new_n87_ & x0));
  assign new_n87_ = ~x3 & ~x4;
  assign new_n88_ = x5 & x6 & x7;
  assign z10 = x2 & (~x1 | (new_n88_ & ~x0 & ~x4));
  assign z11 = x7 & x6 & x5 & ~x4 & new_n91_ & ~x3;
  assign new_n91_ = ~x2 & x0 & x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n93_ & x3;
  assign new_n93_ = ~x2 & ~x0 & x1;
  assign z14 = x7 & x6 & x5 & ~x4 & new_n95_ & x3;
  assign new_n95_ = ~x2 & x0 & ~x1;
  assign z15 = x7 & new_n97_ & x6;
  assign new_n97_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n91_ & x3;
  assign z17 = ~x5 & new_n95_ & x4;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n95_ & ~x3;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n95_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n95_ & ~x4;
  assign z23 = ~x1 & (x2 | (~x0 & x3 & x5));
  assign z24 = ~x1 & (x2 | (new_n81_ & new_n87_ & ~x0));
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n93_ & ~x3;
  assign z26 = x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z27 = x2 & (~x1 | (new_n81_ & new_n87_ & ~x0));
  assign z29 = x7 & new_n111_ & ~x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x2 & (~x1 | (new_n87_ & x0 & new_n85_ & ~x5));
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | (~new_n114_ & ~x2) | x1 | x2;
  assign new_n114_ = x4 ? x5 : (~x5 & (~x0 | ~x6));
  assign z32 = x1 | (~new_n116_ & ~x2);
  assign new_n116_ = (x4 | ((~x0 | (x3 & ~x6)) & ~x5 & (x0 | (x6 & ~x7)))) & (~x4 | x5) & (x0 | (~x3 & ~x4));
  assign z33 = ~new_n118_ | ~x7 | x4 | ~x6;
  assign new_n118_ = x1 & x2 & x0 & (~x3 | x5);
  assign z34 = (x2 & (~x1 | ~x5)) | (~new_n120_ & ~x5) | (~new_n121_ & x5);
  assign new_n120_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign new_n121_ = x3 & ~x4 & ~x6 & ~x7;
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = x1 | (~x2 & (~x0 | ~x4 | x5));
  assign z37 = (~x0 & (x1 ? ~x3 : (~x2 & x5))) | (~x3 & (x1 ^ ~x2)) | (~new_n125_ & (x1 ? x3 : (~x2 & ~x5))) | (x1 & x3 & x5);
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = x1 | (~new_n127_ & ~x2);
  assign new_n127_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x3 & ~x4 & ~x5 & x6 & ~x7));
  assign z39 = (x2 & (~x1 | ~x5)) | ~new_n129_ | (x5 & (~new_n77_ | ~x3));
  assign new_n129_ = ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = new_n131_ | new_n132_ | new_n133_ | ~new_n135_ | (~new_n134_ & x5);
  assign new_n131_ = x0 & ((x4 & ~x5) | (~x1 & ~x4 & x6));
  assign new_n132_ = x2 & (~x0 | ~x1);
  assign new_n133_ = x3 & (~x0 | x1);
  assign new_n134_ = ~x1 & x4;
  assign new_n135_ = (x0 | (~x1 & (x4 | (x6 & ~x7)))) & (~x1 | (x2 & ~x4 & x6 & x7));
  assign z41 = (~x0 & (x1 | ~x2)) | (x1 & (x2 | x3)) | (~x1 & ~x2 & (~x3 | ~x5));
  assign z42 = (x2 & (~x1 | ~x5)) | ~new_n129_ | (~new_n77_ & x5);
  assign z43 = new_n139_ | (~x5 & (new_n133_ | ~new_n140_)) | new_n142_ | (new_n141_ & x5);
  assign new_n139_ = x4 & (x1 | (~x0 & x3));
  assign new_n140_ = (x0 | (~x1 & (x4 | x6))) & (~x1 | (x2 & x6 & x7));
  assign new_n141_ = (x1 | ~x4) & (x6 | x7);
  assign new_n142_ = ~x1 & (x2 | (~x4 & (x0 ? (x6 & ~x7) : x7)));
  assign z44 = (x4 & (x0 | (~x0 & x3))) | (~x0 & (~x4 | (x3 & ~x5))) | ~new_n144_ | (x5 & (x0 | (~new_n77_ & ~x4)));
  assign new_n144_ = (~x0 | (~x3 & ~x6)) & ~x1 & (x1 | ~x2);
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | ~new_n146_ | (x5 & (~x1 | ~x4));
  assign new_n146_ = ~x0 & (x1 | (~x2 & ~x4 & x6 & x7));
  assign z46 = x1 ? (new_n148_ | x0 | x2 | x3) : ~x2;
  assign new_n148_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (new_n150_ | new_n151_ | ~x2)) | (~new_n152_ & ~x2);
  assign new_n150_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n151_ = x0 & (~x3 | x4 | ~x5 | ~x6 | ~x7);
  assign new_n152_ = ~x0 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~new_n154_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n154_ = ~x0 & ~x1 & ~x2 & x3;
  assign z50 = ~new_n156_ | (x0 & (~x1 | ~x3));
  assign new_n156_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z51 = (x0 & (~x1 | (~x2 & x3))) | new_n158_ | (~x0 & (x1 | x2 | ~x3));
  assign new_n158_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | ((~x0 | x2) & (x5 | x6)));
  assign z52 = (x0 & (~x1 | x3)) | (~new_n75_ & ~x4) | (~x0 & (x1 | x2 | ~x3));
  assign z53 = new_n161_ | (~new_n162_ & ~x4) | new_n163_ | new_n164_;
  assign new_n161_ = ~x5 & ((~x2 & ~x3) | (x1 & x3 & ~x4 & x6));
  assign new_n162_ = ((~x5 & ~x6) | ((~x1 | ~x2 | x3) & (x2 | ~x3))) & (~x1 | ~x3 | ~x5 | (x6 & x7));
  assign new_n163_ = x1 & (x0 ? ~x3 : (x2 & x3));
  assign new_n164_ = ~x2 & (x3 ? ~x1 : (x4 | ~x6 | ~x7));
  assign z54 = ((new_n166_ | ~new_n167_) & ~x2) | (x1 & (new_n168_ | ~new_n169_));
  assign new_n166_ = ~x3 & (~x1 | (~x0 & ~x4 & (x5 | x6)));
  assign new_n167_ = (~x0 | ~x3) & ((~x0 & ~x3) | (~x4 & x5 & x6 & x7));
  assign new_n168_ = (~x6 | ~x7) & (x3 ? (~x4 & x5) : x2);
  assign new_n169_ = (~x2 | x3 | (~x4 & x5)) & (~x3 | (~x0 & (x4 | x5 | ~x6)));
  assign z55 = (~x4 & ~new_n75_ & (~x0 | ~x2)) | ~x1 | (~new_n171_ & x0);
  assign new_n171_ = x2 ? (~x4 & x5 & x6 & x7) : x3;
  assign z56 = (~new_n173_ & ~x2) | ~new_n174_ | (~x7 & (x2 | (~x4 & x6)));
  assign new_n173_ = x3 & (x4 | ~x5);
  assign new_n174_ = ~x0 & x1 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x2 & (new_n83_ | (~x0 & x3))) | new_n176_ | ~new_n177_ | (x0 & (x2 | ~x3));
  assign new_n176_ = ~x4 & ((x0 & x5) | (x6 & ~x7));
  assign new_n177_ = x1 & (~x2 | (~x4 & x5 & x6 & x7));
  assign z58 = (new_n180_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n181_ | (~new_n179_ & (x0 | ~x1));
  assign new_n179_ = ~x4 & x6 & x7;
  assign new_n180_ = ~x4 & (x5 | (x1 & x6));
  assign new_n181_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (~x2 & (~new_n183_ | (x0 & (~x1 | ~x3)))) | (x1 & (new_n184_ | (x2 & (~x0 | x3))));
  assign new_n183_ = ~x4 & ~x5 & x6 & x7;
  assign new_n184_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x3 & (x1 | ~x2)) | (x1 & (~x0 | ~x4)) | (~x1 & (x2 | (~x2 & (~new_n88_ | x0 | x4))));
  assign z62 = (~x2 & (~x1 | x3)) | (x1 & ((~new_n75_ & ~x4) | ~x0 | x3));
  assign z12 = 1'b0;
  assign z49 = 1'b1;
  assign z61 = 1'b1;
  assign z09 = z06;
  assign z18 = z06;
  assign z28 = z06;
endmodule


