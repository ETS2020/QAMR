// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n77_, new_n79_, new_n81_, new_n83_, new_n86_, new_n89_,
    new_n91_, new_n94_, new_n105_, new_n107_, new_n109_, new_n112_,
    new_n114_, new_n116_, new_n118_, new_n122_, new_n124_, new_n126_,
    new_n128_, new_n129_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n141_, new_n143_, new_n145_, new_n147_,
    new_n150_, new_n152_, new_n155_, new_n156_, new_n158_, new_n164_;
  assign z00 = z06 | (new_n75_ & ~x4);
  assign z06 = ~x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z06 | (new_n77_ & ~x5);
  assign new_n77_ = ~x6 & ~x7;
  assign z02 = z06 | (new_n79_ & new_n77_ & x5);
  assign new_n79_ = ~x3 & ~x4;
  assign z03 = z06 | (new_n81_ & new_n77_ & x5);
  assign new_n81_ = x3 & ~x4;
  assign z04 = z06 | (new_n81_ & new_n83_ & ~x5);
  assign new_n83_ = x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = ~x0 & (x2 | (new_n86_ & new_n79_ & x1));
  assign new_n86_ = x5 & x6 & x7;
  assign z08 = x2 & (~x0 | (new_n86_ & new_n79_ & x1));
  assign z11 = x7 & x6 & x5 & ~x4 & new_n89_ & ~x3;
  assign new_n89_ = ~x2 & x0 & x1;
  assign z12 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = ~x0 & (x2 | (new_n86_ & new_n81_ & x1));
  assign z14 = x7 & x6 & x5 & ~x4 & new_n94_ & x3;
  assign new_n94_ = ~x2 & x0 & ~x1;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z17 = ~x5 & new_n94_ & x4;
  assign z19 = ~x0 & (x2 | (~x1 & ~x3 & x4));
  assign z20 = (~x0 & x2) | (new_n75_ & new_n79_ & x0 & ~x1 & ~x2);
  assign z21 = ~x6 & ~x5 & ~x4 & new_n94_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n94_ & ~x4;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x0 & (x2 | (new_n83_ & ~x5 & new_n79_ & ~x1));
  assign z25 = ~x0 & (x2 | (new_n79_ & x1 & new_n83_ & ~x5));
  assign z26 = new_n105_ & x7;
  assign new_n105_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z28 = x2 & (~x0 | (new_n107_ & ~x5 & new_n81_ & ~x1));
  assign new_n107_ = x6 & x7;
  assign z29 = ~x0 & (new_n109_ | x2);
  assign new_n109_ = ~x1 & ~x3 & ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x2 & (~x0 | (new_n79_ & x1 & new_n107_ & ~x5));
  assign z31 = (~x0 & (x3 | ~x4)) | ~new_n112_ | (~x4 & (x5 | (x0 & x6)));
  assign new_n112_ = ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (~new_n114_ & ~x4) | ~new_n112_ | (~x0 & (x3 | x4));
  assign new_n114_ = (~x0 | (x3 & ~x6)) & ~x5 & (x0 | (x6 & ~x7));
  assign z33 = ~x2 | (~new_n116_ & x0);
  assign new_n116_ = ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = ((~new_n77_ | ~new_n81_) & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n118_ : ~x2));
  assign new_n118_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = x2 | ~x4 | x1 | (~x0 & x3) | (x0 & ~x5);
  assign z36 = ~new_n94_ | ~x4 | x5;
  assign z37 = (~x0 & (x2 | ~x3 | x5)) | (x2 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n122_)) | (~x1 & ~x3);
  assign new_n122_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (x0 | ~x2)) | ~new_n124_ | (x5 & (x0 ? ~x4 : ~x2));
  assign new_n124_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (x6 & ~x7 & ~x3 & ~x4));
  assign z39 = (~x0 & (x2 | ~x5)) | ~new_n126_ | (x5 & (~new_n77_ | ~x3));
  assign new_n126_ = ~x4 & (x5 | (~x1 & ~x2 & x6 & x7));
  assign z40 = (x3 & (x0 ^ ~x2)) | (~new_n128_ & ~x2) | (~new_n129_ & x0);
  assign new_n128_ = ~x1 & (x4 | (~x5 & (~x0 | ~x6) & (x0 | (x6 & ~x7))));
  assign new_n129_ = (~x4 | (~x2 & x5)) & (~x5 | (~x2 & x4)) & (~x2 | (x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (x0 | ~x2)) | (~new_n77_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n132_ : ~x2));
  assign new_n132_ = ~x1 & (~x2 | x3) & x6 & x7;
  assign z43 = new_n134_ | new_n135_ | new_n136_ | new_n137_;
  assign new_n134_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n135_ = ~x0 & (x2 | ((x4 | ~x5) & (x1 | x3)));
  assign new_n136_ = x4 & (x2 | (x1 & x5));
  assign new_n137_ = ~x5 & ((x2 & (~x6 | ~x7)) | (x1 & (~x2 | x3)));
  assign z44 = (x6 & (x0 | (~x4 & x5))) | ~new_n139_ | (~x4 & (~x0 | (x5 & x7)));
  assign new_n139_ = (~x5 | (~x0 & (~x1 | ~x4))) & (~x0 | (~x3 & ~x4)) & (x0 | (~x4 & x5) | (~x1 & ~x3)) & ~x2 & (~x1 | (x2 & ~x3) | x5);
  assign z45 = ~new_n141_ | ~x7 | x5 | ~x6;
  assign new_n141_ = ~x0 & ~x1 & ~x2 & ~x4;
  assign z46 = new_n143_ | x2 | x3 | x0 | ~x1;
  assign new_n143_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n145_ & (x0 | ~x2)) | (~x2 & (x0 | x1 | x5)) | (x0 & (~x1 | ~x3 | ~x5));
  assign new_n145_ = ~x4 & x6 & x7;
  assign z48 = x0 | (~new_n147_ & ~x2);
  assign new_n147_ = ~x1 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (x0 & (~x1 | x2 | ~x3));
  assign z50 = (~new_n150_ & ~x2) | (x0 & (~x1 | x2 | ~x3));
  assign new_n150_ = ~x4 & ~x5 & x6 & x7;
  assign z51 = new_n152_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n152_ = ~x4 & (((x5 | x6) & (x0 ^ ~x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n75_ & (x0 | (~x0 & ~x2))) | (~x0 & ~x2 & (x1 | ~x3)) | (x0 & (x3 | (~x1 & ~x2)));
  assign z53 = x3 ? (new_n155_ | (~x1 & (x0 | ~x2))) : ((~new_n156_ & ~x2) | (x0 & (x1 | x2)));
  assign new_n155_ = ~x4 & ((x0 & (x5 ? (~x6 | ~x7) : x6)) | (~x2 & (x5 | x6)));
  assign new_n156_ = ~x4 & x5 & x6 & x7;
  assign z54 = (~x1 & (x0 | ~x3)) | (~x0 & (x2 | (new_n158_ & ~x3))) | (x0 & x3) | (~new_n156_ & (x0 | x3));
  assign new_n158_ = ~x4 & (x5 | x6);
  assign z55 = (~x1 & (x0 | ~x2)) | (~x2 & (new_n158_ | (x0 & ~x3))) | (x0 & ~new_n156_ & x2);
  assign z56 = x0 | (~x2 & (new_n143_ | ~x1 | ~x3));
  assign z57 = new_n143_ | ~x1 | x2 | (x0 & ~x3) | (~x0 & x3);
  assign z58 = (x0 & (~x1 | ~x2 | ~x5)) | ~new_n81_ | ~new_n107_ | (~x0 & (x1 | x2 | x5));
  assign z59 = (~new_n164_ & x0) | (~new_n150_ & (x0 ? (x1 & x3) : ~x2));
  assign new_n164_ = ((x1 & x3) | (x2 & (x4 | ~x6))) & (~x1 | ~x2 | ~x3) & (x4 | ~x5) & (x1 | x3);
  assign z60 = (x0 & (x3 | ~x4 | (~x1 & (~x2 | ~x3)))) | (~x2 & (x3 | (~x0 & (~new_n86_ | x1 | x4))));
  assign z61 = ~x2 | (x0 & (new_n158_ | x1 | ~x3));
  assign z62 = new_n158_ | ~x0 | ~x1 | x3;
  assign z09 = 1'b0;
  assign z10 = 1'b0;
  assign z27 = 1'b0;
  assign z15 = z06;
  assign z18 = z06;
endmodule


