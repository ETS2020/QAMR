// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n82_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n94_, new_n96_, new_n99_, new_n105_, new_n107_, new_n109_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n144_, new_n146_, new_n149_, new_n151_, new_n153_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n162_, new_n165_, new_n168_,
    new_n169_;
  assign z00 = ~x4 & ~x5 & ~z19 & ~x6;
  assign z19 = ~x0 & ~x2;
  assign z01 = z19 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z19 & ~x7;
  assign z03 = z19 | (x3 & ~x4 & new_n76_ & x5);
  assign z04 = z19 | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = ~x4 & x5 & x6 & ~z19 & ~x7;
  assign z06 = new_n82_ & ~x6;
  assign new_n82_ = ~x5 & ~x4 & x3 & x2 & ~x0 & ~x1;
  assign z08 = (~x0 & ~x2) | (new_n84_ & x0 & x1 & x2 & ~x3);
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z09 = ~x0 & (~x2 | (new_n87_ & ~x1 & new_n86_ & ~x5));
  assign new_n86_ = x6 & x7;
  assign new_n87_ = ~x3 & ~x4;
  assign z10 = ~x0 & (~x2 | (new_n89_ & x1 & ~x4));
  assign new_n89_ = x5 & x6 & x7;
  assign z11 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = (~x0 & ~x2) | (new_n84_ & x0 & ~x1 & x2 & ~x3);
  assign z14 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z15 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = ~x2 & (~x0 | (new_n89_ & x1 & x3 & ~x4));
  assign z17 = ~x5 & x4 & new_n99_ & ~x2;
  assign new_n99_ = x0 & ~x1;
  assign z18 = ~x5 & x4 & x3 & x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n99_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n99_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n99_ & ~x2;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n105_ & ~x3;
  assign new_n105_ = x0 & x2;
  assign z27 = ~x0 & (new_n107_ | ~x2);
  assign new_n107_ = ~x5 & x6 & ~x7 & x1 & ~x3 & ~x4;
  assign z28 = (~x0 & ~x2) | (new_n109_ & x0 & ~x1 & x2 & x3);
  assign new_n109_ = ~x4 & ~x5 & x6 & x7;
  assign z30 = (~x0 & ~x2) | (new_n109_ & x0 & x1 & x2 & ~x3);
  assign z31 = ~new_n112_ | (~z19 & x1);
  assign new_n112_ = (x5 | (~x2 & (~x0 | ~x4))) & (~x0 | (~x2 & (x4 | (~x5 & ~x6)))) & (~x2 | (x3 & x4));
  assign z32 = (x1 & (x0 | x2)) | (~x3 & (x2 | (x0 & ~x4))) | (~new_n114_ & x0) | (x2 & (x0 | ~x4));
  assign new_n114_ = x4 ? x5 : (~x5 & ~x6);
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = (~x5 & (new_n118_ | ~new_n119_)) | new_n120_ | ~new_n121_;
  assign new_n118_ = x0 & (x2 | (~x4 & ~x7));
  assign new_n119_ = (x0 | (~x3 & x6)) & ~x1 & (x4 | x6);
  assign new_n120_ = (x4 | x7) & (~x0 | x5);
  assign new_n121_ = (x0 | x2) & (~x5 | (x3 & ~x6));
  assign z35 = ((x0 | x2) & (x1 | ~x4 | ~x5)) | (x2 & (x0 | ~x3));
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n124_ & ~x0) | x1 | x5;
  assign new_n124_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x3 & (x2 | (x0 & ~x1))) | (~new_n126_ & (x0 ? (x3 & ~x5) : x2)) | (x5 & (x2 | (x0 & x1 & x3)));
  assign new_n126_ = ~x4 & x6 & ~x7;
  assign z38 = (~x3 & (~x0 | ~x4)) | (~x0 & (~x2 | ~x4)) | (~new_n128_ & ~x4) | x1 | (x0 & x2);
  assign new_n128_ = ~x5 & ~x6;
  assign z39 = (~new_n130_ & (x2 | (x0 & x5))) | (x2 & (x4 | ~x5)) | (x0 & (x4 | (~new_n131_ & ~x5)));
  assign new_n130_ = x3 & ~x6 & ~x7;
  assign new_n131_ = ~x1 & x6 & x7;
  assign z40 = (x1 & (~x0 ^ ~x2)) | (~new_n134_ & x0) | (~new_n133_ & x2);
  assign new_n133_ = x0 ? (~x3 & ~x4 & ~x5 & x6 & x7) : (x3 & x4);
  assign new_n134_ = x4 ? x5 : (~x5 & (x2 | ~x6));
  assign z41 = x2 | (x0 & (x1 ? x3 : (~x3 | ~x5)));
  assign z42 = (x4 & (x0 | x2)) | (~new_n137_ & ~x5) | (~new_n76_ & (x0 ? x5 : x2));
  assign new_n137_ = (~x2 | (x0 & x3)) & (~x0 | (~x1 & x6 & x7));
  assign z43 = (~new_n139_ & ~x4) | (~new_n140_ & x0) | (x2 & ~new_n141_ & x4);
  assign new_n139_ = ((~x6 & ~x7) | (x0 ? ~x5 : ~x2)) & (x0 | ~x2 | x5) & (~x0 | ~x6 | x7);
  assign new_n140_ = ((~x1 & ~x2) | (~x4 & (x5 | x6))) & (~x1 | (x2 & ~x3) | x5);
  assign new_n141_ = ~x1 & x3;
  assign z44 = (~new_n128_ & (x0 | ~x4)) | ~x0 | (x0 & (x3 | x4 | x1 | x2));
  assign z45 = new_n144_ | x0 | (~x0 & (~x1 | ~x2));
  assign new_n144_ = ~x4 & (x5 | x6);
  assign z47 = (~new_n146_ & x0) | (x2 & (~x1 | (new_n144_ & ~x0)));
  assign new_n146_ = x5 & x6 & x7 & x2 & x3 & ~x4;
  assign z48 = x2 | (~new_n146_ & x0);
  assign z49 = (~x4 & (x5 | (~x0 & x6))) | (~new_n149_ & ~x0) | x0 | (x3 & x4);
  assign new_n149_ = ~x1 & x2;
  assign z50 = (x4 & (x0 | x3)) | (x5 & (x0 | ~x4)) | ~x0 | (~new_n151_ & x0);
  assign new_n151_ = x1 & ~x2 & x3 & x6 & x7;
  assign z51 = (~new_n153_ & ~x4) | (x0 & (~x1 | (~x2 & x3))) | (~x0 & x2 & (x1 | ~x3 | x4));
  assign new_n153_ = (~x2 | (~x5 & ~x6)) & (~x0 | (x5 ? (x6 & x7) : ~x6));
  assign z52 = (x2 & ((~new_n128_ & ~x4) | (~x0 & (x1 | (x3 & x4))))) | (x0 & ((~new_n128_ & ~x4) | x3 | (~x1 & ~x2)));
  assign z53 = new_n156_ | (x3 & (~x0 ^ ~x1)) | (~x3 & (~x0 ^ x1)) | new_n157_ | (~new_n158_ & ~x1);
  assign new_n156_ = ~x2 & (~x0 | (x1 & ~x4 & x5));
  assign new_n157_ = ~x4 & (x0 ? (x5 ? (~x6 | ~x7) : x6) : (x1 & (x5 | x6)));
  assign new_n158_ = x5 & x6 & x7 & ~x4 & (~x0 | ~x2);
  assign z54 = (~x1 & (x0 | (x2 & x3))) | (~new_n84_ & (x2 ? ~x3 : x0)) | (new_n160_ & x2) | (x0 & x3);
  assign new_n160_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z55 = new_n162_ | (~x1 & (x2 | (x0 & ~x2))) | (x0 & (x2 ? ~new_n84_ : ~x3));
  assign new_n162_ = ~x4 & (x5 | x6) & (~x0 ^ ~x2);
  assign z56 = x0 | (x2 & (~new_n89_ | new_n141_ | x4));
  assign z57 = (~x1 & (x0 | (x2 & ~x3))) | (~new_n84_ & x2) | (x0 & (new_n165_ | x2 | ~x3));
  assign new_n165_ = ~x4 & (x5 | (x6 & ~x7));
  assign z58 = (new_n144_ & ~x0) | (~new_n84_ & x0) | ~x1 | ~x2 | ~x3;
  assign z59 = (x2 & (~new_n168_ | (x1 & (~x0 | x3)))) | (x0 & (~new_n169_ | (~x3 & (~x1 | ~x2))));
  assign new_n168_ = x0 ? (x4 | ~x6) : (~x3 & ~x4 & ~x5 & x6 & x7);
  assign new_n169_ = (x1 | (x2 & (x4 | ~x6))) & (~x5 | (x2 & x4)) & (x2 | (~x4 & x6 & x7));
  assign z60 = x0 ? (~x1 | x3 | ~x4) : (~new_n84_ | x1 | ~x2 | ~x3);
  assign z61 = x0 ? (new_n144_ | x1 | (~x1 & (~x2 | ~x3))) : x2;
  assign z62 = x0 ? (new_n144_ | ~x1 | (x1 & x3)) : x2;
  assign z07 = 1'b0;
  assign z13 = 1'b0;
  assign z23 = 1'b0;
  assign z24 = 1'b0;
  assign z46 = 1'b1;
  assign z25 = z19;
  assign z29 = z19;
endmodule


