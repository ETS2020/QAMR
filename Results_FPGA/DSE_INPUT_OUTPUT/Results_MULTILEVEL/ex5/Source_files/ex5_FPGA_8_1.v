// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n75_, new_n80_, new_n83_, new_n86_, new_n88_, new_n90_,
    new_n96_, new_n99_, new_n102_, new_n105_, new_n107_, new_n110_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n123_,
    new_n125_, new_n127_, new_n128_, new_n130_, new_n131_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n143_, new_n145_,
    new_n147_, new_n150_, new_n153_, new_n154_, new_n156_, new_n158_,
    new_n163_, new_n165_;
  assign z00 = new_n74_ | (new_n75_ & ~x4);
  assign new_n74_ = ~x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = new_n74_ | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~new_n74_ & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~new_n74_ & ~x7;
  assign z04 = new_n74_ | (new_n80_ & ~x5 & x6 & ~x7);
  assign new_n80_ = x3 & ~x4;
  assign z05 = new_n74_ | (x6 & ~x7 & ~x4 & x5);
  assign z07 = ~x0 & (new_n83_ | x2);
  assign new_n83_ = x1 & ~x3 & ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (new_n83_ | ~x0);
  assign z11 = (~x0 & x2) | (new_n86_ & x0 & x1 & ~x2 & ~x3);
  assign new_n86_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n90_ & x3;
  assign new_n90_ = ~x2 & ~x0 & x1;
  assign z14 = (~x0 & x2) | (new_n86_ & x0 & ~x1 & ~x2 & x3);
  assign z16 = (~x0 & x2) | (new_n86_ & x0 & x1 & ~x2 & x3);
  assign z17 = (~x0 & x2) | (~x2 & x4 & ~x5 & x0 & ~x1);
  assign z19 = ~x0 & (x2 | (~x1 & ~x3 & x4));
  assign z20 = new_n96_ & ~x6;
  assign new_n96_ = ~x5 & ~x4 & ~x3 & ~x2 & x0 & ~x1;
  assign z21 = (~x0 & x2) | (new_n75_ & new_n80_ & x0 & ~x1 & ~x2);
  assign z22 = (~x0 & x2) | (new_n99_ & x0 & ~x1 & ~x2);
  assign new_n99_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = ~x0 & (x2 | (~x1 & x3 & x5));
  assign z24 = ~x0 & (x2 | (new_n102_ & ~x5 & x6 & ~x7));
  assign new_n102_ = ~x1 & ~x3 & ~x4;
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n90_ & ~x3;
  assign z26 = new_n105_ & x7;
  assign new_n105_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z28 = x2 & (~x0 | (new_n80_ & ~x1 & new_n107_ & ~x5));
  assign new_n107_ = x6 & x7;
  assign z29 = ~x0 & (x2 | (new_n102_ & ~x5 & ~x6 & x7));
  assign z30 = x7 & new_n110_ & x6;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n112_ | (~x0 & (x3 | ~x4));
  assign new_n112_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (x1 & (x0 | (~x0 & ~x2))) | (x4 & (x0 ? ~x5 : ~x2)) | ~new_n114_ | (x5 & (x0 ? ~x4 : ~x2));
  assign new_n114_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (~x3 & x6 & ~x7));
  assign z33 = ~x2 | (~new_n116_ & x0);
  assign new_n116_ = ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = (~new_n118_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n119_ : ~x2));
  assign new_n118_ = ~x6 & ~x7 & x3 & ~x4;
  assign new_n119_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (~x2 & (x1 | ~x4 | (~x0 & x3)));
  assign z36 = ~x4 | x5 | x2 | ~x0 | x1;
  assign z37 = ((~x3 | x5) & (x0 ^ ~x2)) | (~new_n123_ & (x0 ? (x3 & ~x5) : ~x2)) | (x0 & (x1 ? (x3 & x5) : ~x3));
  assign new_n123_ = ~x4 & x6 & ~x7;
  assign z38 = ~new_n125_ | (x5 & (x0 ? ~x4 : ~x2)) | (x1 & (x0 | ~x2));
  assign new_n125_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (~x3 & ~x4 & x6 & ~x7));
  assign z39 = (~new_n127_ & (x0 ? x5 : ~x2)) | (~x2 & (x4 | (~x0 & ~x5))) | (x0 & (x4 | (~new_n128_ & ~x5)));
  assign new_n127_ = x3 & ~x6 & ~x7;
  assign new_n128_ = ~x1 & ~x2 & x6 & x7;
  assign z40 = (~new_n130_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n131_ | (x3 & (~x0 | x2));
  assign new_n130_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n131_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = x0 ? ((~x1 & (~x3 | ~x5)) | x2 | (x1 & x3)) : ~x2;
  assign z42 = ~new_n134_ | (x2 & (~x0 | (~x3 & ~x5)));
  assign new_n134_ = (x0 | (x5 & (x4 | ~x7))) & (x4 | ((~x5 | (~x6 & ~x7)) & (~x0 | ~x6 | x7))) & ~x4 & (x5 | (~x1 & x6 & x7));
  assign z43 = (~new_n136_ & ~x4) | new_n137_ | new_n139_ | (~new_n138_ & x4);
  assign new_n136_ = (~x5 | (~x6 & ~x7)) & (~x0 | ~x6 | x7) & (x0 | (~x7 & (x5 | x6)));
  assign new_n137_ = ~x0 & (x2 | ((x4 | ~x5) & (x1 | x3)));
  assign new_n138_ = ~x2 & (~x1 | ~x5);
  assign new_n139_ = ~x5 & ((x1 & (~x2 | x3)) | (x2 & (~x6 | ~x7)));
  assign z44 = (x0 & (~new_n75_ | x4)) | x1 | x2 | x3 | (~x0 & ~x4);
  assign z45 = x0 | (~x2 & (x1 | x4 | ~new_n107_ | x5));
  assign z46 = new_n143_ | x0 | ~x1 | x2 | x3;
  assign new_n143_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n145_ & (x0 | ~x2)) | (~x2 & (x0 | x1 | x5)) | (x0 & (~x1 | ~x3 | ~x5));
  assign new_n145_ = ~x4 & x6 & x7;
  assign z48 = x0 | (~new_n147_ & ~x2);
  assign new_n147_ = ~x1 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z50 = (x0 & (~x1 | x2 | ~x3)) | (~new_n99_ & ~x2);
  assign z51 = new_n150_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & (x2 | (~x2 & (x1 | ~x3))));
  assign new_n150_ = ~x4 & (((x5 | x6) & (x0 ^ ~x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n75_ & (x0 | (~x0 & ~x2))) | (~x0 & ~x2 & (x1 | ~x3)) | (x0 & ((~x1 & ~x2) | x3));
  assign z53 = (x2 & (~x0 | ~x3)) | new_n153_ | (~new_n154_ & ~x3) | (~x1 & x3);
  assign new_n153_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & x3 & (x5 | x6)));
  assign new_n154_ = x5 & x6 & x7 & ~x4 & (~x0 | ~x1);
  assign z54 = (~new_n83_ & x0) | (~x2 & (x3 ? ~new_n86_ : ~new_n156_));
  assign new_n156_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign z55 = (~x1 & (x0 | ~x2)) | (~x2 & (new_n158_ | (x0 & ~x3))) | (x0 & ~new_n86_ & x2);
  assign new_n158_ = ~x4 & (x5 | x6);
  assign z56 = x0 | (~x2 & (new_n143_ | ~x1 | ~x3));
  assign z57 = (x0 & (x2 | ~x3)) | (~x2 & (new_n143_ | ~x1 | (~x0 & x3)));
  assign z58 = (~x3 & (x0 | ~x2)) | (~new_n145_ & (~x2 | (x0 & x2))) | (x0 & (~x2 | (x2 & (~x1 | ~x5)))) | (~x2 & (x1 | x5));
  assign z59 = (~new_n163_ & x0) | (~new_n99_ & ~x2);
  assign new_n163_ = (~x2 | ((~x1 | ~x3) & (x4 | ~x6))) & (x2 | (x1 & x3)) & (x4 | ~x5) & (x1 | (x3 & (x4 | ~x6)));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n165_ | x1 | x2 | x4));
  assign new_n165_ = x5 & x6 & x7;
  assign z61 = (~x1 & (~x2 | ~x3)) | new_n158_ | ~x0 | x1;
  assign z62 = new_n158_ | ~x0 | ~x1 | (x1 & x3);
  assign z06 = 1'b0;
  assign z09 = 1'b0;
  assign z10 = 1'b0;
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z27 = 1'b0;
  assign z49 = 1'b1;
endmodule


