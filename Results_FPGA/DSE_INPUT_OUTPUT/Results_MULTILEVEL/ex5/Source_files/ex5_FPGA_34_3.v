// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n78_, new_n81_, new_n82_, new_n85_, new_n87_, new_n89_,
    new_n91_, new_n93_, new_n95_, new_n100_, new_n105_, new_n108_,
    new_n111_, new_n113_, new_n115_, new_n118_, new_n120_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n129_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n161_, new_n164_,
    new_n167_, new_n170_, new_n171_, new_n173_, new_n174_, new_n177_,
    new_n179_, new_n180_;
  assign z00 = z06 | (new_n75_ & ~x4);
  assign z06 = x2 & x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = (x2 & x3) | (~x3 & ~x4 & new_n78_ & x5);
  assign new_n78_ = ~x6 & ~x7;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = x3 & (x2 | (new_n81_ & new_n82_));
  assign new_n81_ = ~x4 & ~x5;
  assign new_n82_ = x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = x7 & new_n85_ & x6;
  assign new_n85_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & x6 & new_n87_ & x5;
  assign new_n87_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n89_ & x6;
  assign new_n89_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x2 & (x3 | (new_n91_ & ~x0 & x1 & ~x4));
  assign new_n91_ = x5 & x6 & x7;
  assign z11 = (x2 & x3) | (new_n93_ & x0 & x1 & ~x2 & ~x3);
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x3 & (x2 | (new_n91_ & ~x0 & x1 & ~x4));
  assign z14 = x3 & (x2 | (new_n91_ & x0 & ~x1 & ~x4));
  assign z16 = x3 & (x2 | (new_n91_ & x0 & x1 & ~x4));
  assign z17 = ~x5 & x4 & new_n100_ & ~x2;
  assign new_n100_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = (x2 & x3) | (x0 & ~x1 & ~x2 & new_n75_ & ~x3 & ~x4);
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n100_ & ~x2;
  assign z22 = (x2 & x3) | (new_n105_ & x0 & ~x1 & ~x2);
  assign new_n105_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x3 & (x2 | (~x0 & ~x1 & x5));
  assign z24 = ~x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x2 & x3) | (new_n81_ & new_n82_ & ~x2 & ~x3 & ~x0 & x1);
  assign z26 = new_n111_ & x7;
  assign new_n111_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = (x2 & x3) | (new_n115_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n115_ = ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x7 & x6 & new_n87_ & ~x5;
  assign z31 = (~x0 & (x3 | ~x4)) | ~new_n118_ | (~x4 & (x5 | (x0 & x6)));
  assign new_n118_ = ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (~new_n120_ & ~x4) | ~new_n118_ | (~x0 & (x3 | x4));
  assign new_n120_ = (~x0 | (x3 & ~x6)) & ~x5 & (x0 | (x6 & ~x7));
  assign z33 = ~x2 | (~new_n122_ & ~x3);
  assign new_n122_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n125_ & ~x5) | ~new_n126_ | (~new_n124_ & (x2 | x5));
  assign new_n124_ = ~x4 & ~x7;
  assign new_n125_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n126_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 ? ~x3 : (x1 | (~x0 & x3) | ~x4 | (x0 & ~x5));
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n129_ & ~x0) | x1 | x5;
  assign new_n129_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | (~x2 & x5))) | (~x3 & (~x1 | x2)) | (~x2 & x3 & (x5 ? x1 : ~new_n131_));
  assign new_n131_ = ~x4 & x6 & ~x7;
  assign z38 = new_n133_ | new_n134_ | x1 | x2;
  assign new_n133_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n134_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~new_n78_ | ~x3)) | (~new_n136_ & ~x5) | x2 | x4;
  assign new_n136_ = x0 & ~x1 & x6 & x7;
  assign z40 = x2 ? new_n141_ : (new_n139_ | new_n140_ | new_n138_ | x1);
  assign new_n138_ = ~x4 & x5;
  assign new_n139_ = x0 & (x4 ? ~x5 : x6);
  assign new_n140_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n141_ = ~x3 & (~x0 | x4 | x5 | ~x6 | ~x7);
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | (~x3 & x5))) | (x5 & ~new_n78_ & (~x2 | ~x3)) | (~x5 & (x2 ? ~x3 : ~new_n136_));
  assign z43 = x2 ? new_n147_ : ((~new_n146_ & ~x4) | (~new_n145_ & (x4 | ~x5)));
  assign new_n145_ = ~x1 & (x0 | ~x3);
  assign new_n146_ = (x0 | (~x7 & (x5 | x6))) & (~x5 | (~x6 & ~x7)) & (~x0 | ~x6 | x7);
  assign new_n147_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = ~new_n149_ | (~x2 & ((~new_n145_ & (x4 | ~x5)) | (~x4 & ~new_n78_ & x5)));
  assign new_n149_ = (~x0 | (~x3 & ~x4 & ~x5 & ~x6)) & ~x2 & (x0 | x4);
  assign z45 = new_n151_ | (x1 & (~x2 | x3)) | x0 | (~new_n152_ & ~x1);
  assign new_n151_ = ~x4 & (x5 | (x1 & x6));
  assign new_n152_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = new_n154_ | x2 | x3 | x0 | ~x1;
  assign new_n154_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | (~new_n157_ & ~x2) | (x2 & ~x3 & (new_n156_ | ~x1));
  assign new_n156_ = ~x4 & (x5 | x6);
  assign new_n157_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~x3 | (~new_n159_ & ~x2);
  assign new_n159_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (~x3 & (new_n138_ | x0 | (x2 & (new_n161_ | x1))));
  assign new_n161_ = ~x4 & x6;
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n105_ & ~x2) | (~x3 & (new_n138_ | x2));
  assign z51 = (~x0 & (x1 | ~x3)) | (x2 & (new_n138_ | x3)) | new_n164_ | ((x0 | x1) & x3) | (~x1 & ~x3);
  assign new_n164_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (x3 & (x5 | x6)));
  assign z52 = (x1 & (~x0 | x3)) | (x3 & (x0 | x2)) | new_n156_ | (~x1 & ~x2 & ~x3);
  assign z53 = (~x3 & (~new_n167_ | (x0 & (x1 | x2)))) | (~x2 & x3 & (new_n156_ | ~x1));
  assign new_n167_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (x6 & x7 & ~x4 & x5);
  assign z54 = (~x2 & ~x3 & ((new_n156_ & ~x0) | ~x1)) | (~new_n93_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n170_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n171_ & ~x3);
  assign new_n170_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n171_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n173_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n174_ & x2);
  assign new_n173_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n174_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | (x2 & ~x3))) | (~x2 & (new_n154_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n93_ & x2)));
  assign z58 = ~x3 | (~new_n177_ & ~x2);
  assign new_n177_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z59 = (~new_n179_ & x0) | (~new_n180_ & ~x3) | (~new_n105_ & (x3 ? ~x2 : ~x0));
  assign new_n179_ = (x1 | (x2 & x3)) & (x3 | (x2 & (x4 | ~x6)));
  assign new_n180_ = (x4 | ~x5) & (x0 | ~x1 | ~x2);
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n91_ | x1 | x2 | x4));
  assign z62 = new_n156_ | ~x0 | ~x1 | x3;
  assign z61 = ~z06;
  assign z15 = z06;
  assign z18 = z06;
  assign z28 = z06;
endmodule


