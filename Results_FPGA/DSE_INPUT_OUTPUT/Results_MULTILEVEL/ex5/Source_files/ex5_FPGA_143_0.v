// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n81_, new_n83_, new_n85_, new_n88_, new_n90_, new_n98_,
    new_n101_, new_n103_, new_n104_, new_n119_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_;
  assign z00 = ~x4 & ~x5;
  assign z01 = ~x5 & (~x4 | (~x6 & ~x7));
  assign z02 = ~x4 & (~x5 | (~x3 & ~x6 & ~x7));
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z05 = ~x7 & x6 & ~x4 & x5;
  assign z07 = ~x4 & (~x5 | (new_n79_ & ~x0 & x1 & ~x2));
  assign new_n79_ = ~x3 & x6 & x7;
  assign z08 = x7 & new_n81_ & x6;
  assign new_n81_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z10 = new_n83_ & x7;
  assign new_n83_ = x6 & x5 & ~x4 & x2 & ~x0 & x1;
  assign z11 = x7 & x6 & x5 & ~x4 & new_n85_ & ~x3;
  assign new_n85_ = ~x2 & x0 & x1;
  assign z12 = ~x4 & (~x5 | (new_n79_ & x0 & ~x1 & x2));
  assign z13 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x4 & (~x5 | (new_n90_ & x0 & ~x1 & ~x2));
  assign new_n90_ = x3 & x6 & x7;
  assign z15 = ~x4 & (~x5 | (new_n90_ & ~x0 & x1 & x2));
  assign z16 = x7 & x6 & x5 & ~x4 & new_n85_ & x3;
  assign z17 = ~x5 & x4 & ~x2 & x0 & ~x1;
  assign z18 = ~x5 & x4 & x3 & ~x0 & ~x1 & x2;
  assign z19 = x4 & ~x3 & ~x0 & ~x1 & ~x2;
  assign z23 = (~x4 & ~x5) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z31 = (~new_n98_ & x4) | ~x4 | ~x5;
  assign new_n98_ = (~x2 | (~x0 & x3)) & ~x1 & (x0 | x2 | ~x3);
  assign z32 = x4 ? ((x2 & (x0 | ~x3)) | x1 | (~x2 & (~x0 | ~x5))) : x5;
  assign z33 = (~x4 & (~x5 | (~new_n101_ & x6))) | x4 | ~x6 | (~x0 & x6);
  assign new_n101_ = x1 & x2 & x7;
  assign z34 = (~x0 & (x4 | x6)) | (~new_n103_ & ~x4) | (~new_n104_ & x4);
  assign new_n103_ = (x6 | (x3 & ~x7)) & x5 & ~x6;
  assign new_n104_ = ~x1 & ~x2 & ~x5;
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = x5 | (x4 & (~x0 | x1 | x2));
  assign z37 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (~x4 & ~x5) | (x1 & x3);
  assign z38 = x4 ? ((~x0 & (~x2 | ~x3)) | x1 | (x0 & x2)) : x5;
  assign z39 = x4 | (x5 & (~x3 | x6 | x7));
  assign z40 = (x0 & (x2 | ~x5)) | x1 | (~x0 & ~x2 & x3) | ~x4 | (x2 & ~x3);
  assign z41 = ((~x0 | x2) & (x5 | (x1 & x4))) | (x1 & x3 & (x4 | x5)) | (~x1 & (x5 ? ~x3 : x4));
  assign z42 = x4 | (x5 & (x6 | x7));
  assign z43 = x4 ? ~new_n98_ : (x5 & (x6 | x7));
  assign z44 = (x4 & (x1 | (~x0 & ~x2 & x3))) | x0 | x2 | ~x4;
  assign z45 = (~x1 & (x4 | x5)) | (~x4 & x5) | (x4 & (x0 | ~x2));
  assign z46 = x2 | x0 | ~x1 | x3 | ~x4;
  assign z47 = (~x1 & (x4 | x5)) | (~x2 & (x4 | (x0 & x5))) | (x0 & (x4 | (~new_n90_ & x5))) | (~x0 & ~x4 & x5);
  assign z48 = (~new_n119_ & ~x4) | x0 | x1 | x2 | ~x3;
  assign new_n119_ = x5 & x6 & x7;
  assign z49 = x4 ? (x0 | x1 | ~x2 | x3) : x5;
  assign z50 = x4 | (~x4 & x5);
  assign z51 = (x1 & (~x0 | (~x2 & x3))) | (x2 & (~x1 | ~x4)) | (~new_n119_ & ~x4) | (~x1 & (~x4 | (~x2 & (x0 | ~x3))));
  assign z52 = (x1 & (~x0 | x3)) | ~x4 | (~x1 & (x2 ? x3 : (x0 | ~x3)));
  assign z53 = new_n126_ | new_n127_ | new_n129_ | new_n130_ | (~new_n125_ & ~new_n128_);
  assign new_n125_ = x1 & x4;
  assign new_n126_ = x1 & (x0 ? ~x3 : (x2 & x3));
  assign new_n127_ = x0 & (x3 ? ~x1 : x2);
  assign new_n128_ = x3 ? (x2 & x5 & x6 & x7) : ~x2;
  assign new_n129_ = x4 & (x3 ? ~x1 : ~x2);
  assign new_n130_ = ~x2 & ~x3 & (~x5 | ~x6 | ~x7);
  assign z54 = (x2 & (x3 ? ~x1 : x4)) | (~x1 & (x0 | (~x3 & x4))) | (x0 & (x3 | x4)) | (~new_n119_ & (~x4 | (~x2 & x3))) | (~x2 & ((x3 & x4) | (~x0 & ~x3 & ~x4)));
  assign z55 = (x0 & x4 & (x2 | ~x3)) | ~x1 | (~x4 & (~new_n119_ | ~x0 | ~x2));
  assign z56 = new_n134_ | ~new_n135_ | (x0 & (x4 | (x2 & x5)));
  assign new_n134_ = ~x1 & (x4 | (x2 & x3 & x5));
  assign new_n135_ = (~x4 | (~x2 & x3)) & (~x5 | (x2 ? (x6 & x7) : x4));
  assign z57 = new_n138_ | new_n139_ | new_n137_ | new_n140_;
  assign new_n137_ = x4 & (x2 | (~x0 & x3));
  assign new_n138_ = x0 & (x4 ? ~x3 : x5);
  assign new_n139_ = ~x1 & (x4 | (~x3 & x5 & ~x0 & x2));
  assign new_n140_ = x5 & ((~x2 & ~x4) | (~x0 & x2 & (~x6 | ~x7)));
  assign z58 = (~new_n142_ & x0) | ~x1 | ~x2 | ~x3 | (~x0 & ~x4);
  assign new_n142_ = ~x4 & x5 & x6 & x7;
  assign z59 = (~x0 & (x4 | (~x1 & x5))) | (x1 & ((~x2 & x5) | (x3 & x4))) | (~x4 & x5) | (x4 & (~x2 | (~x1 & ~x3)));
  assign z60 = (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n119_ | x1 | x4 | (x2 & ~x3)));
  assign z61 = ~x2 | ~x0 | x1 | ~x3 | ~x4;
  assign z62 = x4 ? (~x0 | ~x1 | x3) : x5;
  assign z09 = 1'b0;
  assign z20 = 1'b0;
  assign z21 = 1'b0;
  assign z25 = 1'b0;
  assign z04 = z00;
  assign z06 = z00;
  assign z22 = z00;
  assign z24 = z00;
  assign z26 = z00;
  assign z27 = z00;
  assign z28 = z00;
  assign z29 = z00;
  assign z30 = z00;
endmodule


