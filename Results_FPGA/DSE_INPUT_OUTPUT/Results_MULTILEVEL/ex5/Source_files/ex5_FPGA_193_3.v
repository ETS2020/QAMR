// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n78_, new_n82_, new_n84_, new_n86_, new_n89_, new_n91_, new_n96_,
    new_n98_, new_n104_, new_n105_, new_n108_, new_n110_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n127_, new_n129_, new_n131_, new_n133_, new_n135_,
    new_n136_, new_n140_, new_n141_, new_n142_, new_n144_, new_n146_,
    new_n147_, new_n149_, new_n151_, new_n153_, new_n156_, new_n158_,
    new_n161_, new_n164_, new_n165_, new_n167_, new_n168_, new_n172_;
  assign z00 = ~x4 & ~x5 & ~z15 & ~x6;
  assign z15 = x2 & x3;
  assign z01 = ~x5 & ~x6 & ~z15 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = x3 & (x2 | (new_n78_ & ~x6 & ~x7));
  assign new_n78_ = ~x4 & x5;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = ~x4 & x5 & x6 & ~z15 & ~x7;
  assign z07 = x7 & new_n82_ & x6;
  assign new_n82_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x2 & (x3 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x2 & (x3 | (new_n86_ & ~x0 & ~x1 & ~x4));
  assign new_n86_ = ~x5 & x6 & x7;
  assign z10 = x2 & (x3 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z11 = x7 & x6 & x5 & ~x4 & new_n89_ & ~x3;
  assign new_n89_ = ~x2 & x0 & x1;
  assign z12 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x3 & (x2 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z14 = x3 & (x2 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z16 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z17 = ~x5 & new_n96_ & x4;
  assign new_n96_ = ~x2 & x0 & ~x1;
  assign z19 = x4 & ~x3 & new_n98_ & ~x2;
  assign new_n98_ = ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n96_ & ~x3;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n96_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n96_ & ~x4;
  assign z23 = x3 & (x2 | (~x0 & ~x1 & x5));
  assign z24 = (x2 & x3) | (new_n98_ & ~x2 & ~x3 & new_n104_ & new_n105_);
  assign new_n104_ = ~x4 & ~x5;
  assign new_n105_ = x6 & ~x7;
  assign z25 = (x2 & x3) | (new_n104_ & new_n105_ & ~x0 & x1 & ~x2 & ~x3);
  assign z26 = ~new_n108_ & x2;
  assign new_n108_ = ~x3 & (x5 | ~x6 | ~x7 | ~x0 | x4);
  assign z27 = x2 & (new_n110_ | x3);
  assign new_n110_ = ~x0 & x1 & ~x4 & ~x5 & x6 & ~x7;
  assign z29 = (x2 & x3) | (new_n98_ & ~x2 & ~x3 & new_n104_ & ~x6 & x7);
  assign z30 = x2 & (x3 | (new_n86_ & x0 & x1 & ~x4));
  assign z31 = ~new_n114_ | (~x0 & (x3 | ~x4));
  assign new_n114_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = new_n116_ | (~x2 & (new_n117_ | ~new_n118_)) | (x2 & ~x3);
  assign new_n116_ = ~x4 & ((~x2 & x5) | (x0 & (~x3 | (~x2 & x6))));
  assign new_n117_ = x4 & (~x0 | ~x5);
  assign new_n118_ = ~x1 & (x0 | (x6 & ~x7 & ~x3 & ~x5));
  assign z33 = ~new_n120_ | ~x7 | x4 | ~x6;
  assign new_n120_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n123_ & ~x5) | ~new_n124_ | (~new_n122_ & (x2 | x5));
  assign new_n122_ = ~x4 & ~x7;
  assign new_n123_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n124_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 ? ~x3 : (x1 | (~x0 & x3) | ~x4 | (x0 & ~x5));
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n127_ & ~x0) | x1 | x5;
  assign new_n127_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | (~x2 & x5))) | (~x3 & (~x1 | x2)) | (~x2 & x3 & (x5 ? x1 : ~new_n129_));
  assign new_n129_ = ~x4 & x6 & ~x7;
  assign z38 = new_n116_ | (x2 & ~x3) | (~x2 & (new_n131_ | x1));
  assign new_n131_ = ~x0 & (x5 | ~x6 | x7 | x3 | x4);
  assign z39 = (~new_n133_ & ~x2) | (~x3 & (x2 | x5));
  assign new_n133_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = x2 ? new_n108_ : (new_n135_ | new_n136_ | new_n78_ | x1);
  assign new_n135_ = x0 & (x4 ? ~x5 : x6);
  assign new_n136_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | (~x2 & ~x5))) | (x2 & ~x3) | (~x2 & (~x0 | (x1 & x3)));
  assign z42 = ~new_n133_ | (x2 & (x3 | ~x5));
  assign z43 = x2 ? new_n142_ : ((~new_n140_ & (x4 | ~x5)) | (~new_n141_ & ~x4));
  assign new_n140_ = ~x1 & (x0 | ~x3);
  assign new_n141_ = (x0 | (~x7 & (x5 | x6))) & (~x5 | (~x6 & ~x7)) & (~x0 | ~x6 | x7);
  assign new_n142_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = (x0 & (~new_n144_ | x4)) | x1 | x2 | x3 | (~x0 & ~x4);
  assign new_n144_ = ~x5 & ~x6;
  assign z45 = (x0 & (~x2 | ~x3)) | new_n147_ | (~new_n146_ & ~x3);
  assign new_n146_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign new_n147_ = ~x2 & (x5 | ~x6 | ~x7 | x1 | x4);
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n149_ | ~x1 | x3));
  assign new_n149_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | new_n147_ | (x2 & ~x3 & (new_n151_ | ~x1));
  assign new_n151_ = ~x4 & (x5 | x6);
  assign z48 = ~new_n153_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n153_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (~x3 & (~new_n98_ | new_n151_));
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n156_ & ~x2) | (x2 & ~x3);
  assign new_n156_ = ~x4 & ~x5 & x6 & x7;
  assign z51 = (x0 & (~x1 | x3)) | (x2 & (new_n78_ | ~x0)) | new_n158_ | (~x0 & (x1 | ~x3));
  assign new_n158_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x0 & (x5 | x6)));
  assign z52 = (x1 & (~x0 | x3)) | (x3 & (x0 | x2)) | new_n151_ | (~x1 & ~x2 & ~x3);
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n151_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n161_ & ~x3);
  assign new_n161_ = ~x4 & x5 & x6 & x7;
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n151_ & ~x0))) | (~new_n161_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n164_ | (x0 & ~new_n165_ & ~x3) | ~x1 | (x2 & x3);
  assign new_n164_ = ~x4 & (x5 | x6) & (~x0 | x3);
  assign new_n165_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n167_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n168_ & x2);
  assign new_n167_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n168_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | ~x3)) | (~x2 & (new_n149_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n161_ & x2)));
  assign z58 = ~x3 | (~x2 & (~new_n86_ | x0 | x1 | x4));
  assign z59 = (~new_n156_ & (~x0 | ~x2)) | new_n172_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n172_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n84_ | x1 | x2 | x4));
  assign z62 = ~x0 | x3 | (x0 & (new_n151_ | ~x1));
  assign z06 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = 1'b1;
  assign z18 = z15;
endmodule


