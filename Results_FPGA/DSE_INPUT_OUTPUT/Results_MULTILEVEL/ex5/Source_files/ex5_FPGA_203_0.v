// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n83_, new_n85_, new_n87_, new_n89_, new_n93_, new_n95_,
    new_n97_, new_n101_, new_n104_, new_n107_, new_n110_, new_n112_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n128_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n149_, new_n152_,
    new_n156_, new_n157_, new_n160_, new_n162_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_;
  assign z00 = z19 | (new_n75_ & ~x4);
  assign z19 = ~x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z19 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & (~x2 | (~x6 & ~x7 & ~x4 & x5));
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = ~x4 & x5 & x6 & ~z19 & ~x7;
  assign z06 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & new_n75_ & x3 & ~x4);
  assign z08 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = ~x3 & (new_n85_ | ~x2);
  assign new_n85_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z10 = (~x2 & ~x3) | (new_n87_ & ~x0 & x1 & x2);
  assign new_n87_ = ~x4 & x5 & x6 & x7;
  assign z12 = ~x3 & (~x2 | (new_n89_ & x0 & ~x1 & ~x4));
  assign new_n89_ = x5 & x6 & x7;
  assign z13 = ~x2 & (~x3 | (new_n89_ & ~x0 & x1 & ~x4));
  assign z14 = ~x2 & (~x3 | (new_n89_ & x0 & ~x1 & ~x4));
  assign z15 = x7 & x6 & x5 & ~x4 & new_n93_ & x3;
  assign new_n93_ = x2 & ~x0 & x1;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x2 & (~x3 | (new_n97_ & x4 & ~x5));
  assign new_n97_ = x0 & ~x1;
  assign z18 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & x3 & x4 & ~x5);
  assign z21 = ~x2 & (~x3 | (new_n97_ & new_n75_ & ~x4));
  assign z22 = x7 & new_n101_ & x6;
  assign new_n101_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z26 = new_n104_ & x7;
  assign new_n104_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n93_ & ~x3;
  assign z28 = (~x2 & ~x3) | (new_n107_ & new_n97_ & x2 & x3);
  assign new_n107_ = ~x4 & ~x5 & x6 & x7;
  assign z30 = x7 & x6 & new_n83_ & ~x5;
  assign z31 = (~x5 & (~x0 | x4)) | (~x0 & (~x2 | ~x4)) | ~new_n110_ | (~x4 & (x5 | x6));
  assign new_n110_ = ~x1 & x3 & (~x0 | ~x2);
  assign z32 = (~new_n112_ & x3) | (x2 & (x0 | x1 | ~x3 | ~x4));
  assign new_n112_ = (~x1 | (x2 & x5)) & (x2 | ((x4 | (~x5 & ~x6)) & x0 & (~x4 | x5)));
  assign z33 = (x3 & (~x2 | (x1 & ~x5))) | (~new_n114_ & x2);
  assign new_n114_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = new_n116_ | new_n117_ | (~new_n118_ & x3) | (x2 & ~new_n119_ & ~x3);
  assign new_n116_ = x1 & (x3 ? ~x5 : x2);
  assign new_n117_ = (x2 | (x3 & x5)) & (x4 | x7);
  assign new_n118_ = (~x2 | (x5 & ~x6)) & (x5 | (x0 & (x4 | (x6 & x7)))) & (~x5 | ~x6);
  assign new_n119_ = ~x0 & ~x5 & x6;
  assign z35 = ~x5 | ~x3 | ~x4 | x1 | (x0 & x2) | (~x0 & ~x2);
  assign z36 = (~new_n122_ & x0) | x1 | x5 | (~new_n123_ & ~x0);
  assign new_n122_ = ~x2 & x3 & x4;
  assign new_n123_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (x2 & (~x3 | x5)) | (x3 & (x5 ? ~new_n97_ : ~new_n125_));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = new_n127_ | new_n128_ | ~new_n110_;
  assign new_n127_ = ~x0 & (~x2 | ~x4);
  assign new_n128_ = ~x4 & (x5 | x6);
  assign z39 = (~new_n130_ & x3) | (x2 & (~x3 | ~x5));
  assign new_n130_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = new_n132_ | new_n127_ | new_n133_ | new_n134_ | (new_n128_ & ~x2);
  assign new_n132_ = (~x0 | ~x2) & (x1 | ~x3);
  assign new_n133_ = x4 & (x2 ? x0 : ~x5);
  assign new_n134_ = x0 & x2 & (x3 | x5 | ~x6 | ~x7);
  assign z41 = x2 | (x3 & (~x0 | x1 | ~x5));
  assign z42 = ~new_n130_ | (~x3 & (~x2 | ~x5));
  assign z43 = (~new_n138_ & ~x4) | new_n141_ | (x3 & (new_n139_ | new_n140_));
  assign new_n138_ = (~x5 | (~x6 & ~x7) | (~x2 & ~x3)) & (x7 | (x2 ? x5 : (~x3 | ~x6))) & (~x2 | x5 | (x0 & x6));
  assign new_n139_ = (x4 | ~x5) & (x1 | (~x0 & ~x2));
  assign new_n140_ = ~x0 & ~x2 & (x6 | x7);
  assign new_n141_ = x2 & x4 & (x0 | x1 | ~x3);
  assign z45 = (~new_n143_ & x1) | (x5 & (~x1 | ~x4)) | x0 | (~new_n144_ & ~x1);
  assign new_n143_ = x2 & (x4 | ~x6);
  assign new_n144_ = ~x2 & x3 & ~x4 & x6 & x7;
  assign z47 = (new_n146_ & ~x0) | ((x0 | x1) & ~x2) | new_n147_ | (~x1 & (x2 | x5)) | (x0 & (~x1 | ~x5));
  assign new_n146_ = ~x4 & (x5 | (x1 & x6));
  assign new_n147_ = (x0 | ~x1) & (~x6 | ~x7 | ~x3 | x4);
  assign z48 = x2 | (x3 & (new_n149_ | x0 | x1));
  assign new_n149_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = (x0 & (x2 | (~x1 & x3))) | (x3 & (~x2 | x4)) | (x2 & (x1 | (~new_n75_ & ~x4)));
  assign z50 = x2 | (x3 & (new_n97_ | x4 | (~x2 & (~new_n152_ | x5))));
  assign new_n152_ = x6 & x7;
  assign z51 = (x0 & (x2 ? ~x1 : x3)) | (~x0 & ((x1 & (x2 | x3)) | (x2 & (~x3 | x4)) | (x3 & ~new_n75_ & ~x4))) | (x2 & ~new_n75_ & ~x4);
  assign z52 = (~x0 & ((x1 & (x2 | x3)) | (x3 & (x4 ? x2 : ~new_n75_)))) | (~x4 & ~new_n75_ & (x0 | x2)) | (~x2 & ~x3) | (x0 & (~x2 | x3));
  assign z53 = ((~x1 | ~x3) & (x0 | ~x2)) | new_n156_ | (~new_n157_ & ~x1) | (~x0 & x1 & x2 & x3);
  assign new_n156_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | ((x5 | x6) & (~x2 | ~x3)));
  assign new_n157_ = x3 & ~x4 & x5 & x6 & x7;
  assign z54 = (x0 & (~x1 | x3)) | (~new_n87_ & (~x2 | ~x3)) | new_n149_ | (~x2 & ~x3) | (~x1 & x2 & x3);
  assign z55 = new_n160_ | (~x1 & (x2 | x3)) | (x0 & ~new_n87_ & x2);
  assign new_n160_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : x3);
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n162_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n162_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x1 & (x0 | ~x3)) | new_n164_ | (~new_n165_ & ~x0) | (x0 & (x2 | ~x3));
  assign new_n164_ = ~x4 & ((x6 & ~x7) | (x0 & x5));
  assign new_n165_ = x2 & ~x4 & x5 & x6 & x7;
  assign z58 = x2 ? (new_n167_ | new_n168_ | ~x1 | ~x3) : (~new_n85_ & x3);
  assign new_n167_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n168_ = x0 & (x4 | ~x5 | ~x6 | ~x7);
  assign z59 = (~new_n170_ & x0) | (~new_n171_ & x2) | (~new_n107_ & (x2 ? ~x0 : x3));
  assign new_n170_ = (x1 | (x2 ^ ~x3)) & (~x2 | x4 | ~x6);
  assign new_n171_ = (x0 | (~x1 & ~x3)) & (x4 | ~x5) & (~x1 | (~x3 & (x4 | ~x6)));
  assign z60 = (x0 & (~x1 | x3 | ~x4)) | ~x2 | (~x0 & (~new_n89_ | x1 | ~x3 | x4));
  assign z61 = x2 ? (new_n128_ | ~x0 | x1 | ~x3) : x3;
  assign z62 = new_n128_ | ~x0 | ~x1 | ~x2 | x3;
  assign z07 = 1'b0;
  assign z11 = 1'b0;
  assign z29 = 1'b0;
  assign z44 = 1'b1;
  assign z46 = ~z19;
  assign z20 = z19;
  assign z24 = z19;
  assign z25 = z19;
endmodule


