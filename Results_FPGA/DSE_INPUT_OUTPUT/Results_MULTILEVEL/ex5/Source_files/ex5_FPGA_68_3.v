// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_,
    new_n104_, new_n107_, new_n109_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n156_, new_n158_, new_n161_,
    new_n163_, new_n164_, new_n166_, new_n169_, new_n172_, new_n174_,
    new_n177_;
  assign z00 = z08 | (new_n75_ & ~x4);
  assign z08 = x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z08 & ~x7;
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z04 = z08 | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z08 | (x6 & ~x7 & ~x4 & x5);
  assign z06 = x2 & (x0 | (new_n75_ & ~x4 & ~x1 & x3));
  assign z07 = (x0 & x2) | (new_n83_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n83_ = ~x4 & x5 & x6 & x7;
  assign z09 = x7 & new_n85_ & x6;
  assign new_n85_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x2 & (x0 | (new_n87_ & x1 & ~x4));
  assign new_n87_ = x5 & x6 & x7;
  assign z11 = x7 & new_n89_ & x6;
  assign new_n89_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n91_ & x3;
  assign new_n91_ = ~x2 & ~x0 & x1;
  assign z14 = x7 & x6 & x5 & ~x4 & new_n93_ & x3;
  assign new_n93_ = ~x2 & x0 & ~x1;
  assign z15 = x2 & (x0 | (new_n87_ & x1 & x3 & ~x4));
  assign z16 = x0 & (x2 | (new_n87_ & x1 & x3 & ~x4));
  assign z17 = ~x5 & new_n93_ & x4;
  assign z18 = x2 & (x0 | (x4 & ~x5 & ~x1 & x3));
  assign z19 = (x0 & x2) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = x0 & (x2 | (new_n75_ & ~x4 & ~x1 & ~x3));
  assign z21 = ~x6 & ~x5 & ~x4 & new_n93_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n93_ & ~x4;
  assign z23 = (x0 & x2) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = (x0 & x2) | (new_n104_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n104_ = ~x4 & ~x5 & x6 & ~x7;
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n91_ & ~x3;
  assign z27 = x2 & (new_n107_ | x0);
  assign new_n107_ = ~x5 & x6 & ~x7 & x1 & ~x3 & ~x4;
  assign z29 = x7 & new_n109_ & ~x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | ~new_n111_ | (x2 & (x0 | ~x3 | ~x4));
  assign new_n111_ = ~x1 & (~x4 | x5) & (x4 | (~x5 & ~x6));
  assign z32 = new_n116_ | (~x2 & (new_n113_ | new_n114_ | ~new_n115_));
  assign new_n113_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n114_ = x4 & (~x0 | ~x5);
  assign new_n115_ = ~x1 & (x0 | (x6 & ~x7 & ~x3 & ~x5));
  assign new_n116_ = ~x0 & (x1 | (x2 & (~x3 | ~x4)));
  assign z34 = new_n118_ | new_n119_ | (~new_n120_ & ~x5) | (x5 & (~x3 | x6));
  assign new_n118_ = x0 & (x2 | (~x4 & ~x5 & ~x7));
  assign new_n119_ = (~x0 | x5) & (x4 | x7);
  assign new_n120_ = (x6 | (x0 & x4)) & ~x1 & (x0 | (x2 & ~x3));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = ((~x0 | ~x2) & (x1 | x5)) | (~new_n123_ & ~x0) | (~x2 & (~x0 | ~x4));
  assign new_n123_ = ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x3 & (~x0 | (~x1 & ~x2))) | (~new_n125_ & (x2 ? ~x0 : (x3 & ~x5))) | (x5 & (~x0 | (x1 & ~x2 & x3)));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = new_n128_ | new_n129_ | (new_n127_ & x0) | x1 | (new_n130_ & ~x0);
  assign new_n127_ = ~x4 & (x5 | x6);
  assign new_n128_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n129_ = x2 & (x0 | ~x4);
  assign new_n130_ = ~x2 & (x5 | ~x6 | x7 | x3 | x4);
  assign z39 = (x2 & (x0 | ~x5)) | ~new_n133_ | (~new_n132_ & x5);
  assign new_n132_ = x3 & ~x6 & ~x7;
  assign new_n133_ = ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = (~new_n135_ & ~x2) | (~x0 & (~new_n136_ | (x2 & (~x3 | ~x4))));
  assign new_n135_ = (~x0 | (x4 ? x5 : ~x6)) & ~x1 & (x4 | ~x5) & (x0 | ~x3);
  assign new_n136_ = ~x1 & (x4 | (~x5 & x6 & ~x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n139_ | (~z08 & x4);
  assign new_n139_ = ((x0 & (x2 | ~x5)) | (~x6 & ~x7)) & (x5 | (x0 & (x2 | (~x1 & x6 & x7))));
  assign z43 = (~x0 & (new_n143_ | new_n144_)) | ((new_n141_ | new_n142_) & ~x2);
  assign new_n141_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n142_ = ~x4 & ((x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n143_ = (x1 | (x2 & ~x3)) & x4;
  assign new_n144_ = ~x4 & ((x2 & (~x5 | x6)) | (x5 & x6) | x7 | (~x5 & ~x6));
  assign z44 = new_n147_ | new_n148_ | x2 | ((new_n141_ | new_n146_) & ~x2);
  assign new_n146_ = ~x4 & x5 & (x6 | x7);
  assign new_n147_ = ~x0 & (~x4 | (x1 & x4));
  assign new_n148_ = x0 & (x5 | x6 | x3 | x4);
  assign z45 = new_n150_ | new_n151_ | new_n152_ | x0;
  assign new_n150_ = x1 & (~x2 | (~x4 & x6));
  assign new_n151_ = x5 & (~x1 | ~x4);
  assign new_n152_ = ~x1 & (~x6 | ~x7 | x2 | x4);
  assign z46 = new_n154_ | x0 | ~x1 | x2 | x3;
  assign new_n154_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (~x2 | (~x0 & ~x4 & x6))) | (~new_n156_ & ~x0) | (x0 & ~x2);
  assign new_n156_ = (~x5 | (x1 & x4)) & (x1 | (~x2 & ~x4 & x6 & x7));
  assign z48 = new_n158_ | x2 | ~x3 | x0 | x1;
  assign new_n158_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~x2 | (~x0 & ((~new_n75_ & ~x4) | x1 | (x3 & x4)));
  assign z50 = ~new_n161_ | (x0 & (~x1 | ~x3));
  assign new_n161_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z51 = (x2 & (x0 | x4)) | (~new_n163_ & x0) | new_n158_ | (~new_n164_ & ~x0);
  assign new_n163_ = x1 & ~x3;
  assign new_n164_ = (x3 | (x2 & x4)) & ~x1 & (x4 | (~x5 & ~x6));
  assign z52 = (x2 & (x0 | (x3 & x4))) | (x0 & (~x1 | x3)) | (~new_n166_ & ~x4) | (~x0 & (x1 | (~x2 & ~x3)));
  assign new_n166_ = ~x5 & (~x6 | (x0 & x5));
  assign z53 = (x1 & (x0 ? (~x2 & ~x3) : (x2 & x3))) | (~new_n83_ & ((~x2 & ~x3) | (~x0 & ~x1 & x3))) | ((new_n127_ | ~x1) & ((~x2 & x3) | (~x0 & x2 & ~x3)));
  assign z54 = (~x1 & ((~x2 & ~x3) | (~x0 & x2 & x3))) | (~new_n169_ & ~x0) | (~x2 & ((~new_n83_ & (x0 | x3)) | (x0 & x3)));
  assign new_n169_ = x3 ? (x4 | (x5 ? (x6 & x7) : ~x6)) : (x2 ? (~x4 & x5 & x6 & x7) : (x4 | (~x5 & ~x6)));
  assign z55 = (x0 & ~x2 & ~x3) | ((~x0 | ~x2) & (new_n127_ | ~x1));
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n172_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n172_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x1 & (~x2 | (~x0 & ~x3))) | new_n174_ | (~x2 & (x0 ^ x3)) | (~x0 & ~new_n83_ & x2);
  assign new_n174_ = ~x4 & ((~x2 & (x5 | (x6 & ~x7))) | (~x0 & x6 & ~x7));
  assign z58 = new_n150_ | new_n151_ | new_n152_ | x0 | ~x3;
  assign z59 = (~x2 & (~new_n177_ | (x0 & (~x1 | ~x3)))) | (~x0 & (~new_n177_ | (x2 & (x1 | x3))));
  assign new_n177_ = ~x4 & ~x5 & x6 & x7;
  assign z60 = (x2 & (x0 | ~x3)) | (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n87_ | x1 | x4));
  assign z62 = ~x0 | (~x2 & (new_n127_ | ~new_n163_));
  assign z26 = 1'b0;
  assign z28 = 1'b0;
  assign z30 = 1'b0;
  assign z33 = 1'b1;
  assign z61 = ~z08;
  assign z12 = z08;
endmodule


