// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n82_, new_n84_, new_n86_, new_n89_, new_n91_, new_n93_,
    new_n95_, new_n100_, new_n102_, new_n103_, new_n105_, new_n108_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n129_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n150_, new_n151_,
    new_n153_, new_n155_, new_n159_, new_n161_, new_n162_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n177_;
  assign z00 = ~x4 & ~x5 & ~z16 & ~x6;
  assign z16 = ~x2 & x3;
  assign z01 = z16 | (~x5 & ~x6 & ~x7);
  assign z02 = (~x2 & x3) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = x3 & (~x2 | (~x6 & ~x7 & ~x4 & x5));
  assign z04 = x3 & (new_n79_ | ~x2);
  assign new_n79_ = ~x4 & ~x5 & x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z16 & ~x7;
  assign z06 = ~x6 & ~x5 & ~x4 & new_n82_ & x3;
  assign new_n82_ = x2 & ~x0 & ~x1;
  assign z07 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & new_n86_ & x6;
  assign new_n86_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & x6 & ~x5 & ~x4 & new_n82_ & ~x3;
  assign z10 = (~x2 & x3) | (new_n89_ & ~x0 & x1 & x2);
  assign new_n89_ = ~x4 & x5 & x6 & x7;
  assign z11 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n95_ & x3;
  assign new_n95_ = x2 & ~x0 & x1;
  assign z17 = ~x2 & (x3 | (x0 & ~x1 & x4 & ~x5));
  assign z18 = ~x5 & x4 & new_n82_ & x3;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x2 & (x3 | (x0 & ~x1 & new_n100_ & ~x4));
  assign new_n100_ = ~x5 & ~x6;
  assign z22 = ~x2 & (x3 | (new_n103_ & new_n102_ & x0));
  assign new_n102_ = ~x1 & ~x4;
  assign new_n103_ = ~x5 & x6 & x7;
  assign z24 = ~x2 & (x3 | (new_n105_ & new_n102_ & ~x0));
  assign new_n105_ = ~x5 & x6 & ~x7;
  assign z25 = ~x7 & x6 & new_n84_ & ~x5;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n108_ & ~x3;
  assign new_n108_ = x0 & x2;
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n95_ & ~x3;
  assign z28 = x3 & (~x2 | (new_n103_ & new_n102_ & x0));
  assign z29 = x7 & new_n112_ & ~x6;
  assign new_n112_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = (~x2 & x3) | (new_n114_ & x0 & x1 & x2 & ~x3);
  assign new_n114_ = ~x4 & ~x5 & x6 & x7;
  assign z31 = ~new_n116_ | (~z16 & x1);
  assign new_n116_ = (x5 | (~x2 & (x3 | ~x4))) & (~x2 | (~x0 & x3 & x4)) & (x3 | x4 | (x0 & ~x5 & ~x6));
  assign z32 = new_n118_ | new_n119_ | (~x2 & (new_n120_ | x3)) | x1 | (x2 & ~x3);
  assign new_n118_ = ~x4 & (x0 | x2);
  assign new_n119_ = x0 & (x2 | ~x5);
  assign new_n120_ = ~x0 & (x4 | x5 | ~x6 | x7);
  assign z33 = ~new_n122_ | ~x7 | x4 | ~x6;
  assign new_n122_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n124_ | new_n125_ | new_n126_ | (x3 & (~x5 | x6));
  assign new_n124_ = (x1 | x5) & (~x2 | ~x3);
  assign new_n125_ = x2 & ((~x3 & (x0 | ~x6)) | x4 | x7);
  assign new_n126_ = ~x2 & (~x0 | x3 | (~x4 & (~x6 | ~x7)));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x2 & x3);
  assign z36 = ~new_n129_ | (~z16 & (x1 | x5));
  assign new_n129_ = x2 ? (~x0 & ~x3 & ~x4 & x6 & ~x7) : (x3 | (x0 & x4));
  assign z37 = x2 ? (~new_n105_ | ~x3 | x4) : (~x0 | ~x1 | x3);
  assign z38 = (x3 & (x0 | ~x2)) | (x0 & (x2 | ~x4)) | (x2 & (~x3 | ~x4)) | x1 | (~x0 & ~new_n79_ & ~x2);
  assign z39 = (~new_n133_ & x2) | x4 | (~new_n134_ & ~x2);
  assign new_n133_ = ~x6 & ~x7 & x3 & x5;
  assign new_n134_ = ~x5 & x6 & x7 & x0 & ~x1 & ~x3;
  assign z40 = (x1 & (~x0 | ~x2)) | (x3 & (x0 | ~x2)) | new_n136_ | (~new_n137_ & x0) | (~x0 & x2 & ~x3);
  assign new_n136_ = ~x4 & ((~x2 & (x5 | (x0 & x6))) | (~x0 & (x2 | ~x6 | x7)));
  assign new_n137_ = (~x4 | (~x2 & x5)) & (~x2 | (~x5 & x6 & x7));
  assign z41 = x2 | x3 | ~x0 | ~x1;
  assign z42 = (~new_n140_ & (x2 | ~x3)) | (x2 & ~x3 & ~x5);
  assign new_n140_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = new_n142_ | new_n143_ | new_n144_ | new_n145_ | new_n146_;
  assign new_n142_ = x0 & ((x2 & x4) | (~x2 & ~x4 & x6 & ~x7));
  assign new_n143_ = x3 & (~x2 | (x1 & ~x5));
  assign new_n144_ = x1 & (x4 | (~x2 & ~x5));
  assign new_n145_ = ~x4 & ((x5 & (x6 | x7)) | (~x5 & ((~x0 & (x2 | ~x6)) | (x2 & (~x6 | ~x7)))) | (~x0 & ~x2 & x7));
  assign new_n146_ = x2 & ~x3 & x4;
  assign z44 = x2 | (~new_n148_ & ~x3);
  assign new_n148_ = (x4 | (x0 & ~x5)) & (~x1 | x2) & (~x0 | (~x4 & ~x5 & ~x6));
  assign z45 = new_n150_ | (new_n151_ & ~x2) | (x2 & (~x1 | (~x4 & x6)));
  assign new_n150_ = (x2 | ~x3) & (x0 | (~x4 & x5));
  assign new_n151_ = ~x3 & (x1 | x4 | (~x0 & ~x4 & (~x6 | ~x7)));
  assign z46 = new_n153_ | x0 | ~x1 | x2 | x3;
  assign new_n153_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n155_ & x2) | (~x3 & (x0 | (~x2 & (~new_n102_ | ~new_n103_))));
  assign new_n155_ = (x0 | x4 | (~x5 & ~x6)) & x1 & (~x0 | (~x4 & x5 & x6 & x7));
  assign z49 = (x0 & (x2 | ~x3)) | (~x2 & ~x3) | (x2 & ((~new_n100_ & ~x4) | x1 | (x3 & x4)));
  assign z50 = x2 | (~x3 & (x0 | (~new_n114_ & ~x2)));
  assign z51 = (~x1 & (x2 ? x0 : ~x3)) | new_n159_ | (~x0 & (~x3 | (x2 & (x1 | x4))));
  assign new_n159_ = ~x4 & (x2 ? (x5 | x6) : (~x3 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = new_n161_ | new_n159_ | ~new_n162_;
  assign new_n161_ = x0 & (x3 ? x2 : (~x4 & x5));
  assign new_n162_ = (x1 | x2 | x3) & (x0 | (x2 ? (~x1 & (~x3 | ~x4)) : x3));
  assign z53 = (x1 & (x0 ? ~x3 : (x2 & x3))) | (x2 & (~new_n164_ | (x0 & (~x1 | ~x3)))) | (~x2 & ~new_n89_ & ~x3);
  assign new_n164_ = ((x3 & x5) | (x1 & (x4 | ~x6))) & ((x6 & x7) | (x1 & (x4 | ~x5))) & (x1 | ~x4) & (x3 | x4 | ~x5);
  assign z54 = (~new_n166_ & ~x4) | new_n167_ | (~new_n89_ & ~new_n168_) | new_n169_;
  assign new_n166_ = (x0 | x2 | (~x5 & ~x6)) & (~x3 | (x5 ? (x6 & x7) : ~x6));
  assign new_n167_ = (x0 | ~x2) & (~x1 | x3);
  assign new_n168_ = ~x0 & (~x2 | x3);
  assign new_n169_ = ~x1 & x3;
  assign z55 = new_n171_ | (~x1 & (x2 | (~x2 & ~x3))) | (x0 & (x2 ? ~new_n89_ : ~x3));
  assign new_n171_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : ~x3);
  assign z56 = x2 ? (~new_n173_ | x0 | x4 | (~x1 & x3)) : ~x3;
  assign new_n173_ = x5 & x6 & x7;
  assign z57 = (x0 & (x2 | (~x2 & ~x3))) | (~new_n89_ & x2) | (~x3 & (~x1 | (new_n153_ & ~x2)));
  assign z58 = ~x3 | (~new_n155_ & x2);
  assign z59 = (x2 & (~new_n177_ | (x3 & (~x0 | x1)))) | (~x3 & ((~new_n114_ & ~x2) | (x0 & (~x1 | ~x2))));
  assign new_n177_ = (~x1 | (x0 & (x4 | ~x6))) & (x4 | (~x5 & (~x0 | ~x6))) & (x0 | (~x4 & ~x5 & x6 & x7));
  assign z60 = (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n173_ | x1 | x4 | (x2 & ~x3)));
  assign z61 = ~x3 | (x2 & (~x0 | x1 | (~new_n100_ & ~x4)));
  assign z62 = (~new_n100_ & ~x4) | ~x0 | ~x1 | x3;
  assign z13 = 1'b0;
  assign z14 = 1'b0;
  assign z21 = 1'b0;
  assign z48 = 1'b1;
  assign z23 = z16;
endmodule


