// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n77_, new_n79_, new_n82_, new_n84_, new_n88_, new_n90_, new_n91_,
    new_n101_, new_n104_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n125_, new_n126_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n142_, new_n144_, new_n148_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n166_, new_n168_, new_n170_, new_n172_, new_n173_;
  assign z00 = ~x6 & ~x4 & ~x5;
  assign z01 = (x2 & x6) | (~x5 & ~x6 & ~x7);
  assign z02 = (x2 & x6) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = ~x7 & ~x6 & new_n77_ & x5;
  assign new_n77_ = x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & new_n79_ & ~x4;
  assign new_n79_ = ~x2 & x3;
  assign z05 = x6 & (x2 | (~x4 & x5 & ~x7));
  assign z06 = x2 & (x6 | (new_n82_ & x3 & ~x4 & ~x5));
  assign new_n82_ = ~x0 & ~x1;
  assign z07 = x6 & (x2 | (new_n84_ & ~x0 & x1 & ~x3));
  assign new_n84_ = ~x4 & x5 & x7;
  assign z08 = x2 & x6;
  assign z11 = x6 & (x2 | (new_n84_ & x0 & x1 & ~x3));
  assign z13 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & x3 & new_n91_ & ~x2;
  assign new_n91_ = x0 & ~x1;
  assign z16 = x6 & (x2 | (new_n84_ & x0 & x1 & x3));
  assign z17 = ~x5 & x4 & new_n91_ & ~x2;
  assign z18 = ~x6 & ~x5 & x4 & x3 & new_n82_ & x2;
  assign z19 = x4 & ~x3 & new_n82_ & ~x2;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n91_ & ~x2;
  assign z21 = (x2 & x6) | (x0 & ~x1 & ~x2 & new_n77_ & ~x5 & ~x6);
  assign z22 = x6 & (x2 | (new_n91_ & ~x4 & ~x5 & x7));
  assign z23 = x5 & x3 & new_n82_ & ~x2;
  assign z24 = x6 & (x2 | (new_n101_ & ~x0 & ~x1 & ~x3));
  assign new_n101_ = ~x4 & ~x5 & ~x7;
  assign z25 = x6 & (x2 | (new_n101_ & ~x0 & x1 & ~x3));
  assign z29 = x7 & new_n104_ & ~x6;
  assign new_n104_ = ~x5 & ~x4 & ~x3 & new_n82_ & ~x2;
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | ~new_n106_ | (x6 & (x2 | ~x4));
  assign new_n106_ = (x4 | (~x2 & ~x5)) & (~x2 | (~x0 & x3)) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n108_ | (~z08 & x1);
  assign new_n108_ = (x3 | ((~x0 | x2 | x4) & (~x2 | x6))) & (~x0 | (x2 ? x6 : (x4 | ~x6))) & (new_n109_ | x2) & (~x2 | x4 | x6);
  assign new_n109_ = (~x4 | (x0 & x5)) & (~x5 | (x0 & x4)) & (x0 | (~x3 & x6 & ~x7));
  assign z34 = (~new_n112_ & (x2 ? ~x6 : x5)) | (~new_n111_ & ~x5) | (~x2 & x5 & x6);
  assign new_n111_ = (x4 | (x6 & (x2 | x7))) & (new_n91_ | x2) & (~x2 | x6);
  assign new_n112_ = x3 & ~x4 & ~x7;
  assign z35 = (x0 & (x2 ? ~x6 : ~x5)) | (~new_n114_ & (~x2 | ~x6)) | (x2 & ~x6 & (~x3 | ~x5)) | (~x0 & ~x2 & x3);
  assign new_n114_ = ~x1 & x4;
  assign z36 = (~new_n114_ & (~x2 | ~x6)) | (x2 & ~x6) | (~x2 & (~x0 | x5));
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n117_)) | x2 | (~x1 & ~x3);
  assign new_n117_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (~x2 | ~x6)) | ~new_n119_ | (~x3 & (new_n121_ | (x2 & ~x6)));
  assign new_n119_ = (~x0 | (x2 ? x6 : (x4 | ~x6))) & (x4 | (x2 ? x6 : ~x5)) & (x0 | new_n120_ | x2);
  assign new_n120_ = ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign new_n121_ = x0 & ~x2 & ~x4;
  assign z39 = ~new_n123_ | (~z08 & x4);
  assign new_n123_ = (x6 | (x3 & x5 & ~x7)) & (x2 | ~x6 | (new_n91_ & ~x5 & x7));
  assign z40 = (x1 & (~x2 | ~x6)) | (~new_n125_ & ~x2) | (~new_n126_ & ~x6);
  assign new_n125_ = (~x0 | (x4 ? x5 : ~x6)) & (x4 | ~x5) & (x0 | (~x3 & (x4 | ~x7)));
  assign new_n126_ = (~x2 | (~x0 & x3)) & (x4 | (x0 & ~x2));
  assign z41 = x2 ? ~x6 : ((~x1 & (~x3 | ~x5)) | ~x0 | (x1 & x3));
  assign z42 = (x4 & (~x2 | ~x6)) | (~x2 & x6 & (~new_n91_ | x5 | ~x7)) | (~x6 & (~x5 | x7));
  assign z43 = new_n134_ | ~new_n132_ | (~new_n130_ & ~x2);
  assign new_n130_ = (new_n131_ | (~x4 & x5)) & (x4 | ((~x5 | (~x6 & ~x7)) & (x0 | ~x7)));
  assign new_n131_ = ~x1 & (x0 | ~x3);
  assign new_n132_ = x6 ? ~x2 : (x4 ? (~x1 & (~x2 | x3)) : new_n133_);
  assign new_n133_ = (x0 | x5) & (~x2 | (x5 & ~x7));
  assign new_n134_ = x0 & ((x2 & x4 & ~x6) | (~x2 & ~x4 & x6 & ~x7));
  assign z44 = (~x2 & (~new_n137_ | (~new_n136_ & ~new_n131_))) | (~x6 & (~new_n138_ | x2));
  assign new_n136_ = ~x4 & x5;
  assign new_n137_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign new_n138_ = (~x1 | ~x4) & (x0 | x4 | x5);
  assign z45 = ~new_n140_ | (~z08 & x0);
  assign new_n140_ = (~x5 | (x2 & (x4 | x6))) & (x6 | (x1 & x2)) & (x2 | (~x1 & ~x4 & x7));
  assign z46 = new_n142_ | x2 | x3 | x0 | ~x1;
  assign new_n142_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (~x2 | x6)) | ~new_n144_ | (x5 & (~x1 | ~x4));
  assign new_n144_ = ~x0 & (x1 | (~x2 & ~x4 & x6 & x7));
  assign z48 = ~new_n79_ | ~new_n82_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign z49 = ~x2 | (~x6 & (~new_n82_ | (~x4 & x5) | (x3 & x4)));
  assign z50 = new_n148_ | x2 | x4 | x5 | ~x6 | ~x7;
  assign new_n148_ = x0 & (~x1 | ~x3);
  assign z51 = (x0 & (new_n79_ | ~x1)) | new_n150_ | z08 | (~new_n151_ & ~x0);
  assign new_n150_ = ~x4 & ((~x2 & ((~x0 & (x5 | x6)) | (x5 & ~x7) | (~x5 & x6))) | (x5 & (x2 | ~x6)));
  assign new_n151_ = ~x1 & x3 & (~x2 | (~x4 & ~x5));
  assign z52 = (~new_n153_ & (~x2 | ~x6)) | (~new_n154_ & ~x2) | (x4 & ~x6 & x2 & x3);
  assign new_n153_ = (x0 | ~x1) & ~new_n136_ & (~x0 | ~x3);
  assign new_n154_ = (x0 | (x3 & (x4 | ~x6))) & (~x0 | x1) & (x4 | x5 | ~x6);
  assign z53 = ~new_n157_ | (~x3 & ((~new_n156_ & ~x2) | (x0 & (x1 | x2))));
  assign new_n156_ = new_n136_ & x6 & x7;
  assign new_n157_ = (new_n158_ | (~x2 & ~x3)) & (~x2 | (~x6 & (x0 | ~x3))) & (~x3 | x4 | ~x6);
  assign new_n158_ = x1 & (x4 | ~x5);
  assign z54 = (~new_n160_ & ~x3) | (~new_n156_ & (x0 | (~x2 & x3))) | (x0 & x3) | (~new_n162_ & x2);
  assign new_n160_ = (~new_n161_ | x0) & x1 & ~x2;
  assign new_n161_ = ~x4 & (x5 | x6);
  assign new_n162_ = x1 & ~new_n136_ & ~x6;
  assign z55 = (x0 & (x2 ? ~x6 : ~x3)) | (~x2 & ~x4 & x6) | (~new_n158_ & (~x2 | ~x6));
  assign z56 = new_n142_ | ~new_n79_ | x0 | ~x1;
  assign z57 = (~x4 & ((x5 & ~x6) | (~x2 & (x5 | (x6 & ~x7))))) | (~new_n166_ & ~x2) | (x2 & ~x6);
  assign new_n166_ = x1 & (x0 | ~x3) & (~x0 | x3);
  assign z58 = ~new_n168_ | ((x0 | ~x3) & (~x2 | (x2 & ~x6)));
  assign new_n168_ = (~x5 | (x2 & (x4 | x6))) & (x2 | (~x1 & ~x4 & x7)) & (x6 | (x2 & (x1 | ~x2)));
  assign z59 = new_n170_ | (~x6 & (~x0 | (x1 & x3) | new_n136_ | (~x1 & ~x3)));
  assign new_n170_ = ~x2 & (new_n148_ | x4 | x5 | ~x6 | ~x7);
  assign z60 = (x0 & (~x1 | ~x4)) | new_n172_ | x3 | (~x0 & (~new_n173_ | x1 | x4));
  assign new_n172_ = x2 & (~x0 | x6);
  assign new_n173_ = x5 & x6 & x7;
  assign z61 = ~x2 | (~x6 & (~new_n91_ | new_n136_ | ~x3));
  assign z62 = new_n161_ | z08 | x3 | ~x0 | ~x1;
  assign z10 = 1'b0;
  assign z27 = 1'b0;
  assign z28 = 1'b0;
  assign z33 = 1'b1;
  assign z09 = z08;
  assign z12 = z08;
  assign z15 = z08;
  assign z26 = z08;
  assign z30 = z08;
endmodule


