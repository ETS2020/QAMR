// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n78_, new_n81_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n95_, new_n106_, new_n108_, new_n110_, new_n113_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n146_, new_n149_, new_n152_, new_n154_, new_n158_,
    new_n159_, new_n162_, new_n164_, new_n165_, new_n167_;
  assign z00 = z08 | new_n75_;
  assign z08 = x0 & x2;
  assign new_n75_ = ~x4 & ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z08 & ~x7;
  assign z02 = z08 | (new_n78_ & x5 & ~x6 & ~x7);
  assign new_n78_ = ~x3 & ~x4;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z04 = z08 | (x3 & ~x4 & new_n81_ & ~x5);
  assign new_n81_ = x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z08 & ~x7;
  assign z06 = x2 & (x0 | (new_n75_ & ~x1 & x3));
  assign z07 = new_n85_ & x6 & x7;
  assign new_n85_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z09 = x2 & (x0 | (new_n78_ & ~x1 & new_n87_ & ~x5));
  assign new_n87_ = x6 & x7;
  assign z10 = x2 & (x0 | (new_n89_ & x1 & ~x4));
  assign new_n89_ = new_n87_ & x5;
  assign z11 = new_n91_ & x6 & x7;
  assign new_n91_ = x5 & ~x4 & ~x3 & x0 & x1 & ~x2;
  assign z13 = (x0 & x2) | (new_n93_ & ~x0 & x1 & ~x2 & x3);
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z14 = x7 & x6 & x5 & new_n95_ & x3 & ~x4;
  assign new_n95_ = ~x2 & x0 & ~x1;
  assign z15 = x2 & (x0 | (new_n89_ & x1 & x3 & ~x4));
  assign z16 = x0 & (x2 | (new_n89_ & x1 & x3 & ~x4));
  assign z17 = new_n95_ & x4 & ~x5;
  assign z18 = ~x5 & x4 & x3 & ~x0 & ~x1 & x2;
  assign z19 = (x0 & x2) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = x0 & (x2 | (new_n75_ & ~x1 & ~x3));
  assign z21 = x0 & (x2 | (new_n75_ & ~x1 & x3));
  assign z22 = x7 & x6 & ~x5 & new_n95_ & ~x4;
  assign z23 = (x0 & x2) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = new_n106_ & x6 & ~x7;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x0 & ~x1 & ~x2;
  assign z25 = z08 | (new_n108_ & new_n81_ & ~x4 & ~x5);
  assign new_n108_ = ~x0 & x1 & ~x2 & ~x3;
  assign z27 = new_n110_ & x6 & ~x7;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = x7 & new_n106_ & ~x6;
  assign z31 = (x0 & (x2 | (~x4 & x6))) | (~x0 & (~x4 | (~x2 & x3))) | ~new_n113_ | (x2 & (~x3 | ~x4));
  assign new_n113_ = ~x1 & (x4 | ~x5) & (~x4 | x5);
  assign z32 = (~x0 & ((~x2 & (x3 | x4)) | (~x4 & (~new_n81_ | x3)))) | ~new_n115_ | (~x3 & (x2 | (x0 & ~x4)));
  assign new_n115_ = (~x0 | (~x2 & (x4 | ~x6))) & (x4 | (~x2 & ~x5)) & ~x1 & (x2 | ~x4 | x5);
  assign z34 = new_n117_ | new_n119_ | (~new_n118_ & ~x0);
  assign new_n117_ = x1 & ((~x2 & ~x5) | (~x0 & x2 & ~x3));
  assign new_n118_ = (~x3 | (x5 & (~x2 | ~x6))) & (x3 | (~x5 & (~x2 | x6))) & (x2 | x5) & (~x2 | (~x4 & ~x7));
  assign new_n119_ = ~x2 & ((~x4 & ~x5 & (~x6 | ~x7)) | (x5 & (~x3 | x4 | x6 | x7)));
  assign z35 = ((~x0 | ~x2) & (x1 | ~x4)) | (~x0 & (x2 ? (~x3 | ~x5) : x3)) | (x0 & ~x2 & ~x5);
  assign z36 = (~new_n122_ & (~x0 | ~x2)) | (~x2 & (~x0 | ~x4)) | (~x0 & (~new_n81_ | x3 | x4));
  assign new_n122_ = ~x1 & ~x5;
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (x3 & (x5 ? x1 : ~new_n124_)) | (x0 & x2) | (~x1 & ~x3);
  assign new_n124_ = new_n81_ & ~x4;
  assign z38 = (~x3 & (x2 | (x0 & ~x4))) | ~new_n126_ | (x2 & (x0 | ~x4));
  assign new_n126_ = (~new_n127_ | ~x0) & ~x1 & (~new_n128_ | x0);
  assign new_n127_ = ~x4 & (x5 | x6);
  assign new_n128_ = ~x2 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~new_n131_ & (~x0 | (~x2 & x5))) | (~x0 & (x4 | ~x5)) | (~x2 & (x4 | (~new_n130_ & ~x5)));
  assign new_n130_ = new_n87_ & ~x1;
  assign new_n131_ = x3 & ~x6 & ~x7;
  assign z40 = (~new_n134_ & ~x2) | (~x0 & (~new_n133_ | (x2 & (~x3 | ~x4))));
  assign new_n133_ = ~x1 & (x4 | (new_n81_ & ~x5));
  assign new_n134_ = (~x0 | (x4 ? x5 : ~x6)) & ~x1 & (x4 | ~x5) & (x0 | ~x3);
  assign z41 = ~x0 | (~x2 & (x1 ? x3 : (~x3 | ~x5)));
  assign z42 = (~new_n137_ & ~x2) | (~x0 & (new_n138_ | x4));
  assign new_n137_ = (~x5 | (~x6 & ~x7) | (~x0 & x4)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign new_n138_ = ~x4 & ((x2 & (~x5 | x6)) | (x5 & x6) | x7 | (~x5 & ~x6));
  assign z43 = new_n142_ | (~x2 & (new_n140_ | new_n143_));
  assign new_n140_ = ~new_n141_ & (x1 | (~x0 & x3));
  assign new_n141_ = ~x4 & x5;
  assign new_n142_ = ~x0 & (new_n138_ | (x4 & (x1 | (x2 & ~x3))));
  assign new_n143_ = ~x4 & ((x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign z44 = (~new_n75_ & x0) | x1 | x2 | x3 | (~x0 & ~x4);
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | ~new_n146_ | (x5 & (~x1 | ~x4));
  assign new_n146_ = ~x0 & (x1 | (new_n87_ & ~x2 & ~x4));
  assign z46 = ~new_n108_ | (~x4 & (new_n81_ | x5));
  assign z48 = new_n149_ | x0 | x1 | x2 | ~x3;
  assign new_n149_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~x2 | (~x0 & (new_n127_ | x1 | (x3 & x4)));
  assign z50 = x2 ? ~x0 : (~new_n152_ | (x0 & (~x1 | ~x3)));
  assign new_n152_ = new_n87_ & ~x4 & ~x5;
  assign z51 = (~new_n154_ & ~x0) | (~x2 & (new_n149_ | (x0 & (~x1 | x3))));
  assign new_n154_ = ~new_n127_ & ~x1 & x3 & (~x2 | ~x4);
  assign z52 = (~x0 & (x1 | (~x2 & ~x3))) | (x2 & (x0 | (x3 & x4))) | new_n127_ | (x0 & (~x1 | x3));
  assign z53 = (x1 & (x0 ? (~x2 & ~x3) : (x2 & x3))) | (~new_n93_ & ((~x2 & ~x3) | (~x0 & ~x1 & x3))) | ((new_n127_ | ~x1) & ((~x2 & x3) | (~x0 & x2 & ~x3)));
  assign z54 = ~new_n159_ | (~x2 & (x3 ? ~new_n93_ : ~new_n158_));
  assign new_n158_ = x1 & (~new_n127_ | x0);
  assign new_n159_ = (x1 | (~x0 & (~x2 | ~x3))) & (new_n93_ | (~x0 & (~x2 | x3))) & (~x0 | (~x2 & ~x3)) & (~new_n149_ | ~x2);
  assign z55 = (x0 & ~x2 & ~x3) | ((~x0 | ~x2) & (new_n127_ | ~x1));
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n162_ | (~x2 & (new_n141_ | ~x3));
  assign new_n162_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x3 & (x0 | ~x1)) | ~new_n164_ | ((x0 | ~x2) & (new_n141_ | ~x1));
  assign new_n164_ = new_n165_ & (x7 | (~x2 & (x4 | ~x6)));
  assign new_n165_ = (~x2 | (~x0 & ~x4 & x5 & x6)) & (x0 | x2 | ~x3);
  assign z58 = (x1 & (~x2 | (~x0 & ~x4 & x6))) | (~new_n167_ & ~x0) | (x0 & ~x2);
  assign new_n167_ = (~x5 | (x1 & x4)) & x3 & (x1 | (new_n87_ & ~x2 & ~x4));
  assign z59 = (x0 & (~x1 | ~x3)) | ~new_n152_ | (x2 & (x1 | x3));
  assign z60 = (x2 & (x0 | ~x3)) | (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n89_ | x1 | x4));
  assign z62 = (x2 & (x0 | ~x3)) | (x3 & (x0 | ~x2)) | ~x0 | (x0 & (new_n127_ | ~x1));
  assign z28 = 1'b0;
  assign z30 = 1'b0;
  assign z33 = 1'b1;
  assign z61 = 1'b1;
  assign z12 = z08;
  assign z26 = z08;
  assign z47 = (x1 & (~x2 | (~x4 & x6))) | ~new_n146_ | (x5 & (~x1 | ~x4));
endmodule


