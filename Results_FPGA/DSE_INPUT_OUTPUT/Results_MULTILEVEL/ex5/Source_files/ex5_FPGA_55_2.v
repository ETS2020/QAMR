// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_,
    new_n99_, new_n102_, new_n104_, new_n108_, new_n109_, new_n112_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n121_, new_n125_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n136_, new_n138_, new_n140_, new_n141_, new_n143_, new_n145_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n157_,
    new_n159_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_;
  assign z00 = z16 | (~x4 & ~x5 & ~x6);
  assign z16 = x0 & ~x2;
  assign z01 = ~x5 & ~x6 & ~z16 & ~x7;
  assign z02 = z16 | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z39 = ~z16 & (~x5 | x6 | x7 | ~x3 | x4);
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z16 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z16 & ~x7;
  assign z06 = ~x6 & ~x5 & ~x4 & x3 & new_n81_ & x2;
  assign new_n81_ = ~x0 & ~x1;
  assign z07 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & new_n85_ & x6;
  assign new_n85_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n87_ & x6;
  assign new_n87_ = ~x5 & ~x4 & ~x3 & new_n81_ & x2;
  assign z10 = (x0 & ~x2) | (new_n89_ & ~x0 & x1 & x2);
  assign new_n89_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n91_ & ~x3;
  assign new_n91_ = x2 & x0 & ~x1;
  assign z13 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z15 = (x0 & ~x2) | (new_n89_ & ~x0 & x1 & x2 & x3);
  assign z18 = (x0 & ~x2) | (~x0 & ~x1 & x2 & x3 & x4 & ~x5);
  assign z19 = ~x2 & (x0 | (~x1 & ~x3 & x4));
  assign z23 = x5 & x3 & new_n81_ & ~x2;
  assign z24 = ~x7 & new_n99_ & x6;
  assign new_n99_ = ~x5 & ~x4 & ~x3 & new_n81_ & ~x2;
  assign z25 = ~x7 & x6 & new_n83_ & ~x5;
  assign z26 = new_n102_ & x7;
  assign new_n102_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & new_n104_ & x6;
  assign new_n104_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n91_ & x3;
  assign z29 = x7 & new_n99_ & ~x6;
  assign z30 = x0 & (~x2 | (new_n108_ & x1 & ~x3 & ~x4));
  assign new_n108_ = new_n109_ & ~x5;
  assign new_n109_ = x6 & x7;
  assign z31 = ((~x0 | x2) & (x1 | ~x4 | ~x5)) | (x2 & (x0 | ~x3)) | (~x0 & ~x2 & x3);
  assign z32 = (x2 & (~x3 | ~x4)) | ~new_n81_ | (~x2 & (~new_n112_ | x3 | x4));
  assign new_n112_ = new_n113_ & ~x5;
  assign new_n113_ = x6 & ~x7;
  assign z33 = ~x0 | (~new_n115_ & x2);
  assign new_n115_ = ~x4 & (x1 | ~x5) & new_n109_ & (~x1 | ~x3 | x5);
  assign z34 = ~new_n117_ | ((~x5 | x6) & (x0 ? x2 : x3));
  assign new_n117_ = ((~x4 & ~x7) | (x0 & ~x2)) & (x3 | (x0 ? ~x2 : new_n118_));
  assign new_n118_ = ~x1 & x2 & ~x5 & x6;
  assign z35 = (x2 & (x0 | ~x3)) | (~x0 & ((~x2 & (x3 | ~x4)) | x1 | (x3 & (~x4 | ~x5))));
  assign z36 = (~x0 & (x1 | ~x2 | x3)) | (x2 & (x0 | (~new_n121_ & ~x3)));
  assign new_n121_ = new_n113_ & ~x4 & ~x5;
  assign z37 = ~z16 & (~new_n112_ | ~x3 | x4);
  assign z38 = (x2 & (x0 | ~x3 | ~x4)) | (~x0 & (x1 | (~x2 & (~new_n112_ | x3 | x4))));
  assign z40 = (x1 & (~x0 | ~x2)) | (~x0 & x2 & (~x3 | ~x4)) | ~new_n125_ | (x3 & (x0 | ~x2));
  assign new_n125_ = ((~x5 & x6) | (~x0 & (x2 | x4))) & (x2 | (~x0 & (x4 | ~x7))) & (~x0 | (~x4 & x7));
  assign z42 = ~new_n127_ | (~z16 & x4);
  assign new_n127_ = ((~x6 & ~x7) | (x0 & (~x2 | ~x5))) & (x5 | (x0 & (~x2 | (new_n128_ & x6 & x7))));
  assign new_n128_ = ~x1 & x3;
  assign z43 = new_n130_ | new_n131_ | (~new_n132_ & ~x0) | (~new_n133_ & x2);
  assign new_n130_ = (x0 ? x2 : ~x4) & (~x5 ^ x6);
  assign new_n131_ = x7 & (x0 ? (x2 & x5) : ~x4);
  assign new_n132_ = (~x1 | (~x4 & (x2 | x5))) & (~x2 | x4 | (x5 & ~x6)) & (x2 | ~x3 | (~x4 & x5));
  assign new_n133_ = (~x4 | (~x0 & x3)) & (~x0 | x5 | (x7 & (~x1 | ~x3)));
  assign z44 = (~x0 & ((x1 & (x4 | (~x2 & ~x5))) | (~x2 & x3 & (x4 | ~x5)))) | x0 | x2 | ~x4;
  assign z45 = (x2 & (new_n136_ | x0 | ~x1)) | (~x0 & ~x2 & (~new_n108_ | x1 | x4));
  assign new_n136_ = ~x4 & (x5 | x6);
  assign z46 = ~new_n138_ | (~x4 & (new_n113_ | x5));
  assign new_n138_ = ~x2 & ~x3 & ~x0 & x1;
  assign z47 = ~new_n140_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n140_ = (x2 | (~x0 & ~x1)) & ~new_n141_ & (~x0 | (x1 & x3 & x5)) & (x1 | (~x2 & ~x5));
  assign new_n141_ = (x0 | ~x1) & (x4 | ~x6 | ~x7);
  assign z48 = x2 | (~new_n143_ & ~x0);
  assign new_n143_ = new_n128_ & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = (x2 & (x1 | (~x4 & x6))) | ~new_n145_ | x0 | ~x2;
  assign new_n145_ = x4 ? ~x3 : ~x5;
  assign z50 = (x4 & (~x2 | x3)) | (x5 & (~x2 | ~x4)) | x0 | x2 | (~new_n109_ & ~x2);
  assign z51 = (~x0 & ((~x3 & (~x2 | ~x4)) | new_n136_ | x1 | (x2 & x4))) | (x2 & (new_n136_ | (x0 & ~x1)));
  assign z52 = (x0 & (~x2 | x3)) | new_n136_ | (~x0 & (x1 | (~x2 & ~x3) | (x2 & x3 & x4)));
  assign z53 = (~new_n150_ & ~x0) | (x2 & ((x0 & (~x1 | ~x3)) | new_n152_ | (~x1 & ~x3) | (~x0 & x1 & x3)));
  assign new_n150_ = (~x3 | (x1 ? ~new_n136_ : new_n151_)) & (x2 | new_n89_ | x3);
  assign new_n151_ = x2 & ~x4 & x5 & x6 & x7;
  assign new_n152_ = ~x4 & ((~x3 & (x5 | x6)) | (x1 & (x5 ? (~x6 | ~x7) : x6)));
  assign z54 = (x2 & (~new_n155_ | (x0 & (~x1 | x3)))) | (~x0 & ~new_n154_ & ~x2);
  assign new_n154_ = x3 ? new_n89_ : (~new_n136_ & x1);
  assign new_n155_ = ((x6 & x7) | (x3 & (x4 | ~x5))) & (x3 | (~x4 & x5)) & (x1 | ~x3) & (x4 | x5 | ~x6);
  assign z55 = new_n157_ | ~x1 | (~new_n151_ & x0);
  assign new_n157_ = ~x4 & ((~x0 & (x5 | x6)) | (x6 & ~x7) | (~x2 & x5));
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n159_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n159_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x1 & (~x2 | ~x3)) | ~new_n159_ | (~x2 & (x3 | (~x4 & x5)));
  assign z58 = ~new_n162_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n162_ = (x2 | (~x0 & ~x1)) & ~new_n141_ & (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (x1 & (x0 ? x3 : x2)) | ~new_n164_ | (~x2 & (x0 | (x3 & x5)));
  assign new_n164_ = x0 ? (~new_n136_ & (x1 | x3)) : new_n165_;
  assign new_n165_ = ~x4 & ~x5 & x6 & x7 & (~x2 | ~x3 | x5);
  assign z60 = ~new_n168_ | (~x0 & (~new_n167_ | (x1 & (x2 | x5))));
  assign new_n167_ = new_n109_ & x5 & ~x4 & (~x2 | x3 | ~x5);
  assign new_n168_ = (x2 | (~x0 & (~x3 | ~x5))) & (~x0 | (x1 & x4 & (~x1 | ~x3)));
  assign z61 = new_n136_ | ~x0 | x1 | ~x2 | ~x3;
  assign z62 = ~x0 | (x2 & (new_n136_ | ~x1 | x3));
  assign z11 = 1'b0;
  assign z14 = 1'b0;
  assign z20 = 1'b0;
  assign z21 = 1'b0;
  assign z03 = ~z39;
  assign z41 = ~z16;
  assign z17 = z16;
  assign z22 = z16;
endmodule


