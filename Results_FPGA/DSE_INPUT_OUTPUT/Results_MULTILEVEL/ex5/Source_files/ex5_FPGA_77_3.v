// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n78_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n93_,
    new_n95_, new_n98_, new_n103_, new_n106_, new_n109_, new_n113_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n127_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n136_, new_n137_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n153_, new_n154_,
    new_n156_, new_n158_, new_n161_, new_n166_, new_n167_, new_n170_,
    new_n173_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = z06 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = x3 & (x2 | (new_n78_ & ~x6 & ~x7));
  assign new_n78_ = ~x4 & x5;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = z06 | (new_n78_ & x6 & ~x7);
  assign z07 = x7 & x6 & new_n82_ & x5;
  assign new_n82_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x2 & (x3 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x2 & (x3 | (new_n86_ & ~x0 & ~x1 & ~x4));
  assign new_n86_ = ~x5 & x6 & x7;
  assign z10 = x7 & x6 & new_n88_ & x5;
  assign new_n88_ = ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = (x2 & x3) | (new_n90_ & ~x2 & ~x3 & x0 & x1);
  assign new_n90_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (x3 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z13 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x3 & (x2 | (new_n84_ & x0 & x1 & ~x4));
  assign z17 = (x2 & x3) | (new_n98_ & ~x2 & x4 & ~x5);
  assign new_n98_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n98_ & ~x2;
  assign z21 = x3 & (x2 | (new_n98_ & ~x4 & ~x5 & ~x6));
  assign z22 = (x2 & x3) | (new_n103_ & x0 & ~x1 & ~x2);
  assign new_n103_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n82_ & ~x5;
  assign z26 = ~new_n109_ & x2;
  assign new_n109_ = ~x3 & (x5 | ~x6 | ~x7 | ~x0 | x4);
  assign z27 = ~x7 & x6 & new_n88_ & ~x5;
  assign z29 = x7 & new_n106_ & ~x6;
  assign z30 = x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | x2 | (~new_n115_ & ~x2);
  assign new_n115_ = (x4 | (~x5 & (~x0 | ~x6))) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n118_ | (~new_n117_ & ~x4);
  assign new_n117_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n118_ = x2 ? x3 : (~x1 & (~x4 | x5) & (x0 | (~x3 & ~x4)));
  assign z33 = ~new_n120_ | ~x7 | x4 | ~x6;
  assign new_n120_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n123_ & ~x5) | ~new_n124_ | (~new_n122_ & (x2 | x5));
  assign new_n122_ = ~x4 & ~x7;
  assign new_n123_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n124_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = ~new_n127_ | (~z06 & (x1 | x5));
  assign new_n127_ = x2 ? (x3 | (~x0 & ~x4 & x6 & ~x7)) : (x0 & x4);
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n129_)) | x2 | (~x1 & ~x3);
  assign new_n129_ = ~x4 & x6 & ~x7;
  assign z38 = new_n131_ | (x2 & ~x3) | (~x2 & (new_n132_ | x1));
  assign new_n131_ = ~x4 & ((~x2 & x5) | (x0 & (~x3 | (~x2 & x6))));
  assign new_n132_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~x3 | x6 | x7)) | ~new_n134_ | (~x5 & (~new_n98_ | ~x6 | ~x7));
  assign new_n134_ = ~x2 & ~x4;
  assign z40 = x2 ? new_n109_ : (new_n136_ | new_n137_ | new_n78_ | x1);
  assign new_n136_ = x0 & (x4 ? ~x5 : x6);
  assign new_n137_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | (~x2 & ~x5))) | (x2 & ~x3) | (~x2 & (~x0 | (x1 & x3)));
  assign z42 = ~new_n140_ | (x2 & (x3 | ~x5));
  assign new_n140_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = x2 ? new_n144_ : (new_n142_ | new_n143_);
  assign new_n142_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n143_ = ~x4 & ((~x0 & ((~x5 & ~x6) | x7)) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n144_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = x2 ? ~x3 : (new_n142_ | ~new_n146_);
  assign new_n146_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x0 & (~x2 | ~x3)) | (~new_n148_ & ~x3) | (~new_n149_ & ~x2);
  assign new_n148_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign new_n149_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = new_n151_ | x2 | x3 | x0 | ~x1;
  assign new_n151_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = new_n154_ | (~new_n153_ & x1) | x0 | (~x1 & (~new_n86_ | ~new_n134_));
  assign new_n153_ = x2 & ~x3;
  assign new_n154_ = ~x4 & (x5 | (x1 & x6));
  assign z48 = ~new_n156_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n156_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = new_n158_ | ~new_n153_ | x0 | x1;
  assign new_n158_ = ~x4 & (x5 | x6);
  assign z50 = (x0 & ((~x1 & ~x2) | ~x3)) | (~new_n103_ & ~x2) | (x2 & ~x3);
  assign z51 = (x2 & (x3 | (new_n78_ & ~x3))) | new_n161_ | (~x2 & (x0 | x1) & x3) | (~x3 & (~x0 | ~x1));
  assign new_n161_ = ~x4 & ((~x2 & x3 & (x5 | x6)) | (~x3 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x3 & (new_n158_ | (~x1 & ~x2) | (~x0 & (x1 | ~x2)))) | (~x2 & x3 & (new_n158_ | x0 | x1));
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n158_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n90_ & ~x3);
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n158_ & ~x0))) | (~new_n90_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n166_ | ~x1 | (x2 & x3) | (x0 & ~new_n167_ & ~x3);
  assign new_n166_ = ~x4 & (x5 | x6) & (~x0 | x3);
  assign new_n167_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n90_ & ~x3) | (~x2 & (new_n151_ | ~x1 | ~x3));
  assign z57 = new_n170_ | (x0 & (x2 | ~x3)) | (~new_n90_ & x2) | ~x1 | (~x0 & x3);
  assign new_n170_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~new_n103_ | ~new_n156_;
  assign z59 = (~new_n103_ & (~x0 | ~x2)) | new_n173_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n173_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n84_ | x1 | x2 | x4));
  assign z62 = new_n158_ | ~x0 | ~x1 | x3;
  assign z15 = 1'b0;
  assign z61 = ~z06;
  assign z18 = z06;
  assign z28 = z06;
endmodule


