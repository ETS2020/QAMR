// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n82_, new_n85_, new_n87_, new_n89_, new_n92_, new_n96_,
    new_n98_, new_n100_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n124_, new_n128_, new_n130_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n150_, new_n152_,
    new_n154_, new_n158_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n171_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = ~x0 & x2;
  assign z01 = z06 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = z06 | (~x3 & ~x4 & new_n76_ & x5);
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z06 & ~x7;
  assign z05 = z06 | (~x4 & x5 & x6 & ~x7);
  assign z07 = ~x0 & (new_n82_ | x2);
  assign new_n82_ = x1 & ~x3 & ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (new_n82_ | ~x0);
  assign z11 = (~x0 & x2) | (new_n85_ & x0 & x1 & ~x2 & ~x3);
  assign new_n85_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (~x0 | (new_n87_ & ~x1 & ~x3 & ~x4));
  assign new_n87_ = x5 & x6 & x7;
  assign z13 = ~x0 & (x2 | (new_n87_ & new_n89_ & x1));
  assign new_n89_ = x3 & ~x4;
  assign z14 = (~x0 & x2) | (new_n85_ & x0 & ~x1 & ~x2 & x3);
  assign z16 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & ~x2 & x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = (~x0 & x2) | (x0 & ~x1 & ~x2 & new_n96_ & ~x3 & ~x4);
  assign new_n96_ = ~x5 & ~x6;
  assign z21 = new_n98_ & ~x6;
  assign new_n98_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = (~x0 & x2) | (new_n100_ & x0 & ~x1 & ~x2);
  assign new_n100_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = ~x0 & (x2 | (~x1 & x3 & x5));
  assign z24 = ~x7 & new_n103_ & x6;
  assign new_n103_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & new_n105_ & x6;
  assign new_n105_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = new_n107_ & x7;
  assign new_n107_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z28 = x2 & (~x0 | (new_n89_ & ~x1 & new_n109_ & ~x5));
  assign new_n109_ = x6 & x7;
  assign z29 = ~x0 & (new_n111_ | x2);
  assign new_n111_ = ~x1 & ~x3 & ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n115_ | (~x0 & (~x4 | (~x2 & x3)));
  assign new_n115_ = (x4 | ((x2 | ~x5) & (~x0 | ~x6))) & ~x2 & (x2 | (~x1 & (~x4 | x5)));
  assign z32 = new_n120_ | (~x2 & (new_n117_ | new_n118_ | ~new_n119_));
  assign new_n117_ = x4 & (~x0 | ~x5);
  assign new_n118_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n119_ = ~x1 & (x4 | ~x5);
  assign new_n120_ = x0 & (x2 | (~x4 & (~x3 | x6)));
  assign z33 = ~x2 | (~new_n122_ & x0);
  assign new_n122_ = ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = ((~new_n76_ | ~new_n89_) & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n124_ : ~x2));
  assign new_n124_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (~x2 & (x1 | ~x4 | (~x0 & x3)));
  assign z36 = x2 ? x0 : (~x4 | x5 | ~x0 | x1);
  assign z37 = ((~x3 | x5) & (~x0 ^ x2)) | (~new_n128_ & (x0 ? (x3 & ~x5) : ~x2)) | (x0 & (x1 ? (x3 & x5) : ~x3));
  assign new_n128_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (x0 | ~x2)) | ~new_n130_ | (x5 & (x0 ? ~x4 : ~x2));
  assign new_n130_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (~x3 & ~x4 & x6 & ~x7));
  assign z39 = new_n132_ | (~x2 & (x4 | (~x0 & ~x5))) | (x0 & (x4 | (~new_n133_ & ~x5)));
  assign new_n132_ = (x0 ? x5 : ~x2) & (~x3 | x6 | x7);
  assign new_n133_ = ~x1 & ~x2 & x6 & x7;
  assign z40 = (~new_n135_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n136_ | (x3 & (~x0 | x2));
  assign new_n135_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n136_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = x0 ? ((~x1 & (~x3 | ~x5)) | x2 | (x1 & x3)) : ~x2;
  assign z42 = (~x0 & (x2 | ~x5)) | x4 | (~new_n139_ & ~x5) | (~new_n76_ & x5);
  assign new_n139_ = ~x1 & (~x2 | x3) & x6 & x7;
  assign z43 = ((new_n141_ | new_n142_) & ~x5) | new_n143_ | new_n144_ | ~new_n145_;
  assign new_n141_ = ~x2 & (x1 | (~x0 & (x3 | (~x4 & ~x6))));
  assign new_n142_ = x0 & ((x1 & x3) | (x2 & (~x6 | ~x7)));
  assign new_n143_ = (x0 ? x5 : ~x2) & (x4 ? x1 : x7);
  assign new_n144_ = ~x0 & (x2 | (~x2 & x3 & x4));
  assign new_n145_ = (~x0 | ~x2 | ~x4) & (x4 | ~x6 | ((x2 | ~x5) & (~x0 | (~x5 & x7))));
  assign z44 = (~new_n147_ & ~x2) | (x0 & (x2 | x3 | ~new_n96_ | x4));
  assign new_n147_ = (~x1 | (x5 & (x0 | ~x4))) & (x0 | (x4 & (~x3 | (~x4 & x5)))) & (x4 | ~x5 | ~x6);
  assign z45 = x0 | (~x2 & (~new_n109_ | x5 | x1 | x4));
  assign z46 = new_n150_ | x0 | ~x1 | x2 | x3;
  assign new_n150_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n152_ & (x0 | ~x2)) | (~x2 & (x0 | x1 | x5)) | (x0 & (~x1 | ~x3 | ~x5));
  assign new_n152_ = ~x4 & x6 & x7;
  assign z48 = x0 | (~new_n154_ & ~x2);
  assign new_n154_ = ~x1 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (x0 & (~x1 | x2 | ~x3));
  assign z50 = (~new_n100_ & ~x2) | (x0 & (~x1 | x2 | ~x3));
  assign z51 = new_n158_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n158_ = ~x4 & (((x5 | x6) & (x0 ^ ~x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n96_ & (x0 | (~x0 & ~x2))) | (~x0 & ~x2 & (x1 | ~x3)) | (x0 & (x3 | (~x1 & ~x2)));
  assign z53 = (x2 & (~x0 | ~x3)) | new_n161_ | (~new_n162_ & ~x3) | (~x1 & x3);
  assign new_n161_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & x3 & (x5 | x6)));
  assign new_n162_ = x5 & x6 & x7 & ~x4 & (~x0 | ~x1);
  assign z54 = (~new_n82_ & x0) | (~x2 & (x3 ? ~new_n85_ : ~new_n164_));
  assign new_n164_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign z55 = (~new_n166_ & ~x2) | ~x1 | (x2 & (~new_n87_ | ~x0 | x4));
  assign new_n166_ = (~x0 | x3) & (x4 | (~x5 & ~x6));
  assign z56 = x0 | (~x2 & (new_n150_ | ~x1 | ~x3));
  assign z57 = new_n150_ | ~x1 | x2 | (x0 & ~x3) | (~x0 & x3);
  assign z58 = (x0 & (~x1 | ~x2 | ~x5)) | ~new_n109_ | ~new_n89_ | (~x0 & (x1 | x2 | x5));
  assign z59 = (~x2 & (~new_n100_ | (x0 & (~x1 | ~x3)))) | new_n171_ | (x2 & (~x0 | (x1 & x3) | (~x1 & ~x3)));
  assign new_n171_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & ~x2 & (~new_n87_ | x1 | x4));
  assign z61 = ~x2 | (x0 & ((~new_n96_ & ~x4) | x1 | ~x3));
  assign z62 = (~new_n96_ & ~x4) | ~x0 | ~x1 | x3;
  assign z27 = 1'b0;
  assign z09 = z06;
  assign z10 = z06;
  assign z15 = z06;
  assign z18 = z06;
endmodule


