// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n80_, new_n82_, new_n84_, new_n89_, new_n91_, new_n93_, new_n96_,
    new_n101_, new_n103_, new_n104_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n125_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n141_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n160_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n185_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_;
  assign z00 = ~x4 & ~x5 & ~z08 & ~x6;
  assign z08 = x2 & x7;
  assign z01 = (x2 & x7) | (~x5 & ~x6 & ~x7);
  assign z02 = (x2 & x7) | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = ~x7 & new_n80_ & x6;
  assign new_n80_ = ~x4 & x5;
  assign z06 = x2 & (new_n82_ | x7);
  assign new_n82_ = ~x0 & ~x1 & x3 & ~x4 & ~x5 & ~x6;
  assign z07 = x7 & (x2 | (new_n84_ & ~x0 & x1 & ~x3));
  assign new_n84_ = ~x4 & x5 & x6;
  assign z11 = x7 & (x2 | (new_n84_ & x0 & x1 & ~x3));
  assign z13 = x7 & (x2 | (new_n84_ & ~x0 & x1 & x3));
  assign z14 = x7 & (x2 | (new_n84_ & x0 & ~x1 & x3));
  assign z16 = x7 & new_n89_ & x6;
  assign new_n89_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & new_n91_ & ~x2;
  assign new_n91_ = x0 & ~x1;
  assign z18 = ~x7 & ~x5 & x4 & x3 & new_n93_ & x2;
  assign new_n93_ = ~x0 & ~x1;
  assign z19 = (x2 & x7) | (new_n93_ & ~x2 & ~x3 & x4);
  assign z20 = (x2 & x7) | (new_n96_ & x0 & ~x1 & ~x2);
  assign new_n96_ = ~x5 & ~x6 & ~x3 & ~x4;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n91_ & ~x2;
  assign z22 = x7 & (x2 | (new_n91_ & ~x4 & ~x5 & x6));
  assign z23 = x5 & x3 & new_n93_ & ~x2;
  assign z24 = ~x7 & new_n101_ & x6;
  assign new_n101_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x2 & x7) | (new_n103_ & new_n104_ & x6 & ~x7);
  assign new_n103_ = ~x2 & ~x3 & ~x0 & x1;
  assign new_n104_ = ~x4 & ~x5;
  assign z27 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = x7 & new_n101_ & ~x6;
  assign z31 = new_n109_ | (~new_n110_ & ~x4) | new_n111_ | x1 | (x4 & ~x5);
  assign new_n109_ = ~x0 & (~x4 | (~x2 & x3));
  assign new_n110_ = ~x2 & ~x5 & ~x6;
  assign new_n111_ = x2 & (x0 | ~x3 | x7);
  assign z32 = new_n113_ | new_n114_ | new_n115_ | new_n116_ | new_n117_ | x1;
  assign new_n113_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n114_ = x2 & (x0 | ~x4);
  assign new_n115_ = x4 & (x0 ? ~x5 : ~x2);
  assign new_n116_ = x0 & ~x4 & (x5 | x6);
  assign new_n117_ = ~x0 & (x7 | (~x2 & (x3 | x5 | ~x6)));
  assign z34 = new_n119_ | (~new_n121_ & ~x2) | (~new_n120_ & ~x7);
  assign new_n119_ = x1 & (x2 ? (~x3 & ~x7) : ~x5);
  assign new_n120_ = (~x2 | ((~x3 | (x5 & ~x6)) & ~x4 & (x3 | (~x0 & x6)))) & (x3 | ~x5) & (x2 | x4 | x5);
  assign new_n121_ = x5 ? (~x6 & ~x7 & x3 & ~x4) : (x0 & (x4 | x6));
  assign z35 = (x0 & (x2 ? ~x7 : ~x5)) | (~new_n123_ & (~x2 | ~x7)) | (~x0 & ~x2 & x3) | (x2 & ~x7 & (~x3 | ~x5));
  assign new_n123_ = ~x1 & x4;
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n125_ & ~x0) | x1 | x5;
  assign new_n125_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (~x1 & ~x3) | (x3 & (x5 ? x1 : ~new_n127_));
  assign new_n127_ = ~x4 & x6 & ~x7;
  assign z38 = new_n129_ | new_n130_ | new_n132_ | (new_n133_ & ~x0) | (~new_n131_ & x0);
  assign new_n129_ = x1 & (~x2 | ~x7);
  assign new_n130_ = ~x3 & ((x2 & ~x7) | (x0 & ~x2 & ~x4));
  assign new_n131_ = x2 ? x7 : (x4 | ~x6);
  assign new_n132_ = ~x4 & (x2 ? ~x7 : x5);
  assign new_n133_ = ~x2 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x4 & (~x2 | ~x7)) | (~new_n135_ & ~x7) | (~x2 & ~new_n136_ & x7);
  assign new_n135_ = x3 & x5 & ~x6;
  assign new_n136_ = x0 & ~x1 & ~x5 & x6;
  assign z40 = new_n138_ | new_n139_ | new_n140_ | ~new_n141_;
  assign new_n138_ = x7 & (x2 | (~x0 & ~x4));
  assign new_n139_ = ~x0 & ((~x2 & x3) | (~x4 & ~x6));
  assign new_n140_ = x0 & (x2 | (~x4 & x6) | (x4 & ~x5));
  assign new_n141_ = (~x2 | (x3 & x4)) & ~x1 & (x4 | ~x5);
  assign z41 = x2 ? ~x7 : ((~x1 & (~x3 | ~x5)) | ~x0 | (x1 & x3));
  assign z42 = (x4 & (~x2 | ~x7)) | (~x2 & x7 & (~new_n91_ | x5 | ~x6)) | (~x7 & (~x5 | x6));
  assign z43 = (~x7 & (new_n145_ | ~new_n146_)) | ((new_n147_ | new_n148_) & ~x2) | (x2 & x7);
  assign new_n145_ = x0 & (x4 ? x2 : x6);
  assign new_n146_ = (~x4 | (~x1 & (~x2 | x3))) & (~x2 | x4 | (x5 & ~x6));
  assign new_n147_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n148_ = ~x4 & ((~x0 & ((~x5 & ~x6) | x7)) | (x5 & (x6 | x7)));
  assign z44 = new_n151_ | (~x2 & (new_n147_ | ~new_n150_));
  assign new_n150_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign new_n151_ = ~x7 & (x2 | (x1 & x4) | (x0 & ~x4 & x6));
  assign z45 = new_n153_ | (x1 & (~x2 | x7)) | x0 | (~new_n154_ & ~x1);
  assign new_n153_ = ~x4 & (x5 | (x1 & x6));
  assign new_n154_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = ~new_n103_ | new_n156_;
  assign new_n156_ = ~x4 & (x5 | (x6 & ~x7));
  assign z48 = ~new_n158_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n158_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = new_n160_ | ~new_n93_ | ~x2 | x7 | (x3 & x4);
  assign new_n160_ = ~x4 & (x5 | x6);
  assign z50 = ~x7 | (~x2 & (new_n162_ | x4 | x5 | ~x6));
  assign new_n162_ = x0 & (~x1 | ~x3);
  assign z51 = (~new_n166_ & ~x2) | (~new_n165_ & ~x7) | (~new_n164_ & (~x2 | ~x7));
  assign new_n164_ = x0 ? x1 : (~x1 & (x4 | ~x6));
  assign new_n165_ = (x0 | (x4 ? ~x2 : x3)) & (x4 | (~x5 & (~x2 | ~x6)));
  assign new_n166_ = (x0 | (x3 & (x4 | ~x5))) & (~x0 | ~x3) & (x4 | (~x5 ^ x6));
  assign z52 = (~new_n168_ & (~x2 | ~x7)) | new_n169_ | new_n171_ | (~new_n170_ & ~x2);
  assign new_n168_ = (x0 | (~x1 & (x4 | ~x6))) & (x4 | ~x5) & (~x0 | ~x3);
  assign new_n169_ = ~x0 & ((~x2 & ~x3) | (x2 & x3 & x4 & ~x7));
  assign new_n170_ = (~x0 | x1) & (x4 | x5 | ~x6);
  assign new_n171_ = x2 & ~x4 & x6 & ~x7;
  assign z53 = (~new_n175_ & ~new_n176_) | new_n177_ | (~x3 & (new_n173_ | new_n174_));
  assign new_n173_ = x0 & (~x7 | (x1 & ~x2));
  assign new_n174_ = ~x2 & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n175_ = x7 & (x2 | ~x3);
  assign new_n176_ = x1 & (x4 | (~x5 & ~x6));
  assign new_n177_ = ~x0 & x2 & x3 & ~x7;
  assign z54 = new_n180_ | new_n182_ | (~new_n181_ & ~x2) | (new_n179_ & ~x4);
  assign new_n179_ = (x5 | x6) & ((~x0 & ~x2 & ~x3) | (x3 & ~x7));
  assign new_n180_ = ~x1 & (x3 ? ~x7 : ~x2);
  assign new_n181_ = ((~x0 & ~x3) | (~x4 & x5 & x6)) & (~x3 | (~x0 & x7));
  assign new_n182_ = ~x7 & (x0 | (x2 & ~x3));
  assign z55 = (x0 & (x2 ? ~x7 : ~x3)) | (~new_n176_ & (~x2 | ~x7));
  assign z56 = ~new_n185_ | (~z08 & (new_n80_ | ~x1));
  assign new_n185_ = (x2 | (~x0 & x3)) & (x7 | (~x2 & (x4 | ~x6)));
  assign z57 = new_n156_ | ~x1 | x2 | (x0 & ~x3) | (~x0 & x3);
  assign z58 = ~new_n188_ | (~z08 & (x0 | ~x3));
  assign new_n188_ = (x7 | (x1 & x2 & (x4 | (~x5 & ~x6)))) & (x2 | (~x1 & ~x4 & ~x5 & x6));
  assign z59 = (~x2 & (new_n162_ | ~new_n104_ | ~new_n190_)) | new_n191_ | (x2 & (~new_n162_ | ~new_n192_));
  assign new_n190_ = x6 & x7;
  assign new_n191_ = ~x4 & (x5 | (x2 & x6));
  assign new_n192_ = ~x7 & (x1 | x3);
  assign z60 = (~new_n194_ & x0) | (x2 & (~x0 | x7)) | x3 | (~new_n195_ & ~x0);
  assign new_n194_ = x1 & x4;
  assign new_n195_ = ~x1 & ~x4 & x5 & x6 & x7;
  assign z61 = ~x2 | (~x7 & (new_n160_ | ~x0 | x1 | (~x1 & ~x3)));
  assign z62 = ((new_n160_ | ~x0 | ~x1) & (~x2 | ~x7)) | (x3 & (~x2 | (x1 & ~x7)));
  assign z09 = 1'b0;
  assign z26 = 1'b0;
  assign z33 = 1'b1;
  assign z10 = z08;
  assign z12 = z08;
  assign z15 = z08;
  assign z28 = z08;
  assign z30 = z08;
  assign z47 = new_n153_ | (x1 & (~x2 | x7)) | x0 | (~new_n154_ & ~x1);
endmodule


