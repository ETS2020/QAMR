// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n81_, new_n83_, new_n84_, new_n86_, new_n91_, new_n94_,
    new_n97_, new_n99_, new_n103_, new_n108_, new_n110_, new_n112_,
    new_n116_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n126_, new_n127_, new_n129_, new_n131_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n169_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n197_, new_n199_, new_n200_, new_n201_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n208_, new_n209_, new_n211_,
    new_n213_, new_n215_;
  assign z00 = (x2 & ~x7) | (~x4 & ~x5 & ~x6);
  assign z01 = ~x7 & ~x6 & ~x2 & ~x5;
  assign z02 = ~x7 & ~x6 & x5 & new_n76_ & ~x4;
  assign new_n76_ = ~x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & (x2 | (x3 & ~x4 & ~x5 & x6));
  assign z05 = ~x7 & x6 & x5 & ~x2 & ~x4;
  assign z06 = x7 & ~x6 & ~x5 & ~x4 & new_n81_ & x3;
  assign new_n81_ = x2 & ~x0 & ~x1;
  assign z07 = (x2 & ~x7) | (new_n84_ & new_n83_ & ~x2 & ~x3);
  assign new_n83_ = ~x0 & x1;
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (~x7 | (new_n86_ & x0 & x1 & ~x3));
  assign new_n86_ = ~x4 & x5 & x6;
  assign z09 = x7 & x6 & ~x5 & ~x4 & new_n81_ & ~x3;
  assign z10 = x7 & x6 & x5 & ~x4 & new_n83_ & x2;
  assign z11 = (x2 & ~x7) | (new_n84_ & x0 & x1 & ~x2 & ~x3);
  assign z12 = x7 & x6 & x5 & ~x4 & new_n91_ & ~x3;
  assign new_n91_ = x2 & x0 & ~x1;
  assign z13 = (x2 & ~x7) | (new_n84_ & new_n83_ & ~x2 & x3);
  assign z14 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z15 = x2 & (~x7 | (new_n86_ & ~x0 & x1 & x3));
  assign z16 = x7 & new_n97_ & x6;
  assign new_n97_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & new_n99_ & ~x2;
  assign new_n99_ = x0 & ~x1;
  assign z18 = x7 & ~x5 & x4 & new_n81_ & x3;
  assign z19 = (x2 & ~x7) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = (x2 & ~x7) | (new_n103_ & x0 & ~x1 & ~x2);
  assign new_n103_ = ~x5 & ~x6 & ~x3 & ~x4;
  assign z21 = ~x5 & ~x4 & x3 & new_n99_ & ~x2 & ~x6;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n99_ & ~x2;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & new_n110_ & x6;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n112_ & ~x3;
  assign new_n112_ = x0 & x2;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n91_ & x3;
  assign z29 = x7 & new_n108_ & ~x6;
  assign z30 = x7 & new_n116_ & x6;
  assign new_n116_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = (x0 & (x2 | (~x4 & x6))) | ~new_n118_ | (~x0 & (~x4 | (~x2 & x3)));
  assign new_n118_ = (x4 | (~x2 & ~x5)) & (~x2 | (x3 & x7)) & ~x1 & (~x4 | x5);
  assign z32 = (~new_n121_ & ~x0) | new_n122_ | ~new_n123_ | (x0 & (new_n120_ | x2));
  assign new_n120_ = ~x4 & x6;
  assign new_n121_ = (x2 | (~x3 & ~x4)) & (x4 | (~x3 & x6 & ~x7));
  assign new_n122_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n123_ = (~x2 | (x4 & x7)) & ~x1 & (x4 | ~x5) & (x2 | ~x4 | x5);
  assign z33 = ~new_n120_ | ~x7 | ~new_n112_ | (~x1 & x5) | (x1 & x3 & ~x5);
  assign z34 = (~x2 & (x5 ? ~new_n127_ : ~new_n126_)) | (x7 & (x2 | x5));
  assign new_n126_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign new_n127_ = x3 & ~x4 & ~x6;
  assign z35 = (x0 & (x2 | ~x5)) | ~new_n129_ | (x2 & (~x3 | ~x5 | ~x7));
  assign new_n129_ = ~x1 & x4 & (x0 | x2 | ~x3);
  assign z36 = new_n131_ | ~new_n99_ | ~x4 | x5;
  assign new_n131_ = x2 & (~x5 | ~x7 | x0 | ~x3);
  assign z37 = (~new_n133_ & ~x2) | (x7 & (x2 | (x3 & ~x5)));
  assign new_n133_ = (x0 | (x3 & ~x5)) & (x1 | x3) & (~x3 | (x5 ? ~x1 : (~x4 & x6)));
  assign z38 = new_n135_ | new_n136_ | (~new_n137_ & x0) | new_n138_ | (new_n139_ & ~x0);
  assign new_n135_ = x1 & (~x2 | x7);
  assign new_n136_ = ~x3 & ((x2 & x7) | (x0 & ~x2 & ~x4));
  assign new_n137_ = x2 ? ~x7 : (x4 | ~x6);
  assign new_n138_ = ~x4 & (x2 ? x7 : x5);
  assign new_n139_ = ~x2 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x4 & (~x2 | x7)) | (~x2 & ~new_n141_ & ~x7) | (~new_n142_ & x7);
  assign new_n141_ = x3 & x5 & ~x6;
  assign new_n142_ = x0 & ~x1 & ~x2 & ~x5 & x6;
  assign z40 = new_n144_ | new_n145_ | new_n146_ | new_n147_ | new_n148_;
  assign new_n144_ = x1 & (~x2 | (~x0 & x7));
  assign new_n145_ = x3 & (x0 ? (x2 & x7) : ~x2);
  assign new_n146_ = x0 & (x2 ? (x7 & (x4 | x5 | ~x6)) : (x4 ? ~x5 : x6));
  assign new_n147_ = ~x4 & ((~x2 & x5) | (~x0 & (x7 | (~x2 & ~x6))));
  assign new_n148_ = ~x0 & x2 & ~x3 & x7;
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | x7)) | (~x2 & ~x7 & (~x5 | x6)) | (~new_n151_ & x7);
  assign new_n151_ = x0 & ~x1 & ~x5 & x6 & (~x2 | x3);
  assign z43 = x2 ? (new_n155_ | ~new_n156_) : (new_n153_ | new_n154_);
  assign new_n153_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n154_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n155_ = x1 & (~x0 | x3);
  assign new_n156_ = (x0 | (x3 & x4)) & x7 & (~x0 | (~x4 & ~x5 & x6));
  assign z44 = x2 ? x7 : (new_n153_ | ~new_n158_);
  assign new_n158_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | (x5 & (~x1 | ~x4)) | ~new_n160_ | (~x1 & (x2 | x4 | ~x6));
  assign new_n160_ = ~x0 & x7;
  assign z46 = ~new_n76_ | ~new_n83_ | (~x4 & (x5 | (x6 & ~x7)));
  assign z47 = (x7 & (new_n163_ | new_n164_ | (~x1 & x2))) | (~new_n165_ & ~x2);
  assign new_n163_ = ~x0 & ~x4 & (x5 | (x2 & x6));
  assign new_n164_ = x0 & (~x3 | x4 | ~x5 | ~x6);
  assign new_n165_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~new_n167_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n167_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (x7 & (~new_n169_ | (~x4 & (x5 | (x2 & x6)))));
  assign new_n169_ = (~x3 | ~x4) & (~x2 | (~x0 & ~x1));
  assign z50 = new_n172_ | (~x2 & (~new_n171_ | (x0 & (~x1 | ~x3))));
  assign new_n171_ = ~x4 & ~x5 & x6 & x7;
  assign new_n172_ = x7 & (x2 | (x3 & x4) | (~x4 & x5));
  assign z51 = new_n175_ | new_n176_ | (~new_n177_ & ~x2) | (new_n174_ & ~x0 & x2);
  assign new_n174_ = x4 & x7;
  assign new_n175_ = (~x2 | x7) & (x0 ? ~x1 : (x1 | ~x3));
  assign new_n176_ = (x5 | x6) & ((~x0 & (x2 ? x7 : ~x4)) | (x0 & x2 & ~x4 & x7));
  assign new_n177_ = (~x0 | ~x3) & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z52 = (~new_n179_ & x0) | new_n180_ | ~new_n181_ | (~x0 & (new_n120_ | x1));
  assign new_n179_ = ~x3 & (x1 | x2);
  assign new_n180_ = ~x2 & ((~x4 & ~x5 & x6) | (~x0 & ~x3));
  assign new_n181_ = (x4 | (~x5 & (~x2 | ~x6))) & (~x2 | (x7 & (~x3 | ~x4)));
  assign z53 = new_n183_ | (~new_n186_ & ~x2) | (x7 & (new_n184_ | ~new_n185_));
  assign new_n183_ = x1 & ((x0 & ~x2 & ~x3) | (x3 & x7 & ~x0 & x2));
  assign new_n184_ = x0 & (x3 ? ~x1 : x2);
  assign new_n185_ = ((x3 ? x6 : ~x2) | (x1 & (x4 | ~x5))) & (~x3 | ((x4 | x5 | ~x6) & (x1 | (~x4 & x5)))) & (~x2 | x3 | x4 | ~x6);
  assign new_n186_ = x3 ? (x1 & (x4 | (~x5 & ~x6))) : (~x4 & x5 & x6 & x7);
  assign z54 = (~new_n188_ & ~x2) | new_n189_ | new_n190_ | new_n191_ | ~new_n192_;
  assign new_n188_ = x3 ? (~x4 & x5 & x6) : (x1 & (x0 | x4 | (~x5 & ~x6)));
  assign new_n189_ = ~x1 & (x0 | (x2 & x3));
  assign new_n190_ = (x4 | ~x5 | ~x6) & (x0 | (x2 & ~x3));
  assign new_n191_ = x0 & (x3 | ~x7);
  assign new_n192_ = (~x2 | x7) & (~x3 | (x7 & (x4 | (~x5 ^ x6))));
  assign z55 = new_n194_ | (~x1 & (~x2 | x7)) | (~new_n195_ & x0);
  assign new_n194_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & x7));
  assign new_n195_ = x2 ? (~x7 | (~x4 & x5 & x6)) : x3;
  assign z56 = (x0 & (~x2 | x7)) | ~new_n197_ | (~x1 & (~x2 | (x3 & x7)));
  assign new_n197_ = x2 ? (~x7 | (~x4 & x5 & x6)) : (x3 & (x4 | (~x5 & (~x6 | x7))));
  assign z57 = (~new_n83_ & ~x3) | new_n199_ | new_n200_ | ~new_n201_;
  assign new_n199_ = (x0 | ~x2) & (~x1 | (~x4 & x5));
  assign new_n200_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n201_ = (x0 | x2 | ~x3) & (~x2 | (~x0 & ~x4 & x5 & x6));
  assign z58 = (new_n203_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n204_ | (~new_n120_ & (x0 | ~x1));
  assign new_n203_ = ~x4 & (x5 | (x1 & x6));
  assign new_n204_ = (~x0 | (x1 & x5)) & x3 & x7 & (x1 | (~x2 & ~x5));
  assign z59 = (~new_n206_ & x0) | new_n209_ | (x7 & (new_n207_ | ~new_n208_));
  assign new_n206_ = (x1 | (x2 & (x3 | ~x7))) & (x2 | x3) & (~x6 | ~x7 | ~x2 | x4);
  assign new_n207_ = x2 & ((~x0 & (x1 | x3)) | (x1 & (x3 | (~x4 & x6))));
  assign new_n208_ = (x0 | (~x4 & x6)) & (~x5 | (x0 & x4));
  assign new_n209_ = ~x2 & (x4 | x5 | ~x6 | ~x7);
  assign z60 = (x2 & (~x7 | (~x0 & ~x3))) | ~new_n211_ | (x3 & (x0 | ~x2));
  assign new_n211_ = x0 ? (x1 & x4) : (~x1 & ~x4 & x5 & x6 & x7);
  assign z61 = ~new_n213_ | (~x4 & (x5 | x6));
  assign new_n213_ = x0 & ~x1 & x2 & x3 & x7;
  assign z62 = ~new_n215_ | (~x4 & (x5 | x6));
  assign new_n215_ = x0 & x1 & ~x3 & (~x2 | x7);
  assign z27 = 1'b0;
endmodule


