// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n82_, new_n84_, new_n87_, new_n90_, new_n93_, new_n95_,
    new_n97_, new_n101_, new_n104_, new_n106_, new_n108_, new_n110_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n124_, new_n125_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n139_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n150_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n162_, new_n164_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_;
  assign z00 = z11 | (new_n75_ & ~x4);
  assign z11 = ~x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z11 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = z11 | (~x4 & x5 & x6 & ~x7);
  assign z06 = ~x6 & ~x5 & ~x4 & new_n82_ & x3;
  assign new_n82_ = x2 & ~x0 & ~x1;
  assign z08 = ~x3 & (~x2 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x7 & x6 & ~x5 & ~x4 & new_n82_ & ~x3;
  assign z10 = (~x2 & ~x3) | (new_n87_ & ~x0 & x1 & x2);
  assign new_n87_ = x6 & x7 & ~x4 & x5;
  assign z12 = ~x3 & (~x2 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z13 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x2 & (~x3 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z15 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & x3 & new_n97_ & ~x2;
  assign new_n97_ = x0 & ~x1;
  assign z18 = ~x5 & x4 & new_n82_ & x3;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n97_ & ~x2;
  assign z22 = ~x2 & (new_n101_ | ~x3);
  assign new_n101_ = x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z26 = ~x3 & (~x2 | (new_n104_ & ~x5 & x0 & ~x4));
  assign new_n104_ = x6 & x7;
  assign z27 = ~x3 & (new_n106_ | ~x2);
  assign new_n106_ = ~x0 & x1 & ~x4 & ~x5 & x6 & ~x7;
  assign z28 = x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & x3 & x2 & x0 & ~x1;
  assign z30 = x7 & new_n110_ & x6;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n112_ | (~z11 & x1);
  assign new_n112_ = (x5 | (~x2 & (~x3 | ~x4))) & (~x2 | (~x0 & x3 & x4)) & (~x3 | ((x0 | x2) & (x4 | (~x5 & ~x6))));
  assign z32 = ~new_n114_ | (~z11 & x1);
  assign new_n114_ = (~x2 | (~x0 & x3 & x4)) & (~x3 | ((x4 | (~x5 & ~x6)) & (x2 | (x0 & (~x4 | x5)))));
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n118_ | new_n119_ | (~new_n120_ & x3) | (x2 & ~new_n121_ & ~x3);
  assign new_n118_ = x1 & (x3 ? ~x5 : x2);
  assign new_n119_ = (x2 | (x3 & x5)) & (x4 | x7);
  assign new_n120_ = (~x2 | (x5 & ~x6)) & (x5 | (x0 & (x4 | (x6 & x7)))) & (~x5 | ~x6);
  assign new_n121_ = ~x0 & ~x5 & x6;
  assign z35 = ~x5 | ~x3 | ~x4 | x1 | (x0 & x2) | (~x0 & ~x2);
  assign z36 = (x0 & (x2 | ~x3 | ~x4)) | ~new_n125_ | (~x0 & (~new_n124_ | ~x2 | x3));
  assign new_n124_ = ~x4 & x6 & ~x7;
  assign new_n125_ = ~x1 & ~x5;
  assign z37 = (~new_n124_ & (~x0 | ~x5)) | ~x3 | (x5 & (~x0 | x1 | x2));
  assign z38 = (~x0 & (~x2 | ~x4)) | (~new_n75_ & ~x4) | x1 | ~x3 | (x0 & x2);
  assign z39 = (~new_n129_ & x5) | ~x3 | x4 | (~new_n130_ & ~x5);
  assign new_n129_ = ~x6 & ~x7;
  assign new_n130_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = new_n134_ | new_n133_ | new_n135_ | new_n136_ | (new_n132_ & ~x2);
  assign new_n132_ = ~x4 & (x5 | x6);
  assign new_n133_ = ~x0 & (~x2 | ~x4);
  assign new_n134_ = (~x0 | ~x2) & (x1 | ~x3);
  assign new_n135_ = x4 & (x2 ? x0 : ~x5);
  assign new_n136_ = x0 & x2 & (~x6 | ~x7 | x3 | x5);
  assign z41 = ~new_n97_ | x2 | ~x3 | ~x5;
  assign z42 = (x4 & (x3 | (x2 & x5))) | (x5 & ~new_n129_ & (x2 | x3)) | (~new_n139_ & ~x5);
  assign new_n139_ = x3 ? (x0 & ~x1 & x6 & x7) : ~x2;
  assign z43 = (~x3 & (~x2 | x4)) | ~new_n142_ | (~new_n141_ & ~x4);
  assign new_n141_ = (x5 | (x0 & (~x2 | (x6 & x7)))) & (~x5 | (~x6 & ~x7)) & (x2 | ~x6 | x7);
  assign new_n142_ = (x2 | ((x0 | (~x4 & x5)) & (~x1 | x5))) & (~x1 | (~x4 & (~x3 | x5))) & (~x0 | ~x2 | ~x4);
  assign z45 = (~new_n144_ & x1) | (x5 & (~x1 | ~x4)) | x0 | (~new_n145_ & ~x1);
  assign new_n144_ = x2 & (x4 | ~x6);
  assign new_n145_ = ~x2 & x3 & ~x4 & x6 & x7;
  assign z47 = (new_n147_ & ~x0) | (~x2 & (x0 | x1)) | new_n148_ | (~x1 & (x2 | x5)) | (x0 & (~x1 | ~x5));
  assign new_n147_ = ~x4 & (x5 | (x1 & x6));
  assign new_n148_ = (x0 | ~x1) & (~x6 | ~x7 | ~x3 | x4);
  assign z48 = x2 | (x3 & (new_n150_ | x0 | x1));
  assign new_n150_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = (~new_n75_ & ~x4) | x0 | x1 | ~x2 | (x3 & x4);
  assign z50 = x2 | (x3 & (~new_n104_ | x5 | new_n97_ | x4));
  assign z51 = (~x0 & ((x1 & (x2 | x3)) | (x3 & ~new_n75_ & ~x4) | ~x3 | (x2 & x4))) | (x0 & (~x1 | ~x2)) | (~x4 & ~new_n75_ & (x0 | x2));
  assign z52 = (~x0 & ((x1 & (x2 | x3)) | (x3 & (x4 ? x2 : ~new_n75_)))) | (x2 & ~new_n75_ & ~x4) | (x0 & x3);
  assign z53 = new_n156_ | new_n157_ | (x3 & (new_n158_ | ~new_n159_));
  assign new_n156_ = (~x1 | (~x4 & x6)) & (x3 ? (~x2 | ~x5) : x2);
  assign new_n157_ = x2 & ((x0 & (~x1 | ~x3)) | (~x0 & x1 & x3) | (~x3 & ~x4 & x5));
  assign new_n158_ = (~x6 | ~x7) & (~x1 | (~x4 & x5));
  assign new_n159_ = (x1 | (~x0 & ~x4)) & (x2 | x4 | ~x5);
  assign z54 = (x0 & (x3 | (~x1 & x2))) | (~new_n87_ & (~x2 ^ ~x3)) | (x3 & (new_n150_ | (~x1 & x2)));
  assign z55 = new_n162_ | (~x1 & (x2 | x3)) | (x0 & ~new_n87_ & x2);
  assign new_n162_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : x3);
  assign z56 = (x0 & (x2 | x3)) | (~new_n87_ & x2) | (x3 & (~x1 | (new_n164_ & ~x2)));
  assign new_n164_ = ~x4 & (x5 | (x6 & ~x7));
  assign z57 = (~x1 & (x0 | ~x3)) | new_n166_ | (~new_n167_ & ~x0) | (x0 & (x2 | ~x3));
  assign new_n166_ = ~x4 & ((x6 & ~x7) | (x0 & x5));
  assign new_n167_ = x2 & ~x4 & x5 & x6 & x7;
  assign z58 = x2 ? (new_n169_ | new_n170_ | ~x1 | ~x3) : (~new_n171_ & x3);
  assign new_n169_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n170_ = x0 & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n171_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z59 = new_n173_ | new_n174_ | new_n176_ | (~new_n177_ & ~x3) | (~new_n175_ & x3);
  assign new_n173_ = x1 & (x3 ? x2 : ~x0);
  assign new_n174_ = x0 & ((~x3 & ~x4 & x6) | (~x1 & (~x3 | (~x2 & x3))));
  assign new_n175_ = x2 ? (x0 & (x4 | ~x6)) : (x6 & x7 & ~x4 & ~x5);
  assign new_n176_ = x5 & (~x4 | (~x0 & ~x3));
  assign new_n177_ = x2 & (x0 | (~x4 & x6 & x7));
  assign z60 = (x2 & ~x3 & (~x0 | ~x1 | ~x4)) | (x3 & (~new_n87_ | x0 | x1 | ~x2));
  assign z61 = new_n132_ | ~x0 | x1 | ~x2 | (~x1 & ~x3);
  assign z62 = new_n132_ | ~x0 | ~x1 | ~x2 | (x1 & x3);
  assign z07 = 1'b0;
  assign z20 = 1'b0;
  assign z29 = 1'b0;
  assign z46 = 1'b1;
  assign z44 = ~z11;
  assign z19 = z11;
  assign z24 = z11;
  assign z25 = z11;
endmodule


