// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n92_, new_n95_,
    new_n97_, new_n103_, new_n105_, new_n109_, new_n111_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n121_, new_n123_,
    new_n125_, new_n126_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n135_, new_n137_, new_n138_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n146_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n160_, new_n162_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_;
  assign z00 = ~x4 & ~x5 & ~z20 & ~x6;
  assign z20 = ~x2 & ~x3;
  assign z01 = z20 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & (~x2 | (~x6 & ~x7 & ~x4 & x5));
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (~x2 & ~x3) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z20 | (~x4 & x5 & x6 & ~x7);
  assign z06 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & new_n81_ & x3 & ~x4);
  assign new_n81_ = ~x5 & ~x6;
  assign z08 = ~x3 & (~x2 | (new_n83_ & x0 & x1 & ~x4));
  assign new_n83_ = x5 & x6 & x7;
  assign z09 = ~x3 & (new_n85_ | ~x2);
  assign new_n85_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z10 = x7 & x6 & x5 & new_n87_ & ~x4;
  assign new_n87_ = x2 & ~x0 & x1;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n89_ & ~x3;
  assign new_n89_ = x2 & x0 & ~x1;
  assign z13 = ~x2 & (~x3 | (new_n83_ & ~x0 & x1 & ~x4));
  assign z14 = x7 & x6 & new_n92_ & x5;
  assign new_n92_ = ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n87_ & x3;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & x3 & new_n97_ & ~x2;
  assign new_n97_ = x0 & ~x1;
  assign z18 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & x3 & x4 & ~x5);
  assign z21 = ~x2 & (~x3 | (new_n97_ & new_n81_ & ~x4));
  assign z22 = x7 & x6 & new_n92_ & ~x5;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z26 = ~x3 & (~x2 | (new_n103_ & x0 & ~x4));
  assign new_n103_ = ~x5 & x6 & x7;
  assign z27 = ~x3 & (new_n105_ | ~x2);
  assign new_n105_ = ~x5 & x6 & ~x7 & ~x0 & x1 & ~x4;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n89_ & x3;
  assign z30 = ~x3 & (~x2 | (new_n103_ & x0 & x1 & ~x4));
  assign z31 = (~x5 & (~x0 | x4)) | (~x0 & (~x2 | ~x4)) | ~new_n109_ | (~x4 & (x5 | x6));
  assign new_n109_ = ~x1 & x3 & (~x0 | ~x2);
  assign z32 = ~new_n111_ | (~z20 & x1);
  assign new_n111_ = (~x3 | ((x4 | (~x5 & ~x6)) & (x2 | (x0 & (~x4 | x5))))) & (~x2 | (~x0 & x3 & x4));
  assign z33 = ~new_n113_ | ~x7 | x4 | ~x6;
  assign new_n113_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n115_ | new_n116_ | (~new_n117_ & x3) | (x2 & ~new_n118_ & ~x3);
  assign new_n115_ = x1 & (x3 ? ~x5 : x2);
  assign new_n116_ = (x2 | (x3 & x5)) & (x4 | x7);
  assign new_n117_ = (~x2 | (x5 & ~x6)) & (x5 | (x0 & (x4 | (x6 & x7)))) & (~x5 | ~x6);
  assign new_n118_ = ~x0 & ~x5 & x6;
  assign z35 = ~x5 | ~x3 | ~x4 | x1 | (x0 & x2) | (~x0 & ~x2);
  assign z36 = ~new_n121_ | (~z20 & (x1 | x5));
  assign new_n121_ = (~x3 | (x0 & ~x2 & x4)) & (~x2 | (~x0 & ~x4 & x6 & ~x7));
  assign z37 = (x2 & (~x3 | x5)) | (x3 & (x5 ? ~new_n97_ : ~new_n123_));
  assign new_n123_ = ~x4 & x6 & ~x7;
  assign z38 = new_n125_ | new_n126_ | ~new_n109_;
  assign new_n125_ = ~x0 & (~x2 | ~x4);
  assign new_n126_ = ~x4 & (x5 | x6);
  assign z39 = (x5 & (x6 | x7)) | (~new_n128_ & ~x5) | ~x3 | x4;
  assign new_n128_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = new_n130_ | new_n125_ | new_n131_ | new_n132_ | (new_n126_ & ~x2);
  assign new_n130_ = (~x0 | ~x2) & (x1 | ~x3);
  assign new_n131_ = x4 & (x2 ? x0 : ~x5);
  assign new_n132_ = x0 & x2 & (x3 | x5 | ~x6 | ~x7);
  assign z41 = ~new_n97_ | x2 | ~x3 | ~x5;
  assign z42 = ~new_n135_ | (~x3 & (~x2 | ~x5));
  assign new_n135_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = (~x3 & (~x2 | x4)) | ~new_n138_ | (~new_n137_ & ~x4);
  assign new_n137_ = (x5 | (x0 & (~x2 | (x6 & x7)))) & (~x5 | (~x6 & ~x7)) & (x2 | ~x6 | x7);
  assign new_n138_ = (x2 | ((~x1 | x5) & (x0 | (~x4 & x5)))) & (~x1 | (~x4 & (~x3 | x5))) & (~x0 | ~x2 | ~x4);
  assign z45 = (~new_n140_ & x1) | (x5 & (~x1 | ~x4)) | x0 | (~new_n141_ & ~x1);
  assign new_n140_ = x2 & (x4 | ~x6);
  assign new_n141_ = ~x2 & x3 & ~x4 & x6 & x7;
  assign z47 = x2 ? (new_n143_ | new_n144_ | ~x1) : (~new_n85_ & x3);
  assign new_n143_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n144_ = x0 & (~x3 | x4 | ~x5 | ~x6 | ~x7);
  assign z48 = x2 | (x3 & ((~new_n146_ & ~x2) | (~x4 & ~x5 & x6)));
  assign new_n146_ = ~x0 & ~x1 & (x4 | ~x5 | (x6 & x7));
  assign z49 = (~new_n81_ & ~x4) | x0 | x1 | ~x2 | (x3 & x4);
  assign z50 = x2 | (x3 & (~new_n103_ | new_n97_ | x4));
  assign z51 = (x0 & (~x1 | ~x2)) | (~x3 & (~x2 | (~x0 & ~x4))) | (~new_n81_ & ~x4) | (~x0 & (x1 | (x2 & x4)));
  assign z52 = (x0 & (~x2 | x3)) | (~x0 & (x1 | (x2 & x3 & x4))) | (~x2 & ~x3) | (~new_n81_ & ~x4);
  assign z53 = new_n152_ | new_n153_ | new_n154_ | (~new_n155_ & x3);
  assign new_n152_ = x0 & (x3 ? ~x1 : x2);
  assign new_n153_ = (~x1 | (~x4 & x6)) & (x3 ? (~x2 | ~x5) : x2);
  assign new_n154_ = x2 & ((~x3 & ~x4 & x5) | (~x0 & x1 & x3));
  assign new_n155_ = ((x6 & x7) | (x1 & (x4 | ~x5))) & (x2 | x4 | ~x5) & (x1 | ~x4);
  assign z54 = (x0 & (~x1 | x3)) | (~new_n157_ & (~x2 | ~x3)) | new_n158_ | (~x2 & ~x3) | (~x1 & x2 & x3);
  assign new_n157_ = ~x4 & x5 & x6 & x7;
  assign new_n158_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z55 = new_n160_ | (~x1 & (x2 | x3)) | (x0 & ~new_n157_ & x2);
  assign new_n160_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : x3);
  assign z56 = (x0 & (x2 | x3)) | (~new_n162_ & x3) | (~new_n157_ & x2);
  assign new_n162_ = x1 & (x2 | x4 | (~x5 & (~x6 | x7)));
  assign z57 = (~x1 & (~x2 ^ ~x3)) | (~new_n165_ & x2) | (~new_n164_ & x3);
  assign new_n164_ = (x4 | ~x6 | x7) & (x2 | (x0 & (x4 | ~x5)));
  assign new_n165_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z58 = (new_n168_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n169_ | (~new_n167_ & (x0 | ~x1));
  assign new_n167_ = ~x4 & x6 & x7;
  assign new_n168_ = ~x4 & (x5 | (x1 & x6));
  assign new_n169_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = new_n171_ | new_n172_ | new_n174_ | (~new_n175_ & ~x3) | (~new_n173_ & x3);
  assign new_n171_ = x1 & (x3 ? x2 : ~x0);
  assign new_n172_ = x0 & ((~x3 & ~x4 & x6) | (~x1 & (~x3 | (~x2 & x3))));
  assign new_n173_ = x2 ? (x0 & (x4 | ~x6)) : (~x4 & ~x5 & x6 & x7);
  assign new_n174_ = x5 & (~x4 | (~x0 & ~x3));
  assign new_n175_ = x2 & (x0 | (~x4 & x6 & x7));
  assign z60 = (x2 & ~x3 & (~x0 | ~x1 | ~x4)) | (x3 & (~new_n157_ | x0 | x1 | ~x2));
  assign z61 = new_n126_ | ~new_n97_ | ~x2 | ~x3;
  assign z62 = x3 | (x2 & (new_n126_ | ~x0 | ~x1));
  assign z07 = 1'b0;
  assign z11 = 1'b0;
  assign z19 = 1'b0;
  assign z25 = 1'b0;
  assign z46 = 1'b1;
  assign z44 = ~z20;
  assign z24 = z20;
  assign z29 = z20;
endmodule


