// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n80_, new_n82_, new_n84_, new_n86_, new_n89_, new_n93_, new_n98_,
    new_n101_, new_n102_, new_n105_, new_n107_, new_n109_, new_n111_,
    new_n114_, new_n116_, new_n118_, new_n119_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n128_, new_n130_, new_n132_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n142_, new_n143_, new_n144_,
    new_n147_, new_n148_, new_n150_, new_n152_, new_n154_, new_n158_,
    new_n161_, new_n164_, new_n165_, new_n168_, new_n170_, new_n172_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = z06 | (new_n80_ & x6 & ~x7);
  assign new_n80_ = ~x4 & x5;
  assign z07 = x7 & new_n82_ & x6;
  assign new_n82_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x2 & (x3 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x7 & new_n86_ & x6;
  assign new_n86_ = ~x4 & ~x3 & x2 & ~x0 & ~x1 & ~x5;
  assign z10 = x2 & (x3 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z11 = (x2 & x3) | (new_n89_ & x0 & x1 & ~x2 & ~x3);
  assign new_n89_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (x3 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z13 = x3 & (x2 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z14 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x3 & (x2 | (new_n84_ & x0 & x1 & ~x4));
  assign z17 = ~x5 & x4 & ~x2 & x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = (x2 & x3) | (x0 & ~x1 & ~x2 & new_n98_ & ~x3 & ~x4);
  assign new_n98_ = ~x5 & ~x6;
  assign z21 = x3 & (x2 | (x0 & ~x1 & new_n98_ & ~x4));
  assign z22 = z06 | (new_n102_ & new_n101_ & x0);
  assign new_n101_ = ~x1 & ~x2;
  assign new_n102_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = x3 & (x2 | (~x0 & ~x1 & x5));
  assign z24 = ~x7 & new_n105_ & x6;
  assign new_n105_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x2 & x3) | (new_n107_ & ~x2 & ~x3 & ~x0 & x1);
  assign new_n107_ = ~x4 & ~x5 & x6 & ~x7;
  assign z26 = ~new_n109_ & x2;
  assign new_n109_ = ~x3 & (~x0 | x4 | x5 | ~x6 | ~x7);
  assign z27 = ~x7 & new_n111_ & x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = x7 & new_n105_ & ~x6;
  assign z30 = x7 & new_n114_ & x6;
  assign new_n114_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n116_ | (~x0 & (x3 | ~x4));
  assign new_n116_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = new_n118_ | (x4 & (~x0 | ~x5)) | ~new_n101_ | (~new_n119_ & ~x0);
  assign new_n118_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n119_ = ~x3 & ~x5 & x6 & ~x7;
  assign z33 = ~x2 | (~new_n121_ & ~x3);
  assign new_n121_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n124_ & ~x5) | ~new_n125_ | (~new_n123_ & (x2 | x5));
  assign new_n123_ = ~x4 & ~x7;
  assign new_n124_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n125_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n128_ & (~x0 | x2)) | x1 | x5 | (~x0 & ~x2);
  assign new_n128_ = ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n130_)) | x2 | (~x1 & ~x3);
  assign new_n130_ = ~x4 & x6 & ~x7;
  assign z38 = new_n118_ | ~new_n101_ | new_n132_;
  assign new_n132_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~new_n134_ | ~x3)) | (~new_n135_ & ~x5) | x2 | x4;
  assign new_n134_ = ~x6 & ~x7;
  assign new_n135_ = x6 & x7 & x0 & ~x1;
  assign z40 = x2 ? new_n109_ : (new_n137_ | new_n138_ | new_n80_ | x1);
  assign new_n137_ = x0 & (x4 ? ~x5 : x6);
  assign new_n138_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | (~x3 & x5))) | (x5 & ~new_n134_ & (~x2 | ~x3)) | (~x5 & (x2 ? ~x3 : ~new_n135_));
  assign z43 = x2 ? new_n144_ : (new_n142_ | new_n143_);
  assign new_n142_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n143_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n144_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = (x0 & (~new_n98_ | x4)) | ~new_n101_ | x3 | (~x0 & ~x4);
  assign z45 = (x0 & (~x2 | ~x3)) | new_n148_ | (~new_n147_ & ~x3);
  assign new_n147_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign new_n148_ = ~x2 & (x5 | ~x6 | ~x7 | x1 | x4);
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n150_ | ~x1 | x3));
  assign new_n150_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | new_n148_ | (x2 & ~x3 & (new_n152_ | ~x1));
  assign new_n152_ = ~x4 & (x5 | x6);
  assign z48 = ~new_n154_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n154_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (~x3 & (new_n152_ | x0 | x1));
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n102_ & ~x2) | (x2 & ~x3);
  assign z51 = (x0 & (~x1 | x3)) | (x2 & (new_n80_ | ~x0)) | new_n158_ | (~x0 & (x1 | ~x3));
  assign new_n158_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x0 & (x5 | x6)));
  assign z52 = (x1 & (~x0 | x3)) | (x3 & (x0 | x2)) | new_n152_ | (~x1 & ~x2 & ~x3);
  assign z53 = (~x3 & (~new_n161_ | (x0 & (x1 | x2)))) | (~x2 & x3 & (new_n152_ | ~x1));
  assign new_n161_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (x6 & x7 & ~x4 & x5);
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n152_ & ~x0))) | (~new_n89_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n164_ | (x0 & ~new_n165_ & ~x3) | ~x1 | (x2 & x3);
  assign new_n164_ = ~x4 & (x5 | x6) & (~x0 | x3);
  assign new_n165_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n89_ & ~x3) | (~x2 & (new_n150_ | ~x1 | ~x3));
  assign z57 = new_n168_ | (x0 & (x2 | ~x3)) | (~new_n89_ & x2) | ~x1 | (~x0 & x3);
  assign new_n168_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~x3 | (~new_n170_ & ~x2);
  assign new_n170_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z59 = (~new_n102_ & (~x0 | ~x2)) | new_n172_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n172_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = x3 ? ~x2 : (x0 ? (~x1 | ~x4) : (~new_n84_ | x1 | x2 | x4));
  assign z62 = x3 ? ~x2 : (~x0 | (x0 & (new_n152_ | ~x1)));
  assign z15 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = 1'b1;
  assign z18 = z06;
endmodule


