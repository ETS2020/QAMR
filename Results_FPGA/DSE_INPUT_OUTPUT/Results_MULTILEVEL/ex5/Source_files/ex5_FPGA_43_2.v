// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n75_, new_n81_, new_n83_, new_n85_, new_n88_, new_n90_,
    new_n92_, new_n96_, new_n101_, new_n104_, new_n110_, new_n112_,
    new_n115_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n129_, new_n131_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n168_, new_n171_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n191_, new_n193_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n206_, new_n208_;
  assign z00 = new_n74_ | (new_n75_ & ~x4);
  assign new_n74_ = x2 & ~x7;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x7 & ~x6 & ~x2 & ~x5;
  assign z02 = ~x7 & (x2 | (~x3 & ~x4 & x5 & ~x6));
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & (x2 | (x3 & ~x4 & ~x5 & x6));
  assign z05 = ~x7 & (new_n81_ | x2);
  assign new_n81_ = ~x4 & x5 & x6;
  assign z06 = x2 & (new_n83_ | ~x7);
  assign new_n83_ = ~x4 & ~x5 & ~x6 & ~x0 & ~x1 & x3;
  assign z07 = (x2 & ~x7) | (new_n85_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n85_ = ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (~x7 | (new_n81_ & x0 & x1 & ~x3));
  assign z09 = x2 & (~x7 | (new_n88_ & ~x0 & ~x1 & ~x3));
  assign new_n88_ = ~x4 & ~x5 & x6;
  assign z10 = new_n90_ & x7;
  assign new_n90_ = x6 & x5 & ~x4 & x2 & ~x0 & x1;
  assign z11 = x7 & x6 & x5 & ~x4 & new_n92_ & ~x3;
  assign new_n92_ = ~x2 & x0 & x1;
  assign z12 = x2 & (~x7 | (new_n81_ & x0 & ~x1 & ~x3));
  assign z13 = (x2 & ~x7) | (new_n85_ & ~x2 & x3 & ~x0 & x1);
  assign z14 = (x2 & ~x7) | (new_n85_ & new_n96_ & ~x2 & x3);
  assign new_n96_ = x0 & ~x1;
  assign z15 = x2 & (~x7 | (new_n81_ & ~x0 & x1 & x3));
  assign z16 = x7 & x6 & x5 & ~x4 & new_n92_ & x3;
  assign z17 = ~x5 & x4 & new_n96_ & ~x2;
  assign z18 = x7 & ~x5 & x4 & x3 & new_n101_ & x2;
  assign new_n101_ = ~x0 & ~x1;
  assign z19 = x4 & ~x3 & new_n101_ & ~x2;
  assign z20 = new_n74_ | (new_n104_ & x0 & new_n75_ & ~x3 & ~x4);
  assign new_n104_ = ~x1 & ~x2;
  assign z21 = new_n74_ | (new_n104_ & x0 & new_n75_ & x3 & ~x4);
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n96_ & ~x2;
  assign z23 = (x2 & ~x7) | (new_n101_ & ~x2 & x3 & x5);
  assign z24 = ~x7 & (x2 | (new_n88_ & ~x0 & ~x1 & ~x3));
  assign z25 = ~x7 & new_n110_ & x6;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = new_n112_ & x7;
  assign new_n112_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z28 = x2 & (~x7 | (new_n88_ & x0 & ~x1 & x3));
  assign z29 = x7 & new_n115_ & ~x6;
  assign new_n115_ = ~x5 & ~x3 & ~x2 & ~x0 & ~x1 & ~x4;
  assign z30 = x2 & (~x7 | (new_n88_ & x0 & x1 & ~x3));
  assign z31 = (x0 & (x2 ? x7 : new_n118_)) | (~new_n119_ & ~x2) | (~new_n120_ & x7);
  assign new_n118_ = ~x4 & x6;
  assign new_n119_ = (x0 | (~x3 & x4)) & ~x1 & (~x4 | x5) & (x4 | ~x5);
  assign new_n120_ = ~x1 & (~x2 | (x3 & x4 & x5));
  assign z32 = new_n122_ | new_n123_ | (~new_n124_ & x2) | x1 | (~new_n125_ & ~x2);
  assign new_n122_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n123_ = x0 & (x2 | (~x2 & ~x4 & x6));
  assign new_n124_ = x4 & x7;
  assign new_n125_ = (~x4 | (x0 & x5)) & (x4 | ~x5) & (x0 | (~x3 & (x4 | (x6 & ~x7))));
  assign z33 = ~x2 | (~new_n127_ & x7);
  assign new_n127_ = x0 & ~x4 & (x1 | ~x5) & x6 & (~x1 | ~x3 | x5);
  assign z34 = ~new_n129_ | (~x5 & (~new_n96_ | (~x4 & (~x6 | ~x7))));
  assign new_n129_ = ~x2 & (~x5 | (x3 & ~x4 & ~x6 & ~x7));
  assign z35 = (x0 & (x2 | ~x5)) | ~new_n131_ | (x2 & (~x3 | ~x5 | ~x7));
  assign new_n131_ = ~x1 & x4 & (x0 | x2 | ~x3);
  assign z36 = x2 ? x7 : (~new_n96_ | ~x4 | x5);
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n134_)) | x2 | (~x1 & ~x3);
  assign new_n134_ = ~x4 & x6 & ~x7;
  assign z38 = new_n137_ | new_n138_ | (~new_n136_ & x0) | new_n139_ | (new_n140_ & ~x0);
  assign new_n136_ = x2 ? ~x7 : (x4 | ~x6);
  assign new_n137_ = x1 & (~x2 | x7);
  assign new_n138_ = ~x3 & ((x2 & x7) | (x0 & ~x2 & ~x4));
  assign new_n139_ = ~x4 & (x2 ? x7 : x5);
  assign new_n140_ = ~x2 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~x3 | x6 | x7)) | ~new_n142_ | (~x5 & (~new_n96_ | ~x6 | ~x7));
  assign new_n142_ = ~x2 & ~x4;
  assign z40 = new_n144_ | new_n145_ | new_n146_ | new_n147_ | new_n148_;
  assign new_n144_ = x1 & (~x2 | (~x0 & x7));
  assign new_n145_ = x3 & (x0 ? (x2 & x7) : ~x2);
  assign new_n146_ = x0 & (x2 ? (x7 & (x4 | x5 | ~x6)) : (x4 ? ~x5 : x6));
  assign new_n147_ = ~x4 & ((~x2 & x5) | (~x0 & (x7 | (~x2 & ~x6))));
  assign new_n148_ = ~x0 & x2 & ~x3 & x7;
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n151_ | (x2 & (~x3 | x5));
  assign new_n151_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = new_n153_ | ~new_n157_ | (x2 & (new_n154_ | new_n155_ | ~new_n156_));
  assign new_n153_ = ~x4 & ((x5 & (x6 | x7)) | (~x0 & (x2 | x7 | (~x5 & ~x6))) | (x0 & x6 & ~x7));
  assign new_n154_ = ~x0 & (x1 | ~x3);
  assign new_n155_ = x0 & (x4 | x5 | ~x6);
  assign new_n156_ = x7 & (~x1 | ~x3);
  assign new_n157_ = (x0 | ((x2 | ~x3 | (~x4 & x5)) & (~x1 | x5))) & (~x1 | (~x4 & (x2 | x5)));
  assign z44 = (x6 & (x0 | (~x4 & x5))) | ~new_n159_ | (~x4 & (~x0 | (x5 & x7)));
  assign new_n159_ = (~x0 | (~x3 & ~x4 & ~x5)) & (x0 | ((x2 | ~x3 | (~x4 & x5)) & (~x1 | x5))) & ~x2 & (~x1 | (~x4 & (x2 | x5)));
  assign z45 = ~new_n161_ | (~new_n74_ & x0);
  assign new_n161_ = (~x7 | ((x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6))))) & (x2 | (~x1 & ~x4 & ~x5 & x6 & x7));
  assign z46 = new_n163_ | x0 | ~x1 | x2 | x3;
  assign new_n163_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = new_n165_ | (~x2 & (x0 | x1)) | ~new_n166_ | (~new_n118_ & (x0 | ~x1));
  assign new_n165_ = ~x0 & ~x4 & (x5 | (x1 & x6));
  assign new_n166_ = (~x0 | (x1 & x3 & x5)) & x7 & (x1 | (~x2 & ~x5));
  assign z48 = ~new_n168_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n168_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = (~new_n75_ & ~x4) | ~new_n101_ | ~x2 | ~x7 | (x3 & x4);
  assign z50 = x2 ? x7 : (new_n171_ | ~x6 | ~x7 | x4 | x5);
  assign new_n171_ = x0 & (~x1 | ~x3);
  assign z51 = (x0 & (~x1 | (~x2 & x3))) | new_n173_ | (~new_n174_ & x2) | (~x0 & (x1 | ~x3));
  assign new_n173_ = ~x4 & (((~x0 | x2) & (x5 | x6)) | (x5 & ~x7) | (~x2 & (~x5 ^ ~x6)));
  assign new_n174_ = x7 & (x0 | ~x4);
  assign z52 = new_n176_ | new_n177_ | ~new_n178_ | (x0 & (new_n104_ | x3));
  assign new_n176_ = ~x2 & ((~x0 & ~x3) | (~x4 & ~x5 & x6));
  assign new_n177_ = ~x4 & (x5 | ((~x0 | x2) & x6));
  assign new_n178_ = (~x2 | x7) & (x0 | (~x1 & (~x2 | ~x3 | ~x4)));
  assign z53 = new_n180_ | new_n181_ | new_n182_ | new_n183_ | ~new_n184_;
  assign new_n180_ = x1 & (x0 ? ~x3 : (x2 & x3));
  assign new_n181_ = x0 & (x3 ? ~x1 : x2);
  assign new_n182_ = (x4 | ~x5 | ~x6) & (x3 ? ~x1 : ~x2);
  assign new_n183_ = ~x1 & ((~x2 & x3) | (x2 & ~x3) | ~x7);
  assign new_n184_ = (x4 | (((~x5 & ~x6) | (x2 ^ ~x3)) & (~x3 | (x5 ^ ~x6)))) & (x7 | (~x2 & x3));
  assign z54 = (~x2 & (new_n186_ | ~new_n187_)) | (x7 & (new_n188_ | ~new_n189_));
  assign new_n186_ = ~x3 & (~x1 | (~x0 & ~x4 & (x5 | x6)));
  assign new_n187_ = (~x0 | ~x3) & ((~x0 & ~x3) | (~x4 & x5 & x6 & x7));
  assign new_n188_ = x0 & (~x1 | x3);
  assign new_n189_ = (~x3 | ((x1 | ~x2) & (x4 | (x5 ^ ~x6)))) & (~x2 | x3 | (~x4 & x5 & x6));
  assign z55 = new_n191_ | ~new_n137_ | (x0 & (x2 ? ~new_n81_ : ~x3));
  assign new_n191_ = ~x4 & (x5 | x6) & (~x0 | ~x2);
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n193_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n193_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = new_n195_ | new_n196_ | (~new_n197_ & ~x2) | (x2 & ~new_n81_ & x7);
  assign new_n195_ = x0 & (x2 ? x7 : ~x3);
  assign new_n196_ = ~x1 & (~x2 | (~x3 & x7));
  assign new_n197_ = (x0 | ~x3) & (x4 | (~x5 & (~x6 | x7)));
  assign z58 = new_n165_ | (~x2 & (x0 | x1)) | ~new_n199_ | (~new_n118_ & (x0 | ~x1));
  assign new_n199_ = (~x0 | (x1 & x5)) & x3 & x7 & (x1 | (~x2 & ~x5));
  assign z59 = (~new_n201_ & x0) | new_n204_ | (x7 & (new_n202_ | ~new_n203_));
  assign new_n201_ = (x1 | (x2 & (x3 | ~x7))) & (x2 | x3) & (~x6 | ~x7 | ~x2 | x4);
  assign new_n202_ = x2 & ((~x0 & (x1 | x3)) | (x1 & (x3 | (~x4 & x6))));
  assign new_n203_ = (~x5 | (x0 & x4)) & (x0 | (~x4 & x6));
  assign new_n204_ = ~x2 & (x4 | x5 | ~x6 | ~x7);
  assign z60 = (~new_n206_ & (~x2 | x7)) | (~x0 & (x2 ? (~x3 & x7) : ~x7)) | (x3 & (~x2 | (x0 & x7)));
  assign new_n206_ = x0 ? (x1 & x4) : (~x1 & ~x4 & x5 & x6);
  assign z61 = ~new_n208_ | (~new_n75_ & ~x4);
  assign new_n208_ = x0 & ~x1 & x2 & x3 & x7;
  assign z62 = (~new_n75_ & ~x4) | ~x0 | ~x1 | new_n74_ | x3;
  assign z27 = 1'b0;
endmodule


