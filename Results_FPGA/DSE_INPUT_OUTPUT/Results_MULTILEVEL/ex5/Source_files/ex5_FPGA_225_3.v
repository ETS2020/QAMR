// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n78_, new_n81_, new_n83_, new_n86_, new_n88_, new_n90_,
    new_n93_, new_n98_, new_n106_, new_n110_, new_n112_, new_n114_,
    new_n116_, new_n117_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n143_, new_n145_,
    new_n147_, new_n149_, new_n152_, new_n154_, new_n157_, new_n159_,
    new_n160_, new_n163_, new_n165_, new_n166_, new_n168_;
  assign z00 = z26 | (new_n75_ & ~x4);
  assign z26 = x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z26 | (~x5 & ~x6 & ~x7);
  assign z02 = z26 | (new_n78_ & x5 & ~x6 & ~x7);
  assign new_n78_ = ~x3 & ~x4;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z26 & ~x7;
  assign z04 = z26 | (x3 & ~x4 & new_n81_ & ~x5);
  assign new_n81_ = x6 & ~x7;
  assign z05 = z26 | (new_n81_ & new_n83_);
  assign new_n83_ = ~x4 & x5;
  assign z06 = x2 & (x0 | (new_n75_ & ~x4 & ~x1 & x3));
  assign z07 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z09 = x2 & (x0 | (new_n78_ & ~x1 & new_n88_ & ~x5));
  assign new_n88_ = x6 & x7;
  assign z10 = x2 & (x0 | (new_n90_ & x1 & ~x4));
  assign new_n90_ = new_n88_ & x5;
  assign z11 = x0 & (x2 | (new_n90_ & new_n78_ & x1));
  assign z13 = (x0 & x2) | (new_n93_ & ~x0 & x1 & ~x2 & x3);
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z14 = x0 & (x2 | (new_n90_ & ~x1 & x3 & ~x4));
  assign z15 = x2 & (x0 | (new_n90_ & x1 & x3 & ~x4));
  assign z16 = x0 & (x2 | (new_n90_ & x1 & x3 & ~x4));
  assign z17 = ~x5 & new_n98_ & x4;
  assign new_n98_ = ~x2 & x0 & ~x1;
  assign z18 = x2 & (x0 | (x4 & ~x5 & ~x1 & x3));
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n98_ & ~x3;
  assign z21 = x0 & (x2 | (new_n75_ & ~x4 & ~x1 & x3));
  assign z22 = x7 & x6 & ~x5 & new_n98_ & ~x4;
  assign z23 = (x0 & x2) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n86_ & ~x5;
  assign z27 = x2 & (x0 | (new_n81_ & ~x5 & new_n78_ & x1));
  assign z29 = (x0 & x2) | (new_n110_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n110_ = ~x4 & ~x5 & ~x6 & x7;
  assign z31 = (x0 & (x2 | (~x4 & x6))) | (~x0 & (~x4 | (~x2 & x3))) | ~new_n112_ | (x2 & (~x3 | ~x4));
  assign new_n112_ = ~x1 & (~x4 | x5) & (x4 | ~x5);
  assign z32 = (~x0 & ((~x2 & (x3 | x4)) | (~x4 & (~new_n81_ | x3)))) | ~new_n114_ | (~x3 & (x2 | (x0 & ~x4)));
  assign new_n114_ = (~x0 | (~x2 & (x4 | ~x6))) & (x4 | (~x2 & ~x5)) & ~x1 & (x2 | ~x4 | x5);
  assign z34 = (x0 & (x2 | (~x4 & ~x5 & ~x7))) | ~new_n116_ | ((~x0 | x5) & (x4 | x7));
  assign new_n116_ = x5 ? (x3 & ~x6) : new_n117_;
  assign new_n117_ = (x6 | (x0 & x4)) & ~x1 & (x0 | (x2 & ~x3));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = (~new_n120_ & (~x0 | ~x2)) | (~x2 & (~x0 | ~x4)) | (~x0 & (~new_n81_ | x3 | x4));
  assign new_n120_ = ~x1 & ~x5;
  assign z37 = (~x3 & (~x0 | (~x1 & ~x2))) | (~new_n122_ & (x2 ? ~x0 : (x3 & ~x5))) | (x5 & (~x0 | (x1 & ~x2 & x3)));
  assign new_n122_ = new_n81_ & ~x4;
  assign z38 = ~new_n124_ | (~x3 & (x2 | (x0 & ~x4)));
  assign new_n124_ = (~x2 | (~x0 & x4)) & (~x0 | new_n75_ | x4) & ~x1 & (x0 | new_n125_ | x2);
  assign new_n125_ = ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z39 = (~new_n128_ & (~x0 | (~x2 & x5))) | (~x0 & (x4 | ~x5)) | (~x2 & (x4 | (~new_n127_ & ~x5)));
  assign new_n127_ = new_n88_ & ~x1;
  assign new_n128_ = x3 & ~x6 & ~x7;
  assign z40 = ~new_n130_ | (x0 & (x2 | (~x4 & x6) | (x4 & ~x5)));
  assign new_n130_ = (~x2 | (x3 & x4)) & ~new_n131_ & ~x1 & (x4 | ~x5);
  assign new_n131_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign z41 = ~x0 | (~x2 & (x1 ? x3 : (~x3 | ~x5)));
  assign z42 = (~new_n134_ & ~x2) | (~x0 & (new_n135_ | x4));
  assign new_n134_ = (~x5 | (~x6 & ~x7) | (~x0 & x4)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign new_n135_ = ~x4 & ((x2 & (~x5 | x6)) | (x5 & x6) | (~x5 & ~x6) | x7);
  assign z43 = new_n138_ | (~x2 & (new_n137_ | new_n139_));
  assign new_n137_ = ~new_n83_ & (x1 | (~x0 & x3));
  assign new_n138_ = ~x0 & (new_n135_ | (x4 & (x1 | (x2 & ~x3))));
  assign new_n139_ = ~x4 & ((x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign z44 = (~new_n141_ & ~x2) | (~x0 & (x2 | ~x4));
  assign new_n141_ = (~x3 | (~x0 & ~x4)) & (~x5 | (~x0 & x4)) & ~x1 & (~x0 | (~x4 & ~x6));
  assign z45 = (~new_n143_ & ~x0) | (~x2 & (x0 | x1 | x4 | (~x4 & x5)));
  assign new_n143_ = (~x2 | (x1 & (x4 | ~x6))) & (x4 | (~x5 & (x2 | (x6 & x7))));
  assign z46 = x0 ? ~x2 : (new_n145_ | ~x1 | x2 | x3);
  assign new_n145_ = ~x4 & (new_n81_ | x5);
  assign z47 = (x1 & (~x2 | (~x4 & x6))) | ~new_n147_ | (x5 & (~x1 | ~x4));
  assign new_n147_ = ~x0 & (x1 | (new_n88_ & ~x2 & ~x4));
  assign z48 = (~x0 & (x1 | x2)) | (~x2 & (new_n149_ | x0 | x1 | ~x3));
  assign new_n149_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~x2 | (~x0 & (x1 | (x2 & (x4 ? x3 : ~new_n75_))));
  assign z50 = x2 ? ~x0 : (~new_n152_ | (x0 & (~x1 | ~x3)));
  assign new_n152_ = new_n88_ & ~x4 & ~x5;
  assign z51 = (x2 & (x0 | x4)) | (x0 & (~x1 | x3)) | (~new_n154_ & ~x4) | (~x0 & (x1 | ~x3));
  assign new_n154_ = (x5 | ~x6) & (~x5 | (x6 & x7)) & (x0 | (~x5 & ~x6));
  assign z52 = (~x0 & (x1 | (~x2 & ~x3))) | (x2 & (x0 | (x3 & x4))) | (~new_n75_ & ~x4) | (x0 & (~x1 | x3));
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | (~new_n93_ & (x2 ? ~x1 : ~x3)) | (~x1 & (~x2 ^ ~x3)) | new_n157_ | (x1 & x2 & x3);
  assign new_n157_ = ~x4 & ~new_n75_ & (x2 ? x1 : x3);
  assign z54 = ~new_n160_ | (~x2 & (x3 ? ~new_n93_ : ~new_n159_));
  assign new_n159_ = x1 & (x0 | new_n75_ | x4);
  assign new_n160_ = (x1 | (~x0 & (~x2 | ~x3))) & (new_n93_ | (~x0 & (~x2 | x3))) & (~x0 | (~x2 & ~x3)) & (~new_n149_ | ~x2);
  assign z55 = ((~x0 | ~x2) & (~x1 | (~new_n75_ & ~x4))) | (x0 & ~x2 & ~x3);
  assign z56 = (~x0 & (~new_n163_ | (~x1 & (~x2 | x3)))) | (~x2 & (x0 | ~x3));
  assign new_n163_ = (x4 | ((~x6 | x7) & (x2 | ~x5))) & (~x2 | (~x4 & x5 & x6 & x7));
  assign z57 = (~x3 & (x0 | ~x1)) | ~new_n165_ | ((new_n83_ | ~x1) & (x0 | (~x0 & ~x2)));
  assign new_n165_ = (x7 | ((x0 | ~x2) & (x4 | ~x6))) & (new_n166_ | x0) & (~x0 | ~x2);
  assign new_n166_ = x2 ? (~x4 & x5 & x6) : ~x3;
  assign z58 = (x1 & (~x2 | (~x4 & x6))) | ~new_n168_ | (x5 & (~x1 | ~x4));
  assign new_n168_ = ~x0 & x3 & (x1 | (new_n88_ & ~x2 & ~x4));
  assign z59 = (x0 & (~x1 | ~x3)) | ~new_n152_ | (x2 & (x1 | x3));
  assign z60 = (~x2 & (x3 | (x0 & (~x1 | ~x4)))) | (~x0 & (~new_n90_ | x1 | x4 | (x2 & ~x3)));
  assign z62 = (~new_n75_ & ~x4) | x2 | x3 | ~x0 | ~x1;
  assign z08 = 1'b0;
  assign z12 = 1'b0;
  assign z28 = 1'b0;
  assign z33 = 1'b1;
  assign z61 = 1'b1;
  assign z30 = z26;
endmodule


