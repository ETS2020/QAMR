// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n78_, new_n81_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n97_, new_n105_, new_n107_, new_n109_, new_n111_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n128_, new_n130_, new_n132_, new_n134_,
    new_n136_, new_n137_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n149_, new_n150_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n161_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n173_, new_n176_, new_n177_;
  assign z00 = z06 | (new_n75_ & ~x4);
  assign z06 = x2 & x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = (x2 & x3) | (~x3 & ~x4 & new_n78_ & x5);
  assign new_n78_ = ~x6 & ~x7;
  assign z03 = x3 & (x2 | (new_n78_ & ~x4 & x5));
  assign z04 = x3 & (new_n81_ | x2);
  assign new_n81_ = ~x4 & ~x5 & x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (x2 & x3) | (new_n84_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n88_ & x6;
  assign new_n88_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x2 & (x3 | (new_n90_ & ~x0 & x1 & ~x4));
  assign new_n90_ = x5 & x6 & x7;
  assign z11 = (x2 & x3) | (new_n84_ & x0 & x1 & ~x2 & ~x3);
  assign z12 = x2 & (x3 | (new_n90_ & x0 & ~x1 & ~x4));
  assign z13 = x3 & (x2 | (new_n90_ & ~x0 & x1 & ~x4));
  assign z14 = x3 & (x2 | (new_n90_ & x0 & ~x1 & ~x4));
  assign z16 = x3 & (x2 | (new_n90_ & x0 & x1 & ~x4));
  assign z17 = (x2 & x3) | (new_n97_ & ~x2 & x4 & ~x5);
  assign new_n97_ = x0 & ~x1;
  assign z19 = (x2 & x3) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = (x2 & x3) | (x0 & ~x1 & ~x2 & new_n75_ & ~x3 & ~x4);
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n97_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n97_ & ~x2;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = (x2 & x3) | (new_n81_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign z25 = ~x7 & new_n105_ & x6;
  assign new_n105_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = x2 & (x3 | (new_n107_ & x0 & ~x4));
  assign new_n107_ = ~x5 & x6 & x7;
  assign z27 = ~x7 & new_n109_ & x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = x7 & new_n111_ & ~x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & x6 & new_n86_ & ~x5;
  assign z31 = x2 ? ~x3 : ~new_n114_;
  assign new_n114_ = (x0 | (~x3 & x4)) & (x4 | (~x5 & ~x6)) & ~x1 & (~x4 | x5);
  assign z32 = new_n116_ | (x2 & ~x3) | (~x2 & (new_n117_ | ~new_n118_));
  assign new_n116_ = ~x4 & ((x0 & (~x3 | (~x2 & x6))) | (~x2 & x5));
  assign new_n117_ = x4 & (~x0 | ~x5);
  assign new_n118_ = ~x1 & (x0 | (~x3 & ~x5 & x6 & ~x7));
  assign z33 = ~new_n120_ | ~x7 | x4 | ~x6;
  assign new_n120_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = new_n122_ | new_n123_ | new_n124_ | new_n125_;
  assign new_n122_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n123_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n124_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n125_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = x2 | ~x4 | x1 | (~x0 & x3) | (x0 & ~x5);
  assign z36 = ~new_n128_ | (~z06 & (x1 | x5));
  assign new_n128_ = x2 ? (x3 | (~x0 & ~x4 & x6 & ~x7)) : (x0 & x4);
  assign z37 = (~x0 & (~x3 | (~x2 & x5))) | (~x3 & (~x1 | x2)) | (~x2 & x3 & (x5 ? x1 : ~new_n130_));
  assign new_n130_ = ~x4 & x6 & ~x7;
  assign z38 = new_n116_ | (x2 & ~x3) | (~x2 & (new_n132_ | x1));
  assign new_n132_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~new_n134_ & ~x2) | (~x3 & (x2 | x5));
  assign new_n134_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = (~new_n136_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n137_ | (x3 & (~x0 | x2));
  assign new_n136_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n137_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x2 & (x3 | ~x5)) | (~new_n78_ & x5) | x4 | (~new_n140_ & ~x5);
  assign new_n140_ = x6 & x7 & x0 & (~x1 | x2);
  assign z43 = (x1 & (new_n142_ | x4)) | (~new_n143_ & ~x4) | ~new_n145_ | (~new_n144_ & x4);
  assign new_n142_ = ~x2 & ~x5;
  assign new_n143_ = (x0 | ((x5 | x6) & (x2 | ~x7))) & (~x5 | (~x6 & ~x7)) & (~x6 | x7 | ~x0 | x2);
  assign new_n144_ = ~x2 & (x0 | ~x3);
  assign new_n145_ = (x5 | ((x0 | (~x2 & ~x3)) & (~x2 | (x6 & x7)))) & (~x2 | (~x3 & (~x5 | (~x6 & ~x7))));
  assign z44 = (~new_n147_ & ~x2) | (~x3 & (x2 | (~x4 & x5)));
  assign new_n147_ = (~x3 | (~x0 & ~x4)) & (x4 | (x0 & ~x5)) & ~x1 & (~x0 | (~x4 & ~x5 & ~x6));
  assign z45 = (x0 & (~x2 | (x2 & ~x3))) | (~new_n149_ & ~x2) | (~new_n150_ & ~x3);
  assign new_n149_ = ~x1 & ~x4 & (x4 | (~x5 & (x0 | (x6 & x7))));
  assign new_n150_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n152_ | ~x1 | x3));
  assign new_n152_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | (~new_n155_ & ~x2) | (x2 & ~x3 & (new_n154_ | ~x1));
  assign new_n154_ = ~x4 & (x5 | x6);
  assign new_n155_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~new_n157_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n157_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (~x3 & (new_n154_ | x0 | x1));
  assign z50 = (x0 & (~x1 | ~x3)) | ~new_n107_ | x2 | x4;
  assign z51 = (~x2 & (~new_n161_ | (x0 & (~x1 | x3)))) | (~x3 & ((new_n154_ & x2) | ~x0 | ~x1));
  assign new_n161_ = (x0 | ~x1) & (x4 | ((x5 | ~x6) & (~x5 | (x6 & x7)) & (x0 | (~x5 & ~x6))));
  assign z52 = (~x3 & (new_n154_ | (~x1 & ~x2) | (~x0 & (x1 | ~x2)))) | (~x2 & (new_n154_ | (x3 & (x0 | x1))));
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n154_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n84_ & ~x3);
  assign z54 = (~x3 & ((~new_n165_ & ~x2) | new_n97_ | new_n166_)) | (~x2 & ~new_n167_ & x3);
  assign new_n165_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n166_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n167_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n169_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n170_ & ~x3);
  assign new_n169_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n170_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n84_ & ~x3) | (~x2 & (new_n152_ | ~x1 | ~x3));
  assign z57 = new_n173_ | (x0 & (x2 | ~x3)) | (~new_n84_ & x2) | ~x1 | (~x0 & x3);
  assign new_n173_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~x3 | (~x2 & (~new_n107_ | x0 | x1 | x4));
  assign z59 = (~new_n176_ & (~x0 | ~x2)) | new_n177_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n176_ = ~x4 & ~x5 & x6 & x7;
  assign new_n177_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n90_ | x1 | x2 | x4));
  assign z62 = (x0 & (new_n154_ | ~x1)) | ~x0 | x3;
  assign z61 = 1'b1;
  assign z15 = z06;
  assign z18 = z06;
  assign z28 = z06;
endmodule


