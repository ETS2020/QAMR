// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n77_, new_n82_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n96_, new_n97_, new_n106_, new_n108_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n122_,
    new_n123_, new_n127_, new_n129_, new_n130_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n142_, new_n144_,
    new_n146_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n158_, new_n160_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n169_;
  assign z00 = ~x4 & ~x5 & ~z07 & ~x6;
  assign z07 = ~x2 & ~x3;
  assign z01 = z07 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & (~x2 | (new_n77_ & ~x6 & ~x7));
  assign new_n77_ = ~x4 & x5;
  assign z03 = (~x2 & ~x3) | (x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z04 = (~x2 & ~x3) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z07 | (new_n77_ & x6 & ~x7);
  assign z06 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & new_n82_ & x3 & ~x4);
  assign new_n82_ = ~x5 & ~x6;
  assign z08 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = ~x3 & (new_n86_ | ~x2);
  assign new_n86_ = ~x0 & ~x1 & ~x4 & new_n87_ & ~x5;
  assign new_n87_ = x6 & x7;
  assign z10 = x7 & x6 & x5 & new_n89_ & ~x4;
  assign new_n89_ = x2 & ~x0 & x1;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n91_ & ~x3;
  assign new_n91_ = new_n92_ & x2;
  assign new_n92_ = x0 & ~x1;
  assign z13 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x2 & (~x3 | (new_n96_ & new_n97_));
  assign new_n96_ = x0 & ~x1 & ~x4;
  assign new_n97_ = x5 & x6 & x7;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z16 = ~x2 & (~x3 | (new_n97_ & x0 & x1 & ~x4));
  assign z17 = ~x2 & (~x3 | (new_n92_ & x4 & ~x5));
  assign z18 = ~x5 & x4 & x3 & x2 & ~x0 & ~x1;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n92_ & ~x2;
  assign z22 = ~x2 & (~x3 | (new_n96_ & new_n87_ & ~x5));
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n106_ & ~x3;
  assign new_n106_ = x0 & x2;
  assign z27 = ~x3 & (new_n108_ | ~x2);
  assign new_n108_ = ~x5 & x6 & ~x7 & ~x0 & x1 & ~x4;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n91_ & x3;
  assign z30 = x7 & x6 & new_n84_ & ~x5;
  assign z31 = ~new_n112_ | (~z07 & x1);
  assign new_n112_ = (~x3 | ((x2 | (x0 & (~x4 | x5))) & (x4 | (~x5 & ~x6)))) & x3 & (~x2 | (~x0 & x4 & x5));
  assign z32 = ~new_n114_ | (~z07 & x1);
  assign new_n114_ = (~x3 | ((x2 | (x0 & (~x4 | x5))) & (x4 | (~x5 & ~x6)))) & (~x2 | (~x0 & x3 & x4));
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = new_n106_ & (~x1 | ~x3 | x5) & (x1 | ~x5);
  assign z34 = (x1 & (~x3 | ~x5)) | ~new_n118_ | ((~x3 | x5) & (x4 | x7));
  assign new_n118_ = (x5 | (~new_n119_ & (~x3 | (x0 & ~x2)))) & (~x5 | (x3 & ~x6)) & (x3 | (~x0 & x2 & x6));
  assign new_n119_ = ~x4 & (~x6 | (x3 & ~x7));
  assign z35 = (~x0 & (~x2 | (x3 & ~x5))) | ((x0 | x3) & ~x4) | x1 | ~x3 | (x0 & (x2 | ~x5));
  assign z36 = (~x0 & (~x2 | x3)) | (~x4 & (x0 | x3)) | (x5 & (x0 | ~x3)) | ~new_n122_ | (x0 & (x2 | ~x3));
  assign new_n122_ = ~x1 & (new_n123_ | x3);
  assign new_n123_ = ~x4 & x6 & ~x7;
  assign z37 = (x5 & (~x0 | x1 | x2)) | ~x3 | (~new_n123_ & ~x5);
  assign z38 = (~x0 & (~x2 | ~x4)) | (~new_n82_ & ~x4) | x1 | ~x3 | (x0 & x2);
  assign z39 = (~new_n127_ & x3) | (x2 & (~x3 | ~x5));
  assign new_n127_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (new_n92_ & x6 & x7));
  assign z40 = ~new_n129_ | (x1 & (x2 ? ~x0 : x3));
  assign new_n129_ = (~x4 | (x2 ? ~x0 : (~x3 | x5))) & new_n130_ & (~x5 | (x2 ? ~x0 : (~x3 | x4)));
  assign new_n130_ = (~x3 | (x2 ? ~x0 : (x0 & (x4 | ~x6)))) & (~x2 | (x0 ? (x6 & x7) : (x3 & x4)));
  assign z41 = x2 | (x3 & (~x0 | x1 | ~x5));
  assign z42 = ~new_n127_ | (~x3 & (~x2 | ~x5));
  assign z43 = (~new_n137_ & ~x7) | (~new_n135_ & x2) | (~new_n134_ & x3);
  assign new_n134_ = (new_n77_ | (x0 ? ~x1 : x2)) & ((~x6 & ~x7) | (x0 ? ~new_n77_ : x2));
  assign new_n135_ = ((~x6 & ~x7) | (x0 ? ~x5 : x4)) & (~x0 | (~x4 & (x5 | x6))) & (x0 | x4 | x5) & (new_n136_ | ~x4);
  assign new_n136_ = ~x1 & x3;
  assign new_n137_ = (~x0 | ~x2 | x5) & (x2 | ~x3 | x4 | ~x6);
  assign z45 = (x0 & (x2 | x3)) | (x2 & (new_n140_ | ~x1)) | (~x2 & ~new_n139_ & x3);
  assign new_n139_ = ~x1 & ~x4 & new_n87_ & ~x5;
  assign new_n140_ = ~x4 & (x5 | x6);
  assign z47 = x2 ? ~new_n142_ : (~new_n86_ & x3);
  assign new_n142_ = (x0 | new_n82_ | x4) & x1 & (~x0 | (new_n97_ & x3 & ~x4));
  assign z48 = x2 | (x3 & ((~new_n144_ & ~x2) | (~x4 & ~x5 & x6)));
  assign new_n144_ = ~x0 & ~x1 & (x4 | new_n87_ | ~x5);
  assign z49 = (x0 & (~x1 | x2)) | (x2 & (x1 | (~x4 & x6))) | ~new_n146_ | ~x2;
  assign new_n146_ = x4 ? ~x3 : ~x5;
  assign z50 = (x4 & (~x2 | x3)) | (x5 & (~x2 | ~x4)) | new_n92_ | x2 | (~x2 & (~new_n87_ | ~x3));
  assign z51 = (x0 & (x2 ? ~x1 : x3)) | (~x0 & ((x1 & (x2 | x3)) | (x2 & (~x3 | x4)) | (x3 & ~new_n82_ & ~x4))) | (x2 & ~new_n82_ & ~x4);
  assign z52 = (~x0 & ((x1 & (x2 | x3)) | (x3 & (x4 ? x2 : ~new_n82_)))) | (~x4 & ~new_n82_ & (x0 | x2)) | (~x2 & ~x3) | (x0 & (~x2 | x3));
  assign z53 = (~new_n153_ & (x3 ? (~x2 | ~x5) : x2)) | (~new_n152_ & x3) | (~new_n151_ & x2);
  assign new_n151_ = (~x0 | (x1 & x3)) & (~new_n77_ | x3) & (x0 | ~x1 | ~x3);
  assign new_n152_ = (new_n87_ | (x1 & (x4 | ~x5))) & (x2 | x4 | ~x5) & (x1 | (~x0 & ~x4));
  assign new_n153_ = x1 & (x4 | ~x6);
  assign z54 = (x0 & (x3 | (~x1 & x2))) | (~new_n155_ & (~x2 ^ ~x3)) | (x3 & (new_n156_ | (~x1 & x2)));
  assign new_n155_ = new_n77_ & new_n87_;
  assign new_n156_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z55 = new_n158_ | (~x1 & (x2 | x3)) | (x0 & ~new_n155_ & x2);
  assign new_n158_ = ~x4 & ~new_n82_ & (x2 ? ~x0 : x3);
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n160_ | (~x2 & (new_n77_ | ~x3));
  assign new_n160_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (~x1 & (~x2 ^ ~x3)) | (~new_n162_ & x2) | (~x2 & ~new_n163_ & x3);
  assign new_n162_ = new_n97_ & ~x0 & ~x4;
  assign new_n163_ = x0 & (x4 | (~x5 & (~x6 | x7)));
  assign z58 = x2 ? ~new_n165_ : (~new_n86_ & x3);
  assign new_n165_ = (~new_n140_ | x0) & (~x0 | (new_n77_ & new_n87_)) & x1 & x3;
  assign z59 = (~new_n167_ & x2) | (x3 & ((~new_n169_ & ~x2) | (x5 & (~x2 | ~x4))));
  assign new_n167_ = x3 ? (x0 & ~x1 & (x4 | ~x6)) : new_n168_;
  assign new_n168_ = (~x0 | (x1 & (x4 | ~x6))) & (~x5 | (x0 & x4)) & (x0 | (~x1 & ~x4 & x6 & x7));
  assign new_n169_ = new_n87_ & ~new_n92_ & ~x4;
  assign z60 = (x0 & (x3 | (~x1 & x2 & ~x3))) | (x2 & ~x3 & (~x0 | ~x4)) | (x3 & (~new_n97_ | x1 | ~x2 | x4));
  assign z61 = new_n140_ | ~new_n92_ | ~x2 | ~x3;
  assign z62 = x3 | (x2 & (new_n140_ | ~x0 | ~x1));
  assign z19 = 1'b0;
  assign z20 = 1'b0;
  assign z24 = 1'b0;
  assign z44 = ~z07;
  assign z11 = z07;
  assign z25 = z07;
  assign z29 = z07;
  assign z46 = z44;
endmodule


