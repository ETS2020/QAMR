// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n78_, new_n80_, new_n83_, new_n85_, new_n87_, new_n92_, new_n94_,
    new_n97_, new_n99_, new_n107_, new_n109_, new_n111_, new_n113_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n127_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n136_, new_n137_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n147_, new_n148_, new_n150_, new_n152_, new_n153_, new_n155_,
    new_n157_, new_n159_, new_n160_, new_n162_, new_n165_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n174_, new_n176_, new_n178_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = x3 & (x2 | (new_n78_ & ~x6 & ~x7));
  assign new_n78_ = ~x4 & x5;
  assign z04 = x3 & (x2 | (new_n80_ & ~x4 & ~x5));
  assign new_n80_ = x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (x2 & x3) | (new_n83_ & ~x2 & ~x3 & ~x0 & x1);
  assign new_n83_ = ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (x3 | (new_n85_ & x0 & x1 & ~x4));
  assign new_n85_ = x5 & x6 & x7;
  assign z09 = x2 & (x3 | (new_n87_ & ~x0 & ~x1 & ~x4));
  assign new_n87_ = ~x5 & x6 & x7;
  assign z10 = x2 & (x3 | (new_n85_ & ~x0 & x1 & ~x4));
  assign z11 = (x2 & x3) | (new_n83_ & ~x2 & ~x3 & x0 & x1);
  assign z12 = x2 & (x3 | (new_n85_ & x0 & ~x1 & ~x4));
  assign z13 = x7 & x6 & x5 & ~x4 & new_n92_ & x3;
  assign new_n92_ = ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x3 & (x2 | (new_n85_ & x0 & x1 & ~x4));
  assign z17 = (x2 & x3) | (new_n97_ & ~x2 & x4 & ~x5);
  assign new_n97_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & new_n99_ & ~x2;
  assign new_n99_ = ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n97_ & ~x2;
  assign z21 = x3 & (x2 | (new_n97_ & ~x4 & ~x5 & ~x6));
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n97_ & ~x2;
  assign z23 = x5 & x3 & new_n99_ & ~x2;
  assign z24 = (x2 & x3) | (new_n80_ & ~x4 & ~x5 & new_n99_ & ~x2 & ~x3);
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n92_ & ~x3;
  assign z26 = ~new_n107_ & x2;
  assign new_n107_ = ~x3 & (x5 | ~x6 | ~x7 | ~x0 | x4);
  assign z27 = ~x7 & new_n109_ & x6;
  assign new_n109_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = (x2 & x3) | (new_n111_ & new_n99_ & ~x2 & ~x3);
  assign new_n111_ = ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = x2 ? ~x3 : ~new_n115_;
  assign new_n115_ = (x0 | (~x3 & x4)) & (x4 | (~x5 & (~x0 | ~x6))) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n118_ | (~new_n117_ & ~x4);
  assign new_n117_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n118_ = (x0 | (~x4 & (x2 | ~x3))) & ~x2 & (x2 | (~x1 & (~x4 | x5)));
  assign z33 = ~x2 | (~new_n120_ & ~x3);
  assign new_n120_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n123_ & ~x5) | ~new_n124_ | (~new_n122_ & (x2 | x5));
  assign new_n122_ = ~x4 & ~x7;
  assign new_n123_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n124_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = (x1 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & ((~x0 & x3) | ~x4 | (x0 & ~x5)));
  assign z36 = (x1 & (~x2 | ~x3)) | ~new_n127_ | (x5 & (~x2 | (x2 & ~x3)));
  assign new_n127_ = x2 ? (x3 | (~x0 & ~x4 & x6 & ~x7)) : (x0 & x4);
  assign z37 = (~x0 & (~x3 | (~x2 & x5))) | (~x3 & (~x1 | x2)) | (~x2 & x3 & (x5 ? x1 : ~new_n129_));
  assign new_n129_ = ~x4 & x6 & ~x7;
  assign z38 = new_n131_ | new_n132_ | x1 | x2;
  assign new_n131_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n132_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~new_n134_ & ~x2) | (~x3 & (x2 | x5));
  assign new_n134_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = x2 ? new_n107_ : (new_n136_ | new_n137_ | new_n78_ | x1);
  assign new_n136_ = x0 & (x4 ? ~x5 : x6);
  assign new_n137_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n134_ | (x2 & (x3 | ~x5));
  assign z43 = x2 ? new_n143_ : (new_n141_ | new_n142_);
  assign new_n141_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n142_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n143_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = x2 ? ~x3 : (new_n141_ | ~new_n145_);
  assign new_n145_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x0 & (~x2 | ~x3)) | (x2 & ~x3 & (new_n147_ | ~x1)) | (~new_n148_ & ~x2);
  assign new_n147_ = ~x4 & (x5 | x6);
  assign new_n148_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n150_ | ~x1 | x3));
  assign new_n150_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = new_n152_ | x0 | (~new_n153_ & ~x1) | (x1 & (~x2 | x3));
  assign new_n152_ = ~x4 & (x5 | (x1 & x6));
  assign new_n153_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~x3 | (~new_n155_ & ~x2);
  assign new_n155_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~new_n157_ | (~x4 & (x5 | (x2 & x6)));
  assign new_n157_ = (~x0 | (x1 & ~x2 & x3)) & x2 & (~x2 | (~x1 & ~x3));
  assign z50 = (~new_n160_ & x0) | ((~x2 | ~x4) & x5) | x2 | (~x2 & (~new_n159_ | x4));
  assign new_n159_ = x6 & x7;
  assign new_n160_ = x1 & x3;
  assign z51 = (~x0 & (x1 | ~x3)) | (x2 & (new_n78_ | x3)) | new_n162_ | (~x1 & ~x3) | (x3 & (x0 | x1));
  assign new_n162_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (x3 & (x5 | x6)));
  assign z52 = (x1 & (~x0 | x3)) | (x3 & (x0 | x2)) | new_n147_ | (~x1 & ~x2 & ~x3);
  assign z53 = (~x3 & (~new_n165_ | (x0 & (x1 | x2)))) | (~x2 & x3 & (new_n147_ | ~x1));
  assign new_n165_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (x6 & x7 & ~x4 & x5);
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n147_ & ~x0))) | (~new_n83_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n168_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n169_ & ~x3);
  assign new_n168_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n169_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n171_ | (~new_n160_ & ~x2) | x0 | (~new_n172_ & x2);
  assign new_n171_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n172_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = new_n174_ | (x0 & (x2 | ~x3)) | (~new_n83_ & x2) | ~x1 | (~x0 & x3);
  assign new_n174_ = ~x4 & ((x6 & ~x7) | (x5 & (x0 | ~x2)));
  assign z58 = ~new_n176_ | ~new_n99_ | x2 | ~x3;
  assign new_n176_ = ~x4 & ~x5 & x6 & x7;
  assign z59 = (~new_n176_ & (~x0 | ~x2)) | new_n178_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n178_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n85_ | x1 | x2 | x4));
  assign z62 = new_n147_ | ~x0 | ~x1 | x3;
  assign z15 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = ~z06;
  assign z18 = z06;
endmodule


