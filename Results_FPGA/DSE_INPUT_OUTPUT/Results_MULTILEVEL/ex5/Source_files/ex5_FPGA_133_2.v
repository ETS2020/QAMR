// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n79_, new_n82_, new_n84_, new_n85_, new_n87_, new_n90_, new_n92_,
    new_n93_, new_n96_, new_n105_, new_n107_, new_n110_, new_n112_,
    new_n114_, new_n115_, new_n117_, new_n119_, new_n123_, new_n125_,
    new_n127_, new_n129_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n144_, new_n146_,
    new_n147_, new_n149_, new_n151_, new_n154_, new_n156_, new_n157_,
    new_n159_, new_n161_, new_n163_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n172_;
  assign z00 = ~x4 & ~x5 & ~z08 & ~x6;
  assign z08 = x2 & ~x3;
  assign z01 = ~x5 & ~x6 & ~z08 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (x2 & ~x3) | (x3 & ~x4 & new_n79_ & ~x5);
  assign new_n79_ = x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z08 & ~x7;
  assign z06 = new_n82_ & ~x6;
  assign new_n82_ = ~x5 & ~x4 & x3 & x2 & ~x0 & ~x1;
  assign z07 = ~x3 & (x2 | (new_n84_ & ~x0 & x1 & ~x4));
  assign new_n84_ = new_n85_ & x5;
  assign new_n85_ = x6 & x7;
  assign z10 = x7 & new_n87_ & x6;
  assign new_n87_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z11 = ~x3 & (x2 | (new_n84_ & x0 & x1 & ~x4));
  assign z13 = (x2 & ~x3) | (new_n90_ & ~x0 & x1 & ~x2 & x3);
  assign new_n90_ = ~x4 & x5 & x6 & x7;
  assign z14 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & x3 & new_n93_ & ~x2;
  assign new_n93_ = x0 & ~x1;
  assign z15 = x2 & (~x3 | (new_n84_ & ~x0 & x1 & ~x4));
  assign z16 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = (x2 & ~x3) | (new_n93_ & ~x2 & x4 & ~x5);
  assign z18 = x2 & (~x3 | (~x0 & ~x1 & x4 & ~x5));
  assign z19 = ~x3 & (x2 | (~x0 & ~x1 & x4));
  assign z20 = ~x3 & (x2 | (new_n93_ & ~x4 & ~x5 & ~x6));
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n93_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n93_ & ~x2;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x3 & (x2 | (new_n105_ & ~x0 & new_n79_ & ~x5));
  assign new_n105_ = ~x1 & ~x4;
  assign z25 = ~x7 & new_n107_ & x6;
  assign new_n107_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z28 = x2 & (~x3 | (new_n85_ & ~x5 & new_n105_ & x0));
  assign z29 = x7 & new_n110_ & ~x6;
  assign new_n110_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z31 = ~new_n112_ | (x0 & (x2 ? x3 : (~x4 & x6)));
  assign new_n112_ = (x2 | ((x0 | (~x3 & x4)) & ~x1 & (~x4 | x5) & (x4 | ~x5))) & (~x3 | (~x1 & (~x2 | (x4 & x5))));
  assign z32 = ~new_n114_ | (~z08 & x1);
  assign new_n114_ = (x3 | (~x2 & (~x0 | x4))) & (~x0 | (x2 ? ~x3 : (x4 | ~x6))) & (~x3 | (x2 ? x4 : x0)) & (new_n115_ | x2);
  assign new_n115_ = x4 ? (x0 & x5) : (~x5 & (x0 | (x6 & ~x7)));
  assign z33 = ~x2 | (~new_n117_ & x3);
  assign new_n117_ = x0 & ~x4 & (x1 | ~x5) & new_n85_ & (~x1 | x5);
  assign z34 = x5 ? (~x3 | x4 | x6 | x7) : (~new_n119_ | (~x4 & (~x6 | ~x7)));
  assign new_n119_ = x0 & ~x1 & ~x2;
  assign z35 = (x0 & (x2 ? x3 : ~x5)) | ((~x2 | x3) & (x1 | ~x4)) | (x3 & (x2 ? ~x5 : ~x0));
  assign z36 = ((~x2 | x3) & (x1 | ~x4)) | (x2 & x3) | (~x2 & (~x0 | x5));
  assign z37 = (~new_n123_ & (~x3 | x5)) | (x3 & (x5 ? x1 : (~new_n79_ | x4))) | (~x1 & ~x3);
  assign new_n123_ = x0 & ~x2;
  assign z38 = (x1 & (~x2 | x3)) | (~new_n125_ & ~x2) | (x3 & (x2 ? (x0 | ~x4) : ~x0));
  assign new_n125_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (x6 & ~x7 & ~x4 & ~x5));
  assign z39 = ~new_n127_ | (~z08 & x4);
  assign new_n127_ = ((~x6 & ~x7) | (x2 ? ~x3 : ~x5)) & (x2 | (x5 ? x3 : (new_n93_ & x6 & x7))) & (~x2 | ~x3 | x5);
  assign z40 = (x1 & (~x2 | x3)) | (~new_n129_ & ~x2) | (x3 & (x2 ? (x0 | ~x4) : ~x0));
  assign new_n129_ = (x4 | (~x5 & (x0 | (x6 & ~x7)))) & (~x0 | (x4 ? x5 : ~x6));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x5 & (x6 | x7)) | z08 | x4 | (~x5 & (~new_n93_ | ~x6 | ~x7));
  assign z43 = ~new_n133_ | (~new_n136_ & x1) | (~x3 & (x2 | (x1 & ~x5)));
  assign new_n133_ = (new_n134_ | ~x0) & (new_n135_ | x4) & (~x3 | ~x4 | x0 | x2);
  assign new_n134_ = (x4 | ~x6 | x7) & (~x2 | (~x4 & (x5 | x6)));
  assign new_n135_ = (~x5 | (~x6 & ~x7)) & (x0 | ((~x3 | (x5 & ~x6)) & ~x7 & (x5 | x6)));
  assign new_n136_ = ~x4 & (~x0 | x5);
  assign z44 = ~new_n138_ | (x2 & (~x3 | (x0 & ~x5 & ~x6)));
  assign new_n138_ = (~x4 | (~x0 & ~x1)) & new_n139_ & (~x1 | x5 | (~x0 & x3));
  assign new_n139_ = (~x0 | (~x5 & ~x6)) & ~x3 & (x4 | (x0 & (~x5 | (~x6 & ~x7))));
  assign z45 = (x0 & (~x2 | x3)) | (~new_n141_ & ~x2) | (x2 & x3 & (new_n142_ | ~x1));
  assign new_n141_ = new_n105_ & new_n85_ & ~x5;
  assign new_n142_ = ~x4 & (x5 | x6);
  assign z46 = x3 | (~x2 & (new_n144_ | x0 | ~x1));
  assign new_n144_ = ~x4 & (new_n79_ | x5);
  assign z47 = ~new_n146_ | (~x0 & ~x4 & (x5 | (x1 & x6)));
  assign new_n146_ = ((~x0 & ~x1) | (x2 & x3)) & (new_n147_ | (~x0 & x1)) & (~x0 | (x1 & x5)) & (x1 | (~x2 & ~x5));
  assign new_n147_ = ~x4 & x6 & x7;
  assign z48 = x2 ? x3 : (new_n149_ | x0 | x1 | ~x3);
  assign new_n149_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~x2 | (x3 & (~new_n151_ | (x2 & (x0 | x1 | x6))));
  assign new_n151_ = ~x4 & ~x5;
  assign z50 = (~x2 & (~new_n85_ | ~new_n151_ | (x0 & (~x1 | ~x3)))) | (x3 & (~new_n151_ | x2));
  assign z51 = (~new_n154_ & x3) | (x2 & ~x3) | (~x2 & (new_n149_ | (~x3 & (~x0 | ~x1))));
  assign new_n154_ = (~x0 | (x1 & x2)) & ~new_n142_ & (x0 | (~x1 & (~x2 | ~x4)));
  assign z52 = (~new_n156_ & x3) | (~x2 & (new_n157_ | (~x3 & (~x0 | ~x1))));
  assign new_n156_ = ~new_n142_ & ~x0 & (x0 | (~x1 & (~x2 | ~x4)));
  assign new_n157_ = ~x4 & (x5 | (~x5 & x6));
  assign z53 = (x0 & (x1 ? (~x2 & ~x3) : x3)) | (~new_n90_ & (x3 ? ~x1 : ~x2)) | (x3 & (new_n159_ | (~x1 & ~x2) | (~x0 & x1 & x2)));
  assign new_n159_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n161_ & ~x2) | (x3 & (new_n149_ | x0 | (~x1 & x2)));
  assign new_n161_ = x3 ? new_n90_ : ((~new_n142_ | x0) & x1 & (new_n90_ | ~x0));
  assign z55 = new_n163_ | (~x3 & (x0 | x2)) | ~x1 | (x0 & ~new_n90_ & x2);
  assign new_n163_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = ((~x2 | x3) & (x0 | ~x1)) | (~x2 & (new_n144_ | ~x3)) | (x2 & ~new_n90_ & x3);
  assign z57 = (~new_n166_ & (x0 | ~x2)) | (x0 & (x2 | ~x3)) | new_n167_ | (~new_n168_ & x2) | (~x0 & ~x2 & x3);
  assign new_n166_ = x1 & (x4 | ~x5);
  assign new_n167_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n168_ = x3 & ~x4 & x5 & x6;
  assign z58 = (~new_n171_ & x3) | (~x2 & (~new_n170_ | x0 | x1 | ~x3));
  assign new_n170_ = new_n85_ & new_n151_;
  assign new_n171_ = (~new_n172_ | x0) & (x1 | ~x2) & (new_n90_ | ~x0);
  assign new_n172_ = ~x4 & (x5 | (x2 & x6));
  assign z59 = (x0 & (~x3 | (~x1 & ~x2))) | new_n172_ | (~new_n170_ & ~x2) | (x2 & (~x0 | x1 | ~x3));
  assign z60 = (x2 & (x0 | ~x3)) | (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n84_ | x1 | x4));
  assign z61 = new_n142_ | ~new_n93_ | ~x2 | ~x3;
  assign z62 = x3 | (~x2 & (new_n142_ | ~x0 | ~x1));
  assign z26 = 1'b0;
  assign z27 = 1'b0;
  assign z30 = 1'b0;
  assign z09 = z08;
  assign z12 = z08;
endmodule


