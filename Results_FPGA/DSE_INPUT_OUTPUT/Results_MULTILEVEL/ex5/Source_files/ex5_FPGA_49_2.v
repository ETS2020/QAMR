// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n80_, new_n83_, new_n85_, new_n87_, new_n89_, new_n92_,
    new_n93_, new_n95_, new_n102_, new_n107_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n118_, new_n120_,
    new_n121_, new_n125_, new_n127_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n144_, new_n145_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n158_, new_n160_, new_n162_, new_n164_,
    new_n165_, new_n167_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n177_, new_n178_, new_n180_, new_n181_, new_n182_;
  assign z00 = z08 | (new_n75_ & ~x4);
  assign z08 = x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z08 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (x2 & ~x3) | (new_n80_ & x3 & ~x4);
  assign new_n80_ = ~x5 & x6 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z08 & ~x7;
  assign z06 = ~x6 & ~x5 & ~x4 & x3 & new_n83_ & x2;
  assign new_n83_ = ~x0 & ~x1;
  assign z07 = x7 & x6 & x5 & ~x4 & new_n85_ & ~x3;
  assign new_n85_ = ~x2 & ~x0 & x1;
  assign z10 = x7 & new_n87_ & x6;
  assign new_n87_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z11 = ~x3 & (new_n89_ | x2);
  assign new_n89_ = x0 & x1 & ~x4 & x5 & x6 & x7;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n85_ & x3;
  assign z14 = (x2 & ~x3) | (new_n93_ & new_n92_ & ~x2 & x3);
  assign new_n92_ = x0 & ~x1;
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & new_n92_ & ~x2;
  assign z18 = x2 & (~x3 | (new_n83_ & x4 & ~x5));
  assign z19 = x4 & ~x3 & new_n83_ & ~x2;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n92_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n92_ & ~x2;
  assign z22 = (x2 & ~x3) | (new_n102_ & x0 & ~x1 & ~x2);
  assign new_n102_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = (x2 & ~x3) | (new_n83_ & ~x2 & x3 & x5);
  assign z24 = ~x3 & (x2 | (new_n80_ & ~x0 & ~x1 & ~x4));
  assign z25 = ~x3 & (x2 | (new_n80_ & ~x0 & x1 & ~x4));
  assign z28 = x2 & (new_n107_ | ~x3);
  assign new_n107_ = x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z29 = ~x3 & (new_n109_ | x2);
  assign new_n109_ = ~x0 & ~x1 & ~x4 & ~x5 & ~x6 & x7;
  assign z31 = new_n111_ | ~new_n112_;
  assign new_n111_ = x0 & (x2 ? x3 : (~x4 & x6));
  assign new_n112_ = (x2 | ((x0 | (~x3 & x4)) & ~x1 & (x4 | ~x5) & (~x4 | x5))) & (~x3 | (~x1 & (~x2 | (x4 & x5))));
  assign z32 = (~z08 & x1) | new_n114_ | new_n111_ | new_n115_ | new_n116_;
  assign new_n114_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n115_ = x3 & (x2 ? ~x4 : ~x0);
  assign new_n116_ = ~x2 & (x4 ? (~x0 | ~x5) : (x5 | (~x0 & (~x6 | x7))));
  assign z33 = ~new_n118_ | ~x3 | x4 | ~x6 | ~x7;
  assign new_n118_ = x0 & x2 & (~x1 | x5) & (x1 | ~x5);
  assign z34 = (~new_n120_ & (x2 ? x3 : x5)) | (~x2 & (x5 ? ~x3 : ~new_n121_)) | (x2 & x3 & ~x5);
  assign new_n120_ = ~x4 & ~x6 & ~x7;
  assign new_n121_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = x2 ? x3 : (~new_n92_ | ~x4 | x5);
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (x3 & (x5 ? x1 : ~new_n125_)) | (~x1 & ~x3);
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (~x2 | x3)) | (~new_n127_ & ~x2) | (x3 & (x2 ? (x0 | ~x4) : ~x0));
  assign new_n127_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x4 & ~x5 & x6 & ~x7));
  assign z39 = (x5 & (~x3 | x6 | x7)) | x4 | (~new_n129_ & ~x5);
  assign new_n129_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = new_n131_ | new_n132_ | ~new_n133_ | (x2 & (~x3 | ~x4));
  assign new_n131_ = x0 & (x2 | (x4 & ~x5) | (~x4 & x6));
  assign new_n132_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign new_n133_ = ~x1 & (x4 | ~x5);
  assign z41 = (~x2 & (~x0 | (~x1 & (~x3 | ~x5)))) | (x3 & (x1 | x2));
  assign z42 = ~z08 & ((x5 & (x6 | x7)) | x4 | (~x5 & (~new_n92_ | ~x6 | ~x7)));
  assign z43 = new_n137_ | new_n138_ | new_n139_ | new_n140_ | new_n141_;
  assign new_n137_ = ~x3 & (x2 | (x1 & ~x5));
  assign new_n138_ = x1 & (x4 | (x0 & ~x5));
  assign new_n139_ = x0 & ((~x4 & x6 & ~x7) | (x2 & (x4 | (~x5 & ~x6))));
  assign new_n140_ = ~x4 & ((x5 & (x6 | x7)) | (~x0 & ((x3 & (~x5 | x6)) | x7 | (~x5 & ~x6))));
  assign new_n141_ = ~x0 & ~x2 & x3 & x4;
  assign z44 = x3 | (~x2 & ((x0 & (~new_n75_ | x4)) | x1 | (~x0 & ~x4)));
  assign z45 = new_n144_ | (x1 & (~x2 | ~x3)) | x0 | (~new_n145_ & ~x1);
  assign new_n144_ = ~x4 & (x5 | (x1 & x6));
  assign new_n145_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = ~new_n147_ | (~x4 & (x5 | (x6 & ~x7)));
  assign new_n147_ = ~x0 & x1 & ~x2 & ~x3;
  assign z47 = new_n149_ | new_n150_ | new_n151_ | ~new_n152_;
  assign new_n149_ = ~x0 & ~x4 & (x5 | (x1 & x6));
  assign new_n150_ = (x0 | x1) & (~x2 | ~x3);
  assign new_n151_ = (x0 | ~x1) & (x4 | ~x6 | ~x7);
  assign new_n152_ = (~x0 | (x1 & x5)) & (x1 | (~x2 & ~x5));
  assign z48 = new_n154_ | ~new_n83_ | x2 | ~x3;
  assign new_n154_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~x2 | (x3 & (~new_n83_ | ~new_n75_ | x4));
  assign z50 = ~new_n145_ | (x0 & (~x1 | ~x3));
  assign z51 = (x0 & ((~x1 & (~x2 | x3)) | (~x2 & (new_n154_ | x3)))) | (~new_n158_ & x3) | (~x0 & ~x2 & ~x3);
  assign new_n158_ = (x4 | (~x5 & ~x6)) & (x0 | (~x1 & (~x2 | ~x4)));
  assign z52 = (~new_n160_ & ~x4) | (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x3 ? (x1 | (x2 & x4)) : ~x2));
  assign new_n160_ = (~x0 | x2 | (~x5 & (x5 | ~x6))) & (~x3 | (~x5 & ~x6));
  assign z53 = (x0 & (x1 ? (~x2 & ~x3) : x3)) | (~new_n93_ & (x3 ? ~x1 : ~x2)) | (x3 & (new_n162_ | (~x1 & ~x2) | (~x0 & x1 & x2)));
  assign new_n162_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~x2 & (x3 ? ~new_n93_ : ~new_n164_)) | (x3 & (new_n154_ | ~new_n165_));
  assign new_n164_ = (x0 | x4 | (~x5 & ~x6)) & x1 & (~x0 | (x6 & x7 & ~x4 & x5));
  assign new_n165_ = ~x0 & (x1 | ~x2);
  assign z55 = new_n167_ | (~x3 & (x0 | x2)) | ~x1 | (x0 & ~new_n93_ & x2);
  assign new_n167_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = (~x4 & ((x6 & ~x7) | (~x2 & x5))) | ~new_n169_ | (x2 & (~x6 | ~x7 | x4 | ~x5));
  assign new_n169_ = ~x0 & x1 & x3;
  assign z57 = (~new_n171_ & (x0 | ~x2)) | new_n172_ | ~new_n173_ | (x0 & (x2 | ~x3));
  assign new_n171_ = x1 & (x4 | ~x5);
  assign new_n172_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n173_ = (x0 | x2 | ~x3) & (~x2 | (x3 & ~x4 & x5 & x6));
  assign z58 = new_n149_ | (~new_n83_ & ~x2) | new_n151_ | ~new_n175_;
  assign new_n175_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (~x2 & (~new_n177_ | (x0 & (~x1 | ~x3)))) | (x3 & (new_n178_ | (x2 & (~x0 | x1))));
  assign new_n177_ = ~x5 & ((~x4 & x6 & x7) | (~x3 & (x0 | x3)));
  assign new_n178_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = new_n180_ | new_n181_ | ~new_n182_;
  assign new_n180_ = x1 & (x3 | (~x3 & x5 & ~x0 & ~x2));
  assign new_n181_ = (x4 | ~x6 | ~x7) & (x3 | (~x0 & ~x2 & ~x3));
  assign new_n182_ = (x5 | (~x3 & (x0 | x2))) & (~x3 | (~x0 & x2)) & (~x0 | x2 | (x1 & (x3 | x4)));
  assign z61 = (~x1 & (~x2 | ~x3)) | ~x0 | x1 | (~new_n75_ & ~x4);
  assign z62 = (x1 & (x2 | x3)) | ~x0 | ~x1 | (~new_n75_ & ~x4);
  assign z27 = 1'b0;
  assign z09 = z08;
  assign z12 = z08;
  assign z15 = x7 & new_n87_ & x6;
  assign z26 = z08;
  assign z30 = z08;
endmodule


