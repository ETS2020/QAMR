// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n80_, new_n82_, new_n84_, new_n86_, new_n89_, new_n91_, new_n94_,
    new_n96_, new_n101_, new_n106_, new_n109_, new_n112_, new_n114_,
    new_n116_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n129_, new_n130_, new_n132_, new_n135_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n168_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n191_, new_n193_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n202_, new_n203_, new_n204_, new_n206_;
  assign z00 = ~x4 & ~x5 & ~z27 & ~x6;
  assign z27 = x2 & ~x7;
  assign z01 = ~x7 & ~x6 & ~x2 & ~x5;
  assign z02 = ~x7 & (x2 | (~x3 & ~x4 & x5 & ~x6));
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & (x2 | (x3 & ~x4 & ~x5 & x6));
  assign z05 = ~x7 & (new_n80_ | x2);
  assign new_n80_ = ~x4 & x5 & x6;
  assign z06 = x7 & ~x6 & ~x5 & ~x4 & new_n82_ & x3;
  assign new_n82_ = x2 & ~x0 & ~x1;
  assign z07 = (x2 & ~x7) | (new_n84_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & x6 & ~x5 & ~x4 & new_n82_ & ~x3;
  assign z10 = x7 & x6 & x5 & new_n89_ & ~x4;
  assign new_n89_ = x2 & ~x0 & x1;
  assign z11 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x2 & (~x7 | (new_n80_ & x0 & ~x1 & ~x3));
  assign z13 = x7 & x6 & x5 & ~x4 & new_n94_ & x3;
  assign new_n94_ = ~x2 & ~x0 & x1;
  assign z14 = (x2 & ~x7) | (new_n84_ & new_n96_ & ~x2 & x3);
  assign new_n96_ = x0 & ~x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z16 = (x2 & ~x7) | (new_n84_ & ~x2 & x3 & x0 & x1);
  assign z17 = (x2 & ~x7) | (new_n96_ & ~x2 & x4 & ~x5);
  assign z18 = x2 & (~x7 | (new_n101_ & x3 & x4 & ~x5));
  assign new_n101_ = ~x0 & ~x1;
  assign z19 = (x2 & ~x7) | (new_n101_ & ~x2 & ~x3 & x4);
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n96_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n96_ & ~x2;
  assign z22 = (x2 & ~x7) | (new_n106_ & x0 & ~x1 & ~x2);
  assign new_n106_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = (x2 & ~x7) | (new_n101_ & ~x2 & x3 & x5);
  assign z24 = ~x7 & new_n109_ & x6;
  assign new_n109_ = ~x5 & ~x3 & ~x2 & ~x0 & ~x1 & ~x4;
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n94_ & ~x3;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n112_ & ~x3;
  assign new_n112_ = x0 & x2;
  assign z28 = x2 & (new_n114_ | ~x7);
  assign new_n114_ = x0 & ~x1 & x3 & ~x4 & ~x5 & x6;
  assign z29 = (x2 & ~x7) | (new_n101_ & ~x2 & ~x3 & new_n116_ & ~x6 & x7);
  assign new_n116_ = ~x4 & ~x5;
  assign z30 = x7 & x6 & new_n86_ & ~x5;
  assign z31 = new_n119_ | ~new_n120_ | (~z27 & (x1 | (~x0 & ~x4)));
  assign new_n119_ = x0 & (x2 ? x7 : (~x4 & x6));
  assign new_n120_ = (x4 | (x2 ? ~x7 : ~x5)) & (x2 | ((~x4 | x5) & (x0 | ~x3))) & (~x2 | (x5 & x7 & (x3 | ~x7)));
  assign z32 = new_n122_ | new_n123_ | new_n119_ | new_n124_ | new_n125_;
  assign new_n122_ = x1 & (~x2 | x7);
  assign new_n123_ = ~x3 & ((x2 & x7) | (x0 & ~x2 & ~x4));
  assign new_n124_ = ~x2 & ((x4 & (~x0 | ~x5)) | (~x4 & x5) | (~x0 & (x3 | (~x4 & ~x6))));
  assign new_n125_ = ~x4 & x7 & (~x0 | x2);
  assign z33 = ~x2 | (~new_n127_ & x7);
  assign new_n127_ = x0 & ~x4 & (x1 | ~x5) & x6 & (~x1 | ~x3 | x5);
  assign z34 = (~x2 & (x5 ? ~new_n130_ : ~new_n129_)) | (x7 & (x2 | x5));
  assign new_n129_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign new_n130_ = x3 & ~x4 & ~x6;
  assign z35 = (x0 & (x2 | ~x5)) | ~new_n132_ | (x2 & (~x3 | ~x5 | ~x7));
  assign new_n132_ = ~x1 & x4 & (x0 | x2 | ~x3);
  assign z36 = x2 ? x7 : (~new_n96_ | ~x4 | x5);
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n135_)) | x2 | (~x1 & ~x3);
  assign new_n135_ = ~x4 & x6 & ~x7;
  assign z38 = new_n122_ | new_n123_ | new_n119_ | new_n137_ | (new_n138_ & ~x0);
  assign new_n137_ = ~x4 & (x2 ? x7 : x5);
  assign new_n138_ = ~x2 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x4 & (~x2 | x7)) | (~new_n141_ & x7) | (~x2 & ~new_n140_ & ~x7);
  assign new_n140_ = x3 & x5 & ~x6;
  assign new_n141_ = x0 & ~x1 & ~x2 & ~x5 & x6;
  assign z40 = (~new_n112_ & x1) | new_n143_ | new_n144_ | new_n145_ | ~new_n146_;
  assign new_n143_ = ~x2 & (x0 ? (~x4 & x6) : x3);
  assign new_n144_ = x0 & ((x4 & (x2 | ~x5)) | (x2 & (x3 | x5 | ~x6)));
  assign new_n145_ = ~x0 & ((x2 & (~x3 | ~x4)) | (~x4 & (~x6 | x7)));
  assign new_n146_ = (x4 | ~x5) & (~x2 | x7);
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | x7)) | (~new_n149_ & x7) | (~x2 & ~x7 & (~x5 | x6));
  assign new_n149_ = x0 & ~x1 & (~x2 | x3) & ~x5 & x6;
  assign z43 = ((new_n151_ | new_n152_) & ~x2) | (x7 & (new_n154_ | (~new_n153_ & x2)));
  assign new_n151_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n152_ = ~x4 & ((x6 & (x5 | (x0 & ~x7))) | (~x0 & ~x5 & ~x6));
  assign new_n153_ = (x0 | (~x1 & x3)) & (~x1 | ~x3) & (~x0 | (~x4 & ~x5 & x6));
  assign new_n154_ = ~x4 & (~x0 | x5);
  assign z44 = (~new_n156_ & ~x2) | (x7 & (new_n154_ | x2));
  assign new_n156_ = ((~x4 & x5) | (~x1 & (x0 | ~x3))) & (x4 | (x0 & (~x5 | ~x6))) & (~x0 | (~x3 & ~x4 & ~x5 & ~x6));
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | (x5 & (~x1 | ~x4)) | ~new_n158_ | (~x1 & (x2 | x4 | ~x6));
  assign new_n158_ = ~x0 & x7;
  assign z46 = x2 ? x7 : (~new_n160_ | (~x4 & (x5 | (x6 & ~x7))));
  assign new_n160_ = ~x0 & x1 & ~x3;
  assign z47 = (~new_n164_ & ~x2) | (x7 & (new_n163_ | (~new_n162_ & x2)));
  assign new_n162_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n163_ = x0 & (~x3 | x4 | ~x5 | ~x6);
  assign new_n164_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = (~x3 & (~x2 | (x0 & x7))) | (~new_n166_ & ~x2) | (x7 & (x2 | (~new_n80_ & x0)));
  assign new_n166_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = new_n168_ | ~new_n101_ | ~x2 | ~x7 | (x3 & x4);
  assign new_n168_ = ~x4 & (x5 | x6);
  assign z50 = x2 ? x7 : (~new_n106_ | (x0 & (~x1 | ~x3)));
  assign z51 = new_n171_ | new_n172_ | new_n174_ | (~new_n173_ & ~x2);
  assign new_n171_ = (~x2 | x7) & (x0 ? ~x1 : (x1 | ~x3));
  assign new_n172_ = (x5 | x6) & ((~x0 & (x2 ? x7 : ~x4)) | (x0 & x2 & ~x4 & x7));
  assign new_n173_ = (~x0 | ~x3) & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign new_n174_ = ~x0 & x2 & x4 & x7;
  assign z52 = (~new_n177_ & x0) | new_n178_ | ~new_n179_ | (~x0 & (new_n176_ | x1));
  assign new_n176_ = ~x4 & x6;
  assign new_n177_ = ~x3 & (x1 | x2);
  assign new_n178_ = ~x2 & ((~x4 & ~x5 & x6) | (~x0 & ~x3));
  assign new_n179_ = (x4 | (~x5 & (~x2 | ~x6))) & (~x2 | (x7 & (~x3 | ~x4)));
  assign z53 = new_n181_ | (~new_n184_ & ~x2) | (x7 & (new_n182_ | ~new_n183_));
  assign new_n181_ = x1 & ((x0 & ~x2 & ~x3) | (~x0 & x2 & x3 & x7));
  assign new_n182_ = x0 & (x3 ? ~x1 : x2);
  assign new_n183_ = ((x3 ? x6 : ~x2) | (x1 & (x4 | ~x5))) & (~x3 | ((x4 | x5 | ~x6) & (x1 | (~x4 & x5)))) & (~x2 | x3 | x4 | ~x6);
  assign new_n184_ = x3 ? (x1 & (x4 | (~x5 & ~x6))) : (~x4 & x5 & x6 & x7);
  assign z54 = (~x2 & (new_n186_ | ~new_n187_)) | (x7 & (new_n188_ | ~new_n189_));
  assign new_n186_ = ~x3 & (~x1 | (~x0 & ~x4 & (x5 | x6)));
  assign new_n187_ = (~x0 | ~x3) & ((~x0 & ~x3) | (~x4 & x5 & x6 & x7));
  assign new_n188_ = x0 & (~x1 | x3);
  assign new_n189_ = (~x3 | ((x1 | ~x2) & (x4 | (~x5 ^ x6)))) & (~x2 | x3 | (~x4 & x5 & x6));
  assign z55 = new_n191_ | ~new_n122_ | (x0 & (x2 ? ~new_n80_ : ~x3));
  assign new_n191_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n193_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n193_ = (x7 | (~x2 & (x4 | ~x6))) & ~x0 & (~x2 | (~x4 & x5 & x6));
  assign z57 = new_n195_ | new_n196_ | (~new_n197_ & ~x2) | (x2 & ~new_n80_ & x7);
  assign new_n195_ = x0 & (x2 ? x7 : ~x3);
  assign new_n196_ = ~x1 & (~x2 | (~x3 & x7));
  assign new_n197_ = (x0 | ~x3) & (x4 | (~x5 & (~x6 | x7)));
  assign z58 = (new_n199_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n200_ | (~new_n176_ & (x0 | ~x1));
  assign new_n199_ = ~x4 & (x5 | (x1 & x6));
  assign new_n200_ = (~x0 | (x1 & x5)) & x3 & x7 & (x1 | (~x2 & ~x5));
  assign z59 = new_n202_ | new_n203_ | new_n204_ | ~x7 | (~x4 & x5);
  assign new_n202_ = x2 & ((~x0 & (x1 | x3)) | (x1 & (x3 | (~x4 & x6))));
  assign new_n203_ = x0 & ((~x2 & (~x1 | ~x3)) | (~x1 & (~x3 | (~x4 & x6))));
  assign new_n204_ = (x4 | x5 | ~x6) & (x1 ? ~x2 : ~x0);
  assign z60 = (~new_n206_ & (~x2 | x7)) | (~x0 & (x2 ? (~x3 & x7) : ~x7)) | (x3 & (~x2 | (x0 & x7)));
  assign new_n206_ = x0 ? (x1 & x4) : (~x1 & ~x4 & x5 & x6);
  assign z61 = ~x2 | (x7 & (new_n168_ | ~x0 | x1 | ~x3));
  assign z62 = new_n168_ | ~x0 | ~x1 | z27 | x3;
endmodule


