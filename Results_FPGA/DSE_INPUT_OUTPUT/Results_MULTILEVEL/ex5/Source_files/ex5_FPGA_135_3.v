// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n79_, new_n81_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n97_, new_n99_, new_n102_, new_n104_,
    new_n111_, new_n114_, new_n117_, new_n119_, new_n120_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n129_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n138_, new_n139_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n160_, new_n162_, new_n163_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n178_, new_n180_, new_n181_, new_n182_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = z06 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = x3 & (x2 | (new_n76_ & new_n79_));
  assign new_n79_ = ~x4 & x5;
  assign z04 = x3 & (new_n81_ | x2);
  assign new_n81_ = ~x4 & ~x5 & x6 & ~x7;
  assign z05 = z06 | (new_n79_ & x6 & ~x7);
  assign z07 = x7 & new_n84_ & x6;
  assign new_n84_ = x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n88_ & x6;
  assign new_n88_ = ~x4 & ~x3 & x2 & ~x0 & ~x1 & ~x5;
  assign z10 = x7 & x6 & new_n90_ & x5;
  assign new_n90_ = ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = (x2 & x3) | (new_n92_ & x0 & x1 & ~x2 & ~x3);
  assign new_n92_ = ~x4 & x5 & x6 & x7;
  assign z12 = x2 & (x3 | (new_n94_ & x0 & ~x1 & ~x4));
  assign new_n94_ = x5 & x6 & x7;
  assign z13 = x3 & (x2 | (new_n94_ & ~x0 & x1 & ~x4));
  assign z14 = x7 & x6 & x5 & ~x4 & new_n97_ & x3;
  assign new_n97_ = ~x2 & x0 & ~x1;
  assign z16 = x7 & new_n99_ & x6;
  assign new_n99_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & new_n97_ & x4;
  assign z19 = (x2 & x3) | (new_n102_ & ~x2 & ~x3 & x4);
  assign new_n102_ = ~x0 & ~x1;
  assign z20 = (x2 & x3) | (x0 & ~x1 & ~x2 & new_n104_ & ~x3 & ~x4);
  assign new_n104_ = ~x5 & ~x6;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n97_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n97_ & ~x4;
  assign z23 = x5 & x3 & new_n102_ & ~x2;
  assign z24 = (x2 & x3) | (new_n81_ & new_n102_ & ~x2 & ~x3);
  assign z25 = (x2 & x3) | (new_n81_ & ~x2 & ~x3 & ~x0 & x1);
  assign z26 = ~new_n111_ & x2;
  assign new_n111_ = ~x3 & (~x0 | x4 | x5 | ~x6 | ~x7);
  assign z27 = ~x7 & x6 & new_n90_ & ~x5;
  assign z29 = x7 & new_n114_ & ~x6;
  assign new_n114_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & x6 & new_n86_ & ~x5;
  assign z31 = (~x0 & (~x4 | (~x2 & x3))) | x2 | (~new_n117_ & ~x2);
  assign new_n117_ = (x4 | (~x5 & (~x0 | ~x6))) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n120_ | (~new_n119_ & ~x4);
  assign new_n119_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n120_ = x2 ? x3 : (~x1 & (~x4 | x5) & (x0 | (~x3 & ~x4)));
  assign z33 = ~new_n122_ | ~x7 | x4 | ~x6;
  assign new_n122_ = x2 & ~x3 & x0 & (x1 | ~x5);
  assign z34 = (x1 & (x2 | ~x5)) | (~new_n125_ & ~x5) | ~new_n126_ | (~new_n124_ & (x2 | x5));
  assign new_n124_ = ~x4 & ~x7;
  assign new_n125_ = (x4 | x6) & (x2 | (x0 & (x4 | x7)));
  assign new_n126_ = (~x5 | (~x2 & x3 & ~x6)) & (~x2 | (~x0 & ~x3 & x6));
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = ~new_n129_ | (~z06 & (x1 | x5));
  assign new_n129_ = x2 ? (x3 | (~x0 & ~x4 & x6 & ~x7)) : (x0 & x4);
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n131_)) | x2 | (~x1 & ~x3);
  assign new_n131_ = ~x4 & x6 & ~x7;
  assign z38 = new_n133_ | new_n134_ | x1 | x2;
  assign new_n133_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n134_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (x5 & (~new_n76_ | ~x3)) | (~new_n136_ & ~x5) | x2 | x4;
  assign new_n136_ = x6 & x7 & x0 & ~x1;
  assign z40 = x2 ? new_n111_ : (new_n138_ | new_n139_ | new_n79_ | x1);
  assign new_n138_ = x0 & (x4 ? ~x5 : x6);
  assign new_n139_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n142_ | (x2 & (x3 | ~x5));
  assign new_n142_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = x2 ? new_n146_ : (new_n144_ | new_n145_);
  assign new_n144_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n145_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n146_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = x2 ? ~x3 : (new_n144_ | ~new_n148_);
  assign new_n148_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = new_n150_ | (x1 & (~x2 | x3)) | x0 | (~new_n151_ & ~x1);
  assign new_n150_ = ~x4 & (x5 | (x1 & x6));
  assign new_n151_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = x2 ? ~x3 : (new_n153_ | x0 | ~x1 | x3);
  assign new_n153_ = ~x4 & (x5 | (x6 & ~x7));
  assign z48 = ~new_n155_ | (~x4 & (x5 ? (~x6 | ~x7) : x6));
  assign new_n155_ = ~x0 & ~x1 & ~x2 & x3;
  assign z49 = ~x2 | (~x3 & (~new_n102_ | new_n157_));
  assign new_n157_ = ~x4 & (x5 | x6);
  assign z50 = ~new_n151_ | (x0 & (~x1 | ~x3));
  assign z51 = (~x2 & (~new_n160_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | ~x1 | (new_n157_ & x2)));
  assign new_n160_ = (x0 | ~x1) & (x4 | ((x0 | (~x5 & ~x6)) & (x5 | ~x6) & (~x5 | (x6 & x7))));
  assign z52 = (~x2 & ((x0 & (~x1 | x3)) | new_n162_ | (~x0 & (x1 | ~x3)))) | (~x3 & (new_n163_ | (~x0 & x1)));
  assign new_n162_ = ~x4 & (x5 | (x6 & (~x0 | ~x5)));
  assign new_n163_ = ~x4 & (x5 | x6) & (~x0 | x2);
  assign z53 = (~x3 & ((x0 & (x1 | x2)) | (~new_n92_ & ~x2) | (x2 & (new_n157_ | ~x1)))) | (~x2 & x3 & (new_n157_ | ~x1));
  assign z54 = (~x3 & (new_n166_ | new_n168_ | (~new_n167_ & ~x2))) | (~x2 & ~new_n169_ & x3);
  assign new_n166_ = x0 & ~x1;
  assign new_n167_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n168_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n169_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n171_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n172_ & ~x3);
  assign new_n171_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & ~x3));
  assign new_n172_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n174_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n175_ & x2);
  assign new_n174_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n175_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | (x2 & ~x3))) | (~x2 & (new_n153_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n92_ & x2)));
  assign z58 = ~x3 | (~x2 & (~new_n178_ | x0 | x1 | x4));
  assign new_n178_ = ~x5 & x6 & x7;
  assign z59 = (~new_n180_ & x0) | (~new_n182_ & ~x3) | (~new_n181_ & (x3 ? ~x2 : ~x0));
  assign new_n180_ = (x1 | (x2 & x3)) & (x3 | (x2 & (x4 | ~x6)));
  assign new_n181_ = ~x4 & ~x5 & x6 & x7;
  assign new_n182_ = (x4 | ~x5) & (x0 | ~x1 | ~x2);
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n94_ | x1 | x2 | x4));
  assign z61 = ~x3 | (~x2 & x3);
  assign z62 = x3 ? ~x2 : (new_n157_ | ~x0 | ~x1);
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z28 = z06;
  assign z47 = new_n150_ | (x1 & (~x2 | x3)) | x0 | (~new_n151_ & ~x1);
endmodule


