// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n106_, new_n109_, new_n111_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n123_,
    new_n124_, new_n127_, new_n128_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n150_, new_n152_,
    new_n154_, new_n155_, new_n157_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n167_, new_n168_, new_n172_, new_n173_, new_n176_;
  assign z00 = z08 | (new_n75_ & ~x4);
  assign z08 = x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z08 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z08 & ~x7;
  assign z05 = z08 | (new_n81_ & x6 & ~x7);
  assign new_n81_ = ~x4 & x5;
  assign z06 = ~x6 & ~x5 & ~x4 & x3 & new_n83_ & x2;
  assign new_n83_ = ~x0 & ~x1;
  assign z07 = (x0 & x2) | (new_n85_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n85_ = ~x4 & x5 & x6 & x7;
  assign z09 = x7 & new_n87_ & x6;
  assign new_n87_ = ~x5 & ~x4 & ~x3 & new_n83_ & x2;
  assign z10 = new_n89_ & x7;
  assign new_n89_ = x6 & x5 & ~x4 & x2 & ~x0 & x1;
  assign z11 = x0 & (x2 | (new_n91_ & x1 & ~x3 & ~x4));
  assign new_n91_ = new_n92_ & x5;
  assign new_n92_ = x6 & x7;
  assign z13 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x0 & (x2 | (new_n91_ & ~x1 & x3 & ~x4));
  assign z15 = x2 & (x0 | (new_n91_ & x1 & x3 & ~x4));
  assign z16 = x0 & (x2 | (new_n91_ & x1 & x3 & ~x4));
  assign z17 = x0 & (x2 | (~x1 & x4 & ~x5));
  assign z18 = ~x5 & x4 & x3 & new_n83_ & x2;
  assign z19 = (x0 & x2) | (~x0 & ~x1 & ~x2 & ~x3 & x4);
  assign z20 = x0 & (x2 | (new_n75_ & ~x4 & ~x1 & ~x3));
  assign z21 = x0 & (x2 | (new_n75_ & ~x4 & ~x1 & x3));
  assign z22 = x0 & (x2 | (new_n92_ & ~x5 & ~x1 & ~x4));
  assign z23 = (x0 & x2) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = (x0 & x2) | (new_n106_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n106_ = ~x4 & ~x5 & x6 & ~x7;
  assign z25 = (x0 & x2) | (new_n106_ & ~x0 & x1 & ~x2 & ~x3);
  assign z27 = x2 & (new_n109_ | x0);
  assign new_n109_ = x1 & ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z29 = x7 & new_n111_ & ~x6;
  assign new_n111_ = ~x5 & ~x4 & ~x3 & new_n83_ & ~x2;
  assign z31 = ~new_n113_ | (~z08 & x1);
  assign new_n113_ = (x0 | ((~x2 | (x3 & x5)) & x4 & (x2 | ~x3))) & (x2 | (x4 ? x5 : (~x5 & (~x0 | ~x6))));
  assign z32 = (~new_n115_ & ~x2) | (~x0 & (~new_n117_ | (~new_n116_ & x2)));
  assign new_n115_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x3 & ~x4)) & ~x1 & (~x4 | x5);
  assign new_n116_ = x3 & x4;
  assign new_n117_ = ~x1 & (x4 | (x6 & ~x7));
  assign z34 = (x0 & (x2 | (~x4 & ~x5 & ~x7))) | ~new_n119_ | ((~x0 | x5) & (x4 | x7));
  assign new_n119_ = x5 ? (x3 & ~x6) : new_n120_;
  assign new_n120_ = (x6 | (x0 & x4)) & ~x1 & (x0 | (x2 & ~x3));
  assign z35 = ((~x0 | ~x2) & (x1 | ~x4)) | (~x0 & (x2 ? (~x3 | ~x5) : x3)) | (x0 & ~x2 & ~x5);
  assign z36 = (x0 & (x2 | ~x4)) | ~new_n124_ | (~x0 & (~new_n123_ | ~x2 | x3));
  assign new_n123_ = ~x4 & x6 & ~x7;
  assign new_n124_ = ~x1 & ~x5;
  assign z37 = (~x3 & (~x0 | (~x1 & ~x2))) | (~new_n123_ & (x2 ? ~x0 : (x3 & ~x5))) | (x5 & (~x0 | (x1 & ~x2 & x3)));
  assign z38 = (~x2 & (new_n127_ | (~new_n128_ & ~x0) | x1)) | (~x0 & (x1 | (~new_n116_ & x2)));
  assign new_n127_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n128_ = ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z39 = (~new_n131_ & (~x0 | (~x2 & x5))) | (~x0 & (x4 | ~x5)) | (~x2 & (x4 | (~new_n130_ & ~x5)));
  assign new_n130_ = new_n92_ & ~x1;
  assign new_n131_ = x3 & ~x6 & ~x7;
  assign z40 = (~new_n133_ & ~x2) | (~x0 & (~new_n134_ | (~new_n116_ & x2)));
  assign new_n133_ = (~x0 | (x4 ? x5 : ~x6)) & ~x1 & (x4 | ~x5) & (x0 | ~x3);
  assign new_n134_ = ~x1 & (x4 | (~x5 & x6 & ~x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~new_n137_ | (x2 & (x0 | ~x5));
  assign new_n137_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z43 = new_n139_ | (~x2 & (new_n142_ | (~new_n81_ & (new_n141_ | x1))));
  assign new_n139_ = ~x0 & (x4 ? (x1 | (x2 & ~x3)) : ~new_n140_);
  assign new_n140_ = (~x2 | (x5 & ~x6)) & (~x5 | ~x6) & ~x7 & (x5 | x6);
  assign new_n141_ = ~x0 & x3;
  assign new_n142_ = ~x4 & ((x0 & x6 & ~x7) | (x5 & (x6 | x7)));
  assign z44 = (~new_n144_ & ~x2) | (~x0 & (~x4 | (x1 & x4))) | x2 | (~new_n145_ & x0);
  assign new_n144_ = ((~new_n141_ & ~x1) | (~x4 & x5)) & (x4 | ~x5 | (~x6 & ~x7));
  assign new_n145_ = new_n75_ & ~x3 & ~x4;
  assign z45 = new_n148_ | new_n147_ | x0 | (x5 & (~x1 | ~x4));
  assign new_n147_ = ~x1 & (~new_n92_ | x2 | x4);
  assign new_n148_ = x1 & (~x2 | (~x4 & x6));
  assign z46 = x0 ? ~x2 : (new_n150_ | ~x1 | x2 | x3);
  assign new_n150_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (~x2 | (~x0 & ~x4 & x6))) | (~new_n152_ & ~x0) | (x0 & ~x2);
  assign new_n152_ = (x1 | (new_n92_ & ~x2 & ~x4)) & (~x5 | (x1 & x4));
  assign z48 = new_n155_ | ~new_n83_ | ~new_n154_;
  assign new_n154_ = ~x2 & x3;
  assign new_n155_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = new_n157_ | ~new_n83_ | new_n116_ | ~x2;
  assign new_n157_ = ~new_n75_ & ~x4;
  assign z50 = x2 ? ~x0 : (~new_n159_ | (x0 & (~x1 | ~x3)));
  assign new_n159_ = new_n92_ & ~x4 & ~x5;
  assign z51 = (x2 & (x0 | x4)) | (~new_n162_ & x0) | new_n155_ | (~new_n161_ & ~x0);
  assign new_n161_ = (x3 | (x2 & x4)) & ~x1 & (new_n75_ | x4);
  assign new_n162_ = x1 & ~x3;
  assign z52 = (x2 & (x0 | (x3 & x4))) | (x0 & (~x1 | x3)) | (~new_n164_ & ~x4) | (~x0 & (x1 | (~x2 & ~x3)));
  assign new_n164_ = ~x5 & (~x6 | (x0 & x5));
  assign z53 = (x1 & (x0 ? (~x2 & ~x3) : (x2 & x3))) | (~new_n85_ & ((~x2 & ~x3) | (~x0 & ~x1 & x3))) | (((~x0 & x2 & ~x3) | (~x2 & x3)) & (new_n157_ | ~x1));
  assign z54 = ~new_n168_ | (~x2 & (x3 ? ~new_n85_ : ~new_n167_));
  assign new_n167_ = x1 & (x0 | new_n75_ | x4);
  assign new_n168_ = (x1 | (~x0 & (~x2 | ~x3))) & (new_n85_ | (~x0 & (~x2 | x3))) & (~x0 | (~x2 & ~x3)) & (~new_n155_ | ~x2);
  assign z55 = (~new_n154_ & x0) | new_n157_ | ~x1;
  assign z56 = (~x1 & (~x2 | (~x0 & x3))) | (~x2 & (new_n150_ | x0 | ~x3)) | (~x0 & ~new_n85_ & x2);
  assign z57 = (~x3 & (x0 | ~x1)) | ~new_n172_ | ((x0 | ~x2) & (new_n81_ | ~x1));
  assign new_n172_ = new_n173_ & (x7 | (~x2 & (x4 | ~x6)));
  assign new_n173_ = (~x2 | (x5 & x6 & ~x0 & ~x4)) & (x0 | x2 | ~x3);
  assign z58 = new_n148_ | new_n147_ | ~new_n141_ | (x5 & (~x1 | ~x4));
  assign z59 = (~new_n176_ & ~x2) | (~x0 & (~new_n159_ | (x2 & (x1 | x3))));
  assign new_n176_ = (~x0 | (x1 & x3 & (~x3 | ~x5))) & (~x3 | (new_n92_ & ~x4));
  assign z60 = (~x2 & (x3 | (x0 & (~x1 | (~x3 & ~x4))))) | (~x0 & (~new_n91_ | x1 | x4 | (x2 & ~x3)));
  assign z62 = ~x0 | (~x2 & (new_n157_ | ~new_n162_));
  assign z26 = 1'b0;
  assign z30 = 1'b0;
  assign z33 = ~z08;
  assign z12 = z08;
  assign z28 = z08;
  assign z61 = z33;
endmodule


