// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_,
    new_n96_, new_n98_, new_n101_, new_n105_, new_n107_, new_n109_,
    new_n112_, new_n114_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n124_, new_n126_, new_n128_, new_n129_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n152_, new_n157_, new_n158_, new_n161_, new_n163_, new_n165_,
    new_n167_, new_n169_, new_n170_, new_n171_, new_n172_;
  assign z00 = ~x4 & ~x5 & ~z20 & ~x6;
  assign z20 = ~x2 & ~x3;
  assign z01 = ~x5 & ~x6 & ~z20 & ~x7;
  assign z02 = ~x3 & (~x2 | (~x4 & x5 & ~x6 & ~x7));
  assign z03 = (~x2 & ~x3) | (x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = z20 | (~x4 & x5 & x6 & ~x7);
  assign z06 = (~x2 & ~x3) | (~x0 & ~x1 & x2 & new_n81_ & x3 & ~x4);
  assign new_n81_ = ~x5 & ~x6;
  assign z08 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n85_ & x6;
  assign new_n85_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = (~x2 & ~x3) | (new_n87_ & ~x0 & x1 & x2);
  assign new_n87_ = x6 & x7 & ~x4 & x5;
  assign z12 = x7 & new_n89_ & x6;
  assign new_n89_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x2 & (~x3 | (new_n93_ & x0 & ~x1 & ~x4));
  assign new_n93_ = x5 & x6 & x7;
  assign z15 = (~x2 & ~x3) | (new_n87_ & ~x0 & x1 & x2 & x3);
  assign z16 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & x3 & new_n98_ & ~x2;
  assign new_n98_ = x0 & ~x1;
  assign z18 = ~x5 & x4 & x3 & x2 & ~x0 & ~x1;
  assign z21 = new_n101_ & ~x6;
  assign new_n101_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = x7 & new_n101_ & x6;
  assign z23 = ~x2 & (~x3 | (~x0 & ~x1 & x5));
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n105_ & ~x3;
  assign new_n105_ = x0 & x2;
  assign z27 = ~x7 & new_n107_ & x6;
  assign new_n107_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z28 = (~x2 & ~x3) | (new_n98_ & x2 & x3 & new_n109_ & ~x4 & ~x5);
  assign new_n109_ = x6 & x7;
  assign z30 = x7 & x6 & new_n83_ & ~x5;
  assign z31 = ~new_n112_ | (~z20 & x1);
  assign new_n112_ = (~x3 | ((x2 | (x0 & (~x4 | x5))) & (x4 | (~x5 & ~x6)))) & (~x2 | (x4 & x5 & ~x0 & x3));
  assign z32 = ~new_n114_ | (~z20 & x1);
  assign new_n114_ = (~x2 | (~x0 & x4)) & x3 & (~x3 | ((x2 | (x0 & (~x4 | x5))) & (x4 | (~x5 & ~x6))));
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n118_ | new_n119_ | (~new_n120_ & x3) | (x2 & ~new_n121_ & ~x3);
  assign new_n118_ = x1 & (x3 ? ~x5 : x2);
  assign new_n119_ = (x2 | (x3 & x5)) & (x4 | x7);
  assign new_n120_ = (~x2 | (x5 & ~x6)) & (~x5 | ~x6) & (x5 | (x0 & (x4 | (x6 & x7))));
  assign new_n121_ = ~x0 & ~x5 & x6;
  assign z35 = (~x0 & (~x2 | (x3 & ~x5))) | ((x0 | x3) & ~x4) | (x0 & (x2 | ~x5)) | x1 | ~x3;
  assign z36 = (~x0 & (~x2 | x3)) | (~x4 & (x0 | x3)) | (x5 & (x0 | ~x3)) | ~new_n124_ | (x0 & (x2 | ~x3));
  assign new_n124_ = ~x1 & (x3 | (~x4 & x6 & ~x7));
  assign z37 = (x5 & (~x0 | x1 | x2)) | ~x3 | (~new_n126_ & ~x5);
  assign new_n126_ = ~x4 & x6 & ~x7;
  assign z38 = new_n129_ | new_n128_ | x1 | new_n105_ | ~x3;
  assign new_n128_ = ~x4 & (x5 | x6);
  assign new_n129_ = ~x0 & (~x2 | ~x4);
  assign z39 = (~new_n131_ & x3) | (x2 & (~x3 | ~x5));
  assign new_n131_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = new_n133_ | new_n129_ | new_n134_ | new_n135_ | (new_n128_ & ~x2);
  assign new_n133_ = (~x0 | ~x2) & (x1 | ~x3);
  assign new_n134_ = x4 & (x2 ? x0 : ~x5);
  assign new_n135_ = x0 & x2 & (x3 | x5 | ~x6 | ~x7);
  assign z41 = x2 | (x3 & (~x0 | x1 | ~x5));
  assign z42 = ~new_n131_ | (~x3 & (~x2 | ~x5));
  assign z43 = (~new_n139_ & ~x4) | new_n142_ | (x3 & (new_n140_ | new_n141_));
  assign new_n139_ = (~x5 | (~x6 & ~x7) | (~x2 & ~x3)) & (x7 | (x2 ? x5 : (~x3 | ~x6))) & (~x2 | x5 | (x0 & x6));
  assign new_n140_ = (x4 | ~x5) & (x1 | (~x0 & ~x2));
  assign new_n141_ = ~x0 & ~x2 & (x6 | x7);
  assign new_n142_ = x2 & x4 & (x0 | x1 | ~x3);
  assign z45 = (x0 & (x2 | x3)) | (x2 & (new_n128_ | ~x1)) | (~x2 & ~new_n144_ & x3);
  assign new_n144_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z47 = x2 ? (new_n146_ | new_n147_ | ~x1) : (~new_n148_ & x3);
  assign new_n146_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n147_ = x0 & (~x3 | x4 | ~x5 | ~x6 | ~x7);
  assign new_n148_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z48 = new_n150_ | x0 | x1 | x2 | ~x3;
  assign new_n150_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = (x0 & (~x1 | x2)) | (x2 & (x1 | (~x4 & x6))) | ~new_n152_ | ~x2;
  assign new_n152_ = x4 ? ~x3 : ~x5;
  assign z50 = (x4 & (~x2 | x3)) | (x5 & (~x2 | ~x4)) | new_n98_ | x2 | (~x2 & (~new_n109_ | ~x3));
  assign z51 = (x0 & (x2 ? ~x1 : x3)) | (~x0 & ((x1 & (x2 | x3)) | (x2 & (~x3 | x4)) | (x3 & ~new_n81_ & ~x4))) | (x2 & ~new_n81_ & ~x4);
  assign z52 = (~x0 & ((x1 & (x2 | x3)) | (x3 & (x4 ? x2 : ~new_n81_)))) | (x2 & ~new_n81_ & ~x4) | (x0 & x3);
  assign z53 = ((~x1 | ~x3) & (x0 | ~x2)) | new_n158_ | (~new_n157_ & ~x1) | (~x0 & x1 & x2 & x3);
  assign new_n157_ = x3 & ~x4 & x5 & x6 & x7;
  assign new_n158_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | ((x5 | x6) & (~x2 | ~x3)));
  assign z54 = (x0 & (~x1 | x3)) | (~new_n87_ & (~x2 | ~x3)) | new_n150_ | (~x2 & ~x3) | (~x1 & x2 & x3);
  assign z55 = new_n161_ | (~x1 & (x2 | x3)) | (~x2 & ~x3) | (x0 & ~new_n87_ & x2);
  assign new_n161_ = ~x4 & ((~x0 & (x6 | (x2 & x5))) | (~x2 & (x6 | (x3 & x5))));
  assign z56 = (x0 & (x2 | x3)) | (~new_n87_ & x2) | (x3 & (~x1 | (new_n163_ & ~x2)));
  assign new_n163_ = ~x4 & (x5 | (x6 & ~x7));
  assign z57 = (~x1 & (~x2 ^ ~x3)) | (~new_n165_ & x2) | (~x2 & x3 & (new_n163_ | ~x0));
  assign new_n165_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z58 = x2 ? (new_n146_ | new_n167_ | ~x1 | ~x3) : (~new_n148_ & x3);
  assign new_n167_ = x0 & (x4 | ~x5 | ~x6 | ~x7);
  assign z59 = (x2 & (x3 ? ~new_n171_ : ~new_n170_)) | (x3 & (new_n169_ | (~new_n172_ & ~x2)));
  assign new_n169_ = x5 & (~x2 | ~x4);
  assign new_n170_ = (~x0 | (x1 & (x4 | ~x6))) & (~x5 | (x0 & x4)) & (x0 | (x6 & x7 & ~x1 & ~x4));
  assign new_n171_ = x0 & ~x1 & (x4 | ~x6);
  assign new_n172_ = x6 & x7 & ~x4 & (~x0 | x1);
  assign z60 = (x0 & (x3 | (~x1 & x2 & ~x3))) | (x2 & ~x3 & (~x0 | ~x4)) | (x3 & (~new_n93_ | x1 | ~x2 | x4));
  assign z61 = new_n128_ | ~x0 | x1 | ~x2 | (~x1 & ~x3);
  assign z62 = new_n128_ | ~x0 | ~x1 | ~x2 | (x1 & x3);
  assign z07 = 1'b0;
  assign z11 = 1'b0;
  assign z19 = 1'b0;
  assign z24 = 1'b0;
  assign z29 = 1'b0;
  assign z44 = 1'b1;
  assign z46 = 1'b1;
  assign z25 = z20;
endmodule


