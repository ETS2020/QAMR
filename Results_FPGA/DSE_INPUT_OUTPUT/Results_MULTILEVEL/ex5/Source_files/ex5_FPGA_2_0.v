// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n74_, new_n77_, new_n82_, new_n84_, new_n86_, new_n87_, new_n89_,
    new_n92_, new_n94_, new_n96_, new_n99_, new_n105_, new_n108_,
    new_n110_, new_n112_, new_n116_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n136_, new_n138_, new_n139_,
    new_n141_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n158_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n168_, new_n170_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n191_, new_n193_, new_n194_, new_n195_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n203_;
  assign z00 = ~x4 & ~x5 & ~new_n74_ & ~x6;
  assign new_n74_ = ~x2 & ~x7;
  assign z01 = ~x7 & ~x6 & x2 & ~x5;
  assign z02 = ~x7 & (~x2 | (new_n77_ & ~x3 & ~x4));
  assign new_n77_ = x5 & ~x6;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & x2 & x3;
  assign z04 = ~x7 & (~x2 | (x3 & ~x4 & ~x5 & x6));
  assign z05 = ~x7 & x6 & x5 & x2 & ~x4;
  assign z06 = (~x2 & ~x7) | (new_n82_ & ~x0 & ~x1 & x2);
  assign new_n82_ = x3 & ~x4 & ~x5 & ~x6;
  assign z07 = ~x2 & (~x7 | (new_n84_ & ~x0 & x1 & ~x3));
  assign new_n84_ = ~x4 & x5 & x6;
  assign z08 = new_n74_ | (new_n87_ & new_n86_ & x0 & x1);
  assign new_n86_ = x2 & ~x3;
  assign new_n87_ = ~x4 & x5 & x6 & x7;
  assign z09 = new_n74_ | (new_n89_ & new_n86_ & ~x0 & ~x1);
  assign new_n89_ = ~x4 & ~x5 & x6 & x7;
  assign z10 = (~x2 & ~x7) | (new_n87_ & ~x0 & x1 & x2);
  assign z11 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n94_ & ~x3;
  assign new_n94_ = x2 & x0 & ~x1;
  assign z13 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x2 & (~x7 | (new_n84_ & x0 & ~x1 & x3));
  assign z15 = x7 & new_n99_ & x6;
  assign new_n99_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = ~x2 & (~x7 | (new_n84_ & x0 & x1 & x3));
  assign z17 = ~x2 & (~x7 | (x0 & ~x1 & x4 & ~x5));
  assign z18 = ~x5 & x4 & x3 & x2 & ~x0 & ~x1;
  assign z19 = ~x2 & (~x7 | (~x0 & ~x1 & ~x3 & x4));
  assign z20 = x7 & ~x5 & ~x4 & new_n105_ & ~x3 & ~x6;
  assign new_n105_ = ~x2 & x0 & ~x1;
  assign z21 = x7 & ~x6 & ~x5 & ~x4 & new_n105_ & x3;
  assign z22 = ~x2 & (~x7 | (new_n108_ & x0 & ~x1));
  assign new_n108_ = ~x4 & ~x5 & x6;
  assign z23 = ~x2 & (~x7 | (new_n110_ & ~x0 & ~x1));
  assign new_n110_ = x3 & x5;
  assign z26 = new_n112_ & x7;
  assign new_n112_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & (~x2 | (new_n108_ & ~x0 & x1 & ~x3));
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n94_ & x3;
  assign z29 = x7 & new_n116_ & ~x6;
  assign new_n116_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x7 & new_n118_ & x6;
  assign new_n118_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = new_n122_ | (x7 & (new_n120_ | ~new_n121_));
  assign new_n120_ = ~x0 & (~x4 | (~x2 & x3));
  assign new_n121_ = (x4 | (~x5 & ~x6)) & ~x1 & (x2 | ~x4 | x5);
  assign new_n122_ = x2 & (x0 | x1 | ~x3 | ~x4 | ~x5);
  assign z32 = (x7 & (~new_n124_ | (~x0 & (~x2 | ~x4)))) | (~new_n125_ & x2);
  assign new_n124_ = ~x1 & (x2 | ~x4 | x5) & (x4 | (x3 & ~x5 & ~x6));
  assign new_n125_ = ~x0 & ~x1 & x3 & x4;
  assign z33 = x2 ? (~new_n127_ | ~new_n128_) : x7;
  assign new_n127_ = x0 & ~x4 & (x1 | ~x5);
  assign new_n128_ = x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = (~new_n130_ & (x7 | (x2 & ~x3))) | (~new_n131_ & x2) | (~new_n132_ & x7);
  assign new_n130_ = ~x1 & ~x5;
  assign new_n131_ = (~x3 | (x5 & ~x6)) & (x3 | (~x0 & x6)) & ~x4 & ~x7;
  assign new_n132_ = x0 & (x4 | x6);
  assign z35 = (x0 & (x2 | (~x5 & x7))) | ~new_n134_ | (x1 & (x2 | x7));
  assign new_n134_ = x2 ? (x3 & (~x3 | (x4 & x5))) : (~x7 | (x4 & (x0 | ~x3)));
  assign z36 = (x1 & (x2 | x7)) | ~new_n136_ | (x5 & (x2 ? ~x3 : x7));
  assign new_n136_ = (~x7 | (x2 ? x3 : (x0 & x4))) & (~x2 | ((x3 | (~x4 & x6)) & ~x0 & ~x3));
  assign z37 = (~x1 & (~x3 | (~x2 & ~x5))) | (~new_n138_ & ~x2) | (~new_n139_ & x2);
  assign new_n138_ = x0 & x7 & (~x1 | ~x3);
  assign new_n139_ = x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z38 = (x0 & (x2 | ~x7)) | (~x3 & (~x0 | ~x4)) | ~new_n141_ | (~x0 & (~x2 | ~x4));
  assign new_n141_ = ~x1 & (x4 | (~x5 & ~x6));
  assign z39 = (x4 & (x2 | x7)) | new_n143_ | (x2 & (~new_n110_ | x6 | x7));
  assign new_n143_ = x7 & (~x0 | x1 | x5 | ~x6);
  assign z40 = new_n145_ | new_n146_ | new_n147_ | (~new_n148_ & x0) | (new_n86_ & ~x0);
  assign new_n145_ = x1 & (x2 ? ~x0 : x7);
  assign new_n146_ = x3 & (x0 ? x2 : (~x2 & x7));
  assign new_n147_ = ~x4 & ((~x0 & (x2 | x7)) | (~x2 & x7 & (x5 | x6)));
  assign new_n148_ = (~x4 | (~x2 & (x5 | ~x7))) & (~x2 | (~x5 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x7 | (x1 & x3) | ~x0 | x2;
  assign z42 = (x4 & (x2 | x7)) | new_n143_ | (x2 & (x7 ? ~x3 : ~new_n77_));
  assign z43 = new_n152_ | new_n153_ | ~new_n154_;
  assign new_n152_ = x3 & ((~x0 & ~x2) | (x1 & ~x5));
  assign new_n153_ = ~x4 & ((~x0 & (~x2 | ~x5)) | (~x5 & (~x7 | (x2 & ~x6))) | (x5 & (~x2 | x6 | x7)));
  assign new_n154_ = (~x1 | (x2 & ~x4)) & (x2 | x7) & (~x2 | ~x4 | (~x0 & x3));
  assign z44 = x2 | (~new_n156_ & x7);
  assign new_n156_ = (~x0 | (~x4 & ~x5 & ~x6)) & ~x1 & ~x3 & (x0 | x4);
  assign z45 = ~new_n158_ | (~new_n74_ & x0);
  assign new_n158_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (~x7 | (~x5 & x6 & ~x1 & ~x4));
  assign z46 = ~new_n160_ | ~x7 | x2 | x3;
  assign new_n160_ = x1 & ~x0 & (x4 | ~x5);
  assign z47 = new_n162_ | new_n163_ | ~new_n164_ | (~x2 & (x0 | x1));
  assign new_n162_ = ~x0 & ~x4 & (x5 | (x1 & x6));
  assign new_n163_ = (x0 | ~x1) & (x4 | ~x6 | ~x7);
  assign new_n164_ = (~x0 | (x1 & x3 & x5)) & (x1 | (~x2 & ~x5));
  assign z48 = x2 | (~new_n166_ & x7);
  assign new_n166_ = ~x0 & ~x1 & x3 & (x4 | (~x5 ^ x6));
  assign z49 = new_n168_ | x0 | x1 | ~x2 | (x3 & x4);
  assign new_n168_ = ~x4 & (x5 | x6);
  assign z50 = x2 | (x7 & (~new_n108_ | new_n170_));
  assign new_n170_ = x0 & (~x1 | ~x3);
  assign z51 = ((x2 | x7) & (x0 ^ x1)) | (~new_n172_ & x7) | (~new_n173_ & x2);
  assign new_n172_ = (x2 | (~x0 ^ ~x3)) & (x4 | ((x5 | ~x6) & (~x5 | x6) & (x0 | (~x5 & ~x6))));
  assign new_n173_ = (x4 | (~x5 & ~x6)) & (x0 | (x3 & ~x4));
  assign z52 = (~x0 & (~new_n175_ | (~new_n74_ & x1))) | new_n176_ | new_n74_ | (~new_n177_ & x0);
  assign new_n175_ = (~x7 | ((x2 | x3) & (x4 | (~x5 & ~x6)))) & (~x2 | ~x3 | ~x4);
  assign new_n176_ = ~x4 & (((x0 | x2) & (x5 | x6)) | (x7 & (x5 ^ x6)));
  assign new_n177_ = ~x3 & (x1 | x2);
  assign z53 = new_n179_ | (new_n181_ & ~x2) | (x2 & (new_n170_ | ~new_n180_));
  assign new_n179_ = x1 & (x0 ? (~x3 & x7) : (x2 & x3));
  assign new_n180_ = ((x3 & x5) | (x1 & (x4 | ~x6))) & ((x6 & x7) | (x1 & (x4 | ~x5))) & (x3 | x4 | ~x5) & (x1 | ~x4);
  assign new_n181_ = x7 & (x3 ? (~x1 | (~x4 & (x5 | x6))) : (x4 | ~x5 | ~x6));
  assign z54 = (x2 & (new_n184_ | ~new_n185_)) | (x7 & (new_n184_ | (~new_n183_ & ~x2)));
  assign new_n183_ = (x3 | (x1 & (x0 | x4 | (~x5 & ~x6)))) & ((~x0 & ~x3) | (~x4 & x5 & x6));
  assign new_n184_ = x0 & (~x1 | x3);
  assign new_n185_ = ((x6 & x7) | (x3 & (x4 | ~x5))) & (x3 | (~x4 & x5)) & (x1 | ~x3) & (x4 | x5 | ~x6);
  assign z55 = new_n187_ | new_n188_ | new_n189_ | ~x1;
  assign new_n187_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign new_n188_ = ~x2 & (~x7 | (x0 & ~x3));
  assign new_n189_ = x0 & (~x7 | (x2 & (x4 | ~x5 | ~x6)));
  assign z56 = (x0 & (x2 | x7)) | ~new_n191_ | (~x1 & (x2 ? x3 : x7));
  assign new_n191_ = x2 ? (~x4 & x5 & x6 & x7) : (~x7 | (x3 & (x4 | ~x5)));
  assign z57 = new_n193_ | new_n194_ | ~new_n195_ | (~x3 & (x0 | ~x1));
  assign new_n193_ = (x0 | ~x2) & (~x1 | (~x4 & x5));
  assign new_n194_ = x2 & (~x5 | ~x6 | x0 | x4);
  assign new_n195_ = x7 & (x0 | x2 | ~x3);
  assign z58 = new_n162_ | new_n163_ | ~new_n197_ | (~x2 & (x0 | x1));
  assign new_n197_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = new_n199_ | new_n200_ | (~new_n89_ & ~new_n201_) | (~x4 & x5);
  assign new_n199_ = x2 & ((x1 & (x3 | (~x4 & x6))) | (~x0 & (x1 | x3)));
  assign new_n200_ = x0 & ((~x2 & (~x1 | ~x3)) | (~x1 & (~x3 | (~x4 & x6))));
  assign new_n201_ = x1 ? x2 : x0;
  assign z60 = (~new_n203_ & (x2 | x7)) | (~x2 & x3 & x7) | (~x0 & x2 & (~x3 | ~x7));
  assign new_n203_ = x0 ? (x1 & ~x3 & x4) : (x5 & x6 & ~x1 & ~x4);
  assign z61 = x2 ? (new_n168_ | ~x0 | x1 | ~x3) : x7;
  assign z62 = new_n168_ | ~x0 | ~x1 | new_n74_ | x3;
  assign z24 = 1'b0;
  assign z25 = 1'b0;
endmodule


