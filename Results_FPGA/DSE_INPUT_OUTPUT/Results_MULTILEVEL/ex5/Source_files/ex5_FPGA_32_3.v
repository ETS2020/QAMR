// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n90_, new_n93_, new_n95_,
    new_n100_, new_n103_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n113_, new_n115_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n126_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n146_, new_n147_, new_n148_, new_n150_, new_n153_,
    new_n155_, new_n157_, new_n159_, new_n163_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  assign z00 = ~x4 & ~x5 & ~z08 & ~x6;
  assign z08 = x0 & x2;
  assign z01 = ~x5 & ~x6 & ~z08 & ~x7;
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z08 & ~x7;
  assign z03 = z08 | (x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z08 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z08 & ~x7;
  assign z06 = x2 & (x0 | (~x1 & x3 & new_n81_ & ~x4));
  assign new_n81_ = ~x5 & ~x6;
  assign z07 = (x0 & x2) | (new_n83_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n83_ = ~x4 & x5 & x6 & x7;
  assign z09 = x7 & new_n85_ & x6;
  assign new_n85_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & ~x1;
  assign z10 = x2 & (x0 | (new_n87_ & x1 & ~x4));
  assign new_n87_ = x5 & x6 & x7;
  assign z11 = x0 & (x2 | (new_n87_ & x1 & ~x3 & ~x4));
  assign z13 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x0 & (x2 | (new_n87_ & ~x1 & x3 & ~x4));
  assign z15 = x7 & new_n93_ & x6;
  assign new_n93_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z16 = x7 & new_n95_ & x6;
  assign new_n95_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = x0 & (x2 | (~x1 & x4 & ~x5));
  assign z18 = x2 & (x0 | (x4 & ~x5 & ~x1 & x3));
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n100_ & ~x3;
  assign new_n100_ = ~x2 & x0 & ~x1;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n100_ & x3;
  assign z22 = x0 & (x2 | (new_n103_ & ~x1 & ~x4));
  assign new_n103_ = ~x5 & x6 & x7;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = (x0 & x2) | (new_n108_ & new_n109_ & ~x2 & ~x3 & ~x0 & x1);
  assign new_n108_ = ~x4 & ~x5;
  assign new_n109_ = x6 & ~x7;
  assign z27 = x2 & (new_n111_ | x0);
  assign new_n111_ = x1 & ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z29 = (x0 & x2) | (new_n113_ & ~x2 & ~x3 & ~x0 & ~x1);
  assign new_n113_ = ~x4 & ~x5 & ~x6 & x7;
  assign z31 = (x0 & (x2 | (~x4 & x6))) | (~x0 & (~x4 | (~x2 & x3))) | ~new_n115_ | (x2 & (~x3 | ~x4));
  assign new_n115_ = ~x1 & (~x4 | x5) & (x4 | ~x5);
  assign z32 = (~x0 & ((~x2 & (x3 | x4)) | (~x4 & (~new_n109_ | x3)))) | ~new_n117_ | (~x3 & (x2 | (x0 & ~x4)));
  assign new_n117_ = (~x0 | (~x2 & (x4 | ~x6))) & (x4 | (~x2 & ~x5)) & ~x1 & (x2 | ~x4 | x5);
  assign z34 = new_n119_ | new_n121_ | (~new_n120_ & ~x0);
  assign new_n119_ = x1 & ((~x2 & ~x5) | (~x0 & x2 & ~x3));
  assign new_n120_ = (~x3 | (x5 & (~x2 | ~x6))) & (x3 | (~x5 & (~x2 | x6))) & (x2 | x5) & (~x2 | (~x4 & ~x7));
  assign new_n121_ = ~x2 & ((~x4 & ~x5 & (~x6 | ~x7)) | (x5 & (~x3 | x4 | x6 | x7)));
  assign z35 = ((~x0 | ~x2) & (x1 | ~x4)) | (~x0 & (x2 ? (~x3 | ~x5) : x3)) | (x0 & ~x2 & ~x5);
  assign z36 = ((~x0 | ~x2) & (x1 | x5)) | (~new_n124_ & ~x0) | (~x2 & (~x0 | ~x4));
  assign new_n124_ = ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x3 & (~x0 | (~x1 & ~x2))) | (~new_n126_ & (x2 ? ~x0 : (x3 & ~x5))) | (x5 & (~x0 | (x1 & ~x2 & x3)));
  assign new_n126_ = ~x4 & x6 & ~x7;
  assign z38 = (~x2 & (new_n129_ | x1 | (~new_n130_ & ~x0))) | (~x0 & (new_n128_ | x1));
  assign new_n128_ = x2 & (~x3 | ~x4);
  assign new_n129_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n130_ = ~x3 & ~x4 & ~x5 & x6 & ~x7;
  assign z39 = (x2 & (x0 | ~x5)) | ~new_n133_ | (~new_n132_ & x5);
  assign new_n132_ = x3 & ~x6 & ~x7;
  assign new_n133_ = ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = new_n135_ | new_n128_ | new_n136_ | x1 | (~x4 & x5);
  assign new_n135_ = x0 & (x2 | (~x4 & x6) | (x4 & ~x5));
  assign new_n136_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign z41 = ~x0 | (~x2 & (x1 ? x3 : (~x3 | ~x5)));
  assign z42 = (x2 & (x0 | ~x5)) | ~new_n133_ | (x5 & (x6 | x7));
  assign z43 = (~x0 & (new_n142_ | new_n143_)) | (~x2 & (new_n140_ | new_n141_));
  assign new_n140_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n141_ = ~x4 & ((x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n142_ = x4 & (x1 | (x2 & ~x3));
  assign new_n143_ = ~x4 & ((x2 & (~x5 | x6)) | (x5 & x6) | x7 | (~x5 & ~x6));
  assign z44 = (x0 & (~new_n81_ | x4)) | x1 | x2 | x3 | (~x0 & ~x4);
  assign z45 = new_n146_ | new_n147_ | new_n148_ | x0;
  assign new_n146_ = x1 & (~x2 | (~x4 & x6));
  assign new_n147_ = x5 & (~x1 | ~x4);
  assign new_n148_ = ~x1 & (x2 | x4 | ~x6 | ~x7);
  assign z46 = x2 ? ~x0 : (~new_n150_ | x0 | (~x4 & (new_n109_ | x5)));
  assign new_n150_ = x1 & ~x3;
  assign z47 = (~x0 & x2 & (~x1 | (~new_n81_ & ~x4))) | (~x2 & (~new_n103_ | x0 | x1 | x4));
  assign z48 = x0 ? ~x2 : (new_n153_ | x1 | x2 | ~x3);
  assign new_n153_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = new_n155_ | ~new_n128_ | x0 | x1;
  assign new_n155_ = ~x4 & (x5 | x6);
  assign z50 = x2 ? ~x0 : (~new_n157_ | ~new_n108_ | (x0 & (~x1 | ~x3)));
  assign new_n157_ = x6 & x7;
  assign z51 = (~new_n159_ & ~x0) | (~x2 & (new_n153_ | (~new_n150_ & x0)));
  assign new_n159_ = (x4 | (~x5 & ~x6)) & ~x1 & x3 & (~x2 | ~x4);
  assign z52 = (~x0 & (x1 | (~x2 & ~x3))) | (x2 & (x0 | (x3 & x4))) | (~new_n81_ & ~x4) | (x0 & (~x1 | x3));
  assign z53 = (x1 & (x0 ? (~x2 & ~x3) : (x2 & x3))) | (~new_n83_ & ((~x2 & ~x3) | (~x0 & ~x1 & x3))) | ((new_n155_ | ~x1) & ((~x0 & x2 & ~x3) | (~x2 & x3)));
  assign z54 = (~x1 & ((~x2 & ~x3) | (~x0 & x2 & x3))) | (~new_n163_ & ~x0) | (~x2 & ((~new_n83_ & (x0 | x3)) | (x0 & x3)));
  assign new_n163_ = x3 ? (x4 | (x5 ? (x6 & x7) : ~x6)) : (x2 ? (~x4 & x5 & x6 & x7) : (x4 | (~x5 & ~x6)));
  assign z55 = new_n155_ | ~x1 | (x0 & (x2 | ~x3));
  assign z56 = (~x1 & (~x2 | x3)) | ~new_n166_ | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n166_ = ~x0 & (x0 | ((x7 | (~x2 & (x4 | ~x6))) & (~x2 | (~x4 & x5 & x6))));
  assign z57 = (~x2 & (~new_n170_ | (~new_n169_ & x0))) | (~x0 & (new_n168_ | ~new_n171_));
  assign new_n168_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n169_ = x3 & (x4 | ~x6 | x7);
  assign new_n170_ = x1 & (x4 | ~x5) & (x0 | ~x3);
  assign new_n171_ = (~x2 | (~x4 & x5 & x6)) & (x1 | x3);
  assign z58 = new_n146_ | new_n147_ | new_n148_ | x0 | ~x3;
  assign z59 = (x0 & (~x1 | ~x3)) | ~new_n157_ | ~new_n108_ | (x2 & (x1 | x3));
  assign z60 = (~x2 & (x3 | (x0 & (~x1 | ~x4)))) | (~x0 & (~new_n87_ | x1 | x4 | (x2 & ~x3)));
  assign z62 = new_n155_ | x2 | x3 | ~x0 | ~x1;
  assign z12 = 1'b0;
  assign z26 = 1'b0;
  assign z28 = 1'b0;
  assign z33 = 1'b1;
  assign z61 = 1'b1;
  assign z30 = z08;
endmodule


