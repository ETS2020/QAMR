// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n83_, new_n85_, new_n87_, new_n90_, new_n96_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n111_, new_n113_, new_n114_,
    new_n116_, new_n118_, new_n119_, new_n123_, new_n125_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n145_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n153_, new_n156_,
    new_n158_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n169_, new_n171_, new_n172_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = ~x1 & x2;
  assign z01 = ~x5 & ~x6 & ~z06 & ~x7;
  assign z02 = z06 | (~x3 & ~x4 & x5 & ~x6 & ~x7);
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z06 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z06 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (~x1 & x2) | (new_n81_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n81_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z10 = x2 & (~x1 | (new_n85_ & ~x0 & ~x4));
  assign new_n85_ = x5 & x6 & x7;
  assign z11 = x7 & new_n87_ & x6;
  assign new_n87_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z13 = (~x1 & x2) | (new_n81_ & ~x0 & x1 & ~x2 & x3);
  assign z14 = x7 & x6 & x5 & ~x4 & new_n90_ & x3;
  assign new_n90_ = ~x2 & x0 & ~x1;
  assign z15 = x2 & (~x1 | (new_n85_ & ~x0 & x3 & ~x4));
  assign z16 = (~x1 & x2) | (new_n81_ & x0 & x1 & ~x2 & x3);
  assign z17 = ~x5 & new_n90_ & x4;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x1 & (x2 | (x0 & ~x3 & new_n96_ & ~x4));
  assign new_n96_ = ~x5 & ~x6;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n90_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n90_ & ~x4;
  assign z23 = ~x1 & (x2 | (~x0 & x3 & x5));
  assign z24 = ~x1 & (x2 | (new_n101_ & ~x5 & x6 & ~x7));
  assign new_n101_ = ~x0 & ~x3 & ~x4;
  assign z25 = ~x7 & new_n103_ & x6;
  assign new_n103_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z26 = x2 & (new_n105_ | ~x1);
  assign new_n105_ = x0 & ~x3 & ~x4 & ~x5 & x6 & x7;
  assign z27 = ~x7 & new_n107_ & x6;
  assign new_n107_ = ~x5 & ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z29 = ~x1 & (x2 | (new_n101_ & ~x5 & ~x6 & x7));
  assign z30 = x7 & x6 & new_n83_ & ~x5;
  assign z31 = new_n111_ | x1;
  assign new_n111_ = ~x2 & ((~x0 & (x3 | ~x4)) | (~x4 & (x5 | (x0 & x6))) | (x4 & ~x5));
  assign z32 = ~new_n114_ | (~new_n113_ & ~x4);
  assign new_n113_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n114_ = (x0 | (~x4 & (x2 | ~x3))) & ~x1 & ~x2 & (x2 | ~x4 | x5);
  assign z33 = ~new_n116_ | ~x7 | x4 | ~x6;
  assign new_n116_ = x1 & x2 & x0 & (~x3 | x5);
  assign z34 = (~new_n118_ & (x2 ? x1 : x5)) | (~x5 & (x1 | (~new_n119_ & ~x2)));
  assign new_n118_ = x3 & ~x4 & ~x6 & ~x7;
  assign new_n119_ = x0 & (x4 | (x6 & x7));
  assign z35 = x1 | (~x2 & ((~x0 & x3) | ~x4 | (x0 & ~x5)));
  assign z36 = ~new_n90_ | ~x4 | x5;
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (~x1 & (x2 | ~x3)) | (x3 & (x5 ? x1 : ~new_n123_));
  assign new_n123_ = ~x4 & x6 & ~x7;
  assign z38 = x1 | (~new_n125_ & ~x2);
  assign new_n125_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x3 & ~x4 & ~x5 & x6 & ~x7));
  assign z39 = (~new_n127_ & (x1 | (~x2 & x5))) | (x1 & (x4 | ~x5)) | (~x2 & (x4 | (~new_n128_ & ~x5)));
  assign new_n127_ = x3 & ~x6 & ~x7;
  assign new_n128_ = x0 & x6 & x7;
  assign z40 = (~new_n105_ & x1) | (~x2 & (new_n131_ | new_n132_ | new_n130_ | x1));
  assign new_n130_ = ~x4 & x5;
  assign new_n131_ = x0 & (x4 ? ~x5 : x6);
  assign new_n132_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign z41 = (~x0 & (x1 | ~x2)) | (x1 & (x2 | x3)) | (~x1 & ~x2 & (~x3 | ~x5));
  assign z42 = ~new_n135_ | (~z06 & x4);
  assign new_n135_ = ((~x6 & ~x7) | (~x1 & (x2 | ~x5))) & (x5 | (~x1 & (x2 | (x0 & x6 & x7))));
  assign z43 = new_n137_ | (~x5 & (new_n138_ | ~new_n139_)) | new_n141_ | (new_n140_ & x5);
  assign new_n137_ = x4 & (x1 | (~x0 & ~x2 & x3));
  assign new_n138_ = x3 & (x1 | (~x0 & ~x2));
  assign new_n139_ = (x0 | (~x1 & (x2 | x4 | x6))) & (~x1 | (x2 & x6 & x7));
  assign new_n140_ = (x6 | x7) & (x1 | (~x2 & ~x4));
  assign new_n141_ = ~x1 & (x2 | (~x2 & ~x4 & (x0 ? (x6 & ~x7) : x7)));
  assign z44 = x1 | (~new_n143_ & ~x2);
  assign new_n143_ = (~x4 | (~x0 & (x0 | ~x3))) & (x0 | (x4 & (~x3 | x5))) & (~x5 | (~x0 & (x4 | (~x6 & ~x7)))) & (~x0 | (~x3 & ~x6));
  assign z45 = (x1 & (~x2 | (~x4 & x6))) | ~new_n145_ | (x5 & (~x1 | ~x4));
  assign new_n145_ = ~x0 & (x1 | (x6 & x7 & ~x2 & ~x4));
  assign z46 = new_n147_ | x0 | ~x1 | x2 | x3;
  assign new_n147_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x1 & (new_n149_ | new_n150_ | ~x2)) | (~new_n151_ & ~x2);
  assign new_n149_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n150_ = x0 & (~x3 | x4 | ~x5 | ~x6 | ~x7);
  assign new_n151_ = ~x0 & ~x4 & ~x5 & x6 & x7;
  assign z48 = x1 | (~new_n153_ & ~x2);
  assign new_n153_ = ~x0 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (x1 & x2);
  assign z50 = x2 ? x1 : (~new_n156_ | (x0 & (~x1 | ~x3)));
  assign new_n156_ = x6 & x7 & ~x4 & ~x5;
  assign z51 = new_n158_ | (x1 & (~x0 | (~x2 & x3))) | (~x1 & ~x2 & (x0 | ~x3));
  assign new_n158_ = ~x4 & (((x5 | x6) & (~x1 ^ x2)) | (x1 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n96_ & (x1 | (~x1 & ~x2))) | (~x1 & ~x2 & (x0 | ~x3)) | (x1 & (~x0 | x3));
  assign z53 = (~x3 & (~new_n161_ | (x0 & (x1 | x2)))) | (~x1 & (x2 | x3)) | (x3 & (new_n162_ | (~x0 & x2)));
  assign new_n161_ = x2 ? (x4 | (~x5 & ~x6)) : (~x4 & x5 & x6 & x7);
  assign new_n162_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = ((new_n164_ | ~new_n165_) & ~x2) | (x1 & (new_n166_ | ~new_n167_));
  assign new_n164_ = ~x3 & (~x1 | (~x0 & ~x4 & (x5 | x6)));
  assign new_n165_ = (~x0 | ~x3) & ((~x0 & ~x3) | (~x4 & x5 & x6 & x7));
  assign new_n166_ = (~x6 | ~x7) & (x3 ? (~x4 & x5) : x2);
  assign new_n167_ = (~x2 | x3 | (~x4 & x5)) & (~x3 | (~x0 & (x4 | x5 | ~x6)));
  assign z55 = new_n169_ | (~x2 & (~x1 | (x0 & ~x3))) | (x0 & x1 & ~new_n81_ & x2);
  assign new_n169_ = ~x4 & (x5 | x6) & (~x2 | (~x0 & x1));
  assign z56 = (~new_n171_ & ~x2) | ~new_n172_ | (~x7 & (x2 | (~x4 & x6)));
  assign new_n171_ = x3 & (x4 | ~x5);
  assign new_n172_ = ~x0 & x1 & (~x2 | (~x4 & x5 & x6));
  assign z57 = (x0 & (x2 ? x1 : ~x3)) | (x1 & ~new_n81_ & x2) | (~x2 & (new_n147_ | ~x1 | (~x0 & x3)));
  assign z58 = (new_n175_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n177_ | (~new_n176_ & (x0 | ~x1));
  assign new_n175_ = ~x4 & (x5 | (x1 & x6));
  assign new_n176_ = ~x4 & x6 & x7;
  assign new_n177_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (~new_n181_ & x1) | ((new_n179_ | ~new_n180_) & ~x2);
  assign new_n179_ = x0 & (~x1 | (x1 & ~x3 & ~x4));
  assign new_n180_ = ((x6 & x7) | (x1 & (~x1 | x4))) & ~x4 & (x1 | ~x5);
  assign new_n181_ = ((~x2 & ~x4) | (x0 & ~x3)) & (x4 | (~x5 & (~x2 | ~x6)));
  assign z60 = x1 ? (~x4 | ((~x0 | x3) & (x2 | x4))) : (~x2 & (~new_n85_ | x0 | x3 | x4));
  assign z61 = x1 | (~x1 & ~x2);
  assign z62 = x1 ? ((~new_n96_ & ~x4) | ~x0 | x3) : ~x2;
  assign z09 = 1'b0;
  assign z12 = 1'b0;
  assign z18 = 1'b0;
  assign z28 = z06;
endmodule


