// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n81_, new_n84_, new_n86_, new_n88_, new_n91_, new_n95_, new_n96_,
    new_n98_, new_n100_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n123_, new_n124_, new_n128_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n150_, new_n152_,
    new_n154_, new_n157_, new_n160_, new_n162_, new_n164_, new_n165_,
    new_n170_, new_n172_;
  assign z00 = ~x4 & ~x5 & ~z15 & ~x6;
  assign z15 = ~x0 & x2;
  assign z01 = z15 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & ~x4 & x5 & ~x6 & ~z15 & ~x7;
  assign z03 = x3 & ~x4 & x5 & ~x6 & ~z15 & ~x7;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z15 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z15 & ~x7;
  assign z07 = ~x0 & (new_n81_ | x2);
  assign new_n81_ = x1 & ~x3 & ~x4 & x5 & x6 & x7;
  assign z08 = x2 & (new_n81_ | ~x0);
  assign z11 = (~x0 & x2) | (new_n84_ & x0 & x1 & ~x2 & ~x3);
  assign new_n84_ = ~x4 & x5 & x6 & x7;
  assign z12 = x7 & new_n86_ & x6;
  assign new_n86_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = (~x0 & x2) | (new_n84_ & x0 & ~x1 & ~x2 & x3);
  assign z16 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = (~x0 & x2) | (x0 & ~x1 & ~x2 & x4 & ~x5);
  assign z19 = ~x0 & (x2 | (~x1 & ~x3 & x4));
  assign z20 = (~x0 & x2) | (new_n95_ & new_n96_ & x0 & ~x1 & ~x2);
  assign new_n95_ = ~x3 & ~x4;
  assign new_n96_ = ~x5 & ~x6;
  assign z21 = new_n98_ & ~x6;
  assign new_n98_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = (~x0 & x2) | (new_n100_ & x0 & ~x1 & ~x2);
  assign new_n100_ = x6 & x7 & ~x4 & ~x5;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n103_ & x6;
  assign new_n103_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x0 & (x2 | (new_n95_ & x1 & new_n105_ & ~x5));
  assign new_n105_ = x6 & ~x7;
  assign z26 = x2 & (~x0 | (new_n95_ & new_n107_ & ~x5));
  assign new_n107_ = x6 & x7;
  assign z28 = x2 & (~x0 | (new_n107_ & ~x5 & new_n109_ & ~x1));
  assign new_n109_ = x3 & ~x4;
  assign z29 = ~x0 & (new_n111_ | x2);
  assign new_n111_ = ~x1 & ~x3 & ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x7 & new_n113_ & x6;
  assign new_n113_ = ~x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z31 = ~new_n115_ | (~x0 & (x3 | ~x4));
  assign new_n115_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = new_n118_ | (x4 & (~x0 | ~x5)) | ~new_n117_ | (~new_n119_ & ~x0);
  assign new_n117_ = ~x1 & ~x2;
  assign new_n118_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n119_ = ~x3 & ~x5 & x6 & ~x7;
  assign z33 = ~x2 | (~new_n121_ & x0);
  assign new_n121_ = ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | ~x3 | x5);
  assign z34 = (~new_n123_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n124_ : ~x2));
  assign new_n123_ = ~x6 & ~x7 & x3 & ~x4;
  assign new_n124_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = x2 | ~x4 | x1 | (x0 & ~x5) | (~x0 & x3);
  assign z36 = ~x4 | x5 | x2 | ~x0 | x1;
  assign z37 = ((~x3 | x5) & (x0 ^ ~x2)) | (~new_n128_ & (x0 ? (x3 & ~x5) : ~x2)) | (x0 & (x1 ? (x3 & x5) : ~x3));
  assign new_n128_ = ~x4 & x6 & ~x7;
  assign z38 = new_n118_ | ~new_n117_ | (~x0 & (~new_n95_ | ~new_n105_ | x5));
  assign z39 = (~x0 & (x2 | ~x5)) | ~new_n132_ | (~new_n131_ & x5);
  assign new_n131_ = x3 & ~x6 & ~x7;
  assign new_n132_ = ~x4 & (x5 | (x6 & x7 & ~x1 & ~x2));
  assign z40 = (x3 & (x0 ^ ~x2)) | (~new_n134_ & ~x2) | (~new_n135_ & x0);
  assign new_n134_ = ~x1 & (x4 | ((x0 | (x6 & ~x7)) & ~x5 & (~x0 | ~x6)));
  assign new_n135_ = (~x4 | (~x2 & x5)) & (~x5 | (~x2 & x4)) & (~x2 | (x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = new_n138_ | new_n139_ | (~z15 & (x4 | (x1 & ~x5)));
  assign new_n138_ = ~x0 & ~x2 & (~x5 | (~x4 & (x7 | (x5 & x6))));
  assign new_n139_ = x0 & (x5 ? (x6 | x7) : (~x6 | ~x7 | (x2 & ~x3)));
  assign z43 = (~x5 & (new_n141_ | new_n142_)) | new_n143_ | new_n144_ | new_n145_;
  assign new_n141_ = ~x2 & (x1 | (~x0 & (x3 | (~x4 & ~x6))));
  assign new_n142_ = x0 & ((x1 & x3) | (x2 & (~x6 | ~x7)));
  assign new_n143_ = (x0 ? x5 : ~x2) & (x4 ? x1 : x7);
  assign new_n144_ = ~x0 & ~x2 & (x4 ? x3 : (x5 & x6));
  assign new_n145_ = x0 & (x4 ? x2 : (x6 & (x5 | ~x7)));
  assign z44 = ~new_n147_ | (x3 & (x0 | (~x2 & x4)));
  assign new_n147_ = (x2 | (~x1 & (x4 | (x0 & ~x5)))) & (~x0 | (~x2 & ~x4 & ~x5 & ~x6));
  assign z45 = x0 | (~x2 & (x1 | x4 | (~x4 & (x5 | (~new_n107_ & ~x0)))));
  assign z46 = new_n150_ | x2 | x3 | x0 | ~x1;
  assign new_n150_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~x0 & (x1 | x2 | x5)) | ~new_n152_ | (x0 & (~x3 | ~x5 | ~x1 | ~x2));
  assign new_n152_ = ~x4 & x6 & x7;
  assign z48 = x0 | (~new_n154_ & ~x2);
  assign new_n154_ = ~x1 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z50 = (~new_n100_ & ~x2) | (x0 & (~x1 | x2 | ~x3));
  assign z51 = new_n157_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n157_ = ~x4 & (((x5 | x6) & (~x0 ^ x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n96_ & (x0 | (~x0 & ~x2))) | (~x0 & ~x2 & (x1 | ~x3)) | (x0 & ((~x1 & ~x2) | x3));
  assign z53 = x3 ? (new_n160_ | (~x1 & (x0 | ~x2))) : ((~new_n84_ & ~x2) | (x0 & (x1 | x2)));
  assign new_n160_ = ~x4 & ((x0 & (x5 ? (~x6 | ~x7) : x6)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n81_ & x0) | (~x2 & (x3 ? ~new_n84_ : ~new_n162_));
  assign new_n162_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign z55 = (~new_n164_ & ~x2) | ~x1 | (~new_n165_ & x2);
  assign new_n164_ = (~x0 | x3) & (x4 | (~x5 & ~x6));
  assign new_n165_ = x0 & ~x4 & x5 & x6 & x7;
  assign z56 = x0 | (~x2 & (new_n150_ | ~x1 | ~x3));
  assign z57 = new_n150_ | ~x1 | x2 | (x0 & ~x3) | (~x0 & x3);
  assign z58 = ((~new_n107_ | ~new_n109_) & (x0 | ~x2)) | (x0 & (~x1 | ~x2 | ~x5)) | (~x2 & (x1 | x5));
  assign z59 = (~x2 & (~new_n100_ | (x0 & (~x1 | ~x3)))) | new_n170_ | (x2 & (~x0 | (x1 & x3) | (~x1 & ~x3)));
  assign new_n170_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & ~x2 & (~new_n172_ | x1 | x4));
  assign new_n172_ = x5 & x6 & x7;
  assign z61 = (~x1 & (~x2 | (x0 & ~x3))) | (~x0 & ~x2) | (x0 & (x1 | (~new_n96_ & ~x4)));
  assign z62 = (~x1 & (x0 | ~x2)) | (~x0 & ~x2) | (x0 & ((~new_n96_ & ~x4) | (x1 & x3)));
  assign z06 = 1'b0;
  assign z09 = 1'b0;
  assign z10 = 1'b0;
  assign z18 = 1'b0;
  assign z49 = 1'b1;
  assign z27 = z15;
endmodule


