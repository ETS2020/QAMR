// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n77_, new_n80_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n93_, new_n99_, new_n101_, new_n104_, new_n107_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n117_, new_n119_, new_n121_,
    new_n125_, new_n127_, new_n129_, new_n131_, new_n132_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n145_, new_n147_, new_n149_, new_n151_, new_n155_, new_n158_,
    new_n160_, new_n162_, new_n163_, new_n165_, new_n169_;
  assign z00 = z06 | (new_n75_ & ~x4);
  assign z06 = ~x0 & x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z06 | (new_n77_ & ~x5);
  assign new_n77_ = ~x6 & ~x7;
  assign z02 = z06 | (~x3 & ~x4 & new_n77_ & x5);
  assign z03 = z06 | (new_n80_ & new_n77_ & x5);
  assign new_n80_ = x3 & ~x4;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z06 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x2 & (new_n86_ | ~x0);
  assign new_n86_ = x1 & ~x3 & ~x4 & x5 & x6 & x7;
  assign z11 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & ~x3 & ~x2 & x0 & x1;
  assign z12 = x2 & (~x0 | (new_n90_ & ~x1 & ~x3 & ~x4));
  assign new_n90_ = x5 & x6 & x7;
  assign z13 = ~x0 & (x2 | (new_n90_ & new_n80_ & x1));
  assign z14 = (~x0 & x2) | (new_n93_ & x0 & ~x1 & ~x2 & x3);
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z16 = (~x0 & x2) | (new_n93_ & x0 & x1 & ~x2 & x3);
  assign z17 = ~x5 & x4 & ~x2 & x0 & ~x1;
  assign z19 = ~x0 & (x2 | (~x1 & ~x3 & x4));
  assign z20 = (~x0 & x2) | (x0 & ~x1 & ~x2 & new_n75_ & ~x3 & ~x4);
  assign z21 = new_n99_ & ~x6;
  assign new_n99_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = (~x0 & x2) | (new_n101_ & x0 & ~x1 & ~x2);
  assign new_n101_ = ~x4 & ~x5 & x6 & x7;
  assign z23 = ~x0 & (x2 | (~x1 & x3 & x5));
  assign z24 = ~x7 & new_n104_ & x6;
  assign new_n104_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n84_ & ~x5;
  assign z26 = x7 & x6 & ~x5 & ~x4 & new_n107_ & ~x3;
  assign new_n107_ = x0 & x2;
  assign z28 = x2 & (~x0 | (new_n80_ & ~x1 & new_n109_ & ~x5));
  assign new_n109_ = x6 & x7;
  assign z29 = ~x0 & (new_n111_ | x2);
  assign new_n111_ = ~x1 & ~x3 & ~x4 & ~x5 & ~x6 & x7;
  assign z30 = x2 & (new_n113_ | ~x0);
  assign new_n113_ = x1 & ~x3 & ~x4 & ~x5 & x6 & x7;
  assign z31 = (~x0 & (x3 | ~x4)) | ~new_n115_ | (~x4 & (x5 | (x0 & x6)));
  assign new_n115_ = ~x1 & ~x2 & (~x4 | x5);
  assign z32 = (~new_n117_ & ~x4) | ~new_n115_ | (~x0 & (x3 | x4));
  assign new_n117_ = (~x0 | (x3 & ~x6)) & ~x5 & (x0 | (x6 & ~x7));
  assign z33 = ~new_n119_ | ~x7 | x4 | ~x6;
  assign new_n119_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = ((~new_n77_ | ~new_n80_) & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n121_ : ~x2));
  assign new_n121_ = ~x1 & ~x2 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (~x2 & (x1 | ~x4 | (~x0 & x3)));
  assign z36 = x2 ? x0 : (~x0 | x1 | ~x4 | x5);
  assign z37 = ((~x3 | x5) & (~x0 ^ x2)) | (~new_n125_ & (x0 ? (x3 & ~x5) : ~x2)) | (x0 & (x1 ? (x3 & x5) : ~x3));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (x0 | ~x2)) | ~new_n127_ | (x5 & (x0 ? ~x4 : ~x2));
  assign new_n127_ = x0 ? (~x2 & (x4 | (x3 & ~x6))) : (x2 | (~x3 & ~x4 & x6 & ~x7));
  assign z39 = (~x0 & (x2 | ~x5)) | ~new_n129_ | (x5 & (~new_n77_ | ~x3));
  assign new_n129_ = ~x4 & (x5 | (~x1 & ~x2 & x6 & x7));
  assign z40 = (~new_n131_ & x0) | (x1 & (~x0 | ~x2)) | ~new_n132_ | (x3 & (~x0 | x2));
  assign new_n131_ = (~x4 | x5) & (x2 | x4 | ~x6);
  assign new_n132_ = (~x5 | (~x2 & x4)) & (x0 | (~x2 & (x4 | (x6 & ~x7)))) & (~x2 | (~x4 & x6 & x7));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (x0 | ~x2)) | (~new_n77_ & (x0 ? x5 : ~x2)) | (~x5 & (x0 ? ~new_n135_ : ~x2));
  assign new_n135_ = ~x1 & (~x2 | x3) & x6 & x7;
  assign z43 = ((new_n137_ | new_n138_) & ~x5) | new_n139_ | new_n140_ | new_n141_;
  assign new_n137_ = ~x2 & (x1 | (~x0 & (x3 | (~x4 & ~x6))));
  assign new_n138_ = x0 & ((x1 & x3) | (x2 & (~x6 | ~x7)));
  assign new_n139_ = (x0 ? x5 : ~x2) & (x4 ? x1 : x7);
  assign new_n140_ = ~x2 & ((~x4 & x5 & x6) | (~x0 & x3 & x4));
  assign new_n141_ = x0 & (x4 ? x2 : (x6 & (x5 | ~x7)));
  assign z44 = (~new_n143_ & ~x2) | (x0 & (~new_n75_ | x3 | x4)) | x2 | (~x0 & ~x4);
  assign new_n143_ = (~x1 | (x5 & (x0 | ~x4))) & (x4 | ~x5 | ~x6) & (x0 | ~x3 | (~x4 & x5));
  assign z45 = ~new_n145_ | ~x7 | x5 | ~x6;
  assign new_n145_ = ~x0 & ~x1 & ~x2 & ~x4;
  assign z46 = x0 | (~x2 & (new_n147_ | ~x1 | x3));
  assign new_n147_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n149_ & (x0 | ~x2)) | (~x2 & (x0 | x1 | x5)) | (x0 & (~x1 | ~x3 | ~x5));
  assign new_n149_ = ~x4 & x6 & x7;
  assign z48 = x0 | (~new_n151_ & ~x2);
  assign new_n151_ = ~x1 & x3 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (x0 & (~x1 | x2 | ~x3));
  assign z50 = (x0 & (~x1 | x2 | ~x3)) | (~new_n101_ & ~x2);
  assign z51 = new_n155_ | (x0 & (~x1 | (~x2 & x3))) | (~x0 & ~x2 & (x1 | ~x3));
  assign new_n155_ = ~x4 & (((x5 | x6) & (x0 ^ ~x2)) | (x0 & (x5 ? (~x6 | ~x7) : x6)));
  assign z52 = (~x4 & ~new_n75_ & (x0 | (~x0 & ~x2))) | (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x2 | (~x2 & (x1 | ~x3))));
  assign z53 = x3 ? (new_n158_ | (~x1 & (x0 | ~x2))) : ((~new_n93_ & ~x2) | (x0 & (x1 | x2)));
  assign new_n158_ = ~x4 & ((x0 & (x5 ? (~x6 | ~x7) : x6)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n86_ & x0) | (~x2 & (x3 ? ~new_n93_ : ~new_n160_));
  assign new_n160_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign z55 = new_n162_ | (~new_n163_ & x2) | ~x1 | (x0 & ~x2 & ~x3);
  assign new_n162_ = ~x4 & ((x6 & ~x7) | (~x2 & (x5 | x6)));
  assign new_n163_ = x0 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n165_ | x2 | ~x3 | x0 | ~x1;
  assign new_n165_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign z57 = new_n147_ | ~x1 | x2 | (~x0 & x3) | (x0 & ~x3);
  assign z58 = (x0 & (~x1 | ~x2 | ~x5)) | ~new_n80_ | ~new_n109_ | (~x0 & (x1 | x2 | x5));
  assign z59 = (~new_n169_ & x0) | (~new_n101_ & ~x2);
  assign new_n169_ = (~x2 | ((~x1 | ~x3) & (x4 | ~x6))) & (x2 | (x1 & x3)) & (x4 | ~x5) & (x1 | (x3 & (x4 | ~x6)));
  assign z60 = (x0 & (~x1 | ~x4)) | x3 | (~x0 & (~new_n90_ | x1 | x2 | x4));
  assign z61 = (~x1 & (~x2 | ~x3)) | ~x0 | x1 | (~new_n75_ & ~x4);
  assign z62 = (~new_n75_ & ~x4) | ~x0 | ~x1 | (x1 & x3);
  assign z18 = 1'b0;
  assign z09 = z06;
  assign z10 = z06;
  assign z15 = z06;
  assign z27 = z06;
endmodule


