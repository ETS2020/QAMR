// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n82_, new_n84_, new_n86_, new_n88_, new_n91_, new_n93_,
    new_n95_, new_n99_, new_n105_, new_n108_, new_n110_, new_n112_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n130_, new_n132_,
    new_n134_, new_n136_, new_n138_, new_n139_, new_n140_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n161_, new_n164_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n176_, new_n178_;
  assign z00 = ~x4 & ~x5 & ~z06 & ~x6;
  assign z06 = x2 & x3;
  assign z01 = z06 | (new_n76_ & ~x5);
  assign new_n76_ = ~x6 & ~x7;
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = x3 & (x2 | (new_n76_ & ~x4 & x5));
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = ~x4 & x5 & x6 & ~z06 & ~x7;
  assign z07 = (x2 & x3) | (new_n82_ & ~x0 & x1 & ~x2 & ~x3);
  assign new_n82_ = ~x4 & x5 & x6 & x7;
  assign z08 = x7 & new_n84_ & x6;
  assign new_n84_ = x5 & ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x2 & (x3 | (new_n86_ & ~x0 & ~x1 & ~x4));
  assign new_n86_ = ~x5 & x6 & x7;
  assign z10 = x2 & (x3 | (new_n88_ & ~x0 & x1 & ~x4));
  assign new_n88_ = x5 & x6 & x7;
  assign z11 = (x2 & x3) | (new_n82_ & ~x2 & ~x3 & x0 & x1);
  assign z12 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & x6 & x5 & ~x4 & new_n93_ & x3;
  assign new_n93_ = ~x2 & ~x0 & x1;
  assign z14 = x7 & x6 & x5 & ~x4 & new_n95_ & x3;
  assign new_n95_ = ~x2 & x0 & ~x1;
  assign z16 = x3 & (x2 | (new_n88_ & x0 & x1 & ~x4));
  assign z17 = ~x5 & new_n95_ & x4;
  assign z19 = x4 & ~x3 & new_n99_ & ~x2;
  assign new_n99_ = ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & new_n95_ & ~x3;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n95_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n95_ & ~x4;
  assign z23 = x5 & x3 & new_n99_ & ~x2;
  assign z24 = (x2 & x3) | (new_n99_ & ~x2 & ~x3 & new_n105_ & ~x4 & ~x5);
  assign new_n105_ = x6 & ~x7;
  assign z25 = ~x7 & x6 & ~x5 & ~x4 & new_n93_ & ~x3;
  assign z26 = ~new_n108_ & x2;
  assign new_n108_ = ~x3 & (x5 | ~x6 | ~x7 | ~x0 | x4);
  assign z27 = x2 & (x3 | (new_n110_ & ~x0 & x1 & ~x4));
  assign new_n110_ = ~x5 & x6 & ~x7;
  assign z29 = x7 & new_n112_ & ~x6;
  assign new_n112_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z30 = x2 & (x3 | (new_n86_ & x0 & x1 & ~x4));
  assign z31 = ~new_n115_ | (~x0 & (x3 | ~x4));
  assign new_n115_ = (x4 | (~x5 & ~x6)) & ~x1 & ~x2 & (~x4 | x5);
  assign z32 = x2 ? ~x3 : (new_n117_ | new_n118_ | ~new_n119_);
  assign new_n117_ = ~x4 & (x5 | (x0 & (~x3 | x6)));
  assign new_n118_ = x4 & (~x0 | ~x5);
  assign new_n119_ = ~x1 & (x0 | (~x3 & ~x5 & x6 & ~x7));
  assign z33 = ~x2 | (x2 & ~x3 & (~new_n122_ | ~new_n121_ | (~x1 & x5)));
  assign new_n121_ = x6 & x7;
  assign new_n122_ = x0 & ~x4;
  assign z34 = new_n124_ | new_n125_ | new_n126_ | new_n127_;
  assign new_n124_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n125_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n126_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n127_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = x2 ? ~x3 : (x1 | (~x0 & x3) | ~x4 | (x0 & ~x5));
  assign z36 = (x0 & (x2 | ~x4)) | (~new_n130_ & ~x0) | x1 | x5;
  assign new_n130_ = x2 & ~x3 & ~x4 & x6 & ~x7;
  assign z37 = (~x0 & (~x3 | x5)) | (x3 & (x5 ? x1 : ~new_n132_)) | x2 | (~x1 & ~x3);
  assign new_n132_ = ~x4 & x6 & ~x7;
  assign z38 = new_n117_ | new_n134_ | x1 | x2;
  assign new_n134_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~x3 & (x2 | x5)) | (~x2 & ((~new_n76_ & x5) | x4 | (~new_n136_ & ~x5)));
  assign new_n136_ = x6 & x7 & x0 & ~x1;
  assign z40 = x2 ? new_n108_ : (new_n138_ | new_n139_ | ~new_n140_);
  assign new_n138_ = x0 & (x4 ? ~x5 : x6);
  assign new_n139_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n140_ = ~x1 & (x4 | ~x5);
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (~x2 | (~x3 & x5))) | (x5 & ~new_n76_ & (~x2 | ~x3)) | (~x5 & (x2 ? ~x3 : ~new_n136_));
  assign z43 = (x1 & (new_n144_ | x4)) | (~new_n145_ & ~x4) | ~new_n147_ | (~new_n146_ & x4);
  assign new_n144_ = ~x2 & ~x5;
  assign new_n145_ = (x0 | ((x5 | x6) & (x2 | ~x7))) & (~x5 | (~x6 & ~x7)) & (~x0 | x2 | ~x6 | x7);
  assign new_n146_ = ~x2 & (x0 | ~x3);
  assign new_n147_ = (x5 | ((x0 | (~x2 & ~x3)) & (~x2 | (x6 & x7)))) & (~x2 | (~x3 & (~x5 | (~x6 & ~x7))));
  assign z44 = x2 ? ~x3 : ((x0 & (~new_n149_ | x4)) | x1 | x3 | (~x0 & ~x4));
  assign new_n149_ = ~x5 & ~x6;
  assign z45 = new_n151_ | (x1 & (~x2 | x3)) | x0 | (~new_n152_ & ~x1);
  assign new_n151_ = ~x4 & (x5 | (x1 & x6));
  assign new_n152_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = new_n154_ | x0 | ~x1 | x2 | x3;
  assign new_n154_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x0 & (~x2 | ~x3)) | (~new_n156_ & ~x3) | (~new_n157_ & ~x2);
  assign new_n156_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign new_n157_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = ~x3 | (~new_n159_ & ~x2);
  assign new_n159_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (~x3 & (~new_n99_ | new_n161_));
  assign new_n161_ = ~x4 & (x5 | x6);
  assign z50 = ~new_n152_ | (x0 & (~x1 | ~x3));
  assign z51 = (~x2 & (~new_n164_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | ~x1 | (new_n161_ & x2)));
  assign new_n164_ = (x0 | ~x1) & (x4 | ((x5 | ~x6) & (~x5 | (x6 & x7)) & (x0 | (~x5 & ~x6))));
  assign z52 = (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x1 | (~x2 & ~x3))) | new_n161_ | (x2 & x3);
  assign z53 = (x0 & (x2 | (x1 & ~x3))) | ((x2 | x3) & (new_n161_ | ~x1)) | (x2 & x3) | (~x2 & ~new_n82_ & ~x3);
  assign z54 = (~x2 & ~x3 & (~x1 | (new_n161_ & ~x0))) | (~new_n82_ & (x0 | x2 | x3)) | (x2 & x3) | (x0 & (~x1 | x3));
  assign z55 = new_n169_ | ~x1 | (x2 & x3) | (x0 & ~new_n170_ & ~x3);
  assign new_n169_ = ~x4 & (x5 | x6) & (~x0 | x3);
  assign new_n170_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = new_n172_ | (~x2 & (~x1 | ~x3)) | x0 | (~new_n173_ & x2);
  assign new_n172_ = ~x4 & ((x6 & ~x7) | (~x2 & x5));
  assign new_n173_ = ~x3 & ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | ~x3)) | (~x2 & (new_n154_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n82_ & x2)));
  assign z58 = ~new_n176_ | ~new_n99_ | x2 | ~x3;
  assign new_n176_ = ~x4 & ~x5 & x6 & x7;
  assign z59 = (~new_n176_ & (~x0 | ~x2)) | new_n178_ | (x0 & (~x1 | (~x2 & ~x3))) | (x2 & (x3 | (~x0 & x1)));
  assign new_n178_ = ~x4 & (x5 | (x0 & x2 & x6));
  assign z60 = x3 ? ~x2 : (x0 ? (~x1 | ~x4) : (~new_n88_ | x1 | x2 | x4));
  assign z62 = ~x0 | x3 | (x0 & ~x3 & (new_n161_ | ~x1));
  assign z18 = 1'b0;
  assign z28 = 1'b0;
  assign z61 = 1'b1;
  assign z15 = z06;
endmodule


