// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n77_, new_n79_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n95_, new_n101_, new_n105_, new_n108_, new_n111_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n119_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n129_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n148_, new_n150_,
    new_n155_, new_n158_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n165_, new_n168_, new_n171_, new_n172_, new_n173_;
  assign z00 = ~x4 & ~x5 & ~z07 & ~x6;
  assign z07 = ~x0 & ~x2;
  assign z01 = ~x5 & ~x6 & ~z07 & ~x7;
  assign z02 = z07 | (~x3 & ~x4 & new_n77_ & x5);
  assign new_n77_ = ~x6 & ~x7;
  assign z03 = z07 | (new_n79_ & new_n77_ & x5);
  assign new_n79_ = x3 & ~x4;
  assign z04 = x3 & ~x4 & ~x5 & x6 & ~z07 & ~x7;
  assign z05 = ~x4 & x5 & x6 & ~z07 & ~x7;
  assign z06 = new_n83_ & ~x6;
  assign new_n83_ = ~x5 & ~x4 & x3 & x2 & ~x0 & ~x1;
  assign z08 = x7 & x6 & new_n85_ & x5;
  assign new_n85_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = ~x0 & (new_n87_ | ~x2);
  assign new_n87_ = ~x1 & ~x3 & ~x4 & ~x5 & x6 & x7;
  assign z10 = x7 & x6 & x5 & new_n89_ & ~x4;
  assign new_n89_ = x2 & ~x0 & x1;
  assign z11 = x7 & x6 & x5 & ~x4 & new_n91_ & ~x3;
  assign new_n91_ = ~x2 & x0 & x1;
  assign z12 = (~x0 & ~x2) | (new_n93_ & x0 & ~x1 & x2 & ~x3);
  assign new_n93_ = ~x4 & x5 & x6 & x7;
  assign z14 = x7 & x6 & x5 & ~x4 & new_n95_ & x3;
  assign new_n95_ = ~x2 & x0 & ~x1;
  assign z15 = x7 & x6 & x5 & ~x4 & new_n89_ & x3;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n91_ & x3;
  assign z17 = ~x5 & new_n95_ & x4;
  assign z18 = ~x0 & (~x2 | (~x1 & x3 & x4 & ~x5));
  assign z20 = ~x2 & (~x0 | (new_n101_ & ~x1 & ~x3));
  assign new_n101_ = ~x4 & ~x5 & ~x6;
  assign z21 = ~x6 & ~x5 & ~x4 & new_n95_ & x3;
  assign z22 = x7 & x6 & ~x5 & new_n95_ & ~x4;
  assign z26 = (~x0 & ~x2) | (x0 & x2 & ~x3 & new_n105_ & ~x4 & ~x5);
  assign new_n105_ = x6 & x7;
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n89_ & ~x3;
  assign z28 = x7 & new_n108_ & x6;
  assign new_n108_ = ~x5 & ~x4 & x3 & x2 & x0 & ~x1;
  assign z30 = x7 & x6 & new_n85_ & ~x5;
  assign z31 = (~x0 & (~x2 | ~x5)) | ~new_n111_ | (x1 & (x0 | x2));
  assign new_n111_ = (~x2 | (~x0 & x3 & x4)) & (~x0 | (x4 ? x5 : (~x5 & ~x6)));
  assign z32 = (x1 & (x0 | x2)) | (~x3 & (x2 | (x0 & ~x4))) | (~new_n113_ & x0) | (x2 & (x0 | ~x4));
  assign new_n113_ = x4 ? x5 : (~x5 & ~x6);
  assign z33 = ~new_n115_ | ~x7 | x4 | ~x6;
  assign new_n115_ = x0 & x2 & (x1 | ~x5) & (~x1 | ~x3 | x5);
  assign z34 = new_n117_ | new_n119_ | (~new_n118_ & x2);
  assign new_n117_ = x1 & (x0 ? ~x5 : (x2 & ~x3));
  assign new_n118_ = (~x3 | (x5 & (x0 | ~x6))) & (x3 | (~x5 & (x0 | x6))) & (~x0 | x5) & (x0 | (~x4 & ~x7));
  assign new_n119_ = x0 & ((~x4 & ~x5 & (~x6 | ~x7)) | (x5 & (~x3 | x4 | x6 | x7)));
  assign z35 = (x3 & (~x4 | (~x0 & ~x5))) | (~x0 & (~x2 | ~x3)) | x1 | (x0 & (x2 | ~x4 | ~x5));
  assign z36 = new_n122_ | new_n123_ | (~new_n124_ & x0) | x1 | (~new_n125_ & ~x0);
  assign new_n122_ = x3 & (~x0 | ~x4);
  assign new_n123_ = x5 & (x0 | (~x0 & ~x3));
  assign new_n124_ = ~x2 & x4;
  assign new_n125_ = x2 & (x3 | (~x4 & x6 & ~x7));
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (x3 & (x5 ? x1 : ~new_n127_)) | (~x0 & ~x2) | (~x1 & ~x3);
  assign new_n127_ = ~x4 & x6 & ~x7;
  assign z38 = (~x3 & (~x0 | ~x4)) | (~x0 & (~x2 | ~x4)) | (~new_n129_ & ~x4) | x1 | (x0 & x2);
  assign new_n129_ = ~x5 & ~x6;
  assign z39 = (~new_n131_ & (x2 | (x0 & x5))) | (x2 & (x4 | ~x5)) | (x0 & (x4 | (~new_n132_ & ~x5)));
  assign new_n131_ = x3 & ~x6 & ~x7;
  assign new_n132_ = ~x1 & x6 & x7;
  assign z40 = (x1 & (x0 ^ x2)) | (~new_n135_ & x0) | (~new_n134_ & x2);
  assign new_n134_ = x0 ? (~x3 & ~x4 & ~x5 & x6 & x7) : (x3 & x4);
  assign new_n135_ = x4 ? x5 : (~x5 & (x2 | ~x6));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = (x4 & (x0 | x2)) | (~new_n138_ & ~x5) | (~new_n77_ & (x0 ? x5 : x2));
  assign new_n138_ = (~x2 | (x0 & x3)) & (~x0 | (~x1 & x6 & x7));
  assign z43 = new_n140_ | new_n141_ | (~new_n142_ & ~x0) | new_n143_ | (~new_n144_ & x0);
  assign new_n140_ = ~x2 & (~x0 | (x1 & ~x5));
  assign new_n141_ = (~x0 | x5) & (x4 ? x1 : (x6 | x7));
  assign new_n142_ = x4 ? x3 : x5;
  assign new_n143_ = ~x7 & ((~x4 & x6) | (x0 & x2 & ~x5));
  assign new_n144_ = (~x1 | ~x3 | x5) & (~x2 | (~x4 & (x5 | x6)));
  assign z44 = ~new_n146_ | x6 | x4 | x5;
  assign new_n146_ = ~x2 & ~x3 & x0 & (~x1 | x2 | x5);
  assign z45 = new_n148_ | x0 | ~x1 | ~x2;
  assign new_n148_ = ~x4 & (x5 | x6);
  assign z47 = (x2 & (~x1 | (new_n148_ & ~x0))) | (x0 & (~new_n150_ | ~new_n79_ | ~x2));
  assign new_n150_ = x5 & x6 & x7;
  assign z48 = x2 | (x0 & (~new_n150_ | ~new_n79_ | ~x2));
  assign z49 = x0 | (x2 & ((~new_n129_ & ~x4) | x1 | (x3 & x4)));
  assign z50 = x2 | (x0 & (~new_n105_ | x5 | ~new_n79_ | ~x1));
  assign z51 = x0 ? (new_n155_ | ~x1 | (~x2 & x3)) : (x2 & (~new_n101_ | x1 | ~x3));
  assign new_n155_ = ~x4 & ((x2 & (x5 | x6)) | (~x5 & x6) | (x5 & (~x6 | ~x7)));
  assign z52 = (x0 & (x3 | (~x1 & ~x2))) | (~x0 & (x1 | ~x2)) | (~new_n129_ & ~x4) | (x3 & x4);
  assign z53 = new_n158_ | (~new_n159_ & ~x4) | (~new_n93_ & ~new_n160_) | ~new_n161_;
  assign new_n158_ = x1 & (x0 ? ~x3 : (x2 & x3));
  assign new_n159_ = ((~x5 & ~x6) | ((~x2 | x3) & (~x0 | x2 | ~x3))) & (~x0 | (x5 ? (x6 & x7) : ~x6));
  assign new_n160_ = (x1 | ~x2) & (~x0 | x3);
  assign new_n161_ = (~x0 | (x3 ? x1 : ~x2)) & (x1 | ~x2 | x3);
  assign z54 = (~x1 & (x0 | (x2 & x3))) | (~new_n93_ & (x2 ? ~x3 : x0)) | (new_n163_ & x2) | (x0 & x3);
  assign new_n163_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z55 = new_n165_ | (~x2 & (~x0 | ~x3)) | ~x1 | (x0 & ~new_n93_ & x2);
  assign new_n165_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = x0 | (x2 & (~new_n150_ | x4 | (~x1 & x3)));
  assign z57 = (~x1 & (x0 | (x2 & ~x3))) | (~new_n93_ & x2) | (x0 & (new_n168_ | x2 | ~x3));
  assign new_n168_ = ~x4 & (x5 | (x6 & ~x7));
  assign z58 = (new_n148_ & ~x0) | (~new_n93_ & x0) | ~x1 | ~x2 | ~x3;
  assign z59 = x0 ? (new_n171_ | new_n172_ | ~new_n173_) : (~new_n87_ & x2);
  assign new_n171_ = (~x1 | ~x3) & (~x2 | (~x4 & x6));
  assign new_n172_ = x5 & (~x4 | (x1 & x3));
  assign new_n173_ = x1 ? (~x3 | (x6 & x7 & ~x2 & ~x4)) : x3;
  assign z60 = x0 ? (x3 | ~x4 | (~x1 & (~x2 | ~x3))) : (x2 & (~new_n150_ | x1 | ~x3 | x4));
  assign z61 = x0 ? (new_n148_ | x1 | ~x2 | ~x3) : x2;
  assign z62 = new_n148_ | ~x0 | ~x1 | x3;
  assign z19 = 1'b0;
  assign z46 = ~z07;
  assign z13 = z07;
  assign z23 = z07;
  assign z24 = z07;
  assign z25 = z07;
  assign z29 = z07;
endmodule


