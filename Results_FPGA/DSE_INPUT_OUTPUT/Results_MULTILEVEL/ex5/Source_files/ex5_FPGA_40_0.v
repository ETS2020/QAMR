// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n82_, new_n84_, new_n87_, new_n89_, new_n91_, new_n94_,
    new_n96_, new_n98_, new_n101_, new_n105_, new_n109_, new_n111_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n123_, new_n125_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n149_, new_n151_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n162_, new_n164_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_;
  assign z00 = z07 | (new_n75_ & ~x4);
  assign z07 = ~x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z07 & ~x7;
  assign z02 = ~x3 & (~x2 | (~x4 & x5 & ~x6 & ~x7));
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (~x2 & ~x3) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = ~x4 & x5 & x6 & ~z07 & ~x7;
  assign z06 = ~x6 & ~x5 & ~x4 & new_n82_ & x3;
  assign new_n82_ = x2 & ~x0 & ~x1;
  assign z08 = ~x3 & (~x2 | (new_n84_ & x0 & x1 & ~x4));
  assign new_n84_ = x5 & x6 & x7;
  assign z09 = x7 & x6 & ~x5 & ~x4 & new_n82_ & ~x3;
  assign z10 = (~x2 & ~x3) | (new_n87_ & ~x0 & x1 & x2);
  assign new_n87_ = x6 & x7 & ~x4 & x5;
  assign z12 = x7 & x6 & x5 & ~x4 & new_n89_ & ~x3;
  assign new_n89_ = x2 & x0 & ~x1;
  assign z13 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = ~x2 & (~x3 | (new_n84_ & x0 & ~x1 & ~x4));
  assign z15 = x7 & x6 & x5 & ~x4 & new_n94_ & x3;
  assign new_n94_ = x2 & ~x0 & x1;
  assign z16 = x7 & new_n96_ & x6;
  assign new_n96_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x2 & (~x3 | (new_n98_ & x4 & ~x5));
  assign new_n98_ = x0 & ~x1;
  assign z18 = ~x5 & x4 & new_n82_ & x3;
  assign z21 = new_n101_ & ~x6;
  assign new_n101_ = ~x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z22 = x7 & new_n101_ & x6;
  assign z23 = ~x2 & (~x3 | (~x0 & ~x1 & x5));
  assign z26 = ~x3 & (~x2 | (x0 & ~x4 & new_n105_ & ~x5));
  assign new_n105_ = x6 & x7;
  assign z27 = ~x7 & x6 & ~x5 & ~x4 & new_n94_ & ~x3;
  assign z28 = x7 & x6 & ~x5 & ~x4 & new_n89_ & x3;
  assign z30 = ~x3 & (new_n109_ | ~x2);
  assign new_n109_ = x0 & x1 & ~x4 & ~x5 & x6 & x7;
  assign z31 = ~new_n111_ | (~z07 & x1);
  assign new_n111_ = (x5 | (~x2 & (~x3 | ~x4))) & (~x2 | (~x0 & x3 & x4)) & (~x3 | ((x0 | x2) & (x4 | (~x5 & ~x6))));
  assign z32 = (~new_n113_ & x3) | (x2 & (~x3 | ~x4 | x0 | x1));
  assign new_n113_ = (~x1 | (x2 & x5)) & (x2 | ((x4 | (~x5 & ~x6)) & x0 & (~x4 | x5)));
  assign z33 = (~new_n115_ & x2) | (x3 & (~x2 | (x1 & ~x5)));
  assign new_n115_ = x0 & ~x4 & x6 & x7 & (x1 | ~x5);
  assign z34 = new_n117_ | new_n118_ | (~new_n119_ & x3) | (x2 & ~new_n120_ & ~x3);
  assign new_n117_ = x1 & (x3 ? ~x5 : x2);
  assign new_n118_ = (x2 | (x3 & x5)) & (x4 | x7);
  assign new_n119_ = (~x2 | (x5 & ~x6)) & (x5 | (x0 & (x4 | (x6 & x7)))) & (~x5 | ~x6);
  assign new_n120_ = ~x0 & ~x5 & x6;
  assign z35 = ((x2 | x3) & (x1 | ~x4 | ~x5)) | (x2 & (x0 | ~x3)) | (~x0 & ~x2 & x3);
  assign z36 = ~new_n123_ | (~z07 & (x1 | x5));
  assign new_n123_ = (~x3 | (x0 & ~x2 & x4)) & (~x2 | (~x0 & ~x4 & x6 & ~x7));
  assign z37 = (x2 & (~x3 | x5)) | (x3 & (x5 ? ~new_n98_ : ~new_n125_));
  assign new_n125_ = ~x4 & x6 & ~x7;
  assign z38 = (~x0 & (~x2 | ~x4)) | (~new_n75_ & ~x4) | x1 | ~x3 | (x0 & x2);
  assign z39 = (~new_n128_ & x3) | (x2 & (~x3 | ~x5));
  assign new_n128_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = new_n132_ | new_n131_ | new_n133_ | new_n134_ | (new_n130_ & ~x2);
  assign new_n130_ = ~x4 & (x5 | x6);
  assign new_n131_ = ~x0 & (~x2 | ~x4);
  assign new_n132_ = (x1 | ~x3) & (~x0 | ~x2);
  assign new_n133_ = x4 & (x2 ? x0 : ~x5);
  assign new_n134_ = x0 & x2 & (~x6 | ~x7 | x3 | x5);
  assign z41 = x2 | (x3 & (~x0 | x1 | ~x5));
  assign z42 = ~new_n128_ | (~x3 & (~x2 | ~x5));
  assign z43 = (~new_n138_ & ~x4) | new_n141_ | (x3 & (new_n139_ | new_n140_));
  assign new_n138_ = (~x5 | (~x6 & ~x7) | (~x2 & ~x3)) & (x7 | (x2 ? x5 : (~x3 | ~x6))) & (~x2 | x5 | (x0 & x6));
  assign new_n139_ = (x4 | ~x5) & (x1 | (~x0 & ~x2));
  assign new_n140_ = ~x0 & ~x2 & (x6 | x7);
  assign new_n141_ = x2 & x4 & (x0 | x1 | ~x3);
  assign z45 = (x0 & (x2 | x3)) | (x2 & (new_n130_ | ~x1)) | (~x2 & ~new_n143_ & x3);
  assign new_n143_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z47 = x2 ? (new_n145_ | new_n146_ | ~x1) : (~new_n147_ & x3);
  assign new_n145_ = ~x0 & ~x4 & (x5 | x6);
  assign new_n146_ = x0 & (~x3 | x4 | ~x5 | ~x6 | ~x7);
  assign new_n147_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z48 = new_n149_ | x0 | x1 | x2 | ~x3;
  assign new_n149_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z49 = ~new_n151_ | (x0 & (~x1 | x2));
  assign new_n151_ = (~x2 | (~x1 & (x4 | ~x6))) & x2 & (~x3 | ~x4) & (x4 | ~x5);
  assign z50 = (x4 & (~x2 | x3)) | (x5 & (~x2 | ~x4)) | new_n98_ | x2 | (~x2 & (~new_n105_ | ~x3));
  assign z51 = (~x0 & ((x1 & (x2 | x3)) | (x3 & ~new_n75_ & ~x4) | ~x3 | (x2 & x4))) | (x0 & (~x1 | ~x2)) | (~x4 & ~new_n75_ & (x0 | x2));
  assign z52 = (~x0 & ((x1 & (x2 | x3)) | (x3 & (x4 ? x2 : ~new_n75_)))) | (x2 & ~new_n75_ & ~x4) | (x0 & x3);
  assign z53 = new_n156_ | new_n157_ | (x3 & (new_n158_ | ~new_n159_));
  assign new_n156_ = (~x1 | (~x4 & x6)) & (x3 ? (~x2 | ~x5) : x2);
  assign new_n157_ = x2 & ((x0 & (~x1 | ~x3)) | (~x0 & x1 & x3) | (~x3 & ~x4 & x5));
  assign new_n158_ = (~x6 | ~x7) & (~x1 | (~x4 & x5));
  assign new_n159_ = (x1 | (~x0 & ~x4)) & (x2 | x4 | ~x5);
  assign z54 = (x0 & (x3 | (~x1 & x2))) | (~new_n87_ & (~x2 ^ ~x3)) | (x3 & (new_n149_ | (~x1 & x2)));
  assign z55 = new_n162_ | (~x1 & (x2 | x3)) | (x0 & ~new_n87_ & x2);
  assign new_n162_ = ~x4 & (x5 | x6) & (x2 ? ~x0 : x3);
  assign z56 = (x0 & (x2 | x3)) | (~new_n164_ & x3) | (~new_n87_ & x2);
  assign new_n164_ = x1 & (x2 | x4 | (~x5 & (~x6 | x7)));
  assign z57 = (~x1 & (~x2 ^ ~x3)) | (~new_n166_ & x3) | (~new_n167_ & x2);
  assign new_n166_ = (x4 | ~x6 | x7) & (x2 | (x0 & (x4 | ~x5)));
  assign new_n167_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z58 = (new_n169_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n171_ | (~new_n170_ & (x0 | ~x1));
  assign new_n169_ = ~x4 & (x5 | (x1 & x6));
  assign new_n170_ = ~x4 & x6 & x7;
  assign new_n171_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = new_n173_ | new_n174_ | new_n175_ | ~new_n178_ | (~new_n176_ & ~new_n177_);
  assign new_n173_ = x1 & (x3 ? x2 : ~x0);
  assign new_n174_ = x0 & ((~x1 & (~x2 | ~x3)) | (~x3 & ~x4 & x6));
  assign new_n175_ = x2 & x3 & (~x0 | (~x4 & x6));
  assign new_n176_ = x2 & (x0 | x3);
  assign new_n177_ = ~x4 & ~x5 & x6 & x7;
  assign new_n178_ = (x2 | x3) & (x4 | ~x5);
  assign z60 = (x0 & (x3 | (~x1 & ~x3))) | (~x3 & (~x0 | ~x4)) | ~x2 | (x3 & (~new_n84_ | x1 | x4));
  assign z61 = ((x2 | x3) & (new_n130_ | ~x0)) | (x2 & ~x3) | (x3 & (x1 | ~x2));
  assign z62 = x3 | (x2 & (new_n130_ | ~x0 | (~x1 & ~x3)));
  assign z11 = 1'b0;
  assign z24 = 1'b0;
  assign z25 = 1'b0;
  assign z44 = 1'b1;
  assign z46 = ~z07;
  assign z19 = z07;
  assign z20 = z07;
  assign z29 = z07;
endmodule


