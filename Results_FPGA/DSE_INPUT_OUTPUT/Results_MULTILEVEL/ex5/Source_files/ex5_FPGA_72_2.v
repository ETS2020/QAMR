// Benchmark "FAU" written by ABC on Mon Aug 17 18:02:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n78_, new_n84_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n93_, new_n100_, new_n102_, new_n103_, new_n106_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n117_, new_n122_, new_n124_,
    new_n126_, new_n128_, new_n129_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n146_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n156_, new_n157_, new_n159_, new_n161_, new_n163_, new_n165_,
    new_n168_, new_n169_, new_n171_, new_n172_;
  assign z00 = z08 | (new_n75_ & ~x4);
  assign z08 = x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z08 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x3 & (x2 | (new_n78_ & ~x6 & ~x7));
  assign new_n78_ = ~x4 & x5;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (x2 & ~x3) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z08 | (new_n78_ & x6 & ~x7);
  assign z06 = x2 & (~x3 | (new_n75_ & ~x4 & ~x0 & ~x1));
  assign z07 = x7 & x6 & new_n84_ & x5;
  assign new_n84_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z10 = x7 & new_n86_ & x6;
  assign new_n86_ = x5 & ~x4 & x3 & x2 & ~x0 & x1;
  assign z11 = ~x3 & (x2 | (new_n88_ & x0 & x1 & ~x4));
  assign new_n88_ = new_n89_ & x5;
  assign new_n89_ = x6 & x7;
  assign z13 = (x2 & ~x3) | (new_n91_ & ~x0 & x1 & ~x2 & x3);
  assign new_n91_ = ~x4 & x5 & x6 & x7;
  assign z14 = (x2 & ~x3) | (new_n91_ & new_n93_ & ~x2 & x3);
  assign new_n93_ = x0 & ~x1;
  assign z16 = (x2 & ~x3) | (new_n91_ & ~x2 & x3 & x0 & x1);
  assign z17 = (x2 & ~x3) | (new_n93_ & ~x2 & x4 & ~x5);
  assign z18 = x2 & (~x3 | (~x0 & ~x1 & x4 & ~x5));
  assign z19 = ~x3 & (x2 | (~x0 & ~x1 & x4));
  assign z20 = ~x3 & (x2 | (new_n93_ & new_n75_ & ~x4));
  assign z21 = (x2 & ~x3) | (new_n100_ & new_n75_ & x3 & ~x4);
  assign new_n100_ = x0 & ~x1 & ~x2;
  assign z22 = z08 | (new_n102_ & new_n100_);
  assign new_n102_ = new_n89_ & new_n103_;
  assign new_n103_ = ~x4 & ~x5;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n106_ & x6;
  assign new_n106_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n84_ & ~x5;
  assign z28 = x7 & new_n109_ & x6;
  assign new_n109_ = ~x5 & ~x4 & x3 & new_n93_ & x2;
  assign z29 = ~x3 & (new_n111_ | x2);
  assign new_n111_ = ~x0 & ~x1 & ~x4 & ~x5 & ~x6 & x7;
  assign z31 = ~new_n113_ | (x0 & (x2 ? x3 : (~x4 & x6)));
  assign new_n113_ = (x2 | ((x0 | (~x3 & x4)) & ~x1 & (~x4 | x5) & (x4 | ~x5))) & (~x3 | (~x1 & (~x2 | x4))) & (~x2 | (x3 & x5));
  assign z32 = (x1 & (~x2 | x3)) | (x3 & (x2 ? (x0 | ~x4) : ~x0)) | (~new_n115_ & ~x2);
  assign new_n115_ = x4 ? (x0 & x5) : ((~x0 | (x3 & ~x6)) & ~x5 & (x0 | (x6 & ~x7)));
  assign z33 = ~x2 | (~new_n117_ & x3);
  assign new_n117_ = x0 & ~x4 & (x1 | ~x5) & new_n89_ & (~x1 | x5);
  assign z34 = x5 ? (~x3 | x4 | x6 | x7) : (~new_n100_ | (~x4 & (~x6 | ~x7)));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = x2 ? x3 : (~new_n93_ | ~x4 | x5);
  assign z37 = (~x0 & (x3 ? x5 : ~x2)) | (x3 & (x5 ? (x1 | x2) : ~new_n122_)) | (~x1 & ~x2 & ~x3);
  assign new_n122_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (~x2 | x3)) | (x3 & (x2 ? (x0 | ~x4) : ~x0)) | (~new_n124_ & ~x2);
  assign new_n124_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x4 & ~x5 & x6 & ~x7));
  assign z39 = (~new_n126_ & x5) | x4 | (~x5 & (~new_n93_ | ~new_n89_ | x2));
  assign new_n126_ = x3 & ~x6 & ~x7;
  assign z40 = ~new_n128_ | (x0 & (x2 | (x4 & ~x5) | (~x4 & x6)));
  assign new_n128_ = (~x2 | (x3 & x4)) & ~new_n129_ & ~x1 & (x4 | ~x5);
  assign new_n129_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~z08 & ((x5 & (x6 | x7)) | x4 | (~x5 & (~new_n93_ | ~x6 | ~x7)));
  assign z43 = (~new_n133_ & ~x5) | (~new_n135_ & x4) | (~new_n137_ & ~x4) | (new_n136_ & x5);
  assign new_n133_ = new_n134_ & (~x1 | (x2 & (~x0 | ~x3)));
  assign new_n134_ = (~x3 | (x0 ? (~x2 | (x6 & x7)) : (x2 & x4))) & (x0 | x2 | x4 | x6);
  assign new_n135_ = (~x1 | x2) & (~x3 | (x0 ? ~x2 : (~x1 & x2)));
  assign new_n136_ = (x6 | x7) & ((~x2 & ~x4) | (x0 & x2 & x3));
  assign new_n137_ = (x2 | (x0 ? (~x6 | x7) : ~x7)) & (x0 | ~x3 | (~x7 & (~x2 | ~x6)));
  assign z44 = (~new_n139_ & ~x2) | (x3 & (x0 | x2 | (~new_n141_ & ~x0)));
  assign new_n139_ = (new_n140_ | (~x4 & x5)) & (~x6 | (~x0 & (x4 | ~x5))) & (~x0 | (~x4 & ~x5)) & (x4 | (x0 & (~x5 | ~x7)));
  assign new_n140_ = ~x1 & (x0 | ~x3);
  assign new_n141_ = x4 ? ~x1 : (x5 & ~x7);
  assign z45 = (x0 & (~x2 | x3)) | (x3 & (new_n78_ | (~new_n144_ & x2))) | (~new_n143_ & ~x2);
  assign new_n143_ = ~x1 & ~x4 & new_n89_ & ~x5;
  assign new_n144_ = x1 & (x4 | ~x6);
  assign z46 = new_n146_ | x2 | x3 | x0 | ~x1;
  assign new_n146_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (x3 & ((~new_n91_ & x0) | (~new_n148_ & x2))) | (~new_n149_ & ~x2);
  assign new_n148_ = x1 & (x0 | new_n75_ | x4);
  assign new_n149_ = ~x0 & ~x1 & ~x4 & new_n89_ & ~x5;
  assign z48 = ~new_n151_ | (~new_n89_ & ((x0 & x3) | (new_n78_ & ~x2)));
  assign new_n151_ = ~new_n152_ & (~x0 | (x2 & (~x3 | ~x4))) & (x2 | (~x1 & x3)) & (~x2 | ~x3);
  assign new_n152_ = ~x5 & ((x0 & x3) | (~x2 & ~x4 & x6));
  assign z49 = (~x3 & (x0 | x2)) | (x0 & (~x1 | x2)) | ~new_n103_ | ~x2 | (x2 & (x1 | x6));
  assign z50 = (x0 & (~x1 | ~x3)) | ~new_n103_ | x2 | (~new_n89_ & ~x2);
  assign z51 = ~new_n156_ | (~z08 & (~x0 ^ ~x1));
  assign new_n156_ = (new_n75_ | ((x0 | (x2 ? ~x3 : x4)) & (~x2 | ~x3 | x4))) & (x0 | (x2 ? (~x3 | ~x4) : x3)) & (x2 | (~new_n157_ & (~x0 | ~x3)));
  assign new_n157_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z52 = ((~x2 | x3) & (~x0 ^ ~x1)) | (x0 & (x2 | (~x2 & x3))) | ~new_n159_ | (~x3 & (x2 | (~x0 & ~x2)));
  assign new_n159_ = (x4 | (x2 ? (~x3 | (~x5 & ~x6)) : (~x5 & (~x6 | (x0 & x5))))) & (x0 | ~x2 | ~x3 | (~x4 & ~x5 & ~x6));
  assign z53 = (x0 & (x1 ? (~x2 & ~x3) : x3)) | (~new_n91_ & (x3 ? ~x1 : ~x2)) | (x3 & (new_n161_ | (~x1 & ~x2) | (~x0 & x1 & x2)));
  assign new_n161_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n163_ & ~x2) | (x3 & (new_n157_ | x0 | (~x1 & x2)));
  assign new_n163_ = x3 ? new_n91_ : ((x0 | new_n75_ | x4) & x1 & (new_n91_ | ~x0));
  assign z55 = new_n165_ | (~x1 & (~x2 | x3)) | (x0 & (x2 ? (~new_n91_ & x3) : ~x3));
  assign new_n165_ = ~x4 & ~new_n75_ & (~x2 | (~x0 & x3));
  assign z56 = ((~x2 | x3) & (x0 | ~x1)) | (x2 & ~new_n91_ & x3) | (~x2 & (new_n146_ | ~x3));
  assign z57 = ((x0 | ~x2) & (new_n78_ | ~x1)) | ~new_n168_ | (x0 & (x2 | ~x3));
  assign new_n168_ = new_n169_ & (x7 | (~x2 & (x4 | ~x6)));
  assign new_n169_ = (x0 | x2 | ~x3) & (~x2 | (x3 & ~x4 & x5 & x6));
  assign z58 = (~new_n171_ & x3) | (~x2 & (~new_n102_ | x0 | x1 | ~x3));
  assign new_n171_ = (~new_n172_ | x0) & (x1 | ~x2) & (new_n91_ | ~x0);
  assign new_n172_ = ~x4 & (x5 | (x2 & x6));
  assign z59 = (x0 & ((~x1 & ~x2) | ~x3)) | new_n172_ | (~new_n102_ & ~x2) | (x2 & (~x0 | x1 | ~x3));
  assign z60 = ((x3 | (~x0 & ~x2)) & (~new_n88_ | x1 | x4)) | (x3 & (x0 | ~x2)) | (x0 & ~x2 & (~x1 | ~x4));
  assign z61 = (~x1 & (~x2 | ~x3)) | ~x0 | x1 | (~new_n75_ & ~x4);
  assign z62 = (x1 & (x2 | x3)) | ~x0 | ~x1 | (~new_n75_ & ~x4);
  assign z09 = 1'b0;
  assign z12 = 1'b0;
  assign z15 = z10;
  assign z26 = z08;
  assign z27 = z08;
  assign z30 = z08;
endmodule


