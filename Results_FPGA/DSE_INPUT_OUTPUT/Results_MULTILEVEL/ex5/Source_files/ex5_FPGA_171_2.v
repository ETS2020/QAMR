// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n83_, new_n85_, new_n88_, new_n90_, new_n92_, new_n94_,
    new_n102_, new_n105_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n117_, new_n118_, new_n122_, new_n124_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n161_,
    new_n163_, new_n165_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_;
  assign z00 = z08 | (new_n75_ & ~x4);
  assign z08 = x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z08 | (~x5 & ~x6 & ~x7);
  assign z02 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & ~x3;
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = ~x7 & x6 & ~x5 & x3 & ~x4;
  assign z05 = ~x4 & x5 & x6 & ~z08 & ~x7;
  assign z06 = x2 & (~x3 | (new_n75_ & ~x4 & ~x0 & ~x1));
  assign z07 = x7 & x6 & new_n83_ & x5;
  assign new_n83_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z10 = x2 & (~x3 | (new_n85_ & ~x0 & x1 & ~x4));
  assign new_n85_ = x5 & x6 & x7;
  assign z11 = ~x3 & (x2 | (new_n85_ & x0 & x1 & ~x4));
  assign z13 = x7 & new_n88_ & x6;
  assign new_n88_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = (x2 & ~x3) | (new_n94_ & ~x2 & x4 & ~x5);
  assign new_n94_ = x0 & ~x1;
  assign z18 = x2 & (~x3 | (~x0 & ~x1 & x4 & ~x5));
  assign z19 = ~x3 & (x2 | (~x0 & ~x1 & x4));
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n94_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n94_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n94_ & ~x2;
  assign z23 = (x2 & ~x3) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = ~x3 & (x2 | (new_n102_ & ~x5 & x6 & ~x7));
  assign new_n102_ = ~x0 & ~x1 & ~x4;
  assign z25 = ~x7 & x6 & new_n83_ & ~x5;
  assign z28 = x2 & (~x3 | (new_n105_ & x0 & ~x1 & ~x4));
  assign new_n105_ = ~x5 & x6 & x7;
  assign z29 = ~x3 & (x2 | (new_n102_ & ~x5 & ~x6 & x7));
  assign z31 = new_n108_ | ~new_n109_;
  assign new_n108_ = x0 & (x2 ? x3 : (~x4 & x6));
  assign new_n109_ = (x2 | ((x0 | (~x3 & x4)) & ~x1 & (~x4 | x5) & (x4 | ~x5))) & (~x3 | (~x1 & (~x2 | (x4 & x5))));
  assign z32 = (~z08 & x1) | new_n111_ | new_n108_ | new_n112_ | new_n113_;
  assign new_n111_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n112_ = x3 & (x2 ? ~x4 : ~x0);
  assign new_n113_ = ~x2 & (x4 ? (~x0 | ~x5) : (x5 | (~x0 & (~x6 | x7))));
  assign z33 = ~x2 | (~new_n115_ & x3);
  assign new_n115_ = x0 & ~x4 & (x1 | ~x5) & x6 & x7 & (~x1 | x5);
  assign z34 = (~new_n117_ & (x2 ? x3 : x5)) | (~x2 & (x5 ? ~x3 : ~new_n118_)) | (x2 & x3 & ~x5);
  assign new_n117_ = ~x4 & ~x6 & ~x7;
  assign new_n118_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 | ~x5)) | (x2 & (~x3 | ~x5)) | x1 | ~x4 | (~x0 & ~x2 & x3);
  assign z36 = (x2 & (x0 | ~x3 | ~x5)) | ~x0 | x1 | ~x4 | x5;
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (x3 & (x5 ? x1 : ~new_n122_)) | (~x1 & ~x3);
  assign new_n122_ = ~x4 & x6 & ~x7;
  assign z38 = (x1 & (~x2 | x3)) | (x3 & (x2 ? (x0 | ~x4) : ~x0)) | (~new_n124_ & ~x2);
  assign new_n124_ = (x4 | (~x5 & (~x0 | (x3 & ~x6)))) & (x0 | (~x4 & ~x5 & x6 & ~x7));
  assign z39 = (x5 & (~x3 | x6 | x7)) | x4 | (~new_n126_ & ~x5);
  assign new_n126_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = new_n128_ | new_n129_ | new_n130_ | x1 | (~x4 & x5);
  assign new_n128_ = x0 & (x2 | (x4 & ~x5) | (~x4 & x6));
  assign new_n129_ = x2 & (~x3 | ~x4);
  assign new_n130_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign z41 = (~x1 & (~x3 | ~x5)) | ~x0 | x2 | (x1 & x3);
  assign z42 = ~z08 & ((x5 & (x6 | x7)) | x4 | (~x5 & (~new_n94_ | ~x6 | ~x7)));
  assign z43 = new_n134_ | new_n135_ | new_n136_ | new_n137_ | new_n138_;
  assign new_n134_ = ~x3 & (x2 | (x1 & ~x5));
  assign new_n135_ = x1 & (x4 | (x0 & ~x5));
  assign new_n136_ = x0 & ((~x4 & x6 & ~x7) | (x2 & (x4 | (~x5 & ~x6))));
  assign new_n137_ = ~x4 & ((x5 & (x6 | x7)) | (~x0 & ((x3 & (~x5 | x6)) | x7 | (~x5 & ~x6))));
  assign new_n138_ = ~x0 & ~x2 & x3 & x4;
  assign z44 = new_n140_ | (x4 & (x0 | x1)) | ~new_n142_ | (new_n141_ & x1);
  assign new_n140_ = x2 & (~x3 | (x0 & ~x5 & ~x6));
  assign new_n141_ = ~x5 & (x0 | ~x3);
  assign new_n142_ = (~x0 | (~x5 & ~x6)) & ~x3 & (x4 | (x0 & (~x5 | (~x6 & ~x7))));
  assign z45 = (x0 & (~x2 | x3)) | (~new_n145_ & ~x2) | (~new_n144_ & x3);
  assign new_n144_ = (x4 | ~x5) & (~x2 | (x1 & (x4 | ~x6)));
  assign new_n145_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = ~new_n147_ | (~x4 & (x5 | (x6 & ~x7)));
  assign new_n147_ = ~x2 & ~x3 & ~x0 & x1;
  assign z47 = (~new_n151_ & ~x2) | (x3 & ((~new_n150_ & x0) | (~new_n149_ & x2)));
  assign new_n149_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n150_ = ~x4 & x5 & x6 & x7;
  assign new_n151_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = (~new_n153_ & ~new_n154_) | new_n155_ | ~new_n156_;
  assign new_n153_ = x6 & x7;
  assign new_n154_ = (x2 | x4 | ~x5) & (~x0 | ~x3);
  assign new_n155_ = ~x5 & ((x0 & x3) | (~x2 & ~x4 & x6));
  assign new_n156_ = (~x0 | (x2 & (~x3 | ~x4))) & (x2 | (~x1 & x3)) & (~x2 | ~x3);
  assign z49 = ~x2 | (x3 & (~new_n158_ | (x2 & (x0 | x1 | x6))));
  assign new_n158_ = ~x4 & ~x5;
  assign z50 = (~x2 & (~new_n153_ | ~new_n158_ | (x0 & (~x1 | ~x3)))) | (x3 & (~new_n158_ | x2));
  assign z51 = (x0 & (~x1 | (~x2 & x3))) | (x2 & (~x3 | (~x0 & x4))) | ~new_n161_ | (~x0 & (x1 | ~x3));
  assign new_n161_ = (x1 | x3) & (x4 | ((~x3 | (~x5 & ~x6)) & (x5 | ~x6) & (~x5 | (x6 & x7))));
  assign z52 = (x0 & (~x1 | x3)) | (x2 & (~x3 | (~x0 & x4))) | (~x0 & (x1 | ~x3)) | (~new_n163_ & ~x4) | (~x1 & ~x3);
  assign new_n163_ = ~x5 & (~x6 | (~x3 & x5));
  assign z53 = (x0 & (x1 ? (~x2 & ~x3) : x3)) | (~new_n150_ & (x3 ? ~x1 : ~x2)) | (x3 & (new_n165_ | (~x0 & x1 & x2) | (~x1 & ~x2)));
  assign new_n165_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~x2 & (x3 ? ~new_n150_ : ~new_n167_)) | (x3 & (new_n168_ | ~new_n169_));
  assign new_n167_ = (x0 | x4 | (~x5 & ~x6)) & x1 & (~x0 | (x6 & x7 & ~x4 & x5));
  assign new_n168_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign new_n169_ = ~x0 & (x1 | ~x2);
  assign z55 = new_n171_ | (~x3 & (x0 | x2)) | ~x1 | (x0 & ~new_n150_ & x2);
  assign new_n171_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = (~x4 & ((x6 & ~x7) | (~x2 & x5))) | ~new_n173_ | (x2 & (~x6 | ~x7 | x4 | ~x5));
  assign new_n173_ = ~x0 & x1 & x3;
  assign z57 = (~new_n175_ & (x0 | ~x2)) | new_n176_ | ~new_n177_ | (x0 & (x2 | ~x3));
  assign new_n175_ = x1 & (x4 | ~x5);
  assign new_n176_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n177_ = (x0 | x2 | ~x3) & (~x2 | (x3 & ~x4 & x5 & x6));
  assign z58 = (new_n179_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n181_ | (~new_n180_ & (x0 | ~x1));
  assign new_n179_ = ~x4 & (x5 | (x1 & x6));
  assign new_n180_ = ~x4 & x6 & x7;
  assign new_n181_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (x0 & (~x3 | (~x1 & ~x2))) | new_n184_ | (~new_n183_ & ~x2) | (x2 & (~x0 | x1 | ~x3));
  assign new_n183_ = ~x4 & ~x5 & x6 & x7;
  assign new_n184_ = ~x4 & (x5 | (x2 & x6));
  assign z60 = ((x3 | (~x0 & ~x2)) & (~new_n85_ | x1 | x4)) | (x3 & (x0 | ~x2)) | (x0 & ~x2 & (~x1 | ~x4));
  assign z61 = ~x2 | (x3 & (~new_n94_ | (~new_n75_ & ~x4)));
  assign z62 = (~new_n75_ & ~x4) | ~x0 | ~x1 | x2 | x3;
  assign z09 = 1'b0;
  assign z12 = 1'b0;
  assign z30 = 1'b0;
  assign z15 = x2 & (~x3 | (new_n85_ & ~x0 & x1 & ~x4));
  assign z26 = z08;
  assign z27 = z08;
endmodule


