// Benchmark "FAU" written by ABC on Mon Aug 17 18:03:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n83_, new_n87_, new_n89_, new_n91_, new_n100_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n116_, new_n117_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n180_;
  assign z00 = z09 | (new_n75_ & ~x4);
  assign z09 = x2 & ~x3;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = ~x5 & ~x6 & ~z09 & ~x7;
  assign z02 = ~x3 & (x2 | (~x4 & x5 & ~x6 & ~x7));
  assign z03 = ~x7 & ~x6 & x5 & x3 & ~x4;
  assign z04 = (x2 & ~x3) | (x3 & ~x4 & ~x5 & x6 & ~x7);
  assign z05 = z09 | (~x4 & x5 & x6 & ~x7);
  assign z06 = x2 & (~x3 | (new_n75_ & ~x4 & ~x0 & ~x1));
  assign z07 = ~x3 & (x2 | (new_n83_ & ~x0 & x1 & ~x4));
  assign new_n83_ = x5 & x6 & x7;
  assign z10 = x2 & (~x3 | (new_n83_ & ~x0 & x1 & ~x4));
  assign z11 = ~x3 & (x2 | (new_n83_ & x0 & x1 & ~x4));
  assign z13 = (x2 & ~x3) | (new_n87_ & ~x0 & x1 & ~x2 & x3);
  assign new_n87_ = ~x4 & x5 & x6 & x7;
  assign z14 = (x2 & ~x3) | (new_n87_ & new_n89_ & ~x2 & x3);
  assign new_n89_ = x0 & ~x1;
  assign z16 = x7 & new_n91_ & x6;
  assign new_n91_ = x5 & ~x4 & x3 & ~x2 & x0 & x1;
  assign z17 = ~x5 & x4 & new_n89_ & ~x2;
  assign z18 = x2 & (~x3 | (~x0 & ~x1 & x4 & ~x5));
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n89_ & ~x2;
  assign z21 = (x2 & ~x3) | (x0 & ~x1 & ~x2 & new_n75_ & x3 & ~x4);
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n89_ & ~x2;
  assign z23 = (x2 & ~x3) | (~x0 & ~x1 & ~x2 & x3 & x5);
  assign z24 = ~x7 & new_n100_ & x6;
  assign new_n100_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x3 & (new_n102_ | x2);
  assign new_n102_ = ~x5 & x6 & ~x7 & ~x0 & x1 & ~x4;
  assign z28 = x7 & new_n104_ & x6;
  assign new_n104_ = ~x5 & ~x4 & x3 & x2 & x0 & ~x1;
  assign z29 = x7 & new_n100_ & ~x6;
  assign z31 = new_n107_ | ~new_n108_;
  assign new_n107_ = x0 & (x2 ? x3 : (~x4 & x6));
  assign new_n108_ = (x2 | ((x0 | (~x3 & x4)) & ~x1 & (~x4 | x5) & (x4 | ~x5))) & (~x3 | (~x1 & (~x2 | (x4 & x5))));
  assign z32 = (~z09 & x1) | new_n110_ | new_n107_ | new_n111_ | new_n112_;
  assign new_n110_ = ~x3 & (x2 | (x0 & ~x4));
  assign new_n111_ = x3 & (x2 ? ~x4 : ~x0);
  assign new_n112_ = ~x2 & (x4 ? (~x0 | ~x5) : (x5 | (~x0 & (~x6 | x7))));
  assign z33 = ~new_n114_ | ~x3 | x4 | ~x6 | ~x7;
  assign new_n114_ = x0 & x2 & (~x1 | x5) & (x1 | ~x5);
  assign z34 = (~new_n116_ & (x2 ? x3 : x5)) | (~x2 & (x5 ? ~x3 : ~new_n117_)) | (x2 & x3 & ~x5);
  assign new_n116_ = ~x4 & ~x6 & ~x7;
  assign new_n117_ = x0 & ~x1 & (x4 | (x6 & x7));
  assign z35 = (x0 & (x2 ? x3 : ~x5)) | ((~x2 | x3) & (x1 | ~x4)) | (x3 & (x2 ? ~x5 : ~x0));
  assign z36 = ~new_n89_ | x2 | ~x4 | x5;
  assign z37 = ((~x3 | x5) & (~x0 | x2)) | (x3 & (x5 ? x1 : ~new_n121_)) | (~x1 & ~x3);
  assign new_n121_ = ~x4 & x6 & ~x7;
  assign z38 = new_n110_ | new_n123_ | (new_n124_ & x0) | x1 | (new_n125_ & ~x0);
  assign new_n123_ = x2 & (x0 | ~x4);
  assign new_n124_ = ~x4 & (x5 | x6);
  assign new_n125_ = ~x2 & (x5 | ~x6 | x7 | x3 | x4);
  assign z39 = (x5 & (~x3 | x6 | x7)) | x4 | (~new_n127_ & ~x5);
  assign new_n127_ = x0 & ~x1 & ~x2 & x6 & x7;
  assign z40 = new_n129_ | new_n130_ | new_n131_ | x1 | (~x4 & x5);
  assign new_n129_ = x0 & (x2 | (x4 & ~x5) | (~x4 & x6));
  assign new_n130_ = x2 & (~x3 | ~x4);
  assign new_n131_ = ~x0 & ((~x2 & x3) | (~x4 & (~x6 | x7)));
  assign z41 = (~x2 & (~x0 | (~x1 & (~x3 | ~x5)))) | (x3 & (x1 | x2));
  assign z42 = ~z09 & ((x5 & (x6 | x7)) | x4 | (~x5 & (~new_n89_ | ~x6 | ~x7)));
  assign z43 = (~x5 & (new_n135_ | ~new_n136_)) | new_n137_ | new_n139_ | (new_n138_ & x5);
  assign new_n135_ = x1 & (~x2 | (x0 & x3));
  assign new_n136_ = (~x3 | (x0 ? (~x2 | (x6 & x7)) : (x2 & x4))) & (x0 | x2 | x4 | x6);
  assign new_n137_ = x4 & ((x1 & ~x2) | (x3 & (x0 ? x2 : (x1 | ~x2))));
  assign new_n138_ = (x6 | x7) & ((x0 & x2 & x3) | (~x2 & ~x4));
  assign new_n139_ = ~x4 & ((~x2 & (x0 ? (x6 & ~x7) : x7)) | (~x0 & x3 & (x7 | (x2 & x6))));
  assign z44 = (~new_n143_ & x3) | (~x2 & (new_n141_ | ~new_n142_));
  assign new_n141_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n142_ = (~x6 | (~x0 & (x4 | ~x5))) & (~x0 | (~x4 & ~x5)) & (x4 | (x0 & (~x5 | ~x7)));
  assign new_n143_ = ~x0 & ~x2 & (x0 | (x4 ? ~x1 : (x5 & ~x7)));
  assign z45 = new_n145_ | (x1 & (~x2 | ~x3)) | x0 | (~new_n146_ & ~x1);
  assign new_n145_ = ~x4 & (x5 | (x1 & x6));
  assign new_n146_ = ~x2 & ~x4 & ~x5 & x6 & x7;
  assign z46 = x3 | (~x2 & (new_n148_ | x0 | ~x1));
  assign new_n148_ = ~x4 & (x5 | (x6 & ~x7));
  assign z47 = (~new_n152_ & ~x2) | (x3 & (new_n150_ | new_n151_ | (~x1 & x2)));
  assign new_n150_ = ~x0 & ~x4 & (x5 | (x2 & x6));
  assign new_n151_ = x0 & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n152_ = ~x0 & ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z48 = new_n154_ | x0 | x1 | x2 | ~x3;
  assign new_n154_ = ~x4 & (x5 ? (~x6 | ~x7) : x6);
  assign z50 = ~new_n146_ | (x0 & (~x1 | ~x3));
  assign z51 = (~new_n157_ & x3) | (x2 & ~x3) | (~x2 & (new_n154_ | (~x3 & (~x0 | ~x1))));
  assign new_n157_ = (~x0 | (x1 & x2)) & (x4 | (~x5 & ~x6)) & (x0 | (~x1 & (~x2 | ~x4)));
  assign z52 = (~new_n160_ & x3) | (~new_n159_ & ~x2);
  assign new_n159_ = (x3 | (x0 & x1)) & (x4 | (~x5 & (x5 | ~x6)));
  assign new_n160_ = (x0 | (~x1 & (~x2 | ~x4))) & ~x0 & (x4 | (~x5 & ~x6));
  assign z53 = (x0 & (x1 ? (~x2 & ~x3) : x3)) | (~new_n87_ & (x3 ? ~x1 : ~x2)) | (x3 & (new_n162_ | (~x0 & x1 & x2) | (~x1 & ~x2)));
  assign new_n162_ = ~x4 & ((~x5 & x6) | (x5 & (~x6 | ~x7)) | (~x2 & (x5 | x6)));
  assign z54 = (~new_n164_ & ~x4) | new_n165_ | (~new_n87_ & ~new_n166_) | ~new_n167_;
  assign new_n164_ = (x0 | x3 | (~x5 & ~x6)) & (~x2 | (x5 ? (x6 & x7) : ~x6));
  assign new_n165_ = (~x1 | x2) & (x0 | ~x3);
  assign new_n166_ = ~x0 & (x2 | ~x3);
  assign new_n167_ = (x1 | ~x2) & (~x0 | ~x3);
  assign z55 = new_n169_ | (~x3 & (x0 | x2)) | ~x1 | (x0 & ~new_n87_ & x2);
  assign new_n169_ = ~x4 & (~x0 | ~x2) & (x5 | x6);
  assign z56 = ((x0 | ~x1) & (~x2 | x3)) | (~x2 & (new_n148_ | ~x3)) | (x2 & ~new_n87_ & x3);
  assign z57 = (~new_n172_ & (x0 | ~x2)) | new_n173_ | ~new_n174_ | (x0 & (x2 | ~x3));
  assign new_n172_ = x1 & (x4 | ~x5);
  assign new_n173_ = ~x7 & (x2 | (~x4 & x6));
  assign new_n174_ = (x0 | x2 | ~x3) & (~x2 | (x3 & ~x4 & x5 & x6));
  assign z58 = (new_n145_ & ~x0) | (~x2 & (x0 | x1)) | ~new_n177_ | (~new_n176_ & (x0 | ~x1));
  assign new_n176_ = ~x4 & x6 & x7;
  assign new_n177_ = (~x0 | (x1 & x5)) & x3 & (x1 | (~x2 & ~x5));
  assign z59 = (x0 & (~x3 | (~x1 & ~x2))) | new_n179_ | (~new_n180_ & ~x2) | (x2 & (~x0 | x1 | ~x3));
  assign new_n179_ = ~x4 & (x5 | (x2 & x6));
  assign new_n180_ = ~x4 & ~x5 & x6 & x7;
  assign z60 = (x2 & (x0 | ~x3)) | (x3 & (x0 | ~x2)) | (x0 & (~x1 | ~x4)) | (~x0 & (~new_n83_ | x1 | x4));
  assign z61 = ~x2 | ((~new_n89_ | new_n124_) & x3);
  assign z62 = new_n124_ | ~x0 | ~x1 | x2 | x3;
  assign z08 = 1'b0;
  assign z12 = 1'b0;
  assign z26 = 1'b0;
  assign z49 = ~x2 | (x3 & (~new_n75_ | x4 | x0 | x1));
  assign z15 = x2 & (~x3 | (new_n83_ & ~x0 & x1 & ~x4));
  assign z27 = z09;
  assign z30 = z09;
endmodule


