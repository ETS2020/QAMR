// Benchmark "FAU" written by ABC on Tue Aug 18 17:07:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_;
  assign z00 = x20 & (~x25 | (new_n47_ & new_n48_ & x21 & x22));
  assign new_n47_ = ~x08 & x09 & ~x10 & x19;
  assign new_n48_ = x23 & x24 & x26;
  assign z01 = x00 & ~new_n50_ & x08;
  assign new_n50_ = x20 & ~x25;
  assign z02 = new_n50_ | (x01 & x08);
  assign z03 = x02 & ~new_n50_ & x08;
  assign z04 = x03 & ~new_n50_ & x08;
  assign z05 = x04 & ~new_n50_ & x08;
  assign z06 = new_n50_ | (x05 & x08);
  assign z07 = x06 & ~new_n50_ & x08;
  assign z08 = x07 & ~new_n50_ & x08;
  assign z09 = (~x08 & x09 & ~new_n59_ & ~x10) | new_n50_ | (x00 & x08);
  assign new_n59_ = x19 & (~new_n60_ | ~x11 | ~x20 | ~x21);
  assign new_n60_ = x24 & x26 & x22 & x23;
  assign z10 = z02 | (~x08 & x09 & ~new_n62_ & ~x10);
  assign new_n62_ = x20 ? (x19 & (~new_n48_ | ~x12 | ~x21 | ~x22)) : ~x19;
  assign z11 = (~x08 & x09 & ~new_n64_ & ~x10) | new_n50_ | (x02 & x08);
  assign new_n64_ = x20 ? (x19 ? (x21 & (~new_n48_ | ~x13 | ~x22)) : ~x21) : ~x21;
  assign z12 = z04 | (~x08 & x09 & ~new_n66_ & ~x10);
  assign new_n66_ = (x20 | ~x22) & (~x25 | ((~x19 | ~x20 | ~x21 | (~new_n67_ & x22)) & (~x22 | (x19 & x21))));
  assign new_n67_ = x24 & x26 & x14 & x23;
  assign z13 = z05 | (~x08 & x09 & ~new_n69_ & ~x10);
  assign new_n69_ = (x20 | ~x23) & (~x25 | ((~x23 | (x19 & x21)) & (~x19 | ~x20 | new_n70_ | ~x21)));
  assign new_n70_ = x22 ? (x23 & (~x15 | ~x24 | ~x26)) : ~x23;
  assign z14 = new_n72_ | (x05 & x08) | (~x08 & x09 & new_n74_ & ~x10);
  assign new_n72_ = x20 & (~x25 | (new_n47_ & new_n73_));
  assign new_n73_ = x21 & x22 & x23 & ~x24;
  assign new_n74_ = x24 & (~new_n75_ | ~x22 | ~x23 | (x16 & x26));
  assign new_n75_ = x19 & x20 & x21;
  assign z15 = new_n50_ | (x06 & x08) | (~x08 & x09 & new_n77_ & ~x10);
  assign new_n77_ = x25 & (~new_n75_ | ~new_n78_);
  assign new_n78_ = x22 & x23 & x24 & (~x17 | ~x26);
  assign z16 = z08 | (~x08 & x09 & ~new_n80_ & ~x10);
  assign new_n80_ = (~x25 | ((new_n81_ | ~x26) & (~new_n75_ | ~new_n82_))) & (x20 | ~x26);
  assign new_n81_ = ~x18 & x19 & x21 & x22 & x23 & x24;
  assign new_n82_ = x22 & x23 & x24 & ~x26;
endmodule


