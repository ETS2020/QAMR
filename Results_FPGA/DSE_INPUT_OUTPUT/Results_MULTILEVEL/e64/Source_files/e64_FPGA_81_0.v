// Benchmark "FAU" written by ABC on Mon Aug 17 19:19:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64;
  wire new_n136_, new_n141_, new_n142_, new_n144_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n162_, new_n163_, new_n165_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n174_, new_n175_,
    new_n176_;
  assign z03 = ~x10 & ~x47;
  assign z04 = x15 & ~z03 & x29;
  assign z05 = z03 | x29;
  assign z06 = z03 | new_n136_;
  assign new_n136_ = x14 & ~x15 & ~x28 & x29 & ~x37 & ~x43;
  assign z07 = ~x15 & ~x28 & x29 & ~x37 & ~z03 & x43;
  assign z10 = ~x15 & x28 & x29 & ~z03 & ~x37;
  assign z11 = ~x15 & x29 & ~z03 & x37;
  assign z14 = ~x10 & (~x47 | (new_n142_ & new_n141_ & ~x28 & x29));
  assign new_n141_ = ~x14 & ~x15;
  assign new_n142_ = ~x37 & ~x43 & x50 & ~x58;
  assign z15 = (~x10 & ~x47) | (new_n144_ & ~x15 & ~x28 & x10 & ~x14);
  assign new_n144_ = x29 & ~x37 & ~x43 & ~x58;
  assign z24 = ~x10 & (new_n146_ | ~x47);
  assign new_n146_ = new_n147_ & new_n148_ & ~x58 & ~x60 & ~x46 & ~x50;
  assign new_n147_ = new_n141_ & x11 & ~x24 & ~x25 & ~x28 & x29;
  assign new_n148_ = ~x37 & ~x39 & ~x40 & ~x43;
  assign z25 = ~x60 & ~x58 & ~x50 & x47 & new_n150_ & ~x46;
  assign new_n150_ = ~x43 & ~x40 & ~x39 & ~x37 & new_n151_ & x29;
  assign new_n151_ = ~x28 & ~x25 & x24 & ~x10 & ~x14 & ~x15;
  assign z28 = ~x60 & ~x58 & ~x50 & new_n153_ & x47;
  assign new_n153_ = ~x46 & ~x43 & ~x40 & ~x39 & new_n154_ & ~x37;
  assign new_n154_ = x29 & ~x28 & x25 & ~x10 & ~x14 & ~x15;
  assign z29 = ~x10 & (~x47 | (new_n156_ & new_n157_));
  assign new_n156_ = x29 & ~x37 & ~x39 & ~x14 & ~x15 & ~x28;
  assign new_n157_ = ~x40 & ~x43 & ~x46 & ~x50 & ~x58 & x60;
  assign z32 = ~x58 & new_n159_ & ~x50;
  assign new_n159_ = x47 & x46 & ~x43 & ~x40 & new_n160_ & ~x39;
  assign new_n160_ = ~x37 & x29 & ~x28 & ~x10 & ~x14 & ~x15;
  assign z33 = ~x10 & (new_n162_ | ~x47);
  assign new_n162_ = new_n163_ & new_n141_ & ~x28 & x29 & ~x37;
  assign new_n163_ = x39 & ~x40 & ~x43 & ~x50 & ~x58;
  assign z34 = z03 | (new_n165_ & ~x14 & ~x15 & ~x28);
  assign new_n165_ = x29 & ~x37 & ~x43 & x58;
  assign z59 = ~x58 & ~x50 & x47 & ~x43 & new_n160_ & x40;
  assign z62 = ~x10 & (~x47 | (new_n168_ & new_n170_));
  assign new_n168_ = new_n169_ & ~x11 & ~x14 & ~x15 & ~x24;
  assign new_n169_ = ~x25 & ~x28 & x29 & ~x30;
  assign new_n170_ = new_n148_ & ~x46 & ~x50 & ~x56 & ~x58 & ~x60;
  assign z63 = ~x10 & (~x47 | (new_n168_ & new_n172_));
  assign new_n172_ = new_n148_ & ~x46 & ~x50 & x56 & ~x58 & ~x60;
  assign z64 = ~x60 & new_n174_ & ~x58;
  assign new_n174_ = ~x50 & x47 & ~x46 & ~x43 & new_n175_ & ~x40;
  assign new_n175_ = ~x39 & ~x37 & x30 & x29 & new_n176_ & ~x28;
  assign new_n176_ = ~x25 & ~x24 & ~x15 & ~x10 & ~x11 & ~x14;
  assign z00 = 1'b0;
  assign z01 = 1'b0;
  assign z02 = 1'b0;
  assign z08 = 1'b0;
  assign z12 = 1'b0;
  assign z13 = 1'b0;
  assign z26 = 1'b0;
  assign z30 = 1'b0;
  assign z39 = 1'b0;
  assign z42 = 1'b0;
  assign z43 = 1'b0;
  assign z45 = 1'b0;
  assign z47 = 1'b0;
  assign z49 = 1'b0;
  assign z52 = 1'b0;
  assign z55 = 1'b0;
  assign z58 = 1'b0;
  assign z61 = 1'b0;
  assign z09 = z03;
  assign z16 = z03;
  assign z17 = z03;
  assign z18 = z03;
  assign z19 = z03;
  assign z20 = z03;
  assign z21 = z03;
  assign z22 = z03;
  assign z23 = z03;
  assign z27 = z03;
  assign z31 = z03;
  assign z35 = z03;
  assign z36 = z03;
  assign z37 = z03;
  assign z38 = z03;
  assign z40 = z03;
  assign z41 = z03;
  assign z44 = z03;
  assign z46 = z03;
  assign z48 = z03;
  assign z50 = z03;
  assign z51 = z03;
  assign z53 = z03;
  assign z54 = z03;
  assign z56 = z03;
  assign z57 = z03;
  assign z60 = z03;
endmodule


