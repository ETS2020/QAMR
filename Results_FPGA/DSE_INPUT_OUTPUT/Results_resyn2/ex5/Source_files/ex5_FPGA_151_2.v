// Benchmark "FAU" written by ABC on Wed Aug 12 19:46:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n75_, new_n80_, new_n84_, new_n85_, new_n88_, new_n90_, new_n94_,
    new_n99_, new_n104_, new_n110_, new_n112_, new_n115_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n130_, new_n132_, new_n134_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n148_, new_n151_, new_n153_, new_n155_, new_n156_;
  assign z00 = z11 | (new_n75_ & ~x4);
  assign z11 = x0 & ~x2;
  assign new_n75_ = ~x5 & ~x6;
  assign z01 = z11 | (new_n75_ & ~x7);
  assign z02 = z11 | (~x3 & ~x4 & ~x7 & x5 & ~x6);
  assign z03 = ~x7 & x5 & ~x6 & ~z11 & x3 & ~x4;
  assign z04 = z11 | (new_n80_ & x3 & ~x4);
  assign new_n80_ = ~x5 & x6 & ~x7;
  assign z05 = x6 & ~x4 & x5 & ~z11 & ~x7;
  assign z06 = (x0 & ~x2) | (new_n75_ & ~x0 & x3 & ~x4 & ~x1 & x2);
  assign z07 = ~x2 & (x0 | (new_n84_ & new_n85_));
  assign new_n84_ = x1 & ~x3 & ~x4;
  assign new_n85_ = x5 & x6 & x7;
  assign z08 = new_n84_ & new_n85_ & x0 & x2;
  assign z09 = (x0 & ~x2) | (new_n88_ & ~x0 & ~x1 & x2 & ~x3);
  assign new_n88_ = ~x4 & ~x5 & x6 & x7;
  assign z10 = new_n90_ & x2 & ~x0 & x1;
  assign new_n90_ = ~x4 & x5 & x6 & x7;
  assign z12 = x0 & (~x2 | (new_n90_ & ~x1 & ~x3));
  assign z13 = ~x2 & (x0 | (new_n90_ & x1 & x3));
  assign z15 = new_n94_ & x2 & ~x0 & x1;
  assign new_n94_ = new_n85_ & x3 & ~x4;
  assign z18 = x2 & x4 & ~x0 & ~x1 & x3 & ~x5;
  assign z19 = ~x1 & ~x3 & x4 & ~x0 & ~x2;
  assign z23 = ~x2 & x3 & ~x0 & ~x1 & x5;
  assign z24 = new_n99_ & x6 & ~x7;
  assign new_n99_ = ~x1 & ~x5 & ~x0 & ~x2 & ~x3 & ~x4;
  assign z25 = ~x2 & (x0 | (new_n80_ & new_n84_));
  assign z26 = x0 & (~x2 | (new_n88_ & ~x3));
  assign z27 = x2 & ~x0 & x1 & new_n80_ & ~x3 & ~x4;
  assign z28 = x0 & (~x2 | (new_n104_ & x3));
  assign new_n104_ = ~x1 & ~x5 & ~x4 & x6 & x7;
  assign z29 = new_n99_ & ~x6 & x7;
  assign z30 = x0 & (~x2 | (new_n84_ & ~x5 & x6 & x7));
  assign z31 = (~x0 | x2) & ((x2 & (x0 | ~x3)) | (~x2 & x3) | ~x5 | x1 | ~x4);
  assign z32 = (~x2 & (~new_n80_ | x3 | x4)) | x0 | x1 | (x2 & (~x3 | ~x4));
  assign z33 = ~new_n110_ | (x1 ? (x3 & ~x5) : x5);
  assign new_n110_ = x0 & x2 & x6 & ~x4 & x7;
  assign z34 = (~x3 & (x1 | ~x2 | x5 | ~x6)) | ~new_n112_ | (x3 & (~x5 | x6));
  assign new_n112_ = ~x4 & ~x7 & (~x0 | (x2 & x3));
  assign z35 = (x2 & (~x3 | ~x5)) | (~x2 & x3) | x0 | x1 | ~x4;
  assign z36 = ~new_n115_ | x0 | x1 | ~x2 | x3;
  assign new_n115_ = ~x4 & ~x5 & x6 & ~x7;
  assign z37 = z11 | ~new_n80_ | ~x3 | x4;
  assign z38 = (~x0 & (x1 | (~x2 & (~new_n80_ | x3 | x4)))) | (x2 & (x0 | ~x3 | ~x4));
  assign z40 = ~new_n119_ | new_n121_;
  assign new_n119_ = (~x2 | ((~x4 | (~x0 & x3)) & (x4 | x7) & (x0 | x4))) & (new_n120_ | x0 | x4) & (new_n120_ | ~x0 | ~x2);
  assign new_n120_ = ~x5 & x6;
  assign new_n121_ = (~x0 | (x2 & x3)) & ((~x4 & x7) | x0 | x1 | (~x2 & x3));
  assign z42 = x4 | ((~x5 | x6 | z11 | x7) & (~new_n123_ | x5 | ~x6 | ~x7));
  assign new_n123_ = ~x1 & x3 & x0 & x2;
  assign z43 = (~new_n125_ & ~x0) | new_n127_ | (~new_n126_ & x2);
  assign new_n125_ = (~x1 | (~x4 & (x2 | x5))) & (x2 | ~x3 | (~x4 & x5)) & (~x2 | x4 | (x5 & ~x6));
  assign new_n126_ = (~x4 | (~x0 & x3)) & ((x7 & (~x1 | ~x3)) | ~x0 | x5);
  assign new_n127_ = (x7 | (x5 ^ ~x6)) & ((~x0 & ~x4) | (x0 & x2 & (x5 | ~x6)));
  assign z44 = ~x4 | x0 | x2 | (x1 & (x4 | (~x2 & ~x5))) | (~x2 & x3 & (x4 | ~x5));
  assign z45 = (x2 | (~new_n104_ & ~x0)) & (~x1 | ~x2 | new_n130_ | x0);
  assign new_n130_ = ~x4 & (x5 | x6);
  assign z46 = x2 | (~new_n132_ & ~x0);
  assign new_n132_ = x1 & ~x3 & (x4 | (~x5 & (~x6 | x7)));
  assign z47 = (x2 | (~new_n104_ & ~x0)) & (~x1 | (~new_n134_ & (~x2 | new_n130_ | x0)));
  assign new_n134_ = ~x4 & x6 & x7 & x0 & x3 & x5;
  assign z48 = x2 | (~x0 & ((~new_n85_ & new_n130_) | x1 | ~x3));
  assign z49 = (~new_n75_ & ~x4) | x0 | x1 | ~x2 | (x3 & x4);
  assign z50 = x2 | (~new_n88_ & ~x0);
  assign z51 = (~x0 & ((~new_n75_ & ~x4) | x1 | ~x3 | (x2 & x4))) | ((~x1 | (~new_n75_ & ~x4)) & x2 & (x0 | (~new_n75_ & ~x4)));
  assign z52 = (x2 & ((~new_n75_ & ~x4) | (x3 & (x0 | x4)))) | (~x0 & ((~new_n75_ & ~x4) | x1 | (~x2 & ~x3)));
  assign z53 = new_n141_ | ((~x1 | (x0 & (~x2 | ~x3))) & ((~x2 & x3) | x0 | (x2 & ~x3))) | (~new_n90_ & (x2 ? ~x1 : ~x3)) | (~x0 & x1 & x2 & x3);
  assign new_n141_ = new_n130_ & ((~new_n85_ & x0) | (~x2 & x3) | (x2 & ~x0 & x1));
  assign z54 = (~new_n143_ & x2) | (~new_n145_ & ~new_n94_ & ~x0 & ~x2);
  assign new_n143_ = ((x6 & x7) | (x3 & (x4 | ~x5))) & new_n144_ & (x3 | (~x4 & x5)) & (x5 | x4 | ~x6);
  assign new_n144_ = (x1 | ~x3) & (~x0 | (x1 & ~x3));
  assign new_n145_ = ~new_n130_ & x1 & ~x3;
  assign z55 = (x0 & x2 & (~new_n85_ | x4)) | (~new_n75_ & ~x0 & ~x4) | (~x1 & (~x0 | x2));
  assign z56 = ~new_n148_ | (~x1 & x3) | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n148_ = ~x0 & ((~x2 & (x7 | x4 | ~x6)) | (x7 & x6 & ~x4 & x5));
  assign z57 = ~new_n148_ | (~x1 & ~x3) | (~x2 & (x3 | (~x4 & x5)));
  assign z58 = ~new_n151_ | (~x2 & (x1 | x5)) | (x0 & ~x5) | ~x3 | (~x1 & x2);
  assign new_n151_ = (x0 | x4 | (~x5 & (~x1 | ~x6))) & ((~x0 & x1) | (x6 & ~x4 & x7));
  assign z59 = ~new_n153_ | (x0 & (new_n130_ | (~x1 & ~x3)));
  assign new_n153_ = (x0 | (new_n88_ & (~x2 | ~x3))) & ((x0 & ~x3) | ~x1 | (~x0 & ~x2)) & (x2 | (~x0 & (~x3 | ~x5)));
  assign z60 = new_n155_ | ~new_n156_;
  assign new_n155_ = ~x0 & (~new_n90_ | (x2 & ~x3) | (x1 & (x2 | x5)));
  assign new_n156_ = (x2 | (~x0 & (~x3 | ~x5))) & (~x0 | (x4 & x1 & ~x3));
  assign z61 = ~new_n123_ | new_n130_;
  assign z62 = ~x0 | (~new_n145_ & x2);
  assign z14 = 1'b0;
  assign z20 = 1'b0;
  assign z21 = 1'b0;
  assign z39 = ~z03;
  assign z41 = ~z11;
  assign z16 = z11;
  assign z17 = z11;
  assign z22 = z11;
endmodule


