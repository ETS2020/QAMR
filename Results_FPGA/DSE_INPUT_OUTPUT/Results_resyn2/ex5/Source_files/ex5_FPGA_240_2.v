// Benchmark "FAU" written by ABC on Wed Aug 12 19:46:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n76_, new_n80_, new_n83_, new_n85_, new_n87_, new_n89_, new_n93_,
    new_n94_, new_n100_, new_n101_, new_n104_, new_n107_, new_n114_,
    new_n121_, new_n122_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n132_, new_n134_, new_n142_, new_n144_, new_n147_,
    new_n150_, new_n152_;
  assign z00 = ~z11 & ~x6 & ~x4 & ~x5;
  assign z11 = x0 & ~x2;
  assign z01 = z11 | (new_n76_ & ~x7);
  assign new_n76_ = ~x5 & ~x6;
  assign z02 = z11 | (~x3 & x5 & ~x6 & ~x4 & ~x7);
  assign z03 = ~z11 & ~x7 & x3 & ~x4 & x5 & ~x6;
  assign z04 = ~z11 & new_n80_ & x3 & ~x4;
  assign new_n80_ = ~x5 & x6 & ~x7;
  assign z05 = ~z11 & ~x7 & x5 & ~x4 & x6;
  assign z06 = new_n83_ & ~x0 & ~x1 & x2;
  assign new_n83_ = x3 & ~x6 & ~x4 & ~x5;
  assign z07 = new_n85_ & ~x2 & ~x3 & ~x4 & ~x0 & x1;
  assign new_n85_ = x5 & x6 & x7;
  assign z08 = new_n87_ & x0 & x2 & x1 & ~x3;
  assign new_n87_ = x6 & x7 & ~x4 & x5;
  assign z09 = (x0 & ~x2) | (new_n89_ & ~x0 & ~x3 & ~x1 & x2);
  assign new_n89_ = ~x4 & ~x5 & x6 & x7;
  assign z10 = (x0 & ~x2) | (new_n87_ & ~x0 & x1 & x2);
  assign z12 = x2 & x0 & ~x1 & new_n87_ & ~x3;
  assign z13 = ~x2 & (x0 | (new_n93_ & x1));
  assign new_n93_ = new_n94_ & x3 & x5;
  assign new_n94_ = ~x4 & x6 & x7;
  assign z15 = new_n93_ & x2 & ~x0 & x1;
  assign z18 = (x0 & ~x2) | (~x5 & ~x1 & x2 & ~x0 & x3 & x4);
  assign z19 = ~x1 & ~x3 & x4 & ~x0 & ~x2;
  assign z23 = ~x2 & (x0 | (x5 & ~x1 & x3));
  assign z24 = x6 & ~x7 & new_n100_ & new_n101_ & ~x2 & ~x5;
  assign new_n100_ = ~x0 & ~x4;
  assign new_n101_ = ~x1 & ~x3;
  assign z25 = new_n80_ & ~x2 & ~x3 & ~x4 & ~x0 & x1;
  assign z26 = ~new_n104_ & x0;
  assign new_n104_ = x2 & (x3 | x4 | ~x7 | x5 | ~x6);
  assign z27 = x2 & ~x0 & x1 & new_n80_ & ~x3 & ~x4;
  assign z28 = new_n107_ & x7 & ~x5 & x6;
  assign new_n107_ = x3 & ~x4 & x2 & x0 & ~x1;
  assign z29 = new_n100_ & new_n101_ & ~x2 & ~x5 & ~x6 & x7;
  assign z30 = new_n89_ & x0 & x2 & x1 & ~x3;
  assign z31 = (x2 & ~x3) | (~x2 & x3) | x0 | ~x4 | x1 | ~x5;
  assign z32 = (x2 & (x0 | ~x3 | ~x4)) | (~x0 & (x1 | (~x2 & (~new_n80_ | x3 | x4))));
  assign z33 = (~x5 & x1 & x3) | ~new_n94_ | ~x0 | ~x2 | (~x1 & x5);
  assign z34 = (~x3 & (x1 | ~x2 | x5 | ~x6)) | ~new_n114_ | (x3 & (~x5 | x6));
  assign new_n114_ = ~x4 & ~x7 & (~x0 | (x2 & x3));
  assign z35 = x1 | (~x2 & x3) | x0 | ~x4 | (x2 & (~x3 | ~x5));
  assign z36 = (~x0 | x2) & (x0 | x1 | ~x2 | ~new_n80_ | x3 | x4);
  assign z37 = ~z11 & (~new_n80_ | ~x3 | x4);
  assign z38 = (~x2 & (~new_n80_ | x3 | x4)) | x0 | x1 | (x2 & (~x3 | ~x4));
  assign z39 = ~z11 & (~x3 | ~x5 | x6 | x4 | x7);
  assign z40 = ~new_n121_ | (x2 & (x0 | ~x3 | ~x4) & (~x0 | x4 | ~x7));
  assign new_n121_ = ~new_n122_ & ((~x0 & x4) | (x0 & ~x2) | (~x5 & x6));
  assign new_n122_ = (~x0 | (x2 & x3)) & ((~x4 & x7) | x0 | x1 | (~x2 & x3));
  assign z42 = ~new_n124_ & (~x5 | x6 | x4 | x7);
  assign new_n124_ = x0 & (~x2 | (new_n89_ & ~x1 & x3));
  assign z43 = new_n128_ | ~new_n126_ | new_n129_;
  assign new_n126_ = new_n127_ & (~x4 | (~x1 & (x2 | ~x3))) & (~x0 | (x2 & ~x4));
  assign new_n127_ = ((x0 & ~x5) | ~x7 | (~x0 & x4)) & ((~x5 & x6) | x4 | (x5 & ~x6));
  assign new_n128_ = ~x5 & ((x0 & (~x7 | (x1 & x3))) | (~x2 & (x1 | x3)));
  assign new_n129_ = x2 & ((~x3 & x4) | ((~x5 | x6) & ~x0 & ~x4));
  assign z44 = x2 | (~x0 & (x1 | x3 | ~x4));
  assign z45 = (x2 | (~x0 & (~new_n89_ | x1))) & (new_n132_ | x0 | ~x1 | ~x2);
  assign new_n132_ = ~x4 & (x5 | x6);
  assign z46 = x2 | (~new_n134_ & ~x0);
  assign new_n134_ = x1 & ~x3 & (x4 | (~x5 & (~x6 | x7)));
  assign z47 = (x2 | (~x0 & (~new_n89_ | x1))) & (~x1 | ((~new_n93_ | ~x0) & (~x2 | new_n132_ | x0)));
  assign z48 = x2 | (~x0 & ((~new_n85_ & new_n132_) | x1 | ~x3));
  assign z49 = (~x0 & (x1 | ~x2)) | (x2 & ((x3 & x4) | x0 | (~new_n76_ & ~x4)));
  assign z50 = x2 | ~new_n89_ | x0;
  assign z51 = (~x0 & ((x2 & x4) | x1 | ~x3)) | (~new_n76_ & ~x4) | (x0 & (~x1 | ~x2));
  assign z52 = (x2 & ((~new_n76_ & ~x4) | (x3 & (x0 | x4)))) | (~x0 & ((~new_n76_ & ~x4) | x1 | (~x2 & ~x3)));
  assign z53 = new_n142_ | (~new_n87_ & (~x1 | ~x2) & (x2 | ~x3)) | ((~x2 | ~x3) & (x0 | (~x1 & x2))) | (~x1 & (x0 | (~x2 & x3))) | (x3 & x2 & ~x0 & x1);
  assign new_n142_ = new_n132_ & ((~new_n85_ & x0) | (~x2 & x3) | (~x0 & x1 & x2));
  assign z54 = ~new_n144_ | ((~new_n85_ | ~x3) & new_n132_ & (~x2 | x3));
  assign new_n144_ = ((~x0 & (x1 | ~x2)) | ((~x0 | x1) & ~x3)) & (new_n87_ | (x2 ^ ~x3)) & (x2 | (~x0 & (x1 | x3)));
  assign z55 = (~new_n87_ & x0 & x2) | (~new_n76_ & ~x0 & ~x4) | (~x1 & (~x0 | x2));
  assign z56 = ~new_n147_ | (~x1 & x3) | (~x2 & (~x3 | (~x4 & x5)));
  assign new_n147_ = (~x2 | (x5 & ~x4 & x6)) & ~x0 & (x7 | x4 | ~x6);
  assign z57 = ~new_n147_ | (~x1 & ~x3) | (~x2 & (x3 | (~x4 & x5)));
  assign z58 = ~new_n150_ | ~x3 | (~new_n94_ & (x0 | ~x1));
  assign new_n150_ = (x0 | x4 | (~x5 & (~x1 | ~x6))) & ((~x0 & ~x1 & ~x2 & ~x5) | (x1 & x2 & (~x0 | x5)));
  assign z59 = (x2 | (~new_n89_ & ~x0)) & (new_n152_ | (~x4 & x5));
  assign new_n152_ = (~new_n89_ | x0 | x1 | x3) & ((x1 ^ ~x3) | ~x0 | (~x4 & x6));
  assign z60 = ((x0 | (~x2 & x3)) & (~x1 | ~x2 | x3 | ~x4)) | (~x0 & (~new_n85_ | (x2 & ~x3) | x1 | x4));
  assign z61 = ~x0 | (x2 & (new_n132_ | x1 | ~x3));
  assign z62 = new_n132_ | ~x0 | ~x2 | ~x1 | x3;
  assign z16 = 1'b0;
  assign z20 = 1'b0;
  assign z41 = 1'b1;
  assign z14 = z11;
  assign z17 = z11;
  assign z21 = z11;
  assign z22 = z11;
endmodule


