// Benchmark "FAU" written by ABC on Tue Aug 11 21:49:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n27_, new_n28_, new_n30_, new_n32_, new_n33_, new_n38_, new_n40_;
  assign z0 = new_n27_ | new_n28_;
  assign new_n27_ = ~x00 & (((~x04 | x05) & x01 & x02) | ((x01 | x02) & x03 & (~x02 | ~x04)));
  assign new_n28_ = x09 & ~x10;
  assign z1 = new_n28_ | (new_n30_ & (~x01 | ~x02 | ~x05 | ~x06));
  assign new_n30_ = (x03 | (x02 & x04)) & ~x00 & (x01 | x02);
  assign z2 = ~new_n33_ | (~new_n32_ & (~x00 | (~x07 & ~x08)));
  assign new_n32_ = ~x02 & (~x10 | x01 | x03);
  assign new_n33_ = (~x09 | x10) & (x01 | x03 | x00 | ~x07);
  assign z3 = new_n28_ | ((~x07 | ~x11) & (x00 | ~x12));
  assign z4 = (~x07 | ~x11) & (~x09 | (x10 & (x07 | ~x00 | ~x08)));
  assign z5 = x10 | (~x09 & (x00 | x07 | x12 | ~x13));
  assign z6 = new_n38_ & (~x00 | ~x08) & ~x10 & ~x07 & ~x09;
  assign new_n38_ = (x02 | x01 | x03) & (~x01 | ~x02 | x00 | ~x14);
  assign z7 = new_n40_ | x10;
  assign new_n40_ = ~x09 & (x07 | (~x08 & (x01 | x02 | ~x03)));
  assign z8 = x10 | (~x09 & (~x00 | x07));
endmodule


