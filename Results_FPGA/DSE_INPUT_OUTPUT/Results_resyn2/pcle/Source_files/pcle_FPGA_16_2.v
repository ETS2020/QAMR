// Benchmark "FAU" written by ABC on Wed Aug 12 18:03:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n31_, new_n32_, new_n34_, new_n36_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n49_;
  assign z0 = new_n31_ & x18 & new_n32_ & x17;
  assign new_n31_ = ~x08 & x09 & ~x07 & ~x10;
  assign new_n32_ = x14 & x13 & x11 & x12 & x15 & x16;
  assign z1 = (new_n34_ & x00) | (new_n31_ & ~x11);
  assign new_n34_ = x08 & (~x07 | x10);
  assign z2 = new_n36_ | (x01 & x08);
  assign new_n36_ = ~x10 & (x07 | ((x11 | x12) & ~x08 & x09 & (~x11 | ~x12)));
  assign z3 = (new_n34_ & x02) | ((x13 | (x11 & x12)) & new_n31_ & (~x13 | ~x11 | ~x12));
  assign z4 = new_n39_ | ((x14 | (x13 & x11 & x12)) & new_n31_ & (~x14 | ~x13 | ~x11 | ~x12));
  assign new_n39_ = x03 & x08 & (~x07 | x10);
  assign z5 = new_n43_ | (~x10 & (x07 | ((new_n42_ | x15) & new_n41_ & (~new_n42_ | ~x15))));
  assign new_n41_ = ~x08 & x09;
  assign new_n42_ = x14 & x13 & x11 & x12;
  assign new_n43_ = x04 & x08;
  assign z6 = new_n46_ | (~x10 & (x07 | (~new_n45_ & new_n41_ & ~new_n32_)));
  assign new_n45_ = ~x16 & (~x15 | ~x14 | ~x13 | ~x11 | ~x12);
  assign new_n46_ = x05 & x08;
  assign z7 = (new_n34_ & x06) | ((new_n32_ | x17) & new_n31_ & (~new_n32_ | ~x17));
  assign z8 = new_n49_ | ((x18 | (new_n32_ & x17)) & new_n31_ & (~x18 | ~new_n32_ | ~x17));
  assign new_n49_ = x10 & x07 & x08;
endmodule


