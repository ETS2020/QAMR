// Benchmark "FAU" written by ABC on Wed Aug 12 18:30:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n56_, new_n57_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_;
  assign z00 = (x03 & x18) | (((~x17 & ~x19) | ~x16 | (x17 & x19)) & ~x18 & (~x15 | x16));
  assign z01 = x18 | ((~x14 | x16) & ((~x20 & ~x17 & ~x19) | ~x16 | (x20 & (x17 | x19))));
  assign z02 = (x16 & (new_n57_ | (~new_n56_ & x21))) | x18 | (~x13 & ~x16);
  assign new_n56_ = ~x20 & ~x17 & ~x19;
  assign new_n57_ = ~x20 & ~x21 & ~x17 & ~x19;
  assign z03 = (x03 & x18) | (((new_n57_ & ~x22) | ~x16 | (~new_n57_ & x22)) & ~x18 & (~x12 | x16));
  assign z04 = new_n60_ | new_n61_;
  assign new_n60_ = ((x23 & (~new_n57_ | x22)) | ~x16 | (new_n57_ & ~x22 & ~x23)) & ~x18 & (~x11 | x16);
  assign new_n61_ = x03 & x18;
  assign z05 = (x03 & x18) | ((new_n63_ | new_n65_ | ~x16) & ~x18 & (~x10 | x16));
  assign new_n63_ = ~new_n64_ & x24;
  assign new_n64_ = ~x20 & ~x21 & ~x17 & ~x19 & ~x22 & ~x23;
  assign new_n65_ = new_n56_ & new_n66_ & ~x21 & ~x24;
  assign new_n66_ = ~x22 & ~x23;
  assign z06 = (x16 & (new_n68_ | (~new_n65_ & x25))) | x18 | (~x09 & ~x16);
  assign new_n68_ = new_n57_ & new_n66_ & ~x24 & ~x25;
  assign z07 = (x03 & x18) | (~new_n70_ & ~x18 & (~x08 | x16));
  assign new_n70_ = (~new_n57_ | ~new_n66_ | x24 | x25 | x26) & x16 & (~x26 | (new_n57_ & new_n66_ & ~x24 & ~x25));
  assign z08 = (x03 & x18) | ((new_n72_ | new_n73_ | ~x16) & ~x18 & (~x07 | x16));
  assign new_n72_ = x27 & (~new_n64_ | x24 | x25 | x26);
  assign new_n73_ = new_n74_ & new_n56_ & new_n66_ & ~x21 & ~x24;
  assign new_n74_ = ~x25 & ~x26 & ~x27;
  assign z09 = (x16 & (new_n76_ | (~new_n73_ & x28))) | x18 | (~x06 & ~x16);
  assign new_n76_ = new_n77_ & new_n57_ & ~x22;
  assign new_n77_ = ~x24 & ~x25 & ~x26 & ~x28 & ~x23 & ~x27;
  assign z10 = (x16 & (new_n79_ | (~new_n76_ & x29))) | x18 | (~x05 & ~x16);
  assign new_n79_ = new_n80_ & new_n64_ & ~x24 & ~x25 & ~x26;
  assign new_n80_ = ~x27 & ~x28 & ~x29;
  assign z11 = (x16 & (new_n82_ | (~new_n79_ & x30))) | x18 | (~x04 & ~x16);
  assign new_n82_ = new_n83_ & new_n64_ & ~x24 & ~x25 & ~x26;
  assign new_n83_ = ~x30 & ~x27 & ~x28 & ~x29;
  assign z12 = (x16 & (new_n85_ | (~new_n82_ & x31))) | x18 | (~x03 & ~x16);
  assign new_n85_ = new_n56_ & new_n66_ & ~x21 & ~x24 & new_n74_ & new_n86_;
  assign new_n86_ = ~x30 & ~x31 & ~x28 & ~x29;
  assign z13 = new_n61_ | (new_n90_ & (new_n88_ | ~x16 | (~new_n85_ & x32)));
  assign new_n88_ = new_n83_ & new_n89_ & new_n56_ & new_n66_ & ~x21 & ~x24;
  assign new_n89_ = ~x31 & ~x32 & ~x25 & ~x26;
  assign new_n90_ = ~x18 & (~x02 | x16);
  assign z14 = new_n61_ | (new_n94_ & ((~new_n88_ & x33) | new_n92_ | ~x16));
  assign new_n92_ = new_n57_ & new_n66_ & ~x24 & ~x25 & new_n86_ & new_n93_;
  assign new_n93_ = ~x32 & ~x33 & ~x26 & ~x27;
  assign new_n94_ = ~x18 & (~x01 | x16);
  assign z15 = (x03 & x18) | (((new_n92_ & ~x34) | ~x16 | (~new_n92_ & x34)) & ~x18 & (~x00 | x16));
endmodule


