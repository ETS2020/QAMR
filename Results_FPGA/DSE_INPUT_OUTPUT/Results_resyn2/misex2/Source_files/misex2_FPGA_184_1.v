// Benchmark "FAU" written by ABC on Wed Aug 12 18:02:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n48_, new_n50_, new_n53_, new_n55_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n72_;
  assign z00 = ~x00 & (new_n46_ | x01);
  assign new_n46_ = x10 & ~x19 & ~x17 & ~x18 & ~x02 & ~x09;
  assign z01 = new_n48_ & x10 & ~x19 & ~x17 & ~x18;
  assign new_n48_ = ~x00 & ~x01 & ~x02 & x09;
  assign z02 = new_n48_ & new_n50_;
  assign new_n50_ = ~x10 & ~x19 & ~x17 & ~x18;
  assign z03 = new_n53_ | (z06 & x12);
  assign z06 = x11 & x00 & x02 & x01 & ~x09 & x10;
  assign new_n53_ = ~x01 & ~x02 & ~x17 & x18 & ~x00 & ~x19;
  assign z04 = new_n55_ & ~x11 & ~x12;
  assign new_n55_ = x00 & x02 & x01 & ~x09 & x10;
  assign z05 = x01 & (~x00 | (new_n57_ & x09));
  assign new_n57_ = x02 & x10;
  assign z07 = ~new_n59_ & x00 & x02;
  assign new_n59_ = x01 & (x09 | ~x10 | ~x11 | x12);
  assign z08 = new_n61_ & new_n62_ & ~x08 & x17 & ~x18 & x19;
  assign new_n61_ = ~x06 & ~x07 & x04 & ~x05;
  assign new_n62_ = ~x00 & ~x01 & x02 & ~x03;
  assign z09 = ~x15 & new_n65_ & x02 & ~x14 & new_n64_ & ~x16;
  assign new_n64_ = ~x00 & ~x01;
  assign new_n65_ = ~x22 & x20 & ~x21 & ~x13 & ~x11 & x12;
  assign z10 = ~x00 & (x01 | (new_n68_ & new_n67_ & new_n69_));
  assign new_n67_ = ~x22 & x20 & ~x21;
  assign new_n68_ = ~x13 & ~x11 & x12;
  assign new_n69_ = x02 & ~x14 & x15 & x16;
  assign z11 = x15 & new_n65_ & x02 & ~x14 & new_n64_ & ~x16;
  assign z12 = (~x00 & x01) | (((~new_n72_ & ~x00) | (~new_n57_ & x01)) & x09 & ~x24);
  assign new_n72_ = (~x02 & ~x19) ? ~x17 : ~x23;
  assign z13 = new_n64_ & x17 & ~x02 & ~x19;
  assign z14 = ~x00 & (x01 | (new_n50_ & ~x02 & ~x09));
  assign z15 = (x01 & (~x00 | ~x10)) | (~x02 & (x00 | x19));
  assign z16 = ~x00 & x01;
  assign z17 = ~x00 & (x01 | x02);
endmodule


