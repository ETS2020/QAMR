...
module FAU ( 
  ...
  assign z2 = (~new_n31_ & (~x10 | x11)) | (~x01 & (new_n32_ | new_n33_));
  assign new_n31_ = (x00 | (~x02 & (x01 | x03 | ~x07))) & (~x02 | x07 | x08);
  assign new_n32_ = ~x00 & ((~x07 & x09 & ~x10) | (~x03 & x10 & x11));
  assign new_n33_ = ~x07 & ~x08 & ((x09 & ~x10) | (~x03 & x10 & x11));
  assign z3 = (x11 ? ~x07 : ~x10) & (x00 | ~x12);
  assign z4 = x07 ? (~x10 & ~x11) : (x00 ? ((~x10 | x11) & (~x08 | ~x09)) : (~x10 | (x10 & x11)));
  assign z5 = (x00 & (x11 | (~x07 & ~x10))) | (x07 & (x11 | (~x10 & ~x11))) | (~x00 & ~x07 & (x10 ? x11 : (x12 | ~x13)));
  assign z6 = ~x07 & ~x10 & (new_n38_ | new_n39_);
  assign new_n38_ = ~x01 & (~x00 | ~x08) & (x02 | (~x03 & x09) | (x03 & ~x09));
  assign new_n39_ = x01 & (x00 ? ~x08 : (~x02 | ~x14));
  assign z7 = x10 ? x11 : ~new_n41_;
