...
module FAU ( 
  ...
  assign z2 = (~x00 & ((x02 & (~x01 | x07)) | (~new_n32_ & ~x01))) | (~x01 & new_n33_ & ~x07);
  assign new_n32_ = (x03 | (~x07 & ~x10)) & (x07 | ~x09 | x10);
  assign new_n33_ = ~x08 & (x02 | (x09 & ~x10) | (~x03 & x10));
  assign z3 = (x07 ? ~x11 : ~x01) & (x00 | ~x12);
  assign z4 = (x07 & ~x11) | (~x01 & ~x07 & (~x00 | (x00 & (~x08 | ~x09))));
  assign z5 = x07 | (~x01 & (x00 | (~x00 & ~x07 & (x10 | x12 | ~x13))));
  assign z6 = ~x01 & new_n38_ & ~x07;
  assign new_n38_ = ~x10 & (~x00 | ~x08) & (x02 | (~x03 & x09) | (x03 & ~x09));
  assign z7 = new_n40_ | x07;
  assign new_n40_ = ~x01 & ((~x08 & (x02 | ~x03)) | x10 | (~x02 & x03 & x09));
  assign z8 = (~new_n42_ & ~x01) | x07;
  assign new_n42_ = (~x09 | (~x02 & x03 & ~x12)) & ~x10 & (x00 | x09);
endmodule
