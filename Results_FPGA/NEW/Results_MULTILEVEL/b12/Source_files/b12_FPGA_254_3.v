...
module FAU ( 
  ...
  assign new_n30_ = x00 & (x07 | x08 | x12);
  assign new_n31_ = ~x02 & (x01 | x03 | ~x10);
  assign new_n32_ = ~x00 & (x07 ? ~x03 : (x09 & ~x10));
  assign new_n33_ = ~x07 & ~x08 & x09 & ~x10 & ~x12;
  assign z3 = ~x12 & (~x07 | ~x11);
  assign z4 = x07 ? (~x11 & (~x00 | ~x12)) : (~x00 | (x00 & ~x12 & (~x08 | ~x09)));
  assign z5 = (x07 & (~x00 | (~x11 & ~x12))) | (~x00 & ~x07 & (x10 | x12 | ~x13)) | (x00 & ~x12);
  assign z6 = ~x07 & ~x10 & (x01 ? ~new_n39_ : new_n38_);
  assign new_n38_ = (~x00 | (~x08 & ~x12)) & (x02 | (x03 & ~x09) | (~x03 & x09));
  assign new_n39_ = x00 ? (x08 | x12) : (x02 & x14);
  assign z7 = (~x00 | ~x12) & (~new_n42_ | (~new_n41_ & ~x08));
  assign new_n41_ = ~x01 & ~x02 & x03;
  assign new_n42_ = ~x07 & ~x10 & (x01 | x02 | ~x03 | ~x09);
  assign z8 = (~x00 & (~x09 | x12)) | (~x12 & ((~new_n41_ & x09) | x07 | x10));
endmodule
