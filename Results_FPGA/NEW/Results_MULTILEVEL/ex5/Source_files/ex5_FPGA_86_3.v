// Benchmark "FAU" written by ABC on Thu Aug  6 17:38:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62;
  wire new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n92_,
    new_n94_, new_n97_, new_n104_, new_n107_, new_n112_, new_n114_,
    new_n115_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n125_, new_n127_, new_n129_, new_n130_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n150_, new_n151_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n161_, new_n163_,
    new_n164_, new_n166_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n178_, new_n181_;
  assign z00 = ~x4 & ~x5 & ~x6 & (~x2 | ~x3);
  assign z01 = ~x5 & ~x6 & ~x7 & (~x2 | ~x3);
  assign z02 = ~x7 & ~x6 & x5 & ~x3 & ~x4;
  assign z03 = ~x7 & ~x6 & x5 & ~x4 & ~x2 & x3;
  assign z04 = ~x7 & x6 & ~x5 & ~x4 & ~x2 & x3;
  assign z05 = ~x4 & x5 & x6 & (~x2 | ~x3) & ~x7;
  assign z07 = x7 & x6 & new_n80_ & x5;
  assign new_n80_ = ~x4 & ~x3 & ~x2 & ~x0 & x1;
  assign z08 = x7 & x6 & new_n82_ & x5;
  assign new_n82_ = ~x4 & ~x3 & x2 & x0 & x1;
  assign z09 = x7 & new_n84_ & x6;
  assign new_n84_ = ~x4 & ~x3 & x2 & ~x0 & ~x1 & ~x5;
  assign z10 = x7 & x6 & new_n86_ & x5;
  assign new_n86_ = ~x4 & ~x3 & x2 & ~x0 & x1;
  assign z11 = x7 & x6 & x5 & ~x4 & new_n88_ & ~x3;
  assign new_n88_ = ~x2 & x0 & x1;
  assign z12 = x7 & new_n90_ & x6;
  assign new_n90_ = x5 & ~x4 & ~x3 & x2 & x0 & ~x1;
  assign z13 = x7 & new_n92_ & x6;
  assign new_n92_ = x5 & ~x4 & x3 & ~x2 & ~x0 & x1;
  assign z14 = x7 & new_n94_ & x6;
  assign new_n94_ = x5 & ~x4 & x3 & ~x2 & x0 & ~x1;
  assign z16 = x7 & x6 & x5 & ~x4 & new_n88_ & x3;
  assign z17 = ~x5 & x4 & new_n97_ & ~x2;
  assign new_n97_ = x0 & ~x1;
  assign z19 = x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z20 = ~x6 & ~x5 & ~x4 & ~x3 & new_n97_ & ~x2;
  assign z21 = ~x6 & ~x5 & ~x4 & x3 & new_n97_ & ~x2;
  assign z22 = x7 & x6 & ~x5 & ~x4 & new_n97_ & ~x2;
  assign z23 = x5 & x3 & ~x2 & ~x0 & ~x1;
  assign z24 = ~x7 & new_n104_ & x6;
  assign new_n104_ = ~x5 & ~x4 & ~x3 & ~x2 & ~x0 & ~x1;
  assign z25 = ~x7 & x6 & new_n80_ & ~x5;
  assign z26 = new_n107_ & x7;
  assign new_n107_ = x6 & ~x5 & ~x4 & ~x3 & x0 & x2;
  assign z27 = ~x7 & x6 & new_n86_ & ~x5;
  assign z29 = x7 & new_n104_ & ~x6;
  assign z30 = x7 & x6 & new_n82_ & ~x5;
  assign z31 = x2 ? ~x3 : ~new_n112_;
  assign new_n112_ = (x0 | (~x3 & x4)) & (x4 | (~x5 & (~x0 | ~x6))) & ~x1 & (~x4 | x5);
  assign z32 = ~new_n115_ | (~new_n114_ & ~x4);
  assign new_n114_ = (~x0 | (x3 & (x2 | ~x6))) & (x2 | (~x5 & (x0 | (x6 & ~x7))));
  assign new_n115_ = x2 ? x3 : (~x1 & (~x4 | x5) & (x0 | (~x3 & ~x4)));
  assign z33 = ~x2 | (~new_n117_ & ~x3);
  assign new_n117_ = x0 & ~x4 & (x1 | ~x5) & x6 & x7;
  assign z34 = new_n119_ | new_n120_ | new_n121_ | new_n122_;
  assign new_n119_ = x1 & (x2 ? ~x3 : ~x5);
  assign new_n120_ = (x2 ? ~x3 : x5) & (x4 | x7);
  assign new_n121_ = ~x3 & (x5 | (x2 & (x0 | ~x6)));
  assign new_n122_ = ~x2 & (x5 ? x6 : (~x0 | (~x4 & (~x6 | ~x7))));
  assign z35 = (x1 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & ((~x0 & x3) | ~x4 | (x0 & ~x5)));
  assign z36 = (x1 & (~x2 | ~x3)) | ~new_n125_ | (x5 & (~x2 | (x2 & ~x3)));
  assign new_n125_ = x2 ? (x3 | (x6 & ~x7 & ~x0 & ~x4)) : (x0 & x4);
  assign z37 = (~x0 & (~x3 | (~x1 & ~x2 & x5))) | (~x3 & (~x1 | x2)) | (~x2 & ((~new_n127_ & (x1 ? x3 : ~x5)) | (x1 & x3 & x5)));
  assign new_n127_ = ~x4 & x6 & ~x7;
  assign z38 = new_n129_ | (x2 & ~x3) | (~x2 & (new_n130_ | x1));
  assign new_n129_ = ~x4 & ((~x2 & x5) | (x0 & (~x3 | (~x2 & x6))));
  assign new_n130_ = ~x0 & (x3 | x4 | x5 | ~x6 | x7);
  assign z39 = (~new_n132_ & ~x2) | (~x3 & (x2 | x5));
  assign new_n132_ = (~x5 | (~x6 & ~x7)) & ~x4 & (x5 | (x0 & ~x1 & x6 & x7));
  assign z40 = x2 ? new_n137_ : (new_n134_ | new_n135_ | new_n136_ | x1);
  assign new_n134_ = x0 & (x4 ? ~x5 : x6);
  assign new_n135_ = ~x0 & (x3 | (~x4 & (~x6 | x7)));
  assign new_n136_ = ~x4 & x5;
  assign new_n137_ = ~x3 & (~x0 | x4 | x5 | ~x6 | ~x7);
  assign z41 = (x1 & (~x2 ^ ~x3)) | (~x1 & (x5 ? ~x3 : ~x2)) | (~x0 & ~x2) | (x2 & ~new_n139_ & ~x3);
  assign new_n139_ = ~x4 & x5 & ~x6 & ~x7;
  assign z42 = (x4 & (~x2 | (x2 & ~x3))) | new_n141_ | (~x5 & (x2 ? ~x3 : ~new_n142_));
  assign new_n141_ = (x6 | x7) & (x2 ? ~x3 : (x1 | x5));
  assign new_n142_ = ~x1 & (x1 | (x0 & x6 & x7));
  assign z43 = x2 ? new_n146_ : (new_n144_ | new_n145_);
  assign new_n144_ = (x4 | ~x5) & (x1 | (~x0 & x3));
  assign new_n145_ = ~x4 & ((~x0 & (x7 | (~x5 & ~x6))) | (x5 & (x6 | x7)) | (x0 & x6 & ~x7));
  assign new_n146_ = ~x3 & (((x6 | x7) & (~x0 | x5)) | x4 | (~x5 & (~x0 | ~x6 | ~x7)));
  assign z44 = x2 ? ~x3 : (new_n144_ | ~new_n148_);
  assign new_n148_ = (~x6 | (~x0 & (x4 | ~x5))) & (x4 | (x0 & (~x5 | ~x7))) & (~x0 | (~x3 & ~x4 & ~x5));
  assign z45 = (x0 & (~x2 | ~x3)) | (x2 & ~x3 & (new_n150_ | ~x1)) | (~new_n151_ & ~x2);
  assign new_n150_ = ~x4 & (x5 | x6);
  assign new_n151_ = ~x1 & ~x4 & ~x5 & x6 & x7;
  assign z46 = (x0 & (~x2 | ~x3)) | (x2 & ~x3) | (~x2 & (new_n153_ | ~x1 | x3));
  assign new_n153_ = ~x4 & (x5 | (x6 & ~x7));
  assign z48 = ~x3 | (~new_n155_ & ~x2);
  assign new_n155_ = ~x0 & ~x1 & (x4 | (x5 ? (x6 & x7) : ~x6));
  assign z49 = ~x2 | (~new_n157_ & ~x3);
  assign new_n157_ = ~x0 & (x4 | ~x5) & (~x2 | (~x1 & (x4 | ~x6)));
  assign z50 = (x0 & (~x3 | (~x1 & ~x2))) | (~new_n159_ & ~x2) | (~x3 & (new_n136_ | x2));
  assign new_n159_ = ~x4 & ~x5 & x6 & x7;
  assign z51 = (~x2 & (~new_n161_ | (x0 & (~x1 | x3)))) | (~x3 & (~x0 | ~x1 | (new_n150_ & x2)));
  assign new_n161_ = (x0 | ~x1) & (x4 | ((x0 | (~x5 & ~x6)) & (x5 | ~x6) & (~x5 | (x6 & x7))));
  assign z52 = (~x2 & ((x0 & (~x1 | x3)) | new_n163_ | (~x0 & (x1 | ~x3)))) | (~x3 & (new_n164_ | (~x0 & x1)));
  assign new_n163_ = ~x4 & (x5 | (x6 & (~x0 | ~x5)));
  assign new_n164_ = ~x4 & (~x0 | x2) & (x5 | x6);
  assign z53 = (~x3 & (~new_n166_ | (x0 & (x1 | x2)))) | (~x2 & x3 & (new_n150_ | ~x1));
  assign new_n166_ = x2 ? (x1 & (x4 | (~x5 & ~x6))) : (~x4 & x5 & x6 & x7);
  assign z54 = (~x3 & (new_n97_ | new_n169_ | (~new_n168_ & ~x2))) | (~x2 & ~new_n170_ & x3);
  assign new_n168_ = x1 & (x0 | x4 | (~x5 & ~x6));
  assign new_n169_ = (x0 | x2) & (x4 | ~x5 | ~x6 | ~x7);
  assign new_n170_ = ~x0 & ~x4 & x5 & x6 & x7;
  assign z55 = new_n172_ | (~x1 & (~x2 | ~x3)) | (x0 & ~new_n173_ & ~x3);
  assign new_n172_ = ~x4 & (~x2 | (~x0 & ~x3)) & (x5 | x6);
  assign new_n173_ = x2 & ~x4 & x5 & x6 & x7;
  assign z56 = (x0 & (~x2 | ~x3)) | (~new_n175_ & ~x3) | (~x2 & (new_n153_ | ~x1 | ~x3));
  assign new_n175_ = ~x4 & x5 & x6 & x7;
  assign z57 = (~x1 & (~x2 | (x2 & ~x3))) | (~x2 & (new_n153_ | (~x0 & x3))) | (~x3 & (x0 | (~new_n175_ & x2)));
  assign z58 = ~x3 | (~new_n178_ & ~x2);
  assign new_n178_ = ~x5 & x6 & x7 & ~x0 & ~x1 & ~x4;
  assign z59 = (x0 & ((~x1 & (~x2 | ~x3)) | (~x3 & (new_n150_ | ~x2)))) | (~new_n159_ & (x3 ? ~x2 : ~x0)) | (~x0 & x1 & x2 & ~x3);
  assign z60 = (~x3 & (x0 ? (~x1 | ~x4) : ~new_n181_)) | (~x2 & (x3 | (x0 & ~x1)));
  assign new_n181_ = (~x1 | (~x2 & ~x5)) & x5 & x6 & x7 & ~x4 & (~x2 | ~x5);
  assign z61 = ~x3 | (~x2 & x3);
  assign z62 = x3 ? ~x2 : (new_n150_ | ~x0 | ~x1);
  assign z06 = 1'b0;
  assign z15 = 1'b0;
  assign z18 = 1'b0;
  assign z28 = 1'b0;
  assign z47 = (x0 & (~x2 | ~x3)) | (x2 & ~x3 & (new_n150_ | ~x1)) | (~new_n151_ & ~x2);
endmodule


