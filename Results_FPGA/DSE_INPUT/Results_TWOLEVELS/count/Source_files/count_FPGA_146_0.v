// Benchmark "FAU" written by ABC on Wed Jul 29 04:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n56_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_;
  assign z00 = (x16 & (~x17 ^ x19)) | x18 | (~x15 & ~x16);
  assign z01 = x18 | (~x14 & ~x16) | (x16 & (((x17 | x19) & x20) | (~x17 & ~x19 & ~x20)));
  assign z02 = (~new_n56_ & x16) | x18 | (~x13 & ~x16);
  assign new_n56_ = (x17 | x19 | x20 | x21) & (~x21 | (~x17 & ~x19 & ~x20));
  assign z03 = (~new_n58_ & x16) | x18 | (~x12 & ~x16);
  assign new_n58_ = (x17 | x19 | x20 | x21 | x22) & (~x22 | (~x17 & ~x19 & ~x20 & ~x21));
  assign z04 = (~new_n60_ & x16) | x18 | (~x11 & ~x16);
  assign new_n60_ = (x17 | x19 | x20 | x21 | x22 | x23) & (~x23 | (~x17 & ~x19 & ~x20 & ~x21 & ~x22));
  assign z05 = (~new_n62_ & x16) | x18 | (~x10 & ~x16);
  assign new_n62_ = (~new_n63_ | x17) & (new_n64_ | ~x24);
  assign new_n63_ = ~x19 & ~x20 & ~x21 & ~x22 & ~x23 & ~x24;
  assign new_n64_ = ~x17 & ~x19 & ~x20 & ~x21 & ~x22 & ~x23;
  assign z06 = (~new_n66_ & x16) | x18 | (~x09 & ~x16);
  assign new_n66_ = (x17 | x19 | ~new_n67_ | x20) & (~x25 | (new_n68_ & ~x17 & ~x19 & ~x20));
  assign new_n67_ = ~x21 & ~x22 & ~x23 & ~x24 & ~x25;
  assign new_n68_ = ~x21 & ~x22 & ~x23 & ~x24;
  assign z07 = (x16 & (new_n70_ | (new_n73_ & ~x17))) | x18 | (~x08 & ~x16);
  assign new_n70_ = ~new_n71_ & x26;
  assign new_n71_ = new_n72_ & ~x22 & ~x23 & ~x24 & ~x25;
  assign new_n72_ = ~x17 & ~x19 & ~x20 & ~x21;
  assign new_n73_ = ~x19 & ~x20 & ~x21 & ~x22 & new_n74_ & ~x23;
  assign new_n74_ = ~x24 & ~x25 & ~x26;
  assign z08 = (~new_n76_ & x16) | x18 | (~x07 & ~x16);
  assign new_n76_ = ~new_n77_ & (x17 | x19 | x20 | ~new_n78_ | x21);
  assign new_n77_ = x27 & (~new_n72_ | ~new_n74_ | x22 | x23);
  assign new_n78_ = ~x22 & ~x23 & ~x24 & ~x25 & ~x26 & ~x27;
  assign z09 = (x16 & (new_n80_ | (~new_n82_ & x28))) | x18 | (~x06 & ~x16);
  assign new_n80_ = ~x17 & ~x19 & ~x20 & ~x21 & new_n81_ & ~x22;
  assign new_n81_ = ~x23 & ~x24 & ~x25 & ~x26 & ~x27 & ~x28;
  assign new_n82_ = new_n83_ & ~x23 & ~x24 & ~x25 & ~x26 & ~x27;
  assign new_n83_ = ~x17 & ~x19 & ~x20 & ~x21 & ~x22;
  assign z10 = x18 | (~x05 & ~x16) | (x16 & (new_n87_ | (new_n85_ & ~x17)));
  assign new_n85_ = ~x19 & ~x20 & ~x21 & ~x22 & new_n86_ & ~x23;
  assign new_n86_ = ~x24 & ~x25 & ~x26 & ~x27 & ~x28 & ~x29;
  assign new_n87_ = x29 & (~new_n83_ | ~new_n88_);
  assign new_n88_ = ~x23 & ~x24 & ~x25 & ~x26 & ~x27 & ~x28;
  assign z11 = (~new_n90_ & x16) | x18 | (~x04 & ~x16);
  assign new_n90_ = ~new_n93_ & (x17 | x19 | x20 | ~new_n91_ | x21);
  assign new_n91_ = ~x22 & ~x23 & ~x24 & new_n92_ & ~x25;
  assign new_n92_ = ~x26 & ~x27 & ~x28 & ~x29 & ~x30;
  assign new_n93_ = x30 & (~new_n64_ | ~new_n74_ | x27 | x28 | x29);
  assign z12 = (~new_n95_ & x16) | x18 | (~x03 & ~x16);
  assign new_n95_ = (x17 | x19 | ~new_n96_ | x20) & (new_n98_ | ~x31);
  assign new_n96_ = ~x21 & ~x22 & ~x23 & ~x24 & new_n97_ & ~x25;
  assign new_n97_ = ~x26 & ~x27 & ~x28 & ~x29 & ~x30 & ~x31;
  assign new_n98_ = new_n64_ & new_n74_ & ~x29 & ~x30 & ~x27 & ~x28;
  assign z13 = (x16 & (new_n100_ | (~new_n103_ & x32))) | x18 | (~x02 & ~x16);
  assign new_n100_ = ~x17 & ~x19 & ~x20 & new_n101_ & ~x21;
  assign new_n101_ = ~x22 & ~x23 & ~x24 & ~x25 & new_n102_ & ~x26;
  assign new_n102_ = ~x27 & ~x28 & ~x29 & ~x30 & ~x31 & ~x32;
  assign new_n103_ = new_n104_ & new_n105_ & ~x25 & ~x26 & ~x27;
  assign new_n104_ = new_n68_ & ~x17 & ~x19 & ~x20;
  assign new_n105_ = ~x28 & ~x29 & ~x30 & ~x31;
  assign z14 = x18 | (~x01 & ~x16) | (x16 & (new_n109_ | (~new_n107_ & x33)));
  assign new_n107_ = new_n104_ & new_n108_ & ~x27 & ~x28 & ~x25 & ~x26;
  assign new_n108_ = ~x29 & ~x30 & ~x31 & ~x32;
  assign new_n109_ = new_n71_ & new_n110_ & ~x28 & ~x29 & ~x26 & ~x27;
  assign new_n110_ = ~x30 & ~x31 & ~x32 & ~x33;
  assign z15 = (x16 & (new_n112_ | (~new_n72_ & x34))) | x18 | (~x00 & ~x16);
  assign new_n112_ = ~x17 & ~x19 & ~x20 & ~x21 & (x22 | (~new_n113_ & ~x22));
  assign new_n113_ = ~x23 & (x23 | (~x24 & (x24 | (~x25 & (x25 | (~x26 & (x26 | (~x27 & (new_n114_ | x27)))))))));
  assign new_n114_ = ~x28 & (x28 | (~x29 & (x29 | (~x30 & (x30 | (~x31 & (x31 | (~x32 & (x32 | ~x33)))))))));
endmodule


